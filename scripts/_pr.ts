...
for(const id of hits){
  ...
  console.log(`\n== ${id} (effects_${ff}) ==`);
  console.log('  TEXT:', (r?.EffectText??'').slice(0,180));
  fresh.forEach((e:any,i:number)=>console.log(`  FRESH[${i}] ${e.effectId} type=${e.effectType} timing=${JSON.stringify(e.timing)} scope=${e.triggerScope}`));
  ...
  else console.log('  (no JSON entry)');
}
