...
//   （不一致0）で到達不能を実証している。
// 18 → 14 ＝ 第8バッチで**センタールリグ条件の4規則を `EffectCost.costReplacement` へ移設**した分
//   （あなたのセンタールリグが＜X＞14枚／対戦相手のセンタールリグの色12枚／2条件の累積1枚／レベルN以上1枚）。
//   ⚠移設は**全カード×盤面マトリクス 1,293,192 通り**（ベット宣言・任意支払い済みの軸も含む）の
//   ダンプ突き合わせで**不一致0**を確認している。A群 51→23カード・真の worklist 46→18カード。
// 14 → 0 ＝ 🏁**第9バッチで A群を全滅させた**（`O-86` クローズ）。残っていた条件つき軽減 14規則のうち
//   ①場のパワーN以上 ②場の＜クラス＞ ③場の＜X＞と＜Y＞ ④ライフ枚数比較 ⑤ゾーン枚数差
//   ⑥ルリグトラッシュの色アーツ2条件 ⑦場の〔色〕＜クラス＞2条件 ⑧相手シグニのバニッシュ履歴 を
//   `EffectCost.costReplacement` へ、`SP36-001` の2文（スペル枚数比例＋アーツ使用の固定減）を
//   `EffectCost.costScaling` の新 count 種（`spellsUsedThisTurn` / `artsUsedThisTurn`）へ移し、
//   到達しても出力が動かない2本（センタールリグのレベル比例／トラッシュの＜クラス＞比例）を撤去した。
//   ⚠**0 になっても計器は残す**＝ratchet は「UI 層で原文 regex を新しく書いたら止まる」ためのもので、
//   `O-86` を閉じたあとはむしろ**再発防止のゲート**として効く（1本でも足すと exit 1）。
const BASELINE_COST_RULES = 0;
