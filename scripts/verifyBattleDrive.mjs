...
async function injectScenario(page, spec) {
  return await page.evaluate(async ({ SUPA_URL, ANON, CPU_PLAYER_ID, spec }) => {
    ...
    if (process.env.DEBUG_INJECT) console.log('[DEBUG_INJECT] gs.life_cloth=', JSON.stringify(gs.life_cloth), 'gs.field.signi=', JSON.stringify(gs.field?.signi));
