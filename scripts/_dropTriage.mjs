...
const MANUAL = {
  ...
  'WX15-047': ['機構待ち', 'トラップ設置の選択肢脱落'], 'WX16-062': ['機構待ち', 'トラップ別効果脱落'],
  'WXDi-CP02-074': ['機構待ち', '1効果脱落(ターン終了時公開・別timing)'], 'WXDi-CP02-089': ['機構待ち', '1効果脱落(アタック開始時付与)'],
  // 要確認＝真の脱落（実バグ候補・個別修正対象・JSON実体で確認済み）
  'WXK01-063': ['実バグ候補', 'POWER_MODIFY +2000脱落(JSON確認済)'], 'WX18-019': ['実バグ候補', 'SEARCH→ENERGY 2枚脱落(JSON=SHUFFLEのみ)'],
  'WDK15-009': ['実バグ候補', '対象2枚→1枚+フィルタ脱落(JSON確認済)'], 'WX17-028': ['実バグ候補', '【出】効果脱落'],
  'WX25-CP1-069': ['実バグ候補', 'トリガー誤り+遅延効果近似'],
};
...
const order = ['機構待ち', '偽陽性', '実バグ候補', '要確認'];
const out = [];
