...
// 🆕🔴**did-it ゲートが働いたログはここへ入れない**（2026-09-16・`O-530`）＝
//   「前段が空振り：「そうした場合」の効果は発生しない」／「前段が「そうした場合」の枚数に届かない：後続の
//   効果は発生しない」は**ゲートが正しく働いた印**で、同じ run に残る差分は原文どおりの部分実行。
//   失敗語（`足りない` 等）を engine 側のその文言に入れると、**自分の修正のたびに I3 が太る**
//   （実測＝`O-530` の初版で 73 → 93）。⇒ engine 側の文言もこの規約に合わせる。
