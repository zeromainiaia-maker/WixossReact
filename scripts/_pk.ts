...
const r=rows.find(r=>r.CardNum==='WXDi-P04-035');
console.log('TEXT:', r?.EffectText?.slice(0,160));
const fresh=mergeManualEffects('WXDi-P04-035', parseCardEffects({...r,effects:[]} as unknown as CardData));
fresh.forEach((e:any,i:number)=>console.log(`[${i}] ${e.effectId} timing=${JSON.stringify(e.timing)} scope=${e.triggerScope} filter=${JSON.stringify(e.triggerFilter)}`));
// which file & parity
const files=['WX','WXDi','WX24_26','WXK','misc']; let ff=''; for(const f of files){const j=JSON.parse(readFileSync(`public/data/effects_${f}.json`,'utf-8'));if(j['WXDi-P04-035']){ff=f;break;}}
console.log('in effects_'+ff+'.json');
const exist=JSON.parse(readFileSync(`public/data/effects_${ff}.json`,'utf-8'))['WXDi-P04-035'];
