...
const BASELINE_ECHO_IDS = 0;   // 🏁2026-09-13 O-356 クローズ＝A群 44 → 0。残りを「engine が決め打ち→固定文」「engine が原文を読む→payload 化（src/data/sourceTextPayloads.ts）」「live 0 の安全網→固定文」で全部閉じた。⚠0 が正常値＝逆翻訳に原文の切り出しを新しく書いたらここで止まる（再発防止ゲート）。 以前＝払い戻し③＝DECLARE_NUMBER の分岐（実際に原文を貼っていたのは DECLARE_NUMBER_RANGE の4効果）を numberChoices から描いた（parser＋engine＋decompiler の三点セット・実バグ2件）。 以前＝払い戻し②＝ARTS_COST_REDUCTION_BY_EFFECT（live 59ノード）を撤去＝コストは costJa が cost payload（useTimeCost/costReplacement）から描く。PLAN の「payload なし」分類は STUB ノード単体で数えた誤り（58カード全部 cost 側に payload があった）。 以前＝払い戻し①＝payload を持つ10 id（COPY_LRIG_NAME_ABILITY / DESIGNATE_SIGNI_ZONE / DEPLOY_RESTRICT / GAIN_SIGNI_BARRIER / PICK_FROM_TRASHED_CARDS / COLLAB / DECLARE_CLASS / SET_OPP_SIGNI_AS_TRAP / CAST_FROM_OPP_TRASH / DOWN_UP_SIGNI_AND_CHOOSE）を payload から描いた後の実数。新設時は 56
