...
const BASELINE = 32; // 🔻**32**（2026-09-15・§5.3 `O-374`＝**全54候補を live JSON と engine まで当たって仕分けた**）＝22件の払い戻し。真バグ3系統（`SPDi43-21-E2` マドカ条件の脱落／`WXK02-057-E1` 条件付きアサシンの付与先／「《X》か《Y》」の両払い）＋逆翻訳の嘘（「代わりに」を差分で描く10効果・`HAS_CARD_IN_FIELD` の体数とレベル・`REVEAL_AND_PICK` の「公開し手札に加え」3効果・`WX22-027-E2` の帰結・`POWER_MULTIPLY` の倍率・`lifeCountReached`・`EXILE_CRAFTS` の枚数）。残32は偽陽性（構造の番号「以下の３つ」・「２回繰り返す＝3回」・「４枚以上なら／そうでなければ」等の等価表現）と payload に数値を持たない表示の穴（【英知】の複数レベル3件・宣言範囲0〜5）で、**1件ずつ判定済み**。以前＝54（2026-09-14・§5.3 `O-372` 第2バッチ）＝`WXK03-025-E2` の defer を実装し、ラベルに「引いた枚数と同じ枚数」が出たぶんの払い戻し（**消化**＝ラベル整備は numberdrift の払い戻しでもある／`O-354` と同型）。以前＝55（2026-09-14・§5.3 O-370①）＝`WXDi-P06-006-E1` の実働 guardAltHand と重複していた defer を除去し、原文の「3枚」が正規の逆翻訳だけで出るようになった。以前＝56：
//   `WXDi-P15-003-E1` に使用条件（ドリームチーム3色）を載せたため、
