...
const BASELINE_HIGH = 1476; // 2026-07-29 タスク12(xliv)(b3): WXDi-P14-053-E1 の引用【常】を GRANT_EFFECT{UNTIL_OPP_TURN_END} で実働化し1477→1476（機能修正）。旧: 2026-07-29 タスク12(xliv) Opus検証: WX09-022 と同文型の WX12-033-E2（PRESERVE カードで held ドリフト＝【ランサー】が無条件付与だった）に activeCondition を外科パッチし1478→1477（機能修正＝条件不成立時の過剰付与を停止）。旧: 2026-07-28 タスク12(xliv)(a3): WX09-022 の欠落 activeCondition（トラッシュに《エナジェ》）を parser から復元し1479→1478（機能実装の発火条件を忠実化）。旧: 2026-07-28 mandatory【出】先頭ゲート: 既存Condition語彙で表現可能な25効果を activeCondition/CONDITIONAL へ配線し1498→1479。旧: 2026-07-28 タスク12 watcher34: 「他のシグニが場に出たとき」30効果を any_ally/any_opp＋既存filter/conditionへ配線し、表現不能4効果を timing:[] で安全停止して1507→1498。旧: 2026-07-28 タスク16(検証時の追加是正): PR-461 の curated JSON が effectId 対応を1つ取り違えており【常】ダブルクラッシュを丸ごと欠落・【自】を timing なしで格納していたのを外科パッチで是正し1509→1507（機能実装＝ダブルクラッシュ復元＋ルリグアタック契機の配線）。カード単位 PRESERVE（E2 が PARTIAL）で build も heldReview も触れない位置にあった。旧: タスク16: WX20-046-E2 のエナ→手札契機を ON_HAND_ADDED として認識し1510→1509。旧: G154族 REMOVE_ABILITIES の count 是正で1511→1510。
