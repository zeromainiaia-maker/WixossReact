...
function main(): void {
  ...
  detail.push(`# 高シグナル欠落 効果総数（重複除外・effectId単位）: ${highAll.size}（ベースライン ${Math.min(BASELINE_HIGH, CURRENT_BASELINE_HIGH)}）`);
  ...
  console.log(`高シグナル欠落 効果総数(重複除外): ${highAll.size} ／ ベースライン: ${CURRENT_BASELINE_HIGH}`);
  console.log(`明細: docs/_vocab_census.txt`);

  if (highAll.size > CURRENT_BASELINE_HIGH) {
    console.error(`\n⚠回帰: 高シグナルがベースライン ${CURRENT_BASELINE_HIGH} を超過（${highAll.size}）。`
      + ' JSON手パッチ時はフィルタ語彙もセットで入れる（or 本ファイルのキー表・ベースラインを実数更新）。');
    process.exit(1);
  }
  if (highAll.size < CURRENT_BASELINE_HIGH) {
    console.log(`\n改善: ${CURRENT_BASELINE_HIGH} → ${highAll.size}。本ファイルの BASELINE_HIGH と PLAN.md §恒久指標を実数更新してよい。`);
  }
}
