...
const BASELINE_HIGH = 1971; // 続き198(Opus): look-pick（別文＋公開し＋class/color/level filter）の pick 脱落是正＝「デッキ上N枚見る。その中から＜C＞のシグニM枚を公開し手札に加え、残りをデッキ下」が汎用 LOOK_AND_REORDER に pick を食われ単なる並べ替えに退化していたのを REVEAL_AND_PICK+filter へ（40枚採用）で 1992→1971 改善＝実数更新。旧1992は続き197(viii)＝WX26-CP1-048 出自条件機構
