...
const BASELINE_HIGH = 1; // 2026-09-10（未開拓プール第244）＝`WD20-008-E1` 1件。**計器の語彙不足であってバグではない**。
// 原文「各プレイヤーは自分のデッキの上からカードを**３枚まで**エナゾーンに置く」を、
// `ENERGY_CHARGE_FROM_DECK` に `upToCount` が**無い**ため **CHOOSE の 0/1/2/3 の4分岐**で表している（妥当な表現）。
// この計器は「Nまで」を `upToCount` 系のキーで探すので、CHOOSE 分岐で表した上限を**見つけられない**。
// 同じ効果が「制限（できない）」でも1件出るが、そちらは `condition:NO_OTHER_ARTS_USED_THIS_TURN` で表現済み。
// 🔑**`ENERGY_CHARGE_FROM_DECK.upToCount` を実装したら 0 へ戻すこと**（そのときは再発防止ゲートとして 0 が正常値）。
// 旧: const BASELINE_HIGH = 0; // 2026-09-06（§5.3 `O-132` 第2バッチ）＝残1件（`WX25-P1-022-E2`）が偽陽性だったので較正して 1→**0**。🔑`PLAY_FREE` は `execPlayFree` が `SEARCH{maxPick:1}`＋0枚可を出す＝それ自体が「1枚まで」の上限スロット（型注記がこのカードを名指しで正準形と書いている）。⇒ **0 が正常値の再発防止ゲート**になった＝新しく語彙が落ちたら即 exit 1。旧:  🆕 2026-09-04 `O-244`（`WXDi-P10-007-E3`）を明示 defer にして 2→1。
...
//   ⚠**穴が埋まったのではなく「嘘をやめた」**＝旧 live は相手の3枚が自分の1枚に化け、
//   さらに自分のシグニをデッキの一番下へ送る**自傷**だった。`DEFERRED_*` は census の高シグナルから外れる
//   （STUB 免除）ので、**この 1 は PLAN §5.3 の `O-243` へ移った**という意味。
// 旧: const BASELINE_HIGH = 3; // 2026-09-03 `O-60` 第17〜20バッチ＝**5→3 は較正（下げ忘れの回収）であって前進ではない。**
//   🔴**この巡の実装は高シグナルを1件も動かしていない**（`O-60` は「engine が原文を読む」形を潰す項目で
//     census の網に載らない）。実測すると **HEAD の明細も既に3件**（`WXDi-P10-007-E3` / `WX21-028-E2` /
//     `WX25-P1-022-E2`）で、前巡が `WX14-016-E1` / `WDA-F02-08-E1` の消滅ぶんを下げ忘れていた。
//   ⇒ 明細（`docs/_vocab_census.txt`）の高シグナル節を HEAD と A/B して**同一**であることを確認済み。
// 旧: const BASELINE_HIGH = 5; // 2026-09-02 索引C **第9巡・15件**（O-84/O-114/O-180/O-151/O-164/O-167/O-177/
//   O-186/O-203/O-205/O-206/O-209/O-210/O-213/O-72）で 6→5。🔴**内訳を混ぜない**：
//   ■**実装 -1**＝`WX21-031-CB-E2`（§5.3 `O-167`）＝**`AUTO` のまま**コスト `bounceSelf`
//     （このシグニを場から手札に戻す）が JSON に載ったので高シグナルから外れた＝**真の前進**。
//     旧はこの句がどのコスト規則にも当たらず**エナ《白》だけで撃てた**（踏み倒し）。
//   ■**この巡で MANUAL 免除に入った分は 0**＝新しく `MANUAL` にした効果（`WX15-010-E1` /
//     `WXK09-029-E2` / `WX16-Re20-E1,E2` / `WX25-P2-052-E2` / `PR-K026-E1` / `WXDi-CP02-001-E1`）は
//     もともと高シグナルに載っていない（STUB/MANUAL 格納側だった）ので、**較正での目減りはしていない**。
//   ■🔴**逆に2効果が AUTO へ戻った**（`WX24-P4-050-E2` / `WXDi-CP01-002-E1`＝parser に規則を足して
//     manual 影武者を削除＝`§6.4 O-42 tripwire` の指示どおり）が、どちらも高シグナルには出ていない。
//   ■**据置**＝残5件（`WXDi-P10-007-E3` / `WX14-016-E1` / `WX21-028-E2` / `WX25-P1-022-E2` /
//     `WDA-F02-08-E1`）は前巡までに理由つきで見送り済み（アンコールのコストは `costs.ts` が読む等）。
// 旧: const BASELINE_HIGH = 6; // 2026-09-02 索引C 5件（O-162/O-199/O-200/O-201/O-202）で 12→6。🔴**内訳を混ぜない**：
//   ■**実装 -1**＝`WXDi-CP02-100-E1`（§5.3 `O-201`）＝**`AUTO` のまま**コスト
//     `trashToDeckBottom` を parser が出すようになったので高シグナルから外れた（免除ではない＝真の前進）。
//   ■🔴**較正（MANUAL 免除）-4**＝`WXK02-004-E3` / `WXK03-014-E3`（`O-200`）／`WX24-P3-043-E1` /
//     `WXEX2-28-E1`（`O-202`）は**手書き（MANUAL）にしたので census の免除に入った**分。
//     ⚠**中身は原文から書き直してあり「移設だけ」ではない**（4件とも旧 live は別物のアクションだった）が、
//     **census の -4 は前進ではなく免除**。前進の証拠は golden の両方向 assert 側にある。
//   ■**据置**＝`WDA-F02-08-E1` / `WX14-016-E1`（`O-199`）は**高シグナルのまま残した**＝
//     アンコールのテキスト形コストは `screens/battle/costs.ts` が読むので **effects JSON には載らない**。
//     計器から見れば「原文にコスト句があるのに JSON に無い」ままなのが正しい（嘘をつかせない）。
// 旧: const BASELINE_HIGH = 12; // 2026-08-31 続き749 census -18（30→12）。🔴**内訳を混ぜない**：
  ...
  //   （`WX05-013-E2`）。⚠**`ON_ATTACK` を部分文字列に含まない**綴りなので既存キーで拾えなかった。
  {
    ...
    extraOk: (js, t) => {
      ...
      if (/"lrigAttack(Guarded|NoDamage)":true/.test(js)) {
        //   🆕2026-08-31 続き758＝**主語が「このルリグ」の綴りも同じ族**（`WXDi-D04-004-sub-E1`＝
        //     「**この**ルリグがアタックしたとき、そのアタック終了時、そのアタックによって対戦相手に
        //     ダメージが与えられていなかった場合」）。旧 regex は「（センター）ルリグN体」しか剥がせず、
        //     同じ受け皿へ配線した瞬間に高シグナルへ昇格していた（＝計器の較正漏れ）。
        const sG = t.replace(/(?:(?:センター)?ルリグ[０-９\d]体|この(?:センター)?ルリグ)がアタックしたとき[、,]そのアタック終了時[、,]/g, '');
