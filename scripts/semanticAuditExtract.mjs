...
59. 🔑**「クラフトではないアーツ」のコスト限定は既に効いている**（2026-09-16・第369バッチ・「O-496」 が stale）＝
    「matchesTrashArtsFromLrigDeckCost」 が 「Type === 'アーツ'」 の**完全一致**を見るので、「'アーツ/クラフト'」 は候補から外れる。
    ⇒ **「クラフト除外が JSON に無い」という finding は報告しない**（engine 側で満たされている）。
    ⚠ただし**逆向き**（原文が「クラフトではない」と書いていないのに engine が除外している）は実在する＝「O-521」 に集約済み。
