...
import { collectGrowCostReductions, calcFieldPowers, collectGrantedFromLayer, checkActiveCondition, calcActiveCostMods, collectCharmShieldSigni, applyContinuousBaseLevelOverride, calcContinuousBlockedActions, collectBanishSubstitutes } from '../src/engine/effectEngine';
import {
  executeEffect,
...
test('色別ARTS_USED_THIS_TURN: 白のアーツを使用していた場合のみ発火（続き106・WX24-D1-11）', () => {
  const fires = (colors?: string[]) => {
    ...
    return collectTurnTriggers(trigCtx(HOST, HOST), 'ON_TURN_END', host, mkState({})).some(e => e.effectId === 'WX24-D1-11-E1');
  };
