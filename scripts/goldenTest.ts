...
test('evalCondition HAND_TRASHED_BY_OPP / ENERGY_TRASHED_BY_OPP（engine）', () => {
  const base = mkCtx({}, {});
  const c1 = { type: 'HAND_TRASHED_BY_OPP', owner: 'self', operator: 'gte', value: 1 } as unknown as import('../src/types/effects').Condition;
  eq(evalCondition(c1, base), false, '未発生なら false');
  const hit = { ...base, ownerState: { ...base.ownerState, hand_trashed_by_opp_this_turn: 2 } };
  eq(evalCondition(c1, hit), true, '相手効果で手札が捨てられていれば true');
  const c2 = { type: 'ENERGY_TRASHED_BY_OPP', owner: 'self', operator: 'gte', value: 1 } as unknown as import('../src/types/effects').Condition;
  eq(evalCondition(c2, hit), false, 'エナ側は独立したカウンタ');
});
test('execTrash: 相手の手札/エナをトラッシュすると相手側のカウンタが増える（engine）', () => {
  const ctx = mkCtx({}, {});
  const r = run({ type: 'TRASH', target: { type: 'HAND_CARD', owner: 'opponent', count: 1 } } as unknown as EffectAction, ctx);
