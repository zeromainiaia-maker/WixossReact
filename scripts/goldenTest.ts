...
test('POWER_SET 相手シグニをパワー5000に固定: delta=value-base(=2000) が temp_power_mods に記録', () => {
  const ctx = mkCtx({}, { signi: [SIGNI_P3000, null, null] });
  const r = run({ type: 'POWER_SET', target: { type: 'SIGNI', owner: 'opponent', count: 1, filter: { cardType: 'シグニ' } }, value: 5000 } as EffectAction, ctx);
  const mods = (r.otherState as PlayerState).temp_power_mods ?? [];
  ok(mods.some(m => m.cardNum === SIGNI_P3000 && m.delta === 2000), `temp_power_mods (${JSON.stringify(mods)})`);
});
test('ENERGY_CHARGE 手札シグニ1: 手札-1 エナ+1', () => {
  const ctx = mkCtx({ hand: 5 }, {});
...
test('STORY_CHANGE: 対象シグニの story_overrides を書き換え', () => {
  const ctx = mkCtx({}, { signi: [SIGNI, null, null] });
