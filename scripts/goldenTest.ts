...
// ROADMAP batch5 第1波：ルリグ共通色の否定/肯定フィルタを、実 executor の候補集合で正負固定する。
test('colorNotMatchesLrig: 相手エナは相手ルリグ非共通色だけが候補', () => {
  ...
  const ctx = mkCtx({}, {});
  ctx.otherState.field.lrig = [redLrig];
  ctx.otherState.energy = [red, blue];
  const eff = { effectId: 'batch5-energy', effectType: 'AUTO', duration: 'INSTANT', mandatory: true,
    action: { type: 'TRASH', target: { type: 'ENERGY_CARD', owner: 'opponent', count: 1, filter: { colorNotMatchesLrig: true } } } } as CardEffect;
  const r = executeEffect(eff, ctx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET', '相手エナ選択へpause');
  if (r.done || r.pending.type !== 'SELECT_TARGET') return;
  eq(JSON.stringify(r.pending.candidates), JSON.stringify([blue]), '共通色の赤は候補外、非共通色の青だけ候補');
});
test('colorNotMatchesLrig: 相手シグニの BANISH/POWER_MODIFY は相手ルリグ非共通色だけが候補', () => {
  const redLrig = findCard(c => c.Type === 'ルリグ' && c.Color === '赤');
  const red = findCard(c => isSigni(c) && c.Color === '赤');
  const blue = findCard(c => isSigni(c) && c.Color === '青');
  for (const action of [
    { type: 'BANISH', target: { type: 'SIGNI', owner: 'opponent', count: 1, filter: { cardType: 'シグニ', colorNotMatchesLrig: true } } },
    { type: 'POWER_MODIFY', target: { type: 'SIGNI', owner: 'opponent', count: 1, filter: { cardType: 'シグニ', colorNotMatchesLrig: true } }, delta: -7000 },
  ] as EffectAction[]) {
    const ctx = mkCtx({}, { signi: [red, blue, null] });
    ctx.otherState.field.lrig = [redLrig];
    const r = executeEffect({ effectId: 'batch5-signi', effectType: 'AUTO', duration: 'INSTANT', mandatory: true, action } as CardEffect, ctx);
    ok(!r.done && r.pending.type === 'SELECT_TARGET', `${action.type} は相手シグニ選択へpause`);
    if (r.done || r.pending.type !== 'SELECT_TARGET') continue;
    eq(JSON.stringify(r.pending.candidates), JSON.stringify([blue]), `${action.type}: 共通色の赤は候補外、非共通色の青だけ候補`);
  }
});

// ROADMAP batch5b: 同一性filter。実executor入口で一致候補だけを残し、参照不能時は空ヒットにする。
test('levelEqLastProcessed: BANISH は直前カードと同レベルだけを候補化し、参照不能なら空', () => {
  const cursorBefore = cursor;
  const ctx = mkCtx({}, { signi: [SIGNI_L1, SIGNI_L2, null] });
  ctx.lastProcessedCards = [SIGNI_L2];
  const action = { type: 'BANISH', target: { type: 'SIGNI', owner: 'opponent', count: 1,
    filter: { cardType: 'シグニ', levelEqLastProcessed: true } } } as EffectAction;
  const hit = run(action, ctx);
  ok(hit.done && hit.otherState.field.signi[0]?.at(-1) === SIGNI_L1 && !hit.otherState.field.signi[1], 'Lv2だけ自動バニッシュ');
  const miss = run(action, { ...ctx, lastProcessedCards: [] });
  ok(miss.done && miss.otherState.field.signi[0]?.at(-1) === SIGNI_L1 && miss.otherState.field.signi[1]?.at(-1) === SIGNI_L2,
    '参照不能なら無制限化せず空ヒット');
  cursor = cursorBefore;
});
test('levelEqLastProcessedCount/LevelSum: SEARCH は枚数・表記レベル合計と一致するカードだけ', () => {
  const cursorBefore = cursor;
  const ctx = mkCtx({}, {});
  ctx.ownerState.deck = [SIGNI_L1, SIGNI_L2, SIGNI_L3, SIGNI_L4];
  ctx.lastProcessedCards = [SIGNI_L1, SIGNI_L2];
  for (const [filter, expected] of [
    [{ cardType: 'シグニ', levelEqLastProcessedCount: true }, SIGNI_L2],
    [{ cardType: 'シグニ', levelEqLastProcessedLevelSum: true }, SIGNI_L3],
  ] as const) {
    const r = run({ type: 'SEARCH', from: { location: 'deck', owner: 'self' }, filter, maxCount: 1,
      then: { type: 'ADD_TO_HAND', owner: 'self' } } as EffectAction, ctx);
    ok(r.done && r.ownerState.hand.includes(expected), '一致レベルだけをSEARCHして手札へ');
  }
  cursor = cursorBefore;
});
test('nameEqLastProcessed/levelEqLrig: TRANSFER_TO_HAND と TRASH の主要入口で完全一致', () => {
  const cursorBefore = cursor;
  const sameNameA = findCard(c => isSigni(c) && !!c.CardName);
  const sameName = cardMap.get(sameNameA)!.CardName;
  const otherName = findCard(c => isSigni(c) && c.CardName !== sameName);
  const ctx = mkCtx({}, { signi: [SIGNI_L1, SIGNI_L2, null] });
  ctx.ownerState.trash = [sameNameA, otherName];
  ctx.lastProcessedCards = [sameNameA];
  const hand = run({ type: 'TRANSFER_TO_HAND', source: { type: 'TRASH_CARD', owner: 'self', count: 1,
    filter: { cardType: 'シグニ', nameEqLastProcessed: true } } } as EffectAction, ctx);
  ok(hand.done && hand.ownerState.hand.includes(sameNameA) && hand.ownerState.trash.includes(otherName), '同名だけ回収');
  const lrig = findCard(c => c.Type === 'ルリグ' && c.Level === '2');
  ctx.ownerState.field.lrig = [lrig];
  const trash = run({ type: 'TRASH', target: { type: 'SIGNI', owner: 'opponent', count: 1,
    filter: { cardType: 'シグニ', levelEqLrig: 'self' } } } as EffectAction, ctx);
  ok(trash.done && trash.otherState.field.signi[0]?.at(-1) === SIGNI_L1 && !trash.otherState.field.signi[1], 'Lv2だけTRASH');
  cursor = cursorBefore;
});
// 5b検証是正: WXK10-001-E1 の再構成で codex が GRANT_KEYWORD（アタックできない付与）を落とした退化を
// Claude が復元＝3段構造（付与→同レベルBOUNCE→未満BANISH）を構造 golden で固定し再発を防ぐ。
test('WXK10-001-E1: アタック禁止付与→levelEqLrig BOUNCE→levelLtOppLrig BANISH の3段を保持', () => {
  const eff = effectsMap.get('WXK10-001')?.find(e => e.effectId === 'WXK10-001-E1');
  ok(!!eff, 'effect在中'); if (!eff) return;
  const act = eff.action as SequenceAction;
  eq(act.type, 'SEQUENCE', 'SEQUENCE');
  eq(JSON.stringify(act.steps.map(s => s.type)), JSON.stringify(['GRANT_KEYWORD', 'BOUNCE', 'BANISH']), '3段構造');
  const g = act.steps[0] as { keyword?: string; duration?: string };
  eq(g.keyword, 'アタックできない', '付与キーワード');
  eq(g.duration, 'UNTIL_END_OF_TURN', 'ターン終了時まで');
  const b = act.steps[1] as { target: { filter?: { levelEqLrig?: string } } };
  eq(b.target.filter?.levelEqLrig, 'opponent', 'BOUNCE は相手ルリグ同レベル');
  const bn = act.steps[2] as { target: { filter?: { levelLtOppLrig?: boolean } } };
  eq(bn.target.filter?.levelLtOppLrig, true, 'BANISH は相手ルリグ未満');
});
test('colorMatchesLrig: 自エナ→手札は自ルリグ共通色だけが候補', () => {
  const redLrig = findCard(c => c.Type === 'ルリグ' && c.Color === '赤');
  const red = findCard(c => isSigni(c) && c.Color === '赤');
  const blue = findCard(c => isSigni(c) && c.Color === '青');
  const ctx = mkCtx({}, {});
  ctx.ownerState.field.lrig = [redLrig];
  ctx.ownerState.energy = [red, blue];
  const r = executeEffect({ effectId: 'batch5-transfer', effectType: 'AUTO', duration: 'INSTANT', mandatory: true,
    action: { type: 'TRANSFER_TO_HAND', source: { type: 'ENERGY_CARD', owner: 'self', count: 1, upToCount: true, filter: { cardType: 'シグニ', colorMatchesLrig: true } } } } as CardEffect, ctx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET', '自エナ選択へpause');
  if (r.done || r.pending.type !== 'SELECT_TARGET') return;
  eq(JSON.stringify(r.pending.candidates), JSON.stringify([red]), '共通色の赤だけ候補、非共通色の青は候補外');
});
...
test('batch11 opponentSelects: 相手エナ選択と新入口2種が opponentResponds を立てる', () => {
  const pendingOf = (action: EffectAction, ctx: ExecCtx) => {
    const result = executeEffect({
      effectId: 'GOLDEN-BATCH11', effectType: 'AUTO', action,
      duration: 'INSTANT', mandatory: true,
    } as CardEffect, ctx);
    ok(!result.done, `${action.type} は選択待ちになる`);
    return (result as { pending: { opponentResponds?: boolean } }).pending;
  };
  const energyPending = pendingOf(
    { type: 'TRASH', target: { type: 'ENERGY_CARD', owner: 'opponent', count: 1 }, opponentSelects: true },
    mkCtx({}, { energy: 2 }),
  );
  eq(energyPending.opponentResponds, true, 'TRASH ENERGY は相手応答');

  const sendPending = pendingOf(
    { type: 'SEND_TO_ENERGY', target: { type: 'SIGNI', owner: 'opponent', count: 1 }, opponentSelects: true },
    mkCtx({}, { signi: [SIGNI, null, null] }),
  );
  eq(sendPending.opponentResponds, true, 'SEND_TO_ENERGY は相手応答');

  const deckPending = pendingOf(
    { type: 'TRANSFER_TO_DECK', source: { type: 'SIGNI', owner: 'opponent', count: 1 }, shuffle: false, position: 'bottom', opponentSelects: true },
    mkCtx({}, { signi: [SIGNI, null, null] }),
  );
...
test('batch11 non-regression: 相手手札TRASHはフラグなしでも相手が選ぶ', () => {
  const result = executeEffect({
    effectId: 'GOLDEN-BATCH11-HAND', effectType: 'AUTO',
    action: { type: 'TRASH', target: { type: 'HAND_CARD', owner: 'opponent', count: 1 } },
    duration: 'INSTANT', mandatory: true,
  } as CardEffect, mkCtx({}, { hand: 2 }));
  ok(!result.done, 'TRASH HAND は選択待ちになる');
  eq((result as { pending: { opponentResponds?: boolean } }).pending.opponentResponds, true, '既定の相手選択を維持');
});
...
test('§6.3 M1: 追加ダウンコストは down_self と別に既存 fieldDown へ（語彙一本化・検証是正）', () => {
  // codex 実装は新語彙 signiDown を並設し、既存 fieldDown と重複して WXDi-P14-040 が二重コスト化した。
  // 既存 fieldDown の regex を「＜A＞か＜B＞の」「(色)の」へ拡張して一本化（支払い/可否/表示は既存経路を流用）。
  const wx14 = parseCardEffects(cardMap.get('WX14-055')!)[0];
  eq(wx14.cost?.down_self, true, '自身の《ダウン》コスト');
  eq(wx14.cost?.fieldDown?.count, 1, '追加シグニ1体ダウン');
  eq(JSON.stringify(wx14.cost?.fieldDown?.filter?.story), JSON.stringify(['鉱石', '宝石']), '鉱石/宝石 OR filter');
  eq(wx14.cost?.fieldDown?.filter?.isUp, true, 'アップ状態要求');
  const gt = parseCardEffects(cardMap.get('WXK11-056')!);
  eq(gt.find(e => e.effectId === 'WXK11-056-E2')?.cost?.fieldDown?.count, 1, '赤1体');
  eq(gt.find(e => e.effectId === 'WXK11-056-E2')?.cost?.fieldDown?.filter?.color, '赤', '赤 filter');
  eq(gt.find(e => e.effectId === 'WXK11-056-E3')?.cost?.fieldDown?.count, 2, '赤2体');
  // WXDi-P14-040-E3（素の「アップ状態のシグニ1体をダウンする」）は fieldDown 単独のまま＝二重コスト回帰の検知
  const p14 = parseCardEffects(cardMap.get('WXDi-P14-040')!).find(e => e.effectId === 'WXDi-P14-040-E3');
  eq(p14?.cost?.fieldDown?.count, 1, '素の文型は fieldDown 1体');
  eq(JSON.stringify(Object.keys(p14?.cost ?? {}).sort()), JSON.stringify(['fieldDown']), 'コストは fieldDown のみ（重複語彙なし）');
});

test('§6.3 M2: 対象固定→任意手札3枚コストは拒否時no-op／承諾時だけ同じ対象をバニッシュ', () => {
  const eff = mergeManualEffects('WXDi-P04-036', effectsMap.get('WXDi-P04-036') ?? []).find(e => e.effectId === 'WXDi-P04-036-E2');
  ok(!!eff, 'manual E2'); if (!eff) return;
  const target = SIGNI_P12000;
  const no = mkCtx({ signi: ['WXDi-P04-036', null, null], hand: 2 }, { signi: [target, null, null] }, 'WXDi-P04-036');
  const rn = run(eff.action, no);
  eq(rn.otherState.field.signi[0]?.at(-1), target, '支払い不能ならバニッシュしない');
  const yes = mkCtx({ signi: ['WXDi-P04-036', null, null], hand: 3 }, { signi: [target, null, null] }, 'WXDi-P04-036');
  const ry = run(eff.action, yes);
  eq(ry.ownerState.hand.length, 0, '手札3枚を実支払い');
  eq(ry.otherState.field.signi[0], null, '固定した対象だけバニッシュ');
});

test('§6.3 M3: 動的フィルタと条件を原文どおり保持', () => {
  const p02 = mergeManualEffects('WXDi-P02-038', effectsMap.get('WXDi-P02-038') ?? []);
  ok(JSON.stringify(p02.find(e => e.effectId === 'WXDi-P02-038-E1')).includes('colorNotMatchesOppLrig'), '相手センタールリグ非共通色');
  ok(JSON.stringify(p02.find(e => e.effectId === 'WXDi-P02-038-E2')).includes('ENERGY_TRASHED_BY_OPP'), '相手エナ2枚条件');
  const p14 = mergeManualEffects('WXDi-P14-043', effectsMap.get('WXDi-P14-043') ?? []);
  ok(JSON.stringify(p14.find(e => e.effectId === 'WXDi-P14-043-E2')).includes('powerLteSelf'), '自身実効パワー以下');
});

test('§6.3 M4: 正面以外／非アタックフェイズでは watcher が発火しない', () => {
  const merged = mergeManualEffects('WX15-055', effectsMap.get('WX15-055') ?? []);
  const em = new Map(effectsMap); em.set('WX15-055', merged);
  const beforeGuest = mkState({ signi: [null, null, SIGNI_L2] });
  const afterGuest = mkState({ signi: [null, null, null] });
  const host = mkState({ signi: ['WX15-055', null, null] });
  const attackCtx = { ...trigCtx(HOST, HOST), effectsMap: em, turnPhase: 'ATTACK_SIGNI' as const };
  const hit = collectBanishTriggers(attackCtx, SIGNI_L2, GUEST, host, afterGuest, beforeGuest);
  eq(fired(hit.entries, 'WX15-055-E1'), true, '正面＋アタックフェイズ＋アップで発火');
  const mainCtx = { ...attackCtx, turnPhase: 'MAIN' as const };
  const missPhase = collectBanishTriggers(mainCtx, SIGNI_L2, GUEST, host, afterGuest, beforeGuest);
  eq(fired(missPhase.entries, 'WX15-055-E1'), false, 'メインフェイズでは非発火');
  const sidePrev = mkState({ signi: [SIGNI_L2, null, null] });
  const missFront = collectBanishTriggers(attackCtx, SIGNI_L2, GUEST, host, afterGuest, sidePrev);
  eq(fired(missFront.entries, 'WX15-055-E1'), false, '正面以外では非発火');
  // 「あなたの」アタックフェイズ＝watcher所有者のターン限定（相手のアタックフェイズ中の正面被バニでは発火しない・検証是正）
  const oppTurnCtx = { ...trigCtx(GUEST, HOST), effectsMap: em, turnPhase: 'ATTACK_SIGNI' as const };
  const missTurn = collectBanishTriggers(oppTurnCtx, SIGNI_L2, GUEST, host, afterGuest, beforeGuest);
  eq(fired(missTurn.entries, 'WX15-055-E1'), false, '相手ターンのアタックフェイズでは非発火');
});

test('§6.3 cost-game wave 2: MANUAL定義が支払い・保存対象・多段閾値を保持', () => {
  const d08 = JSON.stringify(mergeManualEffects('WXDi-D08-012', effectsMap.get('WXDi-D08-012') ?? [])
    .find(e => e.effectId === 'WXDi-D08-012-E1'));
  ok(d08.includes('"exceed":4') && d08.includes('"targetsStored":true') && d08.includes('"type":"EXILE"') && d08.includes('"type":"BANISH"'),
    'WXDi-D08-012: exceed4支払いで同じ対象をEXILE、未払いでBANISH');
  const p15 = JSON.stringify(mergeManualEffects('WXDi-D09-P15', effectsMap.get('WXDi-D09-P15') ?? [])
    .find(e => e.effectId === 'WXDi-D09-P15-E1'));
  ok(p15.includes('"powerRange":{"max":12000}') && p15.includes('"handDiscardGroups"') && p15.includes('"hasGuard":true')
    && p15.includes('"type":"BOUNCE"') && p15.includes('"type":"EXILE"'),
    'WXDi-D09-P15: 先行対象＋2択それぞれの実支払いと適用先');
  const wx16 = JSON.stringify(mergeManualEffects('WX16-033', effectsMap.get('WX16-033') ?? [])
    .find(e => e.effectId === 'WX16-033-BURST'));
  ok(wx16.includes('"owner":"self","count":1') && wx16.includes('"targetsStored":true') && !wx16.includes('"IS_MY_TURN"'),
    'WX16-033-BURST: 自シグニをバニッシュできた場合だけ先行対象をトラッシュ');
  const d01 = JSON.stringify(mergeManualEffects('WXDi-D01-016', effectsMap.get('WXDi-D01-016') ?? [])
    .find(e => e.effectId === 'WXDi-D01-016-E1'));
  ok(d01.includes('"value":20000') && d01.includes('"blind":true') && d01.includes('"value":15000'),
    'WXDi-D01-016: 20000段を先に評価し15000段へelse分岐');
});

test('WXDi-D08-012 挙動: 支払い→EXILE／スキップ→BANISH が同一の先行対象へ（skip側凍結の検証是正）', () => {
  // resume 後の ctx には storedTargetCards が生存しない（BattleScreen は永続化しない）＝
  // pay/skip 両オプションとも interaction 生成時に fixedCardNums へ凍結されていなければならない。
  const eff = mergeManualEffects('WXDi-D08-012', effectsMap.get('WXDi-D08-012') ?? []).find(e => e.effectId === 'WXDi-D08-012-E1');
  ok(!!eff, 'E1'); if (!eff) return;
  const target = SIGNI_P12000;
  const mk = () => {
    const c = mkCtx({}, { signi: [target, null, null] }, 'WXDi-D08-012');
    // エクシード4の原資＝センタールリグ下4枚（最上段はルリグ本体）
    c.ownerState.field.lrig = ['U1', 'U2', 'U3', 'U4', 'LRIG-TOP'];
    return c;
  };
  // 素の executeEffect で段階制御（run() は完走してしまう）：recorder SELECT_TARGET → 任意コスト CHOOSE
  const stepToChoose = (c: ExecCtx) => {
    let r = executeEffect({ effectId: 't', effectType: 'AUTO', action: eff.action, duration: 'INSTANT', mandatory: true } as CardEffect, c);
    ok(!r.done && (r as { pending: { type: string } }).pending.type === 'SELECT_TARGET', 'recorder が対象選択を出す');
    if (r.done) throw new Error('unreachable');
    r = resumeSelectTarget([target], r.pending as never, { ...c, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs } as ExecCtx);
    ok(!r.done && (r as { pending: { type: string } }).pending.type === 'CHOOSE', '任意コスト CHOOSE');
    return r;
  };
  // 支払い経路：EXILE が先行対象へ・ルリグ下4枚がルリグトラッシュへ
  // ⚠resume ctx に storedTargetCards を渡さない＝BattleScreen の pause 跨ぎ実挙動（凍結が効いているかの検証）
  const finishSelect = (r: ExecResult, base: ExecCtx): ExecResult => {
    // EXILE/BANISH の対象選択（候補1でも SELECT_TARGET を出す慣例）を解決。
    // fixedCardNums 凍結の検証のため storedTargetCards は引き継がない。
    if (!r.done && (r as { pending: { type: string } }).pending.type === 'SELECT_TARGET') {
      return resumeSelectTarget([target], (r as { pending: unknown }).pending as never, { ...base, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs } as ExecCtx);
    }
    return r;
  };
  const c1 = mk();
  const r1 = stepToChoose(c1);
  const pay = finishSelect(resumeChoose('pay', (r1 as { pending: unknown }).pending as never, { ...c1, ownerState: r1.ownerState, otherState: r1.otherState, logs: r1.logs } as ExecCtx), c1);
  eq(pay.otherState.field.signi[0], null, '支払い時は対象が場を離れる');
  ok((pay.otherState.excluded ?? []).includes(target), '支払い時は excluded へ');
  eq(pay.ownerState.lrig_trash.length, 4, 'エクシード4を実支払い');
  // スキップ経路：BANISH が同じ先行対象へ（＝エナへ。空振りしない）
  const c2 = mk();
  const r2 = stepToChoose(c2);
  const skip = finishSelect(resumeChoose('skip', (r2 as { pending: unknown }).pending as never, { ...c2, ownerState: r2.ownerState, otherState: r2.otherState, logs: r2.logs } as ExecCtx), c2);
  eq(skip.otherState.field.signi[0], null, '未払い時も対象が場を離れる（空振りしない）');
  ok(skip.otherState.energy.includes(target), '未払い時はバニッシュ＝エナへ');
  eq(skip.ownerState.lrig_trash.length, 0, '未払い時はエクシード消費なし');
});

test('WX16-033-BURST 挙動: 自シグニ選択の pause を跨いで先行対象へ TRASH（storedTargetCards 永続化の契約）', () => {
  // BattleScreen は pause 時に result.storedTargetCards を pending_effect に保存し resume ctx へ復元する
  // （PendingEffect.storedTargetCards＝triggeringCardNum と同型・検証是正）。その契約をここで固定する。
  const eff = mergeManualEffects('WX16-033', effectsMap.get('WX16-033') ?? []).find(e => e.effectId === 'WX16-033-BURST');
  ok(!!eff, 'BURST'); if (!eff) return;
  const target = SIGNI_P12000;
  const ctx = mkCtx({ signi: [SIGNI_L1, SIGNI_L2, null] }, { signi: [target, null, null] }, 'WX16-033');
  let r = executeEffect({ effectId: 't', effectType: 'AUTO', action: eff.action, duration: 'INSTANT', mandatory: true } as CardEffect, ctx);
  ok(!r.done && (r as { pending: { type: string } }).pending.type === 'SELECT_TARGET', '相手対象の選択');
  if (r.done) return;
  r = resumeSelectTarget([target], r.pending as never, { ...ctx, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs } as ExecCtx);
  // 自シグニ2体 → バニッシュ対象選択で pause。ここで storedTargetCards が result に露出していること（永続化の前提）
  ok(!r.done && (r as { pending: { type: string } }).pending.type === 'SELECT_TARGET', '自シグニ選択で pause');
  ok((r.storedTargetCards ?? []).includes(target), 'pause 結果に storedTargetCards が露出（BattleScreen が保存できる）');
  if (r.done) return;
  // BattleScreen の resume と同様に storedTargetCards を復元して続行
  r = resumeSelectTarget([SIGNI_L1], r.pending as never, { ...ctx, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs, storedTargetCards: r.storedTargetCards } as ExecCtx);
  if (!r.done && (r as { pending: { type: string } }).pending.type === 'SELECT_TARGET') {
    r = resumeSelectTarget([target], r.pending as never, { ...ctx, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs, storedTargetCards: r.storedTargetCards } as ExecCtx);
  }
  ok(r.done, '完走');
  ok(r.ownerState.energy.includes(SIGNI_L1) || r.ownerState.trash.includes(SIGNI_L1), '自シグニはバニッシュ済み');
  eq(r.otherState.field.signi[0], null, '先行対象が場を離れる');
  ok(r.otherState.trash.includes(target), '「それをトラッシュに置く」＝バニッシュ（エナ）ではなくトラッシュへ');
});

test('§6.3 cost-game wave 2: SELF_POWER_GTE 境界14999/15000/19999/20000', () => {
  const cond15 = { type: 'SELF_POWER_GTE', value: 15000 } as const;
  const cond20 = { type: 'SELF_POWER_GTE', value: 20000 } as const;
  const at = (power: number) => {
    const c = mkCtx({}, {}, 'WXDi-D01-016');
    c.effectivePowers = new Map([['WXDi-D01-016', power]]);
    return c;
  };
  ok(!evalCondition(cond15, at(14999)) && evalCondition(cond15, at(15000)), '15000境界');
...
// §6.3 G072: 条件前置き付き「対戦相手のシグニがバニッシュされたとき」完結バッチ。
const g072Map = (cardNum: string) => {
  const map = new Map(effectsMap);
  map.set(cardNum, mergeManualEffects(cardNum, effectsMap.get(cardNum) ?? []));
  return map;
};
test('G072 WX05-040-E1: 自ターンMAINの相手バニッシュだけ発火', () => {
  const host = mkState({ signi: ['WX05-040', null, null] }); const guest = mkState({});
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WX05-040'), turnPhase: 'MAIN' };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WX05-040-E1'), true, '自ターンMAIN＋相手で発火');
  eq(has(collectBanishTriggers({ ...ctx, turnPhase: 'ATTACK_SIGNI' }, SIGNI_L2, GUEST, host, guest).entries, 'WX05-040-E1'), false, 'ATTACKでは非発火');
  eq(has(collectBanishTriggers({ ...ctx, activeUserId: GUEST }, SIGNI_L2, GUEST, host, guest).entries, 'WX05-040-E1'), false, '相手ターンMAINでは非発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, HOST, host, guest).entries, 'WX05-040-E1'), false, '自分シグニでは非発火');
});
test('G072 WX11-027-E2: 自ターンMAINの相手バニッシュだけ発火', () => {
  const host = mkState({ signi: ['WX11-027', null, null] }); const guest = mkState({});
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WX11-027'), turnPhase: 'MAIN' };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WX11-027-E2'), true, '自ターンMAIN＋相手で発火');
  eq(has(collectBanishTriggers({ ...ctx, turnPhase: 'ATTACK_SIGNI' }, SIGNI_L2, GUEST, host, guest).entries, 'WX11-027-E2'), false, 'ATTACKでは非発火');
  eq(has(collectBanishTriggers({ ...ctx, activeUserId: GUEST }, SIGNI_L2, GUEST, host, guest).entries, 'WX11-027-E2'), false, '相手ターンMAINでは非発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, HOST, host, guest).entries, 'WX11-027-E2'), false, '自分シグニでは非発火');
});
test('G072 WXDi-P11-TK05-E2: 除去直前にチャーム付きの場合だけ発火', () => {
  const host = mkState({ signi: ['WXDi-P11-TK05', null, null] }); const guest = mkState({});
  const before = mkState({ signi: [SIGNI_L2, null, null] }); before.field.signi_charms = [SIGNI_L1, null, null];
  const noCharm = mkState({ signi: [SIGNI_L2, null, null] });
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXDi-P11-TK05') };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, before).entries, 'WXDi-P11-TK05-E2'), true, 'チャーム付きで発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, noCharm).entries, 'WXDi-P11-TK05-E2'), false, 'チャーム無しで非発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WXDi-P11-TK05-E2'), false, 'before-state無しは非発火');
});
test('G072 WXK11-055-E2: watcher所有者の効果起因だけ発火', () => {
  const host = mkState({ signi: ['WXK11-055', null, null] }); const guest = mkState({});
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXK11-055') };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, undefined, { ownerId: HOST }).entries, 'WXK11-055-E2'), true, '自効果で発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WXK11-055-E2'), false, 'cause無しで非発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, undefined, { ownerId: GUEST }).entries, 'WXK11-055-E2'), false, '相手効果で非発火');
});
test('G072 WX13-051-E1: 自分の龍獣シグニ効果起因だけ発火', () => {
  const dragon = findCard(c => isSigni(c) && (c.CardClass ?? '').includes('龍獣'));
  const nonDragon = findCard(c => isSigni(c) && !(c.CardClass ?? '').includes('龍獣'));
  const host = mkState({ signi: ['WX13-051', null, null] }); const guest = mkState({});
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WX13-051') };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, undefined, { ownerId: HOST, sourceCardNum: dragon }).entries, 'WX13-051-E1'), true, '自龍獣シグニ効果で発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, undefined, { ownerId: HOST, sourceCardNum: nonDragon }).entries, 'WX13-051-E1'), false, '非龍獣で非発火');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WX13-051-E1'), false, 'cause無しで非発火');
});
test('G072 WXEX2-23-E2: 被バニッシュレベル以下をSELECTし傀儡配置までresume', () => {
  const host = mkState({ signi: ['WXEX2-23', null, null] }); const guest = mkState({});
  guest.trash = [SIGNI_L1, SIGNI_L3];
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXEX2-23'), turnPhase: 'ATTACK_SIGNI' };
  const hit = collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest);
  const entry = hit.entries.find(e => e.effectId === 'WXEX2-23-E2');
  eq(entry?.triggeringCardNum, SIGNI_L2, '被バニッシュカードをentryへ保持');
  eq(has(collectBanishTriggers({ ...ctx, turnPhase: 'MAIN' }, SIGNI_L2, GUEST, host, guest).entries, 'WXEX2-23-E2'), false, 'MAINでは非発火');
  ok(!!entry, 'collector発火'); if (!entry) return;
  const execCtx = { ...mkCtx({ signi: ['WXEX2-23', null, null] }, {}, 'WXEX2-23'), otherState: guest, triggeringCardNum: entry.triggeringCardNum } as ExecCtx;
  let r = executeEffect(entry.effect, execCtx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET', '相手トラッシュ選択へpause'); if (r.done) return;
  eq(JSON.stringify(r.pending.candidates), JSON.stringify([SIGNI_L1]), 'レベル超のシグニは候補外');
  r = resumeSelectTarget([SIGNI_L1], r.pending as never, { ...execCtx, ownerState: r.ownerState, otherState: r.otherState, logs: r.logs });
  ok(r.done, 'resume完走');
  eq(r.otherState.trash.includes(SIGNI_L1), false, '相手トラッシュから除去');
  ok(r.ownerState.field.signi.some(z => z?.at(-1) === SIGNI_L1), '自分の場へ配置');
  ok((r.ownerState.field.puppet_signi ?? []).includes(SIGNI_L1), '傀儡状態を登録');
});
// G072 の entry.triggeringCardNum 追加で挙動が変わった既存 watcher の lock-in（Claude 検証で発見）：
// WXK11-020-E2「そのシグニよりパワーの低い対戦相手のシグニ1体をバニッシュ」の powerLtTrigger は
// 従来 ON_BANISH watcher entry に triggeringCardNum が無く未解決＝無制限バニッシュの潜在過剰効果だった。
// 今回の追加で被バニッシュシグニのパワー基準（cardMap 表記パワー）へ正しく解決される＝改善方向の挙動変化を固定する。
test('G072副作用 WXK11-020-E2: 被バニッシュのパワー未満だけが候補', () => {
  const banished = findCard(c => isSigni(c) && c.Power === '8000');
  const low = findCard(c => isSigni(c) && c.Power === '3000');
  const high = findCard(c => isSigni(c) && c.Power === '12000');
  const host = mkState({ signi: ['WXK11-020', null, null] });
  const guest = mkState({ signi: [low, high, null] });
  const ctx = trigCtx(HOST, HOST);
  const entry = collectBanishTriggers(ctx, banished, GUEST, host, guest).entries.find(e => e.effectId === 'WXK11-020-E2');
  eq(entry?.triggeringCardNum, banished, '被バニッシュカードをentryへ保持');
  ok(!!entry, 'collector発火'); if (!entry) return;
  const execCtx = { ...mkCtx({ signi: ['WXK11-020', null, null] }, {}, 'WXK11-020'), otherState: guest, triggeringCardNum: entry.triggeringCardNum } as ExecCtx;
  const r = executeEffect(entry.effect, execCtx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET', '相手シグニ選択へpause'); if (r.done) return;
  eq(JSON.stringify(r.pending.candidates), JSON.stringify([low]), 'パワー8000以上（12000）は候補外・3000のみ');
});

test('G072第2波 正面3枚: 正面だけ発火・WX16-068は攻撃中を抑止', () => {
  for (const [card, id] of [['WX09-011', 'WX09-011-E1'], ['WX16-068', 'WX16-068-E1'], ['WXK11-046', 'WXK11-046-E2']] as const) {
    const host = mkState({ signi: [card, null, null] }); const guest = mkState({});
    const front = mkState({ signi: [null, null, SIGNI_L2] });
    const side = mkState({ signi: [SIGNI_L2, null, null] });
    const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map(card) };
    eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, front).entries, id), true, `${id} 正面`);
    eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, side).entries, id), false, `${id} 非正面`);
    if (card === 'WX16-068') {
      eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, front, undefined, card).entries, id), false, '攻撃中は非発火');
    }
  }
});

test('G072第2波 チャーム2枚: 直前チャーム限定・scope any', () => {
  for (const [card, id] of [['WXK07-074', 'WXK07-074-E1'], ['WXK11-041', 'WXK11-041-E3']] as const) {
    const host = mkState({ signi: [card, null, null] }); const guest = mkState({});
    const before = mkState({ signi: [SIGNI_L2, null, null] }); before.field.signi_charms = [SIGNI_L1, null, null];
    const plain = mkState({ signi: [SIGNI_L2, null, null] });
    const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map(card) };
    const owner = card === 'WXK07-074' ? HOST : GUEST;
    eq(has(collectBanishTriggers(ctx, SIGNI_L2, owner, host, guest, before).entries, id), true, `${id} charm`);
    eq(has(collectBanishTriggers(ctx, SIGNI_L2, owner, host, guest, plain).entries, id), false, `${id} no charm`);
    if (card === 'WXK11-041') eq(has(collectBanishTriggers(ctx, SIGNI_L2, HOST, host, guest, before).entries, id), true, '所有者を問わず発火');
  }
});

test('G072第2波 WX20-025-E2: 中央×自ターンMAINのみ', () => {
  const host = mkState({ signi: ['WX20-025', null, null] }); const guest = mkState({});
  const center = mkState({ signi: [null, SIGNI_L2, null] }); const edge = mkState({ signi: [SIGNI_L2, null, null] });
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WX20-025'), turnPhase: 'MAIN' };
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, center).entries, 'WX20-025-E2'), true, '中央MAIN');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest, edge).entries, 'WX20-025-E2'), false, '端');
  eq(has(collectBanishTriggers({ ...ctx, turnPhase: 'ATTACK_SIGNI' }, SIGNI_L2, GUEST, host, guest, center).entries, 'WX20-025-E2'), false, 'ATTACK');
  eq(has(collectBanishTriggers(ctx, SIGNI_L2, GUEST, host, guest).entries, 'WX20-025-E2'), false, 'beforeなし');
});

test('G072第2波 WXK11-018-E1: watcher未満だけ発火しtriggeringCardNumをエナから場へ', () => {
  const host = mkState({ signi: ['WXK11-018', null, null] }); host.energy = [SIGNI_L2];
  const before = mkState({ signi: ['WXK11-018', SIGNI_L2, null] });
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXK11-018') };
  const entry = collectBanishTriggers(ctx, SIGNI_L2, HOST, host, mkState({}), before).entries.find(e => e.effectId === 'WXK11-018-E1');
  ok(!!entry, '低レベルで発火'); if (!entry) return;
  eq(has(collectBanishTriggers(ctx, SIGNI_L4, HOST, host, mkState({}), mkState({ signi: [SIGNI_L4, null, null] })).entries, 'WXK11-018-E1'), false, '同レベル以上は非発火');
  eq(entry.triggeringCardNum, SIGNI_L2, '被バニッシュを保持');
  const execCtx = { ...mkCtx({ signi: ['WXK11-018', null, null] }, {}, 'WXK11-018'), triggeringCardNum: SIGNI_L2 } as ExecCtx;
  execCtx.ownerState.energy = [SIGNI_L2];
  const r = executeEffect(entry.effect, execCtx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET', '任意の対象選択'); if (r.done) return;
  eq(JSON.stringify(r.pending.candidates), JSON.stringify([SIGNI_L2]), 'エナのtrigger sourceだけ');
  const absentCtx = { ...execCtx, ownerState: { ...execCtx.ownerState, energy: [] } };
  const absent = executeEffect({ ...entry.effect, action: { ...(entry.effect.action as import('../src/types/effects').AddToFieldAction), optional: false } }, absentCtx);
  ok(absent.done, 'エナ不在は不発');
});

test('G072第2波 WXDi-P03-035-E2: self経路でアップ状態だけ発火', () => {
  const after = mkState({}); const up = mkState({ signi: ['WXDi-P03-035', null, null] });
  const down = mkState({ signi: ['WXDi-P03-035', null, null] }); down.field.signi_down = [true, false, false];
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXDi-P03-035') };
  eq(has(collectBanishTriggers(ctx, 'WXDi-P03-035', HOST, after, mkState({}), up).entries, 'WXDi-P03-035-E2'), true, 'アップ');
  eq(has(collectBanishTriggers(ctx, 'WXDi-P03-035', HOST, after, mkState({}), down).entries, 'WXDi-P03-035-E2'), false, 'ダウン');
  eq(has(collectBanishTriggers(ctx, 'WXDi-P03-035', HOST, after, mkState({})).entries, 'WXDi-P03-035-E2'), false, 'beforeなし');
});

test('G072第2波 WX09-014-E2/WD14-011-E1: triggeringCardNum相対レベルを固定', () => {
  for (const [card, id] of [['WX09-014', 'WX09-014-E2'], ['WD14-011', 'WD14-011-E1']] as const) {
    const eff = g072Map(card).get(card)?.find(e => e.effectId === id); ok(!!eff, id); if (!eff) continue;
    const c = mkCtx({ signi: [card, null, null] }, { signi: [SIGNI_L1, SIGNI_L2, SIGNI_L3] }, card);
    c.triggeringCardNum = SIGNI_L2;
    if (card === 'WD14-011') c.ownerState.trash = [SIGNI_L1, SIGNI_L3];
    const r = executeEffect(eff, c);
    ok(!r.done && r.pending.type === 'SELECT_TARGET', `${id} select`); if (r.done) continue;
    eq(JSON.stringify(r.pending.candidates), JSON.stringify(card === 'WX09-014' ? [SIGNI_L2] : [SIGNI_L1]), `${id} level filter`);
  }
});

// G072群C の CONT 経路解消（Opus直修正）: calcContinuousSigniMutations が発生源（sourceCardNum/sourceIsHost）を
// 返し、BattleScreen CONT バニッシュ経路がそれを cause として collectBanishTriggers へ渡す＝
// 「あなたの効果によって…バニッシュされたとき」（banishedByOwnEffect）が CONT バニッシュでも発火する。
test('G072群C CONT経路: mutation が発生源を運び cause で banishedByOwnEffect が発火', () => {
  // 合成 CONT BANISH（mandatory・相手シグニ全）を host の SIGNI_L1 に持たせる
  const contEff: CardEffect = {
    effectId: 'TEST-CONT-BANISH', effectType: 'CONTINUOUS', mandatory: true,
    action: { type: 'BANISH', target: { type: 'SIGNI', owner: 'opponent', count: 'ALL' } } as EffectAction,
    duration: 'PERMANENT', parseStatus: 'MANUAL',
  } as CardEffect;
  const map = new Map(effectsMap);
  map.set(SIGNI_L1, [contEff]);
  const host = mkState({ signi: [SIGNI_L1, 'WXK11-055', null] });
  const guest = mkState({ signi: [SIGNI_L2, null, null] });
  const muts = calcContinuousSigniMutations(host, guest, true, map, cardMap);
  const mut = muts.find(m => m.effectId === 'TEST-CONT-BANISH');
  ok(!!mut, 'CONT BANISH mutation'); if (!mut) return;
  eq(mut.sourceCardNum, SIGNI_L1, '発生源カードを保持');
  eq(mut.sourceIsHost, true, '発生源の所有者を保持');
  eq(JSON.stringify(mut.targetNums), JSON.stringify([SIGNI_L2]), '相手シグニが対象');
  // BattleScreen 8120 相当: mutation の発生源を cause として渡すと WXK11-055-E2（あなたの効果によって）が発火
  const ctx = { ...trigCtx(HOST, HOST), effectsMap: g072Map('WXK11-055') };
...
test('batch5c2 E2E: sharedColor all は共通色ペアだけを確定し不正ペアをresumeで弾く', () => {
  const redA = findCard(c => isSigni(c) && c.Color?.includes('赤'));
  const redB = findCard(c => isSigni(c) && c.Color?.includes('赤') && c.CardNum !== getCardNumG(redA));
  const blue = findCard(c => isSigni(c) && c.Color?.includes('青') && !c.Color?.includes('赤'));
  const ctx = mkCtx({}, {}); ctx.ownerState.trash = [redA, redB, blue];
  const action = { type:'TRANSFER_TO_HAND', source:{ type:'TRASH_CARD', owner:'self', count:2,
    filter:{cardType:'シグニ'}, selectionConstraint:{sharedColor:'all'} } } as EffectAction;
  const r = executeEffect({effectId:'b5c2-shared',effectType:'AUTO',duration:'INSTANT',mandatory:true,action} as CardEffect,ctx);
  ok(!r.done && r.pending.type === 'SELECT_TARGET','sharedColor選択へpause'); if (r.done || r.pending.type !== 'SELECT_TARGET') return;
  const bad = resumeSelectTarget([redA,blue],r.pending as never,{...ctx});
  eq(bad.ownerState.hand.includes(redA),true,'先頭の正当候補は移動');
  eq(bad.ownerState.hand.includes(blue),false,'共通色のない追加候補は拒否');
  const good = resumeSelectTarget([redA,redB],r.pending as never,{...ctx});
  ok(good.ownerState.hand.includes(redA) && good.ownerState.hand.includes(redB),'共通赤ペアは確定');
});

test('batch5c2 FIELD_LRIGS_SHARE_COLOR: exec/active両評価＋2体/3体段階', () => {
  const red = [...cardMap.values()].filter(c => c.Type === 'ルリグ' && c.Color?.includes('赤')).slice(0,3).map(c=>c.CardNum);
  const blue = findCard(c => c.Type === 'ルリグ' && c.Color?.includes('青') && !c.Color?.includes('赤'));
  eq(red.length,3,'赤ルリグ3枚が必要');
  const yes = mkCtx({},{}); yes.ownerState.field.lrig=[red[0]]; yes.ownerState.field.assist_lrig_l=[red[1]];
  const no = mkCtx({},{}); no.ownerState.field.lrig=[red[0]]; no.ownerState.field.assist_lrig_l=[blue];
  const c2 = {type:'FIELD_LRIGS_SHARE_COLOR',owner:'self',minCount:2} as const;
  ok(evalCondition(c2,yes) && !evalCondition(c2,no),'CONDITIONAL正負');
  ok(checkActiveCondition(c2,yes.ownerState,yes.otherState,true,cardMap) && !checkActiveCondition(c2,no.ownerState,no.otherState,true,cardMap),'activeCondition正負');
  yes.ownerState.field.assist_lrig_r=[red[2]];
  const c3 = {type:'FIELD_LRIGS_SHARE_COLOR',owner:'self',minCount:3} as const;
  ok(checkActiveCondition(c3,yes.ownerState,yes.otherState,true,cardMap),'3体共通色で+3000段成立');
  no.ownerState.field.assist_lrig_r=[red[2]];
  ok(!checkActiveCondition(c3,no.ownerState,no.otherState,true,cardMap),'2体だけ共通色では3体段不成立');
});

test('batch5c2 colorMatchesUnderCards: 下カード色の正負候補', () => {
  const red = findCard(c => isSigni(c) && c.Color?.includes('赤'));
  const source = findCard(c => isSigni(c) && c.CardNum !== getCardNumG(red));
  const oppRed = findCard(c => isSigni(c) && c.Color?.includes('赤') && c.CardNum !== getCardNumG(red));
  const oppBlue = findCard(c => isSigni(c) && c.Color?.includes('青') && !c.Color?.includes('赤'));
  const ctx = mkCtx({},{signi:[oppRed,oppBlue,null]},source);
  ctx.ownerState.field.signi = [[red, source], null, null];
  const a={type:'BOUNCE',target:{type:'SIGNI',owner:'opponent',count:1,filter:{cardType:'シグニ',colorMatchesUnderCards:true}}} as EffectAction;
  const r=executeEffect({effectId:'under-color',effectType:'AUTO',duration:'INSTANT',mandatory:true,action:a} as CardEffect,ctx);
  ok(!r.done && r.pending.type==='SELECT_TARGET','候補選択'); if(r.done||r.pending.type!=='SELECT_TARGET')return;
  ok(r.pending.candidates.includes(oppRed) && !r.pending.candidates.includes(oppBlue),'下の赤だけに一致');
});

test('batch5c2 colorMatchesCostTrashed: コスト記録色の正負候補／記録無し空ヒット', () => {
  const red = findCard(c => isSigni(c) && c.Color?.includes('赤'));
  const oppRed = findCard(c => isSigni(c) && c.Color?.includes('赤') && c.CardNum !== getCardNumG(red));
  const oppBlue = findCard(c => isSigni(c) && c.Color?.includes('青') && !c.Color?.includes('赤'));
  const a={type:'BOUNCE',target:{type:'SIGNI',owner:'opponent',count:1,filter:{cardType:'シグニ',colorMatchesCostTrashed:true}}} as EffectAction;
  const ctx=mkCtx({},{signi:[oppRed,oppBlue,null]}); ctx.ownerState.last_cost_trashed_cards=[red];
  const r=executeEffect({effectId:'cost-color',effectType:'AUTO',duration:'INSTANT',mandatory:true,action:a} as CardEffect,ctx);
  ok(!r.done && r.pending.type==='SELECT_TARGET','記録あり候補選択'); if(!r.done&&r.pending.type==='SELECT_TARGET') ok(r.pending.candidates.includes(oppRed)&&!r.pending.candidates.includes(oppBlue),'赤だけ一致');
  const none=executeEffect({effectId:'cost-color-none',effectType:'AUTO',duration:'INSTANT',mandatory:true,action:a} as CardEffect,mkCtx({},{signi:[oppRed,null,null]}));
  ok(none.done && none.ownerState.pending_effect==null,'記録無しは空ヒット');
});

test('NEGATE_NTH_ATTACK: WX10-018 はシグニ/ルリグ共有2回を設定', () => {
...
test('NEGATE_NTH_ATTACK: SP27-016 の②選択後はシグニ/ルリグ共有1回を設定', () => {
  const effect = effectsMap.get('SP27-016')?.find(e => e.effectId === 'SP27-016-E1');
  ok(!!effect, 'SP27-016-E1が必要');
  if (!effect) return;
  const ctx = mkCtx({}, {}, 'SP27-016');
  const first = executeEffect(effect, ctx);
  ok(!first.done && first.pending.type === 'CHOOSE', 'SP27-016はCHOOSE待ちになるはず');
  if (first.done || first.pending.type !== 'CHOOSE') return;
  const result = resumeChoose('c1', first.pending, {
    ...ctx, ownerState: first.ownerState, otherState: first.otherState, logs: first.logs,
  });
  eq(JSON.stringify(result.ownerState.negate_opp_attacks), JSON.stringify({ remaining: 1, signi: true, lrig: true }));
});

test('NEGATE_NTH_ATTACK: WX17-006 はベット節を混ぜずシグニのみ共有2回を設定', () => {
  const effect = effectsMap.get('WX17-006')?.find(e => e.effectId === 'WX17-006-E1');
  const seq = effect?.action.type === 'SEQUENCE' ? effect.action : undefined;
  const nth = seq?.steps.find(a =>
    a.type === 'CONDITIONAL' && a.then.type === 'STUB' && a.then.id === 'NEGATE_NTH_ATTACK');
  ok(!!nth && nth.type === 'CONDITIONAL', 'WX17-006のNEGATE_NTH_ATTACK節が必要');
  if (!nth || nth.type !== 'CONDITIONAL') return;
  const result = run(nth.then, mkCtx({}, {}, 'WX17-006'));
  eq(JSON.stringify(result.ownerState.negate_opp_attacks), JSON.stringify({ remaining: 2, signi: true, lrig: false }));
  ok(seq!.steps.some(a => a.type === 'CONDITIONAL' && a.condition.type === 'IS_BETTING'
    && a.then.type === 'NEGATE_ATTACK'), 'ベット時の別NEGATE_ATTACKを保持するはず');
});

test('NEGATE_NTH_ATTACK honor: ルリグ対象なら消費し、対象外なら状態不変', () => {
  const base = mkState({});
  base.negate_opp_attacks = { remaining: 1, signi: true, lrig: true };
  const positive = consumeNthAttackNegation(base, 'lrig');
  ok(positive.negated, 'lrig:trueなら無効化するはず');
  eq(positive.remaining, 0);
  eq(positive.defender.negate_opp_attacks, undefined, '最終1回を消費したらフラグを除去');

  const signiOnly = mkState({});
  signiOnly.negate_opp_attacks = { remaining: 1, signi: true, lrig: false };
  const negative = consumeNthAttackNegation(signiOnly, 'lrig');
  ok(!negative.negated, 'lrig:falseなら無効化しないはず');
  eq(negative.defender, signiOnly, '対象外攻撃ではstateを変更しない');
  eq(negative.defender.negate_opp_attacks?.remaining, 1, '対象外攻撃では共有カウンタを消費しない');
});

test('POWER_PLUS_BANISHED_POWER: 被バニッシュ実効パワーを白シグニへ次の相手ターン終了時まで加算', () => {
  const white = findCard(c => isSigni(c) && c.Color?.includes('白'));
  ...
  const ctx = mkCtx({ signi: [white, otherWhite, null] }, {}, 'WX24-P2-049');
  ctx.triggeringCardNum = banished;
  ctx.banishedSigniPower = 8000;
  const result = executeEffect(effect, ctx);
  ok(!result.done && result.pending.type === 'SELECT_TARGET', '白シグニの対象選択になるはず');
  if (result.done || result.pending.type !== 'SELECT_TARGET') return;
  const resolved = resumeSelectTarget([otherWhite], result.pending, {
    ...ctx, ownerState: result.ownerState, otherState: result.otherState, logs: result.logs,
  });
  ok(resolved.done, '対象選択後に解決完了');
  ok((resolved.ownerState.power_mods_until_opp_turn ?? []).some(m => m.cardNum === otherWhite && m.delta === 8000),
    '選んだ白シグニへ+8000');
  ok(!(resolved.ownerState.temp_power_mods ?? []).some(m => m.cardNum === otherWhite),
    'UNTIL_OPP_TURN_END は temp_power_mods に入れない');
});

test('POWER_PLUS_BANISHED_POWER: 白シグニ不在またはパワー0はno-op', () => {
  const nonWhite = findCard(c => isSigni(c) && !c.Color?.includes('白'));
  const action = {
    type: 'STUB', id: 'POWER_PLUS_BANISHED_POWER',
    powerPlusBanishedPower: {
      target: { type: 'SIGNI', owner: 'self', count: 1, filter: { color: '白' } },
      duration: 'UNTIL_OPP_TURN_END',
    },
  } as EffectAction;
  const noTarget = mkCtx({ signi: [nonWhite, null, null] }, {});
  noTarget.banishedSigniPower = 8000;
  eq((run(action, noTarget).ownerState.power_mods_until_opp_turn ?? []).length, 0, '白シグニ不在');
  const zero = mkCtx({ signi: [findCard(c => isSigni(c) && c.Color?.includes('白')), null, null] }, {});
  zero.banishedSigniPower = 0;
  eq((run(action, zero).ownerState.power_mods_until_opp_turn ?? []).length, 0, 'パワー0');
});
...
test('VARIABLE_ENERGY_TRASH_LEVEL_BOUNCE: ブルアカ2枚を選んでトラッシュし相手レベル2をバウンス', () => {
  ...
  const ctx = mkCtx({}, { signi: [SIGNI_L1, SIGNI_L2, SIGNI_L3] }, 'WX25-CP1-040');
  ctx.ownerState.energy = [...blueArchive];
  const first = executeEffect(effect, ctx);
  ok(!first.done && first.pending.type === 'SELECT_TARGET', 'エナ0〜3枚の選択になるはず');
  if (first.done || first.pending.type !== 'SELECT_TARGET') return;
  const afterCost = resumeSelectTarget(blueArchive.slice(0, 2), first.pending, {
    ...ctx, ownerState: first.ownerState, otherState: first.otherState, logs: first.logs,
  });
  ok(!afterCost.done && afterCost.pending.type === 'SELECT_TARGET', 'コスト後はレベル2の対象選択');
  if (afterCost.done || afterCost.pending.type !== 'SELECT_TARGET') return;
  eq(JSON.stringify(afterCost.pending.candidates), JSON.stringify([SIGNI_L2]), 'レベル2だけが候補');
  const resolved = resumeSelectTarget([SIGNI_L2], afterCost.pending, {
    ...ctx, ownerState: afterCost.ownerState, otherState: afterCost.otherState, logs: afterCost.logs,
  });
  ok(resolved.done, 'バウンス解決完了');
  eq(resolved.ownerState.energy.length, 1, 'エナから2枚減る');
  ok(blueArchive.slice(0, 2).every(n => resolved.ownerState.trash.includes(n)), '選択2枚がトラッシュへ');
  ok(resolved.otherState.hand.includes(SIGNI_L2), '相手レベル2が手札へ');
});

test('VARIABLE_ENERGY_TRASH_LEVEL_BOUNCE: 一致レベル不在でも選んだコストはトラッシュ', () => {
  const blueArchive = [...cardMap.values()].filter(c => c.CardClass?.includes('ブルアカ')).slice(0, 2).map(c => c.CardNum);
  const action = {
    type: 'STUB', id: 'VARIABLE_ENERGY_TRASH_LEVEL_BOUNCE',
    variableEnergyTrashLevelBounce: { story: 'ブルアカ', maxCount: 3 },
  } as EffectAction;
  const ctx = mkCtx({}, { signi: [SIGNI_L1, SIGNI_L3, null] });
  ctx.ownerState.energy = [...blueArchive];
  const first = executeEffect({ effectId: 'variable-bounce', effectType: 'ACTIVATED', action, duration: 'INSTANT', mandatory: false }, ctx);
  ok(!first.done && first.pending.type === 'SELECT_TARGET', 'エナ選択');
  if (first.done || first.pending.type !== 'SELECT_TARGET') return;
  const result = resumeSelectTarget(blueArchive, first.pending, {
    ...ctx, ownerState: first.ownerState, otherState: first.otherState, logs: first.logs,
  });
  ok(result.done, 'レベル2不在ならバウンス選択なしで完了');
