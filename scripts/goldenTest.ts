...
test('C1 ON_LRIG_GROW: any_opp 相手グロウで発火（watcher のターン中＝turnOwner:self 成立）', () => {
  ...
  const e = collectLrigGrowTriggers(trigCtx(GUEST), HOST, host, guest);
...
test('C1 ON_LRIG_GROW: turnOwner:self ゲート（相手ターン中の相手グロウでは非発火）＝続き73発見・続き75修正', () => {
  ...
  eq(collectLrigGrowTriggers(trigCtx(HOST), HOST, host, guest).length, 0, 'turnOwner:self ゲート');
});
...
test('Stage2 collectTurnTriggers: 設置した遅延トリガーがアタックフェイズ開始時に発火（F-4・engine）', () => {
  ...
  const host: PlayerState = { ...mkState({}), delayed_triggers: [dt] };
  const guest = mkState({});
  const fired = collectTurnTriggers(trigCtx(HOST, HOST), 'ON_ATTACK_PHASE_START', host, guest);
  eq(fired.some(e => e.effectId === 'DELAYED_TRIGGER'), true, 'アタックフェイズ開始時に発火');
  eq(collectTurnTriggers(trigCtx(HOST, HOST), 'ON_TURN_START', host, guest).some(e => e.effectId === 'DELAYED_TRIGGER'), false, '別 timing では非発火');
  eq(collectTurnTriggers(trigCtx(HOST, HOST), 'ON_ATTACK_PHASE_START', mkState({}), guest).some(e => e.effectId === 'DELAYED_TRIGGER'), false, '未設置は非発火');
});
...
test('THIS_CARD_HAS_UNDER filter: 下カードのレベル条件を評価（engine）', () => {
  // mkCtx の自分フィールドに 2枚スタック（下カードのレベルで条件が分岐）を作る
  const base = mkCtx({}, {});
  const src = base.ownerState.field.signi.find(s => s && s.length > 0)?.at(-1);
  if (!src) throw new Error('mkCtx に場シグニが無い');
  const zi = base.ownerState.field.signi.findIndex(s => s?.at(-1) === src);
  const underCn = base.ownerState.field.signi[zi]![0];
  const underLv = parseInt(base.cardMap.get(underCn.includes('#') ? underCn.slice(0, underCn.indexOf('#')) : underCn)?.Level ?? '0');
  // 自スタックの下に1枚積む（同じカードを複製して2枚スタックに）
  const stacked = { ...base, ownerState: { ...base.ownerState, field: { ...base.ownerState.field, signi: base.ownerState.field.signi.map((s, i) => i === zi ? [underCn, src] : s) } }, sourceCardNum: src };
  const hit = { type: 'CONDITIONAL', condition: { type: 'THIS_CARD_HAS_UNDER', filter: { cardType: 'シグニ', level: underLv } }, then: { type: 'DRAW', owner: 'self', count: 1 } } as unknown as EffectAction;
  const miss = { type: 'CONDITIONAL', condition: { type: 'THIS_CARD_HAS_UNDER', filter: { cardType: 'シグニ', level: underLv + 9 } }, then: { type: 'DRAW', owner: 'self', count: 1 } } as unknown as EffectAction;
  const n0 = stacked.ownerState.hand.length;
  eq(run(hit, stacked).ownerState.hand.length, n0 + 1, '下にレベル一致シグニがあれば真');
  eq(run(miss, stacked).ownerState.hand.length, n0, 'レベル不一致なら偽');
  eq(run(hit, { ...base, sourceCardNum: src }).ownerState.hand.length, base.ownerState.hand.length, '下にカードが無ければ偽');
});
