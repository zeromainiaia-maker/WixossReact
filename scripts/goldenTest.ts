...
test('GRANT_PROTECTION 効果耐性 subjectFilter/from/sourceCostMin（Opusタスク9・§6.2 系統②）', () => {
  ...
  //   《クロスアイコン》を持つ＜美巧＞のみバニッシュ保護（hasCrossIcon）。
  {
    const crossBiko = findSigni(c => (c.CardClass ?? '').includes('美巧') && (c.EffectText ?? '').startsWith('《クロスアイコン》'));
    const plainBiko = findSigni(c => (c.CardClass ?? '').includes('美巧') && !(c.EffectText ?? '').startsWith('《クロスアイコン》'));
...
test('グロウ支払い: colorOverrideMap でオフカラーのエナが指定色として払える', () => {
  ...
  eq(canAffordGrowCost(pool, cards, '《白》×1', undefined, undefined, undefined, undefined,
