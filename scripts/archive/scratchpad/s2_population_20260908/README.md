...
## 🔴 追記（同日・ユーザー指摘「S-2 は全部終わったの？」で発覚）

**系統がもう1件残っていた**＝「期限が『次のあなたのエナフェイズ終了時まで』でなく現ターン終了時」。
PLAN の「grep 実測3」を既測と読んだのが誤りで、**あれは「判定式が甘くて取りこぼした」という失敗の記録**だった。

測り直した結果＝**登録5 → 実測10効果**、しかも**候補10件が全部壊れていた**。
🔴**受け皿 `LIMIT_CHANGE_UNTIL_ENERGY_PHASE_END` は名前どおり動いていない**
（`lrig_limit_mod` はターン終了時に `undefined`＝`BattleScreen.tsx:4284` / `:4735`）。
⇒ §5.3 索引 G へ **`O-293`** として登録（全文は `docs/PLAN_DETAIL.md`）。

**S-2 全体＝系統7件＋機構4件／登録23効果 → 実測48効果。**
