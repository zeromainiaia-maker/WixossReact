...
if (ROUND_ROBIN.length >= 2) {
  ...
  // 🆕🔴§5.7 `S-30`（2026-09-21）＝**この表を CPU の版をまたいで比べない**。
  //   実測＝2026-09-20 の表（WD13 75.0% ＞ 天使軸1 ＞ ケトッシー軸 ＞ WD06 18.8%）は、
  //   2026-09-21 には **ケトッシー軸 85.4% ＞ 天使軸1 ＞ WD06 ＞ WD13 8.3%** と**1位と4位が入れ替わった**。
  //   🔑**同じ CPU なら順位は安定する**（seed 1 と seed 100 で同じ並び）＝動くのは**CPU の版が変わったとき**。
  //   ⇒ **「CPU が弱くなった」の判定にこの表を使わない**（それは `--a`/`--b` の mirror A/B の仕事）。
  console.log('🔴**この表は CPU の版をまたいで比べられない**＝同じ山でも CPU の挙動が変われば順位は入れ替わる'
    + '（実測＝2026-09-20 は WD13 が1位・2026-09-21 は4位）。**ポリシーの良し悪しは `--a`/`--b` の mirror A/B で測る。**');
