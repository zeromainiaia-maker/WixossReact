...
{"results":[{"cardNum":"WX01-001","findings":[{"effectId":"WX01-001-E1","severity":"HIGH|MED|LOW","type":"MISSING|WRONG|EXTRA","run":"断る|受ける|両方","quote":"原文の該当句（20字以内）","evidence":"食い違いを示す差分・ログ・候補の行をそのまま引用","claim":"食い違いの内容を日本語1文で","grep":"同じ壊れ方が他カードにもあるなら、それを探すための原文の言い回し（10字以内）。このカード固有なら null"}]}]}
...
function renderCard(t) {
  ...
  for (const tr of t.traces) {
    ...
    prevBoard = board;
    if (tr.runs.both) s += renderRun('断る／受ける 両方同じ', tr.runs.both);
    else s += renderRun('断る', tr.runs.decline) + renderRun('受ける', tr.runs.accept);
  }
  return s;
}
