...
const targets = new Set(['WXEX1-10', 'WXEX1-12', 'WX17-002', 'WDK10-001', 'WDK13-001']);
...
for (let i = 1; i <= 11; i++) {
  ...
  for (const r of data) {
    ...
    const card: CardData = {
      ...
      CardName: r.CardName ?? '',
      ImgURL: '', Type: r.Type ?? '',
      CardClass: '', Color: '', Level: r.Level ?? '',
      GrowCost: '', Cost: '', Limit: '', Power: '',
      Restriction: '', Team: '', Timing: '', Guard: '',
      Coin: r.Coin ?? '', Story: '',
    ...
    console.log(`EffectText hex prefix: ${Buffer.from(card.EffectText.slice(0, 30)).toString('hex')}`);
    // 【起】部分のみ抜き出して表示
    const activated = card.EffectText.match(/【起】[^【]*/g) ?? [];
    activated.forEach((a, i) => {
      console.log(`  [起${i+1}] ${a.slice(0, 150)}`);
    });

    const effects = parseCardEffects(card);
    const actEffects = effects.filter(e => e.effectType === 'ACTIVATED');
    actEffects.forEach(e => {
      console.log(`  → ${e.effectId}: cost=${JSON.stringify(e.cost)} parseStatus=${e.parseStatus}`);
    });
    if (actEffects.length === 0) console.log('  → ACTIVATED効果なし');
  }
}
