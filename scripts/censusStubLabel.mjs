...
const BASELINE_A = 1;   // 2026-09-16 第377バッチ（4 → 1＝4件とも **engine は正しくラベルだけが嘘**だった。
                        //   最大の収穫＝`OPP_ENERGY_COLORLESS_ABILITY_LOSS` のラベルが
                        //   **直前の stale コメント（別 id `SUMMON_FROM_TRASH_TO_HAND_BLACK` のもの）を拾った誤帰属**で、
                        //   「トラッシュから黒シグニを手札へ」と**全く別のカードの説明**が出ていた。
                        //   ⚠残る1件（`PR-Di017A-E1`）は**偽陽性**＝ラベルの「シグニ」が原文に literal に無いだけで、
                        //   `PR-Di017B`（REV:アンコーリング）は実際にシグニ＝**ラベルの方が正確**なので直さない。
                        //   旧: 4（2026-09-13 第315バッチ＝`O-353` Part B の全カードプール宣言描画で 7 → 4）
const BASELINE_B = 0;   // 2026-09-13 第302バッチ（`O-354` で 17→1、`O-355` で 1→0）
// 🔑**B群 0 は「ラベルを綺麗にした」結果ではない**＝最後の1件（`ATTACH_SEARCHED_AS_ACCE` の「手札経由近似」）は
//   **engine を直して**消した（`O-355`）。⚠**ラベルから実装メモだけ消して 0 にしない**＝
//   それをやると engine の逸脱が逆翻訳から見えなくなる（B群を置いている意味が無くなる）。
