...
const re=/ルリグアタックステップ開始時/;
const hits=rows.filter(r=>re.test(r.EffectText??'')).map(r=>r.CardNum);
console.log('CASCADE (ルリグアタックステップ開始時):', hits.join(', '));
...
for(const id of hits){
  ...
  const ff=fileOf(id); const exist=JSON.parse(readFileSync(`public/data/effects_${ff}.json`,'utf-8'))[id];
  console.log(`\n== ${id} (effects_${ff}) ==`);
  console.log('  TEXT:', (r?.EffectText??'').slice(0,170));
  fresh.forEach((e:any,i:number)=>console.log(`  FRESH[${i}] ${e.effectId} timing=${JSON.stringify(e.timing)} scope=${e.triggerScope}`));
