...
for(let i=1;i<=11;i++){try{rows.push(...Papa.parse<Record<string,string>>(readFileSync(`public/data/CardData_Sheet${i}.csv`,'utf-8').replace(/^﻿/,''),{header:true,skipEmptyLines:true}).data);}catch{}}
try{rows.push(...Papa.parse<Record<string,string>>(readFileSync('public/data/CardData_TK.csv','utf-8').replace(/^﻿/,''),{header:true,skipEmptyLines:true}).data);}catch{}
