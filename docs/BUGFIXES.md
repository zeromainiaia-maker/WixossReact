...
- **採用9枚（2ラウンド）**：**①BANISH/target 8枚**（`build:effects` 自動採用7＋WX22-Re01 は effect[0] の MANUAL カップリングで held 抑止のため parser 出力に合わせて JSON parity パッチ1）＝**powerLtSelf**＝WX22-019・WX22-Re01・WXDi-P00-051・WXK10-028・WXK10-068・WXK11-055・WD18-006、**powerGtSelf**＝WXK04-029。**②ビルダー routing 拡張 1枚**＝`parseSigniTarget` を通らない **energy→field / trash→field ADD_TO_FIELD ビルダー**（`parseSentencePart1.ts`）にも `parseSelfComparison` を配線し、自己蘇生分岐（thisCardOnly）を `!/このシグニより/` でガード＝**WXDi-P03-078**（エナから「このシグニよりパワーの低い＜地獣＞」を場に出す＝`ENERGY_CARD` source filter に powerLtSelf）。各ラウンド精密 semantic diff で**純粋な自己参照追加のみ（collateral 0）**を確認。WX13-034/WXK10-074 は HEAD で既に powerLtSelf 済み＝parity 維持（変化なし）。
- **検証**：typecheck・**golden 158/0（+3＝powerLtSelf/powerGtSelf/levelLtSelf の engine 解決を strict 同値境界で assert）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1631→1625（-6）**＝`BASELINE_HIGH` 実数更新。
