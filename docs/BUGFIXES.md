...
- 検証＝全対象で原文一致・**英語ID漏れ 548→505**（7系統は0、WXEX2-12除く）・同型★0 維持（割れ0／5986枚）・typecheck 緑・engine 不変。decompile_sheet2/3/4/5/7/8/9/10＋下流再生成。⚠残る中規模クラスタ（`REPEAT_*`/`DO_THREE_THINGS`/`CAST_FROM_OPP_TRASH`/`COLLAB`/`LOOK_TOP_*`/`SIGNI_REPOSITION`/`ACCE_FROM_HAND`）は原文表現が heterogeneous（枚数動的/語順不揃い/他アクションと同一文/SEQUENCE重複）で単一正規表現に不適＝個別精査要。特に `DO_THREE_THINGS` は一部カード（WXK11-007）で STUB が SEQUENCE 先頭かつ後続 step が③内容を再描画＝重複リスクあり要注意。`BET_*`系（38）は機構待ち。
