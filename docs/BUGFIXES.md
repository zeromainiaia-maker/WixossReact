...
- **② DRAW脱落（systematic のため本カードは MANUAL 温存）**＝condition が持ち上がった後の then「カードを１枚引き、ターン終了時まで、パワー＋7000」を `parseSingleSentence` 直呼び（CLAUSES then 経路）で処理する際、先頭 DRAW が後続の汎用 POWER_MODIFY 規則に飲まれて脱落する（top-level の `parseActionTextInner` drawAndM は通らない）。**本カードは完全形 `CONDITIONAL{lt, SEQUENCE[DRAW 1, POWER_MODIFY +7000 thisCardOnly]}` を parseStatus:MANUAL で温存**（[[effects-json-hand-maintained]]・「手書き効果は MANUAL 刻印必須」に従い直パッチ）。
  - ⚠**systematic 化を試みて revert した知見（続き59 で実験・不採用）**＝`parseSingleSentenceInner` に「カードをN枚引き、X」→`SEQUENCE[DRAW, ...parseSingleSentence(X)]` を足すと18枚が held に載る（全て原文に「カードをN枚引き」が実在＝真の過少バグ）。入れ子SEQUENCEは既存37効果で許容済みのため問題ないが、**2種の副作用で wholesale 採用不可**：(a)**既存の複合ハンドラを preempt して退化**＝WXK03-069「カードを１枚引き、手札からカード１枚をデッキの一番下に置く」が `SEQUENCE[DRAW, TRANSFER_TO_DECK]`（正）→`SEQUENCE[DRAW, LOOK_AND_REORDER]`（誤）に。分割後の rest「手札からカード１枚をデッキの一番下に置く」を isolation で parse すると TRANSFER_TO_DECK でなく LOOK_AND_REORDER になる単独fragment mis-parse が露出する（＝別の潜在バグ）。(b)一方で WX24-P3-022 は UNKNOWN→`SEQUENCE[DRAW, 公開]` に改善する＝**改善と退化が混在**。**真の解決には「単独fragment の parse 品質是正（手札→デッキ移動=TRANSFER_TO_DECK 等）」と「複合ハンドラを preempt しないガード」が前提**＝各カード verify 込みの慎重なバッチとして follow-up（PLAN §4 次の一手）。
