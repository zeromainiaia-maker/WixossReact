...
- 引用が見つからない場合（誤パース＝WX15-059/WX20-069 に原文引用無し／「以下の能力を得る。『…』」型＝引用が得るの後）は `[STUB:引用された能力を付与する（原文参照）]` にフォールバック（破損なし）。
- 跨ぎ抽出なし（最長111字も「…」を得る境界内）・typecheck緑・**同型★0維持**。⚠脱落疑い件数は文型多数決の揺れで±数枚動くが非指標（[docs §7]）。decompiler のみ＝データ/engine 不変。残課題は TODO §7。
