...
- **真因**＝`groupSimilar --all` の★（逆翻訳割れ）2グループは、どちらも**同じ文型の1枚にだけ修正が入り兄弟が旧形のまま**だった。
  - ①`WX24-P3-068-E1`（「対象とし、手札から＜水獣＞のシグニを１枚捨ててもよい。そうした場合、それをバニッシュする」）＝
    MANUAL が「**先に捨てる → あとから対象**」の旧形を固定しており、**対象不在でも手札を捨てられた**。
    母集団（`census:population -- "対象とし、手札から[^。]*捨ててもよい。?\s*そうした場合" --json SELECT_TARGET_ONLY`）は
    **36/37 が修正済み**で、これが唯一の取り残し。兄弟 `WXDi-P14-083-E1`（第246）と同じ
    `SELECT_TARGET_ONLY → STORE_LAST_PROCESSED_TARGETS → OPTIONAL_DISCARD_HAND_CLASS → CONDITIONAL{PAID_ADDITIONAL_COST, targetsStored}` へ直した。
  - ②「対象とし、あなたのターンの場合」＝**4/19 だけ修正済み**・15効果が対象宣言を条件の内側に持つ ⇒ §5.3 索引 A **`O-368`** で登録（実害は相手ターンの「対象になったとき」だけ）。
- **影響枚数**＝修正1枚／登録15効果。
- **付録C の他の主張**＝強制アタック「未配線は WX12-010 のみ」は stale（配線済み）／BURST丸ごと欠落は実測0で有効／保護系 owner 誤りは測り直すコマンドが無く未再測／「アタックフェイズ開始時系は全再生成禁止」は数字が stale（407→471）でルールは §2・LESSONS と重複＝削除。
- **検証**＝`syncManualLive.ts WX24-P3-068` → golden「付録C★ WX24-P3-068-E1」（対象ありで捨ててバニッシュ／**反転**：対象不在なら捨てない）→ `smoke` CRASH 0 → `regen`（逆翻訳が兄弟と同形）→ `groupSimilar` ★ 2→1 → **`npm run gates` 全緑**。`census:cards` の `mech` 0→15（`O-368` の15枚ちょうど）。
- **実機**＝不要（`src/data/` `public/data/` だけ＝§2.2）。
- 🔑**★は「逆翻訳が割れている」＝片方にだけ修正が届いた兆候**として読む（「触らなくてよい」節に置くと、どの計器にも worklist にも出ない）。

## 2026-09-14 — §5.3 末尾「個別カードの機構待ち」の残り2件を再実測＝**どちらも実装済みだった**（第328バッチ）

**ユーザーの指示＝`WXDi-P05-006` と `WX20-Re20` を観測して、実装またはクローズを判断する。**
⇒ **2件とも defer の理由が stale**（＝節のルール「着手前にこの理由が今も有効か再判定する」どおりだった）。
⚠**この節は再実測すると 5/7 が stale**（第323で3件・今回2件）＝**「機構が無い」という見立ては当たらない。**

### ① `WXDi-P05-006`（永遠♡不滅　きゅるきゅる～ん☆）＝旧「choice① ＝着手禁止（ピースカットイン割込み基盤）」

**実装済み。** 受け皿は全部在った＝
`OPP_USING_TEAM_PIECE`（`execUtils.ts:3885`＝`team_piece_cutin_window` を読む）／
**`src/screens/battle/pieceCutin.ts`（カットイン窓そのもののモジュール）**／
`COUNTER_TEAM_PIECE_AND_EXILE`（`execStubPart3.ts`）。
golden も **「defer は解体済み」「①も続き518 で実装済み（応答窓ごと）」**と明示的に assert していた。

✅**実機6本すべて PASS**（`v57TeamConditionPieceNoUseButtonOutsideCutin` / `v58f…` / `v58b…` /
`v58ac…` / `v58d…` / `v58e…`）＝**選択肢①は相手のピースを打ち消してゲームから除外し**（`gExcluded`）、
**選択肢②はドロー＋エナチャージののち元のピースが解決する**（`gLrigTrash`）。

🔴**ただし実機2本が腐っていた（engine ではなくシナリオ側）＝その場で直した。**

| 症状 | 真因 |
|---|---|
| `v58f`・`v58b` とも「使用」ボタンが出ず 24〜30秒でタイムアウト | **host が `WXDi-P04-002` の【使用条件】【チーム】＜アンシエント・サプライズ＞を満たしていなかった**（`WD03-003` 1体）。`LRIG_TEAM_COUNT{gte 3}` は**いま実際に評価される**＝**engine が正しくなった結果、古い盤面が通らなくなった**。⇒ センター＋アシストL/R を同チーム3体に置き換えた |
| `v58b` が **3秒で PASS**（`hLrigTrash` は空） | 🔴**vacuous pass**＝`usedBtn && !pendingSpell && !pendingEffect` は**「使用」を押した同じティックで必ず真**になる（`queryState` はまだ何も反映していない）＝**ピースは1度も解決していなかった**。⇒ **ピースがルリグトラッシュへ行ったこと**を決着条件にした |
| （締め直したら）`窓ログ=true`（期待 false）で FAIL | 🔴**engine のバグに見えたが違った**＝`assist_lrig_l`/`_r` は `CORE_FIELD_KEYS` なので**注入でリセットされない**（§4.4 罠1）。直前に走る `v58f` が guest へ同チームのアシスト2体を置くため、**「窓は開かない」はずの対照でチーム3体が揃っていた**。⇒ guest のアシスト枠を `[]` で明示 |

🔑**空振りで緑だった対照を締め直すと、次に「engine のバグに見える FAIL」が出る**＝
**そこで engine を疑う前に、まず盤面の引き継ぎ（罠1）を潰す。**

### ② `WX20-Re20`（バニラ・スクランブル）＝旧「一体で要る（4軸）＝部分実装しない」

**4軸すべて実装済み。** engine を通して実測した（golden に恒久化）＝

| 旧「無い」とされた軸 | 実測 |
|---|---|
| 選択数依存コスト | `costScaling{declaredChooseCount, minCount:2, maxCount:2}`＝**§5.3 `O-251` で実装済み**・golden が「1つ＝増えない／2つ＝《無》×2」を固定済み |
| 能力なし filter | `noAbilities`＝**正しく効く**（バニラ3枚は出る／能力持ち3枚は**1体も出ない**） |
| 任意複数配置UI | `ADD_TO_FIELD{HAND_CARD, count:'ALL', upToCount:true}`＝**3体とも場に出た** |
| 同一 instance 群のターン終了時 trash | `turn_end_field_trash_targets` が**出した3体と完全一致** |

⚠🔑**最初の観測は「何も起きない」と出た**＝手札に置いたのが `fill()`（**能力持ち**）だったため
`noAbilities` が正しく弾いていただけ。**「動かない」と読む前に、filter を外した同型を1本撃って切り分ける。**
（実際 filter なしでは3体とも出たので、経路の欠落ではないと即断できた。）

### 検証

- `npm run gates` **全緑**（golden **4140 → 4142 PASS / 0 FAIL**）。**golden 2本追加**（`WX20-Re20` の c0/c1＝
  出した個体群とターン終了時トラッシュ予約の一致・`noAbilities` の対照）。
- **実機6本すべて PASS**（上記）。
- ⇒ **PLAN §5.3 末尾から2件とも削除**（参照 5件 → 3件）。
...
**WIXOSS の規則ではアタックできるのは「アップ状態のカード」であって、「1ターンに1回」という独立した制限は無い。**
engine は `performLrigAttack`（`src/screens/BattleScreen.tsx`）に
`if (my.lrig_has_attacked) return false; // ON_ATTACK_LRIGでアップされても再攻撃不可` を持っており、
**効果でアップされた後の再アタックを明示的に禁止していた。**

⇒ 「【自】：あなたのセンタールリグがアタックしたとき、（コスト）そのルリグをアップする」系の
**live 20効果が「アップは起きるが2回目が撃てない」真 no-op** だった（＝カードの存在理由そのものが働かない）。

🔴**しかも門は2箇所にあり、片方（アクション一覧＝アタックボタンの生成 `getLrigActions`）にはその行が無かった**＝
**ボタンは出るのに押しても何も起きない**（§6.4 `O-18` の「押せるのに無反応」）。
🔑**だから golden・smoke・fuzz・census が全部緑のまま通っていた**＝盤面差分にも「ルリグをアップ」としか出ない。
実際、既存の実機シナリオ `fieldDownCostPaysThreeAndWhite`（`WXDi-P04-051-E1`）は
**`lrigUpped === true` を「帰結」として PASS していた**＝**アップの先（再アタック）を誰も見ていなかった。**

### 直し方

- **新設 `src/screens/battle/lrigAttackGate.ts` の `centerLrigAttackBlock(my)` 1本に寄せた**
  （`'ALREADY_DOWN'` / `'ATTACK_LIMIT'` / `null`）。**真偽値の門は落とし、ダウン状態と付与上限（`O-236`）だけを見る。**
- **3地点セット**＝①共通実行経路 `performLrigAttack` ②人間のアタックボタン生成 ③**CPU のアタック判定**
  （CPU はダウン状態しか見ておらず、付与上限も再アタックも届いていなかった）。
- ⚠**`lrig_has_attacked` は残す**＝`LRIG_ATTACKED_THIS_TURN` 等の**条件**が読んでいる（アタック可否とは別軸）。
- ⚠**`lrig_attack_while_down_this_turn` は必ず `LRIG_ATTACK_LIMIT{whileDown}` とセットで立つ**ので、
  上限のカウント比較が止め役になる（ダウン中に無限にアタックできる状態は作れない）。

### 副産物：**「あなたのセンタールリグが」の限定が 6効果で落ちていた**

`parseAllyLrigAttackSubject`（`src/data/effectParser.ts`）が `(?:センター)?` で**センターを読み飛ばして**おり、
`triggerCondition.centerLrigOnly` が付かないまま `any_ally` になっていた＝**アシストルリグのアタックでも誘発する過剰発火**。
`WX19-031-E1` だけが `manualEffects.ts` で手当て済みで、**その1枚が直って見えていたせいで同型5枚が野放し**だった。
- 実測6効果＝`WX19-021-E2` / `WX19-023-E2`(MANUAL) / `WX19-034-E1` / `WX19-054-E1` / `WX20-022-E1` ＋ 既存 `WX19-031-E1`。
- 直し方＝parser で `centerOnly` を返して `triggerCondition.centerLrigOnly` を立てる。
  `WX19-023-E2` は MANUAL なので `npx tsx scripts/syncManualLive.ts WX19-023` で live へ届けた。
- 🔴**逆翻訳にも描くようにした**（`decompileEffects.ts`）＝**描かないと `any_ally` と1文字も違わず、
  原文照合（主軸の検査）がそこだけ効かない。** 直す前は「あなたのルリグがアタックしたとき」と出ていた。

### 検証

- `npm run gates` **全緑**（golden **4133 → 4139 PASS / 0 FAIL**）。
- **golden 3本追加**（`O-366 …`）＝門の両方向＋付与上限の据え置き＋**母集団カウンタ**（20効果を下回ったら FAIL）。
  🔑**反転確認済み**＝旧の門を戻すと 3本中2本が FAIL。
- **実機 `V-222` 2シナリオとも PASS**
  （`node scripts/verifyBattleDrive.mjs v222LrigReattackUp v222LrigReattackControl`）＝
  **アップ後に2回アタックしてライフ 7→5**／**対照（`WX19-021` を1枚抜くだけ）は2回目の「アタック」が提示されない**。
  🔑**実機でも反転確認を取った**＝旧の門を戻すと `upped=true` のまま `attacks=1` で止まる。
  ⚠**最初の drive 実装は「クリックできたこと」を1回と数えて `attacks=2` と誤報告した**＝
  このバグは**押しても盤面が動かない**形なので、**盤面の遷移を観測するまで1回と数えてはいけない。**

### 教訓

- 🔑**母集団は原文の言い回しではなく JSON の形（timing × action）で数える。**
  `O-366` の登録票は regex「このルリグがアタックしたとき…アップする」で **3効果**と書いていたが、
  実際は **20効果**（「あなたのセンタールリグが〜」「シグニ1体をトラッシュして〜そのルリグをアップする」型を取り落としていた）。
- 🔑**登録票の「最短の直し方」も実測でやり直す。** 登録票は
  「`src/screens/` を触らず `STUB{LRIG_ATTACK_LIMIT, limit:2}` を付与すれば閉じる」と書いていたが、
  🔴**回数は上限が在るときしか数えないので `limit:2` では3回アタックできてしまい**、しかも**3効果にしか効かない**。
- 🔑**「帰結まで見た」と書いた実機シナリオでも、その帰結の意味（何のためにアップするのか）まで追う。**

### follow-up

- **§5.3 索引G に `O-367` を登録**＝`WX19-023-E2`「そのアタックの間、そのルリグは【ダブルクラッシュ】を得る」が
  `duration:'UNTIL_END_OF_TURN'`＝**2回目以降のアタックにも乗る過剰付与**。
  ⚠**本バッチで再アタックが実際にできるようになって初めて到達可能になった**（受け皿が無い＝新機構）。


## 2026-09-14 — 🏁`WD18-009-E2` を明示 defer から**実装**へ昇格（第324バッチ）＝**defer 理由が誤りで、真因は engine の非対称だった**

**ユーザーの問い「未完成のものを作ったのか？動かないのか？」への回答＝動かなかった。**
第319バッチの時点で `WD18-009-E2` は **`STUB{DEFERRED_…}`＝実行時にログを出すだけの no-op** で、
**このカードの【自】能力は1度も働いていなかった**（逆翻訳も `【未実装】`）。
⚠ただし**未完成のものを新しく作ったのではない**＝**旧実装のほうが壊れていた**
（`ACCE_TO_ENERGY`＝**場の【アクセ】を全部エナへ送る**まったく別の効果）。第319は
**「誤動作」→「無動作＋可視化」への退避**で、**直っていないことを見えるようにした**状態だった。

### 🔴 簿記のバグ（同日・追記）＝クローズ注記を §5.3 に残して `mech` を膨らませていた

**ユーザーの指摘「まだ §5.3 に `WD18-009-E2` があるが治っていないのか？」への回答＝カードは直っていた。残っていたのは私の簿記ミス。**
`cardProgressCensus.mjs` は **§5.3 の本文からカード番号を拾って `mech`（機構待ち）に数える**ので、
**「実装した」と書いたクローズ注記の引用カードまで「まだ直っていない」に化ける**（`CLAUDE.md` が明記している事故）。
🔑**2バッチ連続で踏んだ**＝第323で `WXDi-P15-003`、第324で `WD18-009-E2` / `WX15-003-E1`、
さらに `O-366` の「除外した別物」1枚。⇒ **`mech` が 9（実体5）** に膨らんでいた。

- **直し方**＝クローズ注記を節ごと削除し、残す文からカード番号を外した（**経緯は BUGFIXES.md と PLAN_PROGRESS.md が正**）。
  実測 **`mech` 9 → 5**。
- 🆕**トリップワイヤを新設**＝**「個別カードの機構待ち」節に『カード番号 ＋ クローズ語』の行があったら赤**。
- 🔴**そのトリップワイヤ自身が最初は偽陽性だった**＝`new RegExp('…')` と書いたため
  **文字列リテラルの `` がバックスペースに化け、何にも当たらないのに緑**になっていた
  （`CLAUDE.md` の `censusDeadState` の罠と同型＝**3回目**）。
  ⇒ **正規表現リテラル `/…/` に書き換えた**（バックスラッシュが1段しか無いので化けようがない）。
  **ダミー行を入れて赤くなることを確認してから採用した。**

### 🔴 再実測の結論＝defer 理由は誤りだった（「受け皿が無い」の**14回目**の外れ）

旧理由＝「原文は**対象を取ったあと**に『このシグニがアクセされていた場合』を判定する形で、既存の
`banishedHadAcce`（発火条件用）とは段階が違う＝除去直前の状態を解決時まで運ぶ機構が要る」。

**実測**＝`triggerCondition.banishedHadAcce` は **`prevOwnerState.field.signi_acce[banishedZone]`**＝
**除去直前の盤面**を読む機構として既にあり、**先例カードも在る**（`WX15-003-E1`）。

### 🔴 真因＝`triggerCollect.ts` の ON_BANISH 3ブロックのうち **self スコープの1本にだけ判定が無かった**

| triggerCondition | ①バニッシュされた本人（self） | ②自分の場の watcher | ③相手の場の watcher |
|---|---|---|---|
| `banishedHadCharm` | ✅ | ✅ | ✅ |
| `banishedFromGateZone` | ✅ | ✅ | ✅ |
| 🔴`banishedHadAcce` | **無し** | ✅ | ✅ |
| 🔴`banishedFromCenterZone` | **無し** | ✅ | ✅ |

⇒ **self スコープの効果に書いても「無言で素通り」する**（＝条件を無視して常に発火＝過剰）。
🔑**同じブロックの直上コメントが、まったく同じ形の事故を警告していた**
（「旧実装はこの2行が `selfScope !== 'self'` のブロックの中にあったため、**自分自身の ON_BANISH では
条件が一度も評価されなかった**＝JSON に条件を書いても無言で素通りする」）。
**その警告が指していた穴が、別のキーで2本残っていた。**

### 直し方

1. **engine**＝self スコープのブロックへ `banishedHadAcce` と `banishedFromCenterZone` の判定を追加。
   ⚠**追加時点の母集団は 0**（self スコープ ON_BANISH でこの2キーを持つ効果は実測0件）＝**既存効果の挙動は1件も変わらない**。
2. **カード**＝`triggerCondition:{banishedHadAcce:true}` ＋
   `ENERGY_CHARGE{TRASH_CARD owner:'self', count:1, filter:{cardType:'シグニ', hasIcon:'アクセ'}}`。
   **新しい型・新しい engine 経路は0**（`hasIcon:'アクセ'` は同じカードの BURST が既に使っている綴り）。
3. ⚠**近似1件（明記する）**＝原文は「対象を取った**あと**に条件判定」だが、ここでは**条件を満たさないと発火しない**
   （＝対象を取らない）。🔑**この差は観測できない**＝対象は「**あなたの**トラッシュ」のカードで、
   live の「対象になったとき」20効果はいずれも「**対戦相手の**能力か効果で**場の**シグニが対象になったとき」型。

### 検証

- `npm run gates` **全緑**・golden **4130 → 4132 PASS / 0 FAIL**・lint warning **256 据置**。
- live 差分は effectId 単位で **`WD18-009-E2` の1件ちょうど**。held / `_partial_fresh` / `_idset_fresh` すべて0件。
- 逆翻訳＝`【自】アクセされているこのシグニがバニッシュされたとき：あなたの《アクセアイコン》を持つシグニ(トラッシュ)1枚を対象とし、それらをエナゾーンに置く`
  ＝**engine の挙動どおり**（近似を原文に寄せて隠していない）。
- **反転確認**＝engine の追加2行を外すと ①「アクセされていないのに発火する」②トリップワイヤ の**2本とも FAIL**、復元で PASS。
- 🆕**トリップワイヤを新設**＝**ON_BANISH の除去直前フラグ4種は3ブロックすべてで判定する**
  （`triggerCondition?.<key>` の出現が3箇所以上）。**これがあれば今回の非対称は作り込んだ時点で赤くなっていた。**
- 🔑**既存 golden 1本を書き換えた**＝`O-360 WD18-009-E2` は「`DEFERRED_` のままであること」を固定する
  **意図的なトリップワイヤ**だった（`続き388 群D` と同じ作法）＝**実装したら赤くなるのが設計どおり**。
  旧実装（場のアクセ全部をエナへ送る）へ戻っていないことも併せて assert する形へ更新した。
- **実機不要と判定**（PLAN §2.2）＝触ったのは `src/engine` `src/data` `public/data` `scripts` だけで
  **`src/screens/` は非変更**。発火判定は `collectBanishTriggers` という**純関数**で、golden から直接呼んで両方向＋
  fail-closed（`prevOwnerState` 不明時は非発火）を固定した。

## 2026-09-14 — §5.3 末尾「根拠つき defer」を全7項目 再実測（第323バッチ）＝**2件が stale・実バグ1件を修正・1件を `O-366` で登録**

**この節は PLAN 自身が「過去2回ともここに埋もれた記述が実測すると30効果／10効果の実バグだった」と警告している場所。**
§5.3 の索引が空になったので、節のルール「**着手前にこの理由が今も有効か再判定する**」に従って7項目を全部測り直した。

| # | 項目 | 再実測の結果 |
|---|---|---|
| 1 | `WD18-009-E2`（アクセ→エナ） | ✅**有効**＝`DEFERRED_…` id が live に在る・母集団1効果のまま |
| 2 | `WXDi-P05-006` choice①（ピースカットイン） | ✅**有効**＝再測も **1効果/1カード** |
| 3 | `WX20-Re20`（一体で要る） | ✅**有効**＝再測も **1効果/1カード** |
| 4 | 🔴**F. 保留①＝47枚の【使用条件】【チーム】** | **stale**＝下記（実バグ1枚を修正） |
| 5 | 🔴**F. 保留②＝`WXDi-P00-026`（ルリグ再アタックがブロッカー）** | **stale**＝下記（`O-366` で登録） |
| 6 | CPU 盤面評価 v2 ／ `doPhaseAdvance` pure 抽出 | ✅**有効**＝どちらも「カードの正しさの穴ではない」というスコープ判断 |
| 7 | BEHAVIOR_AUDIT キュー | ✅**有効**＝再測も **高シグナル23件**（記録どおり・枯渇済み） |

### 🔴 stale ①＝「47枚の【使用条件】【チーム】は正規デッキ常時成立で機能等価＝保留妥当」は誤りだった

実測＝**53効果/53カード**（記録の47枚も古い）。`--json LRIG_TEAM_COUNT` 判定で **OK 10 / MISS 43** だが、
🔑**MISS 43 のうち 42件は別の正準形で既に配線済み**だった
（`FIELD_LRIG_COLOR_COUNT` / `LRIG_ANY_TEAM_COUNT` / `FIELD_LRIGS_HAVE_COLORS`）。
⇒ **「機能等価だから保留」ではなく、機構はとっくに実装されていた。本当の穴は 1枚だけ。**

**実バグ＝`WXDi-P15-003`《ひらけ！ゲート！》（ピース）**
- 原文＝「**【使用条件】【ドリームチーム】合計３種類以上の色を持つ**…あなたのシグニゾーン１つに【ゲート】１つを置く。…」
- 🔴**使用条件が live にどこにも無く、3色そろっていなくても使えた**（過剰）。
- 🔴**真因は「条件を書き忘れた」ではなく効果の形**＝`canUseArtsCondition`（`battleUtils.ts:54`）は
  **最初の `ACTIVATED` 効果の `condition` しか読まない**。このカードは `AUTO/['ON_PLAY']` ＋ `CONTINUOUS` の2本で
  **`ACTIVATED` が1つも無かった**ので `effect` が `undefined` → `return true` ＝**無条件成立**。
- 🔑**ピースの正準形は `ACTIVATED/['MAIN']`**＝実測でピース全118効果がこの形。
  **`AUTO/['ON_PLAY']` はこの1枚だけの外れ値**だった（`CONTINUOUS/null` も同じくこの1枚だけ）。
- **直し方**＝E1 を `ACTIVATED/['MAIN']` ＋ CSV の `Cost` 列（《無》×０）＋
  `condition: FIELD_LRIG_COLOR_COUNT{minLrigs:3, gte 3}`（同じ使用条件の `WX25-P1-048-E1` / `WXDi-P09-003-E1` と同じ綴り）。
  ⚠**発火経路は変わらない**＝`queueCardEffects` はピースに `['AUTO','ACTIVATED']` × `['ON_PLAY','MAIN','ATTACK','SPELL_CUTIN']` を積む。
- 🔴**旧コメントの defer 理由2つがどちらも stale だった**＝①「ピースは ON_PLAY でしか発火しない」（いまは両方積む）
  ②「使用条件は近似省略」（受け皿は兄弟2枚が既に使用）。**両方その場で書き直した。**

### 🔴 stale ②＝`WXDi-P00-026` の「ルリグ再アタック未実装がブロッカー／core 改変が過大リスク」

**JSON は3件とも完成していた**（`GRANT_EFFECT` → sub `AUTO/ON_ATTACK_LRIG` → `UP{LRIG}`・逆翻訳も原文一致）。
ブロッカーの実体は **`BattleScreen.tsx:11522` のアタックゲート1行**で、コメント自身が
`// このターン既に攻撃済み（ON_ATTACK_LRIGでアップされても再攻撃不可）` と書いている＝
**アップは起きるが2回目のアタックができない真 no-op**。母集団は**実測3効果**。
🔑**受け皿 `lrig_attack_limit_this_turn`（`O-236`）も実在**し、`src/screens/` を触らず payload で閉じられる見込み。
⇒ **§5.3 索引G へ `O-366` として登録**（実機が要るので独立バッチ）。

### 🔑 既存の golden が「壊れた方」を固定していた（§5-17）

`続き388 群C WXDi-P15-003: ACTIVATED効果が無く、condition追加は使用ゲートにならない` は
**`ACTIVATED は0`／`condition は付けない`／`ゲートは素通りする` を assert していた**。
🔑**隣の `群D` に「旧アサートは defer を固定していた＝実装したら必ず赤くなる意図的なトリップワイヤ」と明記**されており、
**この赤は設計どおりの信号**だったので、群D と同じ作法で実装後の状態へ書き換えた。
⚠**旧結論の誤り**＝「condition を付けない」ではなく「**ACTIVATED にして condition を効かせる**」が正解だった。

### 検証

- `npm run gates` **全緑**・golden **4127 → 4130 PASS / 0 FAIL**・lint warning **256 据置**。
- live 差分は effectId 単位で **`WXDi-P15-003-E1` の1件ちょうど**。held / `_partial_fresh` / `_idset_fresh` すべて0件。
- 逆翻訳＝`【起】（メイン起動）：〈《無×0》〉あなたの場のルリグが3体以上いて、持つ色が3種類以上の場合、あなたのシグニゾーン1つに【ゲート】1つを置く`＝**原文一致**。
- **反転確認**＝E1 を `AUTO/ON_PLAY` へ戻すと ①使用条件ゲートの golden ②ピース不変条件のトリップワイヤ の**2本とも FAIL**、復元で PASS。
- **新設したトリップワイヤ2本**＝①**ピースは必ず `ACTIVATED` 効果を持つ**（これがあれば登録時点で赤くなっていた）
  ②**使用条件が最初の `ACTIVATED` 以外にしか無いアーツ/ピースは0件**（`canUseArtsCondition` の構造的死角＝
  書いても読まれない＝無言の過剰実行。現在0なので0を維持する）。
- **ラチェット3本を払い戻した**（いずれも**同じ1件の分類是正**＝退化ではない）＝
  `段階2 mandatory集合` 1466→1465 ／ `段階2 condition なし` 1404→1403 ／ `census:numberdrift` **57→56**（消化＝
  使用条件を載せたので原文の「３種類」が逆翻訳に出るようになった）。
- **実機不要と判定**（PLAN §2.2）＝触ったのは `src/data` `public/data` `scripts` だけで **`src/screens/` は非変更**。
  使用条件の評価は `canUseArtsCondition`／`evalUseCondition` という**既存の純関数**で、golden から直接呼んで両方向を固定した。
- ⚠**踏んだ罠（記録）**＝バッチ途中で `git stash` を使ったところ、**live JSON と `docs/decompile_*` が食い違った状態**になり
  `census:numberdrift` が **52** という偽の値を返した（真値は 56）。**生成物を持つリポジトリで stash を挟んだら
  `build:effects` → `regen` をやり直してから計器を読む。**

## 2026-09-14 — 🏁§5.3 索引G `O-365` クローズ＝**登録票の「同時選択の機構が要る」は誤りだった**（第322バッチ・🏁索引G 残0）

### 🔴 まず訂正＝私が書いた登録票の見立てが外れていた

`O-365` は前バッチ（`O-346` の triage）で **私自身が**「真に要るのは**両者がそれぞれ自分の札を選び、まとめて適用する**機構」
と登録した。**実測すると、その機構は要らなかった。**

- 🔑**各プレイヤーは「自分のトラッシュから自分の領域へ」動かすだけで共有プールが無い**＝
  **一方の選択がもう一方の候補に影響しない**⇒ 逐次でも同時でも観測結果が同じ。
- 🔑しかも原文の括弧書きは「（**あなたから**カードを選択し両者が同時に移動させる）」＝**使用者が先**と明記しており、
  engine の「自分→相手の順」で解決する既存の近似と**一致している**。
- ⇒ **残っていた本当の穴は `opponentSelects`（誰が選ぶか）1本だけ**だった。
  🔑`manualEffects.ts` の該当コメントには `O-279` が `owner`（誰のカードか）を直した経緯が残っていたが、
  **`owner` を直しても「誰が選ぶか」は直らない**（この2つは独立）。そこが見落とされていた。

**これで「受け皿が無い／新機構が要る」という見立ては連続13項目外れた**（[LESSONS.md](./LESSONS.md) §4.1）。

### 実害（`WX07-017-E1`《台風一過》・**母集団 実測1効果 / 3ノード**）

原文＝「各プレイヤーは…すべてトラッシュに置く。その後、**各プレイヤーは自分のトラッシュから**シグニを３枚まで
対象とし、それらを場に出す。その後、**各プレイヤーは自分のトラッシュから**シグニを３枚まで対象とし、それらを手札に加える。
その後、**対戦相手は自分のトラッシュから**カードを３枚まで対象とし、それらをエナゾーンに置く。」

⇒ **相手側3ノードすべてで「使用者が相手のリカバリー札を選べた」**＝盤面全消し後に
**相手には一番弱いシグニだけを戻させる**ことができた（過剰）。

| ノード | 受け皿 | 直し方 |
|---|---|---|
| `ADD_TO_FIELD` ← 相手のトラッシュ | `oppPicksAF` が **`HAND_CARD` 限定**だった（`O-309` 当時それしか居なかっただけで限定に意味は無かった） | `TRASH_CARD` まで広げた。**旗のゲートは残す** |
| `TRANSFER_TO_HAND` ← 相手のトラッシュ | 🔑**前バッチ `O-346` で軸を新設済み**＝配線は不要で旗を立てるだけ | `opponentSelects:true` |
| `ENERGY_CHARGE` ← 相手のトラッシュ | 無し＝`EnergyChargeAction.opponentSelects` を新設し `execEnergyCharge` の第8引数へ配線 | 同上 |

🔴**`oppPicksAF` を「`owner==='opponent'` なら」に広げてはいけない**＝**同型の対照が1枚ある**。
`WXEX2-50-E3`「**対戦相手のトラッシュから**シグニ１枚を対象とし、それを対戦相手の場に出す」は
主語が「対戦相手は」ではない＝**使用者が選ぶのが正しい**。**旗のゲートがこの1枚を守っている**（golden で固定）。

### 🔴 逆翻訳が選択者を描いていなかった（`O-346` の積み残しも回収）

`TRASH`／`BOUNCE`／`SEND_TO_ENERGY`／`TRANSFER_TO_DECK` は「（相手が選ぶ）」を描くのに、
**`ADD_TO_FIELD`／`TRANSFER_TO_HAND`／`ENERGY_CHARGE` は描いていなかった**＝
**engine が正しくても逆翻訳だけが嘘をつく**（＝原文照合という主軸の検査がそこだけ効かない。`O-362` の
`BLOCK_CARD_USE` で実際に踏んだ形）。3箇所とも描画を足した。
🔑**副産物＝前バッチの `WXK11-006-E1` も選択者が逆翻訳から消えていた**のがこれで直った
（`O-346` で旗は立てたが描画を足していなかった＝**軸を足したら逆翻訳も同時に足す**）。

### 検証

- `npm run gates` **全緑**・golden **4123 → 4127 PASS / 0 FAIL**・lint warning **256 据置**。
- live 差分は effectId 単位で **`WX07-017-E1` の1件ちょうど**。held / `_partial_fresh` / `_idset_fresh` すべて **0件**。
- 逆翻訳の差分も **2行ちょうど**（`WX07-017-E1` ＋ 積み残しの `WXK11-006-E1`）＝**描画追加が他へ波及していない**ことを機械確認。
- **反転確認**＝①`execEnergyCharge` の第8引数を `false` に戻すと FAIL ②`oppPicksAF` の `TRASH_CARD` 拡張を戻すと FAIL。
  どちらも復元で PASS。逆翻訳側も「自分側に（相手が選ぶ）が出ていないこと」を対照で assert。
- **`O-327` トリップワイヤ**に新しい2形（`ADD_TO_FIELD|TRASH_CARD|N` / `ENERGY_CHARGE|TRASH_CARD|N`）を、
  **配線→対照→許容リスト追記**の順で登録した（先に許容リストへ足して黙らせない）。
- **実機不要と判定**（PLAN §2.2）＝触ったのは `src/types` `src/engine` `src/data` `public/data` `scripts` だけで
  **`src/screens/` は非変更**。`opponentSelects` → `opponentResponds` → 相手側 UI の経路は**既存**
  （`O-309` 第282 / `O-346` 第321 と同じ拡張の形）で、両方向の対照 golden で固定した。
- ⚠**観測メモ（未対応・原文どおりに実装した）**＝CSV 原文では④だけ主語が「**対戦相手は**」で、
  ②③の「各プレイヤーは」と非対称になっている（＝自分側のエナ回収が無い）。
  **CSV が原文の正**（`CLAUDE.md` §3-3″）なのでそのまま実装した。**engine・parser の欠陥ではない。**
...
- **母集団の再較正**：live は14効果/14カード（既存 `declareNamePool` 2件、未指定12件）。原文再照合により、依頼表の `WXK09-039-E1` は相手手札を当てる群で、実際の自デッキ群6件目は `WX22-021-E2` と判明した。相手側6件には触れていない。
- **修正**：`declareNamePool:'self_deck'` と、直交する `declareNameZones` / `declareNameFilter` を追加。既存 `matchesFilter` の `cardType` / `nonColorless` で絞り、重複名を除去する。候補0なら古い `declared_card_name` を消し、架空のフォールバック名は宣言しない。
- **反映**：AUTO 5効果（`WX11-037-E1` / `WX13-054-E1` / `WX18-028-E1` / `WXDi-P14-039-E1` / `WXK09-001-E3`）は原文文型を読む parser 最終 pass、MANUAL 1効果（`WX22-021-E2`）は `manualEffects.ts` と `syncManualLive` で live へ届けた。
- **逆翻訳**：候補領域・カード型・無色除外を payload から描画。`census:payloadkeys` は0種/0ノード、`census:stublabel` A群は10→7へ改善し基準を7へ下げた。
- **反転確認**：修正前の O-353 3本は PASS 1 / FAIL 2（デッキ限定名の2経路が失敗）、修正後は PASS 3 / FAIL 0。既定枝の手札4種制限と既存 `opp_public_signi` の O-306 golden は維持。
- **検証**：`npm run regen` → `npm run gates` 全緑（golden 4074/4074、smoke 10754 OK・全0、fuzz 200ゲーム・全0、高シグナル1/BASELINE 1、lint 0 errors/既存256 warnings）。`src/screens/` は無変更、実機不要。
- 🔍**Claude 側の検証（ベースライン `48a416464` と比較）**＝live を全走査して**刻印が意図した6効果だけ**であることを確認した
  （相手群6件は `(なし)` のまま／既存 `opp_public_signi` 2件も不変）。parser 規則は**カード番号を焼き込まず**
  効果単位の原文で絞り、`declareNamePool === undefined` のときだけ刻むので既存指定を上書きしない。
- 🔴**指示書の誤りを Codex が訂正した**＝自デッキ群の6件目は `WXK09-039-E1` ではなく **`WX22-021-E2`**
  （`WXK09-039-E1` は「対戦相手の手札を見て宣言したカードを捨てさせる」＝Part B 側）。
  **原因**＝Claude 側が逆翻訳シートの一括ダンプを読むとき、**出力順が引数順ではなくシート順**だったため
  原文ブロックとカード番号の対応を1件取り違えた。⇒ **一括ダンプから原文を引くときはカード番号を必ず同じ行に印字する。**
- 🔑**`WX19-026-E1` ほか Part B の6効果は `census:stublabel` A群に「⚠手札」として残るのが正しい**＝
  engine がまだ手札プールなので、ラベルを原文へ寄せると**欠落が逆翻訳から見えなくなる**（`O-354` の規約）。
...
③**計器を作る過程で `census:enginetext` の死角を見つけて塞いだ**。
⚠**codex は2アカウントとも利用上限**（`.codex-work` 17:03／既定 17:35 復帰）だったので、
ユーザー指定のフォールバック順どおり **Claude が実装して最後まで回した**。

### ① 計器の新設＝`npm run census:srcecho`（`scripts/censusSrcEcho.mjs`・`gates` 同梱）

**逆翻訳（`scripts/decompileEffects.ts`）が原文を regex で切り出して戻り値に乗せている箇所**を全数で出す。
🔑**原文をコピーして返す実装では、payload が何であろうと engine が何をしようと逆翻訳は必ず原文と一致する**＝
**そのカードでだけ「原文 × 逆翻訳」の照合が構造的に無効**になる。
**ラチェットは A群の id 数**（🔑**行数ではなく id 数**＝払い戻しの単位が「1 id を payload 化する」だから。
行数は同じ意味のまま増減する）。**注入テストで実際に止まることを確認済み。**

### ② 🔴登録票（自分で書いたもの）が過大だったので訂正した

| | 登録時の見立て | 計器での実測 |
|---|---|---|
| A群 | 88 id / 452ノード / 420カード | **56 id / 129行 / 308ノード / 296カード** |

原因＝`a.id === 'X'` の**14行窓**で数えたので**隣の分岐の行まで数えていた**。
🔑**「粗い窓で数えた見立て」は登録票に書かない**（この項目は自分が前セッションで書いた登録票だった）。

**そして着手前にいちばん効いたのは、A群を「重さ」で割った実測**：

| 群 | 実測 | 意味 |
|---|---|---|
| **①payload あり** | **11 id / 104ノード** | **逆翻訳を直すだけで閉じる**（今回のスコープ） |
| **②payload なし** | **36 id / 204ノード** | **parser が構造化していない**＝逆翻訳だけ直しても描くものが無い＝**三点セット** |
| ③live 0 | 9 id | parser に生成元が無い安全網＝**触らない・消さない** |

⚠**登録票の「payload から描く移行」は A群の3分の2には適用できない**（`ARTS_COST_REDUCTION_BY_EFFECT` 59ノードや
`DECLARE_NUMBER` 27ノードは **STUB ノードが payload を1つも持っていない**）。

### ③ 🔴計器を作る過程で `census:enginetext` の死角を塞いだ（A群 0 → 1 は「較正」）

`OPTIONAL_TRASH_ENERGY_CLASS`（live **33ノード / 33カード**）の engine ハンドラは
**`sourceAbilityText(cur)` でアビリティ原文を読み、エナから払うクラスと枚数を regex で決めていた**
（`effectExecutor.ts:6559`）。
🔴ところが `censusEngineText.ts:103` の検出は **`sourceAbilityText(ctx)` という「引数名つきリテラル」**だったので、
**このハンドラは初版から一度も数えられていなかった**（A群 0＝「engine は原文を読まない」に見えていた）。
🔑**第56バッチで「funnel も数える」と較正したのに、引数名を焼き込んだせいで穴が残った**＝
**funnel は関数名で数える。呼び出し側の変数名に依存しない。**
⇒ 検出を `/\bsourceAbilityText\s*\(/` へ直し（funnel の**宣言**は除外）、`BASELINE_SELF_TEXT` を **0 → 1** へ。
**engine のコードは1行も増えていない＝退化ではなく可視化。** golden 2件も同時に更新した
（旧 assert は引数名リテラルを守る門だったので、**変数名に依存しないこと**を守る門へ作り替えた）。

### ④ 払い戻し①＝payload を持つ10 id を payload から描いた（A群 56 → 46 id）

`COPY_LRIG_NAME_ABILITY` / `DESIGNATE_SIGNI_ZONE` / `DEPLOY_RESTRICT` / `GAIN_SIGNI_BARRIER` /
`PICK_FROM_TRASHED_CARDS` / `COLLAB` / `DECLARE_CLASS` / `SET_OPP_SIGNI_AS_TRAP` /
`CAST_FROM_OPP_TRASH` / `DOWN_UP_SIGNI_AND_CHOOSE`。

**原文エコーを外して戻った語（＝regex が拾い損ねて静かに捨てていた文）**
- `WXEX1-46-E3`＝「**それがチェックゾーンから別の領域に移動される場合、代わりにゲームから除外される**」
  （`exileAfterUse` が描かれていなかった）
- `WD08-008-E1`＝`[STUB:クラス1つを宣言する]` が `declareFromLastProcessed` から描けるようになった
- `WXK05-028-E3` / `WXK07-106-E1` / `WXEX2-49-E2`＝対象の名詞（「シグニ1枚を対象とし」）が戻った

**期間は「原文の期間句」ではなく engine の実装から描いた**（`DEPLOY_RESTRICT`）＝
`power_gte` は `signi_deploy_bans{turnsRemaining:2}`＝**このターンと次のターン**（固定）／
`count` は【常】が恒久・それ以外は `signi_deploy_count_limit` フラグ＝**このターン**。
🔑**原文の期間句を貼ると engine の寿命と食い違っても気付けない。**

### ⑤ 🔴置き換えで「engine の欠落」が見えた2件（原文へ寄せず、そのまま描いた）

1. **`WXDi-P15-039-E1`**＝原文「**次の対戦相手のターンの間**、対戦相手はパワー12000以上のシグニを新たに場に出せない」
   に対し engine は `turnsRemaining:2`＝「**このターンと次のターン**」。⇒ 逆翻訳は engine どおりに描いた。
2. **`WXDi-P11-009-E3`**＝原文「**シグニのない**対戦相手のシグニゾーン１つを指定する」に対し、
   engine は3ゾーンすべてを `available:true` で提示する（`execStubPart2.ts:3533`）＝**空きゾーン限定が無い**。
   payload にも軸が無いので逆翻訳からも消えた。⇒ **`O-358` として登録**。

### 🔑 検証中に自分で踏んだ罠（記録）

**python のヒアドキュメントで TS を書き換えると `\b` が「バックスペース文字」に化ける。**
`goldenTest.ts` の regex が `/<バックスペース>sourceAbilityText/`（`\\b` が1段剥がれて U+0008）になり、**黙って何にも当たらず**
「engine に funnel の呼び出しが 0 箇所」という**嘘の FAIL** を出した。
⇒ **編集後は制御文字（`\a/\b/\v/\f`）を全ファイル走査で確かめる。**
（`CLAUDE.md` が `census:deadstate` の罠として同じことを書いていた＝**罠の記録は計器ごとではなく横断で読む**。）

### 検証

- `npm run gates` **全緑**（typecheck / **golden 4066 PASS** / smoke 10754 OK / fuzz 0 / census 各種 / lint 0 error）
- **ラチェット**＝`census:srcecho` **BASELINE_ECHO_IDS 56 → 46**／`census:payloadkeys` **21 → 19**
  （`declareOptions` / `declareFromLastProcessed` が描画された副産物）／
  `census:enginetext` **BASELINE_SELF_TEXT 0 → 1（較正）**
- **実機は不要と判定**（§2.2）＝触ったのは `scripts/` だけで、`src/` は1行も触っていない。

## 2026-09-13 — §5.3 `O-348` バッチC＝`[STUB]` 側13キー／原文エコー3 id を payload へ（第305バッチ）

**13キー / 66ノードを判定＝12キーは描画追加・1キーだけ `IGNORED`。** `census:payloadkeys` **34 → 21種 / 87 → 21ノード**。

🔴🔑**この回の主産物は「`resonaSummon` が計器に出ていた理由」を追ったら別系統が落ちたこと**＝
**`decompileEffects.ts` が原文を regex で切り出してそのまま返していた**（→ **§5.3 `O-356`** として登録）。
今回そのうち3 id を payload から描く形へ移した＝**`SUMMON_RESONA_FROM_LRIG_DECK` /
`NEGATE_NTH_ATTACK` / `GUARD_LOSS_UNLESS_LRIG`**。

### ① 🔴実バグ＝任意発動のゲートが落ちていた（`WDK13-017-E1`）

**原文**＝「【自】：このシグニがアタックしたとき、あなたのデッキを上からシグニが２枚めくれるまで
**公開してもよい**。その後、それらのレベルの合計が６以上の場合、…**公開したカードをトラッシュに置く**。」

live は `SEQUENCE[STUB{DECK_REVEAL_UNTIL}, CONDITIONAL{…}]` で、
**`execStubPart1.ts:3208` が即座に公開・移動を実行する**＝**プレイヤーが辞退できず強制実行**だった。
🔑**受け皿は既に在った**（`OPTIONAL_ACTIVATE`＝`effectExecutor.ts:7017`）＝先頭に1ノード足すだけで閉じた。
（「受け皿が無い」という見立ての連敗は**連続18項目**になった。）
- golden＝`O-348 C WDK13-017-E1: デッキ公開は OPTIONAL_ACTIVATE で辞退できる`
- **反転確認 ✅**＝修正前の live で `expected=OPTIONAL_ACTIVATE got=DECK_REVEAL_UNTIL` の FAIL。

### ② 🔴原文エコーを外したら、regex が落としていた語が2件戻った

**これがこの作業の値打ち**＝原文を regex で切り出す実装は、**その regex が拾い損ねた文を静かに捨てる**。

| カード | 原文にあって逆翻訳から消えていた語 |
|---|---|
| `WX16-Re18-E1` | 「**この方法で場に出たレゾナの【出】能力は発動しない。**」（regex が `[^。：]*?` で1文しか取らなかった） |
| `WXDi-CP01-024-E1` | 「同じパワーの**＜バーチャル＞の**シグニ」と「**それの【出】能力は発動しない。**」 |

### ③ 逆翻訳が完全な空白だった1件（`WX25-P3-050-E1`）

**旧**＝「あなたの場にいる**指定色**のルリグ１体につき、**以下を繰り返す**」が**5回並ぶだけ**
＝**色も、繰り返す中身も、何ひとつ描かれていなかった**（`PER_OWN_LRIG_COLOR_SCALE`）。
**新**＝白＝【ルリグバリア】1つ／赤＝バニッシュ／青＝3枚ドロー／緑＝エナチャージ3／黒＝相手デッキ10枚トラッシュ
（`scaleColor` を描き、`scaleAction` は `actionJa` で再帰描画）。

### ④ `IGNORED` は1キーだけ＝`placesToField`（理由をコードで確かめた）

**parser 内部の配置アンカー**（後続の「その【出】能力は発動しない」を同ノードの `suppressOnPlay` へ畳むための印）。
🔑**`grep -rn "placesToField" src/` で確かめた**＝出るのは `src/data/parsers/` と型宣言と**コメント1行だけ**で、
**実行時に読む地点は無い**（`BattleScreen.tsx:3331` が見ているのは `action.suppressOnPlay === true` であって
`placesToField` ではない）。「場に出す」も【出】抑止も**別の payload が描く**＝固有の対応語が無い。

### ⑤ 🔴engine の欠落が「可視化」された2件（原文に寄せずそのまま描いた）

1. **`WXK02-035-E2`**＝原文「それがシグニの場合、それを場に出してもよい。**場に出さない場合、それをトラッシュに置く。**」
   に対し、engine は `field.check_rest` に置いたまま**ターン終了時まで残す**（即時トラッシュにしない）。
   ⇒ 逆翻訳を**engine の挙動どおり**に「（バースト確認をせずチェックゾーンに留まり、ターン終了時にトラッシュに置く）」
   と描いた。**新機構待ち＝§5.3 `O-357` に登録**（任意登場を辞退したときのアクション契約が無い）。
   ⚠ **codex の報告は「露出した不一致は無かった」と書いていたが、これが1件目**（報告の §2／§7 には書かれていた）。
2. **`WXDi-P11-006-E1`** は同じ注記が**原文どおり**（「（チェックゾーンにあるカードはターン終了時にトラッシュに置かれる）」）。
   ⇒ **同じ payload でも、原文と合う側と合わない側がある**＝注記は engine を描いて正しい。

### ⑥ Claude が検証中に見つけた小さな乖離（worklist にはしない）

`WX25-P3-050-E1` の赤＝原文「対戦相手のシグニを**２体まで**対象とし、**それらのパワーの合計が12000以下の場合**、
それらをバニッシュする」に対し、live は `BANISH.target.totalPowerMax:12000`＝engine は
**合計が上限を超えないよう選択を制限し、超過分を切り捨てる**（`effectExecutor.ts:11698`）。
⇒ 原文は「選んでから判定（超過なら何も起きない）」、engine は「超過しないよう選ばせる」。
🔑**実測した**＝`totalPowerMax` は **7ノード / 7カード**で、**6カードは原文が
「パワーの合計がN以下に**なるように**」＝engine と同じ形**。ズレているのは `WX25-P3-050` の1件だけで、
**合理的な選択をするかぎり結果は同じ**（わざと超過させて何も起きないようにする選択だけが違う）。
⇒ **登録しない**（1カード・実害なし）。次に読む人が混乱しないようここに残す。

### 検証

- `npm run gates` **全緑**（typecheck / **golden 4066 PASS**（+1）/ smoke 10754 OK / fuzz 0 / census 各種 / lint 0 error）
- **ラチェット**＝`census:payloadkeys` `BASELINE 34 → 21`／`census:numberdrift` `70 → 68`
- **逆翻訳シートの差分は30効果のみ**（全部この13キーの母集団内）。**該当カード以外の行は動いていない。**
- **実機は不要と判定**（§2.2）＝触ったのは `scripts/`・`src/data/`・`public/data/` だけで、
  `src/engine/` も `src/screens/` も触っていない。

### ⚠ この回の運用メモ

**`.codex-work` が利用上限に達した**（298kトークン使用・復帰 17:03）ので、**既定の `~/.codex` へ切り替えて投げ直した**。
中断時に残っていた赤い golden は**破棄せず原文で裏を取り**、確定所見として指示書へ書き足してから引き継いだ
（そこで見つかっていたのが上記①）。

## 2026-09-13 — 🏁§5.3 `O-348` バッチB＝型つき payload キーを全数クローズ（第304バッチ）

**残り12キー / 12ノードを判定＝12キーとも意味があり `IGNORED` は0件。**
これで **`O-348` の「型つきアクション 22種 / 58ノード」は全数クローズ**（残 34種 / 87ノードは `[STUB]` 側）。

🔑**バッチAは実バグ0件だったが、バッチBでは 12ノード中 2件が実バグだった**＝
**母集団が小さいキーほど「1ノードしか無いから parser の書き損じだろう」とは限らない。**

### ① 🔴実バグ `exceptSelfSource`＝同名の別コピーを「自身」と誤判定（`WX17-001`）

**原文**＝「【常】：《カーニバル　―Ｑ―》は**自身以外の**効果を受けない。」

`collectEffectImmuneSigni`（`src/engine/effectEngine.ts:6707`）は
**両方の identity をベースカード番号へ落として**比較していたので、`WX17-001#1` と `WX17-001#2` も
「自身」と判定していた＝**ミラーマッチで対戦相手の同名カードの効果まで耐性を素通りしていた**
（`sourceOwner:'any'`＝どちらの側の効果でも遮断するのがこのカードの効果）。
⇒ **両方に instance id があるときは完全一致**で比べ、片方が base 番号だけの旧経路のときだけ base 比較へ落とす。
- golden＝`§5.3 O-284: WX17-001-E1 は…` に2 assert 追加（同一インスタンスは通す／同名の別コピーは遮断する）
- **反転確認 ✅**＝base 比較へ戻すと「同名の別コピーからの効果は遮断する」が FAIL。

**逆翻訳も嘘だった**＝旧「このシグニは効果を受けない」（**ルリグなのに「シグニ」／「自身以外」が消えていた**）
→ 新「このカードは自身以外の効果を受けない」。

### ② 🔴実バグ `selfTrashCost`＝コストを払えないのに帰結だけ起きる（`WX21-052`）

**原文**＝「あなたの＜天使＞のシグニは『【自】：対戦相手のターン終了時、対戦相手のパワー5000以下のシグニを
２体まで対象とし、**このシグニを場からトラッシュに置いてもよい。そうした場合、**それらをバニッシュする。』を得る。」

旧実装は**対象を先にバニッシュしてから**効果元のトラッシュを試みていたので、
**解決前に効果元が場を離れていると、コストを1つも払わずに相手シグニ2体が消えていた**（過剰実行）。
⇒ `resumeSelectTarget` で**帰結を適用する前に**効果元が場にいるかを確かめ、払えないなら
バニッシュも「そうした場合」の後続も実行しない。
- golden＝`O-348 WX21-052-E1-G: selfTrashCost は効果元を場から払えない場合バニッシュしない`
- **反転確認 ✅**＝ガードを外すと当該テストだけが FAIL。
- ⚠この能力は**付与**（`GRANT_FIELD_SIGNI_ABILITY`）なので、影響は＜天使＞のシグニすべてに及ぶ。

**逆翻訳も嘘だった**＝旧「対戦相手のパワー5000以下のシグニ2体までをバニッシュする」＝**コスト文が丸ごと無く、
無償で2体消せるカードに見えていた**。

### ③ 逆翻訳だけの穴 10キー（engine は原文どおり）

| キー | カード | 落ちていた原文の語 |
|---|---|---|
| `battleOnly` | `WXDi-P15-078` | 「それが**バトルによって**バニッシュされる場合」 |
| `distinctAcrossZones` | `WXDi-CP01-031` | 「場とエナゾーンに…が合計5**種類**ある場合」（領域をまたいだ重複排除） |
| `excludeColorlessZeroGrowCost` / `withoutAttackPhaseIcon` | `WXDi-P05-010` | 焼き込み文を payload から描く形へ（**生成文は不変**） |
| `fireCondition` | `WX05-042` | 「**それがこのターンで３回目である場合**」 |
| `opponentChoosesPileToTrash` | `WXDi-P05-015` | 表向き3枚／裏向きの2束を作り**対戦相手がどちらを捨てるか選ぶ**全文 |
| `preUseVirusChoose` | `WX15-067` | 「【ウィルス】を２つ以上取り除いていた場合、代わりに２つまで選ぶ」 |
| `reattachPreviousAcceOptional` / `sourceThisCard` | `SP27-015` | 「**場にあるこのシグニを**」「付いていた【アクセ】１枚を付けてもよい」 |
| `swapWithLastProcessed` | `WXDi-P08-037` | 「**そのシグニと**あなたのアップ状態のシグニ１体の場所を入れ替える」 |

**いちばん嘘が大きかった2件**
- `WXDi-P05-015-E2`＝旧「デッキ上6枚を公開し、**それがカードの場合、何もしない**、残りをデッキの上に戻す」
  ＝**効果が丸ごと消えていた**（engine は正しく2束を作っていた）。
- `SP27-015-E1`＝旧「場のシグニ1体を、**他の他の**シグニ1体の【アクセ】にする」
  ＝壊れた日本語のうえ、効果元自身が【アクセ】になる軸も後半の付け直しも消えていた。

### 🔑 codex の実装を検証して直した2件

1. 🔴**`swapWithLastProcessed` の新分岐が `suppressOnPlay` を落としていた**
   （原文「そのシグニの【出】能力は発動しない」。**すぐ上の `swapSourceTarget` 分岐は描いていた**）。
   ⚠**新しい早期 return を足すと、その関数が既に描いていた語が黙って消える**＝分岐を足したら
   **同じ関数の他の分岐が描いている軸を全部数え直す**。
2. `battleOnly` が `bySource` を上書きする形（`a.battleOnly ? 'バトルによって' : src`）だったのを
   `src || (battleOnly ? … )` へ。現 live に併存は無いが、**軸を足すときに既存の語を消さない**規約に合わせた。

### 検証

- `npm run gates` **全緑**（typecheck / **golden 4065 PASS**（+1）/ smoke 10754 OK / fuzz 0 / census 各種 / lint 0 error）
- **重め fuzz**＝`npm run fuzz -- --games 2000 --moves 80` で CRASH/HANG/INVARIANT/EXPLOSION **0**
  （`O-350` の教訓＝engine の解決経路を触った回は軽い fuzz の緑を証拠にしない）
- **ラチェット**＝`census:payloadkeys` `BASELINE 46 → 34`／`census:numberdrift` `71 → 70`
- **逆翻訳シートの差分は該当9カードのみ**（`WXDi-P05-010` は payload 化しても生成文が不変＝差分なし）
- **実機は不要と判定**（§2.2）＝触ったのは `src/engine/` と `scripts/`・`docs/` だけで、
  `src/screens/` は触っておらず、新しい型・機構も足していない。

## 2026-09-13 — §5.3 `O-348` バッチA＝逆翻訳が描き落としていた payload キー10種（第303バッチ）

🔴🔑**この回の主産物は engine の修正ではなく「検査面の回復」**＝
**10キーすべて engine は原文どおりに読んでいて、嘘をついていたのは逆翻訳だけだった。**
このプロジェクトの主軸の検査は「原文 × 逆翻訳の目視照合」なので、
**逆翻訳が語を落としている箇所は、そこだけ検査が効いていない＝実バグの隠れ場所**になる。
⇒ **実バグ0件でも払い戻しの価値がある回**（§2.6 の「型を機械の全数検出に変える」の後工程）。

**母集団（着手前に実測・登録票は stale ではなかった）**＝`npm run census:payloadkeys` 未判定 **56種 / 145ノード**。
うち**型つきアクション 22種 / 58ノード**（`case '<型>'` がキーを無視＝取り落ち確定）／`[STUB]` 34種 / 87ノード。
**今回＝型つきのうち母集団の大きい10キー / 46ノード / 44効果**（残り12キーはバッチB）。

| キー | 型 | engine の消費地点 | 落ちていた原文の語 |
|---|---|---|---|
| `lastProcessedFrom` 8 | `LOOK_PICK_CHAIN` | `effectExecutor.ts:8772` | 「この方法で**場に出た**シグニ」（後続の参照範囲） |
| `energyOwner` 7 | `POWER_MODIFY_PER_ENERGY(_COLOR)` | `effectEngine.ts:2974/3022` | 「**あなたの**エナゾーン」 |
| `additionalCostChoose` 6 | `CHOOSE` | `effectExecutor.ts:7336` | 「追加で〜支払っていた場合、**代わりにN つ選ぶ**」 |
| `allowRepeat` 5 | `CHOOSE` | `effectExecutor.ts:7352/7382` | 「**同じ選択肢を2回以上選んでもよい**」 |
| `bestEffort` 5 | `TRASH` | `effectExecutor.ts:7234` | 「（手札を捨てられなくてもカードを２枚引く）」 |
| `declaredCountChoose` 4 | `CHOOSE` | `effectExecutor.ts:7359` | 使用時に宣言した数で選択数が固定される軸 |
| `perAllSigni` 4 | `ATTACH_CHARM` | `effectExecutor.ts:8304` | 「シグニ**１体につき**カード１枚を、**それらの**【チャーム】に」 |
| `frontOnly` 3 | `BANISH_REDIRECT` | `effectEngine.ts:2021` | 「**このシグニの正面の**」 |
| `recordRevealed` 2 | `REVEAL_AND_PICK` | `effectExecutor.ts:8603` | 「この方法で〜が３枚以上**公開された**場合」（選んだ札ではなく公開札全部） |
| `virusOwner` 2 | `POWER_MODIFY_PER_VIRUS_COUNT` | `effectEngine.ts:2961` | 「**対戦相手の場にある**【ウィルス】１つにつき」 |

**10キーとも原文に対応語があった＝`IGNORED` 送りは0件。** `src/` は1行も触っていない。

**いちばん意味が反転していた2件**
- `perAllSigni`（`WX08-046` / `WX08-081` / `WX13-037` / `WX18-038`）＝原文「対戦相手は**自分のシグニ１体につき**
  自分のデッキの上からカード１枚を、**それらの**【チャーム】にする」に対し、旧逆翻訳は
  「対戦相手のデッキの上からカード**1枚**を対戦相手の**好きな数の**シグニの【チャーム】にする」＝
  **1枚を複数体で共有するように読め、枚数が体数ぶん少なく見えていた**。
- `frontOnly`（`WX19-078` ほか）＝原文「**このシグニの正面の**感染状態のシグニ」が
  「**対戦相手の**感染状態のシグニ」＝**盤面全体に効くように読めていた**。

### 🔑 codex の実装を検証して直した2件（どちらも「黙って落ちる」形）

1. 🔴**`declaredCountChoose` が上限の「N つまで」を消していた**＝
   `cntCh` を丸ごと `'使用時に宣言した数だけ選ぶ'` へ差し替えていたので、
   原文「以下の４つから**４つまで**選ぶ」（`WX13-003`）の上限が逆翻訳から消えた。
   ⇒ **上限は残して併記する**形へ（`以下の4つから4つまで選ぶ（使用時に宣言した数だけ選ぶ）`）。
   ⚠**キーを描く作業が、別の語を落とす**ことがある＝`git diff` の before/after を原文と3列で読む。
2. 🔴**`perAllSigni` の分岐が `charm.type === 'DECK_CARD'` 限定で、外れると既定文へ黙って落ちる**＝
   その既定文が「**好きな数の**シグニの【チャーム】にする」＝**今回直したのと同じ嘘**に戻る。
   ⇒ 分岐を `perAllSigni` 単独にし、`DECK_CARD` 以外でも「1体につき1枚ずつ」だけは必ず描く形へ。
   （`to.filter` も既定文では描かれていたので早期 return に持ち込んだ。）

### 検証

- `npm run regen` → `npm run gates` **全緑**（typecheck / **golden 4064 PASS** / smoke 10754 OK / fuzz 0 /
  census 各種 / lint 0 error）。
- **ラチェット2本を実測値へ払い戻し**＝`census:payloadkeys` `BASELINE 56 → 46`（`scripts/censusPayloadKeys.ts`）／
  `census:numberdrift` `BASELINE 72 → 71`（`scripts/censusNumberDrift.mjs`）。
  🔑**ラベル／描画の整備は `numberdrift` の払い戻しでもある**（描いていない payload の数値は逆翻訳から丸ごと落ちる）。
- **逆翻訳シートの差分は該当43効果のみ**（`WXDi-P16-TK01-E1` は `CardData_TK.csv` 由来でシート収録外＝個別 CLI で確認）。
  **該当カード以外の行は動いていない。**
- **実機は不要と判定**（§2.2）＝触ったのは `scripts/` と `docs/` だけで、`src/engine/` も `src/screens/` も触っていない。
...
3. 🔴**計器を書くときのエスケープに注意。** テンプレートリテラルの `\b` / `\s` が1段剥がれて
   `\b`（バックスペース）・`s` になり、**黙って何にも当たらない**規則になった。`census:deadstate` は
