...
- ~~**【ビート】機構**（出現44）~~ → **Phase1-4実装済**（《ビート》[条件]ゲート12＋ON_BECOME_BEAT8＋cost.beat_signi支払い＋コスト型[４枚以下]使用ゲート9。BUGFIXES参照）。**残**: WDK14-013-E1のトラッシュ→beatコスト機構／beat対象のプレイヤー選択UI（現状レベル低い順の自動近似）／MAKE_BEAT正規化／WDK14-008（公開4→1手札＋1ビート→ビート同レベルの相手バニッシュ）。
