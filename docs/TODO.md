...
- ~~コスト払い型 `BANISH_SUBSTITUTE`（WX10-033/WX11-029）が宣言だけで未実装~~ → **v0.402 で実装済**（BUGFIXES.md 参照）。~~`WX06-019` は効果離場型のため未対応~~ → **v0.403 で実装済**（eff果バニッシュ経路 `execBanish` に powerReduction 自動適用フックを追加。BUGFIXES.md 参照）。
