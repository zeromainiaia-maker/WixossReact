...
- **ルリグへの付与（配線未完）:** `WXDi-P05-032`（センタールリグへ「【自】《ターン1回》ルリグアタック時、相手シグニ1体トラッシュ」を付与）← `collectLrigGrantedEffects`(GRANT_LRIG_ABILITY) の出力が augMap[lrigNum] にも `lrig_granted_auto_effects` にも入らず ON_ATTACK_LRIG 収集に届かない。配線が要る。
