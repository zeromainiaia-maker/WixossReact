...
最終更新: 2026-06-20（v0.388 まで）。**F-2 は既存機構でクリーンに実装可能な14枚＋相手場付与1枚（WXDi-P10-072・v0.387）＋THE DOOR自ゲート2枚（WXDi-P15-076/082・v0.388）を完了**。残りは F-2 節末の「残り2枚」（置換2＝専用機構/高リスク）。引き継ぎ詳細は BUGFIXES.md 冒頭の各バッチ記録を参照。
...
**✅ 完了:** WXDi-P15-076/082（v0.388）。バッチA＝WXDi-P15-080/081/077/078（v0.389）。バッチB＝WXDi-P15-059/WXDi-P16-074（v0.390・後者はE1のみ／E2は近似維持）。バッチC＝WXDi-P16-062（v0.391・`inGateZone` フィルタ新設・E1は近似）。バッチD＝WXDi-P15-057（v0.392・E1は近似）。

**残りのゲート参照シグニ（未実装・要追加表現）:**
- `WXDi-P15-056`（電機・E1 自己能力消去型の誤パースが有害／E2 同ゾーンゲートのシグニへ+2000 UNTIL_OPP_TURN_END＝inGateZoneで可だがE1の安全な無害化が要る）/ `WXDi-P15-058`（宇宙・同ゾーンゲートのシグニへ keyword 付与）
- `WXDi-P16-054`（水獣・「相手ターン中の付与」）/ `WXDi-P16-059`（アーム・ガード税付与）/ `WXDi-P16-070`（毒牙・E1 同ゾーンゲートでデッキ下は可／E2 per hand card パワー＝STUB GATE 誤パースが有害なのでE2同時対応が必要）
- 必要な追加表現：①「相手ターンの間」の一時付与（UNTIL_OPP_TURN_END grant）②`POWER_MODIFY_PER_HAND_COUNT`（手札1枚につき）③ガード税・シャドウ（スペル/特定レベル）付与 ④self対象 REMOVE_ABILITIES の thisCardOnly 対応（P15-056-E1 の安全実装に必要）。`inGateZone` フィルタ（v0.391）で「同ゾーンゲートのシグニへの場全体付与」は表現可能。
