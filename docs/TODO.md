...
## 3.5. ⚠ timing flatten 系統（実バグ・当初159枚→**残15枚**・VALUE curation の本丸）

> **進捗サマリ（2026-06-28・R47 時点）＝VALUE 159→15 まで消化（R5-R47）**。残15の大半は**未配線トリガー（新機構が要る・1機構1〜数枚）**。配線済みクラスタ＋ON_DRAW 一族＋正面配置＋チャーム/エナ→トラッシュ＋エクシードコスト＋アクセhost条件＋リフレッシュ＋場→手札＋毒牙パワー減＋相手メイン開始 は枯れた。次の地図は下記「📍 残の分類」。診断＝`npx tsx scripts/_flattenList.ts`。
