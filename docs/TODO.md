...
### 1. 現在地（数字）
- 文型★：1〜14巡で本物バグ約58枚＋汎用フィルタ2種を修正。機構実装①②で計7枚。**15巡目：場出しシグニの【出】抑止（ON_PLAY_ABILITY）系統5枚**（BUGFIXES先頭）。同型★ は常に0（退化なし）を維持。
...
- **後回し（multi-dest pick／要 LOOK_PICK_CHAIN）**: WXDi-P16-035・WXDi-P02-020（look5→手札最大1＋場最大1〜2＋【出】抑止）。15巡で場出し単目的の3枚は再構築済だが、手札＋場の二目的pickは別語彙。
