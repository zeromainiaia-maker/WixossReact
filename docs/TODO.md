...
- **正面 等:** WX20-Re18（正面＋エナ依存の動的自己レベル条件）/ PR-288（ルリグレベル一致）/ WX18-076（被バニッシュ時に「正面にあった」シグニ参照＝離場時ゾーン記録が必要）。※PR-426 は v0.348 で `frontOfSelf`＋`IS_SELF_IN_CENTER_ZONE` を追加し実装済。「自身のパワー以下」は v0.344 で解決済。
