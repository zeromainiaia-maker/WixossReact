...
- **場全体/特定クラス全体への付与:** WX21-052（＜天使＞・任意自トラッシュコスト）/ WD14-001（全領域に【ライフバースト】付与）/ WXDi-P15-061（上の＜解放派＞＝GRANT_SIGNI_ABOVE_ABILITY）。※WX13-034 は v0.344 で `GRANT_FIELD_SIGNI_ABILITY`＋`powerLtSelf` で実装済。
