...
- ~~**【ビート】機構**（出現44）~~ → **Phase1-7 で完了**（[条件]ゲート12＋ON_BECOME_BEAT8＋cost.beat_signi支払い＋コスト型[４枚以下]使用ゲート9＋トラッシュ→beatコスト/WDK14-013＋look→pick beat化宛先/同レベルバニッシュ/WDK14-008＋MAKE_BEAT正規化＋beat対象プレイヤー選択UI。BUGFIXES参照）。**残（低優先）**: トラッシュ版選択ピッカー（WDK14-013）のみ自動近似。全て要実機検証。
