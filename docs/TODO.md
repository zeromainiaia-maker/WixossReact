...
| カード | 内容 | 状態 |
|---|---|---|
| WXDi-P07-073 / WXK07-043 | 「対戦相手のアタックフェイズ開始時」系のアクション再モデルが必要（2枚）。下記「アクション再モデル詳細」参照 | 未対応（dormant 据え置き中） |
...
### アクション再モデル詳細（WXDi-P07-073 / WXK07-043）

両カードとも「対戦相手のアタックフェイズ開始時」系の AUTO だが、現状 JSON のアクションが誤っているため、
**発火させると害になる。修正完了まで timing を `['ATTACK']`（未発火＝dormant）に意図的に据え置いている。**
アクションを正しく直してから timing を `ON_ATTACK_PHASE_START` / `triggerScope: any_opp` へ変更すること。

- **WXDi-P07-073「羅原 In」**
  - テキスト: 「対戦相手のアタックフェイズ開始時、ターン終了時まで、あなたのセンタールリグは『【自】：対戦相手のターン終了時、あなたのすべてのシグニをバニッシュする。』を得る。」
  - 現状 JSON: `BANISH owner:opponent count:ALL`（**owner 反転＋遅延→即時化** の誤 model）。
  - 正しい実装: ルリグへ「対戦相手ターン終了時に**自分の**全シグニをバニッシュ」する遅延能力を付与（`GRANT_LRIG_ABILITY` または `granted_effects` 経由の `ON_TURN_END` 効果）。
- **WXK07-043「羅菌 マグネ」**
  - テキスト: 「対戦相手のアタックフェイズ開始時、このシグニに【チャーム】が付いている場合、ターン終了時まで、このシグニは『【常】：バニッシュされない。』を得る。」
  - 現状 JSON: `GRANT_KEYWORD self keyword:"チャーム"`（**付与キーワードが誤り**）。
  - 正しい実装: チャーム付随を条件（activeCondition / condition）に、自身へバニッシュ耐性（`バニッシュされない`→`バニッシュ不可` / `GRANT_PROTECTION from:['BANISH']` 系）を UNTIL_END_OF_TURN 付与。
