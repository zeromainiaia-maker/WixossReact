...
- **`ON_SPELL_USE` がルリグからしか収集されない＋スペル色フィルタ機構が無い（要エンジン拡張）:** **WX01-033-E1**（幻獣神オサキ＝「あなたが緑のスペルを使用したとき、デッキトップをエナへ」）は ①timing が `ON_PLAY` に誤パース、②そもそも `ON_SPELL_USE` は `BattleScreen` の spell-use ブロックが**キャスターのセンタールリグの effects のみ**を走査するため**シグニの ON_SPELL_USE は発火しない**、③「緑のスペル」のスペル色フィルタ機構が無い（現状 `evalUseCondition` に使用中スペルの色が渡らない）。本実装には PvP/CPU 両経路で「場のシグニからも ON_SPELL_USE を収集」＋「使用スペルの色フィルタ」を足す必要がある。現状 no-op で無害（誤発火はしない）。
