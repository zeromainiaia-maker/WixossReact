...
最終更新: 2026-06-20（v0.401 まで）。**F-2 は全完了**（付与型14枚＋相手場付与 WXDi-P10-072・v0.387＋THE DOOR自ゲート WXDi-P15-076/082・v0.388＋身代わり置換 WXDi-P06-034/WXK05-024・v0.393）。**F-4（THE DOOR）も全完了**（場全体シャドウ付与 WXDi-P15-058-E1・v0.399＋近似精緻化3枚 P15-057/P16-054/P16-074・v0.400）。**F-3 はバトルバニッシュ経路の5枚を対話本実装**（v0.401・要実機検証／効果バニッシュ経路＋別機構2枚は残）。引き継ぎ詳細は BUGFIXES.md 冒頭の各記録を参照。
...
**✅ バトルバニッシュ経路の5枚は v0.401 で対話本実装**（`BANISH_SUBSTITUTE`＋`collectBanishSubstitutes`＋再入 pause/resume＋防御側プロンプト＋CPUヒューリスティック。詳細は BUGFIXES.md）：`WX12-024` `WXEX2-60` `WX20-055` `WXDi-CP01-032` `WXDi-P10-052`。

**残・積み残し:**
- **効果バニッシュ経路:** 上記5枚は**バトルバニッシュのみ**対応。効果バニッシュ（`execBanish`）/バウンス等の場離れは未フック（execBanish 側に置換差し込みが要る。F-2 身代わり置換と共通課題）。
- **実機検証:** 対話 pause/resume・CPU即決はヘッドレス検証不可。PvP／CPU戦での動作確認が必要。
- **別機構の2枚:** `WX25-P1-056`（対戦相手効果による**非バニッシュ離場→バニッシュ**置換＝場離れ全般のフックが要る）/ `WX17-075`（置換ではなく「正面にレベル2以下が出たとき任意バニッシュ」＝`ON_PLACED_FRONT` 任意トリガー機構が要る）。いずれも現状 no-op で無害。
