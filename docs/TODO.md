...
- **【Stage2・着手中(claude)】BattleScreen 配線の純粋抽出＝C 配線の自動検証化**。collect*Triggers を `src/engine/` の pure 関数へ抽出し golden/fuzz から呼べるようにして C 配線（ON_TARGETED 発火等）を自動検証＝C2 宿題を削減する取り組み。
  ...
  - **残＝他の collect*Triggers の抽出**（`collectBanishTriggers`(ON_BANISH・多数)/`collectLeaveFieldTriggers`/`collectFieldTriggers`(ON_PLAY・複雑)/`collectArmorTriggers`/`collectHandDiscardTriggers`/`collectTurnTriggers`(ON_TURN_*/ATTACK_PHASE/LRIG_ATTACK_STEP) 等）と、`detect*`（盤面差分）／フェイズ進行／effect_stack 整列の抽出。これらが済むと既存配線（R5-R58）も golden 化でき C2 が大幅に減る。⚠17000行からの抽出＝中リスク・1ファミリずつ・抽出は薄いラッパ化で挙動不変を保つ。次の候補＝`collectTurnTriggers`（turn系timing多数＋自分が配線した ON_LRIG_ATTACK_STEP_START を含む）か `collectArmorTriggers`（小・クリーン）。
