...
5c. **WX04-005-E3 シグニ場出し数制限（要実機検証・ヘッドレス不可）:** 「すべてのプレイヤーはシグニを1体しか場に出せない（既に2体以上なら1体になるよう捨てる）」を実装。継続STUB `LIMIT_ALL_FIELD_1`（パーサー＋JSON durable）＋`computeFieldSigniLimit`（自他いずれかのセンタールリグが持てば両者適用）。**召喚制限**＝人間ゲート(canFitSomewhere)＋CPUループに体数上限を追加。**捨て補足**＝グロウ時に `reduceFieldSigniToLimit` で両者を上限まで自動削減（人間 executeGrow＋CPUグロウ）。**検証点／近似:** ①捨てる対象は**自動（レベル高優先・選択UIなし）**＝本来は各自が選ぶ、②削減時の **ON_LEAVE_FIELD/ON_TRASH トリガー未収集**、③グロウ時の costOnPlay 分岐では相手側削減の書き込みが漏れる（WX04-005は該当なしだが将来の同型注意）、④出場制限は「シグニを場に出す」全経路（効果による場出し＝ADD_TO_FIELD等）には未適用＝手札召喚のみ。
