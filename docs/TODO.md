...
- **【Stage2・着手中(claude)】BattleScreen 配線の純粋抽出＝C 配線の自動検証化**。collect*Triggers を `src/engine/` の pure 関数へ抽出し golden/fuzz から呼べるようにして C 配線（ON_TARGETED 発火等）を自動検証＝C2 宿題を削減する取り組み。
  ...
  - **残＝他の collect*Triggers の抽出**。クリーン系＝`collectCharmToTrashTriggers`/`collectEnergyToTrashTriggers`/`collectRefreshTriggers`/`collectPowerDecreaseTriggers`/`collectMoveToDeckTriggers`/`collectFreezeTriggers`/`collectBloomTriggers`/`collectSelfEventTriggers`/`collectZoneMovedTriggers`/`collectDriveBecameTriggers`/`collectBeatBecameTriggers`/`collectHandDiscardTriggers`/`collectOppArtsUseTriggers`/`collectArtsUseTriggers`（14）。大物＝`collectTurnTriggers`（ON_TURN_*/ATTACK_PHASE/LRIG_ATTACK_STEP・特殊ケース多数＝中リスク）/`collectFieldTriggers`（ON_PLAY・複雑）。加えて `detect*`（盤面差分・約13個）／フェイズ進行／effect_stack 整列。これらが済むと既存配線（R5-R58）も golden 化でき C2 が大幅に減る。⚠1ファミリずつ・薄いラッパ化で挙動不変を保つ。
