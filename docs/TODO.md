...
- [~] **系統②：GRANT_PROTECTION `count:'ALL'`＋subjectFilter無し＝48件**。**単体保護24件は `count:'ALL'→1` 是正済（2026-07-03・BUGFIXES上部）**。**⚠2026-07-03 追加調査で「アーツ/ルリグ 11件は engine配線が必要」は誤りと判明・訂正**＝アーツ/ルリグ/スペル 発生源の保護は **`collectEffectImmuneSigni`（effectEngine・source-type-aware・count非依存）で既に機能**（CONTINUOUS GRANT_PROTECTION が対象。WX12-018/WX18-034 で immune 確認）。semantic監査は `collectAbilityProtectedSigni`(from=シグニ/any) だけ見て immune コレクタを見落としていた。**genuine な残ギャップは card固有の構造のみ**＝(a)SEQUENCE内 GRANT_PROTECTION（WX08-017・immune/ability 両コレクタが top-level action しか見ない）(b)LAYER付与型（WX15-031＝GRANT_FIELD_SIGNI_ABILITY 経由）。**系統的な保護no-opは無い**（count:1 是正で表現も原文一致に改善済）。残る **広域24件**（「あなたのシグニは…」）は subjectFilter/新機構が要る別課題。分類再取得＝`node scratchpad/_protScan.mjs`・免疫検証＝`scratchpad/_testImmune.ts`。
...
- ~~`LOOK_AT_DECK_AND_LIFE`(3)~~ **✅実装済（2026-07-03）**＝覗き＝情報開示のみ（盤面不変が正しい）・log-only。WX10-070 は完全実装（combo の WX10-068/WX13-048 は宣言/条件部が別STUB）。
- ~~`VARIABLE_DISCARD_AND_DRAW`（1・WX09-Re15）~~ **✅実装済（2026-07-03・BUGFIXES上部）**＝手札を SELECT_TARGET で任意枚捨て→捨てた数+bonus引く（`DrawAction.addLastProcessedCount` で連結・golden 2件）。
...
> **🆕 実機ヘッドレス検証が可能に（2026-06-30）**：`scripts/verifyBattleDrive.mjs`＝実ログイン→CPU戦→盤面注入→**実 UI クリックで効果発火→観測**。WXK09-050 の召喚クリック列（CardModal→召喚→ゾーン→CHOOSE→対象）を `data-testid` で安定自動化し**付与成功まで完走済**。手順・再開ポイント・残り2効果（WD07-012/WXK02-029）の進め方は **[VERIFY_BROWSER.md](./VERIFY_BROWSER.md) 末尾「フル BattleScreen 実機 driver」＋「🔜 明日の続き（RESUME）」が唯一の正**。下記の宿題群もこの方式で順次クローズ可能。
...
- **【Stage2・着手中(claude)】BattleScreen 配線の純粋抽出＝C 配線の自動検証化**。collect*Triggers を `src/engine/` の pure 関数へ抽出し golden/fuzz から呼べるようにして C 配線（ON_TARGETED 発火等）を自動検証＝C2 宿題を削減する取り組み。
  ...
  - ~~第6弾（2026-06-29）＝`collectLeaveFieldTriggers`＋`resolveLeaveFieldDynamicFilters`（ON_LEAVE_FIELD）を抽出＋golden4件追加（PASS 43/43）。triggerFilter(story)/leftToZone=hand ゲートを検証。~~ **✅完了**
  - ~~第7弾（2026-06-29）＝`collectDrawTriggers`/`collectOppDrawTriggers`/`collectMillTriggers`（ON_DRAW self/opp・ON_CARD_MILLED_FROM_DECK）を抽出＋golden4件追加（PASS 47/47）。共通ヘルパ `mkLimitOk`/`ownFieldSources` で重複圧縮。~~ **✅完了**
  - ~~第8弾（2026-06-29）＝set-diff 系6関数（`collectCharmToTrashTriggers`/`collectEnergyToTrashTriggers`/`collectRefreshTriggers`/`collectPowerDecreaseTriggers`/`collectMoveToDeckTriggers`/`collectFreezeTriggers`）を抽出＋golden6件追加（PASS 53/53）。count/detect ヘルパは引数渡しのため BattleScreen 残置。~~ **✅完了**
  - ~~第9弾（2026-06-29）＝クリーン系7関数（`collectSelfEventTriggers`/`collectZoneMovedTriggers`/`collectDriveBecameTriggers`/`collectBeatBecameTriggers`/`collectHandDiscardTriggers`/`collectOppArtsUseTriggers`/`collectArtsUseTriggers`）を抽出＋golden7件追加（PASS 60/60）。~~ **✅完了**
