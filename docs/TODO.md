...
- **正面 等:** WX20-Re18（正面＋エナ依存の動的自己レベル条件）/ WX18-076（被バニッシュ時に「正面にあった」シグニ参照＝離場時ゾーン記録が必要）。※PR-426 は v0.348 で `frontOfSelf`＋`IS_SELF_IN_CENTER_ZONE`、PR-288 は v0.349 で `LRIG_LEVEL_EQ_OPP` を追加し実装済。「自身のパワー以下」は v0.344 で解決済。
- **その他の条件:** WDK16-06H（センターに《楓》がいる）。※WXDi-P05-034（下にカードがある）は v0.342 で `THIS_CARD_HAS_UNDER` を追加し実装済。WXK03-034・WXK03-056（このターン手札をN枚捨てた）は v0.341 で `TURN_HAND_DISCARD_GTE` を追加し実装済。
