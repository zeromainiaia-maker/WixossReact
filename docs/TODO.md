...
- **decompiler 改良候補**: テキスト実行時パース型STUB（`BET_MECHANIC`・`CHOOSE_SAME_OPTION_*`・`DO_THREE_THINGS` 等）を原文反映で描画すれば★偽陽性がさらに減る。
  - ✅**`GRANT_QUOTED_AUTO_ABILITY`/`GRANT_QUOTED_ABILITY` を原文の引用能力で描画（2026-06-28）**: 原文に「…は『【自/常/起/出】…』を得る」がある本物の付与カード（10枚）は引用能力を完全表示。誤パース/引用無しは `[STUB:引用された能力を付与する（原文参照）]` フォールバック。同型★0維持・典型は WXDi-D06-010/WXEX2-20-E3。**残課題**＝「以下の能力を得る。『…』」型（得るの後に引用・WXDi-P07-005/WX25-P1-003等）と複数引用カードは未対応（フォールバック）。WX15-059/WX20-069 は GRANT_QUOTED_AUTO_ABILITY 自体が**誤パース**（原文に引用無し）＝parser 是正案件。
...
- **機構待ち 72枚**＝STUB 56・GRANT_QUOTED付与引用（空/圧縮/遅延）9・トラップ設置(SET_TRAP語彙なし)1・GRANT_KEYWORD CHOOSE脱落1・別timing付与の1効果脱落2・近似注記1＋**WX17-028（【出】動的閾値「公開シグニのレベル合計×1000以下」＝語彙なし）／WX25-CP1-069（遅延条件トリガー「このターン…クラッシュしたとき」＝語彙なし）**。いずれも新機構が要る（GRANT_QUOTED_AUTO 精緻化＝P1_PLAN §5 未着手・動的閾値/遅延トリガーは未語彙）。
...
- **🪤 トラップ未表現クラスタ＝調査完了（系統的でないと判明）**: 「原文に《トラップアイコン》ありJSONにTRAP無し」約20枚を精査した結果、**大半はルール注記の偽陽性**（トラップ本体見出しが無い）、残りも **(i) トラップ設置型＝SET_TRAP/PLACE_TRAP_FROM_REVEALED STUB（WX15-002/082 等・語彙なし＝機構待ち）／(ii) 本体が他効果にマージ済み（WX16-029/041/065/066＝誤構造だが機能present）**。**クリーンにトラップ本体だけ脱落していたのは WX16-062/WX16-064 の2枚のみ＝両方修正済**。残るトラップ案件は「SET_TRAP 設置アクション機構」＋「マージ済み誤構造の分割」＝§5寄りの機構タスク。
