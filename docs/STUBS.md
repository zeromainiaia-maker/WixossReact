...
| 総 STUB ノード件数 | 3708 |
| JSON 0 件・ハンドラのみ（内部/動的生成 STUB） | 370 |
...
|---|---:|---:|---|---|
| `OPTIONAL_ACTIVATE` | 35 | 34 | WD06-018, WD10-001, WDK13-017 |  |
| `OPTIONAL_TRASH_SELF` | 17 | 17 | PR-319, WX06-CB01, WX06-CB03 |  |
| `PREVENT_POWER_MODIFY_BY_OPP` | 7 | 7 | WX05-024, WX12-033, WX20-023 |  |
...
| `EFFECT_LEAVE_PAY_TO_LOSE_SELF_ABILITY` | 4 | 4 | WX25-P2-059, WX25-CP1-039, WX26-CP1-047 |  |
...
| `RULE_REMINDER_TEXT` | 21 | 21 | SP26-003, PR-469, SPDi47-05 | ゲームプレイに影響しない説明テキストは無音でスキップ。 ⚠`UNLIMITED_KEYS` は説明テキストではなく**【常】の実効果**だが、読み手は `BattleScreen` の   キーセット可否ゲートと配置先（`hasUnlim… |
...
| `REMOVE_VIRUS` | 11 | 11 | WD19-001, WX15-028, WX15-040 | 【ウィルス】を取り除く |
| `SONG_FRAGMENT` | 11 | 11 | SPDi47-01, SPDi47-02, SPDi47-03 | エナゾーンから【歌のカケラ】持ちカードをトラッシュに置き、その効果を発動 |
| `LIMIT_CHANGE_UNTIL_ENERGY_PHASE_END` | 10 | 10 | WX24-P3-001, WX24-P3-003, WX24-P3-005 | ルリグリミット修正（エナフェイズ終了まで） 🆕🔴**§5.3 `O-60` 第58バッチ（2026-09-03）＝原文 regex を全部撤去して payload で決める。**   旧実装は `sourceAbilityText(c… |
| `GRANT_GUARD_ICON_HAND_SIGNI` | 9 | 9 | WXDi-P04-049, WXDi-P10-049, WXDi-P13-044 | 手札のシグニにガードアイコンを付与（このターン） |
| `TRASH_SIGNI_UNDER_FIELD_SIGNI` | 9 | 9 | WDK15-001, WDK15-007, WX22-035 | トラッシュからシグニをフィールドシグニの下に置く（ライズ補充） 🆕🔴**§5.3 `O-60` 第58バッチ（2026-09-03）＝原文 regex を全部撤去して payload で決める。**   旧実装は `sourceAbi… |
...
| `LIFE_TO_ENERGY` | 4 | 3 | SP38-004, WX25-CP1-020, WXK09-003 | 対象プレイヤーのライフクロス上1枚を、そのプレイヤーのエナゾーンへ置く。 クラッシュではないためライフバースト/check は発生させない。 |
| `OPP_CHOOSE_YOUR_HAND_DISCARD` | 4 | 4 | WD21-004, WX17-014, WX19-005 | 対戦相手が手札を1枚選んで捨てる |
| `TRASH_ALL_SIGNI_AND_KEY` | 4 | 4 | WX07-017, WX16-033, WXEX2-21 | 🆕🔴**§5.3 `O-60` 第59バッチ（2026-09-03）＝原文 regex を撤去して payload で決める。**   旧実装は `sourceAbilityText(ctx)` に `各プレイヤー\|すべてのシグニ`… |
| `DECLARE_COLOR` | 3 | 3 | WX22-042, WXDi-P07-059, WXDi-P13-051 |  |
| `GAIN_MIKOMIKO_GUARD` | 3 | 3 | WX25-P3-023, WX25-P3-058, WXDi-P12-050 | 【みこみこ親衛隊】を1つ得る |
| `OPP_LRIG_DECK_TO_LRIG_TRASH` | 3 | 3 | SPDi43-25, WX24-P4-014, WX25-P3-019 | 対戦相手が自分のルリグデッキからカード1枚を選んでルリグトラッシュに置く（WX24-P4-014-E3 ②）。 ⚠**選ぶのはカードの持ち主＝対戦相手**なので `opponentResponds` を立てる（`OPP_CHOOSE_YO… |
| `REMOVE_VIRUS_TARGET_ZONE` | 3 | 3 | WD19-007, WX15-064, WX15-115 | この方法で対象にしたシグニと同じゾーンにある【ウィルス】1つを取り除く |
| `REVEAL_CLASS_SIGNI_FROM_HAND` | 3 | 3 | WDK08-Y11, WXK04-034, WXK05-043 | 手札のクラスシグニを好きな枚数公開（公開＝SELECT_TARGET、デッキに触れない） 🆕**絞り込みは payload（`handCardPick`）**（§5.3 `O-60` 第51バッチ・2026-09-03）。 🔴旧実装は… |
| `SET_NEXT_LIFE_CRASH_COUNTER` | 3 | 2 | WX25-P1-004, WXDi-P12-030 | 次にあなたのライフクロスが（限定があればその発生源によって）クラッシュされたとき、対戦相手のライフクロスをクラッシュし返す |
...
| `TK3_DECLARE_DISCARD` | 3 | 3 | WD03-006, WXDi-D09-P04, WXDi-P14-061 | 数字を宣言し、対戦相手の手札から宣言レベルのシグニをすべて捨てさせる |
...
| `UNLIMITED_KEYS` | 2 | 2 | WXK03-003A, WXK03-003B | ゲームプレイに影響しない説明テキストは無音でスキップ。 ⚠`UNLIMITED_KEYS` は説明テキストではなく**【常】の実効果**だが、読み手は `BattleScreen` の   キーセット可否ゲートと配置先（`hasUnlim… |
...
| `BET_ALTERNATIVE` | 1 | 1 | WXDi-P07-059 | ベット強化済みなのでスキップ（BET_MECHANICで処理済み） |
| `CHOOSE_GRANT_FIVE_KEYWORDS` | 1 | 1 | WD21-007 | 以下の5つから1つを選び、対象のシグニ1体が選んだ能力を得る（ベット時はもう1回） |
...
| `DISCARD_IF_NO_CLASS_SIGNI` | 1 | 1 | WXDi-P07-062 | ⚠**payload が無ければ何もしない**（fail-closed）。 |
| `EACH_PLAYER_DRAW_DISCARD` | 1 | 1 | WXDi-P04-010 | 各プレイヤーがカードをN枚引き、M枚捨てる 🆕**§5.3 `O-60` 第23バッチ（2026-09-03）＝枚数は payload（`eachPlayerDrawDiscard`）で受け取る。** 🔴旧実装は `EffectTex… |
...
| `GRANT_BURST_TO_NTH_CHECKED_LIFE` | 1 | 1 | WXDi-P12-036 | 🆕**§5.3 `O-239`（2026-09-04）＝「このターン、1枚目と2枚目にあなたのチェックゾーンに置かれた   ライフクロスは【ライフバースト】「…」を得る」**（`WXDi-P12-036-E1`）。   🔴旧 `DEF… |
| `HAND_NONCOLORLESS_TO_ENERGY` | 1 | 1 | WXK10-083 | 手札から無色でないカードをエナに置く |
| `INSTALL_GAME_GRANTED_AUTO` | 1 | 1 | WX25-P2-009 | ゲーム全体能力付与 |
| `INTERNAL_GRANT_ATTACK_BANISH_TO_ARMORED` | 1 | 1 | WXK04-030 | ターン終了時まで、あなたの血晶武装状態のすべてのシグニは「【自】：このシグニがアタックしたとき、自身のパワー以下の対戦相手のシグニ1体を対象とし、それをバニッシュする」を得る |
| `LRIG_GAIN_ATTACK_PHASE_POWER_DOWN` | 1 | 1 | WX24-P2-030 | アタックフェイズの間、対戦相手のシグニのパワーをあなたの場にあるシグニ1体につき－Nする |
| `LRIG_GAIN_BLOCK_OPP_SIGNI_AUTO` | 1 | 1 | WXDi-P16-044 | 次の対戦相手のターン終了時まで、対戦相手のシグニの【自】能力は発動しない |
| `LRIG_GAIN_OPP_ACTIVATE_COST_UP` | 1 | 1 | WXDi-P15-033 | 対戦相手のカードの【起】能力の使用コストは《無×N》増える |
| `LRIG_GAIN_OPP_SIGNI_AUTO_PAY_GATE` | 1 | 1 | SPDi43-01 | 対戦相手のシグニの【自】能力が発動する場合、対戦相手が指定コストを支払わないかぎりその能力は何もしない |
| `LRIG_UNDER_CARD_OP` | 1 | 1 | WXDi-CP02-054 | `underCardOp` が指す操作を1つだけ行う。 |
...
| `MASS_TRASH` | 1 | 1 | WX11-020 | 対戦相手のエナゾーンにあるすべてのカードと対戦相手の場にあるすべてのシグニをトラッシュに置く |
...
| `NEGATE_ATTACK_ON_TRIGGER` | 1 | 1 | WXDi-P11-055 |  |
| `NON_GUARD_DISCARD_TO_ENERGY` | 1 | 1 | WX24-P2-051 | ガードアイコンなしカードを捨てたとき、そのカードをエナへ |
| `OPP_ENERGY_COLORLESS_ABILITY_LOSS` | 1 | 1 | WXK10-008 | このターン、対戦相手のエナゾーンにあるカードは色と能力を失う |
| `OPP_ENERGY_EXCESS_TRASH` | 1 | 1 | WXEX1-07 | 対戦相手のエナゾーンが閾値以上の場合、1枚トラッシュに 🆕**しきい値は payload（`oppEnergyThreshold`）**（§5.3 `O-60` 第52バッチ・2026-09-03）。 🔴旧実装は `EffectTex… |
| `OPP_REVEAL_SPELL_USE_FREE` | 1 | 1 | WX04-015 | 対戦相手はデッキを上からスペルがめくれるまで公開し、あなたはそのスペルをコストを支払わず限定を無視して使用してもよい（使用しなかった場合、そのスペルは対戦相手のトラッシュに置かれる） |
| `OPP_SIGNI_ENERGY_TO_DECK_BOTTOM` | 1 | 1 | WX25-CP1-003 | 対戦相手のシグニがエナゾーンに置かれる場合、代わりにデッキの一番下に置かれる |
| `OPP_SIGNI_TO_DECK_AND_SHUFFLE` | 1 | 1 | WXK10-006 | 相手シグニをデッキに加えてシャッフル |
| `OPP_SPLIT_LRIG_DECK_LOOK_PILE_ARTS_TO_LRIG_TRASH` | 1 | 1 | WXEX2-12 | 対戦相手は自分のルリグデッキを裏向きで2つの束に分け、あなたはどちらかの束を見てアーツ1枚をルリグトラッシュに置く |
| `OPP_TRASH_TO_DECK_TOP` | 1 | 1 | WXDi-P07-076 | 相手のトラッシュからカードをデッキトップに（もよい） |
| `OPTIONAL_DISCARD_CLASS_SIGNI` | 1 | 1 | PR-328 | 手札からクラスシグニを任意枚数捨てる 🆕**絞り込みと上限枚数は payload（`handCardPick`）**（§5.3 `O-60` 第51バッチ・2026-09-03）。 🔴旧実装は `EffectText + BurstT… |
| `PLACE_REV_SIGNI` | 1 | 1 | PR-Di017A | あなたのライフクロスが1枚以下の場合、このカードを裏返して指定の《REV》のシグニを場に出す（場に出せなかった場合はトラッシュに置く） |
...
| `POWER_MOD_BY_FIELD_CLASS_LEVEL_SUM` | 1 | 1 | WX13-060 | 自分の場の＜X＞シグニのレベル合計×deltaPerLevel で対象のパワーを修正 |
...
| `REMOVE_MIKOMIKO_GUARD` | 1 | 1 | WX25-P3-058 | 対戦相手の【みこみこ親衛隊】を好きな数取り除く |
...
| `TRASH_UNDER_LRIG_CARD` | 1 | 1 | WXK09-001 | センタールリグの下にあるカード1枚をルリグトラッシュに置く |
...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `TRAP_OPERATION` | 21 | 16 | SP26-001, WX15-047, WX15-053 | O-56: TRAP_OP / TRAP_OPERATION の文単位ペイロード。 parser がどの文に一致したかを渡すので、executor はカード全文を読み直さない。 live 18効果・再帰21ノードすべてへの搭載を機械確認後… |
...
| `CONDITIONAL_CARD_COST_BY_OPP_LRIG` | 5 | 5 | WX03-002, WX03-003, WX03-004 | 対戦相手のセンタールリグ色による基本コスト置換の**宣言**。 |
...
| `SET_OPP_SIGNI_AS_TRAP` | 5 | 2 | WX21-003, WX21-025 | 相手のシグニ1体をトラップとして設置 |
| `TRAP_OP` | 5 | 5 | WX16-061, WX17-062, WX21-003 | O-56: TRAP_OP / TRAP_OPERATION の文単位ペイロード。 parser がどの文に一致したかを渡すので、executor はカード全文を読み直さない。 live 18効果・再帰21ノードすべてへの搭載を機械確認後… |
| `DECK_TOP_DECLARED_NUM_TRASH` | 4 | 4 | WX06-014, WXDi-P06-013, WXK03-076 | 宣言した数だけデッキ上からトラッシュへ |
| `DRAW_DISCARD_COUNT_PLUS_N` | 4 | 3 | PR-427, WXEX2-39, WXDi-P00-018 | 捨てた枚数+Nドロー |
...
| `CONDITIONAL_COST_REDUCTION_BY_FIELD` | 3 | 3 | WX10-031, WX12-049, WX15-034 | 場の条件による使用コスト軽減の**宣言**。 |
...
| `GUARD_ALTERNATIVE_COST` | 3 | 3 | WX24-P2-026, WX25-P1-071, WXDi-CP01-005 | あなたが【ガード】する際、《ガードアイコン》を持つカードを1枚捨てる代わりに、指定されたコストを支払ってもよい |
| `LAYER_ABILITY_COPY` | 3 | 3 | WX20-023, WXEX1-32, WXEX2-59 | 対象シグニの《レイヤーアイコン》能力を自シグニにコピー |
| `PLACE_TRAP_FROM_REVEALED` | 3 | 3 | WX16-061, WX17-029, WXEX2-15 | 前のLOOK_AND_REORDERで公開されたデッキ上N枚からトラップ設置 |
| `PLAY_SPELL_FREE_IGNORE_RESTRICTION` | 3 | 2 | WX14-014, WXEX2-14 | 〈場所〉のスペルをコストなし・限定条件無視で使用 |
| `PREVENT_DAMAGE_FROM_OPP_EFFECTS` | 3 | 3 | SPDi44-04, WX25-P1-026, WXK03-011 | 「あなたは対戦相手の**効果によって**ダメージを受けない」の**【常】宣言型**（§5.3 `O-295` 第259バッチ）。 🔴**`prevent_lrig_damage` を書かない**＝上の `PREVENT_LRIG_DAM… |
...
| `PREVENT_POWER_MINUS_BY_OPP` | 3 | 3 | WXDi-P07-085, WXDi-P11-062, WXK03-026 |  |
| `SELF_TRASH_IF_NO_OPP_CHARM` | 3 | 3 | WX13-086, WX13-089, WX13-093 | 相手の場に【チャーム】がなければ自トラッシュ（WX13-086等） |
| `SIGNI_CANT_BOUNCE_FROM_FIELD` | 3 | 3 | WX13-029, WX22-Re04, WXK05-024 |  |
...
| `EXTRA_GUARD_COST_FROM_HAND` | 2 | 2 | WDK04-001, WX19-001 | 対戦相手は、手札から《ガードアイコン》を持つカードを追加で1枚捨てないかぎり【ガード】ができない |
...
| `OPP_SIGNI_LEAVE_TO_TRASH` | 2 | 2 | WX24-P4-002, WXDi-P04-037 | 「〈期間〉、〈フィルタ〉対戦相手のシグニが場を離れる場合、代わりにトラッシュに置かれる」。 |
| `PLACE_SIGNI_UNDER_SIGNI` | 2 | 2 | WXDi-P05-060, WXDi-P09-078 | シグニをあなたの場のシグニの下に置く |
| `PLAY_FREE` | 2 | 2 | PR-474, WX07-014 | この方法で処理したカードをコストを支払わずに使用する |
...
| `PREVENT_NON_FIELD_MOVE_BY_OPP` | 2 | 2 | WXEX2-22, WXK03-011 | **【常】の宣言型**（§6.4 O-3 続き493）。 |
...
| `SET_OPP_SIGNI_POWER_BY_SELF_POWER` | 2 | 2 | PR-K076, WXK11-043 | 自パワーに合わせて相手シグニのパワーを設定 |
...
| `USE_OWN_LRIG_ABILITY_FREE` | 2 | 2 | WX21-Re04, WX22-014 | 🆕**§5.3 `O-283`（2026-09-08）＝「ルリグの能力」をコストなしで使う。**   原文＝`WX22-014-E3`「このルリグの**エクシード能力**１つをコストを支払わずに使用する」／         `WX21-… |
| `ACCE_BANISH_SELF_TRASH` | 1 | 1 | WXK04-031 | アクセを自分のトラッシュへ |
| `ACCE_COST_REDUCTION` | 1 | 1 | WX16-044 | このシグニにアクセするためのエナゾーンにあるシグニの【起】能力の使用コストは減る |
...
| `CAST_FROM_OPP_TRASH` | 1 | 1 | WXEX1-46 | lastProcessedCards未設定時は相手トラッシュからスペル選択 |
...
| `CONDITIONAL_FREE_GROW` | 1 | 1 | WX19-007 | ルリグデッキの指定したルリグへその場でグロウする（コストは payload 次第） |
...
| `GRANT_CHOSEN_ABILITY_FROM_PLAY` | 1 | 1 | WX22-Re04 | 【出】能力で選んだ能力を、このシグニが場にあるかぎり得る |
...
| `GRANT_LRIG_TRASH_ACTIVATE_ABILITY` | 1 | 1 | WXEX2-12 | あなたのルリグトラッシュにあるカードの【起】能力を使用できるようになる |
...
| `HAND_SIGNI_UNDER_SIGNI` | 1 | 1 | WXDi-P15-067 | （原文「あなたの＜解放派＞のシグニ１体の下に置いてもよい」が丸ごと死んでいた）。 🔑置き先の選択を先に済ませ、選んだシグニを `hostCardNum` へ焼き込んでから手札を選ばせる（2段）。 ⚠**payload が無ければ何もしな… |
| `LEAVE_FIELD_TO_DECK_BOTTOM` | 1 | 1 | WXDi-P08-046 | このシグニが場を離れる場合、代わりにこれをデッキの一番下に置く |
| `LEVEL_MOD_PER_COUNT` | 1 | 1 | WX10-036 | このシグニのレベルが、指定されたカードの枚数1つにつき増減する |
...
| `LOOK_TOP_ONE_RETURN_REST_BOTTOM` | 1 | 1 | WXDi-CP01-036 | デッキ上N枚を確認し1枚をトップ・残りをデッキ下に デッキ上N枚を見て1枚をトップへ・残りを好きな順番でデッキ下へ |
...
| `MILL_EACH_REPEAT_ON_NAME` | 1 | 1 | WX12-037 | 🏁**`CONDITIONAL_PER_TRASH` は撤去した**（2026-09-05 §5.3 `O-60` 第74バッチ）＝   **live 0 の死んだ枝**。アビリティブロックの原文に `トラッシュにN枚以上` を当てて … |
...
| `PREVENT_DAMAGE_AND_LIFE_MOVE_BY_OPP` | 1 | 1 | WX19-046 | 「あなたは対戦相手の**効果によって**ダメージを受けない」の**【常】宣言型**（§5.3 `O-295` 第259バッチ）。 🔴**`prevent_lrig_damage` を書かない**＝上の `PREVENT_LRIG_DAM… |
...
| `PREVENT_TARGET_LRIG_ATTACK_THIS_TURN` | 1 | 1 | WX09-Re03 | このターン対象ルリグのアタックを防ぐ |
...
| `RETURN_TRAP_TO_HAND_ONE` | 1 | 1 | WX17-041 | 【トラップ】1つを対象とし、それを手札に戻す |
...
| `SUPPRESS_GAIN_ABILITY` | 1 | 1 | WX13-029 | 🆕SUPPRESS_GAIN_ABILITY:「このターン、あなたのシグニは新たに能力を得られない」（§5.3 `O-159`・2026-08-30）。 🔴**旧実装は上の共通枝で `[保護効果: …]` とログを出すだけの真 no-… |
| `SUPPRESS_LIFEBURST_COLOR_CONDITION` | 1 | 1 | WX25-P3-003 | 条件を満たす対戦相手のライフクロスの【ライフバースト】は発動しない |
...
| `TRASH` | 1 | 1 | WDK07-E09 | この方法で処理したカード（無ければこのカード自身）をトラッシュに置く |
...
| `TRASH_CLASS_TO_HAND_OR_ENERGY` | 1 | 1 | WX26-CP1-022 | トラッシュからクラスシグニを手札かエナへ選択 🆕**絞り込みと上限枚数は payload（`trashPickSplit`）**（§5.3 `O-60` 第53バッチ・2026-09-03）。 🔴旧実装は `EffectText + … |
| `TRASH_FROM_DECK_PER_SIGNI_LEVEL` | 1 | 1 | WXK02-004 | 自場シグニのレベル合計枚数をデッキ上からトラッシュ |
| `TRASH_TRAP_ONE` | 1 | 1 | WX19-064 | 【トラップ】1つを対象とし、それをトラッシュに置く |
| `TRIPLE_ZONE_DISTRIBUTE_FROM_TRASH` | 1 | 1 | WXEX1-19 | あなたのトラッシュからカード1枚をエナゾーンに置き、1枚を手札に加え、1枚をデッキの一番下に置く |
...
| `COLLAB` | 5 | 5 | WXDi-CP01-004, WXDi-CP01-005, WXDi-CP01-006 | コラボ効果（「コラボライバーN人を呼ぶ」） |
...
| `INTERNAL_ASK_ACCE_HOST` | 5 | 5 | WDK07-E07, WDK07-E11, WDK07-E20 | INTERNAL_ASK_ACCE_HOST / INTERNAL_ATTACH_ACCE_TO_HOST（§6.4 O-11・2026-08-16）＝ 「あなたのデッキから〈X〉のシグニ1枚を探して**それの【アクセ】にし**、デッキを… |
...
| `IGNORE_LRIG_RESTRICTION_ARTS` | 3 | 3 | PR-K060, WX05-006, WX14-003 | ルリグ制限アーツを無視（ログのみ） |
| `LIMIT_OPP_DRAW_COUNT` | 3 | 3 | WXEX1-10, WXDi-P12-008, WXK06-004 | ドロー枚数制限（次のターン） 🆕**上限は payload（`drawLimit`）**（§5.3 `O-60` 第52バッチ・2026-09-03）。 🔴旧実装は **カード全文**へ `/合計(\d+)枚までしか引けない/` を当… |
...
| `ONE_ATTACK_PER_TURN` | 3 | 3 | WXDi-P11-071, WXDi-P12-078, WXDi-P15-069 | このシグニは各ターンに一度しかアタックできない |
...
| `BLOCK_OPP_ARTS_SPELL_ACT_NEXT_TURN` | 2 | 2 | WX15-003, WX25-P1-050 | 次のターンの間、対戦相手はアーツとスペルと【起】能力を使用できない |
| `CENTER_LRIG_DISMOUNT` | 2 | 2 | WXK03-036, WXK10-063 | センタールリグがすべての乗機シグニから降りる（ドライブ解除・任意） |
| `CENTER_LRIG_RIDES_ON_SIGNI` | 2 | 2 | WDK01-008, SPK01-01 | センタールリグが選択した1体の乗機シグニに乗る（乗り換え可） |
| `COIN_USE_RESTRICTION` | 2 | 2 | WXDi-P15-008, WXDi-P15-009 | コイン使用先をスペルとシグニに限定（ゲーム中永続） |
| `COPY_CARD` | 2 | 2 | WX21-034, WXDi-P07-041 | ターン終了時まで、このシグニは対象にしたカードとレベル以外が同じカードになる |
| `COPY_SIGNI` | 2 | 2 | WX17-001, WXK04-005 | 自フィールドシグニ1体をトラッシュのシグニと同じカードにする（ターン終了時まで） |
| `CRASH_TO_TRASH_INSTEAD` | 2 | 2 | WX19-034, WX25-P3-032 | このターン相手のライフクロスクラッシュ時、エナではなくトラッシュへ |
| `DECK_SIGNI_LEVEL_OVERRIDE` | 2 | 2 | WX18-065, WXK07-056 | デッキ内指定クラスのシグニレベルをN扱い（このターン） |
| `DEFEAT` | 2 | 2 | PR-422, WXK02-002 | 敗北処理 - ライフクロスを0にしてゲーム終了を誘発 |
...
| `LIFE_BURST_DOUBLE` | 2 | 2 | WD23-006-E, WXDi-P12-035 | 「次に」ありは1回消費、無しはこのターン中の全ライフバーストを2回発動する。 |
...
| `OPP_LRIG_UNDER_TO_LRIG_TRASH` | 2 | 2 | WD23-012-A, WXK11-002 | 対戦相手のセンタールリグの下からカードを対象とし、それらをルリグトラッシュに置く |
| `OPTIONAL_RETURN_SELF_ARTS_FIRST_USE` | 2 | 1 | WX24-P3-036 | このターンにあなたがこのカードを使用したのが一度目の場合、このカードをルリグデッキに戻してもよい |
| `PEEP_HAND` | 2 | 2 | PR-K070, WX24-P4-105 | 相手の手札を覗き見（ログに枚数と名前を表示） |
| `REPLACE_LEAVE_FIELD_WITH_TRASH_UNDER` | 2 | 2 | WXDi-P05-038, WXDi-P08-044 |  |
...
| `ADD_RESONANCE_CONDITION` | 1 | 1 | WX20-052 | あなたのルリグデッキにあるレゾナを、アタックフェイズにも場に出せるようになる |
| `ADJACENT_ZONE_ATTACK` | 1 | 1 | WD20-009 |  |
| `ALL_CARDS_COLOR_CHANGE_BLACK` | 1 | 1 | WXK07-005 | あなたのすべてのカードは黒でもある |
| `ALL_CENTER_LRIG_GAIN_TYPE_GAME_WIDE` | 1 | 1 | PR-471 | ゲーム全体ルリグタイプ付与（effectEngine lrig_gained_types参照） |
| `ALL_CLASS` | 1 | 1 | WX21-021 | このシグニはすべてのクラスを持つ |
| `ALL_PLAYER_MILL` | 1 | 1 | WX22-017 | 各プレイヤーが自分のデッキ上N枚をトラッシュ |
| `ALL_ZONE_BLACK` | 1 | 1 | WDA-F02-17 | このシグニはすべての領域で黒でもある |
...
| `BANISH_THRESHOLD_BOOST_7_15` | 1 | 1 | WX09-027 | あなたの場にある他のシグニの「パワー7000以下のシグニ1体をバニッシュする」が「パワー15000以下のシグニ1体をバニッシュする」になる |
| `BATTLE_BANISH_LIFE_BURST` | 1 | 1 | WXEX2-40 | バトルバニッシュ後に相手側LBを発動 |
| `BATTLE_LEAVE_REPLACE_DOWN_TRASH_UNDER_ENERGY` | 1 | 1 | WXDi-P06-034 | バニッシュされる代わりに、アップ状態のこのシグニをダウンし、このシグニの下からカード1枚とあなたのエナゾーンからカード1枚をトラッシュに置く |
| `BATTLE_LEAVE_REPLACE_WITH_DOWN` | 1 | 1 | WXDi-CP02-TK01A | 「アップ状態のこのシグニが**バトルか対戦相手の効果によって** |
| `BATTLE_LEAVE_REPLACE_WITH_EXILE` | 1 | 1 | WXK05-024 | 「このシグニが場を離れる場合、代わりにこのシグニを |
...
| `BOUNCE_SIGNI_ZONE_CARD` | 1 | 1 | WXK08-024 | 🆕§5.3 `O-313`（2026-09-12）＝**シグニゾーンにある「カード」1枚を手札に戻す**   （`WXK08-024-E2`「各ターン終了時、対戦相手のシグニゾーンからカード１枚を対象とし、それを手札に戻してもよい」）。… |
...
| `CHECK_ZONE_FLIP_FREE_GROW` | 1 | 1 | WXDi-P16-001A | チェックゾーンにあるこのカードを裏返し、コストを支払わずにグロウする |
| `CHECK_ZONE_FREE_CAST` | 1 | 1 | WXDi-P10-007 | ── CHECK_ZONE_FREE_CAST（§5.3 `O-244`・2026-09-04）── 「デッキの上からN枚見る。その中から**コストの合計がM以下になるように**スペルをK枚まで**チェックゾーンに置き**、   残りをデ… |
| `COIN_ABILITY_BOOST` | 1 | 1 | SPK06-01 | 🆕COIN_ABILITY_BOOST: 「このゲームの間、あなたの＜レイラ＞が持つコイン技の《ゲーム１回》を   《ゲーム２回》にし、あなたが次に使用するコイン技の使用コストは《コイン×1》減る」   （§5.3 `O-259` 第1… |
...
| `COUNTER_TEAM_PIECE_AND_EXILE` | 1 | 1 | WXDi-P05-006 | 【使用条件】【チーム】を持つ対戦相手のピース1枚を対象とし、それの効果を打ち消す（打ち消されたピースはゲームから除外される） |
| `CROSS_ZONE_TRIPLE_TARGET_TO_DECK_BOTTOM` | 1 | 1 | WX21-028 | ── CROSS_ZONE_TRIPLE_TARGET_TO_DECK_BOTTOM（§5.3 `O-243`・2026-09-04）── 「**対戦相手の、シグニ1体とエナゾーンにあるカード1枚とトラッシュにあるカード1枚**を対象とし… |
...
| `ENERGY_COLOR_SUBSTITUTE_TRASH` | 1 | 1 | WXK07-005 | エナコストを支払う代わりに、エナゾーンからカードをトラッシュに置いて支払う |
| `ENERGY_NON_COLORLESS_ALL_COLORS` | 1 | 1 | WX14-017 |  |
| `ENERGY_SUBSTITUTE_TRASH_KEY` | 1 | 1 | WXK02-023 |  |
| `ENERGY_SUBSTITUTE_TRASH_SIGNI` | 1 | 1 | WX16-Re06 | エナコストを支払う代わりに、エナゾーンからシグニをトラッシュに置いて支払う |
...
| `FIRST_SPELL_COST_UP` | 1 | 1 | WXDi-P13-072 | 各ターン、対戦相手が最初に使用するスペルの使用コストが《無×1》増える |
...
| `GRANT_NEXT_SPELL_UNCOUNTERABLE` | 1 | 1 | WX04-008 | 次にあなたが使用するスペルは対戦相手の効果によって打ち消されない |
...
| `GROW_BY_EFFECT_SUPPRESS_ON_PLAY` | 1 | 1 | SP38-001 | GROW_BY_EFFECT_SUPPRESS_ON_PLAY（§5.3 `O-83`）＝「この方法でグロウしたルリグの【出】能力は発動しない」。 ⚠**直前の `GROW_BY_EFFECT` の予約へ載せる**＝ターン全体のフラグ（`… |
| `GROW_FROM_LEVEL0` | 1 | 1 | PR-469 |  |
| `HASTARLIQ` | 1 | 1 | WXDi-P05-016 | 対戦相手のシグニゾーン1つに【ハスターリク】1つを置く |
...
| `INTERNAL_ACCE_PICKED_TO_SELF` | 1 | 1 | WDK07-E15 | この方法で公開したカードをこのシグニの【アクセ】にする |
| `INTERNAL_ARTS_RECYCLE_EXECUTE` | 1 | 1 | SP26-007 | アーツをルリグトラッシュからルリグデッキへ回収実行 |
| `INTERNAL_KIYOHIME_CHOOSE` | 1 | 1 | WDK08-L14 | 以下の3つから1つを選ぶ（血晶武装状態なら代わりに3つまで選び、同じ選択肢を2回以上選んでもよい）。①ターン終了時まで対戦相手のすべてのシグニのパワーを－1000する ②対戦相手のシグニ1体を対象とし、それのパワーが4000以下の場合それ… |
...
| `LOOK_TOP_OPP_CHOOSE_TRASH` | 1 | 1 | WXK11-064 | 公開したカードから相手がN枚選んでトラッシュ、残りは手札へ |
| `LOSE_SIGNI_BARRIER` | 1 | 1 | WX24-P1-043 | 対戦相手は【○バリア】Nつを失う |
| `LRIG_ALL_NAMES` | 1 | 1 | WX25-P3-037 | ルリグシステム（未実装残） |
| `LRIG_ATTACK_LIMIT` | 1 | 1 | WXDi-D04-011 | 🏁**§5.3 `O-246`（2026-09-04）＝実装した。**   🔴旧 `GRID_REVEAL_PLUS` は `grid_reveal_plus_one_this_turn` を立てるだけで**読み手が1人もいなかった*… |
| `LRIG_TRASH_TO_UNDER_AND_RETURN_ARTS` | 1 | 1 | WXEX2-84 | あなたのルリグトラッシュからすべてのルリグをこのカードの下に置き、対象のアーツをルリグデッキに加える |
| `MAGIC_BOX_REVEAL` | 1 | 1 | WX24-P3-018 | 場のMBを表向きにしてシグニにする。 |
| `MARK_PLACED_DELAYED_EXILE` | 1 | 1 | WXDi-P13-004A | この方法で場に出したカードが場を離れる場合、代わりにゲームから除外する |
...
| `OPP_FIELD_OR_ENERGY_PER_COLOR_TO_HAND` | 1 | 1 | WX24-P4-022 | 対戦相手のシグニゾーンかエナゾーンから、色ごとにカードを1枚まで対象とし手札に戻す |
| `OPP_HAND_BLIND_LOOK_TO_DECK_BOTTOM` | 1 | 1 | WXDi-P00-037 | 対戦相手の手札をN枚まで見ないで選び、それらを見て1枚を対戦相手のデッキの一番下に置く |
| `OPP_LRIG_ATTACK_COST` | 1 | 1 | WX25-P2-014 | 対戦相手は《無》を支払わないかぎりルリグでアタックできない |
...
| `OPP_SPLIT_HAND_TWO_PILES` | 1 | 1 | WX25-P2-022 | 対戦相手は手札を裏向きで2つの束に分け、あなたが選んだ束を対戦相手が捨てる |
| `OPP_TRASH_LOSE_COLOR_AND_CLASS` | 1 | 1 | WXK11-026 | 対戦相手のトラッシュにあるカードは色とクラスを失う |
| `OPP_TRASH_TO_DECK_TOP_OPP_ORDERS` | 1 | 1 | WXK06-028 | 対戦相手のトラッシュからカードをN枚まで対象としデッキの一番上に置く（置く順番は対戦相手が非公開で選ぶ） |
| `OPP_TURN_NO_ENERGY_COST` | 1 | 1 | WXDi-P03-012 | このターン、対戦相手はエナコストを支払えない |
| `OPP_ZONE_PLACEMENT_RESTRICT` | 1 | 1 | WXDi-P14-068 | CONTINUOUS効果（effectEngineで動的判定） |
| `OPPONENT_OPTIONAL_ENERGY_CHARGE` | 1 | 1 | WXDi-P06-011 | 対戦相手は【エナチャージ】をしてもよい（するかどうかは対戦相手が選ぶ） |
| `OPTIONAL_HAND_REVEAL_NAMED` | 1 | 1 | WX05-038 | 名称指定で手札カードを任意公開 |
| `OPTIONAL_SELF_MILL_TOP` | 1 | 1 | WX24-P4-085 | あなたのデッキの一番上のカードをトラッシュに置いてもよい |
| `PLACE_CHOKKIN` | 1 | 1 | WX17-034 | このシグニがあるシグニゾーンに【貯菌】カウンターを1つ置く |
| `PLACE_DECK_TOP_UNDER_WEAPON_SIGNI` | 1 | 1 | WXK08-088 | ウェポンシグニの下にデッキ上を置く |
| `PLACE_LOOKED_CARD_UNDER_SIGNI` | 1 | 1 | WXK08-084 | あなたの対象のシグニ1体を選び、デッキの上から2枚のうち1枚をその下に置き、残りをデッキの一番下に置く |
| `PLACE_TRASH_SIGNI_UNDER_ALL_WEAPON` | 1 | 1 | WXK08-030 | 全ウェポンシグニの下にトラッシュからシグニを1枚ずつ置く |
| `PRDI035_PARADISE_COLOR` | 1 | 1 | PR-Di035 | 次のアタックフェイズ開始時判定フラグをセット。 |
...
| `RECORD_ON_PLAY_CHOSEN_SIGNI` | 1 | 1 | WXDi-P10-052 | 【出】で選んだシグニを効果元ごとに記録する（「このシグニの【出】能力で選んだシグニ」の受け皿） |
| `REDIRECT_ATTACK_TO_SELF_ZONE` | 1 | 1 | WXDi-CP02-TK01A | 相手シグニの直接アタックをこのシグニゾーンにリダイレクト（BattleScreen側処理） |
| `REDUCE_OPP_HAND_LIMIT` | 1 | 1 | WDK09-009 | 手札上限増加（CONTINUOUS：シグニがフィールドにある間） 手札上限を増やす / REDUCE_OPP_HAND_LIMIT: 相手の手札上限を減らす 🆕**§5.3 `O-60` 第19バッチ（2026-09-03）＝ここは【常… |
...
| `RETURN_SELF_SPELL_TO_HAND` | 1 | 1 | WXK08-040 | このスペルを手札に戻す |
| `RETURN_TO_HAND_AT_TURN_END` | 1 | 1 | WXK08-002 | ターン終了時、それを場から手札に戻す |
| `REVEAL_COUNT_PLUS_ONE_OPTIONAL` | 1 | 1 | WX06-033 |  |
| `REVEAL_DECK_TOP_AND_REDUCE_LRIG_ATTACK_LIMIT` | 1 | 1 | WXDi-D04-011 | 🆕§5.3 `O-236`＝デッキの一番上を公開し、そのレベルに応じて上限を減らす（`WXDi-D04-011-E1` の【自】）。   ⚠**一致しないレベルなら何も減らさない**（原文はレベル1と2しか書いていない＝過剰に減らさない… |
...
| `SELF_SIGNI_COLOR_TO_DECLARED` | 1 | 1 | WX22-042 | このシグニは色を失い、宣言した色を得る（ターン終了時まで） |
...
| `SUPPRESS_OPP_SIGNI_TRIGGERS_THIS_TURN` | 1 | 1 | SP26-002 | §5.3 O-337：対戦相手の全領域のシグニトリガーを、LB 以外このターン抑止する。 |
| `SWAP_DECK_TOP_WITH_SELF_IN_ENERGY` | 1 | 1 | WXDi-P10-047 | DEFERRED_SWAP_OPP_LIFE_TOP_AND_DECK_TOP / DEFERRED_SWAP_DECK_TOP_WITH_SELF_IN_ENERGY:   **2つのゾーンの一番上を入れ替える**（**機構が無い**・… |
...
| `TRASH_SIGNI_ZONE_NON_SIGNI` | 1 | 1 | WXK07-003 | 🆕§5.3 `O-313`（2026-09-12）＝**シグニゾーン1つの「シグニではないカード」全部をトラッシュへ**   （`WXK07-003-E1`「対戦相手のシグニゾーン１つにある、シグニではないすべてのカードをトラッシュに置… |
...
| `TREAT_SELF_AS_RESONA` | 1 | 1 | WX25-P2-052 | 一致させる（非対称の緩和が既存）ので、差し替えるだけで両方に当たる。 ⚠**期間は「次の対戦相手のターン終了時まで」**＝ストア名の `_until_opp_turn` が   `clearUntilOppTurnEffects` の解除… |
| `TRIGGER_OTHER_SIGNI_EICHI_ABILITY` | 1 | 1 | PR-366 | 他の自シグニを選択し、その英知AUTO能力を発動させる |
| `USE_CONDITION_ARTS_USED` | 1 | 1 | WD15-006 | このターンにアーツを使用していた場合、このカードは使用不可 |
| `WHITE_SIGNI_ABILITY_PROTECT` | 1 | 1 | WXDi-P15-085 |  |


---

## 付録: 内部/動的生成 STUB（JSON 0 件・ハンドラのみ 370 種）
...
| `ACCE_FROM_TRASH` | 0 | 0 |  | トラッシュのアクセカードを自分のシグニに付ける |
| `ACCE_SIGNI_GRANT_ABILITY` | 0 | 0 |  | アクセ中のシグニにキーワード能力を付与 |
| `ACCE_TO_ENERGY` | 0 | 0 |  | アクセカードをエナゾーンへ |
...
| `BANISH` | 0 | 0 |  | この方法で処理したカード（無ければこのカード自身）をバニッシュする |
| `BANISH_BY_SELF_GOES_TO_TRASH` | 0 | 0 |  | このシグニによるバニッシュはエナでなくトラッシュへ |
...
| `CHOOSE_HAND_CARD` | 0 | 0 |  | 手札から1枚選択（lastProcessedCardsに設定） |
| `CHOSEN_TO_ENERGY_OR_HAND` | 0 | 0 |  | 選んだカードをエナか手札か選択して追加 |
| `CONDITIONAL_ADD_HAND` | 0 | 0 |  | フィールドにシグニがあれば手札に1枚追加 |
| `CONDITIONAL_SEARCH_IF_FIELD` | 0 | 0 |  | フィールドにシグニがある場合サーチ |
| `CONDITIONAL_SEARCH_IF_RESONA` | 0 | 0 |  | フィールドにレゾナがある場合サーチ |
| `DECK_REVEAL_UNTIL_CLASS` | 0 | 0 |  | デッキを条件が満たされるまで公開する 🆕**§5.3 `O-60` 第75バッチ（2026-09-05）＝停止条件も公開札の行き先も payload（`deckRevealUntil`）で受け取る。** 🔴旧実装は効果元の **カード… |
| `DECLARE_AND_MILL` | 0 | 0 |  | effects.jsonではDECLARE_NUMBER+MILL(useDeclaredCount)に移行済み |
| `DECLARE_NUMBER_POWER` | 0 | 0 |  | パワーの値1つを宣言する |
| `DEFERRED_GRANT_QUOTED_ACTIVATE_ABILITY` | 0 | 0 |  | 引用された【起】能力をプレイヤーが得る（**機構が無い**）。 |
| `DEFERRED_TRASH_ACCE_TO_ENERGY_IF_BANISHED_SOURCE_WAS_ACCED` | 0 | 0 |  | バニッシュ時、トラッシュの《アクセアイコン》を持つシグニ1枚を対象とし、効果元がアクセされていた場合だけエナゾーンに置く（除去直前状態の解決時持ち回りは未実装） |
...
| `EXILE_FROM_CHECK_ZONE` | 0 | 0 |  | この効果が使わせたカードは、チェックゾーンから移動する代わりにゲームから除外される |
...
| `GRANT_CONDITIONAL_ASSASSIN_ABILITY` | 0 | 0 |  | 対象のシグニは「正面に凍結状態のシグニがあるかぎり【アサシン】を得る」を得る |
| `GRANT_LRIG_ABILITY` | 0 | 0 |  | あなたのルリグは指定された能力を得る |
...
| `GROW_COST_ZERO` | 0 | 0 |  | このターン、グロウコストを支払わずにグロウできるようになる |
...
| `INTERNAL_ACCE_ATTACH_TO_ZONE` | 0 | 0 |  |  |
| `INTERNAL_ACCE_PICK_HOST` | 0 | 0 |  |  |
...
| `INTERNAL_CZFC_REMOVE_CHECK` | 0 | 0 |  | 使用する1枚をチェックゾーンから外す（§5.3 `O-244`）。 |
| `INTERNAL_CZT_PICKED` | 0 | 0 |  | `CROSS_ZONE_TRIPLE_TARGET_TO_DECK_BOTTOM` の各段の確定（選んだ1枚を積んで再入）。 |
...
| `INTERNAL_DIRC_DECLARE_ICON` | 0 | 0 |  | ═══ DECLARE_ICON_REVEAL_CHECK（§5.3 `O-163`・2026-09-02 索引 B 第2巡）═══ 「あなたの手札を１枚選ぶ。対戦相手は《白》〜《無》から１つ（と、シグニかスペルから１つ）を宣言する。  … |
| `INTERNAL_DIRC_DECLARE_TYPE` | 0 | 0 |  |  |
| `INTERNAL_DIRC_RESOLVE` | 0 | 0 |  |  |
...
| `INTERNAL_DO_COLLAB` | 0 | 0 |  | 「コラボライバーN人とコラボする」＝ライバートークンをN個取り除く（§5.3 `O-292`）。 |
| `INTERNAL_DOWN_AND_FREEZE_OPP` | 0 | 0 |  | 相手シグニ1体をダウン+全シグニを凍結 |
| `INTERNAL_DOWN_SELECTED_SIGNI` | 0 | 0 |  | 選ばれたシグニをまとめてダウンする（ダウンした分を lastProcessedCards に残す） |
...
| `INTERNAL_GRANT_TRASH_SPELL_USE` | 0 | 0 |  | ── INTERNAL_GRANT_TRASH_SPELL_USE（§5.3 `O-185`・2026-09-04）── 「このターン、あなたは**それら**を使用してもよい」＝**選んだカードを動かさずに、そのターンだけ使用可能にする*… |
| `INTERNAL_HAND_OR_ENERGY` | 0 | 0 |  | 「それぞれ手札に加えるかエナゾーンに置き」を1枚ずつ問うチェーン（タスク12(xlvi)(h)）。 |
| `INTERNAL_HAND_PILE_DISCARD` | 0 | 0 |  |  |
| `INTERNAL_HAND_PILES_CHOOSE` | 0 | 0 |  |  |
| `INTERNAL_HAND_TO_DECK_BOTTOM` | 0 | 0 |  |  |
| `INTERNAL_HAND_TO_ENERGY` | 0 | 0 |  | 手札から任意でエナゾーンに置くは §5.3 `O-60` 第15バッチで既存の `ENERGY_CHARGE{target:{HAND_CARD, upToCount}}` へ移した。 旧 `STUB{HAND_TO_ENERGY_OPT… |
...
| `INTERNAL_HSUS_PICK_HAND` | 0 | 0 |  | `HAND_SIGNI_UNDER_SIGNI` の2段目＝置き先（`stub.value`）を焼き込んで手札を選ばせる。 |
...
| `INTERNAL_LOOKED_CARD_UNDER_APPLY` | 0 | 0 |  |  |
| `INTERNAL_LRIG_PILE_PICK_ARTS` | 0 | 0 |  | 選んだ束を見て、その中からアーツ1枚を対戦相手のルリグトラッシュに置く（O-307 の3段目） |
| `INTERNAL_LRIG_PILES_CHOOSE` | 0 | 0 |  | 対戦相手が分けた2つの束のどちらを見るかを効果使用者が選ぶ（O-307 の2段目） |
| `INTERNAL_LRIG_UNDER_TRASH_SELECTED` | 0 | 0 |  |  |
| `INTERNAL_LTOCT_APPLY` | 0 | 0 |  | 公開カードのうち相手が選んだ分をトラッシュへ、残りを手札へ |
| `INTERNAL_MAGIC_BOX_REVEAL` | 0 | 0 |  | 旧payload無しは従来互換で全MBを処理。内部actionは選ばれたゾーンだけを処理する。 |
| `INTERNAL_MARK_CHOICE_TAKEN` | 0 | 0 |  | `CHOOSE{noRepeat}`（「まだ選んでいないもの１つを選ぶ」）で |
...
| `INTERNAL_MOVE_TO_BEAT` | 0 | 0 |  | 選択シグニをビートゾーンへ移動（TRASH_SIGNI_TO_BEAT の後段） |
...
| `INTERNAL_OPP_LRIG_UNDER_TRASHED` | 0 | 0 |  | 上で選んだ相手ルリグ下のカードを、相手のルリグトラッシュへ移す。 |
| `INTERNAL_OPP_ORDERS_DECK_TOP` | 0 | 0 |  | 選ばれた相手トラッシュの札の置く順番を対戦相手に選ばせる（2枚のときだけ対話） |
...
| `INTERNAL_RETURN_ACCED_CARDS_TO_HAND` | 0 | 0 |  |  |
...
| `INTERNAL_SELF_MILL_TOP_APPLY` | 0 | 0 |  |  |
| `INTERNAL_SELF_MILL_TOP_SKIP` | 0 | 0 |  |  |
...
| `INTERNAL_SIGNI_ZONE_CARD_TO_HAND` | 0 | 0 |  |  |
...
| `INTERNAL_TRASH_FIELD_TRAP_COST` | 0 | 0 |  | 候補提示／INTERNAL_TRASH_SELECTED_FIELD_TRAP_COST: 選択済み1枚の移動。 |
...
| `INTERNAL_TRASH_SELECTED_FIELD_TRAP_COST` | 0 | 0 |  |  |
| `INTERNAL_TRASH_SELECTED_TRAP` | 0 | 0 |  |  |
...
| `OPP_DECK_REVEAL_UNTIL` | 0 | 0 |  | デッキを条件が満たされるまで公開する 🆕**§5.3 `O-60` 第75バッチ（2026-09-05）＝停止条件も公開札の行き先も payload（`deckRevealUntil`）で受け取る。** 🔴旧実装は効果元の **カード… |
| `OPP_DECLARE_CHOICE` | 0 | 0 |  | 対戦相手が《色》を宣言する（ウリス系のエクシード）。 |
...
| `OPP_MAIN_PHASE_LIMIT_DOWN` | 0 | 0 |  | 次の相手メインフェイズの間、センタールリグのリミット-2 |
...
| `PLAY_SPELL_FROM_HAND` | 0 | 0 |  |  |
| `PLAY_SPELL_FROM_HAND_FREE` | 0 | 0 |  |  |
...
| `PREVENT_ALL_SIGNI_POWER_MINUS_BY_OPP` | 0 | 0 |  | 全シグニの相手パワーマイナス防止（effectEngineで動的処理） |
...
| `REDUCE_LRIG_ATTACK_LIMIT` | 0 | 0 |  |  |
| `REMOVE_MIKO_KEYWORD` | 0 | 0 |  | みこみこ親衛隊キーワードをsourceCardNumのシグニのkeyword_grantsから取り除く（WX25-P3-TK03） |
| `REMOVE_OPP_MULTI_ENA` | 0 | 0 |  | OPP_TRASH_TO_DECK_TOP は line 1211 の handler で処理済み（dead code 削除） この2 id は parser から生成されず、生成されれば原文の「能力を失う」と食い違ってエナをトラッシュす… |
| `REMOVE_OPP_MULTI_ENA_ONLY` | 0 | 0 |  | OPP_TRASH_TO_DECK_TOP は line 1211 の handler で処理済み（dead code 削除） この2 id は parser から生成されず、生成されれば原文の「能力を失う」と食い違ってエナをトラッシュす… |
...
| `SELECT_NO_COMMON_COLOR` | 0 | 0 |  | 共通色なしを選択（ログのみ） WX22-050 エンジェル・アウェイク |
| `SELECT_OTHER_SIGNI` | 0 | 0 |  | ソース以外のシグニを選択 |
...
| `SHUFFLE_DECK_POWER_HALF` | 0 | 0 |  | シャッフル後に全シグニのパワーを半減 |
| `SHUFFLE_REMAINDER_INTO_DECK` | 0 | 0 |  | 「残りをデッキに加えてシャッフルする」 |
| `SIGNI_GRANT_QUOTED_CONSTANT_ABILITY` | 0 | 0 |  | 引用【常】能力の付与（**構造化できなかったときの残余**）。 |
...
| `SUMMON_FROM_TRASH_TO_HAND_BLACK` | 0 | 0 |  | あなたのトラッシュから黒のシグニ1枚を手札に加える |
| `SUPPRESS_OPP_SIGNI_ABILITIES` | 0 | 0 |  | 相手フィールドの全シグニの能力を消去 |
| `SWAP_OPTIONAL` | 0 | 0 |  |  |
| `TARGET_AND_DISCARD_HAND` | 0 | 0 |  | 手札を捨てて対戦相手シグニを対象とする効果（スタンドアロン時：手札1枚捨て+相手シグニをlastProcessedCardsへ） |
| `TK3_DISCARD_BY_LEVEL` | 0 | 0 |  | REVEAL_CARDS 確認後、宣言レベルのシグニを相手手札からすべて捨てさせる |
| `TRADE_BANISH_SELF_SIGNI` | 0 | 0 |  | トレード：自シグニ1体をトラッシュに置き、相手シグニ1体をバニッシュ |
...
| `UNKNOWN_NESTED` | 0 | 0 |  | 自シグニを任意でトラッシュに置く（そうした場合に後続効果が発動） |
| `USE_CONDITION_TEXT` | 0 | 0 |  | ゲームプレイに影響しない説明テキストは無音でスキップ。 ⚠`UNLIMITED_KEYS` は説明テキストではなく**【常】の実効果**だが、読み手は `BattleScreen` の   キーセット可否ゲートと配置先（`hasUnlim… |
| `USE_SPELL_FROM_TRASH` | 0 | 0 |  |  |
