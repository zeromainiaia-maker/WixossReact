...
## サマリー（最終生成: 2026-07-10）

| 区分 | 値 |
|---|---:|
| JSON で使用中の STUB id 種類 | 555 |
| 　└ ハンドラ実装あり | 537 |
| 　└ フォールバック（execStub 未処理） | 18 |
| 総 STUB ノード件数 | 2403 |
...
| `GRANT_TO_PLACED_SIGNI` | 2 | 2 | WX24-P1-017, WX25-P3-038 |  |
...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 437 | 424 | WD06-001, PR-427, WD10-009 | OPTIONAL_COST: 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） 主な338件はeffectExecutor.tsがSTUB→CONDITIONAL(IS_MY_TURN)パター… |
| `ARTS_COST_REDUCTION_BY_EFFECT` | 115 | 113 | WD10-006, WD12-006, WD15-006 | アーツコスト軽減マーカー（コストはBattleScreen使用時に算出済み） |
| `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` | 66 | 65 | WD20-001, WDK05-T11, WDK08-Y11 | 他の任意コスト系（SEQUENCEパターン外のフォールバック） |
...
| `GRANT_QUOTED_AUTO_ABILITY` | 22 | 22 | WD21-007, PR-K076, SPDi43-05 | WD21-007型: 「以下の５つから１つを選ぶ。…対象のシグニ１体は選んだ能力を得る。あなたがベットしていた場合、この効果を１回繰り返す。」 |
...
| `DEPLOY_RESTRICT` | 8 | 8 | WX07-006, WX12-008, WXDi-P05-024 | DEPLOY_RESTRICT: 配置制限（CONTINUOUSは動的処理、AUTOはフラグ設置） |
