...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 602 | 564 | WD10-009, WD12-009, WD13-003 | 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） |
...
| `COPY_LRIG_NAME_ABILITY` | 16 | 16 | WX24-P4-011, WX24-P4-012, WX24-P4-013 | ルリグトラッシュのルリグ名/タイプを現在のルリグに追加 |
...
| `STEAL_OPP_TRASH_PUPPET` | 5 | 4 | WDK17-007, WXEX2-23, WXK09-063 | 対戦相手のトラッシュからシグニを傀儡状態であなたの場に出す（WDK17-007）。 |
...
| `GRANT_QUOTED_ACTIVATE_ABILITY` | 2 | 2 | WXDi-P09-066, WXK08-078 | 「【起】...」付与（effectEngineのCONTINUOUS処理で対応） |
...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `TRAP_OPERATION` | 19 | 14 | SP26-001, WX15-047, WX15-053 | トラップ/チェックゾーン操作の統合ハンドラ |
| `DESIGNATE_SIGNI_ZONE` | 14 | 14 | WDK10-009, WX08-021, WX10-051 | シグニゾーンを1つ指定する。 |
| `PLACE_SEED_FROM_REVEALED` | 8 | 8 | WDK07-Y02, WDK07-Y03, WDK07-Y04 | デッキ上4枚を見て1枚を【シード】として設置 |
...
| `TRAP_OP` | 7 | 7 | WX16-061, WX17-044, WX17-062 | ソースカードのテキストに応じて操作判定 |
...
| `PLACE_SEEDS_FROM_REVEALED` | 1 | 1 | WXK04-010 | デッキ上4枚を見て stub.value 枚まで【シード】設置（WXK04-010 アンコール・シード）。 |
| `PLACE_TRAP_FROM_REVEALED` | 1 | 1 | WXEX1-13 | 前のLOOK_AND_REORDERで公開されたデッキ上N枚からトラップ設置 |
...
| `EFFECT_LIMIT` | 4 | 4 | WDK06-C17, WX13-053, WX21-066 | 連続効果の上限枚数をキャップ（直前のパワー修正を上限値でキャップ） |
...
| `CHOOSE_COLOR_FROM_LIST` | 1 | 1 | WX10-025 | エナゾーンの色から選ぶ（最大N色）→ selectedColors に保存 |
...
| `DECLARE_NUMBER_POWER` | 1 | 1 | WXDi-P07-086 | パワー値宣言（3000〜15000）→ declared_guard_restrict_level に保存 |
...
| `CHOOSE_TRAP_ZONE` | 0 | 0 |  | 選択済みカードのゾーン選択 |
...
| `INTERNAL_CBDOP_AFTER_DISCARD` | 0 | 0 |  | 手札捨て後の効果（COUNT_BASED_DRAW_OR_POWER から継続） |
...
| `INTERNAL_DISCARD_LRIG_DECK_ARTS` | 0 | 0 |  | ルリグデッキからアーツをルリグトラッシュへ（CHOOSEの続き） |
...
| `INTERNAL_HAND_TO_ENERGY` | 0 | 0 |  | lastProcessedCardsの手札カードをエナへ移動 |
...
| `INTERNAL_SET_TRAP` | 0 | 0 |  | ゾーン番号をstub.valueで受け取りトラップ設置 |
