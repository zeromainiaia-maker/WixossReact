...
| 15 | `BET_ALTERNATIVE`誤分類 | ✅完了 | ✅ | parseSentencePart3.tsの判定順バグを修正（2026-06-16）。全14枚対処完了：単純スケール型4枚（WX17-005/WD19-006/WD19-007/WXK07-106）、BET_MECHANIC化11枚（WX18-003/WX18-005/WX15-029/WX16-005/WX17-003/WX19-005/WX19-006/WXK04-014/WDK05-T10/PR-K072/WDK12-007/WDK06-R08/SPK16-13E）、BET_ALTERNATIVE除去1枚（WXDi-P07-059）。BET_ALTERNATIVE 0件達成（v0.319）。WDK06-R08①のパワー比較とSPK16-13E①②③の条件チェックは近似実装（#13参照）。**2026-06-17追記**: BET_MECHANIC化により、アーツ使用モーダル側の既存ベットトグル（`parseBetCost`/`is_betting_this_effect`、BET_CONDITIONと共通）と、BET_MECHANIC独自の「ベットしますか？」プロンプト（`INTERNAL_BET_SHOW_4`、コイン1枚固定で実コスト無視）が二重に動く実バグを発見・修正。BET_MECHANICはモーダルの`is_betting_this_effect`を読むだけにし、独自プロンプト・独自コイン消費を撤去。スペル型のWXDi-P07-059はモーダル側にベットUIが無い（executeArts専用）ため、ベット強化（②付与）は依然到達不能。**さらに追記**: WXDi-P07-059を精査中、`keyword_grants`へのキーワード付与が`hasGrantedKeyword`/`hasKeyword`（utils/keywords.ts）の日本語正式名照合と食い違う英語短縮コード（'shadow'/'lancer'/'assassin'/'double_crush'）で書き込まれており**常時非発火**だった実バグを発見（影響: `GRANT_ABILITY_UNTIL_OPP_TURN`＝WXDi-P07-059、`SIGNI_GRANT_QUOTED_CONSTANT_ABILITY`＝WXDi-P10-025/WXDi-P14-008、`choiceTextParser.ts`のGRANT_KEYWORD分岐＝今後BET_MECHANIC等で①②③に「ランサー/アサシン/ダブルクラッシュを得る」を含むカード全般）。日本語名に統一して修正。あわせてWXDi-P07-059はGRANT_ABILITY_UNTIL_OPP_TURNの付与先が`ctx.sourceCardNum`（スペル自身、的外れ）固定だった実バグも発見、`TARGET_ONLY`ステップを追加し`lastProcessedCards`優先に修正（「あなたのシグニ１体を対象とし」を正しく反映）。なお①②の差分（シャドウの対象スコープ宣言色のシグニ限定/全体）はシャドウ自体が現状色スコープ非対応の単純boolean実装のため、ベットの有無に関わらず効果上の差は無い（別系統の機能ギャップ、未着手）。`WX20-056`（RISE_TARGET_SIGNI_GAIN_CONSTANT_ABILITY使用）はそもそも別能力（手札に戻らずダウンせず新たに能力を得られない）でこの5キーワード分岐に一致せず元から未対応、本ラウンドの対象外。 |
