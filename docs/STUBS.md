...
## サマリー（最終生成: 2026-06-19）

| 区分 | 値 |
|---|---:|
| JSON で使用中の STUB id 種類 | 537 |
| 　└ ハンドラ実装あり | 530 |
| 　└ フォールバック（execStub 未処理） | 7 |
| 総 STUB ノード件数 | 2303 |
| JSON 0 件・ハンドラのみ（内部/動的生成 STUB） | 235 |
...
| `EXCEED_OPP_LRIG_TRASH_2` | 1 | 1 | WD23-012-A |  |
| `GRANT_ALL_ZONE_LIFEBURST` | 1 | 1 | WD14-001 |  |
| `NO_BATTLE_DEFENDER` | 1 | 1 | WX25-P1-TK6 |  |
| `SEARCH_ANGEL_TO_FIELD` | 1 | 1 | WX22-005 |  |
| `TREAT_AS_LEVEL1_IN_DECK_TRASH` | 1 | 1 | WXDi-P05-086 |  |
| `UNIMPL_GRANTED_ABILITY` | 1 | 1 | WX09-027 |  |

---

## 実装済み STUB（ハンドラ別）

### execStubPart1.ts（94 種）

| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 412 | 401 | WD06-001, PR-427, WD10-009 | OPTIONAL_COST: 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） 主な338件はeffectExecutor.tsがSTUB→CONDITIONAL(IS_MY_TURN)パター… |
| `ARTS_COST_REDUCTION_BY_EFFECT` | 115 | 113 | WD10-006, WD12-006, WD15-006 | アーツコスト軽減マーカー（コストはBattleScreen使用時に算出済み） |
...
| `CONDITIONAL_POWER_BONUS` | 44 | 39 | WD06-006, WDK08-Y08, WDK10-009 | 条件付きパワーボーナス |
| `RULE_REMINDER_TEXT` | 43 | 42 | SP26-003, SP38-001, PR-K026 | ゲームプレイに影響しない説明テキストは無音でスキップ |
| `GRANT_QUOTED_AUTO_ABILITY` | 42 | 40 | WD13-002, WD21-007, WD22-007-G | WD21-007型: 「以下の５つから１つを選ぶ。…対象のシグニ１体は選んだ能力を得る。あなたがベットしていた場合、この効果を１回繰り返す。」 |
| `DECLARE_NUMBER` | 36 | 33 | WD03-006, WD06-008, WD13-008 | 数字宣言：現在はランダム値で代用 |
...
| `LOOK_OPP_LIFE_TOP` | 29 | 28 | WD06-006, WD06-018, WDK09-017 | 対戦相手のライフクロス上を見る（複数枚パターン対応） |
| `TRADE_BANISH_SELF_SIGNI` | 29 | 29 | WD07-012, WD22-029-G, WDK08-Y01 | トレード：自シグニ1体をトラッシュに置き、相手シグニ1体をバニッシュ |
...
| `GRANT_ABILITY_INNER_TEXT` | 21 | 20 | SPDi43-01, SPDi44-04, WX07-065 |  |
...
| `GRANT_QUOTED_ABILITY` | 17 | 16 | SPK01-11, WX22-Re04, WX24-P1-042 | 引用符付き能力付与（キーワード → keyword_grants、複合能力 → granted_effects） |
| `TARGET_ONLY` | 17 | 17 | SPDi37-06, PR-Di017B, WX24-P4-075 | ターゲット選択のみ（lastProcessedCards に格納し後続ステップへ） |
...
| `DECLARE_CARD_NAME` | 16 | 16 | PR-K046, WX11-037, WX13-048 | カード名宣言（手札のカード名から選択） |
...
| `DECK_REVEAL_UNTIL` | 9 | 9 | WDK04-006, WX04-093, WX17-039 | デッキを条件が満たされるまで公開する |
...
| `DOUBLE_POWER_MINUS` | 7 | 7 | SPDi43-04, WX04-038, WX22-023 |  |
...
| `REMOVE_VIRUS` | 7 | 7 | WD19-001, WX15-028, WX15-040 | ウイルス除去：テキストを解析して適切な数のウイルスを取り除く |
...
| `SUPPRESS_LIFE_BURST_ON_CARD` | 3 | 3 | WX24-P1-003, WX25-P3-032, WXDi-D09-H11 | ライフバースト抑制：対戦相手の suppress_life_burst フラグをセット |
...
| `REMOVE_VIRUS_TARGET_ZONE` | 2 | 2 | WD19-007, WX15-064 | REMOVE_VIRUS_TARGET_ZONE: lastProcessedCards[0]と同じゾーンのウィルスを1個除去（WX15-064型） |
...
| `DECK_REVEAL_UNTIL_CLASS` | 1 | 1 | WX04-050 | デッキを条件が満たされるまで公開する |
...
| `OPP_DECK_REVEAL_UNTIL` | 1 | 1 | WX04-015 | デッキを条件が満たされるまで公開する |
...
| `TK3_DECLARE_DISCARD` | 1 | 1 | WX25-P1-TK3 | TK3_DECLARE_DISCARD: 数字を宣言し、対戦相手の手札から宣言レベルのシグニをすべて捨てさせる （WX25-P1-TK3 ダーク・アナライズ：「数字1つを宣言する。対戦相手の手札を見て、宣言した数字と同じレベルを持つすべて… |
| `TOP_TO_BOTTOM_OPTIONAL` | 1 | 1 | WXDi-P03-050 | デッキトップを見て下に置いてもよい |

### execStubPart2.ts（224 種）
...
| `PLAY_FREE` | 9 | 9 | PR-474, WX07-014, WX14-002 | フリープレイ系：lastProcessedCards[0] のカードをコストなしでプレイ |
...
| `CONDITIONAL_COST_REDUCTION_BY_FIELD` | 6 | 6 | WX10-031, WX12-049, WX15-034 | コスト軽減系（engine: コスト計算システム未実装） CONDITIONAL_COST_REDUCTION_BY_FIELD: フィールド条件（クラス/枚数）でコスト軽減チェック |
...
| `CONDITIONAL_CARD_COST_BY_OPP_LRIG` | 5 | 5 | WX03-002, WX03-003, WX03-004 | CONDITIONAL_CARD_COST_BY_OPP_LRIG: 対戦相手のルリグ属性によるコスト変更チェック |
| `CRAFT_TO_LRIG_DECK` | 5 | 5 | WXDi-P14-006, WXDi-P14-007, WXDi-P14-008 | === バッチ9: ルリグ・条件サーチ・選択系 === CRAFT_TO_LRIG_DECK / ADD_CRAFT_TO_LRIG_DECK: クラフトをルリグデッキへ |
...
| `PLAY_SPELL_FREE_IGNORE_RESTRICTION` | 4 | 3 | WX04-015, WX14-014, WXEX2-14 | PLAY_SPELL_FREE_IGNORE_RESTRICTION: 手札のスペルをコストなし・限定条件無視で使用 |
...
| `MAKE_SERVANT_ZERO` | 3 | 1 | WXDi-P11-031 | ALL_OPP_SIGNI_SERVANT_ZERO / MAKE_SERVANT_ZERO / MAKE_MULTI_SERVANT_ZERO / SIGNI_SERVANT_ZERO: 対象シグニをサーバントZERO（WXDi-P07… |
...
| `DECLARED_NAME_TO_SERVANT_ZERO` | 2 | 2 | WXEX2-10, WXK03-002 | DECLARED_NAME_TO_SERVANT_ZERO: declared_card_name と一致する相手のカードをサーバントZEROに（WXEX2-10） value:'field' 指定時は相手の「場」のみを対象にする（WXK… |
...
| `FROM_TRASH_TO_CENTER_ZONE` | 1 | 1 | WXDi-P03-087 | FROM_TRASH_TO_CENTER_ZONE: トラッシュからカードを中央シグニゾーン（zone[1]）に出す |
...
| `GRANT_LRIG_ABILITY` | 1 | 1 | PR-317 | 能力付与系（CONTINUOUS効果はeffectEngineで処理、AUTO/ACTIVATEDでも来た場合のフォールバック） GRANT_UNDER_SIGNI_*/GRANT_UNDER_LRIG_*/GRANT_LRIG_TRAS… |
...
| `PLACE_VIRUS_TO_2` | 1 | 1 | WX19-045 | PLACE_VIRUS_TO_2: 相手の場のウィルス合計が2になるようにウィルスを置く（WX19-045） |
...
| `REMOVE_MIKO_KEYWORD` | 1 | 1 | WX25-P3-TK03 | REMOVE_MIKO_KEYWORD: みこみこ親衛隊キーワードをsourceCardNumのシグニのkeyword_grantsから取り除く（WX25-P3-TK03） |
...
| `REVEALED_SIGNI_TO_FIELD_REST_TRASH` | 1 | 1 | WX04-093 | 公開したシグニをフィールドに出し、残りをトラッシュ |
...
| `CONDITIONAL_MULTI_CHOOSE_BY_CENTER` | 8 | 8 | WD22-011-G, WX11-023, WX12-005 | CONDITIONAL_MULTI_CHOOSE_BY_CENTER: センタールリグによる複数選択 |
...
| `CHOOSE_N_FROM_LIST` | 7 | 7 | WX13-003, WXEX2-44, WXDi-P05-052 | CHOOSE_N_FROM_LIST: 以下の①②③④からN個選択して実行 |
...
| `GAIN_SIGNI_BARRIER` | 7 | 7 | SPDi43-23, WX26-CP1-001, WXDi-P12-001 | GAIN_SIGNI_BARRIER: 【シグニバリア】を得る（フリーゾーンにトークンとして設置。相手シグニからのダメージ1回を無効） |
...
| `GATE` | 6 | 5 | WDK09-001, WDK09-006, WXDi-P15-010 | GATE: ゲート効果（ログのみ） GATE: 相手のシグニゾーン1つに【ゲート】を設置（次のアタックフェイズに条件付きでアタック不可） |
...
| `MOVE_TO_ATTACKER_FRONT` | 3 | 3 | WX04-029, WXDi-D06-012, WXDi-P02-052 | シグニ移動/リダイレクト系（engine: 移動先変更未実装） MOVE_TO_ATTACKER_FRONT: 相手シグニアタック時、正面が空なら自分をその正面に移動（してもよい） |
| `REPEAT_EFFECT` | 3 | 3 | WX04-093, WX16-042, WX22-016 | REPEAT_N_TIMES / REPEAT_EFFECT: 以下をN回繰り返す |
...
| `CONDITIONAL_ALTERNATE_EFFECT` | 2 | 2 | WD23-044-EA, WX06-024 | CONDITIONAL_ALTERNATE_EFFECT: 条件達成時にダウン済みシグニをトラッシュへ（代替効果） |
...
| `NEGATE_THAT_ATTACK` | 2 | 2 | WX04-004, WXEX2-17 | NEGATE_THAT_ATTACK: 現在のアタックを無効化 |
...
| `BANISH_IF_DISCARDED_3_THIS_TURN` | 1 | 1 | WXK03-021 | BANISH_IF_DISCARDED_3_THIS_TURN: このターン手札3枚以上捨てていればバニッシュ+相手エナトラッシュ（WXK03-021 ON_ATTACK_PHASE_START） |
...
| `BUFF_HOST_WHEN_PLACED_UNDER` | 1 | 1 | WXDi-P11-063 | BUFF_HOST_WHEN_PLACED_UNDER: このカードがシグニの下に置かれたとき上のシグニ+2000（WXDi-P11-063） |
| `CARDS_OUTSIDE_ENERGY_BECOME_WHITE` | 1 | 1 | WX08-005 |  |
| `CENTER_LRIG_COLOR_CHANGE_BLACK` | 1 | 1 | WXK03-006 |  |
| `CENTER_LRIG_DISMOUNT` | 1 | 1 | WXK03-036 | CENTER_LRIG_DISMOUNT: センタールリグがすべての乗機シグニから降りる（ドライブ解除・任意） |
...
| `COST_COLOR_SELECT` | 1 | 1 | WX04-063 | COST_COLOR_SELECT: 支払ったエナの色ごとに1色選択し、選択色のシグニをデッキから手札に加える |
...
| `DISONA_RESTRICTION` | 1 | 1 | WXDi-P12-075 | DISONA_RESTRICTION: 「このターン、あなたは《ディソナアイコン》ではないスペルを使用できない」 （使用条件側「すでに非ディソナスペルを使用していた場合は使用不可」はeffect.conditionで判定済み） |
...
| `EVDIVA_PER_LRIG_COLOR` | 1 | 1 | WX25-P3-050 | EVDIVA_PER_LRIG_COLOR: WX25-P3-050 エビディバ!!!!! 場の色別ルリグ数ぶんに各効果を行う。 白=【ルリグバリア】/青=ドロー3/緑=エナチャージ3/黒=相手デッキ10トラッシュ（決定的）。 赤=対象選… |
| `FIELD_COND_DRAW_REVEAL` | 1 | 1 | WX24-P3-022 | === バッチ15: 公開・アクセ応用・条件ドロー系 === FIELD_COND_DRAW_REVEAL: フィールド条件達成時にデッキ上を公開し同クラスなら手札へ |
...
| `GRANT_PRIOKE_PENDING_ATTACK_TRASH` | 1 | 1 | WX26-CP1-001 | GRANT_PRIOKE_PENDING_ATTACK_TRASH: FUTURE SESSION③ 次のAPS時にプリオケシグニへ能力付与をフラグとして予約 |
...
| `LRIG_RIDE_SIGNI` | 1 | 1 | WXK01-038 | LRIG_RIDE_SIGNI: センタールリグがすべての乗機シグニに乗る（ドライブ状態） |
...
| `PRDI035_PARADISE_COLOR` | 1 | 1 | PR-Di035 | PRDI035_PARADISE_COLOR: 次のアタックフェイズ開始時判定フラグをセット。 |
...
| `WXK07_043_CHARM_BANISH` | 1 | 1 | WXK07-043 | WXK07-043「羅菌 マグネ」: 対戦相手のAPS開始時、チャームがある場合バニッシュされない（ターン終了まで） |


---

## 付録: 内部/動的生成 STUB（JSON 0 件・ハンドラのみ 235 種）
...
| `BET_ALTERNATIVE` | 0 | 0 |  | BET_ALTERNATIVE: ベット強化済みなのでスキップ（BET_MECHANICで処理済み） |
...
| `OPP_GUARD_COST_COLORLESS` | 0 | 0 |  | このターン相手はガードできない（ガードコスト無色版 or ガード禁止） |
...
| `SET_CANCEL_ATTACK_FLAG` | 0 | 0 |  | SET_CANCEL_ATTACK_FLAG: アタックキャンセルフラグをセット（NEGATE_ATTACK_ON_TRIGGERのYes時） |
...
| `SUMMON_RESONA_FROM_LRIG_DECK` | 0 | 0 |  | SUMMON_RESONA_FROM_LRIG_DECK: ルリグデッキからレゾナ1枚を出現条件を無視して場に出す（WX20-069等） |
| `TRASH_ALL_CHARMS_DRAW_CHARGE` | 0 | 0 |  | TRASH_ALL_CHARMS_DRAW_CHARGE: 場の全チャームをトラッシュ→同枚数ドロー+エナチャ |
| `TRASH_AT_TURN_END` | 0 | 0 |  | TRASH_AT_TURN_END: ターン終了時にlastProcessedCardsのシグニをフィールドからトラッシュに置く（WX02-005 ホワイト・ホープ） |
