...
## サマリー（最終生成: 2026-07-18）

| 区分 | 値 |
|---|---:|
| JSON で使用中の STUB id 種類 | 559 |
| 　└ ハンドラ実装あり | 539 |
| 　└ フォールバック（execStub 未処理） | 20 |
| 総 STUB ノード件数 | 2430 |
| JSON 0 件・ハンドラのみ（内部/動的生成 STUB） | 265 |
...
| `LRIG_GRANT_MILL_PER_PREVENTED_DAMAGE` | 2 | 1 | WXDi-D07-007 |  |
...
### execStubPart1.ts（101 種）

| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 448 | 435 | WD06-001, PR-427, WD10-009 | OPTIONAL_COST: 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） 主な338件はeffectExecutor.tsがSTUB→CONDITIONAL(IS_MY_TURN)パター… |
| `ARTS_COST_REDUCTION_BY_EFFECT` | 114 | 112 | WD10-006, WD12-006, WD15-006 | アーツコスト軽減マーカー（コストはBattleScreen使用時に算出済み） |
| `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` | 68 | 67 | WD20-001, WDK05-T11, WDK08-Y11 | 他の任意コスト系（SEQUENCEパターン外のフォールバック） |
| `POWER_MOD_PER_COUNT` | 61 | 61 | WD19-001, WDK06-C17, WDK10-009 | 動的パワー修正（COUNT依存） |
| `TARGET_AND_DISCARD_HAND` | 61 | 57 | PR-195, PR-370, PR-459A | 手札を捨てて対戦相手シグニを対象とする効果（スタンドアロン時：手札1枚捨て+相手シグニをlastProcessedCardsへ） |
| `OPTIONAL_TRASH_ENERGY_CLASS` | 44 | 44 | WD14-009, WDK08-Y14, WX11-006 | 他の任意コスト系（SEQUENCEパターン外のフォールバック） |
| `RULE_REMINDER_TEXT` | 43 | 42 | SP26-003, SP38-001, PR-K026 | ゲームプレイに影響しない説明テキストは無音でスキップ |
| `CONDITIONAL_POWER_BONUS` | 40 | 35 | WD06-006, WDK08-Y08, WDK10-009 | 条件付きパワーボーナス |
...
| `GAIN_ABILITY_THIS_GAME` | 19 | 18 | WX08-015, WX10-011, WX24-P4-036 | ゲーム全体能力付与 |
...
| `REVEAL_AND_PICK` | 14 | 14 | WD23-024-E, WDK13-011, WDK13-017 | デッキから探してもよい（REVEAL_AND_PICK: シグニ検索→手札or場） |
| `TARGET_ONLY` | 14 | 14 | PR-Di017B, WX24-P4-075, WX25-P1-084 | ターゲット選択のみ（lastProcessedCards に格納し後続ステップへ） |
| `OPPONENT_PAY_OPTIONAL` | 13 | 13 | SPDi43-06, WXEX1-34, WXEX2-08 | 対戦相手任意コスト（相手にCHOOSEを提示し、支払うとフラグを立てる） |
| `REVEAL_PICK_HAND_SHUFFLE_BOTTOM` | 12 | 12 | WD23-041-EA, WDK13-011, WDK13-022 | デッキ上N枚公開してM枚を手札に加え残りをデッキ下/トラッシュ/エナゾーンへ |
...
| `GRANT_QUOTED_AUTO_ABILITY` | 10 | 10 | WD21-007, PR-K076, SPDi43-05 | WD21-007型: 「以下の５つから１つを選ぶ。…対象のシグニ１体は選んだ能力を得る。あなたがベットしていた場合、この効果を１回繰り返す。」 |
...
| `BET_ALTERNATIVE` | 5 | 5 | WX15-029, WX17-003, WX17-005 | BET_ALTERNATIVE: ベット強化済みなのでスキップ（BET_MECHANICで処理済み） |
...
| `STEAL_OPP_TRASH_PUPPET` | 3 | 2 | WDK17-007, WXK10-055 | STEAL_OPP_TRASH_PUPPET: 対戦相手のトラッシュからシグニを傀儡状態であなたの場に出す（WDK17-007）。 ベット時は2枚、非ベットは1枚。空きゾーン数・候補数で上限。選択した各カードを INTERNAL_PLAC… |
...
### execStubPart2.ts（214 種）

| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `TRAP_OPERATION` | 17 | 12 | SP26-001, WX15-047, WX15-053 | TRAP_OPERATION: トラップ/チェックゾーン操作の統合ハンドラ |
...
| `ABILITY_CHECK_ELSE_TRASH` | 4 | 4 | WX25-P3-038, WX25-P3-069, WX25-P3-072 | ABILITY_CHECK_ELSE_TRASH: 能力なしなら自トラッシュ |
...
| `PLACE_ACCE_SIGNI_TO_ENERGY` | 2 | 2 | WX22-043, WXEX1-44 | ACCE_TO_ENERGY / PLACE_ACCE_SIGNI_TO_ENERGY: アクセカードをエナゾーンへ |
...
| `PLACE_LIMIT_UPPER` | 11 | 11 | WX24-P1-031, WX24-P2-041, WX24-P4-036 | PLACE_LIMIT_UPPER: 【リミットアッパー】トークンをルリグゾーンに置く（1つまで） トークン効果（ルリグ1体かつレベル3以上でリミット+2）はBattleScreenのリミット計算側で適用 |
...
| `GAIN_LRIG_BARRIER` | 7 | 7 | SPDi43-26, WX24-P1-001, WX24-P3-026 | GAIN_LRIG_BARRIER: 【ルリグバリア】を得る（フリーゾーンにトークンとして設置。ルリグアタック1回を無効） |
...
| `NEGATE_NTH_ATTACK` | 3 | 3 | SP27-016, WX10-018, WX17-006 | NEGATE_NTH_ATTACK: このターン、相手シグニのアタックをN回目まで自動無効化 |
...
| `SELECT_OTHER_SIGNI` | 1 | 1 | WXDi-P10-052 | CONDITIONAL_KEYWORD_BY_CENTER_COLOR は廃止（SP27-002-E3 を CONTINUOUS GRANT_KEYWORD＋ activeCondition AND(LRIG_COLOR, FRONT_S… |
...
## 付録: 内部/動的生成 STUB（JSON 0 件・ハンドラのみ 265 種）
