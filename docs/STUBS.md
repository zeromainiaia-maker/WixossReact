...
| 　└ ハンドラ実装あり | 530 |
| 　└ フォールバック（execStub 未処理） | 4 |
| 総 STUB ノード件数 | 2339 |
| JSON 0 件・ハンドラのみ（内部/動的生成 STUB） | 248 |
...
| `TRADE_BANISH_SELF_SIGNI` | 30 | 30 | WD07-012, WD22-029-G, WDK08-Y01 | トレード：自シグニ1体をトラッシュに置き、相手シグニ1体をバニッシュ |
...
## 付録: 内部/動的生成 STUB（JSON 0 件・ハンドラのみ 248 種）
