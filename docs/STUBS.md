...
## サマリー（最終生成: 2026-07-04）

| 区分 | 値 |
|---|---:|
| JSON で使用中の STUB id 種類 | 552 |
| 　└ ハンドラ実装あり | 536 |
| 　└ フォールバック（execStub 未処理） | 16 |
| 総 STUB ノード件数 | 2387 |
| JSON 0 件・ハンドラのみ（内部/動的生成 STUB） | 266 |
...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 436 | 424 | WD06-001, PR-427, WD10-009 | OPTIONAL_COST: 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） 主な338件はeffectExecutor.tsがSTUB→CONDITIONAL(IS_MY_TURN)パター… |
| `ARTS_COST_REDUCTION_BY_EFFECT` | 115 | 113 | WD10-006, WD12-006, WD15-006 | アーツコスト軽減マーカー（コストはBattleScreen使用時に算出済み） |
| `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` | 65 | 64 | WD20-001, WDK05-T11, WDK08-Y11 | 他の任意コスト系（SEQUENCEパターン外のフォールバック） |
| `POWER_MOD_PER_COUNT` | 59 | 59 | WD19-001, WDK06-C17, WDK10-009 | 動的パワー修正（COUNT依存） |
| `TARGET_AND_DISCARD_HAND` | 56 | 53 | PR-195, PR-370, PR-459A | 手札を捨てて対戦相手シグニを対象とする効果（スタンドアロン時：手札1枚捨て+相手シグニをlastProcessedCardsへ） |
| `RULE_REMINDER_TEXT` | 44 | 43 | SP26-003, SP38-001, PR-K026 | ゲームプレイに影響しない説明テキストは無音でスキップ |
| `CONDITIONAL_POWER_BONUS` | 43 | 38 | WD06-006, WDK08-Y08, WDK10-009 | 条件付きパワーボーナス |
| `OPTIONAL_TRASH_ENERGY_CLASS` | 41 | 41 | WD14-009, WDK08-Y14, WX06-CB03 | 他の任意コスト系（SEQUENCEパターン外のフォールバック） |
| `LRIG_GROW_RESTRICT` | 37 | 37 | WD20-006, WDK06-R09, WDK07-Y07 | グロウ制限：対戦相手の no_grow フラグをセット |
| `DECLARE_NUMBER` | 35 | 33 | WD06-008, WD13-008, WDK04-006 | 数字宣言：CHOOSE UI で 1〜5 を選択し declared_guard_restrict_level に保存 |
...
| `TRADE_BANISH_SELF_SIGNI` | 28 | 28 | WD22-029-G, WDK08-Y01, WDK11-011 | トレード：自シグニ1体をトラッシュに置き、相手シグニ1体をバニッシュ |
| `GRANT_QUOTED_AUTO_ABILITY` | 24 | 23 | WD21-007, PR-K076, SPDi01-132 | WD21-007型: 「以下の５つから１つを選ぶ。…対象のシグニ１体は選んだ能力を得る。あなたがベットしていた場合、この効果を１回繰り返す。」 |
...
| `DECLARE_CARD_NAME` | 19 | 17 | PR-257, PR-K046, WX11-037 | カード名宣言（手札のカード名から選択） |
| `GAIN_ABILITY_THIS_GAME` | 19 | 18 | WX08-015, WX10-011, WX24-P4-036 | ゲーム全体能力付与 |
| `GRANT_ABILITY_INNER_TEXT` | 18 | 17 | SPDi43-01, WX07-065, WXEX1-32 |  |
...
| `GRANT_QUOTED_ABILITY` | 16 | 15 | WX22-Re04, WX24-P1-042, WX24-P3-003 | 引用符付き能力付与（キーワード → keyword_grants、複合能力 → granted_effects） |
...
| `REVEAL_PICK_HAND_SHUFFLE_BOTTOM` | 14 | 14 | WD23-041-EA, WDK01-008, WDK13-011 | デッキ上N枚公開してM枚を手札に加え残りをデッキ下/トラッシュ/エナゾーンへ |
...
### execStubPart2.ts（215 種）

| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `TRAP_OPERATION` | 13 | 9 | WX15-053, WX15-083, WX16-028 | TRAP_OPERATION: トラップ/チェックゾーン操作の統合ハンドラ |
| `DESIGNATE_SIGNI_ZONE` | 12 | 12 | WDK10-009, WX08-021, WX10-051 | DESIGNATE_SIGNI_ZONE: 相手シグニゾーンを1つ指定する |
...
| `PLACE_LIMIT_UPPER` | 10 | 10 | WX24-P1-031, WX24-P2-041, WX24-P4-036 | PLACE_LIMIT_UPPER: 【リミットアッパー】トークンをルリグゾーンに置く（1つまで） トークン効果（ルリグ1体かつレベル3以上でリミット+2）はBattleScreenのリミット計算側で適用 |
...
| `CENTER_LRIG_RIDES_ON_SIGNI` | 3 | 3 | SPK01-01, WXK01-008, WXK01-009 | CENTER_LRIG_RIDES_ON_SIGNI: センタールリグが選択した1体の乗機シグニに乗る（乗り換え可） |
...
## 付録: 内部/動的生成 STUB（JSON 0 件・ハンドラのみ 266 種）
...
| `MARK_MATERIAL_TARGET` | 0 | 0 |  | MARK_MATERIAL_TARGET（改造素材機構 Step2/3b）: 直前に処理したシグニ（lastProcessedCards＝《改造素材》を 使用した対象＜電機＞シグニ）を ownerState.material_used_t… |
