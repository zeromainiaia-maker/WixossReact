...
## サマリー（最終生成: 2026-06-18）

| 区分 | 値 |
|---|---:|
| JSON で使用中の STUB id 種類 | 534 |
| 　└ ハンドラ実装あり | 529 |
| 　└ フォールバック（execStub 未処理） | 5 |
| 総 STUB ノード件数 | 2300 |
...
| STUB ID | 件数 | カード数 | 代表カード | 説明 |
|---|---:|---:|---|---|
| `OPTIONAL_COST` | 409 | 398 | WD06-001, PR-427, WD10-009 | OPTIONAL_COST: 任意コスト（effectExecutorのSEQUENCEインターセプト対象外のエッジケース） 主な338件はeffectExecutor.tsがSTUB→CONDITIONAL(IS_MY_TURN)パター… |
| `ARTS_COST_REDUCTION_BY_EFFECT` | 116 | 114 | WD10-006, WD12-006, WD15-006 | アーツコスト軽減マーカー（コストはBattleScreen使用時に算出済み） |
