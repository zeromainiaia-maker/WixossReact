...
- **A1**　GRANT_QUOTED「…は以下の能力を得る。『…』」**後置型**の decompiler 対応（現状は「…は『…』を得る」前置型のみ描画）＝圧縮bucket ~数十効果。
- **A2**　`GRANT_QUOTED_AUTO_ABILITY` の**誤パース是正**（WX15-059/WX20-069 等＝原文に引用が無いのに付与STUB化）＝parser・数枚。
- **A3**　個別の表現脱落（WXDi-P11-032 アサシン/ランサー/常 の3択 GRANT、WXDi-CP02-074/089 の別timing 1効果脱落 等）＝数枚ずつ。
...
| 機構 | 影響 | リスク | 状態 |
|---|---|---|---|
| 引用AUTO付与（`GRANT_QUOTED_AUTO_ABILITY`） | 中 | 中 | **表現は前進**（decompiler が前置型「…は『…』を得る」を原文描画・2026-06-28）。残＝後置型「以下の能力を得る。『…』」の描画（A1）＋engine 精緻化（B4）＋誤パース是正（A2） |
