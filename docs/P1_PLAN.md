...
- **最終更新**: 2026-06-26
- **直近やったこと**: 文型★15〜17巡（場出し【出】抑止／look→pick→手札16枚／look→pick→エナ・手札+エナ3枚）＋**機構③ LOOK_PICK_CHAIN field宛先**＋配線3枚。同型★0維持。→ `BUGFIXES.md` 先頭参照。
- **次の一手（候補・上から推奨）**:
  1. **別系統の走査**（mechanical・低リスク）：`scratchpad/scan*.mjs` の型で「同じ壊れ方」を新規抽出。未着手の系統例＝owner誤り横展開／条件脱落（「このターン〜していた場合」等）／BURST崩れ。
  2. **`levelLteLastProcessed` フィルタ**（小・低リスク）→ WX25-P1-039 等の「場出しシグニのレベル以下の相手バニッシュ」を解消。
  3. **大型機構**（§5。1つ着手したら §5 の状態を `着手中(名前)` に）。
- **着手中の機構**: なし
- **注意/未解決**: 「脱落疑い件数」は指標にしない（§2）。field後続の重い効果（引用AUTO付与）は機構待ち。
