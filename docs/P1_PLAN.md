...
- **🆕 セッション（2026-07-02・zerom・続き）＝英語ID漏れ STUB をさらに6系統是正（BUGFIXES⑪）**＝`*_SERVANT_ZERO`系4id(8効果)／`SEED_BLOOM`+`SEED_BLOOM_OPTIONAL`(6)／`PLACE_SEED_FROM_REVEALED`(8)／`GAIN_LRIG_BARRIER`(5)／`LOSE_COLOR_ALL_ZONES`(7)／`OPEN_MAGIC_BOX`(5)。全て原文一致・同型★0維持・typecheck緑・engine不変。**英語ID漏れ 548→509**（⑩⑪合計 582→509）。**残る中規模クラスタは heterogeneous で個別精査要**＝`REPEAT_*`/`DO_THREE_THINGS`(WXK11-007等でSEQUENCE重複リスク)/`CAST_FROM_OPP_TRASH`/`COLLAB`/`LOOK_TOP_*`/`SIGNI_REPOSITION`+`MOVE_TARGET_SIGNI_TO_OTHER_ZONE`/`ACCE_FROM_HAND`（原文の枚数動的/語順不揃い/他アクションと同一文のため単一正規表現に不適）。`BET_*`系(38)は機構待ち。全体計画は [ROADMAP.md](./ROADMAP.md)。
