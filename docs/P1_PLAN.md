...
- **C1**　**engine未配線 timing 群の発火配線**＝R33-R58 で新設した ~15種。一覧＝`scripts/decompileEffects.ts` の `engineUnwiredTimings`。
  ...
  - 残＝`ON_MATERIAL_USED`(6・改造素材機構)/`ON_SIGNI_BANISH_OPPONENT_BY_EFFECT`(1・発生源追跡)/`ON_LRIG_UNDER_MOVED`(1・STUB action)/`ON_KEYWORD_GAINED`(1・COPY_ABILITY STUB・scope曖昧)/`ON_DECK_SHUFFLED`(1・shuffle多経路)＝いずれも mechanism-blocked か低ROI。
  - **`ON_MATERIAL_USED` は基盤未実装でブロック確定（2026-06-29 調査）**＝(1)『アーツ/クラフト』型のプレイハンドラが BattleScreen に無く改造素材は使用不可、(2)トークン3択が DO_THREE_THINGS no-op。trigger 配線は (1)(2) 実装が前提（無いと dead code）。完全な実装 blueprint は **TODO §4「改造素材機構」**。Stage2 基盤で trigger 部分は低リスクだが foundation が要る＝大型。
