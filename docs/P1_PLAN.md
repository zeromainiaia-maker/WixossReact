...
- **🆕 Stage2 着手（BattleScreen 配線の pure 抽出・2026-06-29）**＝`collect*Triggers` を `src/engine/triggerCollect.ts` へ pure 化し golden で C 配線を自動検証（C2 宿題を削減）。ON_TARGETED/ON_LRIG_GROW/ON_COIN_PAID/ON_SIGNI_POWER_ZERO_OR_LESS/ON_BLOOD_CRYSTAL_ARMOR/ON_TRASH（3関数）の**6ファミリ抽出済**（golden 37/37・TODO §8）。
...
1. **Stage2 の続き（推奨・検証基盤を強化しつつ C2 を減らす）**＝他の collect*Triggers を `src/engine/triggerCollect.ts` へ pure 抽出し golden 化。手順は確立済（pure関数追加→BattleScreenを薄いラッパ化＝挙動不変→golden テスト追加）。~~`collectArmorTriggers`（小）~~/~~ON_TRASH 3関数~~**✅2026-06-29 抽出済**。次候補＝`collectBanishTriggers`（ON_BANISH・多数）／`collectLeaveFieldTriggers`（`resolveLeaveFieldDynamicFilters` 依存＝やや重）／`collectTurnTriggers`（特殊ケース多数＝中リスク）。詳細は TODO §8【Stage2】。
