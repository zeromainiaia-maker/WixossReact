...
- **🧭 パーサー作業の地盤＝`docs/parser_worklist.md`（cold start はまずこれ）**: held 404 を **LOSS 255（真の弱点＝直す）／ VALUE 149（慣例・対象外）** に重複なく分割した有限ワークリスト。計器 `npx tsx scripts/parserWorklist.ts` で数字を再生成。**完了条件＝LOSS 255→0**。手当たり次第をやめ、バケツ単位（Stage A 局所67→C トリガー73→B action.type中心109）で潰す。各ラウンドで計器の LOSS 減＋同型★0 をゲートにする。**現在値＝held 301 / LOSS 150 / VALUE 151**（R15 commit 後・R15 は filter その他 −4）。
- **直近やったこと（ymst・最新12）**: **R15（トラッシュ対象の hasGuard/noGuard）**。「トラッシュから《ガードアイコン》を持(つ/たない)〔カード/シグニ〕を対象とし、それらを〔エナ/手札〕」の guard フィルタ脱落を是正（`parseSentencePart1.ts` の TRANSFER_TO_HAND＋ENERGY_CHARGE handler・engine 実装済み 433/438）。**LOSS 154→150（−4・filter その他 34→30）／held 305→301**。パーサーのみ・データ無変更・同型★0維持・typecheck緑・要実機検証。**⚠commonClass（3枚）は engine 未実装で保留**（matcher 無し・別タスク）。詳細 `BUGFIXES.md` 先頭。
- **直近やったこと（ymst・最新11）**: **Stage B R14（先頭条件文の CONDITIONAL 化）**。action.type「← -」系（HEAD は CONDITIONAL だがパーサーが条件を落とし bare action を出す）2系統＝①LIFE_CRASHED_THIS_TURN「このターンに〔owner〕のライフクロスがクラッシュされていた場合」②LAST_PROCESSED_HAS_TYPE「この方法でトラッシュに置いたカードの中にスペルがある場合」を CONDITIONAL 化（engine 実装済み）。**⚠ON_TURN_END 札は actionText 先頭に「各ターン終了時、」が残る**ため正規表現で吸収。**LOSS 160→154（−6）／held 311→305**。パーサーのみ・データ無変更（HEAD は元から CONDITIONAL）。同型★0維持・typecheck緑・要実機検証。**没ネタ**＝「デッキ一番上公開→の場合」の SEQUENCE[DECK_TOP_MATCHES] 化は HEAD inconsistent（REVEAL 多数派94/SEQUENCE 少数派13）で +94 退行→revert。詳細 `BUGFIXES.md` 先頭。
- **直近やったこと（ymst・最新10）**: **Stage B 本丸 R13（LOOK_AND_REORDER→REVEAL_AND_PICK 統一）**。「デッキ上N枚見る→その中からピックして手札に加える」を REVEAL_AND_PICK 化。**engine の execLookAndReorder は手札に加える経路が無い＝LOOK のピック札は実機で手札に入らない実害バグ**。`effectParser.ts:1218` の LOOK 分岐が remainder の「デッキ」で誤発火していたのを是正（手札取得時は先に REVEAL を試す）＋データ 123 subtree（≈116枚・8枚ネスト）を surgical 移行＋cardClass→story 5枚。**LOSS 178→160（−18）／held 329→311**。**⚠重要な発見＝この一族の held は16枚だけ（HEAD=REVEAL）だが、138枚が LOOK↔LOOK で一致していた（実害バグだが held でない）**。未対応の言い回し（好きな枚数=ALL/色filter/非ガードアイコン/パワー・コスト）は除外ガードで LOOK 据置＝誤表現を作らない（次の parser 拡張ラウンド）。同型★0維持・typecheck緑・要実機検証。詳細 `BUGFIXES.md` 先頭。
...
- **🎯 次の一手（次の人へ・上から推奨）**＝**現在地は「パーサー LOSS 178→0」の途中**（このセッションで R5-R15＝234→150 消化）。まず `docs/parser_worklist.md` を読む（地盤）。`npx tsx scripts/parserWorklist.ts` で現在値（**held 301／LOSS 150／VALUE 151**）を再生成できる。**⚠Stage A のクリーンな純局所＋LOOK/REVEAL 手札ピック本丸（R13）＋先頭条件文 CONDITIONAL（R14）＋トラッシュ guard filter（R15）は消化済み**＝残りは action.type 54（LOOK/REVEAL/DECK_TOP の inconsistent curation が多く罠・要個別判断）／filter その他 30（thisCardOnly 6・acceHost 6・levelLte系・commonClass等＝engine 実装要確認）／Stage C hard tail が主。**⚠action.type は型ペア診断（BUGFIXES R13 手法）してから着手し、HEAD の多数派と一致する向きにのみ寄せる**（R14 没ネタの DECK_TOP 罠参照）。
  - **⚠ GRANT_ACCE_HOST_ABILITY は LOSS バケツに現存0**（旧バトンの推奨は stale＝R13 の type-pair 診断で確認）。着手不要。
  1. **【R13 の続き・低〜中リスク】LOOK/REVEAL の除外言い回しを parser 拡張**＝R13 で誤表現回避のため LOOK 据置にした **好きな枚数(→pickCount:'ALL')／色フィルタ(赤のカード等)／非ガードアイコン(《ライズ》→hasIcon)／パワー・コスト filter**。parser でこれらを正しく出せるようにし、対応データ（HEAD は今 LOOK）を REVEAL へ移行。engine 実害バグの残り（手札に入らない LOOK ピック札）も同時に解消。**素の「その中から1枚を手札に加える」（公開・まで無し）も同じ実害**だが HEAD と一致＝held でない（移行は任意・engine 正しさのため推奨）。
  2. **【action.type 60 の残り】**＝R13 後に再診断（type-pair 分解スクリプトは BUGFIXES R13 の手法）。SEQUENCE←REVEAL_AND_PICK／CONDITIONAL←LIFE_CRASHED／CHOOSE←BANISH 等のクラスタ。各々 narrow 修正＋同型★0 ゲート。
  3. **【着手しない／別トラック】** ①Stage C hard tail（powerRange anaphora 伝播・2枚クラスタの timing+scope・placedDown＝R11.5 ログ参照）②fromFieldByCostOrEffect（fromZones:["field"]が規約・broad化で+13退行）③duration「次の対戦相手のターン終了時まで」＝145枚慣例。**VALUE 151 は最後にまとめて逆翻訳レビューで**（パーサーでは減らない）。
  - **⚠ R13 の教訓**：worklist の held（parser≠HEAD）だけ見ると一族の規模を見誤る。LOOK/REVEAL 手札ピックは held 16枚だったが、**138枚が LOOK↔LOOK で一致（=held でないが engine 実害バグ）**。`decompile-engine-parity`＝逆翻訳を正したら engine 実装まで揃える。型の値違いだけ（ENERGY_CHARGE↔SEND_TO_ENERGY）は VALUE。診断時は LOSS/VALUE と「held か実害か」を区別する。
