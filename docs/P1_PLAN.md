...
1. **実機検証（C2・着手済→継続推奨）**＝**フル driver `scripts/verifyBattleDrive.mjs` がシナリオ切替式に一般化済**＝盤面注入＋クリック列で実 BattleScreen を駆動できる。**2026-06-30 時点で生STUB 3＋C1 timing `ON_LRIG_GROW` を実 UI 観測クローズ**。残 C1 timing（`ON_TARGETED`/`ON_COIN_PAID`/`ON_DECK_SHUFFLED`/`ON_SIGNI_BANISH_OPPONENT_BY_EFFECT`/`ON_LRIG_UNDER_MOVED` 等）は**同 `scenarios` テーブルに1件足すだけ**で横展開できる（トリガー＝グロウ/コイン払い/シャッフル等を注入＋ドライブ）。発火条件は golden 済なので「実盤面での総合動作」に絞れる。⚠各配線は近似を含む（BUGFIXES の各⚠参照）。
