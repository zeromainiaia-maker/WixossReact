...
- **🆕 Stage2 着手（BattleScreen 配線の pure 抽出・2026-06-29）**＝`collect*Triggers` を `src/engine/triggerCollect.ts` へ pure 化し golden で C 配線を自動検証（C2 宿題を削減）。targeted/lrigGrow/coinPaid/powerZero/armor/trash(3)/banish/leaveField/draw(self/opp)/mill/charm/energy/refresh/powerDecrease/moveToDeck/freeze/selfEvent/zoneMoved/driveBecame/beatBecame/handDiscard/oppArtsUse/artsUse の**24ファミリ抽出済**（golden 60/60・TODO §8）。残る collect は大物2本（turn/field、field に bloom 依存）のみ。
...
1. **Stage2 の続き（推奨・検証基盤を強化しつつ C2 を減らす）**＝他の collect*Triggers を `src/engine/triggerCollect.ts` へ pure 抽出し golden 化。手順は確立済（pure関数追加→BattleScreenを薄いラッパ化＝挙動不変→golden テスト追加）。~~armor/trash3/banish/leaveField/draw3/set-diff6/クリーン7(selfEvent・zoneMoved・driveBecame・beatBecame・handDiscard・oppArtsUse・artsUse)~~**✅2026-06-29 抽出済**。残 collect ＝**大物 collectTurnTriggers（中リスク）/collectFieldTriggers（ON_PLAY・複雑、collectBloomTriggers も依存）**＋detect* 約13個＋フェイズ進行/effect_stack。詳細は TODO §8【Stage2】。
