...
- **最終更新**: 2026-06-26
- **直近やったこと**: **逆翻訳器の生ID残存を一掃**＝未描画アクション21種を自然文化（`[アクション:X]` 約180→0）＋高頻出STUB18種に説明文（`[STUB:X]` ~140→54）。decompiler表示のみの低リスク改善で逆翻訳の網羅性UP。→ `BUGFIXES.md` 先頭2件。**残る生ID＝1回のみ出現の単発STUB 54件のみ**（保護系・特殊ルール系。STUBS.md管理・費用対効果低・後続で随時）。
- **その前**: 機構：傀儡場出しの汎用化（WXK10-055 全効果再構築・smoke8pass）／機構④《自分ターン》《相手ターン》AUTOトリガー基盤＋AUTO30枚。
