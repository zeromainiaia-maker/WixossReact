...
- [ ] 残る大型機構（§5）が実装＋配線済み、または明確にスコープ外と合意。**← P1 で残る唯一の未完項目**。
- ⚠ **「脱落疑いの件数」は完了指標にしない**（§2）。
...
- 直近：実バグ候補3枚＋トラップ本体2枚を MANUAL 修正、付与引用（GRANT_QUOTED）の decompiler 描画を改良（本物の付与10枚が原文一致に）。
...
- ~~**A2**　`GRANT_QUOTED_AUTO_ABILITY` の**誤パース是正**~~ **✅大半完了（2026-06-28）**＝引用無しGQは13枚（TK03A除く）。うち9枚（optional-cost 句「…してもよい。そうした場合」の誤付与化）を `OPTIONAL_COST` 規約に置換し虚偽の付与主張を除去（MANUAL）。TK03A は本物の付与＝decompiler subject を「これ…は『…』を得る」に拡張。**A2残4枚**＝WXDi-P09-079/P10-041/WX25-CP1-060/WD13-002 は別系の機構絡み誤パース（A3/B 寄り）。
- **A3**　個別の表現脱落（WXDi-P11-032 アサシン/ランサー/常 の3択 GRANT、WXDi-CP02-074/089 の別timing 1効果脱落、上記2枚の周辺STUB 等）＝数枚ずつ・parser/機構絡み。
...
- **B3**　遅延条件トリガー（WX25-CP1-069「このターン、…クラッシュしたとき」）。
...
| 遅延条件トリガー（「このターン、…したとき」） | 小（WX25-CP1-069 等） | 中 | 未着手（B3） |
