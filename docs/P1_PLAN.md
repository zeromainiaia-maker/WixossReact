...
- **🎉 LOSS 0 達成（R31）＋VALUE curation 着手（R1-R4・159→107）**。**🔜 次の一手＝VALUE 最後の本丸＝timing flatten 系統 102枚（実バグ・`TODO.md` §3.5）**。発見＝`timing:ON_TURN_END` だが `duration:UNTIL_END_OF_TURN` の【自】で、原文トリガー「〜したとき」を丸ごと落として ON_TURN_END に flatten＝**ターン終了時に付与して即失効＝no-op の実バグ**。直し方＝trigger-type 別（スペル使用/ライズ/ヘブン/ウィルス/レゾナ/トラッシュから場出し/クロス持ち場出し 等）に小クラスタ化→正 timing+scope+filter 再構築→**engine 配線確認**→数枚ずつ→typecheck＋同型★0＋実機。診断＝`npx tsx scripts/_valueTriage.ts timing`。⛔bulk禁止。残 VALUE 107＝この102＋ON_DRAW機構待ち6（§4）等。並行候補＝②LOOK_AND_REORDER 約252枚精査（🔭下記）。⚠R30/R31 の実機検証（§TODO5）も未了。
