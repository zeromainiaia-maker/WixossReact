...
- **🆕 zerom セッション続き（2026-06-29）＝A表現テール 保護系STUB（PREVENT_* 18 id＋SUPPRESS_GAIN_ABILITY）を意味文化**＝能力/パワー/移動/ダウン保護等を生STUB→原文意味文に（engine実装済み・decompiler描画のみ欠落）。色動的（白/赤）・既存近似是正含む。同型★0維持・全ハーネス緑。詳細 BUGFIXES.md 上部。
