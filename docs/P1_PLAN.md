...
- **最終更新**: 2026-06-26
- **直近やったこと**: 文型★15〜17巡＋機構③（field宛先）＋**小機構 `levelLteLastProcessed` フィルタ＋配線3枚**（WX21-022/WX24-P3-026/WX25-P1-039）。同型★0維持。→ `BUGFIXES.md` 先頭参照。
- **重要な調査結果**: 「同じ壊れ方を一括で潰すクリーンな横展開系統はほぼ枯れた」。全カードを語尾tally／STUB tally／凍結ドロップで走査したが、残りは(a)複数択の中の択落ち、(b)カード丸ごとの誤parse、(c)機構待ち、に分散。→ **今後は“系統一括”より“小機構＋少数配線”or“個別★割れ”が中心**になる。
- **次の一手（候補・上から推奨）**:
  1. **`levelLteLastProcessed` の残り配線**（即・低リスク）：WX22-046／WXEX2-17／WXK10-044／WXK10-055（フィルタは実装済。各カードのアクションに付与＋必要なら再構築）。
  2. **個別★割れの精査**：`docs/grouped_sentence_all.txt` の各★を1枚ずつ（偽陽性§4を先に除外）。
  3. **大型機構**（§5。1つ着手したら §5 の状態を `着手中(名前)` に）。最有力＝《相手ターン》トリガー基盤。
- **着手中の機構**: なし
- **注意/未解決**: 「脱落疑い件数」は指標にしない（§2）。field後続の重い効果（引用AUTO付与）は機構待ち。WX24-P3-026-E1 は timing 誤り（原文「メイン開始時」が ON_PLAY）が**別途**残存（今回はフィルタのみ修正）。
