...
- **🎉 LOSS 0（R31）＋VALUE timing flatten 進行中（159→45）**。**🔜 次の一手＝timing flatten 残 ≈45枚（実バグ・`TODO.md` §3.5）**。発見＝`timing:ON_TURN_END` だが `duration:UNTIL_END_OF_TURN` の【自】で原文トリガーを丸ごと落とした flatten＝**付与即失効の no-op 実バグ**。✅済（R5-R13＋R32）＝スペル使用7／ドライブ3／**新規 engine トリガー6種**（ON_ARTS_USE・ON_RISE・ON_CARD_MILLED_FROM_DECK・hasCrossIcon/hasRiseIcon filter・placedFromTrash）＋**R32＝ON_DRAW効果ドロー2/ON_HAND_DISCARDEDディソナ1（既存配線）**。診断＝`npx tsx scripts/_flattenList.ts [正規表現]`（原文＋EXIST/FRESH timing 一覧・**R32 で新設・tracked**）。**残 ≈45**＝ON_DRAW(opp-draw/位相)6＝§4機構待ち／毒牙パワー減2（§3機構④）／傀儡場出し1（WDK17-001・3択＋傀儡フィルタ）／ウィルス配置・除去/凍結/対象化/コイン支払い/グロウ/リフレッシュ/改造素材/エクシードコスト/カードがデッキ移動 等の**未配線トリガー多数**＝機構実装が要る＝個別調査。⛔bulk禁止。⚠ **R5-R13/R32 の engine 配線は全て実機未検証**（§TODO5）。
- **🛠 再開時の確認コマンド**: `npx tsx scripts/parserWorklist.ts`（commit 後 held 45 / **LOSS 0** / VALUE 45。計器の EXIST は `git show HEAD` 読み＝**commit しないと反映されない**）。flatten 一覧＝`npx tsx scripts/_flattenList.ts`（timing 変更のみ・原文＋EXIST/FRESH）。⚠`_valueTriage.ts`/`_resync.ts`/`_manualize2.ts`/`_flattenView.mjs` は未追跡ローカルで**消失済み**（R32 で `_flattenList.ts` を tracked 化して代替）。
