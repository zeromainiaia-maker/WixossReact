...
- **`npm run golden`（`scripts/goldenTest.ts`）＝③正しさの構文ゴールデンテスト（2026-06-29 npm登録）**。主要DSLアクション型ごとに制御盤面で効果を実行し「結果がこうなる」を assert（型単位で正しさを担保＝全カードを帰納的に信頼）。**現状＝PASS 28／FAIL 0**（うち7件は Stage2 で追加した **C1 トリガー収集テスト**＝`triggerCollect.ts` の ON_TARGETED/ON_LRIG_GROW/ON_COIN_PAID 発火条件を HOST/GUEST 盤面で自動検証）。smoke が「壊れないか」を全カードで見るのに対し、本テストは型ごとの「正しさ」＋C1発火条件を見る。C/D 作業時は smoke と併せて回す。
