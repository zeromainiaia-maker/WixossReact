...
- **🆕 zerom セッション（2026-06-30・最新）＝逆翻訳機の本格改善に方針転換（§4「文法崩れ着手禁止」を解除）**。大本の目的「全効果カードで逆翻訳が原文一致」に対し、機械指標（同型★0・held/LOSS/VALUE 0）は達成済だが**逆翻訳機の出力品質（LOOK/REVEAL/CHOOSE の文法崩れ・最大クラスタ51+84枚）が§4で凍結されていた**のが本丸の残作業と判明。第一歩として **`REVEAL_AND_PICK` レンダラの文法崩れを是正**（then フル節が「を1枚」直後に二重主語連結→崩壊。TRANSFER_TO_HAND 59件＋別効果系~25件。配置系「その中から…を[pickN][動詞]」／別効果系「それが[filter]の場合、[then]」の2形に整理）。同型★0維持・typecheck OK・engine不変。**⚠注意＝`_dropTriage`の「文法崩れ」件数は構造分類で文法品質を測れない＝改善は原文照合で確認する**。残＝`LOOK_AND_REORDER`/`LOOK_PICK_CHAIN`/`CHOOSE`圧縮。詳細 BUGFIXES.md 上部。
