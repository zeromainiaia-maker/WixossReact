...
- **🎉 LOSS 0（R31）＋VALUE curation 進行中（159→75）**。**🔜 次の一手＝timing flatten 系統の残 ≈69枚（実バグ・`TODO.md` §3.5）**。発見＝`timing:ON_TURN_END` だが `duration:UNTIL_END_OF_TURN` の【自】で原文トリガーを丸ごと落とした flatten＝**付与即失効の no-op 実バグ**。✅済＝スペル使用7（R5）／ドライブ3（R6）／**ON_ARTS_USE 新設5（R7）／ON_RISE 新設6（R8）／ON_CARD_MILLED_FROM_DECK 新設・set-diff検出12（R9）**。診断＝`node scripts/_flattenView.mjs [正規表現]`／`npx tsx scripts/_valueTriage.ts timing`。**配線済みクラスタから1つずつ**（場出し系＝ON_PLAY+any_ally+filter／手札捨て＝ON_HAND_DISCARDED／アクセ＝ON_ACCE／エクシードコスト＝ON_EXCEED_COST）→timing+scope+filter 再構築→MANUAL→typecheck＋同型★0＋decompile原文一致＋**実機検証**。残未配線＝毒牙パワー減（§3 機構④）／コイン支払い/ウィルス除去/ゾーン移動。⛔bulk禁止。⚠ R5-R9 の engine 配線は全て**実機未検証**（§TODO5）＋ON_ARTS_USE/ON_RISE は CPU 経路未収集＋ミル機構は原因限定/合計N枚が近似。
- **🛠 再開時の確認コマンド**: `npx tsx scripts/parserWorklist.ts`（commit 後 held 107 / **LOSS 0** / VALUE 107。計器の EXIST は `git show HEAD` 読み＝**commit しないと反映されない**）。VALUE 診断＝`npx tsx scripts/_valueTriage.ts [bucket]`（EXIST/FRESH の変更リーフ一覧）。`scripts/_resync.ts <id> --apply`（FRESH 採用）／`_manualize2.ts <id>`（突き合わせ）＝未追跡のままローカル保持。
