...
- **🧭 パーサー作業の地盤＝`docs/parser_worklist.md`（cold start はまずこれ）**: held 404 を **LOSS 255（真の弱点＝直す）／ VALUE 149（慣例・対象外）** に重複なく分割した有限ワークリスト。計器 `npx tsx scripts/parserWorklist.ts` で数字を再生成。**完了条件＝LOSS 255→0**。手当たり次第をやめ、バケツ単位（Stage A 局所67→C トリガー73→B action.type中心109）で潰す。各ラウンドで計器の LOSS 減＋同型★0 をゲートにする。**現在値＝held 305 / LOSS 154 / VALUE 151**（R14 commit 後・R14 は action.type −6）。
...
- **🎯 次の一手（次の人へ・上から推奨）**＝**現在地は「パーサー LOSS 178→0」の途中**（このセッションで R5-R14＝234→154 消化）。まず `docs/parser_worklist.md` を読む（地盤）。`npx tsx scripts/parserWorklist.ts` で現在値（**held 305／LOSS 154／VALUE 151**）を再生成できる。**⚠Stage A のクリーンな純局所＋LOOK/REVEAL 手札ピック本丸（R13）＋先頭条件文 CONDITIONAL（R14）は消化済み**＝残りは action.type 54（LOOK/REVEAL/DECK_TOP の inconsistent curation が多く罠・要個別判断）と Stage C hard tail が主。**⚠action.type は型ペア診断（BUGFIXES R13 手法）してから着手し、HEAD の多数派と一致する向きにのみ寄せる**（R14 没ネタの DECK_TOP 罠参照）。
