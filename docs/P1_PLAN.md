...
- **🆕 Stage2 着手（BattleScreen 配線の pure 抽出・2026-06-29）**＝`collect*Triggers` を `src/engine/triggerCollect.ts` へ pure 化し golden で C 配線を自動検証（C2 宿題を削減）。**collect*Triggers は全28関数を pure 抽出完了**（golden 66/66・TODO §8）。残るは detect*（盤面差分・約13個）／フェイズ進行／effect_stack 整列のみ。
...
1. **Stage2 の続き（推奨・検証基盤を強化しつつ C2 を減らす）**＝他の collect*Triggers を `src/engine/triggerCollect.ts` へ pure 抽出し golden 化。手順は確立済（pure関数追加→BattleScreenを薄いラッパ化＝挙動不変→golden テスト追加）。~~collect*Triggers 全28関数~~**✅2026-06-29 完走**。残 Stage2 ＝detect*（盤面差分・約13個）＋フェイズ進行/effect_stack 整列の抽出のみ。詳細は TODO §8【Stage2】。
