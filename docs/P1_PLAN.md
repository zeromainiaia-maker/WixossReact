...
- **🆕 Stage2 着手（BattleScreen 配線の pure 抽出・2026-06-29）**＝`collect*Triggers` を `src/engine/triggerCollect.ts` へ pure 化し golden で C 配線を自動検証（C2 宿題を削減）。ON_TARGETED/ON_LRIG_GROW/ON_COIN_PAID/ON_SIGNI_POWER_ZERO_OR_LESS/ON_BLOOD_CRYSTAL_ARMOR/ON_TRASH（3）/ON_BANISH/ON_LEAVE_FIELD/ON_DRAW（self/opp）/ON_CARD_MILLED_FROM_DECK の**11ファミリ抽出済**（golden 47/47・TODO §8）。
...
1. **Stage2 の続き（推奨・検証基盤を強化しつつ C2 を減らす）**＝他の collect*Triggers を `src/engine/triggerCollect.ts` へ pure 抽出し golden 化。手順は確立済（pure関数追加→BattleScreenを薄いラッパ化＝挙動不変→golden テスト追加）。~~armor/ON_TRASH3/banish/leaveField/draw3(draw・oppDraw・mill)~~**✅2026-06-29 抽出済**。残 collect ＝charmToTrash/energyToTrash/refresh/powerDecrease/moveToDeck/freeze/bloom/selfEvent/zoneMoved/driveBecame/beatBecame/handDiscard/oppArtsUse/artsUse（クリーン）＋**大物 collectTurnTriggers（中リスク）/collectFieldTriggers（ON_PLAY・複雑）**＋detect* 約13個。詳細は TODO §8【Stage2】。
