...
- **🆕 zerom セッション（2026-06-30・最新）＝A表現テール：アーツ/スペル使用コスト改変句を原文復元**＝`ARTS_COST_REDUCTION_BY_EFFECT`/`ARTS_COST_REDUCTION_BY_CENTER_LRIG` が逆翻訳で内部マーカー `[STUB:アーツコスト軽減マーカー…]` を露出していた122枚を、decompiler が `currentCardText` から「…使用コストは…減る/増える/になる」文を抽出して描画するよう修正（engine 不変・表現のみ）。**STUBマーカー約125件→4件**（残4＝PR-433 コスト色無視／WXDi-P06-066 エナコスト代替／WXK11-014 グロウコスト／WXK11-016 複合＝別機構でフォールバック）。同型★0維持（割れ0／5986枚）・typecheck OK・STUB総数 1392→1267。詳細 BUGFIXES.md 上部。**次のA表現テール候補＝`grep -ohE "\[STUB:[^]]+\]" docs/decompile_sheet*.txt | sort | uniq -c | sort -rn` で残クラスタを1系統ずつ。**
