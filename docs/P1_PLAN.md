...
- **🎉 LOSS 0（R31）＋VALUE curation 進行中（159→48）**。**🔜 次の一手＝timing flatten 残 ≈42枚（実バグ・`TODO.md` §3.5）**。発見＝`timing:ON_TURN_END` だが `duration:UNTIL_END_OF_TURN` の【自】で原文トリガーを丸ごと落とした flatten＝**付与即失効の no-op 実バグ**。✅済（R5-R13）＝スペル使用7／ドライブ3／**新規 engine トリガー6種**（ON_ARTS_USE・ON_RISE・ON_CARD_MILLED_FROM_DECK・hasCrossIcon/hasRiseIcon filter・placedFromTrash）＝アーツ5/ライズ6/デッキミル12/手札捨て・アクセ・エクシード9/場出しGroupA6/クロス・ライズ場出し6/トラッシュから場出し6。診断＝`node scripts/_flattenView.mjs [正規表現]`／`npx tsx scripts/_valueTriage.ts timing`。**残 ≈42**＝傀儡場出し1／毒牙パワー減2（§3 機構④）／《ターン1回》複合トリガー・単発系（ウィルス除去/凍結/コイン支払い/ゾーン移動/リフレッシュ/改造素材 等）が中心＝個別調査。⛔bulk禁止。⚠ **R5-R13 の engine 配線は全て実機未検証**（§TODO5）＋ON_ARTS_USE/ON_RISE は CPU 経路未収集＋ミル機構は原因限定/合計N枚が近似＋レゾナ場出しは ON_PLAY 発火要確認。
