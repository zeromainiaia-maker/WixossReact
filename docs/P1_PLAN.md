...
1. **Stage2 の続き（推奨・検証基盤を強化しつつ C2 を減らす）**＝他の collect*Triggers を `src/engine/triggerCollect.ts` へ pure 抽出し golden 化。手順は確立済（pure関数追加→BattleScreenを薄いラッパ化＝挙動不変→golden テスト追加）。~~armor/trash3/banish/leaveField/draw3/set-diff6/クリーン7/field・bloom~~**✅2026-06-29 抽出済**。残 collect ＝**collectTurnTriggers のみ（中リスク・keyword token/lrig付与/FUTURE SESSION③/PR-Di035 等の特殊ケース多数）**＋detect* 約13個＋フェイズ進行/effect_stack。詳細は TODO §8【Stage2】。
