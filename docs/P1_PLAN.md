...
- **🧭 パーサー作業の地盤＝`docs/parser_worklist.md`（cold start はまずこれ）**: held 404 を **LOSS 255（真の弱点＝直す）／ VALUE 149（慣例・対象外）** に重複なく分割した有限ワークリスト。計器 `npx tsx scripts/parserWorklist.ts` で数字を再生成。**完了条件＝LOSS 255→0**。手当たり次第をやめ、バケツ単位（Stage A 局所67→C トリガー73→B action.type中心109）で潰す。各ラウンドで計器の LOSS 減＋同型★0 をゲートにする。**現在値＝held 329 / LOSS 178 / VALUE 151**（R12 commit 後・R12 は VALUE を−10）。
...
- **🎯 次の一手（次の人へ・上から推奨）**＝**現在地は「パーサー LOSS 178→0」の途中**（このセッションで R5-R12＝234→178 消化）。まず `docs/parser_worklist.md` を読む（地盤）。`npx tsx scripts/parserWorklist.ts` で現在値（**held 329／LOSS 178／VALUE 151**）を再生成できる。**⚠Stage A のクリーンな純局所はほぼ枯れた**＝残りは Stage B（action.type 77）と Stage C hard tail が主。
  1. **【低リスクで LOSS 本体】GRANT_ACCE_HOST_ABILITY 約11枚**（STUB/GRANT_PROTECTION/DRAW/GRANT_KEYWORD に退化＝lost leaf あり＝LOSS本体）。REVEAL 一族の地雷を避けつつ LOSS を動かせる。まず LOSS 分類か診断してから着手。
  2. **【本命・最大レバー】Stage B：LOOK/REVEAL_AND_PICK 一族 約59枚**（SEQUENCE←REVEAL_AND_PICK 23／REVEAL_AND_PICK←LOOK_AND_REORDER 23／SEQUENCE←LOOK_AND_REORDER 10 等）。**設計図は `docs/parser_worklist.md`「🅑 Stage B 着手記録」＋R11.5 の型ペア診断に完成済み**。R4 で「新ハンドラ追加は+129退行」確認済＝必ず **effectParser.ts:1210 の誤発火分岐をピンポイント修正＋1222分岐拡張**（広域ハンドラ追加禁止）。**毎ステップ build→`parserWorklist.ts` で held合計が増えていない（横取り退行ゼロ）ことを必ず確認**。
