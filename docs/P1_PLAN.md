...
- **🎉 LOSS 0 達成＝パーサー整合ワークリスト完了（R31）**。最後の1枚 WX20-026-E3 を engine 実装で配線（`drawBySourceStory` トリガー新設＝自＜凶蟲＞シグニ効果ドローで相手−4000）。詳細 `BUGFIXES.md` 先頭。**🔜 次の一手＝2フェーズの選択**：①**VALUE 159 の curation レビュー**（値違い＝慣例/1件ずつ判断・bulk禁止。`parserWorklist.ts` ②欄。最大は timing 122＝多くが「あなたのアタックフェイズ開始時」系の慣例ズレで全再生成禁止。残バケツ owner 7／count 4／then 4 等の局所から）か、②**LOOK_AND_REORDER 約252枚の取得未実装疑いの精査**（下記🔭・`scripts/_lr.ts`。テキスト駆動で REVEAL_AND_PICK へ寄せる正規化＝LOSS とは別軸の実バグ疑い）。**どちらも要実機/engine 確認の重い curation 案件**。⚠まず R30/R31 の実機検証（§TODO5）を済ませてから拡張に進むのが安全。
- **🛠 再開時の確認コマンド**: `npx tsx scripts/parserWorklist.ts`（commit 後の現在値 held 159 / **LOSS 0** / VALUE 159 を再生成。計器の EXIST は `git show HEAD` 読みなので **commit しないと反映されない**点に注意）。診断ツールは `scripts/_manualize2.ts <id>`（EXIST/FRESH 突き合わせ）・`scripts/_resync.ts <id> --apply`（FRESH 採用）＝未追跡のままローカル保持。
