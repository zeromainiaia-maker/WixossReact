...
- **🧭 パーサー作業の地盤＝`docs/parser_worklist.md`（cold start はまずこれ）**: held 404 を **LOSS 255（真の弱点＝直す）／ VALUE 149（慣例・対象外）** に重複なく分割した有限ワークリスト。計器 `npx tsx scripts/parserWorklist.ts` で数字を再生成。**完了条件＝LOSS 255→0**。手当たり次第をやめ、バケツ単位（Stage A 局所67→C トリガー73→B action.type中心109）で潰す。各ラウンドで計器の LOSS 減＋同型★0 をゲートにする。**現在値＝held 386 / LOSS 233 / VALUE 153**。
- **直近やったこと（ymst・最新）**: **Stage A R5（filter.story・トラッシュ→手札）**。`parseSentencePart1.ts` の TRANSFER_TO_HAND トラッシュ handler に `parseStoryFilter` を**名詞句スパン（`トラッシュから(.*?)手札に加える`）限定**で追加（全文だと WX22-002 等の前置き条件クラスを誤拾い）。**LOSS 234→233 / held 387→386**。retrieved-card の class 取りこぼし132枚を収穫マージで純改善採用。typecheck緑・同型★0維持・要実機検証。詳細は `BUGFIXES.md` 先頭。**次の純局所候補**: count 11／activeCondition 7（WXK04-080「【アクセ】が付いているかぎり」等）／残 filter.story 3（WX20-026/WX21-Re09/WX22-046＝別構造）。本命は依然 Stage B（下記🎯）。
