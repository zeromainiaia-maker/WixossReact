...
- **🎉 LOSS 0 達成（R31）＋VALUE curation 進行中（159→97）**。**🔜 次の一手＝timing flatten 系統の残 ≈92枚（実バグ・`TODO.md` §3.5 に配線分類済み）**。発見＝`timing:ON_TURN_END` だが `duration:UNTIL_END_OF_TURN` の【自】で原文トリガー「〜したとき」を丸ごと落とした flatten＝**付与即失効の no-op 実バグ**。✅済＝スペル使用7（R5）／ドライブ3（R6）。診断＝`node scripts/_flattenView.mjs [正規表現]`（トリガー文＋action）／`npx tsx scripts/_valueTriage.ts timing`。**配線済みクラスタから1つずつ**（場出し系＝ON_PLAY+any_ally+filter／手札捨て＝ON_HAND_DISCARDED／アクセ＝ON_ACCE／エクシードコスト＝ON_EXCEED_COST）→timing+scope+filter 再構築→MANUAL→typecheck＋同型★0＋decompile原文一致＋**実機検証**。⛔未配線（ライズ/アーツ使用/デッキミル/毒牙＝§3 機構④）は機構実装が要る＝触らない。⛔bulk禁止。並行候補＝②LOOK_AND_REORDER 約252枚精査（🔭下記）。⚠R30/R31/R5/R6 の実機検証（§TODO5）も未了。
