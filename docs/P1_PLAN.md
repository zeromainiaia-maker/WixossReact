...
⑤単発STUB 19 id（保護/代替コスト/グロウ/アタック制限等・`scanRemain.mjs` で網羅確認し本体のみ固定文化）⑥残テール3 id（SEQUENCE/CHOOSE/action 内・engine実装済み）。生STUB（id露出）は**当初41種→3種**に。
  - **残3種は engine/CHOOSE 対応とセット（D課題）**＝CONDITIONAL_GROW_AND_KEY_DISABLE・BANISH_ATTACKER_IF_WEAKER_THAN_FRONT（engine grep 0件＝**未実装**＝意味文化すると偽陰性）・SIGNI_GRANT_CHOSEN_ABILITY（CHOOSE 2択能力付与＝近似大）。**生STUB id露出はほぼ枯れた。**
