...
- **🧭 パーサー作業の地盤＝`docs/parser_worklist.md`（cold start はまずこれ）**: held 404 を **LOSS 255（真の弱点＝直す）／ VALUE 149（慣例・対象外）** に重複なく分割した有限ワークリスト。計器 `npx tsx scripts/parserWorklist.ts` で数字を再生成。**完了条件＝LOSS 255→0**。手当たり次第をやめ、バケツ単位（Stage A 局所67→C トリガー73→B action.type中心109）で潰す。各ラウンドで計器の LOSS 減＋同型★0 をゲートにする。**現在値＝held 329 / LOSS 170 / VALUE 159**（R13 commit 後・R13 は LOSS を−8）。
...
- **🎯 次の一手（次の人へ・上から推奨）**＝**現在地は「パーサー LOSS 170→0」の途中**（このセッションで R5-R13＝234→170 消化）。まず `docs/parser_worklist.md` を読む（地盤）。`npx tsx scripts/parserWorklist.ts` で現在値（**held 329／LOSS 170／VALUE 159**）を再生成できる。**⚠Stage A のクリーンな純局所も GRANT_ACCE（R13）も消化済み**＝残りは Stage B（LOOK/REVEAL 一族）と Stage C hard tail が主。
  1. **【本命・最大レバー】Stage B：LOOK/REVEAL_AND_PICK 一族 約59枚**（SEQUENCE←REVEAL_AND_PICK 23／REVEAL_AND_PICK←LOOK_AND_REORDER 23／SEQUENCE←LOOK_AND_REORDER 10 等）。**設計図は `docs/parser_worklist.md`「🅑 Stage B 着手記録」＋R11.5 の型ペア診断に完成済み**。R4 で「新ハンドラ追加は+129退行」確認済＝必ず **effectParser.ts:1210 の誤発火分岐をピンポイント修正＋1222分岐拡張**（広域ハンドラ追加禁止）。**毎ステップ build→`parserWorklist.ts` で held合計が増えていない（横取り退行ゼロ）ことを必ず確認**。
  2. **【済】GRANT_ACCE_HOST_ABILITY**（R13 で wrapper 復元・8枚 LOSS→VALUE）。残6枚（WX16-074/WX17-075/WX17-077/WXK10-074/WXK10-075/WDK07-E14）は内側 MANUAL 専用で再現不能＝LOSS 残（着手しない・最終 VALUE レビュー送り）。
  ...
  - **⚠ R12 の教訓**：worklist の「action.type 77（LOSS）」は**構造差で lost leaf を持つもの**＝LOOK/REVEAL 一族が本体。型の値違いだけ（例 ENERGY_CHARGE↔SEND_TO_ENERGY）は **VALUE**。診断時は LOSS/VALUE を必ず区別する（`parserWorklist.ts` のセクションで判定）。
