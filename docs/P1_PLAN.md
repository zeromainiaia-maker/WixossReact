...
- **最終更新**: 2026-06-26
- **直近やったこと**: 機構③（field宛先）＋`levelLteLastProcessed` フィルタ（WX21-022/WX24-P3-026/WX25-P1-039）＋**engine解決の穴埋め（execBounce/execSearch に lastProcessedCards を渡す）＋WXEX2-17 配線**。同型★0維持。→ `BUGFIXES.md` 先頭参照。
- **重要な調査結果**: 「同じ壊れ方を一括で潰すクリーンな横展開系統はほぼ枯れた」。残りは(a)複数択の中の択落ち、(b)カード丸ごとの誤parse、(c)機構待ち、に分散。→ **今後は“小機構＋少数配線”or“個別★割れ”が中心**。
- **engine注意（今回判明）**: 動的フィルタ（`*LteLastProcessed` 等）は**アクションごとに `resolveDynamicFilter` へ `lastProcessedCards` を渡しているか個別確認が要る**。確認済で渡す＝execBanish/execSendToEnergy/execBounce/execSearch/REVEAL_AND_PICK/applyDirectAction。**未確認の他アクションに同フィルタを付ける時は解決経路を必ず確認**（渡っていないと逆翻訳だけ出てengine無視＝偽陽性）。
- **次の一手（候補・上から推奨）**:
  1. **「コストで捨てたシグニのレベル」参照の小機構**（WX22-046／WXK10-044-E2 が待ち。`handDiscardSigni` で捨てたカードのレベルを追跡する変数が要る。`levelEqDiscardLevelSum` 系の近傍）。
  2. **個別★割れ/誤parseの精査**：WXK10-055（E2 丸ごと誤parse）等を1枚ずつ。`grouped_sentence_all.txt` の★（偽陽性§4を先に除外）。
  3. **大型機構**（§5。1つ着手したら §5 の状態を `着手中(名前)` に）。最有力＝《相手ターン》トリガー基盤。
- **着手中の機構**: なし
