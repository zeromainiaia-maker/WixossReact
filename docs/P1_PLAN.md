...
⑤単発STUB 19 id（保護/代替コスト/グロウ/アタック制限等・全カードの原文 ac を `scanRemain.mjs` で網羅確認し本体のみ固定文化＝二重/脱落回避）。生STUB（id露出）は**当初41種→6種**に激減。
  - **残6種は個別対応要**＝USE_SPELL_FROM_TRASH/ARTS_COLORLESS_MUST_PAY_CENTER_COLOR/CONDITIONAL_GROW_AND_KEY_DISABLE（action STUB 0件＝activeCondition/CHOOSE等の別経路・複雑）・SIGNI_GRANT_CHOSEN_ABILITY（CHOOSE構造）・BLACK_RISE_PLAY_STACK_FROM_TRASH/BANISH_ATTACKER_IF_WEAKER_THAN_FRONT（AUTO trigger 絡み＝timing描画との結合確認要）。`grep -ohE "\[STUB:[A-Z][A-Z0-9_]+\]" docs/decompile_sheet*.txt` で残一覧。**ここまでで生STUB id露出はほぼ枯れ。次は description付きSTUB（`[STUB:日本語説明]`）の原文反映 or 実機検証(C2) が候補。**
