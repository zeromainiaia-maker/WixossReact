...
- **🆕 セッション（2026-07-03・zerom・続き2）＝BEHAVIOR_AUDIT 基盤（段階1）を実装**＝`scripts/behaviorAudit.ts` 新設（`npm run audit` / `audit:queue`）。中核①シナリオビルダー（ラベル付きトークン一次版）②盤面差分器（両者全ゾーンを instanceId 追跡・自然文化・`⚠側跨ぎ` 強調）③トレース出力（原文｜逆翻訳｜差分｜ログ）④要レビュー・キュー（非CONTINUOUS×無変化×低情報ログ＝**811/9269効果**・`docs/_behavior_queue.txt`）を一次実装。全効果を約4秒・決定論・トークン0。docの矛盾（未実装／semanticと相互に主軸）を解消＝behavior主軸・semantic補完。**残＝段階2 シナリオビルダーを効果別対象配置に拡充**（811の大半は空振り＝低パワー/凍結/レベル/クラス対象・trash内source 等を配置すれば消え、残りが真no-opバグ）→段階3 HTML表→段階4 キュー消化。詳細 [BEHAVIOR_AUDIT.md](./BEHAVIOR_AUDIT.md)「実装状況」。
