...
検証: `scripts/archive/_verifyTokenGen.ts`（14項目パス）。`scripts/archive/_verifyG039.ts`（7）/`_verifyRakkaServantZero.ts`（7）に回帰なし。
...
| WX25-P1-TK1〜6 ダーク系（バウンダリー/背闇之陣/アナライズ/闇気揚々/アウト/ヤミノザンシ） | WX25-P1-034 創造の針姫 ヤミノ＝Ⅲ | TK1〜5=E2 `CHOOSE(2/5)`→`ADD_CRAFT_TO_LRIG_DECK`(日本語名解決) / TK6=E1 ON_LEAVE_FIELD `ADD_TO_FIELD` cardName | ✅ 生成確認済（名前完全一致・クラフトType・常時ロード）。各クラフトの効果本体実装は別途 |
...
> 検証: `scripts/archive/_verifyG039.ts`（7項目パス）。
...
- 付与配線（`scripts/archive/fixBarrierGrants.mjs`）: パーサが `GRANT_KEYWORD(keyword:"○バリア")` という
...
### CHOOSE 再構築で対応済み（scripts/archive/fixSacredForceChoice.mjs / fixFutureSessionChoice.mjs）
...
- **WX25-P3-050 エビディバ!!!!!**（`scripts/archive/fixEvdivaPerLrig.mjs` / stub `EVDIVA_PER_LRIG_COLOR`）…
  場の色別ルリグ数ぶんに実行。白=ルリグバリア / 青=ドロー3 / 緑=エナチャージ3 / 黒=相手ミル10 を決定的に実装。
  ※赤（合計12000以下バニッシュ）は対象選択が必要なため未実装ログのみ。
- **WXDi-P15-003 ひらけ！ゲート！**（`scripts/archive/fixOpenGateGrant.mjs`）…
  `GRANT_LRIG_ABILITY.abilities` を実体化。センタールリグに『【起】エクシード4：シグニバリア』『【起】エクシード4：ドロー4』を付与（`lrig_granted_auto_effects` 経由で起動可能）。
- **PR-Di035 OPEN DREAM LAND!**（`scripts/archive/fixOpenDreamLand.mjs` / stub `PRDI035_PARADISE_COLOR`）…
...
- `scripts/archive/_verifyBarrier.ts`（`npx tsx scripts/archive/_verifyBarrier.ts`）でエンジンレベルの自動検証（12項目全パス）。
...
- ~~FUTURE SESSION ②（プリオケ1枚をエナへ送る2段階ピック）~~ → **実装済み**。
  ...
  検証: `scripts/archive/_verifyFutureSession2.ts`（22項目パス）。
...
- ~~PR-Di035 青の相手手札捨ては「先頭3枚固定」で近似~~ → **本実装済み**。即時効果適用後に
  ...
  インタラクションになる（`PRDI035_APPLY_PARADISE`、`scripts/archive/_verifyPrDi035Blue.ts` 13項目パス）。
