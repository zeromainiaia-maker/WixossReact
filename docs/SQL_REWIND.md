...
> 🏁**2026-09-23 に本番へ適用済み**（ユーザーが Supabase の SQL Editor で実行）。
> **実機確認まで通っている**＝`node scripts/verifyRewind.mjs`（§5.1 `V-287`・13項目）。
> ⇒ **もう流す必要は無い。** この先この機能を直すときの**正本**として読む（列・トリガー・RPC の契約）。
> 🔑**新しい環境（別プロジェクト・クローンし直し）へ移すときだけ**、下をもう一度上から流す。
