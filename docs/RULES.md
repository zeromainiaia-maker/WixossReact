...
| R-47 | 同じルリグタイプのルリグが複数場にあるなら〜（EN Rule-based action 5）⇒ 🔑**2026-09-17 ユーザー裁定＝これは「構築の制限」**＝**センタールリグと同じルリグタイプのアシストルリグは入れられない** | `utils/deckBuildLimits.ts` `deckAddBlockReason`（`LRIG_TYPE_CLASH`）＋ `sharesLrigType` | 🔴→✅（**場のルール処理は実装しない**＝同じルリグタイプが場に複数並ぶ状態は**構築で作れない**。⚠**両方向で止める**（アシスト先→同タイプのセンターの順も塞ぐ）／⚠**アシスト同士の同タイプは止めない**（裁定の範囲外）／⚠複合タイプ（`花代/ユヅキ`）は**1つでも重なれば**同タイプ。`V-262`。⚠**既存の保存済みデッキは遡って弾かない**＝追加時の判定だけ） |
