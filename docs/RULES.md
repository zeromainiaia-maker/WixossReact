...
| R-48 | シグニのレベルは**センタールリグのレベル以下**・合計は**リミット以下**（EN Level・Limit） | 合計＝`R-44` でルール処理化済み／**レベル以下は配置ゲートのみ**（手札召喚の `levelOk`／`SigniSummonZoneModal`／`ResonaSummonModal`／CPU） | ❓（2026-09-17＝`planLimitExcess` は `levelOverZones` を**測る**が落とさない。理由＝①原文コーパスで**ルリグのレベルを下げる効果は0枚**（実測）＝配置後にレベル超過になる live の道が無い ②自動トラッシュは**盤面を注入する実機シナリオ27本にしか当たらない** ③「レベル以下」が**配置制限**か**ルール処理**かを一次資料で確定できていない。⇒ **ユーザー判断待ち**（決まれば funnel に1分岐足すだけ）） |
