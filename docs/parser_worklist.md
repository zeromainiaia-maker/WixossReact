...
- 2026-06-27 R15（filter その他・トラッシュ対象の hasGuard/noGuard／ymst）: 「トラッシュから《ガードアイコン》を持(つ/たない)〔カード/シグニ〕を対象とし、それらを〔エナ/手札〕」の guard フィルタ脱落を是正。`parseSentencePart1.ts` の TRANSFER_TO_HAND（spanTxt限定）＋ENERGY_CHARGE（1880）に noGuard/hasGuard 付与（engine matchesFilter 実装済み 433/438）。WXDi-P01-030/P07-029/P00-025/P01-011。**LOSS 154→150（−4・filter その他 34→30）／held 305→301**。パーサーのみ・データ無変更。同型★0維持・typecheck緑。**⚠commonClass（P10-029/CP01-020/CP02-046）は engine 未実装＝保留**（types/decompiler のみ・matcher 無し）。
- 2026-06-27 R14（Stage B・先頭条件文の CONDITIONAL 化／ymst）: action.type「← -」系（HEAD は CONDITIONAL だがパーサーが条件を落とす）2系統。①LIFE_CRASHED_THIS_TURN「このターンに〔owner〕のライフクロスが〔N枚以上〕クラッシュされていた場合、」（WX18-063/064・WXDi-P16-065・WX11-021）②LAST_PROCESSED_HAS_TYPE「この方法でトラッシュに置いたカードの中にスペルがある場合、」（WX12-054/055）。engine 実装済み（execUtils.ts:672/:1003）。⚠ON_TURN_END 札は actionText 先頭に「各ターン終了時、」が残るため正規表現で `(?:[^、]*?(?:終了時|開始時)、)?` を食わせて吸収。**LOSS 160→154（−6・action.type 60→54）／held 311→305**。パーサーのみ・データ無変更（HEAD は元から CONDITIONAL）。同型★0維持・typecheck緑。**没ネタ**＝「デッキ一番上公開→の場合」の SEQUENCE[DECK_TOP_MATCHES] 化は HEAD が REVEAL_AND_PICK 多数派（94）/SEQUENCE 少数派（13）の inconsistent curation で +94 退行→revert（R13 と同じ罠）。
- 2026-06-27 R13（Stage B 本丸・LOOK_AND_REORDER→REVEAL_AND_PICK 統一／ymst）: 「デッキ上N枚見る→その中から〔filter〕を[N枚まで][公開し]手札に加え、残りをデッキ/トラッシュ」のピック文を REVEAL_AND_PICK 化。**engine の execLookAndReorder は手札に加える経路が無い**（effectExecutor.ts:2214）＝LOOK_AND_REORDER のピック札は実機で手札に入らない実害バグ。`effectParser.ts:1218` の LOOK 分岐が remainder の「デッキ」で誤発火していたのを、手札取得時は先に REVEAL_AND_PICK を試すよう是正（§🅑 R4 予告の誤発火）。**未対応の言い回し（好きな枚数=ALL/色filter/非ガードアイコン/パワー・コスト）は除外ガードで LOOK 据置＝誤表現を作らない**。データは HEAD が LOOK のままだった **123 subtree（≈116枚・8枚は CHOOSE/SEQUENCE ネスト）を surgical 置換**（他リーフ温存）＋REVEAL filter の cardClass→story 正規化5枚。**LOSS 178→160（−18・action.type 78→60）／held 339→311**。VALUE 不変・他バケツ全不変（regression 0・count は migration を effectId/ネスト対応にして復帰）。同型★0維持（★0/265グループ）・typecheck緑。**教訓**＝この一族は「16枚が held」ではなく「16枚が held＋138枚が LOOK↔LOOK で一致（実害バグ）」。held だけ見ると小さいが engine 実装まで揃えるのが正道。除外した好きな枚数/色/アイコン等は次の parser 拡張ラウンド。
