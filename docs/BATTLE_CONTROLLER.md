...
| 🆕`controller/performArts.ts`／`performKeyPiece.ts`／`performGrow.ts`／`performSigniAttack.ts`／`performGuardResponse.ts`／`performLifeBurstResponse.ts` | **残りの実行関数**（2026-09-18＝`perform*` 12/12 移設完了）。⚠画面のモーダルは**任意コールバック**で受ける。 |
...
| 🆕`controller/cpuTurn.ts` | **CPU の1手**（§5.7 `S-5c` 第3段・2026-09-18）＝`cpuTurnAction`（1,362行）。材料は `PerformCtx`、画面だけが持つものは `CpuTurnDeps`＝**`actions: CpuTurnActions`（実行関数の画面ラッパ14本＝UI コールバック込み＝人間と同じ経路）**／`allCards`（⚠`PerformCtx.cards` とは別物）／`cpuPlan`／`checkPowerZeroBanish`。場以外の【起】2本は行為者つきなので実行関数を直接呼ぶ（画面ラッパと同値）。golden が「移設先で `perform*Impl(` を直接呼ばない」を見張る。 |
...
| 🆕`src/screens/battle/controller/artsUseTriggers.ts` | **アーツ使用トリガーの収集**（§5.3 `O-131` の2本・2026-09-18）。⚠`ON_ARTS_USE` は使用者の client、`ON_OPP_ARTS_USE` は相手の client だけが集める（二重押し防止）。 |
