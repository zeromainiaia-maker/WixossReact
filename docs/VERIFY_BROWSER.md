...
### 🔎 ON_DECK_SHUFFLED のシャッフル源依存（要 engine 調査・1点）
`deckshuffle` は**シグニ【出】源（WX12-Re20 ベルフェーゴ）では PASS**＝既定スイートに復帰済み。一方 **スペル源（SEARCHER WX02-060《無》×1）では発火しない**：スペルは「相手カットイン応答待ち」を挟んで解決し、解決後も watcher は +5000 されなかった（PR-470A は 5000 のまま）。
- 切り分け＝**シグニ【出】＝スタック解決経路（中央 diff `resolveStackNext`/`BattleScreen.tsx:4767` を通る）＝発火**／**スペル＝カットイン解決経路＝中央 diff を通らず deck_shuffled_count の before/after 差が観測されない**。`collectDeckShuffledTriggers` は golden 緑なので、不足は**スペル解決経路でのシャッフル検出配線**（engine 実機・要 follow-up）。
- スペル発動の安定セレクタ `spellcost-energy-{i}` は追加済み（「発動する」は exact 一致で取る＝CardModal「発動」と区別）。再試行は容易。
