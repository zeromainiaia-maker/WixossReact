...
| 🆕(xlvi) | **原文は「手札に加える」なのに curated が `LOOK_AND_REORDER` のままで、pick が live で実行されない系統**（カードアドバンテージが死ぬ）。✅続き218g で9効果を外科採用。✅**2026-07-29 に4波で29効果を消化**＝第1波5（`pk` 規則を `hasRiseIcon`／`isDisona`／`hasLifeBurst`／色OR へ拡張）＋第2波12（独立SEARCHの `LOOK_PICK_CHAIN` 展開・`remainder.shuffle?`）＋第3波4（中段 `then:'deck_top'` を engine 実装）＋**第4波8＝(e)(f) 残0**（表記ゆれ「N枚**を**見る」／ディソナ複合の後続文を `SEQUENCE` で引き連れ／**OR記述子 `parseOrPickDescriptor`＋新 `TargetFilter.anyOf`**／PRESERVE カードの外科採用）。census 1476→**1463**・golden 891→**936**。<br>**残＝26効果**（第4波で (e)3・(f)2 が残0。live JSON 走査で確定した実数）。分類：**(a) 動的filter 7**〔場のルリグと共通色＝`SPDi01-131-E1`／`WXDi-P02-017-E1`／`WXDi-P15-031-E1`／`WXDi-P15-005-E1`、対象と同名＝`WXK04-045-E1`、共通クラス＝`WXK08-025-E3`／`WX25-P1-041-E1`〕**(b) 複合・条件 10**〔`WXDi-P03-061-E2` は**held 群A＝採用すれば pick は入る**が「加えた枚数ぶん捨てる」が落ちる／`WX12-Re10-E1` 条件付きpick／`WX24-P4-037-E1`／`WX26-CP1-061-E1`＋`-SONG`／`WXDi-P05-015-E2` 束分け／`WXDi-P08-007-E3` REPEAT内／`WXDi-P09-066-E1`／`WXK02-001-E2`／`WX25-P3-052-E1`（エナ経由＝要偽陽性判定）〕**(c) 宣言系STUB 4**〔`PR-434-E1`／`-BURST`／`WX11-037-E1`／`WX24-P1-035-E1`〕**(d) hand-or-energy 3**〔`WX24-P1-039-E2`／`WXDi-P16-086-E1`／`WX11-074-E1`〕**(g) トラップ設置併記 2**〔`WX15-083-TRAP`／`WX19-039-E1`〕。<br>🆕**(h) 別系統の新規在庫＝`REVEAL_AND_PICK` は出ているが filter だけ落ちている17効果**（＝**どのカードでも拾える過剰実行**。`LOOK_AND_REORDER` 系ではないので従来の在庫スキャンに載らなかった）：`WX08-043-E1`／`WX20-072-E1`／`WX21-028-BURST`／`WX21-037-E2`／`WX22-030-E1`／`WXEX1-12-E1`／`WX24-P2-031-E1`／`WX26-CP1-012-E1`／`WX26-CP1-014-E1`／`WX26-CP1-016-E1`／`WXK06-011-E1`／`WXK06-017-E1`／`WXK07-013-E1`／`WXK07-019-E1`／`WXK07-024-E1`／`WXK08-027-E2`／`WDK01-008-E1`。**主因は `LOOK_AND_REORDER + STUB(REVEAL_PICK_HAND_SHUFFLE_BOTTOM)` の融合規則（`effectParser.ts`）が filter を一切運ばないこと**＝ここに filter を通せばまとまって直る見込み。**次に取るなら (h)→(a)** の順が費用対効果が高い。詳細 BUGFIXES 続き218g／2026-07-29節（第1〜4波） |
...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕単独）＝タスク12(xlvi) 第4波＝look-pick の表記ゆれ・後続セグメント・OR記述子を消化し8効果を復元**（golden 926→**936**・census 1468→**1463**〔`BASELINE_HIGH` 更新〕・smoke 10726 全0・fuzz 全0・同型★0・held 257→**263→257**〔対象6カードを `heldReview --adopt`・新規ドリフト0〕）
  ...
  - **(e) 後続セグメント**＝`WXDi-P12-001-E1` は「ディソナのカード1枚を手札＋【シグニバリア】【ルリグバリア】」。pk 規則が**後続文を早期 return で落とす**ためディソナ複合形を規則ごと除外しており、pick もバリアも消えていた。→ 後続文を `SEQUENCE` で引き連れる形へ。⚠**一律に足すのは不可**＝「この方法で〜した場合、」等の条件付き後続が条件ごと落ちて**無条件実行**になる（`WX25-CP1-025-E1` の BOUNCE 等**21効果で実測**）ので、無条件の後続だけを通す条件に絞った。
  - **(f) OR 記述子**＝`parseOrPickDescriptor` を新設し、**独立節の OR**（「スペルか＜原子＞のシグニ」）を新フィールド **`TargetFilter.anyOf`**（`matchesFilter` が再帰評価・兄弟キーとは AND）へ、**名詞共有の修飾断片 OR**（「＜天使＞か＜悪魔＞のシグニ」「白か黒のシグニ」）を既存の `story`／`color` 配列へ落とす。**`cardType` の配列では表現できない**のが要点＝`['スペル','シグニ']`＋`story` にすると **＜原子＞がスペル側にも掛かる**。忠実表現できない形は null を返して従来経路へ落とす（過剰 filter より取りこぼし側に倒す）。
  - **⚠ 同系統で見つかった別種の実害＝「filter だけ落ちた `REVEAL_AND_PICK`」**。`WX24-P1-023-E1`／`WX25-P2-037-E1`／`WXEX1-20-E1` は **どのカードでも拾える過剰実行**（前2者は「合計2枚まで」も 1枚固定に潰れた過小実行を同時に併発）。この形は `LOOK_AND_REORDER` 系ではないため**従来の (xlvi) 在庫スキャンに載っていなかった**。全数スキャンで**17効果**を確定し §3 (xlvi)(h) へ登録（主因は `LOOK_AND_REORDER + STUB(REVEAL_PICK_HAND_SHUFFLE_BOTTOM)` の融合規則が filter を一切運ばないこと）。
  - **PRESERVE カードの外科採用は今回も2件**＝`WXDi-P05-050`（`PARTIAL`）と `WX24-P2-049`（`MANUAL` の `E1b` 持ち。しかも fresh 側では effectId が **E3 へずれる**ため heldReview でも届かない）。effectId アンカーで action だけ差し替え `MANUAL` 化し、兄弟効果の不変を golden で固定した。**4波連続で再発している型。**
  - **golden の書き方を固定**＝**フィルタは構造ではなく候補集合で assert する**（(xliv) の教訓）。公開窓に必ず**非該当カードを混ぜ、それが手札に入らないこと**まで見る。live JSON を HEAD へ戻すと当該 assertion だけが FAIL することを2形（`LOOK_AND_REORDER` へ差し戻し／`filter` だけ削除）で実測済み。
  - **計器**＝decompiler に `anyOf` 描画を追加。ついでに `GAIN_SIGNI_BARRIER`／`GAIN_LRIG_BARRIER` が**両方とも【ルリグバリア】と表示されていた逆翻訳バグ**を是正。
  - **次の一手**＝**Opus：(xlvi) は残26件**＋新規 **(h) filter落ち17件**。**(h)→(a) 動的filter7件** の順が費用対効果が高い（(h) は融合規則1箇所に filter を通せばまとまって直る見込み）。**／(xxix) 残＝任意+cost 933効果と段階3 65効果（タスク12(lv)）／ゲート脱落 [B]20件（Condition 語彙の新設）／タスク16 の [B]29件＋watcher [B]4件／(l) 内側【自】parse 失敗3枚／§6.3 残機構（H1〜H3・I）**。**Sonnet：§7 実機検証の横展開**＝レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓・escapeDiscard 回避モーダル・BET 選択肢・アーツ使用条件20枚。
