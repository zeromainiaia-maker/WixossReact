...
| (lxi) | **🆕「対戦相手が〜しないかぎり」＝支払い回避クローズの無言脱落（2026-07-30 実測・トップレベル未配線60効果）**。原文は「対戦相手が《無》を支払わない／手札をN枚捨てないかぎり、Xする」で、**回避クローズが parser で無言消費され X が無条件実行される過剰実行**。**engine は完備**＝`STUB{OPPONENT_PAY_OPTIONAL, costColors, opponentHandDiscard}` ＋ 直後 `CONDITIONAL` の look-ahead ペア（`effectExecutor.ts:3069`・再開 `resumeOpponentPayOptional`）。parser（`effectParser.ts:2456`）が**エナ型かつ文頭形しか拾っていない**のが唯一の穴＝**1規則で N 効果**の系統バッチ。2026-07-30 に `WX24-P4-018-E2` の1件だけ限定配線して着地済み（live 波及0）。**除外**＝①【ガード】ができない族（`GUARD_EXTRA_COST_BY_OPP`／`OPP_GUARD_COST_COLORLESS` の領分）②アタック制限・無効族（`negated_attacks_escape`／`escapeDiscard` の領分）③引用付与の内側33効果（fresh では既に届いており held 2件が計器＝`WX24-P1-071`／`WX25-P1-005`）④「以下をN回行う」を含むもの⑤主語が「あなたは〜捨てないかぎり」の自己支払い型（`DISCARD_OR_PENALTY` の領分）。⚠**採用時は per-effect 実 diff から採用リストを作り、巻き込み全件を原文照合すること** |
...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が指示と検証＋codex-work が実装）＝PLAN §4 次の一手①「エクシードルリグ本体不一致6件」のうち5件（群A〜E）を配線・群F は honest defer**（golden **1109→1113**・census **1386→1384**（`BASELINE_HIGH=1384`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（5986枚・265群）・held **288→290枚/106→107署名**・manual field loss 0。ゲート独立実行で全緑／live per-effect 差分 **changed 8 / removed 0 / added 1**〔added は入れ子 `WX24-P4-011-E2-next-attack`＝想定内〕）
  - **投入前実測で PLAN 自身の見立てを訂正したのが決め手**＝「本体不一致」は「機構が無い」と読めるが、実測すると**6件中5件は engine 機構が完備で parser／データ側の穴**だった。①`DrawAction.untilHandCount` は実装済みで同一文型の `WX05-003-E3` は MANUAL で手当て済み ②相手の支払い回避は `STUB{OPPONENT_PAY_OPTIONAL}`＋直後 `CONDITIONAL` の look-ahead ペアが完備で **`opponentHandDiscard`（手札捨てで回避）まで実働**、parser が**エナ型かつ文頭形しか拾っていない**のが穴 ③`LifeCrashAction.optional` と `execLifeCrash` の `lastProcessedCards` 記録は既存 ④`lrig_granted_auto_effects` レジストリは収集・クリアとも既存 ⑤群指定は `discardGroups`／`energyTrashGroups`／`fieldTrashGroups` が先例。⇒ 指示書で「新機構を作るな・**この既存語彙を拡張しろ**」と名指しした結果、**新設は3本だけ**（`transferGroups`／`addLastProcessedCount` の水平展開／`consumeOnTrigger`）で、過去2回起きた「同義の既存語彙の横に並行新設」事故は**非再発**。
  - **消化した5件**＝**A** `WX24-P4-014-E2`＋兄弟3件（`WD16-012-E3`／`WX13-016-E1`／`WX26-CP1-004-E1`）を固定1枚から `DRAW.untilHandCount` へ（**逆向きの `WDK08-Y08-E1`＝「多い場合→エナに置く」は非マッチを golden で固定**）／**B** `WX24-P4-018-E2` を `OPPONENT_PAY_OPTIONAL{opponentHandDiscard:3}` ペアへ（無条件バニッシュの過剰実行を停止）／**C** `WX24-P4-015-E2` に `LIFE_CRASH.optional` と `EffectTarget.addLastProcessedCount` を配線（skip=1体・crash=2体）／**D** `WX24-P4-011-E2` を即時UPから `ON_ATTACK_LRIG` 付与AUTO＋`consumeOnTrigger` の1回限り発火へ／**E** `WX24-P4-017-E2` に `transferGroups` を追加しスペル1枚まで＋青シグニ1枚までの2群回収へ。
  - **⚠Claude 検証で是正3件（いずれもゲートが検査しない次元）**＝①**BOM 混入2件**（`scripts/vocabCensus.ts`／`src/screens/BattleScreen.tsx` の先頭に U+FEFF）＝**§5-19 のエンコーディング検査（U+FFFD と3連続以上の `?`）では BOM を検出できない**うえ typecheck も lint も BOM を通すため**全緑のまま素通り**していた。⇒ CODEX_GUIDE §5-19 の検査項目に **BOM（`efbbbf`）** を追加した。②**`WX24-P4-017-E2` の逆翻訳から移動元ゾーン「あなたのトラッシュから」が脱落**（JSON は正しく `TRASH_CARD`＝`transferGroups` が群ごとの filter しか持たないため）。`transferGroupZoneJa()` を新設して source からゾーンを前置き。③`BASELINE_HIGH` の履歴追記漏れ（数値だけ書き換わっていた）を規約どおり補った。
  - **⚠codex の説明が1点誤り＝held +2 の帰属**。codex は「今回の live 差分外＝HEAD 時点の未再生成ドリフト」と説明したが、`docs/_held_fresh.json` を実測すると **`WX24-P1-071` と `WX25-P1-005` は本バッチの群B規則で新たに `OPPONENT_PAY_OPTIONAL` を獲得**しており原因は本バッチ。**live 挙動は不変**（held＝未採用）で方向も正しい（curated が落としている回避クローズを fresh が獲得）＝**群Bの regex は指示書で除外した「引用付与の内側」族へ fresh レベルでは届いている**。この2件は次バッチの採用候補。⚠指示書で挙げた `effectExecutor.ts:190-191`（ターン境界クリア）は**Claude 側の誤引用**で、群D の寿命は `BattleScreen.tsx:3321`／`3702`／`9433` の3経路が正（独立確認済み・`permanentGrant` のみ残す＝今回の付与は「このターン」で正しく消える）。
  - **⚠実測した家族サイズ（次バッチの弾）**＝群Bの「〜ないかぎり」は**トップレベル未配線60効果＋引用付与の内側33効果**の大家族で、**全部が同じ過剰実行バグ**（回避クローズが無言で消え効果が無条件実行される）。engine は完備なので **parser 規則1本で N 効果**が取れる形＝§3 Opusタスク12 (lxi) に登録。今回は「1件必達＋巻き込みは全件原文照合」に絞り、live 波及0で着地した。
  - **次の一手**＝**Opus：①🆕「〜ないかぎり」家族の本消化**（タスク12 (lxi)＝トップレベル60効果。engine 完備・parser のみ。除外は【ガード】族／アタック制限・無効族〔`escapeDiscard` の領分〕／自己支払い型）**②`WX25-P1-056-E1`**（`leaveReplaceBanish` の no-op＝共通離場フック `applyEffectLeaveLrigAbilitySubstitute` で閉じられる見込み。任意選択とバニッシュ先処理の追加が要る）**③「コストの合計」族の STUB/UNKNOWN 14件④`WX11-043-E1` の `crossOnly` 欠落⑤タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑥`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑦§6.4 の配置数制限すり抜け7効果⑧🆕 §6.3 I＝`WX25-P3-028-E2`（群F defer）の3機構**（リフレッシュ不可の2ターン寿命／N回反復〔実データ5効果〕／各回の owner 選択）／(xxix) 残4・(lv) 残2経路・(lx) 2件・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**今回のバッチが持ち込んだUI3件**＝(a) 群B の相手側「支払う／手札3枚捨てる／支払わない」CHOOSE（手札不足で `discard` が選べないこと・不払い時だけバニッシュ対象選択が出ること）(b) 群C の任意ライフクラッシュ（クラッシュしない→1体／した→2体・ライフバースト解決を跨いで対象数が2のままであること）(c) 群E の2群ピッカー（スペル枠と青シグニ枠が独立に「1枚まで」で、片方0枚でも成立すること）／既存＝§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 エクシード本体6件（次の一手①）5件消化後 最新値（本行を直近の正とする）**：census **1386→1384**（`BASELINE_HIGH=1384`）、golden **1109→1113**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288→290枚/106→107署名**（+2＝`WX24-P1-071`／`WX25-P1-005`。**本バッチの群B規則で fresh が `OPPONENT_PAY_OPTIONAL` を獲得したのが原因**＝codex の「未再生成ドリフト」という説明は誤りで、Claude が `_held_fresh.json` 実測で訂正。live 挙動は不変＝未採用のまま次バッチの採用候補）、manual field loss 0。live per-effect 差分 **changed 8 / removed 0 / added 1**（added は入れ子 `WX24-P4-011-E2-next-attack`）。新語彙は `TransferToHandAction.transferGroups`・`EffectTarget.addLastProcessedCount`（`DrawAction` 既存名の水平展開）・`CardEffect.consumeOnTrigger` の3本のみ。⚠census 1384 には **`untilHandCount`／`transferGroups` を計器が語彙として認識する較正**が含まれる（新語彙を数えないと偽陽性になるため）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1113**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1384**（回帰ゲート）・held **290枚/107署名**（2026-07-30 エクシード本体5件消化後に実測）。
>
