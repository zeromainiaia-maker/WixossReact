...
**直近＝2026-09-23＝🆕**ユーザー依頼4件（ログの読みやすさ3件＋「何手目に戻る」）＝SQL 適用後に実機 PASS**（全文は [BUGFIXES.md](./BUGFIXES.md) 第469バッチ）

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🏁**本線キューは全部0**（`V-287` は 2026-09-23 に実機 PASS で返済）＝次の一手はユーザーの設計判断 |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 🏁実機 `V-nn` **0**／🏁機構 `O-nn` **0**／実装キュー **0**／🏁CPU 完成度 **0**／🏁CPU の強さ **0**／リリース作業 **1**（ユーザー）／**作戦データの入力 21デッキ**（ユーザー） |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4406**）｜**実機 PASS**＝`verifyFullMatch.mjs cpu`（6ターン/176手）＋`verifyRewind.mjs`（`V-287` 13項目すべて OK・反転2件込み） |
| ✅**ユーザー作業** | 🏁**[SQL_REWIND.md](./SQL_REWIND.md) は 2026-09-23 に適用済み**（もう流さない＝以後は列・トリガー・RPC の契約の正本）。残っているのは [RELEASE.md](./RELEASE.md) の1件だけ |
| 🆕🔑**この回の芯** | **DDL が要る機能は SQL を docs に置いてユーザーに渡す**（Claude は anon キーしか持たない）＝**アプリ側は列の有無で入口ごと隠す**ので、流す前でも一切壊れない。**2人の同意が要る機構は通し対戦と別の実機ハーネスで測る**（`verifyRewind.mjs`＝決着まで回さない） |
...
- 🆕**2人の同意が要る機構は `node scripts/verifyRewind.mjs`**（`V-287`＝PvP 2ブラウザ・約3分）。
  🔑**通し対戦と分ける**＝確かめたいのは勝敗ではなく「申請 → 同意 → 盤面が戻る」なので決着まで回さない。
  プラミング（build/preview/login/セットアップ進行）は `verifyFullMatch.mjs` から **import して共有**する
  （🔴あちらの main は「直接実行したときだけ」走る＝import で対戦が始まらない）。
...
**残2**（2026-09-25 ユーザー「後で」＝下の2件）。`S-35` は 2026-09-23 にクローズ＝BUGFIXES 第468バッチ。

| ID | 何をするか | 出所 |
|---|---|---|
| `S-36` | 🟡**第1段 済（2026-09-26・BUGFIXES 第473バッチ）**＝原文の全文表示／相手の札・自分の札で別々の狙い方／「効果後に正面を上回る」（自分・相手）／「アップ状態を優先」（自分・相手）。**続きの拡張はユーザーの指示待ち**。— 旧：**作戦の「効果の狙い方」を拡張する**＝いまの狙い方は3つ（強いもの／落とせるもの／弱いもの）と条件4つだけ。2026-09-25 に**効果単位（E1/E2/ライフバースト）**で書けるようにしたので、次は狙い方・条件の語彙を増やす。受け皿＝`resolveCpuTargetMode`／`pickCpuTargets`（`cpuDeckPlan.ts`／`cpuInteraction.ts`） | ユーザー（2026-09-25「良い実装なので後で拡張する」） |
| `S-37` | 🏁**済（2026-09-26・BUGFIXES 第478バッチ）**＝使いどころは**アタックフェイズに使える札だけ**（守り／攻め／両方／使わない・その札が持つ窓だけ）。スペルは一覧から外した。アシストグロウをアタックフェイズの窓にも配線。⚠残＝場のシグニ・ルリグの《アタックフェイズアイコン》【起】を相手のターンに使えるか（gate は自ターンだけ）＝ルール確認待ち | ユーザー（2026-09-25「後で調整する」／2026-09-26 仕様決定） |
🏁**消化済み（`S-1`〜`S-35`）の行は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「§5.7 取る順の消化済みの行（退避）」へ移した。**
