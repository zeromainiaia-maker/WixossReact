...
**直近＝2026-09-21＝§5.7 `S-17` 第3段＝アタックの「損」（ライフバースト・ガード）を確率で見るようにした**（全文は [BUGFIXES.md](./BUGFIXES.md) 第428バッチ）
- 🏁**期待損のチャンネルを1本足した**（`CpuSimBoard.risk` ＋ 新設 `cpuAttackRisk.ts`）＝探索が終端の点数から**手順に沿って累積して**引く。
  🔴**読むのは公開ゾーンだけ**（相手のエナ・トラッシュ・場の中身と**手札の枚数**）＝golden がソースを検査する。
- 🔑**確率は超幾何**（構築時に枚数が決まっている＝**見たバーストが増えるほど残りは下がる**／ベータ混合は符号が逆）。`LB_MAX = 20/40` は**構築ルールの定数**。
- 📏**実測で登録票の前提が外れた**＝LB は**全カード 26.1% に対し実デッキ 48.9%**（ほぼ全デッキが上限）／ガードは **0.3% に対し 18.9%**（中央 8/40）。
- 🔴**既定は 0＝実機は1ビットも変わらない**（`lifeBurstCost` / `guardDeckCount`・`searchAttacks` も false のまま）。A/B は `search-attack-*` プリセット。
- 🔴**自分で入れた穴を2件直した**＝①**ライフ0枚で割れないのに期待損を払っていた**（盤面は動かないのに点数だけ下がる）
  ②🔑**「撃たない」の誤帰属**＝96戦で出た「撃たない」**3件は全部 `risk === 0`**＝条件④（`actionRisk > 0`）を足した。
- 📊**A/B（`search-attack` vs `search-attack-risk`・6デッキ × 8シード＝96戦）＝48組のうち46組が1勝1敗＝差があるとは言えない。**
- 🔑🔴**なぜ変わらないかは解析で出る**＝ライフ1枚の価値は `life − energy = 6000`／期待損は `p × lifeBurstCost`（`p ≤ 1`）
  ⇒ **`lifeBurstCost > 6000` が要る**が**実測のバースト価値は平均 2,960・p90 6,000**＝届かない。
  **実測で裏取り**＝2500 で**0件** / 12000 で**45件（7.7%）**＝**機構は動く。値が閾値に届かないだけ。**
  ⇒ 閾値超えまで上げるのは根拠が無い。むしろ **`life: 7000` 側が高すぎる疑い**＝**多次元の調整は `S-6`**（`lifeBurstCost` も学習対象に入った）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-6`**（自己対戦で重みを学習＝道具は揃った＝1候補8分。🆕`lifeBurstCost` / `guardDeckCount` も対象）。次に `S-22`・`S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 4**（`S-6`・`S-22`・`S-24`・`S-14`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）②**自己対戦がマリガンを一度も踏まない**（`S-24`）。🏁`S-25`（探索が WD13 で弱い）は**単一の重みでは直らない**と実測して `S-6` へ引き継ぎ |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4349**）｜**実機 PASS 5本**（最新＝7ターン/161手/213s）｜反転確認＝`lifeBurstCost` 2500→12000 で「撃たない」0件→45件 |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔥**`S-6`（機械学習）**→ `S-14`（作戦データ）。
