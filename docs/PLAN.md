...
- **🆕 セッション（2026-07-19・続き204-205・Opus 4.8・**タスク7クローズ**＝`PREVENT_DAMAGE`＋`COST_SUBSTITUTE` 実装／**タスク1(a)**＝アタック不可付与の対象決め打ち是正28効果・census 1963→1948・golden 451→464）**
  ...
  - **採用は2段の機械ガードで28効果**（アタックノードだけを抜いた比較＋アタック部を中立化した骨格の完全一致）。**骨格が違う4効果（WX06-002/WX18-003/WDK05-T10/WDK11-007）は据置**＝個別判断へ。**⚠自分で作り込んだ罠を1つ潰した**＝upToCount を `includes('まで')` で見ると「ターン終了時**まで**」に必ず当たり全効果が「N体まで」に化ける（数詞隣接判定へ修正・golden に回帰テスト）。census 1963→**1951**・golden 456→**459**。さらに**タスク12(xxxviii) の閾値フィルタ脱落も同セッションで消化**＝「パワー20000以下の」等を対象節スコープで抽出（全文スキャンは別文節の数値を拾うので禁止）・census 1951→**1949**・golden→**460**。**さらに抽出を巻き上げて付与系4規則（【キーワード】/アタックできない/引用キーワード/シャドウ）で共有＋色フィルタも追加**＝全効果 diff で変化2件のみ（WXDi-P07-009-E1／WXDi-P09-053-E1）・census→**1948**・golden→**461**。**さらに据置4効果も個別判断で全採用（タスク12(xxxvii) クローズ）**＝WX06-002（「代わりに」を無視して基本+代替を両方実行していた過剰効果）・WX18-003/WDK05-T10（`STUB:BET_MECHANIC` 丸ごと no-op → `CHOOSE`+`betChoose` へ。engine/decompiler とも実装済みなので即動く）・WDK11-007（「キーがある場合」を落として2体目も無条件だった過剰効果）。**census に出ない実バグ4件**・golden→**464**。
