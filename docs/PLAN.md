...
| 5 | 小口持ち越し（隙間埋めに最適） | 単点（parser/engine/decompiler混在） | S×件数 | **✅残小口3枚（WXEX1-65／WXEX2-50／WX20-053）を消化（続き265・codex実装/Opus確認）**＝いずれも「幻覚 or 過剰効果」の実害＝正面2-zi＋直前ミル札との同レベル条件（旧＝**自分のシグニをデッキへ送る幻覚**＋無条件実行）、相手場出しを跨ぐ `levelLteLastProcessed`（旧＝**任意レベルの＜凶蟲＞を出せる過剰効果**）、手札/デッキ二重source選択＋デッキ枝だけshuffle（旧＝**source なしの丸裸 ADD_TO_FIELD**）。golden 744→747・census 1545維持。既消化＝catch-all「デッキに戻す」全数再点検（続き264）／**WXDi-P05-009「それ」先行詞幻覚を既存 TRASH_CARD→deck top＋ルリグ下任意消費で解消（続き266、golden 748）＝⚠旧状態は engine では完全 no-op（SOUL_OP が `field.signi` を見るためルリグ発生源では pay が unavailable）だったと実測判明＝実質は新規機能実装**／HAND_COUNT_FILTER 系逆翻訳6不自然行を解消（同）。**残＝置換系統40枚の一般化**（66枚の実ID確定→A/B/C全数分類→Aのみ、分離 pick 単独解決＋置換 else 機構・§6.3級。続き266では未着手）。個別項目（WXDi-P03-005/GRANT_LRIG_ABILITY 5枚/TRANSFER_TO_DECK 混入/WXEX2-25-E3/自己犠牲5枚/WX26-CP1-100/PR-Di038）は続き218e/218i/218k/189/232 で消化済。詳細は [PLAN_DETAIL.md](./PLAN_DETAIL.md) 2026-07-24節・BUGFIXES 各続き |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 748・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1545（＝`BASELINE_HIGH`・回帰ゲート）。
>
