...
- [ ] 🆕**`V-88`＝`PLACE_CARD_UNDER_SIGNI` の `charm_facedown` が下のスタックを触らないこと**（§2.3 の「実機から原理的に観測できない」枠・2026-08-26 続き665 `O-60` 第6バッチ）。**いま実機で観測できない理由**＝挙動差が出るのは `lastProcessedCards` が非空のときだけだが、live の唯一の該当効果 `WX16-003-E2` は**この STUB が `SEQUENCE` の先頭**にあり、実行時点で常に空。⇒ **golden で両方向を固定済み**（`O-60⑥ engine:`＝charm/payload欠落はスタック不変・`processed` は積む）。**観測できるようになる条件**＝①【チャーム】機構（§5.3 `O-81`）が実装される ②`charm_facedown` の前に別ステップを持つ効果が live に現れる。**どちらかが起きたら実機シナリオを1本書く。**
...
| **O-80** | 🆕**`POWER_MOD_PER_COUNT` が「何を数えるか」を持たず、全部 `lastProcessedCards` の枚数で代用している** | **L（live 58効果・複数バッチ）** | **2026-08-26 続き665（`O-60` 第5〜7バッチの選定時）に分離**＝`O-60` の計器で miss8/live58 の最上位だが、**他のバッチと size class が違う**ので別項目にした。■**engine の構造**＝`execStubPart1.ts` の1ハンドラに**5パターン＋3つの状態カウント fallback＋ドロー分岐＋適用先の決定**が全部カード全文 regex で入っている（消費地点は `effectEngine` にもう1つ）。■🔴**本質的な壊れ方は「miss」ではなく「当たったときに間違った数を数える」**＝パターン1（`N体につき±X`）が**無条件に `lastProcessedCards.length` を使う**ので、原文が数えているのが**トラッシュ／場／チェックゾーン／シグニの下／ルリグのレベル合計／手札差**でも**直前に処理した枚数**で計算する。実測で少なくとも **8効果**が該当（`WX24-P2-009-E1`「あなたのトラッシュにあるカード１枚につき」／`WXDi-P06-041-E1`「あなたの場にいる黒のルリグ１体につき」／`WXDi-P12-086-E1`／`WXDi-P11-006-E2`「チェックゾーンにあるカード１枚につき」／`WXDi-CP02-054-E1`・`WXK09-060-E1`「このシグニの下にあるカード１枚につき」／`WXDi-P03-037-E1`「手札の差１枚につき」／`WXDi-P16-090-E1`「ルリグのレベルの合計１につき」）。■**母集団の内訳（実測）**＝パターン1が32件／どれにも当たらない11件／1+2の複合6件／パターン5が5件／既存 payload 1件／その他3件。⚠**「どれにも当たらない11件」には別物が混ざっている**（`WX22-042-E1`「色１つを宣言する」等＝parser の catch-all 流入）。■**進め方の見立て**＝①payload に `countOf`（数える対象＝`processed` / `trash` / `field` / `check_zone` / `under_signi` / `hand` / `energy` / `lrig_level_sum` / `hand_diff` …）＋`filter`＋`divisor`＋`deltaPerUnit`＋`applyTo` を持たせる ②engine 側は**ゾーンを filter で数える共通関数1本**に寄せる（各カウント源は5行程度）③parser は**文型ごとに規則を足す**＝ここが本体。⚠**1バッチ15〜20効果に切る**（58効果を1バッチにしない）。 |
| **O-81** | 🆕**「シグニに手札からカードを裏向きで付ける」＝【チャーム】付与の口が無い** | S〜M | **2026-08-26 続き665（`O-60` 第6バッチ）で分離**＝`WX16-003-E2`「あなたのシグニ１体を対象とし、それに**あなたの手札からカード１枚を裏向きで付ける**。そのシグニが場を離れる場合、追加でこれによって付けたカードを公開し手札に戻す。」✅**誤動作は止めてある**＝`placeUnder{mode:'charm_facedown'}` と刻んで engine は `[未実装]` を addLog（旧実装は**直前に処理したカードをシグニの下へ積んでいた**）。■**受け皿の見立て**＝`PlayerState.field.signi_charms` が既にあり、実機シナリオでも `field.signi_charms` へ直接注入して使っている＝**新機構ではなく「手札から選んで付ける」対話の配線**の可能性が高い。⚠**ただし原文は「場を離れる場合、**追加で**公開し手札に戻す」＝離脱時の置換が要る**ので、そこは既存の離脱 funnel を読んでから決める。■⚠**着手前に母集団を実測する**（「裏向きで付ける」を含む効果の全数）。 |
...
| **O-73** | 🆕**「このターン、〜したとき」の遅延設置が `ON_CARD_MILLED_FROM_DECK` に無い** | S | **2026-08-26 続き661（`O-62` の消化）で分離＝1効果**。`WX24-P3-030-E2`「【起】《ゲーム１回》ジェラシー《黒×0》：**このターン**、あなたの効果１つによってデッキからカードが合計１枚以上トラッシュに置かれたとき、対戦相手のシグニ１体のパワーを－5000する。」＝live は**遅延設置ごと落ちて素の `POWER_MODIFY -5000`**（＝【起】を撃った瞬間に相手を殴る過剰実行）。🔑**受け皿は `O-66`④ の `DELAYED_INSTALL_PREFIXES`**だが、⚠**`ON_CARD_MILLED_FROM_DECK` には `delayed_triggers` の収集経路が無い**（現状ある timing＝`ON_TURN_END`／`ON_SIGNI_BANISH_BATTLE`／`ON_LEAVE_FIELD`／`ON_REFRESH`／`ON_ATTACK_SIGNI`）＝**先に collector 側へ収集を足さないと「設置されるが永久に発火しない」**（過剰実行を no-op へ替えるだけ）。 |
