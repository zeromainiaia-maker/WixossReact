...
- 🔴🔑**そして分かったこと＝`S-16`（メインの探索）は本物のデッキでは弱い**（`search` vs `default`・`WD13`・16戦＝**組 0-4-4／勝率 0.0% [0.0, 49.0]**）。
  ⚠**合成デッキでは「互角」だった**＝**`S-20`／`S-23` の教訓がそのまま出た（山を変えると結論が変わる）**⇒ **`S-25` を登録**（既定は上げない）。
  **アタック探索だけの差は8組すべて1勝1敗**＝弱くはなっていない。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-25`**（**探索が本物のデッキで弱い**＝土台を直す）→ `S-17` 第3段（ライフバースト・ガードを確率で見る）。次に `S-22`・`S-24`・`S-14`・`S-6`。🏁**§5.3 機構 worklist は残0** |
...
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**探索が本物のデッキで弱い**（`S-25`・既定はオフなので実機には出ない）②**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）③**自己対戦がマリガンを一度も踏まない**（`S-24`）。🏁`S-19`（先攻不利）は**再計測で否定**＝山の性質だった |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4347**＝この一連で +2）｜**実機 PASS 4本**（`verifyFullMatch.mjs cpu`）｜反転確認＝`planFor('guest')` を旧実装に戻すと `§5.7 S-23` が FAIL／列挙から `canSigniAttack` を外すと `§5.7 S-17` が FAIL／アタック探索のフォールバックを外すと同 FAIL |
...
| 🔥`S-25` | 🔴**探索（`S-16`）は本物のデッキでは弱い**＝`search` vs `default`（`WD13`・16戦）で**組 0-4-4＝勝率 0.0% [0.0, 49.0]**＝「A のほうが弱い」と言ってよい。⚠**合成デッキ（`VERIFY_DECK_MECH`）では互角 50.0% [39.3, 60.7]** だった（`S-16` クローズ時）＝🔑**`S-20`／`S-23` の教訓がそのまま出た＝山を変えると結論が変わる**。**既定は上げない**（`searchWidth: 0` のまま）。**調べ方**＝①`--census-moves --search-policy` で「探索といまの選択が違う手」を本物のデッキで並べる ②`S-21` の3本（`turnDamage`／`actionBias`／目的関数）を本物のデッキで測り直す（**あの結論も合成デッキで出したもの**）③`search-power`（`fieldPowerScale: 1`）など既存の切り分け用プリセットを本物のデッキで回す。 | 中 | 探索の既定を上げられるようになる＝`S-17` 第3段・`S-6`（学習）の土台。🔴**これを放置すると「探索を足したのに勝率が下がる」が積み上がる** |
...
- **2026-09-20 時点**（第421〜423バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ `S-17` 第1〜2段 ＋ `S-25` 登録）
  ...
  - 🆕📊**探索の強さ（本物のデッキ `WD13`・16戦ずつ）**＝`search` vs `default` **組 0-4-4＝0.0% [0.0, 49.0]（A が弱い）**｜`search-attack` vs `search` **8組すべて1勝1敗（差なし）**
