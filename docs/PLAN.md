...
- **🆕 セッション（2026-07-28・Opus指示＋codex-work実装／Opus検証・7バッチ連投）＝§3 タスク12(xxix) を4系統で消化＋タスク16 の ON_PLAY 誤分類を2波で除去**（golden 868→**882**・census 1511→**1498**・smoke 10725→**10726**）
  - **在庫メモもコード中のコメントも計器も stale／盲点だらけだった**＝①union/unless の機構は engine に実在②BET機構も正常で真因は別系統の手書きパーサ③`BattleScreen.tsx:2922`「自身の ON_PLAY は各配置経路が個別収集済み」が**効果配置では成立していなかった**④`census:timing` は ON_PLAY 誤分類36件のうち33件しか捉えない。**教訓＝投げる前に全収集サイト／全書き手を列挙して実測する**（CODEX_GUIDE §3-3）。
  - **バッチ1（union/unless・16効果）**＝ライフバースト12効果の三重バグ＋union 脱落4効果。⚠検証で `negated_attacks` の保存先規約が2系統に割れているのを発見し、codex が読み手だけ統一して2効果が no-op 化していたのを是正・golden で規約固定。副産物で `REMOVE_ABILITIES` の「N体まで」未実装も是正。
  - **バッチ2（BET・9効果26選択肢）**＝9件忠実化・3件 defer。最悪例 `WX19-006-E1③` は**「相手トラッシュの全スペル除外」本体が消えて「2枚引く」だけ**だった。真因は `parseSingleChoiceText`（364行の手書き regex 群）が本体パーサと別系統であること。一括委譲は試算で**改善3／退化13**＝現時点で不可。
  - **バッチ3（自身【出】配線・段階1）**＝新 collector を全配置経路へ明示 opt-in で配線。`byEffect`/`bySigniEffect` 限定の10効果＝**全経路で永久不発**だったものが発火。
  - **バッチ4・6（段階2）＝codex が2回連続 honest defer**。**どちらの反例も Opus 側の計器の穴を突いた**＝1回目 `WX13-040-E1`→バッチ5へ、2回目 `WX25-P2-026-E1`→バッチ7へ。**defer が2回とも、より価値の高いバッチを生んだ。**
  - **バッチ5（ON_PLAY 誤分類36件）**＝原文が【自】なのに `timing:['ON_PLAY']` の36件は**全件 mandatory＝通常召喚のたびに誤発火していた**（段階2 が原因ではなく既存バグ）。[A]6件を正しい timing へ・[B]29件を `timing:[]` で停止・[C]1件は誤検出。検証中に `PR-461` の effectId 対応ズレ（**【常】ダブルクラッシュ欠落＋【自】が timing 無し**・カード単位 PRESERVE で build も heldReview も届かない）も発見し外科パッチで是正。
  - **バッチ7（watcher 34効果）**＝「あなたの〜シグニ**が**場に出たとき」の watcher が `triggerScope:'self'` になっており**二方向に壊れていた**＝①`collectFieldTriggers` が scope で弾くので**watcher として一度も発火しない**②`isMandatoryOwnOnPlayForNormalSummon` を通るので**自身の召喚時に1回誤発火する**。[A]30件を `any_ally`/`any_opp`＋既存 filter/condition へ配線・[B]4件を `timing:[]` で停止。census 1507→**1498**。
  - **⚠簿記の落とし穴**＝コミット済みの `docs/_held_review.txt` は**HEAD で build を回し直した実測値と食い違う**ことがある（今回265 vs 実測262）。**held を比較するときはファイルの値ではなく再実行した実測値を使う。**
  - **PLAN の自己矛盾を1件修正**＝PLAN:44「`build:effects` は破壊的＝絶対に実行しない」が §5c パイプラインと矛盾。非破壊なので**「実行後に effectId 単位で全数比較する」**へ書き換えた。
  - **次の一手**＝**Opus：(xxix) 段階2 の3度目（1471効果・⚠fuzz/smoke がこの経路を通らず golden が唯一の網。投入前に「原文が【出】でない ON_PLAY」の残存を再スキャンすること）／タスク16 の [B]29件＋今回の [B]4件／(xliv)・(xlii) の残／§6.3 残機構（H1〜H3・I）／タスク14 の残43／`cost.underSelfTrash` 未配線16効果**。**Sonnet：タスク1（§7 実機検証）＝今セッションの7バッチはすべて golden までで実機確認が未実施**。
