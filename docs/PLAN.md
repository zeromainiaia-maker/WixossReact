...
| 5 | 小口持ち越し（隙間埋めに最適） | 単点（parser/engine/decompiler混在） | S×件数 | **✅catch-all「デッキに戻す」の全数再点検＝クローズ（続き264）**＝parser を「対象シグニが明示された2形」に限定（全数 before/after diff で変化3効果・退化0）＋curated 全数照合で実バグ `SP07-009-E1`（自分の場のシグニがデッキへ戻る幻覚）を外科パッチ。golden 741→744・census 1545 維持。残＝置換系統40枚の一般化（分離 pick 単独解決＋置換 else 機構・§6.3級）／WXEX1-65 正面 owner＋レベル比較／**WXDi-P05-009「それ」先行詞解決＝現状もトラッシュ対象を失い場の自シグニをデッキ上へ送る幻覚。任意コスト（ルリグ下消費）を跨いで先に対象化したトラッシュカードを保持する実行経路が要る＝§6.3級**／WX20-053「手札かデッキ」二重ソース SEARCH／WXEX2-50 動的レベル制約＝いずれも単発機構待ち（§6.3送り）。個別項目（WXDi-P03-005/GRANT_LRIG_ABILITY 5枚/TRANSFER_TO_DECK 混入/WXEX2-25-E3/自己犠牲5枚/WX26-CP1-100/PR-Di038）は続き218e/218i/218k/189/232 で消化済。詳細は [PLAN_DETAIL.md](./PLAN_DETAIL.md) 2026-07-24節・BUGFIXES 各続き |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 744・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1545（＝`BASELINE_HIGH`・回帰ゲート）。
>
