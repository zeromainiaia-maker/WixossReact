...
**直近＝2026-09-22＝🏁§5.6 `C-10` 第2段＝CPU がスペルカットイン窓で応答する**（全文は [BUGFIXES.md](./BUGFIXES.md) 第453バッチ）
- 🔴**旧＝「人間のスペルに対してCPUは常にパス」**＝**窓ごと使えていなかった**（61カード／ユーザー作27デッキ中7デッキが該当）。
- 入れたもの＝**実行の1本化**（`controller/performCutinUse.ts`＝242行を逐語で移設し応答者をパラメータ化）＋
  🆕`cpuCutin.ts`（**打ち消せる候補を優先・エナの軽い順・払えないなら撃たない**）＋`cpuTurn` の配線（使わなければ従来どおりパス）。
  ⚠**支払い内訳を人間が選ぶ形（ベット・エンコール）は撃たない**（allowlist）。🆕計器＝`census:play` の `cutin`。
- ⚠🔴**最初 `cpuTurn.ts` に支払い判定を直接書いて golden `§5.3 O-342` に捕まった**＝**2つ目の支払い判定を書くな**の規律が効いた
  ⇒ 材料は `cpuCutinInput`（`cpuMoves.ts`）の1本＝権威は `basicAffordable`。
- 📏**実カード25枚で 18枚は応答を選び、7枚は選ばない**（不採用はすべて規律どおり＝ルリグ限定2／エンコール2／ベット3）。
  🔴📏**自己対戦で実際に踏んだ**＝`タマ改`×`レベル2止め` 3戦で**スペル3回／カットイン応答1回**。
  ⚠**最初の山では踏めなかった**＝**CPU がスペルを使わず窓が開かなかった**＝🔑**「機構が動かない」と「窓が開いていない」を混同しない**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-6`**（機械学習）／🔵**§5.6 `C-11`**（カットインの判断が危険度を見ていない＝A/B で測れる） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／**CPU 完成度 3**（🔵`C-11` 判断の質・⚪`C-12` ピース窓・⚪`C-13` レゾナ）／**CPU の強さ 1**（🔵`S-6`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い** |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4372**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
🏁**`C-10` CPU がスペルカットイン窓で常にパスしていた**（2026-09-22 第452〜453バッチでクローズ）＝
📏母集団＝カットインできる札 **61カード**／**ユーザー作27デッキ中 7デッキ**。
🏁**第1段＝候補列挙の1本化**（`cutinCandidates.ts`）／🏁**第2段＝実行の1本化**（`controller/performCutinUse.ts`＝242行を
逐語で移設し応答者をパラメータ化）＋`cpuCutin.ts`（**打ち消せる候補を優先・エナの軽い順・払えないなら撃たない**）。
📏**自己対戦で実測＝`タマ改`×`レベル2止め` 3戦でスペル3回／カットイン応答1回**（`census:play` の `cutin`）。
⤵**残りは `C-11`〜`C-13` へ採番して切り出した**（レゾナ／ピース応答窓／判断の質）。
golden `§5.6 C-10`／`§5.6 C-10 第2段`

🔵**`C-11` CPU の判断が「相手のスペルの危険度」を見ていない**（2026-09-22・`C-10` から切り出し）＝
いまは**「打ち消せるなら打ち消す・エナの軽い順」だけ**＝**ドローだけのスペルに最後のアーツを吐く**ことがありうる。
🔑**取り方**＝`S-4` の先読み（`cpuLookahead`）で**「打ち消さなかった場合の盤面」と「打ち消してアーツを失った盤面」を採点して比べる**
（`tryCpuUseArts` が既に同じ形を使っている）。📏**測り方**＝`census:play` の `cutin` と **A/B**
（`タマ改`×`レベル2止め` なら窓が開く＝第453バッチで実測）。⚠**母集団はカットイン札 61カード**（`C-10` と同じ）。**狙い＝打ち消す価値のないスペルに札を使わない。**

⚪**`C-12` ピース応答窓で CPU が応答しない**（2026-09-22・`C-10` から切り出し）＝
`collectCutinCandidates` は**ピース応答窓の候補も返す**（`pending_spell.kind === 'piece'`）が、**CPU は未検証**。
📏**母集団＝窓を開く側（【使用条件】【チーム】のピース）11カード／応答しうる札 80カード**。
🔴**ユーザー作27デッキには0枚**＝**A/B でも自己対戦でも測れない**（`S-20` と同型）＝**取るなら golden が直接指標**。
⚠**打ち消しの意味が違う**（選択肢①を選んだときだけ＝既定は打ち消さない）＝`C-10` の判断をそのまま流用しない。**狙い＝ピース窓でも CPU が応答できる。**

⚪**`C-13` レゾナのカットインを CPU が使わない**（2026-09-22・`C-10` から切り出し）＝
`kind:'resona'` の候補は**出す先（シグニゾーン）の選択が要る**ので `pickCpuCutin` が除外している（honest defer）。
📏**母集団＝レゾナ × `SPELL_CUTIN` を含む効果 3カード**（`WX13-005B`／`WX13-006B`／`WX14-006B`）。
🔴**ユーザー作27デッキには0枚**。🔑**受け皿は在る**＝出す先は `pickCpuResonaZone`（`cpuSummon.ts`）が既に決められる
＝**繋ぐだけで済む可能性が高い**（着手前に受け皿を疑う＝§5.3 の作法）。**狙い＝レゾナのカットインも CPU が使える。**
