...
- 🏁**セッション（2026-08-29・続き715・Opus 5 単独）＝§5.3 `O-135` クローズ。スペル解決経路を中央 diff へ一本化し、約370効果の watcher にやっと届いた。**
  ユーザー指示＝**「O-135 を行う」**。
  📊**進捗3計器＝Sheet1 要対応 0 / 863（据置）｜台帳 残 OPEN 558（据置）｜census 高シグナル 520（据置）**
  ⚠**3計器が全部据置なのは正常**＝これは JSON（効果データ）ではなく **`BattleScreen` の配線**の修正で、
  **3計器はどれも live JSON しか見ていない**（＝この層のバグを1件も検出できない。だから実機が要る）。
  gates 全緑（golden 2972 据置・smoke 10702 全0・fuzz 全0・**lint 260→248**・`census:enginetext` 141 据置）。
  全文は BUGFIXES.md 冒頭。
  - 🔴**真因**＝スペル解決（`handleCutinPass`）は `collectBoardDiffTriggers` を1度も呼ばず、
    `ON_PLAY`/`ON_BLOOM`/`ON_DECK_SHUFFLED`/`ON_REFRESH` の**4族だけ手書き**していた。
    ⇒ `ON_BANISH` 166効果・`ON_TRASH` 105・`ON_LEAVE_FIELD` 71・ミル17・ドロー13…**約370効果**が、
    **スペルがその変化を起こしても1件も誘発しなかった**（起こす側＝スペル391カード中 BANISH 130／DRAW 72）。
  - 🔑**登録票の「二重 collection になりうる」は実装を読むと既に解けていた**＝中央 diff の `ON_PLAY` ブロックは
    スペル側の手書きと**同一コード**で、`fieldPlacementOnPlayOpts(spellEff)` を spread すれば置き換わる。
    ⇒ **「族ごとにインライン収集を足す（登録票の②）」を飛ばして③の一本化へ直行できた**（−128/+27行）。
    ⚠**登録票の見立ては3巡連続で実測とズレている**（`O-77`／`O-76`／今回）＝**着手前の実測を省かない。**
  - ⚠**`result.done` で分岐しないようにした**＝`!done`（対話待ち）でこの段が確定させた変化も収集する。
    resume 経路が同じ理由で `midBd` を持っているのに**スペル側だけ落としていた**。
  - ✅**実機＝新設2シナリオが両方 PASS、かつ反転確認（修正前コード）で両方 FAIL**
    （`o135SpellBanishTrigger`＝`WX03-033`→`WXDi-P00-054` の `ON_BANISH`／
    `o135SpellDrawTrigger`＝`WD01-018`→`WXK02-090` の `ON_DRAW`）。
    既存スペル経路14シナリオも全 PASS（`deckshufflespell`/`v91refreshonce`/`ontargeted`×6/`b24`×2 ほか）。
  - 📌**旧バグは「対話なしでインライン完了するスペル」でしか再現しない**＝
    対象選択を伴うスペルは resume 経路が中央 diff を呼ぶので前から動いていた。**観測点の設計でここを外すと緑になる。**
  - 🗑`collectRefreshInline`（前セッション `V-91` で新設）は呼び出し元が消えて dead → 削除。
