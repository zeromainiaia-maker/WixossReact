...
- **🆕 セッション（2026-07-31・続き307・Opus 5〔**Claude が実装まで担当**〕）＝タスク16 `[B維持]` 残3件を全消化＝🏁**タスク16 残0クローズ**（golden **1195→1203**・census **1358→1357**（`BASELINE_HIGH=1357`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings**（+6は既知の hooks 偽陽性・実測）・manual field loss 0・同型★0・held **251→250枚/98署名**。live per-effect 差分 **changed 4 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **17→14**）
  - **①`WXDi-P06-038-E1`（エナ→任意宛先）＝新 timing ゼロで着地**。既存 `ON_ENERGY_TO_TRASH` に `triggerCondition.energyLeftToAnyZone` で相乗りし、collector の枚数選択を1つのヘルパーに集約しただけ（省略時は従来どおり trash 限定）。新設は計器1本（`countEnergyLeftZone`＝**多重集合の減少数**で数える。Set 差分だと「同名2枚のうち1枚だけ移動」を0と誤読する）。⚠**「効果によって」の限定は構造的に満たされていた**＝コスト支払いは中央 diff を通らない（`executeSigniActivated` は手札捨てとコインしか collector を呼ばない）＝原因軸の追加は不要だった。
  - **②`WX05-020-E1`（主体別ターン累計クラッシュ）＝在庫で唯一「累計」が要る件**。`PlayerState.life_crashed_by_signi_this_turn` を新設し**アタックと効果の2経路で加算**。⚠**アタック側は実差分（`opS.life_cloth.length - finalOpState.life_cloth.length`）で数えるのが肝**＝クラッシュ地点を個別に数え上げるとランサー/ダブルクラッシュ/ダメージ無効を必ず取りこぼす。同カードの E2（相手アーツ被効果でダメージ）が効果経路なので、アタック1＋効果1でも合計2で発火する。
  - **③`WXDi-P13-051-E3`（相手効果1つによる手札捨て**か**エナ→トラッシュの OR）＝台帳が「最も重い」とした件**。🔴**難所は重複排除で、しかも実在の問題だった**＝手札捨ては React watcher、エナ→トラッシュは中央 diff と**収集経路が分かれている**ため、2 timing に割ると「1効果で両方やる相手カード」（`WXK02-004`／`WXDi-P10-003`／`WXDi-P13-003A`）で二重発火する。⇒ 新 timing `ON_HAND_OR_ENERGY_LOST_BY_OPP` を作り**中央 diff だけで両方を数える collector 1本**に畳んだ（検出器は既存の `detectHandTrashed`／`countEnergyToTrash` を流用＝新設ゼロ）。
  - **④ ③は本体も潰れていた**＝原文「カードを1枚引く**か**【エナチャージ１】」が `ENERGY_CHARGE_FROM_DECK` 単体に化けていた。真因は `parseDrawOrChoice` が「とき」を含む文を弾くことで、**トリガー句を除去してからでないと本体に届かない**。除去を足したら2択が復元。
  - **⑤ 🆕 同型の未消化を10効果みつけた（今回の対象外・PLAN §3 タスク12 へ登録）**＝「カードを1枚引くか【エナチャージ１】をする」23件のうち14件は CHOOSE なのに、**トリガー句/条件句が前置される形と引用付与の内側では引く選択肢が黙って落ちている**（`WXEX1-41-E1` ほか10件）。同じ「とき」ガードが原因。
  - **次の一手**＝**Opus：タスク16 は残0クローズ**。次は ①🆕**タスク12(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み＝`parseDrawOrChoice` の「とき」ガード。トリガー句除去との組み合わせが要る。**1規則でまとまって開く可能性が高い在庫**）②タスク12(lxviii) 散文形「対戦相手のターンの間、」③§6.3 J 残11 ④(lxvii) CPU `ON_TURN_END` 未収集 ⑤§6.4 配置数制限すり抜け7効果 ⑥(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜307 の全件が実機UI未検証で積み上がっている**。
### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き307（タスク16 `[B維持]` 残3件を全消化＝**タスク16 残0クローズ**・4効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1358→1357**（`BASELINE_HIGH=1357`）、golden **1195→1203**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234→240 warnings**（⚠**退化ではない**＝`useHost`/`useGuest` の `react-hooks/rules-of-hooks` 偽陽性が新経路2本で 102→108。stash 比較で実測）、manual field loss 0、同型★0（5986枚・265群）、held **251→250枚／署名グループ 98 据置**。live per-effect 差分 **changed 4 / added 0 / removed 0**（`WX05-020-E1`／`WXDi-P06-038-E1`／`WXDi-P06-038-E2`／`WXDi-P13-051-E3`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 17→14 効果/14クラスタ**（⚠**実質12**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 3→0（クローズ）**。**新語彙＝timing 2（`ON_SIGNI_CRASHED_LIFE_TOTAL`／`ON_HAND_OR_ENERGY_LOST_BY_OPP`）・`triggerCondition` 2（`energyLeftToAnyZone`／`crashedTotalThisTurn`）・`PlayerState` 1（`life_crashed_by_signi_this_turn`）＝いずれも省略時は従来挙動**。engine 新設は計器1本（`countEnergyLeftZone`）＋collector 2本（`collectSigniCrashTotalTriggers`／`collectOppResourceLossTriggers`）。
