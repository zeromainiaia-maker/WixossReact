...
- **🆕 セッション（2026-07-09・続き58・Sonnet 5・§4タスク(F) §7実機検証の継続＝ON_OPP_POWER_DECREASED＋ON_ENERGY_TO_TRASHでresume経路バグ発見・placedFrontはPASSで対照確認）**＝3シナリオ追加。PLAN §7 R46（毒牙・WX13-036/WXEX2-52）とR43（WD15-015）を `verifyBattleDrive.mjs` 新シナリオ `oppPowerDecreased`（WD11-013→WX13-036）／`energyToTrash`（WD15-014→WD15-015）で検証した結果、**両方とも❌FAIL＝ON_SIGNI_FROZEN（R38・続き40/41）と同型の実バグを2件連続で発見**（未修正・観測記録のみでOpusへ引き継ぎ＝PLAN §3運用ルール「発見したバグの修正自体はOpusに回す」に従う）。**共通原因**＝`collectPowerDecreaseTriggers`（`triggerCollect.ts:900`）／`collectEnergyToTrashTriggers`（同`:808`）はどちらも`BattleScreen.tsx`中央diff（`resolveStackNext`内・3717-3789行）にしか配線されておらず、`handleEffectInteraction`のpendingEntriesブロック（4384-4436・`collectFreezeInline`等5種のinline collectorが既にある）には入っていない＝SELECT_TARGETで完結する単体対象効果（大半のケース）でwatcherが無発火。**2件連続で同根と判明したため、個別カード修正ではなく機構単位の系統的懸念に格上げ**＝中央diffブロックには他にも同型でinline版が無いcollectorが並ぶ（`collectMillTriggers`/`collectCharmToTrashTriggers`＝**R42と同一対象**/`collectRefreshTriggers`/`collectMoveToDeckTriggers`/`collectDrawTriggers`系/`collectAllyPlayOrOppDiscardTriggers`/`collectMaterialUsedOnSigniTriggers`/`collectOppArtsUseTriggers`系・実際に影響するかは個別要確認）。`order`配列には両方とも追加せず除外（既存スイート全緑を維持）。**続けてR41（placedFront）を`placedFront`（WD01-013→WXDi-P03-043）で検証したところ✅PASS**＝`handleSummonSigni`が`collectFieldTriggers`を直接呼ぶ第三の経路はresume/中央diffのどちらにも依存せず無事＝「resume経由の効果解決だけが取りこぼす」という仮説を裏付ける対照データ（`order`復帰済み）。副産物としてON_PLAY/ON_BANISH/ON_ATTACK_SIGNI/ON_BLOOM共有ループの盤面ログ文言が「相手シグニアタック時」固定になる表示バグも発見（機能に影響なし・低優先）。詳細はBUGFIXES続き58（3エントリ）・PLAN §6.3・§7。**検証**＝3シナリオとも単体実行で再現（2 FAIL・1 PASS）・typecheck緑（engine未変更のためgates再実行不要）・`git status` clean（auto-commit済み）。**次の一手＝(A)Opus＝続き56で発見した EQUALIZE_ENERGY owner欠落／EXILE owner反転／duration反転／triggerScope欠落の4系統×8枚の原因調査**（現行parser規則のどこかで owner/duration/triggerScope が後段の汎用ルールに上書きされている疑い）。**(B)Opus＝effect-restriction（配置数制限）**（WXK11-074等・§6 BLOCK機構・実機検証必須）。**(C)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(D)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(E)Opus＝§5c census条件節の残**（ARTS_USED拡張・「代わりに」WX25-P2-068/070・「あり」複合条件WXDi-P11-048）。**(F')Opus＝🆕 resume経路inline collector欠落の機構単位修正**（本セッション発見の2件＝`collectPowerDecreaseInline`／`collectEnergyToTrashInline`を追加＋系統的懸念リストの横断監査→影響ありなら一括是正→`oppPowerDecreased`/`energyToTrash`シナリオ単体再検証→`order`復帰）。**(F)Sonnet＝§7実機検証の継続**（R30/R31/R36/R39-R41/R44/R45残・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up項目・§7既存リスト。⚠R42は(F')の系統的懸念対象＝Opus修正待ちが望ましい）。**(G)持ち越し：SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
...
- **🆕 resume経路inline collector欠落＝2trigger種別で実バグ確認＋系統的懸念（2026-07-09・続き58・§7 R43/R46実機検証で発見・未修正）**＝ON_SIGNI_FROZEN（R38）と同型のバグを**2件連続で確認**＝(a)**ON_OPP_POWER_DECREASED**（WX13-036/WXEX2-52対象・`collectPowerDecreaseTriggers`＝`triggerCollect.ts:900`）：WD11-013（相手シグニ1体-1000・mandatory）をSELECT_TARGET resumeで解決するとwatcher無発火（`guest.temp_power_mods`は正しく-1000になるが`host.temp_power_mods`は終始`[]`）。再現＝`node scripts/verifyBattleDrive.mjs oppPowerDecreased`。(b)**ON_ENERGY_TO_TRASH**（WD15-015/同様カード対象・`collectEnergyToTrashTriggers`＝`triggerCollect.ts:808`）：WD15-014（相手エナ1枚トラッシュ・mandatory）をSELECT_TARGET resumeで解決するとwatcher無発火。再現＝`node scripts/verifyBattleDrive.mjs energyToTrash`。**共通原因**＝どちらも`BattleScreen.tsx`の中央diff（`resolveStackNext`内）にしか配線されておらず、`handleEffectInteraction`のpendingEntriesブロック（4384-4436・`collectFreezeInline`等5種のinline collectorが既にある）には入っていない。**両シナリオとも既定orderから除外済み**。**🆕系統的懸念（未実機検証・静的解析のみ）**＝中央diffブロックには他にも同型で inline版が無い collector が並ぶ＝`collectMillTriggers`(ON_CARD_MILLED_FROM_DECK)・`collectCharmToTrashTriggers`(ON_CHARM_TO_TRASH＝**R42と同一対象**)・`collectRefreshTriggers`(ON_REFRESH)・`collectMoveToDeckTriggers`(ON_CARD_MOVED_TO_DECK)・`collectDrawTriggers`/`collectOppDrawTriggers`(ON_DRAW/ON_OPP_DRAW)・`collectAllyPlayOrOppDiscardTriggers`・`collectMaterialUsedOnSigniTriggers`・`collectOppArtsUseTriggers`/`collectArtsUseTriggers`＝実際にresume経路を通るか（SELECT_TARGET等の対象選択を伴うか）は個別確認が要る。**修正方針**＝`collectFreezeInline`と同型の`collectPowerDecreaseInline`／`collectEnergyToTrashInline`を追加する横展開（新規機構ではなく適用漏れ・低リスク）。その後、系統的懸念リストを横断監査し影響があるものを一括是正するのが本筋（個別カードではなく機構単位）。詳細はBUGFIXES続き58（2エントリ）。
