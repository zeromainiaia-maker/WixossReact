...
**直近＝2026-09-17＝§5.7 `S-4` クローズ＝浅い先読み（A）＋CPU が任意コスト付きのアーツ・スペルを使っていなかった退化の修正**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**ユーザー決定**＝A（浅い先読み）で効果を確かめてから C（対戦丸ごと）へ段階的に進む＝`S-4` → `S-5`（4段）→ `S-6`（機械学習①）。
- `S-4`＝`cpuLookahead.ts`＝盤面をコピーして engine で効果を解決（途中の選択は `cpuInteraction.ts` で答える・本番の盤面と乱数は触らない）→ 盤面を採点。召喚は【出】の結果で、スペルは結果が良くなるなら除去以外も使う（相手の場が空なら撃たない）。
- 🔴**採点で踏んだ罠**＝弱いシグニの除去が「弱いぶん」しか得にならなかった ⇒ **正面が空いている数**（アタックが通る）を足した／凍結スペルが無価値に見えた ⇒ **相手の凍結シグニ**を足した。
- 🔴**別件の退化を修正**＝`O-86`（2026-09-02）でブースト・ベット・アンコール等がコストの payload へ移った後、CPU の許可リストが `energy` だけで **約230効果のアーツ・スペルを使っていなかった**（実機 `v82ResponseArtsPreventAtLowLife` が HEAD でも FAIL）⇒ 任意の宣言コスト／実効コストに織り込み済みの項を分類し、**未分類のキーが増えたら golden が落ちる**ラチェットを足した。
- 🔑**ユーザー指示（同日）**＝CPU はアーツ・スペル・【起】・キー／ピース・アシストグロウ・召喚コスト・任意コストでエナを払って**次のグロウができなくなる支払いを必ずしない**（`cpuGrowReserve.ts`・次のターンのエナチャージは当てにしない）。実機 🆕`V-272`。
- 🔴**ユーザー指摘を修正（同日）**＝CPU がガード札を持っているのに使わない ⇒ 方針を「持っていれば毎回使う」に変更＋効果の捨て札でガード札を捨てていた不具合。実機 🆕`V-271`。
- 🔴**バグ報告1件を修正（同日）**＝CPU が出したシグニの【出】を解決する前に次のシグニを出していた（召喚ループが【出】を最後までためていた）⇒ 1体ごとに解決。実機 🆕`V-270`（反転で FAIL）。
- 実機＝🆕`V-269` PASS×2（相手2体ならＦＲＥＥＺＥを使う／空なら使わない）／アーツ・スペル・召喚系20本 PASS／CPU 通し対戦 PASS×3（⚠この3戦で CPU はスペルを使わなかった＝手札に来ない・盤面の条件。効果の体感はユーザーの対戦で確かめる）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7 `S-5a`**（C 対戦丸ごと・1段目＝メモリ上の persist＋`resolveStackNext` の切り出し）。先に**ユーザーが対戦で先読みの効果を体感**して、C へ進むか確かめる |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（`S-5`・`S-6`）／**リリース作業 1**（RELEASE.md） |
