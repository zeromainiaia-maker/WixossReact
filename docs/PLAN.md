...
- **🆕 セッション（2026-08-14・続き480・Opus 5）＝**§7 V-15／V-16／V-17 を消化**＝**機構は3ブロックとも実機で発火を確認・シナリオ13本中8本が緑**（Codex 起案→Claude 実機検証）。**engine バグ0**・ゲート据置（golden 1976・census 831・lint 0/259）。
  - **✅① 緑8本**＝V-15①（アタック終了時）3/3・V-15②（フェイズ終了時）2/3・V-17（コイン獲得）3/5。**負方向はすべて「盤面を1文字も変えず原因だけ外した対照」**（正面のシグニ有無／アタックしない／所持コインを5枚に）。
  - **🔎② 赤5本は全部「シナリオ側」＝機構は実エンジンログで成立を確認済み**＝V-15②は `left→phaseEnd→trigger` まで観測／V-16 は **watcher 発火＋手札1枚がトラッシュへ**まで観測／V-17 の CPU グロウは **`[CPU] グロウ→【自】（コイン獲得時）→1枚ドロー`**、支払い側は **`SP27-007-E1` が `actions_done` に入らない**＝負方向も成立。**赤の原因は settle 条件と盤面設計**。
  - **🔴③ 検証側（私）の事故＝ドライバを57分ハングさせた**。`clickDecideNofM` の `click()` に **timeout を渡さず既定30秒**待たせたため、**1ティック30秒 × 120ティック**で停止。⚠**ハーネスの10分 kill はラッパーしか止めず通知も来ない**＝**スクショの mtime とプロセス生存で気付くしかない**。⇒ **📌19・📌20（出力はパイプでなくファイルへ）として登録**。
  - **🔑④ 新しい罠を4件登録**（📌19〜22）＝click timeout／出力のファイル化／**「決定 (N/M)」は前方一致＋候補より先に押す**（押し直すと選択がトグルで外れる）／**spec の `deck: []` はリフレッシュを誘発し盤面追跡を壊す**。
  - **🔑⑤ Codex 委譲の所見**＝**追加 testid 0件・既存不変を SHA-256＋`485 additions / 0 deletions` で証明**。**指示書の行番号を5箇所訂正**（通算15回目・`ON_ATTACK_END` の `:9183` は実際にはバトル離場の記録地点）。⭐**成立しない主張を無理に緑にしない判断**も明示（バトル経路からの E1 発火は作り込まず離場記録までを独立シナリオ化）。
  - **▶ 次の一手**＝**残作業は赤5本の作り直し**（V-16 は盤面設計＝デッキに数枚積む＋埋め札を別カードにする／V-15②・V-17 は settle 条件）。**Part D（V-18＋V-19）の指示書は作成済み**（`codex_taskD.md`・scratchpad）。§3 の在庫は **5件**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き480（§7 V-15/16/17 消化）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1976 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（engine/parser/JSON 非改変）。🆕**実機シナリオ＝+13本**（V-15/16/17）＝**緑/赤の内訳が 93緑1赤 → 101緑6赤**（赤6本＝(cxxxiv) 待ち1本＋**シナリオ側の未完5本**）。🆕**Opusタスク12 の在庫＝5件**（据置）。
