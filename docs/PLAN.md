...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-09 続き385＝🏁`parseStatus:UNKNOWN` 完全 no-op 7効果を残0クローズ）**＝golden **1556**・smoke **10686** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **909**（回帰ゲート）・manual field loss 0・lint 0 errors/**254 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **120枚／53群**。**被覆マトリクス miss 280**（`npm run census:wiring`）。**混在カードのレビュー待ち 3カード**（`docs/_partial_fresh.json`・すべて機構ギャップ）。**live の `parseStatus:"UNKNOWN"` は 0**（続き385 で残0）。
>
...
>
> 0. 🎯**次の一手＝`HAS_CARD_IN_FIELD{cardType:'ルリグ'}` がアシストルリグを数え落とす live 19効果**（続き385 検証で発見＝`matchesFilter` の cardType は厳密一致で、CSV の Type は `'ルリグ'` 1234枚と `'アシストルリグ'` 340枚が別値。`lrigZoneTops` がアシストを走査していても黙って落ちる）。母集団＝`WXDi-P08-001`〜`005` 系のピース使用条件9件／`WX24-P4-068/075/082/089` 4件ほか。**先例＝続き385 の `WXDi-P00-002-E1`**（`cardType:['ルリグ','アシストルリグ']` の配列形＋`evalUseCondition` の両方向 golden＋逆翻訳追随）。⚠**`matchesFilter` 側の一般化は禁止**（対象フィルタまで広がる）。次点＝**2ゾーン合算の枚数条件**（`WXDi-P12-056-E1` 1件だが `combinedTrash` の先例があり型を1つ足すだけ）。🏁**`parseStatus:UNKNOWN` の完全no-op は続き385 で残0クローズ済み**（在庫の `PARTIAL 23件` は実測 live 54／高シグナル18 で中身は (iii) 構造混線＝安い在庫ではない）。
...
> このセクションは機構台帳＝**現存の残作業は下記 C／E／F／G／K に集約**（🏁**J 群は 2026-08-08 に残0クローズ**＝J-2 続き380／J-5 続き381／J-1 続き383／J-4 続き384）。消化済み機構の実装詳細（フラグ・ファイル・commit）は BUGFIXES 各日付が一次記録（各機構が日付/commit を明記済み）。完了行の原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-02 整理②」節。
