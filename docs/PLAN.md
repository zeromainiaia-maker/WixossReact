...
- **🆕 セッション（2026-08-13・続き453・Opus 5）＝§6.4 **O-3**「`REMOVE_ABILITIES.until` が engine から読まれない死フィールド」を機構化。7効果を是正し、turn-end 経路の既存リークも同時に塞いだ。golden 1937→1940・census 832 据置・held 106枚/47群 据置**。
  - **🔴① バグの正体＝「型に値がある＝動く」ではなかった**＝`RemoveAbilitiesAction.until` は `EffectDuration` を宣言しているのに、書き込み側 `applyAbilitiesRemoval` が**その値を1度も読んでいなかった**。`PERMANENT` も `UNTIL_OPP_TURN_END` も `NEXT_TURN` も全部「このターン終了時まで」に丸まり、live 144効果のうち **`UNTIL_END_OF_TURN` 125件だけが偶然正しかった**。⚠**PLAN の O-3 行が「型ごとに受け皿があるかを先に実測」と警告していた、まさにその形**。
  - **🔴② 併せて見つけた既存リーク＝turn-end 4経路のうち2本でしか消していなかった**＝`abilities_removed`／`keyword_abilities_removed` を空へ倒していたのは `confirmEndDiscard`（手札上限の捨て札を挟む）側の2本だけで、**最も普通の経路（捨て札なしでターンが終わる）では消えていなかった**＝「ターン終了時まで能力を失う」が次のターン以降も残り続けていた。同じ literal で `temp_power_mods`／`keyword_grants` は消していたのにこの2つだけ抜けていた。
  - **⭐③ 機構は予約1フィールドで足りた**＝`abilities_removed_next_turn` を `clearTurnEndScopedState` で `abilities_removed` へ**昇格**させる2スロット式（`field_grants_next_turn` と同じ考え方）。`NEXT_TURN`＝予約だけ書く（現ターンには効かせない）／`UNTIL_OPP_TURN_END`＝現ターン＋予約の両方を書く、で**2語彙が同じ機構に載る**。クリアと昇格を funnel へ集約し、2フィールドを `turnScopedState.ts` のレジストリに登録した＝**既存の T2 トリップワイヤ（funnel 外の手書きクリアは0件）がそのまま再発防止になる**（最初の取りこぼしを捕まえられなかった理由も「未登録だったから」に尽きる）。
  - **✅④ 直った7効果**＝**engine 側だけで直る5件**（JSON は元から `UNTIL_OPP_TURN_END` で正しく、消費が無かっただけ＝`WX24-P4-002-E1`／`WX25-CP1-093-E1`／`WX26-CP1-028-E2`／`WXDi-P02-055-E1`／`WXDi-P10-005-E3`）＋**parser で `NEXT_TURN` へ2件**（`WX05-019-BURST`／`WX18-029-BURST`）。⚠後者は LB＝相手ターン中に発動するので**現ターンには効かせてはいけない**。
  - **📋⑤ 据置3件＝期間だけ直すと「誤った対象の効果」が長持ちする**（非採用を golden で固定・PLAN O-16 へ登録）＝`WX11-038-E2`（フェイズ限定の受け皿が無い）／`WXEX2-04-E3`・`WX25-P3-014-E2`（**指定ゾーン軸が落ちている**。後者は2ゾーン指定で `designated_zone` が1つしか持てない）。
  - **⚠⑥ 簿記の数字は実測で覆った**＝PLAN の O-3 行は「約16効果」だったが、原文×live を数え直すと**38効果**だった（うち7件を消化＝残31）。**着手前に実測する**規律（§3-1）がそのまま効いた。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-3 の残31**（`STUB` 系が主体。⚠型ごとに受け皿を実測してから）(b)**O-16 の複数ゾーン指定機構**（`designated_zone` が1つしか持てない＝`WX25-P3-014-E2` ほか。O-3 の据置を開ける鍵）(c)**O-6 `WX25-P3-038-E1`**（MANUAL 不可侵。続き452 の `SP38-006` 外科パッチが先例）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452（相手応答モーダル）と続き453（ターン跨ぎの能力喪失）が積まれた。まず `WXEX2-84-E2` のシナリオ1本から。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き453（§6.4 O-3＝`REMOVE_ABILITIES.until` の死フィールド解消）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1940（+3＝`until` 3語彙の書き分け／ターン境界の2スロット寿命＋既存リークの回帰／live 2件の `NEXT_TURN` と据置3件の固定）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 34フィールド**（32→34＝`abilities_removed`／`keyword_abilities_removed` を登録）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）。live effect 単位 diff **2効果・effectId 増減 0/0**。
...
| **O-16** | **能力喪失の据置4件（対象軸が別途壊れている）** | 4効果 | 続き453 で期間だけ直すのを見送った札（非採用を golden で固定）。(a)`WX11-038-E2`＝「次のターンの**メインフェイズ**の間」＝**フェイズ限定の受け皿が無い**（全ターンへ丸めるとアタックフェイズまで奪う過剰実行）(b)`WXEX2-04-E3`＝「シグニゾーン１つを指定する。…**そこにある**シグニは」なのに target が `owner:'self'` かつ `zoneSource` 無し(c)`WX25-P3-014-E2`＝同上＋**2ゾーン指定**で `designated_zone` は1つしか持てない。**要るのは複数ゾーン指定の機構** (d)🆕`SP38-006-E1` の1つ目の内側能力（続き452 で隣接発見）＝「このターン、対戦相手の場にある**キーとシグニ**は能力を失い、**新たに得られない**」が `REMOVE_ABILITIES{SIGNI, count:1, until:'PERMANENT'}`＝**対象種別・体数・期間の3軸とも未忠実**。そのためカードは `parseStatus:'PARTIAL'` のまま温存してある |
