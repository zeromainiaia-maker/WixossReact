...
**直近＝2026-09-21＝§5.7 `S-30`＝「WD13 が 75% → 8.3% に落ちた」を測って否定した（CPU は退化していない）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第436バッチ）
- 🔴**前バッチで自分が登録した見立て（探索の既定の効きが山ごとに極端）を、3つの測定が否定した。**
- 📊**①探索は WD13 でもう弱くない**＝`--a legacy-greedy --b default --deck WD13`＝**組 4-2-2 [30.0, 90.3]＝差なし**
  （`S-25` 時点は 0-4-4＝明確に弱かった）。
- 📊**②むしろ全山で強いか互角**＝6デッキ96戦で**探索なしが強い山は0**（4山で探索が有意に強い）｜
  **合算ライフ差 -2.13 [-2.88, -1.37]＝0 を跨がない**。⇒ **CPU は退化していない。**
- 🔴**③総当たりの山ランキングは CPU の版をまたいで比べられない**＝**同じ CPU なら順位は安定**（seed 1 と seed 100 で同じ並び）
  なのに、**戻せるノブを全部旧値に戻しても 2026-09-20 の表は再現しない**（WD06 が 18.8 → 77.1）。
  戻せない変更（`S-24` のマリガン・`S-28` の色制限の修正）が効いている⇒ **「75.0 → 8.3」はどの変更にも帰属できない。**
  🆕**総当たりの出力に警告を足した**（版をまたいで比べない／ポリシーは mirror A/B で測る）。
- 🔑**④「探索が WD13 のルリグ【起】を使わない」は妥当だった**＝58回すべて**アタック前**で、その札は
  **アタックフェイズでは撃てない**（`Timing` が `-`）＝MAIN では「アップする」が空振り。**`S-25` の「採点の穴」は誤り。**

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-14`**（作戦データのコンボに「使い方」を持たせる）。次に `S-29`。🔵`S-6` 第3段は「大きく振る」なら。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（`S-14`・`S-29`＝🏁`S-30` を否定でクローズ）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4355**・件数不変＝挙動を変えていない）｜⚠**実機は不要**（触ったのは `scripts/` の警告1行だけ＝§2.2） |
...
- **2026-09-21 時点**（第421〜436バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30`）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（🔵`S-6`・`S-14`・`S-29`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
