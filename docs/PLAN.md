...
- **🆕 セッション（2026-08-14・続き481・Opus 5）＝**§7 V-18 決着（6/6 緑）・V-19 は要追試**（Codex 起案10本→Claude 実機検証）。engine/parser/JSON/React **非改変**・ゲート据置（golden 1976・census 831・lint 0/259）。
  - **✅① V-18 が決着＝6/6 緑・engine バグ0**＝【ソウル】付与の **self/any_ally 極性**（付与先だけ変えた対照で self が誤発火しないことを両方向で固定）・【アクセ】トラッシュの **any_ally 極性**（付与先を guest にしただけの対照で非発火）・`WXK10-049` の**ランサー条件**（同じアクセを別シグニへ移すと付かない）。
  - **🔎② V-19 は「場から消える／トラッシュへ行かない／返却ログが出る」まで到達したが `lrig_deck` で観測できない**＝`fieldGone=true notTrash=true log=true` なのに **`host.lrig_deck` が空**。🔑**配線は正しい**（funnel が `lrig_deck` を返し、**2経路とも**永続化＝`BattleScreen.tsx:3676`／`:4099`。**ログは `returned.length>0` のときだけ出る**）。⚠**断定せず要追試**＝候補は**観測側**（spec が `lrig_deck: []` 始まり）と**永続側**（`myEndState` 組み立てで上書き＝カード消失）。**📌12 のとおり engine より先に観測側を疑う**。
  - **🔑③ ドライバの罠を2件追加（📌23・24）**＝**レゾナの配置先は `SELECT_SIGNI_ZONE` の「ゾーンN」で `summon-zone-{zi}` ではない**（取り違えて574秒/606秒を溶かした。`H.clickZone()` へ替えたら前進し 109秒→73秒）／**ラベルを読んだ時点で開いていたモーダルがクリック時には閉じている**（開き直す自己回復で **264秒 FAIL→6秒 PASS**）。
  - **🔑④ Codex 委譲の所見**＝**追加 testid 0件・React 非変更**、**既存不変を SHA-256 で3系統証明**（うち1つは指示書が「コピー元として読め」と書いた `keysAbilityLossTurnEnd*` を名指しで固定＝**危険箇所を自分で特定した**）。**D-5 の前提を実コードで再確認**（`WX16-Re18` は今も1枚しか出ない）。**原文差1件を申告し、かつ「今回の検証はその過剰範囲に依存しない」ことまで明示**（`WXEX2-19-E1` の `hasIcon:'アクセ'` 欠落）。
  - **▶ 次の一手**＝**V-19 の切り分け**（観測側→永続側の順）。残る赤は**10本**＝(cxxxiv) 再現1・V-15/16/17 のシナリオ未完5・V-19 の4。§3 の在庫は **5件**。
