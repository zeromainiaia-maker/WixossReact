...
- **セッション（2026-08-24・続き637・Opus 5）＝§5.1 🅱「実機未検証の返済・最優先」を残0クローズ（V-84／V-85／V-83／V-04／V-16／V-87 の6件）**。新規実機シナリオ **14本**（すべて2〜3回連続PASS）／**実バグ1件を発見・修正**／gates 全緑（**golden 2653**＝+2）。
  - 🔴**V-83 で実バグ**＝`TRASH{DECK_CARD}` が `last_effect_mill_source` を書かず、発生源限定つきミル誘発（`WX24-P3-030-E1`）が**恒久 no-op** だった。**＜悪魔＞の自デッキミルは `TRASH{DECK_CARD}` 経路が35枚以上・`MILL` 経路はわずか2枚**＝ほぼ全部が死んでいた。既存 golden は collector 側だけを見ていて**書き手側の穴を検知できなかった**ので、両経路を固定する golden を追加。
  - **V-04 は母数を実測で確定**＝`planEnergyPayment` は **15サイト**（資料の 17/14/13 はどれも支払いサイト数ではなかった）。**全15サイトが `applyTo` を呼ぶこと**を golden の静的ゲートで固定＝**実機は1経路ずつしか踏めないので母集団は静的に守る**という配分。
  - **見立てと実測**＝`DECLARE_NUMBER` 30カード/31効果（一致）／`ON_ABILITY_ACTIVATED` **2効果だけ**（両方カバーして残0）／`ON_TRASH`+`fromZones:hand` 32効果（記録と一致）／`milledSourceStory` **2効果**（登録票は1件）。
  - **観測できるようにした UI 3箇所**＝`GuardResponseDialog`（`guard-card-{i}`）／`SigniActivatedModal`（`signiactcost-energy-{i}`＝PLAN が「踏むなら testid 追加が先」と名指ししていた地点）／`EndDiscardModal`（`enddiscard-hand-{i}`）。いずれも `data-card-num` 付きで**インスタンスIDで狙える**。
  - ⚠**シナリオの罠を4つ踏んだ**（すべて §4.4 へ追記済み）＝①`field.check`/`field.signi` は注入で引き継がれる ②disabled ボタンを `clickTextOrBtn` が「押せた」と報告する ③効果開始前に完了判定すると位置依存フレークになる ④確認ダイアログが挟まると裏のボタンを押し続けて永久に進まない。

  - 🆕**続き637c＝V-19／V-86 も残0クローズ（🅱 と 🅲 が在庫0）**。**V-19 で実バグ**＝一時レゾナが「場から消えるがルリグデッキにも戻らない」＝**カードがゲームから消滅**（ターン終了の出口3つのうち通常枝だけ `lrig_deck` への戻し入れが欠落＝「手札が少ないターンだけ消える」）。golden +1 で3サイトを静的に固定。
  - 🔴**V-86 の道中でバトル処理の engine バグを2件発見（未修正・§5.3 へ登録）**＝**O-47 バトルで負けたアタッカーがバニッシュされない**（`else` がログを出すだけ）／**O-48 バトルのバニッシュがスタックごとエナへ行く**（下のカードがトラッシュに落ちず `fromLeftFieldUnder` 4効果がバトル経路でだけ恒久 no-op）。**どちらも V-86 とは母集団が別で影響範囲が広い**ため、実測値を添えて登録した（V-86 自体は3経路とも engine バグ0で緑）。

**▶ 次の一手**＝🔴**§5.3 の O-47／O-48（バトル処理の実バグ2件）を先に取る**（ルールの根幹で全対戦に効く・§5.1 に残るのは 🅳 6件＋続き616〜634 の21バッチ）。
...
- 🔴**既存の計器に映らない穴が1種類ある**＝**「受け皿（型・engine）はあるのに parser が配線していない」**。`census` にも `census:wiring` にも出ない。**6回連続で当たった型**なので、「型はあるが live 利用が極端に少ない語彙」を数える計器は最優先の在庫（未着手）。
...
> **残っているもの**＝下記 `V-nn` エントリ **6件**（🅱 と 🅲 は 2026-08-24 に**残0クローズ**／🅳 6。🅳 の各エントリは (a)(b)(c) の複数経路を持つ） ＋ **続き616〜634 の21バッチ**（段2 の parser/engine 改変ぶん・特に実機経路へ直接触ったのは第34「0枚選択可の UI」・第37「クラッシュ主体の限定」・第38「群割当の決定可否」・第40「`END_OF_ATTACK`」）。
...
### 🅳 未着手・機構実装ぶん（engine/golden では固定済み・実機だけ未確認）

> どれも**ヘッドレスでは検証できない層**。⚠着手前に §4.4 を読むこと。
...
> **残 OPEN の軸内訳**＝(軸なし)244／(未分類)105／`filter.story` 60／`キーワード能力` 43／`condition` 30／`timing/trigger` 30／`特殊機構` 27／`filter.状態` 25／`cost` 22／`action丸ごと欠落` 19／`filter.cardName` 16 ほか。**`filter.color`・`count/upTo`・`filter.level`・`duration` はほぼ枯れた。**
  ...
  - 🆕🔑**2026-08-22 続き595＝進め方を「効率順」に組み替えた（次セッションからこの順で回す）**。
    ...
    - **① 未 triage 662件を Codex で段1 triage【最優先・次セッションはここから】**
      ...
      **同じ action 型を隣り合わせて**出す（明細に原文＋指摘＋**live JSON** を同梱）。
      ...
      ⚠**1バッチ 40 findings 前後**（CODEX_GUIDE §5.5＝107件投げたバッチは根拠がテンプレ化した）。
...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-47** | 🆕🔴**バトルで負けたシグニがバニッシュされない（アタッカー側だけ）** | M | **2026-08-24 V-86 で発見（実機ログで確認）**＝`BattleScreen.tsx` の `resolvePendingSigniBattleFor` は `if (myPower >= opPower)` で**防御側だけ**をバニッシュし、`else` は **`「…はバトルに敗北」` とログするだけで何もしない**。実測ログ＝`紅魔姫 シヴァ（10000）vs 極剣 ゴッドイーター（15000）` → `シヴァはバトルに敗北` → **シヴァは場に残ったまま**。ルールは「パワーの低い方がバニッシュ／同値なら**両方**」なので、**①攻撃側が負けても消えない ②同値のとき攻撃側が消えない**の2点が過小。⚠**攻撃側・防御側の双方が人間/CPU で同じ関数を通る**＝全対戦に効く。⚠**着手前に既存の実機シナリオを全数見直す**（「攻撃側が生き残る」前提で書かれたものが緑のまま壊れる可能性）。 |
| **O-48** | 🆕🔴**バトルのバニッシュがスタックごとエナへ行く（下のカードがトラッシュに落ちない）** | S | **2026-08-24 V-86(b) で発見**＝`BattleScreen.tsx` のバトル勝利処理は `energy: [...opS.energy, ...opStack]` と**スタック全部**をエナゾーンへ送る。ルールは「ライズのシグニが場を離れるとき、**下にあったカードはトラッシュ**へ」。⚠**engine 側（`effectExecutor` の効果バニッシュ）は正しい**＝実機で `ピカトリクス→エナ／下のカード→トラッシュ` を確認済み＝**壊れているのは BattleScreen のバトル経路だけ**。実害＝`fromLeftFieldUnder`（「このシグニの下にあったカード」＝live **4効果**）が**バトルで倒された場合に限り候補0件で恒久 no-op**。V-86(b) は効果バニッシュ経路で緑にしてあるので、修正後は**バトル経路のシナリオを1本足す**こと。 |
...
- **2026-08-24 続き637 後（本行が直近の正）**：
  **census 608/608**、**golden 2654**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 269）**、
  ...
  **実機シナリオ定義総数 496**（+20＝V-84/85/83/04/16/87/86。V-19 は既存4本の修理）、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
