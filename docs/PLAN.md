...
- 🏁**セッション（2026-08-26・続き675・Opus 5）＝§5.2 意味照合 段2 第43バッチを Codex CLI へ委譲して消化。「主語・配置の限定が落ちて engine の既定値へ倒れる」9効果**を既存語彙だけで是正（新しい action 型・Condition・TargetFilter フィールドの追加ゼロ）。**段2 台帳 OPEN 677→668**（段2 消化 435→444・段0/段1 は不変＝§5-28′ のズレなし）。
  - 🔑**バッチの切り方は「軸」ではなく「live JSON の構造署名」で割った**＝残 OPEN 677件を live の action 構造でクラスタ化（428群）してから上位群の engine 消費地点を読む。⚠**PLAN §5.2 が指示する第2層（軸 × action型 × type）は器が同じだけの群が混ざる**ので、署名クラスタのほうが systematic な根に当たった。
  - 🔴**母集団は「JSON に語彙が無いか」ではなく engine の消費地点で決まる（§5-3-3′ の再実証・2件）**＝(1) CONTINUOUS `GRANT_PROTECTION` の保護コレクタ**5箇所すべて**（`effectEngine.ts:3956/4934/5133/5282/5608`）は **`target` を見ずに `subjectFilter` だけを読み、無ければ効果元自身を守る**＝`target:{SIGNI,self,count:1}` は「このシグニは」の**正しい**書き方。47効果中37は正しく、**真の穴は3効果だけ**（findings の「thisCardOnly が無い」指摘は偽陽性）。(2) ゾーン限定は原文76枚中**63枚が既に受け皿入り**。
  - 🔴**自作の計器で受け皿を数えるときは「別名の受け皿」を先に列挙する**＝ゾーン限定の判定 regex に `SIGNI_ATTACK_BAN{zones:[1]}` と `PLACE_VIRUS_CENTER` を入れ忘れ、**穴を13枚と過大に見積もった**（実測は2枚）。Codex が実コードで訂正した。⚠**計器が「受け皿ゼロ」と言ったら、まず計器の語彙表を疑う。**
  - 🔑**`manualEffects.ts` の shadow は live では `AUTO` と記録されるのでどの計器からも見えない**＝`WX06-022-E1` は「パターン6e が発火していない」と見立てたが、真因は**古い manual shadow が正しい parser 出力を凍らせていた**こと（`PRESERVE_STATUSES` がカードごと温存）。⚠**`censusManualDrift` の「削除候補」は実体同一のものしか出さない**ので、この形（manual が parser より劣化）は `CHANGED` に紛れて worklist にならない → §5.3 `O-93` へ登録。
  - ⚠**`docs/CODEX_GUIDE.md §2` の「自動コミットフックは現在は存在しない」は誤り＝復活していた**（Codex 完了と同時に `auto: …` コミットが 16ファイルを勝手に確定した）。**ベースラインは必ず commit SHA で確定する**（今回は `2d363ceb0` 固定で検証したので影響なし）。ガイドを訂正済み。
  - ✅ gates 全緑を**独立実行で確認**（golden **2845→2854**／census **563→562**（baseline 更新）／`census:enginetext` A群 141 据置／smoke・fuzz 全0／lint 0 errors・warning ±0）。**live 変化は対象9効果ちょうど**（新規0・消滅0・同カードの巻き添え0）、**held 76→75・新規流入0**（減った1件は前セッションが `_held_review.txt` を再生成せずコミットした stale で、本件とは無関係）。
  - ⚠**実機シナリオは新規ゼロ**＝`git diff` は `src/screens/` に一切触れず（parser＋データのみ）、新機構ゼロ、受け皿5経路はすべて既存。追加した golden は JSON 構造 assert ではなく**実 engine**（`calcFieldPowers`／保護コレクタ5本／`run()` の BANISH）を両方向で通す。§5.1 が段2 parser バッチについて記録した判断（「golden と census が母集団を押さえているぶん実機で踏むべきものは残っていない」）に一致する。

**▶ 次の一手**＝**§5.2 段2 の次バッチ**（残 OPEN 668。⚠**軸ではなく live の構造署名で割る**＝今回の勝ち筋。着手前に `node scripts/archive/semanticAuditLedger.mjs` で数え直す）。機構側なら **`O-93`**（manual shadow の掃除＝`CHANGED` の「parser 由来 12効果」が母集団）か **`O-94`**（ゾーン限定の残穴2枚）。`O-92`／`O-80` 第2バッチも生きている。実機の腐り返済なら §5.1 の `V-89`。
...
> **母集団＝残 OPEN 668件**（`node scripts/archive/semanticAuditLedger.mjs` の実測が唯一の正。2026-08-26 続き675 時点＝段2 消化444／段0除去221／段1偽陽性111／HIGH・MED・LOW＝463・201・4／影響カード501・効果527）。⚠**この数字はすぐ古くなる。着手前に必ず実行して数え直す。**原文 vs effects JSON を LLM で意味比較して見つけた指摘の worklist。仕組みは [SEMANTIC_AUDIT.md](./SEMANTIC_AUDIT.md)、Codex 運用は [CODEX_GUIDE.md](./CODEX_GUIDE.md)。
