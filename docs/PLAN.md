...
**直近＝2026-09-22＝§5.7 `S-31` ② 第4段＝誰も払っていなかったコスト4種（撃てない【起】 37 → 26）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第447バッチ）
- 🔴**測り方を変えて4件まとめて見つけた**＝「CPU が撃てない」ではなく**「実行経路がそのコストキーを読んでいるか」を全数で**見た
  ⇒ 📏**4キー／live 11効果が「提示は通るのに誰も払わない」**＝**人間も CPU も踏み倒していた**
  （`trashArtsFromLrigDeck` 5／`fieldToLrigTrash` 2／`selfToDeckBottom` 2／`chargeCounterRemove` 2＝**【貯菌】0個でも撃てた**）。
- 入れたもの＝**支払い4本**（`fieldToLrigTrash` は `payFieldTrashCost` が行き先分岐を既に持っていた＝条件に足すだけ）
  ＋**提示ゲートの検算3本**＋**人間のアーツ選択UI**（`LrigGrantedModal`）＋ CPU（allowlist・`pickCpuTrashArtsNums`・探索の近似）。
- 🔴**実機ハーネスの「一度も当たっていなかった判定」を直した**＝2026-09-18 に足した「`Lv超過` の候補は押さない」が
  **`pick-N` の兄弟 `<span>` を見ていなかった**ため一度も当たらず、この回の実機で**60秒手詰まり**として再発。
  もう1つ＝`modalStep` だけ「封印」を受け取っておらず、効かない手を押し続けていた。⚠**engine 側のバグではない**。
- 📏**実カード照合**＝提示された **24（場のシグニ）＋14（ルリグ）すべてで支払い選択が成立**（食い違い0）。
  🔴**実機 PASS**＝`VERIFY_DECK_MECH`（6ターン/170手）と `VERIFY_DECK`（6ターン/159手）の両方で決着。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` ②の残り**（撃てない【起】26＝**主に「人間がモーダルで選ぶ」型**）。次に `S-29` |
...
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**撃てない【起】は残 26効果**（`S-31` ②）／うち実行側の口が無いのは `fieldDown`（ルリグ2）だけ |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4365**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**①は測って否定**（第444バッチ）／🏁**③は消化**（第445バッチ＝札の使いどころ）／🏁**②は4段まで**（手札捨て／エナ・場／自分の盤面・デッキ／**誰も払っていなかったコスト4種**）＝**撃てない【起】 210 → 108 → 37 → 26**。**残り＝②の続きだけ**（場のシグニ 17／ルリグ 9）。🔑**残りは主に「人間がモーダルで選ぶ」型**（`discardGroups`／`trashExile`／`beat_signi`）＝**CPU に選ばせる判断を書く**のが仕事（踏み倒しの穴ではない）。⚠実行側の口が無いのは `fieldDown`（ルリグ2）だけ。 | 中 | 作った CPU デッキを意図どおりに回せる |
...
- **2026-09-22 時点**（第421〜447バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
  ...
  - 📏**CPU が撃てない【起】**（§5.7 `S-31` ②・**その文脈の gate が提示するものだけを分母にする**）＝**場のシグニ 697効果中 17（2.4%）／ルリグ 567効果中 9（1.6%）＝合計 26**（第439・442・446・447バッチで 210 → 108 → 37 → 26）。🔴**過去の大きい数字は分母の取り方が違う**＝旧「648（24.6%）」は文脈を混ぜた値／旧「83」は**入口が場ではない【起】**を含んでいた
