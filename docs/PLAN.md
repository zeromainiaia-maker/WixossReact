...
- **🆕 セッション（2026-08-03・続き335・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(lxxxi) 残0クローズ**＝前回 honest defer した2枚を**別機構2本**で実装し、ついでに**スペル本体が丸ごと不発になる過剰ゲート**を1件是正**（ユーザー指示「残した2枚を実装する」）。golden **1331→1333**・census **1289→1288**・全ゲート緑。live JSON の変更は**全5ファイルで3カードのみ**。
  - **①`WXK03-002-E3`＝カード名指定の“ゲーム間”コスト置換**。使用側（クラフト《落華流粋》）の原文には**何も書かれていない**＝続き334 の EffectText 由来 regex 表では原理的に届かない。⇒ **新アクション `SET_CARD_COST_REPLACEMENT` ＋ `PlayerState.card_cost_replacements`**（ゲーム間持続・同名は後勝ち）を新設し、`computeCostReplacement` の**最優先分岐**（EffectText ガードより前）で読む。parser 規則は `ALT_COST_OPP_TURN` の隣に1本。採用は `heldReview --adopt` で**E3 の1効果だけが違う**ことを diff 確認してから。
  - **②`WX21-071`＝使用時の任意支払いでコスト置換**。⇒ `costs.ts` に `parseOptionalDiscardForCost`／`optionalDiscardSatisfied` を新設し `SpellCastModal` に支払いUIを追加（`castSpell` に `discardIndices`）。**見立てでは1枚だったが実測2枚**＝同型の `WX21-035`（赤と緑の＜龍獣＞→《赤×0》）が無償で載った。⚠**貪欲な突き合わせでは足りない**＝多色シグニ（「青黒」の＜電機＞）は両グループに当たるので割り当て次第で成立/不成立が変わる＝**全割り当てをバックトラック**。
  - **🔴 実装中に “本体丸ごと不発” の過剰ゲートを発見して是正**＝この2枚の action は先頭に `STUB OPTIONAL_COST` を持ち、`effectExecutor` の **Pattern⑤** は「skip → **残りステップをスキップ**」。つまり**使用時に支払い済みのプレイヤーが解決中にもう一度聞かれ、自然に「スキップ」を選ぶとスペル本体が消える**（原文では支払わなくても本体は動く）。⇒ この文型では先頭 `OPTIONAL_COST` を落とす。⚠**2枚とも MANUAL＝PRESERVE 保護で `build:effects` が届かない**ので `manualEffects.ts` 修正＋**外科パッチ**（`scripts/archive/lxxxiTailStripUseTimeOptionalCost.mjs`・適用済み／冪等確認済み）。
  - **📉 census 較正 1289→1288**＝no-op STUB を実アクションへ替えたら、**STUB が与えていた「格納」の言い訳が外れて偽陽性が表に出た**（`コスト:《コイン》`）。原文は 《コイン**×0**》＝支払う枚数ゼロ＝JSON に載せるコストが無い⇒ instrument 側を較正。⚠**最初に書いた較正は広すぎた**（ベット由来の《コイン》まで `/BET/` 要求なしで許して 21→17 と不自然に落ちた）＝`/BET/` 要求を残す形へ直して純増分を狙った1件だけに絞った。併せて `BASELINE_HIGH - 2` の**暫定オフセットを解消**（行内履歴と実ゲート値が2ずれたままだった）。
  - **新規在庫を1件登録＝(lxxxv)「使用する際…そうした場合、使用コストは…**減る**」22枚**（今回の「になる」2枚とは支払いの粒度が違う＝枚数比例・「2枚まで」の可変・支払い元がルリグデッキ/ライフ/場/キー）。UI の骨組みは流用できる。前セッション登録の **(lxxxiv)** ＝カットイン経路のベット宣言UI欠落も生きている。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／**🆕(lxxxiv)**／**🆕(lxxxv)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜335 の全件が実機UI未検証**。⚠🔴**続き333〜335 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。実機で確認すべき最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①`SpellCastModal` の任意支払いUI**（候補の絞り込み・組が揃うまで置換されない・揃った時だけ捨てる・**支払わなくても本体が動く**）**②`WXK03-002-E3` を撃った後に《落華流粋》がアーツ一覧で《黒×2》《無×1》で使えること**③続き334 のベット宣言でのコスト変化。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き335（🏁§3 タスク12(lxxxi) 残0クローズ）後 最新値（本行が直近の正）**：golden **1331→1333**（+2＝`SET_CARD_COST_REPLACEMENT` の engine 書き込み〔クラフト追加との順序・置換後コスト・**同名再設定の後勝ち**〕と UI 読み取り〔別カード名には効かない〕／任意支払い2枚の仕様・未払い/支払い済み・**多色シグニのバックトラック**・境界〔1枚/3枚/クラス違い〕・**母集団は実測2枚だけ**・**先頭 `OPTIONAL_COST` が live から落ちていること**）、census **1289→1288**（`コスト:《コイン》` に《コイン×0》較正を追加＝STUB を実アクション化して表に出た偽陽性の是正。**`BASELINE_HIGH - 2` の暫定オフセットも解消**＝ゲート値は `BASELINE_HIGH` そのもの）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0、held **259枚／110群**（`WXK03-002` 採用で元に戻った）。**live JSON の変更は全5ファイルで3カードのみ**＝`WXK03-002`（heldReview 採用）／`WX21-035`・`WX21-071`（MANUAL＝PRESERVE 保護のため**外科パッチ**）。**新語彙 1本＝`SET_CARD_COST_REPLACEMENT`**（＋`PlayerState.card_cost_replacements`）。⚠**支払いUI（`SpellCastModal` の任意支払い）とコスト算出は golden 非カバー**＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き335＝🏁§3 タスク12(lxxxi) 残0クローズ後）**＝golden **1333**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **259枚／110群**。
>
