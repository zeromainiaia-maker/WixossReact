...
- **🆕 セッション（2026-08-14・続き475／475b／475c／475d・Opus 5）＝**§7 実機検証の続き＋そこで見つけた engine バグの一掃**＝**V-01／V-02／V-09④／V-10 が全シナリオ緑（10本）**。**§3 在庫は 8件→3件**＝(cxxv) を「シナリオ偽陽性」として取り下げ、**(cxxvi)(cxxx)(cxxix)(cxxvii)(cxxviii) を残0クローズ**。ゲート全緑（**golden 1964→1972**＝トリップワイヤ8本追加・census 831 据置・smoke 10688／SKIP 0・fuzz 全0・同型★0・held 106 据置・lint 0 errors/259 warnings）。**live JSON は 18枚更新**（(cxxvii) 2枚＋(cxxviii) 17枚のうち重複1）。**
  ...
  - **▶ 次の一手**＝**Opus 側**：§3 の残在庫は **(cxxiii)（ピースのコスト二重請求＝設計裁定が先）** ほか3件のみ。次点＝§6.4 O-20 の残21件・O-19。🆕**今回の副産物 worklist**＝(a)「あなたの**白の**ルリグ１体がアタックしたとき」（`WXDi-P03-035-E1`）は**色修飾が triggerFilter に載らないので timing は ON_ATTACK_SIGNI のまま**＝残（帰結の LRIG 化だけ入った）(b)`SPDi43-28-E1`（「あなたの《335　アキノ》１体がアタックしたとき」＝MANUAL）も同型。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き475／475b／475c／475d（§7 V-01・V-02・V-09④・V-10 決着＋§3 実バグ5件クローズ）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、**golden 1972**（**1964→+8**＝(cxxvi)(cxxx)(cxxix)×2(cxxvii)×2(cxxviii)×2 のトリップワイヤ）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（**採用17枚ぶんは相殺されて据置**）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。🆕**live JSON changed 18枚**（`SPDi43-06`／`WXDi-P05-037`＝(cxxvii)、`WX19-021/023/031/034/054`・`WX20-022`・`WX25-P2-050`・`WX26-CP1-046`・`WXDi-CP01-028`・`WXDi-P03-035`・`WXDi-P04-051`・`WXDi-P06-049`・`WXDi-P07-056`・`WXDi-P12-044`・`WXDi-P13-046`・`WXDi-P16-088`・`WXK11-006`＝(cxxviii)）。🆕**実機シナリオ＝既定 order の総数は据置**（新規追加0・**判定/経路を直したのが9本**・**id を1本 rename**）。**緑/赤の内訳が 45緑12赤 → 57緑0赤**（§3 在庫バグ由来の赤トリップワイヤは全て緑へ反転）。🆕**Opusタスク12 の在庫＝3件**。
