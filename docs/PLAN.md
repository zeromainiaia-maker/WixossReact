...
- 🏁**セッション（2026-08-24・続き646・Opus 5）＝`O-56`＝`TRAP_OP`／`TRAP_OPERATION` の**カード全文 regex 多重ディスパッチ**を撤去**（**実装は Codex へ委譲**・母集団実測／指示書／検証／実機・**engine バグ1件の発見と修正**は Claude）。gates 全緑（**golden 2679→2683**／census 601 据置／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260（増分0）／同型★0）。**実機3本が2回連続 PASS＝1巡が §2.2 の⑤まで完了。差し戻し0・是正3。**
  - ✅**engine が `EffectText + BurstText` を丸ごと `includes()` して枝を決めていた**＝**いま実行している文とは別の文**（別能力・LB・括弧内のルール注記）の語で動作が決まり、**18効果中9件が誤った枝**にいた。`WX17-044-E1` は「残りをトラッシュに置く」に食われて**設置せずトラップを1つ捨てる**、`WXK11-036-E2` は「チェックゾーンにある**かのように**」の否定を読めず**実際にチェックゾーンへ置いて**いた。
  - 🔑**真因は engine ではなく parser**＝emit 地点7箇所すべてが**判別情報ゼロの裸 STUB** を吐いていた。各地点は別々の regex で分岐＝**その時点で意味は確定している**のに捨てていた。⇒ `StubAction` に `trapOp`／`trapSource`／`trapRemainder` 等の optional を足し（**新 action 型ゼロ**）、engine は payload だけで分岐。
  - 🔴**投入前の実測で登録票を訂正**＝`TRAP_OPERATION` は「18効果」ではなく **12**（`TRAP_OP` 6は正しい）＝合計 **18効果・21ノード**。**登録票が外れたのは12回連続**（§4.1）。
  - 🔴**検証側で engine の実バグを1件発見・修正**＝**`burst_as_check` が「発動しますか？」の CHOOSE を1往復すると対象カードを見失い無言で done** していた（`ctx.lastProcessedCards` から読み直していたが、実機は往復のたびに永続化を挟むので残らない）。`WXK11-036-E2` は「`field.check` を変えない」正しい半分だけを達成し**LB を1度も発動しなかった**。⇒ 対象を CHOOSE を出す**前**に確定させ `trapBurstCard` で運ぶ。
  - ⚠**既存 golden がこれを見逃した理由が重要**＝`run()` のオートパイロット（`finish`）は `lastProcessedCards` を**持ち回る**ので往復しても対象が消えない＝**golden の autopilot は実機の永続化往復を再現しない**。⇒ **往復を手で再現する golden**（`lastProcessedCards` を空にして再開・ペイロードを剥がすと落ちる対照つき）を追加した。**この型のバグは今後も golden をすり抜ける。**
  - ⚠🔴**「全文 regex 撤去」は2ハンドラ限りで系統としては残っている**＝Codex の「残存0」申告は**4つの変数名を grep しただけ**。`PLACE_TRAP_FROM_REVEALED` が**公開枚数をカード全文から読む**まま（しかも本バッチが2効果を新たに流し込んだ＝現状は正しく動くが「たまたま当たっている」）。**engine で `EffectText` を読む箇所は実測 200・未分類**。⇒ **`O-60`** へ登録。
  - ⚠**Codex は2バッチ連続でシナリオを「書いたが未実行」で引き渡した**（環境に Chromium 無し）。**走らせたら3本とも落ち**、内訳は engine バグ1件＋シナリオの腐り2件（`flow.picked` ゲートで詰む／デッキ2枚でリフレッシュが走りトラッシュが回収される）。

**▶ 次の一手**＝**§5.2 の段2 消化に戻る**（取り方は第2層＝軸 × live の action 型 × type で5件以上のサブ群。⚠器の型 `SEQUENCE` に乗った群は信号が弱い）。候補＝`キーワード能力 × GRANT_KEYWORD × MISSING/WRONG`（17件）、`filter.story × POWER_MODIFY × MISSING`（7件）、`timing/trigger × DRAW × MISSING`（6件）。⚠**worklist の見立ては標本**＝取ったら必ず生データへ戻して数え直す（**12回連続**で外れている）。🔑**§4.3 の「受け皿はあるのに parser から配線されていない」型が7連続で当たっている**。**`O-51`（並べ替え対話・279効果）は1バッチの粒度に合わない**ので、取るなら先に段取りを決める。
🆕**`O-60`（engine の全文 regex を系統として潰す）は「まず200箇所の機械分類」から**＝いきなりハンドラを直さない。分類が終われば `O-56` と同じ手口で1ハンドラ＝1バッチに割れる。
🆕**Codex 委譲時の必須手順（続き645・646 で2回連続）**＝**シナリオを書かせても実行はさせない前提でスコープを切る**（環境に Chromium が無い）。**実機実行・反転確認・回帰ベースラインは検証側（Claude）が必ず引き取る**。⚠**続き646 は走らせたら3本とも落ち、うち1件は engine の実バグだった**＝「未実行のまま緑」を信用しない。
🆕**golden の autopilot は実機の永続化往復を再現しない**（`finish` が `lastProcessedCards` を持ち回る）＝**CHOOSE / SELECT_TARGET を挟む機構は、往復を手で再現する golden を別に書く**（続き646 の `burst_as_check` はこれが無くて素通りした）。
...
> **母集団＝残 OPEN 730件**（`node scripts/archive/semanticAuditLedger.mjs` の実測が唯一の正。2026-08-24 続き638 時点＝段2 消化379／HIGH・MED・LOW＝507・219・4／影響カード545・効果579）。⚠**この数字はすぐ古くなる。着手前に必ず実行して数え直す。**原文 vs effects JSON を LLM で意味比較して見つけた指摘の worklist。仕組みは [SEMANTIC_AUDIT.md](./SEMANTIC_AUDIT.md)、Codex 運用は [CODEX_GUIDE.md](./CODEX_GUIDE.md)。
...
- **2026-08-24 続き646 後（本行が直近の正）**：
  **census 601/601**、**golden 2683**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 260）**、
  `census:stubs` **A群🔴0／C群0**、manual-fields **0**、**同型★ 0**、
  **live カード 5975 / 効果総数 10693**、`_held_fresh` **75**（-1）／`_partial_fresh` **15**／`_idset_fresh` **45**、
  **意味照合 残 OPEN 713**（段2消化 397・**本バッチは engine/parser 配線のみで台帳は動かず**）、
  **実機シナリオ +3**（`o56TrapSetFromDeckNoDuplicate` / `o56ArrowRainSetsTrapNotTrashesIt` / `o56BurstAsCheckDoesNotEnterCheckZone`）、version **0.502**。（⚠`census:wiring`／`census:timing`／`census:goldentypes` は本バッチ未再計測）
