...
- **🆕 セッション（2026-08-15・続き494・Opus 5）＝§6.4 **O-28 の「《無》×Nを支払わないかぎりアタックできない」5効果を消化＋途中で見つかった engine 一般バグ2件**（**恒久 no-op 5／丸ごと no-op 1／過剰実行 1／対象の過少 1**）。ゲート全緑（**golden 2040**＝+6・**census 831 据置**・smoke 10688 / SKIP 0・lint 0/260）。live changed **6効果/6カード**。
  - **🔑① 引用文が `GRANT_KEYWORD.keyword` に丸ごと入るクラスの《無》版**＝続き490 の fixup を一般化（`rewriteAttackTaxKeywordGrant`）して正準形 `SELECT_TARGET_ONLY→STORE→SIGNI_ATTACK_BAN{targetsStored, unlessPayColorless}` へ。⚠**コスト句の無い `keyword:"アタックできない"`（live 132件）を巻き込まない**ため regex はコスト句必須。⚠**`STATE_COND_BATCH4_ACTIONS` の action リテラルで固定されているカード（`WX11-012-E1`）は parser 規則を足しても変わらない**＝リテラル側に正準形を書く。
  - **🔴② `SELECT_TARGET_ONLY` がルリグ対象を扱えず丸ごと no-op だった**＝`tgt.type !== 'SIGNI'` で `lastProcessedCards:[]` に降りるため、STORE が空→本体が「対象が確定していない」で降りる（ルリグ／ルリグかシグニの3効果）。`LRIG`／`CENTER_LRIG_OR_SIGNI` を追加。🔑**live の形を assert するだけでは足りない**＝golden に**通しで実行して ban が載るか**を見るテストを足した（無ければ「JSON は正しいのに一度も効かない」を緑のまま見逃す）。
  - **🔴③ 入れ子 SEQUENCE で内側の continuation が上書きされ消えていた（engine 一般バグ）**＝`execSequence` は対話に入ると残りステップを `pending.continuation` へ**代入**していたので、内側 SEQUENCE や `LOOK_PICK_CHAIN` のように**自前の continuation を積む** action の残りが無言で落ちていた（`WX24-P4-001-E1` が丸ごと no-op／`WXDi-P01-026-E1` は「残りをトラッシュに置く」が脱落＝**golden の旧 assert がこの壊れた挙動を写していた**ので正方向へ書き換え）。**合成する**へ是正（同じ規約は `resumeOpponentPayOptional` ほか4サイトで既に合成済み＝`execSequence` の2箇所だけが outlier）。母集団＝**入れ子SEQUENCE+後続 47効果／内部continuationを積みうる型が中間ステップ 147効果**。
  - **🔑④ ルリグのアタック税は新軸だが置き場は増やさない**＝`SigniAttackBan.appliesTo:'LRIG'` を1キー足して `signi_attack_bans_this_turn` を共用（turn-end 失効の登録が1つで済む＝続き487/489/493 で3回再発した「失効地点が無い」クラスを招かない）。仕分けは**実行時に対象の `Type`** で行う（`CENTER_LRIG_OR_SIGNI` は選ばれるまで決まらない）。⚠**両方向にガードする**＝`cardNums` 一致だけに頼ると広域 ban がルリグにも掛かる。🔴**既存バグ同時是正**＝`performLrigAttack` の `OPP_LRIG_ATTACK_COST` は「払えないときは else で素通り＝タダでアタック」だった（判定と引き落としを `lrigAttackCostInfo` の1関数へ）。
  - **🔑⑤ ③の副産物で過剰実行が1件顕在化**＝`WX17-003-E1` は `CHOOSE_SAME_OPTION_TWICE` と `_MULTIPLE`（engine では**同じハンドラ**）が2ステップ並び **4回＋4回＝8回**選べていた（③以前は2周目が捨てられて表に出ず・smoke SKIP 0→1 で発覚）。PARTIAL（PRESERVE）なので**外科パッチ＋parser dedup**。golden にデータ不変条件。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残8（`npm run census:stubs` の A群が常時の在庫表・機構ごとに独立着手可）＝`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）／`NEXT_OPP_TURN_END_BODY`（2効果＝遅延予約機構）。→ 🆕**O-30**（`WX24-P4-001-E1` 後半＝「《無》×4を支払わないかぎりバニッシュ」の**支払い回避が丸ごと落ちて無条件バニッシュ**）→ **O-28 の残**（正式名でない `keyword` は 23綴り＝丸ごと文が入っている4件〔`WX25-P2-071-E1`／`WXEX2-71-E3`／`WX24-P1-064-E1`／`WXDi-P05-068-E1`〕＋`使用条件` 4件。⚠`チアガール`／`トラップアイコン`／`シュート` 等は engine が同じ綴りで消費する**正規の内部キーワード**なので着手前に必ず消費地点を grep）→ **O-4**（UNKNOWN）→ **O-25 の残** → **O-26** → **O-27**。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き494 の4経路**＝(a)`WXDi-P03-036-E1`（相手アタックフェイズ開始時にルリグへ《無》×2 の税→**エナが足りるときは払ってアタックできる／足りないときはアタックできない**＝負方向＋対照の対）(b)`WX24-P1-041-E2`（「ルリグかシグニ」の選択モーダルに**ルリグが候補として出る**か）(c)`WX24-P4-001-E1`（アーツ1枚で**ルリグ税＋シグニ付与**の両方が乗るか＝③の continuation 取りこぼしの実機確認）(d)ボタン表示（`アタック（《無》×N）`／`アタック不可（《無》×N）`）。ほか続き488〜493 の未検証分。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き494（§6.4 O-28 のアタック税5効果＋engine の continuation 取りこぼし）後 最新値（本行が直近の正）**：census **831 据置**（較正なし）、**golden 2040**（+6＝ルリグ軸の ban 分離1・軸の相互漏れ1・live 形1・通し実行1・入れ子 continuation 1・選択ループ二重の不変条件1。ほかに `WXDi-P01-026-E1` の据置 assert を正方向へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 45群**（109→104＝5枚採用）、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0・明示 defer 23種/26件**（いずれも据置。C群は 259→258箇所）。🆕**live JSON changed 6効果/6カード**（`WX11-012`／`WX24-P1-041`／`WX24-P4-001`／`WXDi-P03-036`／`WXDi-P05-033`／`WX17-003`。CSV 非改変）。🆕**挙動是正 8**（恒久 no-op 5／丸ごと no-op 1／過剰実行 1／対象の過少 1）。🆕**新機構＝`SigniAttackBan.appliesTo:'LRIG'`＋`lrigAttackBanCost`／`lrigAttackCostInfo`（ルリグアタックの《無》前払いを判定と引き落としで1関数に）／`SELECT_TARGET_ONLY` の LRIG・CENTER_LRIG_OR_SIGNI 対応／`execSequence` の continuation 合成**。⚠**4経路とも実機未検証**（§7 送り）。⚠**continuation 合成の母集団は 47効果（入れ子SEQUENCE+後続）／147効果（内部continuationを積みうる型が中間ステップ）**＝すべて「落ちていた残りが動くようになる」側の変化だが、実機で目に見える差が出るのはこれから。
...
| **O-30** | 🆕**「〈コスト〉を支払わないかぎり〜する」の回避が action 側で落ちる** | 1効果 | 2026-08-15 続き494 で発見（隣接・別系統なので未着手）。`WX24-P4-001-E1` の後半「それは「【自】：このシグニがアタックしたとき、**《無》《無》《無》《無》を支払わないかぎり**、このシグニをバニッシュする。」を得る」が live では `BANISH{thisCardOnly}` だけ＝**支払っても必ずバニッシュ**する過剰実行。⚠アタック**制限**側（「支払わないかぎりアタックできない」）は続き490/494 で機構化済みだが、これは**支払わないかぎり別の結果が起きる**型＝`NegateAttackAction.escapeDiscard`（手札・1回きり）とも別。母集団は `/を支払わないかぎり、/` で数え直すこと |
