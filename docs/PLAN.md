...
- **続き457（§6.4 O-16 完了＝動的 delta のゾーン grant／キーの能力喪失）が持ち込んだ未検証UI 3件＝2026-08-13 続き461 で2件決着・残1件**
  - [ ] **レベル比例のゾーン継続**（`WDK10-009-E2`＝手札1枚を捨てて相手のシグニゾーン1つを指定）＝指定ゾーンのシグニのパワーが**レベル×2000 下がる**（レベル3なら－6000）。⭐**そのゾーンのシグニを入れ替えると新しいシグニのレベルで再計算される**（固定 delta に焼き込んでいたらここで－6000 のまま残る）。⚠**自分の次のターンが始まったら戻る**（相手ターン中は効いたまま）。**⚠機構が別（`FieldGrant{kind:'power', perTargetLevel}`＋`zoneSource:'designated'`）なので次バッチへ送った。**
