...
- **🆕 セッション（2026-08-05・続き347・Sonnet 5〔§7実機検証まとめて消化〕）＝§7実機検証worklistを大幅消化＋実バグ4件を新規発見**（ユーザー指示「実機検証の続きを行う」を複数ラウンド）。**engine/parser/JSON は無変更**（golden/census/held/smoke/fuzzは前回値からすべて据置＝1366/1288/257枚/10679/0）。触った層は`scripts/verifyBattleDrive.mjs`（新規シナリオ約20件＋`H.queryState()`の`sideOf()`拡張）と`docs/PLAN.md`のみ。
  ...
  - **🆕 新規実バグ5件を発見・Opusタスク12(ci)〜(cvi)へ登録**（詳細はPLAN §3の各行）：
    ...
    - **(cvi)** 🆕2026-08-05・Sonnet＝**`executeSigniOnPlayCost`（エクシードNコスト等）経由でスタックに積んだSEQUENCEは、途中に対話ステップ（CHOOSE解決やチェックゾーン確認）または0候補のauto-skipを挟むと続きが失われる**＝2枚のカードで再現。①`SEQUENCE[LIFE_CRASH{optional,triggerBurst}, BANISH{addLastProcessedCount}]`（`WX24-P4-015-E2`）＝任意ライフクラッシュのCHOOSE解決→クラッシュしたカードのチェックゾーン確認（バーストなし「エナに送る」）という2段の中断を経由した後、続く`BANISH`が一度も発火しない（クラッシュ自体は正しく完了・`verifyBattleDrive.mjs exceedDynamicTargetCountB`で2回連続再現）。②`TRANSFER_TO_HAND{transferGroups}`（`WX24-P4-017-E2`）＝スペル枠（候補0枚・無音でauto-skip）の後、続く青シグニ枠（候補1件）のSELECT_TARGETが一度も現れない（`verifyBattleDrive.mjs exceedTwoGroupPickerC`で2回連続再現）。**exceedコストを介さない同型SEQUENCEでは対話を挟んでも継続する例が多数あるため（`sequenceContinuationAcrossGate`等）、`executeSigniOnPlayCost`固有の配線漏れの疑いが濃厚。**
  ...
  - **次の一手**＝**Sonnet**＝§7「残る実機検証項目」の残りを上から＝**タスク12(lxi)第11波**（`WXK06-067-E1`のゾーンを跨いだ選択モーダル・本プロジェクト初のUI）／**タスク12(lxiii)**（選択肢可否表示・中央ゾーン限定ピッカー）／**タスク12(lxi)第2波・第3波**（3択+エナ枝・4つ目の枝）。**Opus**＝タスク12の生き残り在庫〔(lv)残2経路／(lxvi)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／(xcvii)／(c)〕＋**新規(ci)〜(cv)**（5件とも実機検証で確認済み・再現手順はPLAN §3の各行とscripts/verifyBattleDrive.mjsの該当シナリオコメントに記載）。
