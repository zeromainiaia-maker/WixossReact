...
| (lxi) | **✅本消化済み（2026-07-30・29カード30効果）＝残は第2波の「対戦相手は」形13効果**。「対戦相手**が**」形の文中形（対象節つき）とエナ／手札捨て／併記の4コスト形を `matchOpponentUnlessGate()`（`effectParser.ts`）で一般化し、回避クローズが無言消費されて X が無条件実行される過剰実行を停止した（新語彙0・engine の `STUB{OPPONENT_PAY_OPTIONAL}`＋直後 `CONDITIONAL` ペアは既存）。**🆕残＝①「対戦相手は」形13効果（主語分配＝帰結の実行主体が相手自身のため単文パースで owner が反転する。`SPDi43-01-E1`／`WDK10-001-E2`／`WX11-044-BURST`／`WX15-033-BURST`／`WX24-P4-007-E1`／`WX24-P4-023-E3`／`WXDi-P01-028-E1`／`-E2`／`WXDi-P11-009-E3`／`WXDi-P16-091-E1`／`WXK05-001-E1`／`WXK06-047-E1`／`WXK06-067-E1`）＝次バッチの本体**②回避コストがエナ・手札以外3（`WX22-025-E3`／`WXDi-P16-088-E1`／`WXK05-009-E2`＝engine の CHOOSE に選択肢型の追加が要る）③対象節以外の前置き6④「そのシグニ」照応2⑤純加算チェック不一致1（`WXDi-P05-TK01A-E1`）⑥複文2。**除外（別機構の領分・触らない）**＝【ガード】ができない族（`GUARD_EXTRA_COST_BY_OPP`／`OPP_GUARD_COST_COLORLESS`）／アタック制限・無効族（`negated_attacks_escape`／`escapeDiscard`）／自己支払い型（`DISCARD_OR_PENALTY`）。⚠**次バッチも per-effect 実 diff から採用リストを作り、巻き込み全件を原文照合すること**（本消化では引用内へのホイスト4件と owner 反転1件をこの照合で捕まえた）。詳細 BUGFIXES 2026-07-30 の先頭節 |
...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕）＝PLAN §4 次の一手①「タスク12(lxi)〜ないかぎり家族の本消化」＝支払い回避クローズの文中形を parser 規則1本で一般化し 29カード30効果の過剰実行を停止**（golden **1113→1120**・census **1384→1375**（`BASELINE_HIGH=1375`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430・効果実行 7999手／distinct 2702種）・lint 0 errors/**230 warnings 据置**・同型★0（5986枚・265群）・held **290→288枚/107→106署名**・manual field loss 0・BOM/U+FFFD 新規0。ゲート独立実行で全緑／live per-effect 差分 **changed 30 / added 0 / removed 0**）
  - **バグの形**＝「[<対象節>、]対戦相手が《無》を支払わない／手札をN枚捨てないかぎり、X」は**相手が任意で回避コストを払える対話ゲート**（払えば X は起きない）だが、parser が `^対戦相手が《色》…を支払わないかぎり、` の**エナ型かつ文頭形**しか拾っておらず、対象節が1つ前置きされた瞬間にマッチが外れ、**下流の非アンカー規則が帰結 X だけを拾って回避クローズごと無言消費**していた＝**X が無条件実行される過剰実行**（例：`WX25-P1-038-E1` は「相手が《無》×3を払わなければバニッシュ」が**常にバニッシュ**）。**engine は完備**（`STUB{OPPONENT_PAY_OPTIONAL, costColors, opponentHandDiscard}`＋直後 `CONDITIONAL` の look-ahead ペア）なので**新語彙0本**・parser 1本で閉じた。
  - **効いた設計**＝帰結を対象節から切り離すと「それ」の照応先を失うため、**対象節を帰結の前へ戻して丸ごと再パース**する（「対戦相手のシグニ1体を対象とし、」＋「それをバニッシュする」→ 既存の対象パース機構がそのまま効く）。回避コストは4形（エナのみ／手札Nのみ／「手札N捨てるか《色》支払わ」／「《色》支払うか手札N捨て」）を1関数で読む。
  - **⚠投入時の実測で見つけて塞いだ退化2種（本バッチの肝）**＝①**引用「」の内側へのホイスト 4効果**（`WX24-P1-071`／`WX24-P2-073`／`WX25-P3-091`／`WXDi-P15-083`）＝クローズは**付与される能力側**にあるのに STUB が引用の外＝**付与そのもの**へ載り「相手が捨てれば付与も起きない」別物になっていた。引用内マッチを除外したら、`expandGrantEffectRawTexts` の sub 効果再パース経路で**正しく内側に着地**した（`-sub-E1` 群）。②**分割による owner 反転 1効果**（`WXDi-P05-TK01A-E1`「このシグニゾーンにあるシグニ」が `opponent`→`self`）＝**純加算チェック**を新設（本規則を止めた1回パスで「クローズを外す前の読み」を計算し、分割後と完全一致しなければ据置）。回避クローズは X をゲートするだけで X 自体を変えないはずなので、差が出たら分割が解決を狂わせた合図。
  - **採用手順を守った**＝parser 変更は held に落ちるので、`docs/_held_fresh.json` を **26カード27効果すべて per-effect で原文照合**してから `heldReview.mjs --adopt` で一括採用。live 差分は採用リストと完全一致＝**巻き込み0**。加えて **MANUAL 3効果は curated JSON へ直接配線**（parser が届かない＝`PRESERVE_STATUSES` 温存のため。`WX24-P2-061-E1`／`WX24-P2-070-E1`／`WX25-CP1-027-E1`）。`build:effects` 冪等を確認。
  - **残（honest defer・計30効果／内訳は BUGFIXES 冒頭節）**＝最大の弾は**「対戦相手は」形＝主語分配 13効果**（帰結の実行主体が相手自身のため単文パースで owner が反転する。`SPDi43-01-E1`／`WXDi-P01-028-E1/E2`／`WXK05-001-E1` ほか）。以下、回避コストがエナ・手札以外3／対象節以外の前置き6／「そのシグニ」照応2／純加算チェック不一致1／複文2。
  - **次の一手**＝**Opus：①🆕(lxi) 第2波＝「対戦相手は」形13効果**（帰結の owner を相手側へ固定する機構が要る。engine は完備のまま）**②`WX25-P1-056-E1`**（`leaveReplaceBanish` の no-op＝共通離場フック `applyEffectLeaveLrigAbilitySubstitute` で閉じられる見込み。任意選択とバニッシュ先処理の追加が要る）**③「コストの合計」族の STUB/UNKNOWN 14件④`WX11-043-E1` の `crossOnly` 欠落⑤タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑥`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑦§6.4 の配置数制限すり抜け7効果⑧§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・(lx) 2件・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**今回のバッチが持ち込んだUI5件**（相手側3択CHOOSE のエナ不足／手札不足による選択不可・併記型の両選択肢同時表示・ライフバースト経路で相手へ CHOOSE が飛ぶこと・入れ子 SEQUENCE の continuation 残存）／既存＝§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(lxi) 本消化後 最新値（本行を直近の正とする）**：census **1384→1375**（`BASELINE_HIGH=1375`）、golden **1113→1120**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430・効果実行 7999手／distinct 2702種）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **290→288枚/107→106署名**（前回 +2 だった `WX24-P1-071`／`WX25-P1-005` は本バッチで採用され held から抜けた）、manual field loss 0。live per-effect 差分 **changed 30 / added 0 / removed 0**（parser 経由27＋MANUAL 直配線3）、`build:effects` 冪等。**新語彙は0本**＝既存 `STUB{OPPONENT_PAY_OPTIONAL}`＋`CONDITIONAL(IS_MY_TURN)` の look-ahead ペアを parser 側で一般化しただけ。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1120**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1375**（回帰ゲート）・held **288枚/106署名**（2026-07-30 タスク12(lxi) 本消化後に実測）。
>
