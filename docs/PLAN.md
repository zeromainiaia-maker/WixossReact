...
- **🆕 2026-08-15 続き487（§6.4 O-3 の2クラスタ解体）後 最新値（本行が直近の正）**：census **831 据置**（⚠一度 832 まで増えたが計器較正2件で戻した＝`DEPLOY_BAN`／`namesFromTargets` キー追加）、**golden 2001**（+4＝executor 1・funnel 1・寿命1・live 形1）、smoke **10688 / SKIP 0**、fuzz 全0、held **112枚 / 48群**（うち採用済み5枚はレビュー対象外）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **27種/39件 → 30種/35件**＝受け皿2種が消えて固有4種が出た）。🆕**live JSON changed 8カード**（`WXK10-019`／`WX25-P3-001`／`WX25-P3-009`／`WX25-CP1-016`／`WXEX2-19`／`WXK10-004`／`WDK17-008`／`PR-K046`。CSV 非改変）。🆕**挙動是正 7効果**（恒久 no-op 5＋所有者/期間の取り違え1＋永続化バグ1）。🆕**`signi_deploy_power_limit` 廃止**（`signi_deploy_bans` へ統合）。
