...
- 🏁**セッション（2026-08-27・続き678・Opus 5 単独）＝分母を `CardData_Sheet1.csv`（974枚）へ付け替えた最初のバッチ（Sheet1 B1）。対象名詞句の修飾語が対象フィルタへ配線されておらず過剰効果になっていた9効果**を是正。**新設した型は0**（4語彙すべて engine も型も実装済みの純粋な配線ギャップ）。**段2 台帳 OPEN 649→643**（段2 消化 463→469・段0/段1 は不変）。
...
**▶ 次の一手**＝**Sheet1 の次バッチ**（Sheet1 の残 OPEN **63件 / 51カード**。⚠**着手前に `node scripts/archive/semanticAuditLedger.mjs` で数え直し、`CardData_Sheet1.csv` の CardNum で絞る**）。🆕**分母を Sheet1 に固定して回す**＝この節の「Sheet1 要対応 92枚」を減らしていくのが進捗指標。⚠**まだ Sheet1 スコープの計器が無い**（`scripts/cardProgressCensus.mjs` に `--sheet` を足すのが次の小仕事）。機構側なら 🆕**`O-98`**（GRANT 系入口の【キーワード】／レゾナ・3枚）／**`O-97`**（複数【使用条件】の前置で本文ゲートがホイストされない）／**`O-95`**／**`O-96`**／**`O-93`**（manual shadow の掃除）／**`O-94`**。実機の腐り返済なら §5.1 の `V-89`。
...
- **2026-08-27 続き678 後（本行が直近の正）**：
  ...
  **どのフラグも立たないカード 4971 / 5975（83.2%）**（`npm run census:cards`）、
  ...
  **全計器フラグの和集合 92枚 → 87枚**（意味照合OPEN 51／census高シグナル 38／held 6／partial 0／idset 13。⚠**「懸念ゼロ 776枚」は「正しい」ではない**＝計器が見ていないだけ）（⚠**Sheet1 スコープの計器はまだ無い**＝`cardProgressCensus.mjs --sheet` が次の小仕事）
