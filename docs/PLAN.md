...
- **🆕 セッション（2026-08-10・続き416〜417・Opus 5）＝§6.4「任意性脱落」を系統消化。`MANDATORY_SUSPICIOUS` 35→2・census 877→860・golden 1750→1753**。
  - **⭐① 母集団は検出器の数字より遥かに大きかった**＝`MANDATORY_SUSPICIOUS` は「**カードの全効果が mandatory かつ辞退不能**」のときしか鳴らないので、35件は氷山の一角。実際に直したのは **150効果超**（第1バッチの手札捨てだけで120箇所）。**検出器の件数＝母集団ではない**。
  - **🔴② `optional` の受け皿は型ごとに違う＝直す前に engine で実測する**。`TRASH{HAND_CARD/SIGNI}{optional}` と `TAKE_FROM_UNDER_SIGNI{upToCount}` は `resumeSelectTarget` の `stripDidItConditional` が0体選択で「そうした場合」ゲートごと落とすので**既に正しい**。`DRAW`／`DOWN`／`ATTACH_CHARM`／`TRASH{DECK_CARD}` は `optional` を**持たない・見ない**ので**強制**。同じ「〜てもよい」でも直す/直さないが型で割れる（第1バッチで一度誤って「optional:true も壊れている」と書き、実測で訂正した）。
  - **✅③ 第1バッチ（続き416）＝手札捨てコスト**＝素の `TRASH{HAND_CARD,self}` を正準形 `STUB{OPTIONAL_COST,handDiscard}` へ（`applyOptionalHandDiscardCost`）。**107カード採用＋live/`manualEffects.ts` 直編集9件**。⭐**支払い枝は `optionalCostPaySteps` が同じ `TRASH{asCost}` を再生成するので変換前と厳密に等価**＝増えるのはスキップ枝だけ。**120箇所のうち本当に強制だったのは46箇所（45カード）**、残74は表現統一＋支払い可能性チェック＋UI明示。
  - **✅④ 第2バッチ（続き417）＝残クラスタを機構ごと**＝(1)**効果まるごと任意 → `STUB{OPTIONAL_ACTIVATE}` 前置**（17カード・型ホワイトリスト制）(2)**自己トラッシュコストの脱落 → `OPTIONAL_TRASH_SELF` 挿入**（11カード＝engine は実装済みで **parser が一度も生成していなかった**）(3)**任意デッキミル → `MILL{optional}` 化**（6カード）(4)**新設 `OptionalCostSpec.fieldDown`**（4カード＝「アップ状態の＜X＞のシグニN体をダウンし《色》」。従来は DOWN が強制で《色》は脱落）(5)**`underAnySigniTrash.fromThis` 追加**（4カード＝N≧2 の部分払いの穴）。
  - **🔴⑤ 副産物＝付与能力が丸ごと消える構造バグを1つ潰した**＝`parseBlock` の `GRANT_LRIG_ABILITY` 展開が **`resolvedAction.type === …` というトップレベル型でしか判定していなかった**ため、任意コストで `SEQUENCE`/`CONDITIONAL` に包んだ途端に `abilities` が**空**になった（`WX14-042`／`PR-319`）。判定を「木の中に居るか」へ（`grantLrigAbilityNodes`）。**「包んだら別の post-pass が効かなくなる」型の死角**。
  - **⚠⑥ 採用は「署名グループ一括」ではなく正規化比較で1枚ずつ検証した**＝`--adopt-sig` なら `WX25-P3-003` の `exceed:3`（手修正）を巻き添えで失っていた。**live⇄fresh を「今回の変換だけ」で正規化して差分0を機械確認**してから `--adopt`。混在カードは `docs/_partial_fresh.json` から効果単位で採る（`heldReview` は扱わない）。
  - **⚠⑦ 使用時コストには触ってはいけない**＝「この{スペル|アーツ}を使用する際、…てもよい」は `useTimeCost.ts` の領分。触ると「以下のNつから選ぶ」連結条件（**素の TRASH** を要求）が外れて **CHOOSE 本体が丸ごと落ちる**（`WD21-008` で実測）。parser にガード済み。
  - **✅⑧ 計器の誤検出2件も消化**＝`_checkAllEffects.mjs` の `actionIsSkippable` が `GRANT_*_ABILITY.abilities[]`／`GRANT_EFFECT.effect` の**内側へ再帰していなかった**（`WX17-077`／`WX21-052`）。子ノードに `abilities`／`effect`／`thenAction`／`burstAction` を追加。**逆翻訳側**も `OPTIONAL_COST` が `handDiscard`/`fieldDown`/`underAnySigniTrash` を見ておらず「コストを支払ってもよい」に潰れていたのを原文語彙へ（既存 MANUAL にも効く）。`MILL{optional}` の「置いてもよい」も追加。
  - **✅⑨ ゲート**＝`npm run gates` 全緑（golden **1753（+3）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **860**（877→860・`BASELINE_HIGH` 更新済み）/ 同型★ **0** / lint 0 errors）。`npm run build:effects`＋`npm run regen` 実施済み。**⚠実機UI未検証**＝任意コストの pay/skip モーダルが新たに出るカードが多数（§7 送り）。
  - **📋⑩ 次の一手**＝**Opus 側**＝(a)**§6.4 残2件のうち `WX12-010`＝強制アタック機構**（「対戦相手のシグニは可能ならばアタックしなければならない」＝engine に受け皿なし）(b)**§6.4 UNKNOWN の残38ノード**（次に安いのは「代わりにそれをトラッシュに置く」2件＝`WX06-024-BURST`/`WXK08-030-E1`）(c)**§6.4 A群の残7件**（着手可能2件＝`ASSIST_LRIG_ATTACK_THIS_TURN`／`UNDER_CARD_AS_ENERGY_COST`）(d)**置換5本の対話化**（`WXDi-CP01-023` もこれ待ち）。**Sonnet 側**＝§7 実機検証（**今回ぶんの pay/skip モーダルを最優先**）／`/census-batch`。**新規登録バグ**＝`WXDi-P05-037-E1` は原文「**対戦相手は**手札を２枚捨ててもよい」なのに `owner:self`（§3 Opusタスク12 へ登録）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き416〜417（§6.4 任意性脱落の系統消化）後 最新値（本行が直近の正）**：census **860**（877→860・`BASELINE_HIGH` 更新済み）、golden **1753（+3）**、`MANDATORY_SUSPICIOUS` **2**（35→2・`node scripts/_checkAllEffects.mjs`）、held **115カード／49群**、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **38ノード/36カード 据置**。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
