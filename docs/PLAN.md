...
| 1 | **§7 実機検証の横展開** | 検証（driver シナリオ追加のみ） | S×件数 | **✅(a)(b)(c)は続き141（Sonnet）で消化完了**：(a)`trashCounterOpp`（タスク12(iv)修正の反転＝PASS・既定orderに追加）(b)ON_LRIG_GROW④のusageLimit＝`lrigGrowUsageLimit`（タスク12(vi-5)修正の反転＝旧FAILの真因はdriver側のtestId誤りと判明・修正してPASS・既定orderに追加）(c)R37③ ON_SIGNI_POWER_ZERO_OR_LESSのusageLimit＝専用シナリオ`powerzeroUsageLimit`を新規作成しPASS・既定orderに追加。既定order 71→74件。**🆕在庫復活（続き162・Opus で (xxi) 修正）＝`oppDrawOwnEffectOnly` シナリオが PASS 反転可能に**＝ブラウザ harness で確認し PASS なら既定 order へ追加（Opus12(xxi) の反転検証＝Sonnet の主力在庫が復活）。**残ブロック**＝(xiii)/(xix)/(xx) は続き136/137 で既に解消済み（要 §7 反転確認）。WX22-001-E3（クラフトトークン残・§6.4）も引き続き可 |
...
- **🆕 セッション（2026-07-16・続き168・Codex CLI〔gpt-5.6-sol・reasoning high〕が実装／Claude〔Opus 4.8〕が検証・PLAN §3 Opusタスク12(xxvii) の Cluster A/C 据置分から "Tier 1"＝engine に受け皿がある機構待ちを消化）**
  - **ワークフロー実験の4巡目・初の engine 小改修許可**。Cluster A/C で「機構待ち据置」した中から、engine に状態が既にある/小追加で済む4件を Claude が事前選別して投げ、**Codex が4件すべて採用**。
  - **4枚採用・census 2048→2045・golden 369→373・同型★0・gates 全緑（smoke/fuzz 込み）**。
  - **採用①PR-K073-E1（parser）**＝発動条件「レベル2,3,4のシグニがある場合」を `AND[HAS_CARD_IN_FIELD level2/3/4]` で CHOOSE 全体にゲート（従来は条件欠落で常時発動）。**併せて選択肢②の filter 反転バグを是正**＝「《花畑チャイカ》**以外**」が `cardName`（包含＝原文と逆）だったのを `excludeCardName` へ（parser の `《X》以外` 認識を追加）。
  - **採用②WXDi-P03-001-E1（parser・使用条件配線）**＝【使用条件】「あなたの場に青のルリグがいる」を `condition:LRIG_COLOR{青}` として付与。**ACTIVATED の `eff.condition` が BattleScreen のプレイ可否ゲートで `evalUseCondition` 評価される経路を確認**（5036/5052/5065/6045 行）＝実際に使用ゲートとして効く。遅延手札捨ては UNKNOWN のまま据置。
  - **採用③WDK13-008-E1（parser＋engine 小改修）**＝選択肢②に「対戦相手の場にキーがある場合」条件（`HAS_CARD_IN_FIELD{owner:opponent, filter:{cardType:キー}}`）を付与。**engine `HAS_CARD_IN_FIELD`（execUtils:782）に key_piece ゾーン走査を5行追加**（従来は signi+lrig のみ）。「レベル合計12」制約は §5c 合計制約死角のまま残置（キー条件は strictly more correct）。
  - **採用④WXEX1-35-E1（parser）**＝継続条件「《ライズアイコン》を持つシグニが3体あるかぎり」を `activeCondition:HAS_CARD_IN_FIELD{filter:{hasRiseIcon:true}, minCount:3}` で付与（既存 `hasRiseIcon` filter・engine 不要）。内側 GRANT_PROTECTION の count:ALL は机能同值で不変。
  - **Claude 検証（engine 変更ゆえ重点）**＝①全数機械 diff＝変更は対象4効果のみ（`excludeCardName` の parser 変更が他カードへ波及していないことを確認＝excludeCardName 保持は WX01-037〔既存〕と PR-K073 の2枚だけ）②**engine の key_piece 走査の副作用を全数照合**＝全 382 個の HAS_CARD_IN_FIELD 条件のうちキーが誤マッチし得る緩い filter（cardType 非シグニ/ルリグ）は97件だが、全て cardName（キーは持たない）or cardType:レゾナ（キーは Type:キー で不一致）で**キー誤マッチはゼロ**（WDK13-008 自身を除く）③PR-K073 の HEAD 比較＝`cardName:花畑チャイカ`→`excludeCardName` の真の是正を確認 ④census 2045・golden 373・同型★0・`npm run gates`（smoke/fuzz CRASH/INVARIANT 全0）を独立再実行。**退化ゼロ**。
  - **⚠据置の訂正**＝WX09-Re04「対戦相手のターンの場合」は当初 Tier 1 と分類したが誤り。`execUtils:1006` で **`IS_MY_TURN`/`IS_OPPONENT_TURN` は executor がオーナー視点固定で実行時判定不能ゆえ `return true`（no-op）**＝ターン所有条件は engine では強制できず、条件を足しても飾りになる。**アーキ制約でアクセ不能**として据置（新機構タスク扱い）。
  - **次の一手**：Opus＝(xxiii) リコレクト分割（§6.3級 GRANT 機構）・(xxvii) 残 Cluster B（duration・非 POWER_MODIFY 型）/D（timing 取り違え＝タスク16/17 同型で単点是正しやすい）/E（機構欠落）・タスク1残（【常】アタックできない家族）・IS_MY_TURN化残104。**残 Tier 2/3 機構タスク**＝合計レベル制約（WXEX1-45/WDK13-008）・複数レベル探索（WXK06-031-E1）・相手手札選択（WXEX1-50-E2）・遅延効果（WXDi-P03-001 手札捨て）・イベント来歴（WD22-037「効果で場に出た」）・ターン所有条件のアーキ改修（WX09-Re04）。Sonnet＝`oppDrawOwnEffectOnly` PASS 反転確認（続き162 在庫）＋§7フルバッチ回帰。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 373・smoke/fuzz 全0・同型★0・census 2045。
>
