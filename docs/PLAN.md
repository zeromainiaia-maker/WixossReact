...
- **🆕 セッション（2026-08-07・続き371・Opus 5〔**§5d パターンA 第3バッチ**＝「《カード名》以外の」〕）＝ユーザー指示「PLANをよみ作業の続きを行う」。触った層は `parserUtils.ts`（helper 新設）／`parseSentencePart1.ts`（7ビルダー）／`effectParser.ts`（1ビルダー）／`public/data/effects_*.json`（36効果）／`goldenTest.ts`／`vocabCensus.ts`（BASELINE）。**型・engine は無改造**。
  ...
  - **⚠反転13効果は parser では既に直っていて held に溜まっていた**＝`heldReview --adopt` で12枚採用、`WX09-CB02` だけは同居 E1 が MANUAL で**カード単位 PRESERVE**（held に載らない）ため `effectId` アンカーの**外科パッチ**。**census が拾うのは live JSON なので、「parser を直す」と「live へ届かせる」は別作業**。
  - **⚠据置＝【ビート】コスト3効果**（`WDK14-014-E2`／`WXK08-043-E1`／`WXK10-041-E3`）＝`cost.beat_signi` は count しか持たないが、**engine の `analyzeBeatSigniCost` が EffectText から除外名を読んで候補から外している**＝**JSON に語彙が無くても挙動は正しい**。golden で固定した。**census の残件＝バグとは限らない。**
  - **前バッチの教訓を適用**＝配線前に全CSV走査で「《X》以外の」**50ヒット／49効果を全数分類**し、filter にしてはいけない用法が `PR-204-E1`／`PR-238-E1`（条件節「〜以外のアーツを使用していない場合」）の2件だけであること、除外対象が常にカード名であることを確認してから配線した。
  - **ゲート**＝全緑。**golden 1411→1413**（+2）、**census 1184→1181**（−3・`BASELINE_HIGH` 更新。除外クラスタは高シグナル **8→4**）、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors、held **242→231**。A/B 差分 `added=0 removed=0 changed=24`（parser 分）＋採用13効果。
  - **次の一手**＝**§5d パターンA を続ける**。**⚠まず `matchesFilter`／型を grep して「実装済みだが呼ばれていない」を疑う**（3バッチ連続でこれ）。残クラスタ＝「同一性(〜と同じ色/レベル/名前)」21件・「否定フィルタ(〜ではない○○)」14件（レゾナではない＝`excludeResona`／＜C＞ではない＝`cardClassExclude` は**型にある**／**〈色〉除外の型は無い**）・「共通する色」23件。**§5d 末尾の照合済み12件**（owner取り違え・重度混線を含む）も未修正のまま。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-07 続き371（§5d パターンA 第3バッチ＝「《カード名》以外の」の `excludeCardName` 配線）後 最新値（本行が直近の正）**：golden **1411→1413**（+2＝36効果に載ること／反転が戻っていないこと／除外と部分一致の同居／engine の完全一致判定／beat コストの `analyzeBeatSigniCost`）、census **1184→1181**（−3・`BASELINE_HIGH` も 1181 へ更新＝**機能是正**。「除外(〜以外の)」クラスタの高シグナルは **8→4**）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **242→231／署名グループ 104件**（`heldReview` で12枚採用・実測）。**live JSON の変更は36効果／35カード**。**型・engine は無改造**。⚠**残す教訓**＝(a) **3バッチ連続で「語彙はあるが呼ばれていない」だった**＝§5d パターンA は新語彙を足す前に必ず `matchesFilter`／`TargetFilter` を grep する。(b) **パターンA の実害は「過剰」だけではない**＝除外名が `cardName`（部分一致）に入ると**原文と真逆（そのカードしか選べない）**になる＝13効果がこれだった。(c) **「parser を直す」と「live JSON へ届かせる」は別作業**＝反転13効果は parser では既に正しく、held に溜まっていただけ。カード単位 PRESERVE（同居効果が MANUAL）は held に載らないので外科パッチが要る（`WX09-CB02`）。(d) **census の残件＝バグとは限らない**＝【ビート】コスト3効果は `cost.beat_signi` に語彙を載せられないが engine が EffectText から除外名を読んでおり**据置で正しい**。
