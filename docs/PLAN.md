...
**直近＝2026-09-21＝§5.7 `S-22`＝CPU の対象選択が効果の意味を見るようにした（乱数で決めた対象選択 42 → 0）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第434バッチ）
- 🔴**真因**＝損得を **`thenAction` の型だけ**で見ていたが、**`SELECT_TARGET` の最大の塊は「対象宣言」**で
  **`thenAction` は `INTERNAL_NOOP`（印）だけ**＝**帰結は宣言の後ろのステップ**に来る ⇒ **型からは永久に読めず全部乱数**だった。
- 📏**母集団**（本物のデッキ6つ × 1戦）＝CPU が答えた `SELECT_TARGET` **66件のうち32件（48%）が乱数**（選ぶ余地も 22件）。
  内訳＝**対象宣言 17**／`TRANSFER_TO_HAND` 12／`ADD_TO_LIFE` 2。**`both_*` は0件。**
- 🔑**置き場が答えを持っている**＝**`opp_*`＝`harm`／`self_*`＝`benefit`／`both_*`＝乱数のまま**（`targetIntentFor`）。
  🆕**第3の向き `cost`**（`ADD_TO_LIFE{fromHand}`・`ENERGY_CHARGE`）＝**価値の低い順・任意でも選ぶ**
  （`harm` で代用すると「してもよい」が**断る**に化ける）。＋`POWER_MODIFY_PER_*` は `deltaPer*` の符号で読む。
- 🆕**恒久の計器**＝`census:play` の規則 `targetRandom`（**0 が正**）。📏**WD15 4戦で 42 → 0**／実機も 0。
- 📊**A/B（96戦）は6山とも差なし**（合算ライフ差 +0.19 [-0.16, 0.53]）＝**採否は直接指標**（`S-26` と同じ）。
  旧挙動は `legacy-targetrandom`。⚠**残した弱点**＝宣言の帰結にコストが乗る形／`POWER_SET`（実測では尋ねられない）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-24`**（自己対戦がマリガンを一度も踏まない）。次に `S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-24`・`S-14`・🆕`S-29`＝🏁`S-22` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝**自己対戦がマリガンを一度も踏まない**（`S-24`）。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4354**）｜**実機 PASS**（7ターン/164手/217s）｜反転確認＝`legacy-targetrandom` で旧挙動へ戻せる |
...
| `S-29` | 🆕**対象宣言の「帰結のコスト」を見ずに選んでいる**（§5.7 `S-22` の残り）＝「〈シグニ〉1体を対象とし、**それのレベル1つにつき**〈コスト〉を支払ってもよい。そうした場合〜」の族では、`S-22` が入れた「相手の一番強い札を選ぶ」が**払えずに空振る**ことがある（宣言の時点では後続ステップのコストを見ていない）。⚠**母集団は未計測**＝着手前に「宣言のあとにコストが来る効果」を数える（`STUB{SELECT_TARGET_ONLY}` ＋ `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` 族）。⚠**`POWER_SET`（live 230ノード）も `unknown` のまま**だが、**自己対戦の実測では1件も尋ねられていない**（大半が常在効果）＝**こちらは数えてから取る**。 | 小〜中 | 「選ばせる効果」が払えずに空振らなくなる。⚠**`S-16` の探索が宣言の先を読めるなら要らない**＝先に測ってから着手を決める |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🏁~~`S-22`（対象選択が効果の意味を見ていない）~~ → 🔥**`S-24`** → `S-14`（作戦データ）→ `S-29`（宣言の帰結のコスト＝母集団未計測）。
...
- **2026-09-21 時点**（第421〜434バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 4**（🔵`S-6`・`S-24`・`S-14`・🆕`S-29`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4354**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 7本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・**7/164手**）
