...
- **🆕 セッション（2026-08-07・続き369・Opus 5〔🏁🏁**§5c 文型バッチを店じまい**し **§5d「1効果ずつの原文照合」を新設**＋パターンA 第1バッチ〕）＝ユーザーの問い「すべてのカードが完璧に動作することを目標としている。テンプレ1効果も処理しなければいけないか？」に**実測で答えた**セッション。
  - **答え＝はい。むしろ単発テールが本命**。残り高シグナル 1199 のうち **874（約73%）が単発テンプレにしか現れない効果**で、そこから**シード固定で無作為12件**を抽出して原文照合したところ **判定できた11件すべてが実バグ**だった。一方、枚数順に消化してきた「2効果以上」の頭は **33/37 が計器の偽陽性**。
  - **⚠これは構造的**＝**大きいクラスタ＝systematic な文型＝parser が既に正しく扱っている**から偽陽性が濃く、**単発＝汎用規則から漏れた1件もの**だから実バグが濃い。**枚数順に取ると実バグの薄い側から掘ることになる。**
  - **さらに単発テールは「文型の問題ではない」**＝無作為12件の内訳は 対象フィルタ脱落6／owner取り違え・任意性喪失2／条件節・機構ゲート脱落2／重度の構造混線1。**効果はだいたい parse できていて修飾が1つ落ちている**ので、文型テンプレでまとめても作業単位にならない。→ **§5d を新設**（Opusタスク20・母集団 約874効果）。§5c は**回帰ゲートと索引**としてのみ維持。
  - **⚠完了判定の前提も置き直した**＝**census 0 は「すべてのカードが完璧」の十分条件でも必要条件でもない**（死角4項＋対応語彙漏れ）。**最終判定器は BEHAVIOR_AUDIT（§5a）と §7 実機検証**。
  - **パターンA 第1バッチ＝「能力を持たない〜シグニN体」11効果**。`TargetFilter.noAbilities` を新設し判定は既存 `hasNoAbility` へ委譲（**CSV は素のシグニを空文字ではなく `-` で持つ**・場では `abilities_removed` も「持たない」に数える）。parser は `parseNoAbilitiesFilter()` を新設して6ビルダーへ配線、decompiler にも描画を追加。
  - **⚠今回いちばんの学び＝同じ語でも filter にしてはいけない用法が3つある**。全CSV走査で「能力を持たない」38件を分類＝(a)名詞句修飾〔✅filter〕/(b)**付与形**「それらを能力を持たないシグニ**として**場に出す」〔❌出す際に能力を失わせるアクション側〕/(c)条件節「それが能力を持たない**場合**」/(d)リマインダー文。**(b) を filter にすると「トラッシュから選べるのが能力なしシグニだけ」＝原文と逆の過小実行**になる。helper は「として」不後続＋数量詞必須に限定し、golden で (b)5枚・(c)4枚が**載らないこと**も固定した。
  - **ゲート**＝全緑。**golden 1408→1410**（+2）、**census 1199→1189**（−10・`BASELINE_HIGH` 更新）、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors、held 245→242。採用は自動9枚＋フィールド粒度の手採用2枚。
  - **次の一手**＝**§5d パターンA を続ける**（同系統の残り＝「否定フィルタ(〜ではない○○)」19件・「除外(〜以外の)」8件・「同一性(〜と同じ色/レベル/名前)」21件が census 明細にクラスタとして立っている）。§5d 末尾の**照合済み12件**（`WXDi-P11-044-E2` ほか）は未修正のまま入口として置いてある。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-07 続き369（🏁🏁§5c 文型バッチ店じまい＋§5d 新設・パターンA 第1バッチ「能力を持たない」）後 最新値（本行が直近の正）**：golden **1408→1410**（+2＝11効果に載ること／付与形5枚・条件節4枚に**載らない**こと／engine 判定の3方向）、census **1199→1189**（−10・`BASELINE_HIGH` も 1189 へ更新＝**機能是正**）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **245→242**。**live JSON の変更は11カード**。新語彙＝`TargetFilter.noAbilities`。⚠**残す教訓**＝(a) **枚数順は実バグの薄い側から掘る順序**＝大クラスタ＝systematic＝parser が既に正しい（偽陽性 33/37）／単発＝汎用規則から漏れた1件もの（実バグ 11/11）。**母集団の性質でクラスタサイズと実バグ率が逆相関する。**(b) **同じ日本語でも用法で filter にしてよいかが変わる**＝「能力を持たない」は名詞句修飾だけが filter で、付与形「〜として場に出す」を誤爆すると**原文と逆の過小実行**になる。**新しいフィルタ語彙を足すときは全CSV走査して用法を数える**（今回38件→4用法）。(c) **census 0 は「完璧」の十分条件でも必要条件でもない**＝完了判定は BEHAVIOR_AUDIT と実機検証で出す。
