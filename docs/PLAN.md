...
| 🆕(xliv) | **`BANISH_REDIRECT` の残テール（続き217→218b→230 で消化）**。族36効果を全数棚卸し。**✅消化＝(1)「パワーが０以下の」限定脱落2件**（`whenPowerZero` 新設・続き218b）**(2) owner 誤り5件**（続き218b）**(3) 属性フィルタ5件＝✅続き230（Opus）**＝parser で target.filter へ復元（level/isFrozen/infected/hasCharm）＋engine の **battle/power0 経路**（`banishRedirectAppliesFrom` に被バニッシュ属性）で評価。WXK10-053/WXDi-P12-073/WX21-005/WX18-038＋WX19-078（over-fire 縮小）。golden 533。詳細 BUGFIXES 続き230。**残＝⛔§6.3 級**（engine が effect 経路の `banish_redirect` を真偽フラグで持ち `ExecCtx` に effectsMap も無い）：**(a2) 効果経路（`banishDestination`）の 【常】 走査**＝effectsMap を ExecCtx へ通す＋16呼び出し点のシグネチャ変更（今は turn-flag 依存で 【常】 redirect を拾わない既存 under-fire ギャップ・今回変更で悪化せず）**(b) 単体対象4件**（対象選択フローが無い）**(c) 正面限定3件**（target 側ゾーンスコープ機構）。近似すると偽陰性になるため据置。 |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 532・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1841。
>
