...
**Opusタスク12＝未消化の在庫 3件（2026-08-10 続き404・408・417 で登録）**。⚠**2026-08-14 続き475 で (cxxv) を取り下げ（シナリオ偽陽性）**、**続き475b で (cxxvi)(cxxx)**、**続き475c で (cxxix)**、**続き475d で (cxxvii)(cxxviii)** を残0クローズ＝8件→3件。
...
| 🆕(cxxiii) | 🔴**ピースのコストが二重に請求される疑い＋効果が使用時に解決しない**＝`WXDi-P14-002`（CONNECTスピニング・Type=ピース）は CSV `Cost=《赤》×１《無》×２`、live の `E1` も `effectType:'ACTIVATED'`／`timing:['MAIN']`／`cost.energy=[赤1,無2]` と**同じコストを2箇所に持つ**。実機では `KeyUseModal` で印刷 Cost を徴収して「セット」した時点で `executeKeyPiece`（`BattleScreen.tsx:6613`）が `queueCardEffects(..., ['AUTO'], ['ON_PLAY'], ...)` **しか積まない**ため**効果は一切発火せず**（実測 `pEff=-`）、効果を出すには KEY スロットの【起】（`getKeyPieceActions`→`executeKeyActivated`）を別途起動して `cost.energy` を**もう一度**払う必要がある。ルール上ピースは「使用＝コストを1回払って即解決」なので、**セット時徴収＋起動時徴収の二重取り**か、少なくとも「使っただけでは解決しない」過少実行のどちらかに該当する | 続き436（PLAN §7 実機検証バッチ2）＝`connectSpinningChoice4Pay` を実行し、エナ選択・セットまでは完走するのに効果が出ないところで停止したことから実測で特定 | 🆕**2026-08-14 続き475f で母集団を実測＝設計は一意に決まる**。**Type='ピース' は 119枚**（キーは80枚）。うち**118枚が `ACTIVATED` かつ `cost.energy` が印刷 Cost と同額**（＝live JSON の形は正しく、**壊れているのは実行経路のほう**）。現状の `executeKeyPiece`（`BattleScreen.tsx:6556-`）は キー と共通で ①ルリグデッキから抜き ②**印刷 Cost を徴収**し ③**`field.key_piece` へ置き** ④`queueCardEffects(..., ['AUTO'], ['ON_PLAY'])` **しか積まない**。⇒ ピースは**効果が一切走らず**、KEY スロットから【起】を起動して**同額をもう一度**払う羽目になる（＝二重請求）。🔑**あるべき姿＝「使用」＝印刷コストを1回払って ACTIVATED 効果を即解決し、ルリグトラッシュへ置く**（キーゾーンは占有しない）。裏づけ＝**`KEY` を対象に取る非キーカードは live に0件**（＝誰もピースの居場所を見ていない）／原文も「（このピースの**後に**場に出たシグニは影響を受けない）」等の**一回解決**表現。🔴**副作用として現状は2つの過少実行も同居**＝(a) 入口ゲートが `!my.field.key_piece` なので**キーを1枚出していると全ピースが使えない** (b) `canUse` が MAIN/GROW しか許さないので **CSV Timing が「アタックフェイズ」の14枚が永久に使えない**（内訳＝メインのみ104／メイン+アタック11／アタックのみ3／メイン+スペルカットイン1）。⚠**唯一の例外＝`WXDi-P15-003`**（`AUTO/ON_PLAY` ＋ **`CONTINUOUS`**）＝いまは key スロットに居るから CONTINUOUS が読まれている。ルリグトラッシュへ送ると**「このゲームの間、あなたのセンタールリグは…を得る」が失効する**ので、**この1枚だけは解決時に付与ストアへ載せ替える**（か、CONTINUOUS を持つピースは場に残す）判断が要る。⇒ **設計裁定はこの1点だけ**。⚠`WXDi-P13-003A`（ピースがルリグへ変わる `MAYU_ENCOUNTER_A`）は既に特例分岐があるので触らない。⚠**修正は `card.Type === 'ピース'` で厳密にゲートする**＝共通経路だが分岐すればキーは1行も変わらない
...
- **🆕 セッション（2026-08-14・続き475〜475e・Opus 5）＝**§7 実機検証の続き＋そこで見つけた engine バグの一掃**＝**V-01／V-02／V-09④／V-10 が全シナリオ緑（10本）**。**§3 在庫は 8件→3件**＝(cxxv) を「シナリオ偽陽性」として取り下げ、**(cxxvi)(cxxx)(cxxix)(cxxvii)(cxxviii) を残0クローズ**。ゲート全緑（**golden 1964→1973**＝トリップワイヤ9本追加・census 831 据置・smoke 10688／SKIP 0・fuzz 全0・同型★0・held 106 据置・lint 0 errors/259 warnings）。**live JSON は 19枚更新**。**
  ...
  - **🏁③ 実バグ5件を真因まで確定→全件クローズ**＝**(cxxx)【新規】離場置換の決定が消費されず残留**（11経路が `sub.ctx` を捨てていた）／**(cxxvi)** 相互身代わりの instance 複製（**もう場に居ない num を離場ループが再処理**）／**(cxxix)** `lifeCrash` の身代わりが**末尾の `trashStackSpell` へフォールスルーして0枚コストで成立**／**(cxxvii)** 「このアタックを無効にする」が効かない＝**真因は2つ**（parser の `NEGATE_ATTACK{owner:'opponent'}`＋🔴**`resumeOpponentPayOptional` の `pay` 枝が `payOpt.action` を実行せず `thenOnPay` の帰結がエナ払いのときだけ落ちる**）／**(cxxviii)** `WXDi-P04-051-E1` の恒久 no-op＝**「あなたのルリグがアタックしたとき」を拾う収集経路が engine に無かった**（`collectAllyLrigAttackTriggers` を新設してから timing/対象を是正・**live 17枚を機械照合のうえ採用**）。
  ...
  - **▶ 次の一手**＝**Opus 側**：🆕**(cxxiii) は続き475f で母集団を実測し、設計裁定が「1点」まで縮んだ**（ピース119枚中118枚が `ACTIVATED`＋印刷 Cost 同額＝**live の形は正しく実行経路が壊れている**。あるべき姿＝**使用＝1回払って即解決→ルリグトラッシュ**。**残る判断は `WXDi-P15-003` の CONTINUOUS 1枚だけ**）。⇒ **裁定が出れば実装は `card.Type === 'ピース'` で厳密にゲートして入る**（キーは1行も変わらない）。次点＝§6.4 O-20 の残21件・O-19。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き475〜475e（§7 V-01・V-02・V-09④・V-10 決着＋§3 実バグ5件クローズ）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、**golden 1973**（**1964→+9**）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（**採用18枚ぶんは相殺されて据置**）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。🆕**live JSON changed 19枚**（(cxxvii) 2枚＋(cxxviii) 17枚。うち `WXDi-P03-035` は続き475e で triggerFilter も追加）。🆕**実機シナリオ＝既定 order の総数は据置**（新規追加0・**判定/経路を直したのが9本**・**id を1本 rename**）。**緑/赤の内訳が 45緑12赤 → 57緑0赤**。🆕**Opusタスク12 の在庫＝3件**。
