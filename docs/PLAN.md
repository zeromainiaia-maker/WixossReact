...
**▶ 次の一手**＝**§5.2 の段2 消化を続ける**（残 OPEN **678**）。⚠**着手前に必ず `node scripts/archive/semanticAuditLedger.mjs` で数え直す。**
🏁**「条件節が丸ごと落ちる」根の3バッチ（第44 場／第45 トラッシュ・エナ／第46 種類数）は完了**＝この軸の残りは**新機構が要るものだけ**（下の `O-nn` 候補）。**次は別の軸を取る。**
- 🔑**次の候補は `O-63`（turnOwner 167効果）**＝母集団が大きく、engine 側は `collectTurnTriggers` に**ゲート1本**を足すだけで済む見込み。⚠**着手前に「両ターンで発火して実害が出る効果」だけに絞る**（対称な効果は実害が無い＝§4 の 3-4‴ の引き算）。
- 📌**第44〜46 で新たに分かった「機構が要る」もの**（§5.3 へ登録するか、まとめて1バッチにするか要判断）：**①複数ゾーンの合算カウント**（`WXDi-P12-056-E1`＝エナ＋トラッシュの枚数合算／`WXDi-CP01-031-E1`＝場＋エナの distinct name 合算） **②場の【ソウル】存在条件**（`WXDi-P04-034-E1`） **③CONTINUOUS の「2閾値2帰結」**（`WXEX1-40-E1`＝10種類でシグニ耐性・15種類でルリグ耐性を別々に得る） **④ゲーム中持続の毎メインフェイズ選択機構**（`WXDi-P11-003-E1` / `WXDi-P15-003-E1`）。
- 🔑**次に条件系のバッチを組むときの必読3点**（第44〜46 の実測）：
  1. **母集団は「原文が該当する数」ではなく「既存機構で正しく実装済みの群を全部外した残り」**（第44＝587→真の穴36、第45＝40→13、第46＝13）。
  2. ⚠**「受け皿となる条件型が無い＝穴」ではない**＝**RECV 外の専用実行器（STUB のハンドラ）が原文の閾値を直接読んでいる**ことがある（第45で7件・第46で1件）。そこへ一般条件を重ねると**コスト軽減や追加処理の条件を本文全体のゲートへ格上げして意味を変える**。
  3. 🔴**評価器の分岐を足すときは「その分岐に落ちる既存 live が何件あるか」を先に数える**＝`default: return true` から実評価へ変わると**過剰実行が過小実行へ裏返りうる**。
- 🔑**実機シナリオの作り方（第44〜46 で確立）**＝**観測点は「JSON assert では見えない場所」を選ぶ**（CONTINUOUS の条件は `calcFieldPowers` → `checkActiveCondition` を通って**盤面のパワー表示にしか出ない**）。**spec は1点だけ違う対で組む**（負方向テストが「常に不成立」で満点になる罠を構造で排除）。**そのうえで修正前のコードへ戻して1本ずつ FAIL するか確かめる**（第46 では3本のうち1本しか噛んでいなかった）。⚠**UI はパワーを3桁区切りで描画する（`7,000`）＝カンマを剥がしてから数値を見る。**
...
- **2026-08-25 続き651c 後（本行が直近の正）**：
  **census 576/576**、**golden 2756**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 260）**、
  `census:stubs` **A群🔴0／C群0**、manual-fields **0**、**同型★ 0**、`census:goldentypes` **147/147・未カバー0**、
  **live カード 5975 / 効果総数 10693**、`_held_fresh` **77**／`_partial_fresh` **14**／`_idset_fresh` **45**、
  **意味照合 残 OPEN 678**（段2消化 434・**第44〜第46 の3バッチで 694→678 の −16**／段0 221 は不動・段1偽陽性 112→111）、
  **実機シナリオ +8**（第44＝`b44VirusControlPlain`/`b44VirusDeployBlocked`/`b44VirusDeployAllowed`、第45＝`b45TrashLv1Buff`/`b45TrashLv1NoBuff`、第46＝`b46TrashKinds7`/`b46TrashKinds6`/`b46TrashSameName7`）、version **0.502**。（⚠`census:wiring`／`census:timing` は本バッチ未再計測）
