...
- 🏁**セッション（2026-08-27・続き685・Opus 5 単独＋Codex 委譲）＝Sheet1 B7＝「〈条件〉場合、**代わりに**」があると
  照応補正が**自ら降り**、置換側の帰結が `targetsTriggerSource`（＝**自分のアタックしているシグニ**）へ落ちていた。
  live 変化 6 effectId。台帳 OPEN 631→630。**census 536 据置・Sheet1 66 据置**。**
  🔴実機で観測した旧挙動＝**候補に自分のシグニまで出て**、**条件を満たしていないのに置換値 -3000 が走り**、基本値は消えていた。
  - 🔑**真因は「規則が無い」ではなく、既存の照応補正3本が `if (/代わりに/.test(text)) return action;` で降りていた**
    （`effectParser.ts:7681 / 7707 / 7775`）。**見送り契約の理由は「平坦化された2ノードの片方だけ直すと誤 owner が残る」**
    ＝**2ノードを名指しで同時に直せば理由は解消する**。⇒ `foldKawariSubstitution` に**この文型限定**の補正を追加し、
    **3つのガードは1本も外していない**（外すと同じ regex に当たる131効果へ一気に効く）。
  - 🔴**実害は3重**＝①帰結が自分のシグニに当たる ②基本値と置換値が**両方走る** ③置換側が任意コストのゲートの外＝
    **コストを払わなくても撃てる**（踏み倒し）。
  - 🔴**母集団は 138 → 7**（原文で「対象宣言1つ＋照応＋代わりに」138 → 宣言が相手シグニ・選択肢形を除く →
    帰結に `owner:self` か `targetsTriggerSource` が残るもの **7効果**）。**残り131は既に正しいか別軸**＝
    正準形①（`SELECT_TARGET_ONLY→STORE→コスト→gate{targetsStored}`）／正準形②（**差分加算**）／「代わりにNつまで選ぶ」。
    **指示書に「触るな」と明記して実際に差分0。**
  - 🔴**`WXDi-P16-056-E1` は丸ごと据置**（§5.3 `O-105`）＝「場の＜解放派＞シグニの**下**のカード合計4枚以上」の受け皿が無い。
    **target だけ直すと過小（自分に当たる）から過剰（相手が両方食らう）へ裏返る**ので**全部直すか丸ごと据置かの二択**にした。
    非採用契約は golden で固定。
  - ✅**`NO_COMMON_COLOR_AMONG_FIELD_SIGNI` の `count` を省略可にした**（省略＝場の全シグニ・0体は fail-closed）＝
    **型（両 union）＋両評価器＋逆翻訳＋golden の4点**を揃えた。原文が体数を書かないのに `count:3` を当てると**2体盤面で過小**になる。
  - ✅ gates 全緑（golden **2880→2885**（+5テスト）／census **536 据置**／smoke・fuzz 全0／lint 0 errors／同型★0／held 74→73）。
    **live 変化は 6 effectId ちょうど**（新規0・消滅0・`parseStatus` 遷移0）。
  - ✅**実機シナリオ1本を新設・2回連続 PASS・反転確認あり**＝`b7KawariBindsOpponentTarget`。
    🔑**この層は golden では原理的に守れない**＝`storedTargetCards` が**任意コストの resume を跨いで生存するか**は実機経路のみ
    （`freezeStoredTargets` の `FREEZABLE` に `POWER_MODIFY` は無く、生存は `PendingEffect.storedTargetCards` の永続化に依存＝**この組み合わせは今回が初出**）。
  - 🔴**既存シナリオ `b5KawariElseBanishesOnlyLow` の位置依存フレークを直した**（engine のバグではない）＝
    直前シナリオの盤面が残ったまま1ティック目の判定が走っていた（§4.4 罠34）。**盤面が載るまで再注入するループ**を入れて解消。
  - ⚠**2巡続けて Sheet1 が動いていない**（B6 も B7 も ±0）＝**parser の系統バグは Sheet1 の外に厚く分布している**。
  - 📮**送り**＝`O-105`（場全体の「下のカード合計」条件の受け皿）。

**▶ 次の一手**＝**Sheet1 の次バッチ**（**`npm run census:cards -- --sheet 1 --list` から取る**＝要対応 **66枚**。内訳 census 27／意味照合 48（findings 59件）／held 5／idset 1）。
🔑**取り方の勝ち筋**＝カード名の一覧では同一文型が見えない。**`docs/_vocab_census.txt` の語彙カテゴリ別**（B3/B5）か、**台帳 OPEN の claim を live の生データへ戻す**（B6/B7）で割る。
🆕🔴**Sheet1 の枚数を落としたい巡は、着手時に「母集団に Sheet1 のカードが入っているか」を先に数える**（B6・B7 は2巡続けて Sheet1 が 0 減＝**価値は Sheet1 の外にあった**）。逆に「系統バグを潰す」目的なら Sheet1 に縛らなくてよい。
🔴**着手前に必ず3つやる**＝①**engine／UI の消費地点を grep する**（B4＝実は実装済み／B5＝log 専用／B6＝**直すと実機がソフトロックする**／B7＝**engine の `fixOwnerTOSOC` が `targetsTriggerSource` を落とさないので効いていなかった**）②**母集団を live の生データで数え直す**（B5＝型名だけの仕分けは誤り／B6＝268→64→6／B7＝138→7）③**既存の見送り契約に当たったら、その理由が今も有効かをコードで確かめる**（B6＝有効だったので守った／B7＝**理由が解消できたので契約を解いた**）。
✅**進捗は3つの計器を併記する**（2026-08-27 ユーザー決定・§3）＝**Sheet1 要対応 66枚／台帳 OPEN 630／census 536**。⚠**単独の計器に固定しない**。**着手時に「今回どれが動く見込みか」を宣言する。**
機構側なら **`O-105`**（B7 の据置1件）／**`O-104`**（B6 の残り3形）／**`O-102`＋`O-95` をセットで**（同根・合計3効果）／**`O-98`**／**`O-97`**／**`O-101`**／**`O-103`**／**`O-93`**（manual shadow の掃除）／**`O-94`**。実機の腐り返済なら §5.1 の `V-89`。
---
