...
- **🆕 セッション（2026-08-10・続き418〜423・Opus 5）＝「所有者反転／コスト取り違え」を計器で追って6波。census 860→854・golden 1753→1771。68効果/64カード是正（うち🔴自傷11件・🔴コスト取り違え19件）**。
  ...
  - **🔴③ 続き419＝選択肢が2文構成だと CHOOSE が丸ごと消える**（自傷2件＋コスト消失2件）。**`coinCost` を parser が一度も生成していなかった**。
  - **🔴④ 続き420＝`TARGET_AND_DISCARD_HAND` は payload を持たず engine が手札の最後の1枚を捨てていた**。53カードで 捨て札filter脱落23／任意性脱落17／選択不能は全件。18効果を `OPTIONAL_COST{handDiscard}` へ。
  - **🔴⑤ 続き421〜422＝同じ STUB が「幻の手札コスト」でもあった**＝producer が**コストの種類を問わない catch-all**で、原文が**エナ/自己トラッシュ/下から**でも同じ STUB。engine は名前どおり手札を捨てるので**①原文のコストが一切支払われない②原文に無い手札1枚が消える**。**幻 19→1件**。🔴`WXDi-P14-060-E1` は「相手に捨てさせる」が**自分が失って相手が引く**真逆だった。
  - **🔴⑥ 続き423＝`applyDroppedTargetDesignation` の据置条件が2箇所とも甘かった**＝「宣言に filter が無ければ脱落しても等価」としていたが、**落ちるのは filter だけでなく所有者と体数**＝帰結は既定の `SIGNI{owner:'self',count:1}` になる。しかも `extraKeys` が空だと後段の `[].every()` が **true** で無条件 return していた。🔴「対戦相手のシグニを**2体まで**…それらを手札に戻す」→**自分のシグニ1体が戻る**（`WXDi-P02-009-E3`）／ルリグ宣言は regex ごと外れて**自分のシグニを凍結**（`WXDi-P02-040-E2`）。**副産物でパワー制限の脱落も16効果ぶん直った**（従来は無差別に撃てる過剰効果）。
  - **⭐⑦ 計器は「効果単位」で突き合わせる**＝⑥は parser の `enableSourceTextLog()`/`getSourceTextLog()`（effectId → 原文ブロック）で初めて厳密に測れた。カード全文 × カードJSON だと**効果Aの原文を効果BのJSONで救ってしまう**（9件中3件がその artifact）。
  - **⭐⑧ 6波を貫く4つの型**＝(a)**engine にあるのに parser が生成しない**（`OPTIONAL_TRASH_SELF`/`coinCost`/`handDiscard`）(b)**parser が出すのに engine が別の意味で読む**（catch-all STUB）(c)**どちらの型にも無いのに逆翻訳にだけ出る**（死フィールド。**自分で作った**）(d)**「情報が無いから落ちても等価」という据置条件が、別の情報まで落としている**（⑥）。
  - **⚠⑨ 引き継ぎの見積もりは着手前に必ず実測し直す（3回連続で外した）**＝「好きな枚数が最大クラスタ」→5件、「残19件は【起】コスト行」→**該当0件**、そして⑥は「1カードの端案件」と書いていたが**16効果の家族**だった。
  - **⚠⑩ 新しい payload キーを足したら逆翻訳も併せて足す**＝`energyTrash`／`selectionConstraint`／`OPTIONAL_ACTIVATE` が逆翻訳に出ず、**原文照合でコスト取り違えを見つけられない状態**だった。**計器自身の穴を疑うこと。**
  - **✅⑪ ゲート**＝`npm run gates` 全緑（golden **1771（+18）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **854** / 同型★ **0**（265群）/ lint 0 errors）。**要実機検証**＝続き420 の手札コストUI／続き421〜422 のコスト徴収先／続き423 の対象制限（§7 に登録済み）。
  - **📋⑫ 次の一手**＝**Opus 側**＝(a)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` の「能力を持たない」／「このシグニよりパワーの低い」等・**20枚規模**。`parseSigniTarget` の守備範囲外＝過剰対象化が残る）(b)🔴**`WX25-CP1-092-E1`（幻コスト最後の1件）**＝比例コストの語彙が無く本体も「3体まで／それら」が落ちている (c)**残 A群17件の手札コスト変換**（「好きな枚数」5件＝可変枚数＋倍率／`handDiscardGroups` 2件 ほか）(d)`WXEX2-70-E1`／`WX24-P4-048-E2`＝コストも帰結も別物に化けている1カード1機構 (e)**§6.4 A群の着手可能2件**／**`WX12-010`＝強制アタック機構**。**Sonnet 側**＝§7 実機検証（**続き420〜423 のコスト/対象UI を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418〜423（所有者反転／コスト取り違え／対象宣言脱落を計器で6波）後 最新値（本行が直近の正）**：census **854**、golden **1771（+18）**、`MANDATORY_SUSPICIOUS` **2**（据置）、held **123カード／48群**、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**263 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**、🔴幻の手札コスト **1件**（19→1）。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
