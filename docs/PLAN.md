...
- 🏁**セッション（2026-08-26・続き661・Opus 5）＝§5.3 `O-62` を消化＝「あなたの効果**１つ**によって」＝助数詞を挟む原因限定が丸ごと落ちていた（＋否定形の新語彙）**
  - ✅**真因は「リテラル一致の regex に助数詞が挟まると語彙ごと落ちる」**＝`/(?:あなた|対戦相手)の効果によって/` で拾っていたので、原文「効果**１つ**によって」（実測17文）が全部すり抜け、**原因主体を問わずに発火する過剰トリガー**になっていた。A-B 実測で **6効果**が動き、巻き添えはゼロ。
  - 🔑**登録票が保留していた設計判断への答え＝「`byOwnEffect` を足すだけで足りる」**。engine の枚数は**中央 diff の「1解決ぶんの set-diff」**で数えている＝**それ自体が「効果１つ」の単位**（`SEQUENCE` も1解決＝1つの能力）。`minCount` 系はそのままでよく、**足りなかったのは原因主体の限定だけ**だった。
  - 🔴**1件は「過剰」ではなく恒久 no-op だった**＝`SPDi43-13-sub-E1` は `collectEnergyToTrashTriggers` の場 watcher 経路が**原因限定を持たない宣言を無条件に落とす**ため一度も発火せず、限定を足して**初めて動いた**。
  - 🔴**live の `parseStatus:'MANUAL'` が parser 改善を4効果ぶん凍らせていた**＝`build:effects` の `needsReview` は **MANUAL/PARTIAL を最初から除外する**ので `_held_fresh` にも `_partial_fresh` にも出ない。`WXK10-040-E1` は**parser の方が正しかった**（engine のコメントがこのカード番号を名指ししているのに live に無かった）。全リーフ突き合わせで安全確認してから MANUAL を外して採用した。
  - ✅**否定形（§5-2″）＝`banishedNotByOwnEffect` を新設**。⚠`banishedByOwnEffect:false` では書けない（未指定と区別がつかない）。⚠「アクセされている」は**盤面の状態**なので `triggerFilter` では書けず、`banishedHadCharm` と同じ規約で `prevOwnerState.field.signi_acce` を読む `banishedHadAcce` を足した。🔴`WX15-003-E1` は `triggerScope` が既定 self に潰れ、**ルリグ自身のバニッシュを待つ形で構造的に一度も発火しなかった**（実機で反転確認）。
  - ⚠**入れ子（`GRANT_*.abilities[]`）は別経路ではなかった**＝regex を広げたら内側 `-sub-E1` にもそのまま届いた（登録票の懸念は杞憂）。

**▶ 次の一手**＝**§5.3 の `O-73`／`O-74`（本セッションで登録）か、`O-71`／`O-72`／`O-70`／`O-68`／`O-69`**。
- 🔑**「リテラルで拾っている語彙」は助数詞・修飾で簡単に穴が開く**（`O-62`）＝**原文から助数詞ごと数え直す**のが先（`の効果[^、。]{0,6}によって` で測ると 261/17/4 に割れた）。
- 🔑**否定形は `false` では書けない**＝JSON では未指定と区別がつかないので**明示値の別キー**にする。
- 🔴**live の `MANUAL` は parser 改善を凍らせる**＝`_held_fresh`/`_partial_fresh` のどちらにも出ないので**気づけない**。`manualEffects.ts` に無い live 限定 MANUAL を見つけたら、fresh と全リーフで突き合わせて**外せるなら外す**。
---
