...
### 📊 恒久指標（維持中・逐次更新）
- **P1 表現①の systematic 指標**：同型★0（`node scripts/groupSimilar.mjs --all`）。**held は 252枚／署名グループ 108件（2026-07-29 第15波実測・`node scripts/heldReview.mjs`）。LOSS/VALUE は held 188 / LOSS 154 / VALUE 34（2026-07-19 実測・`npx tsx scripts/parserWorklist.ts`・⚠HEAD比較＝未コミットJSONは反映されない）**。第14波258枚/114署名からの−6枚/−6署名は、歌本文重複6枚の採用待ちを全件解消したため。`WX25-P3-052` も明示採用済みで新規heldは残していない。続き29時点（held 79）からの増加は主に**その後の parser 改善で fresh が curated より正しくなった採用待ちバックログ側**（Sonnetタスク6の採用サイクルで消化してから実数を締め直す）。**この数字からさらに増えたら回帰**（JSON手パッチ時は パーサー同修正 or MANUAL化 or ここを実数更新）。旧内訳の詳細は PLAN_DETAIL 参照。
...
- **2026-07-29 第15波 最新値**：歌本文重複6枚を採用し、`WX25-P3-052-E1` のエナpick→レベル合計二分岐を既存語彙だけで実働化。census **1410据置**（`BASELINE_HIGH=1410`）、golden **1000→1004**、held **258枚/114署名→252枚/108署名**。第14波は `WXK04-045-E1` と `WX26-CP1-061-SONG` の同名参照pickを実働化し、通常E1の歌効果混線を除去して census 1412→1410／golden 998→1000。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1004**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1410**（回帰ゲート）・held **252枚/108署名**（2026-07-29 第15波実測）。
>
