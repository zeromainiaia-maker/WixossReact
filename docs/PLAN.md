...
| (xcii) | **🆕 相手の盤面を参照するコスト軽減が未実装＝8枚（2026-08-03・(xc) の全数分類で確定）**＝`computeArtsEffectiveCost` は**相手の場・ライフを受け取らない**（`replaceCtx.oppState` は `turn_arts_used`／`actions_done` だけ）ので、「対戦相手の場にある〔凍結状態のシグニ／【チャーム】／【ウィルス】／能力を持たないシグニ〕1つにつき減る」**6枚**（`WX07-065`／`WX08-028`／`WX08-032`／`WX16-033`／`WX21-Re01`／`SP26-003`）と、`SP38-002`（ライフが相手より多い場合《無×3》減る）・`SPK01-14`（対戦相手のコイン1枚につき《赤×1》減る）が読めない。**着手＝`computeArtsEffectiveCost` の引数に相手 `PlayerState` を足す**＝全呼び出し元（`ArtsModal`／`SpellCastModal`／`CutinModal`／`BattleScreen.getCardActions`）へ波及するので**シグネチャ変更の1バッチとして独立して取ること**。⚠(xc) で入れた「空盤面で1枚も動かない」全数走査 golden をそのまま回帰ゲートに使える |
