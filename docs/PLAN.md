...
- **🆕 セッション（2026-07-08・続き45・Opus 4.8・§4タスク2 残＝動的比較 別基準 `powerLtAnyAlly`）**＝続き43-44（自己参照/トリガー参照）に続く**別基準サブファミリ**を消化。「あなたのいずれかのシグニよりパワーの低い対戦相手のシグニ1体を対象とし、それをバニッシュする」＝**自分の場のシグニのいずれか（＝最大実効パワー）を基準**にした比較が JSON で脱落し全シグニ対象になっていた過剰効果（DB 全2枚 WXDi-P01-020/WXDi-P07-031・両方 census 高シグナル計上）。**型**＝`powerLtAnyAlly` 新設。**parser**＝`parseAnyAllyComparison` を `parseSigniTarget` に配線（2枚とも単一フラグメントで通る＝designation post-pass 不要・過剰語彙を作らず「パワーの低い」のみ）。**engine**＝`resolveDynamicFilter` で `ownerState.field.signi` 各スタック最上位の実効パワー最大 -1 を `powerRange.max` へ解決（参照不能なら制限なしフォールバック）。**decompiler**＋**census keys** に追加。**採用2枚**＝`build:effects` 自動収穫（純改善・collateral 0）。**検証**＝typecheck・**golden 162→163（+1）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1624→1623（-1）**＝`BASELINE_HIGH` 1623 実数更新（WXDi-P07-031 完全解消／WXDi-P01-020 は別カテゴリ「トリガー:アタックしたとき」残存だが動的比較の修正は両方完了）。**次の一手＝(A)Opus＝§4タスク2 残（動的比較）の別基準サブファミリ続き**：lastProcessed（そのシグニ＝場に出た/公開した＝WXK10-031 は STUB `DECK_REVEAL_UNTIL` 実装が前提・WXDi-P08-031 は ADD_TO_FIELD が配置シグニを lastProcessedCards 未設定＝engine 設定もセット＝2枚とも重い）＋printed（表記パワー＝per-candidate 実効/表記比較の新機構＝WX25-CP1-093/WXK10-027・fieldCandidates に per-candidate 述語が要る）＋lrig相対（WX19-042 対戦相手センター/WX09-037 自分センター＝`levelLtLrig` lrigOwner 付きで clean・WXK10-001/WXK11-003 は複雑/制限で別）＋残る高シグナル22（`docs/_vocab_census.txt` 13行目）。**(B)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）の GRANT_QUOTED_AUTO_ABILITY 内側parse。(C)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43の6枚＋続き44の2枚の実機確認）。(D)verifyBattleDrive バッチ状態汚染の根本修正は続き39から持ち越し。**
