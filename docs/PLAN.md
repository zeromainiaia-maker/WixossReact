...
| 1 | **§7 実機検証の横展開** | 検証（driver シナリオ追加のみ） | S×件数 | **✅(a)(b)は続き141（Sonnet）で消化完了**：(a)`trashCounterOpp`（タスク12(iv)修正の反転＝PASS・既定orderに追加）(b)ON_LRIG_GROW④のusageLimit＝`lrigGrowUsageLimit`（タスク12(vi-5)修正の反転＝旧FAILの真因はdriver側のtestId誤りと判明・修正してPASS・既定orderに追加）。**残る(c)R37③ ON_SIGNI_POWER_ZERO_OR_LESSのusageLimit＝専用シナリオが未実装（既存`powerzero`は基本発火のみ）＝新規シナリオ作成から要る**。**残ブロック**＝(xiii) B4引用付与（→Opusタスク17 で解消見込み）／(xix) WX04-005-E3／(xx) ON_TARGETED forced／(xxi) oppDraw発生源。WX22-001-E3（クラフトトークン残・§6.4）も引き続き可 |
