...
- **🆕 セッション（2026-07-22・続き250・Opus 4.8＋Codex 分担・**[P1_COMPLETION_ROADMAP](./P1_COMPLETION_ROADMAP.md) **バッチ1「状態条件節の持ち上げ」第2波＝参照カード属性条件**＝「それ/そのカードが〈属性〉の場合」系の**真バグ13効果を採用**（census 1799→**1792**・golden 584→**588**）**（続き249 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **次の一手（Opus/Codex）**＝**バッチ1第3波候補**＝(a)条件節クラスタの次の同機構群（「このターンに〜していた場合」ターン内履歴系は engine counter の有無で仕分け・「あなたの場に〜がある場合」HAS_CARD_IN_FIELD 残）(b)第2波見送りの機構解消＝REVEAL_AND_PICK 内 did-it continuation（G7 2枚＋同型）・hand-or-energy 宛先（G8）。⚠新規則は `STATE_HOIST_BATCH1_CARDS`／`applyReferenceAttributeBatch2` のカードゲート方式を踏襲。**Sonnet はタスク1（§7 実機検証）**＝TURN_OWNER 配線9箇所＋覚醒/elseAction/レゾナ置換の実機スポットチェック（要実機検証）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 588・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1792。
>
