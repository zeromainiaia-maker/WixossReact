...
- **🆕 セッション（2026-08-16・続き505・Opus 5）＝§6.4 **O-6／O-7 クローズ**（ともに**残0**）。ゲート全緑（**golden 2094**＝+4・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **7効果 / 7カード**。`census:stubs` A群 **18種/22件 据置**（無言 no-op 0）。新機構＝`OptionalCostSpec.selfToEnergy`／`restoreElidedSwapSource`（照応を戻す純テキスト正規化）／`execSendToEnergy` の `thisCardOnly` 分岐。
  - **🔑① O-6 の判断は「AUTO 採用」**＝`WX25-P3-038` の fresh は MANUAL の**上位互換**だった（対象宣言が `POWER_MODIFY{delta:0}` の代用→`SELECT_TARGET_ONLY`／条件が `LAST_PROCESSED_HAS_NO_ABILITIES`→`LAST_PROCESSED_MATCHES{noAbilities}`）。**手で MANUAL を直すのではなく MANUAL を消す**のが正解だった。
  - **🔴② `LAST_PROCESSED_HAS_NO_ABILITIES` は holder を自分側 state に固定していた**＝`abilities_removed` は `applyAbilitiesRemoval` が**能力を消された側**へ書くので、対象（＝**相手の**シグニ）の能力喪失を一度も見られず「代わりにトラッシュ」枝が永久に外れていた。`LAST_PROCESSED_MATCHES` と同じく**カードが実際に居る場から holder を引く**形へ統一（`WX25-CP1-002-E1` も直る）。
  - **⚠③ live レコードが `parseStatus:"MANUAL"` だと `build:effects` は永久に触れない**＝`manualEffects.ts` からエントリを消すだけでは live に届かず、**live JSON の外科パッチ**が要る（`WX25-P3-060` も同型）。
  - **🔑④ O-7 の交換は新機構ゼロで解けた**＝`parseSentencePart3` の二ゾーン交換ビルダーは**その文だけ**を見るので、交換元の名詞句が前の文にしか無い省略形（「そうした場合、**それと**〜」）に一致していなかっただけ。**照応を戻す純テキスト正規化**で既存ビルダー＋`foldSuppressOnPlay` funnel にそのまま載り、レベル/クラス絞り込み・`thisCardOnly` の場側固定・【出】抑止まで自動で付く。
  - **🔴⑤ 原文 regex で数え直したら「1効果」ではなく timing が6効果とも外れていた**＝`trigText`（`/^.*?(?:とき|時)[、,]/`）は**非貪欲**なので「このシグニがアタックしたとき、」で切れ、直後の「**そのアタック終了時**」を一度も見ない。`WX24-P4-052-E2`／`WX25-P1-053-E1`／`WX25-P2-090-E1`／`WX25-P2-093-E1`／`WX25-P3-060-E2` が**アタック解決前**に走り、自分をバニッシュ/退避して**アタックを取り消して**いた。
  - **⚠⑥ ルリグのアタック終了時は engine 未配線**＝`collectAttackEndTriggers` は**アタックしたシグニ自身**の【自】しか見ない。`WX24-P3-055-E2`／`WXK11-006-E4`／`WX14-018-E4` を同じ規則で昇格させると**収集地点の無い無言 no-op** になるので regex を `^この(シグニ|カード)がアタックしたとき、` に限定し、**負方向を golden で固定**した。
  - **🔴⑦ 隣接発見＝「交換の分解表記」2効果はコストを一度も払っていなかった**＝`WX25-P2-090`／`WX25-P2-093` は `STUB{LRIG_UNDER_CARD_OP}`（part4 の総称バケツ）＋**source 無し `ADD_TO_FIELD`**＝**対価を払わず、エナからではなく手札から出す**別動作。`OptionalCostSpec.selfToEnergy`（`OPTIONAL_TRASH_SELF` の行き先違い）で正準形へ寄せた。⚠**`matchesFilter` は `thisCardOnly` を黙って無視する**ので `execSendToEnergy` に分岐を足さないと「自分のシグニを1体選ぶ」UI に化ける。
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-8`／`O-9`／`O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer 18 id/22件の前提再判定）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き505 の4経路**＝(a)`WX25-P2-058`（アタック**解決後**にダイアログが出る／《アイヤイ★クイーン》が居ないと出ない＝**負方向＋対照の対**／エナの＜遊具＞レベル2以下だけが候補／出したシグニの【出】が発動しない）(b)`WX25-P2-090`（支払うと自分がエナへ行き、エナのレベル1＜遊具＞が**ダウン状態で**場に出る／断れば何も起きない）(c)`WX24-P4-052`（自分をバニッシュしても**アタックは通る**＝旧挙動との対）(d)`WX25-P3-038`（**能力を消した**相手シグニがバウンスではなく**トラッシュ**へ）。⚠**すべて実機未検証**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-16 続き505（§6.4 **O-6／O-7 クローズ**＝ともに 残0）後 最新値（本行が直近の正）**：census **823 据置**、**golden 2094**（+4＝①O-7 の据置3軸トリップワイヤを「非採用」から「3軸すべて表現」へ反転 ②`externalSwapSpecs` に `WX25-P2-058-E1` を追加＝E2E 11効果目 ③ON_ATTACK_END 母集団6効果＋ルリグ版の負方向 ④`selfToEnergy` の E2E と `abilities_removed` を相手側から読む回帰ガード）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **102枚 / 44群**（据置）、lint **0 errors / 285 warnings**、`census:stubs` A群＝**18種/22件 据置**（無言 no-op 0）・C群 **172種/253箇所 据置**、`census:goldentypes` **未カバー 0型**、`census:timing` フォールバック **3効果**。🆕**live JSON changed 7効果 / 7カード**（CSV 非改変）。🆕**新機構＝`OptionalCostSpec.selfToEnergy`（`SEND_TO_ENERGY{thisCardOnly}` を pay step にする任意コスト）／`restoreElidedSwapSource`（省略された交換元を復元する純テキスト正規化）／`execSendToEnergy` の `thisCardOnly` 即適用分岐**。⚠**全経路 実機未検証**（§7 送り）。
...
| **O-10** | **明示 defer の棚卸し** | 18 id / 22件 | 🆕**2026-08-16 続き503 実測値＝A群は全体で 18 種 / 22 件**（続き500 の 15種/17件 ＋ **O-28 で新設した3種/5件**＝`DEFERRED_CONVERT_ENERGY_COLOR`〔【コンバート《色》】3効果。⚠**エナ支払いの色照合が `costs.ts` に5箇所散っており 1点 funnel が無い**のが前提〕／`DEFERRED_ATTACK_WHILE_DOWN`〔`WX22-022-E1`。⚠「すでにダウン」判定は `signiAttackGate` ではなく `BattleScreen` の `getMySigniZoneActions` にインラインで入っており CPU 経路にも同じ判定がある＝**先に down 判定を gate へ寄せる**必要がある〕／`DEFERRED_LEAVE_FIELD_REPLACE_WITH_DOWN`〔`WX25-P2-071-E1`。置換の受け皿は「代わりに別カードを離す」型で「代わりに**能力を失って**ダウン」は表現できない〕）。旧＝続き500 は 15種/17件。旧＝続き499 は 20種/22件（この15 id ＋ O-34 の5）。旧＝続き498 は 15種/17件（O-3 の受け皿が残0になった時点）。旧＝続き493 は 23種/26件（この15 id ＋ O-3 の8）。旧＝15 id（続き492・A群 23種/25件）／15 id（続き491）／14 id（16件・2026-08-13 実測）。A群に `DEFERRED_*` として可視化済み。**理由が「先にやることがある」型は前提が消えたら着手できる**（例＝`DEFERRED_UNDER_CARD_AS_ENERGY_COST` は funnel 完成後に実装できた）。**定期的に前提を再判定する**。⚠**A群に出ない無言 no-op のほうが多いことがある**＝続き491 の系統は壊れていた6効果のうち**4件が STUB ですらなく**、続き492 の系統は8効果のうち**7件が STUB ですらなかった**（未消費の `BLOCK_ACTION` id／綴りズレ／ログだけのハンドラ／1回で消費される boolean／走査軸の漏れ）。**在庫表は原文 regex と併用する。** |
