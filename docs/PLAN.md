...
- **🆕 セッション（2026-08-01・続き313・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxxiii)＝トラッシュ領域移動ロック `LOCK_OPP_TRASH_MOVE` を実働化。**これで `WX24-P4-007-E1` が 3/3 になり **タスク12(lxi) は残0クローズ**（第1〜11波＋本件）。(lxxiii) 自身もクローズ**（golden **1218→1222**・census **1354 据置**（parser 不変＝**engine 実装のみ**）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **250枚/98署名 据置**。live per-effect 差分 **changed 0**／`build:effects`・`regen` 冪等。STUBS.md **実装 547→548・フォールバック 30→29**）
  - **① 直したのは宣言 STUB の no-op**＝「次の対戦相手のメインフェイズとアタックフェイズの間、対戦相手のトラッシュにあるカードは対戦相手の効果によって他の領域に移動しない」（`WX24-P4-007-E1` の③／`WXDi-P14-005-E1` の c2＝**全CSVでこの2枚だけ**）。第9波では名前を付けて可視化しただけだった。
  - **② 🔑 母集団は grep でなく**全効果走査**で測った**＝PLAN が「先にトラッシュ発生源の全経路を列挙」と指示していたが、grep は104ヒットで当てにならない。smoke ハーネスを流用して10679効果を実行し「コントローラー自身のトラッシュからカードが消えた効果」を数えたら **330件**。`trashCandidates` の6地点を塞ぐと **17件**（＝バイパス）、全経路を塞いで **0件**。**この3点測定がそのまま射程の証明**になっている。
  - **③ バイパスは7経路だけだった**＝`PLACE_UNDER_SIGNI`（「トラッシュから…このシグニの下に置く」6効果）／`ATTACH_CHARM`（【チャーム】化）／`STUB{TRIPLE_ZONE_DISTRIBUTE_FROM_TRASH}`／`STUB{PICK_FROM_TRASHED_CARDS}`（「この方法でトラッシュに置かれたカードの中から」）／`STUB{TRASHED_CARD_TO_HAND_OR_ENERGY}`／`STUB{TRASH_SIGNI_TO_BEAT}`（【ビート】化）／`STUB{PLACE_TRASH_SIGNI_UNDER_ALL_WEAPON}`。⚠**PLAN が名指しで心配していた `STEAL_OPP_TRASH_PUPPET` は走査に出てこない**＝あれは**相手の**トラッシュを触る効果で、原文が禁じるのは「所有者**自身**の効果」だけ＝**射程外**（旧行の懸念は空振りだった）。
  - **④ 述語1本に集約し、候補生成の段階で止める**＝`isOwnTrashMoveLocked(owner, ctx)`（`owner==='self'` かつロック中かつメイン／アタックフェイズ。**`currentPhase` 不明時はロックしない**＝permissive）。汎用側は `movableTrashCandidates()` へ6地点差し替え、STUB 7経路は同述語で早期 return。**ロック中は候補0＝「対象がない」で自然に no-op**（後付けで盤面を巻き戻す設計にしない＝途中ログが嘘になる）。
  - **⑤ 期間は第10波の予約→昇格に相乗り**＝`lock_trash_move_next_turn` → ターン開始時に `lock_trash_move_this_turn` へ。形が `signi_zone_blocks_next_turn` と完全に同型なので `activateNextTurnSigniZoneBlocks` に載せ、呼び出しサイト4箇所を二重に持たない。フェイズ限定は昇格ではなく**実行時に述語が見る**（`ctx.currentPhase` があるので近似不要）。
  - **次の一手**＝**Opus**：①🆕**タスク12(lxxv) owner 反転**（`WXDi-P14-005-E1` c2＝「対戦相手はデッキの上から10枚トラッシュ」が**自分のデッキ**を削る自傷・実害が明確・母集団を機械走査してから）②**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く可能性が高い）③**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）④**(lxxvi)** ゾーン供給源が違う2枚（受け皿は第10波で出来た＝残りはゾーン決定だけ）⑤**(lxxii) 遅延クリアの片側漏れ**⑥(lxxiv)⑦(lxviii) 散文形「対戦相手のターンの間、」⑧§6.3 J 残11 ⑨(lxvii) CPU `ON_TURN_END` 未収集。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜313 の全件が実機UI未検証で積み上がっている**（第10波の配置モーダル活殺／第11波の跨ぎ選択モーダルは**目視しやすい**ので取りやすい）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1222**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1354**（回帰ゲート）・held **250枚/98署名**（2026-08-01 タスク12(lxxiii) 実働化後に実測）。
>
