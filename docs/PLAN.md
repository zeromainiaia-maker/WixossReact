...
- **⏭️ 次セッションの最優先＝BattleScreen リファクタ Stage2（ユーザー指定・census より優先）**＝ドメイン別 useReducer＋カスタムフック化。Stage1 完遂時点（本§下の「リファクタ Stage1 完遂」参照）＝10,882行・inline モーダル残0・純関数は `src/screens/battle/` 抽出済み。Stage2 の会話設計は失われているため cold start は次の手順で：①BattleScreen.tsx の useState 群を「ドメイン」ごとに棚卸し（盤面/フェイズ進行/モーダル開閉/選択中ターゲット/コスト支払い等）→②相互に同時更新される state 束から1ドメインずつ `useReducer`＋カスタムフック（`src/screens/battle/hooks/useXxx.ts`）へ純移動（挙動無変更・1ドメイン1コミット）→③各コミットで typecheck・lint・golden・smoke・fuzz・vite build・verifyBattleDrive（バッチ11 PASS＋既知汚染2種は分離実行）を回す。小粒の前哨戦＝verifyBattleDrive のシナリオ間状態リセット（バッチFAIL 2種の解消）から入るのも可。
