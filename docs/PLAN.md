...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 735・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1551（＝`BASELINE_HIGH`・回帰ゲート）。
>
