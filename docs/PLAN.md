...
| 14 | リファクタ Stage2→Stage3 純粋バトルコントローラ | BattleScreen構造 | L | ✅Stage2完了＋永続化移行完了（battle_states 全行 I/O 120箇所を `persist` へ移行）＋**reducer 18 action・114/117 commit 経由**（続き244-247／続き271＝13箇所／2026-08-03 第1弾＝5バッチ計31箇所／🆕2026-08-03 第2弾＝2バッチ計11箇所）。設計/移行レシピ＋**定形9つ** `docs/BATTLE_CONTROLLER.md`。✅(b) 効果解決本体＝`RESOLVE_EFFECT_STEP` で残0 ✅(c) spread 系＝`WRITE_STATES`／`opp:undefined`／`markCutinResponseComplete` で残0 ✅(d) CPUターン終了＝`BEGIN_NEXT_TURN` で消化。**残＝3 commit のみ＝全部 (a) の可変アキュムレータ**（`doPhaseAdvance`／`resolveStackNext` のCPUターン強制終了 `Object.assign`／効果解決結果の後乗せ `'host_state' in update` 読み戻し＝`update.effect_stack` ベース push・`update.turn_phase` 読み分岐）。⚠ハンドラ側 payload 構築は golden 非カバー＝機械一括変換不可・1件ずつ手動レビュー要（レシピ BATTLE_CONTROLLER.md §4）。**進め方＝「挙動同値と確信できるものだけ N 箇所」に必達を絞る**（続き271 は codex 実装／Claude 照合、2026-08-03 は Claude 自身が1バッチ5〜7箇所。どちらでも可）。**次の一手＝残り3箇所＝ハンドラのフェイズ別分割という別作業**なので、着手時に scope を切り直す（payload 化では落ちない）。**この作業の締めは §7 実機通し確認と対**（golden はハンドラ側 payload を見ない） |
...
- **🆕 セッション（2026-08-03・続き333・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝§3 タスク14（Stage3 純粋バトルコントローラ）第2弾＝2バッチ・11箇所前進で **(c) spread 系を残0消化****（ユーザー指示「opusタスク14の続きを行う」）。**reducer 経由 102/117 → 114/117（未移行 15→3）／action 17→18／golden 1326→1329。差し戻し0・両バッチ全ゲート緑。**
  - **第6バッチ（6箇所）＝前回 PLAN が「次の一手」に指名していた spread 系を全部掃いた**：①**`apsUpdate`**（CPU の MAIN→ATTACK_ARTS）→ `SET_TURN_PHASE` に**任意の1状態＋`effectStack`** を持たせて移行（`ADVANCE_TURN_WITH_STATE` との違いは**状態書き込みが任意**）②**レゾナのスペルカットイン応答2箇所** → `WRITE_STATE.markCutinResponseComplete`（旧 `resonaSpellCutin ? {...update, pending_spell:{...bs.pending_spell!, cutin_response_complete:true}} : update` ＝**reduceBattle の戻り値を後乗せで書き換える**形だった。`pending_spell` を土台にするので**盤面依存 action の3つ目**）③**`queueCardEffects` の `extraUpdate: Record<string, unknown>`** → 実引数を数えたら `{}`2件と `{[opKey]:opState}`1件だけ＝`opp?:{key,state}` に狭めて `WRITE_STATE` へ合流④**対人じゃんけん2箇所**（提出／解決）＝CPU経路だけ移行済みで手書きが残っていた取りこぼし（定形6）。
  - **第7バッチ（5箇所）＝直前フラグ watcher**（`hand_*_just`／`opp_virus_*_just`／`zone_moved_just`／`drive_became_just`／`beat_became_just`）の `const update: Record<string, unknown> = {}` 命令的構築を**新 action `WRITE_STATES`**（プレイヤー状態**0〜2件**＋任意 `effectStack`）で純粋化。
  - **🟢 「(a) に見えて (a) でない」を1系統見抜いた**＝`applyState` の `...(update[key] as PlayerState ?? base)` は**各キーにつき1回しか呼ばれない**＝読み戻しが常に `base` に落ちる**死んだアキュムレータ**。定形5「移す前にそのキーが何回書かれるか数える」を適用して (a) ではないと確定してから移した（**定形7**として登録）。
  - **確立した定形＋3（計9）**＝⑦読み戻しが常に undefined なら (a) ではない⑧`Record<string, unknown>` のパッチ変数は移行の匂い（computed key で型検査が死んでいる印＝`PlayerStateKey` で受け直すとヘルパー引数まで型が伝播）⑨**ヘルパーの「何でも入る追加パッチ」引数は用途を1つに絞れることが多い**。
  - **残＝3 commit のみ（117箇所中）＝全部 (a) の可変アキュムレータ**＝`doPhaseAdvance`／`resolveStackNext` のCPUターン強制終了（`Object.assign`）／効果解決結果の後乗せ（`'host_state' in update` 読み戻し）。✅(b)(c)(d) は残0。
  - **次の一手**＝**Opus**＝①**タスク14 の残り3箇所は「ハンドラのフェイズ別分割」という別作業**＝payload 化では落ちないので、着手するなら**先に scope を切り直す**（`doPhaseAdvance` をフェイズごとに割ってから移す）②または**タスク12 の生き残り在庫**〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxi)〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）。**Sonnet**＝§7 実機検証＝**続き298〜333 の全件が実機UI未検証**（続き330 第6波で 44効果の選択UIが「N個まで選択」＋スキップに変わった分を含む）。⚠**タスク14 は BattleScreen の構造変更＝ハンドラ側 payload 構築は golden 非カバー**なので、**実機のセットアップ〜ターン終了までの通し確認**が要る（じゃんけん提出/解決／ルリグ選択／マリガン／レゾナ召喚〔特にスペルへのカットイン応答〕／CPUのアタックフェイズ開始／【ライフバースト】／手札公開・手札捨て・ウィルス増減・ゾーン移動・ドライブ化・ビート化の各トリガーを今回触った）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き333（§3 タスク14 Stage3 第2弾・2バッチ）後 最新値（本行が直近の正）**：golden **1326→1329**（+3＝`SET_TURN_PHASE` の状態/スタック任意〔キー集合＋null 明示クリア〕／`WRITE_STATE.markCutinResponseComplete`〔既存フィールド温存・**盤面側を書き換えない純粋性**・省略時はキー無し〕／`WRITE_STATES` の片側・両側・スタック省略・空 states）、census **1289 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0、held **259枚／110群 据置**。**parser/effects JSON は完全不変**（BattleScreen 構造のみ＝`public/data/` に差分なし・live per-effect changed 0・**新語彙0本**）。**reducer 経由 102/117→114/117・`BattleAction` 17→18種**（+`WRITE_STATES`。`SET_TURN_PHASE`／`WRITE_STATE` は payload 拡張）。⚠**ハンドラ側の payload 構築は golden で検出できない**（golden は純粋関数のみ）＝**この作業は実機通し確認と対で締める**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き333＝§3 タスク14 Stage3 第2弾2バッチ後）**＝golden **1329**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1289**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **259枚／110群**。
>
