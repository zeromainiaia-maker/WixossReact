...
**直近＝2026-09-18＝§5.7 `S-5c` 第3段＝シグニアタックのバトル解決（1,588行）を画面から出した**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- `resolvePendingSigniBattleFor`＋`crashOneLife`（115行）を 🆕`controller/resolveSigniBattle.ts` へ**逐語で移設**（元コードとの機械 diff＝`loading` の1行と相対パスだけ）。材料と I/O は `PerformCtx`。**BattleScreen 12,012 → 10,245行**。
- 移設時に1件修正＝付与キーワード（正面以外追加アタック等）を**見ている人の側**（`dynamicKeywords.my`）で引いていた＝CPU のアタックで CPU の付与が引けなかった → アタック側の盤面から計算。
- 同日の前段＝バグ報告「クラッシュのログが2行」（人間側アタック解決の二重実行）／実機シナリオの腐り36本の棚卸し＋**CPU のアタックで倒れた人間のシグニの【自】が「正面にあった」を解決できない実バグ**（`op`→`opS`）。
- 🆕計器の較正＝`census:costtext` が `controller/` を走査していなかった（移設のたびに原文を読む箇所が黙って消える）。
- 検証＝`gates` 全緑／実機 シグニアタック系20本 PASS／CPU 通し対戦 PASS（10ターン決着）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-5c` 第3段の続き**＝ルール処理の残り（`doPhaseAdvance` 683行 → `confirmEndDiscard` 234 → `handleCutinPass` 195）。🏁`resolvePendingSigniBattleFor`（1,588）は 2026-09-18 済。**そのあと `cpuTurnAction`**（1,362行） |
