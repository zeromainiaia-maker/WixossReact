...
- **🆕 セッション（2026-08-10・続き418〜419・Opus 5）＝§6.4「代わりに/選択肢」まわりの所有者反転を2波で系統消化。census 860→854・golden 1753→1761・UNKNOWN 38→36ノード。15効果/14カード是正（うち🔴自傷5件）**。
  - **🔴① 続き418＝「代わりにそれを＜動詞＞」の per-target 形を parser が意図的に除外していた**（`const perTarget = /それ/.test(…) && !/対象とし/.test(…); if (!perTarget)`）。除外された昇格文は**別ステップとして残り**、主語のない「それを〜する」が `owner:'self'` へ反転＝**自分のシグニをバニッシュ/付与**（`WX08-028-BURST`／`WX09-Re01-E2`／`WXDi-CP01-026-E1`）。PLAN が「UNKNOWN 2件」と書いていた家族の**本体は無言の過剰効果**だった。
  - **✅② (A) 盤面状態条件は `CONDITIONAL{then:昇格, else:base}`**。先行詞は **`prevRaw` の対象節を前置して再パース**し、対象は **base のもので上書き**。⚠`prevRaw` はトリガー節を残すので**先頭固定では取れず**、非貪欲な前方一致だと条件節まで飲む＝**末尾の「を対象とし、」から所有者語まで戻して切り出す**。⚠**期間も base から引き継ぐ**（`WXDi-CP01-026-E1` は放置すると【シャドウ】が恒久付与に化ける）。
  - **🔴③ (B) 対象プロパティ条件（「それが感染状態の場合」）を (A) の形で書くと恒久 no-op**＝`LAST_PROCESSED_MATCHES` が**対象選択より前**に評価され常に偽。正しくは `SELECT_TARGET_ONLY → STORE_LAST_PROCESSED_TARGETS → CONDITIONAL{…targetsStored}`。**実装途中で実際にこの穴に落ち、golden の「構造」ではなく「実行結果」テストで捕まえた**＝この家族は構造アサートだけでは守れない。同じ理由で **POWER_MODIFY の値昇格は加算モデル**（base＋差分・`targetsLastProcessed`）。
  - **🔴④ 続き419＝③で潰した「所有者反転」を計器化したら別家族が出た**＝「どちらか１つを選ぶ。①…②〈任意コスト〉てもよい。そうした場合、…」で**選択肢が2文構成だと①②行が番号フィルタで落ち、帰結文だけが単文パスへ流れて CHOOSE が丸ごと消える**。`WXDi-P07-055/072-BURST` は**自分のシグニを手札に戻す/デッキ送り**、`WXDi-P07-094`／`WXDi-D09-P17` は**コストが消えて無条件に強い方**。残存文が「そうした場合、」で始まる形を `buildChoose` へ回して是正。
  - **🔴⑤ `OPTIONAL_COST.coinCost` は engine 実装済みなのに parser が一度も生成していなかった**＝`《コインアイコン》を支払ってもよい` が payload 無し（＝コスト0）に落ちて**コイン未払いで撃てた**。唯一正しかった `WXDi-P07-066-BURST` は **MANUAL 手書き**＝それが正準形だった。**「engine にあるのに生成側が無い」型は続き417 の `OPTIONAL_TRASH_SELF` に続いて2例目**＝疑ったら engine 側の受け皿を先に grep する。
  - **✅⑥ 条件語彙5本を追加＝いずれもゲートが丸ごと落ちて無条件発火していた**＝`THIS_CARD_HAS_UNDER.minCount`（型＋`execUtils`＋decompiler の3点セット）／トラッシュのスペルN種類／場の色N種類（副産物で `WXK11-057-E1` も是正）／`LIFE_COMPARE_OPP{lte}`。⚠最後の1本は `isBatch1OnlyClause` の**カードゲート**に阻まれていた＝閾値形ではない比較形をゲート外に出した。
  - **⚠⑦ 採用経路の落とし穴2つ**＝(a)`PARTIAL` は `PRESERVE_STATUSES` に守られ parser を直しても live に届かない＝**`AUTO` へ戻してから build→adopt**。(b)MANUAL 同居の混在カードは held ではなく **`docs/_partial_fresh.json`** に落ち、**そちらには adopt スクリプトが無い**（当該 AUTO 効果だけ直接移送した）。
  - **⚠⑧ 計器の限界（次に使う人向け）**＝「原文に『あなたの』が無いのに JSON が self」は**引用付与の内側**と**自己コスト**を誤検出する（27件中 真陽性4件）。「①②があるのに CHOOSE が無い」は**「以下の２つを行う」**（両方実行）を誤検出する＝**ヘッダを必ず併せて見る**。
  - **⚠⑨ 判明した既存の限界（回避・記録のみ）**＝`LAST_PROCESSED_MATCHES` のゾーン状態照合（`findFieldZoneState`）は **CardNum で場を引く**ため、**両プレイヤーの場に同じカード番号があると取り違える**。instanceId 化までの既知の穴。
  - **✅⑩ ゲート**＝`npm run gates` 全緑（golden **1761（+8）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **854**（`BASELINE_HIGH` 更新済み）/ 同型★ **0**（265群）/ lint 0 errors）。`build:effects`＋`regen` 実施済み。**実機UI検証は不要**（新モーダルなし・既存の CHOOSE／対象選択／コイン支払い経路のみ）。
  - **📋⑪ 次の一手**＝**Opus 側**＝(a)🔴**`WX25-CP1-004-E1` 選択肢②**＝原文「対戦相手の**ルリグ**１体を対象とし…それをダウンする」なのに `DOWN{SIGNI, owner:'self'}`＝**自分のシグニをダウン**。`TARGET_AND_DISCARD_HAND`（**53カードで使用**）が**対象の種別と所有者を本体へ運べていない**＝選択肢③（シグニ版）は正しいので**ルリグ版だけの穴**。まず53カードを同じ軸で洗うこと。(b)🔴**`WX25-P3-014-E1`**＝`BOUNCE{owner:'self'}` の自傷（base が任意コスト STUB を含む多段構成で SELECT/STORE 組み替えが did-it ゲートと干渉）。(c)**§6.4 A群の着手可能2件**（`ASSIST_LRIG_ATTACK_THIS_TURN`／`UNDER_CARD_AS_ENERGY_COST`）(d)**`WX12-010`＝強制アタック機構**(e)**UNKNOWN 残36ノード**。**Sonnet 側**＝§7 実機検証（**続き416〜417 の pay/skip モーダルを最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418〜419（§6.4「代わりに/選択肢」の所有者反転を系統消化）後 最新値（本行が直近の正）**：census **854**（860→854・`BASELINE_HIGH` 更新済み）、golden **1761（+8）**、`MANDATORY_SUSPICIOUS` **2**（据置・`node scripts/_checkAllEffects.mjs`）、held **115カード／49群**（うち採用済み4枚は除外表示）、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**（38→36）。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
