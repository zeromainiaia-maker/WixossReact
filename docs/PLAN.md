...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕確認＋codex-work 実装）＝タスク12(lv)「任意・無コスト【出】」を配線し、🔴その先の live 該当2件が二重任意指定だったのを是正**（golden 1025→**1029**・census **1412 据置**・smoke 10726 全0・fuzz 全0・lint 0 errors・同型★0・build 冪等〔再生成後差分0〕・`npm run regen` 実施・gates 全緑）
  ...
  - **✅続けて COLLAB（未配線4経路の①）も消化**（golden 1029→**1031**）＝🔴**逆向きのバグ2つ**だった。**穴A＝直接 COLLAB（「コラボライバーN人を呼ぶ」）は `effectType`/`timing` しか見ずに raw effect を直積みしており、`mandatory`・`cost`・`activeCondition`・`usageLimit` を全て無視＝アシストルリグ ON_PLAY 293効果のうち `mandatory:false` 91件（うち cost あり **88件**）がコストを払わず発動していた**（`WXDi-D06-006-E1`＝「手札1枚を捨てる：3枚引く」で手札 2→**5**・トラッシュ 0→**0** を実測）。既存の共通機構 `collectPlacedSelfOnPlayTriggers{placedByEffect:true, sourceIsSigni:false}` へ寄せて解消（修正後は `OPTIONAL_COST` の pay/skip が出て、pay で手札 2→4・トラッシュ 0→1）。**穴B＝CHOOSE 経由の「コラボしてもよい」枝（`INTERNAL_DO_COLLAB`）が `lastProcessedCards` をセットしておらず、配置したアシストの【出】が一切積まれない no-op** も同時に是正。**⚠Claude 追加実測で「コラボしない」枝が誤収集しないこと**（`lastProcessedCards: null`・配置なし）も確認済み。
  - **次の一手**＝**Opus：🆕(lv) 残3経路**〔**✅COLLAB は消化済**／アシスト配置（着手前に `WXDi-P15-034-E1` の二重任意指定疑いを判定）／CPU 2経路は「任意は発動しない」を明示据え置き〕**／(xlvi) は残0クローズ済／(xxix) 残**〔(2)(B) 43件＝`costUnparsed` 47効果が worklist／(1) 未対応コスト80件〕**／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／(l) B群36件＝真の過剰実行は `WDK02-009` のレベル条件落ち1件／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋(xxix)(1)(2)＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。
...
- **2026-07-29 タスク12(lv) 最新値**：グロウ経路に任意・無コスト【出】を配線し、二重任意指定2枚（`WX10-007-E1`／`WX10-021-E1`）を `mandatory:true` へ是正。census **1412据置**（`BASELINE_HIGH=1412`）、golden **1025→1029→1031**〔続きの COLLAB 統合まで含む〕、live **changed 2 / added 0 / removed 0**、build 再実行後差分0、同型★0。**本行を直近の正とする**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1031**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1412**（回帰ゲート）・held **251枚/107署名**（2026-07-29 タスク12(lv) 実測）。
>
