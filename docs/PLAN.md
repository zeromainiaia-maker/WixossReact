...
- **🆕 セッション（2026-07-08・続き44・Opus 4.8・§4タスク2 残＝先頭「対象とし」designation の動的比較を後続ターゲットへ引き継ぐ）**＝続き43で「残（follow-up）」として列挙した系統を消化。「このシグニよりパワーの低い対戦相手のシグニ1体を**対象とし**、《赤》を支払ってもよい。そうした場合、**それを**バニッシュする」型は、**対象選択が先頭 designation 文にあり除去アクション文（「それをバニッシュする」等）には比較語が残らない**ため `parseSigniTarget` 経由の比較パーサが届かず、比較フィルタが全数脱落（過剰効果）していた。**parser のみ**（engine は続き43で全配線済み）＝`parseActionText` を薄いラッパ化し、内側 `parseActionTextInner` の結果に **`applyLeadingSelfComparison(text, action)`** を全 return 横断で適用（単文早期return も多段SEQUENCEも同経路・冪等）。先頭 designation を検出し `parseSelfComparison(text)`＋`parseTriggerComparison(text)` で**基準を厳密に切って**後続 opponent-signi ターゲットへ比較キーを刻む。**採用6枚**＝①**「このシグニより」＝自身（powerLtSelf）**＝WXK05-059/WXK10-062（BANISH・STUBコスト後）・WXK09-052（SEND_TO_ENERGY 単文）・WXK10-040（nested CONDITIONAL・MANUAL保護のため JSON 手パッチ）。②**「そのシグニより」＝トリガー主語（powerLtTrigger）**＝WXK07-030（「あなたのシグニ1体がアタックしたとき、そのシグニより」＝別シグニでも正・ON_ATTACK）・WXK11-041（被バニッシュ・ON_BANISH）。③**「…する。その後、そのシグニより」＝直前の公開/場出しカード（lastProcessed）＝「その後」除外で据置**＝WXK10-031/WXDi-P08-031 は別機構へ保留。⚠初回ビルドで「その」も一律 powerLtSelf 化した誤りを基準切り分け修正後に是正（誤キー除去→再harvest）。DB 全18枚の designation カードを機械照合し既存処理9枚は無傷・WXDi-P10-042（引用付与内「このシグニ」＝被付与シグニ）は MANUAL 温存が正。**検証**＝typecheck・**golden 161→162（+1＝designation の この/その/その後 分岐を shipped JSON で assert）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**（regen 済）・**census 1624/1624**（対象6枚は「STUB/MANUAL格納」バケツで高シグナル計上外＝実数不変）。**トリガー参照2枚（WXK07-030 ON_ATTACK/WXK11-041 ON_BANISH）は engine の triggeringCardNum 経路の実機検証推奨**（golden で resolveDynamicFilter 解決自体は担保・未配線でも silently no-op＝回帰なし）。**次の一手＝(A)Opus＝§4タスク2 残（動的比較）の別基準サブファミリ**：lastProcessed（そのシグニ＝場に出た/公開した/この方法でトラッシュに置いた＝WXK10-031/WXDi-P08-031 等・⚠applyToField が配置シグニを lastProcessedCards 未設定＝engine 設定もセット）＋printed（表記パワー＝per-candidate 実効/表記比較の新機構）＋lrig相対（WX19-042/WXK11-003）＋残る高シグナル23（「対応語彙なし」＝`docs/_vocab_census.txt` 13行目）。**(B)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）の GRANT_QUOTED_AUTO_ABILITY 内側parse。(C)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43の6枚＋続き44の2枚の実機確認）。(D)verifyBattleDrive バッチ状態汚染の根本修正は続き39から持ち越し。**
