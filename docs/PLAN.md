...
| (lxxxiii) | **第2波まで一部消化（2026-08-02）**＝第1波は live 45効果是正。B候補20は分類誤り2（置換／起動能力）を除く18件を collector 経路まで監査し、真のトリガー主語12件／action対象6件へ訂正。live **6効果**へ `triggerFilter.excludeSelf` を実装、専用条件で既に正しい1件は不変。**残B5**＝curated/MANUAL保持または発生源情報欠落で honest defer。action対象6件は残Aへ戻し、**残A20・残B5・C16・owner未是正8**。詳細と全IDは BUGFIXES 2026-08-02 (lxxxiii) 第2波節。 |
