...
- **🆕 セッション（2026-07-12・続き75・Opus 4.8・§3 Opusタスク12＝Sonnet が積んだ実機バグ5件を一括修正・実機10シナリオ全PASS）**＝続き70〜74 で Sonnet が §7 実機検証中に発見し「観測のみ記録」して Opusタスク12（常設受け口）へ積んだ5件を消化。**engine 2件**＝①**ON_TARGETED の usageLimit《ターン1回》が毎回発火**（`collectTargetedTriggers` だけが `usedOncePerTurnIds` を返さず呼び出し元が `actions_done` へ書き戻せない既存パターンの適用漏れ＝戻り値を `{entries,usedHostIds,usedGuestIds}` へ拡張し `mkLimitOk` に統一）／②**ON_CHARM_TO_TRASH がバトルバニッシュ経路で未発火**（`resolvePendingSigniBattleFor` が独自トリガーリストを組み `collectCharmToTrashTriggers` を呼ばない＝**実戦で最頻の経路**が丸ごと未配線。バトル前後の `countCharmsToTrash` diff に対する収集を追加）。**parser 3件**＝③**GRANT_KEYWORD の「あなたの他のシグニ」が自分自身に付与**（parser が `excludeSelf` を出さず engine も未実装＝両方を実装）／④**REMOVE_ABILITIES の owner が原文と真逆**（WX25-P2-055＝能力消去規則が同一文に残るトリガー句の「対戦相手」を拾って `owner:'opponent'` に化け、**相手の能力を消す**効果になっていた＝自己参照を先に確定させ `thisCardOnly` へ。⚠**引用付与文はガード**＝内側の「このシグニは能力を失う」は付与される能力の中身なので触らない。これが無いと WXDi-P10-001 の `count:'ALL'`→`1` 退化が起きる）／⑤**ON_LRIG_GROW の「あなたのターンの間」ゲート未実装**（parser が前置きを抽出せず、相手が自分のターンに通常グロウするだけで**毎回誤発火**＝ON_LEAVE_FIELD と同型の抽出を追加。**横展開で WXDi-P03-039／P03-046／P05-010 も同時是正**）。**parser 影響を全数機械測定＝13枚**（変更前後の parser 出力を全5973枚でダンプ比較→全13枚を原文照合。excludeSelf 7／turnOwner 4／thisCardOnly 2）。**JSON 反映**＝build:effects 自動採用10枚＋heldReview 採用1枚＋MANUAL温存2枚を effectId アンカーで外科パッチ＝**curated が動いたのは意図した13枚のみ**（HEAD とのカード単位機械diffで確認）。**検証**＝`npm run gates` 全緑（**golden 190/190**〔+3〕・smoke/fuzz 全0・**census 1557 維持**・lint 0 errors）／`regen` で同型★0・逆翻訳原文一致／**実機10シナリオ全PASS**（`ontargeted`〜`ontargeted5`・`ontargetedUsageLimit`・`charmToTrash`・`charmToTrashBattle`・`lrigGrowAnyOpp`・`lrigGrowAnyOppP03046`・バッチ実行でも回帰なし）。`ontargetedUsageLimit`/`charmToTrashBattle` を既定 `order` に追加。詳細 BUGFIXES 続き75。
  ...
  - **次の一手＝(A)Opus＝§3 Opusタスク12 の残1件＝続き70の「多段インタラクションSEQUENCE 途中ラウンドの盤面差分トリガー見逃し」**（`handleEffectInteraction` の `!result.done` 分岐が `collectBoardDiffTriggers` を呼ばない＝**stack 未確定の時点でどこまで確定的にトリガーを積んでよいかの設計判断が要る**唯一の残件。再現＝`node scripts/verifyBattleDrive.mjs onPlayAnyOpp`）。**(B)Opus＝Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY 内側parse**（WX24-P1-017／WX25-P3-038＝引用付与の本丸。本セッションで「引用付与文は自己参照ルールを効かせない」ガードを入れた箇所＝WXDi-P10-001/P09-038 の平坦化も、この機構が入れば正しく表現できる）。**(C)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め**（続き71のパターンA〜F）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②・R43②・R44②③ 他多数。**ON_TARGETED①③系列・ON_LRIG_GROW②・R42②は完了**）。**(E)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（driverバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
