...
- **🆕 セッション（2026-08-11・続き424〜426・Opus 5）＝§6.4 を3本消化。「型は正しいのに読む側が居ない」「主語も極性も反転」「入口の形が違うだけで変換が届かない」。golden 1771→1774・census 854 据置**。
  - **🔴① 続き424＝CONTINUOUS の `FORCE_SIGNI_ATTACK` は engine のどこからも読まれていなかった**＝強制アタックには軸が2本（ターン限定フラグ／印字・付与の【常】）あり、**CONTINUOUS は `executeAction` を通らない**のでフラグは永久に立たない。受け皿（フラグ・警告モーダル・バナー・ソフトロック回避）は**全部あった**のに、この1点で印字【常】6カードが恒久 no-op。`resolveForcedSigniAttack` に2軸を一本化。
  - **🔴② 続き424＝`manualEffects.ts` に効果を「足す」と live に永久に届かない（第4の死角）**＝richness ガードが「effectId 集合が変わるカードは丸ごと温存」なので *新規 id の追加*だけが黙って捨てられていた。**手書き効果3件が長期欠落**（`WXK04-003-DECORE`＝【デコレ】そのもの ほか）。
  - **🔴③ 続き425＝「対戦相手は手札をN枚捨ててもよい」を自分が捨てていた**＝part1 の規則が**先頭アンカー無し**で主語を見ていなかった（3効果が自傷。`WXDi-P09-064-E1` は引くのも自分で完全反転）。**engine は `opponentHandDiscard` を最初から持っていた**。
  - **🔴④ 続き425＝`OPPONENT_PAY_OPTIONAL` の極性が1種類しかなかった**＝(a)「〜しないかぎり、X」（65効果）と (b)「〜てもよい。**そうした場合**、X」（2効果）が同じ形に落ち、(b) は**意味が真逆**だった。`thenOnPay` を新設。**極性は次の文にしか現れない**＝文単位の規則では原理的に判定できない。
  - **🔴⑤ 続き426＝任意コスト脱落トリップワイヤの🔴側を 7→0**。残っていた3件は**入口の形がそれぞれ違うだけ**＝(i)**二段の任意**で2段目の `OPTIONAL_COST` を見たガードが「変換済み」と誤判定（`WXDi-P10-039-E2`）(ii)トップが `CONDITIONAL`＋「**《青》を支払い**手札を２枚捨ててもよい」の複合コスト＝**《青》が丸ごと脱落**（`WXDi-P14-044-E1`）(iii)`CHOOSE` 選択肢の内側で**別の選択肢の強制手札捨て**にガードが引っ掛かる＝**手札が2枚無くてもライフをクラッシュできた**（`WXDi-P14-002-E1`）。
  - **⭐⑥ 「計器の網の外」を原文の文型で数え直す**＝`SPDi43-06` はどの worklist にも載っていなかった（トリップワイヤは「素の手札 TRASH」しか見ないのでコストが《無》のこの札は掛からない）。live 全 `OPPONENT_PAY_OPTIONAL` を原文で仕分けて母集団を2件と確定させてから直した。
  - **⭐⑦ 後段の外科パッチは前段の変換を巻き戻す／採用ツールのガードは退化を止める**＝前者は `applyResultConditionalWave3`（`WX25-P2-082`/`100`）、後者は `censusManualDrift --adopt` の timing ガードが `WX04-004-E2` の採用を中止した（**live の MANUAL のほうが正しかった**＝専用 timing `ON_OPP_SIGNI_ATTACK_DIRECT`）。**fresh が常に新しいとは限らない。**
  - **✅⑧ ゲート**＝`npm run gates` 全緑（golden **1774（+3）** / smoke 全0・SKIP 0 / fuzz 全0 / census **854 据置** / lint 0 errors）。`EFFECT_TYPE_MISSING_CONTINUOUS` **1→0**、`FORCED_HAND_COST_KNOWN` の🔴側 **7→0**。`census:stubs` A群 16→18件（増分は明示 defer 2件＝**無言 no-op は 7件のまま**）。**要実機検証9件**（強制アタック3／相手側 CHOOSE 3／新規 pay-skip モーダル3）は §7 に登録済み。
  - **📋⑨ 次の一手**＝**Opus 側**＝(a)**§6.4 A群の着手可能2件**（`ASSIST_LRIG_ATTACK_THIS_TURN`＝アシストルリグのアタック経路そのもの／`UNDER_CARD_AS_ENERGY_COST`＝**支払いUIの経路が15モーダル超**で要設計）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**残 A群17件の手札コスト変換**(e)`WXDi-P14-002` の可変 `choose_count`（「センタールリグのレベル１につき１つまで」）。**Sonnet 側**＝§7 実機検証（**続き424〜426 を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き424〜426（§6.4 強制アタック機構／手書き効果の新規追加の死角／「対戦相手は〈コスト〉てもよい」の主語・極性／任意コスト脱落トリップワイヤ残0）後 最新値（本行が直近の正）**：census **854**、golden **1774（+3）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**（7→0）、smoke 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、`census:stubs` A群 **18件**（明示 defer 11・**無言 no-op 7**）、UNKNOWN **36ノード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
