...
- **🆕 セッション（2026-08-14・続き479・Opus 5）＝**§7 V-14 を決着**＝**Codex 起案14本 → 実機で13本が緑（2回連続 ALL PASS）**。赤1本は**engine 実バグの再現用**（→§3 **(cxxxiv)** 新規登録）。engine/parser/JSON 非改変・ゲート据置（golden 1976・census 831・lint 0/259）。
  - **✅① 4項目すべて実機で確認**＝裏向きフリップバック3本（解決直後は `facedown_signi` に居る／相手のシグニも戻る／元ゾーン占有ならトラッシュ）・解除コストつきアタック制限3本（2体だけがトラッシュ／1体に減らした対照ではボタンが出ない／CPU は左から決定論的）・多重アクセ4本（2枚不発・3枚で発火・2枚目ボタンの enabled/disabled 対照）・付与の寿命3本（`permanentGrant` は END を跨ぐ／`UNTIL_OPP_TURN_END` は CPU END で失効）。
  - **🔴② 新規実バグ (cxxxiv)＝旧 `signi_acce`（string）でゾーンが壊れる**。`acceCardsAt` が string をそのまま返し `.length` が枚数になる＝1枚でも「3枚以上」が真。🔴**実機の実害は誤判定に留まらず、トラッシュへ1文字ずつ展開された**（`["W","D","1","8",...]`）。**Codex が起案段階でソースから発見**し、再現シナリオを赤で置いた。
  - **🔑③ 赤5本の真因は全部「観測側」だった**＝(a)**Playwright の accessible name は全角スペースを ASCII へ畳む**ので、**データと regex がコードポイントまで一致していても掴めない**（📌17）(b)**場のシグニは `StackModal` を開く**が `card-detail-modal` は CardModal 専用（📌18）。**どちらも「画面には出ているのに掴めない」型**で、**決め手は final.png**（📌10）。⚠**私の第一仮説（フェイズ注入が戻される）は外れ**＝repatchTop は決定論性として残したが主因ではない。
  - **🔑④ Codex 委譲の所見**＝**追加 testid 0件・既存不変を SHA-256 で二重証明**（先頭〜`V-13 END` と V-13 ブロック単体）＝§5-22 の要求に完全準拠。指示書の行番号ズレも自分で訂正（通算14回目）。⚠**ただし「実行できないと詰め切れない部分」は今回も残った**＝赤5本はすべて UI セレクタの実挙動に依存し、**ソースだけでは決まらない**（続き436 の結論と同型）。
  - **▶ 次の一手**＝**Part C（V-15〜V-17）／Part D（V-18〜V-19）の指示書は作成済み**（`codex_taskC.md`／`codex_taskD.md`・scratchpad）。**投入前にベースラインと HEAD を取り直す**こと。§3 の在庫は **5件**（(cxxxi)(cxxxii)(cxxxiv)＋続き404・408 の2件）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き479（§7 V-14 決着）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1976 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（engine/parser/JSON 非改変。UI は `stack-detail-modal` の属性追加のみ）。🆕**実機シナリオ＝+14本**（V-14）＝**緑/赤の内訳が 80緑0赤 → 93緑1赤**（赤1本は §3 (cxxxiv) 待ちで既定 order に置いてある）。🆕**Opusタスク12 の在庫＝5件**（(cxxxiv) を登録）。
