...
| **O-2** | **`SEARCH` pending の相手応答ルーティング** | 3効果 | `BattleScreen.tsx` の `respondPlayerId` は **`SELECT_TARGET` と `CHOOSE` しか見ない**ので `SEARCH` が相手へ回らない。`owner:'opponent'` を載せると**自分が相手のデッキを覗く**別物になる。⇒ これを外せば **`DEFERRED_OPPONENT_DECK_REVEAL_FIELD_REFILL`（`WXEX2-84-E2`）／`DEFERRED_EACH_PLAYER_ZONE_RESET_AND_DECK_REFILL`（`WXDi-P01-026-E1`）／`SP38-006-E1`** が同時に動く |
