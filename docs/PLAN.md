...
**直近＝2026-09-20＝🏁§5.7 `S-9`＝強さの A/B 測定台（席ごとのポリシー・席入れ替え・勝率の誤差幅）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**`npm run selfplay:ab -- --a <名> --b <名> --games N [--jobs N] [--first host|guest]`**＝①`cpuPolicy.ts`（既定値の在処を1つに）②**1シード＝2戦の席入れ替え** ③Wilson 95% ＋子プロセス並列（**8ジョブで約7.0倍**）。
- **検算3本**＝決定論（同シードで完全一致）／`default` 同士で**ちょうど 50.0%**／ポリシーが実戦を変えることをトレースで確認（332→328手）。⚠**勝率だけ見ても「配線が生きているか」は分からない**（席の偏りに埋もれる）。
- 🔴**測ってすぐ出た所見＝先攻 28.1% [18.6, 40.1]**（64戦・確定）。**`--first` を反転して切り分け**＝偏りは**手番**について回り、**host 席 40.6% [29.5, 52.9]** は 50% を含む ⇒ 🏁**`S-5` の席の鏡は無罪**。後攻有利は `S-19` へ登録。
- 🔑**席入れ替えは必須**＝片側だけで測ると**±22ポイント**の下駄が乗り、現実的なポリシー改善（数ポイント）を完全に埋める。
- 🔴**要る対戦数**（Wilson 95%・1戦 ≒ 33秒・8並列）＝70%:24戦(2分)／65%:44(3分)／**60%:96(8分)／55%:370(29分)** ⇒ **小さい改善を勝率で測るのは桁で高い**（`census:play` やログの回数で見る）。
- ついでに削除＝`performSummonSigni.ts:58` の `console.log`（**実機でもシグニを出すたびに出ていた**・`S-5c` の置き忘れ）。
- 🔴**ついでに直した②**＝`behaviorAudit.ts` が**決定論ではなかった**（`setRngSeed` を呼んでいなかった）＝`_census_trace_invariants.txt` が**ソース未変更の回でも差分を出していた**（docs だけの commit `b2c5292f0` で18行）＝**退化をノイズから見分けられない状態**。seed 固定 ⇒ 連続2回の出力が md5 まで一致。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-10`**＝盤面の採点のパワー項（探索より前＝探索は評価関数を増幅する）。🆕**`S-19`（先攻が勝てない）はどこへでも差し込める**＝直すと A/B の分散が減るので早いほうが得。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 8**（`S-10`・`S-15`〜`S-19`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先読みの情報漏れ（`simulateEffect` が本物の山から引く＝`S-16` の前提）②**先攻が勝てない 28.1%**（`S-19`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4332**＝+1）｜**実機** `node scripts/verifyFullMatch.mjs cpu` **PASS**（6ターン / 146手 / 197秒で決着） |
...
| `S-10` | **盤面の採点のパワー項**（ユーザー指摘「パワーを上げる効果を過剰に使う」）＝同じ +3000 を**使う前は 750**（`cpuCardStrength` の `powerUp: 0.25`）・**使った後は 3000**（`evaluateBoard` が実効パワーを係数1.0 で加算）と**4倍違う値**で数えている。パワーの価値は本来**閾値関数**（バトルの勝敗が反転するか・除去圏を跨ぐか）で、線形のままでは係数をいくら下げても「十分大きいバフは常に得」が消えない。 | 小〜中 | パワー修正の過剰使用が止まる。⚠`evaluateBoard` は全判断の土台＝`S-9` が先 |
| 🔥`S-15` | **候補列挙器 `listCpuMoves`**＝いま各 `pickCpu*` は「自分の担当の中から**1つ選ぶ**」形で、**そのターンに取れる手を1つのリストにする関数が無い**。可否は既存 gate のまま（§5.6.3）、**「選ぶ」を「列挙する」へ割る**（エナチャージ・グロウ・召喚・【起】・場外【起】・ピース・スペル・アーツ）。 | 中 | 探索の前提。⤵**`S-11`・`S-12` はここに吸収**＝別々の閾値・allowlist を4箇所に増やさない |
| 🔥`S-16` | **前半ブロックの探索**＝`ENERGY`〜`ATTACK_ARTS` を**1つの探索単位**にしてビーム探索（目的関数＝**アタックステップに入る時点の盤面**）。⚠**着手前に塞ぐ**＝いまの先読みは**山の順序を覗いている**（`simulateEffect` が実 state を `structuredClone` → `execDraw`（`effectExecutor.ts:382`）が `state.deck.slice(0, n)`＝**本物の次の1枚**を引く）。 | 大 | **手順の入れ替え**と**資源配分**を CPU が検討するようになる（いまはどちらも固定台本）。⤵**`S-13` はここに吸収** |
...
| `S-18` | **終端評価の「次ターン」項**＝`BOARD_WEIGHTS` はいま `hand: 1500`／`energy: 1000` の**線形の枚数**だけで、「次のグロウコストが払えるか」「手札が0か」「【ガード】が残っているか」を見ない。 | 小〜中 | リソース残しが入る（1ply 伸ばして相手ターンを読むより圧倒的に安い） |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9` を消化）**＝**`S-10`（評価のパワー項）** → `S-15`（候補列挙器）→ `S-16`（前半探索）→ `S-17`（後半＝攻撃探索）→ `S-18`（次ターン項）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-20 時点**（第408バッチ＝🏁§5.7 `S-9` 強さの A/B 測定台）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 8**（`S-10`・`S-15`〜🆕`S-19`・`S-14`・`S-6`＝🏁`S-9` を消化し `S-19` を登録）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4332**＝+1・`census:traceinv` I1=0・`selfplay` 1戦 決着）｜**実機** `verifyFullMatch.mjs cpu` PASS
  - 📏**BattleScreen**＝**6,287行**（±0）
  - 🆕📊**自己対戦 A/B**＝1戦 ≒ 33秒／8ジョブで**約7.0倍**（合計1,026秒→壁時計146秒）｜**先攻の勝率 28.1% [18.6, 40.1]**（64戦・`default` 同士）｜host 席 40.6% [29.5, 52.9]
  - 📊**機構踏破**＝`census:play`（`VERIFY_DECK_MECH` 1戦）**10 / 20**（未計測＝CPU の判断は触っていない）
