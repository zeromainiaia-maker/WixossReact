...
- **🆕 セッション（2026-07-13・続き107・Opus 4.8・PLAN §3 Opusタスク6＝ベット「代わりに」置換機構＝IS_BETTING択一＋betChoose選択数変更を新設）**
  ...
  - **✅ タスク3 DRAW脱落 follow-up の subset 消化（SP24-009）**＝(a)「その後、カードをN枚引き、X」の非先頭連用ドロー脱落を DRAW 安全網の `^(?:その後、)?カードを` 拡張で是正（非退化＝DRAW を足すだけ）。(b)`ADD_TO_LIFE` 枚数抽出（`parseSentencePart1.ts`）が同一文の「N枚引き」draw 枚数を誤取得していた（SP24-009 が count:5 の過剰）のを負後読み `(?!引)` で是正＝「デッキの一番上のカード」は count:1。SP24-009 採用（DRAW 5 追加＋life 1 是正）。census 1447維持。**残＝先頭「すべてのシグニを場からトラッシュ」の compound-sentence 脱落（タスク3(c)）は別途**。
