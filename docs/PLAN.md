...
3. **census「動的比較」の残**＝(b)opp/own センタールリグ（WX19-042 は target が LRIG に誤パース＝mis-parse 修正が先／WXK11-003）・デッキ相対 SEARCH（WDK13-013/WXEX2-28/WXEX2-37/WXK10-033）・条件文（WXK08-005）・lrig相対（WXEX2-25-E3）＝いずれも別機構。~~WXK07-025-E2 の DRAW+condition 復元~~ **✅続き59（Opus）で消化**（`の?場合` parser修正＋完全形MANUAL・BUGFIXES続き59第2件。DRAW脱落の parseSingleSentence 直呼び経路 systematic〔19枚・入れ子SEQUENCE〕は follow-up）。自己参照/トリガー参照/designation/anyAlly/printed/lastProcessed 系は続き43-47で✅消化済み（詳細 [PLAN_DETAIL.md](./PLAN_DETAIL.md)・BUGFIXES）。
...
7. **持ち越し済みの engine/parser 拡張の小口**＝WXDi-P03-005（PAID_ADDITIONAL_COST の「置換モード」拡張）・WX26-CP1-100（SEND_TO_ENERGY のトラッシュ対象化）・GRANT_LRIG_ABILITY系5枚の parser ON_PLAY 誤デフォルト修正・WX25-CP1-051/WXDi-CP02-070 の owner:any・excludeSelf 欠落・続き33発見の原文無関係 `TRANSFER_TO_DECK` 混入5枚（WX24-P2-033等）・SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095。**続き64（Sonnet）発見の2件を追加**＝**(a) WXEX2-50-E3 step1 owner誤パース**（原文「対戦相手のトラッシュから…それを対戦相手の場に出す」が `ADD_TO_FIELD.owner`/`source.owner` ともに `self` に誤生成＝カード全体で唯一の「あなたのターンに対戦相手のシグニが場に出る」自然発火経路がブロックされ R30 (ON_PLAY any_opp) の実機検証がこのカードでは不能）。**(b) `execAttachAcce` fromHand経路の実装バグ**（`effectExecutor.ts:3774`＝1段目SELECT_TARGETのthenActionに未完結の2段目ATTACH_ACCEアクションを渡す設計が、resume解決側`applyDirectAction`（1候補選択→即terminal実行の前提）と噛み合わず、手札からACCEカードを選んだ時点で`zoneIdx<0`により無添付のまま完了する＝【デコレ】キーワード全9枚が機能しない。R45①(ON_ACCE_ATTACH host条件)のWXK05-041で実機FAIL確認。詳細は両方ともBUGFIXES最上部・`verifyBattleDrive.mjs`の`acceAttach`シナリオ（`order`未追加）。
...
- **🆕 セッション（2026-07-11・続き64・Sonnet 5・§4タスク(C) §7実機検証の継続＝R42/R30/R45①/R44/ON_TARGETED①の5件）**＝`verifyBattleDrive.mjs` に5シナリオ追加。**①R42（ON_CHARM_TO_TRASH・WX16-Re05）＝✅PASS**（続き61のcollectBoardDiffTriggers統合がcollectCharmToTrashTriggersも一緒にカバー済みだった＝追加修正不要。副次的な学び＝「バニッシュ＝トラッシュ行き」は誤解、`banishDestination`のデフォルトは持ち主の**エナゾーン**行き）。**②R30（ON_PLAY any_opp・WXK10-022-E1）＝実機検証不能と判明**＝唯一の自然発火経路WXEX2-50が`ADD_TO_FIELD.owner`誤パース（自分/対戦相手取り違え）でブロック中＝parser修正待ち（Opusへ登録・§3タスク7）。**③R45①（ON_ACCE_ATTACH host条件・WXK05-041）＝❌FAIL＝実バグ発見**＝`execAttachAcce`のfromHand分岐（thenActionに未完結アクションを渡す設計）がresume解決側`applyDirectAction`の前提と噛み合わず無添付のまま完了＝【デコレ】キーワード全9枚が機能しない（Opusへ登録・§3タスク7）。**④R44（ON_EXCEED_COST場シグニ・WXDi-P06-078）＝✅PASS**＝ルリグ【起】エクシード１支払い時「発動順序を決めてください」モーダルにwatcherが正しく並び発火確認（3回連続）。**⑤ON_TARGETED①個別確認（WXDi-P02-043）＝✅PASS**＝`ontargeted`（WXDi-P03-067）と同一配線で正しく発火（5回中4回PASS・軽微なタイミングフレークはengine起因ではない）。①④⑤は`order`追加、②③は`order`未追加（Opus修正待ち）。**⚠WXDi-CP02-082（R36②）とON_LRIG_GROW②は「相手ターン中の自分の手札捨て／自分ターン中の相手グロウ」という稀な発火条件で自然な発火経路のあるカードが見当たらず断念**（WX14-003の任意コストは`costText`のみで`costColors`が無く実際には手札を捨てないノーオペ、というコード上の疑わしい点も発見したが深追いせず）。guest側盤面注入の稀な競合も発見し安定化（queryState確認→再PATCHリトライ）。詳細BUGFIXES最上部5件。⚠フルスイート回帰は環境側のプロセス過多（検証run由来の残留chrome/node）で著しく低速化したため中断（各シナリオは単体で複数回再現確認済み）。
  - **前回（続き63・Opus）＝effect-restriction（配置数制限）機構の実装**＝「対戦相手はシグニをN体までしか場に出せない」5枚を実装（DEPLOY_RESTRICT count分岐・census 1572→1567）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝続き64発見の2バグ**（WXEX2-50 owner誤パース／`execAttachAcce` fromHand経路の実装バグ＝§3タスク7）。**(B)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(C)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(D)Sonnet＝§7実機検証の継続**（ON_TARGETED残3枚の個別確認・R42②（バトルバニッシュでhostが離脱したとき）・ON_COIN_PAID/ON_LRIG_GROW③④の follow-up。WXDi-CP02-082(R36②)/ON_LRIG_GROW②/R30/R45①は自然発火経路がブロック中か稀なため後回し）。**(E)持ち越し：WX12-008 の exceed-cost timing 誤パース是正・`handleSelectZoneForEffect`/`handleSelectSigniZoneForEffect` の収集器未配線（続き61 follow-up）・SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染・WX14-003の任意コスト(costText型)がcostColors無しで実際にはコスト無視で発動する疑い（未確認・低優先）。**
...
- **ON_TARGETED（対象になったとき）**：AUTO（14枚）。✅発火自体は実UI検証済み（`WXDi-P03-067`）。**①個別確認＝続き64（Sonnet）でWXDi-P02-043を追加検証しPASS**（`verifyBattleDrive.mjs ontargeted2`＝`ontargeted`と同一配線で正しく発火。5回中4回PASS・軽微なタイミングフレークは既存シナリオと同型で engine 起因ではない）。`order`追加済み。**残＝未検証**＝①残る3枚（WXDi-P11-040/WX25-P2-055/WXDi-D09-H14）の個別確認・②turnOwner:opponent ゲート・③usageLimit《ターン1回》が複数対象でも1回。⚠**forced単一対象（pending無しで自動解決される対象取り）経路は未発火＝follow-up**。
...
- **ON_CHARM_TO_TRASH（R42）**：WX16-Re05。**①実機PASS＝✅続き64（Sonnet）で確認**＝続き61のcollectBoardDiffTriggers統合がcollectCharmToTrashTriggersも一緒にカバーしていたため追加修正不要だった。`verifyBattleDrive.mjs charmToTrash`（WX19-023→WX16-Re05・banish resume経路でチャームがguest.trashへ→watcherが対戦相手シグニに-4000）でPASS。`order`配列に追加済み。**残＝未検証**＝②バトルバニッシュでhostが離脱したとき（効果解決経路外＝未検出の可能性）。
