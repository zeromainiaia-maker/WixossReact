...
- 🏁**セッション（2026-08-27・続き691・Opus 5 単独）＝Sheet1 段2 の4巡目。表記パワー0のカードが「場に出た瞬間に自壊」していた。**
  ユーザー指示は前巡と同じ（段2＝Sheet1／`gates` は最後の1回／簿記を圧縮／処理量を増やす）。
  📊**進捗3計器＝Sheet1 要対応 37→35 / 863（4.3%→4.1%）｜台帳 残 OPEN 591→585｜census 高シグナル 526→526（据置）**。
  Sheet1 findings は **20件/15カード → 14件/12カード**。live 変化 **5 effectId / 3カード**。
  - 🔑**`WX11-032`（表記パワー0）は場に出た瞬間にパワー0以下ルールで自動バニッシュされていた**＝
    「自身が持つ色の種類１つにつき＋4000」が catch-all STUB へ落ちて消えていたため。**実機で反転確認済み**
    （旧 JSON へ戻すと 1 tick で場から消える）。
  - ✅**足した受け皿は4つ**＝`POWER_MODIFY_PER_OWN_COLOR`／`GRANT_PROTECTION.sourceSharedColorWithSelf`／
    `triggerCondition.movedToDeckFromField`／`cost.exceedColors`（**ラベル・可否ゲート・支払い実行の3地点**）。
  - 🔴**この巡いちばんの教訓＝計測ハーネスが本番と違う型を渡すと「無い穴」を見つけてしまう。**
    `cardMap` は本番では `InstanceMap`（`get` が `getCardNum` へフォールバックする）なのに、
    テストが素の `Map` を渡していたため「コスト軽減が10効果とも不発」に見えた。測り直すと **9件中8件は
    HEAD で既に動いていた**。速断した修正5箇所は全部差し戻し、**本当に壊れていた1件だけ**を残した（§4.1 追記）。
  - ⚠**census が動かなかった**＝今回の4件はいずれも census の語彙表に無い軸（型の新設・コストの色指定・
    トリガーの由来限定）。§3 の「計器ごとに見えるものが違う」どおりで、失敗ではない。

**▶ 次の一手**＝**同じ取り方で5巡目**（Sheet1 の残 findings は **14件 / 12カード**）。
着手は `npm run census:cards -- --sheet 1 --list` → 該当カードの原文・live・findings を**まとめて読む**。
⚠**残りはほぼ全部が機構待ち**＝§5.3 の `O-105`〜`O-119`。とくに `O-119`（コスト軽減の実体が
`computeArtsEffectiveCost` のカード全文 regex 35分岐に埋まっている）は **`O-60` A群の本丸**で、
`WX10-053` のように**engine では正しく動いているのに JSON からは見えない**効果を大量に抱えている。
---
...
| **O-107**（🗑**取り下げ**） | ~~`REARRANGE_SIGNI` に `optional` が無い~~ | — | **2026-08-27 Sheet1 B11 で誤登録と判明**＝`RearrangeSigniAction.optional` は**型にも engine にも最初から在った**（`execRearrangeSigni` が `PendingInteractionDef.optional` へ渡す）。**parser だけが出していなかった**＝同日に配線して解決（`WX05-046-E1` ほか）。⚠**教訓**＝「受け皿が無い」と判定する前に**型宣言と executor の両方**を grep する（B11 ではここを読まずに機構待ちへ送っていた）。 |
| **O-108** | 🆕**コスト側の選択制約（「それぞれ名前の異なるN枚」）が `handDiscardSigni` に無い** | S〜M（UI を触る） | **2026-08-27 Sheet1 B10 で分離**＝`WX10-052-E3`「【起】手札から**それぞれ名前の異なる**＜精元＞のシグニを４枚捨てる：無色ではないすべてのシグニをトラッシュに置く。」■`EffectCost.handDiscardSigni`（`src/types/effects.ts:545`）は `{color, story, count, level}` だけで `selectionConstraint` を持たない（**エナ側の `energyTrash` は持っている**＝`:543`。§5-8′ の「兄弟枝にはある」型）。■**取り方**＝型に `selectionConstraint?: SelectionConstraint` を足し、**`SigniActivatedModal` と `LrigGrantedModal` の両方**の `canConfirm` で `distinct:'name'` を強制する（`inferDistinctKind`／`distinctConstraintOf` は既存）。⚠**片方だけ直すと入口によって払えたり払えなかったりする**。■**実機検証が要る**（モーダルの確定ボタン条件を変えるため）。 |
...
| **O-111** | 🆕**スペル自身の使用コストを「〈条件〉1体につき《無×N》減る」＝枚数参照で軽減する受け皿が無い** | S〜M | **2026-08-27 Sheet1 B11 で分離**＝`WX10-053-E1`「このスペルの使用コストはあなたの場にあるカード名に《サーバント》を含むシグニ１体につき《無×2》減る。」■`CostReductionAction`（`src/types/effects.ts:1887`）は `reduction: EnergyCost[]` の**固定値**しか持たず、**盤面枚数を参照する軽減が表せない**。しかも原文は「**このスペルの**使用コスト」＝**使用時に効く**もので、解決時アクションの `COST_REDUCTION`（次のスペルを軽くする）とは**そもそも別軸**。■🔴**同日に2つの誤表現を撤去した**＝(a) engine に消費地点が無い全文 STUB `SPELL_COST_REDUCTION_BY_SERVANT_COUNT`（文全体を飲み込む no-op） (b) manual の `COST_REDUCTION{スペル,無×2,PERMANENT}`（**以後のスペル全部が永続で軽くなる嘘の効果**）。**いまは実行時マーカー `STUB{ARTS_COST_REDUCTION_BY_EFFECT}`（明示 no-op）**＝過小だが嘘は無い状態。■**取り方**＝使用コスト計算側（`spellUseGate` / `SpellCastModal`）に「盤面カウント×単価」の軽減を渡す口を1本。⚠**着手時に「１体につき《無×N》減る」を全 CSV で数え直す**。 |
...
| **O-115** | 🆕**手札コスト `handDiscardSigni` に「それぞれ名前の異なる」制約が無い** | M | **2026-08-27 Sheet1 B12 で分離**＝`WX10-052-E3`「手札から**それぞれ名前の異なる**＜精元＞のシグニを４枚捨てる：無色ではないすべてのシグニをトラッシュに置く」。■live は `cost:{handDiscardSigni:{count:4,story:"精元"}}` だけで、**同名4枚でも払える**（原文より軽い＝踏み倒し）。■受け皿は**対象側には既にある**（`selectionConstraint.distinct:"name"`／`HAND_COUNT_FILTER.distinctName`／`HAS_CARD_IN_FIELD.distinctNames`）＝**コスト側にだけ無い**。■🔴**消費地点が4つある**（§4.2 の「3地点を grep」の拡張）＝`signiActivateGate.ts` の可否判定／シグニ【起】モーダル／`LrigGrantedModal.tsx`／`trashActivateCost.ts`。**1つでも漏らすと「提示は絞られるのに支払いは通る」**。■母集団＝「それぞれ名前の異なる」は 22効果あるが、**コスト側はこの1件だけ**（残りは対象選択・条件で、すべて受け皿あり）。 |
| **O-116** | 🆕**`ON_CARD_MOVED_TO_DECK` の由来限定が「トラッシュから」しか無い** | M | **2026-08-27 Sheet1 B12 で分離**＝`WX05-019-E3`「対戦相手のシグニ１体が**場から**デッキに移動したとき」。■`triggerCondition` は `movedToDeckOwner` / `movedToDeckMinCount` / `movedToDeckFromTrash` の3つしか持たず、**「場から」も「シグニであること」も表せない**＝相手のデッキへ何が戻っても発火する過剰実行。■**取り方**＝`boardDiff.ts` に `countMovedToDeckFromField` を足し（before の `field.signi` スタックに居た札を数える）、`movedToDeckFromField` / `movedToDeckFilter` を collector が読む。■⚠**`movedToDeckFromTrash` と同じ形にする**（キー名も評価順も揃える＝別実装にすると片方だけ直る）。■⚠**2026-08-27 に「〈ゾーン〉から場に出たとき」で同型の穴を直した直後**（B8）＝**由来限定は timing ごとに別実装で散っている**ので、他 timing も同時に数えること。 |
...
| **O-119** | 🆕🔴**カードの使用コスト増減の実体が `computeArtsEffectiveCost` のカード全文 regex に埋まっている（`O-60` A群の本丸）** | L | **2026-08-27 Sheet1 B13 で実測**＝原文に「この〈スペル/アーツ/カード/ピース〉の使用コストは…」を持つ効果は **142件**、うち**ゾーン枚数比例が35件**。■**JSON 側は全部 `STUB{ARTS_COST_REDUCTION_BY_EFFECT}`（no-op マーカー）**で、実際の増減は `costs.ts` の**カード全文 regex 分岐（A/C/D/H/I-1〜I-5…）**が担う。■🔴**この形は JSON を見ても何が起きるか分からず、逆翻訳・census・golden・smoke・fuzz が全部緑のまま意味が壊れる**（B13 の `WXK06-055` は「色名が `黒×1` のまま」で**一度も軽減されていなかった**のに、どの計器にも出なかった）。■**`WX05-019`／`WX10-053` の段2 findings が「JSON に無い」と言っているのはこの構造そのもの**＝`WX10-053` は**engine では正しく 無×7→無×1 に軽減される**が JSON からは見えない（＝閉じられない）。■**取り方**＝`cost.costScaling?: {direction, countZone, countOwner, filter, per, amount}[]` を parser が刻み、`computeArtsEffectiveCost` は**payload があればそれを使う**（regex はフォールバックとして残す）。⚠**increase 側は部分採用禁止**（1項でも読めなければ丸ごと据置＝安くしてはいけない）。reduce 側の部分採用は「重いまま」＝安全側。 |
