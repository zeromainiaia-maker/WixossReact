...
- **🆕 セッション（2026-08-11・続き428〜431・Opus 5）＝§6.4 を4本＝**エナ支払い元の一本化**／**離場置換の前提訂正＋決定層分離**／**離場置換の対話化**／**ライフクラッシュ置換の funnel**。golden 1776→1800・census 854 据置・`MANDATORY_SUSPICIOUS` 1→0**。
  - **🔴① 続き428＝「エナコストの支払い」がどこにも一本化されていなかった**＝支払いモーダル14本が `my.energy` を直読みし、候補生成も控除も **BattleScreen の14サイトに散在**。**半分だけ配線した瞬間に「アーツには使えるが【起】には使えない」無言の不整合**になるので funnel を先に作った（`UNDER_CARD_AS_ENERGY_COST` をその最初の消費者として実装）。
  - **⭐② 安全弁は関数の戻り値に置く**＝`applyTo` を呼び忘れると**エナが1枚も減らない**＝即座に露見する壊れ方になる。**無言にならない壊れ方を選ぶ**のが設計の本体（教訓 (m) の実践）。
  - **🔴③ 続き429＝§6.4 の登録内容そのものが間違っていた**＝「置換5本はすべて『してもよい』の自動適用＝近似」ではなく、原文全数照合で**強制3効果（2軸）／任意10効果（3軸）**に割れる。**強制軸を対話の選択肢に出すとルール違反**なので、原文の語尾から分類を機械ロックする golden を置いた。
  - **🏁④ 続き430＝離場置換の対話化＝10カードの「してもよい」が実際に選べるように**。🔑**離場ループの途中では pause しない**＝決定を **`PlayerState.leave_substitute_choices`** に刻んで**適用前に全部聞く**（PlayerState なので pause を跨いで残る）＝**engine に mid-loop resume を新設せずに済んだ**。
  - **🔴⑤ `opponentResponds` の CHOOSE は「相手が支払う」へ固定ルートされていた**＝コストの無い問いを流すと「エナ不足」で即終了＝**無言で潰れる**。⭐**`opponentResponds` は「誰が答えるか」であって「相手が払う」ではない**（続き411 の教訓の別形）。
  - **🔴⑥ 続き431＝ライフクラッシュ置換3枚は「未実装」ではなく生きた過剰効果／自傷だった**＝原文の「**代わりに**」を読み落として即時実行に化けていた（`WX24-P4-009` は使った瞬間に**自分のデッキが10枚**削れ、`WX25-P3-004` は**タダで相手のライフを1枚**割り、`WXDi-CP01-023` は CONTINUOUS で恒久 no-op）。⚠**parser のすぐ隣に同じ注意書きがあったのに、文型が違うと効かなかった**（続き25 の再発）。
  - **🔴⑦ 副産物＝既存8カードの発生源限定が効いていなかった**＝`REPLACE_NEXT_DAMAGE_WITH_MILL` は `damageSource` を宣言していたのに捨てており、`WX25-P1-010`（「シグニによって」限定）が**ルリグアタックのダメージまで置換**していた。
  - **⭐⑧ 原文の注記が自動適用の安全弁になっていることがある**＝「デッキがN-1枚以下の場合は置き換えられない」を実装してあるので、`optional` を自動適用してもデッキアウトの自傷が構造的に起きない。**「近似で置いてよいか」は原文の注記まで読んでから決める。**
  - **✅⑨ ゲート**＝`npm run gates` 全緑（typecheck / golden **1800（+24）** / smoke 10688 全0・**SKIP 0** / fuzz 全0 / census **854 据置** / census-stubs（明示 defer **17→16**・無言 no-op **0**）/ manual-fields / lint 0 errors）。`npm run regen` 済み。**要実機検証10件**（続き428 の4／続き430 の3／続き431 の3）は §7 に登録済み。
  - **📋⑩ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**置換の対話化の残**＝ライフクラッシュ置換の「してもよい」辞退（離場置換 M2 と同じ枠組み）(e)**離場置換の latent**＝`powerReduction`／`replaceBanish`／`noAbilityDeckBottom` の宣言走査が**印字だけ**で付与ストアを見ていない（現状 live 0件で無害）。**Sonnet 側**＝§7 実機検証（**続き431 の回帰確認＝「使った瞬間に何も起きない」を最優先**、次いで続き428・430）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き428〜431（§6.4 エナ支払い元の一本化／離場置換の前提訂正・決定層分離／離場置換の対話化／ライフクラッシュ置換）後 最新値（本行が直近の正）**：census **854**、golden **1800（+24）**、**`MANDATORY_SUSPICIOUS` 0**（`EFFECT_TYPE_MISSING_CONTINUOUS` も **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
>
> 0. 🎯**次の一手（2026-08-11 続き431 更新）＝§6.4 の残**＝(a)~~離場置換の対話 policy／`WXDi-CP01-023`~~ **✅続き430〜431 で完了**（残は**置換の「してもよい」辞退**＝ライフクラッシュ置換側と、latent＝離場置換3軸の宣言走査が付与ストアを見ていない）。(b)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)**残 A群16件の明示 defer 見直し**（§6.3 の機構が進んだものから解ける）。**Sonnet 側＝§7 実機検証（続き428 のエナ支払い回帰確認が最優先）**。以下は旧「次の一手」（参考）。
>
...
- **UNKNOWN（部分未実装・逆翻訳に `【未実装/UNKNOWN】` として露出）**：**🆕2026-08-10 続き414 に live JSON を全数走査して数え直した実数＝40 ノード／38 カード**（⚠**旧記述「42箇所／31カード」は decompile シートの出現数**で、live のノード数とは別物＝**計器は live JSON を数える**）。**続き414〜415 で 43→38**。
  ...
  - **📋 残り36ノードの内訳（続き418 実測・大きい順）**＝(a)**デッキ公開→処理の変形 2件**（`WX21-028-E1`＝「その中から**すべての**＜天使＞のシグニをエナゾーンに」＝**枚数語が名詞より前**で `parseRevealPickDescriptor` の語順に合わない／`SP38-006-E1-G2`＝主語が対戦相手で1文に「見る」も含む）(b)残りは**単発**（1カード1機構）。
...
- **離場置換の対話化**（続き406 登録 → **2026-08-11 続き429 で前提訂正＋決定層を分離**）
  ...
  - ⚠**latent な穴（現状 live 0件なので無害）**＝`powerReduction`／`replaceBanish`／`noAbilityDeckBottom` の宣言走査は **`cardMap.get(top)?.effects`（印字）だけ**を見ており、付与ストアを見ていない（`applyEffectBanishSubstitute` だけが `effectsMap`→`cardMap.effects` のフォールバック鎖を持つ）。**これらを `GRANT_EFFECT` で付与するカードが出た瞬間に無言で落ちる**＝3軸走査へ揃えるのは M2 と同時が安い。
...
- **🆕 続き427（アシストルリグのアタック機構）が持ち込んだ未検証UI 3件（最優先）**＝**engine 側は golden で固定したが、アタック宣言・ガード応答・CPU は `BattleScreen` にしかなく golden では原理的に踏めない**。前提＝`WX25-P1-048`（ピース）を使う＝**場にルリグ3体で合計3色以上**が要る。
  - [ ] 🔴**ピースを使う前はアタックできない**＝ATTACK_LRIG フェイズでアシストルリグをタップしても「アタック」ボタンが**出ない**（通常ルール）。⚠出たら回帰（常時アタックできる過剰実行）。
  - [ ] **使った後にアタックできる**＝`WX25-P1-048` を使ったターンだけ、レベル1以上の**未ダウンの**アシストにアタックボタンが出る。押すと**そのアシストだけがダウン**し（センターは巻き添えでダウンしない）、**対戦相手にガード応答が出る**。ガードしなければライフ1枚クラッシュ。左右2体とも順にアタックできる。
  - [ ] **フェイズ進行の確認とCPU**＝未アタックのアシストが残っている状態で「次へ」を押すと**スキップ確認**が出る（強制ではないので警告ではない）。CPU 側が `WX25-P1-048` を使った場合、CPU がセンター→アシストの順にアタックしてくる。⚠**ソフトロックしないこと**（アタックできないアシストしか残っていなければ確認なしで進める）。

- **🆕 続き426（任意コスト脱落トリップワイヤの残0化）が持ち込んだ未検証UI 3件（最優先）**＝**pay/skip モーダルが新たに出る**3カード。golden は構造を固定したが提示・徴収は踏めない。
  - [ ] 🔴**踏み倒しが塞がったか**（`WXDi-P14-002` の選択肢④＝「手札を２枚捨ててもよい。そうした場合、対戦相手のライフクロス１枚をクラッシュする」）＝**手札が1枚以下のとき「支払う」が選べない**（`canAfford`）。⚠**手札が足りないのにライフがクラッシュしたら回帰**（旧実装がまさにそれ）。併せて**【使用条件】**（場のルリグ3体で合計3色以上）を満たさないとピース自体が使えないことも見る。
  - [ ] 🔴**《青》＋手札2枚の複合コスト**（`WXDi-P14-044`＝アタックフェイズ開始時）＝pay を選ぶと**エナから《青》1つと手札2枚の両方**が減る。⚠**《青》が減らなければ回帰**（旧実装はコスト句ごと脱落していた）。スキップすると相手シグニはデッキの一番下へ**行かない**。
  - [ ] **二段の任意**（`WXDi-P10-039`＝このカードが捨てられたとき）＝①「手札を1枚捨てる／スキップ」→②払った場合だけ「《青》《無》を支払う／スキップ」→③払った場合だけトラッシュから場に出る。⚠①をスキップしたら②が**出ない**こと。⚠「そのターン終了時」の遅延は未実装（②③が即座に来て正しい）。
