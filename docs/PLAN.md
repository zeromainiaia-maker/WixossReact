...
- **🆕 セッション（2026-07-07・続き35・Sonnet 5・同日第7ラウンド＝§3割付タスク②の続き＝B層 REVEAL_AND_PICK残タスク(d)CHOOSE内包＝最終ラウンド）**＝分類(d)2件（WXDi-P10-004/WX26-CP1-100）に着手。**WXDi-P10-004を採用**＝3択2つ選ぶCHOOSEのうち選択肢①「デッキ上5枚見て＜プリパラ＞のシグニ2枚まで手札に加え、その後手札から＜プリパラ＞のシグニを好きな枚数場に出す」を`SEQUENCE[REVEAL_AND_PICK{...pickCount:2,pickUpTo:true}, ADD_TO_FIELD{source:{type:HAND_CARD,count:'ALL',filter:{cardType:シグニ,cardClass:プリパラ}}}]`に是正（選択肢②③は既に正しくエンコード済みだったため無変更）。**WX26-CP1-100は見送り**＝「あなたのトラッシュからレベル1の＜プリオケ＞のシグニを2枚まで対象とし、それらをエナゾーンに置く」という選択肢が、`SEND_TO_ENERGY`アクション（`effectExecutor.ts`の`execSendToEnergy`確認済み）は`fieldCandidates`のみに対応しトラッシュを対象にできず、既存のcorpus全体を検索しても「トラッシュ→エナゾーンの対象指定移動」パターンが1件も存在しない＝engine側に該当メカニズムが未実装と判断（新規action型の追加が要る）。副次的に「【常】：このカードはすべての領域で＜プリオケ＞として扱う」の表現方法（`STUB{id:'TREAT_AS_CLASS_ALL_ZONES'}`＋`collectTreatAsClassAllZones`がEffectTextを正規表現で読む既存機構）は特定できたため記録。**全ゲート緑**＝typecheck・golden 141・smoke 全0（OK10317）・fuzz 全0・lint 0 errors・同型★0（sheet8再生成）・census **1655→1654**（`BASELINE_HIGH`／本§恒久指標 更新済み）。詳細 BUGFIXES 最上部。**これで続き33由来のB層REVEAL_AND_PICK/LOOK_AND_REORDER残タスク（分類a〜d）はすべて着手完了**＝残るはWXDi-P03-005（PAID_ADDITIONAL_COST置換モード）とWX26-CP1-100（トラッシュ→エナゾーン対象指定移動）のみ、いずれもOpus向けengine拡張が前提。**次の一手＝§3割付タスク①（§5b逆翻訳テールSTUB意味文化）または他のSonnetタスクへ移行が候補（B層タスクはこれで一区切り）。**
