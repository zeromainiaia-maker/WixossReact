...
- **🆕 セッション（2026-07-16・続き159・Opus 4.8・PLAN §3 Opusタスク12(xxii)／続き158「次の一手」＝盤面状態条件節の parser 未 emit を是正）**
  - **4枚採用・census 2098→2096・IS_MY_TURN化 119→111**。続き158 の続き。engine evalCondition・decompiler condJa は既対応なのに `STATE_CONDITION_CLAUSES` に語彙が無く条件節ごと脱落していた盤面状態4種を追加（前段記録に依存しない独立ゲート＝`parseHoistStateCondition` 経由）：`あなたの場にレゾナがある場合`→`HAS_CARD_IN_FIELD{cardType:レゾナ}`（WD09/11/12-018 の「追加で」枝）／`トラッシュにカードがN枚以上ある場合`→`TRASH_COUNT{gte}`（WD22-038-UG/WXDi-D05-012/WXDi-P02-089）／`手札がN枚より多い場合`→`HAND_COUNT{gt}`（WDK08-Y08）／`場にシグニがない場合`→`FIELD_COUNT{eq,0}`（WX04-025）。
  - **採用**＝`heldReview --adopt` 4枚（WD11-018/WD22-038-UG/WXDi-D05-012/WXDi-P02-089）。WD12-018 は自動採用・WX04-025 は既 MANUAL。**据置**＝WD09-018（then が SEARCH 未 parse で UNKNOWN 化）・WDK08-Y08（then「差の分だけ」機構未対応）。
  - **検証**：golden 356・smoke/fuzz 全0・census 2098→2096（BASELINE 更新）・同型★0 維持。詳細 BUGFIXES 続き159。
  - **次の一手**：Opus＝IS_MY_TURN化残111 のうち engine 対応済み条件をさらに（種類/共通クラス系〈distinctNames 対応済みか要確認〉・傀儡状態フィルタ・LRIG色別多分岐 WXK09-003 等）。**「then が機構待ちで UNKNOWN 化」する据置カード群**（WD09-018 の SEARCH-in-then・WDK08-Y08 の差分移動・リコレクト分割 (xxiii)）は §6.3 級の別作業。ほか (xxvii) 残 Cluster C/A/F。Sonnet＝Opus在庫消化待ちまたは補欠。
