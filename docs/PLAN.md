...
| 2 | CHOOSE held 全数機械分類 | 分析（tmp_スクリプト＋分類表） | S〜M | ◎今すぐ（完了が Opus10 を解放） |
...
- **🆕 セッション（2026-07-11・続き69・Opus 4.8・§3 Opusタスク4 引用内CHOOSE＝「（カードをN枚）引くか<B>」トップレベル動作選択 26枚）**＝プレイヤーが2動作から1つ選ぶ「AかB」（「引くか【エナチャージ】」「引くか相手が手札を捨てる」等）を、従来パーサが**先頭動詞だけ拾い片方を無言脱落**させていた過少パースを CHOOSE(2択) で正エンコード。**本命 WXDi-D09-P20**（【常】中央ゾーン限定で「【自】アタック時、引くか相手が捨てる」を得る＝引用付与の**内側**CHOOSE）は grant＋IS_SELF_IN_CENTER_ZONE＋内側CHOOSE の完全形に。**パーサ**＝`parseDrawOrChoice` 新設（DRAW を錨に形A「カードをN枚引くか、B」/形B「AかカードをN枚引く」＝隣接必須・両非UNKNOWNのみ CHOOSE 化・「とき/そうした場合」除外でトリガー主語「か」の誤検出防止）。呼び出しは `parseActionTextInner` 冒頭＋`parseSingleSentenceInner` トリガー句除去直後の2経路。**engine不変**（execChoose 既存）。**採用**＝26枚 heldReview --adopt（原文照合で「見ないで選び」blind・逆順・count 2/3 も正）。**held残置3枚**＝WX20-078/SPK01-14（①②③選択喪失）・WX19-062（宣言一致条件喪失）は変更外の既存degradation＝CHOOSE復元backlog（§3タスク6）へ。**検証**＝golden +4（187）／gates 全緑（**census 1563→1558**・5枚語彙獲得・BASELINE更新・lint 0 errors）／regen 同型★0・decompile「以下の2つから1つを選ぶ【…引く / …】」原文忠実。詳細 BUGFIXES 最上部。
  - **前回（続き68・Opus）＝デッキ相対SEARCH 3枚**（捨てシグニ基準の±Nレベル/共通クラスフィルタ＋execSearch配線バグ修正）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手（⚠§3 の割付は 2026-07-11 に全面再割付済み＝番号は新リスト準拠）＝(A)Opus＝§3 Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY内側parse**（WX24-P1-017／WX25-P3-038＝GRANT_TO_PLACED_SIGNI STUB の引用「【自】…」を得る＝**場に出したシグニへの一時付与**機構＝WXDi-D09-P20 の CONT付与とは別・引用内 CHOOSE とは別作業）。**(B)Opus＝§3 Opusタスク2＝動的比較別機構の残**（WXEX2-28＝直前配置シグニ基準 last-processed相対〔levelGtLastProcessed 相当〕・条件文 WXK08-005・opp/own センタールリグ WXK11-003）。**(C)Sonnet＝§3 Sonnetタスク2＝CHOOSE平坦化復元 held約38枚の全数機械分類**（🆕続き69で held に WX20-078/SPK01-14/WX19-062 の3枚が追加＝①②③選択喪失系。着手前に続き24-29の型で偽陽性を先に切る・採用のグレー判定は Opusタスク10）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（続き66でR30発火経路開通＝WXEX2-50-E3発動→WXK10-022-E1 watcher の ON_PLAY any_opp確認・ON_TARGETED残3枚・R42②・ON_COIN_PAID/ON_LRIG_GROW③④）。**(E)持ち越しの小口は §3 Opusタスク5（WXK10-033-E1据置確認・WXEX2-25-E3 levelLtSelf描画固定・WXEX2-50-E3 step2レベル制約・WX12-008 exceed-cost timing・SEQUENCE下流「そうした場合」・PR-Di038・WX25-P2-095）と Sonnetタスク3（verifyBattleDriveバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 1558枚（2026-07-11 続き69・履歴 …→1572→1567→1566→1563→1558・続き63 DEPLOY_RESTRICT実装で5枚・続き67 WXEX2-25-E3 lrig相対付与で1枚・続き68 デッキ相対SEARCH3枚・続き69 引用内CHOOSE 26枚で5枚解消・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。状態系の残（凍結13・ダウン/アップ38）はコスト節/条件/CONT型（別パス・§6.3）。**消化の入口は `npm run census:clusters`＝文型テンプレのクラスタ表（`docs/_census_clusters.txt`・枚数順）から系統バッチを選び、parser規則→`npm run build:effects`→`node scripts/heldReview.mjs` で署名グループごとに一括採用する（続き23確立・手順詳細は §5c）。**
