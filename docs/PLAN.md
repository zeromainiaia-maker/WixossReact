...
**Opusタスク12＝🏁未消化の在庫は0件（2026-08-08）**＝Sonnet が新しく積むまで待機。
...
- **🆕 セッション（2026-08-09〜10・続き397〜402・Opus 5／実装は Codex 委譲6連投）＝§6.3 C を残0クローズし、§6.3 E を「E-1 残0／E-2 tractable 全消化」まで進めた**・golden 1682→**1698**・census 882→**880**・held 111→**110枚/47群**・同型★0 据置。**6バッチとも差し戻し0。**
  - **⭐① 投入前実測が PLAN の在庫記述を4回連続で覆した（今回も最大の成果）**＝(a) **§6.3 C 残8カードの裏向き系4枚**＝在庫は「`delayed_triggers` しか遅延機構が無いのでターン跨ぎは表せない」としていたが、実測で **`field.facedown_signi`（ゾーン配列・「場のシグニとして扱わない」inert 実装・描画/permute/boardDiff 済み）** と **`pending_facedown_flip`（ターン跨ぎ遅延の先例。`triggerCollect.ts:3589` のコメントが「専用の永続フィールドで持ち越す」と明言）** の2つが既にあった。⇒ honest defer 予定だった `WXDi-P09-009-E3`（二重遅延）まで実装できた。(b) **§6.3 E「引用付与の忠実化」2件は共通機構ではなかった**＝`WX20-036-CB-E1` は **engine 側が既に holder を source として `excludeSelf` を評価済み**（`effectEngine.ts:4777` の `GRANT_PROTECTION`／`:1814` の `POWER_MODIFY`）で、真因は parser。保護節も既存語彙で書けた（`WXK11-021-E1` が完全なテンプレ）。⇒ PLAN の「外側 action の `excludeSelf` では reader に届かない」は**誤り**。(c) **`WDK14-013` は既に消化済み**＝「残は複数候補時のプレイヤー選択のみ」は stale で、`SigniOnPlayCostModal.tsx:581-587` が候補を描画して選ばせ `BattleScreen.tsx:11750` が選択を渡している（古いコメントだけが残存）。(d) **`WX25-P3-023-E2` の「2ターン持続＋相手手札移動 collector が必要」も両方 stale**＝`ON_HAND_ADDED`（`byOpponentEffect`／`excludeGrowPhase`／`handOwner`。テンプレ `WX25-P2-063-E1`）と `GRANT_LRIG_ABILITY{duration:'UNTIL_OPP_TURN_END'}` で**新語彙0**。**⇒ §3-1「在庫行は症状の見立てであって実測ではない」の実例がさらに4つ増えた。**
  - **⭐② 続き397＝§6.3 C 第4波・裏向き4カードで C を残0クローズ**。`face_down_signi`（`string[]`・消費者がその STUB 以外に**0件**＝ゾーンが埋まったままで「同じ場所にシグニがない」が永久に偽）を捨て、`field.facedown_signi` へ移送する `src/engine/facedownSigni.ts` を新設。**ターン終了の3経路（`doPhaseAdvance`／`confirmEndDiscard`／`cpuTurnAction`）で「ターンプレイヤーと非ターンプレイヤーの両方」を解決**＝4枚中2枚は**相手のシグニ**を裏返すので、既存の「自分の場だけ直す」形を写すと永久に裏向きのままになる（投入前に🔴で先出しし回避）。`WXDi-P09-009-E3` は `turn_end_facedown_all`＋`pending_opponent_attack_facedown_returns` の二段で「ターン終了時に裏向き→次の相手アタックフェイズ開始時に復帰」を実装。
  - **⭐③ 続き398〜399＝§6.3 E-1 残0**。`WX20-036-CB-E1` は内側引用文が `rawText`→`parseBlock` で**単独再パースされ結果を上書きする**のが真因で、内側文型を `SEQUENCE[GRANT_PROTECTION{sourceFilter:{hasLifeBurst:false}}, POWER_MODIFY{excludeSelf}]` にした。🔴**codex の訂正が正しかった**＝`collectEffectImmuneSigni` は **action 直下の `GRANT_PROTECTION` しか読まない**ので SEQUENCE 再帰を追加（**5条件の完全一致ゲート**で限定し、SEQUENCE 内 `GRANT_PROTECTION` 8件の既存効果は不変＝検証側で全数走査確認）。`WX24-P2-010-E1` は続き398 で honest defer になったが、**続き399 で「支払えばアタックできる」既存機構（`signi_attack_cost`＝ゲート `getMySigniZoneActions` ＋支払い `performSigniAttack` の2箇所で完結）と、人間3経路＋CPU が `performSigniAttack` を共通利用する事実**を先出しして投げ直し、`BLOCK_ACTION.attackCost.fieldTrash` ＋ `signi_attack_field_trash_costs` で実装（`handleFlipAttack` だけ別経路なのでハードガード）。
  - **⭐④ 続き400〜402＝§6.3 E-2 の tractable を全消化**＝(a)`WX25-P3-023-E2`（起動即発動→2ターン持続の監視能力へ）。🔴**構造だけ直すと恒久 no-op**＝`collectHandAddedTriggers` が `lrig_granted_auto_effects*` を読んでいなかった。**副次で `UNTIL_OPP_TURN_END` の失効が CPU 戦だけ抜けていた**のも PvP と同一呼び出しで是正（該当 live 152効果・方向は過剰実行の解消）。(b) permanent 引用付与4効果＝**新 `GRANT_PLAYER_ABILITY`＋`game_granted_effects`**（「ルリグ」でなく「あなた」が得る）と **`GrantLrigAbilityAction.targetOwner`**（相手ルリグへの恒久付与）を新設。3経路のターン終了を `clearTurnGrantedLrigAbilities` に統合。(c)`WX20-028` 多重アクセ＝**`signi_acce` を `(string|null)[]` → `(string[]|null)[]` へ型移行**（案(a)。tsc が全参照を落とすので追従漏れを機械的に示せる）。死語彙 `ACCE_LIMIT_99/_2`（**`src` に消費実装0件**）を既存 `MULTI_ACCE_LIMIT` へ寄せ、collector を「上限N／ALL／旧値なし=2」対応に拡張。
  - **🔴⑤ 検証側が held 増分2件を「戻す」のでなく採用した**＝`WX20-028` の parser 修正で同じ死語彙だった **`WX16-031-E1`（2枚まで→`value:2`）／`WXK04-053-E1`（好きな枚数→`value:"ALL"`）** が held に現れた。原文照合して2件とも採用し、census 882→**880**・held は着手前と同値の 110枚/47群に収束（`BASELINE_HIGH` も 880 へ更新）。**§5-5「held を元の数値へ戻すことを目的にしない」の実践例。**
  - **🔴⑥ Codex 委譲の運用知見（CODEX_GUIDE §2 へ要追記）**＝(a)**`.codex-work` の認証が失効**（`refresh_token_invalidated`）。(b)**`-c sandbox_mode="danger-full-access"` は Claude 側分類器にブロックされる**（危険フラグ文字列検出）。(c)🔑**`.codex` 本体は `[windows] sandbox = "elevated"` ＋ project trusted のおかげで**サンドボックスフラグ無しで書き込みが通る**（プローブで実測）＝グローバル設定を書き換えずに委譲できる。(d)**ハーネスの10分 kill 後も `codex.exe` は完走する**（続き400/401/402 で3回とも実証。`-o` レポートは後から出る）。(e)🔴**Windows `0xC0000142`（ヘルパー起動失敗）が続き400〜402 で頻発**し、codex 側が最終工程（BUGFIXES 追記・`groupSimilar`・エンコーディング検査・held 再測）を実行できなくなった＝**検証側が引き取る前提でスコープを切ると成立する**。
  - **✅⑦ codex の訂正は今回も全件正しかった**＝(a)`collectEffectImmuneSigni` の SEQUENCE 非再帰（続き398）(b)`execBlockAction` の SIGNI×ATTACK は `blocked_actions` でなく `keyword_grants['アタックできない']`＝**私の指示書の記述が誤り**（続き399）(c)`WX15-002-E2` は**ベースライン時点で既に `permanent:true`**＝私の実測ダンプが330文字で切れていた（続き401）。**指示より実コードを信じさせる文言は今回も機能した。**
  - **📋⑧ 次の一手**＝**§6.3 E の残3件はいずれも「実装しないのが正しい」状態**なので、E は「E-1 残0／E-2 tractable 全消化＋根拠つき defer 3件」でクローズしてよい。内訳＝(a)**`WX17-044`＝別機構待ち**（§6.4「トラッシュ起動のコストUI 残」＝対象14枚の共通穴が先行ブロッカー。単独着手すると同じUIを二重に作る）(b)**`WXDi-P05-006` choice①＝着手禁止**（ピースカットイン割込み基盤）(c)**`WX20-Re20`＝一体で要る**（選択数依存コスト・能力なし filter・任意複数配置UI・同一instance群のターン終了時 trash）。**Opus 側の次点＝§6.4「トラッシュ起動のコストUI」**（14枚共通・潰せば `WX17-044` も開く）／**`WXDi-P07-073-E1`**（付与 AUTO が `triggerScope:'any_opp'` のため新設した付与ストア走査の対象外＝依然として死んだまま）。**Sonnet 側＝§7 実機検証**（下記⑨の3件が最優先）。
  - **⚠⑨ 今回持ち込んだ実機検証項目（golden では原理的に踏めない）**＝(a)**裏向きフリップバック**（`WXDi-P09-034`／`WXDi-P05-037`＝相手シグニ・`WXDi-P01-040`＝自分2体まで・`WXDi-P09-009`＝ターン跨ぎ）(b)**解除コストつきアタック制限**（`WX24-P2-010`＝他シグニ2体トラッシュで解除／CPU の決定論的支払い）(c)🔴**`signi_acce` の型移行は永続化データの形も変える**＝進行中バトル状態（Supabase `host_state`/`guest_state`）に旧形式（文字列直値）が残っていた場合の扱いが未検証。(d)`UNTIL_OPP_TURN_END` の CPU ターン終了失効（該当152効果）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-09〜10 続き397〜402（§6.3 C 残クローズ＋§6.3 E・Codex 委譲6連投）後 最新値（本行が直近の正）**：census **882→880**（−2・ 本体を更新。並行定数の新設なし）、golden **1682→1698**（+16）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム・EXPLOSION 0）、同型★**0 据置**（5986枚・265群）、lint 0 errors/**254 warnings**（増減0）、manual field loss 0、held **111→110枚／48→47群**、live 変化 **4＋1＋1＋1＋3＋4＝14効果**（6波とも added/removed 0・outlier 0＝巻き添えゼロ）。**新設した型**＝（プレイヤーが能力を得る）／（相手ルリグへの付与）／（解除コストつきアタック制限）／／。**型移行**＝ を →****（多重アクセ）。⚠**残す教訓**＝(a) 🔴**「機構待ち」と書かれた在庫の4件が実測で覆った**（§4 進捗サマリ①）＝**着手前に必ず既存機構を grep する**。(b) 🔴**構造を正しくしても収集経路が読まなければ恒久 no-op**＝／ は  を走査していなかった。**付与ストアは「印刷能力とは別に走査が要る」**（同型の穴が  に timing ごと散在＝§4 の次の一手も参照）。(c) 🔴**死語彙は grep 0件で見つかる**＝／／ はいずれも  に消費実装が無く、JSON にだけ居た。**「STUB があるから実装済み」と扱わない。**(d) ⭐**held 増分は「戻す」のでなく1件ずつ原文照合して採用する**＝今回2件（／）が採用に値し census −2 になった。(e) 🔴**BattleScreen 経路の変更は golden に一切映らない**＝ターン終了3経路・アタック経路・アクセ描画は**実機検証項目**（§7）へ回すこと。
...
> このセクションは機構台帳＝**現存の残作業は下記 E／F／G／K に集約**（🏁**C は 2026-08-09 続き394〜396 で主要3家族を残0クローズ**＝計33効果。残8カードのうち裏向き系4枚は §6.4 級として C 項内に移送先を明記／🏁**J 群は 2026-08-08 に残0クローズ**＝J-2 続き380／J-5 続き381／J-1 続き383／J-4 続き384）。消化済み機構の実装詳細（フラグ・ファイル・commit）は BUGFIXES 各日付が一次記録（各機構が日付/commit を明記済み）。完了行の原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-02 整理②」節。
...
- **C. 「この方法／この効果で〜した場合」の後段条件が live で丸ごと脱落（＝無条件実行）**＝🏁**主要3家族は 2026-08-09 続き394〜396 で残0クローズ（計33効果・Codex 委譲3連投）**。詳細は BUGFIXES 同日3件と PLAN_PROGRESS 先頭。**旧記述（「fresh 残42効果」「共通ブロッカーは前段が `lastProcessedCards` に記録しないこと」）は投入前実測で3点とも否定されたので破棄した**＝(a) `docs/_partial_report.txt` は **fresh parser の出力**を測る計器で live の実害を測らない（刻印39件のうち live に `IS_MY_TURN` が残るのは1件のみ）。**生成元（CSV原文×live JSON）から数え直した母集団は60カード**。(b) `lastProcessedCards` の writer は engine に**153箇所**あり完備（`execTrash{DECK_CARD}` は `effectExecutor.ts:1228`）＝機構待ちではなく **parser が条件を1つも生成していなかった**。(c) 条件語彙も既に揃っていた。**消化済み＝第1波11（効果内トラッシュ）／第2波11（公開・手札に加える）／第3波11（手札捨て・エナ配置）。**
  - ⚠**この家族に手を出すときの必須知識＝「記録する」の書き方が前段ごとに3通り**（取り違えると恒久 no-op でゲートに一切映らない）：`execTrash` は無条件に書く／**`REVEAL_AND_PICK` は `recordRevealed: true` のオプトイン**（`effectExecutor.ts:4533`・live 利用者1枚・parser emit 0件）／**`LOOK_AND_REORDER` は `needsInteraction` を返すだけで記録は `resumeLookAndReorder`（`:6791`）＝UI ポーズを跨いだ後**。さらに**コスト由来の捨て札／エナトラッシュは engine ではなく `BattleScreen.tsx:10885-10952` が `last_cost_trashed_cards`／`last_activated_discard_count` に書く**（writer は `src/screens/battle/costs.ts`）。⚠**「公開された全カード（`visible`）」と「手札に加えたカード（`picked`）」は別集合**＝`resumeSearch` は `lastProcessedCards: pending.lastProcessedCardsAfter ?? picked`（`:6626`）なので、**「手札に加えた」条件では `recordRevealed` を付けてはいけない**。
  - ⚠**writer や共有経路を触ったら「後段で `lastProcessed` を読む効果」を実データで全数走査すること**（gates では捕まらない）。続き395 で `execRevealAndPick` の候補0件パスを `[]` 明示に変えた際に実施＝該当13効果は**全件が `REVEAL_AND_PICK` を step 0 に持つ**ため波及0だった。
  - **📋 残り8カード＝「条件脱落」の残作業はほぼ無い**（続き396 実測）。(a) ✅**2枚は既に正しい**＝`WX20-053-E2`（シャッフルは `CHOOSE` の deck 枝の `afterSearch` で表現済み）／`WXEX2-21-E1`（`DECK_COUNT{opponent,eq,0}` あり）＝**抽出計器の偽陽性**。(b) 🔴**裏向き系4枚は §6.4 級の機構バッチ**（→ §6.4 へ移送）＝`WXDi-P01-040-E2`／`WXDi-P05-037-E2`／`WXDi-P09-009-E3`／`WXDi-P09-034-E1`。**条件脱落ではなく機構が空回り**＝`SEQUENCE[SIGNI_FLIP_FACEDOWN, FLIP_FACE_DOWN_SIGNI, TRASH_IF_ZONE_OCCUPIED]` で**同じ解決内で裏返して即座に表へ戻している**（裏向きが0秒）。原文は「このターン終了時」の遅延トリガ。`FLIP_FACE_DOWN_SIGNI`（`execStubPart2.ts:4193`）は `face_down_signi` を**全部**戻す＝「この方法で裏向きにした」部分集合を持たず、`TRASH_IF_ZONE_OCCUPIED`（`:1524`）は **`ctx.sourceCardNum` をトラッシュ＝主語が違う**。⚠`WXDi-P09-009-E3` は「次の対戦相手のアタックフェイズ開始時」＝**ターン跨ぎ**で既存 `delayed_triggers` は THIS_TURN 限定（`src/types/index.ts:453`）＝表せない（honest defer 可）。**指示書 `codex_task397.md` 作成済み**（1枚ずつ完成させる順序制約つき）。(c) 残2枚は別機構＝`WX15-067`（ウィルス除去によるコスト減）／`WXDi-P13-004A`（離場置換＝§6.4 F-3 の家族）。
