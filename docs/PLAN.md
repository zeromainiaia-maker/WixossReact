...
- **🆕 ON_OPP_POWER_DECREASED のresume経路取りこぼし（2026-07-09・続き58・§7 R46実機検証で発見・未修正・WX13-036/WXEX2-52が対象）**＝ON_SIGNI_FROZEN（R38）と同型のバグ。`collectPowerDecreaseTriggers`（`src/engine/triggerCollect.ts:900`）は`BattleScreen.tsx:3765-3789`の中央diff（`resolveStackNext`内）にしか配線されておらず、`handleEffectInteraction`のpendingEntriesブロック（4384-4436・`collectFreezeInline`等5種のinline collectorが既にある）には**入っていない**。WD11-013（【出】相手シグニ1体-1000・mandatory・コストなし）をSELECT_TARGET resumeで解決するとwatcher WX13-036が一度も発火しない（`guest.temp_power_mods`は正しく-1000になるが`host.temp_power_mods`は終始`[]`）。**再現**＝`node scripts/verifyBattleDrive.mjs oppPowerDecreased`（既定orderから除外済み）。**修正方針**＝`collectFreezeInline`と同型の`collectPowerDecreaseInline`を`handleEffectInteraction`に追加する横展開（新規機構ではなく適用漏れ・低リスク）。詳細はBUGFIXES続き58。
