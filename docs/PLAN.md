...
**直近＝2026-09-22＝🔴**【トラップ】の中身が相手に見えていた**（ユーザー指摘・全文は [BUGFIXES.md](./BUGFIXES.md) 第464バッチ）
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4385**）｜**実機 PASS**（`v285TrapHiddenFromOpponent`・反転確認あり） |
...
**残0**。直近の返済＝`V-285`（【トラップ】の中身が相手のログ・画面に見えていた・2026-09-22）／`V-284`（じゃんけんの「あいこ」で止まらない）／`V-283`（コンボの「使い方」を画面から保存）。
