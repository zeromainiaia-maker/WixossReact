...
**直近＝2026-09-21＝§5.7 `S-6` 第1段＝重み候補の「分岐スクリーニング」を作って20個のつまみを全数走査した**（全文は [BUGFIXES.md](./BUGFIXES.md) 第429バッチ）
- 🆕**`npm run selfplay:scan`**（`--diverge`・純関数 `scripts/cpuWeightScan.ts`）＝**候補を両席に置いて1戦回し、打った手の並びを基準と突き合わせる**。
  🔑**`S-25` が「1本ずつ動かして4本とも差なし」で終わった真因**＝`hand=800` は **0-8-0＝1組も動かなかった**＝
  **8分（96戦）払って得た情報は「この数値は判断を1度も変えなかった」だけ。それは1戦（35秒）で分かる。**
- 🔴**分岐は必要条件であって十分条件ではない**＝**分岐0 なら A/B は必ず全組1勝1敗**（掛けるだけ無駄）。**強さの判定は `selfplay:ab` だけ。**
- 📏**②母集団**＝`S-6` の対象は **49個の数値**。いま `--a-set` で振れるのは **20個（41%）**＝
  **`cpuCardStrength.WEIGHTS`(14) / `KEYWORD_VALUE`(8) / `PLAN_WEIGHTS`(6) / `CPU_GUARD_KEEP_VALUE`(1) は `CpuPolicy` に載っていない**（⇒ 第2段）。
- 📏**実測＝35候補 × 6デッキ × 2シード＝432対戦・1,660秒**（山は `S-25` と同じ6つ）。
  **最も早く効くのは `actionBias=1500`（12/12・中央5手目）**／`life` は重みが最大（7000）なのに **2/12 しか動かない**。
- 🔴**分岐0 の8件は「無効な数値」ではなく、原因が3つに割れた**＝
  **(a) 山に札が無い**（`oppFrozen`＝6デッキとも凍結カード0枚）／
  **(b) 判断の主体が移った**（`spellGainMin`＝**探索を切ると 6/12・3/12 で分岐する**＝`S-25` で既定を探索ありにした時点で閾値に届かなくなった）／
  🔴**(c) その重みを読む決定が存在しない**（`lrigLevel`＝**探索を切っても 0/12**＝GROW は「候補の1枚目で必ずグロウ」で盤面を比べない）
  ＋ (d) 頭打ち（`searchDepth=6`）／(e) 閾値に届かない（`growReady=1250`）。
- ⤵**(c) は `S-26` に登録**＝`S-18` が入れた `lrigLevel: 2500` は**本番の判断に1度も効いていない**（当時 `--census-moves` の計測側で確かめてしまった）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-6` 第2段**（①残り29個を `CpuPolicy` へ載せる ②分岐する候補だけを多次元で A/B）。次に `S-26`・`S-22`・`S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 5**（`S-6`・🆕`S-26`・`S-22`・`S-24`・`S-14`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①🆕**`lrigLevel` を読む決定が無い＝グロウは常に候補の1枚目**（`S-26`）②**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）③**自己対戦がマリガンを一度も踏まない**（`S-24`） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4350**）｜**実機は不要な回**（触ったのは `scripts/` と `package.json` だけ＝PLAN §2.2）｜反転確認＝`spellGainMin` は基準を `legacy-greedy` にすると 0/12 → 6/12 |
...
| 🔥`S-6` | **機械学習①＝自己対戦で重みを調整**。🏁**第1段（2026-09-21）＝分岐スクリーニング**＝`npm run selfplay:scan`（`--diverge`／`--scan`／`--cand "key=値"`・純関数 `scripts/cpuWeightScan.ts`・golden `§5.7 S-6`）＝**候補を両席に置いて1戦回し、打った手の並びを基準と突き合わせる**＝**1候補 1戦（35秒）**で「判断を変えるか」が分かる。🔴**分岐0 なら A/B は必ず全組1勝1敗＝掛けるだけ無駄**（`S-25` はそこに8分×4本を払った）。📏**実測＝35候補 × 6デッキ × 2シード＝432対戦**で**分岐0 は8件**＝原因は(a)山に札が無い／(b)判断の主体が移った／(c)その重みを読む決定が存在しない（⇒`S-26`）／(d)頭打ち／(e)閾値に届かない。🔥**第2段＝①残り29個を `CpuPolicy` へ載せる**（`cpuCardStrength.WEIGHTS` 14／`KEYWORD_VALUE` 8／`PLAN_WEIGHTS` 6／`CPU_GUARD_KEEP_VALUE` 1＝**いまは `--a-set` で振れない**＝`cardStrength` は `cpuHandLimit`／`cpuInteraction`／`cpuLookahead` から呼ばれるのでポリシーを通す配管が要る）**②分岐する候補だけを多次元で A/B**（1候補 6デッキ × 8シード＝96戦＝8分）。🔴**1本ずつ動かす探し方は `S-25` で外れた**＝**重み同士の干渉を前提に多次元で探す**。 | 大 | 手で決めた重みを勝率で置き換える |
...
| `S-26` | 🔴**`lrigLevel`（グロウの価値）を読む決定がどこにも無い**＝`S-18` が入れた重み 2500 は**本番の判断に1度も効いていない**。実測（`S-6` 第1段）＝`lrigLevel` を半分にも倍にしても **12対戦中0で分岐しない**（**探索を切った `legacy-greedy` 基準でも 0/12**）。真因＝`controller/cpuTurn.ts` の GROW フェイズは「**グロウ可能なら候補の1枚目で必ずグロウ**（決定論・盤面評価はしない）」＝**グロウの得失を比べる場所が無い**（MAIN の探索は `grow` を候補に出さないので、そちらからも効かない）。⚠**当時 `--census-moves` の計測側の探索で確かめてしまった**＝**計測側の探索は全フェイズで回るが、本番が探索を呼ぶのは MAIN と ATTACK_SIGNI だけ**。⚠**直すと乱数列が動く**（`S-24` と同じ）＝**直した回にベースラインを撮り直す**。 | 小〜中 | グロウの判断が盤面の価値で決まる＝`S-6` が `lrigLevel`／`growReady` を学べるようになる |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔥**`S-6`（機械学習＝🏁第1段の篩は済／🔥第2段）**→ `S-26`（グロウが盤面を見ない）→ `S-14`（作戦データ）。
...
- **2026-09-21 時点**（第421〜428バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 4**（`S-6`・`S-22`・`S-24`・`S-14`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4349**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 5本**（6/133手・8/191手・7/174手・7/170手・**7/161手**）
