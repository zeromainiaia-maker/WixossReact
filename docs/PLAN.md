...
| ID | 内容 |
|---|---|
| (l) | **残4**（B群36枚は✅2026-07-30 に残0クローズ／A群も✅2026-07-30 に「残8→実測残6→4」まで消化）＝**置換イベント横取り系の3文型のみ**＝`WX24-P4-026-E1`（【ガード】コストの置換＋外側の色組み合わせ条件）／`WX16-004-E1`（ホログラフのデッキトップ公開の置換）／`SPDi44-08-E2`＋`WX25-P1-018-E2`（離場置換＋ルリグ自身の付与能力喪失）。⚠**3文型はいずれも内側が「AUTO でパースできてしまうが意味が別物」**（`SPDi44-08` の内側は `REMOVE_ABILITIES{SIGNI opponent}`＝相手シグニの能力を消す真逆）＝**入れ子化そのものを golden で禁止**してある。機構（置換フック／能力喪失）が入るまで honest defer。<br>🆕**新たな据置1件＝`WX12-020-E3`**（「この方法で捨てた手札1枚につき－6000」＝倍率元が現在の手札枚数ではなく捨てた枚数。既存 `POWER_MODIFY_PER_HAND_COUNT` の拡張では意味が混ざる）。<br>✅**`WDK04-006-E1-G` の中身はクローズ**（相手偶奇宣言 `DECLARE_PARITY_OPPONENT`／`NEGATE_ATTACK{LRIG}`／公開札のデッキ下処理）。詳細 BUGFIXES 2026-07-30 の2節・[PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-07-30 整理」節 |
...
> **✅残0でクローズした在庫の完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-07-30 整理」節へ退避**＝**(xxix)**〔任意【出】コスト・計15波・残4は明示保留〕／**(xlvi)**〔look-pick 計73効果・第1〜17波〕／**(lvii)**〔census キー表 `pickUpTo` 較正〕／**(lviii)**〔トラップ公開 LPC 移行9効果・残1は honest defer〕／**(lix)**〔`split_top_bottom` 全4効果〕／**(l) B群**〔付与スコープ36枚47効果〕。それ以前の消化済み在庫〔(i)〜(lvi) の大半〕は同ファイルの 2026-07-19・07-24・07-28・07-29 の各退避節。
...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が指示と検証＋codex-work が実装）＝タスク12(l) の残（A群）と「手札枚数比例」族・公開カード消滅を消化し `WDK04-006-E1-G` をクローズ**（golden **1079→1085**・census **1393→1391**〔`BASELINE_HIGH=1391`〕・smoke **10679** 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held **286枚/106署名 据置**・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 9 / added 0 / removed 0（影響カード8）** で申告と完全一致＝兄弟効果の巻き添え0・変更禁止15効果の差分0・`build:effects` 冪等）
  - **投入前実測で簿記を2点訂正した**（CODEX_GUIDE §3-1「簿記を信用せず実測する」）：①**「A群 残8」は実測 残6**＝`WD21-009-E1`／`PR-204-E1` は 2026-07-23 の §6.3 ルリグ付与バッチで `manualEffects.ts` に付与 `abilities` を埋めた MANUAL 定義があり golden も通っている（live JSON の `abilities:[]` は override が完全置換するため実機には届く＝ガードレール §5-16）。②在庫が挙げていた不足機構**「手札枚数比例値」は機構不足ではなく既存機構への未接続**＝`POWER_MODIFY_PER_HAND_COUNT`（型／`execPowerModifyPerHandCount`）は既にあり、parser が `parseSentencePart3.ts` で無条件に STUB へ倒していただけだった。
  - 🔴**在庫外の実害3系統を投入前に発見してスコープの本命に格上げ**＝①`STUB POWER_MOD_BY_HAND_COUNT`（5効果）は**印刷テキストを regex でスクレイプし必ず相手の場の3シグニ全員へ適用**する＝原文が「あなたの…シグニへ＋1000」の `WX25-CP1-043-E1`／`WX26-CP1-076-E1` は**相手を強化する真逆の実装**、`WXDi-P05-029-E2`／`WX24-P2-005-E1` は「1体」が全員適用 ②`STUB DECK_REVEAL_UNTIL` は廃棄先を原文 regex で決めるため**どの分岐にも当たらない文型では公開札が trash/hand/デッキ下のどこにも入らず消滅**（`WDK04-006-E1-G`／`WX17-039-E1`／検証で `WXK01-037-E2` も判明） ③`【歌のカケラ】`/`【トラップアイコン】` の解析入口が `expandGrantEffectRawTexts` を呼ばず、`WX26-CP1-076-SONG` が **`parseStatus:'AUTO'` のまま完全 no-op**（PARTIAL 降格されないので held も census も素通り＝計器に映らない）。
  - **`WDK04-006-E1-G` の中身は4ステップ全部が原文と食い違っていた**＝`DECLARE_NUMBER`（**自分**が1〜5を宣言し `declared_guard_restrict_level` を立てる別能力）→ **対戦相手**が偶奇を宣言する `DECLARE_PARITY_OPPONENT`（既存 `CHOOSE.opponentResponds` 流用・値は `declared_number` に 偶=0/奇=1）／`NEGATE_ATTACK{target:SIGNI}`（アタックしていない相手シグニを無効化）→ `{type:'LRIG'}`（`execNegateAttack` の LRIG 分岐＝先例 `WXK10-012`②）を新 Condition `LAST_PROCESSED_SIGNI_LEVEL_PARITY_DIFFERS_FROM_DECLARED` の下へ／原文に無い `LOOK_AND_REORDER` を削除。
  - **任意コストを内側に持つ付与2効果**＝`WXK11-052-E1`（未展開で完全 no-op）と `PR-461-E2`（🔴**コスト未払いで自ターン中は必ずルリグがアップする過剰実行**）を先例 `WX01-028-E1` と同じ `CHOOSE{trash_and_up / skip}` へ。⚠`WXK11-052` の「**白の**」限定は **`execGrantEffect` の LRIG 分岐が `target.filter` を評価しない**ため意図的に書いていない（書けば死フラグ）。
  - **⚠Claude 検証で是正1件＝「JSON 差分に映らない engine 挙動変化」**：codex の「群C 既存8効果は不変」は **JSON 差分としては正しいが挙動は変わっていた**＝`WXK01-037-E2`「この方法で**公開された**カードをトラッシュに置く」は trash 判定の regex が「公開した」しか見ておらず**元から消滅していた**カードで、安全網により「消滅→デッキ下」に変化していた（原文は trash なのでどちらも不一致）。trash 判定を広げ、**全5986枚で判定が変わるのはこの1枚だけ**であることを機械列挙で確認。廃棄先3文型の弁別と**デッキ+手札+トラッシュの総枚数保存**を golden に追加し、regex を旧形へ戻す変異で**狙った1本だけ FAIL** することも確認。
  - **記録衛生の是正2点**＝codex が BUGFIXES を**末尾**に `#` 見出しで追記していたので規約どおり先頭 `##` へ移した（§8 既知の再発パターン）／新 STUB・Condition が decompiler で生 ID 表示だったため日本語ラベルを追加（`対戦相手は偶数か奇数かを宣言する`／`この方法で公開されたシグニのレベルが宣言と異なる`）してから 同型★0 を再確認。**数値申告（ゲート・per-effect diff・held・エンコーディング）は全項目が独立実測と一致**。
  - **honest defer**＝置換系4効果（`WX24-P4-026-E1`／`WX16-004-E1`／`SPDi44-08-E2`＋`WX25-P1-018-E2`）は**内側が「AUTO でパースできてしまうが意味が別物」**（`SPDi44-08` の内側は `REMOVE_ABILITIES{SIGNI opponent}`＝相手シグニの能力を消す真逆）＝**見送りそのものを golden で固定**。`WX12-020-E3`（「この方法で捨てた」枚数比例）も既存型の拡張では意味が混ざるため据置。
  - **次の一手**＝**Opus：単独バッチ候補（過剰実行・優先）**〔🔴`WX24-P4-072-E1`＝`PLAY_FREE` から「コストの合計が捨てた枚数以下」が丸ごと落ちて**どんな高コストスペルでも無料で撃てる**／エクシードルリグ6件の本体不一致／`WX20-063-E2`「コスト合計0」・`WXK06-082-E1`「《幻竜 ドラマジ》以外」の filter 脱落／`WXK03-070-E1` の本体欠落〕**／(l) A群残4（置換イベント横取り3文型）・(xxix) 残4は機構コスト待ちで据え置き／(lv) 残2経路（CPU シグニ召喚・CPU グロウ）／ゲート脱落 [B]20件／タスク16 残／§6.3 残機構**。**Sonnet：§7 実機検証**＝**キー付与【起】のエクシード支払いUI**（`LrigGrantedModal`）＋🆕**`DECLARE_PARITY_OPPONENT` の相手応答UI**（`CHOOSE.opponentResponds` の実機経路＝PvP で相手側にモーダルが出るか・CPU 戦は先頭選択肢「偶数」で自動応答する実装）＋分割UI＋宣言UI＋トラップ設置UI＋(xxix) の各支払いUI＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(l) A群／手札枚数比例 最新値（本行を直近の正とする）**：`POWER_MOD_BY_HAND_COUNT`→`POWER_MODIFY_PER_HAND_COUNT` の構造化5効果＋SONG付与展開1＋`WDK04-006-E1-G` の中身＋任意コスト付与2の計**9効果／8カード**。census **1393→1391**（`BASELINE_HIGH=1391`）、golden **1079→1085**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**228 warnings**、同型★0（5986枚）、held **286枚/106署名 据置**、manual field loss 0。live per-effect 差分 **changed 9 / added 0 / removed 0（影響カード8）**、`build:effects` 2回目差分0。⚠**効果総数は 10679 据置**（今回の入れ子化は SONG の1本のみで、その分は census の効果単位側に現れる）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1085**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1391**（回帰ゲート）・held **286枚/106署名**（2026-07-30 タスク12(l) A群／手札枚数比例 実測）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（(lv) 残2経路／(l) A群残4＝置換イベント横取り3文型のみ。**(xxix) は 2026-07-30 に完全クローズ**＝残4件は明示保留、**(l) B群と (xlvi)／(xlii)／(lix)／(lviii) も残0**）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
