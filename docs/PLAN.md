...
- 🏁**セッション（2026-08-27・続き690・Opus 5 単独）＝Sheet1 段2 の3巡目。主犯は「効果ブロックの分離マーカーが【自】：だけだった」こと。**
  ユーザー指示＝①段2 を Sheet1 に絞る ②`gates` は最後の1回だけ ③簿記を圧縮 ④1巡の処理量を増やす。
  📊**進捗3計器＝Sheet1 要対応 40→37 / 863（4.6%→4.3%）｜台帳 残 OPEN 599→591｜census 高シグナル 528→526**。
  Sheet1 findings は **28件/20カード → 20件/15カード**。live 変化 **7変更＋3追加 / 8カード**（意図差分以外ゼロ）。
  - 🔑**この巡の発見＝スペル本文の後ろのトップレベル【起】が本文へ連結され、唱えた瞬間に無料で走っていた**
    （実測3枚＝`WX10-096`／`WXDi-P06-077`／`WX17-044`）。スペル枝もアーツ枝も分離点が **`【自】：` だけ**だった。
  - 🔴**分割した瞬間に `WX17-044` の別バグが露出**＝本文の「…設置し、**残りをトラッシュに置く**」が
    part1 の汎用「あなたの…シグニ…をトラッシュに置く」へ貪欲マッチして `TRASH{自分のシグニ1体}` に化けた。
    **分割前に正しく見えていたのは catch-all が全文を飲んでいたからで、偶然に依っていた。**
  - ✅**engine の新設は2つ**＝`collectAttackerSelfDelayedTriggers`（`INSTALL_DELAYED_TRIGGER` の
    **攻撃側 collector が存在しなかった**）と `consumeOnceDelayedTriggers`（`ON_REFRESH` の `once` 消費）。
    さらに `collectOppArtsUseTriggers` に **usageLimit 判定**（姉妹関数にはあったのにここだけ穴）。
  - ⚠**golden が「収集経路の無い timing」を例文に使っていた**ため、経路を足した日に落ちた。
    テストは消さず**文面を次の未配線 timing へ動かし、陽性側 test を6本追加**（2896→2902）。

**▶ 次の一手**＝**同じ取り方で4巡目**（Sheet1 の残 findings は **20件 / 15カード**）。
着手は `npm run census:cards -- --sheet 1 --list` → 該当カードの原文・live・findings を**まとめて読む**。
⚠**残りは「受け皿が無い」側がさらに濃い**＝機構待ちは §5.3 の `O-105`〜`O-116`。
