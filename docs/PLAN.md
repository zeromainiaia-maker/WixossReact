...
**直近＝2026-09-21＝🏁§5.7 `S-32` ①＝相手の札を「属性」で狙えるようにした（クラス／レベル／パワー帯）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第441バッチ）
- 🔴**前バッチの穴**＝[狙う][避ける]に出るのは**自分のデッキの札だけ**＝**相手のエースを名指しできない**（相手の山は分からない）。
  ⇒ **クラス（41種）・レベル◯以上/以下・パワー◯以上/以下**で書けるようにした。
- 🔑**判定は engine の `matchesFilter` の1本**（CPU 用の第2のフィルタ言語を作らない）。変換は `cpuTargetFilterToTargetFilter` に閉じた。
- 🔴**パワーは実効パワーで見る**＝ctx の口を `targetBonus(id, power)` に変えた（印刷パワーだと強化された相手を見逃す）。
- 画面＝「狙う条件」「避ける条件」の行。⚠**狙うは「以上」・避けるは「以下」**／**空の指定は消す**（`{}` を残すと「作戦あり」と嘘をつく）。
- 📏**母集団**＝シグニ 3,742枚／クラス 41種／レベルとパワー帯の分布に合わせて UI の刻みを決めた。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-32` の残り**（②条件つき ③効果ごとの指示）と **`S-31` の残り**（①N体並べる ②払えないコスト523効果 ③アーツの使いどころ・使わない札）。次に `S-29` |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4359**）｜**実機 PASS**（6ターン/135手/181s ＋ `v268CpuDeckPlan`） |
...
- **2026-09-21 時点**（第421〜441バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4359**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・5/130手・6/133手・**6/135手**）
