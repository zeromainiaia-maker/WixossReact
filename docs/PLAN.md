...
**Opusタスク12＝未消化の在庫**（Sonnet が観測して積んだ engine/parser バグ。**下表は残作業のある在庫のみ**。消化済み在庫〔(i)〜(li) の大半〕の完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §3〔2026-07-19・2026-07-24・2026-07-28退避節〕）：
...
- **🆕 セッション（2026-07-28・Opus指示＋codex-work実装／Opus検証・11バッチ連投）＝§3 タスク12(xxix) を段階2 まで完走し、ON_PLAY 系の既存バグ155件＋実機バグ1件を是正**（golden 868→**891**・census 1511→**1479**・lint 234→**226**）
  - **🔴 最大の学び＝自動ゲートの穴は実在し、実際にバグを隠していた。** 9バッチぶん golden だけで進めたあと `scripts/verifyBattleDrive.mjs` に実機シナリオを足したところ、**`SELECT_SIGNI_ZONE` を経由すると効果配置シグニの【出】が発火しない**バグを即座に検出。golden・smoke・fuzz はすべて緑のまま素通りしていた（`selfPlayFuzz.ts:12-13` が「BattleScreen のオーケストレーションは検証しない」と自ら明記）。**BattleScreen 側の大きな挙動変更は golden だけで完了扱いにしないこと。**
  - **その真因は Opus の見立て4択すべて外れ**＝`resumeSelectTarget` が配置予定カードをトラッシュから抜き `SELECT_SIGNI_ZONE.interaction.cardNum` だけに保持して永続化するため、**再描画時のカードロード集合がそれを走査せず、配置予定カード自身の効果定義が `effectsMap` から脱落**していた。collector は呼ばれ配置も検出できていたのに entry が0件。**推測で直させず「まず実測で切り分けろ」と指示したのが効いた。** 副産物で `SELECT_ZONE` の collector 未配線と、`REVEAL_UNTIL_TO_FIELD` のゾーン選択途中ラウンド取りこぼしも解消（重複 inline 収集の共通化で lint -8）。
  - **(xxix) 本体**＝union/unless 16効果（⚠`negated_attacks` の保存先規約が2系統に割れていたのを検証で発見・是正）／BET 9効果26選択肢（最悪例 `WX19-006-E1③` は**除外本体が消えて2ドローだけ**）／自身【出】段階1（10効果＝全経路で永久不発）／**段階2＝mandatory 1437効果**。
  - **⚠段階2 は3度目で着地。1・2回目の codex の honest defer が毎回 Opus 側の計器の穴を突き、より大きな既存バグを掘り当てた**＝**ON_PLAY 誤分類36件**（原文が【自】なのに通常召喚のたびに誤発火）／**watcher 34件**（`triggerScope:'self'` で一度も発火せず＋自身召喚時に誤発火の二方向破損）／**先頭ゲート条件の脱落46件**（25件是正）。**合計155件が段階2 とは独立に稼働中のバグだった。**
  - **ON_PLAY 系は3種で系統的に壊れていた**＝①timing が違う②scope が違う③条件が落ちている。いずれも**通常召喚経路が mandatory【出】を無条件で積む**（`BattleScreen.tsx:5149`）ため従来から live。
  - **⚠検証で踏んだ落とし穴2つ**＝①**`build:effects` 再実行で JSON 差分が0でもドリフト無しの証明にならない**（held は curated 温存＝fresh と食い違うカードなので**差分0かつ held 増**が起きる）。これで watcher バッチの実バグを1度見逃した ②**非ASCII を JSON へ書くと文字化けする**＝`triggerFilter.cardType` が `"???"` になり watcher 2件が永久不発だった。**外科パッチ後は `grep '?' effects_*.json` で機械確認する。**
  - **PLAN の自己矛盾を1件修正**＝PLAN:44「`build:effects` は破壊的＝絶対に実行しない」が §5c パイプラインと矛盾。非破壊なので**「実行後に effectId 単位で全数比較する」**へ書き換えた。
  - **次の一手**＝**Opus：ゲート脱落の [B]20件（Condition 語彙の新設が要る）／(xxix) 残＝任意+cost 933効果と段階3 65効果（タスク12(lv)）／タスク16 の [B]29件＋watcher [B]4件／(xliv)・(xlii) の残／§6.3 残機構（H1〜H3・I）／タスク14 の残43**。**Sonnet：§7 実機検証の横展開＝`effectPlacedOnPlay`／`effectPlacedOnPlayZoneSelect` の2本が入ったので、この型で他の BattleScreen 依存機構（レゾナ召喚UI・escapeDiscard 回避モーダル・BET 選択肢・アーツ使用条件20枚）にも実機シナリオを足す。**
...
- **2026-07-28〜29 最新値（上記長文履歴の追補）**：`BASELINE_HIGH=1477`。タスク12(xliv)(a3) の実働化に伴い、WX09-022 の欠落していた《エナジェ》`activeCondition` を parser から復元して **1479→1478**、さらに Opus 検証で同文型の **WX12-033-E2**（PRESERVE カードのため build が curated を温存し fresh の条件が届いていなかった＝【ランサー】無条件付与）を外科パッチして **1478→1477**。いずれも計器だけの改善ではなく、条件不成立時の過剰発火を止める機能修正。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 828・smoke 10725 全0（SKIP も 0）・fuzz 全0・同型★0・census 1517（＝`BASELINE_HIGH`・回帰ゲート）・held 265枚/109署名。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（🆕(lvi)・(xliv)・(xlii)の残）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
