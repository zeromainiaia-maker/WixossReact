...
**直近＝2026-09-21＝§5.7 `S-28`＝場のシグニをエナチャージできるようにした（＋CPU だけ色制限を無視していたバグを修正）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第433バッチ）
- 🔴**抜けていた機構**（ユーザー指摘）＝**人間は前から場のシグニをエナへ置ける**のに **CPU は手札しか見ていなかった**。
  🔑**良い手になる場面**＝**正面に格上がいて場に残しても邪魔なシグニ**＝**手札を減らさずレーンを空けられる**
  （ENERGY → MAIN の順なので同じターンに強い札で埋め直せる）。
- 🔴**ついでに見つけたバグ**＝画面に**ほぼ同じ手順が2本**、CPU は**第3の写経**で、**CPU だけが相手の「エナチャージの色制限」を無視**していた。
  ⇒ **新設 `controller/performEnergyCharge.ts` の1本**に寄せた（人間も CPU も同じ関数）。
- 🔑**門は2つ**＝①**正面に格上がいる** ②**手札に「そのゾーンの置き換え」がある**（いま出せて今より強い）。②が無いと**レーンが空くだけ**。
  ⚠🔴**正面は左右が反転する**（自分の 0 は相手の 2）＝golden が両向きを検査。
- 📏**母集団**＝条件が立つ ENERGY 盤面は **WD13 56% / WD06 24% / ケトッシー軸 0%**。
  📏**実測**＝WD13 の自己対戦3戦で**エナチャージ37回中4回（11%）が場のシグニ**／**グロウできない 0回**（`S-26` も効いている）。
- 🔑🔴📊**A/B で初めて明確な差が出た**＝**全山合算のライフ差 -0.69 [-1.09, -0.29]（48組）＝区間が0を跨がない**
  （A は旧実装なので**負＝新機構が良い**）。WD06 は**勝率でも有意**（組 0-4-4）。
  **`S-27`（連続量）を作った投資がここで回収された**（勝率では6山中1山しか光らない）。
- 🔑**3山は `+0.00 [0.00, 0.00]`＝完全に同じ試合**＝その山では1度も起きない＝**母集団の実測（ケトッシー軸 0%）と一致**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-22`**（CPU が「対象を選ぶ」ときに効果の意味を見ていない）。次に `S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-22`・`S-24`・`S-14`＝🏁`S-28` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）②**自己対戦がマリガンを一度も踏まない**（`S-24`）。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4353**）｜**実機 PASS**（6ターン/139手/252s）｜反転確認＝`legacy-fieldcharge` で旧挙動へ戻せる（3山で完全一致を実測） |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🔥**`S-22`（対象選択が効果の意味を見ていない）**→ `S-24` → `S-14`（作戦データ）。
