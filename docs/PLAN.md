...
- 🏁**セッション（2026-08-29・続き711・Opus 5＋Codex）＝§5.3 `O-80` 第2バッチ。`POWER_MOD_PER_COUNT` は「当たらない」より「当たったうえで別のものを数えていた」。**
  ...
  gates 全緑（**golden 2960→2965**・smoke 10702 全0・fuzz 全0・lint 260 据置・同型★0・`census:enginetext` A群 141行 据置）。
  **`POWER_MOD_PER_COUNT` は miss7/live36 → miss4/live17**（採用19効果ぶんちょうど）。全文は BUGFIXES.md 冒頭。
  - 🔴**真因＝実害は「regex が外れる（miss7）」ことではなく、当たった29効果の側にあった。**
    ①`execStubPart1.ts:1679`（10本）と②`effectEngine.ts:2599`（3本）が**効果元カードの全文**に regex を当てており、
    **数え元が常に `lastProcessedCards.length`**（トラッシュを数えろと書いてあっても数えない）／
    **既定分岐が相手3体に満額**（原文が「1体を対象とし」でも3体に効く）／
    **付与能力は効果元が付与先カードなので、まったく別のカードの原文を読む**／**CONTINUOUS は恒久 0**。
  - ✅**採用 19効果（A群14＝ゾーン／場を数える・B群5＝この STUB に居るべきでないもの）／据置 17効果（C群）。**
    🔑**受け皿は A群14件とも既に在った**＝parser が payload を吐いていなかっただけ。**新しいアクション型は 0**、
    既存型へのフィールド追加5本だけ（`CountFromZone.maxCount`/`distinctBy` ほか）。
    🔑**`POWER_MODIFY_BY_SOURCE` と `POWER_MODIFY_PER_HAND_COUNT` に CONTINUOUS collector を新設**＝
    指示書が「この2型に collector は無い」と警告した箇所を、Codex が塞いだ（放置なら**恒久 no-op**）。
  - 🔑**原文データの誤字を1件発見**＝`PR-460` の「パワーを**ー**15000」のマイナスが **U+30FC（長音記号）**。
    CSV 全数走査で同種は**2枚だけ**（`PR-460` / `WX22-Re07`）。CSV は触らず parser 側で限定正規化。
  - 📌**Codex は実装を完走した直後、最終差分確認の途中で利用上限に到達して停止**（報告書は未生成）。
    **検証と簿記は Claude が引き取って完遂**＝不変条件（live 変化は19ちょうど・outlier 0）・
    反転確認（parser 規則を外すと新 golden 2本が赤）・エンコーディング検査（新規増0）を独立実行した。
    🔑**報告書が無くても、この3点を検証側でやり直せば採用判断はできる。**
  - ⚠**census +2 は退化ではなく可視化**＝増分の正体は `WX14-048-E1` / `WXDi-P06-041-E1` の2件だけで、
    どちらも**STUB を外して census の STUB 免除が外れた**もの（続き529 と同型）。`BASELINE_HIGH` を 520 へ実数更新。
  - 📌**分離（§5.3 へ登録）**＝`O-140`（合計値の割り振り・5効果・**現行は相手3体に満額＝過剰実行**）／
    `O-141`（シグニの下のカード枚数・AUTO 経路に受け皿なし・3効果）／`O-142`（「〜と同じだけ」＝直前処理カードの
    パワー値／レベル値を運ぶ受け皿がない・6効果）／`O-143`（`field.check` が1スロットでチェックゾーンを数えられない）。

**▶ 次の一手**＝🆕🔴**`O-51`（§5.3「取る順」2位・279効果）。** 🔴**「L」は実装量ではなくブラスト半径**＝
実装は**約30行**（3箇所へ配線・**新型不要**）で、先例 `INTERNAL_SPLIT_REVEALED`（`execStubPart2.ts:2996`）は**16行**。
対話の受け口も既に全部ある（CPU 自動応答＝`BattleScreen.tsx:673`／実機ドライバも `LOOK_AND_REORDER` を24箇所で処理済み）。
**279効果すべてに対話が1つ増える**ので、**着手前に段取り（どの順で何効果ずつ有効化するか）だけ決める**。
⚠**`src/screens/` を触るので §2.2 の表どおり実機まで必須。**

**`O-80` を閉じ切るなら**＝**第3バッチ＝C群17効果**だが、**5小群それぞれが別機構**（`O-140`〜`O-143`＋lastProcessed 2）なので
**1バッチにまとめない**（`O-76`/`O-77` の catch-all 分解と同じ失敗モード）。**規模順なら `O-140`（5効果・過剰実行が生きている）から。**

**速いレーンを挟むなら**＝§5.2 Sheet2 バッチ2（残 **107件**）。
🔑**取り方はバッチ1 の学びに従う**＝**1カード1 finding の HIGH**（Sheet2 に50件）で `axis` が
`owner/範囲` `count/upTo` `filter.*` のものを10件。**findings が多いカードから取らない**（複雑で機構待ちになりやすい）。
⚠**`build:effects` だけでは既存 id の手修正が live に届かない**＝`syncManualLive.ts` を必ず回す。
⚠**held を申告する前に必ず `build:effects` を回し直す**＝今回 `_held_fresh.json` が 95件で stale（実測 91件）だった。
...
| # | 工程 | やること | 抜けたときに起きること |
|---|---|---|---|
| ① | **取る** | §5 の**上から1件**。並行して別の節から取らない。🆕⚠**`§5.3` だけは例外**＝あの節は**登録順**なので、取る順は **§5.3 冒頭の「取る順」表**に従う（上から取ると1枚の項目を30件消化するまで279効果の `O-51` に着かない） | 作業順がばらつき、どれも終わらない／**規模を見ずに登録順で取ると、影響1枚の項目に時間を溶かす** |
...
> §2.1 ① の「§5 の上から1件」を §5.3 にそのまま当てると、**1枚の項目を30件消化するまで279効果の `O-51` に着かない**。

#### 🆕 取る順（2026-08-29 実測・影響の大きい順）

| 順 | ID | 影響 | 実装量（実測） | リスク | メモ |
|---|---|---|---|---|---|
| **1** | **`O-51`** | **279効果** | 🔴**約30行**（3箇所へ配線・**新型不要**） | **中** | **「L」は実装量ではなくブラスト半径**（279効果すべてに対話が1つ増える）。流用先の先例 `INTERNAL_SPLIT_REVEALED`（`execStubPart2.ts:2996`）は**16行**で「既存の分割UI へそのまま載せるだけ」。対話の受け口も既に全部ある（CPU 自動応答＝`BattleScreen.tsx:673`／実機ドライバも `LOOK_AND_REORDER` を24箇所で処理済み）。**段取り（どの順で何効果ずつ有効化するか）だけ先に決める。** ⚠`src/screens/` を触るので**実機まで必須**（§2.2） |
| 2 | `O-77` | 15効果 | catch-all の分解 | 中 | `LRIG_UNDER_CARD_OP` が22文型の catch-all |
| 3 | `O-76` | 11効果 | catch-all の分解 | 中 | `LOOK_OPP_LIFE_TOP` が20文型の catch-all |
| 4 | `O-135` | 回帰が広い | インライン収集を族ごとに足す | 大 | `O-138` の前提でもある |
| — | **`O-80` 残 C群** | **17効果** | 🔴**5小群がそれぞれ別機構**＝`O-140`（5）／`O-141`（3）／`O-142`（6）／`O-143`（1）＋ lastProcessed 2 | 小〜中 | 🔴**1バッチにまとめない**（`O-76`/`O-77` と同じ catch-all 分解の失敗モード）。**規模順なら `O-140` から**＝5効果で、しかも**いまが過剰実行**（「合わせて－20000」が相手3体それぞれに満額＝計 －60000）。 |
| — | `O-60` | **76カード / 43バッチ** | 1バッチ＝1ハンドラ | 小 | 🔴**「138ハンドラ」は効果数ではない。** 本命は **miss 43ハンドラ**（regex が1本も当たらず既定値に落ちている）で miss カード70枚＝**1バッチ平均1.6カード**。**「1〜3枚のために1バッチ」と同じ構造**なので**後回しでよい**（ratchet ゲートが `gates` に同梱済みで増加は止まっている）。 |
...
| **O-51** | 🆕**「残りを好きな順番でデッキに戻す」の**並べ替え対話**が remainder に無い** | 🆕**実装は約30行**（3箇所へ配線・**新型不要**）／🔴**「L」はブラスト半径**（279効果すべてに対話が1つ増える）＝2026-08-29 較正 | 🔴**2026-08-24 続き640 の実測で登録票を訂正した。** 旧記の「`remainder.position:'any'` は何も起きないどころか誤って上へ置く死フラグ」は**事実ではない**：`location:'deck'` と `position` が top/bottom/split 以外の組は **0件**で、`'any'` の39効果は全部 `trash`(37)／`energy`(2)／`hand`(1) との組であり、engine は `location==='deck'` のときしか `position` を読まない（`effectExecutor.ts:8845`）＝**無害**。 ■**真の母集団（実測）**＝原文に「好きな順番」を含む効果 **377**（カード363枚）。うち **`REVEAL_AND_PICK.remainder{deck,bottom}` 258／`{deck,top}` 21＝真の穴 279**、**既に `LOOK_AND_REORDER{reorder:true}` で正しく表現済み 47**（§5 `3-3⁷` の引き算分―**ここは触らない**）、構造が別 30ほか。 ■**直し方の見立て**＝新型は不要。`remainder` に `reorder?:boolean` を足し、engine 側は **`INTERNAL_SPLIT_REVEALED`（`execStubPart2.ts:2939`）と同じ手口**で既存の `LOOK_AND_REORDER` 対話（`destPosition:'top'|'bottom'`）へ回す。配線先は **3箇所**（`execRevealAndPick`≈`:5925`／`execLookPickChain`≈`:6069`／`resumeSearch`≈`:8835`）。 ⚠🔴**1バッチの粒度に合わない**＝279効果**すべてに対話が1つ増える**（ブラスト半径が大きい）。取るなら**先に段取りを決める**こと。なお CPU は `LOOK_AND_REORDER` を自動応答する（`BattleScreen.tsx:669`＝順を変えない）ので進行不能にはならない。 |
