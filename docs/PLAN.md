...
- **🆕 セッション（2026-08-10・続き418〜421・Opus 5）＝「所有者反転／コスト取り違え」を計器で追って4波。census 860→854・golden 1753→1766。49効果/47カード是正（うち🔴自傷7件・🔴コスト取り違え16件）**。
  - **🔴① 続き418＝「代わりにそれを＜動詞＞」の per-target 形を parser が意図的に除外**していたため、昇格文が**別ステップとして残り**、主語のない「それを〜する」が `owner:'self'` へ反転＝**自分のシグニをバニッシュ/付与**。PLAN が「UNKNOWN 2件」と書いていた家族の**本体は無言の過剰効果**だった。
  - **🔴② (B) 対象プロパティ条件を (A) 盤面状態の形で書くと恒久 no-op**＝`LAST_PROCESSED_MATCHES` が**対象選択より前**に評価され常に偽。**実装途中で踏み、golden の「構造」ではなく「実行結果」テストで捕まえた**。同じ理由で POWER_MODIFY の値昇格は加算モデル。
  - **🔴③ 続き419＝②を計器化したら別家族**＝「どちらか１つを選ぶ。①…②〈任意コスト〉てもよい。そうした場合、…」で**選択肢が2文構成だと CHOOSE が丸ごと消え**、自傷2件＋コスト消失2件。併せて **`OPTIONAL_COST.coinCost` を parser が一度も生成していなかった**（engine は実装済み・唯一正しいカードは MANUAL 手書き）。
  - **🔴④ 続き420＝`TARGET_AND_DISCARD_HAND` は payload を持たず、engine が手札の最後の1枚を問答無用で捨てていた**。53カードで 捨て札filter脱落23／任意性脱落17／選択不能は全件。正準形 `OPTIONAL_COST{handDiscard}` へ18効果を移送＋ルリグ宣言の取りこぼし（自分のシグニをダウン）2件。
  - **🔴⑤ 続き421＝同じ STUB が「幻の手札コスト」でもあった**＝producer が**コストの種類を問わない catch-all** で、原文が**エナゾーン/自己トラッシュ/下から**でも同じ STUB を返す。engine は名前どおり手札を捨てるので、**①原文のコストが一切支払われない（タダ撃ち）②原文に無い手札1枚が消える**の二重バグ（16効果）。`energyTrash`／`underAnySigniTrash`／`OPTIONAL_TRASH_SELF` へ載せ替え。
  - **⭐⑥ 4波を貫く教訓＝「engine にあるのに parser が生成しない」／「parser が出すのに engine が別の意味で読む」**＝前者は `OPTIONAL_TRASH_SELF`(417)／`coinCost`(419)／`handDiscard`(420)、後者が `TARGET_AND_DISCARD_HAND`(421)。**payload 無しの STUB を見たら、①engine 側の受け皿を grep ②その STUB を出す producer の条件を読む**（catch-all なら engine の固定解釈と必ずズレる）。
  - **⚠⑦ 自分の引き継ぎを疑って数え直したのが当たり**＝続き420 で「好きな枚数が最大クラスタ」と書いたが、据置35件を理由別に数え直すと**実際は5件**で、最大は「手札捨て節が本文に無い」19件＝そこに⑤の幻コストが埋まっていた。**引き継ぎの見積もりは着手前に実測し直す。**
  - **⚠⑧ 逆翻訳が計器として穴だった**＝`OPTIONAL_COST` の逆翻訳が `energyTrash` を見ておらず「コストを支払ってもよい」に潰れていた＝**原文照合でコスト取り違えを見つけられない状態**だった（幻コストが長く残った一因）。`energyTrash` と `underAnySigniTrash` の絞り込みを描くようにした。**計器自身の穴を疑うこと。**
  - **⚠⑨ 変換系は必ず全件の対応表を印字して原文と突き合わせる**＝続き420 で spec→filter の取りこぼし検査を空 filter の早期 return より**後ろ**に書いており、カード名指定が落ちて「手札の任意の1枚」に化けていた。表を出したから気づけた。
  - **⚠⑩ 計器の限界**＝「原文に『あなたの』が無いのに JSON が self」は引用付与の内側と自己コストを誤検出（27件中 真陽性4件）。「①②があるのに CHOOSE が無い」は「以下の２つを**行う**」を誤検出＝ヘッダを併せて見る。
  - **✅⑪ ゲート**＝`npm run gates` 全緑（golden **1766（+13）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **854** / 同型★ **0**（265群）/ lint 0 errors）。`build:effects`＋`regen` 実施済み。**要実機検証**＝続き420 の手札コスト pay/skip UI（18効果）＋続き421 のエナコスト徴収（§7 に登録）。
  - **📋⑫ 次の一手**＝**Opus 側**＝(a)**`TARGET_AND_DISCARD_HAND` の残19件を同じ軸で仕分け**（【起】コスト行＝対象外の見込みだが**未確認**。⑦の教訓どおり実測すること）(b)**`OptionalCostSpec` に上限つき可変枚数**（「好きな枚数捨てる」5件）(c)🔴**`WX25-P3-014-E1`**＝`BOUNCE{owner:'self'}` の自傷（did-it ゲートと干渉）(d)**§6.4 A群の着手可能2件**(e)**`WX12-010`＝強制アタック機構**。**Sonnet 側**＝§7 実機検証（**続き420〜421 のコストUI を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418〜421（所有者反転／コスト取り違えを計器で4波）後 最新値（本行が直近の正）**：census **854**（860→854・`BASELINE_HIGH` 更新済み）、golden **1766（+13）**、`MANDATORY_SUSPICIOUS` **2**（据置）、held **122カード／48群**（うち採用済み14枚は除外表示）、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
