...
- **🆕 セッション（2026-07-31・続き308・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxi) 第7波＝`WX24-P4-011-E3` を消化（在庫 残5→4）＝バトルバニッシュの遅延トリガー収集経路を engine に新設**（golden **1203→1205**・census **1357→1356**（`BASELINE_HIGH=1356`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **251→250枚/98署名**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等）
  - **① 直したのは過剰実行**＝「《リコレクト》［4枚以上］このターン、あなたのシグニがバトルによってシグニ1体をバニッシュしたとき、対戦相手が《無》《無》を支払わないかぎり、対戦相手にダメージを与える」が `SEQUENCE[RECOLLECT_GATE, LIFE_CRASH]`＝**起動した瞬間に無条件でダメージ**になっていた（バトルバニッシュを待たず、相手に回避の機会も与えない）。
  - **② 据置理由は engine 側にあった**＝`INSTALL_DELAYED_TRIGGER`（B3）の `delayed_triggers` を見る収集は **ON_LEAVE_FIELD／ON_REFRESH／フェイズ・ターン境界系の3経路だけ**で、バトルバニッシュ経路（`BattleScreen` のバトル解決）は**場のシグニ効果しか走査していなかった**。収集器 `collectBattleBanishDelayedTriggers` を `triggerCollect.ts` へ**pure 新設**し（BattleScreen 側は push 1行＝golden から検査可能にするため）、バニッシュした側の state だけを見る＝原文「**あなたの**シグニが」の主語限定。
  - **③ parser はテーブル駆動化**＝第6波のハードコード（「このターン終了時、」だけ遅延設置へ変換）を `DELAYED_INSTALL_PREFIXES`（前置き regex→timing）へ一般化して1行追加。⚠**足してよいのは engine に収集経路がある timing だけ**（無い timing を足すと「設置されるが永久に発火しない」＝過剰実行を no-op へ替えるだけの別の退化）。全CSV機械走査で該当は **1枚のみ**。
  - **④ 据置 golden を陽性 assert へ転換**（第5波 `TEST-WA-DEFER` と同じ）。第4波の tail-splice 据置 golden も同文面を使っていたので「**収集経路の無い** timing」へ差し替え、生きている不変条件を固定し直した。⚠**据置 golden を残すと将来の採用を誤って禁止する。**
  - **⑤ 🆕 同族の未消化を2効果みつけた（対象外・PLAN §3 タスク12(lxxi) へ登録）**＝`WX26-CP1-040-E2`（無条件ダメージ）／`WX24-P1-011-E1`（即時 UP＋能力消去）＝「このターン、**次に**…バニッシュしたとき、」の設置が落ちて即時実行。受け皿（収集経路）は本波で出来たが、**「次に」＝1回だけの消費セマンティクスが `delayed_triggers` に無い**ので先に機構を決める。
  - **次の一手**＝**Opus**：①🆕**タスク12(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く可能性が高い在庫）②🆕**(lxxi) 遅延設置の「次に」消費機構**（受け皿は出来た）③(lxviii) 散文形「対戦相手のターンの間、」④§6.3 J 残11 ⑤(lxvii) CPU `ON_TURN_END` 未収集 ⑥§6.4 配置数制限すり抜け7効果 ⑦(lxi) 残4／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜308 の全件が実機UI未検証で積み上がっている**（本波の相手側《無》×2 CHOOSE も未検証）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き308（タスク12(lxi) 第7波＝`WX24-P4-011-E3`・1効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1357→1356**（`BASELINE_HIGH=1356`＝設置が落ちて無条件ダメージだった1効果の**機能是正**）、golden **1203→1205**（据置 golden 1本を陽性 assert へ転換＋collect/engine 実走2本を新設）、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251→250枚／署名グループ 99→98**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WX24-P4-011-E3`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 14 据置**（⚠**実質12**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク12(lxi) 残＝5→4**。**新語彙0本**＝`INSTALL_DELAYED_TRIGGER` も `ON_SIGNI_BANISH_BATTLE` も `OPPONENT_PAY_OPTIONAL` も既存。engine 新設は**収集器1本**（`collectBattleBanishDelayedTriggers`＝pure・BattleScreen 側は push 1行）、parser は第6波のハードコードを `DELAYED_INSTALL_PREFIXES` テーブルへ一般化して1行追加、decompiler は遅延設置用の主語分岐を1本追加。
