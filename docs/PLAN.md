...
**直近＝2026-09-16＝第380バッチ：§5.2 round6 R6-0（ハーネス整備）**（全文は [BUGFIXES.md](./BUGFIXES.md)・台帳は `round6/TYPE_LEDGER.md`）
- `behaviorAudit.ts` に ①位置の死角12種 ②盤面の変種（基本／満杯／枯渇） ③候補の全列挙 を入れた。全カード × 3変種で (消滅) **108行 → 1件**（`O-528`＝ルール確認待ち）。
- 死角をふさいだ途中で engine の「カードが消える」実バグを2件直した（トラップ設置の残り札／相手トラッシュ→シグニの下）。
- 🔴**完了条件②の再現率は 6/16 で未達** ⇒ 計画どおり **R6-2（LLM 監査）はやらず R6-1 で閉じる**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.2 round6 の R6-1**（不変条件センサス）。`O-528` はユーザー判断待ち（索引H） |
| 📊**進捗3計器** | Sheet1 要対応 **2 / 863**（`census:cards -- --sheet 1` で測り直す）／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **2**（索引G 1・索引H 1）／実機 **0**／実装キュー **0** |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4263） |
...
**現状**＝round4（全11シート）・段2 台帳・round5（全5,976枚）は完了。round6 は **R6-0 済（2026-09-16・完了条件②の再現率 6/16 で未達）⇒ R6-2 はやらず R6-1 で閉じる。**
...
| ID | 規模 | 何が無いか（一行） |
|---|---|---|
| `O-527` | S・**未計測**（候補1＝`WX16-061-E1`） | 「【トラップ】１つをトラッシュに置く。そうした場合、…」でトラップが無くても後続（デッキ上3枚から設置）が走る＝「そうした場合」が `IS_MY_TURN` で表されている |
...
| ID | 規模 | 何を判断するか（一行） |
|---|---|---|
| `O-528` | S・**1効果**（`WX11-034-BURST`） | 【チャーム】が既に付いている対戦相手のシグニに、新しい【チャーム】を付けられるか（いまは付けられて、古い【チャーム】がどこにも無くなる） |

**`O-528`**＝原文「対戦相手のシグニ１体を対象とし、対戦相手のデッキの一番上のカードをそれの【チャーム】にしてもよい。」。
**いまの実装**＝`execAttachCharm` は対象の既存【チャーム】を見ずに上書き＝古い【チャーム】が消える（R6-0 の全数で唯一残った (消滅)）。
**読みA**＝【チャーム】付きのシグニは対象にできない（候補から外す）。**読みB**＝付けられて、古い【チャーム】はトラッシュへ置かれる。
**コーパス**＝「【チャーム】が付いていない」と書く原文は0件＝minimal pair 無し。
...
- **2026-09-16 時点**（第380バッチ＝round6 R6-0）
  - 📊**進捗3計器**＝Sheet1 要対応 **2 / 863**（held 1・mech 1）｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**
  - 📦**在庫**＝機構 worklist **2**（索引G 1＝`O-527`｜索引H 1＝`O-528`）｜実機 **0**｜実装キュー **0**｜round6 **R6-0 済（②未達）→ R6-1**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden 4263）
