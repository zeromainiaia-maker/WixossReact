...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕単独）＝(xlvi) 第5〜6波（28効果）→ (xxix)(1) 任意+cost 853効果 → (xxix)(2) で英知の全角「＝」取りこぼし24効果を発見・是正**（golden 936→**965**・census 1463→**1452**〔`BASELINE_HIGH` 更新〕・smoke 10726 全0・fuzz 全0・同型★0・held 257→**254**〔減少＝改善〕・build 冪等）
  ...
  - **(xxix)(1)**＝効果で場に出たシグニの任意+cost【出】933効果が**丸ごと不発**だった（通常召喚には支払いモーダルがあるが、効果配置はスタック解決の途中でそのフローに入れない）。**支払い選択を action に埋める**＝`SEQUENCE[OPTIONAL_COST, 元action]` で積み engine の Pattern ⑤ に載せる（BattleScreen 無改造・**golden で検証できる**）。**853件を配線／80件は未対応コストのため据え置き**。
  - **🔴(xxix)(2) が最大の収穫**＝「任意・コストなし65効果」は**3種の混在**だった〔(A) 真の「〜してもよい」8／(B) 原文にコスト句があるのに `cost` が無い43／(C) 英知＝N＝使用条件14〕。**英知の全角「＝」を parser が取りこぼしており24効果が `activeCondition` を丸ごと失っていた**＝**うち15効果は live で「条件なしに発動する過剰実行」**（【常】英知＝５で**無条件**+3000／【常】英知＝８で**無条件**アサシン等）。英知はコストではなく使用条件なので【出】14効果は **mandatory へ復帰**（v0.263 の決定に戻す）、(A)8 は新 `OPTIONAL_ACTIVATE` 包みで発動可否を問う（通常召喚・効果配置の両方）。
  - **🔴(B)43件は意図的に据え置き**＝コスト句があるのに `cost` が無い効果を発動プロンプトで包むと**払わずに撃てる**。parser に `costUnparsed` 印を新設して包み側で拒否し、**PRESERVE カードで印が届かない5効果**を外科パッチ。**母集団全数で「包むなら原文のコスト句は必ず空」を golden の不変条件**にした。
  - **golden の教訓（本セッション通算4つ）**＝①候補集合 assert は filter 自体から候補を作るので**間違った filter を検出できない**→期待値文字列を併記 ②**動的filter は `matchesFilter` では判定できない**→盤面を作って pending SEARCH の `visibleCards` を直に見る ③**「写せるか」を母集団全数で対応キー集合と突き合わせる** ④**危険な側（踏み倒し・過剰実行）を母集団全数の不変条件で塞ぐ**。HEAD 差し戻し／filter 弱体化／`resolveDynamicFilter` 外し／collector 分岐戻し／`costUnparsed` ガード外し／live JSON からの英知条件削除、の6種の変異でそれぞれ狙った本数だけ FAIL することを実測。
  - **⚠要実機検証**＝(xxix)(1) は engine 側だが**表面化は BattleScreen の effect-stack 解決**（`EffectInteractionModal` の任意コスト選択）。(xxix)(2) は **`handleSummonSigni` を直接変更**（通常召喚で「発動しますか？」）。どちらも実機未確認。
  - **⚠未決＝`EICHI_LEVEL_SUM` の operator**。既存実装・MANUAL・v0.263 記録すべてが `'eq'`（ちょうどN）で一貫しているので合わせたが、同一カードに ＝５/＝８/＝１１ が並ぶ設計（`WX17-073`）を見ると **`'gte'` が正しい可能性**。24効果に一律に効くので単独項目として §3 (xxix) に登録した。
  - **次の一手**＝**Opus：(xxix) 残**〔**operator eq/gte の確定（24効果に一律・要ルール確認）**／(2)(B) 43件＝`parseCost` の在庫（`costUnparsed` 印が付いた**331効果**が機械的 worklist）／(1) 未対応コスト80件〕**／(xlvi) 残20件**〔(a)残2・(b)10・(c)4・(d)残2・(g)2＝いずれも新機構が要る〕**／ゲート脱落 [B]20件（Condition 語彙の新設）／タスク16 の [B]29件＋watcher [B]4件／(l) 内側【自】parse 失敗3枚／§6.3 残機構（H1〜H3・I）**。**Sonnet：§7 実機検証の横展開**＝**まず本セッションの (xxix)(1)(2)**（効果配置→任意コスト【出】プロンプト／通常召喚→「発動しますか？」）＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓・escapeDiscard 回避モーダル・BET 選択肢・アーツ使用条件20枚。
