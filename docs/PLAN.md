...
- 🏁**セッション（2026-08-25・続き655・Opus 5）＝①ツール改善（`npm run golden` にテスト名フィルタ）＋②§5.3 `O-66`① を消化＝ライフクロスの「クラッシュされない／N枚までしかクラッシュされない」機構を新設した**。gates 全緑（**golden 2770→2778**／census 574=574／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260）。**実機2本を新設し `O-64`／`O-65` の7本と併せて計9本が2回連続 ALL PASS＋反転確認済み。**
  - ✅**①`npm run golden -- --only "<部分文字列>"`／`--list` を追加**（全2,770本で約89秒 → 絞ると**約1.5秒＝98.3%短縮**）。**引数なしの挙動・出力行数は完全に不変**で `npm run gates`・CI は常に全件。0件マッチは **exit 1**（タイプミスを `PASS 0 / FAIL 0` の緑に見せない＝vacuous PASS 防止）。🔴**フィルタ実行の結果は権威ではない**（`cursor` がテスト間で共有される可変状態＝スキップすると後続が引くカードが変わり**両方向に化けうる**）＝実行時に3行で警告し、運用を §2.1 ④ に明記した。⚠**「`test()` 外側の素のトップレベル15ブロックはフィルタしても走る」は実測すると全体の1.6%**（フロア1.45秒）＝**89秒のほぼ全部が `test()` 本体**だった。
  - ✅**②母集団は実測6効果**（登録票と一致・隠れた追加なし）。**軸は3つ**＝(a)全面防止 (b)「ダメージ以外」限定 (c)1ターン上限。判定を **`src/engine/lifeCrashGate.ts` の1本**へ寄せた。
  - 🔴**クラッシュの実行地点は3つある**＝①シグニアタック（`crashOneLife`）②**ルリグアタック＝`crashOneLife` を通らないインライン**③効果（`execLifeCrash`）。**3地点とも通した**（1つでも漏らすと「シグニは防げるがルリグは素通り」の無言の不整合）。
  - 🔴**作業中に既存バグを1件その場で修正**＝**ルリグアタックの経路だけ `life_crashed_this_turn` を加算していなかった**（他3経路は加算済み）＝既存の `LIFE_CRASHED_THIS_TURN` 条件がルリグアタックのダメージを数え落としていた。
  - 🔑**fail-closed の向きを決めた**＝payload の無い宣言は**無視する**（parser が落としても「効かない」で済み、「全ダメージ無効化」へは倒れない）。golden は**payload の有無まで**見る。
  - ⚠**「ダメージ以外によっては」を「ダメージだけ防ぐ」と読むと守り札が攻撃無効化カードに化ける**／**上限型は全か無かではなく枚数を切り詰める**（原文注記）／**「各プレイヤーの」は相手の場のキーでも自分を守る**＝この3点は golden で両方向を固定した。
  - 📌**follow-up**＝`O-66`②③ は未着手のまま残置。🆕`WD06-008-E1` の「**このターン終了時**、ライフクロスの一番上をトラッシュ」は**遅延が落ちて即時実行**（受け皿なし）＝今は同 SEQUENCE の防止宣言に阻まれて no-op で、**実害は消えたが正しくはない**。

**▶ 次の一手**＝**§5.2 段2 のバッチを取る**（残 OPEN **678**）。⚠🆕**2026-08-25 続き656 の実測＝第2層で 5件以上の群は 17群120件しかなく、上位3群はいずれも parser の場所が別々だった**＝**軸でも第2層でも束ならない群がある**。⇒ **具体的な語彙・文型に降りて生データ（CSV原文 × live JSON）で母集団を測る**ほうが速い。🔴**`census:wiring` の `cardClass × SIGNI[filter]` は掘り尽くし済み（§5.4 (i) に `[x]`）＝再度掘らないこと。**次の候補は `cardClass × (filter無)` 9（★★・条件節用法が濃いので要仕分け）／`powerRange × GRANT_KEYWORD{SIGNI}` 6（★）。あるいは §5.3 の **`O-67`（S・実害あり＝コストなしで起動できる1件を含む）**を先に潰す。
- 🔑**クラッシュの防止・制限は「実行地点を数える」ことから始める**（`O-66`）＝受け皿を1つ作っても、消費地点が3つあれば**2つ素通り**する。`grep` で経路を全部数えてから配線する。
- 🔑**開発ループで golden を絞ってよい／1巡を閉じる前に必ず全件**（`--only` の運用＝§2.1 ④）。
...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-67** | 🆕**「自分のシグニを**バニッシュする**」をコストにする語彙が `EffectCost` に無い** | **S** | **2026-08-25 続き656（§5.2 段2 の母集団調査）で発見。**■**`EffectCost` の場シグニ支払いは `fieldTrash`（場→**トラッシュ**）だけ**で、**バニッシュ（＝場→**エナゾーン**）を表すキーが無い**。⇒ 原文が「バニッシュする：」のコストは、**①丸ごと落ちる**（`WX05-044-E1`「《ダウン》他の＜古代兵器＞のシグニ１体をバニッシュする」＝live cost は `{down_self:true}` のみ＝**コストなしで撃てる**・意味照合でも HIGH/MISSING）か、**②`fieldTrash` へ化ける**（`WX25-P1-022-E1`「レベル２以下の＜原子＞のシグニ１体をバニッシュする」＝live は `fieldTrash`）。🔴**②は「行き先が違う」実害**＝バニッシュならエナゾーンに置かれて**エナが1枚増える**のに、トラッシュへ送られて資源を失う（§4.4 罠8f と同じ取り違え）。■**実測母集団は2効果**（コスト節＝先頭〜最初の「：」に「バニッシュする」を含む効果の全数走査。3件目の `WX25-P1-TK2-E1` は本文側の一致＝偽陽性）。■**要る形の見立て**＝`EffectCost.fieldBanish?: { count; filter?; excludeSelf? }` を足し、支払い経路（`fieldTrash` と同じ選択UI）と**行き先だけエナゾーン**に分岐する。⚠**`fieldTrash` を流用して行き先フラグを足す形にしない**＝`fieldTrashCostCards`（`ON_TRASH` の原因弁別に使う ctx）へ載ると**バニッシュがトラッシュ由来として観測される**。■**小さいが実害あり**＝①は**コストなしで起動できる**バグ。■着手前に母集団を実測し直す（§4.1）。 |
...
- **2026-08-25 続き655 後（本行が直近の正）**：
  **census 574/574**、**golden 2778**（`O-66` で 2770→2778＝engine 7本＋live 2本を新設し `O-65` の DEFERRED トリップワイヤを反転）、
  ...
  **意味照合 残 OPEN 678（不動＝`O-66` は §5.3 の機構項目で台帳の母集団外）**、
  **実機シナリオ +19**（🆕`O-66`＝`b66LifeCrashCapOnce`/`b66LifeCrashCapControl`、`O-65`＝`b65KeyTrash*`/`b65Wall*` 4本、`O-64`＝`b64PowerThreshold*` 3本、`O-63`＝`b63TurnOwner*` 2本、第44〜46 の8本）＝**`O-63`〜`O-66` の9本は ALL PASS を2回連続**、
  **`npm run golden` の所要＝全件 約89秒／`--only` 約1.5秒**（フロア1.45秒＝tsx 起動＋import＋トップレベル素ブロック）
