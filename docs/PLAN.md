...
**直近＝2026-09-17＝デッキ一覧をルリグタイプ別フォルダに分ける／CPU デッキを別の種類で持つ／CPU デッキのランダム選択／フォルダのサムネイル**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**ユーザー決定**＝フォルダはセンターのルリグタイプで自動（複合タイプは複合名1つ・未指定は「未設定」）。CPU デッキは `deck_kind='cpu'` で自分のデッキと混ぜない（全員が自分用に作れる・いまは本人だけ）。
- Supabase＝`decks.deck_kind` ＋ `deck_folders`（フォルダのサムネイル）。判定は `utils/deckFolders.ts`。
- 🔴**リリース時の作業が1件ある**＝管理者の CPU デッキを全員に公開する行ポリシー＝[RELEASE.md](./RELEASE.md) §1.1（クライアントの変更は不要）。
- 実機＝CPU 通し対戦（フォルダ越し選択）PASS／`CPU_RANDOM=ピルルク` PASS／`V-262` PASS／🆕`V-265` PASS。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **`C-8`**（対話応答の pure 化＝§5.6 の最後の1件） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は UI・DB＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／**CPU 完成度 1**（§5.6 `C-8`）／ルール台帳 ⚠0（残件0）／**リリース作業 1**（RELEASE.md） |
