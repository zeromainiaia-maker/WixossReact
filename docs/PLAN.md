...
- 🏁**セッション（2026-08-25・続き660・Opus 5）＝§5.3 `O-66` の残り3件（②③④）を消化＝これで `O-66` は全消化**
  - ✅**②「シグニは〜されない」＝両プレイヤーのシグニ**（`WXEX2-44-E1`・実測1効果）。穴は2段＝①parser が `owner:'self',count:1` へ潰す ②engine に**相手の盤面に立っている宣言がこちらへ届く経路が無い**。⚠`activeCondition` は**宣言元の側から**評価する（`state`/`otherState` と `isOwnerTurn` を入れ替える）＝忘れると「対戦相手の」が反転する。
  - ✅**③`EffectText` を regex で読んでいた STUB 5種（6効果）を payload 化**（`underAbilityGrant` / `moveProtectFilter`）＝`collectGrantedFromUnderSigni` から **`EffectText` の読み取りを全廃**。⚠**fail の向きが2種類ある**＝`underAbilityGrant` は**無ければ何も付与しない**（無いと「下の全能力を得る」に化ける）／`moveProtectFilter` は**無ければ全部が対象**（原文がそう書く）。同じ規約だと思って揃えない。
  - 🔴**③の実機検証で「実戦では一度も動いていなかった機能」を発見・その場で修正**＝`collectGrantedFromUnderSigni` が上のカードの効果を**インスタンスIDだけ**で引いており（実戦の盤面は必ず `WX21-024#1` 形）、**「下のカードの能力を得る」が丸ごと no-op** だった。⚠**golden は素のカード番号で盤面を作るので素通りしていた**＝実機でだけ見える型。
  - ✅**④「このターン終了時、〈本文〉」の遅延落ち**＝登録票は1効果だったが**実測すると原文24文のうち遅延が生きていたのは4件だけ**。真因は「本文が `CHOOSE` のときだけ設置へ変換」という足がかりの制限。**無条件に広げると5効果が退化する**ので3ガード（STUB本文／複数文／照応）を置いて **14効果**を動かした。
  - 🔑**副産物**＝「対戦相手は自分のシグニN体を選びトラッシュに置く」の **N が常に1・`opponentSelects` も落ちる**穴（実測6効果）。④で遅延句を配線して初めて見えた（SONG 専用経路が覆っていた）。
  - ⚠**実機ハーネスに `onplaycost-hand-${i}` を新設**＝**【出】手札コストのセルに testid が無く**、手札コスト持ちカードの実機検証自体が組めなかった。

**▶ 次の一手**＝**§5.3 の `O-71`／`O-72`（本セッションで登録）か、`O-70`／`O-68`／`O-69`**。
- 🔑**「登録票の件数」は標本**（§4.1）＝`O-66`④ は1効果の登録が**実測24文・実変更14効果**だった。**着手前に必ず原文から測り直す。**
- 🔑**広げる変更は A-B 実測で「退化する件数」を先に数える**＝`O-66`④ は無条件に広げると5効果が退化した。ガードは**実データから**決める（想像で足さない）。
- 🔑**golden が緑でも実機で no-op のことがある**＝盤面の**インスタンスID**（`#1`）は golden が再現しない軸。**新機能の実機検証は「本当に動いたか」を盤面差分で見る。**
---
