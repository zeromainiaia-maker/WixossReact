...
- 🏁**セッション（2026-08-24・続き639・Opus 5）＝`O-50` を残0クローズ**（codex-work 委譲・Claude は指示と検証）。gates 全緑（**golden 2662**／census 608 据置／smoke 10693 全0／fuzz 全0／lint 269 増分0／同型★0）。台帳 **730→729**。
  - ✅**「残りをシャッフルしてデッキの一番下に置く」を live が「デッキ全体を `SHUFFLE_DECK`」と表現していた15効果を構造から是正**。`SHUFFLE_DECK` は 15/15 で除去、行き先は 15/15 が `{deck, bottom, shuffle:true}`。**スコープ外0・追加0・削除0。**
  - 🎁**shuffle より重い過小実行が2件同時に直った**＝`PLACE_SEED_FROM_REVEALED` が **`deck.slice(0,4)` と `maxPick:1` をハードコード**していたため、`WXK04-010-E1`「4枚見て**2枚まで**【シード】」が**1枚しか置けず**、`WXEX1-13-E2`「**5枚**見て**2枚まで**【**トラップ**】」が**4枚しか見ず1枚だけ【シード】**になっていた。⇒ 既存 `LOOK_PICK_CHAIN` へ組み替え（新型なし・`then:'seed'` を1語追加しただけ・`'trap'` は既存経路を維持）。
  - 🔑**投入前実測で PLAN の記載を2点訂正**＝①**`WXEX1-13-E1` は誤登録**（原文はシャッフルしない）②**母集団は15ではなく18効果**（うち `WX25-P2-026-E2` は「残り」が**ライフクロス**で `SHUFFLE_DECK` が正しい＝スコープ外へ分離）。
  - 🔴**Claude が第33バッチと同じ穴をまた踏んだ**＝全走査を**カード単位**でやって `WXEX1-06-E1`（原文は「デッキをシャッフルする」）を誤検出。**`docs/_effect_srctext.json` で効果単位に数え直して解消**（§5 `3-3⁹`）。**母集団はカード単位で数えない。**
  - 🔍**指示していない4ファイルを精査して正当と確認**＝`manualEffects.ts`（削除候補0＝影武者でない）／`types/effects.ts`（既存 union に1語）／`vocabCensus.ts`（BASELINE 据置・較正で黙ったのはスコープ内9件のみ・新規0）／`_idset_fresh` 46→**45**。

**▶ 次の一手**＝**§5.2 の段2 消化を続ける**。最有力は **`O-51`**（「残りを好きな順番で」の並べ替え機構）。⚠**着手前に「既存の `LOOK_AND_REORDER.reorder` で正しく表現済みの分」を引き算する**（原文282カードは素のカウント）。それ以外は §5.2 の第2層（軸 × live の action 型 × type）で 5件以上のサブ群から取る（2026-08-24 実測＝320サブ群・5件以上が38個）。
...
- **2026-08-24 続き639 後（本行が直近の正）**：
  ...
  **実機シナリオ定義総数 518**（続き638・639 は parser/engine の構造是正のみで実機シナリオ追加なし）、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
---
## 7. 検証ハーネス
