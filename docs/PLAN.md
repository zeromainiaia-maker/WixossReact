...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15` → `O-10`（明示 defer **残 2 id/2件**。続き507〜515 で8巡済み＝残りはどちらも engine の構造に手を入れる話〔`ON_TARGETED` の収集地点／ピースの応答窓〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 2 id / 2件 | 🆕**2026-08-16 続き515（8巡目）＝3種/3件 → 2種/2件**（`WXDi-P16-001A-E1`＝チェックゾーン裏返し→無償グロウ）。🔑**旧・据置理由「グロウ先の B面 `WXDi-P16-001B` が CardData CSV に無い」は古かった**＝B面は `CardData_TK.csv` に**ルリグ Lv4 として実在**する（A面もピースとして `CardData_Sheet8.csv` にある）。**前提は着手のたびに grep で確かめる。** 新機構は2つだけ＝(a)`lrig_grew_this_turn`〔「このターンにグロウしていない場合」の判定材料。**書くのはグロウの2経路**（人間 `executeGrow` ／ CPU グロウ）＝片方に書き忘れると「CPU のターンだけ条件が通る」ズレになる〕(b)`pending_flip_grow_card`〔engine は**条件判定と予約だけ**。実グロウは BattleScreen が `executeGrow`＝**正規経路**で行う。🔑engine で `field.lrig` へ直接 push すると**グロウ時トリガー（【出】）・リミット再計算・コイン獲得が丸ごと落ちる**（`GROW_FREE` が「BattleScreen 処理」なのと同じ理由）〕。⚠**グロウ先はカード名で載せる**（CardNum は原文に無い）＝engine が `cardMap` から Type=ルリグ で解決し、見つからなければ**何もしない**（壊れたルリグを作らない）。⚠**裏返してルリグになる**ので、ピース使用でルリグトラッシュへ送られた自分自身を取り除く（リコレクト枚数のズレ防止）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き515）。⚠**全経路 実機未検証**＝§7 送り。**■残 2 種 / 2 件（どちらも engine の構造に手を入れる話）**＝`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔**ピース使用への応答窓**が無い＝カットインは現状スペルにしか無い。`pending_spell` 相当の `pending_piece` と、その窓で使えるピースの収集・UI が要る〕／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔`ON_TARGETED` が **`resumeSelectTarget` の適用後**に積まれるので「対象の無効化」の余地が無い＝収集地点を適用前へ動かすのが前提（続き511 で特定）〕。**定期的に前提を再判定する**（続き507〜515 の計18種のうち、**「機構が無い」が本当だったのは2種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付け／見落とし・自分で書いた据置理由の陳腐化・未消費 id・**カードデータが後から入った**だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
