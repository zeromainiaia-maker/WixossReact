...
| ✅(l) | **✅2026-07-30 に B群残0でクローズ（A群残8は明示保留）＝「能力付与の内側が nest されず完全 no-op」**。**A群11件**は前波で3件実働化（`WXDi-P11-038-E2`／`WX26-CP1-005-E1`／`WXDi-P02-034-E1`）、**残8件**＝`WX26-CP1-076-SONG`／`WXK11-052-E1`／`WX24-P4-026-E1`／`WX16-004-E1`／`SPDi44-08-E2`／`WX25-P1-018-E2`／`WD21-009-E1`／`PR-204-E1`（比例値・任意コスト・置換・宣言等の機構不足。部分近似は過剰実行になるため honest defer）。<br>**🆕B群36枚・47効果を 2026-07-30 に消化＝残0**。キーは「【常】：あなたの（レベルN以上の）センタールリグは以下の能力を得る。」の**直後の別ブロック**に付与能力を並べる（原文に入れ子マーカーが無い）ため、宣言文の `rawText` が「。」だけになり `abilities` 空＝executor 完全 no-op、付与能力は**キー自身のトップレベル効果**として登録されていた。parser に境界抽出（**エクシード【起】の走り＋宣言直後の【自】**まで飲み込み、【出】/【常】/「このキーを…」【起】で打ち切る）と `activeCondition` 復元を実装し `GRANT_LRIG_ABILITY.abilities` へ入れ子化。<br>⚠**PLAN の旧評価「優先度は低い／真の過剰実行は WDK02-009 の1件だけ／コスト踏み倒しではない」は2点とも誤りだった**＝(1) `WXK02-023` も「**対戦相手のセンタールリグがレベル２以上であるかぎり**」を前置節の形で持ち、条件が落ちていた（＝落ちていた条件は2件）(2) **エクシードコストはキー【起】経路（`executeKeyActivated`／`KeyActivatedModal`）が `cost.exceed` を一切支払わない**ので、JSON にフィールドが在っても **44効果が実質コスト0で撃てていた**（付与【起】経路 `executeLrigGranted` だけが支払い実装を持つ）。<br>同時に付与【起】経路のゲート不足（timing↔phase 未照合・`condition` 未評価・`once_per_game` なし）と、付与ルリグの SPELL_CUTIN 収集欠落・防御側 `any_opp` の ON_ATTACK_LRIG 収集欠落を是正。census **1394→1393**、golden **1075→1079**、held **292→286枚/106署名**。詳細 BUGFIXES 2026-07-30 (l) 節 |
...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が実装と検証）＝タスク12(l) B群36枚・47効果を消化し B群残0でクローズ**（golden **1075→1079**・census **1394→1393**〔`BASELINE_HIGH=1393`〕・smoke **10679** 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held **292→286枚/106署名**・manual field loss 0。ゲート全緑／live per-effect 差分は影響カード36のみでスコープと完全一致）
  - **主題＝「キーの『あなたの（レベルN以上の）センタールリグは以下の能力を得る。』の内側を構造化する」**。原文は宣言文の**直後に続く別ブロック**として付与能力を並べる（引用符も入れ子マーカーも無い）ため、宣言文の `rawText` が **「。」1文字**になり `abilities` 空＝executor 完全 no-op、付与能力47本は**キー自身のトップレベル効果**として登録されていた。
  - **🔴PLAN の旧評価「優先度は低い／実害は薄い」は2点とも誤りだった**（本セッションで実測して訂正）：①条件が落ちていたのは `WDK02-009` の1件ではなく **2件**＝`WXK02-023`「**対戦相手のセンタールリグがレベル２以上であるかぎり**」を前置節として持つ ②「エクシードコストと《ターン１回》は E2 以降に保持されている＝コスト踏み倒しではない」は**JSON にフィールドが在ることの確認であって支払い経路の確認ではなかった**＝キー【起】経路（`executeKeyActivated`／`KeyActivatedModal`）は**エナ・手札捨て・`trash_key` しか払わず `cost.exceed` を完全に無視する**ので、**44効果が実質コスト0で撃てていた**。付与【起】経路（`executeLrigGranted`）だけがセンター→左右アシストの順に下からN枚を送る支払い実装を持つ。
  - **境界抽出の規則**（原文に入れ子マーカーが無いので構造で切る・live 36枚47効果の全数照合で一致）＝**①【起】でコストに エクシード を持つブロック→付与**（エクシードはルリグ専用コストでキー自身には付かない）**②【自】はまだエクシード【起】を見ていない間だけ付与**（`WXK02-028`「このルリグがアタックしたとき」／`WDK04-006`）**③それ以外で打ち切る**（【出】＝キーの登場時／【常】＝キー自身の常在／「このキーを場からルリグトラッシュに置く」等の非エクシード【起】）。飲み込んだブロックはトップレベルに残さないので**二重発火なし**。スコープ条件は `activeCondition:{LRIG_LEVEL}` へ復元し、**未対応の前置修飾が残る場合は入れ子化そのものを諦める**（条件の黙殺を構造的に防ぐ）。
  - **付与【起】経路のゲート不足3点を同時に是正**（47効果がこの経路に流れ込むため）＝timing↔phase 未照合（《アタックフェイズアイコン》専用がメインでも撃てた）／`condition` 未評価（`manualEffects.ts` の旧コメントが「granted 経路は condition 未評価だからキー top-level のまま保持するのが正しい」と明記していた既知の穴）／`once_per_game` なし。加えて**付与ルリグの SPELL_CUTIN 収集**（`WXK08-005` のスペル打ち消し）と**防御側 `any_opp` の ON_ATTACK_LRIG 収集**（`WDK04-006`）を新設し、アタック側の収集から `any_opp` を除外した。
  - **live 反映の二層**＝AUTO のみ32枚は `heldReview --adopt`、収穫マージが温存する4枚（`SP38-006`／`WX22-006`／`WXK01-028`／`WXK11-015`）は**親の差し替えと飲み込んだ効果の除去だけ**を外科パッチして手修正済みの残りは curated を保持。`WXK01-028` は legacy 採番（`-E2b`/`-E3`）が parser のブロック採番と食い違い srctext と対応が取れずカード全文 fallback に落ちていたので採番を揃えた。
  - **採用に伴って持ち込んだ既存ドリフト6枚は全件 fresh が正しい側**（held に滞留していた分）＝🔴`PR-K044-E2`／`WXK04-006-E2` は curated が**バニッシュを失っていた**過小実行／🔴`WXK03-018-E4` は `owner:'any'` フィルタなしで**相手や非黒シグニまで+5000できた**過剰実行／`WXK03-018-E3` は「＋されない」だけの原文に対し **−も止めていた**／`WXK08-005` の `LRIG_LEVEL_CMP_OPP{lt}` ゲートと `WXK11-015-E3` の続き207 修正が live に届いていなかった／`WDK04-006` は timing が `ON_ATTACK_SIGNI` 誤り。
  - **⚠計器の較正が必要だった**＝入れ子は定義上トップレベル `effectType` に現れないため、census の構造マーカー判定（【常】→CONTINUOUS無 等・【起】個数>ACTIVATED個数）が **16件を「表現済みなのに欠落」と誤検出**した。従来は《レイヤーアイコン》を原文から削る個別ハックだけだったので、**付与 `abilities` を再帰で数える**形へ較正した。**較正単体のベースライン上の影響は0**（HEAD の live JSON で 1394 据置）を worktree で実測確認済み。
  - **⚠今回の落とし穴（次も疑う）**＝①**変異テストを1回すり抜けた**＝「エクシード判定を外して境界を緩める」変異が最初のガードでは検出できず（対象カードは【出】で走りが止まるため）、**「入れ子側に非エクシードコストの【起】が無い」全数ガードを1本足して**塞いだ。②**golden の effectId 参照は入れ子化で壊れる**（5本）＝`findEffectDeep` を導入して追随。③**(li) の「全キー ACTIVATED が1つ以上の phase で surface される」退化ゼロガードは母数が分かれる**ので入れ子側も走査するよう拡張が要る。
  - **次の一手**＝**Opus：🆕`WDK04-006-E1-G` の中身**（`DECLARE_NUMBER`／`DECK_REVEAL_UNTIL` STUB＋`NEGATE_ATTACK` の対象が `SIGNI` のままでアタック中のルリグを指さない＝**timing と収集経路は正しくなったが能力の中身は未実装**）／**単独バッチ候補（過剰実行・優先）**〔🔴`WX24-P4-072-E1`＝`PLAY_FREE` から「コストの合計が捨てた枚数以下」が丸ごと落ちて**どんな高コストスペルでも無料で撃てる**／エクシードルリグ6件の本体不一致／`WX20-063-E2`「コスト合計0」・`WXK06-082-E1`「《幻竜 ドラマジ》以外」の filter 脱落／`WXK03-070-E1` の本体欠落〕**／(l) A群残8・(xxix) 残4は機構コスト待ちで据え置き／(lv) 残2経路（CPU シグニ召喚・CPU グロウ）／ゲート脱落 [B]20件／タスク16 残／§6.3 残機構**。**Sonnet：§7 実機検証**＝🆕**キー付与【起】のエクシード支払いUI**（本波でキー経路→付与経路へ移ったので `LrigGrantedModal` のエクシード選択・可否表示が実機で通るかの確認が最優先）＋分割UI＋宣言UI＋トラップ設置UI＋(xxix) の各支払いUI＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(l) 最新値（本行を直近の正とする）**：キーの「センタールリグは以下の能力を得る。」＋後続ブロックを `GRANT_LRIG_ABILITY.abilities` へ入れ子化（**36枚・47効果**）。census **1394→1393**（`BASELINE_HIGH=1393`）、golden **1075→1079**、smoke **10679/10679** 全0、fuzz 全0、lint 0 errors/**228 warnings**、同型★0（5986枚）、held **292→286枚/106署名**、manual field loss 0。live per-effect 差分 **changed 41 / added 1 / removed 48（影響カード36のみ）**、`build:effects` 2回目差分0。⚠**効果総数が 10722→10679 に減る**のは47効果がトップレベルから入れ子へ移ったため（smoke/census の母数もこの値）。⚠census には**構造マーカー判定を「付与 abilities も再帰で数える」へ較正**した分が含まれる（HEAD の live JSON では 1394 据置＝較正単体の影響0）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1079**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1393**（回帰ゲート）・held **286枚/106署名**（2026-07-30 タスク12(l) 実測）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（(lv) 残2経路／(l) A群残8。**(xxix) は 2026-07-30 に完全クローズ**＝残4件は明示保留、**(l) B群と (xlvi)／(xlii)／(lix)／(lviii) も残0**）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
