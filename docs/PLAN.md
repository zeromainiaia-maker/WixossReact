...
- **🆕 セッション（2026-08-15・続き499・Opus 5）＝§6.4 **O-4 クローズ**＝`UNKNOWN` **25ノード → 0**。ゲート全緑（**golden 2061**＝+4・**census 830→826**＝`BASELINE_HIGH` 実数更新・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・held **103枚/44群**）。live changed **25効果**。`census:stubs` A群 **15種/17件 → 20種/22件**（+5＝明示 defer にした分。**無言 no-op は 0 のまま**）。新機構4＝`REVEAL_BOTH_DECK_TOPS`／`DECLARE_DECK_TOP_ICON`／`SigniAttackBan.turnsRemaining`（O-33 の片方）／`DELAY_TO_NEXT_OWN_TURN_END`。
  - **🔑① `UNKNOWN` は「そのノードが no-op」では済まない**＝条件・対象・選択肢・遅延宣言といった**ゲートを飲み込む**ので、周囲のステップが無条件で走る。実際 25 件の大半が**過剰実行**だった（引用【起】の即時実行／3択が全部実行／必ずアタック無効／必ずダメージ無効／自分のシグニをバニッシュ／両プレイヤーがアタック不可）。
  - **🔴② 入れ子引用の抽出で3回踏んだ**＝非貪欲 regex は内側の `」` で止まる（`WX21-Re19-E2` の本文が切れて UNKNOWN）。深さ数えに替えたら今度は**並列引用**「「A」「B」「C」を得る」で depth が途中 0 に戻って付与ごと消え（5枚）、さらに**主語句の複数出現**でも消えた（`WXK01-038`）。⚠**どれもゲートは全部緑**で、気付けたのは `heldReview` の**held 枚数の差分**だけ。
  - **🔴③ golden が壊れた形を「据置」として固定していた**＝`WXDi-P11-005-E3` の「《無》×2を支払わないかぎりシグニでアタックできない」は `BLOCK_ACTION{ATTACK, owner:'any'}` に落ちて**両プレイヤーが無条件でアタック不可**だったのに、テスト名が「受け皿なし」で正しい状態として assert されていた（続き495 の `WX24-P2-044` と同じクラス）。
  - **⚠④ 新規則の適用層を間違えると grant 丸ごと平坦化する**＝引用【自】の中身向けの規則を `parseActionTextInner` の**前方**に置いたら、外側の「…このルリグは「…」を得る」ごと食って**ON_PLAY で即時実行**になった。引用抽出の**後ろ**へ移すのが正しい層。
  - **⚠⑤ 「あなたは以下の能力を得る」側は触らない**＝`GRANT_PLAYER_ABILITY` の新規則を「対戦相手は」限定にしないと、既存経路（多くは MANUAL の curated 形）を奪って先行するコスト節・使用条件が丸ごと落ちる（実測で10枚が held へ落ちた）。
  - **🔑⑥ defer は「何もしない」に倒すためのもの**＝機構が無い5件は**効果全体**を `DEFERRED_*` STUB へ置き換えた（ノードだけ差し替えると周囲がゲート抜きで走る）。UNKNOWN のままだと `census:stubs` に映らずリスクだけ残る。
  - **▶ 次の一手【Opus 側】**＝🆕`O-34`（今回作った明示 defer 5件＝`DEFERRED_STRIP_ATTACHED_AND_UNDER`／`_USE_SEARCHED_SPELL_OR_TRASH`／`_DECK_SIGNI_LEVEL_OVERRIDE_ALL`／`_PER_OWN_LRIG_COLOR_SCALE`／`_DECLARED_ICON_HAND_DISCARD_BANISH`）→ `O-32`（`REPEAT_N_TIMES` の二重実行3効果。⚠**本文の parse を先に直す**）→ `O-33` の**残り**（`SigniAttackBan.zones`＝ゾーン限定。`turnsRemaining` は今回入れた）→ `O-28` の残23綴り → `O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-10`（明示 defer 20 id/22件の前提再判定）。⚠`census:goldentypes` の**未カバー2型**（`RESERVE_DRAW_PHASE_REPLACEMENT`／`SET_LRIG_BASE_LIMIT`＝続き492 で新設・当時から未カバー）も残っている。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き499 の主な経路**＝(a)`WXDi-P09-036-E1`（両者のデッキトップを公開し、**LB の有無が一致したときだけ**アタック無効）(b)`WX15-002-E2`（相手が《トラップアイコン》の有無を宣言し、**外れたときだけ**ダメージ無効）(c)`WXDi-P11-005-E3`／`WXDi-P16-034-E2`（**次の相手ターンだけ**アタック税がかかり、払えば通る）(d)`WXDi-P03-001-E1`（3枚引き、**次の自分のターン終了時**に手札2枚捨て＝使ったターンには捨てない）(e)`WX24-P4-038-E1`（能力は**付与**され、ルリグをダウンして起動するまで何も起きない）(f)`WXDi-P11-002-E1`（**相手**の各ターン終了時に3択の不利益）(g)`WX17-003-E1`（①を選ばなければ手札は減らない）(h)`PR-Di007-E1`（相手が握手する/しないを選ぶ）。⚠**すべて実機未検証**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き499（§6.4 **O-4 クローズ**＝`UNKNOWN` 残0）後 最新値（本行が直近の正）**：census **826**（830→826。`BASELINE_HIGH` を実数更新済み＝**実装で置き換えた効果が STUB バケツから高シグナル側へ移る分**を差し引いた後の値）、**golden 2061**（+4＝両者公開の一致判定1・宣言の的中/外れ1・アタック禁止のターン数カウントダウン1・次の自分ターン終了時の2スロット予約1。ほかに `WXDi-P11-005-E3` の「据置」assert を正方向へ、`WXDi-P03-001-E1` の「UNKNOWN 温存」assert を予約へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **103枚 / 44群**、lint **0 errors / 260 warnings**、🆕**UNKNOWN 0ノード / 0カード**（25/25 から残0。`parseStatus:'UNKNOWN'` も 0）、`census:stubs` A群＝**20種/22件**（15種/17件から +5種/+5件＝**明示 defer にした分**。無言 no-op は 0 のまま）。🆕**live JSON changed 25効果**（CSV 非改変）。🆕**新機構＝`REVEAL_BOTH_DECK_TOPS`／`DECLARE_DECK_TOP_ICON`（＋`INTERNAL_DECLARE_DECK_TOP_ICON`）／`SigniAttackBan.turnsRemaining`（O-33 の片方）／`DELAY_TO_NEXT_OWN_TURN_END`＋`pending_next_own_turn_end_effects`／`pending_own_turn_end_effects`＋`RESOLVE_OWN_TURN_END_EFFECT`／`GrantPlayerAbilityAction.targetOwner`／`LOOK_AND_REORDER.count:'ALL'`／`extractQuotedGrant`（入れ子引用）／`MARK_PLACED_DELAYED_EXILE`**。⚠**全経路 実機未検証**（§7 送り）。⚠**残した明示 defer 5件**は 🆕`O-34` として §6.4 に登録した。⚠`census:goldentypes` は**未カバー2型**（`RESERVE_DRAW_PHASE_REPLACEMENT`／`SET_LRIG_BASE_LIMIT`＝続き492 で新設・当時から未カバー）。
...
| **O-10** | **明示 defer の棚卸し** | 15 id / 17件 | 🆕**2026-08-15 続き499 実測値＝A群は全体で 20 種 / 22 件**（この 15 id ＋ **O-34 の5件**＝O-4 で UNKNOWN から可視化した分）。旧＝続き498 は 15種/17件（O-3 の受け皿が残0になった時点）。旧＝続き493 は 23種/26件（この15 id ＋ O-3 の8）。旧＝15 id（続き492・A群 23種/25件）／15 id（続き491）／14 id（16件・2026-08-13 実測）。A群に `DEFERRED_*` として可視化済み。**理由が「先にやることがある」型は前提が消えたら着手できる**（例＝`DEFERRED_UNDER_CARD_AS_ENERGY_COST` は funnel 完成後に実装できた）。**定期的に前提を再判定する**。⚠**A群に出ない無言 no-op のほうが多いことがある**＝続き491 の系統は壊れていた6効果のうち**4件が STUB ですらなく**、続き492 の系統は8効果のうち**7件が STUB ですらなかった**（未消費の `BLOCK_ACTION` id／綴りズレ／ログだけのハンドラ／1回で消費される boolean／走査軸の漏れ）。**在庫表は原文 regex と併用する。** |
...
| **O-28** | **`GRANT_KEYWORD.keyword` にゴミが入っているクラス（全数走査済み）** | 23 綴り（残） | 2026-08-15 続き493 で live を機械走査（正式キーワード名でない `keyword` 値を全数列挙）。**`hasKeyword` は正式名でしか照合しないので、文が入っている綴りは一度も効かない無言 no-op**（続き490 で1件だけ見つけた形の母集団）。🏁**「《無》×N を支払わないかぎりアタックできない」5効果は 2026-08-15 続き494 で消化**（`WX11-012-E1`／`WX24-P1-041-E2`／`WX24-P4-001-E1`／`WXDi-P03-036-E1`／`WXDi-P05-033-E2`＝下の「消化済み」節）。**残り**＝丸ごと文が入っているもの4件＝`WX25-P2-071-E1`（「【常】：このシグニが対戦相手の効果によって場を離れる場合、代わりにこの能力を失う。そうした場合、このシグニをダウンする。」）／`WXEX2-71-E3`（正面以外へのアタック）／`WX24-P1-064-E1`（手札2枚以下で【アサシン】）／`WXDi-P05-068-E1`（アタックが無効にされない）。⚠**「正式名でない綴り＝すべてバグ」ではない**＝`チアガール`（32）・`トラップアイコン`（7）・`シュート`（7）等は engine が同じ綴りで消費する**正規の内部キーワード**なので、着手前に必ず消費地点を grep する。⚠`使用条件`（4効果・残）は【使用条件】ヘッダの取りこぼし＝続き493 で `WXDi-P16-002-E1` だけ是正済み。🔑**続き494 の教訓**＝①**`STATE_COND_BATCH4_ACTIONS` の action リテラルで固定されているカードは parser 規則を足しても変わらない**（リテラル側に正準形を書く）②**live の形を assert するだけでは足りない**＝`SELECT_TARGET_ONLY` がルリグ対象を扱えず**通しでは丸ごと no-op** だった。golden は「実行して効果が載るか」まで見る |
| **O-29** | 🆕**「同じ選択肢を複数回選ぶ」ループの表現** | 1効果（是正済・機構は未整備） | 2026-08-15 続き494 で `WX17-003-E1`（ベット―以下から2つ〔ベット時4つ〕まで選ぶ・同じ選択肢を複数回選んでよい）が**選択ループを2周**していた（`CHOOSE_SAME_OPTION_TWICE` と `_MULTIPLE` は engine では同じハンドラ＝4回＋4回）のを外科パッチ＋parser dedup で是正。**残っているのは表現の穴**＝先頭に `UNKNOWN{raw:"以下から２つまで選ぶ"}` が残り、回数はハンドラが**カード全文 regex**から読む（O-20 で潰した型の生き残り）。⚠fresh は `CONDITIONAL{IS_BETTING}` 形で live（PARTIAL・PRESERVE）と食い違っている＝**採るなら MANUAL 化の判断とセット**。母集団は `CHOOSE_SAME_OPTION_*` を含む2効果 |
...
| **O-32** | 🆕**`REPEAT_N_TIMES` がカード全文 regex で自己実行し後続ステップと二重に効く** | 3効果 | 2026-08-15 続き496 で発見。`STUB{REPEAT_N_TIMES}` は engine 側で**カード全文の regex**（`/以下を(N)回行う/`＋パワー/ミル/ドローの各分岐）を読んで**自分でN回ぶん実行**し、`SEQUENCE` の後続ステップ（＝parse された本文）も走るため**二重に効く**。実測＝`WXDi-P07-007-E3` は相手デッキが**16枚**落ちる（原文は3回×4枚＝最大12枚で、各回とも回避可能）。`WXDi-CP02-047-E1` は「パワー－5000」が**全シグニに×3**（原文は毎回1体）＋ミルは1回ぶんだけ。🔑**直し方＝`REPEAT{count, action}`（`execRepeat` は実装済みで continuation も合成する）へ組み替える**。⚠**前提＝本文が parse し切れていること**＝`WXDi-CP02-047-E1` は `POWER_MODIFY` が、`WXDi-CP01-024-E1` は本文が丸ごと live から落ちており、いま組み替えると**過少実行に化ける**。本文の parse を先に直すこと。|
| **O-33** | 🆕**アタック禁止の「ゾーン限定」と「次の相手ターン」の軸** | 1効果 | 2026-08-15 続き496 で据置。`WX25-CP1-050-E1`「次の対戦相手のターンの間、対戦相手は《無》を支払わないかぎり、**中央のシグニゾーンにある**シグニでアタックできない」は live が `BLOCK_ACTION{ATTACK, until:NEXT_TURN}` だけ＝**支払い回避もゾーン限定も落ちている**（払っても中央以外も止まる）。🔑**要るのは `SigniAttackBan` の2キーだけ**＝`zones?`（`banMatches` にゾーン index を渡す＝`matchedSigniAttackBans` の署名変更が全呼び出し元に及ぶ）と `turnsRemaining?`（`SigniDeployBan` と同じターン数カウントダウン＝減算は `clearTurnEndScopedState` の1点）。|
| **O-34** | 🆕**§6.4 O-4 で明示 defer にした5件**（機構不在） | 5効果 | 2026-08-15 続き499 で `UNKNOWN` から可視化（`census:stubs` A群）。(a)`DEFERRED_STRIP_ATTACHED_AND_UNDER`（`WX19-064-E1`＝**対象**シグニの付随物〔チャーム／アクセ／ソウル〕と下カードを剥がす機構。既存 `LRIG_UNDER_CARD_OP` は「このシグニ」＝自身専用のカード全文 regex ハンドラで流用不可）(b)`DEFERRED_USE_SEARCHED_SPELL_OR_TRASH`（`WX20-077-E2`＝**サーチしたスペルをその場で使う**経路。既存 `PLAY_SPELL_FROM_HAND_FREE` は手札からの別軸）(c)`DEFERRED_DECK_SIGNI_LEVEL_OVERRIDE_ALL`（`WXK07-034-E1`①＝`deck_signi_level_override` は state に**あるが読み手が1つも無い**＝デッキ探索/公開のフィルタが見ていない。⚠**setter だけある死フィールド**のクラス）(d)`DEFERRED_PER_OWN_LRIG_COLOR_SCALE`（`WX25-P3-050-E1`＝「あなたの場にいる〈色〉のルリグ1体につき〈効果〉」の倍率スケール。⚠同じ言い回しの**使用コスト減**が8効果あるので regex を広げない）(e)`DEFERRED_DECLARED_ICON_HAND_DISCARD_BANISH`（`WXDi-P12-055-E1`＝対象宣言→手札1枚選択→**相手がアイコンを宣言**→捨てたカードがそれを持たなければバニッシュ、の多段対話）。🔑**defer は効果全体に掛ける**＝ノードだけ差し替えると周囲がゲート抜きで走る（この5件はいずれも直す前に過剰実行だった）。|
