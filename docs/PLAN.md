...
- **🆕 セッション（2026-07-31・続き298・Opus 5〔Claude 検証〕＋codex-work〔実装〕）＝§6.3 の大型機構を2バッチ消化（I＝エクシード本体／J-3＝ライフクロス離脱トリガー）。両バッチとも Claude が差し戻し1回**（golden **1151→1158**・census **1366→1365**（`BASELINE_HIGH=1365`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**232 warnings**〔+2は既存 `useHost`/`useGuest` の rules-of-hooks 偽陽性が collector 1本追加で 27→28 になっただけ〕・manual field loss 0。ゲートは Claude が**各バッチ独立に3回反復**して全緑・フレーク無し／`census:timing` **31→27**）
  - **① §6.3 I＝`WX25-P3-028-E2`（エクシード本体）をクローズ**（commit `c82d0168`／`cc92bed5`）。**投入前実測で PLAN の記述より状況が悪いと判明**＝`STUB{REPEAT_N_TIMES}` は no-op ではなく**生 `EffectText` を再 regex して相手を18枚ミル**し、後続の SEQUENCE 3段目が自分を6枚＝**計24枚の過剰実行**だった（原文は「各回 self/opponent を選んで合計18枚」）。着地＝`PREVENT_REFRESH`＋汎用 `REPEAT{count,action}`（対話停止時に残回数を continuation へ）＋**新 pending を作らず既存 `CHOOSE`**。
  - **② ⚠差し戻し（I）＝寿命が原文より1ターン長い過剰実行**。初版は「自ターン終了で減算するカウンタ（初期値2）」だったが、減算4箇所（`BattleScreen.tsx:3411/3749/4345/9427`）は**いずれもターンを終えるプレイヤー自身の state にしか適用されない**ため T/T+1/**T+2** の3ターン有効になっていた。**決め手は既存規約の発掘**＝`*_until_opp_turn` family は `BattleScreen.tsx:3424-3429` で **`opState` 側**をクリア＝ちょうど2ターン、かつ**同文言の兄弟実装** `BLOCK_OPP_AUTO_ABILITY_EXTENDED`（`execStubPart3.ts:762`）も2ターン。⇒**このコードベースの「このターンと次のターンの間」は一貫して2ターン**。
  - **③ §6.3 J-3＝ライフクロス離脱トリガーを新設し4効果を実働化**（commit `050f081d`／`c28a7dc3`）。`detectLifeClothMoved`（宛先つき・**同名カード重複のため Set でなく multiset 差分**）＋`collectLifeClothMovedTriggers`（新 timing `ON_LIFE_CLOTH_MOVED`）。**既存 `collectLifeClothAddedTriggers` が完全な鏡像テンプレート**で、評価順序・走査範囲・`BLOCK_OWN_SIGNI_AUTO` までコピーできた。
  - **④ ⚠差し戻し（J-3）＝「クラッシュ＝life→trash」という誤った前提**。実戦の `crashOneLife`（`BattleScreen.tsx:6907`）はクラッシュしたライフを **`trash` でなく `field.check`** へ移し、`performLifeBurstResponse`（同 `9788`）で**既定はエナゾーン**へ送る（トラッシュは `CRASH_TO_TRASH_INSTEAD` のときだけ）。⇒ クラッシュ時の差分は `to:'other'`、後続 `check→energy/trash` は **life 差分ゼロ**。初版は `WXDi-P07-052-E1` を `lifeMovedTo:['trash']` だけで表現しており、原文「**クラッシュされるか**トラッシュに置かれたとき」の**クラッシュ側が丸ごと死んでいた**（原文が OR で並べるのは宛先が違うから）。既存 `ON_LIFE_CRASHED` 併記で解決＝`ON_LIFE_CRASHED` 11→12。
  - **🔴 教訓＝2バッチとも golden が「実装」を assert して原文を assert していなかった**。I は `lasts this and next own turn` と実装の寿命モデルを写経、J-3 は**手作りの `life→trash` state を「クラッシュ」と呼んで** PASS させていた。**確認手順＝①原文の寿命/イベントを既存の兄弟実装と照合 ②その経路が engine で本当にその差分を作るかコードで確かめる ③その形を通す golden を要求する**。どちらも差し戻すと codex は正しく直した。
  - **⑤ 台帳 stale を3件訂正**＝`_timing_census_triage.txt` の「ライフクロスにカードN枚が加えられたとき＝`[C]` 新規機構」は誤り（`ON_LIFE_CLOTH_ADDED` は parser/検出器/collector/配線まで実装済・実データ `WD06-001-E2`／`WD20-001-E2`）／`PR-K038-E2` の枚数は **0枚**が正（PLAN「N枚」・triage「1枚」が揺れていた）／**J-3 は `[C]` 2件だけでなく `[B維持]` 2件も同時に開いた**ため**タスク16 残 15→13・§6.3 J 残 13→11**。
  - **次の一手**＝**Opus：🆕 次セッションは タスク16（`[B維持]` 13件）を進める**。①**第2波＝`ON_TARGETED` の origin 種別軸**（`WXDi-D09-H16-E1`／`WXDi-P08-065-E2` の2件が同時に開く。collector は `triggerCollect.ts:334` に実在し**引数追加のみ**＝13件中もっとも投げやすい）②`ON_HAND_DISCARDED` の原因オーナー軸（`WXDi-D09-P16-E2`／`WX24-P2-051-E1`／`WXDi-P13-051-E3` の3件。collector のシグネチャ変更が要る）③`ON_ENERGY_CHARGE` の移動元/原因軸（`WX14-066-E1`／`WXDi-P03-073-E1`／`WXDi-P12-079-E2` の3件＝`BattleScreen.tsx:1630-1638` が増加枚数しか読まない）。⚠**13件とも live は `timing:[]` の安全停止＝過剰実行ではないので、退化させないことが最優先**（J-3 と同じく「その経路が engine で本当に発火するか」を投入前に実測して指示書へ書く）。**⚠(lxvii) CPU ターン終了時 `ON_TURN_END` 未収集（188効果/183カード）は §6.3 J-4 と同じ穴なので一体で見る**／その他＝§6.4 配置数制限すり抜け7効果・§6.3 J-2（4効果）・(lxi) 残5・(xxix) 残4・(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**（単一 worklist は §7「残る実機検証項目」）。⚠**今回の I／J-3 はいずれも実機UI未検証**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き298（§6.3 I＋J-3）後 最新値（本行を直近の正とする）**：census **1366→1365**（`BASELINE_HIGH=1365`＝J-3 でハイティの停止中だったクラッシュ枝を実働化した**機能改善**）、golden **1151→1158**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230→232 warnings**（⚠**+2 は退化ではない**＝`useHost`/`useGuest` の `react-hooks/rules-of-hooks` 偽陽性が collector 1本追加で 27→28 になっただけ。Claude が stash 比較で特定）、manual field loss 0。live per-effect 差分＝**I で changed 1 / J-3 で changed 4**（いずれも added 0 / removed 0・対象効果のみ）。**`npm run census:timing` 31→27 効果/27クラスタ**（⚠**実質25**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は live で既に正しい timing を持つ既知の偽陽性）。**タスク16 残＝`[B維持]` 15→13**／**§6.3 J 残＝`[C]` 13→11**。`ON_LIFE_CRASHED` **11→12**（追加は `WXDi-P07-052-E1` だけ）。**新語彙**＝action 型2（`REPEAT`／`PREVENT_REFRESH`）・timing 1（`ON_LIFE_CLOTH_MOVED`）・`triggerCondition` 3（`lifeMovedOwner`／`lifeMovedTo`／`lifeCountReached`）・`PlayerState` 1（`prevent_refresh_until_opp_turn`）＝**いずれも省略時は従来挙動**。
