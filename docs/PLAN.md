...
| **O-3** | **「次のターンの間」系統の残り** | 9効果（受け皿9種） | 🏁**続き491＝フェイズ／ターンのスキップ系統を解体**＝`NEXT_OPP_TURN_MAIN_PHASE_SKIP`（1）と`SKIP_NEXT_TURN`（1）を消化し、**同じ系統で A群に出ていなかった4件も同時に是正**（`WX05-018-E1`＝`BLOCK_ACTION{ENERGY_PHASE}` の消費地点ゼロ／`WX16-001-E3`＝`SIGNI_ATTACK_PHASE` と `SIGNI_ATTACK_STEP` の綴りズレ／`SP38-006-E4`＝アタック側脱落＋グロウ側の期間取り違え／`WXK06-078-E1`＝`STUB{SKIP_MAIN_PHASE}` がログを1行出すだけ）＝**6効果の挙動是正**。新機構＝`PHASE_SKIP_BLOCK_IDS`＋`resolveNextPhaseWithSkips`（フェイズスキップを**次のフェイズを決める1点**へ集約）／`resolveTurnHandover`（ターン交代判定を**1関数**へ集約＝`extra_turn` と新設 `skip_next_turn` を同じ戻り値で扱う）。**残り**は `UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／`DECLARED_SPELL_NAME_LOCK` 1／`GAIN_OPP_LRIG_TYPE` 1／`NON_FIELD_ZONE_MOVE_IMMUNITY` 1／`FIELD_SIGNI_TO_CHECK_ZONE` 1。**在庫は `npm run census:stubs` の A群で常時見える**／**機構ごとに独立して着手できる**。🔑**続き491 の教訓**＝①**「STUB の在庫」を数えても半分しか見えない**（この系統は壊れていた6件のうち**4件が STUB ですらなかった**＝未消費の `BLOCK_ACTION` id・綴りズレ・ログだけのハンドラ。`census:stubs` は前2つを構造的に見ず、3つ目は「実装済み」と誤判定する）②**フェイズ/ターンのような「進行そのもの」の制限は funnel を1本だけ作る**（フェイズ内の個別アクションを1つずつ封じると封じ漏れが無言ですり抜ける）③**「◯◯フェイズ開始時」フックは遷移元ではなく遷移先で判定する**（飛ばしたフェイズの開始時処理は走らず、実際に入るフェイズのぶんは走る、を1つの書き換えで満たせる）④**条件節を落としたまま機構だけ足すと過剰実行になる**（`WD20-006-E1` は「相手のターンで」「ベットしていなかった場合」の2条件とも脱落していた）。以下は旧記録＝🏁**続き490＝`ATTACK_TAX_HAND_DISCARD`（1）を解体＝`SigniAttackBan.unlessPayHandDiscard` を新設**（「手札をN枚捨てないかぎりアタックできない」＝**アタックするごとに**払う）。**同型の無言 no-op 1件（`WXDi-P05-022-E1`）も同時に是正**＝引用文が丸ごと `GRANT_KEYWORD.keyword` に入り`hasKeyword` の照合外で**一度も効いていなかった**（どの計器にも映らないクラス）。🔑**教訓**＝**「支払えば通る」制限は軸ごとに別関数を生やさない**（`signiAttackBanCost` が`{colorless, handDiscard}` をまとめて返す）。🏁**続き489＝`NEXT_OPP_ATTACK_PHASE_START`（2）を解体＝遅延予約 `DELAY_TO_NEXT_OPP_ATTACK_PHASE` を新設**（予約は**予約した側**に積み、`ON_ATTACK_PHASE_START` の collector が `opState` 側を読む）＝**5効果の挙動是正**。🔴**`negated_attacks` は型に「このターン」と書いてありながら失効地点が1つも無く永続していた**（`turnScopedState` へ登録して解消）。🔑**教訓**＝**受け皿 id は母集団の一部しか映さない**＝**着手時は原文 regex で母集団を数え直す**。🏁**続き488＝`EXTRA_ATTACK_PHASE`（2）／`SELF_RESTRICT_THIS_TURN`（2）／`UNPARSED_NEXT_OPP_TURN_CLAUSE`（1）を解体**＝新機構 `ADD_EXTRA_ATTACK_PHASE`／`SIGNI_DEPLOY_BAN` のキー無し ban／`blocked_actions:'PAY_ENERGY_COST'`／`TRASH_ENERGY_AT_TURN_END`／`POWER_MODIFY.deltaFromZone`＝**8効果の挙動是正**。🔑**教訓**＝**「遅延タイミング宣言」の後続文は絶対に即時実行しない**（機構が入るまでは SEQUENCE 畳み込みで後続を落とす＝過少側に倒す）。🔑**`ADD_TO_FIELD` は `source` が無いと engine で「デッキの一番上」を場に出す**。🏁続き486＝`_THIS_TURN_OPP_CLAUSE`（7効果）を解体＝新機構 `SIGNI_ATTACK_BAN`／既存 `PREVENT_DAMAGE`／盤面リセットで**5効果の恒久 no-op を解消**。🏁**続き487＝`_NEXT_OPP_TURN_CLAUSE`（5）と `_THIS_AND_NEXT_TURN_CLAUSE`（4）を解体**＝新機構 `SIGNI_DEPLOY_BAN`（配置禁止を**場に出す側**に載せる）ほかで**7効果の挙動是正**。⚠**期間だけ直すと「誤った対象の効果」が長持ちする**＝対象軸が壊れている札は据置（`WX11-038-E2`）。🔑**続き486/487 の教訓**＝①**受け皿 id の件数だけ見て設計しない**②**STUB を実装で置き換えると census が +N する**（毎回「計器の較正漏れ」か「本物の穴」かを仕分ける） |
