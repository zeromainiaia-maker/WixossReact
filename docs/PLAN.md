...
- 🔑**判断の分かれ目**＝機械学習（`S-4`）をやるなら C、やらないなら A で先読みの効果を先に見る。**ユーザー判断待ち**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7.3 の A／B／C のどれで進めるか（ユーザー判断）** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 0＋判断待ち1**／**リリース作業 1**（RELEASE.md） |
...
| 🔥**④b** | 🆕**§5.7 CPU の強さ `S-nn`** | **判断待ち** | 探索＋評価のゲーム AI（対戦中に LLM は呼ばない） | §5.7.1 の表 |
...
| 🏁`S-3` | ~~**1ターン分のシミュレータの見積もり**~~ | 調査 | **2026-09-17 クローズ**＝実測と3案は §5.7.3。**どの案で進めるかはユーザー判断待ち** |
...
- ⚠§5.6.6 の「完全ヘッドレス化は数字で必要と言えてから」＝**機械学習（`S-4`）をやるなら C が前提**、やらないなら A で先読みの効果を先に見る、が判断の分かれ目。
