...
- **🆕 セッション（2026-07-11・続き64・Sonnet 5・§4タスク(C) §7実機検証の継続＝ON_CHARM_TO_TRASH（R42）追加）**＝`verifyBattleDrive.mjs` に新シナリオ `charmToTrash`（WX19-023→WX16-Re05）を追加し実機検証・**✅PASS**（続き61のcollectBoardDiffTriggers統合がR42のcollectCharmToTrashTriggersも一緒にカバーしていたことを確認＝追加修正不要）。guest zone0に charm付きWD05-009（P12000）＋zone1にバニッシュ対象外P15000（WX01-053・banish候補を1件に固定してpick順依存を排除）を配置、WX19-023【出】無条件バニッシュ（resume経路）でチャームがtrashへ→watcherが対戦相手シグニに-4000を確認。**副次的な学び**＝「バニッシュ＝トラッシュ行き」は誤解＝`banishDestination`のデフォルトはバニッシュされたシグニを持ち主の**エナゾーン**へ送る（Wixossのルール通り）＝トラッシュに乗るのは外れたチャーム/アクセのみ。guest側盤面注入が稀に競合で反映されない問題も発見し`queryState`確認→再PATCHのリトライで安定化。`order`配列に追加済み・単体3回連続PASS。engine/parser変更なし（テストハーネスのみ）。⚠フルスイート回帰は環境側のプロセス過多（過去の検証run由来の残留chrome/node多数）で著しく低速化したため中断（自身の変更に起因する疑いなし・単体検証で十分な確証あり）。詳細 BUGFIXES 最上部。
  ...
  - **次の一手＝(A)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(B)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(C)Sonnet＝§7実機検証の継続**（R30・R44・R45①残・WXDi-CP02-082（R36②）・R42②（バトルバニッシュでhostが離脱したとき）・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up）。**(D)持ち越し：WX12-008 の exceed-cost timing 誤パース是正・`handleSelectZoneForEffect`/`handleSelectSigniZoneForEffect` の収集器未配線（続き61 follow-up）・SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
