...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第18バッチ。未分類の ★★セルから取る（下の①）。**⚠**セルを取ったらまず全数分類**＝実測で偽陽性（条件節・コスト節のクロス計上）が半分を超えることがあり、偽陽性の型が分かったら**計器側を較正**するのが最短（実装ゼロで miss が減る）。
> 
