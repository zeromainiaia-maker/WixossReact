...
- **🆕 セッション（2026-08-10・続き418・Opus 5）＝§6.4「代わりにそれを〜」＝同一対象への動詞昇格を機構化。census 860→858・golden 1753→1759・UNKNOWN 38→36ノード。11効果/10カード是正（うち🔴自傷バグ3件）**。
  - **🔴① PLAN が「UNKNOWN 2件」と書いていた家族は、実際には10カードの過剰効果クラスタだった**＝原文「＜対象＞を対象とし、それをAする。＜条件＞の場合、**代わりにそれをBする**」の per-target 形を parser の「代わりに」畳み込みが**意図的に除外**していた（`const perTarget = /それ/.test(…) && !/対象とし/.test(…); if (!perTarget)`）。除外された昇格文は**別ステップとして残り**、主語のない「それをバニッシュする」が `owner:'self'` へフォールバック＝**自分のシグニをバニッシュ/付与**していた（`WX08-028-BURST`／`WX09-Re01-E2`／`WXDi-CP01-026-E1`）。**「UNKNOWN が2件」は氷山の一角で、同じ文型の残りは黙って自傷していた**。
  - **✅② (A) 盤面状態条件は `CONDITIONAL{then:昇格, else:base}` へ**（条件は対象決定より前に評価できる＝どちらの枝も対象選択1回で原文と同値）。先行詞は **`prevRaw` の対象節を前置して再パース**し、得られた action の対象を **base の対象で上書き**する。⚠`prevRaw` は `【自】` のトリガー節を残すので**先頭固定では取れず**、非貪欲な前方一致だと直前の条件節まで飲む＝**末尾の「を対象とし、」から所有者語まで戻して切り出す**。⚠**期間も base から引き継ぐ**（`WXDi-CP01-026-E1` は単独再パースだと `PERMANENT` へ落ち、【シャドウ】が黙って恒久付与に化けた）。
  - **🔴③ (B) 対象プロパティ条件（「それが感染状態の場合」等）を (A) の形で書くと恒久 no-op になる**＝`LAST_PROCESSED_MATCHES` が**対象選択より前**に評価され `lastProcessedCards` が空＝**常に偽**。正しくは既存の正準形 `SELECT_TARGET_ONLY → STORE_LAST_PROCESSED_TARGETS → CONDITIONAL{…targetsStored}`（`bindToStoredTarget` 再利用）。**実装途中で実際にこの穴に落ち、golden の「構造」ではなく「実行結果」テストで捕まえた**＝この家族は構造アサートだけでは守れない。
  - **🔴④ 同じ理由で POWER_MODIFY の値昇格は else 置換ではなく加算モデル**＝base を撃って `lastProcessedCards` を書かせ、**差分だけ**を `targetsLastProcessed` で同一対象へ加算（`WX15-027-E1`＝−7000 ＋条件付き −5000）。既存 `tpm`（「それに【チャーム】が付いている場合」）と同型。
  - **✅⑤ 併せて条件語彙4本を追加＝いずれもゲートが丸ごと落ちて無条件発火していた**＝`THIS_CARD_HAS_UNDER.minCount`（型＋`execUtils`＋decompiler の3点セット・`WXK08-030-E1` の下2枚/5枚）／「トラッシュにスペルがN種類以上」（`WXEX1-38-E2`）／「場のシグニが持つ色が合計N種類以上」（`WXDi-P13-036-E1`・副産物で `WXK11-057-E1` も是正）／`LIFE_COMPARE_OPP{lte}`（`WXDi-CP01-026-E1`）。⚠最後の1本は `isBatch1OnlyClause` が「対戦相手のライフクロス」を含む source を**カードゲート**していて、規則を足しても引かれなかった＝**閾値形ではない比較形をゲート外に出した**。
  - **⚠⑥ 多段閾値の引き継ぎ（`bm`）に「N以下」ガードを入れた**＝`minCount` 型は「N以上」しか表せないので、数値だけ差し替えると条件が**黙って反転する**。該当0件だが構造的に塞いだ。
  - **⚠⑦ 採用経路の落とし穴2つ**＝(a)`WXK08-030-E1` は `parseStatus:'PARTIAL'` で守られており parser を直しても live に届かない＝**`PARTIAL`→`AUTO` へ戻してから build→adopt**（§6.4 既知手順）。(b)`WX09-Re01` は MANUAL 同居の混在カードなので held ではなく **`docs/_partial_fresh.json`** に落ちる＝**こちらには adopt スクリプトが無い**ので当該 AUTO 効果だけ live へ直接移送した。
  - **⚠⑧ 判明した既存の限界（回避・記録のみ）**＝`LAST_PROCESSED_MATCHES` のゾーン状態照合（`findFieldZoneState`）は **CardNum で場を引く**ため、**両プレイヤーの場に同じカード番号があると取り違える**（自分側の未凍結コピーを先に拾って条件が偽になる）。instanceId 化までの既知の穴。
  - **✅⑨ ゲート**＝`npm run gates` 全緑（golden **1759（+6）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **858**（`BASELINE_HIGH` 更新済み。**+2 は加算モデルが原文の合計値を literal で持たないための良性検出**＝先行の `WX25-P2-102/107/109` と同じクラス。実質 855 相当）/ 同型★ **0**（265群）/ lint 0 errors）。`build:effects`＋`regen` 実施済み。**実機UI検証は不要**（新モーダルなし・既存の対象選択経路のみ）。
  - **📋⑩ 次の一手**＝**Opus 側**＝(a)**§6.4 A群の着手可能2件**（`ASSIST_LRIG_ATTACK_THIS_TURN`＝WX25-P1-048／`UNDER_CARD_AS_ENERGY_COST`＝WXDi-P10-041）(b)**`WX12-010`＝強制アタック機構**（engine に受け皿なし）(c)**§6.4 UNKNOWN 残36ノード**（次に安いのは「デッキ公開→処理の変形」2件＝`WX21-028-E1`／`SP38-006-E1-G2`）(d)**置換5本の対話化**（`WXDi-CP01-023` もこれ待ち）。**🆕新規登録**＝`WX25-P3-014-E1` は今回の同族だが **🔴`BOUNCE{owner:'self'}` の自傷が残る**（base が任意コスト STUB を含む多段構成で SELECT/STORE 組み替えが did-it ゲートと干渉＝§6.4 に登録）。**Sonnet 側**＝§7 実機検証（**続き416〜417 の pay/skip モーダルを最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418（§6.4「代わりにそれを〜」動詞昇格の機構化）後 最新値（本行が直近の正）**：census **858**（860→858・`BASELINE_HIGH` 更新済み。**+2 は加算モデルの良性検出**＝実質 855 相当）、golden **1759（+6）**、`MANDATORY_SUSPICIOUS` **2**（据置・`node scripts/_checkAllEffects.mjs`）、held **114カード／49群**、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**（38→36）。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
