...
- 🏁**セッション（2026-08-25・続き658・Opus 5）＝§5.3 `O-46` を消化＝複合コスト（連用形）の2要素目以降が丸ごと落ちて踏み倒せていた穴を塞いだ**。gates 全緑（**golden 2781→2783**／census 573/573 据え置き／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 263／同型★0）。**実機は新規6本＋回帰6本＝計12本が2回連続 ALL PASS＋反転確認2通り。**
  - ✅**母集団は原文から2通りの測り方で実測して一致**（①コスト句の連用形＋「、」＝15効果 ②「、」区切りで支払い動詞2つ以上＝16効果）。**live が実際に変わったのは5効果**（A/B 差分で「変化5・追加0・削除0」を機械確認）。
  - 🔴**登録票の見立て（「最後に当たった1つで上書き」）は外れていた**＝`parseCost` は要素ごとに別キーへ書くので上書きは起きない。**真因は活用形の不一致**＝原文は複数要素を並べるとき手前を**連用形**（「捨て、」「置き、」）で書くが、規則は**終止形しか見ていなかった**。⇒ コスト句の**連用形正規化1本**で4効果が同時に直った。⚠**正規化する語は全効果のコスト句から実測して3綴りだけに絞った**（推測で増やさない）。
  - 🔴**支払い側にも穴が2つあった（`O-67` に続き2回連続）**＝①`SigniActivatedModal` が `handDiscardSigni` を知らず**必要枚数0で1枚も選ばせずに発動できた** ②**キー【起】の実行経路にエナ全損の支払いが1行も無かった**。**提示・可否・支払いの3地点を必ず数える。**
  - 🔑**部分採用は踏み倒しになる**＝受け皿の無い要素が1つでもあるなら、解釈できた分も**捨てて `costUnparsed` に倒す**（`WXDi-P03-019-E1` は採ると**自分の場を1体も失わずに相手の場を全滅**できた）。
  - 🔑**同じ穴を過去に3回スポット修正していた**（`trash_self` の `置[くき]`／`trash_key`／`discardSelfFromHand` の `捨て[る、]`）＝**3回目のスポット修正は一般化の合図**。
  - ✅**ついでに逆翻訳の嘘を2つ潰した**＝①`costJa` の未対応コストキー **22種**（生JSON漏れ **42効果以上**）を全数解消 ②`costUnparsed`（＝提示自体が止まる印）が**逆翻訳に1文字も出ていなかった**ので明示した。

**▶ 次の一手**＝**§5.3 の `O-68`／`O-69`（本セッションで登録＝いずれも `O-46` の残り）か、`O-66`②③**。⚠**2026-08-25 続き656 の実測＝`census:wiring` の第2層は束ならない群が多い**（`cardClass × SIGNI[filter]` は掘り尽くし済み＝再度掘らない）。⇒ **具体的な語彙・文型に降りて生データ（CSV原文 × live JSON）で母集団を測る**ほうが速い。🆕**続き658 の実測でその裏が取れた**＝`powerRange × GRANT_KEYWORD{SIGNI}`（miss=6・★）は**6件中4件がリマインダー文／条件節/クロス計上**で、真の穴は `WXK10-027-E1`（【ランサー】を持つパワー15000以上の緑）ほか**1〜2件だけ**だった。
- 🔑**コスト語彙を1つでも足したら「提示ラベル・可否ゲート・支払い実行」の3地点を grep で数える**（`O-67`／`O-46` と2回連続で支払い経路が空だった）。
- 🔑**「規則が無い」ではなく「活用形が違う」だけで語彙は丸ごと落ちる**（`O-46`）＝スポット修正が3回目になったら一般化する。
...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-66** | 🆕**②「シグニは」＝両プレイヤーのシグニを表す保護が書けない ③engine がカード全文 regex から意味を決める STUB（`O-60` 系）**（🏁**①ライフクロスのクラッシュ防止／回数制限は 2026-08-25 続き655 で消化＝`src/engine/lifeCrashGate.ts`**） | **S〜M** | **2026-08-25 続き654（`O-65` の消化）で分離・続き655 で①を消化。**■🏁**①消化済み**＝実測6効果を3軸（全面防止／「ダメージ以外」限定／1ターン上限）で実装。判定は `lifeCrashGate.ts` の1本、消費は**3地点**（`crashOneLife`／ルリグアタックのインライン／`execLifeCrash`）。**実機2本＋反転確認済み。**全文は BUGFIXES.md。■**②`WXEX2-44-E1`「【常】：対戦相手のメインフェイズの間、シグニはバニッシュされない。」の「シグニは」は両プレイヤーのシグニ**だが、`GRANT_PROTECTION` の受け皿は `target.owner:'self'` 前提で、`collectBanishEffectProtectedSigni` も**保護宣言元と同じ side の state しか走査しない**＝現状 `count:1`（自身のみ）に潰れている。⚠**実用上の影響は小さい**（相手のメインフェイズに自分がバニッシュすることは稀）ので優先度は低いが、**逆翻訳が「このシグニは」と描く**ので原文照合では毎回引っかかる。🔑**続き655 の知見が使える**＝`lifeCrashGate.collectLifeCrashPreventions` は**両プレイヤーの盤面を走査する**形（`protects:'each_player'`）を実装済みで、「両者に効く【常】」の書き方はこれを踏襲すればよい。■**③`GRANT_UNDER_SIGNI_AUTO_ABILITY_ATTACK_PHASE`／`PREVENT_SIGNI_MOVE_BY_OPP_EXCEPT_BANISH` は engine 側が `cardMap` の `EffectText` を regex で読んでレベル／色／クラスを決めている**（`effectEngine.ts:5752`／`:6533`）＝**JSON を見ても何が起きるか分からない**（`O-60`／`O-56` と同型）。payload 化すれば `census:stubs` の軸が1本増える。■🆕**④続き655 で分離＝`WD06-008-E1` の「このターン終了時、あなたのライフクロスの一番上のカードをトラッシュに置く」の遅延が落ちて即時実行になっている**（`turn_end_*` 系にライフクロス用の受け皿が無い）。⚠**今は同 SEQUENCE の全面防止宣言に阻まれて no-op**＝実害（自ライフ1枚喪失）は消えたが正しくもない。■**着手前に必ず母集団を実測し直す**（§4.1）。 |
