...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377g＝stale live の一括解消後）**＝golden **1450**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1000**（回帰ゲート）・manual field loss 0・lint 0 errors/**250 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **189枚／89群**・実機 driver 既定order **128件**。**被覆マトリクス miss 320**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(iv) stale live の残りを刈る（続き377g で新設・最も効率が良い）→ その後 (i) 配線ギャップ第13バッチ。**
>    - **① ⭐`build:effects` の非破壊マージは「parser は直っているのに live は古いまま」の在庫を作る**＝収穫マージは**証明可能に無損失な上位集合だけ**を自動採用するので、parser を後から直しても live の古い値は残る。続き377g はこれだけで **census を −30**（parser/engine は無変更）動かした。**手順**＝①`npm run build:effects` ②census 高シグナル ID を `docs/_vocab_census.txt` から集合化 ③live と `docs/_held_fresh.json` を全件突き合わせ ④原文照合して採用。
>    - **⚠採用の作法（続き377g で確立）**＝(a) **効果単位で採る**（`heldReview --adopt` はカード単位なので、同カードの別効果が live=MANUAL のとき fresh が MANUAL 側を退化させる。**`parseStatus==='MANUAL'` は無条件スキップ**）。(b) 🔴**採用値は必ず `build:effects` の出力から取る**＝`parseCardEffects` 直呼びの自作ダンプは **`_sourceTextLog` を参照する post-pass**（`applyLrigColorBatch5` 等）が効かず、`colorNotMatchesLrig` のような**後付けフィールドが黙って落ちる**。自作ダンプは*候補の発見*にだけ使う。(c) **「held のほうが差分が小さい」は採用理由にならない**（`WX21-032-E1` は live も held も誤り）。(d) 採用後は**ベースラインコミットと再帰的にキー比較**して消失キーを1件ずつ説明できるようにする。
>    - **残り在庫の目安**＝held **189枚／89群**のうち census 高シグナルに当たるのは **約40効果**（続き377g で8カードを消化した残り）。**`WX15-093` 型（付与対象の owner 取り違え）や duration 取り違えが濃い。**
>    - **② (i) 配線ギャップの第13バッチ**＝⚠**`cardClass × (filter無)` 28 と `cardClass × SIGNI[filter]` 18 は全数分類済みで「1 regex で N 効果」型ではないと確定**（前者は5系統の寄せ集め／後者は18件中15件がコスト節・条件節・トリガー主語のクロス計上）。**未分類の ★★セルから取る**＝`cardClass × TRASH{HAND_CARD}` 8（68）／`cardClass × TRASH_CARD[filter]` 7（284）／`color × SIGNI[filter]` 10（112）／`powerRange × SIGNI[filter]` 11（499）。
...
>    - ⚠**着手前に必ず読む罠**＝(a) **素の `parseStoryFilter`/`parseLevelFilter`(文全体) を対象フィルタに使わない**＝**`signiClause*Filter` 3兄弟**（クラス／アイコン／レベル）を使う。(b) **曖昧なら付けない**。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で取り出す。(d) **exec 側の配線を必ず確認する**（`matchesFilter` は `excludeSelf`/`upToCount` を見ない）。(e) **セルは入口であって終点ではない**＝ビルダー全体を読む。(f) **枚数・値だけ先に直さない**。(g) **★★セルの miss 数は見込み件数ではない**（実測で 6/6・22/22・18/18 のうち大半が偽陽性の回がある）。(h) **同じ概念に2つのキー綴りが併存していないか先に確かめる**（`hasCrossIcon` vs `hasIcon`／`story` vs `banishedSourceStory`）。(i) **同じ語彙でも入口ごとに壊れ方が違う**。(j) **A/B の件数＝直った件数ではない**。(k) **採用前の確認は JSON パースによる構造比較で**。(l) **セルは母集団の索引であって母集団そのものではない**。(m) **安全弁はコメントの規律ではなく関数の戻り値にする**。(n) 🆕**census が減らないときは、まず live と fresh を全件突き合わせる**（parser ではなく live が古い可能性）。
>    - **✅消化済み**＝「他の」ゲート棚卸し（377）／`noGuard`（377b）／アイコン系（377c）／`levelRange`（377d）／全体バフの語順＋据置held（377e）／ON_ATTACK_SIGNI の味方側トリガー主語＋`excludeSelf` の過剰発火（377f）／**stale live の一括解消 38効果（377g）**。
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **5件**（🆕`WXDi-P00-002-E1` を追加＝【アサシン】等を失い新たに得られない）／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**（`cardClass × (filter無)` の10件がここに合流＝「デッキの上からN枚公開→**その中から＜X＞のシグニ**」でステップが丸ごと `UNKNOWN`）／「場に《ライズアイコン》を持つシグニがN体ある場合／あるかぎり」の条件型 **5件**／左右のシグニゾーン限定の filter キーが無い 3効果／**【ソウル】が付いているシグニの filter キーが無い 1効果**（`WXDi-P04-016-E1`）。
>    - **(iii) 構造混線の在庫**＝`WXK05-043-E1`／`WX24-P3-059-E1` ほか §5d-0 (iii) の登録分／続き377f で分類した8件（`WDK10-001-E3`・`WXEX2-48-E1`＝トラッシュから選んだシグニをライフに加えるはずが `ADD_TO_LIFE{fromTop}`／`WDK07-E07-E1`・`WXK10-074-E1`＝【アクセ】付与が丸ごと脱落／`WX05-080-E1`／`WX25-P2-079-E1`／`WX16-003-E1`／`WD14-011-BURST`〔🆕**377g で解消**〕）／🆕**コスト節の限定脱落**（`WX05-044-E1`＝「他の＜古代兵器＞のシグニ1体をバニッシュする」コストが丸ごと落ちて**無料で撃てる**／`WX14-016-E1`＝アンコールコストが別物／`WX08-036-E1`＝対象側のパワー条件をコスト側に載せている／`PR-322-E1`）。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない。
>    - ⚠**見積もりの現在地**＝census **1000**／被覆マトリクス miss **320**。7バッチの実績＝12／30／17／8／15／11／**38**効果。**census が動かない回でも効果は直る**（377c は census 据置で17効果）＝**census の増減だけで成果を測らない。**
