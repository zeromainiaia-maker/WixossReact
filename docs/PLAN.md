...
- 🏁**セッション（2026-08-26・続き662・Opus 5）＝§5.3 `O-61` を消化＝「〜を対象とし」が「このシグニ」と見分けられず、対象ピッカーを出さずに効果元へ自動適用していた**
  ...
  - 🔑**刻むのは少数側**＝実測で「このシグニ」側は **263ノード**（裸の `【常】：【マルチエナ】` から `【絆常】：【シャドウ:{…}】` まで表記が多様）、「対象とし」側は少数かつリテラルが安定。**刻み漏れの向き**も重要で、前者を漏らすと**自己付与が選択UIに化ける退化**、後者を漏らすと**現状維持**（安全側）。
  - ✅**live A/B＝刻印 186ターゲット / 167カード・`explicitTarget` 以外の差分ゼロ**。うち**実際に挙動が変わるのは 18効果**（効果元がシグニのもの）。残りは効果元がルリグ／アーツ／スペルで元から選択UIが出ていた。
  - 🔴**後段パスの直前で deep clone してはいけない**＝初版は `JSON.parse(JSON.stringify(action))` で複製してから刻んでいたが、この parser は**同じノードオブジェクトを共有**したまま後段が破壊的に書く形が在る（`WX16-042-E1` の `BANISH` は `SEQUENCE` と `REPEAT` 本体で同一参照）。参照が切れて `levelEqLastProcessed` が片方にしか届かなくなり、**`_held_fresh` 76→77 で発覚**（live は不変なので A/B だけでは気づけない）。
  - 🔴**`build:effects` は `PRESERVE_STATUSES` のカードを丸ごと温存する**＝一度書き込んだ誤刻印はガードを直して再生成しても消えない。**A/B は live を `git checkout` でベースラインへ戻してから1回だけ再生成する。**

**▶ 次の一手**＝**§5.3 の `O-75`（本セッションで登録）か、`O-73`／`O-74`／`O-71`／`O-72`／`O-70`／`O-68`／`O-69`**。
- 🔑**「同じ JSON になってしまう2つの原文」を疑う**＝計器（census / census:wiring / 逆翻訳）はどれも**JSON を見る**ので、**原文の区別が JSON に無い**穴はどの計器にも映らない。`O-53`・`O-55` と同じ族。
- 🔑**既定を1つの型だけが持っていたら、それは規約から外れている合図**＝`BANISH` 等が `thisCardOnly` 規約で統一されている中、3型だけが緩い既定を持っていた。
- 🔴**parser の後段パスは参照共有前提**＝clone すると静かに壊れ、**live ではなく held に出る**。
...
- **2026-08-26 続き662 後（本行が直近の正）**：
  **census 572/572**（`O-61` では動かず＝対象ピッカーの有無は census の網に載らない）、**golden 2807**（`O-61` +3）、
  ...
  **live カード 5975 / 効果総数 10693 / MANUAL 効果 1032**（据置。`O-61` は `explicitTarget` の刻印だけ＝**167カード・186ターゲット**）、
  **`_held_fresh` 76 / `_partial_fresh` 12 / `_idset_fresh` 45**（いずれも据置＝parser 改善が凍っていない）、
  **実機シナリオ ±0**（`b41GrantKeywordTargetsOwnSigniOnly` を `O-61` の観測点へ**組み替え**。回帰＝`b41GrantKeywordExpiresAtTurnEnd`／`b66UnderGrantPayload` PASS）、
