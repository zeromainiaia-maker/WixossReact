...
**直近＝2026-09-20＝§5.6 `C-0`＝バグ報告1件を消化し、報告の調べ方とデッキの通り道を道具にした**（全文は [BUGFIXES.md](./BUGFIXES.md) 第417〜420バッチ）
- **バグ2件を修正**（`c32a37ce`）＝①「場に出す」だけの【起】が**空きシグニゾーンが無くても撃てた**（`emptyZoneGate.ts`・live 83効果/81カード）
  ②【出】の**《黒×0》がトラッシュ【起】に一切効いていなかった**＋CPU だけエナ満額＋ターン終了で失効しない（`activateCostZero.ts`・7地点＋`turnScopedState`）。
- 🔥**報告に答える前に `node scripts/replayReport.mjs <file> --replay`**（新設）＝報告の盤面で効果を撃ち直し、**原文 × 提示 × 請求 × 盤面差分**を並べる。
  🔴**②はコードを読んでいた私では見つけられず、ユーザーの指摘で発覚した**＝**推論の前に engine へ通す**（[LESSONS.md](./LESSONS.md) §4.1 先頭）。
- 🏁**ユーザー作の CPU デッキが実機で使えるようになった**＝`listDecks.mjs` / `importDecks.mjs`（新設）。
  実測＝`DECK="ケトッシー軸" node scripts/verifyFullMatch.mjs cpu` **PASS**（6ターン/125手）・機構踏破 **7/23**。
- 🔴**アカウントの読み手を `scripts/verifyAccounts.mjs` へ集約**＝`harness:false` を書き込み系から構造的に外した（**ユーザー本人のアカウントへ書く事故を塞いだ**）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-23`**（本物のデッキで測る）＝取り込みが通ったので**まずこれ**＝`S-20`（A/B の山が型を踏まない）の実体でもある。次に **`S-17`**（終端をアタック後へ）。`S-19`（先攻が勝てない）・`S-22`（CPU の対象選び）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 7**（`S-23`・`S-17`・`S-19`・`S-20`・`S-22`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先攻が勝てない（`S-19`）②A/B の山が型を踏まない（`S-20`）③**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`） |
| ⚠**バグ報告** | 🏁**未消化 0**（`c32a37ce` を消化）。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4345**＝この一連で +3）｜**実機 PASS**＝`verifyFullMatch.mjs cpu`（`VERIFY_DECK` と `ケトッシー軸` の2本）｜反転確認＝バグ2件とも `git stash` で A/B 済み |
...
| 🔥`S-23` | **本物のデッキで CPU を測る**＝2026-09-20 に `importDecks.mjs` が通り、**ユーザー作の26デッキが `claude1` で使える**ようになった（実測＝`ケトッシー軸` で通し対戦 PASS・機構踏破 7/23）。いまの A/B（`selfplay:ab`）と実機スモークは**合成デッキ**（`VERIFY_DECK`＝シグニ40枚＋ルリグ5枚／`VERIFY_DECK_MECH`）しか使っていない。①複数デッキの総当たりで勝率を取る ②デッキごとの機構踏破を合算する ③`S-19`（先攻不利）・`S-20`（山が型を踏まない）が**本物の山でも再現するか**を見る。 | 中 | **`S-20` の実体**＝「測りたい型を踏む山」がもう在る。⚠**1戦の0回は乱数**＝数戦を合算してから判断する（`census:play` の警告） |
...
- **2026-09-20 時点**（第417〜420バッチ＝§5.6 `C-0` バグ報告 `c32a37ce` ＋ 報告の調べ方・デッキの通り道）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 7**（`S-23`・`S-17`・`S-19`・`S-20`・`S-22`・`S-14`・`S-6`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4345**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 2本**（`VERIFY_DECK` 6ターン/130手／`ケトッシー軸` 6ターン/125手）
  - 🆕📏**「場に出す」だけの【起】**＝`ACTIVATED` かつトップレベルが `ADD_TO_FIELD` の効果 **83効果 / 81カード**（`blockedByNoEmptySigniZone` の適用範囲）
  - 🆕📏**`ACTIVATE_COST_ZERO_BLACK`**＝発生源 **1効果 / 1カード**（`WD08-001-E1`）／**受け手＝トラッシュ【起】 38効果 / 37カード**
  - 🆕📏**ユーザー作のデッキ**＝`カルカドール` に **27件**（`cpu` 21 / `player` 6・うちメイン40枚が26件・`cpu_plan` つき5件）。`claude1` へ **52行**（26×2種別）取り込み済み
  - 🆕📊**機構踏破（`ケトッシー軸`・1戦）**＝**7 / 23 機構**（未踏の大半は**その山に札が無い**＝スペル・アーツ・【起】・アシスト系）
