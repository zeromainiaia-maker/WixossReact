...
- **🆕 セッション（2026-08-15・続き486・Opus 5）＝§6.4 **O-3 の最大クラスタ「このターン、対戦相手は…」7効果を解体＝5効果の挙動是正**（**恒久 no-op 5件**・うち1件は数字宣言の**過剰実行**も同時是正）。ゲート全緑（**golden 1997**＝+7・**census 831 据置**・lint 0/259）。
  ...
  - **🔴③ `WX24-P2-014-E2` は「6枚引くだけ」に潰れていた**＝原文は「各プレイヤーは…すべてのクラフトをゲームから除外し、すべてのカードをデッキに加えてシャッフルし、6枚引く。このターン、対戦相手はダメージを受けない」。**汎用 `カードをN枚引く` 規則が文末を先取り**して盤面リセットも相手側の処理も落としていた（規則は part1 の汎用 DRAW より**前**に置く）。ダメージ無効は既存 `PREVENT_DAMAGE{owner:'opponent'}` で表せた＝**engine の新規実装ゼロ**。
  - **⚠④ STUB を実装で置き換えると census が +N する（新しい教訓）**＝`vocabCensus` は STUB を含む効果を「STUB/MANUAL格納（要個別確認）」へ逃がしているので、**STUB が消えた瞬間にその効果の他の未表現語彙が高シグナルへ昇格する**。今回の +2 は「計器の較正漏れ1（`ATTACK_BAN` キー追加）」＋「**本物の穴**1（③の除外節）」＝**両方とも潰して 831 据置に戻した**。「STUB を消したら census が増えた」を一律にベースライン更新で流さないこと。
  - **🔑⑤ 「それ」の解決は `storedTargetCards`**（`lastProcessedCards` ではない）＝後者は多くのアクションが暗黙に読むので、`execAttachCharm` に足すと後続の意味が変わりうる。前者は `targetsStored` を**明示した後続だけ**が読む。**live の ATTACH_CHARM 後続4件を実測**して影響が無いことを確認してから決めた。
  - **📋⑥ 残り3形は固有 `DEFERRED_*` へ分割**＝`ATTACK_TAX_HAND_DISCARD`（`SP38-003`＝アタックごとの手札3枚捨てUI）／`OPP_DECLARED_ARTS_NAME_LOCK`（`WXEX2-09`＝**相手による**カード名宣言UI）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`＝相手の複数選択UI＋強制アタック集合＝O-8(a) と同じ層）。⚠`WXEX2-09` は**カード単位で採用すると E1 が退化する**ので E3 の id だけ手パッチ。
  - **▶ 次の一手**＝**O-3** の残22（`npm run census:stubs` の A群が常時の在庫表。次の最大は `DEFERRED_UNPARSED_NEXT_OPP_TURN_CLAUSE` 5／`_THIS_AND_NEXT_TURN_CLAUSE` 4＝**同じやり方で原文を並べてから設計する**）→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き486（§6.4 O-3 最大クラスタ解体）後 最新値（本行が直近の正）**：census **831 据置**（⚠一度 833 まで増えたが、計器較正1＋実装1で戻した＝④参照）、**golden 1997**（+7＝executor 3・gate 1・データ不変条件1・live 形1・盤面リセット1）、smoke **10688 / SKIP 0**、fuzz 全0、held **108枚 / 48群**、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **25種/43件 → 27種/39件**＝受け皿1種が消えて固有3種が出た）。🆕**live JSON changed 7カード**（`WX10-024`／`WX24-P2-014`／`WX24-P4-039`／`WX25-P2-010`／`SP38-003`／`WXDi-P08-030`／`WXEX2-09`。CSV 非改変）。🆕**挙動是正 5効果**（恒久 no-op 5＋うち1件は過剰実行も同時是正）。
...
>    - **【Opus 側】(3) §6.4 の未消化 worklist**＝番号つき表（`O-n` は着手順ではなく参照用の固定ID・欠番はそのまま）。**🆕2026-08-15 に `O-20`（全文 regex 読み・20サイト）と `O-21`（到達不能ハンドラ・19 id）を消化**（詳細は PLAN_DETAIL「2026-08-15 整理⑮」）。**🆕2026-08-15 続き484 で `O-22`（(a)(b)(c) 全部）／`O-23`／`O-24` も消化**（詳細は PLAN_DETAIL「2026-08-15 整理⑯」）。**🆕2026-08-15 続き485 で `O-19`（watcher の `triggerScope` 推論）と新設 `O-25` の本体（引用能力の自己付与）も消化**。**🆕2026-08-15 続き486 で `O-3` の最大クラスタ（`DEFERRED_UNPARSED_THIS_TURN_OPP_CLAUSE` 7効果）を解体**＝新機構 `SIGNI_ATTACK_BAN`／既存 `PREVENT_DAMAGE`／盤面リセット STUB で**5効果の恒久 no-op を解消**し、残り3形を固有 id へ分割。**次の候補＝`O-3` の残22（`npm run census:stubs` の A群が常時の在庫表・機構ごとに独立着手可。次の最大は `_NEXT_OPP_TURN_CLAUSE` 5／`_THIS_AND_NEXT_TURN_CLAUSE` 4）／`O-4`（UNKNOWN 25ノード/25カード）／`O-25` の残3件＋【常】引用クラス**。⚠**受け皿 id の「件数」だけ見て設計しない**＝続き486 では7件に**5機構**が混ざっており、原文を並べて初めて「4件は既存の土台に乗る／3件だけ新UIが要る」と分かった。⚠**STUB を実装で置き換えると census が +N する**（`vocabCensus` は STUB を含む効果を別バケツへ逃がしているため、その効果の他の未表現語彙が同時に昇格する）＝毎回「計器の較正漏れ」か「本物の穴」かを仕分ける。⚠**在庫は §3-1 のとおり必ず実測してから着手する**＝続き483 では簿記の「58 id」が実測19 id だった（入れ子分岐を重複と誤検出していた）。🆕**続き485 では `O-19` の在庫が「推論依存」の記述に反して実測1効果**だった一方、**隣接に9カード規模の別クラス（`O-25`）が埋まっていた**＝**在庫の大小より「その層が計器に映るか」で優先度を決める**。⚠**`census:stubs` は「ハンドラの有無」で実装ありを判定する**＝ログを1行出すだけのハンドラは素通りする（続き459 の教訓）。
