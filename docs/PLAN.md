...
- 🏁**セッション（2026-08-29・続き716・Opus 5 単独）＝§5.1 `V-89`／`V-92` クローズ。**
  **`V-89` の「4セッション連続で再現する FAIL」は仕様どおりの挙動で、計器（シナリオ）の側が嘘だった。**
  ユーザー指示＝**「V-89、V-92 を行う」**。
  📊**進捗3計器＝Sheet1 要対応 0 / 863（据置）｜台帳 残 OPEN 558（据置）｜census 高シグナル 520（据置）**
  gates 全緑（**golden 2972→2973**・smoke 10702 全0・fuzz 全0・lint 248 据置・`census:enginetext` 141 据置）。
  全文は BUGFIXES.md 冒頭。
  - ✅**`V-92`**＝`O-135` の `result.done === false` 枝を実機で観測（`WX17-045`「1枚引き、手札を1枚捨てる」→
    `WXK02-090` の `ON_DRAW`）。**PASS・反転確認で FAIL**、しかも**`+5000` が1件だけ＝二重 collection なし**。
  - 🔴**`V-89` の結論＝(a) シナリオの腐り。engine は最初から正しかった。**
    `WX20-037` の live はもう `canTrash` の `LOOK_AND_REORDER` ではなく `LOOK_PICK_CHAIN`＋`remainder:trash`。
    しかも**旧 spec の deck は白バニラ5枚だけ**＝赤のシグニが0なので「3枚とも remainder→トラッシュ」が正しい。
    ⇒ **登録票が4回引用してきた `hDeck=2 hTrash=3` は仕様どおりの数字だった**（§4.1「実測そのものが嘘をつく」の別形）。
  - 🔴🆕**道中で engine バグを1件見つけて直した＝ライズ配置条件が【ライズ】41枚中31枚で丸ごと無効だった。**
    `getRiseFilter` の終端が `（この条件` 固定で、その書式を持たない古いカードは全部 null ＝
    **空きシグニゾーンへ普通に召喚できた**（下にカードが無いので「この下から〜」のコストもパワー参照も死ぬ）。
    併せて**色は「あなたの」直後限定**（「レベル２以下の**赤の**」で落ちる）、**レベルは「以上」だけ**（実データの
    主流「以下」「レベルNの」が全落ち）。⇒ 終端・色・レベル・カード名を直して **10→28枚**が正しくゲートされる。
    **残 13枚（複数体ライズ＝2〜3ゾーンを消費して1ゾーンへ積む）は `O-147` に登録**（**1体ぶんだけ通すと
    「半分だけ実装した嘘」**になるのでわざと null のまま＝既知の穴）。
  - ✅**実機3本**＝`lookReorderCanTrash`（作り直し）／🆕`riseGateLevelColor`（**反転確認で FAIL**）／
    🆕`o135SpellMidInteractionTrigger`（**反転確認で FAIL**）。既存8本の回帰も全 PASS
    （`refreshTrigger` は `WX15-073` を空きゾーンへ召喚していた＝**違法な手順で緑だった**ので spec を追随させた）。
  - 📌**実機ドライバで同じ罠を2回踏んだ**＝**同じ index を押し続けるとトグルで選択が外れる**（複数 pick・エナ選択）。
    **押した index を `Set` で覚える**／**ゾーン選択は `.first()` ではなく全マッチ走査**（先頭が disabled だと打ち止め）。
