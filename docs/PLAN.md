...
**直近＝2026-09-22＝🔴「手札の【起】を CPU が撃てない」は誤りだった（測って否定）＋ 真の穴＝カットイン窓の第1段**（全文は [BUGFIXES.md](./BUGFIXES.md) 第452バッチ）
- 🔴**前回の自分の注記が間違い**＝`cpuOffFieldActivate` は最初から手札ゾーンを列挙していた。
  📏**実測（live の `handActivated` 11効果を1つずつ盤面に置く）＝人間に提示 10 / CPU が候補に挙げた 10＝完全一致**。
  ⚠**測定で1度こけた**＝全部 `MAIN` で測って「2/2」と出た（**11効果中8は `ATTACK_ARTS` 専用**・5効果は `LRIG_STORY` 条件つき）
  ＝🔑**窓と前提を効果ごとに揃えないと「提示されない」という嘘の結論が出る**。
- 🔴**残っていた唯一の手札【起】はスペルカットイン窓**＝`cpuTurn.ts` に「**人間のスペルに対してCPUは常にパス**」と書いてある。
  📏**母集団＝カットインできる札 61カード／ユーザー作27デッキ中 7デッキ**（A/B に入る `WD13`・`ケトッシー軸` を含む）＝**測れる穴**。
- 🆕**第1段＝列挙の1本化**（`cutinCandidates.ts`）＝画面の 149行の `useMemo` を**逐語で移設し応答者をパラメータ化**。
  画面はそれを呼ぶだけ（**人間の挙動は不変**＝gates 全緑・実機 PASS）。⚠**第2段＝実行側（242行）のパラメータ化と CPU の判断**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.6 `C-10` 第2段**＝`handleCutinUse`（242行）を応答者パラメータ化して**CPU がカットインで応答**できるようにする |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／CPU 完成度 **1**（🔥`C-10` 第2段）／**CPU の強さ 1**（🔵`S-6`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**CPU はスペルカットイン窓で常にパス**（61カード／7デッキが該当＝`C-10` 第2段） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4371**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK`） |
...
🔥**`C-10` CPU がスペルカットイン窓で常にパスする**（2026-09-22 第452バッチで発見・第1段まで実装）＝
📏**母集団＝カットインできる札 61カード／ユーザー作27デッキ中 7デッキ**（A/B に入る `WD13`・`ケトッシー軸` を含む）。
🏁**第1段＝候補列挙の1本化**（`cutinCandidates.ts`＝画面の 149行を逐語で移設し応答者をパラメータ化・人間の挙動は不変）。
⚠**第2段＝実行側**（`BattleScreen.handleCutinUse` 242行・`my` を51回参照）を応答者パラメータ化し、
**CPU の判断**（打ち消せるなら打ち消す／払えるか／先読みで得か）を書く。🔑**測り方**＝`census:play` に窓の踏破を足し、
A/B は `WD13`・`ケトッシー軸` で回す。golden `§5.6 C-10`
...
- **2026-09-22 時点**（第421〜452バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
