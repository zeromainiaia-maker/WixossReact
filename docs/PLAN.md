...
- **🆕 セッション（2026-07-08・続き48・Opus 4.8・§5c census 文型クラスタ「条件節」＝「このシグニが覚醒状態の場合」CONDITIONAL 持ち上げ 7枚）**＝census 文型クラスタ「条件節(〜の場合)」の未処理系統を消化。「【自】：あなたのアタックフェイズ開始時、**このシグニが覚醒状態の場合**、〜」型で、覚醒状態ゲートが JSON から全数脱落し**アタックフェイズ開始時に無条件発火する過剰効果**になっていた。engine の `THIS_CARD_IS_AWAKENED` 条件（`execUtils.ts:858`・`awakened_signi` 参照）と decompiler（→「このシグニが覚醒状態」）は既に実装済み＝**parser の CONDITIONAL 持ち上げ規則が無かっただけ**。**parser**（`src/data/effectParser.ts`）＝`parseSingleSentenceInner` の状態条件節 CONDITIONAL 持ち上げ**局所 CLAUSES 表**（続き23 確立）に `[/このシグニが覚醒状態の場合/, () => ({ type:'THIS_CARD_IS_AWAKENED' })]` を1行追加（モジュール共有 STATE_CONDITION_CLAUSES ではなく局所表限定＝「代わりに」置換昇格へ波及させない＝WX25-P2-078 は rest ガードで従来どおり除外）。**採用7枚**＝PR-Di038/PR-Di039・WXDi-P14-045/047/049・WX25-P2-072/075（後2枚は CHOOSE 分岐②のみ＝分岐①は既 CONDITIONAL or 別条件〔赤スペル使用歴〕で不変）。**除外＝WX25-P2-095**（原文「覚醒状態の場合、《無》を支払ってもよい。そうした場合バニッシュ」＝覚醒条件は SEQUENCE 全体を包むべきが文分割で step[0] OPTIONAL_COST STUB だけ包み、engine の OPTIONAL_COST+CONDITIONAL(IS_MY_TURN) 結合検出〔`effectExecutor.ts:2249`＝STUB を直接ステップとして期待〕が壊れる。SEQUENCE レベル条件化＋power≥12000 脱落とセットで別途）。**検証**＝typecheck・**golden 169→170（+1＝覚醒中のみ then 発火・非覚醒は no-op の回帰ガード）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**（「このシグニが覚醒状態なら、〜」＝続き23 汎用 CONDITIONAL 描画で一貫）・**census 1623→1621（-2＝CHOOSE 2枚は別分岐条件が残り dedup 減は2）**＝`BASELINE_HIGH` 実数更新。**次の一手＝(A)Opus＝§5c census 条件節クラスタの継続**（`docs/_census_clusters.txt`＝(a)「あなたの場にあるすべてのシグニが＜C＞の場合」7枚〔全場シグニ同色＝新 condition 型 ALL_FIELD_SIGNI_MATCH 要〕・(b)「それが〔filter〕の場合」系のうち**真の条件脱落分のみ**〔WX21-011/014 の DOWN 後 それが色→追加ドロー等＝LAST_PROCESSED_MATCHES 持ち上げ。⚠デッキトップ公開→それが〔filter〕→手札 の REVEAL_AND_PICK 系は filter で表現済みの偽陽性＝触らない〕・(c)「このターンに〔色〕のスペル/アーツを使用していた場合」turn-history 条件〔ARTS_USED 拡張〕）。**(B)Opus＝§4タスク2 残（動的比較 lrig相対）**：WX19-042 は「センタールリグ」に釣られ target が LRIG に誤パース＝target mis-parse 修正が先・残高シグナルは SEARCH デッキ相対/条件文で別機構。**(C)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）GRANT_QUOTED_AUTO_ABILITY 内側parse。(D)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43-44の8枚）。(E)持ち越し：PR-Di038 の duration under-representation（「次の対戦相手のターン終了時まで」→UNTIL_END_OF_TURN・既存脱落）・WX25-P2-095（上記）・verifyBattleDrive バッチ状態汚染（続き39から）。**
