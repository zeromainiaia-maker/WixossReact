...
- **🆕 セッション（2026-07-12・続き73・Sonnet 5・§3 Sonnetタスク1 §7実機検証の継続＝ON_LRIG_GROW②を2枚とも検証完了）**＝`verifyBattleDrive.mjs` に `lrigGrowAnyOpp`（WXDi-P13-047）と `lrigGrowAnyOppP03046`（WXDi-P03-046）を新設。**両方とも2回連続PASS**（単体・隣接シナリオ`cpugrow`/`cpugrowblocked`/`wxk10068banish`とのバッチ実行でも確認・`order`配列に両方追加済み）。guest（CPU）を`cpugrow`と同型構成でGROWフェイズ注入→CPU自動グロウ→host側watcherのSELECT_TARGET解決を確認。**WXDi-P13-047でturnOwnerゲート未実装を発見**＝原文「あなたのターンの間、対戦相手のルリグがグロウしたとき」に対し `WXDi-P13-047-E2` にturnOwner系の条件が無く、guest自身のターン中のグロウでも発火＝実戦ではほぼ常に不成立のはずが常に成立してしまう過剰発火の疑い（修正はせずOpusタスク12へ登録）。**WXDi-P03-046はSELECT_TARGET経由だがR38/R43/R46/R39系統のresume経路取りこぼしバグには非該当と確認**（原因アクション＝CPU自動グロウが対話不要で完了し、watcher側の解決は独立した新規interactionのため）。**変更範囲＝`scripts/verifyBattleDrive.mjs`のみ（シナリオ追加）＝engine/parser不変のため typecheck のみ確認・gates再実行不要**。
  - **前回（続き72・Sonnet）＝ON_TARGETED残3枚を全数検証完了**（WXDi-P11-040/WXDi-D09-H14/WX25-P2-055・excludeSelf未実装／owner誤りの2件を発見しOpus登録）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3 Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY内側parse**（WX24-P1-017／WX25-P3-038）。**(B)Opus＝Opusタスク12（最優先候補・観測結果が3件に増加）＝続き70の「多段インタラクションSEQUENCE途中ラウンドの盤面差分トリガー見逃し」バグ**（BUGFIXES/PLAN§7 R30参照）**に加え、続き72の2件（WXDi-P11-040 excludeSelf未実装・WX25-P2-055 owner誤り）・続き73の1件（WXDi-P13-047 turnOwnerゲート未実装）も同タスクへ合流**。**(C)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め＋採用**（続き71で整理済みのパターンA〜Fから着手）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（R42②・ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②③・他多数＝ON_TARGETED①系列とON_LRIG_GROW②は完了）。**(E)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（verifyBattleDriveバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
