...
- 🏁**セッション（2026-08-28・続き707・Opus 5 単独）＝`O-134` 第1バッチ。Sheet1 要対応 2 → 0（計器の較正のみ）。**
  ...
  **実機シナリオ＝新規なし**（§2.3＝原理的に観測できない。理由は下記の機械確認）。
  ...
  - ⚠🔴**「Sheet1 要対応 0」は「Sheet1 に残作業が無い」ではない。** 同日の調査で、
    **生きている worklist に Sheet1 のカードが8枚**（🆕**2026-08-28 続き708 の実測＝5件消化後**＝
    `O-92`(`WX05-025` `WX10-018`)／`O-123`×6 だけ）。
    **`O-129`×2・`O-128`×2・`O-112`×1・`V-90`・`V-91` は消化済み**＝`O-128` の live 残20カードに Sheet1 は無い。
