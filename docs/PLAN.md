...
**直近＝2026-09-17＝CPU が先攻の1ターン目にメインフェイズごと飛ばしていた（`R-23`）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑ユーザー報告「CPU が Lv1 でシグニを出さない」の原因は **CPU デッキの Lv1 ルリグ入れ忘れ**（バグ報告の盤面で特定・ユーザー確認済み）。
- 🔴切り分け中に実バグ＝CPU だけ `turn_count === 1` で**メインフェイズごと** END へ飛ばしていた（公式は「アタックフェイズだけ」）⇒ `resolveNextPhaseAfterMain` を人間と CPU の共通の出口にした。
- 実機 🆕`V-266` PASS×2（注入で CPU 先攻1ターン目）／反転で FAIL。🔑**先攻の経路は通し対戦では引けない（4戦連続で人間先攻）＝注入で見る**。
