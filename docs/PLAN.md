...
- **🆕 セッション（2026-08-01・続き312・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxi) 第11波＝**手札とエナを跨ぐ単一プール** `HAND_OR_ENERGY_CARD` を新設して `WXK06-067-E1` をクローズ。**(lxi) 残2→残1**（残る1件は (lxxiii) 待ちなので **(lxi) 単独で取る作業はもう無い**）**（golden **1215→1218**・census **1355→1354**（`BASELINE_HIGH=1354`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **250枚/98署名 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等）
  - **① 直したのは過剰実行**＝「対戦相手は、自分のシグニ１体と、自分のエナゾーンからカードを２枚まで対象とし、**対象としたエナゾーンのカードと手札を合計２枚デッキの一番上に置かないかぎり**、対象としたそのシグニをデッキの一番上に置く」が `TRANSFER_TO_DECK{SIGNI opponent 1}` **単体**＝回避が丸ごと落ちて**無条件で**相手シグニ1体をデッキトップへ（しかも選ぶのが効果側だった）。
  - **② 🔴 PLAN の旧診断「選択UIも作れない」は外れていた**＝`EffectInteractionModal` は候補配列をそのまま描くだけで**スコープ非依存**（`scopeDesc` に1行足すだけ）、`resumeSelectTarget` の `TRANSFER_TO_DECK` も**既に hand/energy 双方を見て弁別**していた（instanceId はデッキ配布時に1プレイヤー内で一意）。⇒ 実装は `execTransferToDeck` の `HAND_OR_ENERGY_CARD` 分岐**1本**で足りた。**「機構が無い」と書かれた在庫は、着手前に読み手側の既存実装を確かめる**（第10波と同じ結論）。
  - **③ 「合計N枚」は2枝に割ってはいけない**＝`opponentHandDiscard`+`opponentEnergyTrash` を併記すると「**どちらか一方から**N枚」に化け、原文の内訳自由（手札2／手札1+エナ1／エナ2）を失う。単一プールの枝を1本だけ出し、golden で「非 pay/skip の枝がちょうど1本」を assert して逆戻りを止めた。
  - **④ 原文「対象としたエナゾーンのカード」を「エナゾーン全体」へ広げた根拠**＝対象宣言も支払いも**同じ相手**が行い、対象上限2＝支払い枚数2 なので到達しうる結果集合は「手札∪エナから合計2枚」と一致する（相手は払うつもりのエナだけを対象に取る）。エナカードへの `ON_TARGETED` 語彙も engine に無い。判断根拠は parser 規則の直上に残した。
  - **⑤ parser は前置きの切り出しが「N枚**まで**対象とし」を弾いていた**＝`^([^。]*?を対象とし[、,])` を `(?:を|まで)対象とし` へ。⚠射程は「対戦相手は…ないかぎり」文の内側だけなので、CSV 全体 522 件の「まで対象とし」のうち届くのは本件1件（live 差分 changed 1 で実測確認）。
  - **次の一手**＝**Opus**：①🆕**タスク12(lxxv) owner 反転**（`WXDi-P14-005-E1` c2＝自分のデッキを削る自傷・実害が明確・母集団を機械走査してから）②**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く可能性が高い）③**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）④**(lxxvi)** ゾーン供給源が違う2枚（受け皿は第10波で出来た＝残りはゾーン決定だけ）⑤**(lxxiii)** トラッシュ領域移動制限（**片付けると (lxi) 残1 も同時にクローズ**）⑥**(lxxii) 遅延クリアの片側漏れ**⑦(lxxiv)⑧(lxviii) 散文形「対戦相手のターンの間、」⑨§6.3 J 残11 ⑩(lxvii) CPU `ON_TURN_END` 未収集。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜312 の全件が実機UI未検証で積み上がっている**（第10波の配置モーダル活殺／第11波の跨ぎ選択モーダルは**目視しやすい**ので取りやすい）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1218**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1354**（回帰ゲート）・held **250枚/98署名**（2026-08-01 タスク12(lxi) 第11波後に実測）。
>
