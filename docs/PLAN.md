...
| (xc) | **🆕 `computeArtsEffectiveCost` に無い条件つきコスト軽減が4枚（2026-08-03・(lxxxvii) の全数計測で判明）**＝`WXK05-004`（センタールリグが＜エルドラ＞なら《青×2》減る）／`WXK06-016`（場のシグニ1体につき《青×1》減る）／`SP36-001`（相手がこのターンに使用したスペル1枚につき／アーツ使用で《赤×3》《無×3》減る）／`SP38-002`（ライフが相手より多い場合《無×3》減る）。**カットイン固有ではなくアーツ経路でも同じく不発**＝原文に使用コスト改変句を持つカットインアーツ8枚を精査した副産物。⚠`SP36-001` は「対戦相手がスペルを使用していた場合」＝**カットイン窓こそ効くべき場面**。着手するなら `costs.ts` の `computeArtsEffectiveCost` の規則集に regex を足すだけだが、**母集団は原文に「使用コストは…減る」を持つアーツ全体で数え直すこと**（カットイン可能な53枚に限る話ではない） |
| (xci) | **🆕 `specificCardCostReductions`（指定カード名の《無》軽減）が ArtsModal 経路に無い（2026-08-03・(lxxxvii) の実装中に観測）**＝(lxxxvii) の**鏡像**。カットイン窓とボタン表示（`BattleScreen.getCardActions`）はこの軽減を持っているのに、**メインフェイズのアーツ一覧・コスト選択モーダル（`ArtsModal`）は持っていない**＝ボタンは「使える」と出るのにモーダル内の請求額が軽減前、という食い違いが起きうる。着手前に**発生源カード（`collectSpecificCardCostReductions`）と対象カード名の機械抽出**が先＝実際に食い違う組み合わせが存在するかを先に測ること |
...
- **🆕 セッション（2026-08-03・続き339・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(lxxxvi)＋(lxxxvii) 残0クローズ**（ユーザー指示「(lxxxvi)と(lxxxvii) を行う」）。golden **1341→1343**・**live JSON 変更なし**（UI／engine 配線のみ）・census 1288 据置・全ゲート緑。
  - **(lxxxvi) スペル本体のベットが `ON_COIN_PAID` を積まない**＝`castSpell` はコインを払い `is_betting_this_effect` も立てるのに支払いサイトで `collectCoinPaidTriggers` を呼んでいなかった＝**全コイン支払いサイト中ここだけが穴**。対象＝ベット持ちスペル7枚。**登録時に「解決順序の設計判断が要る」と保留した点は (lxxxix) で解決済み**＝`QUEUE_SPELL.effectStack` の形が既にあり、`pending_spell` 待ちでも**カットイン応答の継続もCPU行動も `if (bs.effect_stack …) return;` で待つ**ので「支払い→トリガー解決→カットイン窓→スペル解決」の順になる。(lxxxix) の離場トリガーと**同じ1本のスタックへ合流**させた。
  - **🔴(lxxxvii) は着手前の全数計測で見立てが変わった**＝カットイン可能アーツ**53枚**を測ると、**`computeArtsEffectiveCost` の条件つき軽減規則にヒットする札は 0/53**。原文に使用コスト改変句を持つ8枚を精査すると `WXK05-004`／`WXK06-016`／`SP36-001`／`SP38-002` は**規則集に無い＝アーツ経路でも同じく不発**（カットイン固有ではない＝新在庫 (xc) へ）、2枚はベット置換で (lxxxiv) 済み、2枚は相手のコストを**増やす**効果で無関係。⇒ **実害は場の CONTINUOUS 軽減**＝live 上の唯一の発生源 `WX03-028`「あなたが使用する**青の**アーツのコストは《無×1》減る」に対し**3枚**（`WX01-018`／`WXK05-004`／`WD23-012-A`）が印刷コストのまま請求されていた。
  - **実装**＝`CutinModal` に `artsBaseCost()` を置き、ArtsModal と同じ chain（`applyContinuousCostDecreases(computeArtsEffectiveCost(…, { oppState, cardCostReplacements }), …, activeCostMods.forMy)`）を通す。⚠**`specificCardCostReductions` はこの経路だけが持つ軽減**なので最後に重ねて失わない（ArtsModal 側が持っていない＝**鏡像の穴**＝新在庫 (xci) へ）。⚠PLAN が警告していた衝突（`additionalColorlessCost`／`effectEnergyCostStr`）は**`source === 'lrig_deck'` の基本コストだけを差し替える**ことで回避。
  - **golden +2**＝①(lxxxvi) の対象母集団がちょうど7枚で全部ベット段階を読めること（収集自体は BattleScreen 層＝非カバーなので、母集団が壊れたら気づける計器）②(lxxxvii) の軽減が効くべき3枚を固定＋**青以外は軽減されない**（`cardColor` フィルタが素通りしていないこと）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxviii)／**🆕(xc)**／**🆕(xci)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜339 の全件が実機UI未検証**。⚠🔴**続き333〜339 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①ベット持ちスペルをベットして撃つとコイン反応【自】が発火し、その解決後にカットイン窓→スペル解決の順になること**②`WX03-028` を場に置くとカットイン窓で青のアーツが《無×1》安く表示され、その安いコストで払えること③非該当色は安くならないこと。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き339（🏁§3 タスク12(lxxxvi)＋(lxxxvii) 残0クローズ）後 最新値（本行が直近の正）**：golden **1341→1343**（+2＝①ベット持ちスペル7枚の母集団固定〔`ON_COIN_PAID` 収集自体は BattleScreen 層＝非カバー〕②場の CONTINUOUS 軽減が効くべきカットインアーツ3枚＋青以外は効かないこと）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**、manual field loss 0、同型★0・held **258枚／109群**（いずれも前回据置＝**parser も JSON も触っていない**）。**live JSON は完全不変**（`public/data/` に差分なし・live per-effect changed 0・**新語彙0本**）＝触った層は `CutinModal`（`artsBaseCost` 追加）と `castSpell`（`collectCoinPaidTriggers` 追加）のみ。⚠**コスト表示とコイン反応の発火順序は golden 非カバー**＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き339＝🏁§3 タスク12(lxxxvi)＋(lxxxvii) 残0クローズ後）**＝golden **1343**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
