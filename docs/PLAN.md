...
- **🆕 セッション（2026-08-15・続き490・Opus 5）＝§6.4 **O-3 の `ATTACK_TAX_HAND_DISCARD`（1効果）を解体＝2効果の挙動是正**（**恒久 no-op 2＝うち1件は計器に映らない無言 no-op**）。ゲート全緑（**golden 2019**＝+2・**census 831 据置**・lint 0/259）。
  ...
  - **▶ 次の一手**＝**O-3** の残7（A群が常時の在庫表）。`SKIP_NEXT_TURN`（`WD20-006`＝ターン丸ごとスキップ）／`UNTIL_NEXT_MAIN_PHASE_CLAUSE`（`WXK01-002`＝3機構の複合）／`NEXT_OPP_TURN_MAIN_PHASE_SKIP`（`WXEX2-19`・**実装レシピは parser コメントに記載済み**）など。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → **O-26**（コスト節の自己トラッシュ脱落2件）。
...
- **🆕 2026-08-14 続き481（§7 V-18 決着・V-19 要追試）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1976 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（engine/parser/JSON/React 非改変）。🆕**実機シナリオ＝+10本**（V-18/19）＝**緑/赤の内訳が 101緑6赤 → 107緑10赤**（赤10＝(cxxxiv) 再現1・V-15/16/17 未完5・**V-19 要追試4**）。🆕**Opusタスク12 の在庫＝5件**（据置）。
