...
**直近＝2026-09-21＝🆕§5.7 `S-31` ③＝札の使いどころ（アーツを守り／攻めで使う・この札は使わない）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第445バッチ）
- 🔴**攻めと守りで事情が違う**（自己対戦で測って分かった）＝**攻めは探索が列挙をそのまま手にするので分類の絞りが掛からない**（もう撃っている）。
  **守り（`pickCpuResponseArts`）だけが分類で絞る**＝📏**アタックフェイズに使えるのに分類できない 19種（25.0%）は指定するまで一生撃たない**（13/21デッキ）。
- 入れたもの＝`CpuDeckPlan.cardUse`（`defense`／`offense`／`both`／`never`）。🔑**アーツは列挙（`listCpuArts`）で効かせる**＝探索と貪欲な経路の両方に届く。
  `never` はスペル・【起】3種・ピースにも効く（配り口は `cpu*Input` 6本）。🔴**召喚は止めない**。
- 📏**直接の効き目**＝`WD01` 8戦で《バロック・ディフェンス》**0回 → 13回**（アーツ全体 25 → 38）。反転＝`never` で `WD05` が **33 → 0**。
- 🔴**途中で自分の設計の穴を潰した**＝窓の指定を選ぶ側にだけ書いた版は**「守りで使う」札を探索が攻めで撃った**。
  🔑**見つけ方＝静的な見立てと自己対戦の実測が食い違ったこと**（「分類できない札は撃たれないはず」が外れた）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` ②の残り**（撃てない【起】108）。次に `S-29` |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4363**）｜**実機 PASS**（`v268CpuDeckPlan` ＋ 通し対戦） |
...
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**①は測って否定**（第444バッチ）／🏁**③は消化**（第445バッチ＝札の使いどころ＝守り／攻め／使わない）／🏁**②は2段まで**（手札を捨てる／エナ・場から払う）＝**撃てない【起】 210 → 108**。**残り＝②の続きだけ**（場のシグニ＝`underSelfTrash` 13／`trashExile` 13／`discardSelfFromHand` 9／`charmTrash` 6・ルリグ＝`fieldTrash` 8（**実行側に口が無い**）ほか＝**1キーあたり6〜13件の小さい山**）。 | 中 | 作った CPU デッキを意図どおりに回せる |
...
- **2026-09-21 時点**（第421〜445バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
