...
- **🆕 セッション（2026-08-03・続き340・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝**§3 タスク12(xc)＝コスト軽減規則を37枚ぶん実装**＋🏁**(xci) 残0クローズ**（ユーザー指示「(xc)と(xci) を行う」）。golden **1343→1346**・**live JSON 変更なし**・census 1288 据置・全ゲート緑。
  - **🔴 両方とも「着手前に母集団を数え直せ」と自分で書いた指示が効いた**＝登録時の見立てと実測が食い違った。
  - **(xc) 母集団は4枚ではなく 111枚→未カバー72枚**＝登録時の「4枚」はカットイン可能アーツに限った部分集合だった。内訳＝(lxxxv) 使用時支払い33／ベット・置換1／既存規則カバー5／**未カバー72**。⚠**最初の分類は間違っていた**＝「盤面を振って動くか」で測ったら `effective: 0` になり規則集が死んでいるように見えたが、**汎用シグニを置いた盤面が各札の条件（＜迷宮＞等）を満たしていなかっただけ**。**規則の regex に原文が当たるか**で測り直して初めて正しい内訳が出た。
  - **実装＝37枚ぶん**：A「センタールリグが＜X＞の場合」**14**／H「場にある〔色/カード名/指定なし〕シグニ1体につき」**11**／D「センタールリグのレベル1につき」**5**／C「ルリグトラッシュのアーツ1枚につき」**2**（`myState` に `lrig_trash?` 追加）／E「＜X＞と＜Y＞が両方ある場合」**1**／`SP36-001`（相手の使用実績で**2文が累積**＝早期 return できない唯一の形）**1**／**既存規則の是正 3**。
  - **🔴既存規則が二重に取りこぼしていた**（`fieldPower`／`fieldClass`）＝①「がある場合**、この**スペルの使用コストは」の**読点を跨げない** ②実データの多数派 `《赤×2》`（括弧**内**表記）に当たらない（`《赤》×2` しか読まない）。⇒ `[^。]*?` ＋ `parseGrowCost(normalizeCostText())` へ寄せて `WX15-034`／`WX20-005`／`WX20-006` が復活。
  - **過剰適用ゼロを機械保証**＝規則を緩める作業なので、**空盤面・ルリグなし・Lv0・相手未行動**でアーツ/スペル/ピース **1236枚**を走査し**印刷コストから動く札が0枚**であることを確認（golden に常設＝(xcii) 以降の回帰ゲートにも使える）。
  - **(xci) は「ArtsModal に無い」ではなく「SpellCastModal に無い」だった**＝発生源2枚の**対象はどちらもスペル**（《フレン・スラッシュ》／《ダークネス・イーター》）。従来この軽減を通していたのは `CutinModal` とアーツのボタン表示だけ＝**本来の効き先で印刷コスト請求**だった。⇒ `applySpecificCardCostReduction()` を新設し3経路（スペル/アーツ/カットイン）を統一。
  - **新規在庫3件**＝**(xcii)** 相手盤面を参照する軽減8枚（`computeArtsEffectiveCost` に相手 `PlayerState` を渡すシグネチャ変更＝独立バッチ）／**(xciii)** 持続する「次に使用する〜が安くなる」5枚（【チェイン】＝状態フィールドが要る）／**(xciv)** 1枚ずつ形が違う長テール約20枚（クラスタ化不能＝低優先）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxviii)／**🆕(xcii)**／**🆕(xciii)**／(xciv)〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜340 の全件が実機UI未検証**。⚠🔴**続き333〜340 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①`WXK05-004` を＜エルドラ＞センターで撃つと一覧・モーダルとも《青》×1《無》×1**②`WXK06-016` が場のシグニ体数ぶん安くなる③`WX22-004` がルリグトラッシュのアーツ枚数で安くなる④`WXDi-CP01-027` を場に出すと《フレン・スラッシュ》が《白》×2 で撃てること。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き340（§3 タスク12(xc) 37枚実装＋🏁(xci) 残0クローズ）後 最新値（本行が直近の正）**：golden **1343→1346**（+3＝①新規規則が**条件成立時だけ**効くこと〔A/D/C/E/H の成立・不成立の対、既存 ＜クラス＞規則の回帰、`SP36-001` の3状態〕②**過剰適用ゼロ**の全数走査〔アーツ/スペル/ピース 1236枚を空盤面で走査し0枚〕③(xci) の対象2枚が実在しスペルであること＋対象名以外/カード名不明/発生源不在では何もしないこと）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**、manual field loss 0、同型★0・held **258枚／109群**（いずれも前回据置＝**parser も JSON も触っていない**）。**live JSON は完全不変**（`public/data/` に差分なし・**新語彙0本**）＝触った層は `costs.ts`（規則6本追加＋既存2本の是正＋`applySpecificCardCostReduction` 新設）と3モーダルの呼び出しのみ。⚠**コスト表示・請求は golden 非カバー**（純関数 `computeArtsEffectiveCost` までは固定済み）＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き340＝§3 タスク12(xc) 37枚実装＋🏁(xci) 残0クローズ後）**＝golden **1346**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
