...
- **🆕 セッション（2026-08-13・続き461〜462・Opus 5）＝§7 実機挙動（P3）を**Codex 委譲で3バッチ連投**＝**新規シナリオ13本すべて2回連続PASS・既定 order 登録**。実バグ1件（続き460）＋ドライバの罠3件を解消。ゲート数値は全て据置（golden 1956・census 831・smoke 10688・lint 0 errors/259 warnings）**。
  ...
  - **▶ 次の一手**＝**Opus 側**：(a)続き459 が可視化した**8つの `DEFERRED_*`＝26効果**（`DEFERRED_UNPARSED_THIS_TURN_OPP_CLAUSE` 7件が最大）(b)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵）(c)**O-4 UNKNOWN 25ノード/25カード**(d)**O-18**（続き460 で登録＝手札スペルのボタン生成が `PLAY_COLORLESS`／`BLOCK_NON_WHITE_SPELL` も見ていない＝押せるが無反応）(e)**O-1 CPU AI 拡張**。**Sonnet 側**：**§7 実機検証の残り**＝O-13 と続き452〜453、および続き403〜436 の未消化ブロック（`WX25-CP1-091` の ON_TURN_END／`WXDi-P14-002` のピース起動が最有力）。⚠**新規シナリオは `'field.check': null` を必ず入れる／ラベル全文一致に依存しない。**
