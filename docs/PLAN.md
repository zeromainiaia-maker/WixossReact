...
| (lxxxiii) | **第7波まで一部消化（2026-08-02）**＝残 collector 2効果（leave field／battle banish）を live JSON まで是正。再測＝語彙167、public `excludeSelf` あり141／なし26。既知誤検出5件に加え、専用原因限定機構で既に正しい耐性4件＋`powerDecreaseExcludeSelf` 1件を正式分離し、**実装課題残16**。life crash は source tracking、cost trash は cost-only cause 機構が必要で honest defer。全経路・実測は BUGFIXES 第7波節。 |
