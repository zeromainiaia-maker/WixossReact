...
| 🆕(lviii) | **`PLACE_TRAP_FROM_REVEALED` STUB が残りの行き先を `deck_bottom` に決め打ち**（2026-07-29・第8波で観測）。原文が「残りを好きな順番でデッキの一番**上**に戻す」の**3効果（`SP26-001-E1`／`WD23-032-A-E2`／`WX20-012-E1`）で行き先が逆**になる。カードを失う種類のバグではなく順序の誤りなので優先度は低い。<br>本命は**この STUB を使う13効果を `LOOK_PICK_CHAIN{then:'trap'}` へ移す**こと（機構は第8波で揃った）＝STUB はカード原文を実行時に regex で読んで枚数を決める設計で、`してもよい`／別文の remainder／`そうした場合、` 内包など文型が割れているため parser 側の規則が数本要る（M）。明細は BUGFIXES 2026-07-29「第8波」節 |
| 🔶(lix) | **pick を伴う「好きな枚数を一番下・残りを一番上」＝分割 remainder**。✅**2026-07-29 第10波で2効果を消化**（`WXDi-P16-086-E1`／`WX24-P3-031-E1`）＝新機構は **`remainder.position:'split_top_bottom'` の1つだけ**で足りた（`resumeSearch` の先頭で残りをデッキから抜き、分割UI を continuation の先頭へ差し込む。⚠早期 return 分岐より**前**に置かないと hand-or-energy 経路で分割が落ちる）。⚠**登録時の見立て「機構2つ（chain ステージの hand-or-energy も要る）」は誤り**＝単一 pick 群なので `REVEAL_AND_PICK.handOrEnergy` でそのまま表せた。<br>**残2件＝機構ではなく入口の語彙**：`WXK02-032-E1`（revealCount が「７－あなたのライフクロスの枚数」＝**動的枚数**で `N枚見る` の入口 regex に掛からない）／`WXK03-048-E1`（pick 先が**「場に出す」**＝`parseRevealPickDescriptor` の行き先語彙が 手札／エナ しか受けない）。どちらも S。明細は BUGFIXES 2026-07-29「第10波」節 |
| 🆕(l) | **能力付与の内側が nest されず完全 no-op**。live 全数実測は **A群11件**（旧「3枚」は誤り）。✅3件を実働化：`WXDi-P11-038-E2`（ルリグ付与【常】→自シグニ全体へシャドウ）／`WX26-CP1-005-E1`（付与【自】→アタック時に相手シグニ1体をデッキ下）／`WXDi-P02-034-E1`（付与【自】→被バニッシュ時にそのレベル×2枚を自ミル）。**残8件**＝`WX26-CP1-076-SONG`／`WXK11-052-E1`／`WX24-P4-026-E1`／`WX16-004-E1`／`SPDi44-08-E2`／`WX25-P1-018-E2`／`WD21-009-E1`／`PR-204-E1`（比例値・任意コスト・置換・宣言等の不足。部分近似は過剰実行になるためdefer）。`WX15-016-E1` は続き282で既に nest・実働化済み。<br>**B群36件（別在庫）**＝`rawText:"。"`。`WDK02-009-E1`／`PR-K022-E1`／`WXK01-007-E1` をCSV照合し、親文「センタールリグは以下の能力を得る。」の句点だけを引用本文にし、直後の【起】群をE2以降へflattenする境界抽出ズレと診断。付与スコープ喪失として新規在庫化（本波未実装）。<br>⚠**Claude 精査（2026-07-29）で優先度は低いと判定**＝36件は**全件キーカード**で、付与スコープ句をクラスタリングすると**34件は「あなたのセンタールリグ」＝条件なし**、真に条件が落ちているのは **`WDK02-009`（「あなたの**レベル２以上の**センタールリグ」）の1件だけ**（もう1件 `WXK02-023` は該当句なし）。さらに**エクシードコストと《ターン１回》は E2 以降に保持されている**（`cost:{exceed:1}`／`usageLimit:once_per_turn`）＝コスト踏み倒しではない。したがって34件は「センタールリグの能力」を「キー自身の能力」として登録している**表現ズレ**で実害は薄く、**真の過剰実行は WDK02-009 のレベル条件落ち1件**。着手するならこの1件＋付与スコープの構造化から。 |
...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕単独）＝タスク12 を第7波（宣言参照 pick 8効果・(c) 残0）→第8波（【トラップ】設置併記 2効果・(g) 残0）→第9波（振り分け `split_top_bottom` 6効果・(d) の主因）→**(lvii) census 較正（単独 commit・機能変更なし）**→第10波（pick＋振り分け 2効果・(lix)）で連続消化**（golden 968→**988**・census 1452→**1414**〔`BASELINE_HIGH` 更新。うち **-38 は (lvii) の計器較正**＝機能変更なし〕・smoke 10726 全0・fuzz 全0・lint 0 errors・held 254→**254**〔新規ドリフト0〕・build 冪等・gates 全緑）
  - **第7波（(c) 宣言参照 pick）**＝原文全数走査で6効果。壊れ方が**3種混在**＝①pick が丸ごと `UNKNOWN`＝no-op 4件 ②🔴filter 落ちで**どの公開札でも拾える過剰実行**（`PR-431-E2`）③🔴**公開札の総取り**（`WX13-054-E1`）。新 filter `nameEqDeclaredName`／`classEqDeclaredClass`（未宣言なら候補ゼロ）＋新 STUB `DECLARE_NUMBER_PLAIN`（**既存 `DECLARE_NUMBER` はガード制限専用**）＋`StubAction.declareOptions`。
  - **第8波（(g) 【トラップ】設置併記）**＝新ステージ **`LookPickChainStage.then:'trap'`＋`remainder.location:'hand'`**。🔴`WX19-039-E1` は設置も手札加えも**丸ごと no-op**、`WX15-083-TRAP` は STUB が残りの行き先をデッキ下に決め打ちしていた。
  - **🔴第9波（(d)）が本セッション最大の構造的発見**＝「好きな枚数を一番下・残りを一番上」の**振り分け機構 `split_top_bottom` は G168 で type/engine/BattleScreen 分割UI/逆翻訳まで全部入っていたのに、parser 側の規則だけが無かった**。そのため手書き MANUAL の `WX13-081`/`082` の2枚しか使っておらず、**同じ文型の他6効果は「一番下」を含むというだけで `position:'bottom'` に潰れ、見た全部がデッキ下へ送られていた**（良い札を上に残す原文の意味が消失）。**新規則の生 parser 出力が人手の MANUAL と完全一致した**のが正しさの独立した裏取り。
  - **⚠層を間違えかけた**＝`WXDi-P15-081` は **`manualEffects.ts` にも定義がある**ので built JSON だけ直すと2層が食い違う（app は built JSON 直読み・golden の `cardMap.effects` は merge 済み）。TS ソース側を直し、**両層の一致を golden の不変条件に固定**した。併せて **MANUAL カードへの fresh 丸ごと採用は禁物**（`WXDi-P15-081-E2` の fresh は【ゲート】条件を落とす＝無条件発動の過剰実行）＝リーフだけ外科パッチ。
  - **⚠engine の落とし穴（第8波）**＝①**既存 `INTERNAL_SET_TRAP` は手札からの設置専用**でデッキ公開札に使うと**複製バグ** ②**`resumeSearch` の `applyDirectAction` ループは `!done` で即 return** するので対話つき thenAction をそこに載せると**外側 continuation が丸ごと落ちる**。
  - **外科性**＝3波とも「生 parser 出力の effectId 差分」＝「live JSON の per-effect 差分」で、第7波8／第8波2／第9波6件**だけ**（第9波は fresh 差分8のうち2件が WX13-081/082＝curated に追いついただけ）。
  - **golden の教訓**＝**「危険な側」を単体テストで名指しする**（未宣言→候補ゼロ／ガード制限を立てない／1枚だけエナへ／トラップ札がデッキから消える／選ばなかった札は上に残る／MANUAL の条件が生き残る）。**変異12種（3波合計）でそれぞれ狙った本数だけ FAIL** することを実測。
  - **🆕計器・既知バグを3件登録**＝**(lvii)** census「「Nまで」上限選択」のキー表に `pickUpTo` が無い（**119効果**・2度再発）／**(lviii)** `PLACE_TRAP_FROM_REVEALED` が remainder を `deck_bottom` 決め打ちで3効果の行き先が逆（本命は同 STUB を使う13効果の `then:'trap'` 移行）／**(lix)** pick を伴う分割4効果（remainder を対話UIにする機構が要る）。
  - **⚠PLAN の予測を1件訂正**＝(d) `WXDi-P16-086-E1` は「`then:'deck_bottom'`＋`_bottomReserved` が要る」と書かれていたが**誤り**。振り分けは既存 `split_top_bottom` で表せ、本当に足りないのは「remainder を対話にする」ことと「chain ステージの hand-or-energy」だった。
  - **⚠要実機検証**＝宣言 CHOOSE／トラップのゾーン選択／**分割UI（`EffectInteractionModal` の split_top_bottom 経路）** はいずれも engine 内は golden で見ているが実機未確認。**特に分割UI は今回いきなり6効果が通るようになった**ので優先度が高い。
  - **✅(lvii) census 較正を単独 commit で消化**＝キー表の `upTo`（小文字u）が `pickUpTo`（大文字U）に部分一致せず、REVEAL_AND_PICK が原文どおり「N枚まで」を表現していても語彙欠落に数えていた偽陽性**42件**を解消（1453→**1415**）。⚠**機能変更なし**＝42件は全数で「JSON に `pickUpTo` あり」かつ「原文に上限表現あり」を機械確認。**golden が1本も動かないこと自体が「機能に触れていない」ことの確認**になる。残 106 のうち 27 は LOOK_PICK_CHAIN だが**型名は key に足さない**（LPC は「ちょうどN枚」を表現できず、一括免除するとその機構ギャップごと隠れる）。
  - **第10波（(lix) pick＋振り分け 2効果）**＝`remainder.position:'split_top_bottom'` を新設し 1415→**1414**。⚠**登録時の自分の見立て「機構2つ」は誤りで、実際は1つで足りた**（単一 pick 群なので `REVEAL_AND_PICK.handOrEnergy` がそのまま使えた）。⚠**書き換えは `resumeSearch` の早期 return 分岐より前**に置く必要がある（後ろだと hand-or-energy 経路だけ分割が丸ごと落ちる＝変異で実測）。残2件は機構ではなく**入口の語彙**（動的枚数「７－ライフ枚数」／pick 先が「場に出す」）。
  - **次の一手**＝**Opus：(xlvi) 残13件**〔(a)残2・(b)10・(d)残1〕**／(lix) 残2件（動的枚数・「場に出す」＝いずれも入口の語彙・S）／(lviii) トラップ STUB 13効果の LPC 移行（機構は第8波で揃った・M）／(xxix) 残**〔(2)(B) 43件＝`costUnparsed` 331効果が機械的 worklist／(1) 未対応コスト80件〕**／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／(l) 内側【自】parse 失敗3枚／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋直近の (xxix)(1)(2)＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）
- **P1 表現①の systematic 指標**：同型★0（`node scripts/groupSimilar.mjs --all`）。**held は 252枚／署名グループ 108件（2026-07-29 第16波実測・`node scripts/heldReview.mjs`。raw 253枚のうち curated一致＝採用済み1枚を除外）。LOSS/VALUE は held 188 / LOSS 154 / VALUE 34（2026-07-19 実測・`npx tsx scripts/parserWorklist.ts`・⚠HEAD比較＝未コミットJSONは反映されない）**。第16波は `WXDi-P08-007` 1枚を採用したぶん raw held が253だが、レビュー対象から自動除外されるため実働 backlog は252/108で据置。続き29時点（held 79）からの増加は主に**その後の parser 改善で fresh が curated より正しくなった採用待ちバックログ側**（Sonnetタスク6の採用サイクルで消化してから実数を締め直す）。**この数字からさらに増えたら回帰**（JSON手パッチ時は パーサー同修正 or MANUAL化 or ここを実数更新）。旧内訳の詳細は PLAN_DETAIL 参照。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1013**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1409**（回帰ゲート）・held **252枚/110署名**（2026-07-29 第17波実測）。
>
