...
- **🆕 セッション（2026-08-11・続き428・Opus 5）＝§6.4「エナ支払い元の一本化」＝支払い funnel を新設し、`UNDER_CARD_AS_ENERGY_COST` をその最初の消費者として実装。golden 1776→1785・census 854 据置**。
  - **🔴① 「エナコストの支払い」がどこにも1本化されていなかった**＝支払いモーダル14本が `my.energy` を直読みし、候補生成も控除も **BattleScreen の14サイトに散在**。この状態で「エナゾーン以外を支払い元にする」語彙を載せると、**半分だけ配線された瞬間に「アーツには使えるが【起】には使えない」無言の不整合**になる（`census` にも `census:stubs` にも出ない）。続き427 の defer 判断（funnel が先）はそのとおりだった。
  - **⭐② funnel の形は2点で決まる**＝(a)**後方互換の index 空間**（`buildEnergyPayPool` の先頭 `my.energy.length` 件がエナゾーンそのもの・同じ順＝既存の `costIndices` がそのまま通る／追加元0件なら従来と完全に等価）(b)**控除は `applyTo` でサイトの state 組み立ての「あと」に当てる**（サイトが `field` を自前で組み直しても取りこぼさない）。
  - **⭐③ 安全弁は関数の戻り値に置く（教訓 (m) の実践）**＝`applyTo` を呼び忘れると**エナが1枚も減らない**＝「ただでアーツが撃てる」という**即座に露見する**壊れ方になる。**無言にならない壊れ方を選ぶ**のが設計の本体。加えて golden にソース走査トリップワイヤ（`BattleScreen.tsx` に `.energy.filter((_, i) => !` が残っていない）を置いた。
  - **🔴④ ゲートも pool にしないと「払えるのに候補に出ない」**＝`canAffordGrowCost`／`canAffordWithExtraCost` の**7箇所**を候補生成と対で直した。片方だけだと UI が黙って矛盾する。
  - **⚠⑤ funnel を通さないものを3つ明示的に残した**＝(a)`signi_attack_cost` (b)ガード追加《無》＝どちらも**選択のない自動控除**で原文の「支払う際」に当たらない (c)`energyTrash*`／レゾナ出現条件＝原文が「**エナゾーンから**トラッシュに置く」＝エナゾーン専用が正しい。**CPU の支払いヒューリスティックも pool を見ない**（近似・ルール違反にはならない）。
  - **⭐⑥ 上限は候補生成の1点で止める**＝「1ターンに3つまで」を pool に載せる枚数で表現し、**13本のモーダルのトグルに検算を撒かない**。構造的に過払いが起きない（副作用＝残り枚数より下カードが多いとき「どれを払うか」は選べない安全側の近似）。
  - **🔴⑦ 同じスタックを2機構が index で触るとズレる**＝カットイン経路だけ `underSelfTrash` と同居しうるので、**同じカードが両方を持たない**ことを golden でロック（現状0件）。
  - **✅⑧ ゲート**＝`npm run gates` 全緑（typecheck / golden **1785（+9）** / smoke 10688 全0・SKIP 0 / fuzz 全0 / census **854 据置** / census-stubs（明示 defer **17→16**・無言 no-op **0**）/ manual-fields / lint 0 errors）。`npm run regen` 済み。**要実機検証4件**は §7 に登録（**回帰確認＝14経路で「選んだエナだけが減る」ことが最優先**）。
  - **📋⑨ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**残 A群16件の明示 defer 見直し**（§6.3 の機構が進んだものから解ける）(e)**置換5本の対話化**（`WXDi-CP01-023` とセット）。**Sonnet 側**＝§7 実機検証（**続き428 の回帰確認を最優先**、次いで続き424〜427）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き428（§6.4 エナ支払い元の一本化＝支払い funnel 新設＋`UNDER_CARD_AS_ENERGY_COST` 実装）後 最新値（本行が直近の正）**：census **854**、golden **1785（+9）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
>
> 0. 🎯**次の一手（2026-08-11 続き428 更新）＝§6.4 の残**＝(a)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)**置換5本の対話化**（`WXDi-CP01-023` とセット＝engine 側は同期的 ctx 変換で pause が張れないので設計変更が要る単独バッチ）(d)**残 A群16件の明示 defer 見直し**（§6.3 の機構が進んだものから解ける）。**Sonnet 側＝§7 実機検証（続き428 の回帰確認が最優先）**。以下は旧「次の一手」（参考）。
>
