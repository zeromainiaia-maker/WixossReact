...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-60** | **engine が「カード全文 regex」で意味を決める箇所が系統として残っている** | **142ハンドラ（実測）／1ハンドラ＝1バッチ** | **手順①（機械分類）は計器になった**＝`npm run census:enginetext`（`scripts/censusEngineText.ts`・明細 `docs/_census_enginetext.txt`・`npx tsx scripts/censusEngineText.ts --id <ハンドラ>` で1件の完全内訳）。■**現在地（2026-08-26 続き664）**＝**A🔴 SELF_TEXT 146行／142ハンドラ**（B 59／C 26）・**regex が1本も当たらないハンドラ 47・miss カード 98**。初回計測は 150行/145ハンドラ・miss 165 だった。■🔴**ゲート（ratchet）は `npm run gates` に同梱済み**＝A群の行数が**増えたら exit 1**／**減っても exit 1**（`BASELINE_SELF_TEXT` の下げ忘れ防止）。■**次の3件**＝`POWER_MOD_PER_COUNT`（miss8/live58）／`DOUBLE_POWER_MINUS`（miss7/live7）／`PLACE_CARD_UNDER_SIGNI`（miss6/live7）。 ■**手口**（4バッチで確立）＝①**先に `grep <STUB_ID> src/engine/` で消費地点を全部数える**（同じ regex が地点ごとにコピーされている＝`COPY_LRIG_NAME_ABILITY` は4地点・`DEPLOY_RESTRICT` は2地点。1地点だと思って直すと残りが古いまま）②**ハンドラ末尾の else を必ず読む**（🔴`LRIG_UNDER_CARD_OP` は**regex すら無い最後の else**で「下のカードを全部トラッシュ」していた＝**計器の miss はこれを測れない**）③parser が判別子を吐く→engine は payload で分岐→**逆翻訳も payload から描く**→regex 撤去→基準を下げる ④**payload 欠落は fail-closed**（「効かない」で済ませ、盤面のカードを失わせない）。 ■✅**消化済み4バッチ**＝第1 `LOOK_OPP_LIFE_TOP`（live 28効果中27で regex 不発）／第2 `LRIG_UNDER_CARD_OP`（live 17効果中15が無条件フォールバックか無言ログ）／第3 `COPY_LRIG_NAME_ABILITY`（消費4地点・action ハンドラは連用形で全滅＋【自】コピーが能力種別を見ず過剰実行）／第4 `DEPLOY_RESTRICT`（消費2地点の重複）。全文は BUGFIXES.md 2026-08-26。 |
