...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.6 `C-1`＋`C-2`＋`C-3`**（乱数の seed 化／CPU のガード判定／機構踏破計器）＝2026-09-16 にユーザー決定で新設した**プレイ駆動の発見器**。§5.3 の索引は全部 残0 |
| 📊**進捗3計器** | Sheet1 要対応 **2 / 863**（`census:cards -- --sheet 1` で測り直す）／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／🆕**CPU 完成度 8**（§5.6 `C-1`〜`C-8`） |
...
**取る順**＝①実機（寝かせるほど切り分けが高くつく）→ ②機構（索引の並び順）→ ③実装キュー（機構不要の候補だけ）→ 🔥**④CPU 完成度（`C-1` から順に）** → ⑤意味照合（①〜④が空のとき）。
...
| **対話応答** | ⚠「最初の available」固定＋`Math.random` | `BattleScreen.tsx:660`（対象シャッフル）／`:2263`（じゃんけん） |

- 🔑**ガードは試合の長さの主因**＝ルリグアタックが毎回素通りするので **CPU 戦は 8ターン / 232秒で終わる**（`verifyFullMatch.mjs` の実測注記）。ここを直すと**全機構の到達深度がまとめて伸びる**。
- ⚠**乱数は `Math.random()` が4箇所**（`BattleScreen:660` `:2263`／`execUtils.shuffle:170`／`battleUtils.shuffle:79`）＝**いま対戦は1つも再現できない**。

#### 5.6.2 取る順（🔑「CPU が強くなる順」ではなく「踏まれていない経路が増える順」）

| ID | 機構 | 規模 | なぜこの順か |
|---|---|---|---|
| `C-1` | **乱数の seed 化**（上の4箇所） | S | 🔴**前提**。CPU が見つけたものを再現できないと golden に落とせない |
...
- **2026-09-16 時点**（第385バッチ＝`O-529`／`O-530` クローズ）
  ...
  - 📦**在庫**＝機構 worklist **0**｜実機 **0**｜実装キュー **0**｜round6 **完了**｜🆕**CPU 完成度 8**（§5.6 `C-1`〜`C-8`・新設）
  - 🔧**ゲート**＝`npm run gates` 全緑（golden 4270・`census:traceinv` I1=0・I3 72）
