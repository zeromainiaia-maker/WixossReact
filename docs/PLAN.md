...
- **🆕 セッション（2026-08-15・続き482・Opus 5）＝§6.4 **O-20 完了＝「実行時にカード全文 regex で意味を決める」層を source 配線で解体**（棚卸し済み21行のうち**コード20サイト**を変換／残1行は O-19 へ統合）。ゲート全緑（**golden 1978**＝+2・census 831 据置・lint 0/259）。
  - **✅① 機構＝既存の計器を実行時に開いただけ**＝`logSourceText`（`effectParser.ts`）が **effectId → 生成元の能力ブロック原文**を全構築サイトで既に記録していたので、`getAbilityBlockTexts(card)`（カード単位キャッシュ）／`abilityBlockTextOf(card, effectId)`／`execUtils.sourceAbilityText(ctx)` として開き、**`executeEffect` が `ctx.sourceEffectId` を補完**（従来は BattleScreen の2箇所でしか載らず特定不能だった）。⚠**effects.json へ焼かない**＝`PRESERVE_STATUSES` が MANUAL/PARTIAL を丸ごと温存するため**手修正カードに永久に載らない**から。**ブロックを特定できなければカード全文＝従来動作**なので退化しない。
  - **✅② 実測で14効果の挙動が是正**＝`WXK10-084-E2`（奇数対象が偶数へ反転して空振り→**正しく発火**）／`WXK10-083-E1`・`WX19-047`（エナ限定なのに**手札まで保護**）／`WXDi-P10-006-E3`（**余分な2ドロー**→1）／`WXK04-002-E3`（E2 の＜紅蓮＞で対象が消えていた→**【アサシン】付与が発火**）／`WXDi-P00-015-E1` 他（**自シグニの配置替えが相手対象化**）／`WXK03-042-E1`（耐性の**過剰付与**）／`WXDi-CP01-005-E1`（1人コラボが**2人 call**）／`WX25-P1-034-E2`（**場に出すクラフトをルリグデッキへ**）／`WXEX2-21-E3`（「すべてのシグニ」を**両者処理**に実装）ほか。
  - **🔴③ 併せて2つの構造問題を発見**＝(a)**同じ STUB id のハンドラが2つある＝58 id**。dispatch は先着勝ちなので**二番手は到達不能**で、**O-20 の2行はまさにその到達不能側を指していた**（live は同ファイル上方＝そちらを直した）→ **O-21** に登録。(b)**`behaviorAudit` の ctx に `sourceEffectId` が無く**、実アプリと違って**resume 経路だけ全文へフォールバック**していた（直すまで「修正したのに相手シグニが動く」ように見えた）→ ハーネスを実機に合わせて修正。
  - **🔑④ 検証は「変換前後の機械差分」で行った**＝`setAbilityBlockScoping(false)` ＋ `behaviorAudit --o20-fulltext` / `--ids`。対象220効果から**「ブロック＝カード全文（他能力なし）＝リスク0」を除いた156カード**を2モードで実行し、**変化17効果＝意図した修正14＋シャッフル乱数3**（同一モード2回でも揺れることを確認）まで説明を付けた。
  - **▶ 次の一手**＝**O-21 の重複ハンドラ棚卸し**（「片方だけ直して直った気になる」を潰す・58 id）→ **O-22**（O-20 で露出した別要因 no-op 3件）→ **O-19**（watcher の `triggerScope` を parser が書く恒久解）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き482（§6.4 O-20 完了）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1978**（+2＝O-20 の契約テスト／トリップワイヤ）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（effects JSON・CSV は非改変。変更は engine 5ファイル＋`effectParser` ＋ `behaviorAudit`／`goldenTest`）。🆕**O-20 変換＝20サイト・挙動是正14効果**。🆕**重複 STUB ハンドラ 58 id を検出（O-21）**。
...
| **O-21** | 🆕**同じ STUB id のハンドラが2つある（二番手は到達不能）** | **58 id** | 2026-08-15 続き482 で発見。dispatch は `execStubPart1 → 2 → 3` の**先着勝ち**（`execStub.ts:14-16` の `??` 連鎖）なので、**同じ id の2つ目以降は永久に呼ばれない**。⚠**O-20 の2行がまさにこれを指していた**＝`POWER_MOD_BY_LRIG_LEVEL_SUM`（:2578）／`POWER_MOD_BY_COLOR_VARIETY`（:2629）は到達不能で、実際に効くのは同ファイル上方の :244 / :308 だった（続き482 で live 側を修正し、到達不能側にはコメントを入れた）。**危険なのは「片方だけ直して直った気になる」こと**＝棚卸しして (a) 到達不能側を消す (b) 消せないなら明示コメントを入れる。列挙は `grep -hoE "stub\.id === '[A-Z_0-9]+'" src/engine/execStubPart*.ts src/engine/effectExecutor.ts \| sort \| uniq -d`（2026-08-15 実測 **58 id**）。⚠**`effectExecutor` 側の重複は CONDITIONAL 専用経路**（`conditional.then` を見る）なので**必ずしも死んでいない**＝1件ずつ経路を確認すること |
| **O-22** | 🆕**O-20 の変換で「別要因の no-op」が露出した3件** | 3件 | 続き482 でブロック限定読みに直したが、**別の要因で依然 no-op / 未実装**。(a) `CHARM_CONDITIONAL_POWER`（`WX07-031-BURST`／`WX08-032-BURST`）＝「代わりに－20000」の置換側なのに **delta を `sourceCardNum` に適用**する＝BURST は場に無いので **`hasCharm` が常に偽＝恒久 no-op**。本来は「同じ SEQUENCE で既に −10000 された**相手シグニ**へ差分を追加」。(b) `CONDITIONAL_PER_TRASH`（`WX12-037-E2`）＝原文は「《メツム》を落としたら**この効果を繰り返す**」でトラッシュ枚数条件ではない＝**別機構**（既定値5を発明しないよう no-op 化済み）。(c) `CRAFT_TO_LRIG_DECK`（`WX25-P1-034-E2`）＝**＜ヤミノアーツ＞のクラフト5種が CSV に無い**（`TOKEN_SETS` の 'ダークアーツ'→`WX25-P1-TK1..5` も実データ無し）＝**データ側の欠落** |
| **O-23** | 🆕**`POWER_MOD_BY_ATTACKER_LEVEL` が「アタックしたそのシグニ」を効果元で代用** | 1機構 | 続き482 に隣接発見。`WXK10-084-E1/E2` の原文は「**アタックしたそのシグニの**レベル１につき」なのに、engine は `ctx.sourceCardNum`（＝能力の持ち主）の Level を使う。`triggerScope: any_ally` なので**別の味方がアタックした場合に誤った倍率**になる。要るのは `triggeringCardNum` の採用（載っているかの確認から） |
