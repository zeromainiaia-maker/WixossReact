...
| (lxxxix) | **🆕 使用時の任意支払いによるコスト軽減のうち「支払い元＝場のシグニをトラッシュ」の2枚が未実装（2026-08-03・(lxxxv) 本消化の honest defer）**＝`WX24-P3-010`（あなたのシグニを好きな数トラッシュ→1体につき《黒×1》減る）／`WX25-P1-110`（レベル2以下の＜古代兵器＞1体をトラッシュ→《黒×1》減る）。(lxxxv) で作った `useTimeCost.ts` は**平坦なゾーンしか触らない**前提（手札／`signi_down` フラグ／ルリグデッキ／ライフ／キー）で、場のシグニを場から動かすには**場スタック（下のカード）の扱いと離場トリガーの発火**が要る＝別の risk class なので同梱しなかった。⚠**先頭の支払いステップも意図的に触っていない**（`WX25-P1-110` は `STUB OPTIONAL_COST` 頭＝Pattern⑤ で**断ると本体が丸ごと不発**の罠が残ったまま）＝UI を作らずに先頭だけ落とすと支払いの機会ごと消えるので、**必ず使用時UIとセットで直すこと**。流用できるのは `UseCostPaymentPanel`（候補ピッカー）と `payUseTimeCost` の分岐追加 |
| (lxxxvi) | **🆕 スペル本体のベットが `ON_COIN_PAID` を積まない（2026-08-03・(lxxxiv) 実装中に観測）**＝`castSpell` は `betCoins` を受けてコインを払い `is_betting_this_effect` も立てるのに、**支払いサイトで `collectCoinPaidTriggers` を呼んでいない**（アーツ/グロウ/キー/シグニ【起】【出】/**カットイン**〔(lxxxiv) で配線済〕の6サイトは呼んでいる）。**影響＝ベット持ちスペル7枚**（`WXDi-D09-P26`／`WXDi-P07-059`／`WXDi-P07-068`／`WXDi-P09-076`／`WXDi-P09-083`／`WXDi-P15-071`／`WXDi-P15-075`）でコイン反応【自】が不発。⚠**単純に足せない**＝スペルは `pending_spell` で相手のカットイン応答を待つ＝**その待ち時間中にスタックへ積むことになる**ので、コイン反応とスペル本体・カットインの解決順序を先に決めること（(lxxxiv) では inline 解決なので順序が自明だった） |
| (lxxxvii) | **🆕 カットイン窓のコスト算出がコスト機構を通っていない（2026-08-03・(lxxxiv) 実装中に観測）**＝`CutinModal` は `lrig_deck` 由来アーツのコストを **CSV の `Cost` 列＋`specificCardCostReductions` だけ**で出しており、`computeArtsEffectiveCost`（EffectText 由来の条件つき軽減）・`applyContinuousCostDecreases`（場の CONTINUOUS 軽減）・`card_cost_replacements`（(lxxxi)① のゲーム間置換）を**どれも通さない**。⇒ **同じアーツがメインフェイズ経由とカットイン経由でコストが食い違う**。(lxxxiv) ではベット置換だけを差し込んだ（対象8枚に必要な最小限）。⚠着手前に**カットイン可能アーツのうち実際に差が出る札を機械抽出**すること＝無条件に `computeArtsEffectiveCost` へ寄せると `additionalColorlessCost`（`WX24-P3-036`）や `effectEnergyCostStr`（場のルリグ/シグニ由来）の枝と衝突する |
...
- **🆕 セッション（2026-08-03・続き337・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝**§3 タスク12(lxxxv)＝「使用する際…してもよい。そうした場合、使用コストは…**減る**」＝使用時の任意支払いによるコスト軽減を実装＝36枚中**31枚**（ユーザー指示「(lxxxv) を行う」）。golden **1334→1339**・live JSON **31カード**・census 1288 据置・同型★0・全ゲート緑。
  - **母集団は PLAN の見立て（22枚）ではなく36枚だった**＝原文「使用する際」＋「使用コストは…減る」を全数機械抽出。実装31枚の支払い元は5種＝**手札13／場のシグニをダウン6／ルリグデッキのアーツ10／ライフクロス1／場のキー1**。除外5枚＝`WX15-067`（既実装）・`WDK15-007`（ベット由来）・`SPK06-01`（「追加で支払う」＝**増額**の誤ヒット）・`WX24-P3-010`／`WX25-P1-110`（場のシグニをトラッシュ＝honest defer）。
  - **🔴 真因は「no-op だった」ではなく「払わされるのに一度も安くならない」**＝軽減マーカーは no-op だが、**支払い側の先頭ステップは engine に実装済みで実際に払わせていた**。`WD12-006` は先頭が `optional:true` すら無い**強制** `TRASH{HAND_CARD}`＝断れない。さらに `STUB OPTIONAL_COST` 頭の7枚は Pattern⑤ の「skip→残りステップをスキップ」で、**支払いを断ると本体が丸ごと不発**（`WX11-044` の相手手札3枚捨て／`WX12-032` のバニッシュ／`WX24-P3-004` の敗北しない が消える）＝(lxxxi) 残テールと同じ罠。
  - **実装**＝①純関数モジュール `src/screens/battle/useTimeCost.ts`（spec parse／候補列挙／軽減適用／選択妥当性／各ゾーンの実移動）②**共有UI `UseCostPaymentPanel`**（支払い元が5種なので `SpellCastModal` と `ArtsModal` で同じ部品を使う。片方に書くと必ず片方だけ壊れる）③`castSpell`／`executeArts` に `useCostPayKeys`。⚠**支払い元が手札のときは既存 `discardIndices` と同じ index 空間でまとめて消す**（別々に消すと index がずれて別のカードが飛ぶ）④parser `stripUseTimeCostReductionStep` で解決中の支払いステップを落とす（`parseSpellEffect` と `parseArtsEffect` の両方）。
  - **⚠固定形は「ちょうどN枚」でだけ効く**＝`WX11-044` はスペル3枚で《青×3》減、2枚では減らないし捨てない（半端な支払いで手札を失わせない）。**一覧の使用可否も軽減後コストで見る**（(lxxxiv) と同じ罠＝`WX24-P2-002` は白エナ0だと一覧で潰れて支払い画面に到達できない）。
  - **採用**＝28枚は `heldReview --adopt`（strip は step の減少＝`isPureSuperset` を通らず held に落ちる）／**3枚は MANUAL＝PRESERVE 保護**で `manualEffects.ts` にも無い＝**外科パッチ** `scripts/archive/lxxxvStripUseTimeCostReduction.mjs`（冪等確認済み）。live 差分は **changed 31 / added 0 / removed 0**、うち29枚は「先頭1本除去のみ」で完全一致・残2枚は相乗りの既存ドリフト（`WX07-024` は**純増**＝欠落していた「相手シグニをトラッシュ」が復活）。
  - **逆翻訳の正直さ**＝支払いステップを落とすと逆翻訳が「支払いが消えた」と読めるので `decompileEffects` に `（使用時に支払う）…` の復元を追加（同型★0 維持）。**(lxxxii) の golden 2件は意図的に反転**（「前置の支払いSTUBを保持」→「残さない」。CHOOSE とコスト減マーカーを守る当時の意図はそのまま）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxvi)／(lxxxvii)／(lxxxviii)／**🆕(lxxxix)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜337 の全件が実機UI未検証**。⚠🔴**続き333〜337 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①`WX24-P2-002` が白エナ0でも一覧に出てルリグデッキの白アーツ1枚で《白》×3→なし**②`WX13-025` の2枚まで比例③`WX07-024` の「好きな数ダウン」④`WX11-044` は3枚そろえないと軽減も捨ても起きない⑤`WX24-P3-004` のライフ払い／`SPK01-07` のキー払い⑥**支払わずに使っても本体が動く**（従来 OPTIONAL_COST 頭で消えていた3枚）。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き337（§3 タスク12(lxxxv)＝使用時の任意支払いによるコスト軽減 31枚）後 最新値（本行が直近の正）**：golden **1334→1339**（+5＝①対象31枚の spec と軽減後コスト〔支払い元・上限・比例/固定・1枚時・上限時・0枚時の据え置き〕②除外5枚を読まないこと〔理由つき〕③固定形の「ちょうどN枚」境界と候補フィルタ④**5ゾーンすべての支払いが盤面を正しく動かすこと**⑤**31枚の解決中の支払いステップが live から落ち、かつ本体ステップが残っていること**＋据え置き2枚の先頭は不変）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings**（+5＝新規2ファイル分）、manual field loss 0、同型★0、held **259枚／110群 → 257枚／109群**。**live per-effect 差分＝changed 31／added 0／removed 0**（29枚は「先頭1本除去のみ」で完全一致・残2枚は採用に相乗りした既存ドリフトで `WX07-024` は純増）。**新語彙 0本**＝engine の action/condition 型は増えていない（UI 層の純関数モジュール `useTimeCost.ts` を新設し、parser は先頭ステップを落とすだけ）。⚠**支払いUIとコスト再計算は golden 非カバー**＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き337＝§3 タスク12(lxxxv) 31枚実装後）**＝golden **1339**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **257枚／109群**。
>
