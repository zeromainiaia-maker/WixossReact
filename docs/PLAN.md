...
| 🆕(lvii) | **census の計器盲点＝「「Nまで」上限選択」カテゴリのキー表に `pickUpTo` が無い**（2026-07-29・第7波で2度目の再発を観測）。キーは `['"upToCount":true','maxCount','upTo']` で、`pickUpTo`（`REVEAL_AND_PICK` が「N枚まで」を忠実表現するキー）にマッチしない。そのため **look-pick を STUB から実アクションへ直すたびに、STUB の一括免除を失った効果が「語彙欠落」側へ移動して census が +1 される**（第5波 `WX12-024-BURST`／第7波 `WXK05-021-E2`）。**実データで該当は119効果**（`pickUpTo` を持ち他の upTo 系キーを持たない効果）＝較正すれば偽陽性がまとめて落ちる。⚠**較正は機能修正の波と混ぜない**（同じ commit でやると「何効果直したか」が読めなくなる）＝単独作業として切る。作業＝`scripts/vocabCensus.ts` の当該 keys に `pickUpTo` を足し、119効果の実減少数を実測して `BASELINE_HIGH` を実数更新するだけ（parser/engine 変更なし・S） |
...
| 🆕(lix) | **pick を伴う「好きな枚数を一番下・残りを一番上」＝分割 remainder（4効果）**（2026-07-29・第9波で切り出し）。pick の無い6効果は第9波で `split_top_bottom` に是正済みだが、**ピックしてから振り分ける**形は remainder が対話になるため未対応＝`WXDi-P16-086-E1`（1枚まで手札orエナ＋分割）／`WX24-P3-031-E1`（＜宇宙＞2枚まで手札＋分割）／`WXK02-032-E1`（1枚手札＋分割）／`WXK03-048-E1`（＜遊具＞3枚まで場出し＋分割）。<br>**要る機構は2つ**：**(1)** `REVEAL_AND_PICK`／`LOOK_PICK_CHAIN` の `remainder.position` に `'split_top_bottom'` を許し、残りを直接動かす代わりに **LOOK_AND_REORDER pending を出して continuation へ繋ぐ**（`execLookPickChain` 末尾と `resumeSearch` の `revealRemainder` の2箇所）**(2)** `WXDi-P16-086-E1` だけは chain ステージ側の hand-or-energy も要る（`handOrEnergy` は現状 `REVEAL_AND_PICK` にしかない）。M。明細は BUGFIXES 2026-07-29「第9波」節 |
...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕単独）＝タスク12(xlvi) を第7波（宣言参照 pick 8効果・(c) 残0）→第8波（【トラップ】設置併記 2効果・(g) 残0）→第9波（振り分け `split_top_bottom` 6効果・(d) の主因）で連続消化**（golden 968→**984**・census 1452→**1453**〔`BASELINE_HIGH` 更新・+1 は計器上の移動1件のみ〕・smoke 10726 全0・fuzz 全0・lint 0 errors・held 254→**254**〔新規ドリフト0〕・build 冪等・gates 全緑）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **984**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1453**（＝`BASELINE_HIGH`・回帰ゲート）・held **254枚/109署名**（2026-07-29 実測）。
>
