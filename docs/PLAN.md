...
- **🆕 セッション（2026-07-07・続き41・Opus 4.8・§7 R38 ON_SIGNI_FROZEN resume経路取りこぼしを修正）**＝続き40（Sonnet）が発見・引き継いだ R38 実バグを修正・実機PASS。**FREEZE を付与する ON_PLAY 効果（SELECT_TARGET で単体対象を凍結する大半のケース）が resume 経路（`handleEffectInteraction`）で完結し `resolveStackNext` の中央 diff（`BattleScreen.tsx:3798`）を通らないため `ON_SIGNI_FROZEN` watcher が無発火**だった問題を、既存の resume-経路取りこぼし対策 inline collector 4つ（`collectDeckShuffleInline`／`collectBanishOppByEffectInline`／`collectLrigUnderMovedInline`／`collectKeywordGainedInline`）と**同型の `collectFreezeInline` を追加**して解消（`detectNewlyFrozen`→`collectFreezeTriggers`→once_per_turn の `actions_done` 反映→`pendingEntries` 合流）。**新規機構ではなく既存パターンの適用漏れの是正**＝engine/parser の共有ロジックは不変。**検証**＝`node scripts/verifyBattleDrive.mjs freezetrigger` が **PASS**（`freeze=true watcher=true`・`guest.signiFrozen=[true,false,false]`・「羅菌 プランクトンの【自】効果（凍結時）」→「小剣 ククリのパワー-1000」を実UIで確認）。golden **151/0**・smoke 全0（10582）・fuzz 全0・typecheck 緑。driver 側の pass 条件を「凍結ログ文字列（経路により未出力＝脆い）」から**凍結 ground-truth（`guest.signiFrozen`）＋watcher ログ**へ変更し、`freezetrigger` を既定 `order` に復帰。推定影響＝WX08-039/WXEX2-02/WXDi-P04-065 の watcher が実戦でも発火するように。詳細 BUGFIXES 最上部・§6.3/§7・VERIFY_BROWSER.md。**次の一手＝(A)Opus側の筆頭は依然 `GRANT_TO_PLACED_SIGNI` の実装（targetsLastProcessed 機構・§6.3＝続き36から持ち越し）＋census「動的比較35枚」＋引用内CHOOSE（WXDi-D09-P20）。(B)Sonnet側は §7 残る実機検証（R30/R31/R36/R39-R46・ON_LRIG_ATTACK_STEP_START全体未検証）から1件ずつシナリオ追加→観測記録、および R38②《ターン1回》/複数同時凍結の追検証。(C)verifyBattleDrive のバッチ実行時状態汚染の根本修正（driver側テスト分離強化）は続き39から持ち越し未着手＝freezetrigger を order 末尾付近に足したため batch では該当しうる（単体再実行で切り分け）。**
