...
- **ON_LRIG_GROW（ルリグがグロウしたとき）**：5枚。✅発火自体は実UI検証済み（`WXDi-P03-039`・CPUセンターグロウも`cpugrow`で確認）。**②相手のグロウでany_opp発火する経路＝✅続き73（Sonnet）でWXDi-P13-047を検証・PASS**（`verifyBattleDrive.mjs lrigGrowAnyOpp`・2回連続）。**同時に原文「あなたのターンの間」のturnOwnerゲートがJSON側に未実装という疑いを発見**（guest自身のターン中のグロウでも発火＝過剰発火の疑い）＝修正せずOpusタスク12へ登録（詳細VERIFY_BROWSER.md）。**残＝未検証**＝③any_opp トリガーがグロウ先ルリグの【出】より先に解決される・④《ターン1回》が2回目グロウで再発火しない・WXDi-P03-046（SELECT_TARGET要でresume経路リスクあり＝未着手）。⚠**アシストルリグのグロウ経路は未配線（センターグロウのみ）＝follow-up**。
