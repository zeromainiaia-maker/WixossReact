...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-08 続き378＝§5d-0(i) 配線ギャップ第19バッチ）**＝golden **1517**・smoke **10686** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **924**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **147枚／71群**・実機 driver 既定order **128件**。**被覆マトリクス miss 291**（`npm run census:wiring`）。**混在カードのレビュー待ち 3カード**（`docs/_partial_fresh.json`・すべて機構ギャップ）。
>
...
>
> 0. 🎯**次の一手＝🆕「`isDisona` の条件節グループ 11効果」（続き378 が実測だけして残した在庫＝§5d-0 (ii) 冒頭に全件の現況あり）。** うち **3件は activeCondition が丸ごと無く無条件で常在バフ**、**5件は条件節ごと落ちて無条件発火**＝**実害が確定していて母集団も確定している**ので最も安い。⚠**条件へ語彙を足す前に両 `matchesFilter`（`execUtils` / `effectEngine`）のパリティを確認する**（続き378 で `effectEngine` 側に `isDisona` が無く5効果が黙って全味方バフになっていた＝**JSON が正しくてもゲートに映らない**型）。次点＝(ii) の `parseStatus:UNKNOWN` 6件／`PARTIAL` 23件、または (i) の ★セル。⚠**★セルは `--cell` を開いて0件でないことを確かめてから投げる**（続き377m・378 で2回連続、上位セルの大半が条件節用法のクロス計上だった）。
>
...
- [ ] **🆕続き378 で実測だけした「`isDisona` の条件節グループ」11効果＝次バッチの最有力**（第19バッチのスコープ外に置いたもの。**全件 live JSON を実測済み**）。
  - **activeCondition が丸ごと無い＝無条件で常在効果が乗る 3件**＝`WXDi-P12-051-E1`（エナに《ディソナ》3枚以上あるかぎり→**無条件で +3000＋シャドウ**）／`WXDi-P12-085-E1`（トラッシュに3枚以上→**無条件 +5000**）／`WXDi-P13-083-E1`（このシグニの下に《ディソナ》→**無条件 +5000**）。
  - **`triggerCondition`／`CONDITIONAL` が丸ごと無い＝無条件発火 5件**＝`WXDi-P12-056-E1`（エナ＋トラッシュ合計7枚以上→無条件ドロー）／`WXDi-P13-078-E1`（場にパワー10000以上の《ディソナ》→無条件エナチャージ）／`WXDi-P13-069-E1`（場に他の《ディソナ》→無条件ドロー＋捨て）／`WXDi-P13-086-E1`（下に《ディソナ》→無条件4ミル）／`WXDi-P13-064-E1`（場に他の《ディソナ》＋相手エナ4枚以上→無条件）。
  - **別機構 3件**＝`WXDi-P13-005-E1`（「捨てたカードが《ディソナ》の場合、代わりに」が2連 BANISH へ平坦化＝(iii)）／`WXDi-P13-007-E3`（「捨てた《ディソナ》1枚につき」の数量比例）／`WXDi-P13-008-E3`（引用付与内の「《ディソナ》のスペルを使用したとき」）。
  - ⚠**着手時の必須確認**＝条件へ `isDisona` 等を足すときは**両 `matchesFilter`（`execUtils` / `effectEngine`）のパリティ**を先に見る。続き378 で `effectEngine` 側に `isDisona` が無く**5効果が黙って全味方バフ**になっていたのと同じ型（型にキーがあっても評価器が知らなければ無視される）。条件型は `ENERGY_COUNT_FILTER`／`HAS_CARD_IN_FIELD` の先例を流用できるか確認し、「このシグニの下に〈filter〉」は `THIS_CARD_HAS_UNDER` が**有無のみ**なので機構追加の要否を判定すること。
