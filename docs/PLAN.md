...
- 🏁**セッション（2026-08-29・続き713・Opus 5 単独）＝§5.3 `O-77` 第1バッチ。catch-all は「効かない」のではなく、払っていないコストを取り立てていた。**
  ユーザー指示＝**「O-77 を行う」**。
  📊**進捗3計器＝Sheet1 要対応 0 / 863（据置）｜台帳 残 OPEN 558（据置）｜census 高シグナル 520（据置）**
  gates 全緑（**golden 2968→2970**・smoke 10702 全0・fuzz 全0・lint 260 据置・同型★0・`census:enginetext` 141 据置）。
  全文は BUGFIXES.md 冒頭。
  - 🔴**登録票の「過剰実行は止めてある」は誤りだった**＝`SEQUENCE[STUB{LRIG_UNDER_CARD_OP}, CONDITIONAL]` は
    `execStubPart1` のハンドラに**到達せず**、`effectExecutor` の**コスト先取り**に食われて
    ①**原文が要求していないコスト**（効果元シグニの下のカード）を取り立て、払うと**ルリグトラッシュへ捨てる**
    ②`available: hasUnder` なので**下にカードが無いシグニでは発動できない**（大多数）。
    🔴**`SPDi43-26-E1` は payload を正しく持ったまま恒久 no-op**だった（効果元がルリグでシグニゾーンに居ない）。
  - ✅**受け皿は既にあり、しかも「同じ文の正解」が live に居た**＝`WX24-P2-075-E1` が**まったく同じ文**を
    `TRANSFER_TO_DECK{thisCardOnly, bottom, optional}` で正しく表していた。⇒ **新しいアクション型は0。**
    **採用4効果／明示保留3効果（honest な `DEFERRED_*`）**で、**コスト先取りに食われる効果 7→0**。
  - 🔴**保留するときは「そうした場合」ごと落とす**＝`CONDITIONAL{IS_MY_TURN}` は did-it ゲートの型ではなく
    **「そうした場合」の綴り**で、成立判定は「任意アクションのスキップ時に `stripDidItConditional` が走る」ことに
    依存している。**実装しないまま id だけ改名すると自分のターンに帰結が常時走る**（コスト先取りより悪い）。
  - 🔴**ついでに直した engine バグ**＝`execTransferToDeck` の SIGNI 経路が `thisCardOnly` のとき
    **`optional` を無視して即実行**しており、「置いて**もよい**」が**強制**になっていた
    （**先例 `WX24-P2-075-E1` も同じ壊れ方**＝手書きで正しく書いても engine 側で潰れていた）。
  - ✅**影武者コピーを1件解消**＝`WX24-P2-075-E1` は「parser が catch-all へ落とすから」という理由で
    手書きされていた。parser が出せるようになり**実体が一致**したので削除し、孤児 MANUAL スタンプを解凍した。
    ⚠**これは `O-42` トリップワイヤが検出してくれた**（golden が「影武者が増えた」と赤くなった＝期待どおりの動作）。
  - 📌**live A/B＝実体が変わった効果は8ちょうど**（採用4＋保留3＋`parseStatus` 解凍1）・**outlier 0**。
  - 📌**実機なし**＝§2.2 の表どおり（`src/screens/` 未接触・**新しいアクション型/条件型を1つも足していない**）。
    対話の funnel は golden が両方向で押さえた（`SELECT_TARGET` が立つ／0体選択で帰結が走らない／
    **旧コスト先取りのログが出ない**）。

**▶ 次の一手**＝🆕**`O-76`（§5.3「取る順」2位・11効果）**＝`LOOK_OPP_LIFE_TOP` が20文型の catch-all。
**進め方は今回と同じ**＝①**まず既存の受け皿型を探す**（今回は「同じ文の正解」が live に居た）
②**engine の消費地点を全部数える**（今回はハンドラに到達しない**第2の消費地点**が本命だった）
③受け皿が無いものだけ honest な `DEFERRED_*` へ改名し、**「そうした場合」のゲートごと落とす**。

**`O-77` を閉じ切るなら**＝**残8効果（C群）**。⚠**どれも別機構**（ライフ操作／【アクセ】化／デッキトップ操作／
シグニゾーン再配置／ライフとデッキの入れ替え／下に置く／チェックゾーン往復）なので**1バッチにまとめない**。
...
|---|---|---|---|---|---|
| **1** | **`O-76`** | **11効果** | catch-all の分解 | 中 | `LOOK_OPP_LIFE_TOP` が20文型の catch-all。**進め方は `O-77` 第1バッチと同じ**＝①まず既存の受け皿型を探す（`O-77` は「同じ文の正解」が live に居た）②**engine の消費地点を全部数える**（`O-77` の本命はハンドラに到達しない**第2の消費地点**だった）③受け皿が無いものだけ honest な `DEFERRED_*` へ改名し、🔴**「そうした場合」ごと落とす** |
| — | **`O-77` 残 C群** | **8効果** | catch-all の分解 | 中 | ⚠**どれも別機構なので1バッチにまとめない**（ライフ操作／【アクセ】化／デッキトップ操作／シグニゾーン再配置／ライフとデッキの入れ替え／下に置く／チェックゾーン往復）。🔴**コスト先取りの過剰実行は第1バッチで解消済み**なので、残りは「効かない」だけ＝急がない |
| 2 | `O-135` | 回帰が広い | インライン収集を族ごとに足す | 大 | `O-138` の前提でもある |
| — | **`O-144`** | **46効果** | 🔑**機構は実装済み・配線だけ**（`manualEffects.ts` 側の手当て） | 小 | `O-51` の並べ替え対話が MANUAL/PARTIAL で凍っている分に届いていない。**退化ではない**（従来どおりの挙動）。**`manualEffects.ts` を触る他バッチで自然に減る**ので、着手前に数え直す |
...
| **O-77** | 🆕**`LRIG_UNDER_CARD_OP` が「配置操作」以外の 22文型の catch-all になっている** | **残 8効果**（初回17ノード／第1バッチで採用4・保留3） | ■**現在地（2026-08-29 続き713・第1バッチ後）**＝**live 10ノード／真の穴 8効果**、🔴**コスト先取りに食われる効果 7→0**。全文は BUGFIXES.md 2026-08-29。 ■🔴**登録票の「過剰実行は止めてある」は誤りだった**＝`SEQUENCE[STUB, CONDITIONAL]` は `execStubPart1` のハンドラに**到達せず**、`effectExecutor` の**コスト先取り**が①原文が要求していないコスト（下のカード）を取り立てて**ルリグトラッシュへ捨て**②`available: hasUnder` で**下が無いシグニでは発動できない**状態にしていた。**この分岐は撤去済み（復活させない）。** ■✅**受け皿は既存**＝`TRANSFER_TO_DECK{position:'bottom', optional:true}`。**同じ文の正解が live に居た**（`WX24-P2-075-E1`）。**新型0。** ■**残 8効果＝どれも別機構なので1バッチにまとめない**＝`WD23-022-E-E3`（ライフ上を見てクラッシュ）／`WDK17-015-E1`（このシグニを【アクセ】化）／`WX24-P4-085-E1`（デッキトップをトラッシュ→レベル比例ミル）／`WXDi-P00-063-E2`（相手デッキトップを相手デッキ下へ）／`WXDi-P06-045-E1`（相手シグニを他のシグニゾーンへ配置）／`WXDi-P08-008-E2`（相手のライフ上とデッキ上を入れ替え）／`WXK08-084-E1`（デッキ上2枚から1枚をシグニの下に置く）／`WXK10-045-E2`（相手手札→チェックゾーン→ターン終了時に戻す）。 ■**進め方**＝第1バッチと同じ＝①**まず既存の受け皿型を探す** ②**engine の消費地点を全部数える**（本命は「ハンドラに到達しない第2の消費地点」だった） ③受け皿が無いものだけ honest な `DEFERRED_*` へ改名し、🔴**「そうした場合」（`CONDITIONAL{IS_MY_TURN}`）ごと落とす**（残すと自分のターンに帰結が常時走る）。 |
