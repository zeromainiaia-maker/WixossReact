...
- 🏁**セッション（2026-08-26・続き673・Opus 5）＝§5.3 `O-89`＝**スコープつきキーワードの直後のブロックが【常】へ飲み込まれる**分割穴。`WXDi-P10-040` は【自】が丸ごと `CONTINUOUS`/`PERMANENT` に抱き込まれ、**その【自】が一度も発火せず、【シャドウ】の付与も消滅**していた。**4カードを是正**。
  - 🔑**真因＝符号化済みの形を分割 regex が知らなかった**＝`【シャドウ（X）】` は分割より前に `encodeShadowScopesInText` が **`【シャドウ:{"underSigniLevelEq":true}】`** へ書き換える設計（`stripRuleParens` にスコープを消されないため）なのに、token 表が**原文形（（…））だけ**を見ていた。⇒ 句点が挿入されず後続マーカーが前ブロックへ流れ込む。**受け皿は engine に最初から在った**（`keywords.ts:447` が `underSigniLevelEq` を実評価している）。
  - 🔑**登録票の「同型が他にもあるはず」は現時点では該当なし**と確認＝素朴に測ると28件出るが、**`『【自】…』`（付与される能力）と「【自】能力」（言及）を除外すると1件**（`WXDi-P10-040`）。個数不足も0。⚠**偽陽性の主因は「【自】の言及」**＝トリガー宣言は必ず `【自】：`／`【自】《…》：`＝**コロンが続く**形。
  - 🔴**`_idset_fresh` で永久に凍る**の実例を2枚踏んだ＝`WXDi-P09-048` / `WX25-P2-118` は **live には古い正しい値**が残っていたので計器に映らず、**parser だけが同じ穴を持ったまま**放置されていた。今回 parser 出力が live と実体一致し `_idset_fresh` 45→43 で解けた（差分は出ない）。**「live が正しい＝parser も正しい」ではない。**
  - ✅**実機は新規1本 PASS ＋ 反転確認済み**＝`o89ShadowKeywordBlockSplit`。**観測点は `SELECT_TARGET` が立つこと**（【自】が AUTO として収集されて初めて対象選択に到達する）。旧形へ戻すと `cands=null`＝**一度も発火しない**側だったことが確定した。
  - ⚠**既存 golden「照応B」は id が E2→E3 へずれた**（2つ目の【常】が復活して効果が2→3）＝回帰ではないので id だけ追随させた。
  - ✅ gates 全緑（golden **2843→2844**／census **567→564**（−3・baseline 更新）／`census:enginetext` A群 141 据置／smoke・fuzz 全0）。

**▶ 次の一手**＝**`O-80` 第2バッチ**（残 36効果。次の塊は**ゾーンを数える系**＝トラッシュ5／場7／手札1／チェックゾーン1／シグニの下2。受け皿は `POWER_MODIFY_PER_FIELD`／`POWER_MODIFY_PER_LEVEL` 系が既存）。⚠**S サイズの単発（`O-89`／`O-90`／`O-91`）は在庫が切れた**＝§5.3 に残るのは `O-60`（M〜L）／`O-80`（残36）／`O-83`／`O-85`／`O-86`。実機の腐り返済なら §5.1 の `V-89`。
