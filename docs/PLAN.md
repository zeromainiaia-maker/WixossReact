...
- **🆕 セッション（2026-08-15・続き485・Opus 5）＝§6.4 **O-19 完了 ＋ 新設 O-25（自己引用付与）を同時消化＝5効果の挙動是正**（**恒久 no-op 1件＋過剰実行3件＋所有者反転1件**）。ゲート全緑（**golden 1990**＝+5・**census 831**＝−2・lint 0/259）。
  - **✅① O-19 の在庫は簿記より遥かに小さかった（実測1効果）**＝「watcher の scope が推論任せ」の残存は `WX25-P1-061-E1` **1件だけ**。ただし**直し方は2層**＝(a)`inferTriggerScope` が読むテキストを**カード全文→能力ブロック限定**（`abilityBlockTextOf`）に変え、続き463 の暫定 guard（「カードが【出】を持つなら推論しない」）を撤去 (b)parser の `allyPlayM` に由来句「**トラッシュから**」を許して `triggerScope:'any_ally'` ＋ `triggerCondition.placedFromTrash` を**parse 時に書く**。⚠**「エナゾーンから」は入れない**（engine に由来ゾーン語彙が無く、限定を落とすと過剰発火）。全カード A/B＝差分は狙った1件のみ。
  - **🔑② 本命は1枚ではなく「二度と溜まらない形」**＝golden に**データ不変条件**を新設した＝「ON_PLAY AUTO で**能力ブロックの主語が『この…』でない**のに `triggerScope` 未指定＝0件」。scope 未指定は engine で self に落ちる＝watcher なら**一度も原文どおりに発火しない**のに、これまで**どの計器にも映らなかった**。新カード追加でこのクラスが増えた瞬間に赤くなる。
  - **🔴③ 隣接で見つけた本丸＝O-25「引用能力の付与が即時実行へ平坦化」**＝原文「（ターン終了時まで、）このシグニは「【自】…」を得る」を parser が**どの付与規則でも拾えず**、引用の**中身をその場で実行**していた。`WX25-P1-061-E1`／`WX25-CP1-048-E1`＝「アタックしたとき−8000／−15000」が**付与した瞬間に**発動（過剰実行）、`WXDi-P09-038-E1`＝付与節が丸ごと落ちて **SEQUENCE[コスト支払いだけ]の恒久 no-op**。残りは `STUB{GRANT_ABILITY_INNER_TEXT}` へ落ち、engine が**実行時にカード原文を regex で読み直して**付与を再現していた（**O-20 で潰した層の生き残り**）。既存の兄弟規則（「それは「Q」を得る」→`GRANT_EFFECT`）と対になる自己付与規則を1本足して構造化（**engine の新機構ゼロ**＝`GRANT_EFFECT{target.filter.thisCardOnly}` と `granted_effects` は既にあった）。9カードを採用。
  - **⚠④ 置き場所を1回間違えた（記録）**＝最初 `parseSentencePart1` に足したが**発火しなかった**。`parseActionText` が先頭の「ターン終了時まで、」を**除去してから**各 part を呼ぶので、**期間つきの規則は除去より前**（`effectParser.ts` の count:'ALL' 版の隣）に置く必要がある。engine 側は `execGrantKeyword` にある「thisCardOnly は選択UIを出さない」分岐が `execGrantEffect` に**無かった**ので追加（無いと「自分自身を選べ」という無意味なモーダルで解決が止まる＝golden で負方向も確認）。
  - **🔴⑤ おまけで別の live バグが1件落ちた**＝`applyLeadingOpponentDesignation`（「そうした場合、それを〜」の照応補正）が**引用能力の中の「対戦相手のシグニ１体を対象とし」を外側の文の照応と誤読**していた。`WXDi-P07-063` は自分に付けるはずの能力が**相手シグニへの付与**に化け、`WX24-P2-007-E1` は「**あなたの**すべてのシグニは…を得る」が **owner:'opponent'**（＝相手の全シグニに付いて自分には付かない）で live に入っていた。走査テキストで引用を伏せ字にするガード1行で両方是正（全カード A/B＝差分2件・どちらも是正方向）。
  - **📋⑥ 残り（O-25 の未消化）**＝`WXDi-P13-065-E2`（先頭が「このシグニの下に…がある場合、」で**条件節が未パース**のため期間アンカーに掛からない）／`WXDi-P15-055-E3`（**MANUAL 不可侵**＝手で更新するか AUTO 採用へ切り替える判断が要る＝O-6 と同型）／`SPDi43-05-E2`（引用の中身が **PARTIAL**＝採用ゲートで保留）。**【常】引用の durational 付与**（原文36箇所）は今回**対象外**＝現行は期間つき即時適用で近似できており、`GRANT_FIELD_SIGNI_ABILITY`（CONTINUOUS 専用）との境界整理が先に要る。
  - **▶ 次の一手**＝**O-3** の残26（`npm run census:stubs` の A群が常時の在庫表・機構ごとに独立着手可）→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残**（上記⑥）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き485（§6.4 O-19 完了＋新設 O-25 消化）後 最新値（本行が直近の正）**：census **831**（`BASELINE_HIGH` 実数更新＝**−2**。引用能力を `GRANT_EFFECT` へ構造化した結果、引用の中身が外側効果の語彙として数えられなくなった分）、**golden 1990**（+5＝データ不変条件1＋トリップワイヤ4）、smoke **10688 / SKIP 0**、fuzz 全0、held **116枚 / 48群**（うち採用済み9枚はレビュー対象外＝**実質 107枚**。純増は `SPDi43-05` の1枚＝PARTIAL 保留）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置）。🆕**live JSON changed 9カード**（`WX24-P2-007`／`WX25-P1-056`／`WX25-P1-061`／`WX25-P2-030`／`WX25-P3-056`／`WX25-P3-059`／`WX25-CP1-048`／`WXK10-079`／`WXDi-P07-063`。CSV 非改変）。🆕**挙動是正 5効果**（恒久 no-op 1＋過剰実行3＋所有者反転1）。
...
| **O-3** | **「次のターンの間」系統の残り** | 22効果（受け皿10種） | 🏁続き448/450（採用14）／🏁続き453＝`REMOVE_ABILITIES.until` の死フィールド解消（7効果）／🏁続き459＝`LRIG_GROW_RESTRICT` ゴミ箱の解体（受け皿8つを固有 `DEFERRED_*` へ分割＝隠れていた26効果を A群に可視化）／🏁**続き486＝最大クラスタ `DEFERRED_UNPARSED_THIS_TURN_OPP_CLAUSE`（7効果）を解体**＝新機構 `SIGNI_ATTACK_BAN`（アタック制限を**アタッカー側 state** に載せ `signiAttackGate` 1本で判定）で3枚、既存 `PREVENT_DAMAGE` で1枚、盤面リセット STUB で1枚＝**5効果の恒久 no-op を解消**し、残り2形＋1形を固有 id 3つへ分割。**在庫は `npm run census:stubs` の A群で常時見える**＝内訳は `DEFERRED_UNPARSED_NEXT_OPP_TURN_CLAUSE` 5／`_THIS_AND_NEXT_TURN_CLAUSE` 4／`EXTRA_ATTACK_PHASE` 2／`NEXT_OPP_ATTACK_PHASE_START` 2／`SELF_RESTRICT_THIS_TURN` 2／`UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`SKIP_NEXT_TURN` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／🆕`ATTACK_TAX_HAND_DISCARD` 1（`SP38-003`＝アタックごとの手札捨てUI）／🆕`OPP_DECLARED_ARTS_NAME_LOCK` 1（`WXEX2-09`＝**相手による**カード名宣言UI）／🆕`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1（`WXDi-P08-030`＝相手の複数選択UI＋強制アタック集合＝O-8(a) と同じ層）。**機構ごとに独立して着手できる**。⚠**期間だけ直すと「誤った対象の効果」が長持ちする**＝対象軸が壊れている札は据置（`WX11-038-E2`＝「次のターンの**メインフェイズ**の間」はフェイズ限定の受け皿が無く据置）。🔑**続き486 の教訓＝STUB を実装で置き換えると census のバケツが変わる**（`vocabCensus` は STUB を含む効果を「STUB/MANUAL格納」へ逃がしている）＝**その効果の他の未表現語彙が同時に高シグナルへ昇格する**ので、消化のたびに「計器の較正漏れ」か「本物の穴」かを1件ずつ仕分ける |
