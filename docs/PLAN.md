...
| # | タスク | 種別 | 規模 | 残っている内容 |
|---|---|---|---|---|
| **1** | 引用付与の内側 ability parse（引用付与残107の本丸） | parser語彙＋engine機構 | M | **✅続き164（Fable 5）で本丸を消化**＝2文型「<対象>を対象とし、<任意コスト>てもよい。そうした場合、それは「【自】…」を得る」（家族18効果）＋内側「対戦相手が《…》を支払わないかぎり」ゲート（33文）を parser 語彙化・14効果採用・実機 `wx24p2018GrantFire` **完全経路2回連続PASS**（既定order追加）・census 2088→2084。engine 変更ゼロ（既存 GRANT_EFFECT/OPPONENT_PAY_OPTIONAL 経路で完結）。**残**＝(a)内側が AUTO 展開不能な「【常】：アタックできない。」家族6効果（粗い即時 BLOCK_ACTION 近似のまま＝granted CONTINUOUS の BLOCK_ACTION 収集機構が要る）(b)内側「代わりに」置換（WX25-P3-038＝タスク6と合流）(c)`GRANT_LRIG_ABILITY` の ON_PLAY 誤デフォルト（タスク5と重複）(d)WX25-P3-085 単文型 grant mis-parse（12(xxiv)残）。第1弾は✅続き75・詳細 BUGFIXES 続き164 |
...
| 🆕(xxx) | **WXEX2-76-E1＝ON_PLAY の scope/対象幻覚（続き179発見・未修正）**。原文「対戦相手のシグニ１体が場に出たとき、対戦相手は自分のデッキの一番上のカードを**そのシグニ**の【チャーム】にする」が ON_PLAY **self**（ナナフシ自身が場に出たとき）＋任意対象 ATTACH_CHARM に化けている。timing センサスの対象外（「場に出たとき」除外規則の死角）＝同型の「対戦相手のシグニが場に出たとき」any_opp＋トリガー元対象の parser 抽出が要る（engine の collectFieldTriggers any_opp path は配線済み・ATTACH_CHARM の to をトリガー元に解決する語彙が論点） |
...
- **🆕 セッション（2026-07-17・続き180・Fable 5→**Opus へ引き継ぎ（作業中）**・タスク16[C]＝台帳下部5パターンの一括機構化＝コード実装完了・採用/簿記が未了）**
  - **状態＝engine／parser／decompiler の実装はすべて完了し、typecheck 緑・golden 396 全PASS・smoke 全0 を確認済み。未了＝JSON採用（33カード）・新規 golden・fuzz/census/regen ゲート・census:timing 再計測・BUGFIXES 続き180 の記載。** auto-commit で HEAD には実装済みコードが載っている。
  - **実装済み内容（5機構）**：
    - **①ON_SIGNI_DOWN／ON_SIGNI_BECOMES_UP 新設**＝`boardDiff.detectNewlyDowned/detectNewlyUpped`（同一在中のみ・lrig up 含む）＋`triggerCollect.collectSigniDownUpTriggers`（freeze と同型。scope 既定 any_ally／triggerFilter story・cardName・excludeSelf／byEffect＝「効果によって」（WX05-040 の公式注釈どおりアタック/コスト除外）／duringAttackPhase／upIncludesLrig／watcher は場＋LRIG＋**キー**（WXK11-015 対応・key_piece/key_piece_extra 走査））。配線3点＝collectBoardDiffTriggers（`collectSigniDownUpInline`・byEffect:true）／performSigniAttack（アタックダウン・byEffect:false・`newOpStateAtkDown` 経由で保存）／checkAndApplyContMutations（常時効果 DOWN/FREEZE 後に検出）。
    - **②自己discard 反応**＝ON_TRASH self＋fromZones:['hand'] へのマッピング（engine は既配線の collectAnyZoneTrashSelfTriggers に **byOwnEffect／trashSourceStory（causeSourceCardNum を中央diff から新規引き渡し）／turnOwner** の3軸を追加）。
    - **③ON_LEAVE_FIELD 跨サイド**＝collectLeaveFieldTriggers を **{entries,usedHostIds,usedGuestIds} 返却へ改修**（既存 any_ally の usageLimit 未処理＝続き104 同型の潜在過剰発火も同時是正・呼び出し3箇所更新・golden 既存3件を .entries に追従済み）＋any_opp 走査（byOwnEffect＝causeOwnerId が watcher 側のときのみ）＋any_ally の byOpponentEffect。
    - **④「効果1つによって…N枚以上」は誤解で、ターン跨ぎ累積ではなく単一解決内の閾値＝既存 per-resolution 軸で正確**（重要な知見）。mill＝regex拡張（合計/によって形・owner 無指定→any）／ON_DRAW＝「N枚以上」＋self 経路に turnOwner・duringAttackPhase・drawByDrawerOwnEffect の3ゲート追加／ON_HAND_DISCARDED＝「N枚以上」regex 拡張。
    - **⑤ゾーンアイコン**＝placedOnTrapZone（signi_traps は実装済みだった＝台帳の[C]判定は誤り）／placedOnGateZone（own_gate_zones。WXK迷宮のゲート設置が未配線の間は正直な no-op＝旧 ON_PLAY self 幻覚より改善）を collectFieldTriggers any_opp path に追加。
  - **影響検証済み**＝全カード fresh diff（baseline＝続き179 push 8e954c64 時点）で**変化33カードのみ・全て意図どおり**：①10枚（WX05-040/WX12-006/WX14-CB01/WX20-051/WXEX1-42/WXEX2-01/WXK11-015/SPDi43-17,18,19）②8枚（WXDi-P10-039/P10-070/P11-066/P08-075/P16-072/P11-069/P14-086/WX24-P1-045）③3枚＋**同族の副産物2枚**（WXK11-049/WXDi-CP01-027/WX19-026＋WX14-027/WXEX2-31＝any_ally+byOpponentEffect+filter が正しく付いた）④7枚＋副産物1枚（WXDi-P08-079/WXDi-CP02-010/WXEX1-49/WXK10-025/WXK10-040/WX11-030/WXDi-CP02-077＋WX24-P3-087＝mill発生源「シグニの効果」限定は落とす既知近似(xxiv)）⑤2枚（WX21-025/WXK10-044）。action は全効果 UNCHANGED（timing/scope/cond/filter のみ変化）。
  - **⏭Opus が続けて行う残作業（順番どおり）**：
    1. `npm run gates`（fuzz/census/lint 含む）で全緑確認（typecheck/golden/smoke は確認済み）。
    2. **JSON 採用（33カード）**＝working dir に `tmp_dumpFresh.ts`（gitignore済・全カード fresh dump スクリプト）と `tmp_showTexts.mjs` が残存。手順は続き179 と同じ＝curated が AUTO なら fresh 置換（durable）・MANUAL は timing/cond のみ手パッチ or 据置を各カード原文照合で判断。⚠WXK11-015-E3 の action（FREEZE の対象がトリガー元か）・WXEX1-42-E1（そのシグニのパワー以下）等、**そのシグニ＝triggeringCardNum 参照の action は個別に targetsTriggerSource/isTriggerSource の要否を確認**（続き179 の WX17-075-E1 と同型の対象幻覚がありうる）。
    3. 新規 golden＝(a)parser 判定（down/up の scope・byEffect・named・duringAttackPhase・upIncludesLrig／自己discard の3軸／跨サイド離脱／mill合計/draw以上）(b)`detectNewlyDowned/Upped` の同一在中判定 (c)`collectSigniDownUpTriggers`（byEffect ゲート・キー watcher WXK11-015・filter）＝JSON採用後に実カードで。
    4. `npm run regen`＋同型★0・`npm run census:timing` 再計測（91/77 から⑤②①③④分の減を確認）・census 減少なら BASELINE_HIGH 実数更新。
    5. BUGFIXES 続き180 記載＋本エントリを完了形に書き換え＋PLAN_PROGRESS 退避＋§3 タスク16 行更新。**既知近似を必ず記録**＝(a)コスト捨て・ガード捨ては中央diff を通らず②は効果起因のみ検出 (b)アップフェイズ一斉アップ・アタック無効化経路のダウンは非検出 (c)mill の発生源シグニ限定は落とす (d)⑤ゲートは設置未配線で no-op (e)**③の usageLimit 是正で既存 any_ally ON_LEAVE_FIELD の《ターン1回》持ちの挙動が変わる**（過剰発火→正・該当実カードの機械列挙を推奨）。
  - **見送り（台帳へ残す）**＝手札増加 timing（WX25-P2-063/SPDi43-11-sub＝ON_HAND_ADDED 新設要）・WXDi-D09-P16（discard の own-effect 原因判定が discarder 経路に無い）・WX24-P2-051（《ガードアイコン》を持たない filter 軸なし）・WXDi-P13-051（OR複合）。
  - **次の一手（このバッチ完了後）**：Opus＝タスク12在庫（(i)(ii)(iii)(v)(vi-4)(viii)残(xi)(xii)(xxiv)残(xxx)）。Sonnet＝新 timing の §7 driver（続き178/179/180分）。
### 📊 恒久指標（維持中・逐次更新）
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 2019【効果単位】**（2026-07-17 続き179 更新＝timing[B]第2弾の10効果是正で 2027→2019・要因特定済みのため実数更新。2026-07-13 続き109 で判定粒度を「カード単位」→「効果単位（effectId）」へ切替。旧カード単位の 1447 とは**計測仕様が違うので比較不能**）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。**前提＝`docs/_effect_srctext.json`（`npm run build:effects` の副産物）が最新であること**（無ければ census は exit 1）。明細 `docs/_vocab_census.txt`・消化の入口は `npm run census:clusters`（§5c）。**切替の根拠と計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109。**
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 396・smoke/fuzz 全0・同型★0・census 2019。
>
