...
- **🆕 セッション（2026-08-10・続き418〜420・Opus 5）＝「所有者反転／コスト脱落」を計器で追って3波。census 860→854・golden 1753→1764。33効果/32カード是正（うち🔴自傷7件）**。
  - **🔴① 続き418＝「代わりにそれを＜動詞＞」の per-target 形を parser が意図的に除外していた**。除外された昇格文は**別ステップとして残り**、主語のない「それを〜する」が `owner:'self'` へ反転＝**自分のシグニをバニッシュ/付与**（`WX08-028-BURST`／`WX09-Re01-E2`／`WXDi-CP01-026-E1`）。PLAN が「UNKNOWN 2件」と書いていた家族の**本体は無言の過剰効果**だった。(A) 盤面状態条件は `CONDITIONAL{then,else}`、(B) 対象プロパティ条件は `SELECT_TARGET_ONLY→STORE→CONDITIONAL{…targetsStored}`。
  - **🔴② (B) を (A) の形で書くと恒久 no-op**＝`LAST_PROCESSED_MATCHES` が**対象選択より前**に評価され常に偽。**実装途中で実際に踏み、golden の「構造」ではなく「実行結果」テストで捕まえた**。同じ理由で **POWER_MODIFY の値昇格は加算モデル**（base＋差分）。
  - **🔴③ 続き419＝②の「所有者反転」を計器化したら別家族が出た**＝「どちらか１つを選ぶ。①…②〈任意コスト〉てもよい。そうした場合、…」で**選択肢が2文構成だと①②行が番号フィルタで落ち、帰結文だけが単文パスへ流れて CHOOSE が丸ごと消える**。`WXDi-P07-055/072-BURST` は**自分のシグニを手札に戻す/デッキ送り**、`WXDi-P07-094`／`WXDi-D09-P17` は**コストが消えて無条件に強い方**。
  - **🔴④ 続き420＝`TARGET_AND_DISCARD_HAND` は payload を持たず、engine が「手札の最後の1枚」を問答無用で捨てていた**（`effectExecutor` Pattern⑥）。53カードで**捨て札filter脱落23／任意性脱落17／選択不能は全件**。続き416 の正準形 `OPTIONAL_COST{handDiscard:{count,filter}}` へ 18効果を移送。併せて「対戦相手の**ルリグ**１体を対象とし」の宣言取りこぼし（本体が `SIGNI{owner:'self'}`＝**自分のシグニをダウン**）を2件是正。
  - **⭐⑤ 3波に共通する教訓＝「engine にあるのに parser が生成しない」型が続いた**＝`OPTIONAL_TRASH_SELF`（続き417）／`OPTIONAL_COST.coinCost`（続き419・engine は最初から実装済みで**唯一正しかったカードは MANUAL 手書き**）／`handDiscard`（続き420）。**「payload 無しの STUB」を見たら engine 側の受け皿を先に grep する**。
  - **⚠⑥ 変換系は必ず全件の対応表を印字して原文と突き合わせる**＝続き420 で spec→filter の「取りこぼし検査」を空 filter の早期 return より**後ろ**に書いており、`《幻水　コノハケロ》` の**カード名指定が丸ごと落ちて「手札の任意の1枚」に化けて**いた。表を出したから気づけた。
  - **⚠⑦ 計器の限界（次に使う人向け）**＝「原文に『あなたの』が無いのに JSON が self」は**引用付与の内側**と**自己コスト**を誤検出（27件中 真陽性4件）。「①②があるのに CHOOSE が無い」は**「以下の２つを行う」**（両方実行）を誤検出＝**ヘッダを併せて見る**。
  - **⚠⑧ 採用経路の落とし穴**＝(a)`PARTIAL` は `PRESERVE_STATUSES` に守られ parser を直しても live に届かない＝**`AUTO` へ戻してから build→adopt**。(b)MANUAL 同居の混在カードは held ではなく **`docs/_partial_fresh.json`** に落ち、**adopt スクリプトが無い**（当該 AUTO 効果だけ直接移送）。
  - **⚠⑨ 既存の限界（回避・記録のみ）**＝`LAST_PROCESSED_MATCHES` のゾーン状態照合は **CardNum で場を引く**ため、**両プレイヤーの場に同じカード番号があると取り違える**。instanceId 化までの既知の穴。
  - **✅⑩ ゲート**＝`npm run gates` 全緑（golden **1764（+11）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **854**（`BASELINE_HIGH` 更新済み）/ 同型★ **0**（265群）/ lint 0 errors）。`build:effects`＋`regen` 実施済み。**実機UI検証あり**＝続き420 で**手札コストの pay/skip モーダルが新たに出るカードが18件**（§7 送り）。
  - **📋⑪ 次の一手**＝**Opus 側**＝(a)**`OptionalCostSpec` に上限つき可変枚数**（「好きな枚数捨てる」＝続き420 で据置した最大クラスタ・`WX26-CP1-051`／`WX24-P3-052`／`WXDi-P05-078` 等）(b)🔴**`WX25-P3-014-E1`**＝`BOUNCE{owner:'self'}` の自傷（base が任意コスト STUB を含む多段構成で SELECT/STORE 組み替えが did-it ゲートと干渉）(c)**§6.4 A群の着手可能2件**（`ASSIST_LRIG_ATTACK_THIS_TURN`／`UNDER_CARD_AS_ENERGY_COST`）(d)**`WX12-010`＝強制アタック機構**(e)**UNKNOWN 残36ノード**。**Sonnet 側**＝§7 実機検証（**続き416〜417 の pay/skip＋続き420 の手札コストUI を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418〜420（「所有者反転／コスト脱落」を計器で3波）後 最新値（本行が直近の正）**：census **854**（860→854・`BASELINE_HIGH` 更新済み）、golden **1764（+11）**、`MANDATORY_SUSPICIOUS` **2**（据置）、held **128カード／48群**（うち採用済み18枚は除外表示）、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
