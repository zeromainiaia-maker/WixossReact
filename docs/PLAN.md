...
- **🆕 セッション（2026-07-09・続き54・Opus 4.8・§4タスク2 動的比較「対戦相手のセンタールリグより低いレベルを持つ、あなたの＜X＞のシグニ」mis-parse 是正＝`levelLtOppLrig` 新設・WX19-042 1枚）**＝WX19-042「【出】：対戦相手のセンタールリグより低いレベルを持つ、あなたの＜空獣＞か＜地獣＞のシグニ１体を対象とし、…それは【ランサー】を得る」が、**先頭修飾句に釣られ対象が LRIG に誤パース＝ランサーが自ルリグに誤付与**（本来の self シグニ対象＋動的レベルフィルタが丸ごと脱落）していた mis-parse を是正。**型** `TargetFilter.levelLtOppLrig`（既存 levelLtSelf/Trigger 同族）。**engine** `resolveDynamicFilter` に解決ブロック＝`otherSt.field.lrig` 頂点（相手中央ルリグ）のレベル-1 を `level.max` へ（参照不能ならフォールバック）。**加えて `execGrantKeyword` の SIGNI 候補算出を `resolveDynamicFilter`→`fieldCandidates` 経路へ変更**＝付与も除去系と同じ resolve 経路に載せた（従来 GRANT_KEYWORD は resolve 系動的フィルタが未解決だった）。**parser** `parseSingleSentenceInner` 冒頭で先頭修飾句を剥がしてから残りを解析→最初の self シグニ対象へ levelLtOppLrig を刻む（剥がさないと LRIG 誤選択）。**decompiler** filterJa 描画追加。**採用1枚**（heldReview 署名 `+SIGNI -LRIG ［1枚］`・collateral 0）。**検証**＝typecheck・**golden 174→175（+1＝相手ルリグLv3→自Lv2に付与/自Lv4は非付与）**・smoke 全0（10582・execGrantKeyword resolve 経路変更でクラッシュなし）・fuzz 全0・lint 0 errors・**同型★0**・**census 1574/1574**（mis-parse 是正は census 計上外・BASELINE 変更なし）。**次の一手＝(A)Opus＝effect-restriction（配置数制限）**（「対戦相手はシグニをN体までしか場に出せない」＝WXK11-074 等の相手シグニ配置数制限＋超過分トラッシュ・自己配置制限〔WX14-033/WX18-075〕・アタック/ガード不可＝§6 BLOCK 機構。**⚠BattleScreen 配置パスの強制＝smoke/golden/fuzz では検証不可・実機検証必須〔§12〕**）。**(B)Opus＝§4タスク2 動的比較 lrig相対の残**（WXEX2-25-E3「このルリグより低いレベルを持つあなたのシグニのパワーを－N」＝付与能力ホスト基準・WXK07-025/WXK08-005＝自/相手センタールリグのレベル比較 condition〔`LRIG_LEVEL_EQ_OPP` 同族の lt/lte 新設〕）。**(C)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋GRANT_QUOTED_AUTO_ABILITY 内側parse（WX24-P1-017/WX25-P3-038）。(D)Opus＝§5c census 条件節の残**（色付きアーツ/スペル使用歴〔ARTS_USED 拡張〕・「それがレゾナの場合、代わりに」WX25-P2-068/070・「あり」複合条件 WXDi-P11-048）。**(E)Sonnet＝§5c 再収穫・§7 実機検証（続き43-54 の採用効果）。(F)持ち越し：SEQUENCE 下流「そうした場合」IS_MY_TURN 常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDrive バッチ状態汚染。**
