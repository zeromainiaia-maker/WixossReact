...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-17` 第2段**（アタックの `applyCpuMoveSim`＝バトル解決・ガード・ライフバーストの近似 → 探索を配線 → 本物のデッキで A/B）。次に `S-22`・`S-24`・`S-14`・`S-6`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 5**（`S-17`・`S-22`・`S-24`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）②**自己対戦がマリガンを一度も踏まない**（`S-24`）。🏁`S-19`（先攻不利）は**再計測で否定**＝山の性質だった |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4347**＝この一連で +2）｜**実機 PASS**＝`verifyFullMatch.mjs cpu`（S-23 の回 6ターン/133手・S-17 の回 8ターン/191手）｜反転確認＝`planFor('guest')` を旧実装に戻すと golden `§5.7 S-23` が FAIL／列挙から `canSigniAttack` を外すと `§5.7 S-17 第1段` が FAIL |
...
| 🔥`S-17` | **後半ブロックの探索**＝`ATTACK_ARTS_OP`（相手の妨害）が終わった**実際の盤面**から `ATTACK_SIGNI`／`ATTACK_LRIG` の手順を探索する。**ここで初めて相手の返しを見る**（ガード・ライフバースト・防御アーツ＝確率／最悪ケース）。🏁**第1段（2026-09-20）＝アタックを候補列挙に載せた**（`CpuMove` の `signiAttack`／`lrigAttack`・本番の選択も同じ列挙から・**挙動不変を3シードの全ログ一致で確認**）＝`S-15` の「打った手は列挙に出ている」の全数照合がアタックにも掛かるようになった。🔥**次＝第2段**＝`applyCpuMoveSim` にアタックを足す（**バトル解決・ガード・ライフバーストの近似**）→ 探索を配線（既定オフ）→ 本物のデッキで A/B（`S-23`）。 | 大 | 攻撃順・アタック誘発・バニッシュ誘発・被アタック誘発をまとめて判断する。⤵**`S-8` はここに吸収**（`S-8` 単独は上位互換に食われる）。📏**第2段の設計値（第1段で実測）**＝アタックの候補は**中央1・最大3**＝**手順の全探索は 3!=6 通りで足りる**｜`ON_ATTACK_SIGNI` **684効果/667枚** に対し**被アタック側は5効果**＝**既定は「撃つ」のまま例外だけ引く** |
...
- **2026-09-20 時点**（第421〜422バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ `S-17` 第1段）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 5**（`S-17`・`S-22`・`S-24`・`S-14`・`S-6`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4347**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 2本**（6ターン/133手・8ターン/191手）
