...
- **🆕 セッション（2026-08-04・続き342・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(xcv) 残0クローズ**＝「能力を持たない」を読む engine 3箇所を是正し判定を1本に統一（ユーザー指示「(xcv)を行う」）。golden **1350→1354**・**live JSON 変更なし**・census 1288 据置・同型★0・全ゲート緑。
  - **🔴 登録時に自分で書いた修正方針が誤りだった**＝「`trim()==='-'` 判定に寄せる」と書いてあったが、実際は**3箇所とも別々のものを見ていた**。⚠**`-` だけ直していたら新しいバグを作っていた**＝`ABILITY_CHECK_ELSE_TRASH` は「それ」を効果元と取り違えていたので、素のシグニがアタックすると**自分のアタッカーを自分のトラッシュへ落とす**。旧実装は `'-'` を truthy と読むおかげで**たまたま常に no-op** だっただけ。
  - **母集団は2箇所5カード（登録どおり）だが、着手して初めて3箇所目が見つかった**＝`LAST_PROCESSED_HAS_NO_ABILITIES`（live 2枚）が**「JSON の effects が0件なら能力なし」**で判定しており、**マルチエナ持ちシグニ52枚を「能力を持たない」と誤判定**していた（マルチエナは常時能力）。
  - **統一した判定＝`execUtils.hasNoAbility`（非対称2段）**＝①`abilities_removed` なら能力なし ②**effects が1件以上あれば能力あり**（0件は根拠にしない＝実測159枚 vs 211枚の差52枚がマルチエナ）③それ以外は原文が空か `-`（素のシグニ実測159枚）。
  - **実装**＝`ABILITY_CHECK_ELSE_TRASH` は「それ」＝**直前 BOUNCE が戻したカード**に読み替えて持ち主の手札→トラッシュへ／`NO_ABILITY_SIGNI_TO_DECK_BOTTOM` は STUB を**宣言だけ**に戻し、実体を場離れ置換チェーンの新関数へ（**被害側の対面が宣言する妨害**なので victim の反対側を引く・チェーン最後・アタックフェイズ限定・**離場11経路すべてに配線**）。
  - **golden +4**＝置換の6経路と不成立条件／「それ」の指す先（**効果元シグニが場に残ること**＝旧バグの再発検知）／live 母集団の内訳固定（STUB 3枚・条件形2枚・宣言1枚）／**マルチエナ持ちは能力ありに倒すこと**。⚠**3箇所とも修正前は常に no-op**＝退化ではなく「初めて動く」ので期待値は原文から立てた。
  - **新規在庫1件**＝**(xcvii)** 離場11経路のうち `execSendToEnergy` だけ lrig 置換を呼んでいない（意図的に触らず登録）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／**🆕(xcvii)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜342 の全件が実機UI未検証**。本件の実機確認は**①素のシグニ（例 `WD01-010`）を相手の場に置き、`WXEX2-30` を自分の場に出してアタックフェイズに効果でバニッシュ→相手のデッキ下に入ること**②`WX25-P3-073` の付与能力でアタックしたとき、能力なしの相手シグニは手札ではなくトラッシュへ行き、**自分のアタッカーが消えないこと**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-04 続き342（🏁§3 タスク12(xcv) 残0クローズ＝「能力を持たない」判定の統一）後 最新値（本行が直近の正）**：golden **1350→1354**（+4＝①`WXEX2-30` の場離れ置換〔バニッシュ/手札戻し/トラッシュ/エナ送り/除外/デッキ戻しの6経路・能力持ちは素通り・メインフェイズでは不成立・宣言者不在・victim が自分側でも成立・`abilities_removed`〕②`ABILITY_CHECK_ELSE_TRASH` の「それ」＝直前 BOUNCE が戻したカード／**効果元シグニが場に残ること**＝旧バグの再発検知／対象なしで盤面不変 ③live 母集団の内訳固定〔STUB 3枚・条件形2枚・宣言1枚〕④**マルチエナ持ちを「能力なし」に倒さないこと**を条件側・置換側の両方で）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**（ローカル248は未追跡の残置 `scripts/_dbgFresh.ts` の3件ぶん）、manual field loss 0、同型★0（265群）・held **258枚／109群 据置**（**parser も JSON も触っていない**）。**live JSON は完全不変**（`public/data/` に差分なし・**新語彙0本**＝engine の action/condition 型は増えていない）＝触った層は `execUtils`（`hasNoAbility` 新設＋`LAST_PROCESSED_HAS_NO_ABILITIES` の委譲）・`execStubPart2`（STUB 2件）・`effectExecutor`（置換1本＋離場11経路への配線）。⚠**修正前は3箇所とも常に no-op**＝計器には一切映っていなかった（smoke も census も緑のまま）＝**実機確認と対で締める**。⚠`docs/STUBS.md`／`grouped_all.txt` の差分には 2026-08-01 以降の未再生成ぶん（続き333〜341 由来）が含まれる。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-04 続き342＝🏁§3 タスク12(xcv) 残0クローズ後）**＝golden **1354**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
