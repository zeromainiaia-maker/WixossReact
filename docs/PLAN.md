...
- **🆕 セッション（2026-08-13・続き456・Opus 5）＝§6.4 **O-16** 第3波＝**アタック禁止のゾーン継続**を機構化。3効果を是正し、**ゾーン継続の3型が出そろった**。golden 1944→1946・census 832 据置・held 106枚/47群 据置**。
  - **🔴① 直した3効果**＝`WXDi-P11-046-E2`／`WXDi-P11-055-E1` は `BLOCK_ACTION{count:1}`＝**指定ゾーンではなく相手シグニ1体を選ぶ**別物。`WXDi-D09-P13-E1` に至っては `STUB{LRIG_GROW_RESTRICT}`＝**そもそも別のアクション**（グロウ制限）に化けており、アタック禁止が丸ごと無かった。
  - **⭐② `FieldGrant` に `kind:'blockAction'`**（`power`＝続き454／`abilityLoss`＝続き455 に続く3つ目）。per-signi 付与では**その時点でそこにいたシグニ**にしか効かず、ゾーンを封じる原文の意図（後から出たシグニも撃てない）が表せない。
  - **⭐③ 消費地点は既存の唯一の funnel だった**＝`calcContinuousBlockedActions` の `cannotAttackSigni`。ここは `signiAttackGate.ts` 経由で**人間UI／共通実行経路 `performSigniAttack`／CPU の3箇所**が通る唯一の場所なので、**1行足すだけで3経路すべてに届く**。⚠**golden も `signiAttackBlockReason` 越しに検証する**＝engine 内部の集合を直接見ると「3経路に届いているか」を測れない。
  - **⚠④ parser の後処理を構造で保証した**＝`WXDi-P11-055-E1` は指定ステップが `CONDITIONAL` の**中**にあり、`alignDesignatedZoneOwner`（続き454）が SEQUENCE 直下しか見ていなかったため**既定 owner のまま「たまたま合っている」**状態だった。条件節・選択肢の枝へも降りるようにした。**「テストが通った＝構造で保証されている」ではない**。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-16 の残2**＝`WDK10-009-E2` は**動的 delta のゾーン grant**（`FieldGrant{kind:'power'}` は固定値なので受け皿が要る）／`SP38-006-E1` は3軸 (b)**O-3 の残31**（`STUB` 系が主体。⚠型ごとに受け皿を実測してから）(c)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵＝続き452 の `SP38-006` 外科パッチが先例）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452〜456 の5件が積まれた（相手応答モーダル／ターン跨ぎの能力喪失／ゾーン指定→後から配置／2ゾーン選択UI／ゾーン封じ）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き456（§6.4 O-16 第3波＝アタック禁止のゾーン継続）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1946（+2＝ゾーンアタック禁止の E2E／live 3効果のゾーン紐づけと保存先一致）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 34フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）、**`FieldGrant` の kind 3種**（`power`／`abilityLoss`／`blockAction`）。live effect 単位 diff **3効果・effectId 増減 0/0**。
- **旧行（2026-08-13 続き455・§6.4 O-16 第2波＝能力喪失のゾーン継続＋複数ゾーン指定）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1944（+2＝ゾーン能力喪失の E2E／2ゾーン指定の E2E）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 34フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）。live effect 単位 diff **2効果・effectId 増減 0/0**。
...
| **O-16** | **「指定したシグニゾーン」の残り** | 2効果 | 🏁**ゾーン継続の3型は出そろった**（続き454＝`kind:'power'` 6効果／続き455＝`kind:'abilityLoss'` 2効果＋複数ゾーン指定／続き456＝`kind:'blockAction'` 3効果）。**残2件は別軸**：(a)`WDK10-009-E2`＝`STUB{POWER_MOD_PER_COUNT}`（**レベル比例**のパワー修正）が指定ゾーンを読まない＝`FieldGrant{kind:'power'}` は固定 delta なので**動的 delta の受け皿**が要る (b)`SP38-006-E1` の1つ目の内側能力＝対象種別（キー＋シグニ）・体数（すべて）・期間（このターン）の3軸とも未忠実（カードは PARTIAL 温存）。⚠**計器の注意**＝`zoneSource` キーの有無だけで数えると `BLOCK_OPP_ZONE_PLACEMENT`（`zoneBlockSource:'designated'` で自前に読む）を**偽陽性**に拾う |
