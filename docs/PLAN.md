...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-60** | **engine が「カード全文 regex」で意味を決める箇所が系統として残っている** | **145ハンドラ（実測）／1ハンドラ＝1バッチ** | 🆕**2026-08-26 続き663＝手順①（機械分類）は完了し、計器になった**＝`npm run census:enginetext`（`scripts/censusEngineText.ts`・明細 `docs/_census_enginetext.txt`・`npx tsx scripts/censusEngineText.ts --id <ハンドラ>` で1件の完全内訳）。■**実測**＝`src/engine/*.ts` の `EffectText` 読み出し **235行**を3分類＝**A🔴 SELF_TEXT 149行／145ハンドラ**（＝**効果元自身**の全文で意味を決める＝本命の worklist）／**B OTHER_CARD 63行**（＝対象候補やデッキトップなど**他カード**の属性判定＝アイコン・キーワードで正当な用法が多い）／**C COMMENT 23行**。⚠**登録票の旧記「確定している具体例1件」は大幅な過小**だった。 ■**優先度は miss で読む**＝ハンドラ内の regex/includes を live の該当カード原文に当てて**1本も当たらない**ものを数える＝**50ハンドラ・138カード**が該当（＝いま既定値へ落ちている＝**現に壊れている**）。⚠**miss=0 は「正しい」ではない**（たまたま当たっているだけ）。⚠**miss 数は「掘る価値の指標」であって「見込み件数」ではない**（§4.3）。 ■🔴**ゲート（ratchet）を `npm run gates` に同梱済み**＝A群の行数が**増えたら exit 1**（新しく全文 regex を書いた）・**減っても exit 1**（払い戻したのに `BASELINE_SELF_TEXT` の下げ忘れ）。 ■**残りの取り方**＝計器の上位から**1ハンドラ＝1バッチ**で `O-56` と同じ手口（parser が判別子を吐く→engine は payload で分岐→逆翻訳も payload から描く→全文 regex 撤去→基準を下げる）。**次の3件**＝`LRIG_UNDER_CARD_OP`（miss16/live18）／`COPY_LRIG_NAME_ABILITY`（miss16/live16）／`DEPLOY_RESTRICT`（miss8/live10）。⚠**着手前に `--id` で原文を全部読む**（催告＝計器は較正済みだが、入れ子の門が複雑なハンドラでは母集団が上振れする）。 ■✅**第1バッチ消化済み**＝`LOOK_OPP_LIFE_TOP`（live 28効果中 **27効果**で regex が1本も当たっていなかった）＝`StubAction.lookZone{zone,count}` を新設。全文は BUGFIXES.md 2026-08-26。 |
...
- **2026-08-26 続き663 後（本行が直近の正）**：
  **census 572/572**（`O-60` では動かず＝engine の全文 regex は census の網に載らない）、**golden 2809**（`O-60` +2）、
  ...
  🆕**`census:enginetext` A群 149行 / 145ハンドラ**（B 63／C 23・miss ありハンドラ 50・miss カード 138）、
  **live カード 5975 / 効果総数 10693 / MANUAL 効果 1032**（据置。`O-60` は `lookZone` の追加だけ＝**18効果**）、
  **`_held_fresh` 76 / `_partial_fresh` 12 / `_idset_fresh` 45**（いずれも据置＝parser 改善が凍っていない）、
  **実機シナリオ +1**（新規 `b60LookZoneOppHand`。回帰＝`b66TurnEndDelayInstalled`／`b66UnderGrantPayload` PASS）、
