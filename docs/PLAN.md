...
- **🆕 セッション（2026-08-11・続き428〜429・Opus 5）＝§6.4 を2本＝**エナ支払い元の一本化**（funnel 新設＋`UNDER_CARD_AS_ENERGY_COST` 実装）と**離場置換の前提訂正＋決定層の分離**。golden 1776→1790・census 854 据置**。
  - **🔴① 続き428＝「エナコストの支払い」がどこにも一本化されていなかった**＝支払いモーダル14本が `my.energy` を直読みし、候補生成も控除も **BattleScreen の14サイトに散在**。この状態で「エナゾーン以外を支払い元にする」語彙を載せると、**半分だけ配線された瞬間に「アーツには使えるが【起】には使えない」無言の不整合**になる。続き427 の defer 判断（funnel が先）はそのとおりだった。
  - **⭐② funnel の形は2点で決まる**＝(a)**後方互換の index 空間**（先頭 `my.energy.length` 件がエナゾーンそのもの＝既存の `costIndices` がそのまま通る／追加元0件なら従来と完全に等価）(b)**控除は `applyTo` でサイトの state 組み立ての「あと」に当てる**（サイトが `field` を自前で組み直しても取りこぼさない）。
  - **⭐③ 安全弁は関数の戻り値に置く（教訓 (m) の実践）**＝`applyTo` を呼び忘れると**エナが1枚も減らない**＝「ただでアーツが撃てる」という**即座に露見する**壊れ方になる。**無言にならない壊れ方を選ぶ**のが設計の本体。golden にソース走査トリップワイヤも設置。
  - **🔴④ 続き429＝§6.4 の登録内容そのものが間違っていた**＝「置換5本はすべて『してもよい』の自動適用＝近似」ではなく、原文全数照合で**強制3効果（2軸）／任意10効果（3軸）**に割れる。`WXEX2-30`「代わりにデッキの一番下に置かれる」と `SPDi44-08`/`WX25-P1-018`「代わりにこの能力を失う」は**「してもよい」が無い＝自動適用が正しい**（近似ではない）。**強制軸を対話の選択肢に出すとルール違反**になるので、原文の語尾から分類を機械ロックする golden を置いた。
  - **⭐⑤ 対話化は「policy の差し替え」に落とせる**＝`applyEffectLeaveSubstitutes` を **collect→choose→apply** に分解。列挙は**投機実行**（`ExecCtx` は不変なので採用しない候補の ctx は捨てるだけ・ログも漏れない）。F-3 の内部選択（下スペル／手札スペル／犠牲／ライフクラッシュ）も1件ずつ option 化したので、**engine の構造を触らずに対話へ移行できる**状態にした。挙動は不変。
  - **⭐⑥ 「実装が無い」と「policy が選ばない」は別物として書く**＝`WX14-026` の lifeCrash は従来**列挙からも落として**いたので「対話を入れても選択肢に出せない」状態だった。`autoEligible:false` にして**列挙はする／自動では選ばない**へ。宿題の性質が「配線待ち」から「policy 待ち」に変わった。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（typecheck / golden **1790（+14）** / smoke 10688 全0・SKIP 0 / fuzz 全0 / census **854 据置** / census-stubs（明示 defer **17→16**・無言 no-op **0**）/ manual-fields / lint 0 errors）。`npm run regen` 済み。
  - **📋⑧ 次の一手**＝**Opus 側**＝(a)**離場置換の対話 policy（M2）**＝設計確定済み・`applyDirectAction`＋`pickQueue` で pause を張る。**ループ5サイトの簿記をポーズ跨ぎで運ぶのが実質**（(b)以降より重い）(b)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）。**Sonnet 側**＝§7 実機検証（**続き428 のエナ支払い回帰確認を最優先**、次いで続き424〜427）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き428〜429（§6.4 エナ支払い元の一本化＋離場置換の前提訂正・決定層分離）後 最新値（本行が直近の正）**：census **854**、golden **1790（+14）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
>
> 0. 🎯**次の一手（2026-08-11 続き429 更新）＝§6.4 の残**＝(a)**離場置換の対話 policy（M2）**＝**設計は確定済み**（`collectLeaveSubstituteOptions` の結果を `CHOOSE{opponentResponds:true}` で被害側へ／pause は `applyDirectAction`＋`pickQueue` の先例で張る）。**実質はループ5サイトの簿記をポーズ跨ぎで運ぶ設計**＝`WXDi-CP01-023` もこれとセット (b)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)**残 A群16件の明示 defer 見直し**（§6.3 の機構が進んだものから解ける）。**Sonnet 側＝§7 実機検証（続き428 のエナ支払い回帰確認が最優先）**。以下は旧「次の一手」（参考）。
>
...
- **離場置換の対話化**（続き406 登録 → **2026-08-11 続き429 で前提訂正＋決定層を分離**）
  ...
  - **📋 残＝対話 policy の実装（M2・単独バッチ）**。設計は確定済み：`collectLeaveSubstituteOptions` の結果を被害側へ `CHOOSE{opponentResponds:true}` で出し、選ばれた option の `resultCtx` を採る。⚠**pause の張り方**＝`applyDirectAction(action, cardNum, ctx): ExecResult`（単票の離場適用・ポーズ可能）と `INTERNAL_HAND_OR_ENERGY` の `pickQueue`（1枚ずつ問い直す先例・`effectExecutor.ts`）を組み合わせる。⚠**呼び出し13サイトのうち6は単票（`applyDirectAction` の case）で無料、5はループ**（`execBanish`／`applyBounce`／`applySend`／`applyTrashField`／`applyToBottom`）で、**ループ前後の簿記（`costLevels`／`trashedPuppet`／`lastProcessedCards`）をポーズ跨ぎで運ぶ設計が要る**＝ここが M2 の実質。⚠**候補が0件の盤面（大多数）は現行の同期パスのままにする**＝リスクを新経路だけに閉じ込める。
