...
| 🆕(xxiii) | **リコレクト分割8件＝深刻な内容欠落（続き138・Sonnet・タスク9トリアージ）**。`effectParser.ts:1945-1946`＝《リコレクトアイコン》系の複数ステップ分解でUNKNOWN断片を無言除外。**✅続き173（Fable 5）で本丸の WX25-P1-001/003/005/007/009（5枚同一テンプレ）を消化**＝「センタールリグ１体を対象とし、ターン終了時まで、それは以下の能力を得る。『【起】エクシード…×3』」を parser 文型1本（`GRANT_LRIG_ABILITY{targetedCenter}`＋既存 `expandGrantLrigAbilities` 展開）で語彙化。**トリアージの「新規GRANT機構が要る§6.3級」は誤り＝engine 受け皿（executor→`lrig_granted_auto_effects`→BattleScreen 付与【起】列挙→`executeLrigGranted` エクシード支払い→ターン境界リセット3箇所）は完備済みだった**。副産物＝内側1本目の pick 脱落（LOOK_AND_REORDER縮退）の規則順序バグを狭い専用規則で是正＝系統15枚採用（うちMANUAL兄弟温存2枚は直接パッチ）。census 2031→2028・golden 385・詳細 BUGFIXES 続き173。**残＝SPDi47-03-E2**（「カードを3枚引き手札を好きな枚数捨てる」の本体action丸ごと消失）／**SPDi47-05-E2**（バニッシュ代替の置換ルール丸ごと消失）／**WX24-P4-016-E3**（GRANT_KEYWORD「マジックボックス」が原文トリガー文と不一致・要再確認）＝リコレクト分割の別型・個別再確認から。詳細 `docs/_partial_triage.txt`。 |
...
- **🆕 セッション（2026-07-17・続き173・Fable 5・Opusタスク12(xxiii) 本丸＝WX25-P1-00X系列のルリグ複数【起】エクシード能力付与を語彙化＋pick脱落の規則順序バグ是正）**
  ...
  - **指標**＝census 2031→**2028**（`BASELINE_HIGH` 更新済み）・golden 383→**385**（付与構造＋pick復元の parse assertion 追加）・smoke 10592 OK 全0・fuzz 全0・同型★0（5986枚）・lint 0 errors。
  - **次の一手**：Opus＝①(xxiii) 残＝SPDi47-03-E2/SPDi47-05-E2/WX24-P4-016-E3（リコレクト分割の別型・要個別再確認）②タスク16[A]約34クラスタ（`docs/_timing_census_triage.txt`）③IS_MY_TURN化残の distinct/sum/多分岐カウント機構・(xxvii)残Cluster B/E。Sonnet＝**タスク1の在庫が1件復活**＝WX25-P1-001 の付与【起】（エクシード支払い→効果実行）の §7 driver シナリオ新設・2回連続PASS確認（「要実機検証」）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 385・smoke/fuzz 全0・同型★0・census 2028。
>
