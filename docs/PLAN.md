...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer **残 13 id/17件**。続き507 で1巡済み＝次は `DEFERRED_GRANT_UNTAP_ON_ATTACK_TO_TEAM_LRIG` が最も着手しやすく、`DEFERRED_GATE_ZONE_GRANT_AUTO_ABILITY` は O-33 据置の `WDK09-001-E2` と機構を共有）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 13 id / 17件 | 🆕**2026-08-16 続き507 で1巡実施＝16種/20件 → 13種/17件**（実装で **6件/5種** 解体、1件は理由を絞って専用 id へ改名、**新規の明示 defer 3件**＝下記の隣接発見）。⚠**A群に出ない同族が母集団の3/4だった**（memory どおり）＝「代わりに…この能力を失う」を原文 regex で数え直すと **14効果**で、A群に映っていたのは4件だけ。🔴**残り10件のうち4件が壊れていた**＝`WX25-P2-TK04-E1`（defer と**同文**なのに `REMOVE_ABILITIES{owner:'opponent'}`＝**相手シグニの能力を恒久的に消す**加害へ化けていた・A群に映らない）／`WX25-P2-059-E1`・`WX26-CP1-047-E1`・`WXDi-CP02-056-E1`（`SEQUENCE[OPTIONAL_COST, CONDITIONAL→REMOVE_ABILITIES]`＝**CONTINUOUS の SEQUENCE は誰も実行しない**完全 no-op）。**■解体した6件**＝(a)`WX09-Re01-E1`〔defer 理由が**原文の誤読**。「【グロウ】…《リメンバ》を含む」は**グロウ条件**で E1 の「かぎり」条件ではない＝同文の `WXEX1-02-E1` と同じ `REMOVE_ABILITIES` で足りた〕(b)`WX22-022-E1`〔**O-8 で作った `signiAttackGate` へ「すでにダウン」判定を寄せて**例外を1点に。⚠同居する `ATTACK_COUNT_BY_POWER` を**実効パワー**で数えるようにしないと印刷15000＝1回固定でこの能力が観測不能な死に機構になる〕(c)`WX25-P3-055-E2`＋(d)`WX25-P2-071-E1`＋(e)`WX25-P2-TK04-E1`〔離場置換に**新軸 `selfAbility`**（`EFFECT_LEAVE_PREVENT_LOSE_SELF_ABILITY`）＝既存 `lrigAbility` 軸のシグニ自身版。🔑**失うのは効果1つだけ**＝`lost_ability_effect_ids_this_turn`（効果単位）。`abilities_removed`（カード単位）で表すと同居能力を巻き添えにする〕(f)`WXK01-002-E1`〔`PREVENT_LRIG_DAMAGE`＋新フラグ `loseAbilityAfterUse`。⚠既定は**回数無制限**なので刻みを見ないと**無限バリア**〕。**■改名1件**＝`WXK03-071-E1` → `DEFERRED_OPP_TURN_ARTS_COST_REDUCTION_ONCE`（「この能力を失う」機構は実装済み＝残るブロッカーは**アーツ使用コストの場宣言軽減**で、収集器が無く**コスト算出の入口が3箇所**）。**■新規 defer 3件**＝`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY`（上の任意コスト版。⚠離場置換の決定層は `resultCtx` を**同期的に**組む契約なので対話 pause を張れず、しかも**払うのは victim オーナー＝`otherState`** で `optionalCostPaySteps`／`canAffordOptionalCostSpec` の `ownerState` 固定と視点が合わない＝**対話つき置換の policy 差し替えと対で実装する**）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き507）。⚠**全経路 実機未検証**＝§7 送り。**■残 13 種 / 17 件**＝`DEFERRED_CONVERT_ENERGY_COLOR`(3)〔⚠エナ支払いの色照合が `costs.ts` に5箇所散り 1点 funnel が無い〕／`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY`(3)／`DEFERRED_ATTACK_NEGATE_IMMUNITY_SELF`／`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`／`DEFERRED_DECLARE_NUMBER_AND_ATTACK_PHASE_RESTRICTIONS`／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`／`DEFERRED_GATE_ZONE_GRANT_AUTO_ABILITY`〔`WDK09-001-E2`（O-33 据置）と**同じ動的ゾーン機構**を共有＝対で実装すると2件同時に解ける〕／`DEFERRED_GRANT_UNTAP_ON_ATTACK_TO_TEAM_LRIG`〔部品（`GRANT_LRIG_ABILITY`／`ON_ATTACK_LRIG`／`UP`）は全部ある＝**次に着手しやすい**。⚠「ルリグをアップして2回目のアタック」が実機で通るかの確認が要る〕／`DEFERRED_OPP_TURN_ARTS_COST_REDUCTION_ONCE`／`DEFERRED_RED_CENTER_LRIG_REPLACE_ABILITIES_UNTIL_END_OF_TURN`／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`／`DEFERRED_TRASH_RED_SIGNI_LEVEL1_PLAY_AND_END_TURN_RETURN`。**定期的に前提を再判定する**（今回も6件中1件は「前提が消えた」ではなく**最初から理由が間違っていた**）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・今回＝14効果中4件が STUB ですらなかった）。 |
