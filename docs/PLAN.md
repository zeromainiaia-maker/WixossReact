...
- **🆕 セッション（2026-08-15・続き493・Opus 5）＝§6.4 **O-3 の `NON_FIELD_ZONE_MOVE_IMMUNITY` を解体＝5効果の挙動是正**（**永続化バグ 1／恒久 no-op 1／片側採用 1／過剰実行 1／汎用 STUB 誤配 1**）。ゲート全緑（**golden 2034**＝+4・**census 831 据置**・lint 0/260）。
  - **🔴① 期間つき予約に失効地点が無く永続していた（3回目の再発）**＝`prevent_opp_trash_from` は **set が1箇所・clear が0箇所**で、`WXK10-083-E1`「**このターンと次のターンの間**、対戦相手の効果によってあなたのエナゾーンにあるカードはトラッシュに移動しない」が**ゲーム終了まで**効いていた（`signi_deploy_power_limit`＝続き487／`negated_attacks`＝続き489 と同じクラス）。`opp_move_immunity{zones, turnsRemaining}` へ置き換え、`signi_deploy_bans` と同じ**ターン数カウントダウン**（減算は `clearTurnEndScopedState` の1点）にした。🔑**「期間つき」と書いてあるフィールドは失効地点を grep して実測する。**
  - **🔑② 消費側は「ctx 側の集合 ∪ state 直読み」で判定する**＝`ExecCtx.otherProtectedZones` を組み立てない経路が実在するので、`activeOppMoveImmunityZones(state)` を新設して4サイトとも両方を見る（片方だけにすると経路によって無言で素通りする）。【常】宣言（`PREVENT_ZONE_MOVE_BY_OPP`／`PREVENT_NON_FIELD_MOVE_BY_OPP`）は **state に書かず** `collectProtectedZones` が effectsMap から読む（書くと「1回で消える」に化ける＝続き492 と同じ規律）。
  - **🔴③ 複合節を片側だけ採用すると残りが無言で落ちる**＝`WXEX2-06-E3`／`WXDi-P16-002-E1` の「…**ダメージを受けず**、…**移動しない**」は前半だけが拾われ後半が消えていた。`SEQUENCE[PREVENT_DAMAGE{scope:'LRIG'}, ZONE_MOVE_IMMUNITY]` で両方載せた。⚠golden にあった「片側だけ採用するな」の**据置トリップワイヤは正方向（両方載る）へ置き換えた**。
  - **🔴④ `WXDi-P16-002-E1` は3つ同時に壊れていた**＝(a)**【使用条件】ヘッダが `GRANT_KEYWORD{keyword:"使用条件"}` に化けていた**（続き490 の「引用文が丸ごと keyword に入る」クラス）ため本来の使用条件（【チーム】＜夢限少女＞＆全員レベル１以上）が条件として載っていなかった (b)複合節の両方が脱落 (c)「次の対戦相手のターン終了時、カードを１枚引き【エナチャージ１】をする」を**使った瞬間に実行**していた。
  - **🔑⑤ 遅延タイミング宣言の本体は即時実行しない（続き488 の再適用）**＝「次の対戦相手のターン終了時、〜」は原文2効果。予約機構が無いので明示 defer（`DEFERRED_NEXT_OPP_TURN_END_BODY`）へ落として**過少側に倒す**。⚠**規則は `parseSentencePart1` の先頭に置く**＝本文側の汎用規則（`カードをN枚引く` 等）が先に食うと宣言が消えて本文だけ残る。`WXDi-P09-066-E1` は無関係な汎用 `STUB{LOOK_AND_REORDER}` に落ちて**計器に映っていなかった**ぶんも同時に可視化。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残8（A群が常時の在庫表）。`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`＝「このターンにアタックしたシグニ」の対象化＋キー自己トラッシュの任意コスト。`attacked_signi_ids` は既存）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）／🆕`NEXT_OPP_TURN_END_BODY`（2効果＝遅延予約機構）。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-28**（🆕下記）→ **O-25 の残** → **O-26** → **O-27**。🆕**O-28＝`GRANT_KEYWORD.keyword` にゴミが入っているクラスの全数走査**＝live を機械走査したところ**29 綴り**が正式キーワード名でなく、うち**「《無》×N を支払わないかぎりアタックできない」5効果は続き490 で作った `SigniAttackBan.unlessPayColorless` にそのまま載る**（`WX11-012-E1`／`WX24-P1-041-E2`／`WX24-P4-001-E1`／`WXDi-P03-036-E1`／`WXDi-P05-033-E2`）。ほか丸ごと文が入っているもの（`WX25-P2-071-E1`／`WXEX2-71-E3`／`WX24-P1-064-E1`／`WXDi-P05-068-E1`）＝**どれも `hasKeyword` の照合外で一度も効かない無言 no-op**。⚠**着手時は受け皿 id ではなく原文 regex で母集団を数え直す**（続き489〜493 で5回とも「A群に出ていない隣の効果のほうが壊れていた」）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き493 の4経路**＝(a)`WXK10-083-E1`（張ったターンと次のターンはエナが落ちず、**その次のターンでは落ちる**＝負方向＋対照の対）(b)`WXK10-004-E1`（手札とエナの両方）(c)`WXEX2-06-E3`／`WXDi-P16-002-E1`（ダメージ無効と移動不可が同時に効くか）(d)`WXDi-P16-002-E1` の使用条件が満たせないとき使えないか。ほか続き488〜492 の未検証分。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き493（§6.4 O-3 `NON_FIELD_ZONE_MOVE_IMMUNITY` 解体）後 最新値（本行が直近の正）**：census **831 据置**（較正なし）、**golden 2034**（+4＝期間軸1・実消費地点1・実カード1・遅延本体1。ほかに据置トリップワイヤ3件を正方向へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **110枚 / 46群**、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **23種/25件 → 23種/26件**＝`NON_FIELD_ZONE_MOVE_IMMUNITY` −1／新設 `NEXT_OPP_TURN_END_BODY` +2＝**隠れていた2件を明示 defer へ可視化**したぶん）。🆕**live JSON changed 5カード**（`WXK10-004`／`WXK10-083`／`WXEX2-06`／`WXDi-P16-002`／`WXDi-P09-066`。CSV 非改変）。🆕**挙動是正 5効果**（永続化バグ 1／恒久 no-op 1／片側採用 1／過剰実行 1／汎用 STUB 誤配 1）。🆕**新機構＝`opp_move_immunity`＋`activeOppMoveImmunityZones`（期間つき移動不可をターン数カウントダウンで持つ）／`ZONE_MOVE_IMMUNITY` アクション／`DEFERRED_NEXT_OPP_TURN_END_BODY`（遅延本体の明示 defer）**。⚠**4経路とも実機未検証**（§7 送り）。⚠**現行の近似**＝保護できるのは hand / energy → トラッシュの移動だけ（デッキ／トラッシュ／ライフ側は移動地点の funnel が無い）。
...
| **O-3** | **「次のターンの間」系統の残り** | 8効果（受け皿8種） | 🏁**続き492＝`UNTIL_NEXT_MAIN_PHASE_CLAUSE`（1）を解体＝「ルリグによってダメージを受けない」の期間軸・走査軸ごと1本化＝9効果の挙動是正**（恒久 no-op 2／無言 no-op 2／1回で消費されていた 5／過剰実行 1／過少実行 1）。新機構＝`isLrigDamagePrevented`（`screens/battle/lrigDamageShield.ts`＝シグニ／ルリグ／アシスト／キーを走査）／`turnScopedState` の境界 `main-phase-start`＋`clearMainPhaseScopedState`（**次のあなたのメインフェイズまで**＝ターン境界を跨ぐ期間）／`SET_LRIG_BASE_LIMIT`（加算ではなく置換）／`RESERVE_DRAW_PHASE_REPLACEMENT`。**残り**は `ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／`DECLARED_SPELL_NAME_LOCK` 1／`GAIN_OPP_LRIG_TYPE` 1／`NON_FIELD_ZONE_MOVE_IMMUNITY` 1／`FIELD_SIGNI_TO_CHECK_ZONE` 1。**在庫は `npm run census:stubs` の A群で常時見える**／**機構ごとに独立して着手できる**。🔑**続き492 の教訓**＝①**「N回まで」ではなく「期間中ずっと」の防御は、回数フラグではなくウィンドウで持つ**（`prevent_lrig_damage` の boolean は1回で消費され、「このターン」の2回目も「次のターンの間」も守れなかった）②**【常】宣言の走査軸はシグニだけでは足りない**（ルリグ本体・アシスト・キーの宣言が丸ごと無視されていた）③**「回数無制限の防御」は消費型より先に判定する**（後ろに置くとバリア等の限りある資源が先に減る）④**宣言型の【常】は state にフラグを書かない**（書くと「1回防いだら消える」に化け、レベル限定も表せない）⑤**「〜引く場合、代わりに〜引く」を汎用ドロー規則に食わせない**（`fromCount` 側が実ドローと読まれ即時ドローの過剰実行になる）。🆕**据置2件**＝`WX24-P3-003-E1`（`GRANT_ABILITY_INNER_TEXT`＝引用能力の中身が未パース）／`WXK04-014-E1`（CHOOSE 選択肢②のダメージ無効が未表現）。以下は旧記録＝🏁**続き491＝フェイズ／ターンのスキップ系統を解体**＝`NEXT_OPP_TURN_MAIN_PHASE_SKIP`（1）と `SKIP_NEXT_TURN`（1）を消化し、**A群に出ていなかった同系統4件も同時に是正**（`WX05-018-E1`／`WX16-001-E3`／`SP38-006-E4`／`WXK06-078-E1`）＝**6効果の挙動是正**。新機構＝`PHASE_SKIP_BLOCK_IDS`＋`resolveNextPhaseWithSkips`／`resolveTurnHandover`。🔑**教訓**＝**「STUB の在庫」を数えても半分しか見えない**（未消費の `BLOCK_ACTION` id・綴りズレ・ログだけのハンドラは計器に映らない）。🏁**続き490＝`ATTACK_TAX_HAND_DISCARD`（1）を解体＝`SigniAttackBan.unlessPayHandDiscard` を新設**（隣接の無言 no-op `WXDi-P05-022-E1` も同時に是正）。🔑**教訓**＝**「支払えば通る」制限は軸ごとに別関数を生やさない**。🏁**続き489＝`NEXT_OPP_ATTACK_PHASE_START`（2）を解体＝遅延予約 `DELAY_TO_NEXT_OPP_ATTACK_PHASE` を新設**＝**5効果の挙動是正**。🔴**`negated_attacks` は型に「このターン」と書いてありながら失効地点が1つも無く永続していた**。🔑**教訓**＝**受け皿 id は母集団の一部しか映さない＝着手時は原文 regex で数え直す**。🏁**続き488＝`EXTRA_ATTACK_PHASE`（2）／`SELF_RESTRICT_THIS_TURN`（2）／`UNPARSED_NEXT_OPP_TURN_CLAUSE`（1）を解体**＝新機構 `ADD_EXTRA_ATTACK_PHASE` ほかで**8効果の挙動是正**。🔑**教訓**＝**「遅延タイミング宣言」の後続文は絶対に即時実行しない**。🏁続き486＝`_THIS_TURN_OPP_CLAUSE`（7効果）＝新機構 `SIGNI_ATTACK_BAN` で**5効果の恒久 no-op を解消**。🏁**続き487＝`_NEXT_OPP_TURN_CLAUSE`（5）と `_THIS_AND_NEXT_TURN_CLAUSE`（4）**＝新機構 `SIGNI_DEPLOY_BAN` ほかで**7効果の挙動是正**。⚠**期間だけ直すと「誤った対象の効果」が長持ちする**＝対象軸が壊れている札は据置（`WX11-038-E2`）。🔑**続き486/487 の教訓**＝①**受け皿 id の件数だけ見て設計しない**②**STUB を実装で置き換えると census が +N する**（毎回「計器の較正漏れ」か「本物の穴」かを仕分ける） |
...
**■ 消化済み**
> ✅**O-3 の `UNTIL_NEXT_MAIN_PHASE_CLAUSE`＝2026-08-15 続き492 で解体**（O-3 自体は継続）。**9効果の挙動是正**（恒久 no-op 2／無言 no-op 2／1回で消費されていた 5／過剰実行 1／過少実行 1・重複あり）。
