...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377i＝収穫マージの効果単位化後）**＝golden **1450**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **960**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **144枚／71群**・実機 driver 既定order **128件**。**被覆マトリクス miss 318**（`npm run census:wiring`）。**混在カードのレビュー待ち 12カード**（`docs/_partial_fresh.json`・続き377i 新設）。
>
...
>    - **⓪ 🆕`docs/_partial_fresh.json` の残り12カード**＝`WX04-002`／`WX04-003`／`WX04-093`／`WX11-026`／`WX12-018`／`WXK07-031`／`WXK10-017`／`WXK10-022`／`WXK10-075`／`WD14-011`／`WDK17-009`／`SP27-015`。**いずれも live のほうが正しい**＝parser が退化させる差分なので、**parser を直すか live 側を MANUAL 化**して行列から落とす。⚠`WX12-018-E1` は fresh だと「アーツ**以外**の効果を受けない」が `from:['アーツ']`＝**意味が反転**する。
...
>    - **✅消化済み**＝「他の」ゲート棚卸し（377）／`noGuard`（377b）／アイコン系（377c）／`levelRange`（377d）／全体バフの語順＋据置held（377e）／ON_ATTACK_SIGNI の味方側トリガー主語＋`excludeSelf` の過剰発火（377f）／stale live の一括解消 38効果（377g）／(iv) stale live の刈り取り 37効果（377h）／🏁**収穫マージの効果単位化 70効果（377i・stale live の構造的原因を解消）**。
...
>    - ⚠**見積もりの現在地**＝census **960**／被覆マトリクス miss **318**。9バッチの実績＝12／30／17／8／15／11／38／37／**70**効果。**census が動かない回でも効果は直る**（377c は census 据置で17効果）＝**census の増減だけで成果を測らない。**
