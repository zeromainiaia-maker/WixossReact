...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15` → `O-10`（明示 defer **残 1 id/1件**＝`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`（ピース使用への応答窓）だけ。続き507〜516 で9巡済み）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 1 id / 1件 | 🆕**2026-08-16 続き516（9巡目）＝2種/2件 → 1種/1件**（`WX25-CP1-060-E2`＝対象になったら裏返って対象から外れる）。🔑**続き511 で特定したブロッカー（`ON_TARGETED` は `resumeSelectTarget` の適用後に積まれる）をそのまま前提にして、「トリガーにしない」で解決**＝**対象宣言の直後・適用の前**（`handleEffectInteraction` の `SELECT_TARGET` 分岐）で宣言された対象から外す（`screens/battle/targetDodgeFlip.ts` の純関数1本）。⚠**該当宣言が対象に1体も居なければ `dodged` は空＝従来とまったく同じ経路**（ホットパスに分岐を足す以上、無関係な盤面で完全に不活性であることが安全条件。golden で「state も1バイトも変わらない」を固定）。⚠「ターン終了時まで【自】能力を失う」は `lost_ability_effect_ids_this_turn`（続き507 の機構）で**効果単位**に落とす＝`abilities_removed`（カード単位）だと同居する【常】E1／【絆常】E3 まで消える。🔴**隣接発見＝旧 live は害の方向が逆だった**＝`SEQUENCE[DEFERRED, REMOVE_ABILITIES{thisCardOnly}]` で**回避せずに自分の全能力を失う**（過少＋過剰の同居）。🔴**もう1つ＝`condition` の `excludeSelf` が `filter` の中に入っていて評価器に届いていなかった**（`HAS_CARD_IN_FIELD` は**条件レベル**の `excludeSelf` しか読まない）＝「他の＜ブルアカ＞」が**自分自身で満たせて**いた。live 全体を走査したところ同型は**この1件だけ**。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き516）。⚠**全経路 実機未検証**＝§7 送り。**■残 1 種 / 1 件**＝`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔`WXDi-P05-006`。**ピース使用への応答窓**が無い＝カットインは現状スペルにしか無く、`pending_spell` 相当の `pending_piece` と、その窓で使えるピースの収集・UI・「打ち消してゲームから除外する」処理が要る。**§6.4 で唯一残った本物の機構待ち**〕。**定期的に前提を再判定する**（続き507〜516 の計19種のうち、**「機構が無い」が本当だったのは1種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付け／見落とし・自分で書いた据置理由の陳腐化・未消費 id・カードデータが後から入った・**ブロッカーを前提にして別の地点で解いた**だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
