...
- 🏁**セッション（2026-08-25・続き649・Opus 5）＝§5.2 段2 第43バッチ＝ターン内履歴条件「このターンに〜していた場合」が丸ごと落ちて無条件実行になる穴を配線**（**今回は Codex へ委譲せず Claude が実装**）。gates 全緑（**golden 2735→2738**／census **600→591**／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260（増分0）／同型★0）。**実機2本が2回連続 PASS＋反転確認済＝1巡が §2.2 の⑤まで完了。台帳 701→694。**
  - ✅**11効果を修正**（＋同じ規則で一緒に直った標本外1件）＝`WXK02-065-E1` は「このターンにシグニが2体以上場から手札に戻っていた場合」の条件が無く**アタックのたび無条件で【エナチャージ２】**、`WD16-016-E1` の出撃制限は**条件が付かず恒久 no-op**で制限を無視して召喚できた。
  - 🔑**母集団は「判定語彙を推測せず、条件ノードの有無で数える」**＝原文に「このターン〜場合」を含む144効果から、条件ノードを持つ113と**専用 STUB に化けている12**を引いて**真に無条件の19効果**。⚠**素朴に「live に `*_this_turn` 語彙があるか」で数えると74件に膨れる**（`TURN_HAND_DISCARD_GTE`／`THIS_CARD_PLACED_BY_CLASS` など**命名規約に乗らない既存型**を取りこぼす）。**測り方を3回作り直して初めて正しい母集団に着いた。**
  - 🔴**評価器は3本ある**＝`evalCondition`（execUtils）／`checkActiveCondition`（effectEngine）／**`evalConditionForContinuous`**。3本目は **`default: return true`（permissive）** ＝出撃制限の条件をここへ足し忘れると**恒久 no-op のまま**で census も golden も緑。3本すべてに同じ式を入れた。
  - 🔴**「N枚目だった場合」を `minCount`（N以上）で近似してはいけない**＝**N+1枚目以降でも発火する過剰実行**になる。`exactCount` を別軸で新設し、golden で「4枚目→false」を固定した。
  - 🔴**実装中に一度退化を踏んで是正した**＝条件を**効果レベルへ hoist** したところ、「対戦相手のシグニ１体を**対象とし**、〜場合、それをバニッシュする」形で**対象取得ごとゲート**され、**既に正しかった8効果の `CONDITIONAL` を壊していた**（held の増分を1件ずつ見て気付いた）。⇒ **文単位の CONDITIONAL 持ち上げ表へ規則を置く**形に直し、見送り側も golden で固定。
  - 🆕**follow-up＝`O-63`**（「〈誰か〉のターン…」の **turnOwner 限定を `collectTurnTriggers` が読んでおらず、両方のターンで発火する**＝実測**167効果**）。本バッチで触った2効果の別軸 finding はこれに当たるので**台帳では閉じていない**。
...
🔴**母集団は「その語彙が live にあるか」ではなく「その意味を担うノードが在るか」で数える**（続き649 で3回測り直した）。⚠**既存型の名前は命名規約に従っていないことがある**＝正規表現で型名を推測すると静かに取りこぼす。
🔴**条件を効果レベルへ hoist するか CONDITIONAL に包むかは意味が違う**＝「対象とし、」の後の条件は**対象取得を含めてゲートしてはいけない**。held の増分を1件ずつ原文照合すると気付ける（§5-5b′ が効いた実例）。
...
- [ ] 🆕**`V-88`＝`v12GrantedEnergyChargeTwice` / `v12GrantedEnergyChargeThirdBlocked` が2本とも FAIL（既存・本バッチ以前から）**。2026-08-25 続き648 に §5.2 第42バッチの関連回帰として実行して発見し、**`git stash` でベースライン `02b97abed` へ戻して同一内容の FAIL を再現＝第42バッチの退化ではないことを機械確認済み**。■**真因＝シナリオの腐り**（分類(a)）。`SPDi43-13-E2` の起動条件は **《ゲーム１回》バイブスMAX《緑×0》** だが spec が `'energy': []` でバイブス前提を作っていないため、**付与そのものが起きない**（両シナリオとも `grantIds=[]` `energy=0` `completed=0` のまま 55〜70秒でタイムアウト）＝**ON_ENERGY_CHARGE の経路には一度も到達していない**。■**着手手順**＝①バイブス（`vibes`）の盤面前提を `spec.hostSet` で作れるか `H.patchPlayerState` の対応キーを確認する ②作れないなら **spec を「バイブスMAX 済み」の状態から始める**か、`lrig_granted_auto_effects` に `SPDi43-13-sub-E1` を直接載せて **ON_ENERGY_CHARGE の観測だけに絞る**（本来の観測対象は付与ではなく付与後の watcher）。⚠**対照の `v12PrintedEnergyChargeControl` は緑**＝ON_ENERGY_CHARGE 経路自体は生きている。
...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-63** | 🆕**「〈誰か〉のターン…」の `turnOwner` 限定が engine で読まれず、両方のターンで発火する** | **M** | **2026-08-25 続き649（§5.2 第43バッチ）の検証で発見**＝原文「【自】：**あなたの**ターン終了時、〜」「【自】：**対戦相手の**ターン終了時、〜」の**ターン主限定**が効いていない。■**実測**＝【自】のトリガー句が「〈あなた／対戦相手〉のターン〈終了時／開始時／の間／に〉」の効果は **255**、うち **167効果**が `triggerCondition.turnOwner` も `IS_MY_TURN`／`TURN_OWNER` 条件も持たない。■🔴**parser だけ直しても直らない**＝`collectTurnTriggers`（`src/engine/triggerCollect.ts`）は **`turnOwner` を一切読んでいない**（`ON_TURN_END`／`ON_TURN_START`／`ON_ATTACK_PHASE_START` などを両プレイヤー分そのまま積む）。**型・parser・collector の3点セット**が要る。⚠他の collector（`:577` `:730` `:773` ほか）は既に `triggerCondition.turnOwner` を読んでいるので、**式はそこから写せる**。■**着手手順**＝①167効果を「両ターンで発火すると実害が出るもの」に絞る（対称な効果＝どちらのターンでも同じ結果になるものは実害なし＝§5-3-4‴ の引き算）②`collectTurnTriggers` にゲートを足す③parser がトリガー句から `turnOwner` を出す④**成立/不成立の両方向**を golden で固定（⚠**ゲートを足した瞬間に「自分のターンにしか発火しない」へ倒れる**ので、相手ターンの効果が消えないことを対照で確かめる）。■**関連**＝第43バッチで触った `WXDi-P03-065-E1`（「あなたのターン終了時」）と `WXK01-042-E1`（「あなたのターンに」）の finding はこの軸なので**台帳では閉じていない**。 |
