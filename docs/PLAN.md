...
- **🆕 セッション（2026-07-13・続き110・Fable 5・戦略②「純P1の系統バッチ消化」開始＝効果単位クラスタ上位2系統35効果を是正・census 2264→2229）**
  ...
  - **次の一手＝戦略②の継続**。入口＝`npm run census:clusters` の上位から次の系統を取る（残上位＝**「Nまで」上限選択12「その中からカードをN枚まで手札に加え…」・条件節「あなたのターンの場合」11〔⚠IS_MY_TURN プレースホルダ常時真＝engine拡張が先〕・公開し8「その中から＜C＞のシグニN枚を公開し手札に加え」・次相手ターン終了時まで8**）。⚠採用はカード単位（effectId→CardNum 変換）・手修正温存カードは外科パッチで取りこぼさない・`_effect_srctext.json` は build:effects の副産物（CSV/parser 変更後は再生成必須）。
### 📊 恒久指標（維持中・逐次更新）
