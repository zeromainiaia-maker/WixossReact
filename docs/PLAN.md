...
**直近＝2026-09-22＝🏁§5.7 `S-29`＝宣言の帰結のコスト（払える対象だけを候補にする）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第450バッチ）
- 📏**母集団を測った**（登録票は未計測）＝「それのレベル１につき〈コスト〉」の族は**キーが6つ**あり、和集合で**live 14効果 / 14カード**。
  🔴**ユーザー作27デッキには0枚**＝**A/B でも自己対戦でも測れない**（`S-20` と同型）。
- 🔴**症状を実カードで再現**＝`WXDi-P04-020-E1`／CPU のエナ2枚・相手にレベル5とレベル2 ⇒
  **旧はレベル5を宣言して空振り／新はレベル2を選んで通る**。⚠**エナ5枚なら従来どおり強い方**（`S-22` を壊していない）。
- 入れたもの＝🆕`cpuDeclarationCost.ts`（効果の木から倍率コストを読む＋払えるか）＋`pickCpuTargets` は**払えない候補を外すだけ**。
  🔑**払える候補が0なら絞らない**＝悪化させない。⚠配線は**本番の応答と探索の先読みの2か所**（片方だけだと両者が別の対象を選ぶ）。
- 🔑**登録票のもう半分（`POWER_SET` 230ノードが `unknown`）は `S-22` の置き場フォールバックで解消済み**＝作業不要だった。
- 🔧**実機 PASS**（両デッキ）＝**どちらも `dist` 作り直し直後の初回**＝第449バッチのじゃんけん修正（`V-284`）が効いている。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-6`**（機械学習＝第1〜2段は消化済み・微調整は差が無いと測り切った）。他は在庫の `V-284` と入口2効果 |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／**実機 `V-nn` 1**（`V-284`＝じゃんけんの回帰ガード）／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 1**（🔵`S-6`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4368**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
- **2026-09-22 時点**（第421〜450バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
