...
2. **census「動的比較 35枚」**（「〜より高い/低い」＝heterogeneous・per-card）。**🔸続き43（Opus）で自己参照9枚＋トリガー参照6枚 着地（powerLt/Gt/levelLt/GtSelf・powerLt/levelLt/GtTrigger・engine 解決を resolveDynamicFilter に集約＝sourceCardNum/triggeringCardNum 引数・trash→field ビルダーにも parseTriggerComparison 配線・census 1631→1624）**。**✅続き44（designation post-pass 6枚）・続き45（`powerLtAnyAlly` 2枚）・続き46（`powerLtPrinted`/`powerGtPrinted` 2枚）・続き47（`powerLtLastProcessed`/`levelLtLastProcessed`＝lastProcessed 個別機構2枚＝WXDi-P08-031/WXK10-031）で消化継続**。残＝(b)opp/own センタールリグ（WX19-042 は target が LRIG に誤パース＝mis-parse 修正が先／WXK11-003）・デッキ相対 SEARCH（WDK13-013/WXEX2-28/WXEX2-37/WXK10-033）・条件文（WXK07-025/WXK08-005）＝いずれも別機構。詳細 BUGFIXES 最上部。
...
- **🆕 セッション（2026-07-09・続き52・Opus 4.8・§5c census 較正＝「制限『できない』」の使用条件 41枚の偽陽性クリア）**＝census「制限『できない』」高シグナル58枚を機械分類し、**41枚は「この能力は〔条件〕の場合にしか使用/発動できない」＝使用条件（useCondition・`eff.condition` で既に正しく表現済み）の偽陽性**と判明（`extractUseCondition`→`parseUseCondition` が LRIG_STORY/SELF_POWER_GTE/HAS_CARD_IN_FIELD{crossState} 等へ解析済み・census keys が BLOCK/PREVENT のみで useCondition を計上しないため過剰計上）。REVEAL_AND_PICK extraOk（続き24）と同型の**census-only 較正**（カード JSON/parser/engine 不変）。**分類内訳**＝偽陽性41／真バグ0（extractUseCondition は完璧）／効果制限〔アタック/場に出せない等＝BLOCK 要〕14／その他3。**実装**＝`scripts/vocabCensus.ts` の pattern に `extraOk`（原文が「しか使用/発動できない」を含み**かつ効果制限を含まず** JSON に condition があれば covered＝真バグを masking しない安全設計）。**効果**＝制限「できない」高シグナル58→17・**census 1616→1588（dedup -28）**・`BASELINE_HIGH` 更新。**検証**＝census 1588/1588・golden 173/smoke 全0/fuzz 全0/lint 0 errors/同型★0（census-only で JSON/decompile 不変・全ゲート緑）。**次の一手＝(A)Opus＝effect-restriction 14枚の実装**（「対戦相手はシグニをN体までしか場に出せない」＝相手シグニ配置数制限の新機構〔WXDi-P05-024/WXK05-009/WX07-006/WX12-008〕・「〜ないかぎり新たに場に出せない」自己配置制限〔WX14-033/WX18-075〕・アタック/ガード不可＝§6 BLOCK 機構）。**(B)Opus＝§5c 条件節の残**（「レベルNの覚醒状態のシグニがある場合」3枚＝HAS_CARD_IN_FIELD に awakened 状態フィルタ要・WX21-016 の「その後」→IS_MY_TURN 先取り解消・色付きアーツ/スペル使用歴）。**(C)Opus＝§4タスク2 動的比較 lrig相対 WX19-042 mis-parse・タスク3 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY。(D)Sonnet＝§5c 再収穫・§7 実機検証（続き43-51 の採用効果）。(E)持ち越し：SEQUENCE 下流 IS_MY_TURN 常時真連鎖精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDrive バッチ状態汚染。**＝「対象処理→それが〔属性〕の場合、追加で〔ボーナス〕」型で直前処理対象の属性ゲートが脱落し追加効果が無条件発火していた過剰効果を是正。engine 既存 `LAST_PROCESSED_MATCHES{filter}`（`execUtils.ts:1067`）へ parser 持ち上げ。**新規則**＝「`(その後、)?それが〔色(か色)?/＜C＞(か＜C＞)?〕の(シグニ|カード)の場合、追加で〜`」→CONDITIONAL{LAST_PROCESSED_MATCHES}（色/＜C＞とも OR）。**⚠「追加で」を必須にして REVEAL_AND_PICK〔「それを手札に加える」＝filter 表現済み〕と区別＝偽陽性回避**。lastProcessedCards 伝播は SEQUENCE step間（`:2389`）＋SELECT_TARGET continuation（`resumeSelectTarget:4132→4150`）で確認済み。**採用2枚**＝WX21-011（敵ダウン→白か赤なら2ドロー）・WX21-014（敵エナ送り→赤か青ならデッキトップをライフへ）。**不採用**＝WX21-007（「それをトラッシュ」が owner:any 誤パース＝honest STUB より悪化・targetsLastProcessed 解決要）・WX21-016（既存「その後」→IS_MY_TURN 変換に先取りされ over-fire 継続）・SP26-007（既に条件表現済み）。**検証**＝typecheck・**golden 172→173（+1＝DOWN した敵の色で追加効果ゲート＝lastProcessedCards 伝播の end-to-end ガード）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1616/1616**（2枚は census 高シグナル計上外・条件脱落は是正・BASELINE 変更なし）。**次の一手＝(A)Opus＝§5c census 条件節の残**（(a)「それが〔レベル/レゾナ〕の場合、追加で」の残・WX21-016 の「その後」→IS_MY_TURN 先取り解消〔over-fire 中〕・WX21-007 の「それ」＝lastProcessed トラッシュ解決・(b)色付きアーツ/スペル使用歴〔engine 追跡新機構〕・(c)「あなたの場にレベルNの覚醒状態のシグニがある場合」3枚〔HAS_CARD_IN_FIELD に awakened 状態フィルタ要〕・(d)「それがレゾナの場合、代わりに」＝WX25-P2-068/070 の 代わりに 置換）。**(B)Opus＝§4タスク2 残（動的比較 lrig相対 WX19-042 mis-parse）。(C)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚 GRANT_QUOTED_AUTO_ABILITY。(D)Sonnet＝§5c 再収穫・§7 実機検証。(E)持ち越し：SEQUENCE 下流「そうした場合」IS_MY_TURN 常時真連鎖の精緻化・色付きアーツ・PR-Di038 duration・WX25-P2-095・verifyBattleDrive バッチ状態汚染。**＝続き48（覚醒）と同じ自己状態条件ファミリの小口消化。「このシグニがアップ状態の場合、〜」の向き状態ゲートが脱落していたのを、engine 既存 `THIS_CARD_IS_UP`/`THIS_CARD_IS_DOWN`（`execUtils.ts:845/852`・`signi_down`）・decompiler（既存）へ **parser の局所 CLAUSES 表に2行追加**して是正。**採用2枚**＝WXDi-P02-038（アップ→敵エナトラッシュ）・WXDi-P04-036（アップ→手札3枚捨ててもよい）。他のアップ札（WX15-055/056＝「正面のシグニがバニッシュされたとき」複合トリガーで別処理・ターン終了時公開系）は捕捉外。ダウンは該当0（WX18-052 は「しか発動しない」使用条件で別）。**検証**＝typecheck・**golden 171→172（+1＝アップ札はアップ中のみ・ダウン札はダウン中のみ発火の相互 no-op ガード）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1616/1616**（2枚とも別の高シグナル語彙が残り dedup 不変・条件脱落自体は是正・BASELINE 変更なし）。**次の一手＝(A)Opus＝§5c census 条件節の残**（(a)「それが〔filter〕の場合」の真の条件脱落分〔WX21-011/014 の DOWN 後 それが色→追加ドロー＝LAST_PROCESSED_MATCHES 持ち上げ・⚠REVEAL_AND_PICK 偽陽性は触らない〕・(b)色付きアーツ/スペル使用歴〔engine で色/使用追跡が要る新機構・無色アーツは配線済〕・(c)「あなたの場にレベルNの覚醒状態のシグニがある場合」3枚〔HAS_CARD_IN_FIELD に awakened 状態フィルタ要〕）。**(B)Opus＝§4タスク2 残（動的比較 lrig相対 WX19-042 mis-parse）。(C)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚 GRANT_QUOTED_AUTO_ABILITY。(D)Sonnet＝§5c 再収穫・§7 実機検証。(E)持ち越し：SEQUENCE 下流「そうした場合」IS_MY_TURN 常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDrive バッチ状態汚染。**＝続き48（覚醒条件）に続く条件節クラスタ。「【自】：あなたのアタックフェイズ開始時、**あなたの場にあるすべてのシグニが＜C＞の場合**、〜」型で、**場の全シグニが同クラス**という盤面ゲートが全数脱落し無条件発火の過剰効果になっていた。続き48 が engine 既存条件の parser 配線のみだったのに対し、今回は**新 condition 型を型＋engine から新設**。**型**（`effects.ts`）`ALL_FIELD_SIGNI_MATCH{owner,filter}`＝場の全シグニ頂点が filter 一致かつ1体以上（空盤面 false＝空振り発火しない）。**engine** `execUtils` evalCondition＋`effectEngine` evalConditionForContinuous の2評価器に同型分岐（CONT 用は将来の default:true 過剰発火予防・現20枚は全 AUTO）。**parser** 局所 CLAUSES 表に2変種＝`＜C＞`→story（CardClass.includes 慣例）、`《X》`→**《ディソナアイコン》は isDisona:true（Story='Dissona'）**・他は cardName（⚠ディソナをカード名化すると永久 false の逆バグ＝回避）。**decompiler** condJa に描画追加。**採用20枚**（署名 `+ALL_FIELD_SIGNI_MATCH +CONDITIONAL` 単一＝純加算・退化なし）＝WX25-CP1-038/042/044・WX26-CP1-048/052/054/058・WXDi-CP01-032・WXDi-CP02-055/058/075/076/080・WXDi-P12-007/009/080・WXDi-P13-010/063/067/073。配置＝top（全体ゲート・大半）/SEQUENCE step[0]（原文で条件を持つ文・下流は「そうした場合」IS_MY_TURN 連鎖 or reminder）/CHOOSE 分岐。**⚠SEQUENCE 下流の「そうした場合」常時真連鎖・WXDi-CP01-032 の nested「レベル合計7の場合」脱落は本変更と独立の既存の不完全さ（別課題）。OPTIONAL_COST STUB は本20枚に無し（discard は plain TRASH＝続き48 WX25-P2-095 のような結合破壊リスクなし）。**検証**＝typecheck・**golden 170→171（+1＝全一致→発火/混在→no-op/空盤面→no-op）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1621→1616（-5・SEQUENCE 下流条件が残るカードは高シグナル継続）**＝`BASELINE_HIGH` 実数更新。**次の一手＝(A)Opus＝§5c census 条件節クラスタの継続**（`docs/_census_clusters.txt`＝(a)「それが〔filter〕の場合」系のうち**真の条件脱落分のみ**〔WX21-011/014 の DOWN 後 それが色→追加ドロー等＝LAST_PROCESSED_MATCHES 持ち上げ。⚠デッキトップ公開→それが〔filter〕→手札 の REVEAL_AND_PICK 系は filter で表現済みの偽陽性＝触らない〕・(b)「このターンに〔色〕のスペル/アーツを使用していた場合」turn-history 条件〔ARTS_USED 拡張〕・(c)「このシグニが〔覚醒/アップ/ダウン〕状態の場合」の残・(d)「あなたの場にレベルNの覚醒状態のシグニがある場合」3枚 等）。**(B)Opus＝§4タスク2 残（動的比較 lrig相対）**：WX19-042 は「センタールリグ」に釣られ target が LRIG に誤パース＝mis-parse 修正が先。**(C)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）GRANT_QUOTED_AUTO_ABILITY 内側parse。(D)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43-44の8枚）。(E)持ち越し：SEQUENCE 下流の「そうした場合」IS_MY_TURN 常時真連鎖の精緻化（続き49 20枚含む多数）・PR-Di038 duration under-representation・WX25-P2-095・verifyBattleDrive バッチ状態汚染（続き39から）。**
