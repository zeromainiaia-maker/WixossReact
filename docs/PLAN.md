...
**Opusタスク12＝未消化の在庫 5件（2026-08-10 続き404・408・417／2026-08-13 続き466 で登録）**。
...
- **✅ 続き431（§6.4 ライフクラッシュ置換）が持ち込んだ未検証UI 3件＝2026-08-13 続き465 で決着**（Codex 起案→Claude 実機検証・**7シナリオが2回連続PASS**・既定 order 登録済み・差し戻し0/是正0）。engine は golden で固定していたが、**ダメージ解決の分岐**は実機でしか踏めなかった。⚠**3枚とも従来「生きた過剰効果／自傷」だった**ので、**まず退化していないこと**から見た。
  ...
  - [ ] 📋**未対応として残す**＝`crash_opponent` × **ルリグアタック**（`BattleScreen.tsx:10799` は `kind==='mill'` しか見ない）。今回は触っていない。
  - [ ] 📋**未実装として送る（バグではない）**＝「〜してもよい」の**辞退がまだできない**（自動適用＝`screens/battle/lifeCrashReplace.ts:24-25`）。原文の「デッキがN-1枚以下の場合は置き換えられない」が効くのでデッキアウトはしない＝対話化は離場置換 M2 と同じ枠組みで別バッチ。

- **🔴 続き430（§6.4 離場置換の対話化）＝2026-08-13 続き466 で実機検証し、**実バグ2件を検出**（→§3 **(cxxv)**／**(cxxvi)**。ルールどおり engine は触らず在庫化）**。シナリオ6本を新設（Codex 起案→Claude 実行）＝**2本 PASS／4本は (cxxv) を検出して FAIL のまま既定 order に置く**（修正されたら緑に反転する生きたトリップワイヤ）。
  - [x] **`count:'ALL'` 経路は完全に正しい**＝`leaveSubAllTargetsAskedPerVictim` **PASS**。**体ごとに1回ずつ問い**（`asks=2`）・応答者は**被害側 CPU**（`responder=00000000-…-0001`）・選択肢は `[banishSubstitute:…:代わりに<カード名>をバニッシュする, none:置換しない]`・**1体目の決定後も全3体が場に残る**（＝「適用前に全部聞く」設計が実機で効いている）・全応答後にまとめて移動。
  - [x] **候補が無ければ問わない**＝`leaveSubNoOptionMeansNoAsk` **PASS**（犠牲＜電機＞を外すと問いなしで victim が通常バニッシュ＝エナへ）。**A1 の対照として必須**。
  - [ ] 🔴**数値 count の効果バニッシュでは問いが1度も出ない**（→§3 (cxxv)）＝`leaveSubCpuAutoRespondsSubstitute`／`leaveSubAskDirectedToVictim`／`leaveSubDecisionNoneIsHonored`／`leaveSubDecisionKeyIsHonored` の4本が FAIL。**engine が従来どおり自動適用**し、注入した決定も honor されず消費もされない。
  - [ ] 🔴**相互身代わりでインスタンスがエナに複製**（→§3 (cxxvi)）。
  ...
  - ⚠**この検証で潰したシナリオ側の偽陽性3種（次に書く人へ）**＝①**`pendingEffect==null && stackLen===0` は「効果の開始前」と「完了後」の両方で true**＝発動クリック直後に判定すると「1度も解決していない盤面」を読む（**効果が走り出したことを1度でも観測するまで判定させないゲート**を入れる）②**`pending?.a === pending?.b` は pending が null のとき `undefined===undefined` で true**＝1周目に誤検出して即 FAIL ③🔑**`H.stdStep` の盲目的な `pick-0` で対象を選ばない**＝`WX19-023-E3` は候補が2体あり、**犠牲の方を直接バニッシュした盤面は身代わり成立時と1バイトも変わらない**（`pendingCandidates` から instanceId の index を引いて `pick-<idx>` を押す）。⚠**さらに「盤面だけを見る判定」も偽陽性**＝**対話が1度も出なくても同じ盤面になる**ので、機構の検査には **`asks===1`（問いログ）を必須条件に入れる**。
  - 以下は当初の検証項目（(cxxv) の解消後に再実行する）。
