...
1. ~~**`GRANT_TO_PLACED_SIGNI` の実装**（「この方法で場に出たシグニは…を得る」＝targetsLastProcessed 機構・§6.3）~~ **✅続き42（Opus）で一次完了**＝parser で「この方法/効果で場に出たシグニは【K】を得る／のパワーを＋N」を `GRANT_KEYWORD`/`POWER_MODIFY`{targetsLastProcessed}（engine 機構は既存）へ振り分け、WX25-P1-044/WX25-P2-039（アサシン）・WX24-P3-037（+3000・次相手ターン終了時まで）を STUB から実アクション化。**残＝引用複合能力付与2枚（WX24-P1-017/WX25-P3-038＝「「【自】…」を得る」＝GRANT_QUOTED_AUTO_ABILITY 系の内側ability parse が要る）＋レベル比例ミル1枚（WX24-P3-039＝「レベル１につき…トラッシュ」＝別スケール機構）は honest STUB 温存**（§6.3）。詳細 BUGFIXES 最上部。
...
- **🆕 セッション（2026-07-07・続き42・Opus 4.8・筆頭 `GRANT_TO_PLACED_SIGNI` を一次実装）**＝続き36から3セッション持ち越しの Opus 筆頭タスクを着地。dual-pick/reveal-pick が場に出したシグニ（`lastProcessedCards`）へ「この方法/効果で場に出たシグニは…」で付与する系統を、**engine 既存の `targetsLastProcessed` 機構**（`GRANT_KEYWORD`/`POWER_MODIFY`/`GRANT_EFFECT`＝選択UIなしで lastProcessedCards へ適用・実装済み）へ **parser で振り分け**て honest STUB を実アクション化。`parseSingleSentenceInner`（`src/data/effectParser.ts`）の旧「全文 STUB 化」分岐を、duration 前置捕捉＋本体3分岐に：**(A)単一キーワード「は【K】を得る」→`GRANT_KEYWORD{targetsLastProcessed}`**（引用「」・変種（）は除外）、**(B)「のパワーを＋N」→`POWER_MODIFY{targetsLastProcessed}`**、**(C)引用複合能力「「…」を得る」・レベル比例「のレベル…」は honest STUB 温存**（⚠greedy `.+` の末尾句点混入を `body.replace(/。+$/,'')` で回避）。`build:effects`→`heldReview --adopt` で **WX25-P1-044/WX25-P2-039（アサシン・UNTIL_END_OF_TURN）・WX24-P3-037（+3000・UNTIL_OPP_TURN_END）の3枚採用**。corpus 全走査で新分岐の発火は上記3枚＋副次 **WX24-P1-026（ランサー＝既存 JSON 手パッチと fresh 一致＝parser/JSON parity 回復・held減）** のみ＝誤爆0。**検証**＝typecheck・**golden 154/0（+3新規＝engine機構2＋構造固定1）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**（regen 済）。**census 1628→1631（+3）**＝回帰ではなく STUB を外したことで blanket STUB 免除を失い、アサシンのリマインダ文/「Nまで」pick 語彙が顕在化した偽陽性（実効果は全て表現済み）＝`BASELINE_HIGH` 実数更新。詳細 BUGFIXES 最上部・§4タスク1・§6.3。**次の一手＝(A)Opus 残タスク＝GRANT_TO_PLACED_SIGNI の残 honest STUB 3枚（引用【自】能力付与 WX24-P1-017/WX25-P3-038＝GRANT_QUOTED_AUTO_ABILITY 系内側parse／レベル比例ミル WX24-P3-039＝lastProcessed レベル総和スケール）・census「動的比較35枚」・引用内CHOOSE（WXDi-D09-P20）。(B)Sonnet 側は §5c 再収穫サイクル（今バッチ着地で再収穫の余地・`/census-batch`）＋§7 残る実機検証（R30/R31/R36/R39-R46）＋R38②《ターン1回》追検証。(C)verifyBattleDrive のバッチ状態汚染の根本修正（driver 分離強化）は続き39から持ち越し未着手。**
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 1631枚（2026-07-07 続き42・履歴 …→1650→1648→1645→1637→1628→1631・⚠1628→1631 の+3は回帰ではなく続き42 GRANT_TO_PLACED_SIGNI 実装で3枚が STUB 免除を外れリマインダ文/pick 語彙が顕在化した偽陽性＝BASELINE 実数更新・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。状態系の残（凍結13・ダウン/アップ38）はコスト節/条件/CONT型（別パス・§6.3）。**消化の入口は `npm run census:clusters`＝文型テンプレのクラスタ表（`docs/_census_clusters.txt`・枚数順）から系統バッチを選び、parser規則→`npm run build:effects`→`node scripts/heldReview.mjs` で署名グループごとに一括採用する（続き23確立・手順詳細は §5c）。**
...
- **GRANT_TO_PLACED_SIGNI 残（続き42で一次実装）**＝「この方法で場に出たシグニは【K】を得る／のパワーを＋N」は parser で `GRANT_KEYWORD`/`POWER_MODIFY`{targetsLastProcessed} へ実装済み（WX25-P1-044/WX25-P2-039/WX24-P3-037）。**残る honest STUB 3枚**＝(a)引用複合能力付与：WX24-P1-017（「【自】バトルバニッシュ時、自アップ＋能力喪失」）・WX25-P3-038（「【自】アタック時、パワー8000以下を手札／能力なしならトラッシュ」）＝内側【自】ability の parse（GRANT_QUOTED_AUTO_ABILITY 系＝現状 log-only STUB）が要る。(b)レベル比例ミル：WX24-P3-039（「場に出たシグニのレベル１につき相手デッキ1枚トラッシュ」）＝lastProcessedCards のレベル総和でスケールする MILL 機構が要る。
