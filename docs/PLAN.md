...
- **🆕 セッション（2026-08-07・続き376・Opus 5〔**§5d-0 工程改善3件の完遂 → 新計器で配線ギャップ5バッチ＋計器較正3バッチ**〕）＝ユーザー指示「PLAN 確認して、①被覆マトリクス常設化 ②真バグ率の測り直し ③worklist を作業種別へ ④§7 を並行」→ 以後「続ける／順番に行う」で連続消化。**触った層＝`scripts/censusWiring.ts`（新設）／`scripts/vocabCensus.ts`（較正）／`src/data/effectParser.ts`・`parserUtils.ts`・`parsers/parseSentencePart1.ts`（配線）／`scripts/goldenTest.ts`／live JSON（held採用＋外科パッチ）／`scripts/verifyBattleDrive.mjs`（§7・Sonnet）。**
  - **✅工程改善3件（§5d-0）＝全部完了。**①**`npm run census:wiring` を新設**（`scripts/censusWiring.ts`・明細 `docs/_census_wiring.txt`）＝(語彙キー × アクション入口) の被覆マトリクス。試作値を再現したうえで**語彙表を32キーへ拡張し miss 541 を検出**。②**真バグ率を測り直した＝14/20＝70%**（続き369 の 92% から低下・95%CI 50〜90%）＝**残りの実バグは約810件**。測定器は `scripts/archive/censusSample.mjs`（シード固定・再現可）。③**worklist を作業種別4区分**〔(i)配線ギャップ (ii)機構ギャップ (iii)構造混線 (iv)計器較正〕へ組み替え（§5d-0 末尾が以後の正）。
  - **✅(i) 配線ギャップ 5バッチ＝66効果を是正**。①`ON_ATTACK_SIGNI` の主語「あなたの[＜X＞の]シグニN体が」16効果（**過小実行**＝scope 既定 self で自身のアタックしか拾わなかった）②BOUNCE 対象の ＜クラス＞ 3効果 ③トラッシュ→デッキ一番下の source ＜クラス＞ 17効果 ④「あなたの〈filter〉シグニを対象とし…それのパワーを±N」18効果（**owner ごと**落ちて相手のシグニにも撃てた）⑤**ディソナ判定が `keyword` 文字列**で両方向に外れていた12効果。
  - **✅(iv) 計器較正 3バッチ＝census −63（実装ゼロ・バグ修正ではない）**。`LOOK_PICK_CHAIN` の「Nまで」（−25・**一括免除ではなく `extraOk` の残渣チェック**で実装）／`FORCE_PLACE_FRONT`（−4）／`COUNTER_SPELL.maxCost`（−2）／**`trash_key`（−31＝1クラスタの高シグナル34件が全部これ1つ）**／条件節の filter 等価表現（−1）。**較正のたびに消えた効果を全件目視し「新規に高シグナルへ出た効果0件」を機械確認済み。**
  - **✅(§7・Sonnet 並行)**＝続き366 の**コイン支払い累計機構**（`coins_paid_this_turn`／`COINS_PAID_THIS_TURN`）を `WXDi-P15-068-E1` で実機検証。`coinsPaidAttackFires`（成立）／`coinsPaidAttackSkipped`（不成立＝退化再発検出）を各2回連続PASS・既定orderへ追加（126→128件）。新規 engine/parser バグは無し。
  - **🔎最大の発見＝census の5つ目の死角「語彙はあるが判定器が違う」**（§5c 末尾の4項に追加）。第5バッチは **census が 1061 のまま1も動かないのに 12効果が実バグ**だった＝`filter:{keyword:"ディソナアイコン"}` は `matchesFilter` が**EffectText の印字**を見るので、CSV の `Story` 属性であるディソナには**両方向に外れる**（98枚中38枚を取りこぼし／非ディソナ17枚を誤ヒット）。**語彙突き合わせ型の計器は原理的に検出できず、被覆マトリクスだけが映した。**
  - **⚠繰り返し踏んだ罠3つ（次の人は必ず読む）**＝(a)**素の `parseStoryFilter(文全体)` を対象フィルタに使わない**＝A/B で7件中4件・20件中3件が誤配線（コスト側／条件節／個数参照／別の対象のクラスを引き込む）。**対象名詞句に隣接**（間に別の「シグニ」を挟まない）ときだけ拾う。(b)**曖昧なら付けない**＝span 内にクラスが2つ以上（「＜A＞と＜B＞」＝別ピック／「＜A＞か＜B＞」＝OR）なら片方だけ載せると**原文と逆の過小実行**になるので**既存の過剰効果を残すほうを選ぶ**。(c)**誤った中間ビルドは live JSON に焼き付く**＝`build:effects` は非破壊マージなので parser を直して回し直しても消えない。**parser を触る前に必ず live JSON をスナップショットし、A/B 差分を全件読んでから次へ進む。**
  - **⚠「他の」ゲートの穴は再発パターン**＝第1バッチ（`ON_ATTACK_SIGNI` 主語）と第5バッチ（`parseSigniTarget` の `isDisona`）が同型で、どちらも `hasOtherSelfSigniNoun` で gate されていた。**`hasOtherSelfSigniNoun` の使用箇所を洗うと同じ穴が出る可能性が高い**（次の一手候補）。
  - **⚠大セルはそのままバッチにならない**＝`cardClass × SIGNI[filter]`（miss 43）を機械分類すると**①対象フィルタの真の脱落 11／条件節（クロス計上）9／別ゾーン指定（クロス計上）13／要目視 10**。**miss 数は「掘る価値の指標」であって「見込み件数」ではない**＝`--cell` で原文を読んで①だけを抜くのが正しい使い方。
  - **ゲート**＝全緑。**golden 1419→1430**（+11＝各バッチの固定＋**両方向のトリップワイヤ**）、**census 1162→1061**（−101・`BASELINE_HIGH` 更新。内訳＝**機能是正 −38／計器較正 −63**）、**被覆マトリクス miss 541→483**、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/248 warnings（**追加0**）、manual field loss 0、held **229→213枚／104群**、実機 driver 既定order **126→128件**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-07 続き376（§5d-0 工程改善3件の完遂 → 配線ギャップ5バッチ＋計器較正3バッチ）後 最新値（本行が直近の正）**：golden **1419→1430**（+11）、census **1162→1061**（−101・`BASELINE_HIGH` 更新。**内訳＝機能是正 −38／計器較正 −63**＝**この2つは別勘定で読むこと**）、**被覆マトリクス miss 541→483**（`npm run census:wiring`・新設）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **229→213枚／署名グループ 104件**、実機 driver 既定order **126→128件**。**是正した効果は計 66**（配線ギャップ5バッチ）＝**census の減少幅と一致しない**（較正ぶんが混ざり、逆に第5バッチは census が動かないまま12効果を直した）。⚠**残す教訓**＝(a) **素の `parseStoryFilter(文全体)` を対象フィルタに使わない**（A/B で7件中4件・20件中3件が誤配線＝コスト側／条件節／個数参照／別の対象のクラスを引き込む）＝**対象名詞句に隣接**するときだけ拾う。(b) **曖昧なら付けない**＝span 内にクラスが2つ以上（「＜A＞と＜B＞」＝別ピック／「＜A＞か＜B＞」＝OR）なら片方だけ載せると**原文と逆の過小実行**になるので**既存の過剰効果を残す**ほうを選ぶ。(c) **誤った中間ビルドは live JSON に焼き付く**＝`build:effects` は非破壊マージなので parser を直して回し直しても消えない＝**parser を触る前に live JSON をスナップショットし、A/B 差分を全件読む**。(d) **held の署名グループ「（type増減なし＝値/構造変更）」は102枚の受け皿**＝`--adopt-sig` を使ってはいけない。採用前に「差分が意図した箇所のみ」を機械確認する。(e) **census の5つ目の死角＝「語彙はあるが判定器が違う」**（`keyword:"ディソナアイコン"` は印字ベース判定でディソナ属性に両方向で外れる）＝**語彙突き合わせ型の計器は原理的に検出できない**。 旧: 2026-08-07 続き375（§5d パターンA 第7バッチ＝続き374 のブロッカー2件を実装）後 最新値（本行が直近の正）**：golden **1418→1419**（+1＝エナ対象のゾーン是正3件／`WD15-018` の条件節／evalCondition のルリグ色解決）、census **1164→1162**（−2・`BASELINE_HIGH` も 1162 へ更新＝**機能是正**。この系統は OK **33→37**／未配線 5→1）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **229 据置／署名グループ 104件**。**live JSON の変更は4効果／4カード**（すべて held 経由で採用）。⚠**残す教訓**＝(a) **ブロッカーを潰すより、既に正準な別の形へ寄せられないか先に見る**＝`SELECT_TARGET_ONLY` 方式は engine 拡張3点（うち1つは循環 import）が要るのに得るのは選択タイミングの前後だけで、同族30効果が使う素の `TRASH{ENERGY_CARD,...,filter}` に寄せれば parser だけで済んだ。(b) **名詞句 span は名詞まで含めて捕る**＝直前で切ると `LRIG_COLOR_NOT_RE` 等が外れ、**ゾーンだけ直って色制限が黙って落ちる**。A/B 差分の中身を必ず読む。(c) **条件節の新規則は `STATE_CONDITION_CLAUSES_V2` に足す**＝「…を対象とし、」プレフィックスを許すループが spread しているのは V2 だけで、`STATE_CONDITION_CLAUSES` に足すと**黙って効かない**。(d) **条件内の動的フィルタは条件評価器側でも解決が要る**＝`matchesFilter` は `colorNotMatchesLrig` を知らないので、未解決だと**黙って無視され過剰条件**になる。**「型にあるキーを条件に入れた」だけでは効かない。**
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き376d＝(i)配線ギャップ 第1・2バッチ＋(iv)計器較正 第1〜3バッチ後）**＝golden **1430**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1061**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **229枚／104群**・実機 driver 既定order **128件**（続き376 の §7 コイン支払い累計2本を追加）。**被覆マトリクス miss 483**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第6バッチ。候補は下の2つで、①を推す。**
>    - **① `hasOtherSelfSigniNoun` ゲートの棚卸し（推奨・新発見の系統）** — 続き376 で消化した**第1バッチ（`ON_ATTACK_SIGNI` の主語抽出）と第5バッチ（`parseSigniTarget` の `isDisona`）は同型の穴**で、どちらも「**『他の』があるときだけ**修飾を拾う」形に gate されていた。`grep -rn "hasOtherSelfSigniNoun" src/data/` で使用箇所を全部出し、**「他の」の有無ではなく対象名詞句かどうかで判定すべき箇所**を洗う。1箇所直すたびに十数効果が動く見込み。
>    - **② `isDisona × TRASH_CARD[filter]`（miss 7・has 1）ほか残セル** — `npm run census:wiring` の★印から。⚠**大セルはそのままバッチにならない**（`cardClass × SIGNI[filter]` は miss 43 のうち本命11・残りはクロス計上）＝必ず `--cell` で原文を読んで仕分けてから取る。
>    - ⚠**着手前に必ず読む3つの罠**（続き376 で実際に踏んだ）＝(a) **素の `parseStoryFilter(文全体)` を対象フィルタに使わない**（A/B で7件中4件・20件中3件が誤配線）＝**対象名詞句に隣接**するときだけ拾う (b) **曖昧なら付けない**（クラス2つ以上の span は片方だけ載せると**原文と逆の過小実行**） (c) **parser を触る前に live JSON をスナップショット**（`build:effects` は非破壊マージなので誤った中間ビルドが焼き付いて消えない）。
>    - **(iv) 計器較正の残**＝条件節クラスタは**279件が269テンプレ＝ほぼ単発**で較正クラスタではないと実測済み（第3バッチの収穫は −1）。**較正で大きく減る余地はほぼ枯れた**＝以後は (i) 中心でよい。
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **4件**（数が確定・全数潰せる）／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**（IDは §5d-0 (ii) に列挙済み）。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。続き376 でコイン支払い累計を消化したので、**次は §7 の残りリストを上から**。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない（グロウ／ベット／アンコール／【出】コストは未検証）。
>    - ⚠**見積もりの現在地**＝残 census 1061／実バグ率 70%（続き376 実測・較正で母集団が締まったぶん**現在は約75%**）＝**実バグ約810件は不変**（較正は偽陽性を抜いただけで実バグを減らしていない）。**続き376 は5バッチで66効果**＝1バッチあたり13効果。バッチ収穫は逓減していない（16→3→17→18→12）＝**被覆マトリクスが効いている。**
