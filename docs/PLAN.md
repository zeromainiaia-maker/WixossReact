...
- **🆕 セッション（2026-08-05・続き354・Sonnet 5〔§7実機検証の続き＝タスク12(lxiii)(a)(b)を消化〕）＝ユーザー指示「実機検証を続ける」で§7次の一手の7番目の項目（タスク12(lxiii)(a)選択肢の可否表示／(b)中央ゾーン限定のピッカー）を検証**。**engine/parser/JSON は無変更**（golden/census/held/smoke/fuzzは前回値からすべて据置＝1366/1288/257枚/10679/0）。触った層は`scripts/verifyBattleDrive.mjs`（新規シナリオ3件追加・全件既定orderへ＝既定order 112→115件）・`docs/PLAN.md`のみ。
  - **✅実機検証クローズ1件（2項目）**＝`wx17040ConditionsFalseNoop`／`wx17040ConditionsTrueExecuteAll`／`centerZoneOnlyPicker`で各2回連続PASS。(a)`WX17-040-E1`＝①②の`choice.condition`（`HAND_COMPARE_OPP`／`ENERGY_COMPARE_OPP`）がCHOOSEの`available`自体を決めること（3条件不成立で①②disabled／3条件成立で①②enabled）、③（`ch.condition`無し＝常にavailable）は選べるが条件不成立なら対象選択にすら進まず静かに無効果、成立時は3つ選択→ドロー＋エナチャージ＋（SELECT_TARGETを経て）バニッシュが全実行、を実機で確認。(b)`WXDi-P02-065-E2`＝対戦相手の場を左中右すべて埋めても、`centerZoneOnly:true`のSELECT_TARGET候補は中央（zone1）の1体だけに絞られる（候補数=1を実測）ことを確認。新規実バグは0件。
  - **次の一手**＝**Sonnet**＝§7「残る実機検証項目」はまだ残る（上から＝併記型(c)／ON_LRIG_GROW④横グロウ経路／(xi)(xxxvi)のUI検証／クラフトトークン実機配置／lrigDownコスト限定2件／driver側低頻度フレーク）。次に着手するならこの並び順の上から。**Opus**＝タスク12の生き残り在庫〔(lv)残2経路／(lxvi)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／(xcvii)／(c)〕＋(ci)〜(cvii)（(cv)は続き353でopponentHandDiscard系への波及を追加確認）。
