...
**直近＝2026-09-17＝§5.7 `S-2` クローズ＝CPU デッキごとの作戦データ（キーカード・優先して出す札・コンボ）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑ユーザー指摘「使うデッキによって強い行動が変わる」「コンボが強い」＝`decks.cpu_plan`（ユーザーが SQL 実行済み）。CPU デッキのデッキ設定「🤖 CPU の作戦」で編集。
- 判断は `cpuDeckPlan.ts`＝`S-1` の強さに足し引き（キーカードは残す・優先は先に出す・コンボは A を先に出して B を温存→A が場に出たら B を最優先）。召喚・エナチャージ・手札上限・マリガン・サーチに効く。
- 実機 🆕`V-268` PASS（作戦なし＝先頭の札をエナ／キーカード指定＝もう一方をエナ・画面の［キー］が DB に届く）／CPU 通し対戦 PASS。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7 `S-3`**（1ターン分のシミュレータの見積もり＝調査だけ） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 1**（`S-3`）／**リリース作業 1**（RELEASE.md） |
...
| 🔥**④b** | 🆕**§5.7 CPU の強さ `S-nn`** | **1** | 探索＋評価のゲーム AI（対戦中に LLM は呼ばない） | §5.7.1 の表 |
