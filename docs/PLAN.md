...
- **🆕 セッション（2026-07-12・続き73・Sonnet 5・§3 Sonnetタスク1 §7実機検証の継続＝ON_LRIG_GROW②を検証・turnOwnerゲート未実装を発見）**＝`verifyBattleDrive.mjs` に `lrigGrowAnyOpp`（WXDi-P13-047）を新設。**2回連続PASS**（単体・隣接シナリオ`cpugrow`/`cpugrowblocked`/`wxk10068banish`とのバッチ実行でも確認・`order`配列に追加済み）。guest（CPU）を`cpugrow`と同型構成でGROWフェイズ注入→CPU自動グロウ→host側watcherのSELECT_TARGET解決でguestエナ1枚トラッシュを確認。**turnOwnerゲート未実装を発見**＝原文「あなたのターンの間、対戦相手のルリグがグロウしたとき」に対し `WXDi-P13-047-E2` にturnOwner系の条件が無く、guest自身のターン中のグロウでも発火＝実戦ではほぼ常に不成立のはずが常に成立してしまう過剰発火の疑い。**修正はせずOpusタスク12へ登録**（PLAN §3・§7・詳細 VERIFY_BROWSER.md）。**変更範囲＝`scripts/verifyBattleDrive.mjs`のみ（シナリオ追加）＝engine/parser不変のため typecheck のみ確認・gates再実行不要**。
