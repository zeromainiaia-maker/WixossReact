...
- **🆕 セッション（2026-07-13・続き109・Opus 4.8・「全カード完成」戦略①＝語彙センサスの効果単位化を完了。計器のみの変更＝engine/JSON 不変）**
  ...
  - **次の一手（次セッション）＝戦略②「純P1の系統バッチ消化」に着手**。入口＝`npm run census:clusters` → `docs/_census_clusters.txt` の上位クラスタ（効果単位で再計測済み＝**条件節629・クラス指定399・小さい数374・「Nまで」上限164・数値不一致145・色フィルタ133** が新しい枚数順）から系統を選び、`/census-batch` スキルの手順（parser規則→`build:effects`→`heldReview --adopt`→ゲート→`regen`）で刈る。⚠**採用はカード単位**なので clusters の effectId は末尾（-E1/-BURST 等）を落として CardNum に変換してバッチを組む。**⚠`_effect_srctext.json` は `build:effects` の副産物＝CSV/parser を変えたら再生成が要る**（census は先頭で存在チェックし、無ければ exit 1）。
### 📊 恒久指標（維持中・逐次更新）
