...
- **🆕 セッション（2026-08-16・続き501・Opus 5）＝§6.4 **O-32 クローズ**＝`REPEAT_N_TIMES` の**二重実行（残0）**。ゲート全緑（**golden 2079**＝+7・**census 825 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・held **104枚/44群**＝据置）。live changed **4効果 / 4カード**。`census:stubs` A群 **15種/17件 据置**（無言 no-op 0 のまま）／C群 **173種257箇所 → 172種253箇所**。新機構＝`REPEAT` 正準形への組み替え（parser）／`RepeatAction.optional`（任意反復）／`PLACE_TRASH_SIGNI_FACING_SAME_POWER`（正面ゾーンへの同パワー配置）／`StubAction.placesToField`・`suppressOnPlay`。**engine 側の全文 regex 実装 106 行を撤去**。
  - **🔴① 「受け皿 STUB が自分で実行する」形は過剰と過少を同時に生む**＝当たる文型では parse された本文と**二重に効き**（`WXDi-P07-007-E3` は相手デッキが16枚落ちた／原文は最大12枚。`WXDi-CP02-047-E1` は「－5000」が**全シグニに×3**）、当たらない文型では**ログだけの無言 no-op**（`WX16-042-E1` の「あと2回まで繰り返してもよい」が1回で終了）。**どちらに転ぶかが原文の言い回し次第**という最悪の形＝正準形へ寄せたうえで**旧実装は消す**（残すと parser 退行で静かに復活する。golden にトリップワイヤを追加）。
  - **🔑② 引用ブロック（「…」）は文分割の前に丸ごと切り出して再入力する**＝`parseActionText` へ渡せば複数文の本文も「N回まわす1つの塊」として保てる。文単位に切ると**先頭文が落ちる**（実測2件＝`WXDi-CP02-047-E1` のパワー－5000／`WXDi-CP01-024-E1` の配置）。
  - **🔑③ 本文が解けないときは包まない**＝`REPEAT{UNKNOWN}` は回数だけ増えた no-op で受け皿より計器上も悪い。このガードのおかげで `WXDi-CP01-024-E1` は機構実装まで従来形に留まり、実装後に自動で載った。
  - **🔑④ 配置アンカーは型ではなくフラグで判定する**＝「それの【出】能力は発動しない」の畳み込み（`foldSuppressOnPlay`）と `BattleScreen` の `fieldPlacementOnPlayOpts` は配置アクションの**型を列挙**していたので、新しい配置 STUB が無言で漏れた（＝置いたシグニの【出】が毎回発動する過剰実行）。`placesToField`／`suppressOnPlay` で両側ともフラグ判定に寄せた。
  - **🔑⑤ 任意反復は実行の「前」に問う**＝後ろで問う形にすると最後の1周が必ず走って原文より1回多くなる。あわせて `WX16-042-E1` は「捨てたシグニと同じレベル」条件も落ちており（＝**どのシグニでもバニッシュできた**）、同族11効果が**コスト経路**（`levelEqDiscardLevelSum`）なのに対しこの1件だけ**効果ステップで捨てる**形＝`levelEqLastProcessed` が正しい軸だった。
  - **▶ 次の一手【Opus 側】**＝`O-33` の**残り**（`SigniAttackBan.zones`＝ゾーン限定）→ `O-28` の残23綴り → `O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29`（🆕`WX22-016-E1` の `REPEAT_EFFECT` 1件がここへ合流＝ベットのコイン枚数ぶん同じ選択肢を選ぶ機構＋MANUAL 更新の判断）→ `O-31` の残2 → `O-10`（明示 defer 15 id/17件の前提再判定）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き501 の4経路**＝(a)`WXDi-CP01-024-E1`（トラッシュ選択が最大3回出て、**同じパワーの札だけ**が候補になり**相手シグニの正面**に出る／出したシグニの【出】が発動しない）(b)`WX16-042-E1`（1回目のあと「繰り返す／繰り返さない」が出て、断れば手札は1枚しか減らない＝**負方向＋対照の対**／バニッシュ候補が**捨てたシグニと同レベルだけ**に絞られる）(c)`WXDi-P07-007-E3`（相手側に**3回とも**回避の3択が出る）(d)`WXDi-CP02-047-E1`（対象選択が3回出て毎回別のシグニを選べる）。⚠**すべて実機未検証**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-16 続き501（§6.4 **O-32 クローズ**＝`REPEAT_N_TIMES` の二重実行 残0）後 最新値（本行が直近の正）**：census **825 据置**、**golden 2079**（+7＝live 不変条件（3効果が `REPEAT` になり本文を保つ）1・`WXDi-CP02-047-E1` の回数分配1・`WXDi-P07-007-E3` のミル上限1・正面配置の本命/対照1・`suppressOnPlay` 畳み込み1・任意反復の断る/続ける対1・`REPEAT_N_TIMES` 0件トリップワイヤ1）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 44群**（据置）、lint **0 errors / 260 warnings**、`census:stubs` A群＝**15種/17件 据置**（無言 no-op 0）・C群 **173種257箇所 → 172種253箇所**、`census:goldentypes` **未カバー 0型**。🆕**live JSON changed 4効果 / 4カード**（CSV 非改変）。🆕**新機構＝`REPEAT` 正準形への組み替え／`RepeatAction.optional`／`PLACE_TRASH_SIGNI_FACING_SAME_POWER`＋`INTERNAL_PLACE_FACING_SAME_POWER`／`StubAction.placesToField`・`suppressOnPlay`**。🗑**engine の `REPEAT_N_TIMES` 全文 regex 実装 106 行を撤去**。⚠**全経路 実機未検証**（§7 送り）。
...
| **O-29** | 🆕**「同じ選択肢を複数回選ぶ」ループの表現** | 1効果（是正済・機構は未整備） | 2026-08-15 続き494 で `WX17-003-E1`（ベット―以下から2つ〔ベット時4つ〕まで選ぶ・同じ選択肢を複数回選んでよい）が**選択ループを2周**していた（`CHOOSE_SAME_OPTION_TWICE` と `_MULTIPLE` は engine では同じハンドラ＝4回＋4回）のを外科パッチ＋parser dedup で是正。**残っているのは表現の穴**＝先頭に `UNKNOWN{raw:"以下から２つまで選ぶ"}` が残り、回数はハンドラが**カード全文 regex**から読む（O-20 で潰した型の生き残り）。⚠fresh は `CONDITIONAL{IS_BETTING}` 形で live（PARTIAL・PRESERVE）と食い違っている＝**採るなら MANUAL 化の判断とセット**。母集団は `CHOOSE_SAME_OPTION_*` を含む2効果 🆕**2026-08-16 続き501 で `WX22-016-E1` が合流**＝`STUB{REPEAT_EFFECT}`（②「このアーツの効果を一度繰り返す」）が engine で**ログだけの無言 no-op**。原文は「ベットする《コインアイコン》**１枚につき**以下の2つから1つを選ぶ。**同じ選択肢を２回以上選んでもよい**」＝この O-29 のループ機構がそのまま要る。⚠**MANUAL 不可侵**（現行の MANUAL は本体のバニッシュ節も落ちており、更新するか AUTO 採用へ切り替えるかの判断とセット＝O-6 と同型）。母集団は `REPEAT_EFFECT` を含む live 1効果（golden のトリップワイヤで固定済み）。|
