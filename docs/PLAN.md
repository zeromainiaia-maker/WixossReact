...
- **🆕 セッション（2026-07-28・Opus指示＋codex-work実装／Opus検証・9バッチ連投）＝§3 タスク12(xxix) を段階2 まで完走し、その過程で発見した ON_PLAY 系の既存バグ155件を是正**（golden 868→**888**・census 1511→**1479**・smoke 10725→**10726**）
  ...
  - **(xxix) 本体**＝union/unless 16効果（⚠`negated_attacks` の保存先規約が2系統に割れていたのを検証で発見・是正）／BET 9効果26選択肢（最悪例 `WX19-006-E1③` は**除外本体が消えて2ドローだけ**だった）／自身【出】段階1（10効果＝全経路で永久不発）／**段階2＝mandatory 1437効果**（`phase1Only` 完全撤去。任意+cost 933と段階3の65は明示除外）。
  - **⚠段階2 は3度目でようやく着地。1・2回目の codex の honest defer が毎回 Opus 側の計器の穴を突き、より大きな既存バグを掘り当てた**＝1回目 `WX13-040-E1`→**ON_PLAY 誤分類36件**（原文が【自】なのに全件 mandatory で**通常召喚のたびに誤発火**）／2回目 `WX25-P2-026-E1`→**watcher 34件**（`triggerScope:'self'` で**watcher として一度も発火せず・自身の召喚時に誤発火**の二方向破損）。さらに段階2 着地後の検証で**先頭ゲート条件の脱落46件**（うち25件を是正）。**合計155件が段階2 とは独立に稼働中のバグだった。**
  - **ON_PLAY 系はこの3種で系統的に壊れていた**＝①timing が違う（36件）②scope が違う（34件）③条件が落ちている（46件中25件是正・20件は語彙不足で [B]）。いずれも**通常召喚経路が mandatory【出】を無条件で積む**（`BattleScreen.tsx:5149`）ため従来から live だった。
  - **⚠検証で踏んだ落とし穴2つ**＝①**`build:effects` 再実行で JSON 差分が0でもドリフト無しの証明にならない**（held は curated 温存＝fresh と食い違うカードなので**差分0かつ held 増**が起きる）。これで watcher バッチの実バグを1度見逃した ②**非ASCII を JSON へ書くと文字化けする**＝`triggerFilter.cardType` が `"???"` になり配線したはずの watcher 2件が永久不発だった。**外科パッチ後は `grep '?' effects_*.json` で機械確認する。**
  - **⚠段階2 の安全性は golden だけが担保**＝`scripts/selfPlayFuzz.ts:12-13` が自ら「BattleScreen 側のオーケストレーション（トリガー収集・effect_stack 整列）は検証しない」と明記。連鎖は `ADD_TO_FIELD` 非置換＋シグニゾーン3枠で構造的に有限（2段連鎖 golden で実証）。
  - **PLAN の自己矛盾を1件修正**＝PLAN:44「`build:effects` は破壊的＝絶対に実行しない」が §5c パイプラインと矛盾。非破壊なので**「実行後に effectId 単位で全数比較する」**へ書き換えた。
  - **次の一手**＝**Opus：ゲート脱落の [B]20件（`signi_traps`/チェックゾーン/能力なし盤面 等の Condition 語彙が要る）／(xxix) 残＝任意+cost 933効果と段階3 65効果（タスク12(lv)）／タスク16 の [B]29件＋watcher [B]4件／(xliv)・(xlii) の残／§6.3 残機構（H1〜H3・I）／タスク14 の残43**。**Sonnet：タスク1（§7 実機検証）＝今セッションの9バッチはすべて golden までで実機確認が未実施。特に段階2 は1437効果が新たに発火するので `scripts/verifyBattleDrive.mjs` にシナリオを足す価値が高い。**
