...
5. **golden 型網羅の追加**（未カバー DSL action型の洗い出し→1型1テスト・§6.4）。**続き82（Sonnet）で121型中未カバー73型を機械抽出→12型を追加**（POWER_SET／ENERGY_CHARGE／ADD_TO_ENERGY／ADD_TO_BEAT／ADD_TO_LIFE／NEGATE_ATTACK／AWAKEN_SIGNI／PLACE_UNDER_SIGNI／STORY_CHANGE／GAIN_BOND／REMOVE_CHARM／DISCARD_BOTH・golden 230→242）。過程で実バグ1件発見＝`applyDirectAction`のENERGY_CHARGE/STORY_CHANGE等caseの欠落（Opusタスク12 (v) へ登録・詳細は同タスク本文）。engine/parser/JSON は無変更（census 1483維持・同型★0維持）。**続き83（Sonnet）でPOWER_MODIFY_PER_*/BY_*系13型を追加**（PER_STACK／PER_LEVEL_SUM／PER_LRIG_LEVEL／PER_LIFE_COUNT／PER_VIRUS_COUNT／PER_ENERGY_COLOR／PER_CHARM／PER_FIELD／BY_TARGET_LEVEL／BY_SOURCE／PER_TRASHED_LEVEL／PER_HAND_COUNT／MULTIPLY・golden 242→255）。CONTINUOUS系は`calcFieldPowers`直呼び・POWER_MODIFYへ委譲する型はcount:1の外部SELECT_TARGET経由でも安全と確認済み・続き82発見バグ（Opusタスク12(v)）を踏む構成のみcount:'ALL'に迂回。engine/parser/JSON は無変更（census 1483・同型★0とも維持）。**続き84（Sonnet）で9型を追加**（DRAW_PER_FIELD_COUNT／ENERGY_CHARGE_FROM_DECK_PER_FIELD_COUNT／PLACE_UNDER_SIGNI(source:hand)経由のPLACE_UNDER_SOURCE_SIGNI／FORCE_SIGNI_ATTACK／TAKE_FROM_UNDER_SIGNI／BLOCK_CARD_USE／ADD_CRAFT_TO_LRIG_DECK／ENERGY_CHARGE_BY_FIELD_COUNT／PLACE_VIRUS・golden 255→264）。過程で新規バグ1件発見＝`POWER_MODIFY_PER_DECK_COUNT`が`effectEngine.ts`のCONTINUOUS計算層に未実装（Opusタスク12(vi)へ登録）。`BLOOD_CRYSTAL_ARMOR`は「同名カードの別コピー」判定がこのテストハーネス（instance IDにsuffixが無い）では表現できず見送り。engine/parser/JSON は無変更（census 1483・同型★0とも維持）。残40型は次セッション以降へ継続（多くはREVEAL_UNTIL_*/GRANT_*_ABILITY等の複合機構＋PLAN §6.1未実装15型）。
...
| 12 | Sonnet 発見バグの修正（常設） | 可変 | 可変 | ◎**在庫4件**（✅続き78で旧8件全消化。新規＝SP27-002 genericKagiri無言消費系統・WXDi-P10-035 引用内BOUNCE owner精査・WXK09-050 dispatch設計・🆕続き81 applyDirectAction TRASH/HAND_CARD 3フィールド欠落。詳細は上のOpusタスク12本文） |
...
| 5 | golden 型網羅追加 | テスト（scripts） | S（1型1テストずつ） | ◎今すぐ（続き82で12型・続き83で13型・続き84で9型追加・golden 264・残40型＝多くはPLAN §6.1未実装15型と複合機構） |
