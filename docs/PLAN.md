...
- **🆕「次のターンの間」が `duration:INSTANT` に潰れる系統（続き448 で第1波・採用6／据置7）**：原文「次のターンの間／次の対戦相手のターンの間」が即時実行になり、**発動したターンに適用される**（または相手 state へ書く近似で**何も起きない**）。⚠**逆翻訳では原理的に見えない**（JSON に「次のターン」を表す語が無いので逆翻訳も「〜する」としか出ない）。census 843 据置・golden 1891→1904。詳細は BUGFIXES 2026-08-12（続き448）。
  ...
  - 📋**残＝受け皿の拡張（次の一手）**＝(a)**クラス／ゾーン限定つきの場全体予約**（`WXEX2-26-E3`＜微菌＞／`WXDi-P14-070-E1`＜電音部＞／`WX05-034-BURST`・`WX10-036-BURST` 中央のシグニゾーン）(b)**「このターンと次のターン」の二重期間**（`WX05-034-BURST`／`WXEX2-26-E3` ほか）(c)**owner=opponent の付与予約**（`WXK05-052-E1`）(d)**コスト付きアタック制限**（`WXDi-P11-005-E3`＝既存 `attackCost` は `fieldTrash` のみ）(e)**複合保護**（`WXDi-P16-002-E1`）。⚠**held +1（`WXDi-P14-070-E1`）はこの受け皿待ちの可視化**＝fresh は `NEXT_TURN` を出すが live の `UNTIL_OPP_TURN_END` の方が原文に近いので**採用しないのが正解**。受け皿ができたら採る。
