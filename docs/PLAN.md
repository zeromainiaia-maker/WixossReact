...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxii)＝`CONDITIONAL_DISCARD` を型ごと退役し `WD16-016-BURST` を多段閾値の昇格置換で組み直す（残0クローズ）**（golden **1130→1131**・census **1373 据置**・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **288枚 据置**・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects` 冪等）
  - **⚠PLAN の診断「executor に dispatch が無い＝完全 no-op」は半分だけ正しかった**。それは**アクション型** `{type:'CONDITIONAL_DISCARD'}` の話で、唯一の使用者 `WD16-016-BURST` が持っていたのは **STUB 形＝ハンドラは存在し、動いていた**。中身が「条件を一切見ず `ctx.ownerState.hand`＝**自分の**手札を1枚捨てる」という別物で、①相手 ≤5 のとき相手1枚＋**自分も1枚**（自傷）②相手 ≥6 のとき相手**0枚**（「代わりに2枚」未実装）＋自分1枚、という挙動だった。**「no-op だろう」と決めつけず実装を読むこと**＝(lxi) 第2波と同じ教訓を2度踏んだ。
  - **新語彙0本**＝既存の「多段閾値の昇格置換」を2点だけ拡張。**(a)** 裸形「N枚以上**の**場合」を許容し（旧regexは「N枚以上**ある**場合」等の動詞形だけ）、**比較演算子を原文の以上/以下から取る**ようにした＝⚠従来は前段条件の**数値だけ**差し替えるため、前段が `HAND_COUNT lte 5` の本件では `lte 6`＝**真逆の条件**になる。**(c)** 枚数のみ形「代わりに(手札を)N枚捨てる」＝主語が省かれ再パースすると owner が self へ反転するので、base の `TRASH{HAND_CARD}` を複製して **count だけ差し替える**（既存の (b) per-target 値のみ形と同じ考え方）。裸形は全CSVでこの1枚だけ＝機械確認済みで巻き込み0。
  - **退役**＝`ConditionalDiscardAction`（型＋union）／`STUB{CONDITIONAL_DISCARD}` のハンドラ／生成元3箇所（parseSentencePart1 のアクション型＝`WX11-044-BURST` が (lxi) 第2波で標準ゲートへ移り到達0／Part3「〜の場合、手札をN枚捨ててもよい」＝**到達0**〔該当2枚は上流の `ALL_FIELD_SIGNI_MATCH` ゲート規則が先に拾って正しく解けている〕／Part4「N枚以上の場合、代わりにM枚捨てる」＝上記へ移行）／decompiler の action 型 case。STUBS.md は 使用中 577→**576** 種・実装 548→**547**。
  - **次の一手**＝**Opus：①🆕 `WX15-033-E2` の fresh が `CONDITIONAL{LIFE_COMPARE_OPP}` を落とすドリフト**（採用すると無条件バニッシュへ退化するので held 据置中＝タスク12(lxiii)。同型の「〜の枚数があなたより多い場合」文型が他にも巻き込まれていないか要確認）**②「コストの合計」族の STUB/UNKNOWN 14件③`WX11-043-E1` の `crossOnly` 欠落④タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑤`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑥§6.4 の配置数制限すり抜け7効果⑦§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝タスク12(lx) の未検証UI 2件／(lxi) 第1・第2波の UI 6件／§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。⚠**(lxii) は engine 直叩き golden で固定済みだが `WD16-016-BURST` の実機UI（相手が捨て札を選ぶ）は未検証**＝§7 に登録した。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxii) 残0クローズ後 最新値（本行を直近の正とする）**：census **1373 据置**（`BASELINE_HIGH=1373`）、golden **1130→1131**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288枚/106署名 据置**、manual field loss 0。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WD16-016-BURST`）、`build:effects` 冪等。**新語彙0本／退役1型＋1 STUB id**（`ConditionalDiscardAction`／`STUB{CONDITIONAL_DISCARD}`。STUBS.md 使用中 577→576 種・実装 548→547）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1131**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1373**（回帰ゲート）・held **288枚/106署名**（2026-07-31 タスク12(lxii) クローズ後に実測）。
>
