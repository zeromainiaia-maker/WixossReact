...
**直近＝2026-09-19＝§5.3 `O-536` クローズ＝「N体まで対象とし…それら」の帰結から「まで」を落とす（14効果 / 14カード）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**真因**＝parser が宣言の `upToCount` を**帰結にもコピー**していた＝`selectOrInteract` の `optional` が立ち、前バッチの `O-535` の自動解決から外れて**宣言した対象をもう一度選び直させて**いた。
- **直し方**＝**後処理1本**（`normalizeStoredTargetUpToCount`）。🔴**置き場所は「全 pass のいちばん最後」**＝対象宣言を引き上げるのが最終ループなので、早い段に置くと**正準形がまだ無く 8効果にしか当たらない**。MANUAL 4効果は手で揃えて `syncManualLive` で配送。
- ⚠**登録票の「13効果」は誤り**（実測14）＝宣言ステップを **id** で数えて専用 STUB の1効果を落としていた。⇒ **構造（`selectTarget` を持つか）で絞る。**
- 🔑**ラチェットは live の全件走査**＝parser 側と MANUAL 側を同じ golden 1本で守る。**旧 live へ戻すと新設2本とも FAIL**＝反転確認済み。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-5`**（ヘッドレスで CPU 同士を回す）＝`S-5c` の続き → **`S-5d` 相手側の応答と人間側ターンの駆動** → その後 `S-6` / `S-8`。🏁**§5.3 機構 worklist は残0**（索引 A〜I すべて） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**14効果の payload を1キー倒しただけ＝どの計器の母集団にも入っていないので動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**後処理で正準形を直すなら「その正準形がいつ組み上がるか」を先に測る**＝parse パイプラインの途中に置くと黙って半分しか当たらない（[LESSONS.md](./LESSONS.md) §4.2）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4324**） |
...
- **2026-09-19 時点**（第403バッチ＝§5.3 `O-536` クローズ）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**14効果の payload を1キー倒しただけ＝どの計器の母集団にも入っていないので動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4324**＝+2・`census:traceinv` I1=0）
