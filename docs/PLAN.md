...
| (lxi) | **✅第1波（「対戦相手**が**」形・29カード30効果）／第2波（「対戦相手**は**」＝主語分配形・8カード9効果）／🆕第3波（分割再パースを基準読み供給へ切替・8効果）とも消化済み。残9効果はいずれも別機構待ちの honest defer**。⚠**教訓①（第2波）＝「ゲートが無い」と決めつけず現状 JSON と engine ハンドラの実装まで読むこと**（13効果を同一文型と括ったが実際は6種の別バグ）。⚠**教訓②（第3波）＝据置理由が複数あっても原因は1つのことがある**（①複文／⑤owner 反転／③そのシグニ照応 の3グループは全部「クローズを外した文を再パースする」設計が原因で、**基準読みを then の供給源にする**だけで同時に開いた）。**🆕残9の内訳**＝①回避コストが engine 非対応3（`WXK05-009-E2`〔アタック回数比例の可変《無》〕／`WXK06-047-E1`／`WXK06-067-E1`〔「デッキの上に置く」型の回避＋「は」形＋前置きアクションの三重〕）②**前置きにアクションがある**2（`WX24-P2-022-E1`「カードを１枚引き、」／`WXDi-P13-075-E1`「対戦相手のすべてのシグニを凍結する。」＝専用 STUB `UPKEEP_OR_NO_UP` も同居）③遅延トリガーの設置が前置き2（`WX24-P4-011-E3`／`WXDi-P06-023-E2`＝後者は現状 UNKNOWN）④3機構一体1（`WX24-P4-007-E1`＝`DO_THREE_THINGS` は別カード用 text パターンで完全 no-op）⑤制限系で別機構1（`WXDi-P11-009-E3`）。**engine の回避手段語彙**＝`costColors`／`opponentHandDiscard`(+`ALL`)／`opponentHandDiscardFilter`／`opponentEnergyTrash`(+`ALL`)／🆕`opponentSigniTrash`。**②を開けるなら「前置きアクションは外に出し末尾の帰結だけを包む tail-splice」が次の一手**（基準読みのどこからが X かを同定する必要がある）。詳細 BUGFIXES 2026-07-31 (lxi) 第3波節 |
...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxi) 第3波＝支払い回避クローズの「分割再パース」を基準読み供給へ切替（残17→9・8効果消化）**（golden **1137→1142**・census **1369→1367**（`BASELINE_HIGH=1367`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **251→250枚**〔署名 97 据置〕・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 8 / added 0 / removed 0**／`build:effects` 冪等）
  - **据置していた3グループは原因が1つだった**＝第1波は「回避クローズを外した文を**再パース**して帰結 X を作る」設計で、①複文「以下の２つから１つを選ぶ。①…②…」2効果／⑤純加算チェック不一致＝owner 反転1効果／③「そのシグニ」照応2効果 を別々の理由で据え置いていたが、**どれも分割そのものが原因**だった。第2波（「は」形）が既に採っていた**「基準読み（本規則を止めた1回パス）を検査ではなく `then` の供給源に使う」**形へ寄せると同時に開く。
  - **適用範囲＝クローズが本文の先頭にある形だけ**（X が本文まるごと＝基準読みそのものになる）。前置きは**アクションを生まないもの**だけ許す＝時点の限定句「そのアタック終了時、」とトリガー句「〜たとき、」。⚠**「このターン、〜したとき、」＝遅延トリガーの設置は除外**（アクションを生む＝丸ごと包むと**設置自体がゲートされる**別物になる）。⚠基準読みが**この family の専用 STUB を既に吐いている**場合も据置（二重に包むと相手に CHOOSE が2回出る）。
  - **「そのシグニ」はトリガー元へ束縛する**＝ゲートを足すだけでは「どの相手シグニでも選べる」過剰対象化が残る。`REMOVE_ABILITIES` は既存フィールドを刻むだけ、`TRASH` は `targetsTriggerSource` を**新設**して execTrash の候補を1体に絞った。⚠**executor が honor する型だけに刻む**（(lxiv) と同じ罠）。
  - **回避手段を1つ水平展開**＝`opponentSigniTrash`（自分のシグニN体を場からトラッシュ）。available 判定つき＝場にシグニが無ければ選べない。⚠**回避句を「か」で split してはいけない**（「場**か**ら」「エナゾーン**か**ら」まで割れる＝実測で踏んだ）。左から1つずつ食う形へ変更した。
  - **複文 CHOOSE は組んでから包む**＝選択肢が別文なので単文パスに届かず、CHOOSE だけ組まれてクローズが無言消費されていた。`SPDi43-07-E1` は更に前の状態条件も落ちていたので、状態条件→効果レベル `condition`／クローズ→ゲートの**両方**が乗るようにした。
  - **8効果採用**（うち `WXEX2-25-E1` は同居 MANUAL でカード単位 PRESERVE のため curated へ直接配線）。残9は honest defer＝内訳は §3 タスク12(lxi) 行。
  - **次の一手**＝**Opus：①🆕 (lxi) 残9のうち「前置きにアクションがある」2効果**（`WX24-P2-022-E1`／`WXDi-P13-075-E1`＝**tail-splice**＝前置きは外に出し末尾の帰結だけ包む形の設計が要る）**②「コストの合計」族の STUB/UNKNOWN 14件③`WX11-043-E1` の `crossOnly` 欠落④タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑤`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑥§6.4 の配置数制限すり抜け7効果⑦§6.3 I＝`WX25-P3-028-E2` の3機構⑧(lxvi) の据置2枚**／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**(lxi) 第3波の相手側 CHOOSE 4択目**／(lxv) の条件つき任意コスト 36枚／(lxiv) の対象ピッカー前置 61枚ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxi) 第3波後 最新値（本行を直近の正とする）**：census **1369→1367**（`BASELINE_HIGH=1367`）、golden **1137→1142**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **251→250枚／署名グループ 97 据置**、manual field loss 0。live per-effect 差分 **changed 8 / added 0 / removed 0**（held 7枚採用＋MANUAL 同居カード1効果を直接配線）、`build:effects` 冪等。**新語彙2本**＝`TrashAction.targetsTriggerSource`／`StubAction.opponentSigniTrash`。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1142**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1367**（回帰ゲート）・held **250枚/97署名**（2026-07-31 タスク12(lxi) 第3波後に実測）。
>
