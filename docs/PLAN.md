...
- **🆕 セッション（2026-08-11・続き435〜440・Opus 5）＝Codex 委譲を5バッチ回し、§7 実機検証3件を PASS へ反転＋§6.4 を4系統消化。golden 1809→1826・census 852→847・UNKNOWN 30→28**。
  - **🔴① §7 実機検証の「クリック手順バグ」は表層で、真因は UI に安定セレクタが無いことだった**（続き436）＝続き435 で FAIL した8シナリオを実測したら、①支払いモーダル5本のうち `KeyUseModal` だけ testid 無し ②**ルリグ行スロットに testid が無く自分と相手で label 文字列が同一**（右アシストを特定できない）③カード詳細のアクションボタンに testid 無し、が原因だった。`keycost-energy-*`／`my|op-lrig-slot-*`／`card-action-*`+`data-action-label` を新設して**3件が2回連続PASSへ反転**（`assistAttackBoth`＝左右アシスト連続アタック／`fezoneDoubleCostPay`＝《青》1と手札2枚の**両方**が徴収される／`sYokusenkiSpellPay`＝候補がスペル1枚だけに絞られ相手手札 2→1）。**シナリオだけ書き直しても何度でも同じ壁に当たっていた。**
  - **🔴② 残った5件は「セレクタ問題ではなくなった」＝別の実バグを2件登録**（§3 (cxxiii)(cxxiv)）＝(a)**ピースのコスト二重請求の疑い**＝`executeKeyPiece` は `AUTO`/`ON_PLAY` しか積まないのに `WXDi-P14-002-E1` は `ACTIVATED`/`MAIN`＝**セットしても効果が発火せず**、KEY の【起】から別途起動すると**印刷 Cost と同じ `cost.energy` をもう一度**払う (b)`WX25-CP1-091` の ON_TURN_END pay/skip が出ない（**原因未確定**＝ターン進行か watcher かの切り分けが未了）。
  - **⭐③ Codex は「実行できない」＝実ブラウザ検証タスクはコードだけ書かせる**（§2 に続き435 で追記済み）。**ソースから答えが決まる作業（セレクタ整備）は高品質に回り、実行しないと詰め切れない部分と綺麗に分離できる**ことを実証した。
  - **🔴④ §6.4 を4系統消化**＝(a)**UNKNOWN 第6バッチ**（続き437・`WX21-028-E1`＝「その中から**すべての**〜」で語順が合わず**公開するだけで振り分けが落ちていた**／`WDK13-022-E1`＝動的枚数）(b)**【マジックボックス】設置の欠落**（続き438・原文7枚中**4枚で設置ステップが JSON に無く**、`WX24-P3-072` は「3枚見て下に戻すだけ」＝撃っても何も起きなかった）(c)**「能力を持たない」2軸配線**（続き439・**5/5 defer 0**＝昇格節の脱落3件＋対象限定の脱落2件）(d)**数量参照が固定値に潰れていた**（続き440・`WXK11-013-E1-G` は**盤面全部をトラッシュして1体しか戻らない**、`WXK11-028-E1` は **`count:0` で1枚も公開しない丸ごと不発**）。
  - **⭐⑤ 罠の先出しが5連続で的中**（予防的使用6〜8例目）＝①`then: STUB{PLACE_MAGIC_BOX}` は**汎用 then ループが `lastProcessedCards` を設定しない**ので無音 no-op（先例＝トラップ設置の専用分岐 `effectExecutor.ts:7194-7204` の鏡写しで解決）②`LAST_PROCESSED_MATCHES` の `ZONE_STATE_KEYS` に `noAbilities` が無く**「効果で能力を失ったシグニ」を取りこぼす**（`WXEX1-55` は E1 で能力を奪い E2 で狙う自己完結コンボ＝主要シナリオ直撃だった）③`PARTIAL` 不可侵・`isPureSuperset` で held に落ちる・effectId 集合を変えるな の3点セット。
  - **🔑⑥ Codex が「count の解決関数は action 型ごとに違う」という新しい死角を発見**（続き440）＝`TRANSFER_TO_HAND` は `resolveNum(src.count)` を使い、**`resolveNum` は `{$ref}` を問答無用で 0 にする**（`execUtils.ts:118-120`）＝**書いても黙って0枚**。⇒ **§5-2「消費側の実コードを読む」は前段（記録するか）だけでなく後段（読めるか）にも適用する**。
  - **⚠⑦ 計器が成熟して偽陽性が優勢＝「投げなかった判断」も成果**＝`census:wiring` 最上位★★セル／【トラップ】設置の任意性／「次にスペルを使用する場合」5件／「対戦相手は捨てる」が `owner:self` 疑い12件——**4つとも開いたら実害なし**だった（語彙がコスト句・条件節・トリガー範囲にあるクロス計上／`pickCount` は上限で0枚確定できる／executor が既に one-shot 消費している／self 側は原文の別節「あなたは捨てる」）。**★★でもセルを開くまで投げない。**
  - **⚠⑧ 在庫の症状記述が古い事例が3件**＝(a) PLAN §6.4「`WX25-P3-014-E1` は `BOUNCE{owner:'self'}` の自傷」は**既に `opponent` へ修正済み**（残っていたのは別の2軸）(b) §3 (cxxii) `WXDi-P05-037-E1` は**既に `OPPONENT_PAY_OPTIONAL` へ修正済み**(c) PLAN §6.4 の「カード内に MANUAL が1つでもあるとカード丸ごと温存」は**続き377i で効果単位に変わっている**。**投げる前に live を見る。**

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き435〜440（§7 実機検証3件PASS＋§6.4 4系統消化）後 最新値（本行が直近の正）**：census **847**（852→847）、golden **1826（+17）**、smoke **10688 / SKIP 0**、**同型★ 0**、held（parserWorklist）**107枚 / 署名グループ 47件**（全バッチで増減0）、lint **0 errors / 259 warnings**、**UNKNOWN 28ノード / 27カード**（30→28）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群（無言 no-op）**0**、`live で noAbilities を持つカード` **11→16**、`verifyBattleDrive` 既定 order **+3**（`assistAttackBoth`／`fezoneDoubleCostPay`／`sYokusenkiSpellPay`）。⚠**`fuzz` の `distinct効果` は乱択で毎回変動する**（同一ツリーで 2682/2674/2670/2666 を実測）＝**指標にも一致基準にも使わない**（見るのは「バグ0・SKIP 0」だけ）。
...
- **🆕 数量参照「この方法で〜したカードと同じ枚数」が固定値に潰れていた（続き440 で消化・残1）**：原文が動的枚数なのに live が `count:1`（や `count:0`）の固定値だった4効果を是正。**✅`WXK11-013-E1-G`**（全シグニをトラッシュ→**同じ枚数まで**戻す。旧＝`count:1`＝**盤面全部を捨てて1体しか戻らない**＝エクシード2を払う価値が消えていた）／**✅`WXK11-028-E1`**（旧＝**`count:0`＝1枚も公開しない丸ごと不発**）／**✅`WXEX2-07-E3`**（全体バニッシュ→同数まで回収。`upToCount` も復元）／**✅`WXDi-P03-009-E3`**（捨てた**青の**カード枚数＝**フィルタ付き**）。census 848→847・golden +5・held 増減0。
  ...
  - 🔴**新しい死角を発見＝`count` の解決関数は action 型ごとに違う**＝`TRANSFER_TO_HAND` は `resolveNum(src.count)`（`effectExecutor.ts:2288`）を使い、**`resolveNum` は `{$ref}` を問答無用で 0 にする**（`execUtils.ts:118-120`）＝**書いても黙って0枚**になる。`WXEX1-44-E2` はこの理由で**据置**（前段の STUB は枚数を記録するので、`resolveNum` 側を直せば通る）。📋**残**＝`resolveNum` を使う `count` 消費地点（`:1005`／`:2238`／`:2288` ほか）を `resolveCountRef` へ寄せるか、**「ここに `$ref` を書くと 0 になる」を golden のトリップワイヤで固定**する。
