...
- **🆕 セッション（2026-07-31・続き305・Opus 5〔**Claude が実装まで担当**〕）＝タスク16 `[B維持]` から `WXDi-CP02-068-E1`（離脱先が「手札かトラッシュ」の OR）を1件消化**（golden **1187→1190**・census **1361 据置**（`BASELINE_HIGH=1361`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・held **251枚/98署名 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **19→18**）
  - **① 残5件を全部読んでから一番筋の良い1件を選んだ**＝`WXK11-049-E1` が**ほぼ同一のテンプレート**（「あなたのターンの間、あなたの効果によって対戦相手のシグニ1体が場から**手札に**移動したとき」＝`any_opp`＋`byOwnEffect`＋`turnOwner:'self'`＋`leftToZone:'hand'`）で、**差分は行き先が「手札 か トラッシュ」の OR になっただけ**だった。本体も既に原文どおり。
  - **② `triggerCondition.leftToZone` を `'hand' | Array<'hand'|'trash'>` へ拡張**（配列は OR）。**既存6効果は素の `'hand'` のままで表記も挙動も不変**＝`['hand']` と同義に扱い、**並行フィールドは新設しない**。判定は `leftToZoneOk()` に集約して `collectLeaveFieldTriggers` の**2走査（any_ally/any 側と any_opp 側）を対称に**した。
  - **③ 波及ゼロ**＝`leftToZone` 保持は live 6件で全部素の `'hand'`。golden に **`WXK11-049-E1` は手札で発火・トラッシュでは非発火**（互換表記が配列化で緩まないこと）を明示固定した。
  - **④ 疑った本体の過剰制限は誤りだった**＝`filter.cardType:'シグニ'` を「原文は種別を限定していないのに絞っている」と疑ったが、**＜ブルアカ＞は `CardClass`（「奏武：ブルアカ」）**で、実データで CardClass に「ブルアカ」を持つのは**シグニ109＋シグニ/クラフト6 のみ**（アーツ/ピース/ルリグには付かない）＝矛盾なし。**台帳に偽の在庫を積まずに済んだ**（(lxix) の `InstanceMap` 誤検出に続き2例目）。
  - **⑤ 今回は逆翻訳の落ちを最初から潰した**＝前2回（`byOwnEffect`／`noGuard`）で連続して踏んだので、`triggerCondition` を触ると同時に decompiler も直した。**手順として「trigger 系フィールドを足したら decompiler を必ず見る」が定着**。
  - **次の一手**＝**Opus：タスク16 残＝`[B維持]` 4件**（本件で 5→4）。①`WXDi-P13-051-E3`（`byOpponentEffect`＋**手札捨てとエナ→トラッシュの2経路 OR**＋「効果1つによって」の解決単位重複排除。⚠**本体も要修正**＝原文「カードを1枚引く**か**【エナチャージ１】」なのに live は `ENERGY_CHARGE_FROM_DECK` 単体＝**CHOOSE が潰れている**）②`WXDi-P06-038-E1`（エナ→**任意宛先**＋原因＝`ON_ENERGY_TO_TRASH` は宛先 trash 限定）③`WX05-020-E1`（**1ターンの累積クラッシュ枚数**をシグニ単位で数える counter が要る）④`WXDi-P11-063-E2`（`ON_PLACED_UNDER_SIGNI` は型宣言のみで collector 参照0件。⚠**本体も要修正**＝原文「このカードの**上にあるシグニ**」なのに live は `owner:'any'` の任意選択）。⚠**4件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**🟢 投入前に必ず「解決時に engine がその値を掴めるか」「本体も原文どおりか」の2点を確かめる**（直近3件とも timing 以外に穴があった）。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜305 の全件が実機UI未検証で積み上がっている**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き305（タスク16＝`WXDi-CP02-068-E1` 離脱先 OR・1効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1361 据置**（`BASELINE_HIGH=1361`）、golden **1187→1190**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251枚／署名グループ 98 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WXDi-CP02-068-E1`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 19→18 効果/18クラスタ**（⚠**実質16**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 5→4**。**新語彙0本**＝`triggerCondition.leftToZone` を `'hand' | Array<'hand'|'trash'>` へ**拡張**しただけ（既存6効果は素の `'hand'` のままで挙動不変＝`['hand']` と同義。並行フィールド新設なし）。engine 変更は判定の `leftToZoneOk()` 集約と2走査の対称化のみ。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1190**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1361**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 `WXDi-CP02-068-E1` 後に実測）。
>
