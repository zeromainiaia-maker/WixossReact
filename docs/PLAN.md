...
- 🏁**セッション（2026-08-27・続き692・Opus 5 単独）＝Sheet1 の「機構待ち」を全数棚卸しし、いちばん大きい塊 `O-119` を消化。**
  ユーザー指示＝①Sheet1 の機構待ちを解消する ②実装は codex に投げる ③思考は Claude が行う
  → 途中で **④「codex はすぐ使用制限になるので最初の1バッチだけ」**（実際に1バッチで上限到達）。
  📊**進捗3計器＝Sheet1 要対応 35→33 / 863（4.1%→3.8%）｜台帳 残 OPEN 585→584｜census 高シグナル 526→528**。
  gates 全緑（golden **2907→2908**）。実機＝新規 `b14costup` PASS（**反転確認済み**）＋ 回帰 `b12spellkiten` / `b13colorpower` PASS。
  - 🔑**`O-119`＝「この〈スペル/アーツ/カード/ピース〉の使用コストは…につき…減る/増える」40効果を
    `cost.costScaling` payload へ移した**（型＋parser＋`computeArtsEffectiveCost`＋**消費5入口**＋逆翻訳＋golden）。
    **live 変化は 40 effectId ちょうど・スコープ外0**。**実挙動が変わったのは2効果だけ**
    （`WX05-034` / `WD16-023` ＝**増加側の汎用分岐が無く過小請求だった**）で、残り38は**挙動不変のまま JSON から見えるようになった**。
  - ⚠**census が +2 したのは較正**＝コストマーカー STUB が消えたことで、そのカードに元からあった
    別軸の欠落（`WX15-060-E1` の相対パワー比較、`SP26-008-E1` の付与能力）が計器に見えた。原文照合で確認済み。
  - 🔴**棚卸しで §5.3 の登録票の誤りを3つ見つけた**＝`O-116` は**B13 で消化済みなのに残っていた**／
    `O-108` と `O-115` は**同一項目**／`O-112` の残りは `movedToDeckFilter` だけ（M ではなく S）。
    ⇒ §5.3 を実測に合わせて整理した。
  - ⚠🔴**Claude の見立てが4件外れた**（通算13回目）＝「旧 regex が効いていないのは10枚」と書いたが実測は**2枚**。
    `computeArtsEffectiveCost`（約280行）を**部分的にしか読まずに分岐の有無を結論した**のが原因。

**▶ 次の一手**＝**B15＝`O-120`（【ランサー】由来のライフクラッシュ）**（Claude が直接実装。**codex は使わない**＝1バッチで使用上限に達するため・ユーザー決定）。
**Sheet1 の残り機構待ちは `O-108` / `O-109` / `O-110` / `O-112` / `O-113` / `O-117` / `O-120`〜`O-129`＝1項目＝1巡**。
🔴**続き692 で「どれも安くない」ことを配線まで実測して登録票に書いた**ので、**着手前の再調査は不要**（そのまま実装に入れる）。
`O-120` を先頭にする理由＝**当てが1本の funnel に収まっている**（`crashOneLife` が `crash_source_card_num` を刻む唯一地点）ことを確認済みで、
**3効果が同じ壊れ方（条件ゼロの過剰実行）で一度に閉じられる**から。
⚠**逆に `O-129`（118カード影響）と `O-128`（34カードの catch-all）は最後に回す**＝母集団が大きく、1件だけ直す形にできない。
...
- [ ] 🆕**`V-91`（2026-08-27 Sheet1 B12）＝`WX09-Re06`「このターン最初のリフレッシュ」の `once` 消費を実機で見る。**
  **golden では両方向を固定済み**（自リフレッシュで発火／相手リフレッシュでは発火しない／発火後に設置が消える）。
  ⚠**実機シナリオを書かなかった理由**＝リフレッシュはデッキ切れでしか起きず、`verifyBattleDrive` の
  盤面注入で `deck:[]` にすると**ドロー処理そのものが先に走って別経路へ落ちる**（本巡では切り分けに至らなかった）。
  ■**取り方**＝`deck` を1枚だけにして `ON_REFRESH` を1回起こし、`delayedTriggers` が 1→0 になることと
  相手ライフが1枚減ることを観測する。**2回目のリフレッシュで**発火しないことまで見て初めて `once` の検証になる。

- [ ] 🆕**`V-90`（2026-08-27 Sheet1 B10 の回帰実行で露出）＝既存シナリオ `banishbyeffect` が `FRESH=1` だと FAIL する。**
  ⚠**本巡の変更とは無関係**＝`git stash` して **HEAD でも `FRESH=1` なら同じ FAIL**（`grants=-` / `stack=0`）。
  **暖まった部屋を引き継いだとき（既存 PLAYING ルーム再利用）だけ PASS する**＝§4.4 の3分類でいう (a) シナリオの腐り。
  ■**切り分けの起点**＝新規ルームでは注入前の盤面（エナ・手札・ルリグ Lv）が違い、
  `WX07-036` の【自】発火に要る前提（除去を撃つコスト or 対象）が揃っていない可能性が高い。
  **まず `FRESH=1` で1回、注入直後の `queryState()` を丸ごと出して既存ルーム時と突き合わせる**。
...
| **O-92** | 🆕**「このターン、〜したとき」の遅延設置が生きているのは 41出現中 5枚だけ** | **M（要仕分け）** | **2026-08-26 続き674（`O-73`）で実測**＝設置句を全数で拾うと **21文型 / 41出現**あり、live で `INSTALL_DELAYED_TRIGGER` になっているのは**5枚のみ**。残りは遅延句が落ちて**即時実行**になっている疑い。🔴**ただし一括で設置へ倒すと退化する**＝最大クラスタ「このターン、**それが**アタックしたとき、」**16件はライフバーストの定型**で、**発動時点で既に相手がアタック中**なので即時実行でほぼ正しい（`WXDi-P04-068-BURST` 等）。■**着手の順序**＝①**16件のバースト定型を先に仕分けて除外**する（原文が「対象とする。このターン、それが…」の形）②残りを timing 別に分け、**collector に収集経路がある timing から**取る（`O-73` と同じ手順＝無い timing を先に足すと「設置されるが永久に発火しない」）。■明確な穴の例＝`WX10-035`（スペル「このターン、あなたのシグニ1体がアタックしたとき、デッキの一番上をエナゾーンに置く」が**即時エナチャージ**）。■⚠`O-71`（遅延本文が「それ」で先行文を照応）と隣接するが別軸＝`O-73` で「**tail 自身が対象を宣言していれば通す**」まで緩めてある。 |
...
| **O-108** | 🆕**コスト側の選択制約（「それぞれ名前の異なるN枚」）が `handDiscardSigni` に無い** | S〜M（UI を触る） | **2026-08-27 Sheet1 B10 で分離**＝`WX10-052-E3`「【起】手札から**それぞれ名前の異なる**＜精元＞のシグニを４枚捨てる：無色ではないすべてのシグニをトラッシュに置く。」■`EffectCost.handDiscardSigni`（`src/types/effects.ts:545`）は `{color, story, count, level}` だけで `selectionConstraint` を持たない（**エナ側の `energyTrash` は持っている**＝`:543`。§5-8′ の「兄弟枝にはある」型）。■**取り方**＝型に `selectionConstraint?: SelectionConstraint` を足し、**`SigniActivatedModal` と `LrigGrantedModal` の両方**の `canConfirm` で `distinct:'name'` を強制する（`inferDistinctKind`／`distinctConstraintOf` は既存）。⚠**片方だけ直すと入口によって払えたり払えなかったりする**。■**実機検証が要る**（モーダルの確定ボタン条件を変えるため）。 ■🆕**2026-08-27 続き692＝旧 `O-115` は本項と同一だったので統合した**（同じ `WX10-052-E3` の同じ穴を2度登録していた）。旧 `O-115` が挙げていた🔴**消費地点4つ**（`signiActivateGate.ts` の可否判定／シグニ【起】モーダル／`LrigGrantedModal.tsx`／`trashActivateCost.ts`）を**1つも漏らさない**こと＝1つでも漏らすと「提示は絞られるのに支払いは通る」。母集団＝「それぞれ名前の異なる」は22効果あるが**コスト側はこの1件だけ**（残りは対象選択・条件で受け皿あり）。 |
...
| **O-112** | 🆕**`ON_CARD_MOVED_TO_DECK` の「何が・どこから」動いたかを collector が持っていない** | M | **2026-08-27 Sheet1 B11 で分離**＝`WX05-019-E3`「対戦相手の**シグニ**１体が**場から**デッキに移動したとき」。■live の `triggerCondition` は `movedToDeckOwner`／`movedToDeckMinCount`／`movedToDeckFromTrash` の3つだけで、**「シグニであること」も「場からであること」も表せない**＝相手のカードがデッキへ動きさえすれば（トラッシュからでも手札からでも）発火する過剰実行。■🔴**受け皿を足すだけでは効かない**＝`collectMoveToDeckTriggers`（`triggerCollect.ts:2519`）は**枚数（`movedToControllerDeck` / `movedToOppDeck`）しか受け取っていない**＝呼び出し元が「どのカードがどのゾーンから動いたか」を渡す配線が要る（`O-64` と同じ「委ね先が読んでいない」型）。■**取り方**＝呼び出し元（中央 diff）で移動元ゾーンとカード番号を集めて渡し、`movedToDeckFromField` ＋ `movedToDeckFilter` を評価する。■母集団＝「デッキに移動したとき」原文6枚のうち、由来・種別限定があるのは `WX05-019` の1枚（着手時に数え直す）。 ■🆕**2026-08-27 続き692 で縮小**＝「**場から**」の受け皿は **B13 で実装済み**（`triggerCondition.movedToDeckFromField`＋`countMovedToDeckFromField`。live の `WX05-019-E3` にも入っている）。**残っているのは「シグニであること」＝`movedToDeckFilter` だけ**なので規模は M ではなく **S**。⚠旧 `O-116` は本項と同じ穴の重複登録だったので削除した。 |
| **O-113** | 🆕**「〜が対戦相手のアーツの効果を受けたとき」＝効果を受けた側からの誘発** | M | **2026-08-27 Sheet1 B11 で分離**＝`WX05-020-E2`「あなたの＜鉱石＞か＜宝石＞のシグニ１体が対戦相手のアーツの効果を**受けた**とき、対戦相手にダメージを与える。」■live は `ON_OPP_ARTS_USE`（＝**相手がアーツを使っただけ**で発火）＋ `activeCondition: HAS_CARD_IN_FIELD{鉱石,宝石}` の近似で、**そのアーツが自分のシグニに当たったか**を見ていない。■🔴**近似を外すと過小→過剰へ裏返る**（条件ごと消えて無条件発火になる）ので、受け皿ができるまで**両方据置**（§5-2″ の同型）。■**取り方**＝アーツ解決時に「効果を受けたカード」を記録する経路（`lastAffectedCards` 相当）を1本足し、`triggerCondition.affectedByOppArtsFilter` で評価する。■✅**同カードの `usageLimit` 脱落（《ターン１回》）は 2026-08-27 Sheet1 B12 で解決済み**＝manual 側に刻印を入れ、`collectOppArtsUseTriggers` に usageLimit 判定を足した（golden 済み）。ここに残るのは**「効果を受けたか」を見る受け皿だけ**。 |
...
| **O-117** | 🆕**「使用コストで《白》《赤》《青》《緑》《黒》すべてが支払われている場合」＝支払った色を条件にする** | M | **2026-08-27 Sheet1 B13 で分離**＝`WX05-016`（エンドホール）。■印刷コストは **《無》×５** なのに、parser が**効果文の色リストをコストとして採用**して `白1赤1青1緑1黒1` に化けている。■⚠**今の形は安全側**＝「5色払ったときだけターン終了」を「5色ちょうどでしか使えない」に狭めた**過小実行**で、過剰実行は作っていない。**印刷コストへ戻すだけだと `FORCE_END_TURN` が無条件になり過剰実行へ裏返る**ので、条件の受け皿ができるまで**据置**（§5-2″ の同型）。■**取り方**＝支払いで消費したエナの色を `last_paid_colors` として記録し、`PAID_COLORS_INCLUDE_ALL` 条件で読む。⚠記録地点は `planEnergyPayment` の1本に寄せる（アーツ／スペル／【起】で別実装にしない）。 ■🆕**2026-08-27 続き692 で実測**＝🔑**受け皿は半分ある**＝`ExecCtx.paidEnergyColorSets`（`execUtils.ts:107`・マルチエナは全5色／無色は空配列）と `PlayerState.pending_spell.paid_energy_colors` が**既に存在する**が、**書いているのはスペル経路の1箇所だけ**（`BattleScreen.tsx:7468`→`:7571`→`:7735`）。**アーツ経路（`:6897` の `artsPay`）は記録していない**。■**`planEnergyPayment` の呼び出しは15サイト**（`energyPaySource.ts:20` に明記）＝**`planEnergyPayment` の戻り値に `paidColors` を足すのが唯一の寄せ方**（サイトごとに書くと必ず片方だけ直る）。■消費は `execStubPart3.ts:1648`（`COST_COLOR_SELECT`）が前例＝**未提供時のフォールバック推定がある**ので、新条件は**フォールバックさせず fail-closed** にすること（推定で「5色払った」に倒すと過剰実行）。 |
...
| **O-120** | 🆕**【ランサー】によるライフクラッシュの由来限定が無い（実測3効果が全部同じ壊れ方）** | S〜M | **2026-08-27 続き692 で実測**＝`WX07-042-E1`（Sheet1）/ `WX19-028-E1` / `WX19-071-E1` は live が `ON_OPP_LIFE_CRASHED` に**条件ゼロ**＝■通常アタックのクラッシュでも発火する過剰実行。付与形の `WX24-P2-055`（【ランサー】か【Ｓランサー】）が4件目。■🔑**当ては確認済み**＝`crashOneLife`（`BattleScreen.tsx:8400`）が**唯一の funnel** ですでに `crash_source_card_num` を被害側 state へ刻んでいる。**同じ場所へ `crash_cause` を足し、`:12241` の収集で `triggerCondition.crashedByKeywords` を読む**（`movedToDeckFromField` と同じ fail-closed の形）。⚠**headless 側の `collectOppLifeCrashedTriggers`（`triggerCollect.ts:3745`）も同時に直す**（片方だけだと golden と実機がずれる）。 |
| **O-121** | 🆕**「このターンに…バニッシュしていた場合」の台帳が無い（実測2効果）** | M | **2026-08-27 続き692 で実測**＝`WX11-031-E1`（Sheet1）「このターンにあなたの＜空獣＞か＜地獣＞のシグニが対戦相手のシグニを合計３体バニッシュしていた場合」と `WXK02-034-E1`「このターンにあなたの効果によって…２体以上バニッシュしていた場合」。**どちらも live は条件ごと落ちて無条件発火**。■既存の `signi_banished_this_turn` は**バニッシュされた側の単なる件数**なので流用不可（誰がバニッシュしたかを持たない）。■🔑**当て**＝`signi_downed_this_turn`（`string[]` 台帳）＋`SIGNI_DOWNED_COUNT_THIS_TURN{owner,filter,operator,value}` と**全く同じ形**に揃える（別実装にすると片方だけ直る）。記録地点は**中央 diff（`BattleScreen.tsx:3168`）が唯一の funnel** で `causeOwnerId` / `causeSourceCardNum` がその場にある。■⚠**併せて測ること**＝`ON_SIGNI_BANISH_OPPONENT` は**バトルバニッシュ経路にしか配線されていない**（`BattleScreen.tsx:9888` のコメント）ので、効果バニッシュではそもそも `WX11-031-E1` が発火しない。 |
| **O-122** | 🆕**【出現条件】で支払ったカードを engine が記録していない（実測1効果）** | S〜M | **2026-08-27 続き692 で実測**＝`WX07-009-E1`（Sheet1）「【出】：このレゾナの**出現条件で同じ名前のシグニ２体をトラッシュに置いていた場合**、あなたのデッキの上からカードを２枚エナゾーンに置く」＝live は条件が無く**出ただけで常に2枚チャージ**。■`appearanceCondition` は JSON にあるが、**支払いで実際にどのカードを置いたかを誰も持っていない**（`resonaSummon.ts:225`）。■**取り方**＝召喚時に支払ったカード番号を `last_appearance_cost_cards` へ記録し、`Condition` を１本新設して**両評価器＋golden の3点セット**（§4.2）。■母集団＝原文「出現条件で」は全 CSV で **2枚**だが `WD11-007` の【出】は参照していない＝**真の母集団は1効果**。 |
...
| **O-125** | 🆕🔴**`BLOCK_ACTION{DRAW_OUTSIDE_DRAW_PHASE}` は engine のどこからも読まれていない死にフラグ** | S〜M | **2026-08-27 続き692 で発見**＝`WX05-022-E1`（Sheet1）【常】「対戦相手は自分のターンの間、グロウフェイズとドローフェイズ以外でカードを引いたりカードを手札に加えることができない」。■`actionId:'DRAW_OUTSIDE_DRAW_PHASE'` の出現箇所は `parseSentencePart1.ts:973`（生成側）**の1つだけ**＝消費地点ゼロ＝**何も禁止していない**（findings は「ドローしか禁止していない」と言うが**実態はもっと悪い**）。■🔑**兄弟枝にはある**＝`DRAW_OR_ADD_TO_HAND_BY_EFFECT` は `effectExecutor.ts:176` / `:2786` で消費されている（§5-8′ の型）。■併せて直す＝原文にある**「自分のターンの間」**と**グロウフェイズの例外**も live に無いし、`until:'END_OF_TURN'` は【常】に対して誤り。母集団＝`WX05-022` / `WXK10-010` の2枚。 ■🆕**2026-08-27 続き692 で配線を実測**＝🔴**素直に `ExecCtx` へフラグを足すと片肺になる**。`deckToEnergyBlocked` 等の CONTINUOUS 由来フラグは **`BattleScreen.tsx:4919`（スタック解決経路）でしか設定されておらず**、ExecCtx の構築地点は BattleScreen だけで**8箇所**（`:4919` `:5283` `:5584` `:5639` `:5689` `:5751` `:7735` `:8098`）ある。⇒ **`activeOppMoveImmunityZones(state)` と同じ「state だけで読める」形へ寄せる**のが正解（`effectEngine.ts:3533` に同じ判断が明記されている）。■**消費地点は2つ**＝`execDraw`（`effectExecutor.ts:176`）と `execTransferToHand`（`:2786`）。`ExecCtx` には `currentPhase` と `isOwnerTurn` が既にあるので**「自分のターンの間」「グロウ/ドロー以外」の判定材料は揃っている**。 |
| **O-126** | 🆕**`COST_INCREASE` に「カットインされたスペルは影響を受けない」の除外が無い** | S | **2026-08-27 続き692 で分離**＝`WX09-Re05-E1`（Sheet1）「（**カットインされたスペルはこの効果の影響を受けない**）」。live の `COST_INCREASE` に除外フラグが無く、カットインにも課税している。■母集団＝「カットインされたスペル」は全 CSV で **3枚**（`WX09-Re05` / `WX10-023` / `WX13-005B`・Sheet1 は2枚）。■⚠**括弧内ルール説明なので `stripRuleParens` で落ちる**＝`cost.exceedColors`（B13）と同じく**剥がす前に符号化**する形になる。 ■🆕**2026-08-27 続き692 で実測**＝`COST_INCREASE` は `execCostIncrease`（`effectExecutor.ts:6810`）が **`PlayerState.cost_modifiers`** へ積む。除外フラグは**この mod へ持たせ**、`applyContinuousCostDecreases` の**3つの呼び出し元**（`artsUseGate.ts` / `spellUseGate.ts` / `CutinModal.tsx`）へ「いまカットインか」を渡す。⚠**「いまカットインか」を各サイトで再判定させない**（判定が割れる）＝引数で受ける1本にする。 |
| **O-127** | 🆕**「好きな枚数…し、**その枚数と同じ枚数**の…」＝直前の選択枚数への後方参照** | M | **2026-08-27 続き692 で分離**＝`WX05-010-E1`（Sheet1・HIGH）「あなたのすべてのライフクロスを見て、その中から**好きな枚数**をトラッシュに置き、**その枚数と同じ枚数**のカードをデッキの上からライフクロスに加える」→ live は `ADD_TO_LIFE{count:1}` の**固定1枚**。■「好きな枚数」自体の受け皿（`count:'ALL'`＋`upToCount`）は既存。**無いのは「直前に選んだ枚数を後段が読む」紐付け**。■母集団＝「好きな枚数」は全 CSV で **82枚**（Sheet1 6枚）だが、**後方参照する形は着手時に数え直す**。 |
| **O-128** | 🆕**【常】の中の引用【自】能力付与が `STUB{GRANT_ABILITY_INNER_TEXT}` へ落ちる** | M〜L | **2026-08-27 続き692 で分離**＝`WX09-Re07-E1`（Sheet1・HIGH×2）【常】「…合計３体あるかぎり、このシグニの基本パワーは10000になり、このシグニは「【自】：対戦相手の効果によってこのシグニが場を離れたとき、…探して場に出し…」を得る」。live は `POWER_SET` ＋ `STUB{GRANT_ABILITY_INNER_TEXT}`＝**付与される【自】が丸ごと消えている**。■`GRANT_ABILITY_INNER_TEXT` は live **34カード**の catch-all で engine が実行時に原文を regex で読み直す＝**`O-60` A群の一部**。■`GRANT_QUOTED_AUTO_ABILITY`（既存・miss3/live4）が受け皿候補。■**1カードだけ直さない**＝着手時に 34カードを分類して同型をまとめて取る。 |
| **O-129** | 🆕**「〈対象宣言〉を対象とし、〈中間動作〉。そうした場合、それを…」で対象の確定が中間動作より後になる** | M（🔴影響 118カード） | **2026-08-27 続き692 で分離**＝`WX02-020-E1`（Sheet1・MED）「対戦相手のシグニ１体を対象とし、**あなたのシグニ１体を場からトラッシュに置く**。そうした場合、それをバニッシュする」＝原文は**先に相手シグニを対象に取る**が、live は自分のシグニを捨てた**後で**相手シグニを選ぶ。■🔴**「parser だけの安い修正」ではない**＝`applyDroppedTargetDesignation`（`effectParser.ts:8915`）は **filter/owner/count が帰結と食い違うときだけ**束縛する保守ガードを持っており、**この効果はどれも一致するので意図的に据置されている**。ガードを緩めると**実測118カード**（文型該当182／うち64は固定済み）が一斉に `SELECT_TARGET_ONLY` 形へ変わる＝PLAN が警告する「無検証置換で約90枚退化」と同じ規模。■**取り方**＝「中間動作が**別プレイヤーのカード**を動かす（＝対象と競合しうる）」等の**狭い追加条件**を作り、変化する効果数を1桁に保ったうえで1件ずつ原文照合する。**件数を目標にしない。** |
