...
- **🆕 セッション（2026-07-09・続き53・Opus 4.8・§5c census 条件節「あなたの場にレベルNの覚醒状態のシグニがある場合」＋トリガー句後の条件節脱落＝CONDITIONAL持ち上げ32枚）**＝「このシグニがアタックしたとき／アタックフェイズ開始時、〜がある/いる場合、〜」型で盤面条件節が丸ごと脱落しアタックのたびに無条件発火していた過剰効果を系統是正。**2つの独立 gap を同時に解消**：**①覚醒状態フィルタの語彙欠落**（本題3枚 WXDi-P14-054/058/066）＝新規 `TargetFilter.isAwakened`（`ownerState.awakened_signi` に CardNum）＋engine の HAS_CARD_IN_FIELD 判定を2評価器へ（`execUtils` inline＋`effectEngine matchesStateFilter`・ルリグ走査ガード3箇所に isAwakened 追加）＋parser CLAUSES 規則＋decompiler filterJa 描画。**②CLAUSES 持ち上げがトリガー句の後ろで不発**（横展開29枚）＝状態条件節 CONDITIONAL 持ち上げループの先頭プレフィックス正規表現が `このシグニがアタックしたとき、`（本ループより後の `:1378` で除去）を許容せず ON_ATTACK_SIGNI カードで条件節が文頭に来ていなかった（ON_ATTACK_PHASE_START のみ上流除去で成立していた非対称）。先頭に `このシグニがアタックしたとき` を許容＝既存全 CLAUSES（HAS_CARD_IN_FIELD 場/トラッシュ・ALL_FIELD_SIGNI_MATCH・HAND/ENERGY_COUNT）がトリガー句後でも一律に効く。**採用32枚**（heldReview 精密diff＋数値/owner/filter 全数機械照合で原文一致確認・純加算 CONDITIONAL ラップ）。**不採用**＝WXDi-P11-048（複合「黒10枚以上あり相手エナ2枚以上」＝AND を単一 clause が `minCount:20` へ誤パース＝退化）。SEQUENCE-nested OPTIONAL_COST 系は field 条件が step[0] コスト提示を正しくゲート＝curated 比純改善（step[1] IS_MY_TURN 常時真は既存別課題）で採用。**検証**＝typecheck・**golden 173→174（+1＝Lv3覚醒シグニ有→発火/非覚醒→no-op/Lv不一致→no-op）**・smoke 全0（10582・nested STUB-in-CONDITIONAL もクラッシュなし）・fuzz 全0・lint 0 errors・**同型★0**・**census 1588→1574（-14）**＝`BASELINE_HIGH` 実数更新。**次の一手＝(A)Opus＝effect-restriction 14枚の実装**（「対戦相手はシグニをN体までしか場に出せない」＝相手シグニ配置数制限の新機構〔WXDi-P05-024/WXK05-009/WX07-006/WX12-008〕・「〜ないかぎり新たに場に出せない」自己配置制限〔WX14-033/WX18-075〕・アタック/ガード不可＝§6 BLOCK 機構）。**(B)Opus＝§5c census 条件節の残**（色付きアーツ/スペル使用歴〔ARTS_USED 拡張〕・「それがレゾナの場合、代わりに」＝WX25-P2-068/070 の代わりに置換・「あり」複合条件 WXDi-P11-048）。**(C)Opus＝§4タスク2 動的比較 lrig相対 WX19-042 mis-parse・タスク3 引用内CHOOSE（WXDi-D09-P20）＋GRANT_QUOTED_AUTO_ABILITY 内側parse（WX24-P1-017/WX25-P3-038）。(D)Sonnet＝§5c 再収穫・§7 実機検証（続き43-53 の採用効果）。(E)持ち越し：SEQUENCE 下流「そうした場合」IS_MY_TURN 常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDrive バッチ状態汚染。**
