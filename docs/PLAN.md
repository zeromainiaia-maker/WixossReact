...
- **🆕 セッション（2026-07-20・続き219〜220・Opus・**§3 タスク12(xxxix)+(xxii)+(vii)＝条件脱落の過剰効果を4バッチで計23枚消化＋(vii)クローズ**。census 1880→**1868**・golden 517→**520**）**（続き218k は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **バッチ2（219b）＝CHOOSE 前状態条件の汎用持ち上げ**＝「場に《X》/＜C＞/レゾナ がある場合、以下の…から選ぶ」形で、CHOOSE 分解がヘッダ以前を捨てて先頭条件が脱落し**毎アタックフェイズ無条件で CHOOSE 発火**していた過剰効果を是正。`parseBlock` の `fieldLevelSetM`（「レベルN,M,K」専用）の直後に `matchLeadingStateCondition`＋「残りが CHOOSE ヘッダ `^以下の[数字]` のときだけ `CardEffect.condition` へ昇格」を追加。**10枚是正**（WX24-P2-048／ディソナ2体 WXDi-P12-006/008／レゾナ SP27-011 ほか・全て `isPureSuperset` 自動採用）。据置2枚＝WX25-P2-085／SPDi43-30（選択肢内に無関係な既存ドリフト有り＝fresh 採用で退化）。全ゲート緑（**golden 518**・CHOOSE 条件持ち上げを固定／**census 1878→1874**）。詳細 BUGFIXES 続き219b。**残＝「アタック無効」系3枚・WXK09-003 赤分岐（ライフクロス→エナ＝新ゾーン遷移§6.3）・WXDi-P06-039 対象照応・Magic Box 3件・上記据置2枚（いずれも §6.3 か別ドリフト）**。
  - **バッチ3（219c）＝手札公開 REVEAL の source 脱落是正（タスク12(xxii)）**＝「手札から＜C＞のシグニをN枚公開する」が bare `{type:REVEAL}`（source/filter/count なし）に潰れ、engine が `lastProcessedCards` を記録せず「この方法でシグニをN枚以上公開した場合」の結果カウント条件が IS_MY_TURN 化していた系統。`parseSentencePart2` の手札公開規則を「公開してもよい」限定から「公開する／N枚・N枚まで・好きな枚数」へ一般化し `REVEAL{source:HAND_CARD}` を復元。**純改善6枚自動採用**（WXDi-P10-045／WXDi-P11-075／WDK08-Y17／WX21-065／WX22-044／WXK04-088）＋**WX21-023 採用**（公開2枚ゲート復活・退化なし）。据置＝WDK08-Y01（対象照応）・WXK04-034（「代わりに」置換）＝別機構。全ゲート緑（**golden 519**・**census 1874→1869**）。詳細 BUGFIXES 続き219c。**残＝WX21-023 の3枚/4枚カスケード閾値・WXK04-034 の「代わりに」置換・「レベルの異なる」等の捕捉不能フィルタ**。
  - **バッチ4（220）＝タスク12(vii) クローズ**＝残り1枚 `WX25-P2-112`「アップ状態のルリグをダウン→この方法でダウンしたルリグと共通する色を持つ相手エナをトラッシュ」を実装。parser は DOWN を SIGNI に取り違え・TRASH を無条件＆色フィルタ無しにしていた。`execDown(LRIG)` がダウンしたルリグを `lastProcessedCards` に記録＋アップ状態チェック＋optional 二択を追加し、`colorMatchesLastProcessed` 動的フィルタ（owner非依存・参照不能で空ヒット＝did-it ゲート兼用）を新設して MANUAL 化。golden 519→520・census 1869→1868。詳細 BUGFIXES 続き220。
  - **次の一手**＝タスク5 の残小口（`WX26-CP1-100`＝SEND_TO_ENERGY のトラッシュ対象化 ほか）／**timing[C] 残43効果（タスク16）**＝上位クラスタが3・2・以降すべて1件のロングテールで機構コスト高＝ROI 低／**タスク12(xxii) 残50件**／**(xxix) 残＝`WX06-014-E2` の JSON が原文と別物**（step1「自分のトラッシュから《古代兵器》5枚をデッキ下」が「相手シグニをデッキ下」に化け・step2「それ」照応欠落＝エクシード相当コスト＋対象照応の機構要・§6.3級）／**置換系統40枚の一般化（§6.3級）**／(xlvi) 残（MANUAL 過剰簡約群＝§6.3級）・(vii)(viii)(xlii)(xliv) の残。**⚠(xlvii) の防御側収集は §7 実機検証対象**（相手ルリグアタック時に `WXDi-D06-010`「そのアタック無効」が発火し `pending_lrig_attack` と噛み合うか＝Sonnetタスク1へ）。**Sonnet はタスク1（§7 実機検証）**。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 526・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1866。
>
