...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377e＝(i)配線ギャップ 第10バッチ後）**＝golden **1448**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1032**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **208枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 349**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第11バッチ。⚠セルを取ったらまず全数分類→偽陽性の型が見えたら計器を較正する（実装ゼロ）。**
>    - **① `cardClass × (filter無)` 33（has 83）★★** ＝較正後も最大セル。⚠**`(filter無)` は条件節用法が濃い**と従来書いていたが、続き377e の較正でその多くは除去済み＝**残りは本物が濃い可能性がある**。まず `--cell` で全数分類する。
>    - **② `cardClass × SIGNI[filter]` 19（429）★★／`cardClass × TRASH{HAND_CARD}` 9（67）★★／`cardClass × TRASH_CARD[filter]` 7（284）★★**。
>    - **③ `isDisona` 24（has 31）** ＝続き376d で判定器は直したが未配線の入口が残る。
>    - **④ `color` 31（302）／`levelExact` 29（103）／`powerRange` 30（507）** ＝⚠`powerRange` は「パワーを＋N」＝**action の値**との取り違えが濃い（**較正候補**）。
>    - **⑤ 小粒だが has があるセル**＝`isPuppet` 10（2）／`isAwakened` 10（3）／`levelParity` 8（5）／`excludeResona` 6（1）／`nonColorless` 5（32★）／`crossState` 4（21★）。
>    - ⚠**着手前に必ず読む11の罠**＝(a) **素の `parseStoryFilter`/`parseLevelFilter`(文全体) を対象フィルタに使わない**＝**`signiClause*Filter` 3兄弟**（クラス／アイコン／レベル）を使う。**新語彙はまずここに兄弟を足せないか見る。** (b) **曖昧なら付けない**（別ピック2本の span に AND で載せると原文と逆の過小実行）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**。(d) **exec 側の配線を必ず確認する**（`matchesFilter` は `excludeSelf`/`upToCount` を見ない）。(e) **セルは入口であって終点ではない**＝ビルダー全体を読む。(f) **枚数・値だけ先に直さない**＝構造が誤ったままだと誤りを増幅する。(g) **★★セルの miss 数は見込み件数ではない**（実測で 6件中5件／22件中14件が偽陽性）。(h) **同じ概念に2つのキー綴りが併存していないか先に確かめる**（`hasCrossIcon` vs `hasIcon`）。(i) **同じ語彙でも入口ごとに壊れ方が違う**＝1つ直したら同族の入口も必ず見る。(j) 🆕**A/B の件数＝直った件数ではない**（live が MANUAL で既に正しい分がある）。(k) 🆕**採用前の確認は JSON パースによる構造比較で**（文字列 strip はネスト括弧で誤検知する）。
>    - **✅消化済み**＝「他の」ゲート棚卸し（377）／`noGuard`（377b）／アイコン系（377c）／`levelRange`（377d）／全体バフの語順＋据置held（377e）。
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **4件**／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**／「場に《ライズアイコン》を持つシグニがN体ある場合／あるかぎり」の条件型 **5件**／🆕**左右のシグニゾーン限定の filter キーが無い 3効果**（`WX25-P3-093-E1`/`E2`／`WXK10-078-E1`。`centerZoneOnly` はある）。
>    - **(iii) 構造混線の在庫**＝`WXK05-043-E1`（owner 取り違え＋level 脱落＋条件節脱落の3重）／`WX24-P3-059-E1` ほか §5d-0 (iii) の登録分。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない。
>    - ⚠**見積もりの現在地**＝census **1032**／被覆マトリクス miss **349**。5バッチの実績＝12／30／17／8／15効果。**census が動かない回でも効果は直る**（377c は census 据置で17効果）＝**census の増減だけで成果を測らない。**
