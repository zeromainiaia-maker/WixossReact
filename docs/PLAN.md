...
### 📌 次の一手（推奨順）
> まず `npm install` → `npm run typecheck && npm run golden && npm run smoke && npm run fuzz` が全部緑になることを確認（CIでも自動実行される）。これが回れば環境OK。現状＝golden 127/127・smoke/fuzz 全0・同型★0・census 1800/1800・parserWorklist held 25（LOSS12/VALUE13）。
>
> **現在の主作業＝§5c census文型バッチの継続消化（続き23確立のパイプライン・cold startはこの5行）**：
> ① `npm run census:clusters` → `docs/_census_clusters.txt` を枚数順に見てテンプレを選ぶ（未処理上位＝「次にダメージを受ける場合」47枚＝§6.1 PREVENT_DAMAGE 機構持ち・「代わりに」昇格置換・「場に《X》がいる」13枚。機構待ちは §6.3 へ送る。⚠着手前に続き24の型＝**全数機械分類で偽陽性を先に切る**：「それが＜C＞73枚」は70枚が REVEAL_AND_PICK 済み偽陽性だった）
