...
**Opusタスク12＝未消化の在庫 5件**（2026-08-10 続き404・408 の2件＋**2026-08-14 続き476 で (cxxxi)(cxxxii)・続き477 で (cxxxiii) を登録**）。⚠**2026-08-14 続き475 で (cxxv) を取り下げ（シナリオ偽陽性）**、**続き475b で (cxxvi)(cxxx)**、**続き475c で (cxxix)**、**続き475d で (cxxvii)(cxxviii)**、**続き475g で (cxxiii)** を残0クローズ＝8件→2件。

| # | 症状 | 発見経緯 | 見立て |
|---|---|---|---|
| 🆕🔴(cxxxiii) | 🔴**付与ストアの `ON_ENERGY_CHARGE` watcher だけが恒久 no-op**＝`GRANT_LRIG_ABILITY` で積んだ `ON_ENERGY_CHARGE` の【自】が、**エナが1枚増えても一度も発火しない**。付与自体は成立している（`lrig_granted_auto_effects` に入り `grantedLrigAutoIds` で観測できる）。⭐**同じスペル経路・同じ盤面で「印刷能力」の `ON_ENERGY_CHARGE` は発火する**ので、**CHOOSE 解決後の中央 diff（`BattleScreen.tsx:1766` の useEffect）そのものは生きている**＝**同ブロック内の `grantedStoreWatchers()` 呼び出し側だけが効いていない** | 2026-08-14 続き477＝§7 V-12 の実機検証。`SPDi43-13-E2` を【起】から撃って付与→`WXDi-P12-082`（羽化）の【エナチャージ１】で `energy 0→1` になるのに `lrig_down` が true のまま。**実機シナリオ 2本が赤で待機**（`v12GrantedEnergyChargeTwice`／`v12GrantedEnergyChargeThirdBlocked`）＋**切り分けの対照が緑**（`v12PrintedEnergyChargeControl`＝`WXDi-P11-073-E1` に +2000）| **同経路の他 timing は生きている**＝`ON_SPELL_USE`（`:7057`）と `ON_SIGNI_BANISH_OPPONENT`（`:8964`）の付与 watcher は同じ続き477 で**実機 PASS**（V-12 参照）。⇒ 疑うのは `:1766` ブロック固有の条件＝`ecLrigTop` の取り方・`addedToEnergy.length === 1` の判定タイミング・`entries` を積んだ後の解決経路のいずれか。⚠**この経路は `actions_done` へ書き戻さない**ので、直した直後に《ターン2回》の超過が露出する見込み（B-4 のシナリオが既にある）|
...
- **🆕 セッション（2026-08-14・続き475〜475g・Opus 5）＝**§7 実機検証の続き＋そこで見つけた engine バグの一掃**＝**V-01／V-02／V-03／V-09④／V-10 が全シナリオ緑**。**§3 在庫は 8件→2件**＝(cxxv) を「シナリオ偽陽性」として取り下げ、**(cxxvi)(cxxx)(cxxix)(cxxvii)(cxxviii)(cxxiii) を残0クローズ**。ゲート全緑（**golden 1964→1975**＝トリップワイヤ11本追加・census 831 据置・smoke 10688／SKIP 0・fuzz 全0・同型★0・held 106 据置・lint 0 errors/259 warnings）。**
  ...
  - **▶ 次の一手**＝**Opus 側**：§3 の残在庫は **2件**（続き404・408 登録分）。次点＝§6.4 O-20 の残21件・O-19。🆕**ピース経路（(cxxiii)）は続き475g で決着**＝「使用＝1回払って即解決→ルリグトラッシュ」。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。
...
- **🔶 V-12 アタック可否ゲート一本化＋付与ストア共通走査（続き404）＝2026-08-14 続き477 で**大半決着＝8シナリオ中6本が緑・2本は engine 実バグで赤**（Codex 起案→Claude 実機検証・**2回連続で同じ内訳**・既定 order 登録済み）。
  ...
  - [ ] 🔴**付与された【自】が ON_ENERGY_CHARGE で発火するか＝実機FAIL＝engine の実バグ**（→§3 **(cxxxiii)**）。`SPDi43-13-E2` を【起】から撃って付与は成立する（`grantedLrigAutoIds` に `SPDi43-13-sub-E1`）のに、**エナが1枚増えてもルリグがアップしない**＝`v12GrantedEnergyChargeTwice`／`v12GrantedEnergyChargeThirdBlocked` が**赤のまま既定 order に置いてある**（engine が直れば緑へ反転する）。
    - ⭐**切り分け済み**＝**同じスペル経路・同じ盤面で印刷能力の `ON_ENERGY_CHARGE`（`WXDi-P11-073-E1`）は発火する**（自身に +2000）＝`v12PrintedEnergyChargeControl` が**緑**。⇒ **経路（CHOOSE 解決後の中央 diff）は生きており、`BattleScreen.tsx:1766` の `grantedStoreWatchers()` 側だけが拾われていない**。
    - 📋**《ターン2回》の超過は未判定**＝1回目すら発火しないので観測に到達しない。**(cxxxiii) を直したら B-4 で回数制限を見る**（この経路は `actions_done` へ書き戻さないので、直後に超過が見える可能性が高い）。
