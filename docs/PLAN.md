...
**直近＝2026-09-22＝🏁§5.7 `S-31` ② 第6段＝残り全部（撃てない【起】 15 → 0）＋実機の「じゃんけんが解決しない」バグ**（全文は [BUGFIXES.md](./BUGFIXES.md) 第449バッチ）
- 🏁📏**撃てない【起】は 0**（場のシグニ 695／ルリグ 567 のどちらも 0）＝**210 → 108 → 37 → 26 → 15 → 0**。
- この回の内訳＝**支払いが無かった3**（`handBottomDeck`／`trapToHand`／`beat_signi`(ルリグ)）＋**提示ゲートの検算が無かった数件**
  （🔴`beat_signi` は「場に1体でも居れば提示」＝**「他のシグニ1体」なのに自分しか居ない盤面でも提示**されていた）＋**CPU の選び方だけ 12**。
- 🔑**可変枚数は最低枚数だけ払う**（`discardUpTo` は0枚）＝**過剰に払わない側へ倒す**。
- 🔴**実機の flake は本物のバグだった**（`V-284`）＝じゃんけん解決の `setTimeout(1800ms)` を**再レンダーのたびに張り直して**おり、
  更新が続くと**永久に解決しない**（画面は「あいこ！」のまま）。📏**今日の実機3回とも初回で発生 → 修正後は初回 PASS**。
- 📏**実カード照合**＝提示された **44（場のシグニ）＋20（ルリグ）すべてで支払い選択が成立**（食い違い0）。
  ⚠**手札・トラッシュを厚くして測る**＝候補が無いと提示ゲートで落ちて**母集団から静かに消える**（計器が緑に見える）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-29`**（宣言の帰結のコスト＝母集団未計測）。⤵`S-31` は ②まで完了 |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／**実機 `V-nn` 1**（`V-284`＝じゃんけんの回帰ガード）／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 1**（`S-29`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**入口が場ではない2効果**（`handExileSelf`／`energyTrashSelf`）は**いま撃てない**＝手札・エナの入口を作るのが残り |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4367**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
**残1**＝🆕**`V-284`＝じゃんけんが「あいこ」で止まらないことの回帰ガード**（2026-09-22 第449バッチで**解決の `setTimeout` を再レンダーのたびに張り直していた**のを修正。⚠**実機3回とも初回で再現 → 修正後は初回 PASS** だが、シナリオ化した回帰ガードはまだ無い）。
...
- **2026-09-22 時点**（第421〜449バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 2**（🔵`S-6`・`S-29`）｜**作戦データの入力 21デッキ**（ユーザー作業）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
