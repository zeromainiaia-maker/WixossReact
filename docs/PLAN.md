...
**直近＝2026-09-19＝§5.7 を第2版へ改訂＝CPU の強さを「ターン2分割の探索」へ再編**（設計判断のみ・`src/` は未変更／経緯は commit `cefebd7f9`）
- 🔴**診断**＝`cpuTurnAction` は**固定台本**（順番がハードコード）＋**決定点ごとに通貨がバラバラ**（`value`／`SPELL_GAIN_MIN`／分類 allowlist／enum 順）＝**貪欲の総和はターンの最善にならない**。
- 🆕**ユーザー決定**＝ターンを `ATTACK_ARTS_OP` で**2ブロックに割って探索**する（割る場所は `TurnPhase` の境界と一致＝新しい配管は不要）。到達点は「読み落としが少ない中級者」。
- 🆕**カンニングの線引き**＝「山の**集合**は知ってよい／山の**順序**は見てはいけない」。⚠**いまの先読みは越えている**（`simulateEffect` → `execDraw`（`effectExecutor.ts:382`）が本物の山の一番上を引く）＝`S-16` の着手前に塞ぐ。
- **キュー第2版**＝`S-9` → `S-10` → `S-15`（候補列挙器）→ `S-16`（前半探索）→ `S-17`（後半＝攻撃探索）→ `S-18`（次ターン項）→ `S-14` → `S-6`。`S-8`/`S-11`/`S-12`/`S-13` は `S-15`〜`S-17` へ**吸収**（登録票は PLAN_DETAIL に残置・先頭に吸収先を明記）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-9`**＝強さの A/B 測定台（席ごとのポリシー＋席入れ替え再戦＋N戦の勝率と誤差）。🔴**判断ロジックを触る項目は全部この後**。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**docs のみの回＝live JSON も `src/` も1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 8**（`S-9`・`S-10`・`S-15`〜`S-18`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝先読みの情報漏れ（`simulateEffect` が実 state を clone するので engine のドローが**本物の山**から引く）。いまは効果1つぶんで実害小＝`S-16` の前提条件として PLAN_DETAIL に登録。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4331**・`selfplay` 1戦 決着・`census:traceinv` I1 0） |
...
- **2026-09-19 時点**（第407バッチ＝§5.7 第2版＝CPU の強さを「ターン2分割の探索」へ再編）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**docs のみの回＝live JSON も `src/` も触っていないので3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 8**（`S-9`・`S-10`・🆕`S-15`〜`S-18`・`S-14`・`S-6`＝`S-8`/`S-11`/`S-12`/`S-13` は吸収して非計上）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4331**＝±0・`census:traceinv` I1=0・`selfplay` 1戦 決着）
  - 📏**BattleScreen**＝**6,287行**（±0＝`src/` 未変更）
  - 📊**ヘッドレス自己対戦**＝1戦 ≒ **332手・14ターン・約36秒**（seed=1）
