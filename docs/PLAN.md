...
- **🆕 セッション（2026-07-07・続き40・Sonnet 5・§7 実機検証シナリオ横展開＝R38で実バグ発見）**＝`scripts/verifyBattleDrive.mjs` に新シナリオ **`freezetrigger`**（WX01-081→WXDi-P04-065）を追加し、**R38「対戦相手のシグニが凍結状態になったとき」の①発火を実UIで検証した結果＝❌FAIL＝実バグを確認**。FREEZE自体は正しく適用される（`guest.field.signi_frozen`が`[true,false,false]`に変化）が、watcherが一切発火しない。**原因を特定**＝`collectFreezeTriggers`/`detectNewlyFrozen`（`BattleScreen.tsx:3798`）は`resolveStackNext`の中央diffにしか配線されておらず、SELECT_TARGETを要するON_PLAY効果がresume経路（`handleEffectInteraction`）で完結する大半のケース（＝FREEZEカードの通常形）で無発火。同種の取りこぼしは`ON_DECK_SHUFFLED`等では`collectXxxInline`パターンで既に対策済みだが`ON_SIGNI_FROZEN`だけ未対応＝**修正は既存パターンの横展開で直る見込み**（詳細・修正方針は §6.3「ON_SIGNI_FROZEN のresume経路取りこぼし」・VERIFY_BROWSER.md）。**engineは触っていない**（観測のみ・PLAN運用ルール通り修正はOpusへ）。`freezetrigger`は既定`order`から除外（既存スイートの全緑を維持）・修正後に単体再検証してから戻す。詳細 BUGFIXES 最上部。**次の一手＝(A)Opus側でON_SIGNI_FROZENのinline collector追加（§6.3参照・低リスクな横展開）＋`freezetrigger`再検証。(B)Sonnet側は引き続き§7残る実機検証項目（R30/R31/R36/R39-R46・ON_LRIG_ATTACK_STEP_START全体未検証等）から1件ずつシナリオ追加→観測記録。(C)verifyBattleDrive のバッチ実行時状態汚染の根本修正（driver側テスト分離強化）は続き39から持ち越し未着手。(D)Opus側で`GRANT_TO_PLACED_SIGNI`実装・census「動的比較35枚」・引用内CHOOSE（WXDi-D09-P20）が続き36から未着手のまま残る。**
...
- **ON_SIGNI_FROZEN のresume経路取りこぼし（2026-07-07・続き40・§7 R38実機検証で発見・WX08-039/WXEX2-02/WXDi-P04-065が対象）**＝`collectFreezeTriggers`/`detectNewlyFrozen`（`BattleScreen.tsx:3798`）は`resolveStackNext`の中央diffにのみ配線されており、SELECT_TARGETを要するON_PLAY/AUTO効果がresume経路（`handleEffectInteraction`、4386-4408行）で完結する（＝effect_stackを使わない）大半のケースでwatcherが無発火。実機で`guest.field.signi_frozen`が正しく`true`に変化するのにwatcherログが一切出ないことを確認済み（`node scripts/verifyBattleDrive.mjs freezetrigger`で再現・既定orderからは除外済み）。**修正方針は確立済みパターンの横展開**＝同じ取りこぼしは`ON_DECK_SHUFFLED`/`ON_SIGNI_BANISH_OPPONENT_BY_EFFECT`/`ON_LRIG_UNDER_MOVED`/`ON_KEYWORD_GAINED`で`collectDeckShuffleInline`等のinline collectorとして`handleEffectInteraction`（4386-4408行）に既に対策済み＝同型の`collectFreezeInline`を追加して`detectNewlyFrozen(bs.host_state/guest_state, 更新後 hostState/guestState)`→`collectFreezeTriggers`を呼び、pendingEntries配列に合流させれば直る想定。修正後はgolden/smoke/fuzz＋`node scripts/verifyBattleDrive.mjs freezetrigger`で単体再検証し、PASSしたら既定orderに戻す。
