...
- 🏁**セッション（2026-08-28・続き699・Opus 5 単独）＝`O-131`。アーツ使用トリガーの収集が「解決の完了地点」の片方にしか無かった。**
  ユーザー指示＝`O-131`。
  📊**進捗3計器＝Sheet1 要対応 26 / 863（3.0%・据置）｜台帳 残 OPEN 575（据置）｜census 高シグナル 522（据置）**
  ＝**3計器とも動かない**。理由＝**live JSON は1バイトも変わらない**（欠けていたのは engine/UI の呼び出し配線で、
  カードの表現は既に正しかった）。⇒ **この巡の成果は実機でしか観測できない**（`b22*` の FAIL→PASS）。
  gates 全緑。実機＝**`b22artshit` / `b22artsmiss` が FAIL→PASS**（`O-113` の観測点が閉じた）。
  - 🔑**真因は1行**＝`selectOrInteract` は**候補が1件でも必ず中断する**（自動適用の分岐が無い）ので、
    **対象を取るアーツは `handleEffectInteraction`（対話解決）で完了する**。収集は
    `resolveStackNext` の完了分岐にしか無く、**対象を取るアーツ＝ほぼ全部**で発火しなかった。
  - 📐**母集団**＝`ON_OPP_ARTS_USE` **6効果** ＋ 裏返しの `ON_ARTS_USE` **9効果**（同じ穴だったので両方直した）。
  - ⚠**golden が緑のままだった理由**＝収集関数を**直接**呼んでおり、**どの完了地点から呼ばれるか**を見ていなかった。
    ⇒ 2つのヘルパが**それぞれ2経路から呼ばれる**ことと、**「候補1件でも中断する」前提**そのものを assert した。
  - 🔴**同じ形を3回踏んだ**＝`O-126`（`cost_modifiers` の読み手が無い）／`O-117`（カットイン窓が
    `forceEndTurn` を読まない）／本項。**値を作る側が正しくても、消費側の入口を全部押さえたかは別問題。**

**▶ 次の一手**＝**`O-130`（`ON_OPP_ARTS_USE` の帰結が「そのシグニ」＝効果を受けたシグニを指せない）**。
`O-131` で発火が通ったので、次は**帰結側**＝`WXK11-019-E2` は「そのシグニをアップし…能力を失う」なのに
live は `REMOVE_ABILITIES{owner:'opponent'}` のみ（アップが落ち、対象も逆）。
`collectOppArtsAffectedOwnSigni` の結果を StackEntry へ載せて本体へ束縛する（`triggeringCardNum` と同型の1本）。
