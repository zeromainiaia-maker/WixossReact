...
**直近＝2026-09-20＝デッキフォーマット（ディーバ／レガシー／オールスター）を新設**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- **判定**＝`src/utils/deckFormat.ts`。ディーバ＝`WXDi*`/`PR-Di*`/`SPDi*` と **`WX24` 以降**（数値判定＝WX27 以降も自動）。
- 🔴🔑**本体の番号だけで判定しない**＝再録で**本体がレガシー番号なのにディーバでも使える札が 91枚**（`WD03-005 コード・ピルルク` ほかセンタールリグ群）。**全印刷を見る。**
- **配線**＝`deckAddBlockReason` に `FORMAT`（第4引数の避難先索引は**必須**）／デッキ設定に 🎴フォーマット／CPU ランダムに**フォーマットの行＋「🎲 全員のルリグ」タイル**（抽選は**絞り込み後**から引く）。
- **DB**＝`decks.deck_format`（`docs/sql/2026-09-20_deck_format.sql`・**実行済み**）。🔴**列が無いとデッキの保存が全部静かに失敗する**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-21`**（探索の目的関数）＝`S-16` の探索は**従来と互角**（A/B 50.0%）で止まっている＝**強くするには終端の評価**（アタックで通るダメージ／重みの学習）が要る。🆕**`S-20`（A/B の山が型を踏まない）は探索の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 6**（`S-21`・`S-17`・`S-19`・`S-20`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先攻が勝てない（`S-19`）②**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4341**＝+2）｜**実機** `node scripts/archive/verifyDeckFormat.mjs`／`verifyCardSearch.mjs` |
...
| 🔥`S-21` | **探索の目的関数を「行動する／しない」に耐えるものにする**（2026-09-20 登録）。🔑**根拠**＝探索は従来と**互角**（A/B 50.0%）だが、**【起】を1度も撃たず**（従来8回）**111手中58手で「打たない」**を選ぶ＝**エナを払う手の価値を1ターンの盤面差でしか見ていない**。いまの `evaluateBoard` は**候補どうしの比較**用で、**場に出す価値が手札1枚より安い**（生パワー×0.25）。案＝①**アタックで通るダメージ**を終端に入れる（`S-17` と同じ向き＝終端をアタック後に置く）②手札と場の相対価値を A/B で決める（`S-6` の学習対象）③**「何もしない」を選ばせない**（行動の中での最善だけを比べる）。🔑**どれも A/B で決める**（手で決めない）。 | 中 | `S-16` の探索が実際に使えるようになる（いまはオフ） |
...
- **2026-09-20 時点**（第415バッチ＝デッキフォーマットの新設）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4341**＝+2・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機** `verifyDeckFormat.mjs` **25/25 PASS**／`verifyCardSearch.mjs` **11/11 PASS**
  - 📏**カードデータの母数**＝本体 **6,666枚**＋トークン **47**＋避難先（variant）**2,572件**。variant は全件が本体1枚かトークンへ一意に解決、本体に同名2枚以上のグループは **0**。
  - 🆕📏**カードプール**＝**ディーバ専用 2,756 / レガシー専用 3,819 / 両方（再録）91** ＝計 6,666。
    **ディーバで使える 2,847 ／ レガシーで使える 3,910**。⚠**再録の 91枚**（`WD03-005` ほかセンタールリグ群）が判定の要＝**本体の番号だけで見ない**。
  - 📏**配信サイズ（Vercel Hobby の上限に対する余裕）**＝`dist` **13.5MB / 25ファイル**（上限＝静的 **100MB**・ファイル数 **15,000**）｜
    初回ロード＝raw 12.54MB → **brotli 1.19MB**（Fast Data Transfer **100GB/月** ≒ **86,000 回**）｜画像は ImageKit｜Functions なし。
  - 📏**BattleScreen**＝**6,287行**（±0）｜`controller/cpuTurn.ts` **1,436行**｜`cpuMoves.ts` **693行**｜`cpuSearch.ts` **109行**｜`utils/cardSearch.ts` **40行**｜🆕`utils/deckFormat.ts` **111行**
  - 📊**CPU の候補数**（`--census-moves`・4戦 432盤面）＝**MAIN 中央2・p90 8・最大18**｜**1手適用 平均0.1ms**｜**探索（幅4深さ4）1盤面 平均3.0ms・最大34ms**
  - 📊**自己対戦 A/B（80戦ずつ）**＝`default` vs **`search`＝50.0% [39.3, 60.7]（互角）**｜`legacy-nextturn` vs `default`＝51.2%（差なし）
