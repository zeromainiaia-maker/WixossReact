...
- **🆕 セッション（2026-07-07・続き39・Sonnet 5・§3割付タスクの棚卸し＋§7実機フォローアップ1件クローズ）**＝カード修正ではなく在庫確認＋実機検証。**(1)§3 Sonnetタスク①②の棚卸し**＝「engine実装済みSTUB idの意味文化」「B層 REVEAL_AND_PICK/LOOK_AND_REORDER pick脱落補完」は続き33-36で既に全消化済みと判明したがPLAN.mdのチェック項目が未更新だったため是正（§3・§5b）。構造走査（全10シート・287件のREVEAL_AND_PICK効果を機械的に全数チェック）で「then/destination欠落」0件を確認、残る例外2件（WDK07-E15・WXDi-P07-010＝STUB）とWXDi-P03-005・WX26-CP1-100はいずれも§6.3でOpus機構待ちとして登録済みであることを確認。JSON/engine無変更。**(2)§7 実機検証フォローアップ＝`deckshufflespell`（ON_DECK_SHUFFLED スペル経路）を再実行**＝`npm run build`→`node scripts/verifyBattleDrive.mjs deckshufflespell`で**PASS**（ログ「スペル経路 ON_DECK_SHUFFLED 発火→PR-470A#1 に +5000 反映確認」）。VERIFY_BROWSER.mdに残っていた「スペル経路は実UI未確認」のfollow-upをクローズ（詳細 BUGFIXES 最上部）。engine/JSON無変更のためgates再実行は不要（実機ドライバのみ）。**次の一手＝§7残る実機検証項目（R30-R46・ON_LRIG_ATTACK_STEP_START全体未検証等）から1件ずつ`verifyBattleDrive.mjs`にシナリオ追加→観測記録（バグ発見時の修正はOpus）。またはOpus側で(1)`GRANT_TO_PLACED_SIGNI`実装(2)census「動的比較35枚」(3)引用内CHOOSE（WXDi-D09-P20）が続き36から未着手のまま残る。**
