...
- **🆕 セッション（2026-08-16・続き504・Opus 5）＝§6.4 **O-5 クローズ**＝ルリグデッキからのレゾナ配置**残0**。ゲート全緑（**golden 2090**＝+2・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・held **102枚 / 44群**＝据置）。live changed **11効果 / 11カード**。`census:stubs` A群 **18種/22件 据置**（無言 no-op 0 のまま）。新機構＝`StubAction.resonaSummon`（count/upTo/filter）／`INTERNAL_PLACE_SUMMONED_RESONAS`（1枚ずつ再帰する複数枚配置）。
  - **🔴① PLAN の「1効果」は母集団の一部**＝原文 regex で数え直すと **11効果**あり、`SUMMON_RESONA_FROM_LRIG_DECK` は**カード全文 regex でクラスだけを読む O-20 クラスの受け皿**だった。1つの受け皿に**5軸**が同居して壊れていた＝①枚数が1枚に潰れる（`WX16-Re18-E1` の「2枚まで」／`WX13-007-E3` の「好きな枚数」＝過少）②レベル（`WD12-007-E1`／`WX07-050-E1`）③色（`WX07-050-E1`）が落ちて**どのレゾナでも出せる**（過剰）④クラスの OR（`WX19-028-E3` の「＜空獣＞か＜地獣＞」）は `includes` に当たらず**無条件**⑤【出】抑止が `RULE_REMINDER_TEXT`＝完全 no-op で**【出】が普通に発火**⑥候補が複数でも**先頭を自動配置**（どれを出すか選べない）。
  - **🔑② 絞り込みは「名詞句の間」だけを見る**＝「ルリグデッキから」〜「レゾナ」の間の修飾句に限定（後続文「ターン終了時、〜」の語を拾わないため）。
  - **🔑③ 【出】抑止は既存の畳み込み funnel に載せる**＝`BLOCK_ACTION{ON_PLAY_ABILITY}` を出し、配置 STUB に `placesToField`（§6.4 O-32 で汎用フラグ化済み）を宣言して `foldSuppressOnPlay` に畳み込ませる＝死アクションは自動で消え、`BattleScreen` の収集も既存判定のまま効く。
  - **🔴④ `resumeSelectTarget` の per-card ループは最初の pause で残りを落とす**＝選んだ2枚を `thenAction` の個別適用で配置したら**1枚しか出なかった**（`if (!result.done) return result;` で抜ける。`ADD_TO_FIELD` が特例で回避しているのがその証拠）。**選んだ全枚数をまとめて1回だけ渡す**分岐を足し、STUB 側は `value`（JSON）のキューで自己チェーンする（`lastProcessedCards` は resume を跨いで生存しない）。
  - **🔑⑤ 枚数は空きゾーン数でも頭打ちにする**＝出せない枚数を選ばせない（選ばせてから「出せません」を防ぐ・§6.4 O-32 の正面配置と同じ規約）。
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29`（`WX22-016-E1` の `REPEAT_EFFECT` が続き501 で合流）→ `O-31` の残2 → `O-6`／`O-7`／`O-8`／`O-9`／`O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer 18 id/22件の前提再判定）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き504 の4経路**＝(a)`WX16-Re18-E1`（ルリグデッキ選択モーダルが出て**2枚まで**選べ0枚も選べる／出したレゾナの【出】が発動しない／ターン終了時に**2枚とも**ルリグデッキへ戻る）(b)`WX13-007-E3`（「好きな枚数」＝空きゾーン数まで選べる）(c)`WX07-050-E1`（**レベル3以下の白**のレゾナだけが候補＝**負方向＋対照の対**）(d)`WX19-028-E3`（＜空獣＞と＜地獣＞の**両方**が候補に出る）。⚠**すべて実機未検証**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-16 続き504（§6.4 **O-5 クローズ**＝ルリグデッキからのレゾナ配置 残0）後 最新値（本行が直近の正）**：census **823 据置**、**golden 2090**（+2＝live 11効果の枚数/絞り込みペイロード＋クラス OR＋【出】抑止畳み込みのトリップワイヤ1・2枚配置とレベル/色絞り込みの実行検証1）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **102枚 / 44群**（据置）、lint **0 errors / 260 warnings**、`census:stubs` A群＝**18種/22件 据置**（無言 no-op 0）・C群 **172種/253箇所 据置**、`census:goldentypes` **未カバー 0型**。🆕**live JSON changed 11効果 / 11カード**（CSV 非改変）。🆕**新機構＝`StubAction.resonaSummon`（count / upTo / filter）／`INTERNAL_PLACE_SUMMONED_RESONAS`（ゾーン選択の pause を跨ぐ1枚ずつの再帰配置）／`resumeSelectTarget` の「全枚数まとめ渡し」分岐**。⚠**全経路 実機未検証**（§7 送り）。
