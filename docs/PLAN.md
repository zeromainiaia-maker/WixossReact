...
- 🏁**セッション（2026-08-25・続き648・Opus 5）＝§5.2 段2 第42バッチ＝【自】トリガー句の「（あなた／対戦相手）の効果によって」＝原因主体の限定が落ちる穴を配線**（**実装は Codex へ委譲**・母集団実測／指示書／検証／実機は Claude）。gates 全緑（**golden 2709→2735**／census **601→600**（`ON_CARD_MILLED_FROM_DECK` を「トラッシュに置かれたとき」の実装語彙へ較正）／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260（増分0）／同型★0）。**実機2本が2回連続 PASS＋反転確認済＝1巡が §2.2 の⑤まで完了。台帳 708→701。差し戻し0・是正1。**
  - ✅**20効果を修正**＝原文「〜**の効果によって**〜されたとき」は「自分でコストとして払った／バトル／ルール処理では発火しない」限定なのに、live の `triggerCondition` に `byOwnEffect`／`byOpponentEffect` が無く**原因を問わず発火する過剰実行**だった。⚠**逆翻訳では気付けない偽陰性**＝逆翻訳文は「手札からトラッシュに置かれたとき」と出るので、原文と並べないと消えたことが見えない（census も golden も緑のまま素通り）。
  - 🔑**型の語彙は既にあり、正しい前例が23効果あった**＝新型ゼロ。engine 側は**中央盤面 diff が既に持つ `causeOwnerId`** を mill／energy-to-trash／zone-moved／power-decrease／move-to-deck の5コレクタへ通し、共通 `effectCauseMatches` で fail-close 評価するだけで済んだ（＝§5-23 の「実装しても恒久 no-op」型ではないことを投入前に呼び出し元で確認済み）。
  - ⭐**Codex が道中で別の恒久バグを発見・修正**＝`WXDi-D05-013-E1`「あなたの効果によってカードが**1枚以上デッキから**トラッシュに置かれたとき」の場 watcher が `ON_TRASH/self` に潰れており、**このカード自身がミルされた場合しか見ない恒久過小実行**だった。`ON_CARD_MILLED_FROM_DECK`＋`milledDeckOwner:'any'` へ再分類。
  - 🔴**Codex 製の実機シナリオは今回も初回 FAIL（4バッチ連続）**＝ただし今回は**判別力ゼロの assert ではなく単純な spec ミス**（観測マーカーを `deck` の index 1 に置いたが【起】は index 0 を取る）。**付与自体は起きており**ログの `grants=[...]` が証拠。1行直して PASS。
  - 🔴🆕**反転確認のあと `public/` を戻しただけでは実機が旧挙動を返す**＝`verifyBattleDrive.mjs` は `vite preview`＝**ビルド済み `dist` 配信**で、`distIsFresh()` の mtime 判定が復元を取りこぼした（実測＝戻したのに2回連続で赤）。**`SKIP_BUILD=0` を付けた瞬間に緑**。⚠**反転確認の直後は必ず `SKIP_BUILD=0`**（付けないと「直したのに赤い」という偽の退化報告になる）。
  - 🟡**関連回帰の `v12GrantedEnergyCharge*` 2本が FAIL したが本バッチの退化ではない**＝`git stash` でベースラインへ戻して**同一内容で FAIL** を確認。真因は**シナリオの腐り**（`SPDi43-13-E2` の《ゲーム１回》バイブスMAX 前提を spec が作っておらず**付与自体が起きない**）。→ §5.1 `V-88`。
  - 🔴**母集団の測り方に穴があった**＝正規表現を `(あなた|対戦相手)の効果によって` と固定したため「あなたの効果**１つ**によって」形を丸ごと取りこぼしていた。`の効果[^、。]{0,4}によって` へ広げると母集団は **69→80効果**・**未対応が10効果残る**（否定形「効果**以外**によって」を含む）。→ §5.3 `O-62`。

**▶ 次の一手**＝**§5.2 の段2 消化を続ける**（残 OPEN **701**）。⚠**着手前に必ず `node scripts/archive/semanticAuditLedger.mjs` で数え直す。**
🔑**第2層（軸 × live の action 型）で割る手順は確立済み**だが、**続き648 の実測では quote クラスタが枯れており（最大6件）、軸×型のサブ群も中身が異質**だった＝**「語彙を横串で1本引いて、live の該当フィールドの有無で数える」ほうが systematic な塊を掘り当てやすい**（今回の cause 軸がその実例＝findings 6効果に対し**実バグは20効果**）。
🔴**母集団の正規表現は「助数詞・挿入句・否定形」で必ず一度広げてから確定する**（続き648 で踏んだ＝「効果**１つ**によって」「効果**以外**によって」が固定パターンから漏れた）。⚠**census と golden は緑のままなので、漏れは計器に映らない。**
🆕**Codex 製の実機シナリオは4バッチ連続で初回 FAIL**＝**「未実行の草案」として扱う運用は正しい**。毎回 ①判別力ゼロの assert ②ドライバの腐り ③観測対象が engine で発生するか ④**spec のインデックス／前提条件**の4点を見る。
...
> **母集団＝残 OPEN 701件**（`node scripts/archive/semanticAuditLedger.mjs` の実測が唯一の正。2026-08-25 続き648 時点＝段2 消化409／段0除去221／段1偽陽性113／HIGH・MED・LOW＝490・207・4／影響カード525・効果555）。⚠**この数字はすぐ古くなる。着手前に必ず実行して数え直す。**原文 vs effects JSON を LLM で意味比較して見つけた指摘の worklist。仕組みは [SEMANTIC_AUDIT.md](./SEMANTIC_AUDIT.md)、Codex 運用は [CODEX_GUIDE.md](./CODEX_GUIDE.md)。
