...
- 🏁**セッション（2026-08-28・続き709・Opus 5 単独）＝`O-123` クローズ。機構は最初から在り、壊れていたのは「アーツを提示するゲート」だった。**
  ユーザー指示＝**「O-123 に着手」**。
  📊**進捗3計器＝Sheet1 要対応 0 / 863（据置）｜台帳 残 OPEN 570（据置）｜census 高シグナル 520（据置）**
  gates 全緑（**golden 2957→2960**・smoke 10700 全0・fuzz 全0・lint 260 据置・同型★0）
  **実機＝新規2本（`o123usetimepay` / `o123usetimenopay`）＋既存4本の回帰がすべて PASS**（golden・実機とも反転確認あり）。
  - 📌**この巡の最大の発見＝登録票の見立てが全部外れていた。**
    登録票は「`costScaling` に `countSource:'thisPayment'` 相当が要る／🔴UI が先＝支払いパネルの再計算が
    成立するか確かめる」と書いていたが、**そのパネルは既に在って動いていた**（`useTimeCost.ts`＝タスク12(lxxxv)/(lxxxix)。
    golden も33枚ぶん張ってあった）。`parseUseTimeCostReduction` に母集団15枚を流すだけで**15/15 が正しく読めた**。
  - ✅**残っていた唯一の穴＝`checkArtsUse.usable` が `affordable`（印刷コスト）を要求していた。**
    つまり**軽減が効くべき場面（＝印刷コストを払えない場面）でだけアーツが一覧から消え**、
    支払いパネルはアーツモーダルの中にあるので**一生払えない**。影響**アーツ24枚**。
    🔑**スペル側は無傷**＝`checkSpellUse.usable` は最初から `affordable` を含めず、
    そのコメントが「⚠`checkArtsUse.usable` とはここが違う」と**食い違いを明記したまま放置**されていた
    （⇒ **worklist が「唯一の Sheet1 項目・6枚」と言っていた Sheet1 の6枚は全部スペルで、最初から動いていた**）。
    ⇒ `affordableWithUseTimePay`（盤面の候補を上限まで払った最良コスト）を1本足し、
    `usable` を `(affordable || affordableWithUseTimePay)` へ。⚠**`affordable` の意味は変えず**、
    `listUsableArts` に `&& check.affordable` を足して **CPU の挙動を1ビットも動かさない**。
  - ✅**副産物＝`WD16-006`（ドント・ステップ）の恒久 no-op を返済**＝「手札の**差**1につき《青×1》減る」は
    盤面のどのゾーンも数えないので `zone` では書けず、live は no-op STUB＝**常に印刷コスト《青》×6 を請求**していた。
    `CostScalingCount` に `handDiff` を追加（型＋評価器＋parser＋golden の4点／負は0クランプ）。
    **live 差分はこの1カードちょうど**（effectId 単位で全数 A/B 比較）。
  - 📌**分離＝`SP36-001`（炎のタマ）→ §5.3 `O-136`**。2文目が「につき」ではない**条件つき定額軽減**で
    `costScaling` では書けず、しかも `parseStatus:'MANUAL'` なので parser を直しても live へ届かない。
  - 📌**教訓（§4.1 へ）**＝**②「母集団を実測する」は、母数だけでなく「受け皿が既にあるか」も測る。**
    今回は数分の実測で作業が「新機構の実装」から「1行のゲート修正」に変わった。

**▶ 次の一手**＝🆕🔴**§2.0 の「速いレーン」で §5.2 を消す。** 具体的には **Sheet2 の意味照合 残 OPEN 91件**（シート別で最多。Sheet1 は 0）。
**1件＝原文を読み直して `manualEffects.ts` に正しい JSON を手で書く → `build:effects` → 逆翻訳を目視 → `golden -- --only` ＋ `smoke`（約5秒）**。
**10件たまったら `gates` 1回・commit 1回・BUGFIXES 10行**（§2.1 ④⑥）。実機は `src/screens/` を触った回だけ（§2.2）。
⚠**着手前に `node scripts/archive/semanticAuditLedger.mjs` で数え直す**（この数字はすぐ古くなる）。
⚠**「移設だけ」は禁止**（§2.0）＝中身を変えずに manual へ移すと**計器から消えるだけ**。

**遅いレーン（同型3枚以上・機構が要るもの）の在庫**＝`O-135`（スペル解決経路が中央 diff を通らない）／
`O-86`（アーツ／スペルの実効コストを UI 層が原文 regex で決めている＝**先に計器**）／
`O-136`（`SP36-001` 1枚）／`O-118`（エクシード色の選択権）＝どちらも S。
実機側は **`V-89`**（`lookReorderCanTrash` の FAIL）。計器側は `census:enginetext` の miss 上位43ハンドラ。

⚠**Sheet1 を「本当に閉じる」話は §5.2 の一部として扱う**＝`census:cards --sheet 1` は 0 だが、
**意味照合を通したのは 611/863 枚**で、残り252枚の内訳は **A：STUB/MANUAL を含む 191枚**（意味照合 round1 は
`groups:["clean"]` ＝**clean群3,244枚だけ**を回して終わっており、本命の stub群2,631枚は丸ごと未着手）＋
**B：現在は純AUTO だが round1 実行時のスナップショットでは stub群に居た 61枚**（群の帰属が固定されたままの計器の穴）。
**新しい計器を作らずに、既存の `semanticAuditExtract.mjs --cards` へ流す**のが本筋。
