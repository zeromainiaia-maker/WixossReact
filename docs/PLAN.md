...
> **残っているもの**＝下記 `V-nn` エントリ **13件**（🅱 5／🅲 1／🅳 7。🅳 の各エントリは (a)(b)(c) の複数経路を持つ） ＋ **続き616〜634 の21バッチ**（段2 の parser/engine 改変ぶん・特に実機経路へ直接触ったのは第34「0枚選択可の UI」・第37「クラッシュ主体の限定」・第38「群割当の決定可否」・第40「`END_OF_ATTACK`」）。
...
- 🆕**🔶 V-84 レベル限定つき【ガード】禁止が、ガード応答UIで実際にそのレベルだけを弾くか（2026-08-22 続き609 で登録・§5.3 O-41）**
...
- **2026-08-23 続き636 後（本行が直近の正）**：
  ...
  **実機シナリオ定義総数 476**、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
