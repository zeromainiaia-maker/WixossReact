...
**直近＝2026-09-20＝§5.6 `C-0`＝バグ報告 `c32a37ce`（空きシグニゾーンが無いのに「場に出す」だけの【起】が撃てた）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-17`**（後半ブロックの探索）＝`S-21` で目的関数を重みで動かす道は尽きた（3本とも差なし）⇒ **終端をアタック後に置く**のが残りの道。🆕**`S-20`（A/B の山が型を踏まない）は探索の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
