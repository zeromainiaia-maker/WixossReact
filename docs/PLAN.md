...
| (lxxiv) | **🆕✅8/9枚採用・残1は honest defer（2026-08-01・codex 実装／Claude 検証）**＝「以下のNつを行う。①②③」母集団10枚のうち `WXK11-002/007/008/009/010`・`WXDi-P05-052`・`WXDi-P06-050`・`WX24-P4-002` を全項目 SEQUENCE 化（`WX24-P4-007` は第9波で消化済み）。詳細は BUGFIXES 2026-08-01 (lxxiv) 節。<br>**残＝`WXK11-001-E1` 1枚だけ**。条件語彙（`ARTS_USED_THIS_TURN`／`SPELL_USED_THIS_TURN`／`OR`）は**すべて既存で相手 owner でも評価できる**ことを確認済み＝ブロッカーは2つだけ：①「あなたのルリグデッキにあるコストの合計が２以上のアーツ１枚を**ゲームから除外してもよい**」が任意コストとして後続をゲートするか未確定（単独パースは `STUB{BANISH_FROM_GAME}` を出すだけ）②**①と②でスキップ対象の owner が食い違う**（①`opponent`／②`self`）＝どちらが正かを原文だけでは確定できない（防御アーツなので相手のアタックステップを止める読みが有力）。旧形（`CHOOSE_N_FROM_LIST`）は golden で据置固定済み。<br>⚠**在庫の旧記述3点は実測で誤りと判明済み**（DTT を持つのは9枚でなく6枚／`LIFE_CRASH` は `triggerBurst:false` でバースト誘発しない＝②は分割パースが正しい／退化するのは2項目だけ）。<br>⚠**残る近似3件**（退化ではないが完全でもない）＝`WXK11-002`④は「対象とし」なのに直下1枚を自動取得（DTT P4 の移設）／**`REMOVE_ABILITIES.until` は engine が読まない死フィールド**（`execRemoveAbilities` はフラットな `abilities_removed` に積むだけ・寿命は BattleScreen のターン境界クリア）／`WX24-P4-002`③の `banish_redirect` は「能力を持たない」限定と2ターン持続を表現していない。 |
...
- **🆕 セッション（2026-08-01・続き317・Opus 5〔**codex 実装／Claude 指示・検証・簿記**〕）＝タスク12(lxxiv)＝「以下のNつを行う。①②③」の項目脱落を是正。母集団10枚を per-card 監査し **8/9枚採用・残1（`WXK11-001`）は honest defer**（golden **1234→1239**〔codex +2 構造／**Claude +3 E2E**〕・census **1352→1351**（`BASELINE_HIGH=1351`）・smoke **10679** 全0・SKIP0・fuzz 全0・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **251枚/99署名 据置**。live per-effect 差分 **changed 8 / added 0 / removed 0**・スコープ外 outlier 0（ベースライン `07e79bd6` 比）／`build:effects`・`regen` 冪等。**新設 STUB 1本のみ**〔engine の旧 DTT パターンからの移設〕・**engine 新機構0**）
  - **① 投入前の全数実測で在庫の記述を3点訂正**＝(a)「残り9枚は DTT の text パターン依存」→**live で DTT を持つのは6枚だけ**（残り3枚は別の壊れ方）(b)「②は `LIFE_CRASH`＝**バーストが誘発する**別物」→**誤り**。`triggerBurst:false` が出ており engine（`effectExecutor.ts:1251`）が守る＝**分割パースのほうが正しい** (c)「無差別に開くと held +9」→**実際に退化するのは2項目だけ**。
  - **② 在庫が書いていなかった実害3件**＝`WXDi-P05-052` は**①のサーチが live から丸ごと欠落**／`WXDi-P06-050`②は**無条件 `DRAW{2}`**（原文は1枚）／`WXK11-007/008/009/010` の②③は**「《色》を支払ってもよい」が落ちて本体だけ無償実行＝8項目が無償化**。ここが本バッチ最大の是正。
  - **③ 先出しした罠が効いた**＝「**`CENTER_LRIG_OR_SIGNI` を使うな**（`execFreeze` に分岐が無くシグニ限定へ落ち、型もゲートも通ったまま**ルリグだけ黙って外れる**）」を指示書に書き、`SEQUENCE[FREEZE{SIGNI ALL}, FREEZE{LRIG}]` で **engine 無改修**着地。**この退化を検出する E2E golden も置いた**。
  - **④ 🔴 検証で是正した3点**＝(a) codex 初版は `parseActionTextInner` 冒頭に **`WXDi-P05-052` の本文を埋め込んだ分岐**を置いていた（後段の引用能力抽出が全文を奪う問題への先回り自体は正しい）→ `tryParseDoAllItems()` へ切り出して**先頭で1回呼ぶ**構造へ（live/held 変化0で等価確認）(b) `parseSpellEffect` の分岐条件が**原文 regex** で、付与能力を持たないカードまで入り `spellFb` 由来の PARTIAL を握り潰す形だった→**木の形**で判定へ (c) codex の golden 2本は**構造 assert のみ**で、本バッチの実害是正（任意コストゲートが本体を止めるか）を検査しない→**E2E 3本を追加**し、`WXK11-008` をエナ0で実行して②③が走らないことを実挙動で固定。
  - **⑤ ⚠正直に残した近似3件**（退化ではないが完全でもない）＝`WXK11-002`④は「対象とし」なのに直下1枚を自動取得（DTT P4 の移設＝従来同等）／**`REMOVE_ABILITIES.until` は engine が読まない死フィールド**（`execRemoveAbilities` はフラットな `abilities_removed` に積むだけ＝codex 報告の「duration が付いて改善」は**不正確**）／`WX24-P4-002`③の `banish_redirect` は「能力を持たない」限定と2ターン持続を表現していない（従来は③が丸ごと欠落＝純改善ではある）。
  - **⑥ BUGFIXES は末尾追記されていた**（規約は先頭）＝既知の再発パターン。先頭へ移して上記の訂正込みで書き直した。
  - **次の一手**＝**Opus**：①**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く見込み＝筆頭）②**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）③**(lxxii) 遅延クリアの片側漏れ**④**(lxxiv) 残1＝`WXK11-001`**（ブロッカーは「①と②で owner が食い違う」の原文確定と「ルリグデッキ除外」の任意コスト化の2点だけ）⑤(lxviii) 散文形「対戦相手のターンの間、」⑥§6.3 J 残11 ⑦(lxvii) CPU `ON_TURN_END` 未収集⑧(lxvi)。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜317 の全件が実機UI未検証で積み上がっている**。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1234**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1352**（回帰ゲート）・held **251枚/99署名**（2026-08-01 タスク12(lxxvii) 実働化後に実測。+1 は honest defer の `WXK03-069`）。
>
