...
|---|---|---|---|---|---|
| **1** | **`O-77`** | **15効果** | catch-all の分解 | 中 | `LRIG_UNDER_CARD_OP` が22文型の catch-all。⚠**着手前に `effectExecutor.ts:4433` を読む**＝`SEQUENCE[STUB{LRIG_UNDER_CARD_OP}, CONDITIONAL]` は**別の消費地点が「下のカードを1枚払うコスト」として先取りする**＝executor のハンドラに到達しない |
| 2 | `O-76` | 11効果 | catch-all の分解 | 中 | `LOOK_OPP_LIFE_TOP` が20文型の catch-all |
| 3 | `O-135` | 回帰が広い | インライン収集を族ごとに足す | 大 | `O-138` の前提でもある |
