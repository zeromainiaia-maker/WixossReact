...
- **🆕 セッション（2026-08-11・続き428〜430・Opus 5）＝§6.4 を3本＝**エナ支払い元の一本化**／**離場置換の前提訂正＋決定層分離**／**離場置換の対話化**。golden 1776→1794・census 854 据置**。
  - **🔴① 続き428＝「エナコストの支払い」がどこにも一本化されていなかった**＝支払いモーダル14本が `my.energy` を直読みし、候補生成も控除も **BattleScreen の14サイトに散在**。この状態で「エナゾーン以外を支払い元にする」語彙（`UNDER_CARD_AS_ENERGY_COST`）を載せると、**半分だけ配線された瞬間に「アーツには使えるが【起】には使えない」無言の不整合**になる。
  - **⭐② funnel は「後方互換の index 空間」＋「applyTo は最後に当てる」の2点で決まる**。**安全弁は関数の戻り値に置く**＝`applyTo` を呼び忘れると**エナが1枚も減らない**＝即座に露見する壊れ方になる。**無言にならない壊れ方を選ぶ**のが設計の本体（教訓 (m) の実践）。
  ...
  - **⭐④ 対話化は「policy の差し替え」に落とせる**＝`applyEffectLeaveSubstitutes` を **collect→choose→apply** に分解（列挙は**投機実行**＝`ExecCtx` は不変なので採用しない候補は捨てるだけ）。F-3 の内部選択も1件ずつ option 化したので、engine の構造を触らずに対話へ移れる状態にしてから M2 に入った。
  - **🏁⑤ 続き430＝M2 完了＝10カードの「代わりに…してもよい」が実際に選べるようになった**（従来は engine が勝手に適用）。🔑**離場ループの途中では pause しない**＝決定を **`PlayerState.leave_substitute_choices`** に刻んで**適用前に全部聞く**（PlayerState なので pause を跨いで残る＝`banish_substitute_choice` と同型）。**engine に mid-loop resume を新設せずに済んだ**のが要点。
  - **🔴⑥ `opponentResponds` の CHOOSE は「相手が支払う」へ固定ルートされていた**＝コストの無い問いを流すと「エナ不足」で即終了＝**無言で潰れる**。`leaveSubstituteAsk` を新設して素の `resumeChoose` へ分けた。⭐**`opponentResponds` は「誰が答えるか」であって「相手が払う」ではない**（続き411 の教訓の別形）。
  - **⭐⑦ 順序を間違えると対象選択の前に問いが出る**＝`count:'ALL'` の hoist は**分岐の内側**に置く。実装中に1度踏んで golden が捕まえた（`SEND_TO_ENERGY` が完了しない）。
  - **✅⑧ ゲート**＝`npm run gates` 全緑（typecheck / golden **1794（+18）** / smoke 10688 全0・**SKIP 0** / fuzz 全0 / census **854 据置** / census-stubs（明示 defer **17→16**・無言 no-op **0**）/ manual-fields / lint 0 errors）。`npm run regen` 済み。
  - **📋⑨ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**離場置換の残 latent**＝`powerReduction`／`replaceBanish`／`noAbilityDeckBottom` の宣言走査が**印字だけ**で付与ストアを見ていない（現状 live 0件で無害）(e)`WXDi-CP01-023`（CONTINUOUS の置換に受け皿なし）。**Sonnet 側**＝§7 実機検証（**続き428 のエナ支払い回帰確認と続き430 の対話（特に CPU 側で固まらないか）を最優先**）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き428〜430（§6.4 エナ支払い元の一本化／離場置換の前提訂正・決定層分離／離場置換の対話化）後 最新値（本行が直近の正）**：census **854**、golden **1794（+18）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
>
> 0. 🎯**次の一手（2026-08-11 続き430 更新）＝§6.4 の残**＝(a)~~離場置換の対話 policy（M2）~~ **✅続き430 で完了**（残は latent＝置換3軸の宣言走査が付与ストアを見ていない／`WXDi-CP01-023` の CONTINUOUS 受け皿）。(b)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)**残 A群16件の明示 defer 見直し**（§6.3 の機構が進んだものから解ける）。**Sonnet 側＝§7 実機検証（続き428 のエナ支払い回帰確認が最優先）**。以下は旧「次の一手」（参考）。
>
...
- ~~**`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査＋`verifyEffects` の誤検出除外**~~ **✅2026-08-10 消化（続き408）**＝**33件を1件ずつ仕分けたら真バグ3件／誤検出30件**だった。誤検出6系統をルールで潰し、**`POWER_VALUE_MISMATCH` 6→0／`MILL_COUNT_MISMATCH` 3→0／`OPTIONAL_SUSPICIOUS` 2→0／`EFFECT_TYPE_MISSING_CONTINUOUS` 15→1（残1＝真バグ `WX12-010`）／`verifyEffects` のタイミング 4→0**。**計器の精度改善が本体**（続き407 の教訓「過剰報告する計器は無いより悪い」の実践）。詳細は BUGFIXES 2026-08-10（続き408）。
  ...
  - **📋 残 1件**＝`WXDi-CP01-023`（**置換**「代わりに…してもよい」＋ CONTINUOUS で受け皿なし＝§6.4「離場置換の対話化」M2 とセット）。
