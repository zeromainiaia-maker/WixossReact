...
- **🆕 セッション（2026-08-13・続き458・Opus 5）＝§6.4 **O-17 完了**（続き457 で登録した隣接発見を同日中に消化）＝**能力喪失の「誰に効くか」が違う3件**を是正。golden 1951→1954・**census 832→831**・held 106枚/47群 据置**。
  - **🔴① `WXK05-010-E2`＝対象種別がキー→シグニに化けていた**（キーを捨てるコストを払って、キーではなく相手シグニが止まる）。⭐`EffectTarget.type:'KEY'`＋`TargetScope:'self_key'/'opp_key'` を新設。**読みは続き457 の `activeKeyAbilitySources` funnel に「`abilities_removed` のキーを外す」1行を足すだけで全経路に届いた**。⚠`alsoKeys`（全キーをフラグで倒す）とは別軸＝こちらは**1枚を選ぶ**。🔴`resumeSelectTarget` の持ち主判定がシグニ／ルリグ枠しか見ておらず、**キーを選ぶと無言で空振り**する穴も同時に塞いだ。
  - **🔴② `SPDi47-01-E2`＝領域を跨ぐのに場のシグニ1体だけ**（原文「対戦相手の**手札と場とエナゾーンとトラッシュにある**シグニは能力を失う」）。`all` 判定が2本しかなく**列挙形がどちらにも当たらない**。
  - **🔴③ `WX24-P4-013-E3`＝レベル条件が落ちて相手シグニ全体から能力を奪う過剰効果**（原文「**この方法で公開されたシグニと同じレベルの**」）。⭐**参照元は既に engine にあった**＝`resumeLookAndReorder` が `lastProcessedCards` を載せるので、落ちていたのは **parser の filter だけ**。後段の手札戻し（照応語「そのシグニ」だけの文）は `IDENTITY_BATCH5B` へ登録。
  - **⭐④ ②③共通の機構＝`EffectTarget.allZones`**（場＋手札＋エナ＋トラッシュ）。`abilities_removed` は cardNum のリストでゾーン非依存なので**候補プールを広げるだけ**で載る。⚠**デッキ／ライフは足さない**＝参照経路が無く「実装したように見えるだけ」になる。⚠**`cardType:'シグニ'` を必ず載せる**＝でないと手札のスペルやアーツまで巻き込む。
  - **🔴⑤ 消費地点を1つ足した**＝トラッシュ起動（`getMyTrashCardActions`）が `abilities_removed` を**一切見ていなかった**＝そのままでは④が「候補を広げただけの見せかけ」になるところだった。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-3 の残31**（`STUB` 系が主体。⚠**型ごとに `NEXT_TURN` の受け皿が engine にあるかを先に実測**＝続き453 の `REMOVE_ABILITIES.until` が「型にフィールドがある＝動く」ではなかった実例）(b)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵＝続き457 の `SP38-006` 外科パッチが直近の先例）(c)**O-4 UNKNOWN 25ノード/25カード**（1カード1機構の単発が大半）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452〜458 の7件（相手応答モーダル／ターン跨ぎの能力喪失／ゾーン指定→後から配置／2ゾーン選択UI／ゾーン封じ／キー能力喪失のターン終了復帰／**キー選択モーダルと領域跨ぎの能力喪失**）。⚠**キー枠は既存の SELECT_TARGET UI が扱ったことのないゾーン**＝候補0枚に見える壊れ方をしうる。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き458（§6.4 O-17 完了＝能力喪失の対象軸3件）後 最新値（本行が直近の正）**：census **831（832→831）**（`BASELINE_HIGH` 831）、golden **1954（+3＝キー1枚選択の E2E／領域跨ぎの E2E／live 対象軸3点の契約）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）、**`FieldGrant` の kind 3種**（`power`〔`perTargetLevel` で動的 delta 可〕／`abilityLoss`／`blockAction`）。live effect 単位 diff **3効果・effectId 増減 0/0**。
...
- **🆕 続き458（§6.4 O-17 完了＝能力喪失の対象軸）が持ち込んだ未検証UI 2件**＝engine は golden で固定したが、**キー枠の選択モーダル**と**トラッシュ起動の抑止**は実機でしか踏めない。
  - [ ] 🔴**キー選択モーダルに相手のキーが出るか**（`WXK05-010` の【起】＝このキーをルリグトラッシュに置く）＝`targetScope:'opp_key'` の SELECT_TARGET が開き、**相手のキーが候補に並ぶ**。⚠**キー枠は既存の SELECT_TARGET UI が扱ったことのないゾーン**＝「候補0枚に見える／モーダルが出ない」壊れ方をしうる。⚠選んだキーの【常】が止まり、**もう1枚のキーは止まらない**（`alsoKeys` の全キー喪失と取り違えていないか）。
  - [ ] 🔴**領域跨ぎの能力喪失がトラッシュ起動を止めるか**（`SPDi47-01` の【起】《ゲーム１回》）＝相手のトラッシュに**トラッシュ起動【起】を持つシグニ**を置いてから撃ち、**そのカードをタップしても【起】ボタンが出ない**ことを見る。⚠ここが変わらなければ消費地点が届いていない（候補を広げただけの見せかけ）。
