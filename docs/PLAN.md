...
- **V-04 🔴 エナ支払い元の一本化（続き428）4件**＝**支払いモーダル14本＋支払いサイト14箇所を funnel（`screens/battle/energyPaySource.ts`）へ寄せた大改修**。純関数は golden で固定したが、**モーダルの候補描画と控除は golden では原理的に踏めない**。⚠**まず「回帰していないこと」から見る**（①が最重要）。
  ...
  - [ ] 🔴**`WXDi-P10-041`（羅植姫 タナバタ）の下カードで払えるか**＝自分のアタックフェイズ開始時にデッキ上2枚がこのシグニの下に置かれる → **アタックフェイズ中**にアーツ／【起】のコスト選択画面を開くと、**エナゾーンのカードに続けて「下のカード」が候補に並ぶ**。選んで支払うと**そのカードがトラッシュへ行き、シグニ本体は場に残る**。
...
- **V-05 🔴 対象宣言の脱落（続き423）2件**＝**16効果**で対象の範囲が変わった。
...
- **V-08 🔴 `OPTIONAL_COST{handDiscard}` のモーダル（続き420）3件**＝**18効果**で「手札を捨てる／捨てない」のモーダルが**新たに出る**（従来は engine が手札の末尾を無言で捨てていた＝UI が一度も出ていなかった）。
  - [ ] **絞り込みが効くか**（`WX18-001-E3`＝「手札から＜悪魔＞のシグニを1枚捨てる」）＝**手札の＜悪魔＞シグニだけが選択候補**になる。⚠従来は**末尾の1枚が問答無用で落ちて**いたので、ここが直った実感が一番出る。
...
- **V-09 🔴 任意性脱落の系統消化（続き416〜417）4件**＝engine 側は golden で固定したが、**任意コストの pay/skip モーダルが新たに出るカードが 140枚超**あり、実UIでの提示・支払い徴収は golden では踏めない。
  - [ ] **手札捨ての任意コスト（`OPTIONAL_COST{handDiscard}`）**＝`WXK09-041` で ①「任意コスト：支払いますか？」が出る ②**スキップで手札が減らず本体も走らない** ③支払うと**＜天使＞のシグニだけ**が選択候補になる ④**手札に＜天使＞が1枚も無いとき「支払う」が選べない**（`canAfford`）。
  ...
  - [ ] **自己トラッシュコスト（`OPTIONAL_TRASH_SELF`）**＝`WX24-P2-060` で ①「このシグニを場からトラッシュに置きますか？」が出る ②**スキップなら自分のシグニも相手のシグニも動かない** ③支払うと**自シグニがトラッシュへ行ってから**相手が手札に戻ること。
  - [ ] **新設 `fieldDown`（アップ状態の自シグニをダウン＋色）**＝`WXDi-P04-051` で ①アップ白シグニが**3体そろっていないと「支払う」が選べない** ②支払うと**3体ともダウンし**《白》も徴収される ③そのあとルリグがアップし能力を失うこと。あわせて `WXK08-052` で**他のシグニの下のカードが候補に出ない**ことを見る。
