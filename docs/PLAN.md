...
12. **Sonnet が積んだ engine/parser バグの修正（常設受け口）**＝Sonnetタスク1（実機検証）・4（一次トリアージ）・8（semantic audit）の観測結果を受けて修正する。**🆕在庫7件（続き77・Sonnetタスク6の§5c再収穫中に発見・parser弱点＝いずれも修正はせず観測のみ登録）**：(a)**EXILE→TRASH誤変換**＝「ゲームから除外する」原文が`TRASH`に化ける（WXDi-CP02-TK03B/WXDi-P11-010B/WXDi-P16-001B/WXK04-035/WXK09-015/WX21-027/WX24-P3-TK1A。うちTK03B・WX21-027は owner まで誤る＝self除外がopponent-trashに化ける）。WX24-P3-TK1Aは同カードの別indexで有効なtiming是正（`ON_PLAY`→`ON_SIGNI_BANISH_OPPONENT`）も併発しているがEXILE退化と同一カード内のため個別採用不可（heldReviewはカード単位）。(b)**多段「あるかぎり」条件付与の一部消失**＝WX24-P1-043（レベル1/2/3の3段階付与のうち1段目しか`GRANT_FIELD_SIGNI_ABILITY.abilities[]`に残らない）・WXDi-P11-046（パワー+3000部分が「ダウンしない」付与に上書きされ消失）。(c)**条件ドロップ**＝WXDi-P16-009（`LIFE_COUNT`条件消失）・WXDi-P16-011（`HAND_COUNT`条件消失）・WXDi-CP02-TK01A（`triggerScope:any_opp`消失）。(d)**inner duration誤変換**＝WX25-P2-062（`GRANT_KEYWORD`内側の`action.duration`が原文「ターン終了時まで」に反し`PERMANENT`化）。詳細 BUGFIXES 続き77。
...
- **🆕 セッション（2026-07-12・続き76・Opus 4.8・**§3 Opusタスク16＝timing 語彙センサスの消化＝計19系統81枚（第1弾9種35枚＋第2弾5系統31枚＋第3弾4系統9枚＋第4弾6枚）**・実機8シナリオ全PASS・golden 223・census 1537→1514・timing フォールバック 209→128・**Opusタスク10 は A〜F を全て解決（完了）**）**
  ...
  - **次の一手＝★Sonnet に交代（続き77は Sonnet）**。Opus 側の大物（timing 語彙センサス／タスク10 パターンA〜F）は**この続き76で出し切った**ため、**Sonnet で回せる定型在庫を優先して消化する**。
    - **(1) Sonnetタスク6＝§5c 再収穫サイクル（最優先・`/census-batch` スキル準拠）**＝**続き76で parser を大幅に触った（timing 19系統＋タスク10 の A〜F）ので、held の中身が入れ替わっている**。`npm run build:effects`→`node scripts/heldReview.mjs` で**新しく採用可能になったカードを再収穫**する（Opus バッチ着地→Sonnet 再収穫の交互サイクル＝続き34→35 の型）。⚠必須ガードレール4点（§3 Sonnet 欄）を厳守。**held は現在 約100枚**。
    - **(2) Sonnetタスク1＝§7 実機検証の横展開**＝続き76で **engine を複数箇所さわった**（`execFreeze` の LRIG 分岐／`execNegateAttack` の LRIG／`execExile` の HAND_CARD／`execDraw`・`execTransferToHand` の BLOCK_ACTION／`collectTurnTriggers` の遅延トリガー収集／`execTrash` のカウンタ）。**これらの実機シナリオを追加する**のが最も価値が高い（golden では pure 関数までしか見ていない）。既存の `acceSelfScope`/`acceOtherScope` が雛形。
    - **(3) Sonnetタスク3＝driver バッチ状態汚染の根本修正**（続き76 でも `signi_acce` の残骸で誤診しかけた＝**注入時の初期化漏れ**が再現）。
    - **(4) Sonnetタスク4＝BEHAVIOR_AUDIT キュー再生成＋一次トリアージ**（parser が大きく変わったのでキューも作り直しが要る）。
    - **⚠Sonnet は engine/parser を触らない**＝作業中に見つけた engine/parser バグは**その場で直さず** §3 Opusタスク12 へ登録して次の Opus セッションに回す。
    - **Opus に戻すときの候補**＝タスク1第2弾（引用付与の内側「代わりに」・`GRANT_LRIG_ABILITY` の ON_PLAY 誤デフォルト）／タスク2（動的比較の残3枚）／タスク5（小口12件）／タスク6〜9（機構系）／**timing センサスの残128は engine に受け皿が無い機構待ち**（「対戦相手が手札を捨てたとき」・「対戦相手のエナゾーンに置かれたとき」・シグニの下からトラッシュ・《トラップアイコン》発動・グロウフェイズ開始時 等）＝§6.3 へ。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 1494枚（2026-07-12 続き77・履歴 …→1567→1566→1563→1558→1557・続き63 DEPLOY_RESTRICT実装で5枚・続き67 WXEX2-25-E3 lrig相対付与で1枚・続き68 デッキ相対SEARCH3枚・続き69 引用内CHOOSE 26枚で5枚・続き71 CHOOSE平坦化復元WXK10-013で1枚解消・続き75 timing語彙8種で1557→1537・続き76 timing語彙 計19系統＋タスク10 パターンA〜F（全解決）で1537→1514・続き77 §5c再収穫（held 85枚採用）で1514→1494・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。状態系の残（凍結13・ダウン/アップ38）はコスト節/条件/CONT型（別パス・§6.3）。**消化の入口は `npm run census:clusters`＝文型テンプレのクラスタ表（`docs/_census_clusters.txt`・枚数順）から系統バッチを選び、parser規則→`npm run build:effects`→`node scripts/heldReview.mjs` で署名グループごとに一括採用する（続き23確立・手順詳細は §5c）。**
