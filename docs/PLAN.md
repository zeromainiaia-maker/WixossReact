...
| 🆕(xliv) | **`BANISH_REDIRECT` の残テール（続き217・Opus。`bySource` の10効果とは別系統）**＝同族30効果のうち、まだ修飾句が JSON に載っていないもの。(a)**対象シグニの属性フィルタ**＝「パワーが０以下の」(`WXDi-P10-009-E3`/`WXDi-CP02-102-E2`)・「レベル１以下の」(`WXK10-053-E1`)・「レベル２以下の」(`WX25-P3-104-E1`)・「凍結状態の」(`WXDi-P12-073-E1`)・「感染状態の」(`WX21-005-E1`)・「【チャーム】が付いている」(`WX18-038-E2`)＝すべて `target.filter` へ落とせるはず。(b)**単体対象**＝「対戦相手のシグニ１体を対象とし、このターン、それが…」が `count:'ALL'` に潰れている(`WX25-P2-060-E2`/`WXDi-P12-054-E2`/`WXDi-P15-044-E1`/`WXK06-048-E1`)。(c)**正面限定**＝「このシグニの正面の」(`WX19-078-E1`/`WXDi-D09-P14-E1`/`WXDi-P10-044-E2`)＝これらは `owner:'self'` になっており **owner も要確認**。(d)`WXK06-048-E1` は原文「対戦相手のシグニ」なのに `owner:'self'`＝**単純な owner バグ**。 |
