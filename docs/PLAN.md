...
| # | タスク | 種別 | 規模 | 残っている内容 |
|---|---|---|---|---|
| 1 | **§7 実機検証の横展開**（主力在庫・推奨） | 検証（driver シナリオ追加のみ） | S×件数 | §7 の「残る未検証項目」を1件ずつ `verifyBattleDrive.mjs` の `scenarios` に追加。雛形＝`acceSelfScope`/`acceOtherScope`（正例と負例をペアで書く）。**発見したバグの修正自体は Opusタスク12 へ回す** |
| 3 | driver バッチ実行の状態汚染 | scripts（engine/JSON 非依存） | M | ⏳部分完了（続き77・105）＝ホワイトリスト方式リセット＋シナリオ毎 `page.reload()` で PASS43/FAIL6 まで改善。残＝低頻度フレークと `oppDraw` 単独FAIL（CPU挙動依存・未解明） |
| 4 | BEHAVIOR_AUDIT キュー再生成＋一次トリアージ（常設） | 計器実行＋分析 | S | `--queue` 再生成→`_bqTriage`→真no-op候補の抽出まで。**仕分け確定と修正は Opusタスク11/12** |
| 6 | §5c 再収穫サイクル（`/census-batch` 準拠） | JSON採用 | S | **Opusタスク1〜6 のいずれかが着地するまで着手しない**（現在プラトー＝空振りになる。続き130 で「安全な採用先ゼロ」を実測確認済み） |
| 8 | semantic audit のスケールアップ＋単点修正 | パイプライン＋JSON単点 | M | ⏳部分完了（続き88・102）＝200枚サンプル中119枚精査・単点是正21件。**残り8バッチ（81枚）＋stub群 約2,180枚は `claude -p` 上限リセット後に再開**（findings/manifest は `scripts/archive/scratchpad/semantic_audit_101/`）。意味判定が割れるもの・機構が要るものは Opusタスク12 へ |
...
- **🆕 セッション（2026-07-14・続き131・Sonnet 5・PLAN §7／§3 Sonnetタスク1＝R40②「opp-draw『自分の効果で』発生源限定なし」を実バグと確定）**
  - **🐛→登録 R40②を実バグと確定**＝新規シナリオ`oppDrawOwnEffectOnly`（host PR-423「対戦相手が**自分の効果で**カードを引いたとき」× host SPDi43-21「あなたのアタックフェイズ開始時…そうした場合対戦相手はカードを1枚引く」＝host自身の効果でguestが引く構図）で検証。`collectOppDrawTriggers`（`triggerCollect.ts:691`）は drawer側の`cards_drawn_by_effect_this_turn`増加しか見ず、増加が「対戦相手自身の効果」か「reactor(watcher所有者)自身の効果」かを区別しない＝host自身の効果でguestが引いただけでPR-423が誤発火（LIFE_CRASH＋自己バニッシュ）することをFRESH=1含め2回連続再現。Opusタスク12(xxi)へ新規登録（診断のみ・修正せず）。シナリオは意図的FAIL回帰として既定order外のまま。
  - **⚠driver状態汚染の再確認**＝2回目確認を「既存ルーム再利用」パスで行ったところ前回実行の残留pending_effect(SELECT_TARGET)で409秒スタックしFAIL（PLAN§3 Sonnetタスク3の既知症状の新規実例）＝`FRESH=1`で再実行して再現確認に切替え。新規シナリオの単発確認はルーム再利用パスに頼らない方が安全という運用知見。
  ...
  - **次の一手**：**Opus側＝①上記R40②(xxi)を含むタスク12在庫の消化②22件の真no-op最終仕分け③タスク1〜6の新語彙系**。Sonnet側＝§7残る未検証項目（ON_COIN_PAID④・ON_LRIG_GROW④）か次のBEHAVIOR_AUDIT一次トリアージ。
### 📊 恒久指標（維持中・逐次更新）
...
**残る実機検証項目（これだけが未消化）**：
- **R40②**＝opp-draw の「自分の効果で」発生源限定なし（近似・未検証）。
