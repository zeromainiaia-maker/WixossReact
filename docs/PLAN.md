...
- **🆕 セッション（2026-07-15・続き137・Opus 4.8・PLAN §3 Opusタスク12＝在庫消化）**
  ...
  - **✅ 検証**：全ゲート緑（typecheck／**golden 326→328**＝targetsTriggerSource/targetsLastProcessed の autoTargetedCards surface テスト2件新設／smoke 全0／fuzz 全0／census 2215 維持／lint 0 error）。**実機 E2E `onTargetedForcedBypass`**（WX12-010〔ON_ATTACK_SIGNI any_opp＋targetsTriggerSource -2000〕× WXDi-P03-067〔ON_TARGETED self=DRAW〕）を空手札注入の絶対値判定に作り直し **FRESH=1 で2回連続PASS**（gHand=1）→既定 order に追加。effects JSON 無変更。詳細 BUGFIXES 続き137。
  - **次の一手**：Opus＝**タスク12 の残在庫**（(v) applyDirectAction default節の残＝続き106で smoke 258→1 まで消化済み・残は (xii) WXEX1-19／(vii)(viii) checkAllEffects 由来の per-card 構造修正 約8枚〔owner混同・optional/条件欠落・CHOOSE化・JSON層＝census/regen 管理が要る〕／(xix) LIMIT_ALL_FIELD_1〔両者シグニ1体制限＋減量の重機構・単発カード〕／(xxi) collectOppDrawTriggers〔発生源追跡＝§6.3級の近似〕／(iii)(xi)(🆕choice.condition) は設計判断待ち）→本丸のタスク1（引用付与の内側 parse）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 328・smoke/fuzz 全0・同型★0・census 2215。
>
