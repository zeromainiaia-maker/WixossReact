...
- **🆕 セッション（2026-07-22・続き252・Opus 4.8＋Codex 分担・**[P1_COMPLETION_ROADMAP](./P1_COMPLETION_ROADMAP.md) **バッチ1「状態条件節の持ち上げ」第4波＝センタールリグ条件＋ターン内履歴/出自条件22効果を採用**（census 1761→**1742**・golden 590→**593**）**（続き251 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **次の一手（Opus/Codex）**＝**バッチ1のクラスタ在庫はほぼ消化**（条件節残＝新カウンタ要の履歴系23・単発テール・粗い網）。次は **ROADMAP バッチ2「対象フィルタ合成」（クラス/色/名前包含/否定/除外 342候補）**＝共通 filter extractor を LOOK/SEARCH/TRASH/移動の各入口へ横展開（第2波の教訓＝投入前に候補の JSON 全件実測で偽陽性を分離してから切ること）。または見送り済み機構の解消（did-it continuation・hand-or-energy・owner:any 場条件・能力なし filter・新カウンタ群）。⚠**codex 指示書に「納品前に build:effects→heldReview を再実行し held 実測値を報告」を必須項目として追加**（CODEX_GUIDE §6 に追記済み）。**Sonnet はタスク1（§7 実機検証）**＝OR/FROM_DECK/PLACED_BY_EFFECT/IS_BETTING ゲートの実機スポットチェック（要実機検証）。
### 📊 恒久指標（維持中・逐次更新）
