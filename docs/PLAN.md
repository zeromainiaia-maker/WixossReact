...
**▶ 次の一手**＝**§5.1 の 🅲／🅳 を上から取る**（🅲 は `V-19` 一時レゾナの返却＝(cxl) 修正済みなので再開できる状態）。残りは **V エントリ 8件（🅲 1／🅳 7）＋続き616〜634 の21バッチ**。
...
> **残っているもの**＝下記 `V-nn` エントリ **8件**（🅱 は 2026-08-24 に**残0クローズ**／🅲 1／🅳 7。🅳 の各エントリは (a)(b)(c) の複数経路を持つ） ＋ **続き616〜634 の21バッチ**（段2 の parser/engine 改変ぶん・特に実機経路へ直接触ったのは第34「0枚選択可の UI」・第37「クラッシュ主体の限定」・第38「群割当の決定可否」・第40「`END_OF_ATTACK`」）。
...
- **2026-08-24 続き637 後（本行が直近の正）**：
  **census 608/608**、**golden 2653**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 269）**、
  ...
  **実機シナリオ定義総数 490**（+14＝V-84/85/83/04/16/87）、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
