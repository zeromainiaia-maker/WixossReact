...
**Opusタスク12＝未消化の在庫 6件（2026-08-10 続き404・408・417／2026-08-13 続き468・471／2026-08-14 続き474 で登録）**。⚠**2026-08-14 続き475 で (cxxv) を取り下げ（シナリオ偽陽性）**、**続き475b で (cxxvi)(cxxx) を残0クローズ**＝8件→6件。
...
- **🆕 セッション（2026-08-14・続き475／475b・Opus 5）＝**§7 実機検証の続き＋そこで見つけた engine バグの修正**＝**V-10 の赤3本を緑へ**、**V-01 を再検証して §3 (cxxv) を「シナリオ偽陽性」として取り下げ**、さらに**そこで局在が確定した実バグ2件（(cxxvi)(cxxx)）を engine で修正して V-01 を 6/6 緑に**した。**Opusタスク12 の在庫は 8件→6件**。ゲート全緑（**golden 1964→1966**＝トリップワイヤ2本追加・census 831 据置・smoke 10688／SKIP 0・fuzz 全0・lint 0 errors/259 warnings・**live JSON changed 0**）。**
  ...
  - **🔴③ 実バグ3件の真因を確定→うち2件を修正**＝**(cxxx)【新規】離場置換の「決定」が消費されず残留**（`applyEffectLeaveSubstitutes` は消費済み ctx を返すのに、**置換不成立側で呼び出し側が捨てて**いた＝**11経路すべて同じ形**）／**(cxxvi)** 相互身代わりの instance 複製＝**身代わりで先に場を離れた instance を離場ループが再処理**し、`removeFromField` が空振りしたまま移動先へ2枚目を push（`effectExecutor.ts:969-974`）＝**カードが増える**。🏁**この2件は続き475b で修正済み**（`sub.ctx` の無条件採用＋`isOnFieldTop` ガード。**golden に end-to-end のトリップワイヤ2本**を足し、**外すと FAIL することも確認**）。残る**(cxxix)** は `lifeCrash` が `autoEligible:false` なのに **`leaveSubstituteAskOptions` が `kind==='optional'` だけで絞るため選択肢に出て**、`applyEffectBanishSubstituteChoice` に分岐が無く**末尾の `trashStackSpell` へフォールスルー＝0枚トラッシュで成立**（実機ログと完全一致）＝**在庫のまま**。
  ...
  - **▶ 次の一手**＝**Opus 側**：**(cxxix)**（`autoEligible:false` を問いから外す最小手＋`applyEffectBanishSubstituteChoice` の未知 costType ガード。⚠**「対話 policy が入れば選択肢に出せる」という `:411-413` のコメントと矛盾しないよう**、実装するのか外すのかを先に決める）、次に **(cxxvii)(cxxviii)**（同型の「対象種別の取り違え」）。次点＝§6.4 O-20 の残21件・O-19。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き475／475b（§7 実機検証の続き＝V-10 決着＋V-01 を 6/6 緑へ＋(cxxvi)(cxxx) の engine 修正）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、**golden 1966**（**1964→+2**＝(cxxvi)(cxxx) のトリップワイヤ）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。**live JSON の変更は 0**（変更は `src/engine/effectExecutor.ts`・`scripts/goldenTest.ts`・`scripts/verifyBattleDrive.mjs`・docs）。🆕**実機シナリオ（`verifyBattleDrive.mjs`）＝既定 order の総数は据置**（新規追加0・判定を直したのが5本）。**緑/赤の内訳が 45緑12赤 → 51緑6赤**（V-10 の3本＋V-01 の6本が緑・`leaveSubAllTargetsAskedPerVictim` は複製検査を足したうえで緑）。🆕**Opusタスク12 の在庫＝6件**（**(cxxv) 取り下げ・(cxxvi)(cxxx) 残0クローズ**）。
