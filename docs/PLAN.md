...
- **🆕 セッション（2026-07-31・続き300・Opus 5〔Claude 実測＋検証〕＋codex-work〔実装〕）＝タスク16 第3波＝「エナへ移動したカード自身」の `ON_ENERGY_CHARGE` を新設し3効果を消化。Claude が3件是正**（golden **1168→1170**・census **1364→1363**（`BASELINE_HIGH=1363`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**232→234 warnings**〔`useHost`/`useGuest` の rules-of-hooks 偽陽性が 100→102＝stash 比較で実測。退化ではない〕・manual field loss 0・同型★0。live per-effect 差分 **changed 3 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **25→22**）
  - **① 投入前実測で PLAN の「次の一手」を差し替えた**。PLAN は `ON_HAND_DISCARDED` の原因オーナー軸を推していたが、実測すると **`collectHandDiscardTriggers` の呼び出し元が BattleScreen に7箇所**あり、しかも3効果の機構がバラバラ（原因オーナーのみ／原因＋ガードアイコン filter＋「そのカード」の同一性／手札とエナの2経路 OR＋解決単位の重複排除）＝CODEX_GUIDE §3-4 の「機構が同じものだけ束ねる」に反する。**`ON_ENERGY_CHARGE` 側は3効果が単一の根本原因を共有**（watcher が場のシグニしか走査せず、エナに移動したカード自身を永久に拾わない）していたので、そちらへ差し替えた。
  - **② 決め手＝先例が既に動いていた**。`ON_ENERGY_FROM_TRASH`（`BattleScreen.tsx:3028`）が**まさに「移動したカード自身がエナゾーンから発火する」中央 diff ループ**で、`WXK09-031/080/081` が現に使用中。今回の3件は **`from` が hand/deck/field に変わっただけ**。移動元つき検出（`detectHandAdded` の `{cardNum, from}`）と、`fromZones`／`movedSelf`／`byOwnEffect`／`duringAttackPhase` を全部 honor する収集器（`collectHandAddedTriggers`）も `ON_HAND_ADDED` 側に揃っていた＝**新 timing・新 action 型は不要**。
  - **③ 3効果とも timing 以外に条件が落ちていた**のを投入前に明細化して罠として渡した＝`WXDi-P03-073-E1` は原文の「エナゾーンにあるカードが**2枚以下の場合**」が JSON から丸ごと消えており、**timing だけ開けると「場からエナに行くたび無条件エナチャージ」の過剰実行**になるところだった。`WX14-066-E1` は「アタックフェイズの間」「ルリグかシグニの効果によって」「手札かデッキから」の3つが欠落。
  - **④ 二重発火は両側ガードで封じた**＝旧 React watcher（`BattleScreen.tsx:1638`）は `movedSelf` を skip、新 collector は `movedSelf:true` 必須。既存 `ON_ENERGY_CHARGE` **8効果は全て `movedSelf` を持たない**ので新経路の収集0件＝golden で不変条件として固定。
  - **⑤ 🔴 Claude 検証で是正3件**＝(a) **`byLrigOrSigniEffect` が `'ルリグ'`/`'シグニ'` の2値しか受理せず過小実行**（`CardData.Type` は **`'アシストルリグ'` 340枚・`'レゾナ'` 46枚を別値で持つ**。ルール上アシストルリグはルリグ・レゾナはシグニで、既存判定も `'ルリグ' || 'アシストルリグ'` を並記＝`execStubPart1.ts:3594`）⇒4値受理へ是正＋golden 追加。(b) **goldenTest.ts 末尾に、集計行と `process.exit(1)` の後ろで走る重複テスト58行**＝**落ちても件数に乗らず exit code も変わらない＝ゲートとして機能しない**ので削除（集計対象の同内容2テストは残置＝golden 1170 不変）。(c) **held +1（`WXK09-031`）の帰属を訂正**＝codex は「対象3効果の残りではない」とだけ書いたが、真因は **parser 変更のスコープ外波及**（`parseSentencePart3` の枝が `ON_ENERGY_FROM_TRASH` の `WXK09-031-E2` にも当たる）。**先頭アンカーでは弁別できないことを実測**し、live は held 未採用＝挙動不変なので据置して関数コメントに設計上の制約を残した。
  - **次の一手**＝**Opus：🆕 タスク16 残＝`[B維持]` 8件**（第3波で `WX14-066-E1`／`WXDi-P03-073-E1`／`WXDi-P12-079-E2` が抜け 11→8）。残りは**軸がバラバラで束ねにくい**＝①`ON_HAND_DISCARDED` 系3件（`WXDi-D09-P16-E2`／`WX24-P2-051-E1`／`WXDi-P13-051-E3`。**`collectHandDiscardTriggers` の呼び出し元7箇所**＋機構3種混在＝**1件ずつ投げるか、原因オーナー軸だけを切り出す**）②`WXDi-P06-038-E1`（エナ→任意宛先＋原因）③`WXDi-P06-072-E1`（バニッシュ主体のパワー軸＝`ctx.effectivePowers` が既にあるので**最も安い可能性**・要実測）④`WX05-020-E1`（ターン累積クラッシュ枚数）⑤`WXDi-P11-063-E2`（シグニの下に置かれた）⑥`WXDi-CP02-068-E1`。⚠**8件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので、退化させないことが最優先。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**今回の3効果も実機UI未検証**（続き298 の I／J-3、続き299 の4効果と合わせて積み上がっている）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き300（タスク16 第3波＝エナ移動カード自身の `ON_ENERGY_CHARGE`）後 最新値（本行を直近の正とする）**：census **1364→1363**（`BASELINE_HIGH=1363`＝停止中3効果の実働化＝機能改善）、golden **1168→1170**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**232→234 warnings**（⚠**退化ではない**＝`useHost`/`useGuest` の `react-hooks/rules-of-hooks` 偽陽性が新経路で 100→102。Claude が stash 比較で実測）、manual field loss 0、同型★0（5986枚・265群）、**held 250→251枚／署名グループ 97→98**（⚠**+1 は本バッチの parser 波及**＝`WXK09-031` が `parseSentencePart3` の枝拡張でスコープ外から held に落ちた。**live は未採用＝挙動不変**。詳細は BUGFIXES 第3波節）。live per-effect 差分 **changed 3 / added 0 / removed 0**（`WX14-066-E1`／`WXDi-P03-073-E1`／`WXDi-P12-079-E2`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 25→22 効果/22クラスタ**（⚠**実質20**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 11→8**。**新語彙2本**＝`triggerCondition.byLrigOrSigniEffect` と、`ON_ENERGY_CHARGE` での `movedSelf`/`fromZones` の受理（どちらも既存フィールドの水平展開＝省略時は従来挙動）。**新 timing・新 action 型はゼロ**。engine 新設は検出器1本（`detectEnergyAddedWithSource`）＋収集器1本（`collectEnergyAddedSelfTriggers`）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1170**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1363**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 第3波後に実測）。
>
