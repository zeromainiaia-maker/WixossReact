...
- **🆕 セッション（2026-08-07・続き373・Opus 5〔**§5d パターンA 第5バッチ**＝動的な同一性参照＋**据置1件の解除**〕）＝ユーザー指示「続ける」。触った層は `parseSentencePart1.ts`（トラッシュ→手札に1本＋whitelist 1枚＋ガード条件の縮小）／`parseSentencePart2.ts`（reveal の語順）／`effectParser.ts`（`IDENTITY_BATCH5B` に2件）／`public/data/effects_*.json`（6効果）／`goldenTest.ts`。**型・engine は無改造**。
  ...
  - **⚠据置は永久ではない＝続き370 からの `WXEX2-06-E2` を解除した**。「そのシグニと同じレベルの無色ではないシグニ」は動的等値を表せないため `nonColorless` を落としていたが、`levelEqTrigger` の配線で**2系統とも表現できるようになった**ので whitelist に追加。**部分filter禁止は「片方を表せない」ときの規律であって、表せるようになったら据置を解く。**
  - **⚠据置を固定していた golden 3件はアサーションごと更新した**＝据置テストは「いま表せない」という事実の記録であって恒久仕様ではない。もう1件の失敗は tripwire（`levelEqualsVar` 参照数 2→3）で、支払い経路が実際に変数を記録することを確認してから数を上げた。
  - **`WX24-P3-063-E1` は正しい MANUAL 定義が held に滞留していただけ**（merge バグではなく採用漏れ）＝**`manualEffects.ts` に書いても live に届いているとは限らない**。
  - **ゲート**＝全緑。**golden 1415→1417**（+2）、**census 1170→1166**（−4・`BASELINE_HIGH` 更新）、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors、held **231→230**。A/B 差分 2→1→2→1。
  - **次の一手**＝**§5d パターンA を続ける**。残クラスタ＝「共通する色」23件（`colorMatchesLastProcessed` 等が**型にはある**ので入口の有無を先に確認）・「クラス指定」155件・「条件節」286件。**新機構が要る登録済み2件**＝①`WXK11-040-E2`（コスト経路の「**名前**」変数が無い。レベル合計版はある）②`WX14-072/075-E1`（「公開する**か**トラッシュに置く」の選択肢構造が潰れている＝パターンD・タスク13）。**§5d 末尾の照合済み12件**も未修正。**`WXK09-029-BURST` の据置再評価**（続き372 で登録）も入口。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先。
