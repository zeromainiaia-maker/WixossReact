...
- **セッション（2026-08-24・続き637・Opus 5）＝§5.1 の最上位 V-84 を残0クローズ**（実機未検証の返済 1件目）。**engine/parser のバグは0**＝§6.4 O-41「レベル限定つき【ガード】禁止」は実機でも原文どおり**そのレベルだけ**を候補から弾いていた。gates 全緑・実機は新規3本＋回帰2本を**2回連続 5/5 PASS**（順序入れ替えでも安定）。
  - **観測できるようにした**＝`GuardResponseDialog.tsx` のガード候補ボタンに `data-testid="guard-card-<i>"` ＋ `data-card-num`。⚠**カード名テキストで照合しない**（全角文字・同名別レベルで壊れる）。
  - **入口を2つとも踏んだ**＝`blocked_actions`（【起】アーツ `WD15-010` の実行結果を直接注入。⚠アーツは「使った側のターン」にしか効かず CPU に使わせられない）と **CONTINUOUS**（`WX18-039` を相手場に実カードで設置）。
  - 🔑**負方向は必ず対照とセット**＝素の `GUARD` へ転落すると全候補が消え「限定が効いていない」と見分けがつかないので、3本とも**押せる側が残ること**を同じ run で assert。
  - 🔴**副産物＝既存 `guardExtraColorless{Sufficient,Insufficient}` の腐りを2件発見・その場で修正**。①`field.check` 未固定（§4.4 の1）②🆕**`field.signi` 未固定**＝`CORE_FIELD_KEYS` は注入で消えず**引き継がれる**ので、V-84(b) が置いた `WX18-039` が残って Lv2 のサーバントが「正しく」弾かれ FAIL していた。**盤面依存の判定をするシナリオは `field.*` を自分で書き切る**。

**▶ 次の一手**＝**§5.1 の実機未検証の返済を続ける**（🅱 の次は **V-85 数字宣言（`DECLARE_NUMBER`）が実機で宣言 UI を出すか**）。残りは **V エントリ 12件＋続き616〜634 の21バッチ**。
...
> **残っているもの**＝下記 `V-nn` エントリ **12件**（🅱 4／🅲 1／🅳 7。🅳 の各エントリは (a)(b)(c) の複数経路を持つ） ＋ **続き616〜634 の21バッチ**（段2 の parser/engine 改変ぶん・特に実機経路へ直接触ったのは第34「0枚選択可の UI」・第37「クラッシュ主体の限定」・第38「群割当の決定可否」・第40「`END_OF_ATTACK`」）。
...
- 🆕**🔶 V-85 数字宣言（`DECLARE_NUMBER`）が実機で本当に宣言 UI を出すか（2026-08-22 続き609 で登録・§5.3 O-41 派生）**
...
- **🔶 V-16 J-1 他能力の発動監視（続き383）＝2026-08-14 続き480 で**機構は実機で確認・シナリオは未緑**（Codex 起案→Claude 実機検証）。**続き571＝実バグ1件を発見**（Opusタスク12 **(cxxxix)**）。
...
- 🆕**🔶 V-87 手札上限超過の捨て札で「そのターン終了時」の遅延予約が効くか（旧 Opusタスク12 (cxli) の観測点・2026-08-23 再登録）**
  ＝現行 `v20DiscardSkipFirstBlocksSecond` は `WX16-042-E1` の**能動 discard へ迂回**しており `confirmEndDiscard` を通らない。
  **手札上限超過で捨てさせる専用シナリオの新設が要る**（`confirmEndDiscard` 経路を通す形）。
...
- **🔶 V-19 一時レゾナの返却（続き433）＝2026-08-14 続き481 で**「場から消える／トラッシュへ行かない／返却ログが出る」まで到達・4シナリオは赤のまま**（要追試）。🆕**続き573＝`SHOTS=1`で再現・原因を特定→観測側ではなくUIの実装漏れ（Opusタスク12 (cxl)）**＝`v19Wx07050NoDiscardDoPhaseAdvanceReturns` が召喚後の「羅星　ハダル」画像クリックで毎ティック `Timeout 3000ms exceeded` を繰り返す件は、**`SigniOnPlayCostModal` が `WX07-050-E1` の `handDiscardSigni` コストを認識せず候補ピッカー自体を描画しない**ため（`img[alt="羅星　ハダル"]` は候補ではなく画面下の通常手札表示に誤ヒットしていた＝クリック対象がモーダルのオーバーレイに覆われていて timeout）。詳細・修正方針は Opusタスク12 (cxl)。**V-19 は Opus 側の (cxl) 修正待ち＝Sonnet側では現状これ以上進められない**（コスト選択UIが無い以上、正しい手順でモーダルを進められない）。
  - [ ] 🔴**出したレゾナがターン終了時にルリグデッキへ戻るか**（`WX07-050`＝【出】／`WX16-Re18`＝【起】）＝**実機で `fieldGone=true` `notTrash=true` `trashDelta=0` `log=true`（`ターン終了時：…をルリグデッキへ戻す`）まで観測**できるのに、**`host.lrig_deck` が空のままで `returned` が立たない**。
    ...
    - ⚠**断定しない**＝候補は①**観測側**（シナリオが読む側／タイミング。spec が `lrig_deck: []` 始まりで `placed.length + 1` の比較が崩れている可能性）②**永続側**（`myEndState` 組み立てのどこかで `lrig_deck` が上書きされ**カードが消失**＝一時レゾナが二度と使えない実害）。🔴**§5.1 📌12 のとおり engine を読み始める前に観測側を疑う**（今日すでに (cxxxiii) が同じ形でシナリオ偽陰性だった）。
...
- 🆕**V-86 続き635 の3経路（未着手・engine/golden は固定済み）**
...
- **2026-08-24 続き637 後（本行が直近の正）**：
  **census 608/608**、**golden 2651**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 269）**、
  ...
  **実機シナリオ定義総数 479**（+3＝V-84）、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
