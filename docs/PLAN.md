...
| (vii) | 「アップ状態のこのシグニをダウンしてもよい」系の対象/自己混同7件（続き89）。**✅続き163（Opus）で4枚 clean 修正**（WX25-P1-055/WXDi-P04-059/WXDi-P13-074/WXDi-CP01-040＝DOWN を self thisCardOnly optional へ・正準形 WD12-013）。**残3枚＝§6.3級**＝WX25-P3-089/WXDi-P15-084（引用付与＝対象シグニ/ルリグへ内蔵能力付与＝タスク1）・WX25-P2-112（アップルリグ down＋「ダウンしたルリグと共通する色」動的フィルタ）。詳細 BUGFIXES 続き163 |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 359・smoke/fuzz 全0・同型★0・census 2088。
>
