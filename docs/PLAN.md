...
- 🏁**セッション（2026-08-27・続き693・Opus 5 単独）＝`O-120`。【ランサー】によるクラッシュの原因限定が無く、通常のバトルダメージでも発火していた。**
  ユーザー指示＝①Sheet1 の機構待ちを解消する ②**codex は最初の1バッチだけ**（B14 で使用上限に到達）＝**以後は Claude が直接実装**。
  ...
  live 変化 **3 effectId ちょうど・スコープ外0**。
  - 🔑**真因は parser の1本の regex**＝`parseAllyLifeCrashSubject` が「シグニ**が**対戦相手のライフ…」を**密着マッチ**していたため、
    **主語と「対戦相手の」の間に原因句（【ランサー】によって）が挟まる**文型で丸ごと外れ、
    **原因条件どころか主語の限定（`any_ally`／シグニ）まで消えていた**（実測3効果）。
  - ✅**足した受け皿**＝`triggerCondition.crashedByKeywords` ＋ `PlayerState.crash_cause` / `pending_crash_causes`
    （`crash_source_card_num` と**同じ添字規約**）。判定は `crashCauseMatches` の**1本**を headless と実機が共有し、**fail-closed**。
  - ⚠**書き手を1つでも落とすと壊れる**ので、発生源を書く**全地点**（通常ダメージ／ダブルクラッシュの pending／
    `execLifeCrash` 2地点／`lifeCost`／リセット2地点）で原因も同時に書いた。
  - 🔄**既存の「据置契約」golden を反転させた**＝旧テストは「原因funnel実装時に本テストを反転」と明記して
    過剰実行を固定していた（CODEX_GUIDE §5-4）。**据置は永久放置ではない**の実例。
  - ⚠**実機で判別力があるのは negative 側**＝`b15plaincrash`（通常クラッシュで引かないこと）。
    positive は修正の有無にかかわらず引くので**単独では反転確認にならない**。

**▶ 次の一手**＝**`O-121`（「このターンに…バニッシュしていた場合」の台帳）**。
実測済み＝`WX11-031-E1`（Sheet1）／`WXK02-034-E1` の**2効果**がどちらも条件ごと落ちて**無条件発火**。
当ては `signi_downed_this_turn` ＋ `SIGNI_DOWNED_COUNT_THIS_TURN{filter}` と**同じ形**へ揃えること（別実装にすると片方だけ直る）。
記録地点は**中央 diff（`BattleScreen.tsx:3168`）が唯一の funnel** で `causeOwnerId` / `causeSourceCardNum` がその場にある。
⚠**併せて測る**＝`ON_SIGNI_BANISH_OPPONENT` は**バトルバニッシュ経路にしか配線されていない**（`BattleScreen.tsx:9888`）ので、
効果バニッシュではそもそも `WX11-031-E1` が発火しない。
**Sheet1 の残り機構待ちは `O-108` / `O-109` / `O-110` / `O-112` / `O-113` / `O-117` / `O-121`〜`O-129`＝1項目＝1巡**
（配線まで実測済みなので着手前の再調査は不要）。
...
| **O-128** | 🆕**【常】の中の引用【自】能力付与が `STUB{GRANT_ABILITY_INNER_TEXT}` へ落ちる** | M〜L | **2026-08-27 続き692 で分離**＝`WX09-Re07-E1`（Sheet1・HIGH×2）【常】「…合計３体あるかぎり、このシグニの基本パワーは10000になり、このシグニは「【自】：対戦相手の効果によってこのシグニが場を離れたとき、…探して場に出し…」を得る」。live は `POWER_SET` ＋ `STUB{GRANT_ABILITY_INNER_TEXT}`＝**付与される【自】が丸ごと消えている**。■`GRANT_ABILITY_INNER_TEXT` は live **34カード**の catch-all で engine が実行時に原文を regex で読み直す＝**`O-60` A群の一部**。■`GRANT_QUOTED_AUTO_ABILITY`（既存・miss3/live4）が受け皿候補。■**1カードだけ直さない**＝着手時に 34カードを分類して同型をまとめて取る。 ■🆕**2026-08-27 続き693（`O-120`）で同族をもう1件確認**＝`WX24-P2-055-E1`「あなたのシグニ１体を対象とし、ターン終了時まで、それは「【自】《ターン１回》：このシグニが【ランサー】か【Ｓランサー】によって…」を得る」は、**live で付与（`GRANT_EFFECT`）自体が落ち**、引用能力の本体（コスト支払い→ライフクラッシュ）が **`ON_ATTACK_PHASE_START` で直接走る別物**になっている。⚠**原因限定以前の問題**なので `O-120` では触っていない。 |
