...
| (lxxxv) | **🆕「使用する際…してもよい。そうした場合、使用コストは…減る」＝22枚が未実装（2026-08-03・(lxxxi) 残テール消化中に実測）**＝(lxxxi) で作った「使用時の任意支払い」機構は**「になる」＝置換の2枚**（`WX21-035`／`WX21-071`）だけを読む。隣接する**「減る」形は22枚**あり、いまはどれも支払いUIが無い＝**常に印刷コストでしか使えない**（例 `WD10-006`＝《赤×4》《無×2》のまま／`WX24-P2-002`〜`010` の5枚＝ルリグデッキのアーツ1枚を置けば《色×3》減るはずが減らない）。⚠**同じ regex では読めない**＝支払いの粒度が違う：(a)「捨てたシグニ1枚につき《黒×2》《無×1》減る」の**枚数比例**（`WX13-025`／`WX20-004`／`WX20-007`／`WD10-006`）(b)「2枚**まで**」の**可変枚数**(c) 支払い元が手札とは限らない（ルリグデッキのアーツ＝`WX24-P2-*`／`WX25-P2-*` の9枚・ライフクロス＝`WX24-P3-004`・場のシグニ＝`WX25-P1-110`・キー＝`SPK01-07`）。**流用できるのは UI の骨組み**（候補絞り込み・組の充足判定・`castSpell`/`executeArts` の discard 引数）。⚠着手前に**先頭の `STUB OPTIONAL_COST` が本体を丸ごとスキップする過剰ゲート**（(lxxxi) 残テールで是正した Pattern⑤ の罠）を同じ22枚でも確認すること＝解決中に「スキップ」を選ぶと後続ステップが飛ぶ  |
...
- **🆕 セッション（2026-08-03・続き336・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(lxxxiv) 残0クローズ**＝スペルカットイン経路に**ベット宣言UI**を実装（ユーザー指示「opusタスク12(lxxxiv)を行う」）。golden **1333→1334**・**live JSON の変更なし**（UI＋engine 配線のみ）・全ゲート緑。
  - **真因＝同じアーツを使う経路が2本あり、片方にだけベットが実装されていた**。`ArtsModal`／`executeArts` はベット枝（`parseBetOptions`→枚数選択→コイン消費→`is_betting_this_effect`→`ON_COIN_PAID`）が揃っているのに、**スペルカットイン窓は別経路**（`CutinModal`／`handleCutinUse`）で、そこにはベットの概念が丸ごと無かった。
  - **影響＝機械抽出で確定8枚**（CSV `Timing` に「スペルカットイン」＋原文に「ベット―」）。うち **`WX17-019`（リフューズ・スペル）は Timing がカットイン専用**＝`ArtsModal` から到達不能なので**ベットを宣言する手段がゲーム内に一つも無かった**（(lxxxi) のコスト置換《青×0》も当然届かない）。残り7枚はメイン/アタックフェイズ経由なら宣言できる＝カットイン窓でだけ不可。
  - **実装**＝①`handleCutinUse` に `betCoins`（コイン消費・`COIN_SPENT`・`bet_coins_paid`・`is_betting_this_effect`。**非宣言時も明示クリア**＝持ち越し防止の既存規約）②**`ON_COIN_PAID` の配線**＝⚠カットインは `executeEffect` で **inline 解決**＝スタックを経由しないので `queueCardEffects(extraEntries)` が使えない。収集エントリを `pushToStack`／`initStack` で自前で積み、commit の `effectStack` で渡す（`FINISH_CUTIN` にも `effectStack?`＝**省略＝不干渉**を追加）③`CutinModal` に `ArtsModal` のベット枝を移植（state は `useCutin.cutinBetAmount`＝`closeCutin` で一括リセット）。
  - **🔴 これが無いと直らなかった1点＝候補一覧の支払い可否も置換後コストで見る**。`WX17-019` は印刷《青》×2 なので、**青エナ2枚が無いと候補ボタンが「エナ不足」で無効化され、ベットを宣言する画面へ辿り着けない**（＝UIを足しただけでは到達不能のまま）。「印刷コストで払える **or** ベット置換後コストで払える」に緩めた。
  - **新規在庫を3件登録**＝**(lxxxvi)** スペル本体のベットが `ON_COIN_PAID` を積まない（ベット持ちスペル7枚。⚠`pending_spell` 待ち中に積む＝解決順序の設計判断が要るので同梱せず）／**(lxxxvii)** カットイン窓のコスト算出が `computeArtsEffectiveCost`／`applyContinuousCostDecreases`／`card_cost_replacements` を通っていない（メイン経由と食い違う）／**(lxxxviii)** ベット分岐が JSON 側に無い3枚（`WX19-006`・`WDK12-007`＝`STUB BET_MECHANIC`／`WD21-007`＝`STUB GRANT_QUOTED_AUTO_ABILITY`）と `betChoose` 未生成＝**アーツ経路でも同じ**穴。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxv)／**🆕(lxxxvi)**／**🆕(lxxxvii)**／**🆕(lxxxviii)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜336 の全件が実機UI未検証**。⚠🔴**続き333〜336 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①相手スペルへのカットイン窓で `WX17-019` が青エナ0でも候補に出てベット1枚で撃てる**②`WD20-007` のベットで《緑×2》《無×2》→《緑×0》③`WX17-023`/`WD18-008`/`WDK10-008` の「ベットしていた場合」枝が解決時に効く④非ベット使用で前回のベットが持ち越されない。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き336（🏁§3 タスク12(lxxxiv) 残0クローズ＝スペルカットインのベット宣言UI）後 最新値（本行が直近の正）**：golden **1333→1334**（+1＝`FINISH_CUTIN` の `effectStack` 指定時のみ `effect_stack` を書く〔省略＝不干渉は既存2件が保証〕。併せて既存のコスト置換テストに**対象8枚の `parseBetOptions` 段階＋`Timing` にカットインを含むこと**と `WX17-019` のベット時《青×0》を追加＝**ベットUIの枚数ボタンはこの options から出る**ので空になれば落ちる）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0、held **259枚／110群 据置**。**parser/effects JSON は完全不変**（`public/data/` に差分なし・live per-effect changed 0・**新語彙0本**＝`BattleAction` も 18種のまま〔`FINISH_CUTIN` の payload 拡張のみ〕）。触った層＝`CutinModal`／`useCutin`／`handleCutinUse`／`battleController`。⚠**ベットUIと支払い配線は golden 非カバー**（BattleScreen/モーダル層）＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き336＝🏁§3 タスク12(lxxxiv) 残0クローズ後）**＝golden **1334**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **259枚／110群**。
>
