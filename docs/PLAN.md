...
- **「このターンにあなたがアーツを使用していた場合」条件（WX25-P1-106-E1・2026-07-03発見）**＝`turn_arts_used` 系の PlayerState フラグ＋アーツ使用経路での記録＋ターン境界リセットが要る（owner は続き13で是正済・条件のみ脱落中＝無条件発火の近似）。同カード BURST のダメージ置換近似は §6.1 `PREVENT_DAMAGE` に合流。
