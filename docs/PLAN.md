...
| 🆕(xxii) | **後置条件節の IS_MY_TURN 誤変換（当初127件）**＝続き143〜212 の12バッチ＋続き241＋続き283〜285で系統消化（LAST_PROCESSED 系条件の一般化・`STATE_CONDITION_CLAUSES` 拡充・`unwrapWrappedRecorder` 等）。**残23件**（段3着手前41→続き283で39→公開軸30→ミル/トラッシュ軸23）。続き285は `MILL` の任意/底/宣言数/until実移動記録、life source `LOOK_AND_REORDER`、既存`TRASH`記録を使い7効果を消化。`TRASH`本体は不変。残は移動系[C]、前段 STUB、または新規状態追跡機構（ビート数/リミット/ターン累計エナ/did-search/アタック幾何/追加コスト・色支払い）が要る。全41件の [A]/[B]/[C] 再分類は `docs/_partial_triage.txt` 2026-07-27段3節、現行明細は `docs/_partial_report.txt`。 |
...
- **🆕 セッション（2026-07-28 続き292・Opus）＝§3 タスク12(lii) 修飾語なし「シグニN体を対象とし」の owner:self 誤りを消化**（golden 803→808・census 1521 据置）
  - **症状**＝所有者を限定しない「シグニ1体を対象とし、それを〜する」が `owner:self` へ落ち、原文が「どちらのプレイヤーのシグニでもよい」と言う対象を自分に限っていた。除去系では**自分のシグニしか狙えない＝実質使えない効果**（WX07-027-E2／WD10-009 ほか）。
  - **在庫メモの要求どおり全数機械分類を先に実施**＝対象句を全カードから抽出し、**その名詞句の直前セグメント**（。、「」『』（）：区切り）に所有格があるかで判定。対象句 **3670**（OPP 3093／SELF 474／**BARE 101**）。BARE 101句を単独パースして実 owner で仕分け＝**SELF 31（★真の候補）／既に正しい 27／NO_TARGET 21／偽陽性 22（正面18＋読点分断4）**。明細 `docs/_bare_signi_triage.txt`・生成器 `scripts/archive/bare_signi_owner_scan/`。
  - **engine が先に壊れていた**＝`owner:any` は **`execPowerModify` だけ**が両フィールドを候補にしており、他は `ownerState(owner)` に素通しで**片側（多くは相手側）に潰れて**いた＝既に any を出していた24句も半分しか効いていない。共通ヘルパー `fieldCandidatesByOwner`／`sideOfFieldCard` を新設し、BANISH／BOUNCE／UP／DOWN／FREEZE／LEVEL_MODIFY／POWER_SET／GRANT_PROTECTION を両フィールド＋`both_field` スコープへ統一（適用側は選ばれた1枚の所属で解決。**相手側にしか効かない保護は相手側候補にだけ適用**）。
  - **parser** は `signiClauseOwner` を新設し**対象句がある文だけ・その直前セグメントで**判定。「対戦相手」が文中にあれば従来どおり先に opponent へ倒す＝**既存の opponent 判定は一切変えない**（「対戦相手は自分のシグニ1体を対象とし」形を反転させないため）。
  - **⚠差し戻し1回**＝GRANT_PROTECTION 規則にも当てたら「このシグニは**対戦相手の効果によって**バニッシュされない」形26枚で**自己保護が相手シグニへの付与に反転**（文中の「対戦相手」はバニッシュの主体であって所有者ではない）。当該規則は self 据置へ戻し、targeted 形の WX21-015／WXK07-028 は honest defer。
  - **影響範囲を実測**＝parser 出力を変更前後で**全9283カード分スナップショットして差分**を取り、**27カード・すべて owner の self→any のみ・構造差分ゼロ・意図外0件**を確認してから採用した。
  - **検証**＝`npm run gates` 全緑。golden 5件追加（両フィールド候補＋スコープ／**自分の場のシグニを選ぶと自分の場から消える**＝相手が巻き添えにならない／DOWN・FREEZE の所属側適用／**自分側候補に相手の耐性を適用しない**／WX07-027-E2 実データ）。regen 後、逆翻訳が「自分または対戦相手のシグニ1体をバニッシュする」と原文どおり出ることを確認。⚠**census 据置**＝語彙欠落の計器で owner の誤りは元から未計上。⚠**実機未検証**。
  - **副産物（未修正・記録のみ）**＝(a)「アップするかダウンする」の2択が DOWN 単体に潰れる（WX19-029 ほか）(b) `CENTER_LRIG_OR_SIGNI` のルリグ選択肢が engine に無い (c) WD17-009-E1「正面のシグニ」が frontOfSelf 経路で owner:self のまま。
  - **次の一手**＝**Opus：タスク12 の在庫（(lv)/(xxii)残/(xxxix)残）または §6.3 残機構／タスク14 の残43**。**Sonnet：タスク1（§7 実機検証）＝レゾナ召喚UIの残り①ATTACK 窓 ②SPELL_CUTIN 窓 ③REQUIRES_NEW_FLOW ④支払いトリガー発火**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 782・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1527（＝`BASELINE_HIGH`・回帰ゲート）。続き282で gates 10回連続 FAIL 0。
>
