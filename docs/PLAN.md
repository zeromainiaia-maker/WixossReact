...
- **🆕 セッション（2026-08-15・続き488・Opus 5）＝§6.4 **O-3 の「追加のアタックフェイズ」「このターンの自己制限」「次の相手ターン終了時まで」3クラスタ（5効果）を解体＝8効果の挙動是正**（**過剰実行4＋恒久 no-op2＋対象取り違え2**）。ゲート全緑（**golden 2012**＝+11・**census 831 据置**・lint 0/259）。
  - **🔴① 最大の発見＝「遅延タイミング宣言」の後続文が全部その場で走っていた**＝原文「〜追加のアタックフェイズを加える。**この方法で加えたアタックフェイズの開始時**、〜」「**次の対戦相手のアタックフェイズ開始時**、〜」は文単位に切ると本文が後続ステップとして並ぶ＝**宣言した瞬間に実行**される。`WXK06-026-E1`＝メインフェイズ中に全シグニがアップ／`WX22-010-E3`＝場出し＋ルリグアップが即時／`SPDi43-24-E2`＝使った瞬間に相手のアタックを1回無効化。**受け皿 id を見ただけでは分からない壊れ方**（STUB は no-op でも、その後ろが走ってしまう）。
  - **🔑② 新機構 `ADD_EXTRA_ATTACK_PHASE` は「次のフェイズを決める1点」に置く**＝`resolveNextPhaseAfterAttack` が (a) ATTACK_LRIG の次を END でなく **ATTACK_ARTS** に戻し (b) **キューを1件減らし** (c) `onStart` を `pending_extra_attack_phase_start_effects` へ移す、を**同じ戻り値で**返す。別分岐にすると減らし忘れ＝**無限ループ**／減るのに進まない＝不発のどちらかになる。`ON_ATTACK_PHASE_START` の収集判定を `phase==='MAIN'` から **`nextPhase==='ATTACK_ARTS'`** に変えないと2周目の開始時トリガーが1つも走らない。⚠CPU 経路は `SET_TURN_PHASE` しか commit できず state を書けないので**通していない**（母集団2枚＝ルリグ【起】とスペルで CPU は能動使用しない＝安全側の近似・O-1 で解消）。
  - **🔑③ 「支払えない」は「支払い元を作る側」に載せる**＝「このターン、あなたは１以上のエナコストを支払えない」（`SPK01-10-E1`）は `buildEnergyPayPool` が **空配列を返す**だけで表せる＝`canAffordGrowCost`／`canAffordWithExtraCost` は**エナ1以上のコストだけ** false になり《色×0》は通る（原文の「１以上の」がそのまま出る）。14本の支払いモーダルに検算を撒かない。CPU は pool を通らず `cpuSt.energy` を直読みするのでグロウ判定にだけ同じ関数を当てた。
  - **🔴④ 「ハンドラ持ちの汎用 id」に落ちた文は `census:stubs` に映らない**＝`SPK01-10-E1` の「ターン終了時、それらを**エナゾーンから**トラッシュに置く」は既存規則（「場から」しか見ない）に外れて `STUB{OPTIONAL_COST}` へ落ち、**丸ごと no-op**（2枚エナチャージの対価だけが消えていた）。A群に出ないので専用 id `TRASH_ENERGY_AT_TURN_END` へ寄せて実装（消化は人間2経路＋CPU1経路の turn-end funnel）。
  - **🔴⑤ `ADD_TO_FIELD` は `source` が無いと「デッキの一番上」を場に出す**（`execAddToField` の `!src` 分岐）＝`WX22-010-E3` の「それらを場に出し」は参照先（チェックゾーンへ移したシグニ）が未実装なので、残すと**無関係なシグニが1体増える**。畳み込み時に落とした（`dropDanglingDeckTopPlacement`）。
  - **🔴⑥ 対象選択を挟む経路にも同じ delta 解決を入れる**＝新設 `POWER_MODIFY.deltaFromZone`（「エナゾーンにある＜X＞のカード１枚につき＋N」）を `execPowerModify` だけに足したところ、選択UIを通る対象（`thisCardOnly` を含む）では `applyDirectAction` 側の `resolveNum` が `{$ref}`/0 に潰して**無言でパワー±0**になった。**同じ値を2箇所で解くなら両方直す。**
  - **🟢⑦ 副産物＝「このシグニを場からトラッシュに置き、〜」の連用中止形を規則化**（3効果）＝`WX20-049-E1` は自己トラッシュが**丸ごと脱落**（対価なしでサーチ）、`PR-422-E2`／`WX26-CP1-066-E1` は `TRASH{SIGNI self 1}`＝**好きなシグニを選ぶUI**に化けていた（原文は「このシグニ」固定）。前置を剥がして再帰パースする1本で3件とも是正され、`WX26-CP1-066-E1` は後続の入れ子条件（「あなたの場に他の＜プリオケ＞のシグニがある場合」）と `GAIN_LRIG_BARRIER` まで正しく載った。
  - **▶ 次の一手**＝**O-3** の残10（A群が常時の在庫表）。次は `NEXT_OPP_ATTACK_PHASE_START` 2（**即時実行は停止済み・機構は未実装**＝「次の相手アタックフェイズ開始時に本文を実行する遅延予約」。受け皿は `pending_opponent_attack_facedown_returns` と同じ形で作れる）→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き488（§6.4 O-3 の3クラスタ解体）後 最新値（本行が直近の正）**：census **831 据置**（⚠一度 832 まで増えたが計器較正1件で戻した＝`ADD_EXTRA_ATTACK_PHASE` を「トリガー:アタックフェイズ開始時」の対応語彙に追加）、**golden 2012**（+11＝新機構3・live 形3・データ不変条件/トリップワイヤ5）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **108枚 / 48群**、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **30種/35件 → 28種/31件**）。🆕**live JSON changed 8カード**（`SPK01-10`／`PR-422`／`SPDi43-24`／`WX20-049`／`WX22-010`／`WX26-CP1-066`／`WXDi-P07-050`／`WXK06-026`。CSV 非改変）。🆕**挙動是正 8効果**（過剰実行4＋恒久 no-op2＋対象取り違え2）。🆕**新機構＝`ADD_EXTRA_ATTACK_PHASE`／`blocked_actions:'PAY_ENERGY_COST'`／`TRASH_ENERGY_AT_TURN_END`／`POWER_MODIFY.deltaFromZone`／`DeployBlockReason:'ALL_BAN'`**。
