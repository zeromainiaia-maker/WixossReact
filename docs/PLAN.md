...
| 3 | DRAW 脱落の parseSingleSentence 直呼び経路 | parser修正 | S〜M | **🆕続き228で「デッキの一番上のカードをエナゾーンに置き、X」連用中止の後続脱落2件を消化**（WX15-098/WX19-030-E2＝連用中止 splitter に追加・census 1845→1843。詳細 BUGFIXES 続き228）。残＝WX20-071（3項＋DURING_OPP_TURN/IS_SELF_ACCED 条件が hoist されず split ガードに掛かる）・split ガードで止まる複合（WXK07-042/WX20-049/WX26-CP1-066）・対戦相手ドロー idiom・per-count ドロー・入れ子条件内・`対象とし`挟みのエナ置き＋バニッシュ（WX05-024/WX13-034）。先頭自ドロー（WXDi-P13-001）は✅fresh で DRAW 捕捉済（残＝then節の TRANSFER_TO_DECK 脱落は別系統）。**🆕使用条件ピースの多段条件節脱落（続き229 観測）**＝ドリームチーム系ピース（WXDi-P08-003「白/赤/黒のルリグがいる場合」3段・WXDi-P13-001「その後、《ディソナ》3体ある場合」）で「その後、〜の場合、〜」の条件節付き後続セグメントが丸ごと脱落し REVEAL/DRAW 等の先頭アクションだけ残る系統（使用条件＋parenthetical 前置き＋多段 その後条件節）。(a)(b)(c) 主部分は✅続き107 |
