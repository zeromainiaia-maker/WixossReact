...
- **🆕 セッション（2026-08-07・続き370・Opus 5〔**§5d パターンA 第2バッチ**＝「無色ではない〜」〕）＝ユーザー指示「続ける」。触った層は `parseSentencePart1.ts`（SEARCH 合成に1行＋whitelist に4枚）／`public/data/effects_*.json`（9効果）／`goldenTest.ts`／`vocabCensus.ts`（BASELINE）。**型・engine は無改造**。
  - **`nonColorless` は既に型にも `matchesFilter` にも実装済みだった**＝壊れていたのは**フィルタ合成の配線**。`WXK02-010`「あなたのデッキから**無色ではない**シグニを3枚まで探して…」等が**無色シグニまで探せる過剰効果**になっていた。**「語彙が無い」ではなく「語彙はあるが呼ばれていない」**＝§5d パターンA で繰り返し出る形。
  - **⚠段階ロールアウト機構を尊重した**＝「トラッシュ→手札」経路には既存の**カード whitelist**（`TTH_FILTER_BATCH2_WAVE1_CARDS`）があり、これは過去に全体有効化が危険だったための機構。**一気に外さず、原文照合した4枚だけ**を追加した。
  - **⚠据置＝`WXEX2-06`**「**そのシグニと同じレベルの**無色ではないシグニ1枚」＝動的等値と `nonColorless` を同時に表せないため既存ガードが落とす（**部分 filter だけを採用しない**方針）。golden に「据置であること」も固定。
  - **前バッチの教訓を実際に適用した**＝配線前に**全CSV走査で「無色ではない」55件を分類**し、「〜として」（付与形）・「〜場合」（条件節）のような**filter にしてはいけない用法が無い**ことを確認してから配線した。
  - **ゲート**＝全緑。**golden 1410→1411**（+1）、**census 1189→1184**（−5・`BASELINE_HIGH` 更新）、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors、held 242 据置。採用は自動8枚＋フィールド粒度の手採用1枚。
  - **次の一手**＝**§5d パターンA を続ける**。census 明細に残るクラスタ＝「同一性(〜と同じ色/レベル/名前)」21件・「否定フィルタ(〜ではない○○)」の**無色以外**（レゾナではない／＜C＞ではない／〈色〉ではない＝`excludeResona`／`cardClassExclude` は型にあるが**〈色〉除外の型は無い**）・「除外(〜以外の)」8件。**いずれも「語彙はあるが配線されていない」か「型が1つ足りない」のどちらか**なので、まず型の有無を確認してから取る。§5d 末尾の**照合済み12件**（owner取り違え・重度混線を含む）も未修正のまま。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-07 続き370（§5d パターンA 第2バッチ＝「無色ではない〜」の `nonColorless` 配線）後 最新値（本行が直近の正）**：golden **1410→1411**（+1＝9効果に載ること／`WXEX2-06` の据置／engine の有色・無色判定）、census **1189→1184**（−5・`BASELINE_HIGH` も 1184 へ更新＝**機能是正**）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **242 据置**。**live JSON の変更は9カード**。**型・engine は無改造**（`nonColorless` は実装済みで、壊れていたのは parser のフィルタ合成の配線）。⚠**残す教訓**＝(a) **§5d パターンA の実態は「語彙が無い」ではなく「語彙はあるが呼ばれていない」**＝新しい型を足す前に**既存フィールドの有無を必ず確認**する。(b) **段階ロールアウトの whitelist は理由があって存在する**＝過去に全体有効化が危険だった機構なので、一気に外さず**原文照合した分だけ**足す。(c) **2系統を同時に表せない形は「部分 filter だけ採用しない」**（`WXEX2-06`）＝中途半端な採用は別種の過小実行を生む。
