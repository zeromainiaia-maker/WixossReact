...
- **🆕 セッション（2026-07-31・続き306・Opus 5〔**Claude が実装まで担当**〕）＝タスク16 `[B維持]` から `WXDi-P11-063-E2`（このカードがシグニの下に置かれたとき／このカードの上にあるシグニ）を1件消化＝同文型の兄弟4効果の過剰実行も同時に是正**（golden **1190→1195**・census **1361→1358**（`BASELINE_HIGH=1358`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・held **251枚/98署名 据置**。live per-effect 差分 **changed 5 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **18→17**）
  - **① 穴は3層あった**（どれか1つでも残ると no-op か別物）＝(a) timing 語彙なし（`isKnownUnwiredAutoTrigger` の停止形）(b) 対象語彙なし＝「このカードの上にあるシグニ」がパワー修整チェーンの**最後の else** に落ちて `owner:'any'/count:1`＝**場のシグニ1体を任意選択**に潰れていた (c) **engine の唯一の発火地点が到達不能**。
  - **② 🔴 (c) が本命の発見**＝`INTERNAL_PLACE_SELF_UNDER_SIGNI` は **execStubPart1 と execStubPart2 に同名で2つあり**、`execStub` は part1→part2 の順に引く。part1 側（`lastProcessedCards` 変種）が**必ず値を返す**ため、part2 側（`stub.value` のホスト＋配置後に `ON_PLACED_UNDER_SIGNI` を実行する変種）は**丸ごと死にコード**だった。⇒ part1 を `stub.value == null` に限定して素通りさせた（生成元は part1:610 が value なし／part2:3285 が value 付きで**排他**）。⚠**続き304 の「同名 STUB が2つあり片方が到達不能」と同じ罠が別 id で再発**＝`execStubPart*` に同名 id が無いかを最初に grep する手順にする価値がある。
  - **③ 新語彙1本**＝`TargetFilter.aboveSelf`（既存 `acceHost` の兄弟＝装着経路が【アクセ】ではなく**スタック下**）。honor は2経路＝AUTO/ACTIVATED は `execPowerModify` が `resolveAboveSelfCardNum` で1体に確定し**選択UIを出さず**適用（原文に「を対象とし」が無い＝`autoTargetedCards` にも積まない）／CONTINUOUS は `calcFieldPowers` に「スタック下カード→ホスト」ループを新設（`signi_acce` ループのスタック版）。**新 timing 収集経路・新 collector はゼロ**。
  - **④ 兄弟4効果は「停止」ではなく過剰実行だった**（`WXK08-086-E1`／`WXDi-P03-057-E2`／`WXDi-P05-050-E2`／`WXDi-P05-060-E1`）＝【常】で `calcFieldPowers` が `count≠ALL` を「効果元自身」に解決する既存規則により **自分に +N していた**。`aboveSelf` を付けたことで自己バフが止まり、下に置かれたときだけホストに乗る。`WXDi-P05-050` はカード単位 PRESERVE（E1 が MANUAL）なので E2 だけ外科パッチ＝parser 出力と一致（`parseStatus` のみ差）。
  - **⑤ 逆翻訳は今回も同時に直した**（3回連続で定着）＝`aboveSelf` の主語＋`ON_PLACED_UNDER_SIGNI` の timing 文言。《名前》限定のときは「シグニ」を付けない（原文どおり「このカードの上にある《コードアクセル　ヒャッハー》のパワーを」）。
  - **次の一手**＝**Opus：タスク16 残＝`[B維持]` 3件**（本件で 4→3）。①`WXDi-P13-051-E3`（`byOpponentEffect`＋**手札捨てとエナ→トラッシュの2経路 OR**＋「効果1つによって」の解決単位重複排除＝**最も重い**。⚠**本体も要修正**＝原文「カードを1枚引く**か**【エナチャージ１】」なのに live は `ENERGY_CHARGE_FROM_DECK` 単体＝**CHOOSE が潰れている**）②`WXDi-P06-038-E1`（エナ→**任意宛先**＋原因＝`ON_ENERGY_TO_TRASH` は宛先 trash 限定）③`WX05-020-E1`（**1ターンの累積クラッシュ枚数**をシグニ単位で数える counter が要る）。⚠**3件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**🟢 投入前に必ず「解決時に engine がその値を掴めるか」「本体も原文どおりか」「同名 STUB/収集関数が二重に無いか」の3点を確かめる**（直近4件とも timing 以外に穴があった）。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜306 の全件が実機UI未検証で積み上がっている**。
### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き306（タスク16＝`WXDi-P11-063-E2` シグニの下に置かれたとき／上にあるシグニ・5効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1361→1358**（`BASELINE_HIGH=1358`＝対象が `owner:any/count:1` の任意選択に潰れていた5効果の**機能是正**。⚠**是正5効果に対し減少は3**＝高シグナル総数は重複除外）、golden **1190→1195**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251枚／署名グループ 98 据置**。live per-effect 差分 **changed 5 / added 0 / removed 0**（`WXDi-P11-063-E2`／`WXK08-086-E1`／`WXDi-P03-057-E2`／`WXDi-P05-050-E2`／`WXDi-P05-060-E1`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 18→17 効果/17クラスタ**（⚠**実質15**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 4→3**。**新語彙1本**＝`TargetFilter.aboveSelf`（「このカードの上にあるシグニ」＝`acceHost` の兄弟。省略時は従来挙動）。**新 timing・新 action 型・新 collector はゼロ**。engine 変更は ①`execPowerModify` の1体確定＋直接適用 ②`calcFieldPowers` の「スタック下カード→ホスト」ループ新設 ③**`execStubPart1` の同名 STUB を `value == null` に限定**（part2 の `ON_PLACED_UNDER_SIGNI` 発火が到達不能だった死にコードの復活）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1195**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1358**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 `WXDi-P11-063-E2` 後に実測）。
>
