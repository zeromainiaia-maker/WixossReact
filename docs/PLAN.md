...
**Opusタスク12＝未消化の在庫 6件（2026-08-10 続き404・408・417／2026-08-13 続き466・468 で登録）**。
...
- **✅ 続き424（§6.4 強制アタック機構の配線）が持ち込んだ未検証UI 3件＝2026-08-13 続き467 で決着**（Codex 起案→Claude 実機検証・**6シナリオが2回連続PASS**・既定 order 登録済み・**差し戻し0／是正0**）。engine 側は golden で固定していたが、**enforcement は `BattleScreen` のフェイズ進行ゲートにしかなく golden では原理的に踏めなかった**。
  ...
  - [ ] （併せて）**`WX12-010` の【出】**＝場に出したとき「対戦相手のすべてのシグニを好きなように配置し直す」モーダルが出て、**辞退もできる**こと。⚠2段目「移動したシグニをアップしてもよい」は**明示 defer**（出なくて正しい）。**⚠本バッチではスコープ外**＝`REARRANGE_SIGNI` は別機構なので混ぜなかった（§3-4）。
