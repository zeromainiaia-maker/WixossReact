...
- **🆕 セッション（2026-07-22・続き253・Opus 4.8＋Codex 分担・**[P1_COMPLETION_ROADMAP](./P1_COMPLETION_ROADMAP.md) **バッチ2「対象フィルタ合成」第1波＝トラッシュ→手札回収のフィルタ脱落30効果を採用**（census 1742→**1715**・golden 593→**596**）**（続き252 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **修正（Codex 実装）**＝`parserUtils.extractNounPhraseFilter` 新設（バッチ2の共通 filter extractor・今回はトラッシュ→手札1入口だけへカードゲート配線＝`TTH_FILTER_BATCH2_WAVE1_CARDS`）。群A PRESERVE 11＝外科パッチ（兄弟効果不変）／群B 複色OR・無色4／群E 名前包含4／群F 無色否定7／群C 複合対象の SEQUENCE 分割4。**engine バグ1件同時修正**＝effectEngine ローカル matchesFilter の `nonColorless` が `'無'`（データ表記）を除外せず＝execUtils と3値統一。見送り3（levelEqTrigger 不在・レベル合計制約・共通色参照）は golden 非採用固定＋部分採用を parser 後処理で抑止。
  - **検証（Claude）**＝per-effect 機械 diff＝**30効果ちょうど・巻き添え0**。held **187枚/73グループ＝ベースライン一致を実測**（続き252の stale 申告事故は再発せず＝§6 義務化が機能）。MANUAL 兄弟温存・SEQUENCE 構造・逆翻訳照合・golden 両方向とも確認。差し戻し0件。
  - **ゲート**＝全緑（golden **596**・smoke 10722全OK・fuzz 全0・census **1715**・lint 0 errors・同型★0・held 73）。詳細 BUGFIXES 続き253。
  - **次の一手（Opus/Codex）**＝**バッチ2第2波**＝`extractNounPhraseFilter` を次の入口へ横展開（候補在庫の大きい順＝①POWER_MODIFY 対象句 class36+color15・②TO_ENER 系・③TO_TRASH 系。毎回投入前に候補 JSON 全件実測で偽陽性分離してから10〜30件で切る）。副産物＝既存 held 在庫に WX07-003-E2（スペル回収なのに curated が cardType:シグニ＝fresh が正）等の採用待ちあり（Sonnetタスク6圏）。**Sonnet はタスク1（§7 実機検証）**＝トラッシュ回収 filter（複色OR/nonColorless/cardName）の実機スポットチェック＋続き252分（OR/FROM_DECK/PLACED_BY_EFFECT/IS_BETTING）継続（要実機検証）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 593・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1742。
>
