...
### 5c. 語彙センサス1469枚の系統別消化（2026-07-04新設・続き17で母集団拡大・過剰効果＝フィルタ/条件/制限脱落バグ）

**目標＝`npm run census` の高シグナル1469枚（25計測）を系統別バッチで0へ逓減。** 過剰効果（フィルタ・条件・使用制限の脱落で対象/発火が広がる・ゲームを壊す側）は behavior-audit の無変化キューに掛からない別種のバグ母集団（発見経緯は §4 続き15、拡充は続き17）。

- **優先順（続き17改訂）**＝(1) **条件節781**＝条件丸ごと脱落→無条件発火は実害最大＆最大母集団（`condition`/`activeCondition` 系語彙ゼロのカード。まず頻出形「場に○のシグニがある場合」「手札N枚の場合」等でサブ系統化）。(2) 除去系の対象フィルタ脱落（クラス339=`story`・色105・パワー閾値83・レベル閾値90・凍結13・ダウン/アップ38・数値不一致153）。(3) 制限/様相の脱落（ターン1回28・ゲーム1回3・任意→強制23）。(4) 保護/付与系（同一性46・共通色66・能力なし10）。(5) 語彙自体が無い系統＝最上級（6枚・`TargetFilter` に `superlative:{key,dir}` 新設）・**正面32**（`frontOfSelf` はあるが使用3件＝parser 未配線疑い）・動的比較の残36・合計制約27 は §3「機構実装の型」で新語彙＋engineセット実装。
