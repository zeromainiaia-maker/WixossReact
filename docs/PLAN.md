...
| 6 | 「代わりに」残テールの機構系 | engine新機構（置換） | L | **B1残5枚のうち4枚✅消化（続き256・Opus）**＝WXDi-P11-067（既存 TURN_HAND_DISCARD_GTE）・WX14-070（新 THIS_CARD_UPPED_FROM_DOWN_THIS_TURN＝execUp記録）・WDK17-014（新 COST_TRASHED_PUPPET）・WX25-P2-101（新 COST_DISCARDED_SIGNI_LEVEL）。別対象二重POWER_MODIFYの過剰効果を条件置換へ。census 1567→1563・golden 729。**残**＝WXK06-071（デッキ移動の持続turn-counter＋多段2重ゲート＝別実装）／D:置換ルール9／C:コスト代替6／E:リコレクト2（真の§6.3級）。tractable 分4枚＋WX16-021 は続き235b。詳細 BUGFIXES 続き256/235b |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 729・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1563（＝`BASELINE_HIGH`・回帰ゲート）。
>
