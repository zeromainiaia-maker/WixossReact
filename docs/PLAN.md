...
- **🆕 H. タスク12(xxii) から正式送りの不足機構3件（続き296・2026-07-28）**＝(xxii) 本体は完全クローズしたが、以下は `UNKNOWN{raw}` で honest defer 中（**過剰実行はしない＝現状は安全側**）。実装時は `raw` に原文全文が入っている。
  ...
  - **✅ I. `WX24-P3-069-E1` ガード追加コスト枚数化（2026-07-30）**＝collectorを合計枚数へ拡張し、STUB `count` 省略時1を維持。既存ルリグ付与ストアのCONTINUOUS走査、警告《無》×N、`energy.length < N`、確定N枚徴収を一体で配線した。新たに有効化される付与ストア効果は全数走査で同カードの1効果だけ。
    - **🆕 I' ガード追加《無》の枚数が parser で落ちている（2026-07-30 の確認で発見・回帰ではなく既存の過小徴収）**＝`WXDi-P01-035-E1`（コードハート　Ｐロマピ）の原文は「対戦相手は追加で**《無》《無》**を支払わないかぎり【ガード】ができない」だが、`parseSentencePart2.ts:2184` の `対戦相手は追加で.*を支払わないかぎり【ガード】ができない` が**枚数を捨てて** `GUARD_EXTRA_COST_BY_OPP`（count なし）を吐くため**1枚しか徴収していない**。I で collector が `count` を honor するようになったので、**parser 規則1本で《無》の連数を `count` に載せれば閉じる**。実データの該当は STUB 11効果（うち collector が走査する CONTINUOUS は6効果＝`WXDi-P01-035`／`P06-031-E2`／`P13-006`／`P14-050`／`P16-059`／`CP01-041`）で、**原文が複数《無》なのは `WXDi-P01-035` の1件のみ**（他5件は《無》×1＝省略時1枚で正しい）。⚠残る AUTO/ACTIVATED 5効果（`WX24-P2-047`／`WX25-P2-001`／`WX24-D1-05`／`WXDi-P05-005`／`WXDi-P13-046`）は collector が CONTINUOUS しか見ないため従来どおり `execStubPart1.ts:4541` のフラグ経路＝**枚数化の対象外**（別途診断が要る）。
