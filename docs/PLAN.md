...
2. **census「動的比較 35枚」**（「〜より高い/低い」＝heterogeneous・per-card）。**🔸続き43（Opus）で自己参照9枚＋トリガー参照6枚 着地（powerLt/Gt/levelLt/GtSelf・powerLt/levelLt/GtTrigger・engine 解決を resolveDynamicFilter に集約＝sourceCardNum/triggeringCardNum 引数・trash→field ビルダーにも parseTriggerComparison 配線・census 1631→1624）**。残＝(a)自己参照/トリガー参照でも parseSigniTarget/builder 非経由（STUB `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` 内＝WXK05-059/WXK10-062・SEND_TO_ENERGY bespoke＝WXK09-052/WXK11-041・nested CONDITIONAL 内 BANISH＝WXK10-040/WXK07-030）、(b)lastProcessed（その後/場に出たそれ＝⚠`applyToField` が配置シグニを lastProcessedCards 未設定＝engine 設定もセット）・printed（表記されているパワー＝per-candidate 実効/表記比較の新機構）・opp/own センタールリグ（WX19-042/WXK11-003）・「あなたのいずれかのシグニ」。詳細 BUGFIXES 最上部。
...
- **🆕 セッション（2026-07-08・続き47・Opus 4.8・§4タスク2 残＝動的比較 個別機構待ち `powerLtLastProcessed`/`levelLtLastProcessed` の2枚を実装）**＝続き43-46の動的比較サブファミリで「個別機構待ち」として保留していた **lastProcessed 基準の2枚**（WXDi-P08-031/WXK10-031）を消化。「（先行アクションで場に出た/公開した）**そのシグニより〔パワー/レベル〕の低い**対戦相手のシグニ1体を対象とし…」＝直前に処理したシグニ（`lastProcessedCards[0]`）基準の strict-less-than 比較。self/trigger と同語「そのシグニより」だが**「その後」＝同一効果内の先行アクションで生じたシグニ**を指す別機構。**型** `powerLtLastProcessed`/`levelLtLastProcessed` 新設（既存 Lte の strict 版・参照不能なら**空ヒット**＝対象なし）・**engine** `resolveDynamicFilter` に2ケース追加・**parser** `parseLastProcessedComparison`（「その後」マーカー＋「そのシグニより低い」）を `parseSigniTarget` に配線→**WXDi-P08-031 は AUTO で powerLtLastProcessed 獲得**（ADD_TO_FIELD 配置シグニは resumeSelectTarget/execPlaceSigniOnField が既に lastProcessed セット済み＝engine 追加不要だった）。**WXK10-031 は AUTOパース構造崩れ（BOUNCE→TRASH 誤訳・「公開カードをトラッシュ」欠落）のため MANUAL 化**（`OPTIONAL_COST《無》→DECK_REVEAL_UNTIL→BOUNCE{levelLtLastProcessed}`）＋engine 2点追補：**(a)** `DECK_REVEAL_UNTIL` に「公開したカードをトラッシュに置く」（`toTrashAllRU`・未対応だと deck 除去後に行き場を失い**消失**する実バグ）、**(b)** `resumeOptionalCost`/`resumeChoose` の continuation が `...ctx` で **lastProcessedCards を捨てていた**のを継承へ修正。**採用2枚**（WXDi=AUTO 自動収穫・WXK10=MANUAL）。**検証**＝typecheck・**golden 164→169（+5＝strict境界・参照不能空ヒット・WXK10 機構統合・実効果を任意コスト pay 経由で駆動する回帰ガード）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1623/1623**（両カードとも過剰効果は是正済みだが別カテゴリ〔WXDi-P08-031＝原文DRAW脱落／WXK10-031＝BURST語系〕で dedup 総数は不変）。**次の一手＝(A)Opus＝§4タスク2 残（動的比較）**：lrig相対（WX19-042 は「センタールリグ」に釣られ target が LRIG に誤パース＝target mis-parse の修正が先・WX09-037 は MANUAL級誤パース＝clean でない）＋残る高シグナル22（`docs/_vocab_census.txt` 13行目・大半は SEARCH デッキ相対〔WDK13-013/WXEX2-28/WXEX2-37/WXK10-033〕や条件文〔WXK07-025/WXK08-005〕で別機構）。**動的比較の「対象とし→除去」系は lastProcessed 含めほぼ消化完了・残は lrig相対/デッキ相対の個別機構。**別系統として **WXDi-P08-031 の「カードを3枚引き」DRAW 脱落**（原文DRAW→JSON無・census line475 の系統的 compound-sentence パーサ欠落・数百枚）が未対応（本タスク外）。**(B)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）の GRANT_QUOTED_AUTO_ABILITY 内側parse。(C)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43の6枚＋続き44の2枚＋lastProcessed の WXK10-031 任意コスト実機確認）。(D)verifyBattleDrive バッチ状態汚染の根本修正は続き39から持ち越し。**
- **（続き46）§4タスク2 残＝動的比較 別基準 `powerLtPrinted`/`powerGtPrinted`**＝続き43-45（自己参照/トリガー参照/いずれかのシグニ）に続く**per-candidate 表記比較サブファミリ**を消化。「表記されているパワーよりパワーの低い/高いシグニ1体を対象とし…」＝**各候補の実効パワーと自身の表記パワーの比較**（低い＝低下中／高い＝増強中）が脱落し全シグニ対象になっていた過剰効果。DB 全5枚のうち **3枚（WX24-P4-054/WXDi-P08-072/WXK09-050）は bespoke STUB で実装済み**（`SIGNI_GRANT_CHOSEN_ABILITY` 等・census 免除）で据置し、**plain な2枚のみ汎用フィルタ化**＝WX25-CP1-093（低い・POWER_MODIFY）/WXK10-027（高い・GRANT_KEYWORD）。**型** `powerLtPrinted`/`powerGtPrinted` 新設・**parser** `parsePrintedComparison` を `parseSigniTarget`＋GRANT_KEYWORD ビルダー（`kwSigniFilter`）の2経路に配線・**engine** `fieldCandidates` の per-candidate ループに判定追加（`実効(effectivePowers ?? 表記)` vs `表記`・resolveDynamicFilter ではなく候補フィルタ層＝各候補が自身の表記と比較されるため・非数値パワーは対象外）・decompiler/census keys 追加。**採用2枚**（純改善・collateral 0）。**検証**＝typecheck・**golden 163→164（+1）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1623/1623**（2枚は動的比較カテゴリからは外れたが別カテゴリ〔トリガー:アタックしたとき／パワー閾値／数値不一致〕の未対応で dedup 総数は不変＝過剰効果自体は是正済み）。**次の一手＝(A)Opus＝§4タスク2 残（動的比較）**：lastProcessed（WXK10-031 は STUB `DECK_REVEAL_UNTIL` 実装が前提・WXDi-P08-031 は ADD_TO_FIELD が配置シグニを lastProcessedCards 未設定＝engine 設定もセット＝2枚とも重い）＋lrig相対（WX19-042 は「センタールリグ」に釣られ target が LRIG に誤パース＝target mis-parse の修正が先・WX09-037 は MANUAL級誤パース＝clean でない）＋残る高シグナル22（`docs/_vocab_census.txt` 13行目・大半は SEARCH デッキ相対〔WDK13-013/WXEX2-28/WXEX2-37/WXK10-033〕や条件文〔WXK07-025/WXK08-005〕で別機構）。**動的比較の「対象とし→除去」系はほぼ消化完了・残は個別機構待ち。****(B)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）の GRANT_QUOTED_AUTO_ABILITY 内側parse。(C)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・トリガー参照 続き43の6枚＋続き44の2枚の実機確認）。(D)verifyBattleDrive バッチ状態汚染の根本修正は続き39から持ち越し。**
