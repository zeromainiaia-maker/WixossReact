...
- **🆕 セッション（2026-08-10・続き403・Opus 5）＝§6.4「トラッシュ起動のコストUI」を消化**（対象14枚の共通穴。エナコスト以外が全部未対応で、該当【起】はトラッシュUIに**そもそも出ていなかった**）。
  ...
  - **⭐③ 対応したコスト**＝`discard`(+`discardFilter`)／`handDiscardSigni`（手札選択UI）・`coin`・`removeOppVirus`・`charmTrash`・`lrigDown`・`exceed`（自動 or 選択）。**《アタックフェイズアイコン》起動**＝`getMyTrashCardActions` を `MAIN` 固定から `MAIN`/`ATTACK_ARTS` の2窓へ（`ATTACK_ARTS_OP`＝相手ターンは自分のトラッシュ起動の窓ではないので開けない）。
  - **🔴④ アタックフェイズ窓を開けた副作用を先に塞いだ**＝`WX11-049-E2` の使用条件「対戦相手の場に【チャーム】が３枚ある場合にしか使用できない」は **`COND_STUB`＝常に true** だった。**それまで能力ごと到達不能だったので無害だったが、窓を開けた瞬間に「無条件で使える過剰実行」に変わる**。使用条件 **`CHARM_COUNT` を新設**（type／`evalCondition`／parser 規則／decompiler）し、`heldReview --adopt WX11-049` で採用（**変更は当該1枚のみ**を機械 diff で確認）。
  - **🔧⑤ ついでに直った潜在バグ**＝`handDiscardSigni` の合致判定を `costs.ts` の `matchesHandDiscardSigni` へ共有化。`LrigGrantedModal` の写経版は **`level` 指定を見ていなかった**（`WXK03-047` ほか6効果＝レベル指定を無視して任意のシグニを捨てられた）。
  - **🔧⑥ golden のカーソル依存を1件修正**＝`task12(lxiii)` の `victim = fresh()` は **対象フィルタ「パワー5000以下」を満たすとは限らず**、前段のテストが1枚多く払い出すとバニッシュ側の assert だけが落ちた（今回の JSON 変更で顕在化）。フィルタを満たすまで払い出す形へ。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（typecheck / golden **1705 PASS**（+7）/ smoke / fuzz / census 880＝ベースライン維持 / lint 0 error）。`npm run regen` 済み（`WX11-049-E2` の逆翻訳が `[条件STUB:...]` → 「対戦相手の場の【チャーム】が3枚以上の場合」へ）。
  - **⚠⑧ 実機検証が要る（golden では原理的に踏めない）**＝対象14枚の**トラッシュゾーンUIからの実発動**。特に (a)`WXDi-P04-042`（レベル2のルリグ2体ダウン＝センター→アシストの自動支払い）(b)`WX19-029`（アタックフェイズ＋エナ2＋＜遊具＞2枚捨て＝**唯一の複合コスト**、`asDown:true` でダウン状態配置）(c)`WXDi-P12-053`（《ディソナアイコン》フィルタ）(d)`WXDi-P16-082`（コイン2＋ON_COIN_PAID 連鎖）。
  - **📋⑨ 次の一手**＝(a)**`collectIncreaseActCost`（相手の起動能力コスト+1）がトラッシュ起動に未適用**＝現状シグニ【起】経路だけが見ている。(b)**トラッシュ起動の CPU 使用は未実装**（§6.4 CPU AI 欄）。(c)`WX17-044` は §6.4 待ちではなくなったが**別要因3つで依然ブロック**（§6.3 E 残3件の (a) に再判定を追記済）。(d)Opus 側の次点は §6.4 上部の **`cannotAttackSigni` 3箇所配線**／**付与ストアの任意 timing・任意 scope 走査**（`WXDi-P07-073-E1` が依然死んだまま）。
