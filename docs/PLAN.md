...
| 🆕(xlvi) | **parser は `REVEAL_AND_PICK`（手札に加える）を出すのに curated が古い `LOOK_AND_REORDER` のまま held ドリフトしカードアドバンテージが死んでいた系統**。✅続き218g で9効果を外科的採用（census 1886→1880）。✅**2026-07-29 に2波で17効果を消化**＝第1波5効果（`pk` 規則を `hasRiseIcon`／`isDisona`／`hasLifeBurst`／色OR へ拡張）＋第2波12効果（色/レベル/種別ごと・hand＋trash/energy の独立SEARCHを `LOOK_PICK_CHAIN` へ展開＝合計枚数への誤簡約を回避。engine 拡張は `remainder.shuffle?` の1フィールドのみ）。census 1476→**1469**・golden +31・held 268→257。**残＝27件中の未採用10件**＝`WXDi-P03-061`（加えた枚数ぶん手札を捨てる複合）・`SEQUENCE` 化する多段・`then:'deck_top'` が要る群3（未配線の宣言を避けて honest defer 中）ほか＝`docs/_held_review.txt` の LOOK 署名群が現物。詳細 BUGFIXES 続き218g／2026-07-29節（第1波・第2波） |
...
- **2026-07-28〜29 最新値（上記長文履歴の追補）**：**`BASELINE_HIGH=1469`（2026-07-29 現在の回帰ゲート値）**。1476 以降の推移＝**1476→1471**〔タスク12(xlvi) 第1波＝`LOOK_AND_REORDER` の消えていた単一 pick-to-hand 5効果を復元〕→**1471→1469**〔第2波＝多段/複数グループ pick 12効果を `LOOK_PICK_CHAIN` へ展開。⚠**減少2に対し実際に挙動が変わるのは12効果**＝census 側に対応パターンが無い分は増減しない〕。以下は 1479→1476 の経緯：タスク12(xliv)(a3) の実働化に伴い、WX09-022 の欠落していた《エナジェ》`activeCondition` を parser から復元して **1479→1478**、さらに Opus 検証で同文型の **WX12-033-E2**（PRESERVE カードのため build が curated を温存し fresh の条件が届いていなかった＝【ランサー】無条件付与）を外科パッチして **1478→1477**。(b3) `WXDi-P14-053-E1` の引用【常】を長期付与ストアへ実働化して **1477→1476**。いずれも計器だけの改善ではなく機能修正。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **922**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1469**（＝`BASELINE_HIGH`・回帰ゲート）・held **257枚/110署名**（2026-07-29 実測）。
>
