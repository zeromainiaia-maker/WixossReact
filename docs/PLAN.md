...
| 🆕(xxxiii) | **`collectTrashTriggers` の any_opp watcher パスが usageLimit 未評価（続き182発見・未修正）**。ON_BANISH の同パスでは評価済み＝(x)/(vi-5) 同族の残穴。該当箇所（`triggerCollect.ts` の any_opp ループ直後）にコメント済み。(xxxii) の対象外＝未検証で挙動を変えないため据置いた。修正は `limitOkWatcher` を足して watcher 側 usedIds へ積むだけ |
| 🆕(xxxiv) | **「コストか効果によって場からトラッシュに置かれたとき」の `fromFieldByCostOrEffect` が parser 未 emit（続き182発見・未修正）**。原文にこの句を持つカードは**15枚**あるが JSON で当該フラグを持つ効果は**2件のみ**＝バトル/ルール処理起因のトラッシュでも発火する過剰発火（engine 側のゲートは `collectTrashTriggers` に既存＝G204）。parser で句を検出して `triggerCondition.fromFieldByCostOrEffect` を付けるだけで塞がる見込み。⚠ally パス側にも同ゲートを足す必要あり（現状は path1 のみ評価） |
