...
**直近＝2026-09-19＝§5.7 `S-5d` 第1段＝対話の解決（`pending_effect` の resume）7本を画面から出した**（リファクタ・挙動不変／全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**ここが最初の理由**＝`createHeadlessBattle` は「対話が立ったら止まる＝**答えるのは呼び出し側の仕事**」と決めてあるのに、**その「答える」7本（566行）が画面にしか無かった**＝ヘッドレスではどちら側の対話にも答えられない。
- 移設＝`controller/effectInteraction.ts`（`makeEffectInteractionHandlers`）。**本体は逐語**（機械照合で確認）。🔑**7本を1つの factory にした**＝個別 export にすると材料を配り直すことになり逐語でなくなる。
- 画面のモジュール直下の2本（`finalizePendingSpellPlacement`／`nextRespondPatch`）と薄いラッパ4本も一緒に移した（**呼び出しがこの7本の中だけ**だった）。`BattleIo` に `flushLogs` を追加（`appendLogs` の `defer` と対・ヘッドレスは no-op）。
- **BattleScreen 7,644 → 7,028行。**
- ⚠`census:deadstate` の `hand` の読み回数が 1999→2016 に動くが**計器の性質**（`indexOf` の部分一致で `handleXxx` も数える）＝**dead キー一覧とラチェットは不変**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-5d` 第2段**＝CPU の自動応答を画面の `useEffect`（約70行）から出す → 第3段＝ヘッドレスの対戦ループ（CPU 同士）→ その後 `S-6` / `S-8`。🏁**§5.3 機構 worklist は残0** |
...
| ⚠**直近の不具合** | 🔴**移設の当たり判定は「呼び出しがその塊の中だけか」を typecheck の未使用検出で測る**＝勘で残すと画面と移設先に同じ式が二重に残る。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4326**） |
...
- **2026-09-19 時点**（第404バッチ＝§5.7 `S-5d` 第1段）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4326**＝+2・`census:traceinv` I1=0）
  - 📏**BattleScreen**＝**7,028行**（前回 7,644＝−616。2026-09-18 朝の 16,930 からは −9,902）
