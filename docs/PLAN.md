...
- **🆕 セッション（2026-08-13・続き460・Opus 5）＝§7 実機挙動（P3）＝続き459 の「使用禁止」を実UIで固定。**Codex に委譲してシナリオ起案→Claude が実機検証**（`docs/CODEX_GUIDE.md` の分担）。**新規シナリオ4本すべて2回連続PASS・既定 order 登録**、途中で**実バグ1件を検出して修正**。ゲート数値は全て据置（golden 1956・census 831・smoke 10688・lint 0 errors/259 warnings）**。
  - **✅① 続き459 の未検証UI 2件が決着**＝(a)`wxex166SpellLockPeriod`＝`WXEX1-66` のスペル封じが**①予約 `USE_SPELL:NEXT_TURN` →②CPUターン開始（DRAW）で bare 昇格→③CPUターン終了で消滅**の3段を実機で確認＝**恒久ロック（`PERMANENT`）への退化なし** (b)`prArtsSpellSplitIds`＝`PR-427` で `USE_ARTS:NEXT_TURN`／`USE_SPELL:NEXT_TURN` の**両方**が積まれ、死 actionId `ARTS_AND_SPELL` は0件。
  - **🔴② 実バグ検出＝手札スペルの「発動」ボタンだけ封じゲートが無かった**（`BattleScreen.tsx:7373`）。実行入口 `castSpell`（`:6769`）にはガードがあるので**ルール違反にはならない**が、封じ中でもボタンが出て**押しても無反応＝無言の no-op**。⚠**ルリグデッキのスペル/クラフト（`:7508`）とアーツ（`:7529`）は最初から同じゲートを持っており、手札スペルだけが抜けていた**＝同じ概念のゲートが入口ごとに散っている型。条件1つ追加で修正し、負方向シナリオが FAIL→PASS へ反転。
  - **⭐③ 検出できたのは「負方向テストに対照を必ず付けた」から**＝アーツの「使用」は `costOk`/`condOk` でも消えるので、**非表示は封じの証明にならない**。**盤面を1文字も変えず `blocked_actions` だけ空にした対照**（`spellArtsUnblockedUiShowsUseButtons`）を対で置いて初めて「消えたのは封じ由来」と確定できる。**今後の負方向シナリオはこの対をセットで作ること。**
  - **⚠④ 文字列は完全一致で見る**＝`includes('USE_SPELL')` は予約 `USE_SPELL:NEXT_TURN`（まだ効かない）にも当たり「効いている」と誤読する。engine の寿命は2スロット式（`effectExecutor.ts:3208` 付与→`turnScopedState.ts:154` 予約だけ越冬→`:168` 剥がして有効化）。
  - **🔴⑤ ドライバの罠2件（engine が壊れて見える FAIL を出す）**＝(a)**`field.check` だけは `injectScenario` がリセットしない**（`CORE_FIELD_KEYS` に含まれる）＝前シナリオの未確認ライフクラッシュのモーダルが全画面に残り**クリックが1つも通らない**（24反復空振り＝「アタックが発火しない」に見えた）⇒ spec に `'field.check': null` を明示する (b)**`ATTACK_ARTS_OP` は `NON_TURN_PLAYER_PHASES`**＝進行ボタン「アーツ終了」を持つのは**非ターンプレイヤー＝driver 側**で、押さないと**CPU のターンが永久に終わらない**。⚠切り分けは `scratchpad-verify/<id>-final.png` が決定的だった（**ログだけ見ると engine 側に誤診する**）。
  - **⚠⑥ Codex 委譲の実績**＝ネットワーク遮断で実行はできないが**シナリオ起案は成立する**（4本中3本が起案どおりに動いた）。しかも**指示書の誤り（私が「手札スペルのゲートは `:7508`」と書いた）を実コードを読んで訂正**してきた＝**投げる前の行番号は自分で開いて確かめること**。
  - **▶ 次の一手**＝**Opus 側**：(a)続き459 が可視化した**8つの `DEFERRED_*`＝26効果**（`DEFERRED_UNPARSED_THIS_TURN_OPP_CLAUSE` 7件が最大）(b)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵）(c)**O-4 UNKNOWN 25ノード/25カード**(d)**O-1 CPU AI 拡張**。⚠**他の「ログだけハンドラ」STUB の掃き出し**も継続候補。**Sonnet 側**：**§7 実機検証**＝O-13 と続き452〜458 の6件（続き459 分は本セッションで消化済み）。⚠**新規シナリオは `'field.check': null` を必ず入れる**（⑤a）。
