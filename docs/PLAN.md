...
| (lxiv) | **🆕 タスク12(lxiii) の消化で見つけた別機構 2件（2026-07-31）**＝①`WXK11-031-E1` の対象「パワー5000以下」フィルタと「それ」の照応が落ちる（原文「対戦相手のパワー5000以下のシグニ１体を対象とし、〈条件〉、手札を１枚捨ててもよい。そうした場合、それをバニッシュする」＝**対象宣言が条件・任意コストより前**にある形で、タスク12(liii) の正準形〔`SELECT_TARGET_ONLY`→`STORE_LAST_PROCESSED_TARGETS`→…→`targetsStored`〕が要る。現状はフィルタなしの相手シグニ1体＝**過剰実行**）②`WX18-032-E2`／`WXK11-031-E2`「対戦相手は手札が**N枚になるように**カードを捨てる」が `TRASH{HAND_CARD count:N}`＝**N枚捨てる**に化けている（手札枚数に応じて「N枚まで減らす」語彙が無い。`DRAW` の `untilHandCount` と同形の受け皿が要る。手札がN枚以下なら0枚・多いほど多く捨てる＝現状は過小/過剰の両方向）。詳細 BUGFIXES 2026-07-31 (lxiii) 節の「残」 |

> **✅残0でクローズした在庫の完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-07-30 整理」節へ退避**＝**(xxix)**〔任意【出】コスト・計15波・残4は明示保留〕／**(xlvi)**〔look-pick 計73効果・第1〜17波〕／**(lvii)**〔census キー表 `pickUpTo` 較正〕／**(lviii)**〔トラップ公開 LPC 移行9効果・残1は honest defer〕／**(lix)**〔`split_top_bottom` 全4効果〕／**(l)**〔置換イベント横取り3文型を含む A群/B群 全体・残0〕／**🆕(lx)**〔①`WX25-P1-056-E1` の非バニッシュ離場→バニッシュ置換を engine 9サイトへ配線／②`WX12-020-E3` の「この方法で捨てた枚数」比例＝新語彙 `deltaPerLastProcessedCount`。2026-07-31 に残0クローズ・詳細 BUGFIXES 2026-07-31 節〕／**🆕(lxii)**〔`CONDITIONAL_DISCARD` を型ごと退役し `WD16-016-BURST` を多段閾値の昇格置換へ。2026-07-31 に残0クローズ〕／**🆕(lxiii)**〔「対戦相手の〈ゾーン〉があなたより多い場合」＝両者比較ゲートの脱落4枚＋「中央のシグニゾーン」限定の脱落4枚。2026-07-31 に残0クローズ・残は (lxiv) へ〕。それ以前の消化済み在庫〔(i)〜(lvi) の大半〕は同ファイルの 2026-07-19・07-24・07-28・07-29 の各退避節。
...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxiii)「対戦相手の〈ゾーン〉があなたより多い場合」＝両者比較ゲートの脱落4枚＋「中央のシグニゾーン」限定の脱落4枚（残0クローズ）**（golden **1131→1132**・census **1373 据置**・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **288枚 据置**〔署名グループ 106→**104**〕・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 7 / added 0 / removed 0**／`build:effects` 冪等）
  - **⚠登録時の診断「fresh 側のドリフト（curated が正）」は誤りだった**。真因は `isBatch1OnlyClause` が「対戦相手のライフクロス」を含む条件節を **25枚のカード allowlist（`STATE_HOIST_BATCH1_CARDS`）に限定**していたことで、同型は全CSVで4枚あり **curated 側も大半が条件を落としていた**（`WX17-040` ①②・`WX18-032-E1`・`WXK11-031-E1` は無条件発火）。**held を解く前に「curated が正」という前提を疑うこと**が教訓。
  - **①両者比較ゲート（4枚8箇所）**＝新語彙2本 `HAND_COMPARE_OPP`／`ENERGY_COMPARE_OPP`（既存 `LIFE_COMPARE_OPP` と同形＝`cmp(自分, operator, 相手)`。閾値比較の `HAND_COUNT`/`ENERGY_COUNT` とは別物）。ゲート緩和は**「あなたより多い」形だけ**＝閾値形（「〜がN枚以下の場合」）は引き続き batch1 限定。表記ゆれ「ライフクロス**の枚数**が」も吸収。**選択肢の内側**に条件が来る2箇所（`WX17-040`③・`WX18-032-BURST`②）は `liftChoiceOptionCondition` の既存ガードどおり **choice.condition に上げず `CONDITIONAL` を内包**（原文は「対象とし」が条件より前＝解決時判定）。
  - **②`centerZoneOnly` の脱落（4枚）**＝`parseSigniTarget` が「中央のシグニゾーンにある〜シグニ」を読んでおらず**どのゾーンのシグニでも取れる過剰実行**だった（`WX15-033-E2`／`WX20-025-E3`／`WXDi-P02-065-E2`／`WX24-P2-091-E1`）。engine 側（`matchesStateFilter` の zoneIdx===1）は実装済みで parser が渡していなかっただけ。⚠「このシグニが中央のシグニゾーンにある**かぎり／場合**、…」＝効果元の位置条件を巻き込まないよう読点なしの名詞句修飾に限定。
  - **③engine の穴も塞いだ＝条件で包んだ前段が did-it ゲートから外れる**。`WXK11-031-E1` は包む前は `TRASH{HAND_CARD self}` が直下ステップで「対象なし→残りスキップ」に掛かっていたが、条件を付与した途端に外れて**条件不成立でも「そうした場合」のバニッシュが撃てる**新しい過剰実行が生まれるところだった。`execSequence` の3ゲート（TRASH／DOWN／`DID_IT_GATED_TYPES`）を **else なし `CONDITIONAL` の `then` まで見る**ように変更（`gateStep`）。
  - **次の一手**＝**Opus：①🆕 タスク12(lxiv) の2件**（`WXK11-031-E1` の対象「パワー5000以下」フィルタと「それ」の照応＝対象宣言が条件・任意コストより前にある形／`WX18-032-E2`・`WXK11-031-E2`「手札が**N枚になるように**捨てる」が `TRASH{count:N}`＝N枚捨てるに化けている）**②「コストの合計」族の STUB/UNKNOWN 14件③`WX11-043-E1` の `crossOnly` 欠落④タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑤`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑥§6.4 の配置数制限すり抜け7効果⑦§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝タスク12(lx)/(lxii)/(lxiii) の未検証UI／(lxi) 第1・第2波の UI 6件／§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxiii) 残0クローズ後 最新値（本行を直近の正とする）**：census **1373 据置**（`BASELINE_HIGH=1373`）、golden **1131→1132**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288枚 据置／署名グループ 106→104**、manual field loss 0。live per-effect 差分 **changed 7 / added 0 / removed 0**（`WX15-033`／`WX17-040`／`WX18-032`／`WX20-025`／`WXDi-P02-065`／`WX24-P2-091`／`WXK11-031`）、`build:effects` 冪等。**新語彙2本**＝`HAND_COMPARE_OPP`／`ENERGY_COMPARE_OPP`。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1132**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1373**（回帰ゲート）・held **288枚/104署名**（2026-07-31 タスク12(lxiii) クローズ後に実測）。
>
