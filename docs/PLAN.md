...
- 🏁**セッション（2026-08-28・続き697・Opus 5 単独）＝`O-126`。`cost_modifiers` は書かれるだけで誰も読まない死にストアだった。**
  ユーザー指示＝**Sheet1 の機構待ちを解決する**。
  📊**進捗3計器＝Sheet1 要対応 29→28 / 863（3.4%→3.2%）｜台帳 残 OPEN 579→578｜census 高シグナル 522（据置＝parser 不変）**。
  gates 全緑（golden **2912→2915**）。実機＝新規2本 `b19costup` / `b19costupnone` PASS（**反転確認済み**）＋回帰2本 PASS。
  - 🔑**登録票は「除外フラグが無い」と言っていたが、課税自体が1度も起きていなかった**＝
    `execCostIncrease` が積む `PlayerState.cost_modifiers` の**読み手がどこにも無い**（実測 `WX09-Re05` / `WXK11-003` の2枚が完全な no-op）。
    ⇒ 収集を **`calcActiveCostMods` の1本**に寄せた（人間UI と CPU で `costModsForMy` を組む場所が2つあるため）。
  - 🔴**配線した瞬間に効き始める寿命の穴も同時に塞いだ**＝ターン終了クリアが BattleScreen 4経路の手書きで
    **ターンプレイヤー側にしか当たっていなかった**。相手に課す修正は**相手の state** に載るので1ターン余分に残る。
    ⇒ 手書きを削除し `clearTurnEndScopedState`（両プレイヤーに当たる唯一の境界）へ `advanceCostModifiers` を1本。
  - ⚠**「カットインされたスペルは対象外」は死にフラグを作らずに満たした**＝コスト請求も使用封じも**宣言時**に済み、
    `pending_spell` が立つのはその**後**（遡って効く経路が無い）。**golden のトリップワイヤ**で前提が崩れたら落ちる。
  - ⚠**過去の BUGFIXES に「読まれていない」と書かれた機構は、迂回実装されただけで残っていることがある**
    （`NEXT_OPP_TURN` だけ別フィールドで迂回し、本体は死んだままだった）。

**▶ 次の一手**＝**`O-127`（「好きな枚数…し、**その枚数と同じ枚数**の…」＝直前の選択枚数への後方参照）**。
`WX05-010-E1`（Sheet1・HIGH）。「好きな枚数」自体の受け皿（`count:'ALL'`＋`upToCount`）は既存で、
**無いのは「直前に選んだ枚数を後段が読む」紐付け**。⚠**「好きな枚数」82枚のうち後方参照する形が何枚かは着手時に数え直す**。
**Sheet1 の残り機構待ちは 6カード**＝`WX02-020`(`O-129`) / `WX05-010`(`O-127`) / `WX05-016`(`O-117`) /
`WX05-020`(`O-113`) / `WX05-022`(`O-125`) / `WX09-Re07`(`O-128`)。
⚠**`O-129`（118カード影響）と `O-128`（34カードの catch-all）は最後に回す。**
⚠**既存 golden が旧バグを固定している可能性を毎回疑う**（`O-120`／`O-122` と2巡連続で反転が要った＝§5-17）。
⚠🆕**登録票は4回連続で実態と違った**（`O-116` 消化済み／`O-108`＝`O-115` 重複／`O-110` は既に遅延形／
`O-126` は**読み手ごと死んでいた**）＝**着手前に live と実コードを必ず1分見る。**
特に「フラグが無い」型の登録票は、**そのフラグを読む側が生きているか**から見る。
