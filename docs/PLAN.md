...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15` → `O-10`（明示 defer **残 4 id/4件**。続き507〜512 で6巡済み＝残りはすべて1件ずつ実機構が要る〔`ON_TARGETED` の収集地点／ピースのカットイン窓／グロウ履歴 state＋A-B面フリップ／トラッシュ内の効果免疫〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 4 id / 4件 | 🆕**2026-08-16 続き512（6巡目）＝5種/5件 → 4種/4件**（`WXK07-001-E1` の引用【自】＝数字宣言＋3制限）。🔑**3制限のうち2つは受け皿が既にあった**＝(b)「宣言された数字と同じレベルのシグニで【ガード】ができず」は`DECLARE_NUMBER` → `declared_guard_restrict_level(s)` で、`GuardResponseDialog` が既に読んでいる／(c)「一度しかアーツを使用できない」は `BLOCK_ACTION{ARTS_LIMIT_1}`。🔴**ただし `ARTS_LIMIT_1` は parser が生成するのに engine/UI の誰も読んでいなかった**＝`WX13-007-E1`「対戦相手は各ターンに一度しかアーツを使用できない」も**恒久 no-op**だった（memory「未消費の `BLOCK_ACTION` id」クラス）。`isArtsUseBlocked()` に集約して**表示ゲートと実行ゲートの両方**へ配線。(a)「無色のカードでエナコストを支払えず」だけが新機構＝`cannot_pay_colorless_this_attack_phase` ＋ 支払い可否 funnel（`canAffordGrowCost`/`canAffordWithExtraCost` の `banColorlessPay`）。⚠**無色カードはマルチエナ扱いでも落とす**（ワイルドとして通すと制限が骨抜き）。⚠**境界は `attack-phase-start` ではなく `turn-end`**＝制限は**相手側 state** に載るのに `clearAttackPhaseScopedState` はターンプレイヤー自身にしか適用されない。🔴**隣接発見＝`declared_guard_restrict_level(s)` も turn-end の一部経路にしか手書きクリアが無かった**（宣言側と読み手が別プレイヤーなので残りうる）＝レジストリへ登録し、消費は `consumeDeclaredGuardRestrictLevel`（funnel）へ寄せた。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き512）。⚠**全経路 実機未検証**＝§7 送り。**■残 4 種 / 4 件（すべて1件ずつ実機構が要る）**＝`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`〔**3部品**＝①「このターンにセンタールリグがグロウしていない」を数える state が**存在しない**②チェックゾーンの A/B 面フリップ ③特定カードへのコスト無しグロウ〕／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔ピース使用へのカットイン窓〕／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔`ON_TARGETED` が **`resumeSelectTarget` の適用後**に積まれるので「対象の無効化」の余地が無い＝収集地点を適用前へ動かすのが前提（続き511 で特定）〕／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`〔トラッシュ内カードの能力喪失＋効果免疫＝相手トラッシュに触る全効果に及ぶ〕。**定期的に前提を再判定する**（続き507〜512 の計16種のうち、**「機構が無い」が本当だったのは4種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付け・自分で書いた据置理由の陳腐化・**未消費 id**だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
