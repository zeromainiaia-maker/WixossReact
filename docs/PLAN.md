...
**直近＝2026-09-19＝§5.7 `S-5d` 第2段＝CPU の対話応答の「振り分け」を画面から出した**（リファクタ・挙動不変／全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**残っていたもの**＝`cpuInteraction.ts`（**何と**答えるか）は純関数なのに、**「どのハンドラへ渡すか」は画面の `useEffect`（70行）**にあった＝第1段で「答える7本」を移したあとも**ヘッドレスは CPU の対話に答えられなかった**。
- 移設＝`battle/cpuInteractionRespond.ts`（`decideCpuInteractionResponse`）。🔑**待ち時間（`setTimeout`／`CPU_ACTION_DELAY`）は画面に残す**＝純関数へ持ち込むと**自己対戦が実時間に縛られる**。`[CPU] 選択:` のログは戻り値で返し、書くのは画面（I/O の口を増やさない）。
- ⚠**決めるタイミングを「遅延の前」に揃えた**＝旧実装は型によって遅延の前／後で乱数の消費時点が違った。**`cpuDriver` は `pending_effect` の間は走らない**（`cpuDriver.ts:20`）ので**待っている間に盤面も乱数も動かない**＝同値と確かめてから寄せた。
- golden＝新設2本（型ごとの宛先／空きゾーンが無ければ答えない＝**反転確認済み**／画面が `pickCpu*` を直接呼ばない）＋既存3本を `cpuRespondSource()` へ較正（**配線の本数は不変**）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-5d` 第3段**＝ヘッドレスの対戦ループ（人間側ターンの駆動とガード／ライフバーストの応答＝CPU 同士を画面なしで回す）→ その後 `S-6` / `S-8`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**画面のリファクタ＝live JSON を1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**`goldenTest.ts` は CRLF**＝python で書き換えると改行が壊れ、**`git diff` では気づけない**（`--numstat` は正規化して見せる）。バックスラッシュを含む置換は `Edit` ツールで行う（[LESSONS.md](./LESSONS.md) §4.3）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4328**） |
...
| 🔥`S-5` | **C 対戦丸ごとのシミュレータ**（§5.7.3 の案C・段階的）＝🏁`S-5a` メモリ上の persist＋`resolveStackNext` の切り出し（`controller/memoryPersist.ts`・`controller/stackResolve.ts`）→ 🏁`S-5b` 盤面差分トリガーの収集（`controller/boardDiffTriggers.ts`）→ 🏁`S-5c` 実行関数 `perform*` 12本・ルール処理4本・`cpuTurnAction`（2026-09-18 済＝`controller/`）→ 🔥**`S-5d` 相手側の応答と人間側ターンの駆動**（🏁第1段＝対話の解決7本を移設＝`controller/effectInteraction.ts`／🏁第2段＝CPU の対話応答の振り分けを純関数へ＝`battle/cpuInteractionRespond.ts`／🔥第3段＝ヘッドレスの対戦ループ＝人間側ターンの駆動とガード／ライフバーストの応答）。各段で `gates`＋実機の回帰 | 3〜5週間 | 画面なしで CPU 同士の対戦を回す＝勝率で強さを測る。ターン進行のバグ探しにも使える | 🆕🔑**ユーザー指示（2026-09-18）＝相手のシグニが「バニッシュされたとき」の効果を持つ場合も判断に入れる**（＝バニッシュが相手の得になる盤面では殴らない／別の除去を選ぶ）。母集団と取り方は [PLAN_DETAIL.md](./PLAN_DETAIL.md) の `S-5` 要件メモ。
...
- **2026-09-19 時点**（第405バッチ＝§5.7 `S-5d` 第2段）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**画面のリファクタ＝live JSON を触っていないので3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4328**＝+2・`census:traceinv` I1=0）
  - 📏**BattleScreen**＝**6,987行**（前回 7,028＝−41。2026-09-18 朝の 16,930 からは −9,943）
