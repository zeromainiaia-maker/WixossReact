...
- **🆕 セッション（2026-07-10・続き60続き・Sonnet 5・§4タスク(F) §7実機検証の継続＝ON_REFRESH（R45②）追加）**＝`verifyBattleDrive.mjs` に新シナリオ `refreshTrigger`（WXDi-P04-043→WX15-073）を追加し実機検証・**✅PASS（3回連続）**。host デッキを**残り1枚**（trash1枚）にし、WX15-073（【出】E1バニッシュ候補0件で即done・E2ドローがデッキ最後の1枚を引いてちょうど0枚化）を召喚してリフレッシュを1回だけ発生させ、watcher WXDi-P04-043（幻竜姫 ドラゴンメイド・ON_REFRESH・refreshedOwner:'any'）の発火を確認＝ログ「幻竜姫　ドラゴンメイドの【自】効果（リフレッシュ時）」。機構的にR31/oppDrawと同型（対話なしDRAW/no-op経由のリフレッシュは`resolveStackNext`の`done`ブランチ内で`applyRefreshOnDone`→中央diffが完結し、resume経路取りこぼしとは無関係）。**⚠重要な罠を発見・回避**＝デッキを最初から0枚にすると、E1（バニッシュ0件の即done）の時点で既にリフレッシュ条件が成立し1回目が発火→E2解決後の2回目リフレッシュで「同ターン中2回目のリフレッシュは強制終了」ルール（`BattleScreen.tsx:3511`）が発動しターンが即終了、watcher収集の機会を失う（初回試行で「ターンが強制終了されました」ログを確認）。デッキを残り1枚にして1回きりのリフレッシュにする設計に変更して解消＝**リフレッシュ関連の実機シナリオは「デッキを空にする」のではなく「ちょうど1回だけ0枚化させる」よう組むこと**を教訓化。`order`配列に追加済み。詳細 VERIFY_BROWSER.md/BUGFIXES.md「ON_REFRESH（R45②）」節・PLAN §7。
  - **前々回＝手札捨て/トラッシュ flatten（R36）追加・実バグ発見＝❌FAIL**＝新シナリオ `handDiscard`（WDA-F02-17→WXK10-065）で続き58のresume経路取りこぼし理論の新規インスタンスを確認（2回連続再現・未修正・Opus引き継ぎ）。WXK10-065【出】「あなたは手札を1枚捨てる」（TRASH HAND_CARD self count1・SELECT_TARGET要）で手札に残ったWDA-F02-17自身を選ばせると、ground truth（hHand 2→0・hTrash 0→1）は正しいが自己参照ON_TRASH watcher（fromZones:hand）が一度も発火しない＝`selectOrInteract`（`execUtils.ts:1206`）が候補1件でも必ずSELECT_TARGETを要求する仕様のためresume経路（`handleEffectInteraction`）で完結し`collectAnyZoneTrashSelfTriggers`（中央diffのみ配線）が呼ばれない＝R43/R46/R39と同型。系統的懸念に`collectDeckTrashSelfTriggers`も追加（未検証）。`order`からは除外。詳細 VERIFY_BROWSER.md/BUGFIXES.md「手札捨て/トラッシュ flatten（R36）」節。
  - **前回＝opp-draw（R40）追加＝✅PASS（2回連続）**＝新シナリオ `oppDraw`（WXDi-P15-091→WX12-047）。CPU自動アタック（`wd07012`と同型）でguest自身の効果ドローを発生させhostのON_DRAW any_opp watcherが反応することを確認。機構的にR31(drawBySourceStory)と同型。詳細 VERIFY_BROWSER.md「opp-draw（R40）」節。
  - **検証（全体）**＝各シナリオ単体で複数回連続実行し安定再現・typecheck緑（engine/parser不変のためgates再実行不要・script/docsのみの変更）。
  - **次の一手＝(A)Opus＝逆翻訳忠実性の systematic課題＝durational付与の先頭位置「ターン終了時まで」でaction内durationが落ち decompileの期間注記が脱落する約102枚**（§5b decompileテール・engine非依存・一括置換禁止＝系統確立→各カードverifyの型で。まず該当母集団を機械抽出）。**(B)Opus＝effect-restriction（配置数制限）**（WXK11-074等・§6 BLOCK機構・実機検証必須）。**(C)Opus＝§7実機検証で続き58/60が発見した resume経路取りこぼし（oppPowerDecreased/energyToTrash/outsideDrawPhase/handDiscard＝4件FAIL）の根本修正**（`result.done`に関わらず両経路から共通で呼べる収集関数への統合リファクタ）。**(D)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(E)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(F)Sonnet＝§7実機検証の継続**（R30/R44/R45①残・WXDi-CP02-082（R36②）・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up項目。⚠R42はR39型の可能性ありOpus修正待ちが望ましい）。**(G)持ち越し：SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
...
- ~~**🆕 resume経路inline collector欠落＝機構原因を特定（2026-07-09・続き58・§7 R43/R46実機検証で発見・R41/R31で対照確認・未修正）**~~ **✅続き61（Opus）で根本修正＝盤面差分トリガー収集を `collectBoardDiffTriggers` に統合（両経路から共通呼び出し）。4シナリオ（R46/R43/R39/R36）実機FAIL→全PASS・回帰なし・全ゲート緑。詳細 BUGFIXES 最上部。以下は当時の原因分析（記録保存）。**＝ON_SIGNI_FROZEN（R38）と同型のバグを**2件連続で確認**＝(a)**ON_OPP_POWER_DECREASED**（WX13-036/WXEX2-52対象・`collectPowerDecreaseTriggers`＝`triggerCollect.ts:900`）：WD11-013（相手シグニ1体-1000・mandatory）をSELECT_TARGET resumeで解決するとwatcher無発火。再現＝`node scripts/verifyBattleDrive.mjs oppPowerDecreased`。(b)**ON_ENERGY_TO_TRASH**（WD15-015/同様カード対象・`collectEnergyToTrashTriggers`＝`triggerCollect.ts:808`）：WD15-014（相手エナ1枚トラッシュ・mandatory）をSELECT_TARGET resumeで解決するとwatcher無発火。再現＝`node scripts/verifyBattleDrive.mjs energyToTrash`。**両シナリオとも既定orderから除外済み**。
  ...
  - **修正方針**＝`collectFreezeInline`と同型の`collectPowerDecreaseInline`／`collectEnergyToTrashInline`／`collectDrawInline`等を追加する場当たり的対応は、SEQUENCE構造次第で同じcollectorが再度FAILしうる（R39が実例）ため**対症療法止まり**。根本修正としては、**`result.done`に関わらず両経路（`resolveStackNext`のdoneブランチ／`handleEffectInteraction`のresume完了時）から共通で呼べる収集関数に統合するリファクタ**が本筋（個別inline追加より優先度を上げるべき・Opus判断）。
...
- **手札捨て/トラッシュ flatten（R36）**：WDA-F02-17-E3／WXDi-CP02-082（自ターンE1／相手ターンE2の出し分け）。**①実機FAIL＝実バグ発見（2026-07-09・続き60・Sonnet・未修正・Opus引き継ぎ）**＝`verifyBattleDrive.mjs handDiscard`（WXK10-065の【出】手札1枚捨てでWDA-F02-17自身を選ばせる）でON_TRASH(self,fromZones:hand)がFAIL＝R43/R46/R39と同型のresume経路取りこぼしの新規インスタンス（詳細 §6.3・BUGFIXES続き60）。②WXDi-CP02-082は未検証のまま。
