...
| 7 | §6.1 未実装action型の engine 実装 | engine実装 | M（1型ずつ） | 残2型＝`PREVENT_DAMAGE`(5・ダメージ層の置換機構が要る)・`COST_SUBSTITUTE`(2・コスト支払いUI横断)。~~`PLAY_FREE_FROM_TRASH`(2)~~ **✅続き202（Fable 5）で実装＝`execPlayFreeFromTrash` 新設（トラッシュ/ルリグトラッシュ→コスト合計閾値＋filterで絞り SEARCH→STUB `USE_SPELL_FROM_TRASH` で主効果実行）。派生＝二重積みガード・parser 色フィルタ脱落・decompiler 誤表現も是正。詳細 BUGFIXES 続き202** |
