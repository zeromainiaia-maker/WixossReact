...
- **🆕 セッション（2026-07-16・続き158・Opus 4.8・PLAN §3 Opusタスク12(xxii)／続き157「次の一手」(a)＝LAST_PROCESSED reveal 経路の条件節 parser 未 emit を是正）**
  - **8枚是正・census 2101→2098・IS_MY_TURN化 128→119**。engine の条件評価器は対応済みなのに parser がその条件を emit していなかった3種の穴を系統的に塞いだ（PARTIAL 刻印 IS_MY_TURN化クラスタの残）：①**level parity**（レベルが偶数/奇数のシグニ＝`matchesFilter.levelParity` 対応済み）を `parseLastProcessedMatchesCondition`／`parseBareBranchCondition`／`parseThisWayGenericCount` の3関数に追加（WDK04-011/WXDi-P10-073/WXK01-106/WXK01-107）②**bare story**「＜X＞の場合」（「のシグニ」無しの種別問わ story 一致）を `parseLastProcessedMatchesCondition` に追加（WX25-CP1-054/WXDi-CP02-063＝デッキ公開＜ブルアカ＞）③**`LAST_PROCESSED_POWER_GTE`**（それのパワーがN以上＝engine 実装済み・parser 未 emit）を block 抽出に追加＝直前 POWER_MODIFY の delta を addDelta に渡す（WXK11-065・WX03-046 は既に MANUAL 済み）。
  - **採用**＝値変更で held 落ち→`heldReview --adopt` 6枚＋WXK11-065 は PARTIAL 温存のため JSON 直接パッチ。**据置**＝WXK07-106（BET UNKNOWN）・WDK16-13/WXK08-033（登録者数 AND 複合枝＝(xxii) 既知）・WX14-072（対象外）。
  - **検証**：golden 356・smoke/fuzz 全0・census 2101→2098（BASELINE 更新）・同型★0 維持。詳細 BUGFIXES 続き158。
  - **次の一手**：Opus＝IS_MY_TURN化残119 のうち engine 対応済み条件の parser emit をさらに（盤面状態「あなたの場にレゾナがある場合」WD09/11/12-018・トラッシュ枚数「N枚以上ある場合」WXDi-D05-012/P02-089・手札枚数 WDK08-Y08 等＝`HAS_CARD_IN_FIELD`/`TRASH_COUNT` 系は既存語彙で拾える可能性大）。ほか (xxvii) 残 Cluster C/A/F。Sonnet＝Opus在庫消化待ちまたは補欠。
