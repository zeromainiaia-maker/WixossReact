...
- **🆕 セッション（2026-08-15・続き498・Opus 5）＝§6.4 **O-3 クローズ**＝「次のターンの間」系統の受け皿**7種すべてを解体**（**9効果の挙動是正**）。ゲート全緑（**golden 2057**＝+7・**census 830 据置**＝新語彙のキー較正込み・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・UNKNOWN 25/25 据置）。live changed **9効果/9カード**。`census:stubs` A群 **22種/24件 → 15種/17件**（無言 no-op は 0 のまま）。新機構5＝`RETURN_FACEDOWN_LRIG_ZONE_TO_HAND`／`FIELD_SIGNI_TO_CHECK_ZONE`／`GAIN_LRIG_TYPE`（＋`lrig_gained_types_timed`・`effectiveLrigClass`）／`DECLARE_CARD_NAME_LOCK`（＋`cardNameUseBlocked`）／`SigniAttackBan.exceptCardNums`。
  - **🔑① 遅延を跨いだ照応は「参照先が state に永続化されている形」なら解ける**＝`WXDi-P09-066-E1` の「そのカード」＝`facedown_lrig_zone_cards` を発火時に読む（`RETURN_FACEDOWN_LRIG_ZONE_TO_HAND`）。置く側も 🔴`STUB{SOUL_OP}`＝該当分岐なしの**丸ごと no-op** だった。隣接の 🔴`SPDi43-02-E2` は「対戦相手は手札をすべて裏向きに置く」が**自分の手札を1枚トラッシュ**（`TARGET_AND_DISCARD_HAND`）＋返却が `RULE_REMINDER_TEXT`＝**取り上げたまま戻らない**の二重バグだった。
  - **🔑② 受け皿 STUB を実装で置き換えると `DEFERRED_` 前提の gate が外れる**＝`WX22-010-E3` の「その後、それらを場に出し」を落とす `dropDanglingDeckTopPlacement` は `DEFERRED_` があるときだけ働く。広げないと `ADD_TO_FIELD{source なし}`＝**デッキの一番上を場に出す**過剰実行になる。往復（`FIELD_SIGNI_TO_CHECK_ZONE`）は**1アクションに畳む**＝経由地の照応を運ぶ器を作らない。
  - **🔑③ 置換は被置換アクションのペイロードへ畳む**＝`WDK07-Y07-E1`「代わりにそのシグニを手札に戻してから開花する」は後続ステップでは間に合わない（素の開花が先に「シグニあり＝不可」で終わる）。`SEED_BLOOM.bounceOccupant` に載せ、選択の向こう側（`INTERNAL_BLOOM_SEED`）まで運ぶ。
  - **🔴④ 原文 regex で母集団を数え直して隣接バグを発見**＝「ルリグタイプを追加で得る」は3効果あり、`WDK17-001-E2` は【起】なのに **CONTINUOUS 専用**の `STUB{INHERIT_OPP_LRIG_TYPE}`（`collectLrigNameAliases` は CONTINUOUS しか走査しない）に落ちて**恒久 no-op**だった。`GAIN_LRIG_TYPE{turns:'GAME'}` へ。得たタイプの実利は**グロウ互換**と**「〇〇限定」**の2軸なので `effectiveLrigClass` の1本に集約した。
  - **🔴⑤ カード名の使用封じは主要経路を素通りしていた**＝`blocked_card_names`／`blocked_card_names_game` は手札スペルとルリグデッキのタップ経路でしか読まれず、**アーツ一覧（`artsCandidates`）と実行入口（`executeArts`）は判定していなかった**（既存 `BLOCK_CARD_USE`／`NAME_BAN` 全体に波及）。判定を `cardNameUseBlocked` に集約。あわせて `blocked_card_names` の失効が **turn-end でターンプレイヤー側だけ**＝相手に課した分が1ターン長く残っていたのを turn-scoped レジストリ登録で是正した。
  - **⚠⑥ `heldReview --adopt` はカード単位＝巻き添えを必ず diff で確認する**＝`WXEX2-09` を採用したら同居の `WXEX2-09-E1`（実装済み STUB `RISE_LEAVE_DISCARD_STACK`）が `UNKNOWN` へ退化していた（curated 値へ外科的に復元）。⚠**コスト無しの相手応答 CHOOSE は `opponentResponds` だけだと `resumeOpponentPayOptional` で「エナ不足」＝無言に潰れる**＝`costlessOpponentChoice`（`leaveSubstituteAsk` の一般版）を新設。
  - **▶ 次の一手【Opus 側】**＝**O-3 は完了**。次は 🆕`O-32`（`REPEAT_N_TIMES` の二重実行3効果。⚠**本文の parse を先に直す**＝いま組み替えると過少実行に化ける）→ 🆕`O-33`（アタック禁止の**ゾーン限定**と**次の相手ターン**軸＝`SigniAttackBan` に `zones?`／`turnsRemaining?` の2キー。⚠`matchedSigniAttackBans` の署名変更が全呼び出し元に及ぶ）→ `O-28` の残23綴り → `O-4`（UNKNOWN 25ノード/25カード）→ `O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-10`（明示 defer 15 id/17件の前提再判定）。⚠`census:goldentypes` に**未カバー2型**（`RESERVE_DRAW_PHASE_REPLACEMENT`／`SET_LRIG_BASE_LIMIT`＝続き492 で新設・当時から未カバー）＝PLAN の「未カバー0」は stale。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き498 の9経路**＝(a)`WXDi-P09-066-E1`（手札1枚がルリグゾーンへ消え、**次の相手ターン終了時**に戻る）(b)`SPDi43-02-E2`（相手の手札が**全部**消え、そのターンの終了時に**全部**戻る＝自分の手札は減らない）(c)`WX22-010-E3`（追加アタックフェイズ開始時に＜遊具＞がアップし直し、**もう一度アタックできる**）(d)`WDK07-Y07-E1`（シードのゾーンにシグニが居ても、そのシグニが手札に戻って開花する）(e)`WDK17-008-E1`①／(f)`WDK17-001-E2`（相手のルリグタイプを得て、**そのタイプ限定のアーツ／グロウ先**が使えるようになる）(g)`WXDi-P08-030-E1`（相手が選ばなかったシグニがアタックできない）(h)`PR-K046-E1`（**次の**相手ターンだけ宣言スペルが撃てない＝課したその場では撃てる）(i)`WXEX2-09-E3`（相手は宣言したアーツ以外を**一覧からも**使えない）。⚠**すべて実機未検証**。
