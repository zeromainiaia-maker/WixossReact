...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が指示と検証＋codex-work が実装・2バッチ連投）＝Opusタスク12 (l) を残0でクローズ（置換イベント横取り3文型＝ガードコスト置換／効果離場置換／ホログラフ公開置換）**（golden **1090→1096**〔codex 6本＋Claude 2本〕・census **1386 据置**・smoke **10679** 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held **286枚/106署名 据置**・manual field loss 0。ゲート全緑／live per-effect 差分は第1波 **changed 3**・第2波 **changed 3** でいずれも申告と一致・`build:effects` 冪等）
  - **投入前実測で「機構が無いから honest defer」という前提を崩したのが両バッチの決め手**＝(l) 残4 は「機構（置換フック／能力喪失）が入るまで defer」と簿記されていたが、実測すると**3群のうち2群は既存機構が実在**していた。①`WX24-P4-026-E1` のガード代替は `game_guard_alt_hand`＋`handleGuardWithHandAlternative`＋`GuardResponseDialog` が**UI まで完成済み**で、rawText は生成 regex（`execStubPart1.ts:3069`）に**そのまま一致**していた（残差は「rawText が生成地点に届いていない」「持続が違う」の2点だけ）。②離場置換は `banishPrevent`（場に残す＋`abilities_removed`）と `findEffectLeavePowerReductionSubstitute`（相手効果離場の横取り）が**論理構造ごと先例**として在った。③第2波では codex が挙げた defer 理由3点のうち「上3枚の順序選択後にトップ公開の pause/resume が未実装」が**誤り**で、`LOOK_AND_REORDER` は既に対話アクション（`execLookAndReorder`→`needsInteraction`→`resumeLookAndReorder`・実機 `BattleScreen.tsx:4453`）だった＝**新 interaction 型ゼロで着地**。
  - **新設した機構は2本だけ**＝(1)**効果離場の共通置換フック** `applyEffectLeaveLrigAbilitySubstitute`（`effectExecutor.ts:179`）を **BANISH/BOUNCE/TRASH/SEND_TO_ENERGY/TRANSFER_TO_DECK/EXILE の6経路＋direct再開点**へ配線し、`SPDi44-08-E2`／`WX25-P1-018-E2` の「クロス状態シグニが相手効果で場を離れる場合、代わりにこのルリグはこの能力を失う」を1回きりの盾として実装。(2)**ホログラフのデータ側マーカー** `CardEffect.holograph`（parser がコスト表記から立て**付与能力へ伝播**）＋公開2地点（`REVEAL_DECK_TOP`／公開形 `LOOK_AND_REORDER{count:1,private:false}`）の限定横取り。持続は `*_until_opp_turn` ファミリ（A/C群）と既定の `lrig_granted_auto_effects`（B群・クリア3経路）へそれぞれ正しく載せた。
  - **⚠Claude 検証で是正2件（どちらも「ゲートが検査しない次元」）**：①**A群の色条件が「2枚の割り当て」を色の独立カウントで書いていた**＝`AND[白 eq1, 赤青緑黒 eq1]` は多色シグニ（白黒等・実データに**30枚**）が1枚で色フィルタ2本を満たすため、**白黒1枚だけの pick で付与が成立（過剰）／白黒+赤では他色2枚で不成立（過小）**の両方向に外れる。既存語彙のみで `AND[白 gte1, 赤青緑黒 gte1, 白赤青緑黒 eq2]` へ置換し原文と完全一致させた。②**ホログラフ識別が共有関数内のカード固有ハードコード**＝`executeEffect` に `ctx.sourceEffectId === 'WX15-002-sub-E1' || 'WXEX2-15-E2'` を直書き（CODEX_GUIDE §5-18 違反）。しかも `WX15-002-sub-E1` は**parser が生成する採番**＝採番変更で**計器に映らないまま機構が静かに死ぬ**。データ側マーカーへ移行し、**死フラグ検査 golden**（live で該当2カードだけに立つ／非ホログラフ兄弟には立たない／全カード走査）を追加。⚠**live JSON は PRESERVE/held で据え置かれ parser 回帰を映さない**と分かったため、伝播規則は **fresh parse でも別途固定**した（両是正とも変異試験で狙った1本だけ FAIL することを確認）。
  - **codex の傾向（更新）**＝2波とも数値申告（ゲート・per-effect diff・held・エンコーディング）は独立実測と一致。**BUGFIXES 先頭 `##` 追記が第2波でようやく定着**（末尾追記は3回連続の再発だった）。変異試験も第2波では**指示せずとも自発実施**。**外すのは一貫して「原文の意味を素朴なカウントへ落とす」ときと「データに置くべきものをコードへ置く」とき**＝どちらも緑のゲートを素通りする。
  - **次の一手**＝**Opus：①エクシードルリグ6件の本体不一致**（`WX24-P4-011-E2` 次回アタック時アップ→即時UP／`WX24-P4-014-E2` 差分ドロー→固定1枚／`WX24-P4-015-E2` 任意ライフクラッシュ＋動的対象数→強制＋固定1体／`WX24-P4-017-E2` スペル＋青シグニ各1枚→青シグニのみ／`WX24-P4-018-E2` 相手の手札3枚支払い回避→無条件バニッシュ／`WX25-P3-028-E2` リフレッシュ不可の誤分類＋3回の選択脱落。詳細 BUGFIXES 2026-07-29 §「live 原文照合」節）**②🆕 `WX25-P1-056-E1`**（`leaveReplaceBanish` の no-op＝今回の共通離場フックで閉じられる見込み。任意選択とバニッシュ先処理の追加が要る）**③「コストの合計」族の STUB/UNKNOWN 14件**（本体が無い＝実装すれば既存 filter がそのまま効く）**④`WX11-043-E1` の `crossOnly` 欠落**（原文「《クロスアイコン》…の右【出】」なのにクロス外でも【出】発火）**／(xxix) 残4・(lv) 残2経路・ゲート脱落 [B]20件・タスク16 残・§6.3 残機構・`WX12-020-E3`（捨て枚数比例）**。**Sonnet：§7 実機検証**＝🆕**ガード代替ボタン（手札1枚捨て）の表示と確定**／🆕**ホログラフ置換の3枚並べ替えUI→トップ公開**／`SigniActivatedModal` の「エナ1枚以上」複数選択と捨て枚数に応じた `PLAY_FREE` 候補絞り込み＋`DECLARE_PARITY_OPPONENT` の相手応答UI＋キー付与【起】のエクシード支払いUI＋分割UI＋宣言UI＋トラップ設置UI＋(xxix) の各支払いUI＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(l) 残0クローズ後 最新値（本行を直近の正とする）**：census **1386**（`BASELINE_HIGH=1386`・別置）、golden **1090→1096**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**228 warnings**、同型★0（5986枚・265群）、held **286枚/106署名 据置**、manual field loss 0。live per-effect 差分は第1波 **changed 3 / added 0 / removed 0**（`WX24-P4-026-E1`・`SPDi44-08-E2`・`WX25-P1-018-E2`）、第2波 **changed 3 / added 0 / removed 0**（`WX16-004-E1` 本体＋`WX15-002-E2`・`WXEX2-15-E2` の `holograph` マーカーのみ）、`build:effects` 冪等。新語彙は `CardEffect.holograph`（データ側ホログラフ判定）・`LookAndReorderAction.revealTopAfterReorder`・`StubAction.leaveVictimFilter`・`GrantLrigAbilityAction.duration` の4本。
