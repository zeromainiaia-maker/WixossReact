...
- **🆕 セッション（2026-07-11・続き64・Sonnet 5・§4タスク(C) §7実機検証の継続＝R42/R30/R45①の3件）**＝`verifyBattleDrive.mjs` に3シナリオ追加。**①R42（ON_CHARM_TO_TRASH・WX16-Re05）＝✅PASS**（続き61のcollectBoardDiffTriggers統合がcollectCharmToTrashTriggersも一緒にカバー済みだった＝追加修正不要。banish resume経路でチャームがtrashへ→watcherが対戦相手シグニに-4000。副次的な学び＝「バニッシュ＝トラッシュ行き」は誤解、`banishDestination`のデフォルトは持ち主の**エナゾーン**行き。`order`追加済み・単体3回連続PASS）。**②R30（ON_PLAY any_opp・WXK10-022-E1）＝実機検証不能と判明**＝「あなたのターンに対戦相手のシグニが場に出る」を起こせる唯一のカードWXEX2-50（大幻蟲エンマコロギ）が`ADD_TO_FIELD.owner`誤パース（自分/対戦相手を取り違え）でブロック中＝parser修正待ち（Opusへ登録・§3タスク7）。**③R45①（ON_ACCE_ATTACH host条件・WXK05-041）＝❌FAIL＝実バグ発見**＝`execAttachAcce`のfromHand分岐（thenActionに未完結アクションを渡す設計）がresume解決側`applyDirectAction`の前提（1候補選択→即terminal実行）と噛み合わず、手札からACCEカードを選んだ時点で無添付のまま完了＝【デコレ】キーワード全9枚が機能しない（Opusへ登録・§3タスク7）。副次的発見＝WXK04-003の【起】能力2つがコストラベル表示バグで同文言になり区別不能。②③とも`order`未追加（Opus修正待ち）。guest側盤面注入の稀な競合も発見し安定化（queryState確認→再PATCHリトライ）。詳細BUGFIXES最上部3件。⚠フルスイート回帰は環境側のプロセス過多（検証run由来の残留chrome/node）で著しく低速化したため中断（各シナリオは単体で複数回再現確認済み）。
