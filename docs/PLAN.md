...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer **残 10 id/12件**。続き507・508 で2巡済み＝次は `DEFERRED_RED_CENTER_LRIG_REPLACE_ABILITIES_UNTIL_END_OF_TURN` が最も着手しやすい〔`lrig_abilities_disabled` と `ON_GUARD{lrigAttackGuarded}` は実装済み〕。⚠`O-33` の据置は続き508 で解体済み）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 10 id / 12件 | 🆕**2026-08-16 続き508（2巡目）＝13種/17件 → 10種/12件**（実装で **5件/3種** 解体）。🔑**3件とも「新機構が要らなかった」型**＝(a)`WXDi-P16-062-E1`〔原文 regex（「同じシグニゾーンに【ゲート】」）で数え直すと同族は**20効果**あり、**18効果は既に「引用を平らにして `condition:SAME_ZONE_HAS_GATE` を持つ AUTO/CONTINUOUS」で実装済み**＝**1件だけ取り残されていた**。支払い回避は O-31 の正準形 `SEQUENCE[OPPONENT_PAY_OPTIONAL, CONDITIONAL{IS_MY_TURN}→本体]` をそのまま使う。⚠「**各**アタックフェイズ開始時」＝`triggerScope:'any'`〕(b)`WXDi-P00-026-E1`〔**parser（AUTO）は既に正しい `GRANT_EFFECT{LRIG}` を作れていた**＝live の `MANUAL` 刻印だけが古い defer を固定していた（§6.4 O-6 の第3の死角）。AUTO 採用〕(c)`DEFERRED_CONVERT_ENERGY_COLOR`（3効果）〔旧理由「色照合が `costs.ts` に5箇所散っていて 1点 funnel が無い」は**古かった**＝エナの**追加色 funnel**（`canAffordGrowCost` の `extraColorMap`／`myEnergyExtraColors`）が既にあり `FIELD_ENERGY_SIGNI_GAIN_COLOR`／`ALL_ZONE_BLACK` が同経路で動いている。新設は収集器 `collectConvertEnergyColors`（**エナゾーンだけ**を走査）1本。🔴**隣接発見＝funnel の渡し忘れが5サイト**＝`GrowModal`(2)／`AssistGrowModal`(2)／`AssistActivatedModal`／`EnergyActivatedModal` は `extraColorMap` を `undefined` で呼んでおり、既存の `FIELD_ENERGY_SIGNI_GAIN_COLOR`／`ALL_ZONE_BLACK` も**グロウ／アシスト／アクセ発動の支払いでは効いていなかった**＝全サイトへ配線した〕。🆕**ついでに O-33 の据置 `WDK09-001-E2` も解体**（下の O-33 行を参照）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き508）。⚠**全経路 実機未検証**＝§7 送り。**■残 10 種 / 12 件**＝`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY`(3)〔続き507 新設。対話つき置換の policy 差し替えと対〕／`DEFERRED_ATTACK_NEGATE_IMMUNITY_SELF`〔`WX24-P4-016-E3`「このターン、あなたの効果によってシグニのアタックは無効にならない」＝**自分が課した無効化を解除する利得側**なので現状は過少（無害）。要るのはターン限定フラグ＋無効化サイトの照会〕／`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`／`DEFERRED_DECLARE_NUMBER_AND_ATTACK_PHASE_RESTRICTIONS`／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔`WX25-CP1-060-E2`。裏向き→表向きの往復そのものは `facedownSigni.ts` で書けるが、**観測できる効果は「新しいオブジェクトになって相手の対象指定が外れる」**＝*対象の無効化*という別機構が要る〕／`DEFERRED_OPP_TURN_ARTS_COST_REDUCTION_ONCE`／`DEFERRED_RED_CENTER_LRIG_REPLACE_ABILITIES_UNTIL_END_OF_TURN`〔`lrig_abilities_disabled` と `ON_GUARD{lrigAttackGuarded}` は既にある＝**次に着手しやすい**〕／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`／`DEFERRED_TRASH_RED_SIGNI_LEVEL1_PLAY_AND_END_TURN_RETURN`。**定期的に前提を再判定する**（続き507＋508 の計8種のうち、**「機構が無い」が本当だったのは2種だけ**＝残りは原文の誤読・取り残し・理由の陳腐化だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
