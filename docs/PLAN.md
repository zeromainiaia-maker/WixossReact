...
- **🆕 セッション（2026-08-15・続き496・Opus 5）＝§6.4 **O-31「対戦相手が〈コスト〉を支払わないかぎり、X」の残り＝4効果の挙動是正**（すべて**回避クローズが落ちた過剰実行**＝払っても帰結が起きる）。ゲート全緑（**golden 2044**＝+2・**census 831→830**・smoke 10688 / SKIP 0・lint 0/260）。live changed **4効果/3カード**。
  - **🔑① 在庫は「効果単位」で数え直す**＝続き495 のカード単位スキャンは**同じカードの別効果が支払い語彙を持つだけで PAY と出る偽陰性**があった。`docs/_effect_srctext.json`（効果ごとの原文）で走査＝**35効果/34カード**、うち支払い語彙なし8効果を1件ずつ live と突き合わせ、🔴実バグ4／🟢偽陽性2（`UPKEEP_OR_NO_UP` が支払いUIまで持っている）／⏸別層2（引用中身未パース・明示 defer）に仕分けた。
  - **🔑② 「付与そのものを止める支払い」は標準ペアで表せる**（`WXDi-P05-023-E1/E2`）＝除外④（帰結が「制限そのもの」の形は汎用ゲートで包まない）に**「〜を得る」＝能力の付与は例外**を追加。🔴**純加算チェックが「分割後のほうが正しい」ケースを弾いていた**＝回避クローズを挟んだ全文では対象解析が `filter:{cardType:'シグニ'}` を落とすため不一致になり据置されていた。**情報が増える向きだけ**許すよう緩和（波及は実測でこの1カードのみ）。
  - **🔴③ 「以下をN回行う。「…」」の引用括りで回避が消えていた**（`WXDi-P07-007-E3`）＝`isInsideQuote`（除外⑥）に当たっていた。**文全体が1つの引用のときだけ**括りを外す `stripFullSentenceQuote` を両ゲートの入口へ。あわせて「手札をN枚捨てるか《無》…を支払わ」＝**手札とエナの2枝を両方載せる**語彙と、帰結「デッキの上からN枚トラッシュ」を追加。⚠**この効果は回避以外の軸がまだ壊れている**＝`STUB{REPEAT_N_TIMES}` がカード全文 regex で3回ぶん自分で実行し後続ステップも走るため**実測16枚**落ちる（原文は最大12枚）→ 新設 **O-32**。
  - **🔴④ 【常】の正面アタック禁止が「払っても通らない」だった**（`WXDi-P16-047-E1`）＝`STUB{BLOCK_FRONT_SIGNI_ATTACK}` を `cannotAttackSigni`（無条件禁止）へ入れており「対戦相手は《無》を支払わないかぎり」が丸ごと落ちていた（live 母集団1効果＝その1件が支払い形）。`ContinuousBlockResult.cannotAttackSigniUnlessPayColorless` を新設して**別集合**へ振り分け（同じ集合だと「払えば通る」が「絶対に通らない」に化ける）。支払い額は**parse 時に STUB の `value` へ焼き込む**（🔴支払い句は CONTINUOUS の条件剥がしループで消えるので、剥がす前の**能力ブロック本文**から復元）。
  - **🔑⑤ 判定と引き落としは1関数へ**＝新設 `signiAttackColorlessCost`（ban 由来＋【常】由来の合算）を `signiAttackBlockReason`／`performSigniAttack` の引き落とし／アタックボタンの表示の**3箇所すべて**が見る。軸ごとに足すと「判定は通るのに引き落としが0」＝タダでアタックできる穴になる（続き494 でルリグ側の同じ穴を踏んだ）。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残8（`npm run census:stubs` の A群が常時の在庫表・機構ごとに独立着手可）＝`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）／`NEXT_OPP_TURN_END_BODY`（2効果）。→ 🆕**O-32**（`REPEAT_N_TIMES` 3効果＝カード全文 regex で自己実行し後続ステップと二重に効く。**本文が parse し切れていない2件が `REPEAT{count,action}` 化の前提**）／🆕**O-33**（`WX25-CP1-050-E1`＝`SigniAttackBan` に**ゾーン限定**と**次の相手ターン**の軸を足す）→ **O-28 の残23綴り** → **O-4**（UNKNOWN）→ **O-25 の残** → **O-26** → **O-27** → **O-29**。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き496 の3経路**＝(a)`WXDi-P05-023-E2`（相手に「《無》×3を支払う／支払わない」が出て、**払えば付与されない／払わなければアタック不可になる**＝負方向＋対照の対）(b)`WXDi-P07-007-E3`（**手札1枚捨て／《無》×1／支払わない**の3択が出るか）(c)`WXDi-P16-047-E1`（正面のシグニがエナ1以上なら**《無》×1 を前払いしてアタックできる**／エナ0ならアタックボタンが出ない＝対照の対）。ほか続き488〜495 の未検証分。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き496（§6.4 O-31「対戦相手が払う」側の回避クローズ）後 最新値（本行が直近の正）**：census **830**（831→830＝回避クローズの復元で高シグナル欠落 −1。`BASELINE_HIGH` も更新）、**golden 2044**（+2＝【常】の正面アタック禁止が支払いで解除できる挙動1・live データ不変条件1。ほかに `(ci)` の母集団カウントを `OPPONENT_PAY_OPTIONAL` 73→**76**／costColors 付き 38→**40**／非搭載 35→**36** へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 45群**（据置）、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0・明示 defer 23種/26件**（据置）。🆕**live JSON changed 4効果/3カード**（`WXDi-P05-023`(2効果)／`WXDi-P07-007`／`WXDi-P16-047`。CSV 非改変）。🆕**挙動是正 4**（すべて回避クローズが落ちた過剰実行＝払っても帰結が起きる）。🆕**新機構＝`ContinuousBlockResult.cannotAttackSigniUnlessPayColorless`（【常】の「払えば通る」アタック禁止を無条件禁止と別集合に持つ）／`signiAttackColorlessCost`（ban 由来＋【常】由来を合算する判定・引き落とし共通の1関数）／`stripFullSentenceQuote`（文全体が引用の形の括り外し）**。⚠**3経路とも実機未検証**（§7 送り）。⚠**同じカードに残る別軸**＝`WXDi-P07-007-E3` は `REPEAT_N_TIMES` が二重に効いて**実測16枚**落ちる（原文は最大12枚）＝O-32。
