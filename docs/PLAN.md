...
- **🆕 セッション（2026-08-11・続き424〜427・Opus 5）＝§6.4 を4本消化し、**A群「無言の no-op」を残0にしてゲート化**。golden 1771→1776・census 854 据置**。
  - **🔴① 続き424＝CONTINUOUS の `FORCE_SIGNI_ATTACK` は engine のどこからも読まれていなかった**＝強制アタックには軸が2本（ターン限定フラグ／印字・付与の【常】）あり、**CONTINUOUS は `executeAction` を通らない**のでフラグは永久に立たない。受け皿は**全部あった**のに印字【常】6カードが恒久 no-op。`resolveForcedSigniAttack` に2軸を一本化。
  - **🔴② 続き424＝`manualEffects.ts` に効果を「足す」と live に永久に届かない（第4の死角）**＝richness ガードが「effectId 集合が変わるカードは丸ごと温存」なので *新規 id の追加*だけが黙って捨てられていた（手書き効果3件が長期欠落）。
  - **🔴③ 続き425＝「対戦相手は〈コスト〉てもよい」で主語も極性も反転**＝(a) part1 の規則が先頭アンカー無しで主語を見ず、3効果が自傷 (b) `OPPONENT_PAY_OPTIONAL` の極性が「払わなかったら then」の1種類しかなく、「そうした場合」形の2効果が**意味が真逆**だった（`thenOnPay` を新設）。**極性は次の文にしか現れない**＝文単位の規則では原理的に判定できない。
  - **🔴④ 続き426＝任意コスト脱落トリップワイヤの🔴側を 7→0**。残っていた3件は**入口の形がそれぞれ違うだけ**（二段の任意／トップが CONDITIONAL＋複合コスト／CHOOSE 選択肢の内側）。`WXDi-P14-002` は**手札が2枚無くてもライフをクラッシュできた**。
  - **🏁⑤ 続き427＝§6.4 A群「無言の no-op」7件→0件**。**1件を実装／6件を明示 defer（`DEFERRED_*`）へ**。実装は `ASSIST_LRIG_ATTACK_THIS_TURN`（`WX25-P1-048`＝アシストルリグのアタック経路）＝状態 `assist_lrig_attack_min_level`／判定は `assistLrigAttackableSlots` の1本に**人間UI・CPU・フェイズ進行の3経路**／実行は `performLrigAttack(slot)` を一般化。🔴**副産物**＝ダメージ解決のダブル/トリプルクラッシュ判定が「攻撃側＝センタールリグ」固定だった（攻撃元を `pending_lrig_attack_num`→`lrig_attacked_by_num` で運ぶ）。
  - **⭐⑥ `npm run census:stubs` をゲートに昇格**（`npm run gates` に追加）＝A群🔴側が0になったので**増えたら止める**計器に変えた。ダミー id を live に仕込んで**実際に exit 1 することを確認**してから入れた。⚠止めるのは🔴側だけ＝`DEFERRED_*` は理由つきの保留なので数えない。
  - **⭐⑦ 「着手しない理由が確定している」なら id にそう書く**＝6件は理由が確定済みなのに素の id だったため、毎回 🔴 worklist を汚していた。`COUNTER_TEAM_PIECE_CUTIN_DEFERRED` に至っては `_DEFERRED` が**接尾辞**で計器から見えていなかった。**計器が読む形で保留を宣言する**のが正しい。
  - **⭐⑧ A群は「半分だけ実装済み」が多い（7件とも）**＝`WX25-P1-048` もガード応答・ダメージ解決・per-slot のダウン管理がすべて既存で、要ったのは「枠を選べるようにする」ことだけだった。**後段の機構が既にあるかを先に確かめる。**
  - **✅⑨ ゲート**＝`npm run gates` 全緑（typecheck / golden **1776（+5）** / smoke 全0・SKIP 0 / fuzz 全0 / census **854 据置** / **census-stubs（新規ゲート）** / manual-fields / lint 0 errors）。`EFFECT_TYPE_MISSING_CONTINUOUS` **1→0**、`FORCED_HAND_COST_KNOWN` の🔴側 **7→0**、`census:stubs` A群🔴側 **7→0**。**要実機検証12件**（強制アタック3／相手側 CHOOSE 3／新規 pay-skip 3／アシストアタック3）は §7 に登録済み。
  - **📋⑩ 次の一手**＝**Opus 側**＝(a)🆕**エナ支払い元の一本化**（`UNDER_CARD_AS_ENERGY_COST` の前提。支払いモーダル17本が `my.energy` を直読み）(b)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(e)**残 A群17件の手札コスト変換**。**Sonnet 側**＝§7 実機検証（**続き424〜427 を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き424〜427（§6.4 を4本消化＝強制アタック機構／手書き効果追加の死角／主語・極性の反転／トリップワイヤ残0／**A群🔴側 残0＋ゲート化**）後 最新値（本行が直近の正）**：census **854**、golden **1776（+5）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 17件＝明示 defer 17／無言 no-op 0**（ゲート化済み）、smoke 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
- ~~**`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査＋`verifyEffects` の誤検出除外**~~ **✅2026-08-10 消化（続き408）**＝**33件を1件ずつ仕分けたら真バグ3件／誤検出30件**だった。誤検出6系統をルールで潰し、**`POWER_VALUE_MISMATCH` 6→0／`MILL_COUNT_MISMATCH` 3→0／`OPTIONAL_SUSPICIOUS` 2→0／`EFFECT_TYPE_MISSING_CONTINUOUS` 15→1（残1＝真バグ `WX12-010`）／`verifyEffects` のタイミング 4→0**。**計器の精度改善が本体**（続き407 の教訓「過剰報告する計器は無いより悪い」の実践）。詳細は BUGFIXES 2026-08-10（続き408）。
  ...
  - **📋 残 1件**＝`WXDi-CP01-023`（**置換**「代わりに…してもよい」＋ CONTINUOUS で受け皿なし＝§6.4「置換5本の対話化」とセット）。
  ...
  - **（消化前の内訳・記録）続き416 時点の16件**
    ...
    - (c)**置換**＝`WXDi-CP01-023`（「代わりに…してもよい」＋ CONTINUOUS で受け皿なし）＝§6.4「置換5本の対話化」とセット。
...
- **🆕 エナ支払い元の一本化（`UNDER_CARD_AS_ENERGY_COST` の前提・続き427 で登録）**＝支払いモーダル**17本**が `my.energy` を直接読み、候補生成（`my.energy.map(...)`）も控除（選択 index を `my.energy` から除去）も各所に散っている。「エナゾーン以外を支払い元にする」語彙（`WXDi-P10-041`＝シグニの下のカード。ほかにも増えうる）を載せるには、**候補生成と控除を1本の関数に寄せる**のが先。⚠**半分だけ配線してはいけない**＝「アーツには使えるが【起】には使えない」という無言の不整合になる（`census:stubs` にも `census` にも出ない）。
