...
- 🏁**セッション（2026-08-24・続き638・Opus 5）＝ドキュメント整理2件＋段2 第33バッチを codex-work へ委譲して残0まで検証**。gates 全緑（**golden 2659**／census 608 据置／smoke 10693 全0／fuzz 全0／lint 269 warnings 増分0／同型★0）。台帳 **734→730**。
  - 📚**`CODEX_GUIDE.md` を 172KB→50KB へ再編**＝過去バッチ実績64件・起動法の変遷・ガードレール事故の全文を新設の **`CODEX_LOG.md`** へ分離。**節番号（§1〜§8）と `§5-nn` は据え置き**（BUGFIXES.md 等から `§5-5` だけで20箇所以上参照されているため）。壊れていた表（生パイプで7列に割れた行・4列の行・空行で6断片に分断された表）も修復。`CLAUDE.md` の docs 一覧にも両方を追加＝cold start から辿れるようにした。
  - ✅**段2 第33バッチ＝「残りをシャッフルしてデッキの一番下に置く」の配線漏れ**（codex-work 委譲・Claude は指示と検証）。**採用11効果**（`REVEAL_AND_PICK.remainder.shuffle` / `LOOK_AND_REORDER.shuffle` の既存受け皿へ配線）。差分は**全件 `shuffle:true` の追加のみ**で不変条件 11/11 合格・追加0・削除0。
  - 🔑**投入前の実測が2回効いた**＝①PLAN §5.2 の①「未 triage 662件を Codex で段1 triage【最優先】」は**既に完了**していた（実測＝未 triage **0**／段1 真バグ確定 725）②findings のサブ群8件は engine 実装状況で真っ二つで、**「好きな順番で」4件は `position:'any'` を書いても死フラグ**だった（`effectExecutor.ts:5935/6078/8844` が `'bottom'` 以外を一律デッキ上として扱う）。**findings のまま投げていたら半分が JSON だけ直る no-op になっていた。**
  - 🔴**Claude の母集団計器に偽陽性2件**＝**カード単位**でマッチさせたため、同じカードの別効果が「好きな順番で」でも拾った（`WXDi-D04-021-BURST`／`WXDi-CP02-007-E3`）。codex が効果単位で訂正し**兄弟効果の誤付着を防ぐ golden** まで追加。**先回りメモが覆されたのは通算12回目**（§5 `3-3⁹` の再発＝母集団は effectId 単位で数える）。
  - 🔴**engine バグ1件を Claude が修正**＝`logSourceText` の常時記録化に伴う `_currentParseSourceTextStack` の `push`/`pop` が `try/finally` 未保護で、**再入 parse の例外で別カードの Map へ書き込む**汚染経路があった（深さ復元へ修正）。
  - 🆕**計器 `heldReview.mjs` に `--adopt-partial-effect` が新設された**＝curated 兄弟を置換せず AUTO 効果だけを1件ずつ採用する経路。**以後 live が動いたら `--adopt` だけでなくこちらも疑う。**

**▶ 次の一手**＝**§5.2 の段2 消化を続ける**。最有力は **`O-50`＝「残りをシャッフル」を全デッキ `SHUFFLE_DECK` と表現している15効果の構造是正**（今回フラグでは直らないと判明して据置した分＝母集団が確定済み）。その次は **`O-51`**（「残りを好きな順番で」の並べ替え機構・原文282カード）。
...
| ID | 項目 | 規模 | ブロッカー／次の一手 |
|---|---|---|---|
| **O-51** | 🆕**「残りを好きな順番でデッキの下に置く」の並べ替えが engine に無い** | M | **2026-08-24 段2 第33バッチの投入前実測で判明**＝`REVEAL_AND_PICK.remainder.position` は `'bottom'` 以外を**一律「デッキの上」**として扱う（`effectExecutor.ts:5935` / `:6078` / `:8844`）＝**`position:'any'` を書いても何も起きないどころか誤って上へ置く死フラグ**になる。原文の母集団は **282カード**（CSV 原文・総数6712 で検算済み）。⚠**受け皿は `LOOK_AND_REORDER.reorder`（＝「順番を自由に決められる」）に既にある**ので、**新型を作らず REVEAL_AND_PICK の remainder 処理からその UI を再利用する**のが筋。⚠着手前に **`LOOK_AND_REORDER` 側で既に正しく表現されている分を引き算する**（§5 `3-3⁷`）＝282 のうち相当数は既存の受け皿で足りているはず。 |
| **O-50** | 🆕**「残りをシャッフルしてデッキの一番下に置く」を live が「デッキ全体を `SHUFFLE_DECK`」と表現している** | S | **2026-08-24 段2 第33バッチで据置した15効果**（`WX20-072-E1`／`WX21-037-E2`／`WDK01-008-E1`／`WX25-P3-047-E1`／`WXEX1-13-E1`/`-E2`／`WXK04-007-E1`/`008-E1`/`009-E2`/`010-E1`／`WXK05-007-E3`／`WDK07-Y02-E2`/`Y03-E2`/`Y04-E2`/`Y07-E1`）。原文は**公開した残りだけ**をランダム化してデッキ下へ置くのに、live は**デッキ全体を混ぜる**（LAR の destination が `top` ＋後続に全デッキ `SHUFFLE_DECK` が並ぶ木）。⚠**`remainder.shuffle` フラグを足すだけでは直らない**（第33バッチで実証済み・golden で据置を固定してある）＝**木ごと bottom 受け皿へ組み替える**必要がある。**母集団は確定済みなので次バッチにそのまま使える。** |
