...
- **🆕 セッション（2026-08-10・続き409・Opus 5）＝§6.4「生ID残存 `[STUB:]` 907箇所の仕分け」を消化。仕分け計器 `npm run census:stubs` を新設し、A群の初回1件を実装**。
  ...
  - **📌⑥ A群の残 12件で一番安いのは `EXILE_ARTS_FROM_LRIG_DECK_SKIP_SIGNI_STEP`（WXK11-001）**＝**シグニアタックステップのスキップ機構は既に完備**（`BLOCK_ACTION{actionId:'SIGNI_ATTACK_STEP'}`→`screens/battle/attackStepPhase.ts:11`。同カード①のルリグ側は既にこれで動いている）。欠けているのは**任意コスト「ルリグデッキのコスト合計2以上のアーツ1枚をゲームから除外」の語彙だけ**。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（golden **1742（+1）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **880 据置** / lint 0 errors・**254 warnings 増減0**）。`npm run regen` 実施済み（表示語彙を足したため）。**live JSON 変更0**。
  - **📋⑧ 次の一手**＝**Opus 側**＝(a)**タスク12 (cxxi)＝任意性脱落35件**（続き408 の登録分・単独バッチ）(b)**§6.4 A群の残12件**（上の ⑥ から）(c)**UNKNOWN 42箇所・31カード**／CPU AI（→§8）(d)置換5本の対話化判断。**Sonnet 側**＝§7 の実機未検証UI（続き403 の14枚＋404 の3件＋405 の2件＋406 の1件＋**409 の1件＝`WX25-CP1-074` のダメージ抑止**）と §5c census 文型バッチ。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き409（§6.4 STUB 仕分け計器）後 最新値（本行が直近の正）**：census **880 据置**、golden **1742（+1＝signiDamageGate）**、smoke **10686/10686** 全0・SKIP 0、fuzz 全0、lint 0 errors/**254 warnings**（増減0）、manual field loss 0、golden 型カバレッジ **128/128 据置**、**live JSON 変化 0 効果**（`npm run regen` は表示語彙のため実施）。**🆕 STUB 仕分けの実数（`npm run census:stubs`）**＝逆翻訳の `[STUB:…]` **906箇所**（うち生ID露出 **275**）／**A 実装の穴 19種21件（明示 defer 7種9件・無言 no-op 12種12件）／B 宣言型 18種32件／C 表示だけの穴 176種259箇所／D 健全**。⚠**残す教訓**＝(a) 🔴**「実装済みか」を id 文字列の grep で測ってはいけない**＝実装軸は**ハンドラ／engine 別経路／ペイロードキー／カード番号**の4本あり、較正中に実測で5種が「実装済みなのに実装の穴」と誤報された。(b) 🔴**前方一致の探索は向きを逆にする**＝「id の前方一致を src から探す」は `ASSIST_LRIG_ATTACK_THIS_TURN`↔`RETURN_ASSIST_LRIG_TO_DECK` のような他人の名前に当たる。**コード側の matcher を集めて id を test する**（実測でコード全体に matcher は1本だけ）。(c) 🔴**生成側（parser/manualEffects）と型宣言（`src/types/`）は「消費」ではない**＝混ぜると真 no-op が実装済みに化ける。(d) ⭐**`genStubsMd.mjs` の「フォールバック」欄は仕分けに使えない**（軸が1本＋id 正規表現が `[A-Z0-9_]+` で日本語入り id を取りこぼす）。
