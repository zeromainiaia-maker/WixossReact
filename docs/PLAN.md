...
**直近＝2026-09-21＝§5.7 `S-27`＝A/B の感度を上げた（勝敗の二値ではなく「対ごとの残ライフ差」で測る）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第431バッチ）
- 🔴**問題**＝`S-6` 第2段は **192組のうち144組（75%）が1勝1敗**で、`pairRate` がそれを分母から外す＝**1候補8分で決着する組は10〜14**。
- 🏁**直した**＝`AbGameResult.margin`（A 視点の残ライフ差）＋ `summarizeAb` の `pairMargins`／`marginMean`／**t 区間**（`tCritical95`＝**z で済ませない**）。
  🔴**補助であって主の判定ではない**＝目的関数は勝率。表に毎回そう書く（golden が文言を固定）。
- 🔴**実装中に前提が外れた**＝**ライフ0 はまだ敗北ではない**（0 の状態でさらに受けて負け）＝**勝者もライフ0で勝てる**。
  実測＝**両者ライフ0での決着 27/96（28%）**。⇒ `margin===0` は「符号の食い違い」ではなく**接戦**＝`marginTied` で別に数え、**割合＝感度の上限**を毎回出す。
- 📏**反転確認＝`S-6` 第2段と同じ4束・同じシードで測り直し**＝**勝敗の数字は1セルも変わらない**（決定論）／**標本は 14組 → 48組（3.4倍・最大6倍）**。
- 🔑🔴**結論が「測れていない」から「差が無い」へ上がった**＝**4束とも全山合算のライフ差が ±0.5枚に収まった**
  （-0.02 / +0.19 / +0.00 / -0.04）＝**「ライフ半枚ぶんより大きい差は無い」と言ってよい**。勝率では区間が [0%,100%] まで開いていた。
- ⚠**山ごとには3セルが有意**だが **24セル検定して 5% の期待値は 1.2**＝偶然の範囲⇒**多重比較の警告を出力に足した**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-26`**（`lrigLevel` を読む決定が無い＝グロウは常に候補の1枚目）。次に `S-22`・`S-24`・`S-14`。**`S-6` 第3段は「大きく振る候補」を探す段へ**（微調整は差が無いと測り切った）。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 4**（`S-26`・`S-22`・`S-24`・`S-14`＝🏁`S-27` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4351**）｜**実機は不要な回**（触ったのは `scripts/` の3本だけ＝PLAN §2.2）｜反転確認＝同じ4束・同じシードで**勝敗の数字が1セルも変わらない** |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🔥**`S-26`（グロウが盤面を見ない）**→ `S-22` → `S-24` → `S-14`（作戦データ）。
