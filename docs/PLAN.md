...
**直近＝2026-09-16＝第378バッチ：`O-524`（場が満杯だと「場に出す」カードが消える）／`O-526`（一番下ミルのログ）をクローズ**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- `applyDirectAction` の `ADD_TO_FIELD` を「取り除く前に空きゾーン判定」へ。置けなかったカードは `lastProcessedCards` からも外し、`execPlaceSigniOnField` がそれを引き継ぐ。
- round6 の母集団 129枚を再トレース＝**消滅 132効果 → 0**（残1＝`WXEX2-08-E4` の「ゲームから除外」＝ハーネスの死角）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.2 round6 の R6-0**（ハーネス整備）。`O-525` はユーザー判断待ち（索引H） |
| 📊**進捗3計器** | Sheet1 要対応 **2 / 863**（`census:cards -- --sheet 1` で測り直す）／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **1**（索引H 1）／実機 **0**／実装キュー **0** |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4261） |
...
- **2026-09-16 時点**（第378バッチ＝`O-524`／`O-526` クローズ）
  - 📊**進捗3計器**＝Sheet1 要対応 **2 / 863**（held 1・mech 1）｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**
  - 📦**在庫**＝機構 worklist **1**（索引H 1＝`O-525`）｜実機 **0**｜実装キュー **0**｜round6 **R6-0 未着手**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden 4261）
