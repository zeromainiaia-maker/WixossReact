...
- **🆕 セッション（2026-08-11・続き432・Opus 5）＝§6.4「消化済み項目が残した宿題」を残0にする。golden 1800→1804・census 854 据置**。
  - **🔴① 登録されていた宿題の1つは「穴」ではなかった**＝「起動コスト増加がトラッシュ起動に未適用」は**仕様どおり**（原文が「**センタールリグとシグニ**の【起】」に限定＝トラッシュのカードはシグニではない。新しい札が「この**カード**を」と書き分けているのが根拠）。⭐**「未適用の入口」を全部穴と決めつけない＝原文の限定を先に読む。** goldenTest に**適用する3入口／適用しない5入口の表**を固定して誤修正を防いだ。
  - **🔴② 実際の穴は別の入口だった＝カットイン窓**＝`CutinModal` が `collectIncreaseActCost` を見ておらず、**同じ【起】能力なのにメインフェイズ経由とカットイン経由でコストが食い違って**いた。⚠**同じファイルの上部にアーツ実効コストで同型の穴を踏んだ記録が既にある**＝この入口は食い違いが再発しやすい。候補一覧の可否と支払いモーダルの必要枚数の**両方**に載せた。
  - **🔴③ `PR-470A` の入れ替え配置がパワー配置制限を素通り**＝登録は「体数は不変なので count 制限は無関係」で止まっていたが、`deployLimitBlockReason` は **count とパワーの2軸**。⭐**「count は無関係」で止めると同じ funnel の別の判定を見落とす。** `fieldCountAdjust:1` で通して**体数には掛からず・パワーには掛かる**形にした。
  - **✅④ 離場置換の宣言走査を3軸へ**（続き430 で登録した latent）＝印字だけ見ていた3本に `declaredContinuousEffects`（印字＋付与2ストア）を通した。**live 0件なので挙動は不変**（golden 全通過が根拠）。「関数本体に印字直読みが残っていない」ロックを設置。
  - **✅⑤ ゲート**＝`npm run gates` 全緑（typecheck / golden **1804（+4）** / smoke 10688 全0・SKIP 0 / fuzz 全0 / census **854 据置** / census-stubs / manual-fields / lint 0 errors）。**§6.4「消化済み項目が残した宿題」は残0**（`POWER_THRESHOLD_TRASH` は live 0件の契約テスト済みなので対象外）。
  - **📋⑥ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 36ノード/34カード**（1カード1機構の単発が大半）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**置換の「してもよい」辞退**＝ライフクラッシュ置換側（離場置換 M2 と同じ枠組み）(e)`WXDi-P09-064` の相手側可変枚数コスト／`WXDi-P07-010-E2` の繰り返す遅延ゲート。**Sonnet 側**＝§7 実機検証（**続き431 の回帰確認＝「使った瞬間に何も起きない」を最優先**、次いで続き428・430）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き432（§6.4 消化済み項目の宿題を残0）後 最新値（本行が直近の正）**：census **854**、golden **1804（+4）**、**`MANDATORY_SUSPICIOUS` 0**、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、UNKNOWN **36ノード/34カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
