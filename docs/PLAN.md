...
2. **census「動的比較 35枚」**（「〜より高い/低い」＝heterogeneous・per-card）。**🔸続き43（Opus）で自己参照9枚＋トリガー参照2枚 着地（powerLt/Gt/levelLt/GtSelf・powerLt/levelLt/GtTrigger・engine 解決を resolveDynamicFilter に集約＝sourceCardNum/triggeringCardNum 引数・census 1631→1624）**。残＝(a)自己参照でも非経由の3枚（STUB `TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST` 内＝WXK05-059/WXK10-062・SEND_TO_ENERGY bespoke＝WXK09-052・nested CONDITIONAL 内 BANISH＝WXK10-040）、(b)**トリガー参照の trash→field / SEND_TO_ENERGY 経路**（WD14-011/WX24-P1-015/WXEX1-25/WXEX1-52＝トラッシュから「そのシグニより低い/高いレベル」・WXK11-041＝SEND_TO_ENERGY・WXK07-030＝nested＝いずれも parseSigniTarget 非経由＝ビルダーへ parseTriggerComparison 配線＋leftCard「場を離れたとき」との timing 曖昧性解消が要る）、(c)lastProcessed（その後/場に出たそれ＝⚠`applyToField` が配置シグニを lastProcessedCards 未設定＝engine 設定もセット）・printed（表記されているパワー＝per-candidate 実効/表記比較の新機構）・opp/own センタールリグ・「あなたのいずれかのシグニ」。詳細 BUGFIXES 最上部。
...
- **🆕 セッション（2026-07-07・続き43・Opus 4.8・§4タスク2 census「動的比較」の自己参照サブファミリを機構化）**＝「このシグニ/自身より〔パワー/レベル〕の〔低い/高い〕対戦相手のシグニ」等、**効果元シグニ自身を基準にした比較フィルタが JSON で全数脱落**（35枚を機械分類＝`dyn=[]`／STUB でもない＝比較なしで全シグニを対象にする**過剰効果**）していたのを、自己参照サブファミリだけ1機構として確立。**型**＝`powerLtSelf`/`powerLteSelf`（既存）に **`powerGtSelf`/`levelLtSelf`/`levelGtSelf` を新設**（mirror family）。**engine**＝自己参照解決を `execBanish` 限定インライン → 中心の **`resolveDynamicFilter` に集約**（`sourceCardNum` を新引数で受け、全10呼び出し箇所へ `ctx.sourceCardNum`/`ctx.effectivePowers` を配線）＝BANISH だけでなく TRASH/DOWN/SEND_TO_ENERGY/ADD_TO_FIELD/SEARCH 等**全ターゲット系で自己比較が効く**（従来 execBanish 以外は silently 無視のバグも同時に解消）。**parser**＝`parseSelfComparison` を `parseSigniTarget` に配線（**「このシグニ／自身より」限定**＝別基準は対象外・条件文脈 WXK11-048/WX17-075/WX10-036 は非経由で誤爆なし）。**decompiler** 3レンダラ追加（原文一致）。**採用11枚（3ラウンド）**＝①BANISH/target 自己参照8枚（build:effects 自動採用7＋WX22-Re01 parity パッチ1）＝powerLtSelf: WX22-019/WX22-Re01/WXDi-P00-051/WXK10-028/WXK10-068/WXK11-055/WD18-006、powerGtSelf: WXK04-029。②自己参照ビルダー routing 拡張＝energy/trash→field ADD_TO_FIELD ビルダーにも `parseSelfComparison`（自己蘇生分岐を `!/このシグニより/` でガード）＝**WXDi-P03-078**。③トリガー参照2枚＝新設 `parseTriggerComparison`（「そのシグニより〔パワー/レベル〕の低い/高い」＝triggeringCardNum 基準・型 powerLt/levelLt/GtTrigger 新設・resolveDynamicFilter に triggeringCardNum 引数追加）＝**WXK11-020**（バニッシュされたそのシグニよりパワーの低い→powerLtTrigger・ON_BANISH は BattleScreen が triggeringCardNum 設定済）・**WX09-014**（場に出たそのシグニより低いレベル→levelLtTrigger）。⚠parseTriggerComparison は「その後」除外＋マッチ後方「場に出」除外で leftCard/placement mis-parse を回避（WX14-009 の DOWN spurious を手除去）。**各ラウンド精密 semantic diff で純粋な追加のみ（collateral 0）**を確認。**検証**＝typecheck・**golden 160/0（+5）**・smoke 全0（10582）・fuzz 全0・lint 0 errors・**同型★0**・**census 1631→1624（-7）**＝`BASELINE_HIGH`＋PATTERNS keys 8種 更新。**トリガー参照2枚は engine 配線ため要実機検証（ON_PLAY ally の WX09-014 は verifyBattleDrive 推奨）**。**次の一手＝(A)Opus＝§4タスク2 残**：トリガー参照の trash→field/SEND_TO_ENERGY 経路（WD14-011/WX24-P1-015/WXEX1-25/WXEX1-52/WXK11-041/WXK07-030＝ビルダーへ parseTriggerComparison 配線＋leftCard「場を離れた」との timing 曖昧性解消）＋自己参照 STUB/nested 残3枚（WXK05-059/WXK10-062/WXK09-052/WXK10-040）＋lastProcessed（⚠applyToField が配置シグニを lastProcessedCards 未設定）＋printed（表記パワー＝per-candidate 新機構）＋lrig相対（WX19-042/WXK11-003）。**(B)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46・今回のトリガー参照2枚の実機確認も）。****(B)Opus＝タスク3 引用内CHOOSE（WXDi-D09-P20）＋残STUB2枚（WX24-P1-017/WX25-P3-038）の GRANT_QUOTED_AUTO_ABILITY 内側parse。(C)Sonnet＝§5c 再収穫（`/census-batch`）＋§7 実機検証（R30/R31/R36/R39-R46）。(D)verifyBattleDrive バッチ状態汚染の根本修正は続き39から持ち越し。**
