...
**直近＝2026-09-20＝🏁§5.7 `S-15`（候補列挙器）＋`S-16` 第1〜3段（探索の土台）＋🏁`S-18`（次ターン項）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**`S-15`**＝`cpuMoves.ts`＝その盤面で取れる手を全部 `CpuMove[]` で出す。各 `pickCpu*` を「列挙／選ぶ」に割り、**本番も探索も同じ列挙を通る**。**挙動不変**（自己対戦3シードの全ログ一致）＋**「打った手は必ず列挙に出ている」を `selfplay`・golden が常時照合**。
- **`S-16`**＝①**先読みのカンニングを塞いだ**（`hideDeckOrder`＝正準順に並べ直して盤面 seed で混ぜる＝**山の順序を見ない**）②**探索用の1手適用** `applyCpuMoveSim`（支払いと使用済みの印を写す・適用できない4種は探索の外）③**ビーム探索** `searchCpuMove`（`cpuSearch.ts`・**未配線・既定 幅0**）。
- 🏁**`S-18`**＝盤面の採点に**次のターン**を入れた（`lrigLevel`＝**グロウの価値**／`growReady`／`handEmpty`／`guardKept`）。🔴**左右対称に数える**（`guardKept` だけ自分側＝相手の手札は非公開）。
- 🔑🔴**探索を実測して取る順が変わった**＝`S-18` 前は**探索が 113手中 75手で「打たない」**（エナチャージ −500・**ルリグのレベルが採点に無くグロウが損に見える**）⇒ `S-18` を `S-16` の配線より先に消化 ⇒ **「打たない」 74→58・一致 24→40・グロウを選ぶようになった**。
- ⚠**勝率では測れない規模**＝`legacy-nextturn` vs `default` は **80戦で 51.2% [40.5, 61.9]**（組の内訳＝1勝1敗 39/40＝**席の偏り（先攻 33.8%）に支配**）。⇒ 止め時は勝率ではなく**探索の挙動**に置いた。
- **実測（`--census-moves`）**＝MAIN の候補 中央2・p90 8・最大18／**1手適用 平均0.1ms**／**探索（幅4深さ4）1盤面 平均3.0ms・最大34ms**＝**コストは問題にならない**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-16` の配線**（探索を `cpuTurnAction` の MAIN へ繋ぐ＝ポリシーの数値 `searchWidth`/`searchDepth` で入切・既定 0 から上げる判断は A/B）。⚠**エナチャージは探索の担当から外す**（手札 1500 → エナ 1000 で常に損に見える＝「置くか」ではなく「どれを置くか」の判断）。🆕**`S-20`（A/B の山が型を踏まない）は探索の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 6**（`S-16` の配線・`S-17`・`S-19`・`S-20`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先読みの情報漏れ（`S-16` の前提）②先攻が勝てない 28.1%（`S-19`）③**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4337**＝+4）｜**実機** `node scripts/verifyFullMatch.mjs cpu` **PASS**（9ターン / 218手 / 287秒）※この日は3回回して1回目だけ FAIL（見張りのバグ＝同日修正） |
...
| 🔥`S-16` | **前半ブロックの探索**＝`ENERGY`〜`ATTACK_ARTS` を**1つの探索単位**にしてビーム探索（目的関数＝**アタックステップに入る時点の盤面**）。🆕**2026-09-20 に第1〜3段まで実装済み**＝①先読みの情報漏れを塞いだ（`hideDeckOrder`）②探索用の1手適用 `applyCpuMoveSim`（適用できない4種＝`assistGrow`/`resona`/`rise`/`piece` は探索の外）③ビーム探索 `searchCpuMove`（`cpuSearch.ts`・**未配線・既定 幅0**）。🔥**残りは配線**＝`cpuTurnAction` の MAIN で `searchCpuMove` を使い、選ばれた手を本番の実行関数へ流す（`perform*` への dispatcher が要る）。**入切はポリシーの数値**（`searchWidth`/`searchDepth`・既定 0＝挙動不変）で、**上げる判断は A/B**。⚠**エナチャージは探索の担当から外す**（手札 1500 → エナ 1000 で常に損＝「置くか」ではなく「どれを置くか」の判断）。🏁前提だった `S-18` は消化済み（探索の「打たない」 74→58・グロウを選ぶようになった）。 | 大 | **手順の入れ替え**と**資源配分**を CPU が検討するようになる（いまはどちらも固定台本）。⤵**`S-13` はここに吸収** |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~（2026-09-20＝`S-16` の配線の前提として先に消化）→ 🔥**`S-16`（前半探索）の配線** → `S-17`（後半＝攻撃探索）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-20 時点**（第412バッチ＝🏁§5.7 `S-18` 次ターン項／`S-16` 第1〜3段）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 6**（`S-16` の配線・`S-17`・`S-19`・`S-20`・`S-14`・`S-6`＝🏁`S-15`・🏁`S-18` を消化）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4337**＝+4・`census:traceinv` I1=0・`selfplay` 決着＋打った手の照合 外れ0）｜**実機** `verifyFullMatch.mjs cpu` PASS
  - 📏**BattleScreen**＝**6,287行**（±0）｜`controller/cpuTurn.ts` **1,372行**（列挙を `cpuMoves.ts` **693行**・探索を `cpuSearch.ts` **109行**へ）
  - 📊**CPU の候補数**（`--census-moves`・4戦 432盤面）＝ENERGY 中央5／GROW 0〜1／**MAIN 中央2・p90 8・最大18**／ATTACK_ARTS 0〜1
  - 🆕📊**探索のコスト**（幅4・深さ4）＝**1盤面 平均3.0ms・p90 9ms・最大34ms**／展開 平均3.9・最大54｜**1手適用（`applyCpuMoveSim`）平均0.1ms**（適用できるのは候補の 89%）
  - 🆕📊**探索といまの選択の一致**＝同じ 40／違う 13／**打たない 58**（`S-18` 前＝同じ 24／違う 13／打たない 74）
  - 📊**自己対戦 A/B**＝`legacy-nextturn` vs `default`＝**80戦で 51.2% [40.5, 61.9]＝差なし**｜`legacy-power` vs `default`＝370戦で 50.0%（据置）｜**先攻の勝率 33.8%**（`S-19`）
