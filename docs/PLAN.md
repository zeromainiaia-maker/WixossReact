...
**直近＝2026-09-22＝§5.7 `S-31` ② 第5段＝残りの踏み倒し8効果＋入口違いの2効果（撃てない【起】 26 → 15）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第448バッチ）
- 📏**同じ読み方で8効果**＝`underAnySigniTrash` 2／**`exceed`（シグニの【起】にも在る）** 2／`multiZoneExile` 1
  （**提示ゲートだけ在って支払いが無かった**）／ルリグの `fieldDown` 2・`life_crash` 1。
- 🔑**`fieldDownCost.ts` を新設して支払いを1本に**＝場のシグニ【起】のインライン実装を funnel へ出し、ルリグにも配線
  （写経で2本目を書かない）。他は**既存 funnel を呼ぶだけ**だった。
- 🔴**入口が場ではない【起】を2効果外した**（`handExileSelf`／`energyTrashSelf`＝`O-262` の家族）＝
  parser が入口フラグを立てておらず、**場のシグニとして提示＋支払い無し＝完全な踏み倒し**だった。
  ⚠**撃てなくなる**（正しい入口はまだ無い）が**踏み倒しより安全側**。
- 📏**実カード照合**＝提示された **38（場のシグニ）＋15（ルリグ）すべてで支払い選択が成立**（食い違い0）。
  🔴**実機 PASS**（両デッキ）。⚠**初回だけ序盤で手詰まり FAIL → 再実行で PASS・原因未特定**（engine 再現なし）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` ②の残り**（撃てない【起】15＝**ほぼ「CPU に選ばせる判断」だけ**）。次に `S-29` |
...
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**撃てない【起】は残 15効果**／⚠**入口が場ではない2効果**（`handExileSelf`／`energyTrashSelf`）は**いま撃てない**＝手札・エナの入口を作るのが残り |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4366**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**①は測って否定**／🏁**③は消化**（札の使いどころ）／🏁**②は5段まで**（手札捨て／エナ・場／自分の盤面・デッキ／誰も払っていなかったコスト4種／**残りの踏み倒し8効果**）＝**撃てない【起】 210 → 108 → 37 → 26 → 15**。**残り＝②の続きだけ**（場のシグニ 9／ルリグ 6）。🔑**性質が変わった＝人間の支払いUIも支払いも在り、残りは「CPU に選ばせる判断を書く」だけ**（⚠例外＝`handBottomDeck`／`trapToHand` は支払いが無い）。⚠**入口が場ではない2効果**（`handExileSelf`／`energyTrashSelf`）は手札・エナの入口を作るのが残り。 | 中 | 作った CPU デッキを意図どおりに回せる |
...
- **2026-09-22 時点**（第421〜448バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
  ...
  - 📏**CPU が撃てない【起】**（§5.7 `S-31` ②・**その文脈の gate が提示するものだけを分母にする**）＝**場のシグニ 695効果中 9（1.3%）／ルリグ 567効果中 6（1.1%）＝合計 15**（第439・442・446・447・448バッチで 210 → 108 → 37 → 26 → 15）。🔴**過去の大きい数字は分母の取り方が違う**＝旧「648（24.6%）」は文脈を混ぜた値／旧「83」は**入口が場ではない【起】**を含んでいた
