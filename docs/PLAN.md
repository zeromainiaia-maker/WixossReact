...
**直近＝2026-09-19＝実機シナリオの既存 FAIL 85本を全部消化＋engine 実バグ2件**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**元85本（A・B 17本＋C 68本）をすべて解消**。腐りの系統は7つ＝①対象選択が任意コストより先 ②アタッカーはバトルでバニッシュされない（相手に殴らせる） ③配置レベル制限がルール処理に ④ピースはルリグ3体 ⑤CHOOSE のラベル ⑥観測が画面ログ文字列 ⑦`pick-0` のトグル。
- ⚠**「残0」＝個別／小群で全部 PASS**。**68本を1回で回すと毎回1本前後が落ちる**（実測3回＝66/67/67・顔ぶれは毎回違う）＝2026-09-18 記録の「CPU の待ち予算に対するタイミングの揺れ」と同じ形。**1回のバッチ FAIL を即バグと読まない。**
...
- ⚠golden にテストを1本足すと**共有 POOL カーソルがずれて無関係なテストが落ちる**（`withSavedCursor`＝[LESSONS.md](./LESSONS.md) §4.3）。新しい罠＝[DRIVE_TRAPS.md](./DRIVE_TRAPS.md) §4.4-125〜130。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.3 `O-535`**（「対象とし〜そうした場合」で同じ対象を2回聞かれる＝631効果/608カード。まず `ON_TARGETED` の二重発火を minimal pair で測る）→ その後 **§5.7 `S-5d`** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**ハーネスと engine の回＝live JSON を1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 機構 worklist **1**（`O-535`）／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／リリース作業 **1**（RELEASE.md）／🏁**実機シナリオの既存 FAIL 0**（元85本＝全部解消） |
...
| ID | 母集団 | 何が無いか |
|---|---|---|
| `O-535` | **631効果 / 608カード**（live 実測） | 「〈対象〉を**対象とし**、〈任意コスト〉して**もよい**。そうした場合〜」の形で**同じ対象を2回選ばされる**（`SELECT_TARGET_ONLY` で1回、帰結の `targetsStored` 側でもう1回）。`selectOrInteract` に「候補が確定している場合の自動解決」が無い |
