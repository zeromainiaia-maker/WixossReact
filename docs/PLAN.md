...
12. **Sonnet が積んだ engine/parser バグの修正（常設受け口）**＝Sonnetタスク1（実機検証）・4（一次トリアージ）・8（semantic audit）の観測結果を受けて修正する。**✅続き78（Opus）で在庫8件を全消化**＝(a)EXILE→TRASH誤変換7枚系統（parser にEXILE 3形を新設・5枚は fresh==curated 化・TK1A timing是正/P13-089 no-op是正を採用）・(b)多段「あるかぎり」2枚（**真因2本を発見**＝genericKagiri の引用跨ぎ消費と qfSelf 貪欲丸呑み。`THIS_CARD_HAS_UNDER{filter}`/`rawStages`/`LOSE_SIGNI_BARRIER` 新設＋**連用中止「パワーは＋Nされ、<B>」系統48枚**へ横展開）・(c)条件ドロップ3枚（ガードC例外化＋manualEffects triggerScope追加）・(d)inner duration（`restoreLeadUntilEndOfTurn` 共通化＝**母集団112枚を機械分類で一括採用**）・(e)GRANT_CHOSEN_ABILITY汎用ハンドラ点検（実欠陥は WXK08-026 の `_SELF` 誤対象選択＝効果元自動化＋クラス限定適用を追加）。**計148枚採用・golden 230・census 1483**。詳細 BUGFIXES 続き78。**🆕新規在庫3件（続き78で発見・据置）**：(i)**SP27-002-E3**＝引用付与内側の「このシグニの正面のシグニのパワーが15000以上であるかぎり」条件が genericKagiri（isTimingMarker＝**無言消費で PARTIAL にもならない**）で脱落し無条件アサシン付与に退化＝**genericKagiri の isTimingMarker 設計を silent-fallback 刻印に見直す系統課題**。(ii)**WXDi-P10-035**＝連用中止復元の引用内【自】で「それを手札に戻す」が `BOUNCE{target.owner:'self'}` にエンコードされる＝`TARGET_OPP_SIGNI_OPTIONAL_COLOR_COST`+lastProcessed 慣例との整合を要精査。(iii)**WXK09-050**＝parser規則（続き76 タスク10 パターンC）が `GRANT_CHOSEN_ABILITY` を再生成し続け held に残存＝Part1固有ハンドラ（パワー比較フィルタ）との dispatch 設計を解消するまで採用不可。
...
- **🆕 セッション（2026-07-12・続き79・Sonnet 5・Sonnetタスク1＝続き76/78で新規実装された6機構の実機検証を`verifyBattleDrive.mjs`へ追加・**進行中（中断・環境問題の調査へ切替）**）**
  ...
  - **✅ 2/6 実機PASS確認**＝`freezeLrig`（guest.field.lrig_frozen=true 確認）・`negateAttackLrig`（guest.negated_attacks に対象lrigのcardNumが載ることを確認）。**両方とも続き76のパターンB実装が実機で正しく動くことを実証**。
  - **残4件は3回のデバッグサイクルでUI操作側の不備を特定・修正済み（未再検証）**＝(1)`exileHandBlind`＝手札2枚ピッカーが**クリックの度に選択→解除をトグルする**バグ（`決定 (2/2)`が出るまでのみクリックするガードに修正）。(2)`blockDrawByEffect`/`trashCounterOpp`＝ON_PLAY任意コストのUIが`optcost-pay`ではなく別コンポーネント`SigniOnPlayCostModal`（testid`onplaycost-energy-N`＋プレーンな「発動」「スキップ」ボタン）だった（当初の思い込みが誤り）。(3)`delayedAttackTrigger`＝センターLRIG画像の再クリックがアクションボタン検出より優先され、前セッション（`blockDrawByEffect`）の**未解決モーダルが画面に残留したまま次シナリオへ流れ込む**汚染を誘発していた＝優先順位をボタン検出→画像クリックの順に反転。
  ...
  - **⚠️ ユーザー指摘＝ビルド+ログイン+CPU戦セットアップだけで毎回15〜30分かかっている（ドキュメント上の想定「数分」を大幅超過）。原因調査はモデルをOpusに切り替えて実施する方針＝本セッションでは深追いせず、次回Opusセッションへの引き継ぎ情報としてここに事実だけ記録する。**
    - **一次観測**＝`node scripts/verifyBattleDrive.mjs`を4回連続実行した結果、**`npm run preview`（vite preview）の子プロセスが一度も正常終了せず、ポート4173〜4222の50個に積み上がって常駐**していることを`Get-NetTCPConnection`で確認（1回の実行で新規ポートが1つ増える＝古いpreviewサーバが再利用されずviteが自動で次のポートへ逃げている）。`node.exe`104個・`cmd.exe`54個・`chrome.exe`/`chrome-headless-shell.exe`24個が同時に常駐（本セッション開始時点で既にこの水準＝前セッション群からの蓄積）。
    - **スクリプト自身に後始末ロジックはある**（`scripts/verifyBattleDrive.mjs`末尾の`finally { proc.kill(); spawn('taskkill', ['/pid', ..., '/T', '/F']); }`）が、**機能していない形跡**＝`taskkill /PID <mainNodePid> /T /F`を外部から手動実行したところプロセスツリーは正しく全滅した（有効な手段であることは実証済み）ため、**スクリプト内部の呼び出しが実行完了前にNode本体が`process.exit()`してしまっている（fire-and-forgetのspawnをawaitしていない）か、外部要因（ツール側のタイムアウトやセッション中断）でNode自体の`finally`ブロックに到達せず異常終了しているケースが積み重なった**のではないかという仮説（未検証）。CPU使用率は8.9%・空きメモリ約18GB/33GBで「今この瞬間」は逼迫していないが、**大量プロセスによるWindowsのファイルロック競合（同じ`dist/`ディレクトリを50個のpreviewサーバが開いたまま、新しい`vite build`が上書きしようとする）がビルド遅延の実体ではないか**という仮説がある＝**Opusでの検証・原因確定・恒久修正待ち**。
    - **次のOpusセッションで見るべき箇所**＝①`buildFirst()`/`startDev()`の後始末を`await`で確実化する設計に直すか検討、②起動時に同名プロセス（`npm run preview` matching this project path）を一括krillする前処理を追加するか検討、③`spawn(...,{shell:true})`が生む孫プロセスをWindowsで確実にkillする方法（プロセスグループkill等）を調査、④そもそも巻き添えの49個の孤児プロセスをどう安全に一括cleanupするか（ユーザー承認必須・今回は本人依頼で手動killした自分の分＝PID 31264のみ処理し、残りは未着手のまま保持）。
  - **次の一手＝Opusで上記の実行環境の遅延原因を調査・恒久修正 → その後 Sonnet で残4シナリオ（`exileHandBlind`/`blockDrawByEffect`/`delayedAttackTrigger`/`trashCounterOpp`）の再実行・PASS確認 → `order`配列への追加・BUGFIXES.md記帳・commit**。シナリオのコード自体は完成しており、再実行するだけで良い状態（`node scripts/verifyBattleDrive.mjs exileHandBlind blockDrawByEffect delayedAttackTrigger trashCounterOpp`）。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 1483枚（2026-07-12 続き78・履歴 …→1567→1566→1563→1558→1557・続き63 DEPLOY_RESTRICT実装で5枚・続き67 WXEX2-25-E3 lrig相対付与で1枚・続き68 デッキ相対SEARCH3枚・続き69 引用内CHOOSE 26枚で5枚・続き71 CHOOSE平坦化復元WXK10-013で1枚解消・続き75 timing語彙8種で1557→1537・続き76 timing語彙 計19系統＋タスク10 パターンA〜F（全解決）で1537→1514・続き77 §5c再収穫（held 85枚採用）で1514→1494・続き78 タスク12根治＋held 148枚採用で1494→1483・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。状態系の残（凍結13・ダウン/アップ38）はコスト節/条件/CONT型（別パス・§6.3）。**消化の入口は `npm run census:clusters`＝文型テンプレのクラスタ表（`docs/_census_clusters.txt`・枚数順）から系統バッチを選び、parser規則→`npm run build:effects`→`node scripts/heldReview.mjs` で署名グループごとに一括採用する（続き23確立・手順詳細は §5c）。**
