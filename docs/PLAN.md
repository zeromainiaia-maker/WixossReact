...
| 5 | 小口持ち越し（隙間埋めに最適） | 単点（parser/engine/decompiler混在） | S×件数 | **✅残小口3枚（WXEX1-65／WXEX2-50／WX20-053）を消化（続き265・codex実装/Opus確認）**＝いずれも「幻覚 or 過剰効果」の実害＝正面2-zi＋直前ミル札との同レベル条件（旧＝**自分のシグニをデッキへ送る幻覚**＋無条件実行）、相手場出しを跨ぐ `levelLteLastProcessed`（旧＝**任意レベルの＜凶蟲＞を出せる過剰効果**）、手札/デッキ二重source選択＋デッキ枝だけshuffle（旧＝**source なしの丸裸 ADD_TO_FIELD**）。golden 744→747・census 1545維持。既消化＝catch-all「デッキに戻す」全数再点検（続き264）。残＝置換系統40枚の一般化（分離 pick 単独解決＋置換 else 機構・§6.3級）／**WXDi-P05-009「それ」先行詞解決＝現状もトラッシュ対象を失い場の自シグニをデッキ上へ送る幻覚。任意コスト（ルリグ下消費）を跨いで先に対象化したトラッシュカードを保持する実行経路が要る＝§6.3級**。個別項目（WXDi-P03-005/GRANT_LRIG_ABILITY 5枚/TRANSFER_TO_DECK 混入/WXEX2-25-E3/自己犠牲5枚/WX26-CP1-100/PR-Di038）は続き218e/218i/218k/189/232 で消化済。詳細は [PLAN_DETAIL.md](./PLAN_DETAIL.md) 2026-07-24節・BUGFIXES 各続き |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 747・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1545（＝`BASELINE_HIGH`・回帰ゲート）。
>
