...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 2027【効果単位】**（2026-07-17 続き174 更新・2026-07-13 続き109 で判定粒度を「カード単位」→「効果単位（effectId）」へ切替。旧カード単位の 1447 とは**計測仕様が違うので比較不能**）。⚠続き177 で 2026 へ 1 改善したが要因未特定のため `BASELINE_HIGH` は 2027 据置（2026≤2027 で gate PASS）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。**前提＝`docs/_effect_srctext.json`（`npm run build:effects` の副産物）が最新であること**（無ければ census は exit 1）。明細 `docs/_vocab_census.txt`・消化の入口は `npm run census:clusters`（§5c）。**切替の根拠と計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109。**
