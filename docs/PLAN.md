...
- **🆕 セッション（2026-07-13・続き107・Opus 4.8・PLAN §3 Opusタスク6＝ベット「代わりに」置換機構＝IS_BETTING択一＋betChoose選択数変更を新設）**
  ...
  - **次の一手（Opus）＝** タスク6続き（WX25-P3-116＝STATE_CONDITION_CLAUSESへアーツ色条件追加・WX06-002・WX16-021置換ルール）が費用対効果高。betChoose 実カード11枚は各 choice の parser 品質を上げれば採用可（choice sub-clause の filter/条件脱落を1枚ずつ是正＝§5b/§6 テールと合流）。または §6.2 系統②残（GRANT_PROTECTION 広域24件）・(vii)(viii) 構造的バグ。**(vi-5)/(x) トリガー usageLimit 書き戻しは実機ドライバ付き専用セッション推奨**。
