...
- **🆕 セッション（2026-08-09・続き385・Opus 5／実装は Codex 委譲）＝🏁§5d-0 (ii)「`parseStatus:UNKNOWN` の完全 no-op」を残0クローズ（7効果）**・golden 1548→**1556**・census 910→**909**・被覆マトリクス miss 281→**280**。
  - **⭐① 投入前実測が在庫記述を3点とも覆した**＝(a) UNKNOWN は 6件でなく **7件**（`WX25-P1-048-E1` が増えていた）(b)「`PARTIAL` 23件」は実測 live **54件**／census 高シグナル **18件**で、中身は **(iii) 構造混線**＝(ii)「安い在庫」ではない (c) 🔴**`docs/_partial_report.txt` の「IS_MY_TURN化 40件」は 38件が既に `MANUAL` で解消済み**＝刻印レポートは *fresh parse* の記録であって **live 実害の一覧ではない**（live 残は `WX26-CP1-058-E1` の1件だけ）。**在庫行は症状の見立てであって実測ではない**（§3-1 の実例がまた1つ増えた）。
  - **✅② 7効果の内訳と着地**＝**群A** アシストルリグ最上段をルリグデッキへ戻す4（新型 `RETURN_ASSIST_LRIG_TO_DECK`＋純関数 `collectReturnableAssistLrigTops`。**下のカードは場に残す**＝スタック `at(-1)` だけ移す）／**群B** `WX09-019-E3`「これをトラッシュに置く」＝既存 `thisCardOnly` で表現（新型ゼロ）／**群C** `WXDi-P00-002-E1`＝既存 `RemoveAbilitiesAction` に `keywords?:string[]` を追加し、一過性付与と CONTINUOUS 付与の**両方**を遮断／**群D** `WX25-P1-048-E1`＝アシストルリグのアタック機構が無いので**専用宣言 STUB へ honest defer**（既存の実働 STUB を借りていないことをコードで確認＝§5-5e 遵守）。**live 変化 7効果・added/removed 0**。
  - **🔴③ 検証側で1件是正＝「評価器が対象ゾーンを走査している」と「そのゾーンのカードが filter に一致する」は別**。codex は群C の使用条件をセンター限定 `LRIG_COLOR` から**アシストも走査する** `HAS_CARD_IN_FIELD{cardType:'ルリグ'}` へ広げた（**判断そのものは正しい**）が、`matchesFilter` の cardType は **`types.includes(card.Type)` の厳密一致**（`execUtils.ts:590-597`）で、CSV の Type は **`'ルリグ'` 1234枚と `'アシストルリグ'` 340枚が別値**＝`lrigZoneTops` がアシストを走査していても**黙って全部落ちてセンター限定に戻っていた**。⚠**codex の golden が検出できなかった理由が2つとも既知の弱点の再発**＝(a) アシストゾーンに `Type==='ルリグ'` のカードを座らせた (b) assert が `condition.type` の**構造 assert だけ**で評価器を1度も呼んでいない（§5-5d）。→ `cardType:['ルリグ','アシストルリグ']` の配列形へ是正し、golden を**実 Type='アシストルリグ'** ＋ `evalUseCondition` の**成立/不成立 両方向**へ差し替え、**逆翻訳（`decompileEffects.ts:458` も同じ文字列比較で「場に《白》の“シグニ”がいる」という嘘を出していた）も同じ回で追随**させた。
  - **📋④ 次の一手**＝**Opus 側**＝🆕**同型の `HAS_CARD_IN_FIELD{cardType:'ルリグ'}` が live に 19効果**（`WXDi-P08-001`〜`005` 系のピース使用条件「あなたの場に○と○と○のルリグがいる」9件／`WX24-P4-068/075/082/089`「レベル4以上のルリグ」4件ほか）＝**すべてアシストルリグを数え落としている疑い**。Diva のピースはセンター＋アシスト2の**3体**を前提にした使用条件なので、原文照合のうえ配列形へ寄せる（⚠**`matchesFilter` 側でレゾナ→シグニと同じ緩和を入れる一般化は禁止**＝「ルリグ1体を対象とし」の**対象フィルタ**まで広がる）。次点＝§6.3 の残 **C／E／F／G／K**。**Sonnet 側**＝§7 実機検証（⚠**未検証UIが続き380〜385 で計8件**＝今回の群A アシスト選択UIが加わった）。
