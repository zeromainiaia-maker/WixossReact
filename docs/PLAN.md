...
- **🆕 セッション（2026-08-01・続き316・Opus 5〔**codex 実装／Claude 指示・検証・簿記**〕）＝タスク12(lxxvii)＝連用中止「Aし、Bし、Cする」で動作が無言脱落する効果を是正。**codex は6効果、Claude の検証で 8効果**へ拡大して残0クローズ**（golden **1229→1234**・census **1353→1352**（`BASELINE_HIGH=1352`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **250→251枚/98→99署名**〔+1 は honest defer〕。live per-effect 差分 **changed 8 / added 0 / removed 0**（ベースライン `af28e1d4` 比）／`build:effects`・`regen` 冪等。**engine 改修0・新語彙0本**）
  - **① 投入前の実測で在庫の見立てを訂正**＝在庫は「原因は2分割しかしないこと」としていたが、走査すると **3項以上の実バグ2件に対し2項で語尾がリスト外の実バグ4件**＝**主因は語尾リストの不足**。`WXK03-051-E1` は**症状が逆**（先頭が残り後半が落ちる）。これを指示書に書いて投げた。
  - **② 直したのは全部「動作が1つ消える」**＝`WXDi-P04-028-E3`（**相手全シグニのエナ送り**＝本体）／`WDK14-007-E1`（**自ライフクラッシュ**＝デメリット）／`WXDi-P10-003-E1`（中間2動作）／`WX18-025-E1`／`WXK03-051-E1`／`WXEX2-48-BURST`／🆕`WXK04-006-E1`（末尾 DRAW）／🆕`WX24-P3-022-E1`（**丸ごと `UNKNOWN`**＝公開が起きず後続のバースト判定も常に偽だった）。
  - **③ 🔴 検証で覆した点＝語尾を「枚数まで決め打ち」すると同型カードが黙って落ちる**。codex 初版は `カードを２枚引き(?=、【エナチャージ２】)` 等**特定カードの本文を埋め込んだ先読み3つ**を持ち、**理由コメントが無かった**。1つずつ外して計測すると **live diff は変化0**なのに **held は +3**＝⚠**live diff だけで「守るものが無い」と判断したのは誤り**（held に落ちる fresh の変化は live に映らない）。3件を原文照合すると**2件は改善・1件は中立**で、決め打ちは「巻き込み防止」ではなく**修正の取りこぼし**を作っていた。枚数を一般化して2件を追加採用。
  - **④ ⚠これは Claude の指示書の欠陥でもある**＝「held は 250/98 に**戻して終わる**」と数値で固定したため、**regex を絞って改善を捨てる**方向のインセンティブになった。**今後は「held の増分は1件ずつ原文照合して採用/据置を判断する」と書く**（CODEX_GUIDE §5-5 の運用注記を更新すること）。
  - **⑤ honest defer 1件**＝`WXK03-069-E1` は fresh が `LOOK_AND_REORDER` になるが curated の `TRANSFER_TO_DECK` のほうが素直＝**退化でも改善でもない**ので据置。据置であることも golden で固定した（held 251 の +1 はこれ）。
  - **次の一手**＝**Opus**：①**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く見込み＝筆頭）②**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）③**(lxxii) 遅延クリアの片側漏れ**④(lxxiv)⑤(lxviii) 散文形「対戦相手のターンの間、」⑥§6.3 J 残11 ⑦(lxvii) CPU `ON_TURN_END` 未収集⑧(lxvi)。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜316 の全件が実機UI未検証で積み上がっている**。
### 📊 恒久指標（維持中・逐次更新）
