...
**Opusタスク12＝未消化の在庫 4件**（2026-08-10 続き404・408 の2件＋**2026-08-14 続き476 で (cxxxi)(cxxxii) を登録**）。⚠**続き477 で登録した (cxxxiii) は 続き478 の実機検証で取り下げ**（＝engine バグではなくシナリオ偽陰性。詳細は下の行）。⚠**2026-08-14 続き475 で (cxxv) を取り下げ（シナリオ偽陽性）**、**続き475b で (cxxvi)(cxxx)**、**続き475c で (cxxix)**、**続き475d で (cxxvii)(cxxviii)**、**続き475g で (cxxiii)** を残0クローズ＝8件→2件。
...
| ~~🆕🔴(cxxxiii) 旧記述~~ | 🔴**付与ストアの `ON_ENERGY_CHARGE` watcher だけが恒久 no-op**＝`GRANT_LRIG_ABILITY` で積んだ `ON_ENERGY_CHARGE` の【自】が、**エナが1枚増えても一度も発火しない**。付与自体は成立している（`lrig_granted_auto_effects` に入り `grantedLrigAutoIds` で観測できる）。⭐**同じスペル経路・同じ盤面で「印刷能力」の `ON_ENERGY_CHARGE` は発火する**ので、**CHOOSE 解決後の中央 diff（`BattleScreen.tsx:1766` の useEffect）そのものは生きている**＝**同ブロック内の `grantedStoreWatchers()` 呼び出し側だけが効いていない** | 2026-08-14 続き477＝§7 V-12 の実機検証。`SPDi43-13-E2` を【起】から撃って付与→`WXDi-P12-082`（羽化）の【エナチャージ１】で `energy 0→1` になるのに `lrig_down` が true のまま。**実機シナリオ 2本が赤で待機**（`v12GrantedEnergyChargeTwice`／`v12GrantedEnergyChargeThirdBlocked`）＋**切り分けの対照が緑**（`v12PrintedEnergyChargeControl`＝`WXDi-P11-073-E1` に +2000）| **同経路の他 timing は生きている**＝`ON_SPELL_USE`（`:7057`）と `ON_SIGNI_BANISH_OPPONENT`（`:8964`）の付与 watcher は同じ続き477 で**実機 PASS**（V-12 参照）。⇒ 疑うのは `:1766` ブロック固有の条件＝`ecLrigTop` の取り方・`addedToEnergy.length === 1` の判定タイミング・`entries` を積んだ後の解決経路のいずれか。⚠**この経路は `actions_done` へ書き戻さない**ので、直した直後に《ターン2回》の超過が露出する見込み（B-4 のシナリオが既にある）|
...
- **🆕 セッション（2026-08-14・続き476〜477・Opus 5）＝**§7 実機検証を Codex 委譲で2ブロック消化**＝**V-11 完全決着・V-12 は8本中6本緑**。engine/parser/JSON は**1行も触っていない**（成果物は実機シナリオ14本と、実バグ3件の登録）。ゲート据置（golden 1975・census 831・lint 0 errors/259 warnings）。
  ...
  - **🔴③ 実バグ3件を新規登録**（いずれも**今回のスコープ外なので直していない**）＝**(cxxxi)** ルリグの【起】《ダウン》が誰もダウンさせない（`down_self` の判定・支払いとも `field.signi` しか探さない＝ルリグでは常に -1）／**(cxxxii)** `signi_deploy_power_limit` を戻す箇所が0（count 版は5箇所ある）／**(cxxxiii)** 付与ストアの `ON_ENERGY_CHARGE` watcher だけ恒久 no-op。
  - **⭐④ (cxxxiii) は「範囲を絞って」渡した**＝赤を報告するだけで終えず、**同じ盤面・同じスペル経路で「印刷能力」の同 timing を撃つ対照**（`v12PrintedEnergyChargeControl`）を1本足したところ**緑**＝**中央 diff（`BattleScreen.tsx:1766` の useEffect）は生きており、同ブロック内の `grantedStoreWatchers()` 呼び出し側だけが効いていない**と1ブロックまで確定できた。**次に直す人は原因箇所を探さなくてよい。**
  - **🔑⑤ Codex 委譲＝2連投とも「起案のみ・実行は検証側」で成立**。続き476 で🔴**未申告の既存シナリオ改変**（無関係な `waitForTimeout` の書き換え）を検出→**CODEX_GUIDE §5-22「既存不変は自己申告でなく機械的に証明させる」を新設**したところ、続き477 では**0 deletions＋除去後 SHA-256 一致**を自分から示してきて**1回で定着**。⚠**検証側が訂正した点はすべて「実行しないと決まらない UI の事実」**（スペルは「発動」／`REARRANGE_SIGNI{swap}` は画像クリックで即確定／正面を塞がないとライフクラッシュで停止／同名2体だと非アタッカーを掴む／`【起】コストなし` のラベル衝突で nth 必須）＝**起案フェーズでログ文言と観測点を確定させるのは無理**という前提でスコープを切ること。
  - **▶ 次の一手**＝**Opus 側**：§3 の在庫は **5件**（(cxxxi)(cxxxii)(cxxxiii)＋続き404・408 の2件）。**(cxxxiii) は範囲確定済みで最も安い**＝直すと V-12 の赤2本が緑へ反転し、続けて《ターン2回》の超過（`actions_done` へ書き戻さない経路）が露出する見込み。次点＝§6.4 O-20 の残・O-19。**Sonnet 側**：§7 の未消化ブロック（`V-13` 以降）。⚠**V-11 の未カバー（`deployCountCapOpponent`）を踏むなら V-25 として別立て**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き476〜477（§7 V-11 決着・V-12 6/8 緑）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1975 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（engine/parser/JSON 非改変）。🆕**実機シナリオ＝+14本**（V-11 6本・V-12 8本）＝**緑/赤の内訳が 60緑0赤 → 72緑2赤**（赤2本は §3 (cxxxiii) 待ちで既定 order に置いてある）。🆕**Opusタスク12 の在庫＝5件**。
