...
- 🏁**セッション（2026-08-28・続き702・Opus 5 単独）＝`O-132` 第1バッチ。語彙センサスの較正（実装は1行も変えていない）。**
  ユーザー指示＝**選択肢A（`O-132` を取る）**。
  🔴**これは前進ではなく計器の較正**（PLAN §3 の原則）＝**`src/` と `public/` は1バイトも変わっていない**
  （`git diff --name-only HEAD -- src/ public/` が 0件）。**ゲームの挙動は完全に同一。**
  📊**進捗3計器＝Sheet1 要対応 20→8 / 863（2.3%→0.9%・census フラグ 12→0）｜台帳 残 OPEN 575（据置）｜census 高シグナル 516→491**
  - ✅**11カテゴリ・13キーを追加**＝`GUARD_MAX_LV`/`GUARD_LV`（レベルが `BLOCK_ACTION.actionId` の文字列に埋まる形）／
    `compareToSelf`（`FRONT_SIGNI` の自己比較）／`sourceSharedColorWithSelf`（保護元の色一致）／
    `countFromZone`・`perAllSigni`（数量比例）／`costThreshold`（合計制約）／`STACK_SPELL`（下に置く）／
    `ON_OPP_SIGNI_ATTACK`（アタックトリガー＋triggerScope）／`attackerOwner`（遅延トリガーの主語）／
    `costSubstitute`（コスト側の置換）／`"once":true`（「このターン最初の〜である場合」）／
    `"upTo":true`・`PLAY_FREE_FROM_TRASH` ＋ **空枝 CHOOSE の `extraOk`**（「してもよい」の3表現）。
  - 🔑**消えた25効果は全数を1件ずつ目視した**（`docs/_effect_srctext.json` の由来ブロック × live JSON）。
    **新しく高シグナルへ入った効果は0。** 同じカテゴリに**残った**効果が本当の欠落であることも確認した
    （例＝`シグニの下に置く` の残り8件は全部 `LOOK_AND_REORDER` 単独＝実際に「下に置く」が落ちている）。
  - 🔴**較正の唯一の危険＝キーが免罪符になる**（キーが JSON から消えても census は静かに合格し続ける）。
    ⇒ golden に**「較正キーが live に実在する」トリップワイヤ**を1本追加（25ペア）。
    **空振りでないことも実測済み**＝1ペアを別文字列へ変えると FAIL する。
  - ⚠**実機検証は対象なし**＝アプリのバイト列が変わっていないので、実機で観測できる差が**原理的に存在しない**
    （PLAN §2.3 の最も強い形）。両方向の固定は上の golden が担う。
  - ⚠**`O-132` は閉じていない**＝較正したのは **Sheet1 が指した11カテゴリだけ**。
    残る高シグナル 491 は**偽陽性かどうか未調査**なので、§5.3 の登録票は「第1バッチ消化」に書き換えて残した。

**▶ 次の一手**＝**Sheet1 の残り8枚を1枚ずつ**（census フラグは0になったので、ここからは
`audit` 3（`WX02-020`／`WX05-022`／`WX09-Re07`）・`held` 3（`WX05-025`／`WX06-CB03`／`WX07-029`）・
`partial` 1（`WX04-052`）・`idset` 1（`WX05-021`）＝**別々の検出器が指す8枚**の照合になる）。
⚠**`held`／`partial`／`idset` は「parser の改善が live へ届いていない」フラグ**なので、
**まず `docs/_held_fresh.json` / `_partial_fresh.json` / `_idset_fresh.json` の diff を読む**（CLAUDE.md §O-39）。
機構側を進めるなら **`O-130`**（`ON_OPP_ARTS_USE` の帰結が「そのシグニ」を指せない）か **`O-128` 第2バッチ**。
⚠🔴**Sheet1 が 0.9% になっても「Sheet1 は終わり」ではない**＝フラグの立たない855枚は
**計器が見ていないだけ**。シートを閉じるには**シート限定の意味照合再監査**という別の検出パスが要る。
