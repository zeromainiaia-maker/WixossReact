...
**直近＝2026-09-20＝🏁§5.7 `S-16`＝メインフェイズの探索（配線まで完了・既定オフ・従来と互角）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**探索の本体**＝`cpuSearch.ts`（列挙 `S-15` → 近似適用 `applyCpuMoveSim` → 採点 `evaluateBoard` のビーム）。**1回の呼び出しで1手**＝打つたびに再入して探索し直す。
- 🏁**配線**＝`cpuTurnAction` の MAIN。🔴**実行は人間と同じ `perform*`**（近似適用では打たない）／召喚はループが置く／**扱えない4種**（アシストグロウ・レゾナ・ライズ・ピース）は従来の優先順のまま。
- **入切はポリシーの数値**（`searchWidth`/`searchDepth`・**既定 0＝オフ**）＝実機の挙動は不変（自己対戦の全ログが配線前と一致）。
- 🔴🔑**A/B が「探索は弱い」と出たが、真因は配線のバグだった**＝1回目 72.5%（探索が有意に弱い）→ **行動の回数を数えたら シグニ配置 62→38・【起】 8→0** →
  **探索が選んだ召喚を置いたあと `return` していなかった**（そのまま MAIN が終わる＝**1ターン1体**）⇒ 修正して測り直し＝**50.0% [39.3, 60.7]＝互角**。
  🔑**教訓＝A/B が悪いとき、まず疑うのは評価関数ではなく配線。行動の回数を数える。**
- 🔥**次は `S-21`（探索の目的関数）**＝互角で止まっている理由＝**探索は【起】を1度も撃たず・111手中58手で「打たない」**＝エナを払う手の価値を1ターンの盤面差でしか見ていない。
...
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先攻が勝てない（`S-19`・直近の A/B でも先攻 27.5〜40%）②**A/B の山が型を踏まない**（`S-20`）。🏁先読みの情報漏れは解消（`hideDeckOrder`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4338**）｜**実機** `node scripts/verifyFullMatch.mjs cpu` **PASS**（9ターン / 219手 / 295秒）※この日は4回回して1回目だけ FAIL（見張りのバグ＝同日修正） |
...
- **2026-09-20 時点**（第413バッチ＝🏁§5.7 `S-16` 探索の配線）
  ...
  - 📊**先攻の勝率**＝直近の A/B で **27.5〜40.0%**（`S-19`・ポリシーと無関係に出る偏り）
