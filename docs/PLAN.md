...
- **🆕 セッション（2026-07-11・続き67・Opus 4.8・§3 Opusタスク3 動的比較 lrig相対の残＝WXEX2-25-E3）**＝WXEX2-25-E3（原文「対戦相手のセンタールリグ１体を対象とし、ターン終了時まで、それは『【常】：**このルリグより低いレベルを持つ**あなたのシグニのパワーを－8000する。』を得る」）が引用付与構造を丸ごと落とし `POWER_MODIFY owner:self ALL -8000`（＝**自分の全シグニに-8000する有害な誤パース**）だった。正＝相手センタールリグへ CONT POWER_MODIFY を付与（付与先LRIG視点で owner:self＝相手自身のシグニ・「このルリグより低いレベル」＝付与先LRIGレベル未満）。**既存インフラ活用**＝`GRANT_EFFECT`(target LRIG owner:opponent)→`execGrantEffect` が相手LRIGの `granted_effects` に格納→effectsMap が相手分merge→`calcFieldPowers`(両視点で `applyEffects`)が付与CONTを拾う。**engine唯一の新規**＝CONT POWER_MODIFY パスの `applyDeltaToState`→`matchesFilter` は動的マーカー `levelLtSelf` を無視するため、効果元 `topNum`（＝付与先LRIG）のレベル基準で解決する `resolveContSelfLevel` を新設（levelLtSelf→level.max=hostLv-1／levelGtSelf→min=hostLv+1／参照不能→制限なし）。既存で CONT×levelLtSelf を使うカードは0件＝回帰なし。相手は自ターン中グロウ不可＝付与期間中LRIGレベル不変で静的解決と等価。**表現**＝`manualEffects.ts` に MANUAL 追記→`build:effects`→`heldReview --adopt WXEX2-25`（curated MANUAL化）。**検証**＝golden +2（181・①相手LRIG(Lv4)付与の levelLtSelf が Lv3に-8000/Lv4は不変 ②GRANT_EFFECT→相手LRIGで granted_effects格納）／gates 全緑（**census 1567→1566**・BASELINE更新・lint 0）／regen 同型★0・decompile E3 が引用付与＋レベル比較を構造化。詳細 BUGFIXES 最上部。
  - **前回（続き66・Opus）＝WXEX2-50-E3 owner誤パース是正**（「対戦相手の場に出す」検出→owner opponent・R30発火経路開通）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse**（WXDi-D09-P20／WX24-P1-017／WX25-P3-038＝「「【自】…」を得る」の内側 ability parse）。**(B)Opus＝§3 Opusタスク3 残の動的比較別機構**（デッキ相対 SEARCH＝WDK13-013/WXEX2-28/WXEX2-37/WXK10-033・条件文 WXK08-005・opp/own センタールリグ WXK11-003）。**(C)Sonnet＝§7実機検証の継続**（🆕 **続き66でR30の発火経路が開通**＝WXEX2-50-E3 を発動して相手フィールドにシグニを出し WXK10-022-E1 watcher の ON_PLAY any_opp 発火を確認・シナリオ追加。加えて ON_TARGETED残3枚・R42②・ON_COIN_PAID/ON_LRIG_GROW③④）。**(D)持ち越し：WXEX2-25-E3 の decompiler levelLtSelf描画が「このシグニより」固定（host=LRIGなら「このルリグより」が原文忠実・同型★0維持で機能影響なし・低優先）・WXEX2-25 E1(ON_PLAY any_opp+unless discard)/E2(それぞれレベル異なる欠落)の別課題・WXEX2-50-E3 step2レベル制約未反映・WXK04-003 ボタンラベル表示バグ・WX12-008 exceed-cost timing・SEQUENCE下流「そうした場合」・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
