...
| 🆕(lvi) | **`manualEffects.ts` の override が JSON 側の `cost`/`condition`/`timing` を落として実機で踏み倒しになる 12効果**（2026-07-28・(xxxix) 作業中に Opus が全数検出）。`mergeManualEffects`（`src/data/effectParser.ts:7148` 経由・BattleScreen:670 の `buildEffectsMap` で実行時マージ）は **effectId 単位の完全置換**なので、manual 側に書かれていないトップレベルフィールドは消える。`eff.cost?.energy` は `BattleScreen.tsx:6475` 等で**発動可否とエナ徴収に実使用**されるため実害。**cost 欠落10件**＝WXK08-055-E1／WXK11-070-E1／WD21-009-E1／PR-238-E1／WX16-048-E1／WX16-023-E1／WXK10-008-E1／WXDi-P11-001-E1／WX24-P3-036-E1／WXDi-P05-006-E1、**cost＋condition 欠落**＝PR-204-E1、**timing 欠落**＝WX17-041-BURST。検出器は `tmp_manual_field_loss.mjs` 相当（JSON と manual のトップレベル差分）。⚠ついでに **CODEX_GUIDE §5-16 の「App は JSON を直接 fetch するので manual は実機に届かない」という記述が不正確**（BattleScreen は `buildEffectsMap`→`mergeManualEffects` を通る）＝要訂正 |
