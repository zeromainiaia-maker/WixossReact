...
**Opusタスク12＝未消化の在庫 2件（2026-08-10 続き404 で登録）**。
...
- **🆕 セッション（2026-08-10・続き406・Opus 5）＝§6.4「F-3 効果バニッシュ経路（身代わり置換の execBanish フック）」を消化**。続き404・405 と**同じ形の穴**が3連続＝「判定は実装済みなのに、それを必要とする経路の一部からしか読まれていない」。
  ...
  - **🔑③ 決定論的な選択順を安い順に固定**＝①スタック下のスペル→②手札のスペル→③他シグニの犠牲。**`victimOwner==='opponent'` のときだけ置換**（自分の効果で自分のシグニをバニッシュする「コスト/利得」型を、自動適用で他シグニの犠牲へすり替えない）。
  - **⚠④ `WX14-026`（`lifeCrash`）は engine 対象外にした**＝ライフクラッシュは【ライフバースト】確認フロー（`field.check`）を伴い効果解決の途中に同期的に差し込めない。中途半端に「バーストを飛ばすクラッシュ」を書くより除外が正しい。バトル経路の対話実装が引き続き担当。
  - **🔴⑤ effectsMap 依存をまた避けた**＝`ExecCtx.effectsMap` はスタック解決1経路でしか代入されない（続き296／405 と同じ罠）。victim 側の場トップぶんだけローカル Map を組み、`ctx.effectsMap` →（無ければ）`ctx.cardMap.get(...)?.effects` の順で引く。**後者は `App.tsx` が live JSON を各 CardData に載せているので実アプリでも確実に引ける**（既存の離場置換2本と同じ channel）。
  - **🔴⑥ PLAN の在庫記述がまた古かった（続き402〜406 で6回連続）**＝この項目は「残＝`WX17-075`（`ON_PLACED_FRONT` 任意トリガー・別機構）だけ」と書いていたが、**`WX17-075` はそもそも身代わり側ではなくバニッシュ**トリガー**側**のカードで、`placedFront`／`frontLowerLevelThanSource` として `collectFieldTriggers` に配線済みだった。**投入前に live JSON と engine を実測する**手順が今回も記述より先に正解を出した。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（typecheck / golden **1721→1726**（+5）/ smoke **10686/10686** 全0・SKIP 0 / fuzz 200ゲーム 全0 / census **880 据置** / manual field loss 0 / lint 0 errors・**254 warnings 増減0**）。**live JSON 変更0＝engine 配線のみ**（`npm run regen` 不要）。
  - **⚠⑧ 実機検証が要る**＝**効果バニッシュで身代わりが自動で走るか**（`WX12-024`／`WX10-033`）、**バトルバニッシュは従来どおり対話モーダルのままか**（自動適用に化けていないこと）、`WX14-026` は効果バニッシュでは身代わりされないこと（仕様）。§7 に登録済み。
  - **📋⑨ 次の一手**＝**Opus 側**＝(a)**置換系をまとめて対話化するか決める**（現状は置換5本すべて「してもよい」の自動適用。engine 側は同期的な ctx 変換なので pause を張れず**設計変更が要る**＝単独バッチ）(b)Opus タスク12 の在庫2件〔(cxix) `SPDi43-11-E2` の timing 誤パース／(cxx) ON_ENERGY_CHARGE watcher の usageLimit 未管理〕(c)`collectIncreaseActCost` がトラッシュ起動に未適用（続き403 が残した1件）(d)§6.4 残＝`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査／golden の型網羅。**Sonnet 側**＝§7 の実機未検証UI（続き403 の14枚＋続き404 の3件＋続き405 の2件＋続き406 の1件）と §5c census 文型バッチ。
