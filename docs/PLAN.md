...
- **🆕 セッション（2026-08-10・続き418〜422・Opus 5）＝「所有者反転／コスト取り違え」を計器で追って5波。census 860→854・golden 1753→1769。52効果/50カード是正（うち🔴自傷8件・🔴コスト取り違え19件）**。
  - **🔴① 続き418＝「代わりにそれを＜動詞＞」の per-target 形を parser が意図的に除外**していたため、昇格文が別ステップとして残り `owner:'self'` へ反転＝**自分のシグニをバニッシュ/付与**。PLAN が「UNKNOWN 2件」と書いていた家族の**本体は無言の過剰効果**だった。(A) 盤面状態条件は `CONDITIONAL{then,else}`、(B) 対象プロパティ条件は `SELECT_TARGET_ONLY→STORE→CONDITIONAL{…targetsStored}`。
  - **🔴② (B) を (A) の形で書くと恒久 no-op**＝`LAST_PROCESSED_MATCHES` が対象選択より前に評価され常に偽。**実装途中で踏み、golden の「実行結果」テストで捕まえた**。同じ理由で POWER_MODIFY の値昇格は加算モデル。
  - **🔴③ 続き419＝②を計器化したら別家族**＝「どちらか１つを選ぶ。①…②〈任意コスト〉てもよい。そうした場合、…」で**選択肢が2文構成だと CHOOSE が丸ごと消え**、自傷2件＋コスト消失2件。併せて **`coinCost` を parser が一度も生成していなかった**。
  - **🔴④ 続き420＝`TARGET_AND_DISCARD_HAND` は payload を持たず、engine が手札の最後の1枚を問答無用で捨てていた**。53カードで 捨て札filter脱落23／任意性脱落17／選択不能は全件。正準形 `OPTIONAL_COST{handDiscard}` へ18効果を移送＋ルリグ宣言取りこぼし2件。
  - **🔴⑤ 続き421〜422＝同じ STUB が「幻の手札コスト」でもあった**＝producer が**コストの種類を問わない catch-all**で、原文が**エナゾーン/自己トラッシュ/下から**でも同じ STUB を返す。engine は名前どおり手札を捨てるので**①原文のコストが一切支払われない②原文に無い手札1枚が消える**（19効果）。`energyTrash`／`underAnySigniTrash`／`OPTIONAL_TRASH_SELF` へ載せ替え、**幻は 19→1件**。🔴`WXDi-P14-060-E1` は「相手に捨てさせてもよい」なのに**自分の手札が落ちて相手が1枚引く**真逆の効果だった。
  - **🔴⑥ 自分が続き421 で入れた `underAnySigniTrash.filter` は死フィールドだった**＝`OptionalCostSpec` は **JSON payload 用（`types/effects.ts`）と解決後の runtime 用（`execUtils.ts`）で別々に宣言**されており、parser のキャスト書き込みは**どちらの型にも無いまま**逆翻訳にだけ出ていた＝**C群（表示だけの穴）を自分で作っていた**。両方の型に足して pay/afford の双方で honor。**golden は構造ではなく「支払い可否（runtime）」で固定**＝構造アサートでは死フィールドを検出できない。
  - **⭐⑦ 5波を貫く3つの型**＝(a)**engine にあるのに parser が生成しない**（`OPTIONAL_TRASH_SELF`/`coinCost`/`handDiscard`）(b)**parser が出すのに engine が別の意味で読む**（catch-all STUB＝`TARGET_AND_DISCARD_HAND`）(c)**どちらの型にも無いのに逆翻訳にだけ出る**（死フィールド）。**payload 無し／広すぎる id の STUB を見たら、①engine の受け皿を grep ②producer の条件を読む ③型が2つに割れていないか確かめる**。
  - **⚠⑧ 引き継ぎの見積もりは着手前に必ず実測し直す（2回連続で外した）**＝続き420 で「好きな枚数が最大クラスタ」→実際5件、続き421 で「残19件は【起】コスト行＝対象外」→**該当0件**で、代わりに幻コストが埋まっていた。
  - **⚠⑨ 新しい payload キーを足したら逆翻訳も併せて足す**＝`energyTrash`／`selectionConstraint`／`OPTIONAL_ACTIVATE` が逆翻訳に出ず「コストを支払ってもよい」「[STUB:…]」に潰れていた＝**原文照合でコスト取り違えを見つけられない状態**で、幻コストが長く残った一因。**計器自身の穴を疑うこと。**
  - **⚠⑩ 変換系は必ず全件の対応表を印字して原文と突き合わせる**＝続き420 で spec→filter の取りこぼし検査を空 filter の早期 return より後ろに書いており、カード名指定が落ちて「手札の任意の1枚」に化けていた。
  - **✅⑪ ゲート**＝`npm run gates` 全緑（golden **1769（+16）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **854** / 同型★ **0**（265群）/ lint 0 errors）。**要実機検証**＝続き420 の手札コスト UI（18効果）＋続き421〜422 のコスト徴収先変更（19効果）＝§7 に登録済み。
  - **📋⑫ 次の一手**＝**Opus 側**＝(a)🔴**`WX25-CP1-092-E1`（幻コスト最後の1件）**＝「３体まで対象とし、**それらのシグニ１体につき**エナ1枚」＝**比例コストの語彙が無い**うえ本体も `POWER_MODIFY{owner:'any',count:1}` で「3体まで／それら」が落ちている＝**コストだけ直しても正しくならない**ので `OptionalCostSpec` の per-target-count 倍率と1件で扱う (b)**残 A群17件の変換**（「好きな枚数」5件＝可変枚数＋「この方法で捨てた枚数につき」倍率／`handDiscardGroups` 2件／《トラップアイコン》filter 未対応1件 ほか）(c)🔴**`WX25-P3-014-E1`**＝`BOUNCE{owner:'self'}` の自傷 (d)**§6.4 A群の着手可能2件** (e)**`WX12-010`＝強制アタック機構**。**Sonnet 側**＝§7 実機検証（**続き420〜422 のコストUI を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き418〜422（所有者反転／コスト取り違えを計器で5波）後 最新値（本行が直近の正）**：census **854**（860→854・`BASELINE_HIGH` 更新済み）、golden **1769（+16）**、`MANDATORY_SUSPICIOUS` **2**（据置）、held **110カード／48群**、同型★ **0**（265群）、smoke **10686/10686** 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors/**256 warnings**、manual field loss 0、golden 型カバレッジ **128/128**、UNKNOWN **36ノード/34カード**、🔴幻の手札コスト **1件**（19→1）。母数＝効果カード **5975**／効果 **10683**／MANUAL効果 **1118**。
...
- ~~**`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査＋`verifyEffects` の誤検出除外**~~ **✅2026-08-10 消化（続き408）**＝**33件を1件ずつ仕分けたら真バグ3件／誤検出30件**だった。誤検出6系統をルールで潰し、**`POWER_VALUE_MISMATCH` 6→0／`MILL_COUNT_MISMATCH` 3→0／`OPTIONAL_SUSPICIOUS` 2→0／`EFFECT_TYPE_MISSING_CONTINUOUS` 15→1（残1＝真バグ `WX12-010`）／`verifyEffects` のタイミング 4→0**。**計器の精度改善が本体**（続き407 の教訓「過剰報告する計器は無いより悪い」の実践）。詳細は BUGFIXES 2026-08-10（続き408）。
  ...
  - **📋 残 2件**＝`WX12-010`（別件＝`EFFECT_TYPE_MISSING_CONTINUOUS` の真バグ「対戦相手のシグニは可能ならばアタックしなければならない」＝**強制アタック機構が未実装**）／`WXDi-CP01-023`（**置換**「代わりに…してもよい」＋ CONTINUOUS で受け皿なし＝§6.4「置換5本の対話化」とセット）。
