...
- **🆕 セッション（2026-08-02・続き330・Opus 5〔**codex-work 実装／Claude 指示・検証・簿記**〕）＝タスク12**(lxxxii) 第1〜5波で🏁**残0クローズ**（ユーザー指示「codex-work に(lxxxii)を投げる」→「`WD21-008-E1` も」→「`WXK08-002-E1` も」→「続けて投げる」×2）。**計8効果 是正＋4件が誤登録と判明**＝残10→**0件**。**差し戻し2件**（第2波・第5波）。
  - **第5波（残1→0・クローズ）＝`PR-Di013-E1` の「選んだ能力を得る」を実働化**。①が `DRAW` の**即時実行**になっており「アタックしたとき引く」が「今すぐ引く」だった＋②③が**永久に選べない**（engine の選択肢生成に分岐なし）。⇒ 3枝を `GRANT_LRIG_ABILITY`（`ON_ATTACK_LRIG`／`once_per_turn`／`UNTIL_END_OF_TURN`）へ是正。**先例 `WXDi-P05-032-E1`（ほぼ同文）に形を揃えた**。「追加コストで2つ選べる」は `CHOOSE.additionalCostChoose` 1本追加＝既存兄弟4本と同形。**波及先は live 全数で1効果のみ**。
  - **🔴第5波の差し戻し＝「実装は正しいが実機に一切届いていない」**。codex は `manualEffects.ts` にだけ書き、**ゲートは全緑だった**が実機は no-op。理由は①**実機は effects JSON を直読み**（`App.tsx:126`。`mergeManualEffects` の呼び出し元は build 経路と verify だけ）②**`build:effects` の PRESERVE**（`buildEffectsJson.ts:158`）が**既存 MANUAL カードを丸ごと温存**するので JSON に出ない（build を回しても md5 不変を実測）③**golden/smoke は merge を通す＝実機と違う経路**だから緑になる。⚠**第3波で同手順が通ったのは、あちらが既存 JSON に MANUAL を持たなかったから**＝**既存 MANUAL の有無で結果が正反対**。⚠**気付けた決め手は codex 報告の「per-effect diff changed 0」**＝JSON を変えるべき修正での changed 0 は「届いていない」証拠。⇒ JSON への外科パッチ＋**「JSON に入っていなければ落ちる」golden** を要求して是正（changed 0→**1**）。
  - **第4波（残7→3）＝「追加コストを払うと選択数が増える」5効果を実測で仕分け**。⚠**在庫の見立て「新語彙が要る・1バッチ丸ごと必要」は誤りだった**＝engine は `CONDITIONAL_MULTI_CHOOSE_BY_CENTER_LEVEL_GTE`（`execStubPart1.ts:2444+`）が**実行時に原文を解析して選択数と支払いCHOOSEを動的に組む**設計で既に実装済み、かつ `choose_count` 上書きの兄弟が4本（`recollect`/`recollectArts`/`betChoose`/`preUseVirusChoose`＝`effectExecutor.ts:3694-3723`）。**素の CHOOSE に置き換えていたら動いているものを壊していた**。⇒ **新語彙0本・engine-only（生成JSON 完全不変）**で2件を実働化：**`WD23-044-EA-E1`**（青1を払って何も起きない恒久 no-op＝`CONDITIONAL_ALTERNATE_EFFECT` が別文型専用の実装だった）／**`WX26-CP1-024-E1`**（支払っても `self_optional_effect_taken` が立たず「代わりに2つ」が効かない＋エナのトラッシュ自体が走っていなかった）。差し戻し0・是正0。
  - **⛔`SP26-005-E1`／`SP38-004-E1` は元から完全動作＝在庫への誤登録**（払うと count=2・選択肢2個）。**在庫から外した**。⚠**教訓＝「壊れている」と登録された在庫は、engine 直叩きの実行トレースで裏を取るまで信用しない**（10件中2件＝2割が誤登録）。トレースは使い捨て1本（`executeEffect`→`resumeChoose` を手で回す）で数分。
  - **第4波の honest defer 1件＝`PR-Di013-E1`**（3択のうち候補1個しか出ず、かつ原文「センタールリグは選んだ能力を得る」なのに `DRAW` を**即時実行**）。3枝を `GRANT_LRIG_ABILITY`（ターン終了時まで）として組む必要があり、**②③だけ即時実行を足すフェイク実装は入れなかった**（足すと過剰実行が2つ増える）。
  - **第4波の波及範囲は狭く固定**＝`OPTIONAL_TRASH_ENERGY_CLASS` は40効果が共有するが、新分岐は「後続が `ARTS_EXTRA_COST_CONDITION`」に限定し、**同 STUB を持つ効果は live 全数で1件のみ**（実測）＝残り39効果は経路に入らない。`CONDITIONAL_ALTERNATE_EFFECT` も**1件のみ**。
  - **第3波（残8→7）＝`WXK08-002-E1`（歌舞熱曲）を段階1で着地**。live は CHOOSE が消えたうえ①の断片だけが残り「**source 不明の `ADD_TO_FIELD`（何も出ない）＋自分のシグニ1体を強制バウンス**」＝赤3無1 を払い得の完全な壊れ方だった。⇒ `CHOOSE{1/3}` を載せ、②を既存語彙 `BANISH{owner:'any',count:2,upToCount:true,totalPowerMax:10000}` で忠実化。**①③は新機構待ちのため明示 defer だが、fresh をそのまま採ると「自分のシグニをバニッシュ／自分のシグニの能力を奪う」過剰実行になる**ので **STUB で no-op 化して害を消した**（指示書で「defer する枝は元のまま残さず STUB に落とせ」を最上位制約にした）。差し戻し0・Claude 是正0（簿記2点を追記）。
  - **⚠第3波の投入前実測**＝fresh は既に CHOOSE を開いていた（第1波/第2波の緩和の成果）＝**残っていたのは各枝の質**。退化5点のうち3点が過剰実行（①即時バウンス／②`owner:'self',count:1`／③`REMOVE_ABILITIES{**SIGNI**}`＋付与欠落）。**必達を「CHOOSE を載せる＋②だけ正しく」の1点に絞った**ので codex は段階2/3 を honest defer で正しく着地。既存語彙の在庫表（`totalPowerMax`／`ADD_TO_FIELD.source={TRASH_CARD}`／`INSTALL_DELAYED_TRIGGER`／`GRANT_LRIG_ABILITY`）を指示書に載せ、新語彙0本で済んだ。⚠**`SET_LEVEL_1` は parser が生成するだけで engine に分岐が無い**（`parseSentencePart1.ts:2548` が生成元・engine は `SET_LEVEL_RANGE` しか解さない）＝①実装時の障害として登録。
  - **⚠第3波は `parseStatus:'MANUAL'` 温存**（`manualEffects.ts`）＝held 265→264 は「解決」ではなく「curated 固定」。**①③実装時は MANUAL を外して parser へ戻す**。②の文型「合計N以下になるように**M体まで**」は CSV 全数で本カード1件のみ＝parser 一般化の波及先0（実測）なので取り逃しは無い。engine 側は `totalPowerMax` が `target.count` を無視する穴を是正したが、**既存4件は全て `count:"ALL"` で非影響**（`typeof count === 'number'` ガード＝直下 `totalLevelMax` と同形）。
  - **第2波（残9→8）＝前置に実アクションを含む形の救済**。⚠**在庫は1件だったが実測すると同型3件**＝`WD21-008-E1`（＜宇宙＞・《赤×2》）／`WX20-003-E1`（＜精械＞・《白×2》）／`SPK01-07-E1`（キー・《緑×3》）。前置は3件とも `TRASH`（or `STUB{TRASH_OWN_KEY_OPTIONAL}`）＋`CONDITIONAL{IS_MY_TURN}{then:STUB{ARTS_COST_REDUCTION_BY_EFFECT}}`＝第1波の「素の STUB のみ」条件を2重に外していた。**1件だけ直すと同型★が割れる**ので3件そろえて着地。許可する木を狭く固定（`BET_MECHANIC`・`GRANT_CHOSEN_ABILITY*` は入れない）。`WD21-008-E1` の `optional:true` 脱落も補完。defer 1件＝`WX20-007-E1`（枚数比例のコスト減語彙が無い）。
  - **⚠第2波の投入前実測で在庫メモの母集団が誤りと判明**＝「先頭形258＋文中形46・脱落10」→実測 **先頭形157／文中形210／CHOOSE 無し35**。しかも**35件の大半は壊れていない**＝約17件は `BET_MECHANIC`（9件）や `GRANT_CHOSEN_ABILITY` 系など **engine の STUB が実行時に原文を解析する設計**（`effectParser.ts:5753-5768`）＝**素の CHOOSE に置き換えると退化する**。件数つき分類表を「⛔触らない群」として指示書に固定した。
  - **🔴 Claude が差し戻した1件＝codex が新規に作った過剰実行**＝`WX20-003-E1` ②「**対戦相手のターンの場合**、ターン終了時まで、対戦相手のセンタールリグは能力を失う」の**条件が丸ごと脱落**。codex は「能力を失う」汎用枝が対象を SIGNI 固定で組む誤りを `REMOVE_ABILITIES{LRIG,opponent}` へ直した（これは正しい）が、専用枝が**裸で return** したため先頭条件節が捨てられた。**元からのバグではなく、対象を正しく直したことで初めて実効性を持ったぶんが過剰実行になった**のが要点。⚠**`TURN_OWNER` は `IS_MY_TURN`（常時真プレースホルダ）と違い engine で実評価される**（`execUtils.ts:1229`＋`:39 isOwnerTurn`）＝no-op では済まない。材料は全部既存（語彙 `:2657`／持ち上げ `liftChoiceOptionCondition :3353-3368`）だったので**仮説・engine 実評価の根拠・安全側の直し方・golden assert・「既存23効果が減らないこと」の検証**まで書いて差し戻し、最小修正で是正（`TURN_OWNER` 保持 **23→24**）。
  - **第1波（残10→9）＝文中形 CHOOSE の救済閾値を緩めて `WXK09-004-E1` を是正**。差し戻し0・Claude 是正0。
  - **真因は3点の重なり**＝`以下のNつからMつを選ぶ` が**文中**に来ると文分割フィルタ（`effectParser.ts:5702-5706`）がヘッダと①②…を**両方**捨てて CHOOSE が丸ごと消える。既存の救済規則（`:5730-5743`／2026-08-01 `WX09-Re02` 由来）は「前置が `ARTS_COST_REDUCTION_BY_EFFECT` STUB のみ×**2本以上**」限定で、本件は ①前置**1本だけ** ②marker id が **`ARTS_COST_REDUCTION_BY_CENTER_LRIG`**（⚠**在庫メモの記載は誤りだった**＝投入前実測で判明） ③前置が単一 STUB だと `parseActionText` が SEQUENCE を返さず `prefixSteps` が空、の3点で外していた。修正は**この3点を緩めるだけ**（カード番号分岐なし・「代わりにNつ選ぶ」除外ガードは維持）。
  - **ゲート最終値（2波通算）**＝golden **1305→1307**・census **1289 据置**（⚠4効果とも高シグナル語彙に**載っていない**＝実働化しても数字は動かない。`BASELINE_HIGH` 変更なし）・smoke **10679** 全0・SKIP0・fuzz 全0・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **258→265枚/109→116署名**（第2波での増減0）。live per-effect 差分は第1波 **changed 1**／第2波 **changed 3**、いずれも added 0・removed 0・outlier 0・兄弟効果変更0（`BET_MECHANIC` 系・`GRANT_CHOSEN_ABILITY*` 系の changed も 0）。⚠**held は codex 報告の 266 が誤りで実測 265 が正**（258+7=265 で内訳と整合）＝第2波で訂正した＝**codex の集計値も鵜呑みにせず `node scripts/heldReview.mjs` で数え直すこと**。
  - **🔴 held +7カードは「採用禁止」の在庫**＝`WXK05-002`/`WXK05-004`/`WXK07-002`/`WXK08-001`/`WXK08-002`/`WXK08-003`/`SPK01-14`。parser 緩和が **MANUAL/PARTIAL 温存中のカードの fresh にも波及**して CHOOSE を組んだもので **live は非影響**だが、`WXK08-002` の fresh には退化が4点ある（手札回収欠落／場出し source 欠落／`owner:self,count:1`（本来は相手・合計10000以下・2体まで）／引用能力付与欠落）。**再収穫サイクルで無検証採用すると退化が入る**＝1カードずつ原文照合してから採る。
  - **⚠ 残った過小実行を明示**＝`WXK09-004-E1` の選択肢②は原文「対象のシグニ1枚を**手札に加え**、対象のシグニ1枚を場に出す」の**前半が落ちて `ADD_TO_FIELD` だけ**。複数対象・複数行先の**別 parser バグ**で、変更前は CHOOSE ごと消えていたため**純増ではあるが完全ではない**。
  - **⚠ 投入前実測がそのまま効いた**＝10効果の原文・live action・`parseStatus` を先に全数実測して指示書に埋めたので、codex は「4グループに割れる（A 前置1本／B 前置に実アクション混在／C 選択数変更型／D 別バグ同居）」を正しく認識し、**必達 A-1 だけ着地・残り9件は honest defer** に着地した。C群を素の CHOOSE に落とす退化は起きていない。
  - **次の一手**＝**Opus**＝(lxxxii) **残7件**（`WXK08-002-E1` は第3波で段階1着地・②のみ実働）。⚠**`WXK08-002` の段階2/3 は別在庫として生きている**＝③引用AUTO（《ターン２回》＋ガード時発火）のルリグへの複合付与／①トラッシュ場出し＋ターン終了時の同一性追跡つき帰還＋未実装 `SET_LEVEL_1`。**C群5件は新語彙が要る**（支払い結果で `choose_count` が変わる条件つき CHOOSE ＋ engine/UI 配線）＝1バッチ丸ごと必要。**D群2件**は別在庫同居で低優先。⚠**別在庫として積んだ2件**＝`WD21-008-E1` ③の複合回収語彙（`UNKNOWN` のまま＝選べるが no-op）／`WX20-007-E1` の枚数比例コスト減。**Sonnet**＝§7 実機検証（続き298〜330 の全件が実機UI未検証。**今回の4効果は CHOOSE の選択UIが新たに出るので優先度が高い**）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-02 続き330 第5波後 最新値（本行が直近の正）＝🏁(lxxxii) 残0クローズ**：golden **1311→1312**（+1＝`PR-Di013-E1` を **`manualEffect()` 経路と JSON 生読み経路の両方**で同一検証。スペルの発生源を `field.check` に置き、**①選択直後は手札不変／ルリグアタック時に初めて1枚引く**という即時実行と能力付与を区別する assert、`once_per_turn`、2回目不発火、ターン終了時消滅、支払い時 count=2／未払い時 count=1、2枝同時選択で両方付与、フラグ消費まで固定。`withSavedCursor` 済み）、census **1289 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0、held **264枚／115署名 据置**。live per-effect 差分＝全5ファイルで **changed 1（`PR-Di013-E1`）**／added 0／removed 0／outlier 0。**新語彙 1本＝`CHOOSE.additionalCostChoose`**（保持効果は live 全数で1件。engine 変更はすべて同フィールドのガード内＝既存 CHOOSE・既存 `OPTIONAL_COST` は分岐に入らない）。⚠**PRESERVE 保護カード（既存 JSON が MANUAL/PARTIAL）は `manualEffects.ts` を書いても JSON に出ない**＝外科パッチが要る（第5波の差し戻し理由）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-02 続き330＝task12(lxxxii) 第5波後＝🏁残0クローズ）**＝golden **1312**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0（265群）・census **1289**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **264枚／115群**（⚠うち6枚は採用禁止在庫＝§3 (lxxxii) 行）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫**＝現存は **(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／🆕(lxxxi)** の5件。**(lxx)／(lxxviii)／(lxxxiii)／🏁(lxxxii) は 2026-08-02 に残0クローズ**。それ以外の残0在庫の完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md) の各整理節。**🏁タスク16 も残0クローズ**。**Sonnet の主力は タスク1（§7 実機検証）**＝未検証UIの単一 worklist は §7 に集約。⚠**§6.3 H と続き298〜Batch F の全件が UI 未検証**。
