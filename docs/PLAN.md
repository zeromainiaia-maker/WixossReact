...
- **🆕 セッション（2026-07-17・続き183・Codex 実装／Opus 4.8 検証・簿記・タスク12(xxxiii)(xxxiv) 消化＝**完了**）**
  ...
  - **(xxxiv)＝「コストか効果によって場からトラッシュに置かれたとき」の `fromFieldByCostOrEffect` を parser で emit**。CSV exact phrase で15枚を機械特定。既存フラグ持ちは MANUAL 2枚のみで残13枚はバトル/リムーブ等ルール処理でも過剰発火していた。parser は主語を剥がす前の全文で句を検出し `fromZones` と合成。engine は self に加え any_ally/any_opp watcher へ同ゲートを追加。**派生で decompiler の G204 分岐が ally 主語を「このシグニ」へ上書きする退化を発見・修正**（組み立て済み scope 主語へ原因句だけ差し込む形へ）。全 leaf diff はフラグ追加のみ・同型★0。
  - **⚠採否は「ゲートを殺したら赤くなるか」で確認**（続き182 の教訓を踏襲）。golden 追加3件は parser 規則・self/ally engine ゲート・any_opp watcher usageLimit をそれぞれ殺すと赤になる境界を分離してある。
  - **次の一手**：Opus＝タスク12在庫の残（(i)(ii)(iii)(viii)残(xi)(xii)(xxiv)残(xxx)(xxxi)＋**(xxxv)**＝(xxxiv) の近傍表記で未ゲートの兄弟（「効果によって場から」10枚は `byEffect` が `collectTrashTriggers` で未評価＝実過剰発火・優先度高／(b)WXDi-P02-037・(c)シグニの下から3枚・(d)any_opp `triggerFilter` 未評価）。Sonnet＝新 timing の §7 driver（続き178/179/180分）＋**続き181/182/183 で発火条件が是正された any_ally watcher の実機確認**（ON_BANISH 16効果＝特にルリグ watcher WX22-011-E2／ON_TRASH＝WX24-P1-015-E1 等／WDK08-L01-E1）。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 2001【効果単位】**（2026-07-17 続き182 更新＝タスク12(xxxii)＝ON_TRASH/ON_BLOOD_CRYSTAL_ARMOR の any_ally scope 脱落の是正で 2003→2001・実数更新。続き181 でタスク12(vi-4)＋ON_BANISH any_ally scope 脱落の是正で 2016→2003。続き180 でタスク16[C]の5機構JSON採用により 2019→2016。続き179で timing[B]第2弾の10効果是正で 2027→2019。2026-07-13 続き109 で判定粒度を「カード単位」→「効果単位（effectId）」へ切替。旧カード単位の 1447 とは**計測仕様が違うので比較不能**）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。**前提＝`docs/_effect_srctext.json`（`npm run build:effects` の副産物）が最新であること**（無ければ census は exit 1）。明細 `docs/_vocab_census.txt`・消化の入口は `npm run census:clusters`（§5c）。**切替の根拠と計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109。**
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 410・smoke/fuzz 全0・同型★0・census 2001。
>
