...
- **🆕 セッション（2026-07-09・続き58・Sonnet 5・§4タスク(F) §7実機検証の継続＝ON_OPP_POWER_DECREASED）**＝PLAN §7 R46（毒牙・WX13-036/WXEX2-52）を `verifyBattleDrive.mjs` 新シナリオ `oppPowerDecreased`（WD11-013→WX13-036）で検証した結果、**❌FAIL＝ON_SIGNI_FROZEN（R38・続き40/41）と同型の実バグを発見**（未修正・観測記録のみでOpusへ引き継ぎ＝PLAN §3運用ルール「発見したバグの修正自体はOpusに回す」に従う）。**原因**＝`collectPowerDecreaseTriggers`（`triggerCollect.ts:900`）は`BattleScreen.tsx:3765-3789`の中央diff（`resolveStackNext`内）にしか配線されておらず、`handleEffectInteraction`のpendingEntriesブロック（4384-4436・`collectFreezeInline`等5種のinline collectorが既にある）には入っていない＝SELECT_TARGETで完結する単体対象POWER_MODIFY（WD11-013等の大半のケース）でwatcherが無発火。**修正方針**＝`collectFreezeInline`と同型の`collectPowerDecreaseInline`を追加する横展開（新規機構ではなく適用漏れ・低リスク）。`order`配列には追加せず除外（既存スイート全緑を維持）。詳細はBUGFIXES続き58・PLAN §6.3。**検証**＝`node scripts/verifyBattleDrive.mjs oppPowerDecreased`単体実行でFAIL再現・typecheck緑（engine未変更のためgates再実行不要）・`git status` clean（auto-commit済み）。**次の一手＝(A)Opus＝続き56で発見した EQUALIZE_ENERGY owner欠落／EXILE owner反転／duration反転／triggerScope欠落の4系統×8枚の原因調査**（現行parser規則のどこかで owner/duration/triggerScope が後段の汎用ルールに上書きされている疑い）。**(B)Opus＝effect-restriction（配置数制限）**（WXK11-074等・§6 BLOCK機構・実機検証必須）。**(C)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(D)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(E)Opus＝§5c census条件節の残**（ARTS_USED拡張・「代わりに」WX25-P2-068/070・「あり」複合条件WXDi-P11-048）。**(F')Opus＝🆕 ON_OPP_POWER_DECREASED resume経路取りこぼし修正**（本セッション発見・`collectPowerDecreaseInline`追加＋`oppPowerDecreased`シナリオ単体再検証→`order`復帰）。**(F)Sonnet＝§7実機検証の継続**（R30-R45残＝ACCE_ATTACH/EXCEED_COST/ENERGY_TO_TRASH/CHARM_TO_TRASH等・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up項目・§7既存リスト）。**(G)持ち越し：SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
