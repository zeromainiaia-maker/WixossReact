...
- 🏁**セッション（2026-08-29・続き714・Opus 5 単独）＝§5.3 `O-76` クローズ ＋ `O-77` 第2バッチ。2つの catch-all を空にした（17効果）。**
  ユーザー指示＝**「O-76、O-77 を行う」**。
  📊**進捗3計器＝Sheet1 要対応 0 / 863（据置）｜台帳 残 OPEN 558（据置）｜census 高シグナル 520（据置）**
  gates 全緑（**golden 2970→2972**・smoke 10702 全0・fuzz 全0・lint 260 据置・同型★0・
  `census:enginetext` 141 据置）。全文は BUGFIXES.md 冒頭。
  - ✅**裸の STUB（＝id が嘘をつく catch-all 流入）を両方 0 にした**＝
    `LOOK_OPP_LIFE_TOP` **27→18効果**（真の穴 9→**0**）／`LRIG_UNDER_CARD_OP` **10→2効果**（真の穴 8→**0**）。
    残っているのは**全部 payload つき＝正当な用法**。⚠登録票の `O-76`「11効果」は実測 **9** だった。
  - 🔴**live の過剰実行を1件発見して直した**＝`WXDi-P04-045-E1`「手札からスペル2枚を公開して**もよい**。
    そうした場合、カードを1枚引く」が、**何も公開しないまま自分のターンなら必ず1枚引いていた**
    （STUB が payload 無しの no-op ＋ `CONDITIONAL{IS_MY_TURN}` が素通り）。
    ⚠**`LRIG_UNDER_CARD_OP` と違いこの id にはコスト先取りが無い**ぶん、**帰結だけが走る**形だった。
  - ✅**受け皿があった2文型は typed へ**（`REVEAL{optional}` ／ `TRANSFER_TO_DECK{TRASH_CARD, opponent, top}`）、
    **残り15文型は honest な `DEFERRED_*` へ**（`miscStubMap` に**原文の帰結まで**記載）。**新しいアクション型は0。**
  - 🔑**「裸の STUB が1つだけ」のときにしか触らない**（fail-closed）＝`WXDi-P08-008-E2` は
    **payload つきの「公開する」を残したまま**「入れ替えてもよい」だけを defer できた。
    ⚠**golden の最初の assert（「その id が1つも残っていないこと」）は強すぎて赤くなった**＝
    「**payload の無い裸の id** が残っていないこと」へ直した（テストが過剰な assert を捕まえた）。
  - 🔴**逆翻訳の嘘を1件直した**＝`REVEAL{HAND_CARD}` のレンダラが `filter.cardType` を無視して
    **常に「シグニ」**と書いていた（`WXDi-P04-045-E1` が原文「スペル」と食い違い）。名詞を payload から描く。
  - 📌**live A/B＝実体変化 18ちょうど・outlier 0**。うち `WXDi-P00-037-E1` は**私の変更ではなく**
    兄弟効果がカード単位採用で連れられたもの（stale な `costUnparsed` が落ちただけ・action は同一）。
  - 📌**踏んだ落とし穴＝held の読み値が stale**（§CODEX_GUIDE §6 の但し書きそのもの）＝採用直後は
    **92→106（+14）**に見え、その14件は**いま採用したカードそのもの**だった。
    `build:effects` → `heldReview` を回し直すと **90**（−2）。**報告前に必ず再生成する。**

**▶ 次の一手**＝🆕**`O-135`（§5.3「取る順」1位・回帰が広い）**＝インライン収集を族ごとに足す。`O-138` の前提でもある。
⚠**規模が「大」なので、着手前に母集団と回帰範囲を実測してから切ること**（この2巡は登録票が2回とも実測とズレた
＝`O-77` は「過剰実行は止めてある」が誤り、`O-76` は 11→9）。

**軽い巡を挟むなら**＝`O-144`（46効果・**機構は実装済みで配線だけ**＝`manualEffects.ts` 側に `reorder` を足す）
か、§5.2 Sheet2 バッチ2（残 **107件**・**1カード1 finding の HIGH** から10件）。
...
|---|---|---|---|---|---|
| **1** | **`O-135`** | 回帰が広い | インライン収集を族ごとに足す | **大** | `O-138` の前提でもある。⚠**着手前に母集団と回帰範囲を実測してから切る**＝2026-08-29 の2巡は**登録票が2回とも実測とズレた**（`O-77` の「過剰実行は止めてある」は誤り／`O-76` は 11→9）。 |
| 2 | **`O-144`** | **46効果** | 🔑**機構は実装済み・配線だけ**（`manualEffects.ts` 側に `reorder` を足す） | 小 | `O-51` の並べ替え対話が MANUAL/PARTIAL で凍っている分に届いていない。**退化ではない**（従来どおりの挙動）。**`manualEffects.ts` を触る他バッチで自然に減る**ので、着手前に数え直す |
| 3 | **`O-140`** | **5効果** | 「総量を割り振る」機構の新設 | 中 | `O-80` 残の最有力＝**いまが過剰実行**（「合わせて－20000」が相手3体それぞれに満額＝計 －60000）。⚠**受け口は `O-51` の分割UI と同じ**なので流用できる見込み |
