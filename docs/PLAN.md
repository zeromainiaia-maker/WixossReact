...
- **🆕 セッション（2026-07-04・zerom・続き16）＝語彙センサス§5c 凍結バッチ＝除去系の過剰効果7枚是正（census 529→522）**＝続き15で新設した `npm run census` の高シグナル「凍結状態フィルタ」20枚から、**バニッシュ/バウンス/デッキ下の対象フィルタ脱落**7枚を系統是正（原文「凍結状態のシグニを対象」→JSONは全シグニ＝過剰効果）。**根本原因＝parser のインライン target ビルダー（BOUNCE/BANISH-all/TRANSFER_TO_DECK）が `parseSigniTarget` と違い状態フィルタを落としていた**→共通ヘルパー `parseFrozenFilter` 新設＋4ビルダー配線（デッキ下は `parsePowerFilter` も欠落→追加）＝parser を先に正して curated 7ノードに `isFrozen`（+P16-075 は powerRange{max:3000}）をパッチ＝**乖離を作らない**（held 25 不変）。engine の matchesFilter は isFrozen 実装済＝配線不要。残＝WXEX1-02-E1（REMOVE_ABILITIES count/filter）・WXDi-P02-065-E1（count 2 vs 1）・WXDi-P01-003（アーツ本体欠落）＝§6.3 機構待ち。census 残13は条件/トリガー/CONT 型で別パス（次バッチ）。typecheck緑・golden 113・smoke/fuzz 全0・同型★0・逆翻訳原文一致。詳細 BUGFIXES 上部。**次の一手＝§5c の残バッチ（ダウン/アップ66・パワー閾値84・レベル閾値90）を同パターン（parser 状態/閾値ヘルパー→curated パッチ）で消化。**
...
- **🆕 語彙センサス（過剰効果＝フィルタ脱落の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 522枚（2026-07-04・初回529→凍結バッチ7枚是正・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。凍結の残13は条件/トリガー/CONT型（別パス）。
...
### 5c. 語彙センサス529枚の系統別消化（2026-07-04新設・過剰効果＝フィルタ脱落バグ）

**目標＝`npm run census` の高シグナル529枚を系統別バッチで0へ逓減。** 過剰効果（フィルタ脱落で対象が広がる・ゲームを壊す側）は behavior-audit の無変化キューに掛からない別種のバグ母集団（発見経緯は §4 続き15）。

- **優先順**＝(1) BANISH/BOUNCE/TRASH の対象フィルタ脱落（凍結20・ダウン/アップ66・パワー閾値84・レベル閾値90）＝除去系の過剰効果が実害最大。(2) 保護/付与系（同一性46・共通色66）。(3) 語彙自体が無い系統＝最上級（8枚・`TargetFilter` に `superlative:{key,dir}` 新設＋候補収集側の相対絞り込みが要る）・動的比較の残36・合計制約27 は §3「機構実装の型」で新語彙＋engineセット実装。
...
- **凍結フィルタ脱落の残テール（2026-07-04・続き16・除去系7枚は是正済）**＝(a)WXEX1-02-E1（REMOVE_ABILITIES CONT が count:1・filter無し＝「対戦相手の凍結状態のシグニは【常】【自】を失う」＝全frozen対象＋能力種別限定が要る・CONT収集器層）。(b)WXDi-P02-065-E1（count:2 だが原文「1体」＝「2体以上ある場合」を count に誤取り込み＋「≥2 frozen存在」条件も欠落）。(c)WXDi-P01-003（アーツ本体「凍結2体までトラッシュ」が丸ごと欠落し【使用条件】のみ）。census 残13（条件/トリガー/CONT型）と併せて次の凍結精密化バッチで消化。
