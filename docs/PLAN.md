...
**直近＝2026-09-22＝🏁**公開していないデッキサーチの札名が共有ログに出ていた**（§5.3 `O-537`＝`V-286` の最後の1系統・全文は [BUGFIXES.md](./BUGFIXES.md) 第466バッチ）
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4398**）｜**実機 PASS**（`v286HiddenInfoNotInSharedLog`・反転確認あり） |
| 🔑**この2回の道具** | `src/engine/hiddenInfo.ts`（`logCardLabel`＝公開領域／この解決で公開した札だけ名前を出す・`privateLine`＝自分だけに見える行）・`addPrivateLog`・`scripts/archive/censusHiddenInfoLeak.mjs`（トレースで漏れを全数で数える＝群1 **1**／群2 **40**・残りは全件偽陽性） |
