...
- **🆕 セッション（2026-07-16・続き161・Opus 4.8・PLAN §3 Opusタスク12(xxii)／続き160「次の一手」＝結果カウント条件の前段判定漏れ〈BANISH/EXILE/SEND_TO_ENERGY〉を是正）**
  - **2枚採用・census 2093→2092・IS_MY_TURN化 107→104**。「この方法で〜バニッシュした/ゲームから除外した/エナゾーンに置いた場合」の結果カウント条件が、前段が lastProcessedCards を記録するのに parser の `prevRecords` ゲートに `BANISH`/`EXILE`/`SEND_TO_ENERGY` が入っておらず IS_MY_TURN 化していた。`prevIsProcessRecorder` を `prevRecords` に OR 追加（`prevSetsProcessed` は据置＝`parseThisWayTrashCondition` の「トラッシュに置かれた」限定分岐に無反応で安全）。影響3枚のみ。
  - **採用**＝WX14-021（スペル3枚以上除外＝`{cardType:スペル,minCount:3}`・prev=EXILE）・WX21-059（シグニ2体 banish＝`{cardType:シグニ,minCount:2}`・prev=BANISH）。**据置**＝WX14-072（CHOICE の TRASH mis-parse・続き158 同様）。
  - **検証**：golden 357・smoke/fuzz 全0・census 2093→2092（BASELINE 更新）・同型★0 維持。詳細 BUGFIXES 続き161。
  - **次の一手**：Opus＝IS_MY_TURN化残104 のうち engine 対応済み条件をさらに（種類＝`TRASHED_DISTINCT_LEVELS_GTE`/`TRASHED_STORY_COUNT_GTE` は engine 実装済み・ただし対象カードは前段が STUB/CONDITIONAL包みで記録せず要前段対応／すべて〜である＝ALL_MATCH／【ライフバースト】持つ＝`LAST_PROCESSED_HAS_BURST`〈マジックボックス flip の記録要確認〉／`LRIG_COLOR{opponent}`＝WXK09-003／`CARDS_DRAWN_BY_EFFECT`＝WXK09-040 等の単発条件）。**前段が STUB/CONDITIONAL包みで lastProcessed 未記録**の一群（めくれ公開・両者デッキ底・任意手札公開・入れ子SEQ prev・THIS_CARD_IS_UP 包み TRASH）は前段の記録機構/gate-through-conditional が要る別作業。ほか (xxvii) 残 Cluster C/A/F。Sonnet＝Opus在庫消化待ちまたは補欠。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 357・smoke/fuzz 全0・同型★0・census 2092。
>
