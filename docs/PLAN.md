...
| (lxxxiii) | **第4波まで一部消化（2026-08-02）**＝既存対象引き継ぎで live 4効果（`WX26-CP1-092-E1`／`WXK11-065-E1`／`WXDi-CP02-066-E1`／`WXK10-067-E1`）を是正し、誤検出5件を正式分離。`WX25-CP1-087-E1` は可変対象数を任意デッキ破棄へ渡す既存経路がなく honest defer。再測＝語彙167、実装対象160、あり124／**残36**。全ID・理由は BUGFIXES 第4波節。 |
