...
- **🆕 セッション（2026-07-19・続き204-205・Opus 4.8・**タスク7クローズ**＝`PREVENT_DAMAGE`＋`COST_SUBSTITUTE` 実装／**タスク1(a)**＝アタック不可付与の対象決め打ち是正28効果・census 1963→1949・golden 451→460）**
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 1949【効果単位】**（2026-07-19 続き205＝アタック不可付与の対象決め打ち是正28効果で1963→1951・対象節の閾値フィルタで1951→1949、続き203＝終止形サーチ退化＋levelGtLastProcessed で1967→1965・前置CHOOSE復元＋ルリグデッキ戻し幻覚除去12枚採用で1965→1963。続き201＝`TRANSFER_TO_DECK` 幻覚除去で1968→1967、続き200＝名前filter 2効果の復元で1970→1968）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。前提＝`docs/_effect_srctext.json` が最新であること。明細 `docs/_vocab_census.txt`、過去の計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109以降。
