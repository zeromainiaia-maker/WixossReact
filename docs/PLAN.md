...
- **🆕 セッション（2026-08-16・続き502・Opus 5）＝§6.4 **O-33 クローズ**＝アタック禁止の**ゾーン限定軸（残0）**。ゲート全緑（**golden 2082**＝+3・**census 825 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・held **104→103枚 / 44群**）。live changed **9効果 / 9カード**。`census:stubs` A群 **15種/17件 据置**（無言 no-op 0 のまま）。新機構＝`SigniAttackBan.zones`＋`SigniAttackBanAction.zones`（`turnsRemaining` は続き499 で導入済み＝これで O-33 の2キーが揃った）／`signiZoneIndexJa`（ゾーン語の対応表を1本に集約）。
  ...
  - **🔴③ 原文 regex で数え直したら同族2件がもっと壊れていた**＝`WX24-P1-038-E2`／`WXDi-P03-027-E2` は `BLOCK_ACTION{SIGNI, owner:'any', until:END_OF_TURN}`＝**両プレイヤーの全ゾーン**が、しかも**1ターンだけ**止まっていた（原文は「対戦相手の中央だけ・次の相手ターン終了時まで」）。
  - **🔴④ engine が全文 regex で選択肢を組むカードは「選択肢の2文目」が top-level へ漏れる**＝続き499 で `CHOOSE_SAME_OPTION_*` に入れたガードと**同じクラスが3ビルダーに散っていた**。**6効果が「選ばなくても必ず走る」過剰実行**（`WD22-011-G-E1`＝常に2ドロー／`WXK05-003-E1`＝常に手札1枚捨て＋バニッシュ／`WXK10-009-E1`＝常にトラッシュから場出し＋全ダウン／`WXK10-011-E1`＝常に全シグニ＋5000／`WXK03-TK-01B-E1`＝常に【アサシン】付与＋手札2枚捨て／`WX13-003-E1`＝常に手札1枚捨て）。⚠ガードは **`①` が本文にあるときだけ**（`CHOOSE_N_FROM_LIST` には「プレイヤーを1人まで選ぶ」形もあり、後続は**本体**）。⚠見出しの宣言（`ARTS_COST_REDUCTION_BY_EFFECT`）は残す。
  - **🔴⑤ `banish-vs-ener-send` が choice 経路で再発していた**＝`choiceTextParser` の「対戦相手のシグニ１体…エナゾーンに置く」が **`BANISH` 近似**だった（置換・バニッシュ時トリガー・「バニッシュされない」耐性で両方向にズレる）。`SEND_TO_ENERGY` へ是正。⚠選択肢テキストは**複文になりうる**＝先に当たった1文で return すると前半が消える（`WXK10-011-E1`② のエナからの場出しが丸ごと落ちていた）。
  - **▶ 次の一手【Opus 側】**＝`O-28` の残23綴り → `O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29`（`WX22-016-E1` の `REPEAT_EFFECT` 1件が続き501 で合流）→ `O-31` の残2 → `O-10`（明示 defer 15 id/17件の前提再判定）。⚠`O-33` の**据置分**＝`WDK09-001-E2`（「【ゲート】があるシグニゾーン」＝動的ゾーン）は別軸として未着手。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き502 の4経路**＝(a)`WX25-CP1-050-E1`（次の相手ターンだけ**中央のシグニだけ**にアタックボタンの《無》×1 注記が出て、左右は無条件でアタックできる＝**負方向＋対照の対**）(b)`WX24-P1-038-E2`／`WXDi-P03-027-E2`（**自分の**シグニは止まらない＝旧 `owner:'any'` の是正確認）(c)`WXK10-011-E1`（3択が出て①エナ送り／②エナから場出し＋全体＋5000／③中央限定のアタック禁止。**選ばなかった選択肢の効果が走らない**）(d)`WD22-011-G-E1`／`WXK05-003-E1`／`WXK10-009-E1`／`WXK03-TK-01B-E1`／`WX13-003-E1`（同上）。⚠**すべて実機未検証**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-16 続き502（§6.4 **O-33 クローズ**＝ゾーン限定アタック禁止 残0）後 最新値（本行が直近の正）**：census **825 据置**、**golden 2082**（+3＝`zones` の中央/左右＋入れ替えの対1・live 不変条件（3効果が `SIGNI_ATTACK_BAN{zones}`・`owner:'opponent'`）1・choice ビルダーの漏れ無し（6効果）＋見出し宣言の温存＋非①形の本体温存1。ほかに `WXK10-011-E1` のパワー節 assert を「選択肢②の中」へ、任意コスト トリップワイヤの既知リストから `WXK05-003-E1` を削除）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **103枚 / 44群**（104→103）、lint **0 errors / 260 warnings**、`census:stubs` A群＝**15種/17件 据置**（無言 no-op 0）・C群 **172種/253箇所 据置**、`census:goldentypes` **未カバー 0型**。🆕**live JSON changed 9効果 / 9カード**（CSV 非改変）。🆕**新機構＝`SigniAttackBan.zones`／`SigniAttackBanAction.zones`／`signiZoneIndexJa`**。⚠**全経路 実機未検証**（§7 送り）。
