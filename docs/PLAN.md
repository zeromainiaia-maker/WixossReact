...
- **🆕 セッション（2026-08-15・続き490・Opus 5）＝§6.4 **O-3 の `ATTACK_TAX_HAND_DISCARD`（1効果）を解体＝2効果の挙動是正**（**恒久 no-op 2＝うち1件は計器に映らない無言 no-op**）。ゲート全緑（**golden 2019**＝+2・**census 831 据置**・lint 0/259）。
  - **🔑① 「支払えば通る」制限は軸ごとに別関数を生やさない**＝新設 `SigniAttackBan.unlessPayHandDiscard`（「手札をN枚捨てないかぎりアタックできない」＝**アタックするごとに**払う）に合わせ、`signiAttackBanColorlessCost` を `signiAttackBanCost` へ一般化して **`{colorless, handDiscard}` をまとめて返す**形にした（解除不能な ban が1つでも掛かっていれば `null`）。軸ごとに関数を分けると「片方の軸だけ見る gate」が生まれて**無言ですり抜ける**。判定は既存 `signiAttackGate`（新 reason `ATTACK_BAN_HAND_COST`）／引き落としは `performSigniAttack`／選択は新モーダル `AttackHandDiscardCostModal`＝**アタックボタンのラベルにも解除コストを出す**（出さないと「押したら知らないモーダルが開く」になる）。
  - **🔴② 引用文が丸ごと `GRANT_KEYWORD.keyword` に入る形は「無言 no-op」で、どの計器にも映らない**＝`WXDi-P05-022-E1`「対戦相手のシグニ１体を対象とし、…それは「【常】：手札を１枚捨てないかぎりアタックできない。」を得る」は `keyword` が**文そのもの**になり、`hasKeyword` は正式名でしか照合しないので**一度も効かなかった**（狙われたシグニが無条件でアタックできる）。**STUB ですらないので `census:stubs` の A群にも出ない**＝受け皿 id を数えても永遠に見つからないクラス。正準形 `SELECT_TARGET_ONLY → STORE_LAST_PROCESSED_TARGETS → 〈ban〉{targetsStored}`（`WX10-024-E2` の《無》×N 版と同じ組み立て）へ post-pass で組み替えた。**keyword が「正式名でない」ことを条件にすれば汎用に効く。**
  - **⚠③ 「アタックするごとに払う」と「1回きりの回避」は別機構**＝`unlessPayHandDiscard`（毎回・ban は消費しない）と `NegateAttackAction.escapeDiscard`（1回きり・消費する）。原文の括弧書き「（アタックするごとに捨てる）」が両者を分ける。⚠再入経路（`negateEscape` からのアタック再実行）で**二重請求しない**よう `attackHandDiscardAlreadyPaid` を既存 `attackFieldTrashAlreadyPaid` と同じ場所で立てた。
  - **✅④ 母集団の実測**＝「〜しないかぎりアタックできない」は原文24件あるが、**手札捨て版は3件**（うち1件 `WXDi-P05-023-E1` は既存 `OPPONENT_PAY_OPTIONAL` で表現済みの別形）。**受け皿 id の1件だけを見て設計すると隣の無言 no-op を取り逃す。**
  - **▶ 次の一手【Opus 側】**＝**O-3** の残7（A群が常時の在庫表）。`SKIP_NEXT_TURN`（`WD20-006`＝ターン丸ごとスキップ）／`UNTIL_NEXT_MAIN_PHASE_CLAUSE`（`WXK01-002`＝基本リミット上書き＋ダメージ無効＋次ドロー2枚の3機構複合）／`NEXT_OPP_TURN_MAIN_PHASE_SKIP`（`WXEX2-19`・**実装レシピは parser コメントに記載済み**）。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → **O-26**（コスト節の自己トラッシュ脱落2件）。⚠**着手時は受け皿 id ではなく原文 regex で母集団を数え直す**（続き489/490 で2回とも「A群に出ていない隣の効果のほうが壊れていた」）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き488〜490 で追加した3つのUI経路が未検証**＝(a)**追加のアタックフェイズ**（`WXK06-026`＝ATTACK_LRIG→ATTACK_ARTS へ戻り、2周目の `ON_ATTACK_PHASE_START` が走るか）(b)**エナ支払い封じ**（`SPK01-10`＝《色×0》は通り、エナ1以上のコストだけ押せなくなるか＝**負方向＋対照の対**で書く）(c)🆕**`AttackHandDiscardCostModal`**（`SP38-003`＝手札が足りるとき／足りないときの対）。ほか §7 の未消化ブロックから。
