...
| 9 | smoke SKIP 268 解消 | scripts（smokeTest autopilot） | S | ◎**再開可（続き106でOpus12(vi-2)着地＝SKIP 258→1）**。残1件の最終確認のみ |
...
- **🆕 セッション（2026-07-13・続き106・Opus 4.8・PLAN §3 Opusタスク12(vi-2)/(v)/(vi)＝`applyDirectAction` の型対応漏れ6型を修正・smoke SKIP 258→1）**
  - **✅ Sonnet が続き82-96 で積み上げた `applyDirectAction`（`effectExecutor.ts:4696`）の型対応漏れを根治**＝SELECT_TARGET/SEARCH で選ばれた単一 `cardNum` に thenAction を適用する分岐で、`case` の無い型が `default` 節で**元アクションを丸ごと再実行**し、選んだ `cardNum` を無視して元 `target` を素通しで再評価していた（同一 SELECT_TARGET 無限再発行＝実UIフリーズ相当／別対象すり替え）。**smoke の「autopilot loop: SELECT_TARGET」258件の真因**。
  - **新設4 case（選択済み単一 `cardNum` へ直接適用）**＝`ENERGY_CHARGE`（81件・`SEARCH→then:DECK_CARD` が主形）／`TRANSFER_TO_DECK`（125件・最多）／`GRANT_PROTECTION`（73カード）／`POWER_SET`（10件）。いずれも既存 exec 関数（`applyCharge`/`insertToDeck`/`applyProtection`/`execPowerSet`）と同じ単一カード適用ロジックを移植。
  - **thenAction 書き換え2型**＝`execPowerModifyPerField`／`execPowerModifyPerLrigLevel` は選択前に delta が確定するため、`selectOrInteract` へ**解決済み `POWER_MODIFY{delta}` を thenAction として渡す**よう変更（既存 `execPowerModifyPerLevelSum` idiom に統一）＝applyDirectAction の既存 `POWER_MODIFY` case が処理し PER_* case 欠落による default 再入を回避。
  - **検証**＝golden 286→**293**（count:1 の選択経路7件を回帰追加＝修正前は autopilot hang/誤対象）／`npm run gates` 全緑／**smoke SKIP 258→1**（CRASH/HANG/INVARIANT 0）／fuzz 0／census **1461維持（退化なし）**／同型★0。engine のみ変更・JSON/parser/decompiler 不変＝regen 不要。詳細 BUGFIXES 続き106。**📌 Sonnetタスク9（smoke SKIP 258 解消）のブロッカーを除去済み**。
  - **✅ 第2バッチ＝タスク12(vi-3)(vi-4)＝`triggerCollect.ts` の LRIGゾーン走査漏れを5コレクタで根治**（`collectPowerZeroTriggers`/`collectFieldTriggers`(own+opp)/`collectTurnTriggers`(相手ルリグ)/`collectOppArtsUseTriggers`/`collectHandDiscardTriggers`(自ルリグ)）。`field.signi` のみ走査で `field.lrig` を欠き、LRIG watcher が構造的に絶対発火しなかった実カード20枚超（WX22-013/WXDi-P14-009・WX12-001/WX14-003/WXDi-P08-007・WXEX2-26・ON_ATTACK_PHASE_START 11枚・WX16-003・アロス4枚）を、`ownFieldSources` 置換／専用 lrig ブロック（activeCondition ゲート付き）で解消。golden 293→**298**（LRIG発火5件を回帰追加）・**census 1461維持・fuzz 0**（distinct効果 2648→2652）。engine のみ・regen 不要。詳細 BUGFIXES 続き106。
  - **✅ 第3バッチ＝タスク12(vi-5)の一部＝`collectCoinPaidTriggers` の usageLimit 書き戻し欠落を修正**（ON_COIN_PAID《ターン1回/2回》のノーガード＝実機 `coinPaidTwice` で WXDi-P15-069 が3回目も発火）。返り値を `{entries, usedIds}` 化＋`mkLimitOk` 使用、`BattleScreen.tsx` の6コイン支払サイトで `applyCoinPaidUsed` により `actions_done` へ書き戻し。golden 298→**299**（twice_per_turn 書き戻し回帰）・census 1461維持・fuzz 0。詳細 BUGFIXES 続き106。**残＝(vi-5)の二面コレクタ3種（`collectBanishTriggers` 18枚・`collectPowerZeroTriggers` 6枚・`collectLrigGrowTriggers` 4枚＝usedHostIds/usedGuestIds が要る）は次セッションへ**。
  - **✅ 第4バッチ＝タスク12(ix)＝`execBlockAction` の SIGNI/ATTACK 分岐が count/filter を無視し全ブロックしていたのを修正**。ACTIVATED/AUTO の `count:N`（owner opponent/any）＝「対戦相手のシグニN体を対象としアタックできない」型（WX18-009 等）が相手の全シグニをブロックしていた過剰効果を、filter適用＋`selectOrInteract` の N体選択で是正（選択解決後は既存 `lastProcessedCards` 経路が個別付与＝applyDirectAction 追加不要）。golden 299→**302**・smoke SKIP 1・census 1461維持・fuzz 0。詳細 BUGFIXES 続き106。
  - **✅ 第5バッチ＝(ix)追走＝CONTINUOUS「このシグニはアタックできない」の `calcContinuousBlockedActions` 拾い漏れを修正**。第4バッチで対象外と切り分けた `owner:self,count:1` の CONTINUOUS 5枚（WX05-023/WX13-043/WXK05-047/WX17-034/PR-402）は、`scanField` が parser の出す `ATTACK(SIGNI,owner:self)` 形を拾わず**印刷能力が無効化**されていた（`ATTACK_SIGNI_SELF` 形しか見ていなかった）。自己アタック封じ分岐に当該形も追加＝能力保持シグニ自身を `cannotAttackSigni` へ。golden 302→**303**・census 1461維持・fuzz 0。詳細 BUGFIXES 続き106。
  - **✅ 第6バッチ＝タスク9／§6.2 系統①(b)＝「あなたか対戦相手のデッキをN枚トラッシュ」17枚の選択脱落を CHOOSE 化**（トリガーループ非依存の JSON データ修正へ切替＝残タスク12は全て脆弱な戦闘/召喚解決ループの dual-state 書き戻しでリスク過大のため）。原文「あなたか対戦相手」が `TRASH{DECK_CARD,owner:self}` にハードコードされ常に自デッキだけ削っていた18ノードを、既存テンプレ WXDi-P04-082 の `CHOOSE{self_deck,opp_deck}` へ置換（新規engine機構ゼロ）。入れ子CHOOSE（WXDi-D07-022-BURST②）含め smoke 0破綻・**同型★0維持**・逆翻訳が原文どおり2択表示・census 1461維持。系統①(b) を全消化。詳細 BUGFIXES 続き106。
  - **✅ 第7バッチ＝タスク4＝色別 ARTS_USED_THIS_TURN 機構を新設**（「このターンにあなたが(色)のアーツを使用していた場合」条件脱落4枚＝WX24-D1〜D4-11 の真バグ是正）。`turn_arts_used_colors` state＋Condition.color＋evalUseCondition＋executeArtsでの色記録＋ターン境界6箇所リセット＋parser色付き規則＋decompiler色表示。golden 303→**304**・**census 1461→1458**（BASELINE_HIGH更新）・同型★0維持・fuzz 0。⚠5枚目 WX25-P3-116（黒）は「代わりに」置換パターン（タスク6前提）で対象外。詳細 BUGFIXES 続き106。
  - **✅ 第8バッチ＝タスク6＝「代わりに」置換の五面カード4枚を CONDITIONAL 化**（WX06-003赤/004青/005緑/006黒＝「センタールリグが(色)でライフ2枚以下の場合、代わりに強化」が `SEQUENCE[基本,強化]` 両実行の過剰効果だったのを是正）。parser の `STATE_CONDITION_CLAUSES` に複合条件 `AND[LRIG_COLOR, LIFE_COUNT]` を1件追加＝既存「代わりに」置換ロジックが発火し `CONDITIONAL{cond, then:強化, else:基本}` を生成→`build:effects`→`heldReview --adopt` で4枚採用（新規engine不要・置換機構は続き28-29で既存）。golden 304→**305**・**census 1458→1454**（BASELINE_HIGH更新）・同型★0維持・fuzz 0。⚠WX06-002（GRANT_KEYWORD/BLOCK_ACTION混在）・WX25-P3-116（アーツ色の代わりに）はタスク6の続きへ。詳細 BUGFIXES 続き106。
  - **次の一手（Opus）＝** タスク6続き（WX25-P3-116＝STATE_CONDITION_CLAUSESへアーツ色条件追加・WX06-002・WX16-021置換ルール・ベット系「代わりに」多数＝IS_BETTING表エントリで横展開可）が費用対効果高（census直減）。または §6.2 系統②残（GRANT_PROTECTION 広域24件）・(vii)(viii) 構造的バグ。**(vi-5)/(x) トリガー usageLimit 書き戻しは実機ドライバ付き専用セッション推奨**。
