...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer **残 8 id/10件**。続き507〜509 で3巡済み＝残りは1件ずつ機構が要るものばかりで、最も近いのは `DEFERRED_ATTACK_NEGATE_IMMUNITY_SELF`〔ターン限定フラグ＋アタック無効化サイトの照会だけ〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 8 id / 10件 | 🆕**2026-08-16 続き509（3巡目）＝10種/12件 → 8種/10件**（`WXK08-002-E1` のアーツ3択のうち **①③を実装**＝**3択すべてが実体を持つ**ようになった）。**■③**「あなたの赤のセンタールリグ１体を対象とし、ターン終了時まで、それは能力を失い『【自】《ターン２回》：このルリグのアタックが【ガード】されたとき、このルリグをアップする。』を得る」＝新 STUB `SELF_LRIG_LOSE_ABILITY`（既存 `OPP_LRIG_LOSE_ABILITY` の自分側版）＋`GRANT_EFFECT{LRIG}`。🔑**付与は `GRANT_EFFECT`（per-card ストア）で書く**＝`GRANT_LRIG_ABILITY` にすると `grantedStoreWatchers` が`lrig_abilities_disabled` で落として**自分で消してしまう**（原文は「失い、得る」＝得た側は残る）。読み手は `collectLrigAttackGuardedTriggers`（augmented effectsMap 経由）。**■①**「あなたのトラッシュから赤のシグニ１枚を対象とし、ターン終了時まで、それの基本レベルを１にする。それをレベル１のシグニとして場に出す。ターン終了時、それを場から手札に戻す」＝新 STUB `SET_STORED_BASE_LEVEL`（既存 `SET_BASE_LEVEL{until:END_OF_TURN}` は **`sourceCardNum` 固定**で「それ」を指せない）＋`RETURN_TO_HAND_AT_TURN_END`（解決は新 funnel `screens/battle/turnEndHandReturn.ts`＝**ターン終了2経路**で同じ関数。⚠**手札上限チェックより前**に手札へ入れる＝memory「エンドフェイズは①ターン終了時効果→②手札上限」）。🔴**隣接発見＝ターン限定なのに失効地点が無い state が3つ**＝(a)`attack_phase_level_overrides`〔**失効地点が1つも無く永続**。しかも読み手が `EICHI_LEVEL_SUM` の1箇所だけで、レベル参照の funnel（`applyContinuousBaseLevelOverride` の cardMap 上書き）に載っておらず**「基本レベルをNにする」がフィルタにもレベル比較にも一切効かない**ほぼ死んだ store だった＝両方是正〕(b)`lrig_abilities_disabled`〔手書きクリアが**自分側の2経路だけ**で、`OPP_LRIG_LOSE_ABILITY` が書く**相手側**は一度も落ちず永続しうる（`negated_attacks` と同じクラス）〕(c)`turn_end_return_to_hand`〔新設〕＝**3つとも turn-scoped レジストリへ登録**。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き509）。⚠**全経路 実機未検証**＝§7 送り。**■残 8 種 / 10 件**＝`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY`(3)〔続き507 新設。対話つき置換の policy 差し替えと対〕／`DEFERRED_ATTACK_NEGATE_IMMUNITY_SELF`〔利得側なので現状は過少（無害）〕／`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔ピースへのカットイン窓が要る〕／`DEFERRED_DECLARE_NUMBER_AND_ATTACK_PHASE_RESTRICTIONS`／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔`WX25-CP1-060-E2`。裏向き→表向きの往復自体は `facedownSigni.ts` で書けるが、**観測できる効果は「新しいオブジェクトになって相手の対象指定が外れる」**＝*対象の無効化*という別機構が要る〕／`DEFERRED_OPP_TURN_ARTS_COST_REDUCTION_ONCE`〔場宣言のアーツコスト軽減＝収集器＋コスト算出3入口〕／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`。**定期的に前提を再判定する**（続き507〜509 の計10種のうち、**「機構が無い」が本当だったのは3種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付けだった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
