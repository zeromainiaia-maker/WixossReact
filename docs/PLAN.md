...
| ~~🆕(lvi)~~ | **✅完全クローズ（2026-07-28）＝21効果／PLAN記載12件は実測21件だった（`appearanceCondition` 9件は新規発見）**。CSV原文と全件一致を確認し、manual override へ cost 10・cost＋condition 1・timing 1・appearanceCondition 9を明示復元、defer 0。修正前の appearance 9件は別効果の救済0で、無料召喚ではなく召喚不能だったことも実経路で訂正。常設ゲート `npm run check:manual-fields` を `npm run gates` に追加。CODEX_GUIDE §5-16 は、実機が `buildEffectsMap`→`mergeManualEffects` を通り manual は届くこと、真の危険は完全置換のフィールド脱落であることへ訂正。 |
