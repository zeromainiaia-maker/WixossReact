...
**直近＝2026-09-21＝🏁§5.7 `S-32` ②③＝狙い方の切り替え（効果ごと・盤面の条件つき）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第443バッチ）
- 🔑**②と③を1つの機構に**＝**規則の列**（`{sourceCards, when, mode}`）＝**上から順に最初に当たった1つ**が狙い方を上書き。
  `sourceCards` 空＝どの効果でも／指定あり＝その札の効果のときだけ（③）。`when`＝ライフ2枚以下・相手の場3体など（②）。
- 🔑**条件は閾値まで名前に入れた閉じた集合**＝自由な式は**画面・保存・判定の3か所で解釈がずれる**。
- 🔑**解決は `resolveCpuTargetMode` の1本**＝`pickCpuTargets` には解決済みの `mode` だけ渡す（`S-22` の形を壊さない）。
- ⚠**「どの効果でも・いつでも」の規則は落とす**（既定と同じなのに**上に置くと下の規則が永久に届かない**）。
- ⚠🔴**規則の札はデッキに無ければ落とす／［狙う］［避ける］の札は落とさない**＝**同じ `targeting` の中で向きが違う**
  （効果を出すのは自分の札／狙う相手は相手の札）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` の残り**（撃てない【起】108・①N体並べる・③アーツの使いどころ）。🏁**`S-32` は①②③すべて消化**。次に `S-29` |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4361**）｜**実機 PASS**（7ターン/165手/217s ＋ `v268CpuDeckPlan`） |
...
- **2026-09-21 時点**（第421〜443バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4361**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・5/130手・6/133手・6/135手・7/161手・**7/165手**）
