...
| 12 | Sonnet 発見バグの修正（常設） | 可変 | 可変 | ◎**在庫8件**（✅続き78で旧8件全消化。新規＝SP27-002 genericKagiri無言消費系統・WXDi-P10-035 引用内BOUNCE owner精査・WXK09-050 dispatch設計・続き81 applyDirectAction TRASH/HAND_CARD 3フィールド欠落・~~続き93 applyDirectAction 未対応6型~~ ✅続き106で修正（smoke SKIP 258→1）・続き95 `collectPowerZeroTriggers`がfield.lrigを走査せずLRIG watcher（WX22-013/WXDi-P14-009）が絶対発火しない・続き96 同型のLRIGゾーン走査漏れが`triggerCollect.ts`内6コレクタに系統的に存在し実カード20枚が影響（ON_ATTACK_SIGNI/ON_ATTACK_PHASE_START/ON_OPP_ARTS_USE/ON_HAND_DISCARDED/ON_BANISH）・🆕続き99 `collectCoinPaidTriggers`がusageLimit用actions_done書き戻しを一切行わずON_COIN_PAIDの《ターン1回/2回》が実質ノーガード・続き102/🆕続き103 `execBlockAction`（`effectExecutor.ts:1536`）のtarget.type==='SIGNI'&&actionId==='ATTACK'分岐がtarget.count/upToCount/target.filterを一切読まず、対象選択ステップが無いカードは対象オーナーの全シグニを無差別ブロックする＝全数調査完了・41件確定（自己1体制限系＋選択N体系の両方が同一フォールバックで過剰効果化。カード名一覧は上のOpusタスク12本文）。詳細は上のOpusタスク12本文） |
