...
>
> 0. ✅**§5d-0 の工程改善3件は 2026-08-07 続き376 で完了**。以後の worklist は **§5d-0 末尾の「🗂 作業種別 worklist」が正**（クラスタ別は索引としてのみ残す）。**取る順は (iv)計器較正 →(i)配線ギャップ →(ii)機構ギャップ →(iii)構造混線**＝この順にコストが安い。
>    - ①**`npm run census:wiring` を常設化**（`scripts/censusWiring.ts`／明細 `docs/_census_wiring.txt`）＝**miss 541件を1パス検出**（続き376b で語彙表を大クラスタへ拡張＝203→541）。★★／★のセルから取る。⚠**miss 数は「掘る価値の指標」で「見込み件数」ではない**（クロス計上あり＝§5d-0 ①の⚠⚠）。
>      **🎯いま最も条件のよいセル＝`triggerSubjectClass × TRIGGER{ON_ATTACK_SIGNI}`（miss 8・has 37）**＝「あなたの＜X＞のシグニがアタックしたとき」の主語限定が落ちて**どのシグニのアタックでも発火**。同型44件中36件は配線済みで、無いのは8件・全部同じ timing。
>    - ②**真バグ率を測り直した＝14/20＝70%**（続き369 の 92% から低下。95%CI 50〜90%）＝**残りの実バグは約810件**。⚠**サンプルの30%は計器の誤検出**で、直す対象ではない。
>    - ③作業種別 worklist へ組み替え済み。**まず取るべきは (iv) の5項目**（実装ゼロで census が減る。うち `LOOK_PICK_CHAIN`＝「Nまで」較正だけで **28件**）。
>    - ⚠**現ペースだと完了まで約100セッション**（直近7セッションで 82効果・census −37、バッチ収穫は 36→12→6→4→4 と逓減）＝**探索コストを下げるのが先**、という前提は変わっていない。
...
**✅第1バッチ完了（続き376b）＝`triggerSubjectClass × TRIGGER{ON_ATTACK_SIGNI}` は miss 8→0（has 37→45）。** 詳細は下の「✅消化済みセル」。**census 1162→1151（−11）／全体 miss 541→525。**
