...
- **🆕 セッション（2026-08-15・続き492・Opus 5）＝§6.4 **O-3 の `UNTIL_NEXT_MAIN_PHASE_CLAUSE` を解体＝9効果の挙動是正**（**恒久 no-op 2／無言 no-op 2／1回で消費されていた 5／過剰実行 1／過少実行 1**）。ゲート全緑（**golden 2030**＝+3・**census 831 据置**〔+1 は較正で解消〕・lint 0/260）。
  - **🔑① 「N回まで」ではなく「期間中ずっと」の防御は、回数フラグではなくウィンドウで持つ**＝「（対戦相手の）ルリグによってダメージを受けない」12効果の期間軸を `PREVENT_DAMAGE{scope:'LRIG'}`（`prevent_damage_windows`）1本に集約した。従来の受け皿 STUB（`PREVENT_LRIG_DAMAGE_THIS_TURN`／`_UNTIL_NEXT_TURN`）はどちらも `prevent_lrig_damage`＝**1回で消費される boolean** を立てるだけで、①「次のターンの間」は turn-end でクリアされ**一度も効かず**（`WXK10-019-E2`＝恒久 no-op）②「このターン」も**2回目以降が素通り**していた（5効果）。
  - **🔴② 【常】判定の走査軸がシグニだけだった**＝`WXK03-001-E1`（**ルリグ本体**の「手札0枚のかぎり」）と `WXK11-012-E2`（**キー**の「レベル２以下のルリグによって」）は宣言が読まれず**無言 no-op**。新設 `isLrigDamagePrevented`（`screens/battle/lrigDamageShield.ts`）でシグニ／ルリグ／アシスト／キーを走査する。⚠**レベル限定は宣言の `value` から読む**＝parser が落とすと「すべてのルリグから守る」過剰効果になるので parser 側にも `value` を載せた。⚠**宣言型の【常】は state にフラグを書かない**（書くと「1回防いだら消える」に化け、レベル限定も表せない）＝該当 execStub ハンドラを no-op ログへ。
  - **🔑③ 「回数無制限の防御」は消費型より先に判定する**＝従来この判定は**バリア／`prevent_next_damage`／置換ミルより後ろ**にあり、無制限に防げる盤面でも限りある資源が先に減っていた。funnel 化と同時に順序を是正。
  - **🔑④ 新期間「次のあなたのメインフェイズまで」（`MY_NEXT_MAIN_PHASE`）**＝`WXK01-002-E2` の3機構（基本リミット12／ルリグダメージ無効／次ドロー1→2枚）はすべてこの期間で、**相手のターンを丸ごと跨ぐ**＝`UNTIL_OPP_TURN_END` では**次のドローフェイズが範囲外**になる。`turnScopedState` に境界 `main-phase-start` と `clearMainPhaseScopedState()` を新設し**失効地点をこの1関数だけ**にした（呼び出しは `ON_MAIN_PHASE_START` 収集と同じ人間/CPU の2箇所・golden にソース走査のトリップワイヤ）。新アクション＝`SET_LRIG_BASE_LIMIT`（**加算ではなく置換**＝`computeEffectiveLrigLimit` の basicOverride 層）／`RESERVE_DRAW_PHASE_REPLACEMENT`（読みは既存の `applyLrigDrawPhaseReplacement` と同じ1関数）。
  - **🔴⑤ 「〜引く場合、代わりに〜引く」を汎用ドロー規則に食わせない**＝置換の `fromCount` 側が実ドローと読まれ、`WXK01-002-E2` は**使った瞬間に1枚引く**過剰実行になっていた（parser の汎用 `DRAW` より**前**に規則を置く）。あわせて `WXK02-022-E2` の再parseで、live から**丸ごと脱落していた**「対戦相手のシグニ１体をトラッシュに置き」も復元（過少実行）。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残8（A群が常時の在庫表）。`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → **O-26**（コスト節の自己トラッシュ脱落2件）。🆕**据置で登録した2件**＝`WX24-P3-003-E1`（`GRANT_ABILITY_INNER_TEXT`＝引用能力の中身が未パース）／`WXK04-014-E1`（CHOOSE 選択肢②のダメージ無効が未表現）。⚠**着手時は受け皿 id ではなく原文 regex で母集団を数え直す**（続き489〜492 で4回とも「A群に出ていない隣の効果のほうが壊れていた」）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き492 の4経路**＝(a)`WXK01-002-E2`（相手ターンを跨いで①ダメージが通らない②次のドローで**2枚**引く③自分のメインフェイズ開始でリミットが12から戻る）(b)`WXK10-019-E2`（張ったターンには効かず次の相手ターンに効く**負方向＋対照の対**）(c)`WXK03-001-E1`／`WXK11-012-E2`（ルリグ本体・キーの【常】／レベル境界の対）(d)「このターン」版でルリグアタックを**2回**受けて2回とも防ぐか。ほか続き488〜491 の未検証分（追加のアタックフェイズ／エナ支払い封じ／`AttackHandDiscardCostModal`／フェイズ・ターンのスキップ）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き492（§6.4 O-3 `UNTIL_NEXT_MAIN_PHASE_CLAUSE` 解体）後 最新値（本行が直近の正）**：census **831 据置**（⚠一度 832 まで上がったが `RESERVE_DRAW_PHASE_REPLACEMENT` の較正で解消＝「STUB を実装で置き換えると +N する」既知パターン）、**golden 2030**（+3＝期間軸1・走査軸1・`WXK01-002-E2` の3機構1。ほかに T1/T6 のレジストリ実数を更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **112枚 / 47群**、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **24種/26件 → 23種/25件**）。🆕**live JSON changed 7カード**（`WXK01-002`／`WXK10-019`／`PR-K019`／`WX26-CP1-004`／`WX26-CP1-023`／`WX26-CP1-073`／`WXK02-022`。CSV 非改変）。🆕**挙動是正 9効果**（恒久 no-op 2／無言 no-op 2／1回で消費されていた 5／過剰実行 1／過少実行 1・重複あり）。🆕**新機構＝`isLrigDamagePrevented`（ルリグダメージ無効の走査軸を1本へ）／`turnScopedState` の境界 `main-phase-start`＋`clearMainPhaseScopedState`（「次のあなたのメインフェイズまで」＝ターン境界を跨ぐ期間）／`SET_LRIG_BASE_LIMIT`／`RESERVE_DRAW_PHASE_REPLACEMENT`／`PREVENT_DAMAGE.untilNextMainPhase`**。⚠**4経路とも実機未検証**（§7 送り）。

> **過去の計測 14 行（続き478〜491）は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-15 整理⑰」節へ退避**（2026-08-15）。**直近の正は上の1行**。
