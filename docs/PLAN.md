...
**直近＝2026-09-21＝🏁§5.7 `S-31` ② 第2段＝エナ・場から払うコストとルリグ【起】の手札捨て（撃てない【起】 210 → 108）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第442バッチ）
- 🔴**まず母集団を測り直した**＝前回の「523効果」は**場のシグニ用 allowlist を全部の【起】に当てた過大値**だった
  （ルリグ【起】では自動で払える `exceed` 106 などが混ざっていた）。
  🔑**allowlist が文脈ごとに違うなら、母集団も文脈ごとに数える。**
- 📏**文脈別の実測**＝場のシグニ 738効果中 **134 → 75（18.2%→10.2%）**／ルリグ 571効果中 **76 → 33（13.3%→5.8%）**
  ＝**102効果が撃てるようになった**。
- 🆕**`pickCpuEnergyTrashIndices`**（エナ・52効果）＝🔴**グロウの予約を壊さないエナを先に落とす**（`S-26` の失敗を作らない）。
  予約を守れる組み合わせが無ければ**予約を諦めて払う**（撃てないより良い）。
- 🆕**`pickCpuFieldTrashZones`**（場・23効果）＝候補は `fieldTrashSelectableZones` の1本・**弱いシグニから**。
- **ルリグ【起】に手札捨て・エナトラッシュを配線**（43効果）＝⚠**`fieldTrash` は入れない**（実行側に口が無い＝踏み倒しになる）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` の残り**（撃てない【起】108・①N体並べる・③アーツの使いどころ）と **`S-32` の残り**（②条件つき ③効果ごとの指示）。次に `S-29` |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（🔥`S-31` 残り・🔥`S-32` 残り・`S-29`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**撃てない【起】は残 108効果**（場のシグニ75／ルリグ33＝`S-31`） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4360**）｜**実機 PASS**（7ターン/161手/224s） |
...
| 🔥`S-32` | **効果の対象の指示**（2026-09-21 ユーザー要望）。🏁**第1段**（大まかな指示3種＋固有のカード指定）と🏁**①属性での指定**（クラス41種／レベル／パワー帯・`matchesFilter` の1本・**実効パワー**で判定）は消化済み。**残り**＝②**条件つき**（「ライフが2枚以下なら」「相手の場が3体なら」で狙い方を切り替える）③**効果ごとの指示**（いまはデッキ全体で1つ＝「この【起】のときだけ弱いものを狙う」が書けない）。📏**「下げて落とせる」が効くのは21デッキ中5つ**（`WD11` 6／`WD07` 5／`WD08` 4／`WD05` 3／`WD14` 3）＝🔴**A/B の6デッキには0件**＝既定の測定台では測れない（`S-20` と同型）。 | 中 | 作った CPU デッキの「誰を狙うか」を指示できる |
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**②は2段まで消化**（第439バッチ＝手札を捨てるコスト／第442バッチ＝エナ・場から払うコストとルリグ【起】の手札捨て）＝**撃てない【起】 210 → 108**（場のシグニ 738効果中75／ルリグ 571効果中33）。**残り**＝②の続き（場のシグニ＝`underSelfTrash` 13／`trashExile` 13／`discardSelfFromHand` 9／`charmTrash` 6／`removeOppVirus` 6・ルリグ＝`fieldTrash` 8（**実行側に口が無い**）／`trashArtsFromLrigDeck` 5）／①**「N体並べる」条件**（`SP06`／`SP07`／`天使軸1`＝2枚の順序では表せない・実測3デッキ）／③**アーツの使いどころ**（守り／攻め）・**使わない札**の指定。⚠**着手前に「どれが欲しいか」をユーザーに確かめる**。 | 中〜大 | 作った CPU デッキを意図どおりに回せる |
...
- **2026-09-21 時点**（第421〜442バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4360**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・5/130手・6/133手・6/135手・**7/161手**）
  - 📏**対象の狙い方「落とせるものを優先」が効くデッキ**（§5.7 `S-32`・2026-09-21）＝**21デッキ中5つ**（`WD11` 6件／`WD07` 5／`WD08` 4／`WD05` 3／`WD14` 3）｜🔴**A/B の6デッキには0件**
  - 📏**CPU が撃てない【起】**（§5.7 `S-31` ②・🆕**文脈別に測り直した**）＝**場のシグニ 738効果中 75（10.2%）／ルリグ 571効果中 33（5.8%）＝合計 108**（第439・442バッチで 210 → 108）。🔴**旧「648（24.6%）」は文脈を混ぜた過大値**（ルリグでは自動で払える `exceed` 106 などが混ざっていた）
