...
- 🏁**`S-25` をクローズ**（2026-09-21）＝**WD13 だけ弱い原因は「単一の重みのズレ」では説明も修復もできない**＝
  `fieldPowerScale=1` は WD13 単体 2-6-0 でも**6デッキへ広げると 合算 組 53.8% [29.1, 76.8]＝ほぼ五分**（`hand=800` は **0-8-0**）。
  ⇒ **多次元の調整は `S-6`** へ。🆕**道具を2つ残した**＝`--a-set "key=数値"`（`patchCpuPolicy`）＋ポリシー × 山の表＝**1候補8分**で評価できる。
...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-6`**（自己対戦で重みを学習＝道具は揃った＝1候補8分）／または **`S-17` 第3段**（ライフバースト・ガードを確率で見る）。次に `S-22`・`S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 5**（`S-6`・`S-17`・`S-22`・`S-24`・`S-14`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**探索は WD13 でだけ弱い**（`S-25` でクローズ＝単一の重みでは直らないと実測・`S-6` へ引き継ぎ）②**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）③**自己対戦がマリガンを一度も踏まない**（`S-24`）。🏁`S-19`（先攻不利）は**再計測で否定**＝山の性質だった |
...
| 🔥`S-6` | 機械学習①＝自己対戦で `WEIGHTS`／`PLAN_WEIGHTS`／盤面の採点の重みを調整。🆕🔑**2026-09-21＝道具が揃った**＝**`--a-set "key=数値"`**（`patchCpuPolicy`＝コードを変えずに候補を作れる）＋**ポリシー × 山の表**（`--decks`）＝**1候補の評価が6デッキ × 8シード＝96戦で8分**。🔴**1本ずつ動かす探し方は `S-25` で外れた**（`fieldPowerScale`／`hand`／`openLane`／`turnDamage` の4本とも差なし）＝**重み同士の干渉を前提に多次元で探す**。🆕**2026-09-19＝`S-5` が終わって着手可能**（`npm run selfplay -- --games N --seed S` で勝率が取れる＝1戦 約35秒） | 大 | 手で決めた重みを勝率で置き換える |
