...
- **🆕 セッション（2026-08-11・続き432〜434・Opus 5）＝§6.4 の宿題を残0にし、UNKNOWN を2バッチ消化。golden 1800→1809・census 854→852・UNKNOWN 36→30**。
  - **🔴① 続き432＝登録されていた宿題の1つは「穴」ではなかった**＝「起動コスト増加がトラッシュ起動に未適用」は**仕様どおり**（原文が「センタールリグとシグニの【起】」に限定）。⭐**「未適用の入口」を全部穴と決めつけない＝原文の限定を先に読む。** 適用する3入口／しない5入口を golden で表として固定。**実際の穴はカットイン窓**（同じ【起】能力なのに入口でコストが食い違っていた）。
  - **🔴② 続き432＝`PR-470A` の入れ替え配置がパワー配置制限を素通り**＝⭐**「count は無関係」で止めると同じ funnel の別の判定を見落とす**（`deployLimitBlockReason` は count とパワーの2軸）。併せて離場置換の宣言走査を印字だけ→3軸へ（live 0件なので挙動不変）。
  - **🔴③ 続き433〜434＝UNKNOWN は「未実装」ではなく過剰効果だった**（2バッチとも）。(a)`WX07-050`／`WX16-Re18` の「ターン終了時、レゾナをルリグデッキに戻す」が落ちて**レゾナが場に居座り続け**ていた。(b)**任意コストの文が UNKNOWN になると本体だけ無条件に走る**＝`WX24-P1-065-E1`② は**コストなしで相手の手札を落とし**、`WX25-CP1-091-E2` は**タダで毎ターンエナチャージ**していた。
  - **⭐④ ターン終了処理・離場・支払いは「経路が複数ある」前提で書く**＝レゾナ返却は BattleScreen の**ターン終了2経路**の両方へ（片方だけだと「手札が多いターンだけ戻らない」）。⚠**置いたレゾナはゾーン選択の pause を跨ぐ**ので ctx ではなく state に記録する。
  - **🔴⑤ エナ送りが共通の対象パーサを通していなかった**＝BANISH は `parseSigniTarget` を通すのに SEND_TO_ENERGY は narrow regex で filter を手組みしており、「ダウン状態の」が UNKNOWN・「同じレベル」が脱落（教訓 (i) の再来）。⚠**`IDENTITY_BATCH5B` は付与能力の内側に届かない**（実測）。
  - **⭐⑥ キー名で探すと「無い」と誤読する**＝「同じレベル」11カードを `levelEqDiscardSigniOffset` で探して「全部落ちている」と読んだが、実際は `levelEqDiscardLevelSum`／`levelEqLastProcessed` の**別キー**で載っていた（罠 (h) の実例）。**直す前に別綴りを確かめる。**
  - **⭐⑦ parser は「届く断片」でアンカーする**＝先頭の「ターン終了時、」は文分割で剥がれる。緩めて単数形まで拾うと別カード群（アシストルリグ返却）を巻き込むので、原文全数を確認して限定した。
  - **✅⑧ ゲート**＝`npm run gates` 全緑（typecheck / golden **1809（+9）** / smoke 10688 全0・SKIP 0 / fuzz 全0 / census **852（-2・ベースライン締め直し）** / census-stubs / manual-fields / lint 0 errors）。`npm run regen` 済み。**§6.4「消化済み項目が残した宿題」は残0**。
  - **📋⑨ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 残30ノード/29カード**（**引き続き過剰効果になっているものから**取る）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**置換の「してもよい」辞退**（ライフクラッシュ置換側）。**Sonnet 側**＝§7 実機検証（**続き431・434 の回帰確認＝「タダで撃てない」ことを最優先**）。
...
- **UNKNOWN（部分未実装・逆翻訳に `【未実装/UNKNOWN】` として露出）**：**🆕2026-08-10 続き414 に live JSON を全数走査して数え直した実数＝40 ノード／38 カード**（⚠**旧記述「42箇所／31カード」は decompile シートの出現数**で、live のノード数とは別物＝**計器は live JSON を数える**）。**続き414〜415 で 43→38**。
  ...
  - **📋 残り30ノードの内訳（続き434 実測）**＝(a)**デッキ公開→処理の変形 2件**（`WX21-028-E1`＝「その中から**すべての**＜天使＞のシグニをエナゾーンに」＝**枚数語が名詞より前**で `parseRevealPickDescriptor` の語順に合わない／`SP38-006-E1-G2`＝主語が対戦相手で1文に「見る」も含む）(b)残りは**単発**（1カード1機構）。
- **🆕「代わりにそれを〜」動詞昇格の残（続き418 で登録）**：(a)🔴**`WX25-P3-014-E1` は `BOUNCE{owner:'self'}` の自傷が残る**＝原文「能力を持たない対戦相手のシグニ１体を対象とし、《白》を支払ってもよい。そうした場合、それを手札に戻す。それがレベル２以下の場合、代わりにそれをトラッシュに置く」。base が任意コスト STUB を含む多段構成で、対象プロパティ版の `SELECT_TARGET_ONLY→STORE` 組み替えが `execSequence` の「そうした場合」did-it ゲートと干渉するため据置した（**SELECT/STORE はコストステップより**前**に挿す**という既存の制約に合わせる必要がある）。(b)**逆翻訳の表示**＝対象プロパティ版が `LAST_PROCESSED_MATCHES` の汎用文「この方法で〜を1枚以上処理したなら」で出る（意味は正しいが原文の「それが〜の場合」ではない）＝`census:stubs` の**C群（表示だけの穴）**扱い・優先度低。
...
- **続き426（任意コスト脱落トリップワイヤの残0化）が持ち込んだ未検証UI 3件**＝**pay/skip モーダルが新たに出る**3カード。golden は構造を固定したが提示・徴収は踏めない。**⚠2026-08-11（続き435）＝`verifyBattleDrive.mjs` にシナリオ6件を試作し実機ヘッドレス検証を試みたが、skip側3件中2件のみPASS・pay側は全件ドライバのクリック手順バグで未完成**（`fezoneDoubleCostSkip` PASS＝skipで青1/手札2/対象すべて不変を確認。`connectSpinningChoice4Pay`／`connectSpinningChoice4Insufficient`／`fezoneDoubleCostPay` はFAIL＝いずれもピース/エナ選択UIのクリック対象特定でスクリプトが詰まり本題（pay成立可否）まで到達できていない＝**engine側のバグとは確認できていない**、シナリオ側の作り直しが必要）。
...
- **続き434（§6.4 UNKNOWN＝任意コストの落ち）が持ち込んだ未検証UI 3件**＝**3枚とも従来「タダで撃てた」**ので、**まず支払いを要求されることの確認**から見る。**⚠2026-08-11（続き435）＝`verifyBattleDrive.mjs`にシナリオ試作。`WXEX2-20`は完全PASS、`WX24-P1-065`②はskip側のみPASS（pay側はドライバ未完成）、`WX25-CP1-091`は全パターンFAIL（ドライバ未完成）**。
  - [ ] 🔴**`WX24-P1-065`②＝コストを払わないと相手の手札が落ちないか**（回帰確認）＝自分のアタックフェイズ開始時に「①／②」の選択が出て、**②を選ぶと「手札からスペルを1枚捨てる／捨てない」の pay/skip が出る**。**捨てなければ相手の手札は減らない**。⚠**支払いを聞かれずに相手の手札が減ったら回帰**。⚠候補に**スペルだけ**が出ること（シグニが選べたら限定漏れ）。**skip側は確認済みPASS（`sYokusenkiSpellSkip`＝自分のスペル・相手手札とも不変）。pay側（候補限定・相手手札-1）は未確認（`sYokusenkiSpellPay`がドライバ未完成でFAIL）**。
  - [ ] 🔴**`WX25-CP1-091`＝コストを払わないとエナチャージされないか**（回帰確認）＝自分のターン終了時に「このシグニの下から＜ブルアカ＞のカードを3枚トラッシュに置く／置かない」の pay/skip が出て、**置かなければエナチャージされない**。⚠下に＜ブルアカ＞が3枚無いときは**支払い枝が出ない**こと（部分払いで撃てたら抜け穴）。**未確認（`kokonaUnderThreePay`/`ThreeSkip`/`Insufficient`いずれもドライバ未完成でFAIL＝「ターン終了」ボタンのクリック手順に問題がある疑い）**。
