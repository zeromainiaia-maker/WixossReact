...
| (xcv) | **🆕 engine の「能力を持たない」判定が素のシグニ158枚を1枚も拾えない（2026-08-04・(xcii) 実装中に発見）**＝CSV は能力なしシグニを**空文字ではなく `-`** で持つ（実測158枚・空文字は0枚）のに、`execStubPart2.ts:2238`（`NO_ABILITY_SIGNI_TO_DECK_BOTTOM`）と同 `:2427`（`ABILITY_CHECK_ELSE_TRASH`）は `!!EffectText`／`!!EffectText.trim()` で判定する＝**`'-'` が truthy なので必ず「能力あり」に倒れ、当該分岐が一生不発**。対象は live 5カード（`WXEX2-30`／`WX25-P3-038`／`WX25-P3-069`／`WX25-P3-072`／`WX25-P3-073`）。**修正は `trim() === '' \|\| trim() === '-'` の共通ヘルパー1本に寄せる**（costs.ts 側は (xcii) で対応済み＝そちらと同じ判定にすること）。⚠ついでに**CONTINUOUS `REMOVE_ABILITIES` による能力喪失**（「凍結状態のシグニは能力を失う」等）を数えるかも決める＝engine は `collectRemovedAbilities` を持つので engine 側なら合わせられる |
...
- **🆕 セッション（2026-08-04・続き341・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(xcii) 残0クローズ**＝相手の盤面を参照するコスト軽減8枚を実装（ユーザー指示「(xcii) を行う」）。golden **1346→1350**・**live JSON 変更なし**・census 1288 据置・全ゲート緑。
  - **🔴 登録時の見立て「シグネチャ変更の1バッチ」は外れた**＝「`computeArtsEffectiveCost` の引数に相手 `PlayerState` を足す＝全呼び出し元へ波及する」と書いてあったが、**呼び出し4経路はいずれも既に `{ oppState: op }` として相手 `PlayerState` を丸ごと渡していた**。**受け取る側の型（`CostReplaceCtx.oppState`）が `turn_arts_used`／`actions_done` の2フィールドだけで、残りを捨てていただけ**＝受け口に `field`／`life_cloth`／`coins`／`abilities_removed` を足すだけで全経路に届いた（**呼び出し側の変更ゼロ**・位置引数9個を10個に増やさずに済んだ）。
  - **実装＝8枚**：I-1 合算形2（「＜X＞のシグニ1体**か**相手の〔語〕1つにつき」＝**「か」は択一ではなく両方の合計**）／I-2 累積形1（`WX16-033`＝2つの軽減が重なる＝早期 return できない）／I-3 相手のみ3（凍結／【ウィルス】／能力なし）／I-4 相手コイン1／I-5 ライフ枚数比較1。⚠**I-1・I-2 を I-3 より先に見る**（どちらの原文も I-3 に部分一致する）。⚠**regex はカード全文でなく「この{スペル|アーツ}の使用コストは…」の文だけに当てる**（全文だと他カードのコストを下げる文を巻き込む）。
  - **🔴「能力を持たないシグニ」＝CSV は空文字ではなく `-` で持っていた**（素のシグニ実測**158枚**・空文字は**0枚**）＝`!!EffectText` 系の判定では**1枚も当たらない**。costs.ts 側は `'' or '-'` 判定＋`abilities_removed` も数えるようにした。⚠**engine の既存2箇所は取りこぼしたまま**＝新在庫 (xcv)。
  - **🔴 ルリグデッキのカードをタップした経路だけ実効コストを見ていなかった**＝`getMyLrigDeckCardActions` のアーツ枝は置換とカード名指定軽減しか通さず、**条件つき軽減〔(xc) の37枚＋今回の8枚〕と場の CONTINUOUS 軽減を素通り**＝**同じアーツが一覧からは使えるのにカードをタップすると使えない**（(lxxxvii) のカットイン窓と同じ食い違い）。⇒ `ArtsModal` Phase1 と同じ式へ統一。
  - **過剰適用ゼロを機械保証**＝ベースライン `ecfc253d` の関数と現行を **1236枚 × 3盤面**で総当たり A/B ⇒ **差が出たのは意図した8枚だけ**／空盤面 **0枚**。golden に**相手盤面フルの全数走査**（動くのは8枚だけ・期待値をコスト文字列まで固定）を常設。
  - **新規在庫2件**＝**(xcv)** engine の「能力なし」判定が `-` を取りこぼす（live 5カードの分岐が一生不発）／**(xcvi)** `ArtsModal` Phase2 のカード名指定軽減**二重適用**（今日は対象アーツ0枚で休眠する時限バグ）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxviii)／(xciii)／(xciv)／**🆕(xcv)**／**🆕(xcvi)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜341 の全件が実機UI未検証**。⚠🔴**続き333〜341 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①`WX07-065` が相手の凍結シグニ体数ぶん安くなる**②`SP26-003` が相手の【ウィルス】数×《黒×2》安くなる③`SP38-002` がライフ優位のときだけ《無×3》安くなる④**ルリグデッキのアーツを直接タップした場合も一覧と同じコストで使えること**。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-04 続き341（🏁§3 タスク12(xcii) 残0クローズ＝相手の盤面を参照するコスト軽減8枚）後 最新値（本行が直近の正）**：golden **1346→1350**（+4＝①相手の場を数える軽減〔凍結2体・**シグニ不在ゾーンの凍結フラグは数えない**・【ウィルス】2つ・素のシグニと能力持ちの区別・`abilities_removed`〕②合算形の自分側/相手側/両方0と累積形の3状態③相手コイン枚数とライフ枚数比較〔多い/同数/少ない/**相手状態が無ければ減らさない**〕④**相手盤面フルの全数走査＝アーツ/スペル/ピース1236枚のうち動くのは8枚だけ**を期待コスト文字列まで固定）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**（⚠ローカルで248に見えるのは**未追跡の残置ファイル `scripts/_dbgFresh.ts`（3件）**が混じるため＝ベースライン commit の worktree で実測して245を確認済み。本作業の増分は**0**）、manual field loss 0、同型★0・held **258枚／109群**（いずれも前回据置＝**parser も JSON も触っていない**）。**live JSON は完全不変**（`public/data/` に差分なし・**新語彙0本**）＝触った層は `costs.ts`（`CostReplaceCtx.oppState` の型拡張＋規則5本）と `BattleScreen.getMyLrigDeckCardActions`（実効コスト算出を `ArtsModal` Phase1 と同式へ統一）のみ。⚠**コスト表示・請求は golden 非カバー**（純関数 `computeArtsEffectiveCost` までは固定済み）＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-04 続き341＝🏁§3 タスク12(xcii) 残0クローズ後）**＝golden **1350**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
