...
- **🆕 セッション（2026-08-01・続き315・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxxv)＝主語形「対戦相手は（自分の）デッキの上から…トラッシュに置く」を相手ミルとして読む。**在庫は1件で登録されていたが実測7効果**が同じ向きに壊れていた（**自分のデッキを削る自傷＝符号が逆**）。残0クローズ**（golden **1226→1229**・census **1354→1353**（`BASELINE_HIGH=1353`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **250枚/98署名 据置**。live per-effect 差分 **changed 7 / added 0 / removed 0**／`build:effects`・`regen` 冪等。**engine 改修0・新語彙0本**）
  - **① 🔴 在庫の旧診断は外れ**＝「`buildChoose` が item を単独パースする際に主語が落ちている疑い」とあったので、まず指示どおり CHOOSE を持つ471効果の item 主語落ちを機械走査したら**1件だけ**。しかもその1文を**単独でパースしても同じ結果**になる＝**CHOOSE は無関係**で、真因は `parseSentencePart1` のミル規則が所有格「対戦相手**の**デッキ」しか見ていない**一般欠落**だった。
  - **② 母集団は 1→7**＝`WX08-020-E1`／`WX18-037-E1`／`WX25-CP1-024-E1`／`WXDi-P07-007-E3`／`WXDi-P10-003-E1`／`WXDi-P14-005-E1` ③／`WXEX2-27-E3`。⚠**走査 regex を「置く」で切ると `WX25-CP1-024-E1`（「置いて**もよい**」）が漏れる**＝語尾の揺れで母集団が変わるので、数えたあとに必ず「広げた regex でもう一度数える」。
  - **③ 誤爆よけ2枚**＝①「対戦相手」と「デッキ」の間に**「あなた」が入る文は除外**（「対戦相手がスペルを使用したとき、**あなたの**デッキの上から…」＝トリガー句に相手が出るだけ）②「あなたのデッキの上から」明示なら除外。どちらも golden で固定。
  - **④ 連用中止の分割で主語が消えていた**＝「対戦相手は A し、B する」を SEQUENCE へ割る既存 splitter が**右半分から主語を落とす**（`WXDi-P10-003-E1`）。元文が「対戦相手は」で始まり右半分に「あなた」が無いときだけ主語を前置するようにした。**blast radius は実測1カード**（held が +1 しか増えない）。
  - **⑤ 🆕 新規登録1件**＝**(lxxvii)** `WXDi-P10-003-E1` は原文4動作のうち**中間2つ（シグニ・エナ）が丸ごと落ちている**（連用中止 splitter が2分割しかしないため）。本波は owner の符号だけ直した＝自傷→正しい向きの strict improvement で、動作数の欠落は別問題として送出。
  - **次の一手**＝**Opus**：①**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く見込み＝現在の筆頭）②**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）③**(lxxvii) 連用中止のN分割**（⚠共有部品なので**母集団を数えてから**）④**(lxxii) 遅延クリアの片側漏れ**⑤(lxxiv)⑥(lxviii) 散文形「対戦相手のターンの間、」⑦§6.3 J 残11 ⑧(lxvii) CPU `ON_TURN_END` 未収集。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜315 の全件が実機UI未検証で積み上がっている**（配置モーダルの活殺〔第10波・(lxxvi)〕と跨ぎ選択モーダル〔第11波〕は**目視しやすい**ので取りやすい）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1229**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1353**（回帰ゲート）・held **250枚/98署名**（2026-08-01 タスク12(lxxv) 実働化後に実測）。
>
