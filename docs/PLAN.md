...
| (lxxxiii) | **第3波まで一部消化（2026-08-02）**＝第3波は action対象のうち engine が filter を実評価する live **3効果**（`WD14-011-E2`／`WX25-CP1-074-E1`／`WXDi-CP01-038-E1`）を是正し、`GRANT_PROTECTION` の additive `excludeSelf` 読取も実装。再測＝母集団167、あり122／**残45**（STUB混在29・owner:any混在9）。残45は誤検出5、トリガー側5、STUB/対象保持・OR・引用・同一対象照応待ち（分類重複あり）として全ID・理由を BUGFIXES 第3波節に記録。 |
