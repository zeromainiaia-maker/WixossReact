...
| 14 | リファクタ Stage2→Stage3 純粋バトルコントローラ | BattleScreen構造 | L | ✅Stage2完了＋永続化移行完了（battle_states 全行 I/O 120箇所を `persist` へ移行）＋**reducer 17 action・102/117 commit 経由**（続き244-247／続き271＝13箇所／🆕2026-08-03＝5バッチ計31箇所）。設計/移行レシピ＋**定形6つ** `docs/BATTLE_CONTROLLER.md`。**残＝reducer純粋化の本体（15 commit・うち1件は実質移行済み）**＝**残るのは (a) の可変アキュムレータ型がほぼ全て**（`doPhaseAdvance`／`resolveStackNext` の前半／CPUターン強制終了＝`'host_state' in update` の読み戻し・`update.effect_stack` ベース push・`update.turn_phase` 読み分岐。**フェイズ別ハンドラ分割が先**）＋アーツ/レゾナ召喚の spread 数件。✅(b) 効果解決本体＝`RESOLVE_EFFECT_STEP` で残0 ✅(d) CPUターン終了＝`BEGIN_NEXT_TURN` で消化。⚠ハンドラ側 payload 構築は golden 非カバー＝機械一括変換不可・1件ずつ手動レビュー要（レシピ BATTLE_CONTROLLER.md §4）。**進め方＝「挙動同値と確信できるものだけ N 箇所」に必達を絞る**（続き271 は codex 実装／Claude 照合、2026-08-03 は Claude 自身が1バッチ7箇所。どちらでも可）。**次の一手＝残るアーツ/レゾナ召喚の spread 数件（`apsUpdate`／`resonaSpellCutin` 三項／`extraUpdate`）を先に掃き、最後に (a) の可変アキュムレータ4箇所**（`doPhaseAdvance`／`resolveStackNext` 前半／ターン強制終了／マリガン系）＝**これはハンドラのフェイズ別分割という別作業**なので、着手時に scope を切り直す |
...
- **🆕 セッション（2026-08-03・続き332・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝§3 タスク14（Stage3 純粋バトルコントローラ）を5バッチ・31箇所前進**（ユーザー指示「Claude が少しずつ行って」→「続ける」×4）。**reducer 経由 72/117 → 102/117（未移行 45→15）／action 14→17／golden 1317→1326。差し戻し0・全バッチ全ゲート緑。**
  ...
  - **残＝15 commit**（grep ベース。うち1件は `reduceBattle` の結果を変数経由で commit する実質移行済み＝じゃんけんの `setTimeout` 経路）。✅(b)＝残0・✅(d)＝`BEGIN_NEXT_TURN` で消化。**残るのはほぼ全部 (a) の可変アキュムレータ型**＝`doPhaseAdvance`／`resolveStackNext` 前半／ターン強制終了／マリガン系（`'host_state' in update` の読み戻し・`update.effect_stack` ベース push・`update.turn_phase` 読み分岐）。
  - **次の一手**＝**Opus**＝①**タスク14 の残り**＝まずアーツ/レゾナ召喚の spread 数件（`apsUpdate`／`resonaSpellCutin` 三項／`extraUpdate`）を掃き、**最後に (a) の可変アキュムレータ4箇所**＝これは**ハンドラのフェイズ別分割という別作業**なので着手時に scope を切り直す（payload 化では落ちない）②またはタスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxi)〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）。**Sonnet**＝§7 実機検証＝**続き298〜331 の全件が実機UI未検証**（特に第6波で 44効果の選択UIが変わった＝「N個まで選択」＋スキップが新たに出る）。⚠**タスク14 は BattleScreen の構造変更なので、実機のセットアップ〜ターン終了までの通し確認が入ると安心**（じゃんけん／ルリグ選択／マリガン／ENDフェイズ手札上限／ガード／ライフクラッシュの経路を今回触った）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き332（§3 タスク14 Stage3・5バッチ）後 最新値（本行が直近の正）**：golden **1317→1326**（+9＝`RESOLVE_EFFECT_STEP` 4本〔継続/完了・settle の3分岐・スペル解決のキー集合・**新スタックが settle に勝つ順序**〕／`BEGIN_NEXT_TURN` 2本〔`turn_count` +1・**追加ターンは `active_user_id` を書かない**〕／`ADVANCE_TURN_WITH_STATE` の opp/effectStack 任意／`WRITE_STATE` の条件式 undefined／`RESOLVE_JANKEN` の勝敗・あいこ非対称）、census **1289 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0、held **259枚／110群 据置**。**parser/effects JSON は完全不変**（BattleScreen 構造のみ＝`public/data/` に差分なし・live per-effect changed 0・**新語彙0本**）。**reducer 経由 72/117→102/117・`BattleAction` 14→17種**（+`BEGIN_NEXT_TURN`／`RESOLVE_EFFECT_STEP`／`RESOLVE_JANKEN`）。⚠**ハンドラ側の payload 構築は golden で検出できない**（golden は純粋関数のみ）＝**この作業は実機通し確認と対で締める**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き332＝§3 タスク14 Stage3 5バッチ後）**＝golden **1326**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1289**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **259枚／110群**。
>
