...
| 5 | 小口持ち越し（隙間埋めに最適） | 単点（parser/engine/decompiler混在） | S×件数 | **置換else系統を現行73効果で全数確定・A10/B3/C13/OK47（続き267）**。分離pick resolverのparser変更は全5971カードbefore/afterで **32カード改善・退化0**（当初の「2効果」は主対象だけの過小報告）。うち旧freshと一致していたAUTO/PARTIAL **23効果をcuratedへaction限定で外科採用**し、filter/pickCount/pickUpTo/REVEAL_AND_PICK統合を実戦へ反映（続き268、golden 749→750、census 1545→1537）。候補0件のremainder移動もengine是正。**残＝A 8**（WX15-029／WXDi-D09-H29／D09-P25／P03-063／072／080／P14-025／WXK06-027）、**B 3**、**C 13**はhonest defer。詳細 BUGFIXES 続き267/268 |
