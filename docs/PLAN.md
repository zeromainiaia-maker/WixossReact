...
- 🏁**セッション（2026-08-27・続き688・Opus 5 単独）＝Sheet1 の段2 残 OPEN を**1バッチで21件**消化した**
  （ユーザー指示＝①段2 を Sheet1 に絞る ②`gates` は最後の1回だけ ③簿記を圧縮 ④1巡の処理量を増やす）。
  📊**進捗3計器＝Sheet1 要対応 65→49 / 863（7.5%→5.7%）｜台帳 残 OPEN 629→609｜census 高シグナル 535→532**。
  - 🔑**規模を上げられた理由＝「探索を1回にまとめた」こと**。Sheet1 の 47カード（58 findings）を**全部読んでから**
    「**受け皿が既に engine にある形**」だけを選んで1バッチにした。**判断は1件ずつ従来どおり**で、
    変えたのは固定費の払い方＝`build:effects`／`heldReview`／`regen`／`gates` を**巡の最後に1回だけ**払う。
  - ✅**engine の新設は2つだけ**＝`execExile` の `ENERGY_CARD` 分岐（無言 no-op だった）と、
    `execTrash` の `count:'ALL'` 経路が `optional` を honor する枝（規則名が「（任意）」なのに強制だった）。
  - 🔴**`parseStoryFilter` が全文を舐めて条件節のクラスを対象へ引き込んでいた**（`WX10-062-E1`）＝
    位置で切る修正を入れ、**live A/B の巻き添えはゼロ**（22 effectId / 21カード＝意図した差分だけ）。
  - 🔴**TS 文字列に `\d` を書くと `d` になる**（全角 `[０-９]` のおかげでテストは通り、**半角だけ静かに外れる**寸前だった）。
  - ⚠**実機 driver の `build スキップ`**＝dist が古いまま走ると**自分の変更が入っていない画面**を検証してしまう
    （`deckshufflespell` の一度目の FAIL はこれ。`SKIP_BUILD=0` で PASS）。

**▶ 次の一手**＝**同じ取り方をもう1巡回す**（Sheet1 の残 findings は **38件 / 29カード**）。
着手は `npm run census:cards -- --sheet 1 --list` → 該当カードの原文・live・findings を**まとめて読む** → 受け皿の有無で仕分け。
⚠**残りは「受け皿が無い」側が濃い**見込み＝機構が要るものは §5.3 の `O-107`〜`O-110` に登録済み（本巡で分離した4件）。
機構側なら **`O-107`**／**`O-108`**／**`O-109`**／**`O-110`**／`O-106`／`O-105`。実機の腐り返済なら §5.1 の `V-89`／`V-90`。
---
...
| **O-107** | 🆕**`REARRANGE_SIGNI` に `optional` が無い（実測1効果）** | S | **2026-08-27 Sheet1 B10 で分離**＝`WX05-046-E1`「【出】手札から＜迷宮＞のシグニを１枚捨てる：対戦相手のすべてのシグニを好きなように配置し直して**もよい**。カードを１枚引く。」■`RearrangeSigniAction`（`src/types/effects.ts:2136`）に `optional` フィールドが無く、**コストを払った時点で再配置が強制**になる。■**取り方**＝型に `optional?: boolean` を足し、`execRearrangeSigni` に「配置し直す／しない」の CHOOSE を1本（同日 B10 で `execTrash` の `count:'ALL'` 経路へ入れた枝と同形）。**parser 側は「配置し直してもよい」を読むだけ**。■⚠**母集団は着手時に「配置し直してもよい」で全数実測する**（B10 の実測ではこの1効果）。 |
