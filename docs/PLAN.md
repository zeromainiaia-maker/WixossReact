...
- **🆕 セッション（2026-08-11・続き441・Opus 5）＝§6.4「`$ref` が消費側で黙って0に潰れる」死角を一掃。live 6効果の恒久不発を解消し、再発防止トリップワイヤ2本を設置。golden 1826→1834・census 847→846**。
  - **🔴① バグの正体＝engine に数量解決関数が2つあり、片方が `$ref` を警告なく捨てていた**＝`resolveCountRef`（`execUtils.ts:122`）は解くが `resolveNum`（`:118`）は `typeof n === 'number' ? n : 0` ＝**object ref を問答無用で 0**。⚠**逆翻訳は JSON を読むので「同じ枚数」と正しく描画される＝原理的に気付けない偽陰性**で、golden・census・smoke・fuzz も全緑のまま素通りしていた。**engine 4サイト**（`execBounce` 非ALL／`execTrash` の `HAND_CARD{blind}`・`ENERGY_CARD`／`execTransferToHand` 非ALL）を `resolveCountRef` へ。⭐**固定値には厳密に恒等**（`:136`）なので既存挙動は不変＝これが安全な理由。
  - **🔴② 実害＝`WXDi-P10-008-E3` は《緑》×3+《無》ゲーム1回の大技なのに相手のエナが1枚も落ちず**、`WXK01-038-E1` は**スペルの主目的（相手の手札破壊）が0枚**、`WDK05-T07-E1` は**アーツ後半が丸ごと不発**だった。**同居していた別軸3件**も是正＝`WX24-P1-014-E2` は🔴**ゾーン違い**（原文は相手エナ→相手手札なのに**場のシグニ**をバウンス）／`WXDi-P13-007-E3` はディソナ限定の欠落（直さないと**過剰**化する）／`WX24-P2-003-E1` は🔴**`$ref` と固定値が2ステップ間で入れ替わり**。
  - **⭐③ 計器＝「消費関数へ紐づける」棚卸しが効いた**＝live の `$ref` 全26箇所を (祖先 action 型, キー) で抽出し、`resolveNum`/`resolveCountRef` の全呼び出しを**囲む関数名**へ紐づけて突き合わせると **20件は正常・6件が不発**と一意に割れた。**解決関数は関数単位で分かれており、`execTrash` だけがゾーン分岐で NUM/REF 混在**という最も危険な形だった。
  - **🔑④ トリップワイヤ2本が本体**＝C1（live の `$ref` 位置を「消費側が `resolveCountRef` と確認済み」のホワイトリストに限定）＋C2（「枚数キーを `resolveNum` で解く関数」の集合を凍結＝増えても、直した4関数へ戻っても赤）。⚠**残る30関数は live に `$ref` が1件も無いので予防的置換はしない**判断（C1 と対で守る）。
  - **⭐⑤ E2E golden は「修正前に落ちること」を検算して初めて価値が出る**＝engine を一時的に修正前へ戻すと**6本すべてが「0枚」で落ち**、C2 も退化を検出した。⚠このバグは逆翻訳に映らないので**JSON 構造 assert（`$ref` が入っているか）は無価値**（修正前から入っていた）＝盤面を組んで「何枚動いたか」を見る以外に検出手段が無い。
  - **⚠⑥ `WXEX1-44-E2` は defer 継続だが理由が入れ替わった**＝`resolveNum` は解消済みで、真のブロッカーは**前段 STUB が原文と別機構**（原文「**手札から**《アクセアイコン》を持つシグニ2枚まで」に対し `PLACE_ACCE_SIGNI_TO_ENERGY` は**場のアクセゾーンを全部**エナへ送る）。⚠**アクセは CardClass ではない**＝専用フィルタ `hasIcon:'アクセ'` が既にあり、要るのは**手札選択の機構だけ**。
  - **🔴⑦ PLAN の要約が原記録から劣化していた**＝本項の旧記述「前段の STUB は枚数を記録する**ので**通る」に対し、**続き440 の BUGFIXES には「数量参照だけ直しても前段は一致しない」と正しく書かれていた**。⇒ **PLAN の1行サマリを根拠に着手せず、原記録（BUGFIXES）を読む**。⚠実際この劣化のせいで Codex は自分が続き440 で出した正しい結論を覆して誤採用した（検証側が差し戻し）。
  - **⚠⑧ Codex は使用量上限で停止（リセット 2026-08-18 13:41・`.codex-work` は要再ログイン）**＝engine 4サイトと parser 群Bまで到達し、E2E golden・トリップワイヤ・検証は未着手のまま停止したので Claude 側で完成させた。⚠**自動コミットフックが未検証の中断成果を `47a1e053d` としてコミットした**ため、検証ベースラインは `HEAD` ではなく **`5f7c34b3c`** を使った（CODEX_GUIDE §2 の既知の罠を実際に踏んだ）。
  - **▶ 次の一手**＝**Opus 側**：§6.4 の残りは (a)**CPU AI 拡張**（唯一の大物・単独フェーズ扱い）(b)**UNKNOWN 残28ノード**のうち engine 機構待ち3件（`REVEAL_UNTIL_TO_HAND` の repeat／SEARCH 後の `INTERNAL_KEEP_ON_DECK_TOP`／`owner:'opponent'` の SEARCH ルーティング）(c)`WX24-P3-033-E1` の2群振り分け（型と engine の追加が要る）(d)`WXEX1-44-E2` の手札選択機構。**Sonnet 側**：`verifyEffects` の「アクション[STUB代替?] 5件／[要確認] 3件」未仕分け（名前照合の誤検出が混じっている見込み＝計器の精度改善）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き441（`$ref` 消費側0潰れの一掃）後 最新値（本行が直近の正）**：census **846**（847→846・`BASELINE_HIGH` 更新済み）、golden **1834（+8＝E2E 6本＋トリップワイヤ2本）**、smoke **10688 / SKIP 0**、**同型★ 0**（グループ265）、held（parserWorklist）**107枚 / 署名グループ 47件**（増減0）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 28ノード / 27カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群（無言 no-op）**0**、**live の `$ref` 26箇所＝全て `resolveCountRef` が解ける位置**（C1 が固定）、**枚数を `resolveNum` で解く関数 30個**（C2 が凍結・live に `$ref` 無しで無害）。生パース effect 単位 diff **3効果・outlier 0**。
...
- **🆕【マジックボックス】設置の欠落（続き438 で消化・残1）**：原文に「【マジックボックス】として…設置し」を持つ**7カード中4カードで設置ステップが live JSON から丸ごと落ちていた**（`WX24-P3-072-E1` は `LOOK_AND_REORDER{count:3, dest:deck/bottom}` **だけ**＝**デッキを3枚見て下に戻すだけでカードの主目的が何も起きない**過少実行）。**✅2026-08-11 続き438 で3件消化**（`WX24-P3-072-E1`／`067-E1`／`070-E1`）＝`REVEAL_AND_PICK{pickCount:1,pickUpTo:true,then:STUB{PLACE_MAGIC_BOX},remainder:{deck,bottom}}`。census 851→848・golden +4。engine は `PLACE_MAGIC_BOX`（`execStubPart3.ts:1257`）→`INTERNAL_SET_MAGIC_BOX`（`:1272`）が**元から完備**で、parser が繋いでいないだけだった。
  ...
  - 📋**残1＝`WX24-P3-033-E1`（取捨炎択）**＝「1枚まで**設置**し、**＜トリック＞のシグニ1枚を公開し手札に加え**、残りを下」＝**移動先の異なる2群振り分け**。既存 `transferGroups` は「同一移動元から複数条件を**手札へ**」の表現で**別々の移動先を表せず**、`LOOK_PICK_CHAIN` にも MB 用 stage が無い＝**型と engine の追加が要る**ため据置（**非採用を golden で固定済み**）。⚠**設置だけ足す部分採用はしない**（原文とズレたまま固定されるため）。
