...
**直近＝2026-09-17＝第396バッチ：§5.6 `C-9` 続き＝リムーブのゾーン後始末（`R-41`/`R-49`）とトリガーの解決順（`R-52`）を直す**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**リムーブ（メインフェイズに自分のシグニをトラッシュに置く）だけがゾーンの後始末を1つもしていなかった**＝【チャーム】【アクセ】【ソウル】が浮いたまま残って**カードが消え**、ダウン・凍結も残って**次にそのゾーンへ置いたシグニがいきなりダウン**していた ⇒ `leaveFieldZone.ts` `clearZoneOnSigniLeave` の1本へ（`reduceFieldSigniToLimit` も同じ関数へ）。
- 🔴**解決中に足すトリガーがターンプレイヤー優先で並んでいなかった**（初回の整列だけ正しかった）＝ライフバースト解決は「非ターンプレイヤー → ターンプレイヤー」の順に渡していた ⇒ `pushToStack` も `buildQueue` を通す。
- 一致を確認して 👀＝`R-49`（リムーブの回数・体数）／`R-51`（色コストと無色エナ）／`R-50` の後半2つ（スペル→トラッシュ／チェックゾーン残りはターン終了時）。
- 実機 新規1本（`V-255`）PASS＋修正前のコードで FAIL を確認。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔴**ユーザー判断待ち4件**（[RULES.md](./RULES.md) の `R-27`／`R-45b`／`R-47`／`R-50`＝どれも「読みを決めないと盤面からカードを消す側の実装になる」）／**`O-532`**（リミット超過のルール処理）／**`C-8`**（対話応答の pure 化） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は engine・UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **2**（索引B `O-532`／索引G `O-531`）／実機 **0**／実装キュー **0**／**CPU 完成度 2**（§5.6 `C-8`・`C-9`）／ルール台帳 ⚠5（うち読み待ち3） |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4290） |
---
...
**残0**（直近＝`V-255`＝`c9removecleanup`＝2026-09-17 第396バッチで PASS＋修正前のコードで FAIL を確認。リムーブのゾーン後始末の回帰ガード／`V-253`＝`c9refreshturnend`／`V-254`＝`c9refreshturnendone`（反転＝1回目では終わらない）＝2026-09-17 第395バッチで PASS＋修正前のしきい値で FAIL を確認。2回目のリフレッシュでターンが終わる回帰ガード／`V-251`＝`c9resonabanish`／`V-252`＝`c9lrigtriplecrush`＝2026-09-17 第394バッチで PASS＋**修正前のコードで FAIL を確認**（レゾナがエナへ／トリプルが1枚）。レゾナの行き先とルリグの【トリプルクラッシュ】の回帰ガード／`V-248`〜`V-250`＝`c7cpukey`／`c7cpupiece`／`c7cpupieceonelrig`（反転＝ルリグ1体では使わない）＝2026-09-17 第393バッチで PASS。CPU のキー・ピースとピースの体数ルールの回帰ガード／`V-247`＝2026-09-17 第391バッチでクローズ＝CPU の起動の停止・二重実行。回帰ガード＝`v247AfterCpuRiseNoTrigger`（判別力あり）／`v247AfterCpuRise`／`v247AfterCpuAssistGrow`／`c3cpufirstturngrow`／`V-242`〜`V-246`＝`c2cpuguard`／`c4cpuhandlimit`／`c5cpuassistgrow`／`c5cpuresona`／`c6cpurise`＝2026-09-17 第390バッチで PASS。CPU のガード・手札上限・アシストグロウ・レゾナ・ライズの回帰ガード／`V-240`＝`c9lancerreplaced`／`V-241`＝`c9extraturnup`＝2026-09-17 第389バッチで PASS＋修正前のコードで FAIL を確認。ランサー置換とアップフェイズの受け手の回帰ガード／`V-239`＝`battleequalpower`＝2026-09-17 第388バッチで PASS。同値バトルの回帰ガード／`V-238`＝`bugreport`＝報告導線の回帰ガード／`V-237`＝`distinctlevelshortpick`＝`O-530` の実機ソフトロック回帰ガード）。
...
| 🔥`C-9` | **ルール規則の棚卸し**（バトル／ダメージ／フェイズ／ゾーン移動の行き先） | M〜L | **台帳＝[RULES.md](./RULES.md)**（公式ルールの1文 × 実装箇所 × 状態）。**2026-09-17 着手**＝✅1／🔴→✅9／👀19／**⚠5**（残りは `R-27`／`R-44`＋`R-48`（＝`O-532`）／`R-45b`／`R-47`。うち `R-27`・`R-45b`・`R-47`・`R-50` は**ルールの読み待ち**）。取り方は RULES.md §4（⚠を上から・写経を全部数える・純関数＋golden） |
