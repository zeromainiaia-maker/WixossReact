...
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／**リリース作業 1**（RELEASE.md）／🆕**実機シナリオの既存 FAIL 85**（全件実行で初めて可視化・一覧は [_drive_full_fail_2026-09-19.md](./_drive_full_fail_2026-09-19.md)＝A 相打ち前提12／B 辞退はスキップ5／C 未分類68） |
