...
- **🆕 セッション（2026-08-10・続き409〜415・Opus 5）＝§6.4 を連続消化。①仕分け計器 `npm run census:stubs` を新設し A群（無言の no-op）を6件実装（12→7）②UNKNOWN を2バッチ5効果消化（43→38ノード・census 880→877）**。
  - **⭐① まず計器を作った（PLAN の指示どおり）**＝`scripts/censusStubs.ts`（明細 `docs/_census_stubs.txt`／`--id <STUB_ID>` で1件の完全内訳）。**907箇所 → A 実装の穴 21件（明示 defer 9・無言 no-op 12）／B 宣言型 32件／C 表示だけの穴 259箇所／D 健全**。**「STUB＝未実装」ではないので大半は表示の問題**と実測で確定した。
  - **🔴② 実装軸は4経路ある（計器の較正で判明・実測で5種が誤検出だった）**＝(1)`execStubPart1-3` のハンドラ (2)engine 別経路（CONTINUOUS 宣言型を `effectEngine`/`BattleScreen` が読む） (3)**ペイロードキー**（`PREVENT_POWER_MODIFY_BY_OPP` は id を一度も比較せず `act.powerModifyProtection` だけを読む） (4)**カード番号**（`MAYU_ENCOUNTER_FLIP_AND_GROW` は `screens/battle/mayuEncounter.ts` が `'WXDi-P13-003A'` で分岐）。
  - **🔴③ 「id の前方一致を src から探す」実装は誤検出だらけになる**＝`ASSIST_LRIG_ATTACK_THIS_TURN` が `RETURN_ASSIST_LRIG_TO_DECK` に、`PREVENT_POWER_MODIFY_BY_OPP` が `PREVENT_POWER_MINUS_BY_OPP` に当たった。**向きを逆にして「コード側の matcher（`.id.match(/…/)`／`.id.startsWith()`）を集めて id を test する」**のが正しい（実測でコード全体に matcher は**1本だけ**＝`fieldLimit.ts:65`）。同様にペイロードキーも**汎用名（`text`/`raw`/`costText`）を除外し、そのキーを持つ STUB が2種以下**のときだけ根拠にする。
  - **⚠④ 生成側（parser / manualEffects）の言及は「消費」ではない**＝STUB を作る側なので、そこにしか出てこない id は「作られるが誰も読まない＝真 no-op」。`src/types/` も型宣言であって消費ではない（除外しないと `powerModifyProtection?: {…}` を実装と誤認する）。
  - **✅⑤ A群の初回1件を実装＝`CANNOT_DEAL_DAMAGE_TO_OPPONENT`**（`WX25-CP1-074-E1` が付与する【常】「このシグニは対戦相手にダメージを与えない」）。新設 `src/screens/battle/signiDamageGate.ts`（**印字＋付与2ストアの3軸**走査）を `resolvePendingSigniBattleFor` の**2つのダメージ地点**＝(a) ランサー/Sランサーの追加クラッシュ (b) 正面空・アサシンのライフアタック に配線。⚠**片方だけ止めると「バトルに勝ったときだけダメージが通る」半端な近似**になる。表示語彙も `miscStubMap` に足した（C群 −1）。
  - **✅⑥ A群2件目＝`EXILE_ARTS_FROM_LRIG_DECK_SKIP_SIGNI_STEP`（WXK11-001②「ルリグデッキのコスト合計2以上のアーツ1枚をゲームから除外してもよい。そうした場合、このターン、シグニアタックステップをスキップする」）を実装（続き410）**。⭐**「半分だけ実装済み」を先に探すと安い**＝**後段のスキップ機構は既に完備**で（`BLOCK_ACTION{actionId:'SIGNI_ATTACK_STEP'}`→`screens/battle/attackStepPhase.ts:11`。**同カード①のルリグ側が既にこれで動いていた**）、欠けていたのは任意コスト側だけだった。新語彙 `StubAction.exileArtsFromLrigDeck{count,minTotalCost}` ＋ engine 3段（CHOOSE→`self_lrig_deck` 選択→除外＋後段を `exec` で再利用）。⚠**行先が `excluded`（ゲームから除外＝リフレッシュにも戻らない）なので `trashArtsFromLrigDeck`（ルリグトラッシュ行き）を流用してはいけない**（golden にトリップワイヤ）。⚠**閾値・枚数は parser が構造に載せる**＝engine 側で `text` を再パースしない。**A群 12→11**。
  - **✅⑥-2 A群3・4件目（続き411）**＝`OPP_LRIG_DECK_TO_LRIG_TRASH`（WX24-P4-014②「対戦相手は自分のルリグデッキからカード１枚をルリグトラッシュに置く」）と `PLAY_MILLED_SIGNI_DELAYED_TRASH`（WXDi-P09-079「デッキからレベル１のシグニがトラッシュに置かれたとき、そのシグニを場に出す／ターン終了時トラッシュ」）。⚠**`opponentResponds` は「誰がクリックするか」だけを変える**＝ctx の視点は反転しないので候補も適用先も `ctx.otherState` のまま（表示ラベルだけ viewer 相対に是正した＝`opp_hand` の先例と同型）。⚠行先は **`lrig_trash`**（通常トラッシュではない）。🔴**後者の真因は collector 側**＝`collectMillTriggers` が `milledCardFilter` の**一致件数だけ数えてカードそのものを捨てていた**ので「そのシグニ」が引けなかった＝`triggeringCardNum` に載せた（**この timing の live 16効果のうち triggerSource を読むものは 0** ＝投入前に全数確認してから入れた）。ターン終了時トラッシュは既存 `turn_end_field_trash_targets`、配置制限は `deployLimit.ts` を通す（**直接 `field.signi` に書く実装は配置制限すり抜けの常習箇所**＝golden にトリップワイヤ）。
  - **✅⑥-3 A群5件目（続き412）＝`ARTS_ATTACK_EMPTY_ZONE_AS_FRONT`（WX16-021 驚天動地）**＝「＜英知＞のシグニがシグニのない相手シグニゾーンにアタックする場合、正面にあるかのようにダメージ」。⚠**【側面アタック】は既定で空ゾーンだと何も起きず UI も空ゾーンを提示しない**＝**解決とアタック先ボタンの2箇所**を同じ関数（`screens/battle/sideAttackDamage.ts`）で判定した。クラス限定は live JSON の構造に載せ engine で `costText` を再パースしない。⚠`WX16-021` は `manualEffects.ts` に無い MANUAL エントリ＝**live 直編集が正**（`build:effects` の手修正不可侵で保持されることを実測確認）。🔴**JSON を `json.dump` で書き戻すとファイル全体が再フォーマットされる**（113k 行の差分になった）＝**文字列置換で局所編集する**。
  - **🔴⑥-4 `FLIP_SELF_FACE_DOWN_UP`（WX25-CP1-060）は「半分だけ実装すると no-op より悪い」と判定して defer（続き412）**＝原文は「このシグニを裏向きにし、表向きにする。ターン終了時まで、この方法で表向きになったシグニは**【自】能力を失う**」。**利益＝fizzle（別オブジェクトになって対象が消える）／費用＝【自】喪失**なので、費用だけ実装すると純粋な弱体化になる。⚠**現アーキテクチャでは fizzle が作れない**＝`collectTargetedTriggers` は `BattleScreen:4634` で**効果適用後の盤面**を受け取って発火し ON_TARGETED はスタックで後から解決する＝対象決定をキャンセルする地点が無い。**⭐「実装できる」と「実装してよい」は別**＝A群を機械的に消化しない。
  - **✅⑥-5 A群6件目（続き413）＝`MAGIC_BOX_FLIP_GRANT_ASSASSIN_DC`（WX24-P4-016-E3）＝新 timing `ON_MAGIC_BOX_FLIPPED`**。§4「新規 timing 配線の確立パターン」どおり6段（timing／detector／collector／中央 diff funnel／golden／逆翻訳）。⚠**detector は「MB ゾーンから消えた」だけでは数えない**＝行先がトラッシュか場のシグニであることを要求（`MUGEN_Q_RESET_AND_FLIP` の盤面ごと除外を誤検出しない）。⚠**watcher は印字ではなく「そのターンだけ付与される」ので collector が付与ストアも走査する**（印字だけ見ると恒久 no-op＝§6.3 の教訓）。付与先は `lrig_granted_auto_effects`（`permanentGrant` 無し＝ターン終了時に落ちる。`game_granted_auto_effects` はターン境界クリアが無く持ち越す）。**⭐producer 側（MB を表向きにするカード）は6枚ぶん実装済みだった**＝着手前に producer の有無を確かめると「watcher を作っても永久 no-op」を避けられる。
  - **📌⑥-6 残った A群のうち2件は着手不能と判明（続き413）**＝🆕`CHECK_ZONE_FLIP_FREE_GROW`（WXDi-P16-001A）は**B面 `WXDi-P16-001B` のカードデータが CSV に無い**＝グロウ先が Level/Power/Type を持たない壊れたルリグになる。**機構ではなくデータの欠落**（live effects JSON には B面の効果があるのに `cardMap` に載らず到達不能）。／`ASSIST_LRIG_ATTACK_THIS_TURN`（WX25-P1-048）は**アシストルリグのアタック経路そのものが無い**（per-assist の攻撃済み管理・アタックボタン・ガード応答が要る）＝§6.4 で残る中では最大。
  - **✅⑧ UNKNOWN 第1バッチ（続き414）＝「その中から＜X＞のシグニN枚を場に出し、残りを好きな順番でデッキの一番下に置く」3効果**（`SP27-004-E1/E2`・`WX14-046-E1`）を `REVEAL_AND_PICK{then:ADD_TO_FIELD, remainder:deck/bottom}` へ。**census 880→877**（BASELINE 更新済み）。🔴**副産物＝潜在バグ**＝`parseRevealPickDescriptor` は最初から `dest:'field'` を解けていたのに `makeRevealPickStub` が **`'hand'` へ落としていた**＝この文型を通した瞬間「場に出す」が黙って「手札に加える」に化ける（golden にトリップワイヤ）。🔴**手順の必須知識**＝`buildEffectsJson.ts` の `PRESERVE_STATUSES` は **MANUAL だけでなく PARTIAL も不可侵**なので、**parser を直しただけでは live に届かない**（§6.3 K と同じ第3の死角）。**①parser 修正 →②live の `parseStatus` を `PARTIAL`→`AUTO` へ戻す →③`build:effects` →④`heldReview.mjs --adopt`** の4手が要る。
  - **✅⑨ UNKNOWN 第2バッチ（続き415）＝ライフバースト抑制2効果**（`WXEX1-32-LAYER-E1`／`WX25-P3-036-E1`）。**原因は regex が所有者句を挟む形に未対応**（「その**対戦相手の**カードの…」）。🔴**parser を直すだけでは足りなかった**＝`WXEX1-32` は【レイヤー】付与の **CONTINUOUS** で、既存の受け皿3軸（実行時ターンフラグ／英知限定／プレイヤー付与）の**どれにも載らない**＝そのまま採ると**逆翻訳にだけ出て動かない**。新設 `screens/battle/lifeBurstSuppress.ts` が **`crash_source_card_num`（発生源）だけ**を見て印字＋【レイヤー】付与を走査する第4軸を足した（盤面全体を見る実装は過剰抑制＝golden にトリップワイヤ）。⭐**教訓＝受け皿は effectType ごとに別軸のことがある。採用前に「この effectType でその STUB は実際に読まれるか」を確かめる**（確かめずに採ると `census:stubs` の C群＝表示だけの穴が増えるだけ）。**UNKNOWN 40→38**。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（golden **1750（+9）** / smoke **10686/10686** 全0・SKIP 0 / fuzz 全0 / census **880 据置** / lint 0 errors・**254 warnings 増減0**）。`npm run build:effects`＋`npm run regen` 実施済み。**live JSON 変化は WXK11-001 と WX16-021 の2効果のみ**（続き411 は engine/collector のみで live 変化0）。
  - **📋⑧ 次の一手**＝**Opus 側**＝(a)**タスク12 (cxxi)＝任意性脱落35件**（続き408 の登録分・単独バッチ）(b)**§6.4 UNKNOWN の残38ノード**（次に安いのは**「代わりにそれをトラッシュに置く」2件**＝`WX06-024-BURST`/`WXK08-030-E1`）(c)**§6.4 A群の残7件（着手可能2件＝`ASSIST_LRIG_ATTACK_THIS_TURN`／`UNDER_CARD_AS_ENERGY_COST`。他5件は defer 理由が確定済み）**（`npm run census:stubs` の【A】節が worklist。**着手時はまず後段の機構が既にあるかを確かめる**＝続き410 の教訓）(c)**UNKNOWN 42箇所・31カード**／CPU AI（→§8）(d)置換5本の対話化判断。**Sonnet 側**＝§7 の実機未検証UI（続き403 の14枚＋404 の3件＋405 の2件＋406 の1件＋**409〜411 の3件＝`WX25-CP1-074` のダメージ抑止／`WXK11-001` のルリグデッキ除外 CHOOSE・選択モーダル／`WX24-P4-014` の相手ルリグデッキ選択モーダル／`WX16-021` の空ゾーン側面アタック／`WX24-P4-016` の MB 公開 watcher**）と §5c census 文型バッチ。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-10 続き409〜415（§6.4 STUB 仕分け計器＋A群6件＋UNKNOWN 2バッチ）後 最新値（本行が直近の正）**：census **877**（880→877・`BASELINE_HIGH` 更新済み）、golden **1750（+9）**、**UNKNOWN 38ノード/36カード**（43→38）、held **113カード／48群 据置**、同型★ **0**、smoke **10686/10686** 全0・SKIP 0、fuzz 全0、lint 0 errors/**256 warnings**（⚠**+5 は実数の測り直しぶん**＝旧 254 は `--cache` 込みの値で、キャッシュを消したベースライン実測は **251**。増分2は中央 diff funnel に1ブロック足したことで `useHost`/`useGuest`（**React hook と誤認される名前**の局所ヘルパ）の偽陽性が2件増えたもの＝既存の全ブロックが同じ warning を出している。残り3は未追跡の `scripts/_dbgFresh.ts`）、manual field loss 0、golden 型カバレッジ **128/128 据置**、**live JSON 変化 1 効果**（`WXK11-001-E1` のみ。`npm run build:effects`＋`npm run regen` 実施）。**🆕 STUB 仕分けの実数（`npm run census:stubs`）**＝逆翻訳の `[STUB:…]` **906箇所**（うち生ID露出 **275**）／**A 実装の穴 14種16件（明示 defer 7種9件・無言 no-op 7種7件）／B 宣言型 18種32件／C 表示だけの穴 176種259箇所／D 健全**（初回実測は A 19種21件・無言 no-op 12＝続き409〜413 で6件消化）。⚠**残す教訓**＝(a) 🔴**「実装済みか」を id 文字列の grep で測ってはいけない**＝実装軸は**ハンドラ／engine 別経路／ペイロードキー／カード番号**の4本あり、較正中に実測で5種が「実装済みなのに実装の穴」と誤報された。(b) 🔴**前方一致の探索は向きを逆にする**＝「id の前方一致を src から探す」は `ASSIST_LRIG_ATTACK_THIS_TURN`↔`RETURN_ASSIST_LRIG_TO_DECK` のような他人の名前に当たる。**コード側の matcher を集めて id を test する**（実測でコード全体に matcher は1本だけ）。(c) 🔴**生成側（parser/manualEffects）と型宣言（`src/types/`）は「消費」ではない**＝混ぜると真 no-op が実装済みに化ける。(d) ⭐**`genStubsMd.mjs` の「フォールバック」欄は仕分けに使えない**（軸が1本＋id 正規表現が `[A-Z0-9_]+` で日本語入り id を取りこぼす）。
...
- ~~**`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査＋`verifyEffects` の誤検出除外**~~ **✅2026-08-10 消化（続き408）**＝**33件を1件ずつ仕分けたら真バグ3件／誤検出30件**だった。誤検出6系統をルールで潰し、**`POWER_VALUE_MISMATCH` 6→0／`MILL_COUNT_MISMATCH` 3→0／`OPTIONAL_SUSPICIOUS` 2→0／`EFFECT_TYPE_MISSING_CONTINUOUS` 15→1（残1＝真バグ `WX12-010`）／`verifyEffects` のタイミング 4→0**。**計器の精度改善が本体**（続き407 の教訓「過剰報告する計器は無いより悪い」の実践）。詳細は BUGFIXES 2026-08-10（続き408）。
  - ~~**🔴 副産物＝任意性脱落 35件**~~ **✅2026-08-10 第1バッチ消化（続き416）＝`MANDATORY_SUSPICIOUS` 35→18**。**最大クラスタ＝「〈手札からN枚捨て〉てもよい。そうした場合、…」を parser 規則1本で系統是正**（`applyOptionalHandDiscardCost`・`effectParser.ts`）。素の `TRASH{HAND_CARD,owner:self}` を engine の正準形 **`STUB{OPTIONAL_COST, handDiscard{count,filter}}`** へ置換＝**107カード採用（heldReview 95＋混在カード12）＋ live/`manualEffects.ts` 直編集9件**。census **877→868**。
    ...
    - 🔴**`mandatory:false` も `TRASH{optional:true}` も代用にならない**＝前者は **ON_PLAY 以外では engine が読まず**、ON_PLAY では「コスト無し mandatory:false」が収集フィルタ両方の穴に落ちて**一切発火しない**。後者は「0枚選択」でも直後の「そうした場合」ゲートが通り**払わずに本体が撃てる**（実測 `WX24-P4-050`／`WXK08-052`）。
    ...
    - **golden トリップワイヤ新設**＝「素の `TRASH{HAND_CARD,self}` ＋ did-it ゲート」を全 live から機械検出し、既知リスト外は即FAIL（`FORCED_HAND_COST_KNOWN`）。**残 worklist はこのリストの 🔴 側 7件**（下記）。
  - **🔴 残 18件の内訳（続き416 実測・次バッチの worklist）**
    ...
    - (d)**計器の誤検出2件**＝`WX17-077`（`CHOOSE{upTo}` が `abilities` の内側にあり `actionIsSkippable` が再帰しない）／`WX21-052`（`BANISH{selfTrashCost}` を辞退可能と見ない）。**検出側を直す**。
...
- ~~**生ID残存＝表示or実装の穴**（仕分ける計器が要る）~~ **✅2026-08-10 消化（続き409）＝計器 `npm run census:stubs`（`scripts/censusStubs.ts`・明細 `docs/_census_stubs.txt`／`--id <STUB_ID>` で1件の完全内訳）を新設して仕分け済み**。**907箇所の内訳（初回実測）＝A 実装の穴 21件（うち明示 defer `DEFERRED_*` 9・無言 no-op 12）／B 宣言型 32件／C 表示だけの穴 259箇所／D 健全**。**続き409〜413 で無言 no-op を6件消化して現在 A 16件（明示 defer 9・無言 no-op 7）**。⚠**「STUB＝未実装」ではないので実装軸は4経路すべてを見る**＝(1)`execStubPart1-3` のハンドラ (2)engine 別経路（CONTINUOUS 宣言型を `effectEngine`/`BattleScreen` が読む） (3)**ペイロードキー**（`PREVENT_POWER_MODIFY_BY_OPP` は id を一度も比較せず `act.powerModifyProtection` だけを読む） (4)**カード番号**（`MAYU_ENCOUNTER_FLIP_AND_GROW` は `screens/battle/mayuEncounter.ts` が `'WXDi-P13-003A'` で分岐）。**この4軸を見ないと実装済みが「実装の穴」に化ける**（較正中に実測で5種が誤検出だった）。**残 worklist ＝下記 A群 12件**。
  - **🔴 A群＝無言の no-op（残 7件・1カード1機構・逓減しない・1件ずつ）**：**着手可能は2件**＝`ASSIST_LRIG_ATTACK_THIS_TURN`（WX25-P1-048＝**アシストルリグのアタック経路そのものが無い**＝per-assist の攻撃済み管理・アタックボタン・ガード応答が要る。§6.4 で残る中では最大）／`UNDER_CARD_AS_ENERGY_COST`（WXDi-P10-041＝シグニの下のカードをエナとして支払う。**代替エナ支払いの既存機構** `myEnergyTrashSubInfo`／`ENERGY_SUBSTITUTE_TRASH_KEY` が近いが、コスト支払いUIは経路が多い）。
    - **📋 残5件は着手しない理由が確定済み**：`COUNTER_TEAM_PIECE_CUTIN_DEFERRED`（WXDi-P05-006＝**§6.3 E-2 (b) の着手禁止**）／`GRANT_UNTAP_ON_ATTACK_TO_TEAM_LRIG`（WXDi-P00-026＝**§6.3 F の保留**）／`ATTACK_NEGATE_IMMUNITY_SELF`（WX24-P4-016＝**原文が誰のシグニを指すか確定できない**＝裁定確認が先）／`FLIP_SELF_FACE_DOWN_UP`（WX25-CP1-060＝**実装すると退化する**＝下記）／🆕`CHECK_ZONE_FLIP_FREE_GROW`（WXDi-P16-001A＝**B面 `WXDi-P16-001B` のカードデータが CSV に無い**＝グロウ先が Level/Power/Type を持たない壊れたルリグになる。**機構ではなくデータの欠落**。⚠live effects JSON には `WXDi-P16-001B-E1/E2` が存在するが `cardMap` に載らないので**到達不能**）。
    - 🔴**`FLIP_SELF_FACE_DOWN_UP` は「半分だけ実装すると no-op より悪い」型（2026-08-10 続き412 実測）**＝原文は「このシグニを裏向きにし、表向きにする。ターン終了時まで、この方法で表向きになったシグニは**【自】能力を失う**」。**利益は「別オブジェクトになって対象が消える（fizzle）」・費用は【自】喪失**なので、費用だけ実装すると純粋な弱体化になる。⚠**現アーキテクチャでは fizzle が作れない**＝`collectTargetedTriggers` は `BattleScreen:4634` で**効果適用後の盤面**（`afterHostState`）を受け取って発火し、ON_TARGETED はスタックに積まれて後から解決する＝対象決定をキャンセルする地点が無い。**着手するなら「対象の無効化」機構が先**。
    - ⭐**A群は「半分だけ実装済み」が多い＝後段の機構が既にあるかを先に確かめる**。実績6件ともそうだった＝WXK11-001② はスキップ機構が完備／WX24-P4-014② は相手選択（`opponentResponds`）とルリグデッキ移動が既存／WXDi-P09-079 は timing・triggerCondition・ターン終了時トラッシュがすべて既存／WX16-021 は【側面アタック】の解決経路が既存／WX24-P4-016 は**マジックボックスを表向きにする側（producer）が6カードぶん実装済み**で、**死んでいたのは watcher 側だけ**だった。
...
- **UNKNOWN（部分未実装・逆翻訳に `【未実装/UNKNOWN】` として露出）**：**🆕2026-08-10 続き414 に live JSON を全数走査して数え直した実数＝40 ノード／38 カード**（⚠**旧記述「42箇所／31カード」は decompile シートの出現数**で、live のノード数とは別物＝**計器は live JSON を数える**）。**続き414〜415 で 43→38**。
  ...
  - **📋 残り38ノードの内訳（続き415 実測・大きい順）**＝(a)**「代わりにそれをトラッシュに置く」置換 2件**（`WX06-024-BURST`／`WXK08-030-E1`）(b)**デッキ公開→処理の変形 2件**（`WX21-028-E1`＝「その中から**すべての**＜天使＞のシグニをエナゾーンに」＝**枚数語が名詞より前**で `parseRevealPickDescriptor` の語順に合わない／`SP38-006-E1-G2`＝主語が対戦相手で1文に「見る」も含む）(c)残りは**単発**（1カード1機構）。
