...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377d＝(i)配線ギャップ 第9バッチ後）**＝golden **1445**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1041**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **211枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 376**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第10バッチ。⚠セルを取ったらまず全数分類→偽陽性の型が見えたら計器を較正する（実装ゼロ）。**
>    - **① `cardClass` 116（has 1445）＝残る最大クラスタ。** ★★セルは `cardClass × SIGNI[filter]`／`cardClass × POWER_MODIFY{SIGNI}`／`cardClass × TRASH_CARD[filter]` ほか。⚠続き376d で「miss 43 のうち本命11」と実測済み＝**条件節用法の誤検出が濃い**。**まず `cardClass` の原文正規表現が条件節（「あなたの場に＜X＞のシグニがある場合」）を拾っていないか較正を検討する**（`levelRange` で 38件減らせた前例）。
>    - **② `isDisona` 24（has 31）** ＝続き376d で判定器は直したが未配線の入口が残る。
>    - **③ `color` 31（302）／`levelExact` 29（103）／`powerRange` 30（507）** ＝大クラスタの残り。⚠`powerRange` は「パワーを＋N」＝**action の値**との取り違えが濃い（較正候補）。
>    - **④ 小粒だが has があるセル**＝`isPuppet` 10（2）／`isAwakened` 10（3）／`levelParity` 8（5）／`excludeResona` 6（1）／`nonColorless` 5（32★）／`crossState` 4（21★）。
>    - ⚠**着手前に必ず読む9つの罠**＝(a) **素の `parseStoryFilter`/`parseLevelFilter`(文全体) を対象フィルタに使わない**＝**`signiClause*Filter` 3兄弟**（クラス／アイコン／レベル）を使う。**新語彙はまずここに兄弟を足せないか見る。** (b) **曖昧なら付けない**（別ピック2本の span に AND で載せると原文と逆の過小実行）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**。(d) **exec 側の配線を必ず確認する**（`matchesFilter` は `excludeSelf`/`upToCount` を見ない）。(e) **セルは入口であって終点ではない**＝ビルダー全体を読む。(f) **枚数・値だけ先に直さない**＝構造が誤ったままだと誤りを増幅する。(g) **★★セルの miss 数は見込み件数ではない**（3バッチ連続で実測＝6件中5件／22件中14件が偽陽性）。(h) **同じ概念に2つのキー綴りが併存していないか先に確かめる**（`hasCrossIcon` vs `hasIcon`）。(i) 🆕**同じ語彙でも入口ごとに壊れ方が違う**＝1つ直したら同族の入口も必ず見る。
>    - **✅消化済み**＝「他の」ゲート棚卸し（377）／`noGuard`（377b）／アイコン系（377c）／`levelRange`（377d）。
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **4件**／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**（IDは §5d-0 (ii)）／「場に《ライズアイコン》を持つシグニがN体ある場合／あるかぎり」の条件型 **5件**。
>    - **(iii) 構造混線の在庫**＝`WXK05-043-E1`（owner 取り違え＋level 脱落＋条件節脱落の3重）ほか §5d-0 (iii) の登録分。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない。
>    - ⚠**見積もりの現在地**＝census **1041**／被覆マトリクス miss **376**。4バッチの実績＝12／30／17／8効果。**census が動かない回でも効果は直る**（377c は census 据置で17効果）＝**census の増減だけで成果を測らない。**
