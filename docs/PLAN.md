...
**直近＝2026-09-17＝§5.7 `S-3` クローズ＝1ターン分のシミュレータの見積もり（調査のみ・コード変更なし）**（全文は §5.7.3）
- 実測＝CPU のターン進行 1,273行（DB 書き込み23）＋スタック解決 381行＋対話の解決 298行＋CPU が使う実行関数 2,688行（人間と共有）＋相手側の応答 615行。engine と判定・選択（`*Gate.ts`／`cpu*.ts`）は既に純粋、DB 書き込みは `persist.commit` の1つの口。
- 3案＝**A 浅い先読み**（engine だけで試す・2〜4日）／**B 1ターン分のシミュレータ**（約5,000行を React から切り離す・2〜3週間）／**C 対戦丸ごと**（B＋相手側・自己対戦と機械学習の前提・＋1〜2週間）。
- 🔑**ユーザー決定（同日）**＝**A で先読みの効果を先に確かめ、そのあと C へ段階的に進む**＝`S-4`（A）→ `S-5`（C・4段）→ `S-6`（機械学習①）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7 `S-4`（A 浅い先読み）**＝`S-4a` 自動応答つきの効果解決＋盤面の採点 → `S-4b` 召喚 → `S-4c` スペル・アーツ。その後 `S-5`（C 対戦丸ごと・段階的） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-4`〜`S-6`）／**リリース作業 1**（RELEASE.md） |
...
| 🔥`S-4` | **A 浅い先読み**＝盤面をコピーして engine だけで効果を解決し、結果の盤面を採点して選ぶ（§5.7.3 の案A）。段＝`S-4a` 純関数（自動応答つきの効果解決＋盤面の採点）→ `S-4b` 召喚（【出】の結果で比べる）→ `S-4c` スペル・アーツ（使うか・どれを使うか） | 2〜4日 | 除去・スペルを**この盤面での結果**で選ぶ（`S-1` の静的な点数を置き換える）。**効果を実機の対戦で確かめてから C へ** |
