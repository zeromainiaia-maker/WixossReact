...
- **🆕 セッション（2026-07-28・Opus指示＋codex-work実装／Opus検証・8バッチ連投）＝§3 タスク12(xxix) を完走（段階2まで着地）＋ON_PLAY 誤分類を2波で除去**（golden 868→**887**・census 1511→**1498**・smoke 10725→**10726**）
  ...
  - **(xxix) の内訳**＝バッチ1 union/unless 16効果（⚠検証で `negated_attacks` の保存先規約が2系統に割れているのを発見・是正）／バッチ2 BET 9効果26選択肢（最悪例 `WX19-006-E1③` は**除外本体が消えて2ドローだけ**だった）／バッチ3 自身【出】段階1（10効果＝全経路で永久不発）／**バッチ8 段階2＝mandatory 1437効果**（`phase1Only` を完全撤去。任意+cost 933と段階3の65は明示除外）。
  - **⚠段階2 は3度目でようやく着地。1・2回目は codex が honest defer し、その反例が毎回 Opus 側の計器の穴を突いた**＝1回目 `WX13-040-E1`→**ON_PLAY 誤分類36件**（原文が【自】なのに全件 mandatory で**通常召喚のたびに誤発火**）／2回目 `WX25-P2-026-E1`→**watcher 34件**（`triggerScope:'self'` で**watcher として一度も発火せず・自身の召喚時に誤発火**の二方向破損）。**合計70件が段階2 とは無関係に稼働中のバグだった。**
  - **⚠検証で見つけた重要な取り違え＝`build:effects` 再実行で JSON 差分が0でもドリフト無しの証明にならない**。held は「curated を温存＝fresh と食い違う」カードなので**差分0かつ held 増**が起きる。これで watcher バッチの実バグを1度見逃した＝`WX07-001-E1`／`WXEX2-58-E2` の `triggerFilter.cardType` が **JSON 側で文字化けして `"???"`** になっており（parser は `レゾナ` を出力）、**[A] 配線したはずの watcher が2件とも永久不発**だった。外科パッチで是正・held 262 へ復帰。**非ASCII を JSON へ書いたら `grep '?'` で機械確認する。**
  - **⚠段階2 の安全性は golden だけが担保**＝`scripts/selfPlayFuzz.ts:12-13` が自ら「BattleScreen 側のオーケストレーション（トリガー収集・effect_stack 整列）は検証しない」と明記しており、**smoke/fuzz はこの経路を通らない**。連鎖は `ADD_TO_FIELD` が非置換（空きゾーンのみ）でシグニゾーン3枠＝構造的に有限、2段連鎖 golden で実証。
  - **PLAN の自己矛盾を1件修正**＝PLAN:44「`build:effects` は破壊的＝絶対に実行しない」が §5c パイプラインと矛盾。非破壊なので**「実行後に effectId 単位で全数比較する」**へ書き換えた。
  - **次の一手**＝**Opus：(xxix) 残＝任意+cost 933効果（支払いプロンプトの新フロー）と段階3＝タスク12(lv) の65効果／タスク16 の [B]29件＋watcher [B]4件／(xliv)・(xlii) の残／§6.3 残機構（H1〜H3・I）／タスク14 の残43／`cost.underSelfTrash` 未配線16効果**。**Sonnet：タスク1（§7 実機検証）＝今セッションの8バッチはすべて golden までで実機確認が未実施。特に段階2 は1437効果が新たに発火するので実機での確認価値が高い。**
