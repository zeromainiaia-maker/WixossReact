...
**直近＝2026-09-17＝`R-47` の残件を決着＝既存の保存済みデッキはユーザーが全消去（対戦開始時の検証は足さない）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**裁定**＝「すでにあるデッキをすべて消去して解決する」＝構築の制限（`LRIG_TYPE_CLASH`）を**遡って満たす手段はデータ側で取る**。対戦開始時のデッキ検証は実装しない（コード変更なし）。
- 実施＝ユーザーが Supabase の SQL Editor で `delete from public.decks where name not in ('VERIFY_DECK', 'VERIFY_DECK_MECH');`。⚠**Claude からは消せなかった**＝`decks` は RLS で本人の行しか見えず、MCP は access token 未設定で `Unauthorized`、`.env.local` は anon キーだけ。
- ⚠**検証用デッキ2つは残した**（スクリプトが作る＝手で組んだデッキではない）。うち `VERIFY_DECK_MECH` は**意図的に `LRIG_TYPE_CLASH` を2組含む**（Lv0 のウムル＝ノル／タウィル＝ノル〔種別ルリグ〕×アシストのウムル＝ドロー／タウィル＝ハウリング）＝`verifySetupDeck.mjs` が「計器用の山なので構築上限は見ない」と明記した例外。対戦開始時に検証しない裁定なので動作は変わらない。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **`C-8`**（対話応答の pure 化＝§5.6 の最後の1件） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回はデータ削除のみ＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／**CPU 完成度 1**（§5.6 `C-8`）／ルール台帳 ⚠0（残件0） |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4296・第401バッチ時点。今回はコード変更なし） |
---
