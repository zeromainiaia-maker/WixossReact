...
- 🏁**セッション（2026-08-26・続き676・Opus 5）＝§5.2 意味照合 段2 第44バッチを Codex CLI へ委譲して消化。「効果元自身のパワーを基準にした対象上限」が JSON から丸ごと落ちて過剰効果になっていた16効果**を是正（新設は `TargetFilter.powerLteSelfHalf` の**1フィールドだけ**）。**段2 台帳 OPEN 668→659**（段2 消化 444→453・段0/段1 は不変＝§5-28′ のズレなし）。
  - 🔑**バッチの切り方＝「engine に受け皿があるのに parser が出していないキー」で割る。** 残 OPEN を live の構造署名でクラスタ化（397群）しても**上位群は器が同じだけ**で根が違った（`AUTO|ON_PLAY|cond無` 115件・`ACTIVATED|MAIN` 152件）。claim テンプレでも656群＝ほぼシングルトン。⚠**第43バッチの「構造署名」も万能ではない**＝**受け皿の有無**という軸に切り替えて初めて systematic な塊に当たった。
  - 🔴**真因は `parseSelfComparison()` が比較級しか見ていなかったこと**＝「このシグニ**より**パワーの低い」は拾うのに「このシグニ**の**パワー**以下**」を一切見ていなかった。engine 側（`TargetFilter.powerLteSelf` → `effectExecutor.ts:2490-2498`）は**最初から在った**＝純粋な配線ギャップ。母集団は原文一致20効果のうち**14が穴**（他パターン込みで25→直す16／別軸9）。
  - 🔴**`powerLteSelf` の解決は fail-open**（`sourceCardNum` が引けないとフラグを外して**制限なし**に倒れる）＝**JSON にキーを足しただけでは効いていないことに気付けない**。golden は**①ちょうど上限は取れる ②上限+1 は取れない ③参照不能時は fail-open** の3点セットを採用13効果ぶん表駆動で固定した。
  - 🔑**parser が追いつくと manual 影武者が露出する**＝`WX16-045` が parser 出力と実体同一になり §6.4 O-42 のトリップワイヤが正しく発火（golden 1 FAIL）。manual を削除し、**live の `parseStatus` も `MANUAL`→`AUTO` へ直した**（本体は1バイトも変えず）。⚠**live を MANUAL のまま残すと `PRESERVE_STATUSES` が効き続け、その効果にだけ parser 改善が永久に届かない**（§6.4 O-40／O-93 の実例）。
  - ⚠**Codex は Windows `0xc0000142` で最終報告を書けずに中断した**（実装とゲートは完了）＝CODEX_GUIDE §2 の落とし穴表どおり。**報告書・台帳・簿記・実機は Claude が引き取った**（`codex_b44_report.md` は Claude が実測して書いた）。
  - ✅ gates 全緑を**独立実行で確認**（golden **2854→2859**／census **562 据置**／`census:enginetext` A群 141 据置／smoke・fuzz 全0／lint 0 errors・warning 263 ±0／同型★0）。**live 変化は16 effectId ちょうど**（新規0・消滅0・スコープ外への波及0）、**held/partial/idset は 75/12/43 で ±0**。
  - ✅**実機シナリオ2本を新設して各2回連続 PASS・既定 order へ追加**＝`powerLteSelfCeiling`（ピッカーの候補が `["WX01-051#1"]` の1件だけになり、上限+1 の P15000 は候補にも出ず場に残る）＋対照 `powerLteSelfCeilingNoTarget`（候補0件なら【起】は発動するが何も起きない）。**SELECT_TARGET の候補絞り込みは golden/smoke/fuzz が原理的に守れない層**（§2.2）。⚠**バニッシュ先はエナゾーンでトラッシュではない**＝観測点を `trashCards` にして一度「未完走」と誤判定した。

**▶ 次の一手**＝**§5.2 段2 の次バッチ**（残 OPEN **659**。⚠**着手前に `node scripts/archive/semanticAuditLedger.mjs` で数え直す**。🆕**割り方は「engine に受け皿があるのに parser が出していない語彙」で取る**＝第44バッチの勝ち筋。構造署名クラスタは第43バッチでは効いたが第44バッチでは器が同じだけの群しか出なかった）。機構側なら **`O-95`**（「このシグニと共通する色を持たない他の＜X＞」条件）／**`O-96`**（「対象とし」と「そうした場合」の照応）／**`O-93`**（manual shadow の掃除）／**`O-94`**（ゾーン限定の残穴2枚）。`O-92`／`O-80` 第2バッチも生きている。実機の腐り返済なら §5.1 の `V-89`。
...
> **母集団＝残 OPEN 659件**（`node scripts/archive/semanticAuditLedger.mjs` の実測が唯一の正。2026-08-26 続き676 時点＝段2 消化453／段0除去221／段1偽陽性111／HIGH・MED・LOW＝454・201・4／影響カード495・効果520）。⚠**この数字はすぐ古くなる。着手前に必ず実行して数え直す。**原文 vs effects JSON を LLM で意味比較して見つけた指摘の worklist。仕組みは [SEMANTIC_AUDIT.md](./SEMANTIC_AUDIT.md)、Codex 運用は [CODEX_GUIDE.md](./CODEX_GUIDE.md)。
...
- **2026-08-26 続き676 後（本行が直近の正）**：
  **census 562/562**（段2 第44バッチは**据置**＝`BASELINE_HIGH` 変更なし）、**golden 2859**（第44バッチ +5＝採用13効果の境界/境界+1/fail-open を表駆動で1本・ネスト付与の索引1本・`WXK11-048-E1`/`WXK02-051-E1` 各1本・据置判断の contract 1本）、
  smoke **10696 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors**（warning 263・±0）、
  `census:stubs` **A群🔴0／C群0**、manual-fields **0**、**同型★ 0**、`census:goldentypes` **未カバー0**（`EffectAction` 型 **149**・据置＝第44バッチは新しい action 型を作っていない）、
  **`census:enginetext` A群 141行 / 137ハンドラ**（B 59／C 27・据置＝第44バッチは engine の原文 regex に触れていない）、
  **`POWER_MOD_PER_COUNT` の live 36効果**（据置。⚠`O-80` の進捗は A群行数ではなくこの数で測る）、
  **live カード 5975 / 効果総数 10696 / MANUAL 効果 1033 / PARTIAL 21**（**MANUAL が 1034→1033**＝`WX16-045-E3` の manual 影武者を撤去し live も `AUTO` へ直した。⚠**この数字が動くのは live の `parseStatus` まで直したときだけ**＝`O-93` が言う「live が AUTO 表記の shadow」はここでは検知できない）、
  **`_held_fresh` 75 / `_partial_fresh` 12 / `_idset_fresh` 43**（すべて ±0・新規流入0）、
  **どのフラグも立たないカード 4964 / 5975（83.1%）**（`npm run census:cards`・据置）、
  **意味照合 段2 台帳＝残 OPEN 659**（段0 221／段1 111／段2 消化 453／HIGH 454・MED 201・LOW 4／影響カード 495・効果 520）、
  **実機シナリオ +2**（`powerLteSelfCeiling` / `powerLteSelfCeilingNoTarget`＝各2回連続 PASS・既定 `order` へ追加済み）
