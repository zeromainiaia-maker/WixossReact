...
| 3 | driver バッチ実行の状態汚染 | scripts（engine/JSON 非依存） | M | ⏳部分完了（続き77・105）＝ホワイトリスト方式リセット＋シナリオ毎 `page.reload()` で PASS43/FAIL6 まで改善。残＝低頻度フレークと `oppDraw` 単独FAIL（CPU挙動依存・未解明）。現在シナリオは**80定義／70既定実行** |
