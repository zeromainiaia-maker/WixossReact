...
- **🆕 セッション（2026-07-12・続き75・Opus 4.8・**Opusタスク12を全6件消化して在庫ゼロ化→続けてタスク1の第1弾**・実機11シナリオ全PASS・golden 192・census 1557維持）**
  ...
  - **次の一手＝(A)Opus＝§3 Opusタスク16＝timing 語彙センサスの消化を続ける（最優先・`npm run census:timing`）**＝**残 294効果が「場に出たとき」に誤発火している**（履歴 376→353→323→294）。**次の上位クラスタ**＝`エクシードのコストとしてルリグトラッシュに置かれたとき`13（ON_EXCEED_COST）／`ライズされたとき`11（ON_RISE）／`ドライブ状態になったとき`11（ON_SIGNI_BECOMES_DRIVE）／`手札からトラッシュに置かれたとき`10（ON_TRASH）／`ガードステップ以外で手札を捨てたとき`8（ON_HAND_DISCARDED）／`アーツを使用したとき`7（ON_ARTS_USE）…**いずれも engine 側に収集関数が既にある**＝同じ型で消化できる。**1クラスタ≒30分・+20〜30枚**が続き75での実績。**engine 側に収集関数が既にあるものは parser に regex 1本＋scope 抽出で直る**（続き75で計50枚を実証）。手順・ガードレール（トリガー句を除去しない／MANUAL温存・EXILE→TRASH 据置系を機械diffで除外）は §3 タスク16 に明記。**(A')Opus＝タスク1の第2弾**＝WX25-P3-038 の内側「代わりに」置換（タスク6の置換機構と合流）・`GRANT_LRIG_ABILITY` の ON_PLAY 誤デフォルト（タスク5）。**(B)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め**（続き71のパターンA〜F）。**(C)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（**R30の発火経路が開通したので R30②③（turnOwner:self ゲート・《ターン1回》）が新たに検証可能**。他に ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②・R43②・R44②③ 等。**ON_TARGETED①③系列・ON_LRIG_GROW②・R42②・R30①は完了**）。**(D)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（driverバッチ状態汚染＝本セッションでも `deployRestrict` がバッチ実行時のみ FAIL＝再現性を確認）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
