...
- **🆕 セッション（2026-07-16・続き167・Codex CLI〔gpt-5.6-sol・reasoning high〕が実装／Claude〔Opus 4.8〕が検証・PLAN §3 Opusタスク12(xxvii) Cluster C＝owner/対象範囲の誤りを消化）**
  ...
  - **採用①＝WXK06-031-E2（parser 規則）**＝「トラッシュからすべての黒のカードをデッキに加えてシャッフル」に `filter:{color:黒}` を付与（従来 filter 欠落で全色回収の過剰）。`parseSentencePart1.ts` の TRASH→DECK 全回収規則に**単色明記の狭い regex**（`トラッシュからすべての([白赤青緑黒])のカードをデッキに加え`）を追加＝複色/後続条件の色混入を防止。
  - **採用②＝WXEX1-57-E1（MANUAL）**＝「白か黒の＜天使＞のシグニ」の色 filter が `color:黒` のみ→`color:['白','黒']`。共通 parseColorFilter は複色名詞句を意図的に単色へ落とすため、このカードだけ `manualEffects.ts` に MANUAL 上書き（owner/revealCount/pickCount/then/remainder は自動パース維持・対象 filter だけ原文どおり）。engine `matchesFilter` は color 配列対応。
  - **Claude 検証（報告の鵜呑みなし）**＝①機械 diff＝変更は WXK06-031-E2（filter 追加のみ）と WXEX1-57-E1（color 配列＋parseStatus:MANUAL）の2効果のみ。parser 規則の巻き添え兄弟変更なし ②WXK06-031-E1（複数レベル探索）は parseStatus:AUTO のまま維持＝fresh の PARTIAL drift を採用せず ③**WXEX1-35 の count机能同值を engine で裏取り**＝GRANT_PROTECTION collector（effectEngine:2774 等）は host シグニ自身を保護し target の count を展開しない＝count:ALL は count:1 と機能同値で実害なし（Codex の据置理由が正しい）④census 2048・golden 369・同型★0・`npm run gates` 全緑を独立再実行で確認。**退化ゼロ**。
  - **据置（理由付き）**＝WX10-061-E1/WX11-038-E1（続き151/153 で既に是正済み＝無変更）／WXEX1-50-E2（ADD_TO_FIELD owner self→opponent は明確だが「相手手札から無色でないレベル3以下を選択」機構が要り owner だけでは不完全＝§6.3）／WXEX1-35-E1（ライズ×3 の継続条件が丸ごと欠落＝条件機構が要る・count は机能同值で据置）／WXK06-031-E1（レベル1〜4を1枚ずつ探す複数探索＋先頭対象参照＝§6.3級）。
  - **次の一手**：Opus＝(xxiii) リコレクト分割（§6.3級 GRANT 機構）・(xxvii) 残 Cluster B（duration）/D（timing）/E（機構欠落）・タスク1残（【常】アタックできない家族）・IS_MY_TURN化残104。**Cluster C 残（WXEX1-50-E2/WXEX1-35/WXK06-031-E1）は engine 機構の新設が要る新機構タスク扱い**。Sonnet＝`oppDrawOwnEffectOnly` PASS 反転確認（続き162 在庫）＋§7フルバッチ回帰。
### 📊 恒久指標（維持中・逐次更新）
