...
- 🏁**セッション（2026-08-28・続き696・Opus 5 単独）＝`O-108`。コスト側の「それぞれ名前の異なる」が無く、同名4枚でも払えていた。**
  ユーザー指示＝①Sheet1 の機構待ちを解消する ②codex は最初の1バッチだけ ③**このバッチで区切る**。
  📊**進捗3計器＝Sheet1 要対応 30→29 / 863（3.5%→3.4%）｜台帳 残 OPEN 580→579｜census 高シグナル 523→522**。
  gates 全緑（golden **2911→2912**）。実機＝新規2本 `b18distinctok` / `b18distinctsame` PASS（**反転確認済み**）＋回帰2本 PASS。
  - 🔑**真因は parser の非捕獲グループ**＝「それぞれ名前の異なる」を**読んだうえで捨てて**いた。
    捕獲して `selectionConstraint:{distinct:'name'}` を刻むだけで表現できた（受け皿はエナ側に最初から在った＝§5-8′）。
  - ✅**判定は3本の共有関数へ集約**（可否ゲート／確定ボタン3モーダル／タップ時ガード）。
    **1つでも漏らすと「提示は絞られるのに支払いは通る」片肺**になる。
  - ⚠**`trashActivateCost` は未対応のまま**（該当 live 効果0）＝死フラグを作らず、
    **golden のトリップワイヤ**（該当が現れたら FAIL）で将来に備えた。
  - ⚠**実機の罠**＝シグニ【起】は**モーダルを開くだけでは支払いUIが出ない**（`[data-action-label]` を先に選ぶ）。

**▶ 次の一手**＝**`O-126`（`COST_INCREASE` に「カットインされたスペルは影響を受けない」の除外が無い）**。
`WX09-Re05-E1`（Sheet1）。実測済み＝`execCostIncrease` が `PlayerState.cost_modifiers` へ積み、
消費は `applyContinuousCostDecreases` の**3呼び出し元**（`artsUseGate` / `spellUseGate` / `CutinModal`）。
除外フラグは mod 側へ持たせ、「いまカットインか」は**引数で受ける1本**にする（各サイトで再判定させない）。
⚠**括弧内ルール説明なので `stripRuleParens` で落ちる**＝`cost.exceedColors`（B13）と同じく**剥がす前に符号化**する。
**Sheet1 の残り機構待ちは 7カード**＝`WX02-020`(`O-129`) / `WX05-010`(`O-127`) / `WX05-016`(`O-117`) /
`WX05-020`(`O-113`) / `WX05-022`(`O-125`) / `WX09-Re05`(`O-126`) / `WX09-Re07`(`O-128`)。
⚠**`O-129`（118カード影響）と `O-128`（34カードの catch-all）は最後に回す。**
⚠🆕**既存 golden が旧バグを固定している可能性を毎回疑う**＝`O-120`／`O-122` と**2巡連続**で反転が要った（§5-17）。
⚠🆕**登録票は3回連続で実態と違った**（`O-116` 消化済み／`O-108`＝`O-115` 重複／`O-110` は既に遅延形）＝
**着手前に live と実コードを必ず1分見る。**
