...
- **🆕 セッション（2026-07-19・続き204-205・Opus 4.8・**タスク7クローズ**＝`PREVENT_DAMAGE`＋`COST_SUBSTITUTE` 実装／**タスク1(a)**＝アタック不可付与の対象決め打ち是正28効果・census 1963→1951・golden 451→459）**
  ...
  - **採用は2段の機械ガードで28効果**（アタックノードだけを抜いた比較＋アタック部を中立化した骨格の完全一致）。**骨格が違う4効果（WX06-002/WX18-003/WDK05-T10/WDK11-007）は据置**＝個別判断へ。**⚠自分で作り込んだ罠を1つ潰した**＝upToCount を `includes('まで')` で見ると「ターン終了時**まで**」に必ず当たり全効果が「N体まで」に化ける（数詞隣接判定へ修正・golden に回帰テスト）。census 1963→**1951**・golden 456→**459**。
  - **次の一手**＝Opus はタスク2残＝WXK08-005（キーの使用タイミング動的付与＝§6.3級）・タスク12在庫（続き204b のグロウ経路プラミング穴／今回の据置4効果／付与対象の閾値フィルタ脱落）。Sonnet は§3タスク6の未採用37効果の精密diff・採用判定。Sonnet は§3タスク6の未採用37効果の精密diff・採用判定（据置のまま実行可能）。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 1951【効果単位】**（2026-07-19 続き205＝アタック不可付与の対象決め打ち是正28効果で1963→1951、続き203＝終止形サーチ退化＋levelGtLastProcessed で1967→1965・前置CHOOSE復元＋ルリグデッキ戻し幻覚除去12枚採用で1965→1963。続き201＝`TRANSFER_TO_DECK` 幻覚除去で1968→1967、続き200＝名前filter 2効果の復元で1970→1968）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。前提＝`docs/_effect_srctext.json` が最新であること。明細 `docs/_vocab_census.txt`、過去の計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109以降。
