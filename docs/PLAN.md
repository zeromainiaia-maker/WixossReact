...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕単独）＝タスク12(xlvi) を第5〜6波で28効果消化したのち、(xxix)(1) の任意+cost【出】853効果を配線**（golden 936→**961**・census 1463→**1454**〔`BASELINE_HIGH` 更新〕・smoke 10726 全0・fuzz 全0・同型★0・held 257→**257**〔新規ドリフト0〕）
  - **(xlvi) 第5波 (h)＝融合規則1箇所が真因**＝`LOOK_AND_REORDER + STUB(REVEAL_PICK_HAND_SHUFFLE_BOTTOM) → REVEAL_AND_PICK` が **revealCount／pickCount／残りの行き先しか運んでいなかった**（filter も「N枚まで」も「手札に加えるかエナゾーンに置く」も落ちて**どのカードでも拾える過剰実行**）。新 `parseRevealPickDescriptor`＋新機構 `handOrEnergy`＋複数 pick 群3形の `LOOK_PICK_CHAIN` 展開で **17効果＋同系統9効果**を是正。
  - **(xlvi) 第6波 (d)1＋(a)5**。🔴**ここで engine の潜在バグを発見**＝**`execLookPickChain` が `stage.filter` を `resolveDynamicFilter` に通していなかった**（`matchesFilter` は `colorMatchesLrig` 等を**理解せず黙って無視する**）。既存 LPC は全部静的 filter だったので live 実害は無かったが、動的filter を parser で表現した瞬間に「センタールリグと共通する色を持つシグニ」が**単なる「シグニ」に化ける**位置にあった。新語彙＝`colorMatchesAnyLrig`／`colorMatchesNonCenterLrig`／`LookPickChainStage.notSharesClassWithPrev`。
  - **🆕(xxix)(1)＝効果で場に出たシグニの任意+cost【出】**。collector が任意【出】をコストの有無を問わず全部落としており、通常召喚には支払いモーダルがあるのに**効果配置では933効果が丸ごと不発**だった。**支払い選択を action に埋める**＝`SEQUENCE[OPTIONAL_COST, 元action]` で積み、engine 既存の Pattern ⑤ へ載せる（BattleScreen 無改造・**golden で検証できる**）。`OptionalCostSpec` で表現できる **853件を配線／80件は未対応コストのため据え置き**（⚠**払っていないコストを踏み倒して効果だけ通す方が、発火しないことより有害**）。JSON/parser は無変更。
  - **golden の教訓を3段深めた**＝①**候補集合 assert は filter 自体から候補を作るので「間違った filter」を検出できない**（filter の*消失*しか見ない）→期待値文字列を併記 ②**動的filter は `matchesFilter` では判定できない**（実行時解決）→**盤面を作って pending SEARCH の `visibleCards` を直に見る** ③**「写せるかどうか」を母集団全数で対応キー集合と突き合わせる**＝取りこぼし0の機械証明。HEAD 差し戻し／filter 弱体化／`resolveDynamicFilter` 外し／collector 分岐戻しの4種の変異で、それぞれ狙った本数だけ FAIL することを実測。
  - **⚠要実機検証**＝(xxix)(1) はコード変更こそ engine 側だが、**表面化するのは BattleScreen の effect-stack 解決**（`EffectInteractionModal` の任意コスト選択）。効果で場に出したシグニの【出】《色》プロンプトが実機で出ることは未確認。
  - **次の一手**＝**Opus：(xxix)(1) 据え置き80件**（exceed/fieldTrash/lrigDown 等の支払い表現＝§6.3 送り候補）／**(xxix)(2) 段階3＝`mandatory:false`＋cost無し65効果**（タスク(lv) と同根＝任意・無コスト【出】の共通選択機構）／**(xlvi) 残20件**〔(a)残2・(b)10・(c)4・(d)残2・(g)2＝いずれも新機構が要る〕／**ゲート脱落 [B]20件（Condition 語彙の新設）／タスク16 の [B]29件＋watcher [B]4件／(l) 内側【自】parse 失敗3枚／§6.3 残機構（H1〜H3・I）**。**Sonnet：§7 実機検証の横展開**＝**まず本セッションの (xxix)(1)**（効果配置→任意コスト【出】プロンプト）＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓・escapeDiscard 回避モーダル・BET 選択肢・アーツ使用条件20枚。
