...
**直近＝2026-09-20＝カード番号の検索が「再録の番号」で引けなかったのを直した**（148パック中147で結果が欠けていた／全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**真因**＝再録・別絵柄は `CardData_Variants.csv`（3列だけ）へ退避してあり、本体は**最初に収録されたパックの番号しか持たない**のに、検索は**本体の番号だけ**に当てていた。
- **直し**＝判定を純関数へ（**新規 `src/utils/cardSearch.ts`**）＝**番号は避難先にも当てる／大文字小文字を無視／出るのは本体のカード**（行に「別番号：」を表示）。**追加容量ゼロ**・`src/data/` `public/data/` は無変更。
- **実測**＝`SPDi38-` **0 → 66**／`SP32-` **0 → 37**／`PR-` 135 → 372／`WX24-` 416 → 495。⚠**退避の理由（Vercel 容量）は失効済み**＝dist **13.5MB / 上限 100MB**・初回ロード **brotli 1.19MB**・画像は ImageKit。
...
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先攻が勝てない（`S-19`・直近の A/B でも先攻 27.5〜40%）②**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし（今回のカード検索の欠落はユーザー報告＝**修正済み**） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4339**＝+1）｜**実機** `node scripts/archive/verifyCardSearch.mjs` **11/11 PASS**（反転確認あり） |
