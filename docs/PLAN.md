...
- **🆕 セッション（2026-08-11・続き442・Opus 5）＝§6.4「デッキから探す→シャッフル→デッキの一番上に置く」4効果を機構化。裸 `STUB{REVEAL_AND_PICK}` による**万能サーチ化**（過剰効果）を解消。golden 1834→1841・census 846 据置・held 107/47 据置**。Codex 委譲＋Claude 検証。
  - **🔴① バグの正体＝ペイロード無しの裸 `STUB{REVEAL_AND_PICK}` が「何でも吸い込むゴミ箱」だった**＝ハンドラ（`execStubPart1.ts:3080-3102`）は**実行時にカード原文を再parse**（`/シグニ([０-９\d]+)枚を探して/`）し、デッキを `Type==='シグニ'` で決め打ちフィルタして**デッキ全体から好きなシグニ1枚を無条件で手札へ**入れる。⇒ 原文が「次のドローを仕込む」だけの効果が**万能サーチ（チューター）に化けていた**＝過少実行ではなく**過剰効果**。⚠**engine で原文を再パースしない**という本プロジェクトの規約（`sideAttackEmptyZoneAsFront` 等）に違反する実装だった。
  - **🔴② 同居していた別軸2件**＝`WXK02-070`／`WXK03-049` は `CONDITIONAL{IS_MY_TURN}→STUB{DECK_TOP_TO_LIFE}` を持っていた。`DECK_TOP_TO_LIFE` も原文を再parseする実装で、原文に「ライフクロス」が**1文字も無い**この2枚では `owner='self'`／`addCount=1` に落ち、**タダで自分のライフが1枚増えていた**。どちらも原文に対応が無いので削除。
  - **⭐③ 受け皿は既存 `SearchAction` で足りた**＝`{from,filter,maxCount,upToTarget,then,afterSearch}`（`types/effects.ts:1183`）がこの文型にそのまま一致。**新 action 型・新 timing の新設ゼロ**。任意性は既存 `upToTarget`、「一番目か二番目」は既存 `CHOOSE` 2択＋`TransferToDeckAction.position` に `'second'` を足すだけで着地した（**defer も「一番目固定」への丸めもせず4/4採用・defer 0**）。
  - **🔑④ 先出しした罠が的中（予防的使用9例目）＝シャッフルの順序**＝原文は「探す→**シャッフル**→一番上に置く」だが engine の SEARCH 契約は `thenAction`→`afterAction` の順（`effectExecutor.ts:7210` の MB 分岐が実例）。素直に書くと**トップに置いた直後にシャッフルされて選択札が流れる**＝効果が無音で消える。先例として `execLookPickChain` の `_topReserved`（`:5085-5087`／`:5141`＝「最後に当てる」設計）を名指しした結果、codex は**選択札をデッキに残したままシャッフルを先に実行し、最後に instanceId で位置確定**する形で解決した。⚠**適用ガードは木の形**（`SEQUENCE` に裸STUBがあるか）で、カード固有の原文 regex ではない＝ガードレール5c を満たす。
  - **🔴⑤ 検証側が5枚目を発見＝`WD23-013-A-E1`（アンコール）は live で完全な no-op**＝原文はこのバッチと**同じ文型**なのに JSON は `TRANSFER_TO_DECK{DECK_CARD}` だけで **SEARCH が丸ごと無い**。⚠**本バッチの回帰ではない**（ベースラインでも `execTransferToDeck` の6分岐を素通りして末尾 `return done(ctx)` に落ちていた＝挙動は厳密に不変）。⚠**裸STUB を入口にした棚卸しでは原理的に見つからない**（この効果は裸STUBを持たない）＝**live の `TRANSFER_TO_DECK{DECK_CARD}` 全6箇所を走査して発見した**。続き425 の `SPDi43-06`（計器の網の外）と同型の死角。
  - **⚠⑥ 指示書（Claude）側の誤り1件**＝「`applyDirectAction` に `TRANSFER_TO_DECK` の case が無い・全17case を列挙した」は**誤り**（case は存在した）。原因は **`awk NR>=7709 && NR<=8200` と走査範囲を根拠なく打ち切ったこと**。codex が実測で訂正したが、一時的に case を重複させて golden 18件失敗の手戻りが出た。⇒ **「全 case を列挙した」と書く前に関数の終端まで走査したか確かめる**（行番号で切る `sed`/`awk` は長い関数の途中で切れる）。
  - **⭐⑦ スコープの切り方が効いた**＝裸STUBは live **11カード**で共有されており、**4枚だけ取って残り7枚を「変更禁止リスト」化**して投げた（reveal-until／手札公開コスト／色参照振り分け／宣言数字＝4つの別機構）。結果**巻き添え0**＝per-effect diff は対象4件のみ・effectId 増減0・7枚は正規化JSON全件一致。⚠**残り7枚も今なお万能サーチ化した既知バグ**である点は変わらない（T1 で「裸STUBはこの7枚だけ」を固定）。
  - **▶ 次の一手**＝**Opus 側**：(a)**`WD23-013-A-E1` の SEARCH 復元**＝機構は今回できたので安い（parser の `foldDeckSearchToTop` regex が `探(…)。` の句点を要求するため「探し**て**デッキをシャッフルし」を拾えない＝ここを広げる。⚠裸STUBが無いので木のガードも別途要る）(b)**`WX24-P3-033-E1` の【マジックボックス】2群振り分け**＝⚠**PLAN の「型と engine の追加が要る」は過大**だった＝`LOOK_PICK_CHAIN` は既に**複数 stage で行き先を変えられ**、`then:'trap'` という**設置 stage の先例**を持つ（`types/effects.ts:1486`）。`'magic_box'` を union に足して `lookPickThenAction`（`effectExecutor.ts:5058`）に1行足すだけで、受け側の `PLACE_MAGIC_BOX` 分岐（`:7205-7219`・続き438 実装済み）はそのまま使える (c)**裸STUB 残り7枚**を機構ごとに（reveal-until 3枚が最大の塊）(d)**CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：`verifyEffects` の「アクション[STUB代替?] 5件／[要確認] 3件」未仕分け（名前照合の誤検出が混じっている見込み＝計器の精度改善）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き442（デッキサーチ→デッキ上への機構化）後 最新値（本行が直近の正）**：census **846**（据置・`BASELINE_HIGH` 846）、golden **1841（+7＝E2E 4本＋構造検査＋T1/T2）**、smoke **10688 / SKIP 0**、**同型★ 0**（グループ265）、held（parserWorklist）**107枚 / 署名グループ 47件**（増減0）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 27ノード / 26カード**（28/27→27/26）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群（無言 no-op）**0**、**live の裸 `STUB{REVEAL_AND_PICK}` 7カード**（T1 が固定・11→7）、**`DECK_TOP_TO_LIFE` 2カード**（T2 が固定・4→2）、**live の `TRANSFER_TO_DECK{DECK_CARD}` 6箇所**（うち `WD23-013-A-E1` は SEARCH 欠落で no-op＝未着手）。live effect 単位 diff **4効果・outlier 0・effectId 増減 0/0**。
...
>    - **【Opus 側】(3) §6.4 の未消化 worklist**＝2026-08-12 に **`O-1`〜`O-15` の番号つき表へ整理済み**（消化済みの詳細は PLAN_DETAIL「2026-08-12 整理⑧」へ退避）。⚠**この行が名指ししていた `WX24-P3-033-E1` は続き443 で消化済み**（在庫は §3-1 のとおり実測してから着手する）。**次の一手の候補＝`O-2`（`SEARCH` の相手応答ルーティング＝3効果を同時に解放）／`O-3`（「次のターンの間」の残り）／`O-6`（`WX25-P3-038-E1` の MANUAL 不可侵）**。
...
> **消化済みの詳細は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-12 整理⑧」へ全文退避**（続き403〜450）。
> ここは**生きている worklist だけ**を置く。番号 `O-n` は着手順ではなく**参照用の固定ID**（消化しても番号は再利用しない）。
> 一次記録は [BUGFIXES.md](./BUGFIXES.md) の各日付。**着手前に §3-1 のとおり在庫を実測すること**
> （2026-08-12 の整理時、消化済みなのに残件として残っていた行が2つあった）。
...
| ~~**O-2**~~ | ~~**`SEARCH` pending の相手応答ルーティング**~~ | — | 🏁**2026-08-13 続き451 で消化**（下の消化済み欄・BUGFIXES 同日）。**残った宿題2件**＝(a)**実機UI未検証**（相手応答モーダルの実表示／CPU 自動応答）＝§7 送り・O-13 に同梱 (b)`SP38-006-E1` は **PARTIAL のまま**＝1つ目の内側能力「対戦相手の場にあるキーとシグニは能力を失い、新たに得られない」が `REMOVE_ABILITIES{SIGNI, count:1, until:'PERMANENT'}`＝**対象種別（キー＋シグニ）・体数（すべて）・期間（このターン）の3軸とも未忠実**。「新たに得られない」は継続宣言が要るので O-4/§6.3 の家族 |
| **O-3** | **「次のターンの間」系統の残り** | 約16効果 | 続き448/450 で第1〜2波（採用14）。残＝**`STUB` 11件**（宣言型・**1件1機構なのでまとめて切らない**）／`REMOVE_ABILITIES` 2件／`GRANT_PROTECTION`・`BANISH`・`FREEZE` 各1件。⚠**型ごとに `NEXT_TURN` の受け皿があるかを先に実測**（「型にフィールドがある＝動く」ではない） |
...
| **O-10** | **明示 defer の棚卸し** | 16 id | `npm run census:stubs` の A群に `DEFERRED_*` として可視化済み。**理由が「先にやることがある」型は前提が消えたら着手できる**（例＝`DEFERRED_UNDER_CARD_AS_ENERGY_COST` は funnel 完成後に実装できた）。**定期的に前提を再判定する** |
...
| **O-13** | **実機UI未検証（→§7送り）** | 数件 | エナ支払い元 funnel（14サイト＋14モーダル）ほか、続き409〜413 の A群実装（相手ルリグデッキ選択モーダル・新規 CHOOSE 等）。**ヘッドレスでは検証できない層** |
