...
**直近＝2026-09-21＝§5.7 `S-24`＝自己対戦がマリガンを踏むようにした（0 → 全戦・両席）＋規則を「レベル1を優先して持つ」へ**（全文は [BUGFIXES.md](./BUGFIXES.md) 第435バッチ）
- 🔴**真因**＝判断も実行も純関数だったのに、**繋いでログを出す段が画面にしか無かった**＝ヘッドレスは戻す札を空にした固定呼び出し
  ＝**`S-2`（作戦データのキーカードを戻さない判断）が A/B に一度も乗っていなかった**。⇒ `controller/performMulligan.ts` の1本へ。
- 📏**母集団**＝本物のデッキ6つ × 400手札で**初手の 87% がマリガン対象**（自己対戦はそれを1戦あたり2席ぶん飛ばしていた）。
- 🆕**規則（ユーザー決定）＝レベル1を優先して持つ**＝Lv3以上は常に戻す／**手札の Lv1 が2枚未満なら Lv2 も戻して掘る**。
  📏**Lv1 が0枚の手札 6.1% → 4.1%**（Lv1 は山に12枚しかないので手札平均2.2枚が上限）。旧規則は `legacy-mulligan`。
- 📊**A/B（96戦）は6山とも差なし**（合算ライフ差 +0.38 [-0.18, 0.93]）＝採否は直接指標とユーザーの決定による。
  🔧**台の健全性**＝`default` 同士は全組1勝1敗・50.0%。
- 🔴**ベースラインを撮り直して WD13 の失速が見えた**（総当たり4デッキ96戦）＝**WD13 75.0% → 8.3%／ケトッシー軸 45.8% → 85.4%**。
  原因は `S-24` でも `S-22` でもない（どちらも WD13 単体で差なし）＝**`S-25` の既定引き上げの効きが山ごとに極端**⇒ 🆕**`S-30`** に登録。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-30`**（探索の既定の効きが山ごとに極端＝WD13 8.3%）。次に `S-14`・`S-29`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（🆕`S-30`・`S-14`・`S-29`＝🏁`S-24` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝**WD13 の勝率が 75% → 8.3%**（`S-30`）。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4355**）｜**実機 PASS**（7ターン/165手/243s）｜反転確認＝`legacy-mulligan`（規則）。⚠**台の修正そのものは反転しない**＝この回にベースラインを撮り直した |
...
| 🔥`S-30` | 🆕🔴**探索の既定（`searchWidth/Depth: 4/4`）の効きが山ごとに極端**＝`S-24` でベースラインを撮り直したら**総当たり4デッキ96戦で WD13 75.0% → 8.3%／ケトッシー軸 45.8% → 85.4%**（幅が [18.8, 75.0] → [8.3, 85.4] に広がった）。⚠**原因は `S-24` でも `S-22` でもない**＝どちらも WD13 単体の A/B で差なし（組 2-4-2／0-7-1）。🔑**`S-25` の実測とそのまま一致する**（強い＝ケトッシー軸 8-0-0／弱い＝WD13 0-4-4 だけ）。**見立て**＝`S-25` ①＝盤面の採点に「アップ／ダウン」の価値が無く、WD13 は**ルリグの【起】を探索が使わない**（`WD13-003-E2`＝default 30回 / search 0回）。⚠**着手前に `--a legacy-greedy --b default --deck WD13` で切り分け直す**（既定を上げたのは2026-09-21＝いまの `default` は当時と別物）。 | 中 | 山によらず探索が勝てるようになる。⚠**既定を下げ戻す判断もありうる**（`legacy-greedy`）＝先に測る |
...
- **2026-09-21 時点**（第421〜435バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 4**（🔵`S-6`・🆕`S-30`・`S-14`・`S-29`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4355**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 8本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・**7/165手**）
  ...
  - 📊**総当たり（4デッキ・96戦・止まり0・443秒／🆕2026-09-21 に `S-24` で撮り直し）**＝ケトッシー軸 **85.4%（41/48）** ＞ 天使軸1 56.3% ＞ WD06 50.0% ＞ 🔴**WD13 8.3%（4/48）**（旧＝WD13 75.0 ＞ 天使軸1 60.4 ＞ ケトッシー軸 45.8 ＞ WD06 18.8＝**探索の既定を上げた効きが山ごとに極端**⇒ `S-30`）
