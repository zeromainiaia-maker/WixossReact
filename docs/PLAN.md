...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer **残 5 id/5件**。続き507〜511 で5巡済み＝残りはすべて1件ずつ実機構が要る〔`ON_TARGETED` の収集地点／ピースのカットイン窓／グロウ履歴 state／数字宣言／トラッシュ内の効果免疫〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 5 id / 5件 | 🆕**2026-08-16 続き511（5巡目）＝6種/8件 → 5種/5件**（`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY` の3件を実装）。**■** 原文「あなたの〈filter〉のシグニ１体が対戦相手の効果によって場を離れる場合、〈コスト〉を支払ってもよい。そうした場合、代わりにターン終了時まで、このシグニはこの能力を失う。」＝離場置換の**新軸 `selfAbilityPay`**（`EFFECT_LEAVE_PAY_TO_LOSE_SELF_ABILITY`）。続き507 の `selfAbility` 軸のコスト付き版。🔑**据置理由（続き507）は「決定層が `resultCtx` を同期的に組む契約なので対話 pause を張れず、しかも払うのは `otherState` で `optionalCostPaySteps` の `ownerState` 固定と視点が合わない」だったが、`selectOptionalCostEnergy`（state 非依存で「どのエナを払うか」を返す）を使えば同期的に払えた**＝対話 policy は要らなかった（任意軸の自動適用は `discardSpell`／`trashStackSpell` と同じ既存の近似）。⚠**victim（守られる側）と宣言元（能力を失う側）は別**＝原文は「あなたの〈filter〉のシグニ１体が…**この**シグニはこの能力を失う」。⚠**払えないときは成立させない**（タダで置換すると「無条件で無敵」になる）。⚠**無料の軸より後ろ**に置く（自動 policy は先頭から採るので、先に置くとタダで済む置換があるのに資源を払う）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き511）。⚠**全経路 実機未検証**＝§7 送り。**■残 5 種 / 5 件（すべて1件ずつ実機構が要る）**＝`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`〔**3部品**＝①「このターンにセンタールリグがグロウしていない」を数える state が**存在しない**②チェックゾーンの A/B 面フリップ ③特定カードへのコスト無しグロウ〕／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔ピース使用へのカットイン窓〕／`DEFERRED_DECLARE_NUMBER_AND_ATTACK_PHASE_RESTRICTIONS`〔数字宣言＋3種の制限〕／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔🆕**続き511 で真のブロッカーを特定**＝裏向き→表向きの往復自体は`facedownSigni.ts` で書けるが、観測できる効果は「新しいオブジェクトになって**相手の対象指定が外れる**」。ところが engine の `ON_TARGETED` は **`resumeSelectTarget` が効果を適用し終えた後**に積まれる（`BattleScreen.tsx` の SELECT_TARGET 分岐＝`result` が既に確定済み）ので、この地点では「対象を無効化する」余地が無い。**収集地点を適用前へ動かす**のが前提〕／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`〔トラッシュ内カードの能力喪失＋効果免疫＝相手トラッシュに触る全効果に及ぶ〕。**定期的に前提を再判定する**（続き507〜511 の計15種のうち、**「機構が無い」が本当だったのは3種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付け・**自分で書いた据置理由の陳腐化**だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
