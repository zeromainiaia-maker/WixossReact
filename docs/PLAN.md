...
- **🆕 セッション（2026-07-11・続き65・Opus 4.8・§3 Opusタスク7(b) 【デコレ】fromHandアクセ装着の2バグ修正）**＝続き64（Sonnet）がOpusへ登録した「`execAttachAcce` fromHand経路の実装バグ」を実UI駆動で追い、**連鎖する2つの独立バグ**を修正して【デコレ】キーワードを機能させ R45①（ON_ACCE_ATTACH host条件）を実機PASSさせた。**バグ①（engine・`effectExecutor.ts`）**＝fromHand の2段選択（step1手札→step2ホスト）で step1 の `thenAction` に未完結の ATTACH_ACCE を渡す設計が resume 解決側（「選ばれた候補＝ホストシグニ」前提）と噛み合わず無添付終了。→AttachAcceAction に内部フィールド `_selectingAcceFromHand`/`_pickedAcceCard` を追加し、step1 完了時にホスト選択 SELECT_TARGET を再発行、選んだアクセカードを **action 自体に埋め込んで** step2 へ引き渡す（interaction 境界越えの ctx persistence に非依存）。**バグ②（`BattleScreen.tsx`・`battleCardNums`）**＝バグ①修正後も WXK05-041-E2 が発火しない現象＝`addState` が `signi_acce` を走査しておらず（`signi_charms`/`signi_soul` は走査するのに）、手札から装着され signi_acce に移ったアクセカードが battleCardNums→effectsMap から脱落し自身の ON_ACCE_ATTACH を検出できなかった。→`addState` に `signi_acce` 走査を追加。**検証**＝golden +1（178・`ATTACH_ACCE fromHand`）／`npm run gates` 全緑（typecheck／smoke 0／fuzz 0／census 1567不変／lint 0 errors）／**実機 `verifyBattleDrive.mjs acceAttach` ✅PASS（2回連続・deterministic）**＝WXK04-003-E1（ルリグON_ACCE_ATTACH）と WXK05-041-E2（アクセ自身・accedHostMinLevel:4）が両方 actions_done 記録・既定orderに追加。詳細 BUGFIXES 最上部。
  - **前回（続き64・Sonnet）＝§7実機検証5件**（R42/R44/ON_TARGETED① PASS・R30/R45① はブロック中/実バグでOpus登録）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3タスク7(a) WXEX2-50-E3 step1 owner誤パース**（原文「対戦相手のトラッシュから…それを対戦相手の場に出す」が owner/source.owner ともに self に誤生成＝R30 ON_PLAY any_opp の唯一の自然発火経路をブロック＝続き64発見の残り半分）。**(B)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(C)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(D)Sonnet＝§7実機検証の継続**（ON_TARGETED残3枚の個別確認・R42②（バトルバニッシュでhostが離脱したとき）・ON_COIN_PAID/ON_LRIG_GROW③④の follow-up）。**(E)持ち越し：WXK04-003 の【起】2能力が両方「【起】コストなし」表記になるボタンラベル表示バグ（`getMyLrigFieldActions` costParts が coin非考慮）・WX12-008 の exceed-cost timing 誤パース是正・SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染・WX14-003の任意コスト(costText型)がcostColors無しで実際にはコスト無視で発動する疑い（未確認・低優先）。**
