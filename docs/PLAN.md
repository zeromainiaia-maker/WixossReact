...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxv)＝条件つき任意コストの「条件節だけが黙って消える」51効果＝parser ガードD の退役（残0クローズ）**（golden **1134→1137**・census **1370→1369**（`BASELINE_HIGH=1369`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **277→251枚**〔署名 109→**97**〕・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 40 / added 0 / removed 0**／`build:effects` 冪等）
  - **登録時の見立ては「held 15枚に既存ドリフトが同居」だったが、出所は parser の1行だった**。`tryWrapLeadingStateCond`（「〈状態条件〉の場合、〈X〉」を `CONDITIONAL` へ持ち上げる規則）の**ガードD**が、`X` が `OPTIONAL_COST` 系 STUB のとき `continue` して**条件節ごと捨てて**いた。当時（続き110）は effectExecutor の Pattern ④/⑤ が「STUB が SEQUENCE の直接ステップ」前提で、包むと支払フローが壊れたため正当だった。
  - **しかしタスク12(xi) で engine に「包み形の解体」が入っている**（`CONDITIONAL{gate, then: STUB OPTIONAL_COST系}` を評価し、成立なら包みを解いて Pattern ④/⑤ へ委譲・不成立なら対の「そうした場合」本体ごと読み飛ばす。46効果）。**ガードだけが stale で残り、条件を落とす副作用だけが生きていた**＝`WX24-P1-011`「あなたの場に＜アーム＞のシグニがある場合」等 **51効果が無条件発火**。ガードを外すだけで直る＝**新機構0・engine 無改修**。
  - **⚠「engine が対応していないから parser で落とす」系のガードは、engine が後で追いついても自動では消えない**。ガード文言に根拠となる engine 側の実装位置が書いてあるなら、**着手時にその実装を読み直す**こと（今回はガードの前提が2バッチ前に無効化されていた）。外す前に**全51効果で `then` が素の STUB 単体**であること（engine が解体できる形）を実測した。
  - **巻き添えの回帰を1つ潰した**＝`applyTargetLevelScaling`（(liii) のレベル倍率正準形化）が「ゲート直前の**素の**STUB」しか見ておらず、コストが `CONDITIONAL` に包まれた途端に同定が外れ、**包みを残したまま空の `OPTIONAL_COST` を二重に足して**いた（`SPDi43-29`）。`coreOfCondWrap` で中を覗き**包みは維持して中身だけ差し替える**形に修正。
  - **「あなたのエナゾーンに＜X＞の〈シグニ|カード〉がN枚以上ある場合」を一般化**＝＜美巧＞５枚 固定だった規則を `ENERGY_COUNT_FILTER` へ一般化（「カード」形は `cardType` を課さない／「N**種類**以上」は `distinctName`）。`applyBoardZoneStateBatch3` の個別ハードコード2枚（`WX26-CP1-090`／`WXDi-CP02-087`）は**原文が「カード」なのに `cardType:シグニ` 固定で過剰に狭かった**ので一般規則へ寄せて退役。
  - **36枚採用**（(lxiv) 積み残しの7枚を含む・全数 per-effect 照合）。副次で `WXK02-060`／`WXK09-042`／`WXK09-044` の「パワー減が**自分の**シグニに向いていた」誤り（`targetsTriggerSource`）も是正。**据置2枚**＝`WX24-P3-057`（E1 の `CHOOSE` を fresh が失う別ドリフトが同居）／`WX26-CP1-101`（条件節に居た ＜プリオケ＞ が `GRANT_KEYWORD` の対象フィルタから落ちる＝ゲートは得るがフィルタを失う両損）。
  - **次の一手**＝**Opus：①「コストの合計」族の STUB/UNKNOWN 14件②`WX11-043-E1` の `crossOnly` 欠落③タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**④`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑤§6.4 の配置数制限すり抜け7効果⑥§6.3 I＝`WX25-P3-028-E2` の3機構⑦🆕 据置2枚の別ドリフト**（`WX24-P3-057` の「あなたか対戦相手のデッキ」`CHOOSE` 脱落）／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**(lxv) の条件つき任意コスト 36枚**（ゲート不成立で支払いプロンプトが出ないことの確認）／**(lxiv) の対象ピッカー前置 61枚**（影響枚数が最大）／(lx)(lxii)(lxiii) の未検証UI／(lxi) 第1・第2波の UI 6件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxv) 残0クローズ後 最新値（本行を直近の正とする）**：census **1370→1369**（`BASELINE_HIGH=1369`）、golden **1134→1137**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **277→251枚／署名グループ 109→97**、manual field loss 0。live per-effect 差分 **changed 40 / added 0 / removed 0**（36枚採用）、`build:effects` 冪等。**新語彙0本／退役2つ**＝parser ガードD（`tryWrapLeadingStateCond` の OPTIONAL_COST 系 skip）と `applyBoardZoneStateBatch3` のエナ枚数ハードコード2枚。engine 無改修。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1137**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1369**（回帰ゲート）・held **251枚/97署名**（2026-07-31 タスク12(lxv) クローズ後に実測）。
>
