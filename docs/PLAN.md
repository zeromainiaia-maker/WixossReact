...
- **🆕 セッション（2026-07-07・続き36・Opus 4.8・§3割付タスク①＝引用付与の残＝2バッチ）**＝**【第2バッチ＝複合活性条件 AND 抽出】**「あなたのセンタールリグが＜色＞で、このシグニが中央のシグニゾーンにあるかぎり」が `parseActiveCondition` の generic フォールバックに飲まれ**条件丸ごと脱落＝無条件付与の過剰効果**だった系統を、`AND[LRIG_COLOR, IS_SELF_IN_CENTER_ZONE]` 抽出規則1本で是正（engine `checkActiveCondition` は AND/LRIG_COLOR 実装済＝engine不要）。**4枚採用**（WX06-032/035/WX10-068/WX15-054＝いずれも即時アクションへの平坦化を条件つきGRANT_FIELD_SIGNI_ABILITYへ是正）＋WX06-026自動採用。golden+2・census **1654→1650**。**【第1バッチ＝triggerScope any_opp 推定】**granted 内 `【自】：対戦相手のターン終了時、…` が triggerScope 省略＝engine 既定 `self` で**自身のターン終了時に誤発火**（WX21-056/061 で発見）。**parser 1点**＝`ON_TURN_END`/`ON_TURN_START` の**トリガー句 `/対戦相手のターン(?:終了|開始)時(?:に)?[、,]/`**（直後読点限定＝「…終了時**まで**」duration 句を誤爆させない）で `triggerScope='any_opp'`。**decompiler**＝「対戦相手のターン終了/開始時」前置＋`〔範囲〕`除外。**是正27枚**（純上位集合60枚自動採用→修正版parserで全数再検証し duration 句由来の誤爆31枚をrevert＋WXDi-P03-036-E2除去＋MANUAL/EXILE温存6枚を外科パッチ）＝corpus全走査で取りこぼし0。golden旧テスト（self誤発火を固定化）をany_opp意味論へ書換。**全ゲート緑**＝typecheck・golden **143/143**・smoke 全0（OK10317）・fuzz 全0・lint 0 errors・**同型★0**（全10シート再生成）・**census 1654→1650**。**parser/engine/decompiler 全変更＝smoke/golden/fuzz 実施**。詳細 BUGFIXES 最上部2件。**次の一手＝§3 Opus①の残＝(a)WX21-056/061 の本体は granted 内「このシグニを場からトラッシュに置いてもよい」が `parseSentencePart3.ts:1738` の雑STUB `OPTIONAL_TRASH_ENERGY_CLASS` に mis-parse（自トラッシュ任意コスト＝27枚が該当・engine機構要）。(b)引用内 CHOOSE/選択肢展開（WXDi-D09-P20等）・(c)場全体常時付与 ALLGRANT。いずれも過剰効果リスクで held 残置・Opus 継続。**granted 内 `【自】：対戦相手のターン終了時、…` が triggerScope 省略＝engine 既定 `self` で**能力保持シグニ自身のターン終了時に誤発火**（WX21-056/061 で発見・原文と真逆タイミング）。engine（`collectTurnTriggers`）は相手フィールド any_opp/any 分岐を実装済で、**欠けていたのは parser の triggerScope 抽出だけ**。**parser 1点**＝`ON_TURN_END`/`ON_TURN_START` かつ **トリガー句 `/対戦相手のターン(?:終了|開始)時(?:に)?[、,]/`**（直後読点＝トリガー限定・「…終了時**まで**」duration 句を誤爆させないのが肝）で `triggerScope='any_opp'`。granted サブ能力も `parseBlock` 経由で自動適用。**decompiler 2点**＝any_opp 時「対戦相手のターン終了/開始時」前置＋`〔範囲:any_opp〕`除外リスト追加。**是正 27枚**（build:effects 純上位集合 60枚自動採用→修正版 parser で全数再検証し **duration 句由来の誤爆 31枚を revert**＋WXDi-P03-036-E2 除去＋MANUAL/EXILE温存で伝播しない 6枚を effectId 外科パッチ）＝corpus 全走査で取りこぼし0確認。**golden**＝旧テスト（WXDi-P05-039-E1 の self 誤発火を固定化していた）を any_opp 意味論へ書き換え。**全ゲート緑**＝typecheck・golden **141/141**・smoke 全0（OK10317）・fuzz 全0・lint 0 errors・**同型★0**（全10シート再生成）・**census 1654 不変**（triggerScope は census 次元外）。**parser/engine/decompiler 全て変更＝smoke/golden/fuzz 実施**。詳細 BUGFIXES 最上部。**次の一手＝§3 Opus タスク①の残＝(a)WX21-056/061 の引用付与本体は granted 内「このシグニを場からトラッシュに置いてもよい」が `OPTIONAL_TRASH_ENERGY_CLASS` STUB に mis-parse＝self-trash 任意コストの parse 修正が要る。(b)複合条件「Xで、Yかぎり」AND抽出（WX06-032/035）・(c)引用内 CHOOSE/選択肢展開（WXDi-D09-P20 等）・(d)場全体常時付与 ALLGRANT。いずれも過剰効果リスクで held 残置・Opus 継続。**
