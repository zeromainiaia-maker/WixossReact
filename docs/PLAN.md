...
| (lxxxi) | ✅**主要部消化（2026-08-03・続き334）＝残2枚**。全数分類の結果、`ARTS_COST_REDUCTION_BY_EFFECT` 保持 **110効果**のうち「〜に**なる**」＝置換は **14件**（残り92件は「減る/増える」＝既存の軽減規則の担当・4件は別機構）。うち **12枚13効果**を `costs.ts` の新関数 **`computeCostReplacement`** で消化（ベット形9枚／`WX09-Re02` の2段階／`WX05-038` 場のカード名／`WD22-041-UG` トラッシュ枚数）。golden 1330→1331。詳細 BUGFIXES 2026-08-03 (lxxxi) 節。**残＝別機構が要る2枚**＝①`WX21-071`（`SpellCastModal` に**任意・フィルタつき手札捨てUI**が無い＝「青と黒の＜電機＞を1枚ずつ捨てたら《黒×0》」）②`WXK03-002-E3`（**カード名指定の置換をゲーム間 PlayerState に永続化**する仕組みが要る＝《落華流粋》の使用コストを《赤1緑1無1》→《黒2無1》。CONTINUOUS 収集の `collectSpecificCardCostReductions`〔軽減版〕には乗らない） |
...
- **🆕 セッション（2026-08-03・続き334・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝§3 タスク12(lxxxi)「アーツ/スペル使用コストの**条件つき置換**（「使用コストは《X》に**なる**」）」を実装＝**12枚13効果が印刷コストでしか使えなかった**のを解消**（ユーザー指示「opusタスク12(lxxxi) を行う」）。golden **1330→1331**・全ゲート緑・**parser/effects JSON 完全不変**（UI コスト算出層のみ）。
  - **真因は engine ではなく「コスト算出が UI 層の regex 表にある」こと**＝`ARTS_COST_REDUCTION_BY_EFFECT` は `execStubPart1.ts:467` の no-op マーカーで、実コストは `src/screens/battle/costs.ts` の **`computeArtsEffectiveCost`（EffectText を regex で読む）** が決めていた。その規則が**全部「減る」＝印刷コストから引く形**（`removeNColorFromCost` 系）で、**置換の受け口が「対戦相手のセンタールリグが〜」1本しか無かった**。⇒ PLAN が言う「engine 未実装」は正確には**この表に置換の受け口が無い**。
  - **着手前の全数分類（PLAN の指示どおり実測）**＝`ARTS_COST_REDUCTION_BY_EFFECT` 保持は **110効果/110カード**。原文で **「減る/増える」92 ／「〜になる」＝置換 14 ／別機構 4**。⇒ **本件の対象は 14 だけ**と確定してから着手した。
  - **実装＝`costs.ts` に `computeCostReplacement()` を新設**（戻り値 `null`＝置換なし。`computeArtsEffectiveCost` の**軽減規則より前**で呼ぶ）。4系統＝①**ベット形9枚** ②**`WX09-Re02` の2段階**（`turn_arts_used`／`actions_done:'USE_SPELL'`＝engine の `ARTS_USED_THIS_TURN`／`SPELL_USED_THIS_TURN` と同じ判定源。**「両方」を先に見る**＝両方成立なら《白×0》が勝つ）③**場に《カード名》**（`WX05-038`）④**トラッシュN枚以上**（`WD22-041-UG`）。`《X×0》` は `'なし'` へ畳む。
  - **🟢 ベット形で「機構だけ入れても届かない」を1つ潰した**＝ベット宣言は**モーダル内**なので、一覧（宣言前）は印刷コストのまま。そのままだと**エナ不足でカードが開けず宣言に到達できない**（`WD17-006`＝《赤》×4→無料の札が永久に使えない）。⇒ 使用可否判定にだけ `isBetting:true` の結果を OR で足し（`ArtsModal` Phase1・`BattleScreen.getCardActions` アーツ枝の**両方**）、Phase2 は `betAmount` に追従して再計算＋**ベットON/OFFで選択済みエナを白紙化**（枚数要件が変わるため）。`SpellCastModal` は同関数経由で自動的に載る。
  - **未消化2枚は honest defer（別機構が要る）**＝`WX21-071`（`SpellCastModal` に**任意・フィルタつき手札捨てUI**が無い）／`WXK03-002-E3`（**カード名指定の置換をゲーム間 PlayerState に永続化**する仕組みが要る）。**新規在庫 (lxxxiv) を1件登録**＝**スペルカットイン経路にベット宣言UIが丸ごと無い**（`handleCutinUse` に `betCoins` も `is_betting_this_effect` も無い）＝該当8枚・うち `WX17-019` はカットイン専用で**ベットが一生宣言できない**。
  - **検証**＝golden に1本追加（ベット9枚の宣言前/後・`WX09-Re02` 4状態・`WX05-038` 場の有無・`WD22-041-UG` の24/25枚境界・既存「対戦相手ルリグ色」経路の回帰）。加えて**全CSV全カード走査**で新規置換の発火が**想定12枚＋同名再録のみ・想定外0件**、既存12枚は**置換前後で完全同値**を機械確認。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／**(lxxxi) 残2枚**／**🆕(lxxxiv)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜334 の全件が実機UI未検証**。⚠🔴**本件も続き333（タスク14）と同じく BattleScreen/モーダル層＝golden から叩けない**＝実機で**ベット宣言時のコスト変化**（`WD17-006`《赤》×4→無料・`WDK06-R07`《赤2無2》→《赤1無1》等）と **`WX09-Re02` の2段階**（相手がアーツ*か*スペル→《白1無4》／両方→無料）を確認すること。タスク14 の**セットアップ〜ターン終了の通し確認**が引き続き最優先。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き334＝§3 タスク12(lxxxi) 後）**＝golden **1331**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1289**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **259枚／110群**。
>
