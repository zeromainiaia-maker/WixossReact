...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕確認＋codex-work 実装）＝タスク12(lv)「任意・無コスト【出】」を配線し、🔴その先の live 該当2件が二重任意指定だったのを是正**（golden 1025→**1029**・census **1412 据置**・smoke 10726 全0・fuzz 全0・lint 0 errors・同型★0・build 冪等〔再生成後差分0〕・`npm run regen` 実施・gates 全緑）
  - **(lv) の原票は半分すでに解消済みだった**＝`WXEX2-71-E2` は (xxix)(2) の「英知＝N はコストではなく使用条件」決着で `mandatory:true` になり症状が消滅。`OPTIONAL_ACTIVATE` 包み機構も既に実装済みで、人間の通常召喚と効果配置には配線済み。**残作業＝機構の新設ではなく「機構が届いていない収集経路を塞ぐこと」**だった。
  - **未配線経路を5つ洗い出し、①だけを配線**（必達1本に絞った）＝①`executeGrow`（人間のグロウ）＝任意・無コスト【出】を `console.warn` で捨てていた＝**(lv) の症状そのもの**。新 `collectOptionalNoCostOnPlayForGrow` で `activeCondition`→`condition` の順に評価し `wrapOptionalOnPlay` で包む。包めないものだけ警告に残す（踏み倒しを作らない）。
  - **🔴Claude 確認で発見＝配線先の live 該当2件（`WX10-007`／`WX10-021`）はそもそも `mandatory:false` が誤り**。原文「【出】：デッキの一番上を見る。それが〜シグニの場合、それを場に出**してもよい**。」＝**「してもよい」は後半だけ。「見る」は強制**。任意性は内側の `ADD_TO_FIELD.optional:true` で表現済みで engine も尊重していた＝**二重指定**。そのまま包むと①原文に無い「発動しますか？」が増えて**二重プロンプト** ②「発動しない」で**強制の「見る」すら実行されない**。
  - **決定打＝同型効果の全数比較**＝`SEQUENCE[LOOK_AND_REORDER, ADD_TO_FIELD{optional:true}]` は live に5件あり、parser の正準形規則が生成する3件は**すべて `mandatory:true`**。`false` は MANUAL の2件だけ＝データ側の誤りと確定。**(xxix)(2) の英知判断と同型の論理**＝「任意なのが効果の一部だけで、内側の action で表現済みなら効果ヘッダは mandatory」。
  - **是正**＝2件を `mandatory:true` へ。curated 温存対象で `manualEffects.ts` に定義が無いため、再生成耐性を持たせて `scripts/fixLrigColorFilters.mjs` の外科補正層（`build:effects` 後段）へ登録。**機構は保持**（将来の真の全体任意【出】と他経路のため）。live **changed 2 / added 0 / removed 0**、build 再実行後差分 **0件**。
  - **engine 直叩き実測（Claude 独立再現）**＝是正後は `LOOK_AND_REORDER`→`SELECT_TARGET` で LOOK が必ず先に走り、「出す」で `SELECT_SIGNI_ZONE` まで到達しデッキから当該カードが除かれる／「出さない」で盤面不変。**golden に「同型5件は `mandatory:false` を持たない」を live 全数の不変条件として固定**＝再発を止める。
  - **⚠委譲運用の教訓**＝codex は指示された経路を正確に塞ぐが、**その経路が拾うデータ自体が正しいかは疑わない**。「機構を配線したら、その機構が拾う live 該当を全数、原文と突き合わせる」までが確認フロー。
  - **✅続けて COLLAB（未配線4経路の①）も消化**（golden 1029→**1031**）＝🔴**逆向きのバグ2つ**だった。**穴A＝直接 COLLAB（「コラボライバーN人を呼ぶ」）は `effectType`/`timing` しか見ずに raw effect を直積みしており、`mandatory`・`cost`・`activeCondition`・`usageLimit` を全て無視＝アシストルリグ ON_PLAY 537効果のうち `mandatory:false` 160件（うち cost あり **153件**）がコストを払わず発動していた**〔⚠当初 PLAN/BUGFIXES に書いた「293／91／88」は Claude の集計スクリプトが CSV を `split(',')` で読んでカラムずれを起こした過小値。`Papa.parse` で数え直して訂正〕（`WXDi-D06-006-E1`＝「手札1枚を捨てる：3枚引く」で手札 2→**5**・トラッシュ 0→**0** を実測）。既存の共通機構 `collectPlacedSelfOnPlayTriggers{placedByEffect:true, sourceIsSigni:false}` へ寄せて解消（修正後は `OPTIONAL_COST` の pay/skip が出て、pay で手札 2→4・トラッシュ 0→1）。**穴B＝CHOOSE 経由の「コラボしてもよい」枝（`INTERNAL_DO_COLLAB`）が `lastProcessedCards` をセットしておらず、配置したアシストの【出】が一切積まれない no-op** も同時に是正。**⚠Claude 追加実測で「コラボしない」枝が誤収集しないこと**（`lastProcessedCards: null`・配置なし）も確認済み。
  - **✅さらにアシストルリグ配置（②）も消化**（golden 1031→**1033**）＝COLLAB と**逆向きの穴**。`assistOnPlay` が `mandatory !== false` だけで絞っており、**任意【出】160件がコストの有無を問わず丸ごと不発**だった（537効果中）。薄い入口 `collectAssistOnPlayTriggers` を足して中身は既存 `collectPlacedSelfOnPlayTriggers` をそのまま呼ぶ形にし、任意【出】の収集が **0→146件**（包めない13件は踏み倒し回避で据え置き）。**`placedByEffect:false`**＝通常のアシストグロウは効果由来でないため（母集団の `byEffect`／`bySigniEffect` は全537件で**0件**なので取りこぼしゼロ）。🔴**着手前判定どおり `WXDi-P15-034-E1` の二重任意指定を是正**（原文の【出】は「1つを選ぶ」で強制・任意は②内の支払いのみ・JSON は既に内側 `OPTIONAL_COST` で表現済み）。⚠**同時候補の `WXDi-P11-032-E2` は「【出】《無》《無》：」＝コスト付きなので誤りではない**＝「action が CHOOSE」だけを混入シグナルにすると誤検出する。
  - **次の一手**＝**Opus：🆕(lv) 残2経路**〔**✅COLLAB・✅アシスト配置は消化済**／残るは CPU シグニ召喚・CPU グロウ＝「CPU は任意効果を発動しない」を明示据え置き中。CPU に任意選択をどう扱わせるかの方針決めが要る〕**／(xlvi) は残0クローズ済／(xxix) 残**〔(2)(B) 43件＝`costUnparsed` 47効果が worklist／(1) 未対応コスト80件〕**／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／(l) B群36件＝真の過剰実行は `WDK02-009` のレベル条件落ち1件／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋(xxix)(1)(2)＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。
