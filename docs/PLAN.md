...
- **🆕 セッション（2026-07-23・Opus 4.8＋Codex 分担・§6.3 G072 残6枚完結バッチ＝条件前置き付き「対戦相手のシグニがバニッシュされたとき」6/6 消化**（census 1702→**1698**・golden 637→**644**）**（続き254 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  - **投入前実測（Claude）**＝6枚中5枚は parser の前置きストリッパ不足で **self 誤変換**（相手バニッシュで発火しない偽陰性＋自身バニッシュで誤発火の偽陽性の両方向実害）、WXEX2-23-E2 のみ trigger 正で **action が「自トラッシュから ADD_TO_FIELD」＝奪取先逆・レベル上限なし・傀儡化なし**の別種実害。全6枚 AUTO＝PRESERVE 非該当。機構は全て既存の延長と確認（duringMainPhase 未配線・prevOwnerState 既存・causeOwnerId/causeSourceCardNum がスコープ内・STEAL_OPP_TRASH_PUPPET+levelLteTrigger 既存）してから群A〜Dに分けて投入。
  - **修正（Codex 実装）**＝群A メインフェイズ前置き（parser＋collectBanishTriggers section2/3 の duringMainPhase ゲート・WX05-040-E1/WX11-027-E2）／群B `banishedHadCharm` 新設（除去直前 signi_charms・4呼び出し系統全部に before-state 配線・WXDi-P11-TK05-E2）／群C `banishedByOwnEffect`/`banishedSourceStory` 新設（cause 引数＝中央 board-diff のみ渡す・バトル/パワー0非発火が正・CONT は発生源未保持で保守的非発火・WXK11-055-E2/WX13-051-E1）／群D WXEX2-23-E2 を STEAL_OPP_TRASH_PUPPET{levelLteTrigger} で MANUAL 化＋watcher entry へ triggeringCardNum 保持。スコープ外巻き込み（WX20-025-E2）を自力検出→regex 絞り→復元。resume 跨ぎ傀儡配置 golden まで指示どおり＝**差し戻し0の2例目**。
  - **検証（Claude・是正1件）**＝per-effect 機械 diff＝5効果ちょうど＋MANUAL1・巻き添え0・held +1（WXEX2-23 署名のみ）。**codex の「triggeringCardNum 依存0件」申告は誤り**＝WXK11-020-E2 の `powerLtTrigger`（動的filter語彙）が依存しており、従来未解決＝**無制限バニッシュの潜在過剰効果が今回の entry 保持で解消**（改善方向の挙動変化）。golden lock-in を Claude が追加（644）。教訓は CODEX_GUIDE §8 に記録＝「新しく値が入るフィールドの既存依存はコード grep でなく解決器の語彙リスト×JSON 走査で調べさせる」。詳細 BUGFIXES 2026-07-23。
  - **次の一手**＝**Opus 側**：(A) 続き254 の方針判断（P1完了宣言 or バッチ5 実測）が引き続き未決。§6.3 側は G072 クローズで残る近縁が「このシグニより低いレベルを持つ〜」watcher 相対レベル系（parser 5203行コメントの据置分）と CONT mutation の cause 追跡（群C の保守的非発火解消）＝どちらも小粒の続きバッチ候補。**Sonnet 側**：タスク1（§7 実機検証）継続。WXEX2-23-E2 の optional-cost 跨ぎ傀儡配置は §7 driver での実機検証推奨リストに追加してよい。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 644・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1698。
>
...
- **~~GRANT_LRIG_ABILITY の低品質展開~~ ✅2026-07-23（codex実装/Claude確認・完結バッチ）で3枚消化**＝新機構4本（`turn_arts_used_names`＋`NO_OTHER_ARTS_USED_THIS_TURN`・`LRIG_UNDER_TRASH_ANY`（新scope self_lrig_under・任意枚数選択→枚数記録）・`MILL.countPerLastProcessed`・`declared_guard_restrict_levels[]`＋GuardResponseDialog 消費＋トリプルクラッシュ3枚）で PR-204（他アーツ未使用＋エクシード2実払い→アップ＝無限アタック危険物の解消）／WD21-009（2/4/5多段＝resume跨ぎgolden固定）／PR-238（選択枚数×5両者ミル）を実装。⚠Claude是正2件＝トリクラのクラッシュ枚数拡張が**シグニアタック経路のみでルリグアタック経路（肝心の側）未配線**だったのを配線・PR-470A のフェッチが「同名」誤読（別名カード PR-470B）で常に不発＝`fetchCardName` 名指しへ是正。**残＝WX15-016 のみ**（付与【自】の「そのアタックを無効にする」＝進行中アタックのキャンセル機構なし・据置）。旧記述＝WX15-016（付与【自】の「この方法で置いたカードが《バーストアイコン》を持つ場合アタック無効」＝条件が IS_MY_TURN に誤約・相手ターントリガーで恒久false）／WD21-009（ルリグ下N枚→2/4/5枚の多段閾値＝平坦化すると無条件ガード封じ+トリクラの過剰発火）／PR-204（「カード2枚をルリグトラッシュに置いてもよい。そうした場合アップ」＝支払いゲート脱落で毎アタック無償ルリグアップ＝無限アタック）／PR-238（置いた枚数×5の比例ミル）。共通で「optional支払い→そうした場合」のコストゲート機構＋多段閾値が要る。SPDi43-03/11/12/13・WXK11-052 の「ターン終了時までこのルリグは『【自】…アップ』を得る」も同根（現状は即時 UP LRIG の平坦化近似＝§7の実機検証対象外）。
- **BURST内「そうした場合/〜の場合」の要新語彙テール7枚（2026-07-04・続き19・TRASH前段32偽陽性と易3枚是正後の残）**＝(a)~~LAST_PROCESSED フィルタ条件: WXEX1-43-BURST・WXEX1-36-BURST~~ **✅続き24で是正**（`LAST_PROCESSED_MATCHES{filter}` 新設・heldReview採用）。(b)**then内容欠落**: WD23-023-E-BURST（「デッキから1枚探してライフクロスに加えシャッフル」→SHUFFLE のみ＝SEARCH の then ADD_TO_LIFE 対応要）。(c)**任意化**: WX14-026-BURST（「クラッシュしてもよい」→強制 LIFE_CRASH）＝optional crash＋conditional ゲート。(d)**コストと対象の参照逆転**: ~~WX16-033-BURST~~ **✅2026-07-23（codex実装/Claude確認・コストゲート第2波）**＝STORE→自シグニBANISH→`LAST_PROCESSED_COUNT_GTE`ゲート→TRASH{targetsStored}（`storedTargetCards` の pause 跨ぎ永続化＝`PendingEffect` 配線を Claude 補筆）。~~WX17-041-BURST~~ **✅2026-07-23（完結バッチ）**＝`RETURN_TRAP_TO_HAND_ONE`（signi_traps から1つ選択→手札・0個なら空記録で bounce 不発）＋targetsStored BOUNCE。(d) クローズ。(e)**源違い**: WX19-Re10-BURST（「手札1枚をライフクロスに加える」→fromTop＝デッキから）＝ADD_TO_LIFE の from hand 対応要。
...
- **自パワー閾値条件の残テール（2026-07-03発見・素直な21枚は是正済）**＝(a)「代わりに」昇格型：WXDi-P01-054（【起】バニッシュの昇格）／WXDi-P12-067（被バニ反応の昇格）。(b)多段閾値型：~~WXDi-D01-016（15000/20000）~~ **✅2026-07-23（codex実装/Claude確認）**＝20000先評価→else 15000 の相互排他 CONDITIONAL（従来は 15000ゲート後に無条件blind捨てが常時実行＝二重ハンデス実バグ）。~~PR-470A~~ **✅2026-07-23（完結バッチ）**＝10000段DRAW＋25000段=自身→ルリグデッキ＋`fetchCardName` 名指しフェッチ（⚠codex の「同名」誤読＝別名カード PR-470B で常に不発、を Claude 是正）。(b) クローズ。(c)【起】の自パワー条件：WXDi-P03-062（起動時 evalUseCondition 配線の確認要）。
...
- ~~**G072 残6枚**（条件前置き付きの相手シグニ被バニッシュ反応）~~ **✅2026-07-23（codex実装/Claude確認）で6枚全消化**＝5枚は前置きが剥がせず self 誤変換（相手バニッシュで発火しない偽陰性＋自身バニッシュ誤発火の両方向）、WXEX2-23-E2 のみ action 側が「自トラッシュから ADD_TO_FIELD」の別種実害。**群A**＝WX05-040-E1/WX11-027-E2（「あなたのメインフェイズの間」→ any_opp＋既存 `duringMainPhase` を collectBanishTriggers section2/3 へ配線＋turnOwner:'self'）。**群B**＝WXDi-P11-TK05-E2（新 `banishedHadCharm`＝除去直前 `prevOwnerState.field.signi_charms[banishedZone]` 判定・4呼び出し系統すべてに before-state 配線・不明時は保守的非発火）。**群C**＝WXK11-055-E2/WX13-051-E1（新 `banishedByOwnEffect`/`banishedSourceStory`＝collectBanishTriggers に cause 引数を追加し中央 board-diff 経路のみ `causeOwnerId`/`causeSourceCardNum` を渡す。バトル/パワー0は cause 無し＝正しく非発火・CONT mutation は発生源未保持のため保守的非発火）。**群D**＝WXEX2-23-E2（既存 `STEAL_OPP_TRASH_PUPPET`+`levelLteTrigger` で MANUAL 化＋ON_BANISH watcher entry へ `triggeringCardNum`＝被バニッシュカードを保持）。⚠**entry.triggeringCardNum 追加の副作用で WXK11-020-E2 の `powerLtTrigger` 無制限バニッシュ潜在バグも解消**（codex は「依存0件」と誤申告＝Claude 検証で発見し golden lock-in 追加）。golden 637→644・census 1702→1698・held +1（MANUAL署名のみ）・巻き添え0。詳細 BUGFIXES 2026-07-23。
