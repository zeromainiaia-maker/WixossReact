...
| 1 | ~~§7 実機検証の横展開（旧・主力在庫）~~ **⛔ほぼ枯渇** | 検証（driver シナリオ追加のみ） | S×件数 | **§7 の残8項目のうち6項目が Opusタスク12 待ちでブロック**（(xiii) B4引用付与／(xix) WX04-005-E3／(xx) ON_TARGETED forced／(xxi) oppDraw発生源／usageLimit書き戻し＝R37③・ON_LRIG_GROW④）。**Sonnet が今すぐ取れるのは WX22-001-E3（クラフトトークン残・§6.4）だけ**。Opus12 が消化されたら再び主力在庫に戻る（意図的FAIL回帰シナリオを PASS へ反転させる作業が溜まっている） |
