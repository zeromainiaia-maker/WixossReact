...
**Opusタスク12＝未消化の在庫 8件（2026-08-10 続き404・408・417／2026-08-13 続き468・471／2026-08-14 続き474・475 で登録）**。⚠**2026-08-14 続き475 で (cxxv) を取り下げ（シナリオ偽陽性）・(cxxx) を新規登録**したので件数は据置。

| # | 症状 | 発見経緯 | 見立て |
|---|---|---|---|
| 🆕(cxxx) | 🔴**離場置換の「決定」が消費されずに `PlayerState` へ残留する**＝`applyEffectLeaveSubstitutes`（`effectExecutor.ts:671-686`）は先頭で `consumeLeaveSubstituteDecision` を呼んで**決定を取り除いた ctx** を作るが、**置換が成立しなかったとき（`replaced:false`）の呼び出し側がその ctx を捨てている**。実測＝`leave_substitute_choices` に `{'WX12-024#5':'none'}` を注入すると**盤面は完全に正しい**（victim だけがエナへ通常バニッシュ・問い0件＝決定は honor されている）のに、**解決後も `choices={"WX12-024#5":"none"}` が残る**。残ると `leaveSubstituteAskOptions`（`:702`）が「決定済み」と見て**以後その instance には二度と問わない**（トラッシュから戻した等で再登場すると無言で辞退扱い）。⚠**11箇所すべてが同じ形**＝`const sub = applyEffectLeaveSubstitutes(...); if (sub.replaced) { …sub.ctx… } ` の**else 側で `sub.ctx` を落としている**（`:964`／`:1048`／`:1229`／`:1575`／`:4931`／`:8317`／`:8332`／`:8347`／`:8363`／`:8469`／`:9119`） | 2026-08-14 続き475（PLAN §7 V-01 の実機再検証）＝`leaveSubDecisionNoneIsHonored` の**盤面判定はすべて通り、残留チェックだけが落ちた**ので局在が一撃で確定 | 直し方＝**`sub.ctx` を無条件に採る**。ループ5箇所（`:964`/`:1048`/`:1229`/`:1575`/`:4931`）は `if (sub.replaced)` の前に `cur = sub.ctx;` を1行足すだけ（⚠`execBanish`（`:964`）だけは `s = ownerState(own, cur)` を**先に**取っているので `cur` 更新後に取り直す）。`applyDirectAction` の6箇所は `ctx` を直接使っているので局所別名へ寄せる。⚠**検査用シナリオは既にある**（`leaveSubDecisionNoneIsHonored`＝現在 FAIL＝直れば緑に反転） |
...
| 🆕(cxxix) | 🔴**F-3 身代わりが「コスト0」で成立し、シグニが無償でバニッシュを回避する疑い**＝`WX14-026`（羅石　スイカリン）は `substituteCost:{lifeCrash:1}` なのに、**効果バニッシュで実機ログが `身代わり：羅石　スイカリンの下からスペル0枚をトラッシュして羅石　スイカリンのバニッシュを回避` を出し、`victimLeft=false`／`guest.life 7→7`**（＝**ライフも払わずシグニも場に残る**）。⚠**原文は「代わりにあなたのライフクロス１枚をクラッシュしてもよい」**なので、**ライフを払わずに回避できるのは過剰効果**。⚠**`WX14-026` は「対戦相手のターンの間」限定**（`activeCondition:{TURN_OWNER, owner:'opponent'}`）で、実機は host の MAIN・turn 2＝**victim 側から見て相手ターン**で撃っている | 2026-08-14 続き474（PLAN §7 V-10 の実機検証）＝`effectBanishLifeCrashSubstituteNotOnEffect` が「効果バニッシュでは身代わりされない（engine 対象外＝仕様）」を確認するはずが、**別種の身代わりが成立**した | 🆕**真因確定（2026-08-14 続き475）＝`lifeCrash` は「自動適用しない」だけで「問いには出る」ので、CPU がそれを選ぶと分岐が無く末尾へ落ちる**。①`collectLeaveSubstituteOptions`（`effectExecutor.ts:608-622`）は `lifeCrash` を **`kind:'optional'`／`autoEligible:false`** で列挙する。②`leaveSubstituteAskOptions`（`:705`）は **`kind==='optional'` だけで絞り `autoEligible` を見ない**＝選択肢として出る。③CPU は先頭を選ぶので `applyEffectLeaveSubstitutes`（`:682`）が決定を採るが、そこも **`autoEligible` を見ない**。④`applyEffectBanishSubstituteChoice`（`:463-509`）は `sacrifice` と `discardSpell` しか分岐せず、**`lifeCrash` は末尾の `trashStackSpell` 経路へフォールスルー**＝スペルが1枚も無いので**0枚トラッシュ＝コスト0で成立**し、`身代わり：…の下からスペル0枚をトラッシュして…` のログが出る（実機ログと完全一致）。⇒ 直し方＝**(a) `applyEffectBanishSubstituteChoice` に `lifeCrash` 分岐を実装する**（`field.check` を挟むので同期適用できない＝要設計）**か (b) 実装するまで `leaveSubstituteAskOptions` から `autoEligible:false` を除外する**（＝(cxxix) を無害化する最小手）。⚠**(b) だけだと「対話 policy が入れば選択肢として出せる」という `:411-413` のコメントと矛盾する**ので、`applyEffectBanishSubstituteChoice` の末尾に**未知 costType を黙って trashStackSpell 扱いしないガード**を必ず併せて入れる。⚠**検査用シナリオは既にある**（`effectBanishLifeCrashSubstituteNotOnEffect`＝現在 FAIL） |
...
| 🆕(cxxvi) | 🔴**相互身代わりで同一インスタンスがエナに複製される**＝`count:'ALL'` のバニッシュで、victim A が B を身代わりに指定し、victim B が A を指定すると、**最終盤面のエナに同じインスタンスが2枚**現れる。実測＝`gEnergy=["WX12-024#52","WX12-024#52","WD03-013#53"]`（`#52` が重複）／同時に **`leave_substitute_choices` が1件消費されずに残留**（`{"WX12-024#52":"banishSubstitute:WX12-024#52:WX12-024#51"}`） | 2026-08-13 続き466＝`leaveSubAllTargetsAskedPerVictim` の実機ログ。⚠**シナリオはこの複製を見ていなかったので緑のままだった**＝**2026-08-14 続き475 で複製検査を必須条件に追加**し、**現在は赤のトリップワイヤ**（`🔴【instance複製＝§3 (cxxvi)】…重複=["WX12-024#52"]`） | 🆕**真因確定（続き475）＝`applyBanish` のループが「その instance がまだ場にあるか」を見ていない**。`#51` の身代わりで `#52` が先にエナへ移った後、ループが `#52` 自身の番を処理し、`removeFromField(#52)` は**空振り**したまま `banishDestination` が**エナへもう1枚 push** する（`effectExecutor.ts:969-974`）。**残留した決定（→(cxxx)）も同じ1手で説明できる**＝`#52` は既に場に無いので `collectEffectBanishSubstituteChoices` が空を返し（`:428`）→`replaced:false`→消費済み ctx が捨てられる。⇒ 直し方＝**ループ先頭で「場に居ない num は skip」**（`s.field.signi.some(st => st?.at(-1) === num)`）。⚠**同じ形の11経路すべてに要る**（(cxxx) と同一箇所）。⚠**fuzz が拾えていない**（乱択で相互指定の盤面に到達しない）＝**再現盤面が判明している今のうちに golden で固定する**のが安い |
...
- **🆕 セッション（2026-08-14・続き475・Opus 5）＝**§7 実機検証の続き**＝**V-10 の赤3本を緑へ**、さらに**V-01 を再検証して §3 在庫バグ (cxxv) を「シナリオ偽陽性」として取り下げ**（4本中3本が調査ゼロで緑に反転）。**残った赤は engine の実バグ3件**（(cxxvi)/(cxxix)/新規 (cxxx)）で、**3件とも真因を file:line まで確定**した。⚠**engine / parser / live JSON / `src/` は1行も変更していない**（変更は `scripts/verifyBattleDrive.mjs` と docs のみ）。**ゲート全緑・数値据置**（golden 1964・census 831・smoke 10688／SKIP 0・fuzz 全0・lint 0 errors/259 warnings・live JSON changed **0**）。**
  - **✅① V-10 が決着**＝`effectBanishSubstituteRunsAutomatically`／`effectBanishNoSubstituteWithoutSacrifice`／`effectBanishSubstituteDiscardsSpell`＋既存の `battleBanishSubstituteStillInteractive` が **2回連続 ALL PASS**。⭐**続き474 の「効果バニッシュ＝問いなし自動適用」という前提が誤り**で、`BANISH{count:1}` は `resumeSelectTarget` の hoist（`effectExecutor.ts:7412`）を通るため**被害側へ問いが1件出るのが現行の正**。期待値を `asks===1 かつ victim 名を含む` へ変え、**機構が動いた証拠は「問いログ＋身代わりログ」の2本立て**にした（§7 📌4）。
  - **⭐② (cxxv) を取り下げ＝在庫バグではなくシナリオ偽陽性**。「数値 count では離場置換の問いが出ない」の正体は、**シナリオが `pendingCandidates` の index をそのまま `pick-<idx>` に使っていた**こと＝`opp_field` はモーダルが候補を reverse 描画するので **`pick-0` が victim ではなく犠牲シグニ**を指し、**犠牲を直接バニッシュしていた**（victim は対象ですらない＝問いが出なくて当然／**盤面は身代わり成立時と同一**）。`clickPendingInstance` へ差し替えるだけで **`leaveSubCpuAutoRespondsSubstitute`／`leaveSubAskDirectedToVictim`／`leaveSubDecisionKeyIsHonored` が PASS**（`asks=1`・`responder=CPU`・options＝`banishSubstitute…`／`none:置換しない`）。🔑**罠6 が判明したのは続き469 で、この4本を書いた続き466 より後**＝**新しい罠は既存の赤在庫へ遡って適用する**（→§7 📌12）。
  - **🔴③ 残った実バグ3件は真因まで確定**＝**(cxxx)【新規】離場置換の「決定」が消費されずに残留**（置換不成立時に `applyEffectLeaveSubstitutes` の消費済み ctx を呼び出し側が捨てている＝**11経路すべて同じ形**）／**(cxxvi)** 相互身代わりの instance 複製＝**`applyBanish` のループが「もう場に居ない num」を再処理**して移動先へ2枚目を push（`:969-974`）＝**(cxxx) と同じ1手で直る**／**(cxxix)** `lifeCrash` は `autoEligible:false` なのに **`leaveSubstituteAskOptions` が `kind==='optional'` だけで絞るため選択肢に出て**、`applyEffectBanishSubstituteChoice` に分岐が無く**末尾の `trashStackSpell` へフォールスルー＝0枚トラッシュで成立**（実機ログと完全一致）。
  - **🔴④ シナリオ側の偽陽性・フレークを3種つぶした**（→§7 📌12〜14）＝①**「今は緑」が写り込んだバグを見ていないだけ**＝`leaveSubAllTargetsAskedPerVictim` は複製を見ておらず緑だった⇒**エナの重複 instance を必須条件に追加して赤のトリップワイヤ化** ②**settled 直後の判定は早すぎる**＝盤面は先に真になるが `game_logs` が数百ms遅れる⇒**PASS しない間は最大12反復ぶん待って確定**（単体PASS・バッチFAIL の位置依存フレークを解消）③**pick が React に載らないと `決定 (1/1)` が永久に出ず 211秒溶ける**⇒`SELECT_TARGET` が続く間は **pick からやり直す自己回復**を追加。
  - **▶ 次の一手**＝**Opus 側**：**(cxxvi)+(cxxx) を1手で直す**のが最優先（**`applyBanish` 系ループ11経路に「場に居ない num は skip」＋`sub.ctx` の無条件採用」**＝赤2本が緑へ反転）。次に **(cxxix)**（`autoEligible:false` を問いから外す最小手＋未知 costType のガード）、**(cxxvii)(cxxviii)**（同型の「対象種別の取り違え」）。次点＝§6.4 O-20 の残21件・O-19。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き475（§7 実機検証の続き＝V-10 決着＋V-01 再検証）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、golden **1964 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。**live JSON の変更は 0**（本セッションは `scripts/verifyBattleDrive.mjs` と docs のみ＝engine/parser/`src/` 不変）。🆕**実機シナリオ（`verifyBattleDrive.mjs`）＝既定 order の総数は据置**（新規追加0・**判定を直したのが5本**＝V-10 の3本＋V-01 の共通 driver（4シナリオ共有）＋`leaveSubAllTargetsAskedPerVictim` の複製検査追加）。**緑/赤の内訳が 45緑12赤 → 49緑8赤**（V-10 の3本と V-01 の3本が反転／`leaveSubAllTargetsAskedPerVictim` が新たに赤＝(cxxvi) のトリップワイヤ）。🆕**Opusタスク12 の在庫＝8件 据置**（**(cxxv) を取り下げ・(cxxx) を新規登録**したので差引0）。
...
- **🔶 V-10 F-3 身代わりを効果バニッシュへ配線（続き406）＝2026-08-14 続き474 で実機検証**（Codex 起案→Claude 実行・シナリオ5本を既定 order へ登録）。⚠**Codex は `0xC0000142`（通算8回目）で最終工程が実行できず、ゲート・SHA・エンコーディング検査は検証側が引き取った**（全緑・削除0・BOMなし）。
  ...
  - [ ] 🔴**(cxxix)＝`WX14-026` が「コスト0」で身代わり成立**（→§3）。`effectBanishLifeCrashSubstituteNotOnEffect` が **FAIL**＝**ライフを払わずシグニが場に残る**。🆕**続き475 で真因確定**＝`lifeCrash` は `autoEligible:false` だが **`leaveSubstituteAskOptions` が `kind==='optional'` だけで絞るので選択肢には出る**→CPU が選ぶ→**`applyEffectBanishSubstituteChoice` に `lifeCrash` 分岐が無く末尾の `trashStackSpell` へフォールスルー**＝**0枚トラッシュで成立**（実機ログ `…の下からスペル0枚をトラッシュして…` と完全一致）。
  - 📋**参考（旧記述）**＝下の項目が当初の検証内容。
