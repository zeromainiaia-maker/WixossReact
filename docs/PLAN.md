...
- **🆕 セッション（2026-07-31・続き304・Opus 5〔**Claude が実装まで担当**〕）＝タスク16 `[B維持]` から `WX24-P2-051-E1`（《ガードアイコン》を持たない捨て札をエナへ）を1件消化**（golden **1184→1187**・census **1361 据置**（`BASELINE_HIGH=1361`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・held **251枚/98署名 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **20→19**）
  - **① 語彙は全部そろっていた＝新語彙0本**。本体 STUB `NON_GUARD_DISCARD_TO_ENERGY` は実装済み、捨て札の限定は既存 **`TargetFilter.noGuard`**（`effects.ts:451`／`execUtils.ts:631`）、「コストか効果によって」は `collectHandDiscardTriggers` の既定挙動と一致（【ガード】宣言の手札捨ては `hand_discarded_just`・`asCost` のどちらも立たず元から来ない）＝**原因軸の追加すら不要**だった。
  - **② 🔴 timing を足すだけでは「発火するが何も起きない」no-op になるところだった**＝STUB は対象を `ctx.lastProcessedCards` から取るが、**AUTO トリガー経由の ExecCtx にはこのフィールドが入らない**（`BattleScreen.tsx:4142` の entry→ExecCtx 構築に含まれない）。そのまま採用すると**何も起きないまま《ターン1回》だけ消費**する。⇒ collector が entry に **`triggeringCardNum`＝捨てられたカード**（`triggerFilter` があるときは**一致した1枚**）を載せ、STUB は `lastProcessedCards` が空ならそれを使う形にした。**golden に「STUB が実際にトラッシュ→エナへ動かす」E2E を置いて固定**。
  - **③ 波及ゼロを実測**＝`triggeringCardNum` を消費する語彙（`targetsTriggerSource`／`isTriggerSource`／`powerLteTrigger`／`levelEq|Gt|LtTrigger`）を ON_HAND_DISCARDED 32効果で全数走査して **0件**。trigger 系に見えた `WX24-P1-059-E1` の `thisCardOnly` は **`ctx.sourceCardNum` 経由**（`effectExecutor.ts:610`）で無関係と確認。
  - **④ ⚠同 STUB id の実装が2つあり `execStubPart2.ts:1509` 側は到達不能**（dispatch は part1→part2→part3 で part1 の分岐が必ず返す）。実装の正は part1 側であることをコメントで固定した（**両方直すと片方が永久に死んだまま divergent になる**）。
  - **⑤ 逆翻訳の落ち＝2回連続で自分が踏んだ**。初版は捨て札の限定（`noGuard`）が出ず timing の既定文言だけだった。前回（`byOwnEffect`）と同じ失敗モードなので、**`triggerFilter`/`triggerCondition` に何か足したら decompiler も必ず見る**を手順として固定すべき。
  - **次の一手**＝**Opus：タスク16 残＝`[B維持]` 5件**（本件で 6→5）。①`WXDi-P13-051-E3`（`byOpponentEffect`＋**手札捨てとエナ→トラッシュの2経路 OR**＋「効果1つによって」の解決単位重複排除＝`ON_HAND_DISCARDED` 系の最後の1件で**最も重い**）②`WXDi-P06-038-E1`（エナ→任意宛先＋原因）③`WX05-020-E1`（ターン累積クラッシュ枚数）④`WXDi-P11-063-E2`（シグニの下に置かれた）⑤`WXDi-CP02-068-E1`。⚠**5件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**🟢 投入前に必ず「解決時に engine がその値を掴めるか」を確かめる**＝本件も前件も、timing だけ戻すと no-op／条件素通りになる罠が実際にあった。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜304 の全件が実機UI未検証で積み上がっている**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き304（タスク16＝`WX24-P2-051-E1` 非ガード捨て札→エナ・1効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1361 据置**（`BASELINE_HIGH=1361`）、golden **1184→1187**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251枚／署名グループ 98 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WX24-P2-051-E1`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 20→19 効果/19クラスタ**（⚠**実質17**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 6→5**。**新語彙0本**＝本体 STUB も `TargetFilter.noGuard` も既存。engine 変更は**「そのカード」の受け渡し**のみ＝`collectHandDiscardTriggers` が entry に `triggeringCardNum` を載せ、STUB が `lastProcessedCards` 空時にそれを使う（AUTO 経由の ExecCtx に `lastProcessedCards` が入らず no-op になる罠の解消）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1187**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1361**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 `WX24-P2-051-E1` 後に実測）。
>
