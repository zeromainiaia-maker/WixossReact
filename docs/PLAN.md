...
**直近＝2026-09-21＝🏁`V-283`（コンボの「使い方」の保存）＋🏁§5.7 `S-31` ②＝CPU が手札を捨てるコストを払えるようになった**（全文は [BUGFIXES.md](./BUGFIXES.md) 第439バッチ）
- 🏁**`V-283`**＝`v268CpuDeckPlan` に②'を足して実機で押した（`cpu_plan.combos[0].steps` が DB に届く）。
  ⚠**罠**＝デッキの中身をハードコードして `selectOption` が30秒タイムアウト（`VERIFY_DECK` は中身が変わる）⇒ **選択肢を読んで先頭2枚**にした。
- 🔴**`S-31` ②の真因**＝`S-14` で書けるようになった `WD16` のコンボが**1回も動かなかった**＝その【起】のコスト
  （手札から＜電機＞を1枚捨てる）が**CPU の自動支払い allowlist に無く、そもそも撃てなかった**。
- 📏**母集団**＝live の ACTIVATED **2,632効果のうち 648（24.6%）/ 603枚**が「CPU が払えないコスト」を含む。
  この回は**手札を捨てる2キー（`handDiscardSigni` 63／`discard` 62＝125効果）**だけを取った。
- 🆕**`pickCpuDiscardCostIndices`**＝**順番は手札上限の捨て札と同じ1本**（弱い札から・【ガード】は最後・作戦データつき）／
  **1枚ずつの可否は人間のモーダルと同じ関数**。**本番の実行と探索の近似適用の両方に同じ index を渡す**
  （🔴探索に払わせないと「タダで撃てる【起】」に見える）。
- 📏**実測＝`WD16` の Ｆ・Ｍ・Ｓ の【起】が 0回 → 16回（4戦）**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31`**（残り＝①「N体並べる」条件 ③条件つき／アーツの使いどころ／使わない札。⚠**どれが欲しいかユーザーに確認**）。次に `S-29` |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（🔥`S-31` 残り・`S-29`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**CPU が払えないコストは残 523効果**（`exceed`／`energyTrash`／`fieldTrash` ほか＝`S-31`） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4358**）｜**実機 PASS**（5ターン/130手/168s ＋ `v268CpuDeckPlan`） |
...
- **2026-09-21 時点**（第421〜439バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（🔵`S-6`・🔥`S-31` 残り・`S-29`）｜**作戦データの入力 21デッキ**（ユーザー作業）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4358**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・**5/130手**）
