...
| (lxxxiii) | **第13波まで一部消化（2026-08-02）**＝`SP27-015-E1` を live 実働化。`FIELD_SIGNI_TO_ACCE` の共通離場処理前に元アクセ instance を保持し、前半host選択後の任意3段目resumeで別の自シグニへ移設する。`WXDi-P14-087-E1` は `life-crash source event metadata persistence`（攻撃／効果／コスト全入口＋check/pending対応）待ちとして近似せず defer。正式非該当10件を除く**実装課題残5**＝`PR-366-E3`／`WX20-036-CB-E1`／`WX24-P2-010-E1`／`WX06-019-E1`／`WXDi-P14-087-E1`。全経路・実測は BUGFIXES 第13波節。 |
...
- **census 現行補正（第12波）**：高シグナル欠落 **1290**（`WXEX2-19-E2` の field-sourceアクセ live 実働化で 1291→1290）。上行の長期推移文中に残る1291は第11波時点の旧値。
