...
**■ 未消化（着手順の目安つき）**
- **`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査**＋`verifyEffects` の「定義なし」誤検出（注釈・トークン）の除外改善。**2026-08-10 実測**＝`node scripts/_checkAllEffects.mjs` は**検出33件（うち `MANDATORY_SUSPICIOUS` 7件）＋警告 `ONPLAY_DEAD_OPTIONAL` 15件**、`npm run verify`（`scripts/verifyEffects.ts`）は**アクション[STUB代替?] 5件／アクション[要確認] 3件／タイミング 4件・STUB参照カード135件**。**計器の精度改善＝小バッチ**。⚠続き407 の教訓「**過剰報告する計器は無いより悪い**」がそのまま当てはまる枠＝**まず33件を1件ずつ「真バグ／誤検出」に仕分けてから、誤検出側をルールで削る**（先に削ると真バグを一緒に消す）。
- **生ID残存＝表示or実装の穴**：逆翻訳に出る `[STUB:X]` は **2026-08-10 実測で 907 箇所／種類は上位が `GAIN_ABILITY_THIS_GAME` 19・`SIGNI_REPOSITION` 7・`RETURN_SELF_ARTS_TO_LRIG_DECK` 6・`PREVENT_POWER_MODIFY_BY_OPP` 5**（`grep -ho '\[STUB:[A-Z_]*' docs/decompile_sheet*.txt | sort | uniq -c | sort -rn`）。一覧は `STUBS.md`。`[条件:X]`/`[アクション:X]` は解消済み。**「STUB＝未実装」ではない**（実装済みハンドラの表示名でもある）ので、**まず「表示だけの穴」と「実装の穴」を仕分ける計器が要る**。
