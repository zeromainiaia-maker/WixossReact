...
| 🆕(xxix) | **semantic audit stub群 round3（2,101枚・findings 2,799件・続き146）**＝①duration系統 **✅続き148で34効果是正**・②選択肢欠落 **✅続き149で84効果是正**（詳細 BUGFIXES 続き148/149）。**残**＝③「そうした場合」IS_MY_TURN 誤変換で PARTIAL 刻印に掛からない盲点2件（WX04-030-BURST／WX06-014-E2）＝計器の死角・実母数は152件より広い。owner取り違え等の未検証222クラスタは次回候補。詳細 `docs/_semantic_audit_stub_round3_triage.txt` |
...
| 🆕(xliii) | **census の系統的偽陽性＝`BANISH_REDIRECT` 族（続き217・Opus）**。census のカテゴリ判定が `BANISH_REDIRECT` を「エナゾーンに置く」「〜される場合」に対応する語彙として知らないため、**JSON が原文どおり正しくても高シグナルに残り続ける**（`WXDi-CP02-071-E1`／`WX09-022-E1`／`WXK11-032-E1`／`WXDi-CP02-072-E3` 等）。⚠**マスキングで実欠落を隠す危険があるため独立したパスで慎重に**＝カテゴリに語彙を教える前に、その族の全効果が本当に原文どおりかを個別確認してから。 **🆕続き218 で手順の前例ができた**＝「ダウン/アップ状態フィルタ」カテゴリの `lrigDown` 偽陽性を、①族の全21効果を個別確認（うち6件が実バグと判明・先に修正）→②`extraOk` で**表現済みのときだけ**合格（コスト句を除いた残りに状態語が残ればフラグ維持）という順で解消した。**同じ順序で進めること**（keys に語彙を足すだけの無条件マスクにしない）。 |
...
- **🆕 セッション（2026-07-19・続き218・Opus・**タスク12(xli) を完了**＋(xliv) 第1弾＝「N種類以上」条件・lrigDown コスト限定・BANISH_REDIRECT のパワー0限定/owner・場出しのみ pick の4機構。golden 503→508・census 1919→1895）**
  ...
  - **後半（218b）＝タスク12(xliv) `BANISH_REDIRECT` 残テールの第1弾**。族36効果を全数棚卸しし、**engine が今の設計で忠実に表現できる分だけ**消化＝(1)「パワーが０以下の」限定脱落2件（`WXDi-P10-009-E3`／`WXDi-CP02-102-E2` が**相手の全バニッシュを常時トラッシュ送り**にしていた過剰発火。`whenPowerZero` 新設＋`power0_banish_to_trash_opp_only` をパワー0消滅経路に配線・ターン境界リセット3箇所）(2) owner 誤り5件（「正面の」3・「それが」照応1・「このシグニによって」1＝すべて `self` に落ちて**原文と逆の意味**だった。うち `WXDi-D04-016-E2` は curated が正で fresh だけ誤っていた既存の乖離）。
  - **⛔残りは §6.3 級と判定**＝`BANISH_REDIRECT` は PlayerState の**真偽フラグ**実装で `action.target` を見ない。属性フィルタ5件は**凍結/チャーム/感染がゾーン添字の状態で `removeFromField` 後には消えている**ため `banishDestination` へ除去前 state を渡すシグネチャ変更（呼び出し16箇所）が要り、単体対象4件は**対象選択フロー自体が無い**。近似すると「JSON が限定を主張して engine が無視する」偽陰性になるので据置した（(xliv) 行に理由つきで記載）。
  - **末尾（218c）＝タスク12(xli) 残(a) の消化。これも単カードではなく15効果の系統だった**＝`LOOK_PICK_CHAIN{then:"field"}` という正しい語彙は engine・decompiler とも実装済みなのに、それを生成する dual-pick 規則が**手札段と場段の両方**を要求していたため、「その中からシグニをN枚まで場に出し、残りを…」という**場出しだけの形**が bare `LOOK_AND_REORDER` へ縮退し**場に出す部分が丸ごと消える no-op** だった。単段規則を新設（フィルタは既存ヘルパ流用・残りは デッキ下/デッキ上/トラッシュ/エナ の4種）。engine の `remainder=energy` 未実装（残りが黙ってデッキに残る）も併せて修正。
  - **⚠今回いちばん重要な副次発見＝`PARTIAL` 誤刻印は計器汚染では済まない**。新規則の効果が軒並み PARTIAL になった原因は**前置の「あなたの」が規則の外に残り** `parseActionText("あなたの")`→UNKNOWN→`markSilentFallback()` を踏んでいたこと（実際には何も失っていない）。**`build:effects` は MANUAL/PARTIAL を含むカードを丸ごと温存する**ため、**PARTIAL を刻んだ時点でそのカードには以後の parser 改善が永久に届かなくなる**。規則に `(?:あなたの)?` を含めて解消し、**同じ欠陥を持っていた先行 dual-pick 規則にも適用**した。**新規則を足すときは parseStatus が AUTO で出ることを必ず確認する**。
  - **退化を1件ガードで回避**＝`WX24-P2-001` は後続の**引用能力付与**（「それは『【自】：このシグニがアタックしたとき…』を得る」）を前後文の平坦化で拾うと**引用の中身が即時実行に化ける**ため、規則ごと対象外にした（`GRANT_QUOTED_AUTO_ABILITY` 経路待ち）。MANUAL 兄弟を持つ2枚は `heldReview` に載らないので**AUTO 効果だけ外科的パッチ**。
  - **次の一手**＝Opus は **(xliii)＝`BANISH_REDIRECT` 族の census 偽陽性**（族の全数棚卸しは続き218b で済んでおり、続き218 で確立した「族を全数確認してから緩める」手順をそのまま適用できる）。**(xliv) 残12効果は §6.3 の target スコープ機構待ち＝取らない**。ほかに timing[C] 残43効果／タスク12(xxii) 残50件。**棚卸し候補**＝既に PARTIAL で採用済みのため温存され続けているカード群（`WX24-P1-017`／`WX25-P3-038` ほか）の実態確認。**Sonnet はタスク1（§7 実機検証）**＝今回 **要実機検証＝センター/レベル限定の lrigDown コスト**が増えた。
### 📊 恒久指標（維持中・逐次更新）
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 1895【効果単位】**（2026-07-19 続き218＝Opus・①「N種類以上」条件の語彙化で1919→1916／②`lrigDown` コスト限定の実装＋表現済みのときだけ偽陽性を解消して1916→1899）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。前提＝`docs/_effect_srctext.json` が最新であること。明細 `docs/_vocab_census.txt`、過去の計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109以降。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 508・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1895。
>
