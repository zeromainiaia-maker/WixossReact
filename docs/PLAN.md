...
- **🆕 セッション（2026-07-19・続き204・Opus 4.8・**タスク7「未実装action型」をクローズ**＝`PREVENT_DAMAGE`＋`COST_SUBSTITUTE` の engine 実装・golden 451→456）**
  ...
  - **次の一手**＝**タスク7は消滅**したので、Opus はタスク2残＝WXK08-005（キーの使用タイミング動的付与＝§6.3級）・タスク1残(a)（granted CONTINUOUS の BLOCK_ACTION 収集機構）・タスク12（今回登録のグロウ経路プラミング穴を含む在庫）から取る。Sonnet は§3タスク6の未採用37効果の精密diff・採用判定（据置のまま実行可能）。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 1963【効果単位】**（2026-07-19 続き203＝終止形サーチ退化＋levelGtLastProcessed で1967→1965・前置CHOOSE復元＋ルリグデッキ戻し幻覚除去12枚採用で1965→1963。続き201＝`TRANSFER_TO_DECK` 幻覚除去で1968→1967、続き200＝名前filter 2効果の復元で1970→1968）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。前提＝`docs/_effect_srctext.json` が最新であること。明細 `docs/_vocab_census.txt`、過去の計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109以降。
