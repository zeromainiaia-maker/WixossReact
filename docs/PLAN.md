...
- 🏁**セッション（2026-08-28・続き703・Opus 5 単独）＝Sheet1 残8枚を1枚ずつ。🔴実機だけが見つけた engine バグ2件。**
  ユーザー指示＝**選択肢A（Sheet1 の残り8枚）**。8枚は `audit` 3／`held` 3／`partial` 1／`idset` 1＝
  **別々の検出器**が指していたので1枚ずつ出所を辿った。
  ...
  - 🔴🔴**この巡の本題は Sheet1 ではない**＝「JSON を直したのに実機で何も起きない」を追った結果、
    **`GRANT_FIELD_SIGNI_ABILITY` を持つ live 71効果が、場のシグニでは1件も収集されていなかった**ことが判明。
    **golden も smoke も fuzz も census も全部緑のまま**で、**実機シナリオだけが見つけられる層**だった。
    - (A) `BattleScreen` の `augMap` が **素の `Map`**＝`new Map(baseEffectsMap)` は `InstanceMap` の
      **実エントリ（CardNum キー）だけを複製**してフォールバック機能を捨てる。付与コレクタは場のシグニを
      **instanceId**（`'WX11-053#1'`）で引くので **`effectsMap.get(top)` が常に undefined**＝付与宣言0件。
      ⚠最後に `new InstanceMap(augMap)` で包み直しているので**外から見た型は正しく、内部だけが壊れていた**。
    - (B) その手前のゲート `hasFieldGrant` が **action 直下しか見ていなかった**＝連用中止形
      （`SEQUENCE[POWER_SET|POWER_MODIFY, GRANT_FIELD_SIGNI_ABILITY]`・live 11効果）で収集が呼ばれなかった。
      ⚠**すぐ下の `hasPlayerFieldGrant` は最初から SEQUENCE を見ており、片側だけの穴**。
    - ⇒ **A と B は両方直さないと通らない**（反転確認2本で実測）。
  - ✅**parser の退化2件**（held/partial が正しく止めていた）＝
    ①`OPTIONAL_TRASH_SELF` が一度も挿されなくなっていた（7効果。素の任意 TRASH は `asCost` が立たず
    **自己トラッシュが「効果によるトラッシュ」に見える**）②チャーム保護の対象クラスが
    `parseStoryFilter` の「先頭の条件節のクラスは落とす」規約（Sheet1 B10）に巻き込まれて消えていた
    （`WX04-052-E1`＝**あなたの全シグニを守る過剰実行**）。
  - ✅**`O-128` (a)**＝連用中止「基本パワーはNになり、…「【自】」を得る」の枝を追加（2効果）。
    🔴**A/B が退化を1件捕まえた**＝引用が**【常】**の形は既存経路が `SEQUENCE[POWER_SET, GRANT_PROTECTION]` へ
    平坦化しており、`collectEffectImmuneSigni` は**この正準形だけ**を読む＝付与へ包むと耐性が消える。
    ⇒ **引用が【自】のときだけ**付与へ回す（golden にトリップワイヤ）。
  - ✅**台帳 closure 4件**＝`WX02-020`／`WX05-022`（2本）は **B27・B24 で既に直っていたのに書いていなかった**だけ。
    live を1件ずつ確認してから閉じた。
  - 🆕**`O-133` を登録**＝**live 限定の MANUAL スタンプ**（`manualEffects.ts` に定義が無いのに live が MANUAL）
    ＝**実測 612 / 1,035効果**。収穫マージの「手修正は不可侵」で素通りし、
    **held / partial / idset のどのバケツにも出ない第4の死角**。本巡でも parser 修正が2効果に届かなかった。

**▶ 次の一手**＝**`O-133`（live 限定 MANUAL スタンプ 612効果の仕分け）**を推す。
本巡で「parser を直したのに live に届かない」を実際に2回踏んでおり、**計器が1つも無い**のが一番効く。
取り方＝①`manualEffects.ts` に定義が無い live MANUAL を全数列挙する計器を作る
②live と fresh が**実体同一**なものは**スタンプを AUTO へ落として解凍**（本巡の `WXK10-033-E1` と同じ手）
③実体が違うものだけ人が読む。⚠**一括で AUTO へ落とさない**＝意図的な curation（`WX17-077-E2` のような
別設計）を潰す。
機構側なら **`O-130`**（`ON_OPP_ARTS_USE` の帰結が「そのシグニ」）か **`O-128` 第2バッチ**（残り25カード）。
⚠🔴**Sheet1 要対応 0 は「Sheet1 が終わった」ではない**＝4つの検出器が**いま何も指していない**だけで、
**フラグの立たない863枚は計器が見ていない**。本巡がまさにその証拠＝Sheet1 を追った副産物として
**どの計器にも映らない engine バグ2件**が出た。**計器がゼロを指しても実機シナリオを増やす手は止めない。**
...
| **O-133** | 🆕**live 限定の MANUAL スタンプ＝`manualEffects.ts` に定義が無いのに live が MANUAL**（実測 612 / 1,035効果） | M／計器を先に作る | **2026-08-28 続き703（Sheet1 残8枚）で分離**＝収穫マージは live の `parseStatus:MANUAL/PARTIAL` を**効果単位で不可侵**にする（`buildEffectsJson.ts:274`）。**JSON を直接手修正した効果を守るための仕様**だが、その結果**`manualEffects.ts` に出所が無い MANUAL 効果は parser の改善を永久に受け取れず、しかも `_held_fresh` / `_partial_fresh` / `_idset_fresh` の**どのバケツにも出ない**（＝§5.3 K の「第3の死角」とも別の**第4の死角**）。■**実測**＝live の MANUAL/PARTIAL **1,035効果**のうち **612効果**が `manualEffects.ts` に定義を持たない。■**実害を実際に踏んだ**（続き703）＝`OPTIONAL_TRASH_SELF` の parser 修正が **`WXK10-033-E1` と `WX17-077-E2` に届かなかった**。前者は live と fresh が**トラッシュの形以外まったく同一**だったのでスタンプを AUTO へ落として解凍した。■**取り方**＝①**まず計器**（`manualEffects.ts` の定義集合と live の MANUAL 集合を突き合わせて全数列挙する census を作る。`censusManualDrift.ts` は「manual にあるのに live へ届かない」側の計器なので**向きが逆**＝流用できない）②live と fresh が**実体同一**（`equalIgnoringParseStatus`）なものは**スタンプを AUTO へ落として解凍**する＝ここが機械的に処理できる本体 ③実体が違うものだけ人が読む。■🔴**一括で AUTO へ落とさない**＝意図的な curation が混ざる（`WX17-077-E2` は「ラベル付き CHOOSE の2枝それぞれが自己トラッシュを含み `upTo:true` で『してもよい』を表す」別設計で、機械の正準形で上書きすると**設計意図が消える**）。■**副産物の見込み**＝解凍した効果には parser の改善がまとめて流れ込むので、**live 変化が大きくなる**。1バッチ15〜20効果で切り、必ず A/B 全数照合する。 |
