...
- 🏁**セッション（2026-08-24・続き645・Opus 5）＝`O-49`＝バニッシュ先変更の**アタッカー側ミラー**を配線**（**実装は Codex へ委譲**・母集団実測／指示書／検証／実機は Claude）。gates 全緑（**golden 2677→2679**／census 601 据置／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260（増分0）／同型★0／`effects_*.json` 変更0）。**実機5本が2回連続 PASS＋反転確認済み＝1巡が §2.2 の⑤まで完了。差し戻し0・是正1（後述）。**
  - ✅**O-47 が新設したアタッカー敗北／相打ち経路は行き先が既定固定**（トップ→エナ／下→トラッシュ）で、防御側が持つ**行き先変更9系統が一切効かなかった**。`myS↔opS` を入れ替えてミラー（**新型・新 STUB・新 state フィールドはゼロ**）。実害の代表＝防御側の「対戦相手のシグニが**このシグニとのバトルによって**バニッシュされる場合〜トラッシュ」（**bySource 群 10効果**）が、**アタッカーが負けたときだけエナ加速を許していた**。
  - 🔑**投入前の実測で「ミラーしてよい語彙」を確定した**（登録票が「実測しろ」と指示していた分）＝`BANISH_REDIRECT` **34効果は全件「対戦相手のシグニ」主語**＝holder は常に被バニッシュ側の対戦相手なので機械的にミラー可。**除外は4つ**＝`isShoot`（アタッカー自身のキーワード）／`opp_signi_energy_to_deck_bottom`（**live 0効果**）／`isSelectedPowerZeroBanishRedirect`（パワー0消滅専用）／身代わり ladder（別機構＝`O-58` へ）。
  - 🔴**道中で既存の視点反転バグを1件併修**＝`FROZEN_SIGNI_BANISH_TO_DECK_BOTTOM`（`WXDi-P13-071`）は原文が「**対戦相手の**凍結シグニが**このシグニとのバトルによって**〜」なのに、**被バニッシュ側自身の state から holder を探していた**＝本来の持ち主が持つと発火せず相手が持つと誤発火。あわせて `collectFrozenBanishOverrides` が `activeCondition` を一切見ていなかったのも是正（§4 の「`checkActiveCondition` は case の無い型を `return true` に落とす」）。
  - 🔑**`ON_TRASH` のアタッカー側が未配線だった**＝O-47 は `ON_BANISH`／`ON_LEAVE_FIELD` だけ張っており、トラッシュ行きを新設すると**「消えるのに誰も反応しない」穴**を新しく作るところだった。行き先計算が立てた `banishedMyWentToTrash` をトリガー収集と**共有**（条件式を二度書かない）。
  - ⭐**再発防止は件数固定ではなく集合一致で置いた**（§5-27）＝golden が**防御側／アタッカー側の語彙集合をソースから毎回再導出**して比較する。将来防御側に語彙が足されてアタッカー側が追従しなければ落ちる。
  - ⚠**Codex はシナリオを書いたが1度も実行できなかった**（環境に Chromium 未導入）＝**未実行コードのまま引き渡された**。検証側で ①5本を実行 ②**反転確認**（正方向3本が修正前 FAIL・対照2本は前後とも緑）③2回連続 PASS ④O-47/O-48 既存4本の回帰（全 PASS） ⑤シナリオのカード同定7枚を CSV 照合 まで実施。**是正1件＝BUGFIXES の実機記述が「実行は未」のままだったので実測値へ差し替え。**

**▶ 次の一手**＝**§5.2 の段2 消化に戻る**（取り方は第2層＝軸 × live の action 型 × type で5件以上のサブ群。⚠器の型 `SEQUENCE` に乗った群は信号が弱い）。候補＝`キーワード能力 × GRANT_KEYWORD × MISSING/WRONG`（17件）、`filter.story × POWER_MODIFY × MISSING`（7件）、`timing/trigger × DRAW × MISSING`（6件）。⚠**worklist の見立ては標本**＝取ったら必ず生データへ戻して数え直す（**11回連続**で外れている）。🔑**§4.3 の「受け皿はあるのに parser から配線されていない」型が6連続で当たっている**＝新しい群を取るときは、まず**その語彙の受け皿が engine に既にあるか**を先に確かめると新型ゼロで直ることが多い。🆕**`O-56`（`TRAP_OPERATION` 18効果／`TRAP_OP` 6効果）は Codex 委譲に向く**（機構が1つ・母集団が明確）。**`O-51`（並べ替え対話・279効果）は1バッチの粒度に合わない**ので、取るなら先に段取りを決める。
🆕**Codex 委譲時の必須手順**＝**シナリオを書かせても実行はさせない前提でスコープを切る**（続き645 で環境に Chromium が無く5本が未実行のまま返ってきた）。**実機実行・反転確認・回帰ベースラインは検証側（Claude）が必ず引き取る**。
...
- **2026-08-24 続き645 後（本行が直近の正）**：
  **census 601/601**、**golden 2679**、smoke **10693 / 全異常0 / SKIP 0**、fuzz 全0、**lint 0 errors（warnings 260）**、
  `census:stubs` **A群🔴0／C群0**、manual-fields **0**、**同型★ 0**、
  **live カード 5975 / 効果総数 10693**、`_held_fresh` **76**／`_partial_fresh` **15**／`_idset_fresh` **45**、
  **意味照合 残 OPEN 713**（段2消化 397・**本バッチは engine 配線のみで台帳は動かず**）、
  **実機シナリオ +5**（`o49AttackerBanishRedirectToTrash` / `o49AttackerNoRedirectGoesToEnergy` / `o49AttackerRedirectRespectsLevelFilter` / `o49AttackerRedirectRejectsLevel2` / `o49AttackerSelfExileReplacesLeave`）、version **0.502**。（⚠`census:wiring`／`census:timing`／`census:goldentypes` は本バッチ未再計測。`effects_*.json` 無変更のため `_held_fresh` 等は続き644 の値を据え置き）
