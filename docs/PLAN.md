...
**直近＝2026-09-21＝§5.7 `S-26`＝エナチャージをターンをまたいで決めるようにした（グロウできない失敗を 15 → 8）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第432バッチ）
- 🔴**登録票の向きが逆だった**（ユーザー指摘で訂正）＝**グロウしないことがかなりの悪手**で、**グロウできなくなるのは前のターンにエナを使いすぎたから**
  ＝🔑**ターンをまたいだ考えが必要**。⇒ 直す場所は GROW ではなく **ENERGY フェイズ（何をエナに置くか）**。
- 📏**②母集団**＝6デッキ × 4戦で**グロウ機会 214 のうち 15（7%）が払えない**（**色7／枚数8**）。
  **この失敗はどの計器にも映っていなかった**（盤面も勝敗も静かに悪くなるだけ）。
  もう1つ＝**ターン1でレベル1のシグニをエナへ置く**ため **MAIN で空きゾーンがあるのに出せる札が無い盤面が 22/98（22%）**。
- 🏁**直した3つ**＝①**次のグロウに要る色を確保しに行く**（`chargeGrowColor`）②**いま出せる札が足りないなら温存**（`chargeKeepPlayable`）
  ③当分出せない札の割引を数値化（`chargeFarLevelScale`＝旧 0.6 のまま）。
- 🔑🔴**「このターンのグロウ」を見ても遅い**＝失敗3件は**全部**「今回は無料／払える → 無色をチャージ → 次のターンに払えない」だった
  ⇒ **1回グロウしたあとの盤面**で見る（`chargeNeedColors`）。**判断のための近似は `cpuGrowReserve.ts` に閉じた**（`cpuTurn.ts` は実行だけ）。
- 🆕**恒久の計器**＝`census:play` の規則 `growUnpayable`（**0 が正**）。
- 📏**結果＝15 → 8（-47%・色起因は 7 → 0）**。📊**A/B（96戦）は勝率もライフ差も差なし**（-0.25 [-0.76, +0.26]）
  ＝`S-27` の結論どおり**勝率は鈍い**ので、**採否は直接指標**に置いた。旧挙動は `legacy-charge` に保存。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-28`**（**盤面のシグニをエナチャージする選択肢が CPU に無い**＝`handleEnergyChargeFromSigni` が画面にしか無い）。次に `S-22`・`S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 4**（🆕`S-28`・`S-22`・`S-24`・`S-14`＝🏁`S-26` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①🆕**盤面のシグニをチャージできない**（`S-28`）②**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）③**自己対戦がマリガンを一度も踏まない**（`S-24`）。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い＝チャージ1枚/ターンでは埋まらない |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4352**）｜**実機 PASS**（6ターン/134手/217s）｜反転確認＝`legacy-charge` プリセットで旧挙動へ戻せる |
...
| 🔥`S-28` | 🔴**盤面のシグニをエナチャージする選択肢が CPU に無い**（2026-09-21 ユーザー指摘）＝**人間は `BattleScreen.handleEnergyChargeFromSigni` で場のシグニをエナへ置ける**のに、CPU は `pickCpuEnergyChargeIndex(cpuSt.hand, …)`＝**手札しか見ていない**（一度も踏んだことが無い）。🔑**良い手になる場面**＝**自分のシグニの正面にそれより大きいシグニがいて、場に残しても邪魔なとき**＝チャージに回せば**手札を減らさずレーンを空けられる**。⚠**実装が画面にしか無い**＝`controller/` へ出さないとヘッドレス（自己対戦）でも踏めない（`S-5` の移設の型）。⤵`S-26` の第2段。⚠**母集団は未計測**＝着手前に「正面が格上で邪魔なシグニ」が何回出るかを数える。 | 中 | CPU が手札を減らさずにレーンを空けられる＝`census:play` に新しい踏破が1つ増える |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🔥**`S-28`（盤面のシグニをチャージ）**→ `S-22` → `S-24` → `S-14`（作戦データ）。
