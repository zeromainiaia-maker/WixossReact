...
- **🆕 セッション（2026-07-13・続き110・Fable 5・戦略②「純P1の系統バッチ消化」初回バッチ＝効果単位クラスタ最上位「対戦相手のアップ状態のシグニ」21効果を是正）**
  - **✅ 効果単位 census の最上位クラスタ（12＋①選択肢型9＝21 BURST効果）**＝「対戦相手のアップ状態のシグニ１体を対象とし、ターン終了時まで、それのパワーを－15000する」。真因＝`parseSentencePart1.ts` のパワー修整対象分岐が「アップ状態の」等の状態接頭辞を許容せず default `{owner:'any', フィルタ無し}` へ落ちていた＝**owner脱落（自分のシグニも対象に選べる）＋isUp脱落（ダウン状態も対象化）の二重過剰効果**。修正＝接頭辞群に アップ/ダウン/凍結状態 を追加し `parseSigniTarget`（isUp 抽出済み）へ委譲する **parser 1行**（isUp は型/engine/decompiler 全配線済み＝「既存 regex の穴」型・詳細 BUGFIXES 続き110）。
  - **採用の精密確認**＝held 新規流入が対象17枚ちょうど（git HEAD の `_held_fresh.json` と機械diff・巻き添えゼロ）→ `--adopt` 17枚＋**手修正温存4枚**（WXDi-D06-021/P09-083/P11-083/P15-075＝held に出ない）を effectId 外科パッチ＝**21/21 取りこぼしなし**（続き75「timing 是正だけは取りこぼさない」と同型の運用）。
  - **検証**＝golden 306→**307**（構造固定1件追加）・smoke/fuzz 全0・**census 2264→2243**（バッチ分ちょうど・BASELINE_HIGH 更新）・同型★0・逆翻訳原文一致。
  - **次の一手＝戦略②の継続**。入口＝`npm run census:clusters` の上位から次の系統を取る（残上位＝**「Nまで」上限選択12「その中からカードをN枚まで手札に加え…」・条件節「あなたのターンの場合」11〔⚠IS_MY_TURN プレースホルダ常時真＝engine拡張が先〕・「このターンにあなたがスペルを使用していた場合」9・公開し8「その中から＜C＞のシグニN枚を公開し手札に加え」・次相手ターン終了時まで8**）。⚠採用はカード単位（effectId→CardNum 変換）・手修正温存カードは外科パッチで取りこぼさない・`_effect_srctext.json` は build:effects の副産物（CSV/parser 変更後は再生成必須）。
### 📊 恒久指標（維持中・逐次更新）
