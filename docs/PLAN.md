...
- **🆕 セッション（2026-08-13・続き459・Opus 5）＝§6.4 **O-3**＝**`LRIG_GROW_RESTRICT` が未パース節の名無しゴミ箱になっていた**のを解体。**計器が「A群 0件」と嘘をついていた**根本原因で、隠れていた **26効果の真 no-op を worklist に出した**。併せて同族の実バグ3件を修正。golden 1954→1956・census 831 据置・held 106枚/47群 据置**。
  - **🔴① 計器が嘘をついていた根本**＝`census:stubs` は**ハンドラの有無**で「実装あり」を判定するが、`LRIG_GROW_RESTRICT` のハンドラは **`addLog` を1行出すだけの完全な no-op**。そのため A群（真 no-op）が **0件のまま緑**だった。⚠**ログを1行出すだけのハンドラはこの計器を素通りする**＝名前と実体の一致は計器の前提条件で、計器が保証してくれるものではない。
  - **🔴② しかも本来の用途でも使われていない**＝BattleScreen のグロウ色制限は `currentLrig.EffectText` を直接 regex で読む**独立実装**で、STUB はどこからも参照されていない。
  - **🔴③ 実測＝live 31効果のうち27効果がグロウ制限と無関係**。parser の9生成地点のうち**本来のグロウ制限は1つだけ**で、残り8つは「この語形は解けないので捨てる」ための受け皿だった（「このターンと次のターンの間…」「次の対戦相手のターン…」「このターン、対戦相手は…」で始まる**あらゆる文**＋個別6種）。
  - **⭐④ 直し方＝受け皿に正直な名前を付けた**＝8つを固有の `DEFERRED_*` へ分割。接頭辞の規約でゲートは緑のまま、**A群の在庫が 14種16件 → 24種42件**になり隠れていた26効果が worklist に出た。逆翻訳にも日本語で「【未実装】…の制限節」と出る。
  - **🔴⑤ 併せて直した同族の実バグ3件**＝(a)**`ARTS_AND_SPELL` は誰も読まない死 actionId**（live 5効果・封じ判定は actionId の完全一致なので `USE_ARTS`/`USE_SPELL` のどちらにも当たらない）→**2本に割った** (b)**`WXEX1-66-E1` が恒久スペルロック**（「次の**あなたの**ターンまで」を読めず `PERMANENT` へ倒れる＝一度アタックするとゲーム中ずっと相手がスペルを使えない）→期間判定を `blockUntilFromText` へ一本化 (c)`WX13-005B-E1`② の語順違い（「スペルとアーツ」）。
  - **⚠⑥ 採用判断で計器に2回止められた**＝(a)`WXDi-P16-002` を採用してしまい**続き389 のトリップワイヤが赤くなった**（id を改名しても据置の理由は変わらない）＝revert して assertion を新 id へ更新 (b)**MANUAL/PARTIAL 温存6枚**は `heldReview` に載らず、**golden の live 全走査が1枚ずつ取りこぼしを検出**した＝カード単位 PRESERVE は「直したつもり」を作るので live 全走査の契約テストを必ず置く。
  - **▶ 次の一手**＝**Opus 側**：(a)**新設した8つの `DEFERRED_*`＝26効果の在庫**（機構ごとに独立して着手できる。`DEFERRED_UNPARSED_THIS_TURN_OPP_CLAUSE` 7件が最大）(b)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵）(c)**O-4 UNKNOWN 25ノード/25カード**(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。⚠**他の「ログだけハンドラ」STUB が同じ穴を作っていないか**を一度掃くと良い（今回の教訓の一般化）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452〜459 の8件。
...
- **🆕 続き459（§6.4 O-3＝使用禁止の期間と合成 actionId）が持ち込んだ未検証UI 2件**＝engine は golden で固定したが、**封じが解けること**は実機でしか踏めない。
  - [ ] 🔴**スペル封じが次の相手ターンで解けるか**（`WXEX1-66` のシグニでアタック）＝相手のターン中はスペルを使えず、**その次（自分のターンを挟んだ後）には使える**。⚠**従来は恒久ロック**（`PERMANENT`）だったので、いつまでも使えないままなら退化していない＝**直っていない**。
  - [ ] **アーツとスペルが両方止まるか**（`PR-427` のシグニでアタック）＝合成 actionId を2本に割ったので、**どちらか片方しか止まらなければ配線漏れ**。⚠従来は**どちらも止まらなかった**（死 actionId）。
