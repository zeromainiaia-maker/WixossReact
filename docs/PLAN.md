...
**直近＝2026-09-19＝§5.7 `S-5d` 第3段＝ヘッドレスの対戦ループ＝🏁`S-5` が動いた（画面なしで CPU 同士が決着まで回る）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**`npm run selfplay`（`scripts/headlessSelfPlay.ts`）**＝1戦 ≒ **300〜430手・14〜18ターン・約35秒**。4シード試して**全戦決着**（勝者は host 席・guest 席の両方）。
- **画面にしか無かった3つを出した**＝①材料3本（`controller/battleMaterials.ts`＝カード表・効果表・有効パワー）②ルール処理8本（`controller/ruleChecks.ts`＝パワー0以下・リミット超過・リフレッシュ・クラッシュ ほか）③対戦ループ（`controller/headlessMatch.ts`）。**BattleScreen 6,987 → 6,287行**。
- 🔑**「人間側ターンの駆動」は席の鏡（`mirrorSeats`）1本**＝`cpuTurnAction` は guest 席＝`CPU_PLAYER_ID` 前提なので、**盤面を鏡にして渡し書き戻しを同じ関数で戻す**（involution）。host 用の CPU を別に書かない。
- 🔴**踏んだ罠3つ（どれも例外もゲートの赤も出ない）**＝①**`PerformCtx.userId` は相手（クライアント）の席**＝CPU 自身を渡すと**自分にターンを渡し続ける** ②**応答待ち（`cpuWaitingForHuman`）の席へ譲る**＝譲らないと空回り ③**`guest_id` は `CPU_PLAYER_ID` 固定**（違うと黙って `idle`）。
- ⚠**材料は組み直さない**＝ctx のたびに作ると**1手 120秒**（全カードの parse）＝静的な効果表は1回・盤面ごとは WeakMap・渡すカードは対戦に出る分だけ。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-9`**＝強さの A/B 測定台（**判断ロジックを触る項目は全部この後**）。🆕**2026-09-19＝§5.7 を第2版へ改訂**＝ターンを `ATTACK_ARTS_OP` で2ブロックに割って探索する設計にし、`S-8`／`S-11`／`S-12`／`S-13` を新設の `S-15`〜`S-17` へ吸収した。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**画面のリファクタ＝live JSON を1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 8**（`S-9`・`S-10`・`S-15`〜`S-18`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**「1手も進まない」型の壊れ方はゲートに映らない**（例外も不変条件違反も起きない）＝**ヘッドレスは `idle`／`cap` を必ず失敗として扱う**（`npm run selfplay` が exit 1）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4331**・`selfplay` 1戦 決着） |
...
| 🔥`S-9` | **強さの A/B 測定台**＝いまの `npm run selfplay` は**両席が同じ `cpuTurnAction`・同じ山**（`HeadlessMatchDeps` に席ごとの指定が無い）＝勝率は対称で**先攻有利と乱数しか出ない**。①席ごとのポリシー切替（変更を flag で包み A=旧／B=新）②**席を入れ替えて同じシードをもう1戦**（先攻有利の相殺）③N戦の勝率＋誤差幅。 | 小（半日） | **以降の全項目の前提**＝これが無いと「強くなった」が感想になる。`S-6` の土台でもある |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版）**＝`S-9`（測定台）→ `S-10`（評価のパワー項）→ `S-15`（候補列挙器）→ `S-16`（前半探索）→ `S-17`（後半＝攻撃探索）→ `S-18`（次ターン項）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-19 時点**（第406バッチ＝§5.7 `S-5d` 第3段＝ヘッドレスの対戦ループ）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**画面のリファクタ＝live JSON を触っていないので3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 8**（`S-6`・`S-8`＋🆕`S-9`〜`S-14`＝2026-09-19 登録）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4331**＝+3・`census:traceinv` I1=0・🆕`selfplay` 1戦 決着）
  - 📏**BattleScreen**＝**6,287行**（前回 6,987＝−700。2026-09-18 朝の 16,930 からは −10,643）
  - 📊**ヘッドレス自己対戦**＝1戦 ≒ **300〜430手・14〜18ターン・約35秒**（4シード全戦決着）
