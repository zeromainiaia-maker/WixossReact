...
- **🆕 セッション（2026-08-13・続き451・Opus 5）＝§6.4 **O-2**「`SEARCH` pending の相手応答ルーティング」を機構化。defer 3効果を解除し、同経路の潜在バグ4件を是正。golden 1934→1937・census 833→832・held 107→106枚/47群**。
  - **🔑① 何が無かったか**＝`BattleScreen` は pending の応答者を **`SELECT_TARGET` と `CHOOSE` の2型にベタ書き**で判定しており、`SEARCH` を相手へ回す経路が存在しなかった。そのため「**対戦相手は**（自分の）デッキの上からN枚を見て／公開して、その中から…」は `owner:'opponent'` を書くと「**自分が**相手のデッキを覗いて相手の場に出す札を選ぶ」別物になるため丸ごと defer されていた。
  - **⭐② 設計＝2つの軸を分離した**＝`owner`／新設 `deckOwner`＝**誰のカードか**（どのデッキを掘り／残りをどこへ送り／誰の場に出すか）、新設 `opponentResponds`＝**誰がクリックするか**。engine の ExecCtx 視点は従来どおり反転しない（続き411 の教訓）。**片方だけだと別々の間違いになる**ので parser は必ず両方を書き、golden がその組を固定する。応答者判定は新設 `pendingRespondsOpponent`（`execUtils.ts`）へ集約し、**BattleScreen のベタ書き2箇所を置換**＝以後 pending 型を増やしたときの配線漏れがここ1本に閉じる。
  - **🔴③ 併せて見つけた潜在バグ4件（いずれも live 実例0件＝この機構を開くまで到達不能だった）**＝(a)`applyDirectAction` の **`ADD_TO_HAND`/`ADD_TO_ENERGY`/`ADD_TO_BEAT` が `owner` を持つのに読んでいなかった**（`lookPickThenAction` は owner つきで組むので、相手のデッキから抜いて**自分の**手札へ入る＝カードが盤面を跨ぐ）(b)**resume 系4本が次 pending の `respondPlayerId` を一律に落とす**＝配置チェーンの2枚目以降を効果オーナーが操作する（共通ヘルパー `nextRespondPatch` へ。相手応答型でなければ空オブジェクト＝**従来挙動と厳密に同じ**）(c)`execLookPickChain` に**場出し段の空きゾーン cap が無い**＝超過ピックは「空きゾーンなし」分岐へ落ちるが**その時点でデッキから抜かれている**ので消失する（7枚公開の `WXDi-P01-026-E1` で到達）(d)`INTERNAL_SPLIT_REVEALED` の戻し先が `destOwner:'self'` 固定。
  - **⚠④ 採用経路は2通り＝`SP38-006` は held にも partial にも載らない**。`WXEX2-84`／`WXDi-P01-026` は `heldReview --adopt`（カードID単体）だが、`SP38-006` は **PARTIAL＝カード単位 PRESERVE** で `_held_fresh` にも `_partial_fresh` にも出ないため effectId アンカーの**外科パッチ**。**`parseStatus` は PARTIAL のまま残した**＝同じ効果の1つ目の内側能力が別軸で未忠実（下記）でレビュー印を落とすべきでないため。
  - **📋⑤ 残した宿題2件**＝(a)**実機UI未検証**＝相手応答モーダルの実表示（PvP の相手側／CPU 自動応答）は golden では踏めない＝§7・O-13 へ (b)**`SP38-006-E1` の1つ目の内側能力**「対戦相手の場にあるキーとシグニは能力を失い、新たに得られない」が `REMOVE_ABILITIES{SIGNI, count:1, until:'PERMANENT'}`＝**対象種別（キー＋シグニ）・体数（すべて）・期間（このターン）の3軸とも未忠実**。「新たに得られない」は継続宣言が要る＝O-4／§6.3 の家族として PLAN §6.4 O-2 行に登録済み。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-3「次のターンの間」の残り**（約16効果。⚠**型ごとに `NEXT_TURN` の受け皿があるかを先に実測**する）(b)**O-6 `WX25-P3-038-E1`**（MANUAL 不可侵＝手で MANUAL を更新するか AUTO 採用へ切り替えるかの判断。今回の `SP38-006` 外科パッチが同型の先例）(c)**O-4 UNKNOWN の残り**（代表＝`WXDi-P09-036-E1` の両者同時公開＋比較）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：**§7 実機検証**＝今回の相手応答モーダル（`WXEX2-84-E2` が最も踏みやすい＝エクシード2で相手シグニ全一掃→相手が2枚公開から場を建て直す）を1本足すのが最優先。次いで §3 (cxxiv) `WX25-CP1-091` の ON_TURN_END 切り分け。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き451（§6.4 O-2＝`SEARCH` の相手応答ルーティング）後 最新値（本行が直近の正）**：census **832**（833→832・`BASELINE_HIGH` 832）、golden **1937（+3＝応答者ルーティングの型網羅／`deckOwner` の領域分離／ゾーン選択の引き継ぎ。ほか旧 defer トリップワイヤ4件を新しい正へ反転）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（107→106）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（27/26→25/25）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（16種→14種）。live effect 単位 diff **3効果・effectId 増減 0/0**。
...
>    - **【Opus 側】(3) §6.4 の未消化 worklist**＝2026-08-12 に **`O-1`〜`O-15` の番号つき表へ整理済み**（消化済みの詳細は PLAN_DETAIL「2026-08-12 整理⑧」へ退避）。⚠**この行が名指ししていた `WX24-P3-033-E1` は続き443 で消化済み**／**`O-2` は 2026-08-13 続き451 で消化済み**（在庫は §3-1 のとおり実測してから着手する）。**次の一手の候補＝`O-3`（「次のターンの間」の残り約16効果。⚠型ごとに `NEXT_TURN` の受け皿があるかを先に実測）／`O-6`（`WX25-P3-038-E1` の MANUAL 不可侵＝続き451 の `SP38-006` 外科パッチが同型の先例）／`O-4`（UNKNOWN 25ノード／25カード）**。
