...
| (lxi) | **「対戦相手が/は〜しないかぎり」＝支払い回避クローズ**。第1〜9波で消化し、**残3**（下記。波ごとの枚数は退避先）。波ごとの実装内容・訂正した旧診断・確立した投げ方は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-01 整理」節へ退避**（一次記録は BUGFIXES 2026-07-30〜08-01 の各波節）。<br>**残3**＝①`WXK06-067-E1`（エナ＋手札の**合計N枚**という混成コスト）＝**ゾーンを跨ぐ候補プール**が engine に無い。`TargetScope` が単一ゾーン前提なので選択UIも作れない。<br>②`WX24-P4-007-E1` は**③のみ残**（①②は第9波で実働化＝0/3→2/3）。③の領域移動制限機構は在庫 **(lxxiii)**。<br>③`WXDi-P11-009-E3`（指定シグニゾーンへの配置禁止＋《無》×5 の支払い回避）＝`BLOCK_OPP_ZONE_PLACEMENT` が `disabled_signi_zones` を書くが**配置フローがこのフィールドを読んでいない**（書きっぱなしの死にフィールド）＋回避の支払いは「配置しようとした時点」で発生するので標準ペアでは表せない。<br>⚠**生きている注意点3つ**＝(a)**engine の回避手段語彙**は `costColors`／`opponentHandDiscard`(+`Filter`/`ALL`)／`opponentEnergyTrash`(+`ALL`)／`opponentSigniTrash`／`opponentSigniToDeckTop`／`opponentPayColorlessPerSigniAttack`（可変《無》）の6系統。新しい回避手段はここへ足す。(b)**遅延設置の前置きは `DELAYED_INSTALL_PREFIXES`（`effectParser.ts`）のテーブルへ足す**が、**engine に遅延収集経路がある timing だけ**（無い timing は「設置されるが永久に発火しない」＝過剰実行を no-op へ替えるだけの別の退化）。現在の経路＝`ON_TURN_END`／`ON_LEAVE_FIELD`／`ON_REFRESH`／フェイズ系／`ON_SIGNI_BANISH_BATTLE`（第7波）／`ON_ATTACK_SIGNI`（第8波・防御側設置）。(c)**「読めなかった」の顔は `UNKNOWN` だけではない**＝no-op STUB（`RULE_REMINDER_TEXT` 等）を受け入れると「過剰実行を no-op へ替えただけ」で直った顔をする。帰結の単独再パースには no-op STUB ガードを掛ける。 |
...
| (lxxiv) | **🆕「以下のNつを行う。①②③」の残り9枚は `DO_THREE_THINGS` の text パターン依存＝per-card 監査待ち（2026-08-01・第9波の実測で判明）**＝母集団10枚（`WXK11-001/002/007/008/009/010`／`WXDi-P05-052`／`WXDi-P06-050`／`WX24-P4-002`／`WX24-P4-007`）のうち第9波が開けたのは `WX24-P4-007` だけ。⚠**残り9枚は engine の text パターンが実際に動いている**＝素の分割パースへ切り替えると退化する（実測：`WX24-P4-002`「すべてのルリグとシグニをダウンし凍結」→ `FREEZE{SIGNI count:1}`＝ルリグが消える／`WXK11-002`「ライフクロス1枚を**トラッシュに置く**」→ `LIFE_CRASH`＝**バーストが誘発する**別物）。⇒ **1枚ずつ「engine パターンの実挙動 vs 分割パース」を原文照合**してから移す。無差別に SEQUENCE 化すると held が +9 して意味が落ちる（第9波で一度踏んで戻した）。既知の疑いは semantic audit にも登録済み（`WXK11-008`＝パワー12000以下の条件が落ちる／`WXK11-002`＝4項目なのに DO_THREE_THINGS）。 |
...
| (lxxii) | **🆕 `delayed_triggers` のターン終了クリアが**ターンプレイヤー側だけ**（2026-07-31・タスク12(lxi) 第8波の実装中に発見）**＝`BattleScreen.tsx:3397`（PvP通常終了）／`3779`（PvP確認後）／`9550`（CPU）はいずれも**ターンを終えた側**の state にしか `delayed_triggers: undefined` を書かず、同じブロックで作る**相手側**（次のターンプレイヤー）はクリアしない。⇒ **防御側が自分のターン外に設置した `duration:'THIS_TURN'` の遅延トリガーが1ターン持ち越される**（次に自分がターンを終えるときにようやく消える）。⚠**現時点で誤発火する実カードは確認できていない**（第8波の `WXK05-009-E2` は収集地点が防御側限定なので持ち越し中は拾われない）が、`ON_LEAVE_FIELD`／`ON_REFRESH` の収集は**両プレイヤーの `delayed_triggers` を走査する**ので防御側設置分には効きうる。⚠直すのは1行×3箇所だが **BattleScreen は golden から叩けない＝計器に一切映らない**。着手するなら「どのカードが防御側設置になりうるか」を先に列挙して回帰の当たりを付けること。 |
| (lxxi) | **🆕「このターン、次にあなたのシグニがバトルによって…バニッシュしたとき、〈X〉」＝遅延設置が落ちて即時実行になる2効果（2026-07-31・タスク12(lxi) 第7波の機械走査で発見）**＝`WX26-CP1-040-E2`（live は `SEQUENCE[POWER_MODIFY, LIFE_CRASH]`＝**無条件ダメージ**）／`WX24-P1-011-E1`（live は即時 UP＋能力消去）。第7波で `ON_SIGNI_BANISH_BATTLE` の**遅延収集経路は配線済み**なので engine の受け皿はある。⚠**残る障害は「次に」＝1回だけの消費セマンティクス**が `delayed_triggers` に無いこと（`consumeOnTrigger` は `GRANT_*` 付与能力側の語彙）。そのまま載せると「そのターン中、バニッシュのたびに発火」という**別の過剰実行**になる＝**消費機構を先に決める**（発火時に収集経路が state から当該 dt を除去する設計になるが、収集は `BattleScreen` 側＝**golden から叩けない**点に注意）。⚠`WX24-P1-011-E1` は更に「そのあなたのシグニ（＝バニッシュした側）をアップ」「そのシグニ（＝被バニッシュ側）は能力を失う」の**2種類の「その」照応**が同居する。 |
| (lxx) | **🆕「カードをN枚引くか<B>」の選択肢が黙って落ちる10効果（2026-07-31・タスク16 残0クローズの実測で発見）**＝原文「カードを１枚引くか【エナチャージ１】をする」は全23件あり、**14件は正しく `CHOOSE`（引く／エナチャージ）になっているのに、10件は `ENERGY_CHARGE_FROM_DECK` 単体に潰れて「引く」が消えている**（プレイヤーが選べるはずの選択肢を失う＝機能欠落）。⚠**原因は特定済み**＝`parseDrawOrChoice`（`effectParser.ts`）が **「とき」「そうした場合」を含む文を一律で弾く**（トリガー文との誤読回避）ため、**トリガー句/条件句が前置される形**（「…したとき、カードを1枚引くか…」「…の場合、…」）と**引用付与の内側**では本体に届かない。⇒ 直し方は2択＝(a) 各 timing のトリガー句除去を足す（`WXDi-P13-051-E3` で実証済み＝除去したら2択が復元した）(b) ガードを「`^カードを…引くか` の**アンカー前**に「とき」がある場合だけ弾く」へ緩める。**(b) なら1規則でまとまって開く可能性が高い**。⚠**着手前に全23件の live を再実測すること**（本行の10件は 2026-07-31 時点の実測値）。実IDは `WXEX1-41-E1`／`WXEX2-66-E1`／`WXEX2-66-E2`／`WXK10-006-E1`／`WXDi-D07-002-E1`／`WXDi-P02-030-E1`／`WXDi-P11-008-E1`／`WXDi-P13-061-E1`／`WXDi-P15-004-E1`／`WXDi-CP01-027-E2` |
...
- **🆕 セッション（2026-08-01・続き310・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxi) 第9波＝`WX24-P4-007-E1`（完全 no-op）の①②を実働化して 2/3 へ。⚠(lxi) 残3は**変わらない**（③のみ残＝クローズしない）**（golden **1208→1210**・census **1355 据置**（`BASELINE_HIGH=1355`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **250枚/98署名 据置**。live per-effect 差分 **changed 2 / added 0 / removed 0**／`build:effects`・`regen` 冪等）
  - **① 直したのは完全 no-op**＝「以下の３つを行う。①対戦相手は自分のシグニ１体を場からトラッシュに置かないかぎり手札を３枚捨てる。②対戦相手は手札を３枚捨てないかぎり自分のシグニ１体を選びトラッシュに置く。③〈トラッシュの領域移動制限〉」が `STUB{DO_THREE_THINGS}` 1本で、**engine 側は別カード用の text パターンでしか動かない**＝青黒2エナを払って何も起きなかった。
  - **② ①②は engine 改修0で開いた**＝②は元から読めており、①は「対戦相手**は**」形の cost パーサに `opponentSigniTrash` が無かっただけ（「が」形にある同名フィールドの水平展開）。
  - **③ 🔴 教訓＝「読めた」は「正しい」ではない**。素朴に「全項目が UNKNOWN でなければ SEQUENCE 化」したら **held が 250→260** に膨らんだ。母集団10枚のうち**9枚は DO_THREE_THINGS の text パターンが実際に動いており**、単独パースのほうが情報が少ない（`WX24-P4-002`「**すべての**ルリグとシグニをダウンし凍結」→ `FREEZE{SIGNI count:1}`＝ルリグが消える／`WXK11-002`「ライフクロス1枚を**トラッシュに置く**」→ `LIFE_CRASH`＝**バーストが誘発する**別物）。⇒ 射程を「全項目が標準ペアか本波の宣言 STUB」に絞り held を据置へ戻した。
  - **④ 教訓＝入れ子 SEQUENCE の標準ペアは continuation を実走で確かめる**。第5波が平坦化を入れた経緯があるため疑ったが、`SEQUENCE[ペア1, ペア2, STUB]` は途切れなかった＝平坦化せず **engine 実走 golden**（相手 CHOOSE が2回／①不払いで手札6→3／②不払いでシグニ2→1）で固定。
  - **⑤ ③は engine 未実装の宣言 STUB `LOCK_OPP_TRASH_MOVE` として可視化**（母集団2枚）。逆翻訳に「**（未実装）**」を明示し、census は 1355 据置＝**欠落として数え続ける**ことを実測で確認（計器を甘くしていない）。
  - **⑥ 🆕 新規登録3件**＝**(lxxiii)** ③の領域移動制限機構（`trashCandidates` 7地点＋トラッシュ直操作 STUB の全経路列挙が先）／**(lxxiv)** 「以下のNつを行う」残り9枚の per-card 監査（無差別 SEQUENCE 化は退化）／**(lxxv)** `WXDi-P14-005-E1` c2 の owner 反転＝「**対戦相手は**デッキの上から10枚トラッシュ」が**自分のデッキ**を削る自傷。
  - **次の一手**＝**Opus**：①🆕**タスク12(lxxv) owner 反転**（実害が明確・母集団を機械走査してから）②**(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く可能性が高い）③**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）④**(lxxii) 遅延クリアの片側漏れ**（1行×3箇所だが計器に映らない）⑤(lxxiii)/(lxxiv)⑥(lxviii) 散文形「対戦相手のターンの間、」⑦§6.3 J 残11 ⑧(lxvii) CPU `ON_TURN_END` 未収集 ⑨§6.4 配置数制限すり抜け7効果。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜310 の全件が実機UI未検証で積み上がっている**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-08-01 続き310（タスク12(lxi) 第9波＝`WX24-P4-007-E1` の①②／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1355 据置**（`BASELINE_HIGH=1355`＝③が宣言 STUB として**欠落に数え続けられる**ことを実測確認）、golden **1208→1210**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **250枚／署名グループ 98 据置**。live per-effect 差分 **changed 2 / added 0 / removed 0**（`WX24-P4-007-E1`／`WXDi-P14-005-E1`）、`build:effects`・`regen` 冪等。**タスク12(lxi) 残＝3 据置**（本波はクローズしない＝`WX24-P4-007-E1` は 0/3→2/3）。**新語彙0本**＝`opponentSigniTrash` は「が」形からの水平展開、`LOCK_OPP_TRASH_MOVE` は engine 未実装の宣言 STUB（STUBS.md 記載）。**engine 改修0**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1210**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1355**（回帰ゲート）・held **250枚/98署名**（2026-08-01 タスク12(lxi) 第9波後に実測）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（**現存＝(lv) 残2経路／(lxi) 残3／🆕(lxvi)〜(lxxv)**。2026-07-31〜08-01 に積んだ **(lxx)(lxxi)(lxxii)(lxxiii)(lxxiv)(lxxv)** が未着手の主力で、**(lxxv)〔`WXDi-P14-005-E1` c2 の owner 反転＝自分のデッキを削る自傷〕が実害の明確さで筆頭**。**(l) は A群/B群とも 2026-07-30 に残0クローズ、🆕(lx)／(lxii)／(lxiii)／(lxiv)／(lxv) も 2026-07-31 に残0クローズ**、**(xxix) も完全クローズ**＝残4件は明示保留、**(xlvi)／(xlii)／(lix)／(lviii) も残0**）＋**🏁タスク16 は残0クローズ**（J-3 で 15→13、第2波 13→11、第3波 11→8、第4波 8→7、原因軸 7→6、非ガード捨て札 6→5、離脱先 OR 5→4、シグニの下に置かれた 4→3、🆕**残3件の一括消化で 3→0**）（`cost.underSelfTrash` は 2026-07-30 に残0クローズ。**✅再トリアージは 2026-07-31 第1波で完了＝`[A昇格候補]` は使い切り、`[C]` 13件は §6.3 J へ送出済み**。残は collector の引数/軸不足＝engine 軽量拡張が要る群で、**軸ごとに束ねると複数枚が同時に開く**＝§3 タスク16 の行を読む）**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証）＝在庫はここだけで、Opus が機構バッチを回すたびに積み上がっている**（未検証UIの単一 worklist は **§7「残る実機検証項目」に集約済み**＝§4 進捗サマリと二重に持たない）。⚠**§6.3 H の4機構は engine/golden では固定したが UI 経路は計器に一切映っていない**＝ここが最優先。タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
