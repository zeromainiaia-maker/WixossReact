...
- **🆕 セッション（2026-07-31・続き302・Opus 5〔**Claude が実装まで担当**〕）＝codex のトークン枯渇によりユーザー指示で分担を切替。タスク12(lxix)＝「あなたのパワーN以上のシグニがアタックしたとき」の2効果を是正**（golden **1174→1181**・census **1363→1361**（`BASELINE_HIGH=1361`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・held **251枚/98署名 据置**。live per-effect 差分 **changed 2 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **21 据置**＝元から timing 有り）
  - **① 対象は安全停止ではなく「両方向に誤っている live バグ」**＝`WXDi-P02-079-E2`／`WXK07-030-E2` は `timing` こそ付いているが `triggerScope:'self'`／`triggerFilter:null` で、①能力保持シグニ自身がアタックすれば**パワー不問で発火**（過剰）②**他の適格シグニのアタックでは発火しない**（過小）。⚠**timing が付いているので `census:timing` には出ない＝計器の外**にあった（第4波の実測中に発見）。
  - **② 決め手＝`ON_ATTACK_SIGNI` は収集経路が2本ある**。`BattleScreen.tsx:7141`（アタッカー自身の直収集）は**`triggerScope` も `triggerFilter` も一切見ずに全部拾う**／`collectFieldTriggers`（`triggerCollect.ts:2798`）は any_ally を拾うが**自身は除外**（`topNum === triggeringCardNum` で continue）。⇒**`any_ally` に直すだけでは自分のアタック時に条件が素通りする**ため、①parser（主語＋`powerRange`）②`collectFieldTriggers` に実効パワーを渡す ③**直収集経路に `triggerFilter` 評価を新設**、の3点を一体で実装した。
  - **③ 波及範囲は投入前に全数実測してゼロ**＝場イベント系で `triggerFilter` 持ちは87効果あるが **`powerRange` 持ちは `WX14-025-E1` の1件だけ**（原文「パワーが8000以下の場合」＝現在パワーなので実効値を渡すのは**是正方向**。未登録なら表記値へフォールバックし回帰しない）／`ON_ATTACK_SIGNI` の `triggerFilter` 持ちは **`WX01-029-E1`（色:赤）の1件だけ**で自身が赤なので通る（golden で巻き添え無しを固定）。
  - **④ 第4波の教訓がそのまま効いた**＝**実効パワー Map のキーは生値**（`calcFieldPowers` は場のスタック頂点をキーにする）。一方 `cardMap`/`battleCardMap` は **`InstanceMap`**（`battleUtils.ts:50`）で `#N` を自動フォールバックするので丸めても安全＝**2つの Map で正規化の要否が逆**。両方の行にコメントで固定した。⚠当初「`collectFieldTriggers` の `cardMap.get(triggeringCardNum)` に `getCardNum()` が無いのはバグでは」と疑ったが、**`InstanceMap` を読んで誤りと確認**して登録を取り下げた（台帳に偽の在庫を積まずに済んだ）。
  - **⑤ golden は述語まで**＝`BattleScreen` の直収集経路は golden から叩けないので、他の味方経路は `collectFieldTriggers` を直接叩き（実効15000で発火／14999で非発火／**実効パワー不明なら表記3000へフォールバックして非発火**＝実効値を見ている証明）、自己アタック経路は同じ述語（`matchesFilter(card, tf, power)`）を固定した。⚠**実機UI は未検証**。
  - **次の一手**＝**Opus：タスク16 残＝`[B維持]` 7件**（本件はタスク16 の枠外だったので残数は不変）。①`ON_HAND_DISCARDED` 系3件（`WXDi-D09-P16-E2`／`WX24-P2-051-E1`／`WXDi-P13-051-E3`＝**機構3種混在＋`collectHandDiscardTriggers` の呼び出し元7箇所**なので**1件ずつ**）②`WXDi-P06-038-E1`（エナ→任意宛先＋原因）③`WX05-020-E1`（ターン累積クラッシュ枚数）④`WXDi-P11-063-E2`（シグニの下に置かれた）⑤`WXDi-CP02-068-E1`。⚠**7件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**🆕 計器の外にある系統に注目する価値が高い**＝(lxix) は `census:timing` に出ない live バグで、同型（timing は正しいが主語/条件が落ちている）が他にもある可能性。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜302 の全件が実機UI未検証で積み上がっている**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き302（タスク12(lxix)＝アタック主体のパワー閾値・2効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1363→1361**（`BASELINE_HIGH=1361`＝主語と閾値が丸ごと落ちて両方向に誤っていた2効果の**機能是正**）、golden **1174→1181**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251枚／署名グループ 98 据置**。live per-effect 差分 **changed 2 / added 0 / removed 0**（`WXDi-P02-079-E2`／`WXK07-030-E2`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 21 据置**（⚠**この2件は元から timing を持つ＝計器の外にあった live バグ**）。**タスク16 残＝`[B維持]` 7 据置**（本件は枠外）。**新語彙0本**＝既存 `triggerFilter.powerRange` と `matchesFilter` の既存第3引数のみ。engine 変更は**収集2経路への配線**（`collectFieldTriggers` に実効パワーを渡す／`BattleScreen.tsx:7141` の直収集経路に `triggerFilter` 評価を新設）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1181**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1361**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク12(lxix) 後に実測）。
>
