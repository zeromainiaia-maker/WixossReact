...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377c＝(i)配線ギャップ 第8バッチ後）**＝golden **1443**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1048**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **211枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 422**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第9バッチ。⚠まず「その語彙は本当に未配線か（別の綴りで載っていないか）」を確かめる。**
>    - **① `cardClass` 116（has 1445）／`levelRange` 79（249）／`levelExact` 29（103）／`color` 31（302）の大クラスタ**＝★★セルは `cardClass × SIGNI[filter]` ほか。⚠**条件節用法の誤検出が濃い**ので `--cell` で全数分類してから取る。
>    - **② `isDisona` 24（has 31）** ＝続き376d で判定器（`isDisona`）は直したが、まだ載っていない入口が残る。
>    - **③ `isPuppet` 10／`isAwakened` 10／`levelParity` 8／`excludeResona` 6／`nonColorless` 5** ＝小粒だが has があるので穴は明確。
>    - ⚠**着手前に必ず読む8つの罠**＝(a) **素の `parseStoryFilter(文全体)` を対象フィルタに使わない**＝**対象名詞句に隣接**するときだけ拾う。(b) **曖昧なら付けない**（別ピック2本の span に AND で載せると原文と逆の過小実行）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**（live diff だけ見ると PRESERVE 分を見落とす）。(d) **exec 側の配線を必ず確認する**（`matchesFilter` は `excludeSelf`/`upToCount` を見ない）。(e) **セルは入口であって終点ではない**＝ビルダー全体を読む。(f) **枚数・値だけ先に直さない**＝構造が誤ったままだと誤りを増幅する。(g) 🆕**★★セルでも中身を読むまで収穫は分からない**（`levelRange × BANISH{SIGNI}` は miss 6 のうち5件が偽陽性）。(h) 🆕**同じ概念に2つのキー綴りが併存していないか先に確かめる**（`hasCrossIcon` vs `hasIcon:'クロス'`＝engine は両方見る）＝**較正で済むなら実装ゼロ**。
>    - **✅消化済み**＝「他の」ゲート棚卸し（続き377）／`noGuard` セル（続き377b）／アイコン系（続き377c）。
>    - **(iv) 計器較正の残**＝**「キー名は違うが engine では等価」型がまだ他にもありうる**（続き377b の `hasGuard:false`≡`noGuard:true`、続き377c の `hasIcon`）＝`censusWiring.ts` の `jsonRe` で個別に潰す。**較正は実装ゼロで miss が減るので、セルを取る前に必ず確認する。**
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **4件**／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**（IDは §5d-0 (ii)）。🆕**「場に《ライズアイコン》を持つシグニがN体ある場合／あるかぎり」の条件型が無い 5件**（`WX15-068-E2`／`WX17-053-E2`／`WX18-030-E1`／`WX18-030-E2`／`WX20-024-E1`）。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない。
>    - ⚠**見積もりの現在地**＝census 1048（回帰ゲート）／被覆マトリクス miss **422**。**続き377c は census が1も動かないまま17効果を直した**＝**census の増減だけで成果を測らない**（語彙が census に無い系統は被覆マトリクスにしか映らない）。3バッチの実績＝12／30／17効果。
