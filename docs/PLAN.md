...
**直近＝2026-09-17＝第395バッチ：§5.6 `C-9` 続き＝2回目のリフレッシュ（`R-28`）とキーの枠（`R-46`）を規則どおりに直す**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**「2回目のリフレッシュでターンを終了」が効果スタック解決の1経路にしか無かった**（数える側は全経路が通る1本で正しかった）＝スペル解決・選択の再開・ドローフェイズでは終わらず、**無限ループの止め金が抜けていた** ⇒ 判定を `refreshTurnEnd.ts` の1本にし、ルール処理 funnel を受け皿に（強制終了の経路 2→3）。
- 🔴**キーを場から取り除く4経路のうち3つが枠を見分けていなかった**＝増設枠（`key_piece_extra`）のキーを使うと**メイン枠のキーが消滅**・1箇所は全消しで**増設枠のキーが消滅** ⇒ `keyZone.ts` `removeKeyToLrigTrash` の1本へ。
- `R-29`（マリガン）は一致を確認して 👀。🔴`R-44`/`R-48`（リミット超過のルール処理が**そもそも無い**／`reduceFieldSigniToLimit` は死にコード）は持ち主の選択 UI が要るので **`O-532`** へ登録。
- 実機 新規2本（`V-253`／反転 `V-254`）PASS＋しきい値を3にすると FAIL を確認。**通し対戦 CPU も PASS**（新 funnel が通常進行を壊さないことの確認）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.6 `C-9` の続き**（[RULES.md](./RULES.md) の ⚠9行＝次は `R-47`／`R-49`／`R-50`／`R-51`／`R-52`）／**`O-532`**（リミット超過のルール処理）／**`C-8`**（対話応答の pure 化）。❓`R-27`（追加ターン予約中の強制終了）と `R-45b`（レゾナクラフトの離場）は**ユーザー判断待ち** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は engine・UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **2**（索引B `O-532`／索引G `O-531`）／実機 **0**／実装キュー **0**／**CPU 完成度 2**（§5.6 `C-8`・`C-9`）／ルール台帳 ⚠9 |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4288）／通し対戦 `verifyFullMatch cpu` PASS |
---
...
**残0**（直近＝`V-253`＝`c9refreshturnend`／`V-254`＝`c9refreshturnendone`（反転＝1回目では終わらない）＝2026-09-17 第395バッチで PASS＋修正前のしきい値で FAIL を確認。2回目のリフレッシュでターンが終わる回帰ガード／`V-251`＝`c9resonabanish`／`V-252`＝`c9lrigtriplecrush`＝2026-09-17 第394バッチで PASS＋**修正前のコードで FAIL を確認**（レゾナがエナへ／トリプルが1枚）。レゾナの行き先とルリグの【トリプルクラッシュ】の回帰ガード／`V-248`〜`V-250`＝`c7cpukey`／`c7cpupiece`／`c7cpupieceonelrig`（反転＝ルリグ1体では使わない）＝2026-09-17 第393バッチで PASS。CPU のキー・ピースとピースの体数ルールの回帰ガード／`V-247`＝2026-09-17 第391バッチでクローズ＝CPU の起動の停止・二重実行。回帰ガード＝`v247AfterCpuRiseNoTrigger`（判別力あり）／`v247AfterCpuRise`／`v247AfterCpuAssistGrow`／`c3cpufirstturngrow`／`V-242`〜`V-246`＝`c2cpuguard`／`c4cpuhandlimit`／`c5cpuassistgrow`／`c5cpuresona`／`c6cpurise`＝2026-09-17 第390バッチで PASS。CPU のガード・手札上限・アシストグロウ・レゾナ・ライズの回帰ガード／`V-240`＝`c9lancerreplaced`／`V-241`＝`c9extraturnup`＝2026-09-17 第389バッチで PASS＋修正前のコードで FAIL を確認。ランサー置換とアップフェイズの受け手の回帰ガード／`V-239`＝`battleequalpower`＝2026-09-17 第388バッチで PASS。同値バトルの回帰ガード／`V-238`＝`bugreport`＝報告導線の回帰ガード／`V-237`＝`distinctlevelshortpick`＝`O-530` の実機ソフトロック回帰ガード）。
...
| ID | 規模 | 何が無いか（一行） |
|---|---|---|
| `O-532` | M | **リミット／レベル超過のルール処理が無い**＝リミットは配置時のゲートだけで、後から下がっても場のシグニが減らない（RULES.md `R-44`／`R-48`・リミットを変える効果9カード＋アシスト離場・持ち主が選ぶ UI が要る） |

#### 索引 G. 母集団 1〜2効果（速いレーンが既定）

| ID | 規模 | 何が無いか（一行） |
|---|---|---|
| `O-531` | S | バトルのライズ置換が**下のカードを全部**トラッシュし、**任意**も問わない（原文は2枚・〜してもよい／1枚・強制）＝STUB に枚数 payload が無い（2効果） |
...
| 🔥`C-9` | **ルール規則の棚卸し**（バトル／ダメージ／フェイズ／ゾーン移動の行き先） | M〜L | **台帳＝[RULES.md](./RULES.md)**（公式ルールの1文 × 実装箇所 × 状態）。**2026-09-17 着手**＝✅1／🔴→✅8（ランサー置換・アップフェイズの受け手・ライズ置換の後始末・ピースの体数・レゾナの行き先・ルリグの【トリプルクラッシュ】・**2回目のリフレッシュ**・**キーの枠**）／👀16／**⚠9**。取り方は RULES.md §4（⚠を上から・写経を全部数える・純関数＋golden） |
