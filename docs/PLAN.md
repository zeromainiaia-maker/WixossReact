...
- **🆕 セッション（2026-08-15・続き497・Opus 5）＝§6.4 **O-3「次のターンの間」系統の受け皿2種を解体**（遅延予約機構の新設＋`WDK06-R09` の是正＝**2効果**）。ゲート全緑（**golden 2050**＝+6・**census 830 据置**・smoke 10688 / SKIP 0・lint 0/260）。live changed **2効果/2カード**。`census:stubs` A群 **23種/26件 → 22種/24件**。
  - **🔑① 「次の対戦相手のターン終了時、〜」の遅延予約（`DELAY_TO_NEXT_OPP_TURN_END`）**＝続き493 は機構が無く明示 defer で即時実行を止めるところまでだった。**続き489 の `DELAY_TO_NEXT_OPP_ATTACK_PHASE` と同型**＝予約は**予約した側**（そのターンの非ターンプレイヤー）に積み、`ON_TURN_END` の collector が **`opState` 側**を読む。🔑**`myState` を読むと自分のターン終了時に誤発火する**（golden に軸のトリップワイヤ＝裏返すと FAIL）。⚠ターン境界を跨ぐので `turnScopedState` にも `delayed_triggers` にも載せない。
  - **🔴② 遅延を跨いだ照応は予約できない**＝`WXDi-P09-066-E1` の本文「**その**カードを手札に加える」（＝裏向きでルリグゾーンに置いたカード）は単独文として読むと `TRANSFER_TO_HAND{DECK_CARD}`＝**デッキから引く**へ化ける。`^(その|それ)` の本文は**受け皿のまま残す**（過少側）。⇒ 格上げできたのは `WXDi-P16-002-E1` だけ。
  - **🔴③ `WDK06-R09-E1` は相手のターン終了時に自分のシグニを1体タダでバニッシュしていた**（実測）＝受け皿 STUB が no-op のまま execSequence の任意コスト Pattern が pay/skip を出し、pay で `BANISH{owner:'self'}` が走っていた（対象・コスト・所有者の3点とも誤り）。新設語彙は2つだけ＝**`TargetFilter.attackedThisTurn`**（判定は state を持つ `fieldCandidates` の層。`attacked_signi_ids` は**アタックした側の state** に積まれるので候補を作る state と同じ側で読める）と **`OptionalCostSpec.trashOwnKey`**（⚠**キーゾーンはシグニゾーンと別**＝既存 `fieldToLrigTrash` では払えない。支払いは既存 `INTERNAL_TRASH_OWN_KEY` を再利用）。
  - **⚠④ 後続文が二重実行になる**＝「そうした場合、それらをバニッシュする。」は別文なので素の `CONDITIONAL{IS_MY_TURN, then: BANISH{owner:'self'}}` が後ろに残る。`dropRedundantStepAfterTargetOppSigniOnly` に落とす規則を追加（golden で `BANISH` がちょうど1回であることを固定）。⚠**カード単位 PRESERVE で held に載らない**（同居の `E2b` が MANUAL）＝**effectId アンカーの外科パッチ**で live を訂正（fresh parser の出力をそのまま書き戻す）。
  - **🔑⑤ 逆翻訳にコストと絞り込みを出す**＝`attackedThisTurn`／`trashOwnKey` を足さないと「コストを支払ってもよい」に潰れて**原文照合でコスト取り違えを見つけられない**。
  - **▶ 次の一手【Opus 側】**＝**O-3 の残6種**＝`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_DECLARED_ARTS_NAME_LOCK`（`WXEX2-09`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）／`DECLARED_SPELL_NAME_LOCK`（`PR-K046`）／`GAIN_OPP_LRIG_TYPE`（`WDK17-008`）／`FIELD_SIGNI_TO_CHECK_ZONE`（`WX22-010`）＋`NEXT_OPP_TURN_END_BODY` 残1（照応問題）。⚠`PR-K046` は**スペル名の宣言UI**（手札からではなく任意のカード名）＋`blocked_card_names` の**次の相手ターン**への期間延長が要る＝既存 `DECLARE_CARD_NAME` は流用できない。→ **O-32**（`REPEAT_N_TIMES` の二重実行3効果）／**O-33**（アタック禁止のゾーン限定・次の相手ターン軸）／**O-28 の残23綴り** → **O-4**（UNKNOWN）→ **O-25 の残** → **O-26** → **O-27** → **O-29** → **O-31 の残2**。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き497 の3経路**＝(a)`WXDi-P16-002-E1`（使った瞬間には引かず、**次の相手ターンが終わるとき**に1枚引き＋エナチャージ1＝負方向＋対照の対）(b)`WDK06-R09-E1`（相手のターン終了時に**相手のアタック済みシグニだけ**が候補に出る／キーを払えばバニッシュ・払わなければ何も起きない／**自分のシグニは一切減らない**）(c)キーが無い盤面で支払い枝が出ないこと。ほか続き488〜496 の未検証分。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き497（§6.4 O-3 の受け皿2種＝遅延予約機構＋`WDK06-R09`）後 最新値（本行が直近の正）**：census **830 据置**（較正なし）、**golden 2050**（+6＝予約の非即時実行1・予約の取り出し実行1・collector の走査軸1・`attackedThisTurn` 1・`trashOwnKey` 1・live データ不変条件1。ほかに続き493 の「受け皿へ落とす」assert を**予約へ格上げされた側**の正方向へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 45群**（据置）、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**22種/24件**（23種/26件から −1種/−2件＝`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 残0／`NEXT_OPP_TURN_END_BODY` 2件→1件。**無言 no-op は 0 のまま**）。🆕**live JSON changed 2効果/2カード**（`WXDi-P16-002`／`WDK06-R09`。CSV 非改変）。🆕**挙動是正 2**（遅延本体の不発1／自分のシグニをタダでバニッシュ1）。🆕**新機構＝`DELAY_TO_NEXT_OPP_TURN_END`＋`pending_next_opp_turn_end_effects`＋`RESOLVE_NEXT_OPP_TURN_END_EFFECT`（続き489 のアタックフェイズ版と同型）／`TargetFilter.attackedThisTurn`／`OptionalCostSpec.trashOwnKey`**。⚠**3経路とも実機未検証**（§7 送り）。⚠**残した近似**＝遅延を跨いだ照応（「そのカード」）は予約が参照先を束縛しないので受け皿のまま（`WXDi-P09-066-E1`）。
...
**■ 消化済み**
> ✅**O-3 の受け皿2種＝2026-08-15 続き497 で解体**（O-3 自体は継続＝残6種＋照応1）。**挙動是正 2**（遅延本体の不発1／自分のシグニをタダでバニッシュ1）。
