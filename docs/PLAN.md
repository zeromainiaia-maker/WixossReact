...
- **🆕 セッション（2026-07-31・続き303・Opus 5〔**Claude が実装まで担当**・codex トークン枯渇のため継続〕）＝タスク16 `[B維持]` から `ON_HAND_DISCARDED` 系を1件（`WXDi-D09-P16-E2`「あなたが自分の効果によって捨てたとき」）**（golden **1181→1184**・census **1361 据置**（`BASELINE_HIGH=1361`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・held **251枚/98署名 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **21→20**）
  - **① 3件のうち機構が単独で完結する1件を選んだ**＝`WXDi-D09-P16-E2` は**原因オーナー軸だけ**。残る2件は別機構が乗る（`WX24-P2-051-E1`＝原因＋《ガードアイコン》filter＋「そのカード」の同一性／`WXDi-P13-051-E3`＝手札とエナの2経路 OR＋解決単位の重複排除）＝**別件**。本体（`TRASH{HAND_CARD,opponent,1,blind}`）は既に原文どおりで、足りないのは timing と原因軸だけだった。
  - **② 決め手＝engine に既に「誰の効果が原因か」を表せる構造があった**。1回の効果解決では `ctx.ownerState`＝効果の持ち主／`ctx.otherState`＝その相手で**固定**なので、**「どちらの state へ `hand_discarded_just` を書いたか」がそのまま原因の所在**（既存 `hand_trashed_by_opp_this_turn` が同じ規約）。**同じ state が1解決で両方を受けることは構造上ありえない**ので **state ごとの boolean で厳密に表せる**＝`PlayerState.hand_discarded_just_by_opp` を新設し、`hand_discarded_just` と同じ5地点で立て・同じ2地点でクリアした。
  - **③ 呼び出し元7箇所のうち引数を渡したのは watcher の2箇所だけ**。残る5箇所はコスト支払い or 自分起因なので既定 `false` で正しい＝**「7箇所あるから重い」という当初の見立ては外れ**、実際に触るのは2箇所だった（**呼び出し元の数ではなく、原因が既知かどうかで数えるべきだった**）。
  - **④ `byOwnEffect` の判定は `!asCost && !byOppEffect`**＝**コスト支払いの手札捨ては「効果によって」ではない**ので除外／ターン終了時の手札上限処理はそもそも `hand_discarded_just` を立てない。⚠**相手フィールド側の watcher ループでは `byOwnEffect` の意味が確定しない**（watcher の持ち主が discarder ではない）ため、実データ0件を確認のうえ**保守的に非発火**にした。
  - **⑤ 波及ゼロを投入前に実測**＝`ON_HAND_DISCARDED` は live 31効果あるが **`byOwnEffect` 持ちは0件**（`triggerCondition` 持ち4件は全部 `turnOwner` のみ）。golden で「未宣言の既存効果（`WXEX2-12-E2`）は原因に関わらず従来どおり発火」も固定。
  - **⑥ ⚠自分で踏んだ穴＝逆翻訳から原因が落ちた**。初版は「ガードステップ以外であなたが手札を捨てたとき」と出て `byOwnEffect` が消えていた（**新フィールドが逆翻訳から落ちる**＝codex に何度も指摘してきた既知の失敗モードを自分で踏んだ）。`decompileEffects.ts` に反映して是正。
  - **次の一手**＝**Opus：タスク16 残＝`[B維持]` 6件**（本件で 7→6）。①`WX24-P2-051-E1`（原因＝コストか効果＋《ガードアイコン》を持たない filter＋「**そのカード**」をトラッシュからエナへ＝**捨てたカードの同一性**が要る）②`WXDi-P13-051-E3`（`byOpponentEffect`＋**手札捨てとエナ→トラッシュの2経路 OR**＋「効果1つによって」の解決単位重複排除）③`WXDi-P06-038-E1`（エナ→任意宛先＋原因）④`WX05-020-E1`（ターン累積クラッシュ枚数）⑤`WXDi-P11-063-E2`（シグニの下に置かれた）⑥`WXDi-CP02-068-E1`。**🟢 ①は本件の `byOppEffect` 引数と `asCost` がそのまま使える**（残るは filter と同一性）＝次に投げやすい。⚠**6件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**その他**＝タスク12(lxviii) 散文形「対戦相手のターンの間、」／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜303 の全件が実機UI未検証で積み上がっている**。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き303（タスク16＝`ON_HAND_DISCARDED` の原因オーナー軸・1効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1361 据置**（`BASELINE_HIGH=1361`）、golden **1181→1184**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251枚／署名グループ 98 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WXDi-D09-P16-E2`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 21→20 効果/20クラスタ**（⚠**実質18**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 7→6**。**新語彙1本**＝`PlayerState.hand_discarded_just_by_opp`（boolean・`hand_discarded_just` と同じ5地点で立て同じ2地点でクリア）＋`collectHandDiscardTriggers` の `byOppEffect` 引数。`triggerCondition.byOwnEffect` は**既存フィールドの水平展開**（ON_TRASH/ON_LEAVE_FIELD に続き ON_HAND_DISCARDED でも honor）。**新 timing・新 action 型はゼロ**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1184**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1361**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 `ON_HAND_DISCARDED` 後に実測）。
>
