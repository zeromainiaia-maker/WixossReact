...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-08 続き379＝`isDisona` 条件節グループ8効果）**＝golden **1531**・smoke **10686** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **919**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **145枚／71群**・実機 driver 既定order **128件**。**被覆マトリクス miss 283**（`npm run census:wiring`）。**混在カードのレビュー待ち 3カード**（`docs/_partial_fresh.json`・すべて機構ギャップ）。
>
...
>
> 0. 🎯**次の一手＝(ii) 機構ギャップの安い在庫＝`parseStatus:UNKNOWN` の完全no-op 6件／`PARTIAL` 23件**（理由が `docs/_partial_report.txt` に出ている＝調査コストが低い）。次点＝**2ゾーン合算の枚数条件**（`WXDi-P12-056-E1` 1件だが `combinedTrash` の先例があり型を1つ足すだけ）。
...
> このセクションは機構台帳＝**現存の残作業は下記 C／E／F／G／🆕K＋J（J-1/J-4）に集約**（J-2 は 2026-08-08 続き380・J-5 は続き381 で消化）。消化済み機構の実装詳細（フラグ・ファイル・commit）は BUGFIXES 各日付が一次記録（各機構が日付/commit を明記済み）。完了行の原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-02 整理②」節。
...
- **🆕 K. `manualEffects.ts` ↔ live JSON の乖離 105/427 カード（2026-08-08 続き381 実測・新規登録）**＝**`build:effects` は live 側 `parseStatus:MANUAL` を不可侵にするので、`manualEffects.ts` を後から直しても live には永久に届かない**（`buildEffectsJson.ts:187`「手修正は不可侵」＝**JSON 直編集**の保護が意図だが、`mergeManualEffects` の出力も MANUAL なので新しい manual エントリも同じ網に掛かる）。`_held_review` にも `_partial_fresh` にも出ない**第3の死角**＝どのゲートも計器も検出しない。実例＝夢限-Q-（`WXDi-P11-010A`/`010B`）は stub・collector・golden・実機シナリオまで揃っていたのに live だけが古く、**機構が丸ごと死んでいた**（続き381 で復旧）。⚠**一括同期は不可（双方向の乖離）**＝`PR-426` のように **live のほうが新しい**ケースがある（タスク12(cxv) の E3 は live にだけ在り、`manualEffects.ts` には無い）。**1カードずつ「どちらが新しいか」を原文照合して決める**。⚠計測は使い捨てスクリプトで再現できる（`MANUAL_EFFECTS` の全 key について `mergeManualEffects(parseCardEffects(card))` と live を canon 比較）。**着手時は「manual が新しい」側から取る**＝夢限と同型の「実装済みなのに死んでいる機構」が他にも埋まっている可能性が高い。
...
- **J. timing collector 不在の13効果（2026-07-31・タスク16 の `[C]` から正式送り）＝🆕 J-3・J-2・J-5 消化により実残 4効果**＝原文が【自】なのに**そのイベントを検出する collector が engine に存在しない**群。現在は `timing:[]` で安全停止中＝**放置しても過剰実行は起きない**（着手優先度は低いが、機構台帳としてはここが定位置）。判定根拠は `docs/_timing_census_triage.txt`「2026-07-31 [B]群の停止理由 機械再検証」節に `ファイル:行` つき。**5家族に束ねると1家族＝1バッチで複数枚が同時に開く**：
