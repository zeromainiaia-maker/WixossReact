...
| 🆕(xxxix) | **逆翻訳全文照合で検出した「条件以外の原文不一致」計24効果（続き210/211/212・Codex。各件の不一致内容は BUGFIXES 続き210-212 に明記）**。**続き213（Opus）で全23件を実測＝同型クラスタが無く Codex バッチ化しない判断**（唯一まとまる「このアタックを無効にし」系3枚は攻撃無効化 action 型が engine に無い §6.3級）。Claude 側での個別対応か §6.3 送りが妥当 |
...
| 1 | **§7 実機検証の横展開** | 検証（driver シナリオ追加のみ） | S×件数 | **✅(a)(b)(c)＋`oppDrawOwnEffectOnly`＋続き173/174 復活分2件は消化済み＝既定order 75件**（経緯の全文は PLAN_DETAIL §3）。残＝§7 の未消化項目（(xi) skip検証・(xxxvi) グロウ支払いUI・ON_LRIG_GROW④ ゲット・グロウ経路）＋WX22-001-E3（§6.4） |
...
- **🆕 セッション（2026-07-20・続き218h・Opus・**§3 タスク12(xxix)③＝「そうした場合」プレースホルダの did-it ゲートを engine に新設。155効果152カードの過剰発火を系統解消**。golden 510→513）**
  - **真因**＝parser は「そうした場合、」を**常時 true の `CONDITIONAL{IS_MY_TURN}`** で表す慣例エンコード（§9-9）。engine が「前段が実際に何かをしたか」を判定しないと**前段が空振り（対象なし）でも本体が実行される**。実測すると `TRASH`(自分)/`DOWN`(自分) だけが既存ゲートを持ち、**`BANISH`/`BOUNCE`/`DOWN`(相手)/`FREEZE`/`TRANSFER_TO_DECK`/`TRANSFER_TO_HAND`/`SEND_TO_ENERGY`/`LIFE_CRASH` は全滅**だった（登録時の「盲点2件」は過小評価＝**単カードではなく engine の構造欠落**）。
  - **⚠計器の教訓（再利用可）**＝最初に書いた「空振り盤面で then がスキップされるか」だけの**片側計器は全型 PASS に見えたが、観測値が伝播しておらず何も測っていない空計器**だった。**対照群（成功盤面では発火すべき）を足した両側計器**に作り直して初めて実態（8型全滅）が出た。**この種のゲートは必ず両側で測る**（片側だと「全部抑制する」退化が満点に見える）。同様に `sed`/`python` ヒアドキュメント経由の一時編集が**無言で不発**していた事故もあり、検証は必ず「修正を外すとテストが落ちる」ことの実測まで行う。
  - **修正（2点・独立に効く）**＝(a)`execUtils.selectOrInteract` の候補0件を `done({...ctx, lastProcessedCards: []})` に（従来は残留値を持ち越し、直前の成功ステップの値でゲートがすり抜けていた）。(b)`execSequence` に汎用 did-it ゲートを新設＝既存の粗い `TRASH`/`DOWN` 2本（残りSEQUENCEを丸ごと捨てる）は挙動温存し、後段で**プレースホルダ1ステップだけを消費**（`else` は実行・以降の独立ステップは残す＝過剰も過小も作らない）。対象は `DID_IT_GATED_TYPES` の allowlist＝**`lastProcessedCards` に記録する型だけ**（`DRAW` 等の常成功型を入れると過小実行に化けるため入れない）。
  - **影響**＝新たにゲートが掛かるサイト **155件 / 152カード**（BANISH 52・TRANSFER_TO_DECK 36・DOWN 34・BOUNCE 14・LIFE_CRASH 12・TRANSFER_TO_HAND 5・EXILE 1・SEND_TO_ENERGY 1。うち LIFE_BURST 5）。
  - **検証**＝全ゲート緑（typecheck・**golden 510→513**・smoke CRASH等0・fuzz 0・census 1880維持・lint 0 errors）。追加 golden 3件は**修正を外すと落ちることを実測確認**（`expected=3 got=5` 等）＝空テストでないことを担保。(a)(b) 個別に無効化して対応テストが落ちることも確認。詳細 BUGFIXES 続き218h。
  - **🆕同セッション後半（続き218i）＝§3 タスク5「GRANT_LRIG_ABILITY 系 ON_PLAY 誤デフォルト」を精査したら実体は別物**＝**内側能力の `triggerScope` 脱落**だった（外側 ON_PLAY は【出】＝アシストルリグ登場時で妥当）。相手アタック検出 regex（effectParser 4364）が「対戦相手の…**シグニ**がアタックしたとき」だけを見ており、**複合主語「シグニかルリグ」「ルリグかシグニ」等を取りこぼし** scope 未設定に。内側能力は `collectFieldTriggers`(2429) が **any_opp/any しか収集しない**ため、未設定＝既定 `self` は**どのイベントでも拾われない＝完全な no-op**＝「相手のアタックを無効にする」「相手デッキ10枚ミル」等の**防御能力が丸ごと死んでいた**（誤発火ではなく死亡）。原文センサスで取りこぼし11ヒット/11カード（うち内側付与4・残7はアーツ/スペルの使用タイミングで無害）を確定し、regex 拡張で**3効果是正**（`WXDi-D06-010`／`WX24-P2-046`／`WXDi-P09-036`）。**`WX15-002-E2`（センタールリグ単独）は意図的に据置**＝engine に相手ルリグアタックの収集経路が無く、拾うと相手シグニのアタックで誤発火する過剰効果を新設するため（→**タスク12(xlvii) に新規登録**）。「かルリグ」半分は `markSilentFallback`＋PARTIAL で計器化。採用は held の74枚大グループに混在していたため**個別 `--adopt`**＋curated→fresh 全 leaf 差分で「fresh が失う行ゼロ」を機械確認。全ゲート緑（**golden 513→514**・census 1880維持）。追加 golden は regex を戻すと落ちることを実測確認済み。詳細 BUGFIXES 続き218i。
  - **🆕同セッション後半その2（続き218j）＝前半で自ら登録した (xlvii) を同セッションで機構ごと消化**。「対戦相手のルリグがアタックしたとき」に**防御側**の付与AUTO を拾う経路が engine に無く（`ON_ATTACK_LRIG` 収集は BattleScreen が**アタック側の `my.lrig_granted_auto_effects` しか見ていなかった**）、`ON_ATTACK_SIGNI` 側だけ `collectFieldTriggers` に any_opp 経路がある**非対称**が真因。**engine→parser→decompiler の3層を揃えて完結**＝(1)`collectLrigAttackDefenderTriggers` 新設（any_opp/any だけ拾い未設定 self は攻撃側の担当＝二重発火なし）(2)BattleScreen 配線（防御側は playerId も `actions_done` 書き戻し先も別）(3)parser の timing 判定に複合主語＝2要素 `['ON_ATTACK_SIGNI','ON_ATTACK_LRIG']`／ルリグ単独＝`['ON_ATTACK_LRIG']` を追加（**⚠総称フォールバックより前に判定**しないと ON_ATTACK_SIGNI へ落ちて相手シグニのアタックで誤発火）＋scope 抽出を `timing.includes` へ (4)decompiler が `このルリグ` 始まりを scope 主語置換の対象外にしており **any_opp なのに「＊この＊ルリグが」と描いて原文照合を誤らせていた**表示バグも修正。**4効果が完全化**（218i の silent fallback が解消＝PARTIAL→AUTO が機械的な証拠）。全ゲート緑（**golden 514→516**）。新規 golden 2件は engine 収集を無効化／parser を1要素 timing に戻すと**それぞれ落ちることを実測確認**。詳細 BUGFIXES 続き218j。
  - **次の一手**＝タスク5 の残小口（`WXDi-P03-005`／`WX26-CP1-100` ほか）／**timing[C] 残43効果（タスク16）**＝ただし上位クラスタが3・2・以降すべて1件のロングテールで1件あたりの機構コストが高く ROI は低い／**タスク12(xxii) 残50件**／**(xxix) 残＝`WX06-014-E2` の JSON が原文と別物**（step1「自分のトラッシュから《古代兵器》5枚をデッキ下」が「相手シグニをデッキ下」に化け・step2「それ」照応も欠落＝エクシード相当コスト＋対象照応の機構要・§6.3級）／(xlvi) 残（MANUAL 過剰簡約群＝§6.3級）・(vii)(viii)(xlii)(xliv) の残。**⚠(xlvii) で入れた防御側収集は §7 実機検証の対象**（相手ルリグアタック時に `WXDi-D06-010`「そのアタックを無効にする」が実際に発火し、ガード応答 `pending_lrig_attack` と噛み合うか＝Sonnetタスク1へ）。**Sonnet はタスク1（§7 実機検証）**。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 510・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1880。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**🆕2026-07-19 整理時点**＝**Opus の主戦場は タスク12 の生き残り在庫（(xliii)(xliv)・(xxxix)・(xxii)残50件・(xxix)残・(vii)(viii)(xlii)の残）＋タスク16 残43効果**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUIほか）**＝タスク6は Opus の新語彙着地待ち・タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
