...
**直近＝2026-09-21＝🆕§5.7 `S-32`＝効果の対象の狙い方をデッキで指示できるようにした（大まか → 細かい）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第440バッチ）
- **ユーザー要望**＝「相手のパワーの大きいもの」「パワーを下げることでバニッシュできるもの」「固有のカード指定」。
- **入れたもの**＝①**大まかな指示**（強いもの＝既定／**下げて落とせるものを優先**／弱いもの）②**固有のカード指定**（[狙う][避ける]＝±12000）。
  画面はデッキ設定「🤖 CPU の作戦」。🔴**既定は `S-22` の挙動そのまま**＝指定したデッキだけ動く。
- 🔴**「落とせる」が分かるのはパワーを下げる／固定する効果だけ**＝バニッシュ等は並べ替えない
  （「全部 true」にすると**効いているのか分からない指示**になる）。
- 📏**母集団**＝「下げて落とせる」が効くのは **21デッキ中5つ**（`WD11` 6／`WD07` 5／`WD08` 4／`WD05` 3／`WD14` 3）。
  🔴**A/B の6デッキには1件も無い**＝**既定の測定台では測れない型**（`S-20` と同じ形）⇒ だから既定を変えずに指示にした。
- ⚠**踏んだ罠**＝`cardValue` は `強さ × 100 + レベル` なので**外側に足した重みは効かない**（8000 の差＝800,000）。
  🔑**「重みを足したのに変わらない」ときは足した場所の単位を疑う。**

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-32` の残り**（相手の札を名指しで狙う／条件つき）と **`S-31` の残り**（①N体並べる ②払えないコスト523効果 ③条件つき・アーツの使いどころ・使わない札）。次に `S-29` |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（🔥`S-32` 残り・🔥`S-31` 残り・`S-29`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**CPU が払えないコストは残 523効果**（`S-31`） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4359**）｜**実機 PASS**（6ターン/133手/185s ＋ `v268CpuDeckPlan`） |
...
**残0**（直近＝🏁`V-283`＝**コンボの「使い方」を画面から保存できる**＝2026-09-21 に `v268CpuDeckPlan` へ②'として足して PASS）。
...
| 🔥`S-32` | 🆕**効果の対象の指示**（2026-09-21 ユーザー要望）。🏁**第1段は消化済み**（第440バッチ＝**大まかな指示3種**（強い／**下げて落とせる**／弱い）＋**固有のカード指定**（狙う／避ける・±12000）＋編集 UI＋実機）。**残り**＝①🔴**相手の札を名指しで狙えない**（[狙う][避ける]に出るのは**自分のデッキの札だけ**＝相手の山は分からない。案＝クラス／レベル／パワー帯など**属性で指定**する）②**条件つき**（「ライフが2枚以下なら」「相手の場が3体なら」で狙い方を切り替える）③**効果ごとの指示**（いまはデッキ全体で1つ）。📏**「下げて落とせる」が効くのは21デッキ中5つ**（`WD11` 6／`WD07` 5／`WD08` 4／`WD05` 3／`WD14` 3）＝🔴**A/B の6デッキには0件**＝既定の測定台では測れない（`S-20` と同型）。 | 中 | 作った CPU デッキの「誰を狙うか」を指示できる |
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**②は消化済み**（第439バッチ＝**手札を捨てるコスト**を CPU が払えるようにした＝`handDiscardSigni` 63／`discard` 62）。**残り**＝①**「N体並べる」条件**（`SP06`／`SP07`／`天使軸1`＝「名前の異なる＜原子＞3体で全体＋2000」等＝**2枚の順序では表せない**・実測3デッキ）③**条件つき**（相手の盤面・ライフ・ターン数）／**アーツの使いどころ**（守り／攻め）／**使わない札**の指定。📏**②の残り＝523効果**（`exceed` 106／`betOptions` 68／`costReplacement` 57／`energyTrash` 52／`costScaling` 47／`fieldTrash` 31／`trashExile` 15 ほか）＝**ルリグ【起】・場以外の【起】は別 allowlist**（未着手）。⚠**着手前に「どれが欲しいか」をユーザーに確かめる**（①〜③で実装が全く別物）。 | 中〜大 | 作った CPU デッキを意図どおりに回せる |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🏁~~`S-22`（対象選択が効果の意味を見ていない）~~ → 🏁~~`S-24`（自己対戦のマリガン）~~ → 🏁~~`S-30`（測って否定＝CPU は退化していない）~~ → 🏁~~`S-14`（作戦データの「使い方」）~~ → 🔥**`S-31`（CPU の作戦をもっと細かく＝ユーザー要望・🏁②の手札捨ては消化）** → 🔥**`S-32`（対象の指示＝🏁第1段は消化）** → `S-29`（宣言の帰結のコスト＝母集団未計測）。
...
- **2026-09-21 時点**（第421〜440バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🆕`S-32` 第1段）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 4**（🔵`S-6`・🔥`S-31` 残り・🔥`S-32` 残り・`S-29`）｜**作戦データの入力 21デッキ**（ユーザー作業）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4359**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・5/130手・**6/133手**）
  - 📏**対象の狙い方「落とせるものを優先」が効くデッキ**（§5.7 `S-32`・2026-09-21）＝**21デッキ中5つ**（`WD11` 6件／`WD07` 5／`WD08` 4／`WD05` 3／`WD14` 3）｜🔴**A/B の6デッキには0件**
  - 📏**CPU が自動で払えないコスト**（§5.7 `S-31` ②・2026-09-21）＝live の ACTIVATED **2,632効果のうち 648（24.6%）/ 603枚**。🏁**手札を捨てる2キー（125効果）は消化**／**残り 523効果**（`exceed` 106／`betOptions` 68／`costReplacement` 57／`energyTrash` 52 ほか）
