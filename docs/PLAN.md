...
**直近＝2026-09-22＝🏁§5.1 `V-284`（じゃんけんの回帰ガード）＋🏁入口が場ではない【起】2効果**（全文は [BUGFIXES.md](./BUGFIXES.md) 第451バッチ）
- 🏁**`V-284`**＝**ソースの形で固定**（React の予約の仕方が壊れていた＝純関数は全部正しいので golden にも smoke にも映らない）＝
  ①手の組で予約 ②🔴**cleanup でタイマーを消さない** ③失敗時は予約を解放 ④クリック経路も同じ予約を見る ＋ reducer の純関数検査。
  📏**反転確認＝修正前のソースでは4項目すべて FAIL**。🆕ハーネスにも**名前付きの失敗**を足した（「あいこ」12周で落とす）＝
  🔑**旧は3回踏んで3回とも「ビルド直後の flake」と誤判定した**（症状に名前が無かった）。
- 🏁**入口が場ではない2効果**＝`handExileSelf`／`energyTrashSelf` は**コストが「その札がその置き場に在ること」を要求する**形
  ⇒ parser の入口フラグを**コスト側からも立てた**（`O-262` と同じ理屈・2行）。📏live のフラグは**狙った2効果だけ +1**。
  ⚠🔴**ゾーンごとに経路が違う**（手札＝`handActivateCost`／トラッシュ・エナ＝`trashActivateCost`）＝
  最初に両方へ足して**死んだ分岐**を作ったので権威側だけに置き直した。⚠**「捨てる」と「除外する」は行き先が違う**。
- 📏**実カード照合**＝2効果とも**その置き場から提示・場では非提示・支払いでその札が置き場から出る**。🔧**実機 PASS**（両デッキ）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-6`**（機械学習＝第1〜2段は消化済み）。⚠小さい在庫＝**手札の【起】を CPU が撃てない**（`handActivateCost` 側に CPU の口が無い） |
...
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**手札の【起】は人間だけが撃てる**（CPU の口が無い＝踏み倒しではない） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4370**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
- **2026-09-22 時点**（第421〜451バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
