...
- **🆕 セッション（2026-07-09・続き60後半・Sonnet 5・§4タスク(F) §7実機検証の継続＝手札捨て/トラッシュ flatten（R36）追加・実バグ発見）**＝`verifyBattleDrive.mjs` に新シナリオ `handDiscard`（WDA-F02-17→WXK10-065）を追加し実機検証・**❌FAIL＝続き58のresume経路取りこぼし理論の新規インスタンスを確認（2回連続再現・未修正・Opus引き継ぎ）**。WXK10-065【出】「あなたは手札を1枚捨てる」（TRASH HAND_CARD self count1・SELECT_TARGET要）で手札に残ったWDA-F02-17自身を選ばせると、ground truth（hHand 2→0・hTrash 0→1）は正しいが自己参照ON_TRASH watcher（fromZones:hand）が一度も発火しない。原因＝`selectOrInteract`（`execUtils.ts:1206`）は候補1件でも必ずSELECT_TARGETを要求する仕様のため、このTRASHアクション自体がresume経路（`handleEffectInteraction`）で完結し、`collectAnyZoneTrashSelfTriggers`（`resolveStackNext`中央diffのみ配線・resume側inlineなし）が呼ばれない＝R43/R46/R39と同型。系統的懸念に`collectDeckTrashSelfTriggers`（ON_TRASH self・fromZones:deck）も追加（未検証）。`order`からは除外（Opus修正待ち）。**ハマりどころ**＝host中央ルリグ未設定だとログイン直後のLv0/Limit0初期ルリグのままになり「召喚」ボタン自体が出ない（`'field.lrig':['WD01-001#1']`を明示設定して解消・カード側の問題ではなく盤面注入の不備だった）。詳細 VERIFY_BROWSER.md/BUGFIXES.md「手札捨て/トラッシュ flatten（R36）」節・PLAN §6.3・§7。
  - **前半＝opp-draw（R40）追加＝✅PASS（2回連続）**＝新シナリオ `oppDraw`（WXDi-P15-091→WX12-047）。host に watcher WXDi-P15-091（羅石　ラブラドライト・【自】ON_DRAW・triggerScope:any_opp・《ターン1回》対戦相手が効果でカードを引いたとき自分も1枚引く）、guest（CPU）に WX12-047（幻水　ヤリイカ・【自】このシグニがアタックしたとき条件なしでカードを1枚引く）を配置し、CPU自動アタック（`wd07012`と同型・クリック不要）で発火。ログ「[相手]幻水　ヤリイカの【自】効果（シグニアタック時）」→「[自分]羅石　ラブラドライトの【自】効果（対戦相手ドロー時）」・hHand 5→6 で ground truth・watcher 双方確認。機構的にR31(drawBySourceStory)と同型＝原因アクション（DRAW・対話不要）は`resolveStackNext`の`done`ブランチで正常収集＝resume経路取りこぼしの穴とは無関係。ハマりどころ＝ライフクロスクラッシュ確認モーダル（「エナに送る」ボタン）未クリックで初回FAIL→クリック候補追加で解消。`order`配列に追加済み。詳細 VERIFY_BROWSER.md「opp-draw（R40）」節。
  - **検証（両方）**＝各シナリオ単体2回連続実行で安定再現・typecheck緑（engine/parser不変のためgates再実行不要・script/docsのみの変更）。
