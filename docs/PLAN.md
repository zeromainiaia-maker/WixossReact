...
- **🆕 セッション（2026-08-11・続き432〜433・Opus 5）＝§6.4 の宿題を残0にし、UNKNOWN 消化を1バッチ。golden 1800→1807・census 854 据置・UNKNOWN 36→34**。
  - **🔴① 続き432＝登録されていた宿題の1つは「穴」ではなかった**＝「起動コスト増加がトラッシュ起動に未適用」は**仕様どおり**（原文が「**センタールリグとシグニ**の【起】」に限定＝トラッシュのカードはシグニではない）。⭐**「未適用の入口」を全部穴と決めつけない＝原文の限定を先に読む。** goldenTest に**適用する3入口／適用しない5入口の表**を固定して誤修正を防いだ。
  - **🔴② 実際の穴は別の入口だった＝カットイン窓**＝`CutinModal` が `collectIncreaseActCost` を見ておらず、**同じ【起】能力なのにメインフェイズ経由とカットイン経由でコストが食い違って**いた。⚠**同じファイルの上部にアーツ実効コストで同型の穴を踏んだ記録が既にある**＝この入口は食い違いが再発しやすい。
  - **🔴③ `PR-470A` の入れ替え配置がパワー配置制限を素通り**＝⭐**「count は無関係」で止めると同じ funnel の別の判定を見落とす**（`deployLimitBlockReason` は count とパワーの2軸）。`fieldCountAdjust:1` で通した。
  - **🔴④ 続き433＝UNKNOWN も「未実装」ではなく過剰効果だった**＝`WX07-050`／`WX16-Re18` の「ターン終了時、（その）レゾナをルリグデッキに戻す」が UNKNOWN に落ち、**一時的に出すはずのレゾナが場に居座り続けていた**。⚠**戻し先はトラッシュではなくルリグデッキ**／⚠**ターン終了処理は2経路ある**（片方だけだと「手札が多いターンだけ戻らない」）／⚠**置いたレゾナはゾーン選択の pause を跨ぐ**ので ctx ではなく state に記録する。
  - **⭐⑤ parser は「届く断片」でアンカーする**＝先頭の「ターン終了時、」は文分割で剥がれる。緩めて `それをルリグデッキに戻す`（単数）まで拾うと**アシストルリグを戻す【起】**を巻き込むので、原文全数（38件）を確認して2形だけに限定した。
  - **✅⑥ ゲート**＝`npm run gates` 全緑（typecheck / golden **1807（+7）** / smoke 10688 全0・SKIP 0 / fuzz 全0 / census **854 据置** / census-stubs / manual-fields / lint 0 errors）。`npm run regen` 済み。**§6.4「消化済み項目が残した宿題」は残0**。
  - **📋⑦ 次の一手**＝**Opus 側**＝(a)**UNKNOWN 残34ノード/32カード**（**過剰効果になっているものから**取る＝「未実装」と思い込まない）(b)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(c)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(d)**置換の「してもよい」辞退**（ライフクラッシュ置換側）。**Sonnet 側**＝§7 実機検証（**続き431 の回帰確認を最優先**、次いで続き428・430・433）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き432〜433（§6.4 宿題残0＋UNKNOWN 消化）後 最新値（本行が直近の正）**：census **854**、golden **1807（+7）**、**`MANDATORY_SUSPICIOUS` 0**、golden `FORCED_HAND_COST_KNOWN` の🔴側 **0**、**`census:stubs` A群 16件＝明示 defer 16／無言 no-op 0**（ゲート化済み）、smoke 10688 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、**UNKNOWN 34ノード/32カード**。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出・実害なし）。
...
- **UNKNOWN（部分未実装・逆翻訳に `【未実装/UNKNOWN】` として露出）**：**🆕2026-08-10 続き414 に live JSON を全数走査して数え直した実数＝40 ノード／38 カード**（⚠**旧記述「42箇所／31カード」は decompile シートの出現数**で、live のノード数とは別物＝**計器は live JSON を数える**）。**続き414〜415 で 43→38**。
  ...
  - **📋 残り34ノードの内訳（続き433 実測）**＝(a)**デッキ公開→処理の変形 2件**（`WX21-028-E1`＝「その中から**すべての**＜天使＞のシグニをエナゾーンに」＝**枚数語が名詞より前**で `parseRevealPickDescriptor` の語順に合わない／`SP38-006-E1-G2`＝主語が対戦相手で1文に「見る」も含む）(b)残りは**単発**（1カード1機構）。
