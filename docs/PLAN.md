...
**直近＝2026-09-20＝🏁§5.7 `S-10`＝盤面の採点のパワー項を「線形」から「閾値」へ**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**真因は重みではなく関数形**＝同じ +3000 を「使う前 750（`cardStrength`）／使った後 3000（`evaluateBoard`）」と**4倍違う値**で数えていた。係数を下げるだけでは直らない（線形なら大きいバフは常に得）。
- **3点変えた**＝①生パワーは `fieldPowerScale`(0.25) ②離散の `laneWin`(1500)＝正面とのバトルに勝てているレーン差（判定は `cpuAttackValueOf` の再利用）③🆕**パワー0以下は「居ない」として数える**（ルール処理でバニッシュされる＝**パワーマイナスの価値の本体**）。旧は `legacy-power`。
- 🔑**止め時2本**＝①**370戦 A/B で 185-185＝50.0% [44.9, 55.1]**（勝率は下がっていない）②過剰使用＝「バフだけのスペルを撃つ価値あり」に見えるノードが **330/708(46.6%) → 1/708(0.1%)**。
- 🔴**`laneWin` を `openLane` と同値にすると「除去」と「格上で立つ」が同点**になり除去の価値が消える（golden `§5.7 S-4` が捕まえた）⇒ 半分にし、**golden の閾値も重みから導く形**に直した。
- 🔴**測定台の穴**＝**A/B の山に `POWER_MODIFY` が1枚も無かった**（28種すべて）＝**直した当のものを踏まずに「勝率は下がらない」と出していた** ⇒ `S-20` に登録。**「A/B 50% だから安全」と読まない。**
- ⚠**n=96 では 55.2%（旧が強そう）に見えた**＝**区間が 50% を跨いでいる間は読まない**（370戦でちょうど 50.0% に収束）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-15`**（候補列挙器）。🆕**`S-20`（A/B の山が型を踏まない）は `S-11` 相当の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 8**（`S-15`〜`S-20`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先読みの情報漏れ（`S-16` の前提）②先攻が勝てない 28.1%（`S-19`）③**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4333**＝+1）｜**実機** `node scripts/verifyFullMatch.mjs cpu` **PASS**（8ターン / 178手 / 241秒で決着） |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10` を消化）**＝**`S-15`（候補列挙器）** → `S-16`（前半探索）→ `S-17`（後半＝攻撃探索）→ `S-18`（次ターン項）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-20 時点**（第409バッチ＝🏁§5.7 `S-10` 盤面の採点のパワー項）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 8**（`S-15`〜🆕`S-20`・`S-14`・`S-6`＝🏁`S-10` を消化し `S-20` を登録）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4333**＝+1・`census:traceinv` I1=0・`selfplay` 1戦 決着）｜**実機** `verifyFullMatch.mjs cpu` PASS
  - 📏**BattleScreen**＝**6,287行**（±0）
  - 📊**自己対戦 A/B**＝`legacy-power` vs `default`＝**370戦で 50.0% [44.9, 55.1]**｜**先攻の勝率 33.5% [28.9, 38.5]**（370戦・`S-19`）｜12ジョブで壁時計 1,122秒（対戦合計 12,514秒＝約11倍）
  - 🆕📊**パワー修正の過剰使用**＝「バフだけのスペルを撃つ価値あり」に見えるノード **330/708（46.6%）→ 1/708（0.1%）**（自分側プラス `POWER_MODIFY` 708ノード/651カード）
