...
**直近＝2026-09-21＝§5.7 `S-14` ②母集団＝作戦データを21デッキ下書きし、その過程で「作戦データが探索に効いていない」バグを直した**（全文は [BUGFIXES.md](./BUGFIXES.md) 第437バッチ）
- 📏**着手前の実測**＝作戦データが入っているのは **27デッキ中2つ**／`use` 列が要るコンボは **corpus に0件**
  ⇒ **ユーザー決定＝「先に下書きする」**（PLAN §5.7.0 の「LLM は作るときだけ使う」）。
- 🆕**下書き＝21デッキ全部**（[CPU_DECK_PLANS.md](./CPU_DECK_PLANS.md)・キーカード20／優先札34／コンボ14）。
  **カード番号は全数検証済み**（デッキに無い番号0）。⚠**DB へは書かない**＝入力はユーザーが画面で行う。
  🔑**下書きを作って初めて `S-14` の母集団が確定した＝3件**（`WD06`／`WD08`／`WD16`＝どれも「【起】で使う」が書けないと成立しない）。
- 🔴**下書きを当てて測ったら、作戦データが探索に1度も効いていなかった**＝`S-25` で既定を「探索あり」へ上げた瞬間、
  召喚の判断が `pickCpuDeployCard`（`planDeployBonus` を足す側）から**探索（`evaluateBoard` だけ）**へ移り、
  **6つの重みのうち4つが死んでいた**（A/B で3デッキが**完全に同じ試合**）。
  ⇒ **`CpuSearchOpts.moveBonus`** を新設して配線（**加点は打つ前の盤面で測り、手順に沿って累積する**・式は1本）。
- 📏**直接指標＝作戦データが判断を変えた対戦 1/4 → 8/8**（最初の分岐 63手目 → 26手目）。
  📊A/B（96戦）は6山とも差なし＝**これは配線の修復**であって強さの調整ではない。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | ⚠**`S-14` の実装（`{num, use}` の列）はユーザー判断待ち**（母集団3件・下書き済み）。取るなら **§5.7 `S-29`**。🔵`S-6` 第3段は「大きく振る」なら |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（`S-14`＝判断待ち・`S-29`）／リリース作業 **1**（RELEASE.md）／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**グロウできない残8件**＝枚数不足かつ手札に該当色が無い |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4356**）｜**実機 PASS**（8ターン/177手/239s）｜反転確認＝`--a-set "plan.*=0"` |
...
| 🔵`S-14` | **作戦データのコンボに「使い方」を持たせる**＝`CpuDeckCombo` はいま `{first, then}` のカード番号だけで**出す／起動する／アーツで撃つを書けない**。`{num, use}` の列へ（`CpuDeckPlanModal` の編集 UI も）。🆕**2026-09-21＝母集団を実測して確定＝3件**（`WD06` リュウグウの【起】／`WD08` ウムル＝フィーラ【出】→ネビュラをトラッシュから【起】／`WD16` Ｆ・Ｍ・Ｓ の【起】→Ｇ・Ｌ・Ｋ）＝**下書き（[CPU_DECK_PLANS.md](./CPU_DECK_PLANS.md)）を作って初めて出てきた**（それ以前は corpus に0件）。⚠**「N体並べる」条件（`SP06`／`SP07`／`天使軸1`）はこの型でも書けない**＝別の型が要る。🏁**前提だった「作戦データが探索に効かない」バグは第437バッチで修正済み**（`CpuSearchOpts.moveBonus`）。🔵**着手はユーザー判断待ち**＝作戦データの入力（21デッキ）が先。 | 中 | `S-16` の探索が届かない長いコンボを手で指定できる。⚠実装は `src/screens/` を触る＝**実機まで必須** |

🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🏁~~`S-22`（対象選択が効果の意味を見ていない）~~ → 🏁~~`S-24`（自己対戦のマリガン）~~ → 🏁~~`S-30`（測って否定＝CPU は退化していない）~~ → 🔵**`S-14`（作戦データ＝②母集団まで完了・実装はユーザー判断待ち）** → 🔥**`S-29`（宣言の帰結のコスト＝母集団未計測）**。
...
- **2026-09-21 時点**（第421〜437バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ `S-14` ②母集団）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（🔵`S-6`・🔵`S-14`・`S-29`）｜**作戦データの入力 21デッキ**（ユーザー作業）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4356**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・**8/177手**）
  - 📏**CPU デッキの作戦データ**（§5.7 `S-14`・2026-09-21）＝**入力済み 2 / 21デッキ**｜下書き＝[CPU_DECK_PLANS.md](./CPU_DECK_PLANS.md)（キーカード20／優先札34／コンボ14）｜**`{num, use}` が要るコンボ 3件**（`WD06`／`WD08`／`WD16`）
