...
- **🆕 セッション（2026-07-12・続き105・Sonnet 5・verifyBattleDrive状態汚染の根本修正＋census文型バッチ「CHOOSE選択肢②条件」6枚是正）**
  ...
  - **✅ 続いて §5c census文型バッチへ切替＝「条件節(〜の場合)」クラスタからCHOOSE選択肢②型の6枚を発見・是正**＝「①基本効果。②あなたの場に(色)の＜C＞のシグニがある場合、より強い効果。」型で、3枚は条件節が丸ごと脱落・3枚はさらに深刻＝条件の色/ストーリーが誤って選択肢アクションの対象フィルタへ混入し「対戦相手の白のプリオケシグニを対象」等の別効果に化けていた（WX25-P3-092／WX26-CP1-011/013/015/017/018）。**`ChoiceOption.condition`（選択肢の選択可否ゲート・`execChoose`で配線済みの既存語彙）で是正＝parser/engine変更なし**。副産物＝decompilerがCHOOSE選択肢の`condition`を描画しない欠落も発見し合わせて修正（`scripts/decompileEffects.ts`）。golden 283→284・census 1477→1471。⚠**「あなたのターンの場合」クラスタ（9枚）は着手せず据置＝engine の`IS_MY_TURN`がプレースホルダ常時真のため、CLAUSES追加は見た目だけの無意味な変更になる**（parser内コメントで既に対象外と明記済みと確認・engine拡張が要る＝Opus）。
  - **✅ 続けて「パワー閾値(NN以上/以下)」クラスタから「対象のパワーN以下のシグニ…手札を1枚捨ててもよい。そうした場合、バニッシュする」型9枚を是正**（LIFE_BURST中心7枚＋派生2枚＝WXDi-D09-H20/H24・WXDi-P05-058・WXDi-P08-059・WXDi-P10-054・WXDi-P12-066・WXDi-P14-081・WXDi-P03-059・WX24-P4-050）＝(a)「捨ててもよい」が`TRASH.optional`未設定で強制discard化・(b)「パワーN以下」の対象制限がBANISHから丸ごと脱落し任意の相手シグニを除去できる過剰効果、の二重バグ。**既存idiom（`optional:true`＋`CONDITIONAL(IS_MY_TURN)`＋`powerRange`。WXDi-D08-013等の実装済み先例を踏襲）で是正＝parser/engine変更なし**。golden 284→285・census 1471→1467。
  - **✅ 続けて「あなたの場に色と色のシグニがある場合」CLAUSES新設＋ルリグアタックトリガーのプレフィックス構造欠陥を修正＝計10枚是正**（WX14-010/WX14-013/WX20-009/WX20-015/WX20-019・WX24-P3-046＋副次効果4枚＝WDK04-001/WDK11-001/WXDi-P05-052/WXDi-P16-010）＝新規`AND[HAS_CARD_IN_FIELD×2]`規則を追加したところ狙った5枚がマッチせず、原因は「このルリグがアタックしたとき、」がCLAUSES先頭プレフィックスの許可リストに無く条件節ごと素通りしていた構造欠陥（「このシグニが」＝シグニアタック時は続き23で対応済みだったが「このルリグが」＝ルリグアタック時が未対応のまま残っていた）と判明＝プレフィックス追加で新規規則に加え既存3規則（`HAS_CARD_IN_FIELD`カード名版・`ENERGY_COUNT`）も連鎖的に是正。golden 285→286・census 1467→1461（`BASELINE_HIGH`更新済み）。詳細（3バッチとも）BUGFIXES 続き105。
  - **次の一手＝** (a) §5c census文型バッチの継続（`npm run census:clusters`で次のクラスタを選定）、(b) §7の残＝B4引用付与実発火・B2/B3・ビート機構Phase1-7・機構④誤parse3枚・F-3身代わり対話等の「その他の実機検証待ち」リスト（PLAN §7下部）、(c) semantic audit残り8バッチ再開は続き103が指摘した前提崩れ（prompts/raw消失）への対処が先、(d) `oppDraw`の単独実行でも再現するFAILの深掘り（優先度低）、(e) census「このターンにあなたが(色)のアーツを使用していた場合」クラスタ（WX24-D1-11〜D4-11の4枚）は真バグ確認済みだが色別`turn_arts_used`トラッキングが engine に無く新規機構が要る＝Opusタスクへ登録要。
