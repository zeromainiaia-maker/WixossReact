...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕が指示と検証＋codex-work が実装）＝タスク12(xxix) を9波連続で消化**（golden **1033→1063**・census **1412→1400**〔`BASELINE_HIGH=1400`〕・smoke 10726 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held 293/107 据置・manual field loss 0。**全9波でゲート全緑／差し戻し0／JSON per-effect diff は毎回スコープと完全一致**）
  - **通しの主題＝「任意【出】のコストを、払わずに撃てる／払えなくて撃てない、の両方を潰す」**。`optionalOnPlayCostStub`（`triggerCollect.ts`）で写せない任意cost【出】は **80相当→20**（⚠**実質は16**＝残20のうち `discardUpTo` 4件は `COUNT_BASED_DRAW_OR_POWER` の早期 return で収集されており機能上の問題は無い）、`costUnparsed` は **47→25**（AUTO・ON_PLAY・任意は **33→11**）。
  - **第1波＝🔴エクシードの踏み倒し21効果**。`SigniOnPlayCostModal` が `cost.exceed` を読まず「コスト: なし」で確定できた＝**ルリグの下を1枚も払わずに【出】エクシードN が撃てた**。支払い実体 `INTERNAL_PAY_EXCEED`・Pattern③④・`CutinModal` の選択UIは**全部実装済み**で、Pattern⑤ とこのモーダルにだけ配線が無かったのが真因＝**新機構ゼロで解消**。同時に **Pattern⑤ の `handDiscardGroups` 素通り**（`WX20-058-E1`）と `WXDi-P11-076-E1`（エクシード7）の踏み倒しも解消。
  - **第2波＝`costUnparsed` 17効果を既存コスト語彙へ**。`handToEnergy`/`handToUnderSelf`/`lifeTrash`/`lifeToHand` は**支払い実装だけあってデータが無い**状態だった。⚠`costUnparsed` 47のうち**14件は原文にコスト句が無い偽陽性**。
  - **第3波＝「N枚まで／好きな枚数 捨てる」7効果**。engine は **action 側が SELECT_TARGET で自分で徴収する**設計で、乗っていない7効果が**捨てが起きず no-op**。🔴`WXDi-P13-041-E1` は no-op に加え**相手シグニ3体全部**に修正する過剰実行（原文「1体」）。既存経路の欠陥3件も同時解消。
  - **第4波＝`fieldTrash` 18効果**。🔴**先にコスト限定の脱落4件を是正**（傀儡状態／《ライズアイコン》／他の赤の／レベル1+2 が素の `{cardType:'シグニ'}` ＝**モーダルで本来払えないシグニでも払えていた**）。`fieldTrashGroups` はUIごと対応。🔴`WXDi-P00-067-E1` の `powerLtLastProcessed` 復元（無条件バニッシュの是正）。
  - **第5波＝`handToEnergy` 7＋`handToUnderSelf` 4**。`execPlaceUnderSigni` は**在場でないと黙って no-op を返す**ため、支払い可否に「効果元が在場か」を入れないと踏み倒しになる。
  - **第6波＝ライフクロス8効果**。engine を `LIFE_CRASH{triggerBurst:true}` で正しく実装し、**UI 側の「トラッシュするだけ・バースト不発」という近似も本物のクラッシュへ揃えた**（`field.check`/`pending_crashed_cards` は既存機構）。🔴副次発見＝**UI の複数枚クラッシュが `slice` 順のままで最上段の2枚目を先に処理**していた順序バグを是正。
  - **第7波＝ダウンコスト**。既存 `execDown(LRIG)` が**センター専用**でアシストを落とせないため、**8件中2件だけ採用し6件を意図的に見送り**（踏み倒しより不発）。
  - **第8波＝その不足機構を埋めて6件を回収**。UI にだけあった支払い（センター→アシストL→R）を `battle/lrigDownCost.ts` の純関数へ切り出し、**UI と engine が同じ関数を呼ぶ**形に。engine は `INTERNAL_PAY_EXCEED` の先例に倣った STUB 1本のみ。**既存 `DOWN{LRIG}` の意味は広げていない**。
  - **第9波＝【ビート】コスト7効果**。原文が3種（自身＋1体／他1体／《名前》以外1体）に割れているのを効果ごとに確定。🔴`analyzeBeatSigniCost` の「《名前》以外」が**同名の別コピーを候補に残していた**のを是正（UI 経路にも同じ helper が効く）。
  - **⚠簿記が実測とズレていた点2つ（次セッションはここを疑う）**＝①指示書の「写せない59」は第1波時点の値で、第2波が新語彙を作った分だけ母数が増えて stale だった ②`docs/_held_review.txt` は `21a24900` 以降再生成されておらず **held 251→293 の +42 は本セッション起因ではない**（ベースラインコミットの worktree で再生成して 293/107 の一致を確認済み）。**投入前に必ず数え直す**（CODEX_GUIDE §3-1）。
  - **委譲運用の所見**＝codex は**指示書の実測表に書いた誤りは正確に直し、書かなかった部分の原文照合が甘い**という既知傾向のまま。一方「見送り理由」「スコープ外で見つけた不一致」の自主申告は9波とも高品質（第1波でエクシードルリグ6件の本体不一致、第4波で `WX20-063-E2`/`WXK06-082-E1` の filter 脱落、第7波で「アシストDOWN不足」を機構名指しで申告し第8波の設計に直結）。**Claude 側が投入前に per-effect の誤り明細を作るほど事故率が下がる**という §7 の教訓が9波とも再現。Claude 検証で足した是正は3件（第3波の cost 落としを `discardUpTo` 限定へ／第5波のカード除外を名前付きゲート `OPTIONAL_ON_PLAY_COST_REF_DEFERRED` へ／第8波の `getCardNum` 写経に循環参照回避の理由コメント）。
  - **次の一手**＝**Opus：(xxix)(1) 残 20（実質16）**〔`trashArtsFromLrigDeck` 6／`removeOppVirus` 3／`deckTrash` 2／`fieldToLrigTrash` 1／`fieldTrashGroups` 1（`WX14-045-E1`＝engine 支払いのみ未対応・UIは対応済）／`charmTrash` 1／`underSelfTrash` 1／明示保留1（`WXDi-P16-080-E1`）／`discardUpTo` 4＝**機能上は問題なし**〕**／(xxix)(2) 残 11**〔相互制約・可変資源・複数ゾーン全捨てで新機構待ち〕**／🆕単独バッチ候補（過剰実行・優先）**〔🔴`WX24-P4-072-E1`＝`PLAY_FREE` から「コストの合計が捨てた枚数以下」の制約が丸ごと落ちて**どんな高コストスペルでも無料で撃てる**（経路が ACTIVATED で別）／エクシードルリグ6件の本体不一致（`WX24-P4-011-E2` の即時UP・`WX24-P4-018-E2` の unless 脱落 等＝**第1波でコストを払うようになった分いま効いてくる**）／`WX20-063-E2`「コスト合計0」・`WXK06-082-E1`「《幻竜 ドラマジ》以外」の filter 脱落〕**／(lv) 残2経路（CPU シグニ召喚・CPU グロウ）／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋**(xxix) で新設/改修した支払いUI（エクシード選択・場シグニ選択＋`fieldTrashGroups`・可変枚数捨て・ライフクラッシュのバースト接続）**＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-29 タスク12(xxix) 9波後の最新値（本行を (xxix) 系の正とする）**：**任意cost【出】母集団 958／`optionalOnPlayCostStub` で写せない 20**（⚠**実質16**＝残20のうち `discardUpTo` 4件〔`WXDi-D07-018-E1`／`WXDi-P04-029-E2`／`WXDi-P05-031-E2`／`WXDi-P16-038-E1`〕は `wrapOptionalOnPlay` の `COUNT_BASED_DRAW_OR_POWER` 早期 return で収集済み＝機能上の穴ではない。**この計器は「optionalOnPlayCostStub が写せるか」だけを測っている**ので、残数を worklist にする前に必ず内訳を見ること）。**`costUnparsed` 総数 25／AUTO・ON_PLAY・任意 11**（初期47／33。⚠47のうち14件は原文にコスト句が無い偽陽性）。golden **1063**、census **1400**（`BASELINE_HIGH=1400`）、smoke **10726/10726**、fuzz 全0、lint 0 errors/228 warnings、同型★0、held **293枚/107署名**、manual field loss 0。⚠**80/71/59/54/43/35/33/27 等の旧値は母数が違う**（波ごとに新語彙が増えたため）＝**投入前に必ず数え直す**。
