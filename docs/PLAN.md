...
- **🆕 セッション（2026-08-07・続き375・Opus 5〔**§5d パターンA 第7バッチ**＝続き374 で登録したブロッカー2件の実装〕）＝ユーザー指示「続ける」。触った層は `effectParser.ts`（`applyDroppedEnergyDesignation` 新設＋条件節1本）／**`execUtils.ts`（evalCondition のルリグ色解決）**／`public/data/effects_*.json`（4効果・すべて held 経由で採用）／`goldenTest.ts`。
  ...
  - **次の一手**＝**§5d パターンA を続ける**。「共通する色」クラスタの相手エナ系統は**残1**（`WX25-P3-003-E1`＝LB封じ STUB・別機構）。次は**まだ手つかずの大きいクラスタ**＝「クラス指定(＜X＞のシグニ)」155件・「条件節(〜の場合)」286件・「小さい数(2-5枚/体)不在」182件。**やり方は同じ**＝①原文でサブパターンに分類 ②「もう直っている割合」を測る ③既存の型・語彙・段階ロールアウト表を先に探す。**新機構待ち登録**＝`WXK11-040-E2`（コスト経路の名前変数）／`WX14-072・075-E1`（選択肢構造の潰れ）／`WXK09-029-BURST`（据置再評価）。**§5d 末尾の照合済み12件**も未修正。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先。
