...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-6`**（機械学習）。⚠小さい在庫＝`C-10` の残り（レゾナのカットイン／ピース応答窓／危険度を見た判断） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／CPU 完成度 **1**（`C-10` の残り＝レゾナ／ピース窓／判断の質）／**CPU の強さ 1**（🔵`S-6`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
...
⚠**残り**＝レゾナのカットイン（出す先の選択）／ピース応答窓／**相手のスペルの危険度を見た判断**（いまは「打ち消せるなら打ち消す」だけ）。
