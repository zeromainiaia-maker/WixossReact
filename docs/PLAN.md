...
- 🏁**セッション（2026-08-27・続き687・Opus 5 単独）＝B9＝固定挙動 catch-all `TRADE_BANISH_SELF_SIGNI` が
  「カードに書いていない除去」を **26効果**で実行していた。** engine のハンドラは **payload も原文も見ない固定実装**で、
  常に「自分のシグニ1体をトラッシュ →**相手のシグニ1体をバニッシュ**」を走らせる。適用先の原文は
  「エナゾーンから」「このシグニを」「【チャーム】を」など様々で、**原文にない除去**と**間違ったコスト**の二重バグだった。
  - 🔑**本質はゲート**＝`IS_MY_TURN`（常時真）→ `PAID_ADDITIONAL_COST`。**払わなければ本体が走らなくなる**。
    実機の反転確認で旧挙動が数値で出た＝`host 2→2`（コスト未払い＝踏み倒し）／`guest 2→1`（原文にないバニッシュ）。
  - ✅**新機構ゼロ**＝`OPTIONAL_COST` の payload（`fieldTrash`/`energyTrash`/`charmTrash`/`handDiscard`/`costColors`）は全部既存。
  - 🔴**自分で踏んで直した3件**（すべて既存ゲートが捕まえた）＝①`OPTIONAL_TRASH_SELF` があるのに `selfTrash` を並行新設（§5-5e）
    ②**B7 の「代わりに」置換を食い潰した**（同じカードに両方の規則が当たり、先に走った私の pass が then/else を捨てた）
    ③修飾の部分取りこぼし（「AかB」を AND に、「AとB」を OR に、アイコン限定を脱落）⇒ **未消費の修飾が残るなら据置**へ。
  - 📊**3計器はどれも動かない**（Sheet1 65／OPEN 629／census 535）＝**この失敗クラスをどの計器も見ていなかった**。
    ⇒ golden にトリップワイヤ（残存 `TRADE_BANISH_SELF_SIGNI` が許容リスト6件と集合一致・両方向 FAIL）を置いた。
  - 🔴**「同一機構で30〜50件」はもう存在しない**（ユーザー指示で規模を上げようとして実測）＝
    census 文型クラスタ最大5／台帳 OPEN 629件中613クラスタ＝ほぼ singleton／「N体まで」の `upToCount` 欠落は 301→**実数9**／
    「〜てもよい」の任意落ちは 172→**安全形7**。**量が残っているのは「1つの catch-all STUB を共有する群」だけ**。

**▶ 次の一手**＝**同型の catch-all STUB を1本ずつ潰す**（B9 で有効性が確認できた取り方）。
残りの共有 catch-all＝`LOOK_AND_REORDER`（live **190**・⚠`O-60` が「parser 生成地点が15箇所」と警告）／
`LRIG_UNDER_CARD_OP`（**17**）／`TARGET_AND_DISCARD_HAND`（**16**・うち原文と噛み合わないのは9）／`SOUL_OP`。
🔑**取り方**＝①`grep <STUB_ID> src/engine/` で消費地点を全部数える ②**原文と噛み合っていない件数**を実測して母集団にする
③既存 payload へ写せる分だけ採り、写せない形は**トリップワイヤの許容リスト**に理由つきで載せる。
⚠**Sheet1 の枚数を動かしたい巡は別軸**（要対応 **65枚**＝`npm run census:cards -- --sheet 1 --list`）。
機構側なら **`O-106`**／**`O-105`**／**`O-104`**／**`O-102`＋`O-95`**／**`O-98`**／**`O-97`**／**`O-101`**／**`O-103`**／**`O-93`**／**`O-94`**。実機の腐り返済なら §5.1 の `V-89`。
---
