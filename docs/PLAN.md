...
| 🆕(xxxi) | **✅続き184（Opus）で clean な2枚を消化＝`DRAW_PER_LRIG_LEVEL` 新設**（`POWER_MODIFY_PER_LRIG_LEVEL` に倣い types/engine/parser/decompiler。「あなたのセンタールリグのレベル１につきカードを１枚引く」を per-level ドローへ。WX12-013-E1/WDK07-E09-E2 採用・golden +1・census 2001維持。詳細 BUGFIXES 続き184）。**残＝(a)「引くか【エナチャージ】」CHOOSE 2枚（WXK10-004-E1/WX26-CP1-003-E1①）＝CHOOSE ごと消え `ENERGY_CHARGE count:1` に潰れ＝エナ枝の `ENERGY_CHARGE_PER_LRIG_LEVEL` 新設＋CHOOSE 構築が要る／(b)「そのシグニのレベル１につき」1枚（WD21-001-E2）＝めくったカードのレベル比例（lastProcessed level 参照の別機構）** |
...
- **🆕 セッション（2026-07-18・続き186・Codex 実装／Opus 4.8 検証・是正・簿記・タスク12(xxxv) 消化＝(a)(b)(d) 完了・(c) §6.3送り）**
  ...
  - **(b) WXDi-P02-037-E2**＝Codex が新フラグ `fromFieldByCostOrOwnEffect` で厳密実装（暫定残渣なし）。**(d)** any_opp watcher に triggerFilter/excludeSelf を予防追加（該当0・続き181教訓）。
  - **残渣＝(c)「コストか効果によってシグニの下から」3枚（WX18-062/WX22-027/WXK03-033）＝§6.3送り**（ON_TRASH 効果0件・「下カード→トラッシュ」の detector/timing/collector が engine に無く新機構要）。(b) の派生 WX24-P4-016 MB 表向きトリガー収集も §6.3 未登録のまま。
  - **次の一手**：Opus＝タスク12在庫の残（(i)(ii)(iii)(viii)残(xi)(xii)(xxiv)残(xxx)(xxxi)残(xxxv)-c＋ON_BANISH据置4件）。Sonnet＝新 timing の §7 driver（続き178/179/180分）＋続き181/182/183/186 で発火条件が是正された any_ally/byOwnEffect watcher の実機確認。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 414・smoke/fuzz 全0・同型★0・census 2001。
>
