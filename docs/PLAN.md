...
- 🏁**セッション（2026-08-27・続き686・Opus 5 単独＋Codex 委譲→上限で引き継ぎ）＝B8＝「〈ゾーン〉から場に出たとき」の
  由来限定が**「トラッシュから」しか実装されていなかった**。「エナゾーンから」「手札以外から」は
  **`timing: []`＝【自】が一度も発火しない恒久 no-op**（3効果）と**由来限定なしの ON_PLAY＝手札召喚でも発火する過剰効果**（2効果）に割れていた。
  live 変化 6 effectId。**3計器が全部動いた＝Sheet1 66→65／台帳 OPEN 630→629／census 536→535**。**
  - 🔑**計器ゼロの穴だった**＝`census:timing` は「ON_PLAY へ**フォールバックした**効果」しか数えないので `timing:[]` は見えない。
    census にも golden にも smoke/fuzz にも出ず、**逆翻訳も「発火しない」ことは書けない**。
    ⇒ golden に**トリップワイヤ**（AUTO かつ timing 空の集合＝許容リスト一致・両方向 FAIL）を新設した。
  - 🔑**parser は「表現できないから安全停止」で意図的に `timing=[]` にしていた**（コメントも残っていた）＝
    **過剰発火を避ける判断としては正しいが、恒久 no-op を誰も見ていない場所に置く形**になっていた。
  - ✅**配線の面は 6＋6箇所**（`collectFieldTriggers('ON_PLAY')`／`collectPlacedSelfOnPlayTriggers`）＝
    **投入前に全部列挙して指示書へ書いた**（§5-15／§5-20）。元々**由来を渡していたのは2箇所だけ**だった。
  - 🔴**実機で engine の穴を1つ見つけた（golden は全部緑だった）**＝`execAddToField` は
    **ゾーン選択インタラクションの前に元の領域からカードを取り除く**ので、**resume 後の盤面差分では由来が復元できず**
    fail-closed で**過剰から過小へ裏返っていた**。⇒ **配置した瞬間に由来を記録する**（`signi_placed_origin_this_turn`）方式へ変更。
    ⚠`turnScopedState.ts` のレジストリ登録が必須（typecheck が落ちて気付いた）。
  - 🔴**held 計器が parser の退行を1つ捕まえた**＝新しい由来句の抽出を**密着マッチ**にしたため、
    **由来句が主語より前に出る語順**（「トラッシュから＜古代兵器＞のシグニ１体が場に出たとき」）で `placedFromTrash` が消え、
    **held 73→78**。live は curated が守っていたので**gates も逆翻訳も緑のまま**だった。⇒ 緩い形へ戻して 73 に復帰。
  - ✅**実機2本を新設・各2回連続 PASS・反転確認あり**＝`b8OnPlayOriginTrashFires`（対照＝トラッシュから出せば発火）＋
    `b8OnPlayOriginHandBlocked`（本題＝手札召喚では発火しない）。
    🔴**負方向の witness を1度作り直した**＝「相手の場が不変」では**反転確認が PASS のまま**だった（問いは出ているのに
    支払い未完了で盤面が動かない）。⇒ **「配置後に問いが1度でも立ったか」**を発火の証拠にした。
  - 🔴**Codex が利用上限で途中停止**（`You've hit your usage limit`／exit 1）。**実装の大半は disk に着地していた**ので
    **Claude が引き継いで完成**させた。⇒ **途中停止は全部やり直しではない**＝`git status` と gates で着地点を確かめて引き取る。
  - ⚠**Claude 側の指示書の誤り**＝§5-27 は「**許容リスト＋件数 assert** にしないと在庫が残っている段階で即 FAIL する」なのに、
    指示書へ逆に「許容リスト方式にしない」と書き、**スコープ外の1件でゲートが赤くなった**。
  - 📮**送り**＝`O-106`（「**【出】能力を持つ**あなたのシグニが場に出たとき」＝トリガー元の**能力の有無**で絞る受け皿が無い）。

**▶ 次の一手**＝**Sheet1 の次バッチ**（**`npm run census:cards -- --sheet 1 --list` から取る**＝要対応 **65枚**。内訳 census 26／意味照合 47（findings 58件）／held 5／idset 1）。
🔑**取り方の勝ち筋**＝カード名の一覧では同一文型が見えない。**`docs/_vocab_census.txt` の語彙カテゴリ別**（B3/B5）／**台帳 OPEN の claim を live の生データへ戻す**（B6/B7）／**「機械で検出できる壊れ方」を1つ決めて全数を3分する**（B8＝`timing:[]` / 由来限定なし / 既に正しい）。
🆕🔑**B8 で効いた取り方＝「ある失敗クラスを丸ごと 0 にして、二度と戻らないようゲートで留める」**。Sheet1 の枚数を追うより、
**計器が1つも見ていない壊れ方**を探して計器ごと作るほうが、同じ穴の再発を止められる（`timing:[]` は3効果が静かに死んでいた）。
🔴**着手前に必ず3つやる**＝①**engine／UI の消費地点を grep する**（B4＝実は実装済み／B6＝直すと実機がソフトロック／B7＝engine の後追い補正が効いていない／B8＝**由来を渡している呼び出し口は6箇所中2箇所だけ**）②**母集団を live の生データで数え直す**（B6＝268→64→6／B7＝138→7／B8＝12を3分）③**既存の見送り契約に当たったら、その理由が今も有効かをコードで確かめる**（B7＝解消できたので契約を解いた／B8＝parser の「安全停止」コメントがまさにそれ）。
✅**進捗は3つの計器を併記する**（2026-08-27 ユーザー決定・§3）＝**Sheet1 要対応 65枚／台帳 OPEN 629／census 535**。⚠**単独の計器に固定しない**。**着手時に「今回どれが動く見込みか」を宣言する。**
機構側なら **`O-106`**（B8 の据置1件）／**`O-105`**（B7 の据置1件）／**`O-104`**（B6 の残り3形）／**`O-102`＋`O-95` をセットで**／**`O-98`**／**`O-97`**／**`O-101`**／**`O-103`**／**`O-93`**／**`O-94`**。実機の腐り返済なら §5.1 の `V-89`。
---
