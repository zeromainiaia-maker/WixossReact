...
| 8 | semantic audit のスケールアップ＋単点修正 | パイプライン＋JSON単点 | M | **✅seed202607サンプル200枚（stub100+clean100）は続き144で全数監査完了**（詳細`docs/_semantic_audit_scaleup2_triage.txt`）。**✅続き145でstub群スケールアップ第1弾＝新規200枚（seed202608）完走**（母集団2,401枚から既監査100枚を除外した候補2,301枚から新規サンプリング・findings242件・詳細`docs/_semantic_audit_stub_round2_triage.txt`）。**本ラウンドからCodex CLI併用体制**＝`claude -p`はユーザー指示でトークン節約のため停止し残りはCodex（ChatGPT無料プラン・`codex exec --json`）で完走＝実行コストゼロで継続可能と確認済み。**次の一手＝stub群残り約2,101枚への第2弾**（`manifest.json`の`picked`を除外リストとして新シードでサンプリング→Codex主体で実行→Claudeは精査のみ、の運用を継続）。意味判定が割れるもの・機構が要るものは Opusタスク12 へ |
...
- **🆕 セッション（2026-07-16・続き145・Sonnet 5・PLAN §3 Sonnetタスク8＝semantic auditスケールアップ・stub群第1弾＋Codex CLI併用体制の確立）**
  ...
  - **🌟ヘッドライン＝機械検索で確定した系統バグ**：LLM findingsに繰り返し現れた「STUB→CONDITIONAL(IS_MY_TURN)→ENERGY_CHARGE(DECK_CARD)」という誤り方（原文「支払ってもよい。そうした場合、それ〔対象化済みカード〕をエナゾーンに置く」の"それ"が無視される）を、**effects JSON全体からこの形状で機械的に全数検索し8件を確定**（`WXDi-P05-073-BURST`/`WX24-P4-048-E2`/`WX25-P2-026-E1`/`WX26-CP1-086-BURST`/`WXK05-027-E2`/`WXK05-070-E1`/`WXK10-048-BURST`/`WDK08-Y11-BURST`）。STUB idは5種バラバラだが原文は全件同一構文＝**任意コストイディオムのintercept実装（`effectExecutor`側）の共通バグと推定**。8件を単一パッチで一括是正できる可能性が高い。**Opusタスク12(xxviii)へ登録**。詳細`docs/_semantic_audit_stub_round2_triage.txt`。
  - ほかの新規クラスタ＝STUB id/パラメータ意味不一致20件（stub群特有）／timing取り違え10件超／GRANT_LRIG_ABILITY機構欠落（続き144(xxvii)と同型）／duration誤り10件超／owner取り違え10件超／条件節欠落40件超（最多）。
