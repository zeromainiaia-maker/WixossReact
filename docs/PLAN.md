...
| 12 | Sonnet 発見バグの修正（常設） | 可変 | 可変 | ◎**残1件＝続き70の多段SEQUENCE盤面差分見逃し**（設計要・M）。他5件は✅続き75で消化 |
...
- **🆕 セッション（2026-07-12・続き74・Sonnet 5・§3 Sonnetタスク1 §7実機検証の継続＝R42②とON_TARGETED③で実バグを2件発見）**＝**(1) R42②**＝PLAN §7「ON_CHARM_TO_TRASH」残②「バトルバニッシュでhostが離脱したとき」を`charmToTrashBattle`シナリオで検証。❌FAIL（2回連続再現）＝戦闘（アタックの力比べ）でチャーム付きシグニがバニッシュされると、ground truth（チャームが正しくguest.trashへ移動）は正常なのに watcher（WX16-Re05のON_CHARM_TO_TRASH）が一度も発火しない。原因＝`resolvePendingSigniBattleFor`（`BattleScreen.tsx:6344`）が独自のトリガーリストを構築し`collectCharmToTrashTriggers`の呼び出し元`collectBoardDiffTriggers`を一切呼ばない。**(2) ON_TARGETED③**＝usageLimit《ターン1回》を`ontargetedUsageLimit`シナリオで検証。❌FAIL（2回連続再現）＝同一ターン内に2回対象化すると2回目も発火してしまう。原因＝`collectTargetedTriggers`（`src/engine/triggerCollect.ts:41-89`）が他コレクターと違い`usedOncePerTurnIds`を返さず`actions_done`が更新されない。**両方とも修正方針まで特定した状態でOpusタスク12へ登録**（詳細VERIFY_BROWSER.md/BUGFIXES.md）。両シナリオとも`order`配列には追加せず（FAIL）。**変更範囲＝`scripts/verifyBattleDrive.mjs`のみ＝engine/parser不変のため typecheck のみ確認・gates再実行不要**。
  - **前回（続き73・Sonnet）＝ON_LRIG_GROW②を2枚とも検証完了**（WXDi-P13-047/WXDi-P03-046・turnOwnerゲート未実装を発見しOpus登録）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3 Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY内側parse**（WX24-P1-017／WX25-P3-038）。**(B)Opus＝Opusタスク12（最優先候補・観測結果が5件に増加）＝続き70の「多段インタラクションSEQUENCE途中ラウンドの盤面差分トリガー見逃し」バグ**（BUGFIXES/PLAN§7 R30参照）**に加え、続き72の2件（WXDi-P11-040 excludeSelf未実装・WX25-P2-055 owner誤り）・続き73の1件（WXDi-P13-047 turnOwnerゲート未実装）・続き74の2件（ON_CHARM_TO_TRASHのバトルbanish経路未配線・ON_TARGETEDのusageLimit未機能＝両方とも修正方針を特定済み）も同タスクへ合流**。**(C)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め＋採用**（続き71で整理済みのパターンA〜Fから着手）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②・他多数＝ON_TARGETED①③系列・ON_LRIG_GROW②・R42②は完了）。**(E)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（verifyBattleDriveバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）＝**高シグナル欠落 1557枚（2026-07-11 続き71・履歴 …→1567→1566→1563→1558→1557・続き63 DEPLOY_RESTRICT実装で5枚・続き67 WXEX2-25-E3 lrig相対付与で1枚・続き68 デッキ相対SEARCH3枚・続き69 引用内CHOOSE 26枚で5枚・続き71 CHOOSE平坦化復元WXK10-013で1枚解消・明細 `docs/_vocab_census.txt`）**。この数字から増えたら回帰（スクリプトが exit 1）。JSON手パッチでフィルタ語彙を足せば自然に減る＝減ったら `BASELINE_HIGH` とここを実数更新。DSLに新語彙を足したらキー表（PATTERNS）にも追加する。状態系の残（凍結13・ダウン/アップ38）はコスト節/条件/CONT型（別パス・§6.3）。**消化の入口は `npm run census:clusters`＝文型テンプレのクラスタ表（`docs/_census_clusters.txt`・枚数順）から系統バッチを選び、parser規則→`npm run build:effects`→`node scripts/heldReview.mjs` で署名グループごとに一括採用する（続き23確立・手順詳細は §5c）。**
...
- **ON_LRIG_GROW（ルリグがグロウしたとき）**：5枚。✅発火自体は実UI検証済み（`WXDi-P03-039`・CPUセンターグロウも`cpugrow`で確認）。**②相手のグロウでany_opp発火する経路＝✅続き73（Sonnet）で2枚とも検証完了**＝WXDi-P13-047（`lrigGrowAnyOpp`・2回連続PASS・**原文「あなたのターンの間」のturnOwnerゲートがJSON側に未実装という疑いを発見**＝guest自身のターン中のグロウでも発火する過剰発火の疑い＝修正せずOpusタスク12へ登録）／WXDi-P03-046（`lrigGrowAnyOppP03046`・2回連続PASS・SELECT_TARGET経由だがR38/R43/R46/R39系統のresume経路取りこぼしバグには非該当と確認＝原因アクション自体は対話不要でwatcher側の解決は独立した新規interactionのため）。詳細VERIFY_BROWSER.md。**残＝未検証**＝③any_opp トリガーがグロウ先ルリグの【出】より先に解決される・④《ターン1回》が2回目グロウで再発火しない。⚠**アシストルリグのグロウ経路は未配線（センターグロウのみ）＝follow-up**。
- **ON_TARGETED（対象になったとき）**：AUTO（14枚）。✅発火自体は実UI検証済み（`WXDi-P03-067`）。**①個別確認＝続き64（Sonnet）でWXDi-P02-043を追加検証しPASS**（`verifyBattleDrive.mjs ontargeted2`＝`ontargeted`と同一配線で正しく発火。5回中4回PASS・軽微なタイミングフレークは既存シナリオと同型で engine 起因ではない）。`order`追加済み。**①残る3枚＝✅続き72（Sonnet）で全数検証完了**（`ontargeted3`=WXDi-P11-040／`ontargeted4`=WXDi-D09-H14／`ontargeted5`=WX25-P2-055・3件とも単体PASS・`order`追加済み）。検証中に**2件の実データ疑義を発見**＝(a)WXDi-P11-040のGRANT_KEYWORDにexcludeSelf相当のフィルタが無く原文「他のシグニ」を無視して自分自身に付与、(b)WX25-P2-055のREMOVE_ABILITIES target.ownerが`'opponent'`だが原文は自己参照（本来`'self'`のはず）＝**修正せずOpusタスク12へ登録**（詳細 VERIFY_BROWSER.md）。**③usageLimit＝❌続き74（Sonnet）で実バグを発見（未修正・Opus引き継ぎ）**＝`verifyBattleDrive.mjs ontargetedUsageLimit`（2回連続再現）。同一ターン内に2回対象化すると2回目も発火してしまう＝`collectTargetedTriggers`（`src/engine/triggerCollect.ts:41-89`）が他コレクターと違い発火時の`usedOncePerTurnIds`を返さず`actions_done`が更新されないことが根本原因（コード読解で確定・修正方針も特定済み）＝Opusタスク12へ登録。**残＝未検証**＝②turnOwner:opponent ゲート。⚠**forced単一対象（pending無しで自動解決される対象取り）経路は未発火＝follow-up**。
...
- **ON_CHARM_TO_TRASH（R42）**：WX16-Re05。**①実機PASS＝✅続き64（Sonnet）で確認**＝続き61のcollectBoardDiffTriggers統合がcollectCharmToTrashTriggersも一緒にカバーしていたため追加修正不要だった。`verifyBattleDrive.mjs charmToTrash`（WX19-023→WX16-Re05・banish resume経路でチャームがguest.trashへ→watcherが対戦相手シグニに-4000）でPASS。`order`配列に追加済み。**②バトルバニッシュ経路＝❌続き74（Sonnet）で実バグを発見（未修正・Opus引き継ぎ）**＝`verifyBattleDrive.mjs charmToTrashBattle`（2回連続再現）。戦闘でチャーム付きシグニが力比べに負けてバニッシュされる（ground truth正常＝チャームは正しくguest.trashへ）が、watcherが一度も発火しない。原因＝`resolvePendingSigniBattleFor`（バトル解決）が独自のトリガーリストを構築し`collectCharmToTrashTriggers`を一切呼ばない＝効果banish経路（`collectBoardDiffTriggers`）のみ配線され戦闘banish経路が未配線。実戦で最も頻繁な経路（通常の戦闘）でON_CHARM_TO_TRASH系カードが機能しない実害の大きいバグ＝Opusタスク12へ登録。詳細VERIFY_BROWSER.md。
...
- **ON_PLAY any_opp + targetsTriggerSource（R30）**：WXK10-022-E1。続き64（Sonnet）で発見したブロック要因（WXEX2-50-E3の owner誤パース）は続き66（Opus）で是正済み＝発火経路は開通した。**①❌続き70（Sonnet）で実機検証を実施→新規バグを発見（未修正・Opus引き継ぎ＝Opusタスク12）**＝WXEX2-50【起】のSEQUENCE（①対戦相手のトラッシュのシグニを対戦相手の場に出す→②自分のトラッシュの＜凶蟲＞シグニを自分の場に出す）を実行すると、ground truth（両カードとも正しいゾーンに配置）は正しいのに watcher（WXK10-022-E1）が一度も発火せず `guest.abilities_removed` が空のまま＝コード読解で確定した原因は`handleEffectInteraction`（`BattleScreen.tsx:4097`）の`!result.done`分岐（SEQUENCE途中でまだ次のインタラクションが要る場合）が host_state/guest_state をDBへコミットしつつ`collectBoardDiffTriggers`（続き61導入）を一切呼ばない（BANISH検出のみの特例処理）ため、**途中ラウンドで完了した盤面変化（本カードのstep1配置）が、次のラウンドが`result.done===true`で完了した時点の diff ベースライン（`bs.guest_state`）に既に取り込まれてしまい diff がゼロになる**という新系統のバグ（続き58/61が修正した「1ラウンドで完了する効果のresume取りこぼし」とは別＝「2ラウンド以上インタラクションを要するSEQUENCEの途中ラウンド」が対象）。再現＝`node scripts/verifyBattleDrive.mjs onPlayAnyOpp`（既定orderには含めず・詳細は同ファイルのシナリオ直上コメント）。②turnOwner:selfゲート・③usageLimit《ターン1回》は上記バグ修正後にあらためて検証。
