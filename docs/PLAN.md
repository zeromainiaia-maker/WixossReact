...
- 🏁**セッション（2026-08-24・続き637・Opus 5）＝§5.1「実機未検証の返済」を全節（🅱／🅲／🅳）残0クローズし、そのまま §5.3 の O-47／O-48 も残0クローズ**。返した V エントリは **14件**（V-04/16/19/30/35/44/45/58/63/83/84/85/86/87）。新規実機シナリオ **35本**／**engine・parser の実バグ6件を発見・修正**／gates 全緑（**golden 2655**）。
  - 🔴**バトル処理のルール違反2件（O-47/O-48）**＝**負けた／相打ちのアタッカーがバニッシュされない**（`else` がログを出すだけ・全対戦に効く）／**バニッシュがスタックごとエナへ行く**（下のカードがトラッシュに落ちない）。🎁**進行不能バグ2件が同時に解けた**。
  - 🔴**V-19 一時レゾナのカード消失**（ターン終了の出口3つのうち通常枝だけ `lrig_deck` への戻し入れが欠落）／🔴**V-83 `TRASH{DECK_CARD}` がミル発生源を書かず誘発が恒久 no-op**／🔴**V-35 「それら」が1体しか撃てない**（帰結の count が 1 固定）。
  - **V-30 の「設計限界」は解けた**＝`turn_phase:'END'` の直接注入をやめて**フェイズボタンを実際に歩く**（§4.4 の 8g）だけで、続き566 以来の境界越えが通った。**同じ手が V-19 でも効いた。**
  - 🔑**engine のルールを直す前にベースラインを取る**（§4.4 の 8j）＝O-47 では修正後の赤を「自分が壊した／元から赤い／盤面が旧挙動前提で腐った」の3つに切り分けられた。
  - **見立てより実測**＝`DECLARE_NUMBER` 30カード（一致）／`ON_ABILITY_ACTIVATED` **2効果だけ**／`outsideMainPhase` **9効果**（見立て5）／`fromLeftFieldUnder` **4効果**／`allowRepeat` **2効果**／`targetsStored` **170アクション中の食い違いは1件**／エナ支払いサイト **15**（資料は 17/14/13）。

  - 🏁**続き637f＝§5.1 を完全に残0クローズ**＝21バッチのうち**実機経路へ直接触った4本**（第34/43「0枚選択可の UI」・第37「クラッシュ主体の限定」・第38「群割当の決定可否」・第40「`END_OF_ATTACK`」）を実機で返済（engine バグ0・新規7本）。**母集団も実測**＝`pickUpTo` 144+53／`ON_OPP_LIFE_CRASHED` 45（うち filter 31）／`selectionConstraint.groups` 19／`END_OF_ATTACK` 5。

**▶ 次の一手**＝🏁**§5.1（実機未検証の返済）は在庫0**。次は **§5.2 意味照合監査の段2 消化**（残 OPEN 734）か、**§5.3 の機構 worklist**（🆕 O-49／O-48 の follow-up・O-45／O-46）から取る。
