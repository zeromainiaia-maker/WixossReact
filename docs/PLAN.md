...
| (xcviii) | **🆕 `ON_DRAW` も CPU 経路に収集が無い＝live 13効果／13カード（2026-08-04・(lxvii) の対応表作成で観測）**＝人間経路は UP→DRAW で `collectDrawTriggers` を呼ぶが、`cpuTurnAction` は呼ばない＝**CPU がターン開始ドローをしても「カードを引いたとき」が発火しない**。⚠(lxvii) は「ターン/フェイズ境界 timing」に限定したので手つかず。**着手＝`collectCpuTurnTriggers` と同じ薄いラッパで CPU の UP→DRAW に足すだけ**（(lxvii) で ON_TURN_START を足した位置と同じ）。⚠ドロー以外の CPU 中ドロー（効果ドロー）は共有経路（`resolveStackNext`）を通るので既に発火する＝**ターン開始ドローだけの穴** |
...
- **🆕 セッション（2026-08-04・続き343・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(lxvii) 残0クローズ**＝CPU ターンのフェイズ/ターン境界トリガーを人間経路と同じ pure collector へ統一（ユーザー指示「タスク12を続ける」＝在庫から影響最大のものを選択）。golden **1354→1357**・**live JSON 変更なし**・census 1288 据置・全ゲート緑。触った層は `BattleScreen.tsx` のみ。
  ...
  - **🔴 併せて発見＝CPU 経路の【ハスターリク】が人間側でなく CPU 側を指していた**（人間経路の式 `isHost ? 'guest_state' : 'host_state'` の流用。人間経路の `isHost` は「ローカル＝ターンプレイヤー」前提だが**CPU ターンではローカルが非ターンプレイヤー**なので反転する）。人間の【ハスターリク】が CPU ターンに発動せず CPU 側の予約を消していた。⚠**触らないと本改修が CPU 状態を壊す**位置だったので同バッチで是正が必須だった。
  - **`ON_TURN_END` は人間経路と同じ `__TURN_END__` マーカー型**（1回だけ収集→スタック→return→再入で素通り）。⚠エンドフェイズの公式順（①終了時効果→②手札上限→③終了）に合わせ、CPU 側クリーンアップより**必ず前**に置いた。
  - **golden +3**＝①CPU（guest）をターンプレイヤーとして6 timing すべてが CPU 側 entries を返し `playerId` が guest・**人間側の usageLimit を消費しない** ②CPU ターンでも `any_opp` watcher が人間の場から発火し `playerId` は host ③**影響母数の固定**。⚠**配線自体は golden から叩けない**（BattleScreen 層）。
  - **新規在庫1件**＝**(xcviii)** `ON_DRAW`（live 13効果）も同じ非対称＝CPU の**ターン開始ドローだけ**「カードを引いたとき」が発火しない。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxviii)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／(xcvii)／**🆕(xcviii)**〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜343 の全件が実機UI未検証**。本件の実機確認は**①CPU ターンの終了時に「ターン終了時」【自】が実際に解決されること**（手札上限調整より前）②CPU のメインフェイズ開始時／アタックフェイズ開始時に人間側の《相手の〜フェイズ開始時》【自】が発火すること③人間の【ハスターリク】が CPU ターンに発動すること。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-04 続き343（🏁§3 タスク12(lxvii) 残0クローズ＝CPU ターンのフェイズ/ターン境界トリガー統一）後 最新値（本行が直近の正）**：golden **1354→1357**（+3＝①CPU＝guest をターンプレイヤーとした6 timing の pure collector 戻り〔entries の `playerId`・人間側 usageLimit 不消費〕②CPU ターンでの `any_opp` watcher 発火と解決主体 ③**影響母数の固定**〔`ON_TURN_END` 187／`ON_ATTACK_PHASE_START` 非 self 57／`ON_MAIN_PHASE_START` 31／`ON_TURN_START` 3／`ON_LRIG_ATTACK_STEP_START` 1／`ON_GROW_PHASE_START` 2〕）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**（ローカル248は未追跡の残置 `scripts/_dbgFresh.ts` の3件ぶん）、manual field loss 0、同型★0・held **258枚／109群 据置**（**parser も JSON も触っていない**）。**live JSON は完全不変**（**新語彙0本**・`BattleAction` も 18種のまま）＝触った層は `BattleScreen.tsx` のみ（`collectCpuTurnTriggers` 新設＋CPU 5経路への配線＋【ハスターリク】の side 是正＋当該遷移を `SET_TURN_PHASE`→`ADVANCE_TURN_WITH_STATE`）。⚠**CPU 経路の配線は golden 非カバー**＝固定できるのは pure collector の戻りと母数まで。**実機通し確認と対で締める**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-04 続き343＝🏁§3 タスク12(lxvii) 残0クローズ後）**＝golden **1357**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫**＝現存は **(lv) 残2経路／(lxvi)／(lxviii)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／(xcvii)／🆕(xcviii)** の9件。**🏁(xcii)(xcv)(lxvii) は 2026-08-04 に残0クローズ**。**(lxx)／(lxxviii)／(lxxxiii)／🏁(lxxxii) は 2026-08-02 に残0クローズ＝§3 の表から退避済み**（完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-02 整理③」）。それ以外の残0在庫の完了行原文は同ファイルの各整理節。**🏁タスク16 も残0クローズ**。**Sonnet の主力は タスク1（§7 実機検証）**＝未検証UIの単一 worklist は §7 に集約。⚠**§6.3 H と続き298〜Batch F の全件が UI 未検証**。
