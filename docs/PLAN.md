...
- **🆕 セッション（2026-07-11・続き64・Sonnet 5・§4タスク(C) §7実機検証の継続＝R42/R30/R45①/R44の4件）**＝`verifyBattleDrive.mjs` に4シナリオ追加。**①R42（ON_CHARM_TO_TRASH・WX16-Re05）＝✅PASS**（続き61のcollectBoardDiffTriggers統合がcollectCharmToTrashTriggersも一緒にカバー済みだった＝追加修正不要。副次的な学び＝「バニッシュ＝トラッシュ行き」は誤解、`banishDestination`のデフォルトは持ち主の**エナゾーン**行き）。**②R30（ON_PLAY any_opp・WXK10-022-E1）＝実機検証不能と判明**＝唯一の自然発火経路WXEX2-50が`ADD_TO_FIELD.owner`誤パース（自分/対戦相手取り違え）でブロック中＝parser修正待ち（Opusへ登録・§3タスク7）。**③R45①（ON_ACCE_ATTACH host条件・WXK05-041）＝❌FAIL＝実バグ発見**＝`execAttachAcce`のfromHand分岐（thenActionに未完結アクションを渡す設計）がresume解決側`applyDirectAction`の前提と噛み合わず無添付のまま完了＝【デコレ】キーワード全9枚が機能しない（Opusへ登録・§3タスク7）。**④R44（ON_EXCEED_COST場シグニ・WXDi-P06-078）＝✅PASS**＝ルリグ【起】エクシード１支払い時「発動順序を決めてください」モーダルにwatcherが正しく並び発火確認（3回連続）。①④は`order`追加、②③は`order`未追加（Opus修正待ち）。guest側盤面注入の稀な競合も発見し安定化（queryState確認→再PATCHリトライ）。詳細BUGFIXES最上部4件。⚠フルスイート回帰は環境側のプロセス過多（検証run由来の残留chrome/node）で著しく低速化したため中断（各シナリオは単体で複数回再現確認済み）。
  ...
  - **次の一手＝(A)Opus＝続き64発見の2バグ**（WXEX2-50 owner誤パース／`execAttachAcce` fromHand経路の実装バグ＝§3タスク7）。**(B)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(C)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(D)Sonnet＝§7実機検証の継続**（WXDi-CP02-082（R36②）・R42②（バトルバニッシュでhostが離脱したとき）・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up。R30/R45①はOpus修正待ちのため後回し）。**(E)持ち越し：WX12-008 の exceed-cost timing 誤パース是正・`handleSelectZoneForEffect`/`handleSelectSigniZoneForEffect` の収集器未配線（続き61 follow-up）・SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
