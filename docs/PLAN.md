...
| 🆕(xlvi) | **原文は「手札に加える」なのに curated が `LOOK_AND_REORDER` のままで、pick が live で実行されない系統**（カードアドバンテージが死ぬ）。✅続き218g で9効果を外科採用。✅**2026-07-29 に5波で計51効果を消化**＝第1波5（`pk` 規則を `hasRiseIcon`／`isDisona`／`hasLifeBurst`／色OR へ拡張）＋第2波12（独立SEARCHの `LOOK_PICK_CHAIN` 展開・`remainder.shuffle?`）＋第3波4（中段 `then:'deck_top'` を engine 実装）＋第4波8（表記ゆれ「N枚**を**見る」／後続文を `SEQUENCE` で引き連れ／**OR記述子 `parseOrPickDescriptor`＋新 `TargetFilter.anyOf`**）＋**第5波26＝(h) 残0**（融合規則 `LOOK_AND_REORDER+STUB(REVEAL_PICK_HAND_SHUFFLE_BOTTOM)` に **新 `parseRevealPickDescriptor`** で filter／「N枚まで」／新機構 **`handOrEnergy`** を通し、複数 pick 群3形を `LOOK_PICK_CHAIN` へ展開。同系統9効果も同時是正）。census 1476→**1457**・golden 891→**951**。<br>**残＝26効果**（(e)(f)(h) は残0。live JSON 走査で確定した実数）。分類：**(a) 動的filter 7**〔場のルリグと共通色＝`SPDi01-131-E1`／`WXDi-P02-017-E1`／`WXDi-P15-031-E1`／`WXDi-P15-005-E1`、対象と同名＝`WXK04-045-E1`、共通クラス＝`WXK08-025-E3`／`WX25-P1-041-E1`〕**(b) 複合・条件 10**〔`WXDi-P03-061-E2` は**held 群A＝採用すれば pick は入る**が「加えた枚数ぶん捨てる」が落ちる／`WX12-Re10-E1` 条件付きpick／`WX24-P4-037-E1`／`WX26-CP1-061-E1`＋`-SONG`／`WXDi-P05-015-E2` 束分け／`WXDi-P08-007-E3` REPEAT内／`WXDi-P09-066-E1`／`WXK02-001-E2`／`WX25-P3-052-E1`（エナ経由＝要偽陽性判定）〕**(c) 宣言系STUB 4**〔`PR-434-E1`／`-BURST`／`WX11-037-E1`／`WX24-P1-035-E1`〕**🆕(d) hand-or-energy 3**〔`WX24-P1-039-E2`／`WXDi-P16-086-E1`／`WX11-074-E1`。**engine 機構（`handOrEnergy`）は第5波でそろった**が parser は未着手＝3件とも今も bare `LOOK_AND_REORDER`＝**pick が丸ごと no-op**（実測）。最も安いのは `WX24-P1-039-E2`（`pk` 規則が hand-or-energy を除外しているだけ）／`WXDi-P16-086-E1` は残りが3分割／`WX11-074-E1` は「選んだ色1つにつき」＝色宣言との連動が要る〕**(g) トラップ設置併記 2**〔`WX15-083-TRAP`／`WX19-039-E1`〕。<br>**次に取るなら (d)→(a)**。詳細 BUGFIXES 続き218g／2026-07-29節（第1〜5波） |
