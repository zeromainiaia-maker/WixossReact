...
- 🏁**セッション（2026-08-28・続き698・Opus 5 単独）＝`O-127` / `O-117` / `O-113` の3件（ユーザー指示）。**
  📊**進捗3計器＝Sheet1 要対応 28→26 / 863（3.2%→3.0%）｜台帳 残 OPEN 578→575｜census 高シグナル 522（据置）**。
  gates 全緑（golden **2915→2920**）。実機＝新規4本 PASS（`b20lifeswap(zero)` / `b21end5colors` / `b21endsamecolor`）
  ＋ 🔴**`b22artshit` は FAIL のまま**（`O-113` の実機＝原因は上流で `O-131` に登録。
  `b22artsmiss` は PASS するが**発火自体が無いので空振りの PASS**）。
  - ✅**`O-127`**（「その枚数と同じ枚数」の後方参照）＝**受け皿は engine に全部在った**（`canTrash` ／
    `destLocation==='life'` で `lastProcessedCards` に trashed を入れる分岐 ／ `resolveCountRef`）。
    型コメントが**このカードを名指し**しているのに parser が一度もその形を出していなかった。母集団1枚。
  - ✅**`O-117`**（支払った色を条件にする）＝**登録票はまた実態と違った（5回連続）**。
    アーツの請求額は CSV `Cost` 由来で `effect.cost.energy` は**アーツでは読まれない**＝5色コストは飾りで、
    実害は **`FORCE_END_TURN` が無条件**（過剰実行）。真の出所は `manualEffects.ts` の古い手書き。
    🔴**実機が golden で守れない穴を2つ捕まえた**＝①カットイン窓は**別の支払いサイト**
    ②**カットイン窓は `result.forceEndTurn` を一度も読んでいなかった**（＝この札は一度も効かなかった）。
    ⇒ `applyForcedTurnEnd` へ抽出して2経路から呼ぶ形にした。
  - ⚠**`O-113`**（相手アーツの効果を「受けた」とき）＝機構と golden は入ったが**実機は未達**。
    **切り分け済み**＝live を旧・近似形へ戻しても発火しないので、原因は上流（`O-131`）。
    同時に先在バグ2件を修正＝`myStateForTrigger` の二重反転（人間側は**常に相手の場**を走査）／
    `battleCardMap.get(entry.cardNum)` が `#` 付き id を引けない。
  - 🆕**実機ドライバに `top.effectStack` 注入を追加**＝相手アーツの解決を直接作れるようにした。

**▶ 次の一手**＝**`O-131`（`ON_OPP_ARTS_USE` が実機で一度も発火していない）**。
`b22artshit` を FAIL のまま残してあるので、**`resolveStackNext` の該当ブロックに到達しているか**から確かめる
（到達していれば `collectOppArtsUseTriggers` の入力を、していなければ分岐条件を疑う）。
これを直すと `O-113` の実機観測が閉じ、`WX05-020` / `WXK11-019` / `WX16-003` がまとめて生き返る。
**Sheet1 の残り機構待ちは 3カード**＝`WX02-020`(`O-129`) / `WX05-022`(`O-125`) / `WX09-Re07`(`O-128`)。
⚠**`O-129`（118カード影響）と `O-128`（34カードの catch-all）は最後に回す。**
⚠**既存 golden／計器が旧バグを固定していないか毎回疑う**（今回も census 較正1本＋走査ガード3本＋
既存 golden 1本が旧形を固定していた＝§5-17）。
⚠**登録票は5回連続で実態と違った**＝**着手前に live と実コードを必ず1分見る。**
特に「フラグが無い」型は**そのフラグを読む側が生きているか**から、
「過小実行」と書かれている型は**本当に過小か（逆に過剰ではないか）**から見る。
...
| **O-131** | 🔴🔴**`ON_OPP_ARTS_USE` は実機で一度も発火していない（golden は緑）** | M | **2026-08-28 `O-113` の実機（`b22artshit`）で発見**。■**切り分け済み**＝live を `O-113` 以前の近似形（`activeCondition: HAS_CARD_IN_FIELD`）へ**戻しても発火しない**＝原因は `O-113` の新条件ではなく**その上流**。ゲームログには「[相手] 相手アーツ（注入）」「…をダウン」まで出ており、**アーツ自体は解決している**のに `ON_OPP_ARTS_USE` の収集結果がスタックに積まれない。■**同日に直した先在バグ2件はこれとは別**＝①`myStateForTrigger` の二重反転（`ownerIsHost` で場合分けすると常に相手の場を走査していた）②`battleCardMap.get(entry.cardNum)` は素の CardNum しか引けない（`#` 付きインスタンス id だとブロックごと素通り）。**どちらを直しても実機はまだ発火しない**ので、原因は3つ目にある。■**取り方**＝`b22artshit` を FAIL のまま残してあるので、`resolveStackNext` の該当ブロックに到達しているかを最初に確かめる（到達していれば `collectOppArtsUseTriggers` の入力を、していなければ分岐条件を疑う）。■母集団＝`ON_OPP_ARTS_USE` を持つ全効果（`WX05-020` / `WXK11-019` / `WX16-003` ほか）。 |
