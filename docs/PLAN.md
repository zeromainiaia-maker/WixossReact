...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-8`**＝アタックするかどうかの判断（PLAN の「**`S-5` の後が本命**」がこれ）。`S-6`（重みの自己調整）も**自己対戦が回るので着手可能**になった。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**画面のリファクタ＝live JSON を1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（`S-6`・`S-8`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
...
| 🔥**④b** | 🆕**§5.7 CPU の強さ `S-nn`** | **2** | 探索＋評価のゲーム AI（対戦中に LLM は呼ばない） | §5.7.1 の表 |
...
| `S-11` | **【起】に採点を接続**＝`pickCpuSigniActivated` は**ゾーン順→効果定義順で最初に撃てたもの**（盤面評価なし・コメントに明記）。`S-4` の `scoreCardUseGain` を当てる。 | 小 | 「撃てるから撃つ」が「得だから撃つ」になる。⚠`S-10` の後（採点が壊れたまま繋ぐと過剰使用が増える） |
| `S-12` | **アーツ／スペルの使用条件を利得へ**＝`pickCpuOffensiveArts` は**アタッカーが塞がれているとき・除去分類だけ**（強化・展開・ドローのアーツは永久に使われない）。分類 allowlist を先読みの利得に置き換える。 | 中 | 攻めのアーツの幅が広がる |
| `S-13` | **系列（2手）の先読み＝コンボの一般解**＝いまコンボは `planDeployBonus` 経由で**召喚の加点にしか効かない**（＝【出】コンボだけ）。`simulateEffect` の後の盤面で候補を出し直して2手目を採点すれば、作戦データ無しで「【起】→アーツ」「スペル→召喚」が拾える。 | 大 | 起動・アーツ・スペルを跨ぐコンボを CPU が打てる。⚠分岐数は着手時に実測して枝刈りを決める |
| `S-14` | **作戦データのコンボに「使い方」を持たせる**＝`CpuDeckCombo` はいま `{first, then}` のカード番号だけで**出す／起動する／アーツで撃つを書けない**。`{num, use}` の列へ（`CpuDeckPlanModal` の編集 UI も）。 | 中 | `S-13` の探索が届かない長いコンボを手で指定できる |

🔑**取る順（2026-09-19 ユーザー決定）**＝`S-9`（測定台）→ `S-10`（パワー項）→ `S-11`（【起】）→ `S-12`（アーツ／スペル）→ `S-8`（アタック判断）→ `S-13`（2手系列）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
🔴**判断ロジックを触る項目は全部 `S-9` の後**＝`evaluateBoard` は召喚・スペル・アーツ・対象選択の共通の土台なので、A/B が無いと「直ったが弱くなった」を判別できない。**登録票は [PLAN_DETAIL.md](./PLAN_DETAIL.md) の同じ ID。**
...
- 🔑**強さは CPU 同士の勝率で測る**（`S-3` 以降）。それまでは golden で「それらしい選択」を固定し、通し対戦で回帰が無いことだけを見る。
