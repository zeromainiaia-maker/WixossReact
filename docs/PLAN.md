...
- 🏁**セッション（2026-08-27・続き689・Opus 5 単独）＝Sheet1 段2 の2巡目。11効果を1バッチで消化した。**
  B10 と同じ取り方（母集団をシートに固定 → 全部読んでから「受け皿が既に engine にある形」だけを採る）。
  📊**進捗3計器＝Sheet1 要対応 49→40 / 863（5.7%→4.6%）｜台帳 残 OPEN 609→599｜census 高シグナル 532→528**。
  - 🔑**この巡の発見＝穴のほとんどは「綴り違いで表から外れる」**。`DURING_ATTACK_PHASE` は主語が前に出るだけ
    （「**このシグニは**アタックフェイズの間、」）で `^` アンカーが外れて**恒久バニッシュ耐性**になり、
    `CONJ_FIN` は「**場から**トラッシュに置き」しか持たず「**それを**トラッシュに置き」で除去が丸ごと落ちていた。
    ⇒ **新語彙より先に「兄弟枝にある綴りが自分の枝にもあるか」を見る**のが安い（§5-8′ の再実証）。
  - ✅**engine の新設は1つだけ**＝`SelectionConstraint.same:'level'`（型＋**両評価器**、レベル不明は fail-closed）。
  - 🗑**撤去2件**＝①**engine に消費地点が無い全文 STUB**（`SPELL_COST_REDUCTION_BY_SERVANT_COUNT`）が
    文全体を飲み込んでおり、採用されれば `WX10-053` は**何もしないカード**になるところだった
    ②**`WX10-053` の manual 影武者**（parser より劣化＝サーバント限定が3か所とも無く、
    コスト軽減が `PERMANENT` で以後のスペル全部に効く嘘の効果）。
  - 🔴**動いている実装を奪わない**＝「配置し直してもよい」の regex に読点を許したら、engine 実装済みの
    専用 STUB `SIGNI_REPOSITION`（`WXEX2-04` を名指し）を横取りした。範囲を元へ戻した。
  - ⚠**manual で凍っているカードは parser を直しても live に届かない**（対象29カード中5枚）。
    うち劣化は `WX10-053` だけで、残り4枚は manual 側が正しかった＝**「held に無い」は正常な仕分け結果**。

**▶ 次の一手**＝**同じ取り方で3巡目**（Sheet1 の残 findings は **28件 / 20カード**）。
着手は `npm run census:cards -- --sheet 1 --list` → 該当カードの原文・live・findings を**まとめて読む**。
⚠**残りは「受け皿が無い」側が濃い**＝機構待ちは §5.3 の `O-105`〜`O-113`。
実機の腐り返済なら §5.1 の `V-89`／`V-90`（`deckshufflespell` も同型＝`V-90` に追記済み）。
---
