...
| (lxxxiii) | **第6波まで一部消化（2026-08-02）**＝action/host 対象5効果（任意コスト後の同一対象ランサー2／レイヤーコピー／赤シグニ下敷き／選択のみ）と trigger 主語2効果を **live JSON まで**実評価。再測＝語彙167、public `excludeSelf` あり139／なし28、既知誤検出5を除く**実装課題残23**。trigger 2件は manual 宣言だけではカード単位 `PRESERVE_STATUSES` に遮断されたため live を外科パッチし、live 生値の collector E2E と `build:effects` 2巡保持で固定。`WDK06-C17-E1`／`WX25-CP1-089-E1` は対象保持を含む STUB 本体、アクセ2件は `ATTACH_ACCE` 未到達のため defer。【常】系 STUB は第7波。全経路・実測は BUGFIXES 第6波節。 |
