...
| (viii) | checkAllEffects 精査で残った複合バグ（続き90）。**✅WX25-CP1-062**（欠落第1能力を MANUAL 復元・heldReview 採用・census 2215→2214）・**✅WX17-028**（TRANSFER_TO_DECK(TRASH) に optional を engine 対応＋JSON付与・golden 332）＝続き137（Opus）。残＝WX26-CP1-048（出自条件欠落+owner混同）・WXDi-P10-034（次メインフェイズ遅延+分岐）・WX16-038（アイコン条件）・WX16-070（値のCHOOSE化）・WDK16-13/WXK08-033（2条件ADD_TO_FIELD＋登録者数条件）。大半 parser混線でMANUAL上書き＋heldReview採用が要る |
...
> **⚠2026-07-15（続き134）の棚卸し結果＝在庫がほぼ枯渇している**。続き133 で BEHAVIOR_AUDIT 高シグナル22の精査が「新規バグ0件」で終わったことにより、**タスク1（主力在庫）・4 が同時に枯れ、6 は元からブロック**。**いま Sonnet が即着手できるのは 9（新設・最優先）／8（`claude -p` 上限リセット済み）／3 の3本だけ**。9 と 8 を消化し切ると Sonnet 側は本当に空になる＝**その時点で Opus のタスク12（在庫）と 1〜6（新語彙）を進めないと Sonnet に流す観測対象が生まれない**。

| # | タスク | 種別 | 規模 | 残っている内容 |
|---|---|---|---|---|
| **9** | **🆕 PARTIAL 刻印 151件のトリアージ（最優先・唯一の未開封在庫）** | 計器読み＋分類（parser/engine 非変更） | M | **`docs/_partial_report.txt`（`npm run build:effects` の副産物・続き38 の `markSilentFallback` 刻印）が一度もトリアージされていない**。内訳＝**IS_MY_TURN化 127／multi-dest分割 11／リコレクト分割 8／ON_DISCARDED_AS_COST 4／ON_OPP_POWER_DECREASED 2／ON_CARD_MILLED_FROM_DECK 1**。IS_MY_TURN化＝parser が条件抽出に失敗して**偽になり得る条件を無言で常時trueへ落とした**もの（例：「この方法でカードを１枚以上捨てた場合、」「その後、あなたのライフクロスが1枚以下の場合、」）＝**過剰実行バグの母集団**。**やること**＝151件を原文照合して「(a)実害あり＝条件が実際に偽になり得る／(b)慣例で無害＝§9-9 の LIFE_BURST内 IS_MY_TURN 等の意図的常時true／(c)機構待ち」に3分類し、(a) を文型クラスタでまとめて **Opusタスク12 の在庫へ登録**する。**parser 修正自体は Opus**（ガードレール②）。分類の一次成果物は `docs/_partial_triage.txt`（新規）に置く |
| 1 | **§7 実機検証の横展開（🆕2026-07-15＝一部復活）** | 検証（driver シナリオ追加のみ） | S×件数 | **✅続き135（Opus）の在庫消化で3項目のブロックが解けた＝いま取れる**：(a)**`trashCounterOpp`**（WX14-040-E3・タスク12(iv) 修正済み＝`node scripts/verifyBattleDrive.mjs trashCounterOpp` を再実行し PASS へ反転するか確認→既定 order へ）(b)**ON_LRIG_GROW④の usageLimit**（タスク12(vi-5) 修正済み＝ゲット・グロウでの2回目グロウで《ターン1回》が再発火しないこと。⚠続き132 は「driver で lrigTop が変化せず再現不能」と記録＝シナリオ側の作り直しが要る）(c)**R37③ ON_SIGNI_POWER_ZERO_OR_LESS の usageLimit**（同上）。**残ブロック**＝(xiii) B4引用付与（→Opusタスク17 で解消見込み）／(xix) WX04-005-E3／(xx) ON_TARGETED forced／(xxi) oppDraw発生源。WX22-001-E3（クラフトトークン残・§6.4）も引き続き可 |
| 3 | driver バッチ実行の状態汚染 | scripts（engine/JSON 非依存） | M | ⏳部分完了（続き77・105）＝ホワイトリスト方式リセット＋シナリオ毎 `page.reload()` で PASS43/FAIL6 まで改善。**🆕2026-07-15（続き135・Opus）の全数実行＝71定義中の既定 order 全実行で 65 PASS / 5 FAIL**：(a)**バッチ状態汚染が3件残存**＝`blockDrawByEffect`／`exileHandBlind`／`delayedAttackTrigger`（**個別実行では3件ともPASS**・手札が 6→9 や 12 になるなど前シナリオの残留が明白）＝リセットのホワイトリストに漏れている状態フィールドがある。(b)`oppDraw` 単独FAIL（既知・CPU挙動依存）。(c)**🆕`lrigGrowAnyOppP03046` が FRESH=1 でも FAIL＝CPUがグロウ判断に至らない**（`[CPU] グロウ` ログ自体が出ない＝`growTargetId` の候補探索で落ちている。engine 配線は健全＝同経路の `lrigGrowAnyOpp`/`cpugrow`/`cpugrowblocked` は PASS）。**続き120 の GROW_COST_REDUCTION per-count 是正でCPUのコスト支払い可否が変わった疑い＝シナリオ spec（guest energy）側の見直しが要る**。現在シナリオは**81定義／71既定実行**（続き135 で `onPlayUsageLimit` を追加） |
...
| 8 | semantic audit のスケールアップ＋単点修正 | パイプライン＋JSON単点 | M | ⏳部分完了（続き88・102）＝200枚サンプル中119枚精査・単点是正21件。**`claude -p` 上限は続き102（2026-07-11）から4日経過＝リセット済みのはず＝即再開可能**。残り8バッチ（81枚）＋stub群 約2,180枚（findings/manifest は `scripts/archive/scratchpad/semantic_audit_101/`）。意味判定が割れるもの・機構が要るものは Opusタスク12 へ |
...
**⚠いまサイクルが Opus 側で詰まっている**＝Sonnet が積んだ在庫（Opusタスク12 の (i)〜(xxi)＋🆕2件＝**約20件**）が消化されないまま溜まり、その結果 Sonnet 側の 1・4・6 が同時に待ちに入った。**Opus は「新語彙を開く」より先に、まず タスク12 の在庫消化を優先する**（＝Sonnet の在庫を再生産する行為でもある）。
...
- **🆕 セッション（2026-07-15・続き137・Opus 4.8・PLAN §3 Opusタスク12＝在庫消化）**
  ...
  - **✅ タスク12(viii)＝WX25-CP1-062 の欠落第1能力を復元**＝parser が2能力を1効果に混線させ欠落していた「ターン終了時 手札を捨てて＜ブルアカ＞+4000」を `manualEffects.ts` の完全 MANUAL 上書きで復元（build:effects→`heldReview.mjs --adopt`）。逆翻訳が原文2能力と一致・**census 2215→2214**。副次で build:effects が保留中の parser 純改善13枚を adopted_gain（無損失）同時採用。(viii) 残＝WX26-CP1-048/WXDi-P10-034/WX16-038/WX16-070/WX17-028/WDK16-13/WXK08-033。
  - **✅ 検証**：全ゲート緑（typecheck／**golden 326→331**＝autoTargetedCards surface 2件＋LIMIT_ALL_FIELD 3件／smoke 全0／fuzz 全0／**census 2215→2214**／同型★0／lint 0 error）。**実機 E2E `onTargetedForcedBypass`**（WX12-010〔ON_ATTACK_SIGNI any_opp＋targetsTriggerSource -2000〕× WXDi-P03-067〔ON_TARGETED self=DRAW〕）を空手札注入の絶対値判定に作り直し **FRESH=1 で2回連続PASS**（gHand=1）→既定 order に追加。effects JSON 無変更。詳細 BUGFIXES 続き137。
  - **次の一手**：Opus＝**タスク12 の残在庫**（(vii)(viii) checkAllEffects 由来の per-card 構造修正 約8枚〔owner混同・optional/条件欠落・CHOOSE化・大半 parseStatus:AUTO＝parser修正＋census/regen 管理が要る〕／(xii) WXEX1-19 無限ループ〔resumeSelectTarget の設計非互換〕／(xxi) collectOppDrawTriggers〔発生源追跡＝§6.3級の近似〕／(iii)(xi)(🆕choice.condition) は設計判断待ち。(v) は続き106で smoke 258→1 まで消化済み・残は (xii)。(vi-4) は該当実カード0の潜在バグ）→本丸のタスク1（引用付与の内側 parse）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 331・smoke/fuzz 全0・同型★0・census 2214。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**🆕2026-07-15（続き134）の棚卸しで分担の推奨が変わった**＝**Opus は最優先で タスク12 の在庫（約20件）を消化する**（Sonnet 側の 1・4・6 がこの在庫待ちで同時にブロックされている＝サイクルが Opus で詰まっている）。**Sonnet は 🆕タスク9（PARTIAL 刻印 151件のトリアージ・唯一の未開封在庫）→ タスク8（semantic audit・`claude -p` 上限リセット済み）→ タスク3（driver）** の順。旧推奨だった タスク1（§7横展開）・タスク4（キュー）は**枯渇したので取らない**（理由は §3 Sonnet 表）。
