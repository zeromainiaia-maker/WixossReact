...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が指示と検証＋codex-work が実装・5バッチ連投）＝§6.3 H 節を残 `UNKNOWN` 0 で完全クローズ（H1 メルト・ファクト／H2 夢限 -Q-／H3 未知の邂逅／H4 マユB面の配置数制限）＋I′ ガード追加《無》族11効果**（golden **1101→1109**・census **1386 据置**・smoke **10679** 全0・SKIP0・fuzz 全0・lint 0 errors/**228→230 warnings**・同型★0（5986枚・265群）・held **286→288枚/106署名 据置**・manual field loss 0。各バッチとも gates 独立3回で全緑）
  - **⚠簿記事故＝この5バッチは実装時に §4 の入れ替えが6コミット分まるごと漏れていた**（BUGFIXES と §6.3 は各コミットで更新済みだったので、**PLAN §4 だけが task16 `cost.underSelfTrash` 時点で止まっていた**）。cold start で §4 しか読まない次セッションは「golden 1101・H は未実装」という**存在しない現在地**を掴む。⇒ 実装とは別に `/baton` を1回回して6コミット分をまとめて締めた（本要約）。**機構バッチを連投するときは最後に必ず §4 を締める。**
  - **H4 で見つけた地雷（最重要・横展開する）＝UNKNOWN を実装するときは fresh parser が既に何を吐いているかを先に測る。** `WXDi-P13-003B-E2` は fresh が主語を見ない `STUB DEPLOY_RESTRICT` を生成し、engine ハンドラは cap を `otherState` にしか書かないため、**live の `UNKNOWN` を素朴に採用すると「対戦相手は1体までしか出せない」へ意味が反転した過剰実行になる**＝UNKNOWN が安全側で止めていただけだった。`DEPLOY_RESTRICT` を「配置数制限を含む1文」から主語（対戦相手／あなた／すべてのプレイヤー）と cap を読む形へ是正。**副産物**＝`WXK06-004-E1`「すべてのプレイヤーは２体まで」が相手にしか cap を立てていなかった過小実行も両者適用へ是正。
  - **H2 で PLAN 自身の診断が誤りと判明したので訂正した**＝「`faceDown` は表示 prop のみ＝新設が要る唯一の点」は**誤診断**。「裏向きにする」の実体は**両面ルリグのB面化**で、要ったのは新 state ではなく**既存 `card_identity_overrides` の再利用＋反転 timing `ON_LRIG_FLIP` の新設**だった。**この経路がそのまま H3 のピース両面反転に効き**、前回 defer の理由（グロウ先がルリグデッキ外だと通せない）も `executeGrow` の optional 引数（instanceId／baseState／freeCost／consumeGrowAction）で解消＝**H2→H3 は機構の再利用で連鎖的に落ちた**。
  - **⚠Claude 検証での是正3件**（いずれもゲートが検査しない次元）＝①**H1 の新しいウィルス除去サイトが `opp_virus_removed_just` を立てておらず** `ON_OPP_VIRUS_REMOVED`／`ON_OPP_VIRUS_CHANGED` が落ちていた（実データ該当4効果＝`WD19-009-E1`／`WX21-045-E1`／`WX21-068-E1`／`WX21-030-E1`）＝既存の除去6サイトが守っている規約から外れていた。②**I′ で codex が同じ外科パッチを `buildEffectsJson` と `mergeManualEffects` の2箇所に置いていた**ので後者を撤去（JSON 側に値があり挙動不変）。③codex の当初報告「意味差は2効果だけ」は誤りで、effectId 単位の全数比較では**4効果**が正。
  - **新たに開いた課題（登録済み）**＝①**🆕 §6.4 へ追加＝`signi_deploy_count_limit` が効くのは通常召喚UI／召喚ゾーンモーダル／CPU召喚の3箇所だけで、engine 側の効果配置（`execAddToField` ほか）は cap も `signi_deploy_power_limit` も見ずにすり抜ける**（該当7効果に共通する既存の穴。塞ぐと7効果が同時に厳しくなる面配線なので単独バッチが要る）。②`executeSigniOnPlay` の `beatZones` が「場ゾーン index」と「トラッシュ index」で同じ集合を共有（併記カードは実データ0件で実害なしだが、出たら無言 abort）。③ガード付与ストアの走査が `activeCondition` を評価しない。④`WX20-Re20` は honest defer 継続（選択数依存コスト＋能力なし filter＋任意複数配置＋同一instance群のターン終了時 trash が一体で要る）。
  - **次の一手**＝**Opus：①エクシードルリグ6件の本体不一致**（`WX24-P4-011-E2` 次回アタック時アップ→即時UP／`WX24-P4-014-E2` 差分ドロー→固定1枚／`WX24-P4-015-E2` 任意ライフクラッシュ＋動的対象数→強制＋固定1体／`WX24-P4-017-E2` スペル＋青シグニ各1枚→青シグニのみ／`WX24-P4-018-E2` 相手の手札3枚支払い回避→無条件バニッシュ／`WX25-P3-028-E2` リフレッシュ不可の誤分類＋3回の選択脱落。詳細 BUGFIXES 2026-07-29 §「live 原文照合」節）**②`WX25-P1-056-E1`**（`leaveReplaceBanish` の no-op＝共通離場フック `applyEffectLeaveLrigAbilitySubstitute` で閉じられる見込み。任意選択とバニッシュ先処理の追加が要る）**③「コストの合計」族の STUB/UNKNOWN 14件**（本体が無い＝実装すれば既存 filter がそのまま効く）**④`WX11-043-E1` の `crossOnly` 欠落⑤タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る＝停止理由が陳腐化している）**⑥`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑦🆕 §6.4 の配置数制限すり抜け7効果**／(xxix) 残4・(lv) 残2経路・(lx) 2件・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけで、機構バッチのたびに積み上がっている**＝🆕**今回の5バッチが持ち込んだUI4件**＝(a) ガード追加《無》の N枚徴収（警告表示・エナ不足ならガード不成立・確定時 N枚徴収／UI を迂回する経路でも不足なら成立させないこと）(b) `WDK14-013-E1` のトラッシュ＜悪魔＞候補ピッカー（**候補が必要数を超えるときだけ出る**／省略時は従来の先頭自動選択）(c) メルト・ファクトの**支払い前**ウィルス除去UI（`SpellCastModal`＝除去数で《黒×2》軽減と本体択の上限1→2が変わる／選択変更でエナ選択がクリアされること）(d) 夢限 -Q- の反転と未知の邂逅の無料グロウ（B面能力・Limit 9 への切替・同ターンの通常グロウが封じられること）／既存＝【起】「このシグニの下からN枚トラッシュ」の選択UI・ガード代替（手札1枚捨て）・ホログラフ置換の3枚並べ替え→トップ公開・(xxix) の各支払いUI・レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓ほか。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 §6.3 H 節クローズ後 最新値（本行を直近の正とする）**：census **1386 据置**（`BASELINE_HIGH=1386`）、golden **1101→1109**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430・効果実行 7982手／distinct 2680種）、lint 0 errors/**230 warnings**（H3/H4 の新規ヘルパー2本ぶん +2）、同型★0（5986枚・265群）、held **286→288枚/106署名 据置**（+2＝`WXDi-P05-005`／`WXDi-P11-010B`。**今回の変更に由来しない未再生成ドリフトが顕在化しただけ**＝BUGFIXES H4 節⑧）、manual field loss 0。新語彙＝timing `ON_LRIG_FLIP`（B面反転）・条件 `CENTER_LRIG_NOT_GROWN_THIS_TURN`・STUB `MUGEN_Q_RESET_AND_FLIP`・state `opp_guard_extra_colorless_this_turn`／`signi_deploy_count_limit_next_turn`／`pending_spell.pre_use_virus_removed`・`ChooseAction.preUseVirusChoose`。**`GUARD_EXTRA_COST_BY_OPP`／`OPP_GUARD_COST_COLORLESS` は boolean → `count`（省略時1）へ意味拡張**（既存6 CONTINUOUS は非回帰）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1109**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1386**（回帰ゲート）・held **288枚/106署名**（2026-07-30 §6.3 H 節クローズ後に実測）。
>
