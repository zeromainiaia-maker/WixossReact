...
- **🆕 セッション（2026-07-31・Opus 5〔Claude 検証〕＋codex-work〔実装〕）＝タスク12(lxi) 第5波・第6波を「少しずつ投げる」分担で消化（残7→5）**（golden **1145→1150**・census **1367→1366**（`BASELINE_HIGH=1366`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **250枚 据置**〔署名 97 据置〕・manual field loss 0。ゲート全緑／live per-effect 差分は**各波とも changed 1 / added 0 / removed 0**／`build:effects`・`regen` とも冪等）
  - **運用＝「codex 実装／Claude 検証」を 1波1効果で回した**（`docs/CODEX_GUIDE.md` 準拠。投入前に Claude が現状 JSON・engine 実装・parser を実測して指示書に雛形と行番号を書く → codex 実装 → Claude が独立ゲート＋per-effect diff＋原文照合）。**1バッチ1効果でも成立する**＝機構が新しい波では、件数よりも「罠を行番号で先出しできているか」が品質を決める。
  - **第5波＝`WXK06-047-E1`**（残7→6）。原文「対戦相手は、自分のシグニ１体を対象とし、それをデッキの一番上に置かないかぎり手札を１枚デッキの一番下に置く」が **二重に誤っていた**＝①回避クローズが消えている②帰結が「相手の手札1枚」ではなく「相手のシグニ1体」に化けている（回避コスト側の対象宣言と帰結の移動先が融合）＝**相手シグニの無条件デッキ送り**。`OPPONENT_PAY_OPTIONAL` の兄弟語彙 **`opponentSigniToDeckTop`** を1本足して標準ペアへ是正（engine 新機構0）。
  - **第6波＝`WXDi-P06-023-E2`**（残6→5）。「このターン終了時、対戦相手が《無》を支払うか手札を１枚捨てないかぎり、このカードをルリグデッキに戻す」が `UNKNOWN` の**完全 no-op**＝アシストルリグの主機能が丸ごと死んでいた。**ゲートを設置の外に出すのではなく、`INSTALL_DELAYED_TRIGGER{ON_TURN_END}` の内側に標準ペアを入れる**形で解決（設置は無条件・回避判断はターン終了時）。engine は遅延トリガーへ `sourceCardNum` を焼き込み**3収集経路で復元**＋`RETURN_SELF_ARTS_TO_LRIG_DECK` をアシストルリグゾーンへ拡張。
  - **⚠教訓④＝「片肺だから採用しない」の前に、その片肺が本バッチ由来か既存の系統穴かを数える**。codex は第6波の実装をゲート全緑で完成させたのに「CPU ターン終了経路が `ON_TURN_END` を収集しない」を理由に**全撤回して停止**した。実測すると **CPU の未収集は既存 188効果／183カードに共通**＝採用しなければ PvP でも死んだまま・採用すれば PvP は原文どおり動く＝**採用が厳密に優位**だった。停止条件を訂正して再投入し着地。
  - **⚠教訓⑤＝発生源の復元は「その値を読む側」を全部洗う**。codex は「`sourceCardNum` を読む分岐は0件」と申告したが、依存は **`entry.cardNum` の“カード種別”を読むアーツ使用ゲート**（`BattleScreen.tsx:4291`）に宿っていた＝**アーツ由来の遅延トリガー6枚が発火のたびに `ON_ARTS_USE` を二重発火**する。`effectId` で弁別して Claude が是正（**BattleScreen は golden から叩けない＝計器に一切映らない**）。
  - **次の一手**＝**Opus：①🆕 (lxvii) CPU ターン終了時の `ON_TURN_END` 未収集**（`ON_TURN_END` 188効果/183カードが CPU ターンで全不発。着手するならまず**全 timing の「人間経路／CPU経路」対応表**を作る＝1 timing ずつ潰すと同じ穴を繰り返す）**②「コストの合計」族の STUB/UNKNOWN 14件③`WX11-043-E1` の `crossOnly` 欠落④タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑤`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑥§6.4 の配置数制限すり抜け7効果⑦§6.3 I＝`WX25-P3-028-E2` の3機構⑧(lxvi) の据置2枚**／(lxi) 残5・(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕第5波の相手側 CHOOSE「自分のシグニをデッキの一番上に置く」枝／🆕第6波のターン終了時ダイアログ（PvP のみ）／(lxi) 第3波の相手側 CHOOSE 4択目／(lxv) の条件つき任意コスト 36枚／(lxiv) の対象ピッカー前置 61枚ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxi) 第5波・第6波後 最新値（本行を直近の正とする）**：census **1367→1366**（`BASELINE_HIGH=1366`）、golden **1145→1150**（第5波 +2／第6波 +3）、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **250枚／署名グループ 97 据置**、manual field loss 0。live per-effect 差分は**各波とも changed 1 / added 0 / removed 0**（`WXK06-047-E1`／`WXDi-P06-023-E2`）、`build:effects`・`regen` とも冪等。**新語彙2本**＝`StubAction.opponentSigniToDeckTop`（第5波）／`InstallDelayedTriggerAction.sourceCardNum`（第6波）。engine 拡張＝`RETURN_SELF_ARTS_TO_LRIG_DECK` のアシストルリグゾーン対応・遅延トリガー3収集経路の発生源復元・`ON_ARTS_USE` 二重発火の抑止（`BattleScreen.tsx:4291`）。
