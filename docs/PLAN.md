...
- **🆕 セッション（2026-07-12・続き72・Sonnet 5・§3 Sonnetタスク1 §7実機検証の継続＝ON_TARGETED残3枚を全数検証完了）**＝`verifyBattleDrive.mjs` に `ontargeted3`（WXDi-P11-040）/`ontargeted4`（WXDi-D09-H14）/`ontargeted5`（WX25-P2-055）を新設。**3件とも単体実行でPASS**（ontargeted3のみバッチ実行では既知の状態汚染でFAILしたが単体では安定再現・`order`配列に3件とも追加済み）。**検証中に実データ疑義を2件発見**＝(a)WXDi-P11-040のGRANT_KEYWORD（あなたの「他の」シグニへ【シャドウ】付与）にexcludeSelf相当のフィルタが無く、他allyがいない盤面では自分自身に付与されてしまう（原文と不一致）。(b)WX25-P2-055のREMOVE_ABILITIES target.ownerが`'opponent'`だが原文「このシグニは【常】能力を失う」は自己参照＝本来`'self'`のはず（JSON通りに動作＝host側が能力喪失することを実機確認）。**いずれも修正はせずOpusタスク12へ登録**（PLAN §3・§7・詳細 VERIFY_BROWSER.md）。**変更範囲＝`scripts/verifyBattleDrive.mjs`のみ（シナリオ追加）＝engine/parser不変のため typecheck のみ確認・gates再実行不要**。
  - **前回（続き71・Sonnet）＝CHOOSE平坦化復元held全数機械分類**（54枚精査→純改善1枚採用・残53枚をパターンA〜FでOpusタスク10へ整理）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3 Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY内側parse**（WX24-P1-017／WX25-P3-038）。**(B)Opus＝Opusタスク12（最優先候補）＝続き70で発見した「多段インタラクションSEQUENCE途中ラウンドの盤面差分トリガー見逃し」バグの根本修正**（`handleEffectInteraction`の`!result.done`分岐でも盤面差分トリガーを評価する設計＝BUGFIXES/PLAN§7 R30参照）に加え、続き72で新規発見の2件（WXDi-P11-040 excludeSelf未実装・WX25-P2-055 owner誤り）も同タスクへ合流。**(C)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め＋採用**（続き71で整理済みのパターンA〜Fから着手）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（R42②・ON_COIN_PAID/ON_LRIG_GROW③④・ON_TARGETED②③・他多数＝ON_TARGETED①系列は続き72で完了）。**(E)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（verifyBattleDriveバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
