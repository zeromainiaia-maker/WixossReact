...
| **O-28** | 🆕**`GRANT_KEYWORD.keyword` にゴミが入っているクラス（全数走査済み）** | 29 綴り／うち即着手可 5効果 | 2026-08-15 続き493 で live を機械走査（正式キーワード名でない `keyword` 値を全数列挙）。**`hasKeyword` は正式名でしか照合しないので、文が入っている綴りは一度も効かない無言 no-op**（続き490 で1件だけ見つけた形の母集団）。🏁**即着手可＝「《無》×N を支払わないかぎりアタックできない」5効果**（`WX11-012-E1`／`WX24-P1-041-E2`／`WX24-P4-001-E1`／`WXDi-P03-036-E1`／`WXDi-P05-033-E2`）＝続き490 で作った `SigniAttackBan.unlessPayColorless`／`signiAttackBanCost` に**そのまま載る**。ほか丸ごと文が入っているもの＝`WX25-P2-071-E1`（「【常】：このシグニが対戦相手の効果によって場を離れる場合、代わりにこの能力を失う。そうした場合、このシグニをダウンする。」）／`WXEX2-71-E3`（正面以外へのアタック）／`WX24-P1-064-E1`（手札2枚以下で【アサシン】）／`WXDi-P05-068-E1`（アタックが無効にされない）。⚠**「正式名でない綴り＝すべてバグ」ではない**＝`チアガール`（32）・`トラップアイコン`（7）・`シュート`（7）等は engine が同じ綴りで消費する**正規の内部キーワード**なので、着手前に必ず消費地点を grep する。⚠`使用条件`（5効果）は【使用条件】ヘッダの取りこぼし＝続き493 で `WXDi-P16-002-E1` だけ是正済み（残4件）。 |
