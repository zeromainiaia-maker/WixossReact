...
- **🆕 セッション（2026-08-10・続き409〜414・Opus 5）＝§6.4 を連続消化。①仕分け計器 `npm run census:stubs` を新設し A群（無言の no-op）を6件実装（12→7）②UNKNOWN の第1バッチ3効果を構造化（43→40ノード・census 880→877）**。
...
- **UNKNOWN（部分未実装・逆翻訳に `【未実装/UNKNOWN】` として露出）**：**🆕2026-08-10 続き414 に live JSON を全数走査して数え直した実数＝40 ノード／38 カード**（⚠**旧記述「42箇所／31カード」は decompile シートの出現数**で、live のノード数とは別物＝**計器は live JSON を数える**）。**続き414 で 43→40**。
  ...
  - **📋 残り37ノードの内訳（続き414 実測・大きい順）**＝(a)**ライフバースト抑制 2件**（`WXEX1-32-LAYER-E1`／`WX25-P3-036-E1`。既存 STUB `SUPPRESS_LIFE_BURST_ON_CRASH`／`SUPPRESS_LIFE_BURST_ON_CARD` が受け皿）(b)**「代わりにそれをトラッシュに置く」置換 2件**（`WX06-024-BURST`／`WXK08-030-E1`）(c)**デッキ公開→処理の変形 2件**（`WX21-028-E1`＝「その中から**すべての**＜天使＞のシグニをエナゾーンに」＝**枚数語が名詞より前**で `parseRevealPickDescriptor` の語順に合わない／`SP38-006-E1-G2`＝主語が対戦相手で1文に「見る」も含む）(d)残りは**単発**（1カード1機構）。**(a)(b) は既存語彙があるので次に安い。**
