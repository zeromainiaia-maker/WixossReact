...
**直近＝2026-09-19＝§5.3 `O-535` クローズ＝宣言済みの対象を2回聞かない（631効果 / 608カード）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**真因**＝`selectOrInteract` に「候補が確定しているなら自動解決」の枝が無く、「〈対象〉を**対象とし**、〈任意コスト〉して**もよい**。**そうした場合**〜」が**①宣言 → ②支払い → ③同じ対象を問い直す**になっていた。
- **直し方**＝**`executeAction` の出口に funnel を1本**（対話を作った地点の ctx を復元して `resumeSelectTarget` へ流す＝型ごとの特例を再実装しない）。自動解決の条件は5つ（宣言の印／`pending.optional` でない／候補 ≦ `count`／`unplaceableCards` 無し／合計制約無し）。
- 🔴**UX の粗ではない**＝`SELECT_TARGET` の resume ごとに `collectTargetedTriggers` が走る＝`ON_TARGETED` が二重に立つ（素で二重発火するのは1効果＝`WX25-CP1-060`／17効果は `usageLimit` で2回目が落ちていた）。
- ⚠**一度広げて戻した**＝`upToCount` は「N体**まで**」と「〜して**もよい**」の**両方**に使われていて engine からは区別できない（golden 4本が「0体を選ぶ自由が消えた」で落ちた）。残 **13効果**は parser 側＝`O-536`。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.3 索引 I `O-536`**（`O-535` の残り＝帰結側に残る `upToCount` を parser で落とす・13効果/13カード）→ その後 **§5.7 `S-5d`** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**engine とテストだけの回＝live JSON を1バイトも触っていないので3計器は動かない**） |
| 📦**在庫** | 機構 worklist **1**（`O-536`）／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**同じキーが2つの意味を持っていないかを live で数えてから読み替える**＝`upToCount`（宣言の「まで」／帰結の「してもよい」）。区別できないなら engine で直さず parser 側へ回す。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4322**） |
...
| ID | 母集団 | 何が無いか |
|---|---|---|
| `O-536` | **13効果 / 13カード**（live 実測） | 「〈対象〉を**N体まで**対象とし…**それら**を〜する」の**帰結側に `upToCount` が残る**ので、宣言した対象をもう一度選び直させる（`O-535` の自動解決から外れる）。engine 側からは「宣言の『まで』」と**帰結の「〜してもよい」**（`WX12-010-E3`）を区別できない＝**直すのは parser** |
