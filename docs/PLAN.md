...
**直近＝2026-09-21＝🏁§5.7 `S-31` ①＝「N体並べる」条件は測って否定（すでに効いていた）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第444バッチ）
- 📏**母集団**＝自分の場の並びを条件にする効果 **147**（`POWER_SET` 86／`POWER_MODIFY` 22＝**109（74%）はパワーに出る**）。
  ユーザー作21デッキのうち**16デッキ・27種**が該当（下書きで見立てた3デッキより広い）。
- 🔴**結論＝作戦データは要らない**＝**盤面の採点は `powersOf`（`calcFieldPowers`）で「並べた後の実効パワー」を見る**ので、
  3体めを置くと全員が上がることが**そのまま点数に出る**（実測＝8,000 → 21,000＝印刷パワー7,000ぶんを超える）。
  印刷パワーが上の対照より高く採点されることも確認。⇒ **`S-31` ① はクローズ**。
- ⚠**残り38効果**（`GRANT_KEYWORD` ほか）は**パワーに出ない**が、強さ表が `activeCondition` を見ず
  **満額で数えている＝過大評価**（取りこぼしではない）＝`S-1` 側の小さな穴として記録。
- ⚠🔴**測定で2回踏んだ罠**＝`calcFieldPowers` に**カード番号キーの素の Map** を渡すと
  CONTINUOUS が1つも当たらない（engine は instance ID で引く）＝**「効いていない」に見える**。
  🔑**「効かない」を見つけたら、まず対照を1つ置く**（条件なしの効果も効かない＝harness 側と切り分かる）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` の残り**（②撃てない【起】108・③アーツの使いどころ／使わない札）。次に `S-29` |
...
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**撃てない【起】は残 108効果**（`S-31`） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4362**）｜⚠**実機は不要**（コードを1行も変えていない回＝§2.2） |
...
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**②は2段まで消化**（手札を捨てる／エナ・場から払う）＝**撃てない【起】 210 → 108**。🏁**①は測って否定**（第444バッチ＝「N体並べる」条件の見返りは**実効パワー経由で採点に既に効いている**＝147効果中109がパワーに出る）。**残り**＝②の続き（場のシグニ＝`underSelfTrash` 13／`trashExile` 13／`discardSelfFromHand` 9／`charmTrash` 6・ルリグ＝`fieldTrash` 8（**実行側に口が無い**）ほか＝**1キーあたり6〜13件の小さい山**）／③**アーツの使いどころ**（守り／攻め）・**使わない札**の指定。⚠**着手前に「どれが欲しいか」をユーザーに確かめる**。 | 中〜大 | 作った CPU デッキを意図どおりに回せる |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🏁~~`S-22`（対象選択が効果の意味を見ていない）~~ → 🏁~~`S-24`（自己対戦のマリガン）~~ → 🏁~~`S-30`（測って否定＝CPU は退化していない）~~ → 🏁~~`S-14`（作戦データの「使い方」）~~ → 🔥**`S-31`（CPU の作戦をもっと細かく＝ユーザー要望・🏁②の手札捨ては消化）** → 🏁~~`S-32`（対象の指示）~~ → `S-29`（宣言の帰結のコスト＝母集団未計測）。
...
- **2026-09-21 時点**（第421〜444バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4362**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・7/175手・5/130手・6/133手・6/135手・7/161手・**7/165手**）
