...
- **🆕 セッション（2026-07-31・続き309・Opus 5〔**Claude が実装まで担当**〕）＝タスク12(lxi) 第8波＝`WXK05-009-E2` を消化（在庫 残4→3）＝防御側が設置する `ON_ATTACK_SIGNI` 遅延＋アタック回数比例の可変《無》**（golden **1205→1208**・census **1356→1355**（`BASELINE_HIGH=1355`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**240 warnings 据置**・manual field loss 0・同型★0（265群）・held **251→250枚/98署名**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等）
  - **① 直したのは過剰実行**＝「このキーを場からルリグトラッシュに置く：このターン、対戦相手のシグニがアタックしたとき、対戦相手が、このターンにシグニがアタックした回数1回につき《無》を支払わないかぎりそのシグニをトラッシュに置く」が `TRASH{SIGNI opponent 1}`＝**キーを切った瞬間に相手シグニを1体（好きに選んで）トラッシュ**していた。
  - **② 第7波のテーブルに1行足すだけで設置側は済んだ**（`DELAYED_INSTALL_PREFIXES`）。新しいのは**設置者が防御側**な点＝発火は「対戦相手＝ターンプレイヤーのアタック」。収集器 `collectSigniAttackDelayedTriggers` を pure 新設して BattleScreen の**防御側 AUTO 収集地点**（`opAtkedEntries`）へ push し、設置者視点の弁別子 `trigger.attackerOwner`（`leftOwner`/`refreshedOwner` と同形）を新設。帰結の「そのシグニ」は `triggeringCardNum`＋`targetsTriggerSource`（第3波の語彙）で解ける。
  - **③ 可変《無》＝新フィールド0で解いた**＝`StubAction.opponentPayColorlessPerSigniAttack` を立てると、実行時に**支払う側**の既存 `attacked_signi_ids.length` を《無》の枚数として解決する（アタックのたびにコストが上がる）。
  - **④ ⚠教訓＝「読めなかった」の顔は `UNKNOWN` だけではない**。遅延設置の帰結は単独文として再パースするため、「そのシグニをトラッシュに置く」が `STUB{RULE_REMINDER_TEXT}`＝**完全 no-op** に落ちて既存ガードを素通りしていた（＝過剰実行を no-op へ替えるだけで「直った」ように見える）。ガードを no-op STUB へも広げ、照応は「それ」へ書き換えて発火元へ束縛した。
  - **⑤ ⚠教訓＝可変コストは**両側**の解決地点に入れる**。SEQUENCE 標準ペア側だけ直すと STUB フォールバック側で `costLen 0`＝「支払不可」と読まれ**必ず帰結が撃たれる**逆向きの過剰実行になる。あわせて「回数0＝《無》×0＝タダ回避」の穴も塞いだ。
  - **⑥ ⚠教訓＝golden の `mkCtx`/`mkState` は共有プールの `cursor` を進める**。新テストを前に足したら**無関係な `task12(lxiii)` が落ちた**（下流の `fresh()` が別カードを返す）。盤面を作るテストは `const savedCursor = cursor; … cursor = savedCursor;` で挟む（第7波の2本にも遡って追加）。
  - **⑦ 🆕 新規登録＝(lxxii)**＝`delayed_triggers` のターン終了クリアが**ターンプレイヤー側だけ**で、防御側が設置した THIS_TURN 遅延が1ターン持ち越される（本カードでは誤発火しないが `ON_LEAVE_FIELD`／`ON_REFRESH` には効きうる）。
  - **次の一手**＝**Opus**：①🆕**タスク12(lxx)「引くか【エナチャージ】」の選択肢脱落10効果**（原因特定済み・1規則でまとまって開く可能性が高い在庫）②🆕**(lxxi) 遅延設置の「次に」消費機構**（受け皿は第7波で出来た）③🆕**(lxxii) 遅延クリアの片側漏れ**（1行×3箇所だが計器に映らない）④(lxviii) 散文形「対戦相手のターンの間、」⑤§6.3 J 残11 ⑥(lxvii) CPU `ON_TURN_END` 未収集 ⑦§6.4 配置数制限すり抜け7効果 ⑧(lxi) 残3／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**続き298〜309 の全件が実機UI未検証で積み上がっている**（本波の「アタックごとに増える《無》の CHOOSE」も未検証）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き309（タスク12(lxi) 第8波＝`WXK05-009-E2`・1効果／**Claude 実装**）後 最新値（本行を直近の正とする）**：census **1356→1355**（`BASELINE_HIGH=1355`＝キーを切った瞬間に相手シグニを無条件で1体トラッシュしていた1効果の**機能是正**）、golden **1205→1208**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**240 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **251→250枚／署名グループ 99→98**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WXK05-009-E2`）、`build:effects`・`regen` 冪等。**タスク12(lxi) 残＝4→3**。**新語彙2本**＝`InstallDelayedTriggerAction.trigger.attackerOwner`（`leftOwner`/`refreshedOwner` の兄弟）と `StubAction.opponentPayColorlessPerSigniAttack`（可変《無》＝**新 PlayerState フィールド0**＝既存 `attacked_signi_ids` を読むだけ）。engine 新設は**収集器1本**（`collectSigniAttackDelayedTriggers`＝pure・BattleScreen 側は push 1行）。**新 timing・新 action 型はゼロ**。
