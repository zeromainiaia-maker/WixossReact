...
- **🆕 セッション（2026-07-31・続き299・Opus 5〔Claude 実測＋検証〕＋codex-work〔実装〕）＝タスク16 第2波＝`ON_TARGETED` に「対象を取った能力の origin 種別」軸を新設し4効果を消化。差し戻し0**（golden **1158→1168**・census **1365→1364**（`BASELINE_HIGH=1364`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**232 warnings 据置**・manual field loss 0・同型★0・held **250枚/97署名 据置**。ゲートは Claude が独立に再実行して全緑／live per-effect 差分 **changed 4 / added 0 / removed 0**／`build:effects` 冪等／`census:timing` **27→25**）
  - **① 新語彙は1本だけ**＝`triggerCondition.targetedOrigins`（**配列要素間 OR・要素内 AND**）。**新 timing・新 action 型・新 collector はゼロ**＝`collectTargetedTriggers`（`triggerCollect.ts:344`）に **origin 引数と before 盤面を足しただけ**。`origin` 省略時は従来どおり素通り＝**既存16効果は完全不変**（per-effect diff で機械確認）。
  - **② 開いた2効果（`timing:[]` の安全停止 → 実働）**＝`WXDi-D09-H16-E1`（アップ状態の味方シグニが**アシストルリグ か ライフバースト**の対象になったとき）／`WXDi-P08-065-E2`（＜宝石＞の味方シグニが**相手シグニの【出】能力**の対象になったとき）。origin 3軸はいずれも**既存データで引けた**＝`CardData.Type==='アシストルリグ'`／`effectType==='LIFE_BURST'`（先例 `GrantProtectionAction.sourceEffectType`）／`timing.includes('ON_PLAY')`。
  - **③ 同じ軸で既存の過剰実行2件も閉じた**＝`WXDi-P03-056-E1`／`WXDi-P13-089-E2`。parser が `対戦相手の(?:シグニの)?` の**「シグニの」を読み捨てていた**ため、相手のルリグ/アーツ/スペルの対象になっても発火していた。どちらも本体が「**その対戦相手のシグニ**」を指す＝origin がシグニ前提の効果。
  - **④ 投入前実測で先出しした罠2つが両方効いた**＝(a) H16 の `CONDITIONAL{IS_MY_TURN}` は誤変換ではなく **`OPTIONAL_COST` 直後の「そうした場合」parser 慣例**（`effectExecutor.ts:2709-2714` が支払いゲートとして受理）＝「触るな」と書いて余計な改変を防いだ。(b) **「アップ状態」を after 盤面で見ると「そのシグニをダウンする」効果で発火しなくなる**＝before 盤面（`bs.host_state`/`bs.guest_state`）を渡せと指定し、golden も **宣言前アップ→適用後ダウンで発火／宣言前ダウンで非発火**の両方を固定した。
  - **⑤ 呼び出し元2箇所を行番号で名指ししたので両方に配線された**（`BattleScreen.tsx:4237` の `autoTargetedCards` 経路／`4508` の `SELECT_TARGET` 経路）＝§5-15/20 の「同型配線が複数箇所あると1箇所で満足する」失敗モードは非再発。
  - **⑥ Claude 検証で見つけた2件（codex 報告に無し・本バッチ由来ではない）**＝(a) **decompile シートの既存未再生成ドリフト**＝J-3 の4効果（`WXK08-028-E1`／`WD23-023-E-E1`／`PR-K038-E2`／`WXDi-P07-052-E1`）の逆翻訳が `ON_LIFE_CLOTH_MOVED` を出すようになったが JSON は不変＝**前回セッションが最後の実装後に `npm run regen` を回していなかった**だけ（シート差分を本バッチの影響と読むと誤帰属する）。(b) `scripts/fixLrigColorFilters.mjs` の `delete obj.triggerScope` は**挙動不変**（collector が `?? 'self'` で既定を補う）＝`build:effects` 冪等化のための安定化で live 影響0。
  - **次の一手**＝**Opus：🆕 タスク16 残＝`[B維持]` 11件**（第2波で `WXDi-D09-H16-E1`／`WXDi-P08-065-E2` が抜け 13→11）。次に投げやすいのは ①**`ON_HAND_DISCARDED` の原因オーナー軸**（`WXDi-D09-P16-E2`／`WX24-P2-051-E1`／`WXDi-P13-051-E3` の3件＝`collectHandDiscardTriggers`〔`triggerCollect.ts:2438-2449`〕に **`causeOwner` 引数が無い**＝シグネチャ変更が要る）②**`ON_ENERGY_CHARGE` の移動元/原因軸**（`WX14-066-E1`／`WXDi-P03-073-E1`／`WXDi-P12-079-E2` の3件＝`BattleScreen.tsx:1630-1638` が増加枚数しか読まない）。**⚠11件とも live は `timing:[]` の安全停止＝過剰実行ではないので、退化させないことが最優先**（第2波と同じく「その経路が engine で本当に発火するか」「timing 以外にズレていないか」を投入前に実測して指示書へ罠として先出しする）。**🆕 新規在庫＝タスク12(lxviii)**（散文形「対戦相手のターンの間、」が `turnOwner` を生まない過剰実行＝下表）。その他＝§6.3 J 残11（J-1/J-2/J-4/J-5）・(lxvii) CPU `ON_TURN_END` 未収集・§6.4 配置数制限すり抜け7効果・(lxi) 残5・(xxix) 残4・(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**（単一 worklist は §7「残る実機検証項目」）。⚠**今回の4効果も実機UI未検証**（前回の I／J-3 も未検証のまま積み上がっている）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き299（タスク16 第2波＝`ON_TARGETED` origin 種別軸）後 最新値（本行を直近の正とする）**：census **1365→1364**（`BASELINE_HIGH=1364`＝停止中だった2効果の実働化＋既存2効果の origin 限定という**機能改善**）、golden **1158→1168**（+10＝4効果の正/誤 origin 両方向＋H16 の宣言前アップ/ダウン）、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**232 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、held **250枚／署名グループ 97 据置**。live per-effect 差分 **changed 4 / added 0 / removed 0**（`WXDi-D09-H16-E1`／`WXDi-P08-065-E2`／`WXDi-P03-056-E1`／`WXDi-P13-089-E2`）、`build:effects` 冪等・`regen` 差分0。**`npm run census:timing` 27→25 効果/25クラスタ**（⚠**実質23**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は live で既に正しい timing を持つ既知の偽陽性＝`clearTimingFallback` 未呼び出しによる計器の残留）。**タスク16 残＝`[B維持]` 13→11**。**新語彙1本のみ**＝`triggerCondition.targetedOrigins`（OR-of-AND。省略時は従来挙動＝既存16効果不変）。**新 timing・新 action 型・新 collector はゼロ**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1168**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1364**（回帰ゲート）・held **250枚/97署名**（2026-07-31 タスク16 第2波後に実測）。
>
