...
| 12 | Sonnet 発見バグの修正（常設） | 可変 | 可変 | ◎**在庫7件**（✅続き78で旧8件全消化。新規＝SP27-002 genericKagiri無言消費系統・WXDi-P10-035 引用内BOUNCE owner精査・WXK09-050 dispatch設計・続き81 applyDirectAction TRASH/HAND_CARD 3フィールド欠落・続き93 applyDirectAction 未対応6型〔TRANSFER_TO_DECK/GRANT_PROTECTION/ENERGY_CHARGE/POWER_SET/POWER_MODIFY_PER_FIELD/POWER_MODIFY_PER_LRIG_LEVEL＝smoke SKIP258件の真因・実UIフリーズ相当〕・続き95 `collectPowerZeroTriggers`がfield.lrigを走査せずLRIG watcher（WX22-013/WXDi-P14-009）が絶対発火しない・🆕続き96 同型のLRIGゾーン走査漏れが`triggerCollect.ts`内6コレクタに系統的に存在し実カード20枚が影響（ON_ATTACK_SIGNI/ON_ATTACK_PHASE_START/ON_OPP_ARTS_USE/ON_HAND_DISCARDED/ON_BANISH）。詳細は上のOpusタスク12本文） |
