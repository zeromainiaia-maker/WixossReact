...
- **🆕 セッション（2026-07-11・続き68・Opus 4.8・§3 Opusタスク3 動的比較＝デッキ相対SEARCH 3枚）**＝「この方法で捨てた（handDiscardSigniコストの）シグニより±Nレベル/共通クラス」を SEARCH filter から落としていた3枚を実装＝WDK13-013-E1（捨てレベル+1）／WXK10-033-E2（捨てレベル+2＋共通クラス）／WXEX2-37-E3（捨てレベルより低い）。**既存インフラ拡張**＝`last_discarded_signi_level`＋`levelLteDiscardSigni`（既存）に、`last_discarded_signi_class`（記録・BattleScreenコスト支払い箇所）＋フィルタ `levelLtDiscardSigni`（<）・`levelEqDiscardSigniOffset:N`（=捨レベル+N）・`classMatchesDiscardSigni`（共通クラス）を追加し、`resolveDiscardLevelFilter` を4フィルタ対応に拡張。**配線バグも修正**＝`resolveDiscardLevelFilter` が ADD_TO_FIELD 経路でしか呼ばれておらず **execSearch が呼んでいなかった**（既存 levelLteDiscardSigni も SEARCH経路では効いていなかった）＝execSearch の filter 解決に追加。**表現**＝3枚 MANUAL→adopt・decompiler に3フィルタの原文語を追加。**検証**＝golden +2（183・levelEqDiscardSigniOffset/levelLtDiscardSigni を deck固定3枚で決定論検証）／gates 全緑（**census 1566→1563**・BASELINE更新・lint 0）／regen 同型★0・decompile 3枚ともレベル/クラス制約を描画。詳細 BUGFIXES 最上部。
  - **前回（続き67・Opus）＝WXEX2-25-E3 lrig相対の動的比較実装**（GRANT_EFFECT で相手LRIGへ CONT POWER_MODIFY(levelLtSelf)付与＋`resolveContSelfLevel`）。詳細 [PLAN_PROGRESS.md](./PLAN_PROGRESS.md)。
  - **次の一手＝(A)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse**（WXDi-D09-P20／WX24-P1-017／WX25-P3-038＝「「【自】…」を得る」の内側 ability parse）。**(B)Opus＝§3 Opusタスク3 残の動的比較別機構**（WXEX2-28＝直前配置シグニ基準 last-processed相対〔levelGtLastProcessed 相当〕・条件文 WXK08-005・opp/own センタールリグ WXK11-003）。**(C)Sonnet＝§7実機検証の継続**（🆕 **続き66でR30の発火経路が開通**＝WXEX2-50-E3 を発動して相手フィールドにシグニを出し WXK10-022-E1 watcher の ON_PLAY any_opp 発火を確認・シナリオ追加。加えて ON_TARGETED残3枚・R42②・ON_COIN_PAID/ON_LRIG_GROW③④）。**(D)持ち越し：WXK10-033-E1（アタック時トラッシュ＋手札からLv3配置）の据置確認・WXEX2-25-E3 decompiler levelLtSelf描画「このシグニより」固定（低優先）・WXEX2-50-E3 step2レベル制約未反映・WXK04-003 ボタンラベル表示バグ・WX12-008 exceed-cost timing・SEQUENCE下流「そうした場合」・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
