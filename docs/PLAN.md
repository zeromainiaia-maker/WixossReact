...
- 🏁**セッション（2026-08-26・続き674・Opus 5）＝§5.3 `O-73`＝「このターン、〜したとき」の**遅延設置**が `ON_CARD_MILLED_FROM_DECK` に無く、`WX24-P3-030-E2` は**【起】を撃った瞬間に相手を殴る**過剰実行になっていた。**型＋engine＋parser＋decompiler の4点**で配線。
  - 🔑**真因は2つ重なっていた**＝(1) 登録票が名指ししたブロッカー＝`collectMillTriggers` が `delayed_triggers` を読んでいない (2) **parser 側のガードが過剰**＝`O-71` の「tail に『それ』が出たら畳まない」に巻き込まれていたが、本件の「それ」は**同じ tail の中で対象宣言が完結**している。⇒ **`この方法で` は落とす／`それ` は tail が `を対象とし` を含むときだけ通す**の2段に割った。
  - 🔴**collector を先に足すのが必須**＝parser だけ直すと「設置されるが永久に発火しない」＝**過剰実行を no-op へ替えるだけ**の別の退化になる。実機の判定を**2段**（①設置時は殴らない ②ミルで発火する）にしたのはこれを区別するため。
  - 🔑**母集団は登録票の「1効果」で正しいが、周辺は広い**＝設置句を全数で測ると **21文型/41出現**、live で遅延が生きているのは**5枚だけ**。⚠**残り36枚が全部バグではない**（最大クラスタ16件は**ライフバーストの定型**で即時実行がほぼ正しい）。仕分けを要するので §5.3 `O-92` へ登録した。
  - ⚠**実機で罠を3つ踏んだ**（すべて §4.4 の既知項の新実例）＝**罠8z**（ミル役に選んだ `WX02-070` が**＜悪魔＞**で同カードの【自】が同時発火し stack=2 で停止）／**罠8p の同型**（手札カードは毎ティック押すと**トグルで閉じる**）／**罠2b**（ゾーン選択には `data-action-label` が無いので、手札ブロックを先に置くと**出したばかりのゾーン選択を閉じる**）。
  - ⚠**判定順序の実装ミスも直した**＝旧実装には「設置」が無いので `powerAtInstall` が永久に null になり、**反転確認が「前提崩れ」と報告**されていた。「設置を1度も観測しないまま相手のパワーが下がった」を旧挙動として明示的に捕まえる。
  - ✅ gates 全緑（golden **2844→2845**／census **564→563**（baseline 更新）／`census:enginetext` A群 141 据置／smoke・fuzz 全0）。実機 `o73DelayedMillTrigger` は**3連続 PASS で安定確認**＋反転確認済み。

**▶ 次の一手**＝**`O-92`**（`O-73` の実測で出た広い母集団＝遅延設置が生きているのは 41出現中5枚だけ。**まず16件のバースト定型を仕分けて除外**してから timing 別に取る）か、**`O-80` 第2バッチ**（残 36効果・ゾーンを数える系16が次の塊）。実機の腐り返済なら §5.1 の `V-89`。
