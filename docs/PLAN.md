...
- **🆕 セッション（2026-08-05・続き348・Sonnet 5〔§7実機検証worklist 7件を全消化〕）＝ユーザー指示「§7の実機検証を行う」で残っていた7項目（第2波(a)／エクシード本体5件(a)(b)(c)／タスク16残0クローズ3件）を全消化＋新規実バグ(cvi)を発見**。**engine/parser/JSON は無変更**（golden/census/held/smoke/fuzzは前回値からすべて据置＝1366/1288/257枚/10679/0）。触った層は`scripts/verifyBattleDrive.mjs`（新規シナリオ7件＋`onplaycost-exceed-{i}`testid新設＋`H.queryState()`の`sideOf()`に`lifeCards`/`signiDown`追加）・`src/screens/battle/modals/SigniOnPlayCostModal.tsx`（testid追加のみ）・`docs/PLAN.md`／`BUGFIXES.md`のみ。
  - **✅実機検証クローズ＝§7「残る実機検証項目」が worklist として全件消化**（`secondWaveEnergyBranch`／`exceedBanishGateA`／`exceedDynamicTargetCountB`／`exceedTwoGroupPickerC`／`energyLeftAnyZoneTrigger`／`doubleCrashUpTrigger`／`oppResourceLossChoose`、各2回連続PASSまたは意図的FAIL・既定`order`94→101件）。**このプロジェクト初のLRIG「【出】エクシードN」コストUI**（`SigniOnPlayCostModal`のルリグの下からN枚選択→発動/スキップ）を実機で新規に駆動。
  ...
  - **次の一手**＝**Sonnet**＝§7「残る実機検証項目」はユーザーが選んだ7項目を消化したのみで**まだ多数残る**（上から＝`WXDi-P11-063`のメモリア下配置選択／`WXK08-086`等【常】版自己バフ停止／トラッシュ領域移動ロック3件／`WXK06-067-E1`のゾーンを跨いだ選択モーダル／相手側CHOOSE4つ目の枝／タスク12(lxiii)(a)(b)／併記型(c)／ON_LRIG_GROW④横グロウ経路／(xi)(xxxvi)のUI検証／クラフトトークン実機配置／lrigDownコスト限定2件／driver側低頻度フレーク）。次に着手するならこの並び順の上から。**Opus**＝タスク12の生き残り在庫〔(lv)残2経路／(lxvi)／(lxxxviii)／(xciii)／(xciv)／(xcvi)／(xcvii)／(c)〕＋(ci)〜(cvi)（6件とも実機検証で確認済み・再現手順はPLAN §3の各行とscripts/verifyBattleDrive.mjsの該当シナリオコメントに記載）。
