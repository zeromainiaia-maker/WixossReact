...
**直近＝2026-09-17＝§5.7 `S-7` 着手＝CPU が場以外の【起】（トラッシュ・手札・エナ）を使う**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**ユーザー指示**＝`WD08-009`（＝`WX22-Re17` コードアンチ ネッシー）のようなトラッシュの【起】を CPU も使う・場以外の【起】を最優先。WD08-009 の【起】自体は正しく実装済みと engine で確認。
- 提示の判定を1本に（`offFieldActivateGate.ts`＝人間の3入口と CPU が共有）／実行は人間と同じ `executeTrashActivated`・`executeHandActivated`（行為者つきへ一般化）／CPU は払ったあとの盤面を先読みして得なものだけ（`cpuOffFieldActivate.ts`・グロウ用エナの予約つき）。
- 🔴**人間にも効く不具合2件を同時に修正**＝回数制限の無い場以外の【起】が1ターン1回に制限されていた／手札の【起】 `WX18-036-E3` が払えないコストのまま提示されていた（→ `O-533`）。
- 実機＝🆕`V-273`（トラッシュ・相手がいれば使う／場が空なら使わない）・🆕`V-274`（手札）＋人間側トラッシュ・エナ【起】の既存12本。
- ⚠`S-7` の残り＝人間のターンのアーツステップで CPU が手札の【起】を使う応答。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-7` の残り**＝人間のターンのアーツステップ（`ATTACK_ARTS_OP`）で CPU が手札の《アタックフェイズアイコン》【起】を使う応答 → §5.3 `O-533` → `S-5a` |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **1**（`O-533`）／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-7`・`S-5`・`S-6`）／**リリース作業 1**（RELEASE.md） |
...
| ID | 内容 | 規模 | 母集団 |
|---|---|---|---|
| `O-533` | 手札の【起】で「このカードを手札から公開し、場のシグニN体をトラッシュ」＝`HandActivatedModal`／`executeHandActivated` が `fieldTrash` を払えず、自分を捨ててしまう（2026-09-17 `S-7` で提示を止めた） | S | 1効果 |
...
| 🔥🔥`S-7` | **場以外の【起】を CPU が使う（最優先）**＝トラッシュ（`trashActivated` **38効果/37枚**）→ 手札（`handActivated` **10/10**）→ エナ（`energyActivated` **1/1**）。🔑例＝`WD08-009`（`WX22-Re17-E2`「【起】《黒》《黒》《無》：対戦相手のシグニ１体のパワーを－8000。このカードをトラッシュからデッキの一番下に置く」） | 1〜2日 | 🔴**いまの CPU は場のシグニ・ルリグの【起】しか使わない**＝トラッシュ・手札・エナにある【起】は一度も使わない（2026-09-17 ユーザー質問で確認）。**規律は §5.6.3**＝提示の判定は人間と同じ（`getMyTrashCardActions` の条件＋`canOfferTrashActivate`）／実行は人間と同じ `executeTrashActivated` 系を**行為者つきに一般化**して使う／CPU は候補から選ぶだけ（`cpuOffFieldActivate.ts`＝先読みの得＋グロウ用エナの予約）。内訳（実測）＝コストはエナ 13・トラッシュ除外 7・エナ＋除外 6・手札の自分を捨てる 8・その他 15／タイミングは MAIN を含む 27・アタックフェイズを含む 24・スペルカットイン 1 |
| 🔥`S-5` | **C 対戦丸ごとのシミュレータ**（§5.7.3 の案C・段階的）＝`S-5a` メモリ上の persist＋`resolveStackNext` の切り出し → `S-5b` CPU が使う実行関数 → `S-5c` `cpuTurnAction` → `S-5d` 相手側の応答と人間側ターンの駆動。各段で `gates`＋実機の回帰 | 3〜5週間 | 画面なしで CPU 同士の対戦を回す＝勝率で強さを測る。ターン進行のバグ探しにも使える |
