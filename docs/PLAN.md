...
- **2026-07-29 第14波 最新値**：`WXK04-045-E1` と `WX26-CP1-061-SONG` の同名参照pickを実働化し、`WX26-CP1-061-E1` の歌効果混線を除去。census **1412→1410**（SONG 原文が E1 にも割り当たる計器混線を分離し、新語彙キーも登録。回帰ゲート実数1410）、golden **998→1000**、held 実測値は本節下の cold-start 行を正とする。第13波は `WX11-074-E1` の固定2段pickを実働化し golden 997→998。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1000**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1410**（回帰ゲート）・held **258枚/114署名**（2026-07-29 第14波実測）。
>
