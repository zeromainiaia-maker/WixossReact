...
- **🆕 セッション（2026-08-15・続き483・Opus 5）＝§6.4 **O-21 完了＝到達不能な二番手 STUB ハンドラの解体**（19 id / 21ブロック）。**うち1件は「本物の実装が no-op に潰されていた」live バグ**。ゲート全緑（**golden 1980**＝+2・census 831 据置・lint 0/259）。
  ...
  - **✅③ 残り18 id（20ブロック）は旧版の死コードとして削除**（−352行）＝先着が新しい正しい実装、後発が旧実装というパターン（パワー修正族は**先着＝相手に適用／後発＝自分に適用**、`MULTI_SIGNI_TO_ENERGY` は**先着＝相手シグニ／後発＝自シグニ**など）。⚠**削除前に安全条件を機械確認**（先着がトップレベル・影サイトが生きた id と条件を共有しない＝21箇所すべて単一 id）。削除後に **live 24効果を実行して差分は `BET_CONDITION` の1件とシャッフル乱数のみ**＝死コードだった証明になった。
  - **🔑④ golden は「重複の存在」ではなく「到達可能性」を固定した**＝重複自体は正当なので、**先着が必ず return かつ後発がある**ときだけ赤にする。id の凍結リストではないので新しい id にも自動で効く。
  - **▶ 次の一手**＝**O-22**（O-20 で露出した別要因 no-op 3件）→ **O-24**（`WXK06-030-E2` の過剰実行）→ **O-19**（watcher の `triggerScope` を parser が書く恒久解）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き483（§6.4 O-21 完了）後 最新値（本行が直近の正）**：census **831 据置**、**golden 1980**（+2＝O-21 の到達可能性トリップワイヤ／`BET_CONDITION` の両方向）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**、held **106枚 / 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**。🆕**live JSON changed 0**（effects JSON・CSV 非改変）。🆕**engine −352行**（死コード20ブロック削除）。🆕**到達不能ハンドラ 19 id → 0**。
...
| **O-24** | 🆕**`OPP_TRASH_FIELD_SIGNI_AND_ENERGY` が全体を流す（過剰実行）** | 1効果 | 2026-08-15 続き483 の O-21 棚卸しで隣接発見。`WXK06-030-E2` の原文は「対戦相手は、自分の場からシグニ**１体**と自分のエナゾーンからカード**１枚**を対象とし、それらをトラッシュに置く」なのに、engine は**相手の場のシグニ全部＋エナ全部**をトラッシュする。⚠**相手に選ばせる**（`opponentResponds`）形でもある＝続き411 の教訓どおり ctx の視点は反転しないので候補と適用先を明示的に `otherState` 側へ向けること |
...
> **詳細は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-15 整理⑮」／一次記録は [BUGFIXES.md](./BUGFIXES.md) 2026-08-15 の2本。**
