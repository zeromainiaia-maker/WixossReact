...
**直近＝2026-09-16＝§5.2 round6（原文 × 実行結果）の試行と計画／PLAN の掃除**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 試行30枚と再現率セット16枚を codex で監査し、`O-524`〜`O-526` を登録した（コード修正なし）。round6 の計画は §5.2。
- PLAN から経緯と教訓を抜いた（教訓は LESSONS.md／DRIVE_TRAPS.md、掃除前の全文は PLAN_DETAIL.md 末尾）。
  🔴**第350バッチで §2「作業の流れ」が丸ごと消えていた**ので戻した。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.3 索引A `O-524`**（場が満杯だと「場に出す」カードが消える）→ `O-526` → **§5.2 round6 の R6-0**。`O-525` はユーザー判断待ち（索引H） |
| 📊**進捗3計器** | Sheet1 要対応 **2 / 863**（`census:cards -- --sheet 1` で測り直す）／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **3**（索引A 2・索引H 1）／実機 **0**／実装キュー **0** |
| 🔧**ゲート** | この回は `scripts/` と `docs/` だけ＝`gates` 未実行（前回 全緑・golden 4258） |
...
**現状**＝round4（全11シート）・段2 台帳・round5（全5,976枚）は完了。**round6 を R6-0 から始める。**
...
| ID | 規模 | 何が無いか（一行） |
|---|---|---|
| `O-524` | S・**132効果**（汎用盤面で踏んだ分） | 🔴場が満杯のとき「場に出す」カードが元の領域から消える＝`applyDirectAction` の `ADD_TO_FIELD` が取り除いた後で空きゾーンを判定している（`effectExecutor.ts:13894`） |
| `O-526` | S・**13効果** | デッキの一番下を削る `MILL{fromBottom}` のバトルログが「デッキ上から」と書く（挙動は正しい・`effectExecutor.ts:10434`） |
...
| ID | 規模 | 何を判断するか（一行） |
|---|---|---|
| `O-525` | S・**10効果** | 「《クロスアイコン》〜の左」の見出し直後の**普通の**【出】【自】【常】【起】をクロス限定とするか（いまは `crossOnly:true`＝クロスしないと発動しない） |

**`O-525`**＝`WX11-041` ほか10枚。原文＝「《クロスアイコン》《羅石　ロズオラ》の左**【出】**：…２枚トラッシュに置く。…**【クロス自】**：このシグニが《ヘブン》したとき、…」。
**いまの実装の読み**＝見出し直後の能力もクロス限定（読みB）。**読みA**＝【クロス〜】と書かれた能力だけがクロス限定。
**コーパス**＝同じ形は10枚で10枚とも `crossOnly` あり＝カードをまたいだ minimal pair は無い。カード内では**10枚とも後ろの能力にだけ【クロス自】と明記**（読みA寄りの手掛かり）。⚠CSV が印刷上のアイコンを落としている可能性は原文だけでは否定できない。
...
- **2026-09-16 時点**（round6 試行と PLAN 掃除の回）
  - 📊**進捗3計器**＝Sheet1 要対応 **2 / 863**（held 1・mech 1）｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**
  - 📦**在庫**＝機構 worklist **3**（索引A 2＝`O-524`/`O-526`｜索引H 1＝`O-525`）｜実機 **0**｜実装キュー **0**｜round6 **R6-0 未着手**
  - 🔧**ゲート**＝この回は `scripts/`・`docs/` のみで `gates` 未実行（直前の第377バッチで全緑・golden 4258）
