...
- **🆕 セッション（2026-08-07・続き375・Opus 5〔**§5d パターンA 第7バッチ**＝続き374 で登録したブロッカー2件の実装〕）＝ユーザー指示「続ける」。触った層は `effectParser.ts`（`applyDroppedEnergyDesignation` 新設＋条件節1本）／**`execUtils.ts`（evalCondition のルリグ色解決）**／`public/data/effects_*.json`（4効果・すべて held 経由で採用）／`goldenTest.ts`。
  - **①エナゾーン対象宣言のゾーンごと取り違え（3効果）**＝帰結が `TRASH{SIGNI, owner:'any'}`＝**場のシグニを落とす別物**だった。`applyDroppedEnergyDesignation` を新設し `TRASH{ENERGY_CARD, opponent, filter}`（同族30効果と同じ正準形）へ。全CSV4効果のうち4件目 `WXK06-053-E1` が MANUAL で既に正準形＝**正解の裏付け**になった。
  - **⚠前セッションで想定した `SELECT_TARGET_ONLY` 方式は採らなかった**＝あれは**コスト量が対象に依存する**形の機構で、ここは固定コスト。持ち込むと engine 拡張3点（`execStubPart1` の SIGNI 専用／`execTrash` の ENERGY_CARD 分岐が `targetsStored` 未対応／`execStubPart1`→`effectExecutor` は**循環 import**）が要るのに、得るのは選択タイミングの前後だけ。**ブロッカーを潰すより、既に正準な別の形へ寄せるほうが正しい。**
  - **⚠名詞句 span は名詞（「カード」）まで含めて捕る**＝直前で切ったら `LRIG_COLOR_NOT_RE` が外れ、**ゾーンだけ直って色制限が黙って落ちた**。A/B 差分で気づいた＝**「直った」と思ったら必ず差分の中身を読む。**
  - **②`WD15-018-E1`（条件節ごと落ちて無条件バニッシュ）**＝`ENERGY_COUNT_FILTER` 節を追加。**前セッションで想定した `ENERGY_HAS_CARD` の移植は不要**（`evalCondition` に既に `ENERGY_COUNT_FILTER` があった）。⚠**真のブロッカーは別**＝`matchesFilter` は `colorNotMatchesLrig` を知らず、未解決のまま渡すと**黙って無視され「エナに何かあるか」だけの過剰条件**になる。`evalCondition` 側で cond.owner のルリグ基準に解決する処理を足し、golden で「赤ルリグ＋赤エナだけ→不成立」を固定した。
  - **⚠規則を足す表を間違えると黙って効かない**＝`STATE_CONDITION_CLAUSES` に足したが、「…を対象とし、」プレフィックスを許すループが spread しているのは **`STATE_CONDITION_CLAUSES_V2` だけ**だった。**新しい条件節は V2 に足す。**
  - **⚠golden が1件落ちたのは前セッションで自分が張ったトリップワイヤ**（「取り違えのまま色フィルタだけ足していないこと」）＝据置が解消されたので**期待値を反転**した。設計どおりの発火。
  - **ゲート**＝全緑。**golden 1418→1419**（+1）、**census 1164→1162**（−2・`BASELINE_HIGH` 更新。この系統は **OK 33→37／未配線 5→1**）、smoke 10679 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors、held **229 据置**。
  - **🚧次の一手＝§5d-0 の工程改善3件（ユーザー合意済み・通常のバッチより優先）**。①**被覆マトリクスの常設スクリプト化**（`npm run census:wiring`＝「語彙はあるが呼ばれていない」を機械検出。続き375 の試作で**134件を1パス検出**）②**残1162の真バグ率を無作為20件で測り直す**（続き369 の 11/12 は易しい系統を7バッチ消化した後なので古い。**ここが 90% か 50% かで見積もりが倍違う**）③**worklist を作業種別へ組み替え**（配線／機構／構造混線／計器較正）。**設計・根拠・実測値はすべて §5d-0 に書いてある。** ⚠**見積もりの現実**＝直近7セッションで 82効果・census −37（約12効果/セッション）、残 **1162効果**＝このままなら**約100セッション**。しかもバッチ収穫は逓減中（36→12→6→4→4）。**探索コスト自体を下げないと終わらない。** それが済んだら通常バッチへ復帰＝未着手の大クラスタは「条件節」283・「小さい数」181・「クラス指定」154。**新機構待ち登録**＝`WXK11-040-E2`／`WX14-072・075-E1`／`WXK09-029-BURST`（据置再評価）。**§5d 末尾の照合済み12件**も未修正。**Opusタスク12 の在庫は3件**〔(cxiii)(cxiv)(cxv)〕。**Sonnet**＝§7 実機検証（タスク1）＝続き366 の (cxvi) 分が最優先（⚠**P1 だけ完璧にしても「全カードが完璧に動く」には届かない**ので §7 は並行して進める）。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-07 続き375（§5d パターンA 第7バッチ＝続き374 のブロッカー2件を実装）後 最新値（本行が直近の正）**：golden **1418→1419**（+1＝エナ対象のゾーン是正3件／`WD15-018` の条件節／evalCondition のルリグ色解決）、census **1164→1162**（−2・`BASELINE_HIGH` も 1162 へ更新＝**機能是正**。この系統は OK **33→37**／未配線 5→1）、smoke **10679/10679** 全0・SKIP0、fuzz 全0、同型★**0 据置**（265群）、lint 0 errors/**248 warnings**（追加0）、manual field loss 0、held **229 据置／署名グループ 104件**。**live JSON の変更は4効果／4カード**（すべて held 経由で採用）。⚠**残す教訓**＝(a) **ブロッカーを潰すより、既に正準な別の形へ寄せられないか先に見る**＝`SELECT_TARGET_ONLY` 方式は engine 拡張3点（うち1つは循環 import）が要るのに得るのは選択タイミングの前後だけで、同族30効果が使う素の `TRASH{ENERGY_CARD,...,filter}` に寄せれば parser だけで済んだ。(b) **名詞句 span は名詞まで含めて捕る**＝直前で切ると `LRIG_COLOR_NOT_RE` 等が外れ、**ゾーンだけ直って色制限が黙って落ちる**。A/B 差分の中身を必ず読む。(c) **条件節の新規則は `STATE_CONDITION_CLAUSES_V2` に足す**＝「…を対象とし、」プレフィックスを許すループが spread しているのは V2 だけで、`STATE_CONDITION_CLAUSES` に足すと**黙って効かない**。(d) **条件内の動的フィルタは条件評価器側でも解決が要る**＝`matchesFilter` は `colorNotMatchesLrig` を知らないので、未解決だと**黙って無視され過剰条件**になる。**「型にあるキーを条件に入れた」だけでは効かない。**
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き375＝§5d パターンA 第7バッチ後）**＝golden **1419**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1162**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **229枚／104群**・実機 driver 既定order **126件**。
>
...
>
> 0. 🚧**まず §5d-0 の工程改善3件を片付ける**（2026-08-07 続き375 でユーザー合意・**通常のバッチより優先**）＝①`npm run census:wiring` の常設スクリプト化（「語彙はあるが呼ばれていない」を機械検出・試作で**134件を1パス検出**）②残1162の**真バグ率を無作為20件で測り直す**（続き369 の 11/12 は古い）③worklist を**作業種別**（配線／機構／構造混線／計器較正）へ組み替え。**設計・根拠・実測値は §5d-0**。⚠**現ペースだと完了まで約100セッション**（直近7セッションで 82効果・census −37、残 1162効果、バッチ収穫は 36→12→6→4→4 と逓減）＝**探索コストを下げるのが先**。
...
### 5d-0. 🚧**次セッションで最優先の工程改善3件**（2026-08-07 続き375 でユーザー合意・実測根拠つき）

> **なぜ**＝続き369〜375 の7セッションで **82効果／census 1199→1162（−37）** を消化した。平均 **約12効果・census −5.3／セッション**で、残 **1162効果**（クラスタ union で数え直して 1163＝census 公称と一致・実数）。このまま1件ずつ探すと **約100セッション**。しかも**バッチあたりの収穫は逓減している**（36→12→6→4→4）。**探索コスト自体を下げないと終わらない。**
>
> ⚠**この3件を先に片付けてから通常のバッチに戻ること。**（1）と（2）が終わると以後の見積もり精度が桁で上がる。

#### ① 被覆マトリクスを常設スクリプト化（`npm run census:wiring`）— 半セッション
...
**効果**＝作業単位が「1効果」から「**1セル（語彙×入口）**」に変わる。134件は **6〜10バッチ**で消化できる見込み（手探りなら15〜20セッション相当）。

⚠**偽陽性は必ず混じる**（リマインダー文の語・engine が原文から直接読んでいる形＝【ビート】コスト等）。**セルを取るたびに全CSV走査で用法を分類してから配線する**という §5d の既存規律は変えない。

#### ② 残1162の**真バグ率を測り直す**（無作為20件・シード固定）— 半セッション

**根拠＝続き369 の「11/12 が実バグ」は古い。** 易しい系統を7バッチ消化した後なので、いまの母集団の実バグ率は不明。
**ここが 90% なら残り約1050件、50% なら約580件＝見積もりが倍違う。この1回の測定が一番効く。**

**手順**＝`docs/_vocab_census.txt` の高シグナル union（1162）からシード固定で20件抽出 → 原文と live JSON を並べて目視照合 → 下の**作業種別**（③）で分類 → 比率を PLAN §5d-0 に実数で記録する。
