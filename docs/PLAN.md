...
| `S-37` | **作戦の「使いどころ」でスペルが「使わない」しか選べず死んでいる**＝守り／攻めの窓を持つのがアーツだけのため（`cpuPlanUseModesFor`）。スペルに「いつ使うか」を持たせる設計から決める（メインのスペルは1窓＝`pickCpuMainSpell`） | ユーザー（2026-09-25「後で調整する」） |
