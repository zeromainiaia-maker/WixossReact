...
- **🆕 セッション（2026-08-03・続き338・Opus 5〔**Claude 単独：実装・検証・簿記**〕）＝🏁**§3 タスク12(lxxxix) 残0クローズ**＝使用時の任意支払いのうち「支払い元＝**場のシグニをトラッシュ**」の2枚を実装し (lxxxv) を 31枚→**33枚**に（ユーザー指示「(lxxxix) を行う」）。golden **1339→1341**・live JSON **2カード**・census 1288 据置・同型★0・全ゲート緑。
  - **defer の理由どおり別の risk class だった**＝他の支払い元（手札／ダウンフラグ／ルリグデッキ／ライフ／キー）は平坦なゾーンを触るだけだが、場のシグニは**場スタックを崩す**うえ**離場トリガーの発火**が要る。
  - **①場からの除去は `execUtils.removeFromField` に委譲**＝自前で `field.signi[zi]` を空にすると、下のカード・チャーム・アクセ→トラッシュ／ソウル→ルリグトラッシュ／ダウン・凍結・血晶武装フラグのリセット／`card_identity_overrides` の後始末を全部取りこぼす。⚠**`removeFromField` はインスタンスIDでゾーンを引く**＝同じ文字列が2ゾーンに居ると先頭しか外れない（場の要素は `CardNum#n` で一意という不変条件に依存。検証で素の CardNum を2ゾーンに置くと2体払いが壊れることを確認済み）。
  - **②離場/トラッシュのトリガーを中央 diff（`collectBoardDiffTriggers`）で拾う**＝**これが defer の本体**。支払ったシグニを **`fieldTrashCostCards`** に載せて `byEffectCause=false`（＝コストによる支払い＝「効果によってトラッシュに置かれたとき」には該当しない）という既存の区別をそのまま使う。アーツは `queueCardEffects` の `extraEntries` へ、スペルは `QUEUE_SPELL` に `effectStack?`（**省略＝不干渉**）を足して同時に載せた。⚠`pending_spell` 待ちの間にスタックが載るが、**カットイン応答の継続もCPU行動も `if (bs.effect_stack …) return;` で待つ**ので「支払い→離場トリガー→カットイン窓→スペル解決」の順になる（実機でしか確かめられない＝要検証）。
  - **③原文の語順が2通り**＝「シグニ**を好きな数**場から」（比例・上限なし）と「…シグニ**１体を場から**」（固定・1体）で数量詞の位置が違う＝**1本の regex では読めない**ので別々に2本置いた。
  - **④parser の strip ゲートを1種ぶん広げた**＝(lxxxv) では「使用時UIを実装した支払い元の先頭句だけ」を許してこの2枚を意図的に除外していた。UI ができたので許可句を追加。`WX25-P1-110` は MANUAL なので (lxxxv) の外科パッチへ1枚追加（冪等）。これで同カードの `STUB OPTIONAL_COST` 頭＝**断ると本体が丸ごと不発**の罠も解消。
  - **live 差分は changed 2 / added 0 / removed 0 で、2枚とも「先頭1本除去のみ」で完全一致**（相乗りドリフトなし）。
  - **次の一手**＝**Opus**＝タスク12 の生き残り在庫〔(lv) 残2経路／(lxvi)／(lxvii)／(lxviii)／(lxxxvi)／(lxxxvii)／(lxxxviii)〕・(lxxxii) 由来の別在庫4件（PLAN_PROGRESS の続き331 行）・§6.3 機構台帳。**Sonnet**＝§7 実機検証＝**続き298〜338 の全件が実機UI未検証**。⚠🔴**続き333〜338 はいずれも BattleScreen/モーダル層＝golden から叩けるのは純関数まで**。最優先は依然**タスク14 のセットアップ〜ターン終了の通し**で、本件は**①`WX24-P3-010` を自分のシグニ2体払いで撃つと《黒》×4→《黒》×2 になり2体が下のカードごとトラッシュへ行く**②払ったシグニの `ON_LEAVE_FIELD`/`ON_TRASH` が発火する（かつ「効果によって」条件の札は発火しない）**③🔴`WX25-P1-110`（スペル）の順序**＝支払い→離場トリガー→カットイン窓→スペル解決 ④支払わずに使っても本体が動く。
### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-03 続き338（🏁§3 タスク12(lxxxix) 残0クローズ＝使用時の任意支払い「場のシグニをトラッシュ」2枚）後 最新値（本行が直近の正）**：golden **1339→1341**（+2＝①場のシグニ払いの後始末〔2ゾーン払いで下のカード・チャーム→トラッシュ／ソウル→ルリグトラッシュ／ダウン・凍結フラグのリセット／選んだゾーンだけ空く／ダウン中でも候補／`WX25-P1-110` のクラス・レベルフィルタ〕②`QUEUE_SPELL` の `effectStack` 指定時のみ `effect_stack` を書く。併せて (lxxxv) の3テーブルを 31→33 へ更新）、census **1288 据置**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**245 warnings 据置**、manual field loss 0、同型★0、held **257枚／109群 → 258枚／109群**。**live per-effect 差分＝changed 2／added 0／removed 0**（2枚とも「先頭1本除去のみ」で完全一致＝相乗りドリフトなし）。**新語彙 0本**＝engine の action/condition 型は増えていない（`BattleAction` も 18種のまま＝`QUEUE_SPELL` の payload 拡張のみ）。⚠**支払いUI・離場トリガーの発火順序は golden 非カバー**＝実機確認と対で締める。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-03 続き338＝🏁§3 タスク12(lxxxix) 残0クローズ後）**＝golden **1341**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1288**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**・held **258枚／109群**。
>
