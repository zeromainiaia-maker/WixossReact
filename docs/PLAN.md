...
- **🆕 セッション（2026-07-28 続き294〜296・Opus指示＋codex実装／Opus検証・是正3件）＝§3 タスク12(xxii) を完全クローズ（残10効果を3バッチで消化）**（golden 815→828・census 1519→1517・held 263→265枚/109署名据置）
  ...
  - **続き295（3効果・engine変更1箇所）**＝**前バッチの [C] 判定3件が誤りだった**ことを投入前実測で確定＝WDK08-Y01（`execReveal` の `HAND_CARD` source・`TRASHED_DISTINCT_LEVELS_GTE{allSigniDistinct}`・`storedTargetCards` が全部既存）／WXK06-031（`levelRange`＋`selectionConstraint{distinct:'level'}` で「レベル1〜4を1枚ずつ」と同値・`resumeSearch` が制約を実強制）／WXK11-024（`execTransferToDeck` が `optional && count!=='ALL'` と書かれ **ALL のときだけ optional が無視**されていた1箇所だけ）。**新 condition/state は0本**。
  - **続き296（3効果）**＝**クローズ条件を「原文の完全実装」ではなく「(あ)正しい条件に置換 or (い)`UNKNOWN{raw}` で過剰実行を停止」と定義し直して着地**。WX15-067-E1 は**何も起きないスペル**だった本体2択を復元（ウィルス節は `UNKNOWN`）／WXDi-P11-010A-E1 は**毎グロウフェイズにトラッシュ全部をデッキへ戻す暴走**を新 `EFFECTIVE_LRIG_LIMIT_GTE{9}` で停止（実効リミット式を `src/screens/battle/lrigLimit.ts` へ抽出し画面と条件で共有）／WXDi-P13-003A-E1 は**自軍シグニに存在しない「使用条件」キーワードを永続付与していた誤 `GRANT_KEYWORD`** を撤去し本文全体を `UNKNOWN` に。**⚠設計判断＝「代償だけ実装して見返りを defer」は禁止**（手札全捨て＋エナ全トラッシュだけ実装すると今より悪化する）。
  - **⚠検証是正3件（Claude）**＝**(1) `EFFECTIVE_LRIG_LIMIT_GTE` が実機で常に false になる dead flag だった**＝評価器が `!!ctx.effectsMap && …` なのに **BattleScreen の `ExecCtx` 構築8箇所すべてが `effectsMap` 未設定**。codex の golden は ctx を手組みして map を入れるため**緑のまま通過**（「golden 緑・実機 no-op」の再発）。engine 直呼びで実測確定→トリガー ctx に配線。**(2) 続き294 の負方向 golden が空振り**＝`logs.includes('追加コスト')` は engine の実ログ「任意コスト：発動しますか？」と一致せず**両方向で false**。実文字列へ是正＋肯定方向を追加し、閾値変異で FAIL することを機械証明。**(3) `self_deck_to_energy_this_turn` の加算漏れ1経路**（`execEnergyChargeByFieldCount`）を補完。
  - **検証**＝`npm run gates` 全緑（golden 828／smoke 10725 全0／fuzz 200ゲーム全0／lint 222w・0e）・`npm run regen` 済・同型★0。**per-effect 機械 diff は3バッチとも意図した効果ちょうど・巻き添え0**。⚠**実機未検証**。
  - **§6.3 へ送る不足機構**＝WX15-067（支払い前ウィルス除去UI／除去数連動コスト軽減／除去2以上で CHOOSE 上限1→2）・WXDi-P11-010A（3ゾーン一括デッキ戻し／ルリグデッキと場の「自身以外」一括除外／**ルリグ裏向き状態**）・WXDi-P13-003A（未グロウ＋ドリームチーム使用条件／**ピース両面反転**／特定カード指定の無料グロウ）。
  - **次の一手**＝**Opus：タスク12 の残在庫（(lv)／(xliv)／(xxxix)残／(xlii)残）または §6.3 残機構（上記3件の新規登録分を含む）／タスク14 の残43**。**Sonnet：タスク1（§7 実機検証）＝レゾナ召喚UIの残り①ATTACK 窓 ②SPELL_CUTIN 窓 ③REQUIRES_NEW_FLOW ④支払いトリガー発火**。
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（(xliv)・(xxxix)・(xlii)の残）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
