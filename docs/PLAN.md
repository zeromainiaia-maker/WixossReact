...
**直近＝2026-09-20＝§5.7 `S-23`＝本物のデッキで CPU を測れるようにした（`S-20`・`S-19` も同時に決着）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第421バッチ）
- 🔴**`S-20` の前提が実測で確定**＝A/B の既定の山が踏みうる効果の型は **23種**、ユーザー作26デッキの和集合は **85種**＝**64種は既定の山に一度も出てこない**。
  **`POWER_MODIFY` は既定の山に 0枚／26デッキ中23デッキが持つ**＝`S-10` は**直した当の型を1ノードも踏まずに**「勝率は下がらない」と出していた。
- 🏁**山を差し替えられるようにした**＝`scripts/selfPlayDecks.ts`（新設・**名前が一意に決まらなければ例外**）＋`--deck` / `--deck-a` / `--deck-b` / `--decks`（総当たり）/ `--logs-out`。
  **作戦データ（`S-2`）は席ごと**（`cpuPlans`）＝デッキごとに違うので1つに畳めない。**出力に「その山が踏みうる型の数」を必ず出す**（`S-20` ③）。
- 🔴**`S-19`（先攻が勝てない）は再計測で否定**＝本物のデッキ4つの総当たり96戦で **先攻 59.4% [49.4, 68.7]**（合成デッキは 28.1% [18.6, 40.1]＝**区間が重ならない**）。
  ⇒ **engine の先攻不利ではなく `VERIFY_DECK_MECH` 固有の偏り**。🔑**勝率を書くときは必ず山の名前も書く。**
- 📊**総当たり（96戦・止まり0）**＝WD13 **75.0%** ＞ 天使軸1 60.4% ＞ ケトッシー軸 45.8% ＞ WD06 **18.8%**。**山ごとの機構踏破 13〜14 / 23**（実機1戦は 7/23）。
  **未踏9のうち8は「その山に札が無い／アシスト役が未設定」**（実測）で、残る1つ＝**マリガンだけがハーネスの制限**⇒ `S-24` を登録。
- 🏁**同じセッションで `S-17` 第1段も入れた**（第422バッチ）＝**アタックを候補列挙に載せた**（`signiAttack`／`lrigAttack`）＝
  本番の選択も同じ列挙から取るようにし、**「打った手は列挙に出ている」の全数照合がアタックにも掛かる**ようになった。
  🔴**挙動不変を実測**（自己対戦3シードの全ログが一致）。📏**アタックの候補は中央1・最大3**＝第2段の探索は**手順 3!=6 通り**で足りる。
- 🏁**`S-17` 第2段も入れた**（第423バッチ）＝アタックの**近似適用**（`battleOutcome` → `banishDestination` ／ ライフクラッシュ）と探索の配線（**既定オフ**）。
  🔴**実測で設計を変えた＝探索が決めるのは「順番」だけ**＝最初の配線は**同点の「何もしない」に負けてアタックを1戦20回捨てていた**。
- 🔴🔑**`S-25`＝その「探索は弱い」も私の早とちりだった**（第424バッチで訂正）＝**WD13 1つ**で測って一般化していた。
  **6デッキ × 8シードで測り直すと `search` は `default` より強い＝合算 組 83.3% [66.4, 92.7]**（強い3／差なし2／**弱いのは WD13 だけ**）。
  **WD13 の中身**＝探索は**ルリグの【起】を1度も使わない**（default 30回）／召喚が 1.27 vs 1.59体/ターン／**試合が倍に伸びる**＝
  🔑**盤面の採点に「アップ」の価値が無く、自分のシグニを戻すと `openLane` を失うだけに見える**。
  🆕道具＝**ポリシー × 山の表**（`--a <policy> --b <policy> --decks "A,B,C"`）。⚠**「差あり」が出ても山が1つなら一般化しない。**
- 🏁🔥**2026-09-21＝ユーザー決定で CPU の既定を「探索あり」へ上げた**（`searchWidth/Depth` 0 → 4/4・旧値は `legacy-greedy` に保存）＝
  **実機の CPU がメインフェイズを探索で打つ**。反転の実測＝`legacy-greedy` vs `default` が**ケトッシー軸 0-0-8／WD13 4-4-0**＝前日の表の鏡像。
  回帰＝`selfplay` 決着・止まり0／**機構踏破 15/23**（スペル・アーツ・ライズ・ピース・キーが出ている）／**実機 PASS 6ターン132手・遅くなっていない**。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**`S-25` ①＝WD13 だけ弱い原因**（盤面の採点に「アップ」の価値が無い＝`S-6` の重み学習と同じ土俵）。次に `S-17` 第3段・`S-22`・`S-24`・`S-14`・`S-6`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 6**（`S-25`・`S-17`・`S-22`・`S-24`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①**探索は WD13 でだけ弱い**（`S-25`・他の5デッキでは強いか差なし・既定はオフなので実機には出ない）②**CPU が「対象を選ぶ」ときに効果の意味を見ていない**（`S-22`）③**自己対戦がマリガンを一度も踏まない**（`S-24`）。🏁`S-19`（先攻不利）は**再計測で否定**＝山の性質だった |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4347**）｜**実機 PASS 4本**（`verifyFullMatch.mjs cpu`・最後の回は `scripts/` だけなので不要）｜反転確認＝`§5.7 S-23`／`§5.7 S-17` の両方で取得済み |
...
| 🔥`S-17` | **後半ブロックの探索**＝`ATTACK_ARTS_OP`（相手の妨害）が終わった**実際の盤面**から `ATTACK_SIGNI`／`ATTACK_LRIG` の手順を探索する。🏁**第1段（2026-09-20）＝アタックを候補列挙に載せた**（`signiAttack`／`lrigAttack`・本番の選択も同じ列挙から・挙動不変）。🏁**第2段（同日）＝近似適用と探索の配線**（`simAttack`＝ダウン→`ON_ATTACK_SIGNI` の【自】→`battleOutcome`→`banishDestination`／ライフクラッシュ）＝**入切は `CpuPolicy.searchAttacks`・既定 false**。🔴**実測で設計を変えた＝探索が決めるのは「順番」だけ**（`null` は従来の価値表へ落とす）＝**同点の「何もしない」に負けてアタックを20回捨てていた**。📏A/B（`WD13`・16戦）＝**アタック探索だけの差は8組すべて1勝1敗＝ほぼ同じ打ち方**。🔥**次＝第3段**＝**ライフバースト・ガードを確率で見る**（いまの近似は楽観＝**「撃たない」が正しくなるのはここから**）。 | 大 | 攻撃順・アタック誘発・バニッシュ誘発・被アタック誘発をまとめて判断する。⤵**`S-8` はここに吸収**。⚠**第3段の前に `S-25`（探索が本物のデッキで弱い）を片付ける**＝土台の探索が負けているうちは順番を磨いても勝率に出ない |
...
| 🔥`S-25` | 🏁**② 既定を上げた（2026-09-21 ユーザー決定）**＝`DEFAULT_CPU_POLICY.searchWidth/Depth` を **0 → 4/4**＝**実機の CPU が探索で打つ**（旧値は `legacy-greedy` プリセットに保存）。根拠＝**6デッキ × 8シード＝96戦で 合算 組 83.3% [66.4, 92.7]**（強い＝ケトッシー軸 8-0-0／天使軸1 6-2-0／WD06 6-2-0｜差なし＝WD15 3-5-0／WD16 2-5-1｜**弱い＝WD13 0-4-4 だけ**）。⚠アタック探索（`searchAttacks`）は **false のまま**（差が無い）。🔥**残り＝① WD13 だけ弱い原因**＝探索は**ルリグの【起】を1度も使わない**（default は30回）／召喚 1.27 vs 1.59体/ターン／試合が倍に伸びる。見立て＝**盤面の採点に「アップ」の価値が無く**（`turnDamage` 既定0）、**自分のシグニを戻すと `openLane` を失うだけに見える**（`WD13-003-E2` を -1000 と採点）。⇒ **採点の重みの問題＝`S-6`（自己対戦で重みを学習）と同じ土俵**。既存プリセットでは直らない（`search-power`／`search-act-mild`／`search-act`／`search-damage` を実測済み）。🆕**2026-09-21＝重みを CLI から差し替えられるようにした**（`--a-set "fieldPowerScale=1,openLane=1500"`＝`patchCpuPolicy`）＝**プリセットを足さずに仮説を試せる**（`S-6` の前提）。📏**WD13 の重み掃引（各8シード）**＝`fieldPowerScale=1` **2-6-0**（唯一方向が良い）／`turnDamage=3000,fieldPowerScale=1` 2-5-1／`hand=800` **0-8-0（1組も動かない）**／`openLane=1500` 0-7-1。⇒ **見立て＝「盤面に出す価値が安すぎる」（`fieldPowerScale: 0.25`）**。⚠どれも「差あり」ではない＝**方向の情報**。🔥**次＝`fieldPowerScale=1` を6デッキで測る**（WD13 が直っても他の山が落ちるなら採らない）。 | 中 | **道具は揃った**＝`npm run selfplay:ab -- --a <policy> --b <policy> --decks "A,B,C"`（**ポリシー × 山の表**）。🔑**1つの山の勝率でポリシーを一般化しない** |
| `S-24` | **自己対戦がマリガンを一度も踏まない**＝`scripts/headlessSelfPlay.ts` は `applyMulligan(state, [])` 固定（対戦開始＝じゃんけん・マリガン・ルリグ配置は**画面の SETUP 経路にあり `controller/` へ未移設**＝`S-5` の但し書き）。実測＝`census:play` で**4デッキ × 48戦すべて「マリガン 0回」**＝**自己対戦の台が構造的に踏めない唯一の機構**（他の未踏8つは「その山に札が無い／アシスト役が未設定」）。🔑**`S-2` の作戦データはマリガンにも効く**（`planKeepsInMulligan`）＝キーカードを戻さない判断が A/B に一度も乗っていない。⚠**直すと乱数列が動く**＝過去の勝率と比較できなくなるので、**直した回にベースラインを撮り直す**。 | 小〜中 | 自己対戦が実機と同じ開始手順になる＝`S-6`（重みの自己調整）がマリガンの重みも学べる |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🔥**`S-17`（後半＝攻撃探索）**→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-21 時点**（第421〜425バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ `S-17` 第1〜2段 ＋ `S-25` の測り直しと**既定の引き上げ**）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 6**（`S-25`・`S-17`・`S-22`・`S-24`・`S-14`・`S-6`）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4347**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 4本**（6/133手・8/191手・7/174手・7/170手）
  - 🆕📏**山が踏みうる効果の型**＝既定の合成デッキ `VERIFY_DECK_MECH` **23種** ／ ユーザー作26デッキの**和集合 85種**（**64種は既定の山に出てこない**）。`POWER_MODIFY` ＝**既定の山 0枚 / 26デッキ中23デッキ**
  - 🆕📊**総当たり（4デッキ・96戦・止まり0・378秒）**＝WD13 **75.0%（36/48）** ＞ 天使軸1 60.4% ＞ ケトッシー軸 45.8% ＞ WD06 **18.8%（9/48）**
  - 🆕📏**先攻の勝率**＝**本物のデッキ 59.4%（57/96）[49.4, 68.7]** ／ **合成デッキ 28.1%（18/64）[18.6, 40.1]**＝**区間が重ならない**（`S-19` は山の性質だった）
  - 🆕📊**機構踏破（自己対戦・48戦ずつ）**＝**13〜14 / 23**（実機1戦は 7/23）。**未踏9のうち8は「山に札が無い／アシスト役が未設定」**・**マリガンだけがハーネスの制限**（`S-24`）
  ...
  - 🆕📏**アタック系トリガー**＝`ON_ATTACK_SIGNI` **684効果/667枚**（登録票の 665/648 から +19）｜`ON_ATTACK_LRIG` **60/59**｜`ON_BANISH` **157/157**｜`ON_LEAVE_FIELD` **68/68**｜**被アタック側は 5効果**
  - 🆕📊**探索の強さ（`search` vs `default`・6デッキ × 8シード＝96戦・止まり0）**＝**合算 組 83.3% [66.4, 92.7]**｜強い＝ケトッシー軸 8-0-0／天使軸1 6-2-0／WD06 6-2-0｜差なし＝WD15 3-5-0／WD16 2-5-1｜🔴**弱い＝WD13 0-4-4 だけ**
  - 🆕📏**WD13 で探索が使わないもの**＝**ルリグの【起】 0回 / default 30回**（`WD13-003-E2`）｜召喚 **1.27 / 1.59 体（1ターン）**｜試合の長さ **28・17T / 13・14T**
  - 🆕📊**アタック探索だけの差**＝`search-attack` vs `search`（WD13）**8組すべて1勝1敗（差なし）**
  - 🆕🔥**CPU の既定（2026-09-21）**＝`searchWidth/searchDepth` **4/4**（探索あり・`searchAttacks` は false）｜旧値＝`legacy-greedy`｜反転の実測＝`legacy-greedy` vs `default` **ケトッシー軸 0-0-8／WD13 4-4-0**｜実機 **6ターン/132手/176s**｜機構踏破 **15/23**（自己対戦2戦）
