...
| (lxxxiii) | **一部消化（2026-08-02）**＝効果原文の実測母集団は **170効果**、投入前 `excludeSelf` あり79／なし91。なし91の内訳＝A action target 48／B trigger 20／C STUB 16／D 誤検出7、さらに `owner:any` 混入27。今回 A は **34/48** を live 是正し、同じ対象に混入していた `owner:any→self` は19効果で同時是正。原文照合済みの有効 outlier 11効果も同規則で是正し、live の `excludeSelf` 増加は計45効果・誤巻き込み0・既存からの減少0。**残A 14**＝STUB内コスト／引用付与平坦化／同一対象照応で、filter を宣言するだけでは engine が読まないため defer。**残B20・C16・owner未是正8** も collector/機構単位で継続。詳細と全IDは BUGFIXES 2026-08-02 (lxxxiii) 節。 |
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-02 task12(lxxxiii) 部分消化後）**＝golden **1278**・smoke **10679** 全0（SKIP も 0）・fuzz 全0（distinct 2708種）・同型★0・census **1300**（回帰ゲート）・manual field loss 0・lint 0 errors/**240 warnings 据置**（`npx eslint . --no-cache --ignore-pattern "tmp_*.ts"` 実測）・held **260枚**。census 1313→1300 は「あなたの他の…シグニ」の対象拡大を止めた**機能是正**。
>
