...
- **🆕 デッキ全体サーチ→シャッフル→デッキの一番上（続き442 で消化・残1＋別家族7）**：原文「あなたのデッキから（限定つき）カード/シグニ1枚を探す→デッキをシャッフル→そのカードをデッキの一番上（or 一番目か二番目）に置く」の**4効果**を既存 `SearchAction` へ機構化（`WXK02-031-E2`／`WXK02-070-E1`／`WXK03-049-E1`／`WD23-024-E-E1`）。**新 action 型ゼロ・defer 0**。census 846 据置・golden 1834→1841。詳細は BUGFIXES 2026-08-11（続き442）。
  ...
  - 📋**残1（同じ文型の5枚目・未着手）＝`WD23-013-A-E1`（アンコール）は live で完全な no-op**＝原文「あなたのデッキからカード１枚を探してデッキをシャッフルし、そのカードをデッキの一番上に置く」なのに JSON は `TRANSFER_TO_DECK{DECK_CARD,top,shuffle}` **だけで SEARCH が無い**（ベースラインでも `execTransferToDeck` の6分岐を素通りして `return done(ctx)`＝続き442 の回帰ではない）。**機構ができたので安い**＝parser の `foldDeckSearchToTop` は `探(して公開する|してもよい|す)。` と**句点**を要求するため「探し**て**デッキをシャッフルし」を拾えない。⚠**裸STUBを持たないので、裸STUB を入口にした棚卸しでは原理的に見つからない**（`live の TRANSFER_TO_DECK{DECK_CARD}` 全6箇所の走査で発見）＝続き425 の `SPDi43-06` と同型の計器の死角。
  - 📋**別家族7枚（裸 `STUB{REVEAL_AND_PICK}` の残り・いずれも今なお万能サーチ化した実バグ）**＝**reveal-until 3枚**（`WDK13-011`／`WXK07-054-CB`＝「公開されたシグニのレベルの合計がN以上になるまで」／`WXK07-034`＝「レベル4のシグニが2枚めくれるまで」＝⚠停止条件に加え **`RevealUntilToHandAction` は `revealClass` しか持たずレベル指定ができない**）／**手札公開コスト2枚**（`WXK05-027`／`WXK05-071`＝原文は「**手札から**＜水獣＞をN枚公開してもよい」＝コストであってデッキ操作ではない）／`PR-457`（デッキ上2枚とセンタールリグの共通色を照合して全部エナへ）／`WXDi-P11-043`（デッキトップと宣言数字の照合）。**T1（live の裸STUBはこの7枚だけ）で固定済み**＝増えたら golden が赤くなる。
