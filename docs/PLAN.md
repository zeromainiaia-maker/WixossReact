...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**`S-25` ②＝既定を `searchWidth: 4` へ上げるかの判断（ユーザー）**／`S-25` ①＝WD13 だけ弱い原因（盤面の採点に「アップ」の価値が無い）。次に `S-17` 第3段・`S-22`・`S-24`・`S-14`・`S-6`。🏁**§5.3 機構 worklist は残0** |
...
| 🔥`S-25` | 🔴**登録票の前提は誤りだった（2026-09-20 に自分で訂正）**＝「探索は本物のデッキで弱い」は**1つの山（WD13）からの一般化**。**6デッキ × 8シードで測り直すと `search` は `default` より強い**＝**合算 組83.3% [66.4, 92.7]**（A連勝25／1勝1敗18／B連勝5）。内訳＝**強い**＝ケトッシー軸 8-0-0／天使軸1 6-2-0／WD06 6-2-0｜**差なし**＝WD15 3-5-0／WD16 2-5-1｜🔴**弱い＝WD13 0-4-4 の1つだけ**。**残る問い2つ**＝①**WD13 で何が起きているか**（実測＝探索は**ルリグの【起】を1度も使わない**（default は30回）／**1ターンの召喚が 1.27 vs 1.59体**／**試合が倍に伸びる**（28・17T vs 13・14T）。`WD13-003-E2`＝エクシード1「白のシグニ1体を手札に戻す＋シグニ1体をアップ」を探索は **-1000** と採点する＝**盤面の採点に「アップ」の価値が無く、バウンスは openLane を失うだけに見える**）②🔥**既定を `searchWidth: 4` へ上げるか**（＝実機の CPU が強くなる。**判断はユーザー**）。既存の切り分けプリセットでは WD13 は直らない（`search-power` 2-6-0・vs default 1-7-8／`search-act-mild` 1-7-0／`search-act` 2-2-4／`search-damage` 差なし）。 | 中 | **道具は揃った**＝`npm run selfplay:ab -- --a <policy> --b <policy> --decks "A,B,C"`（**ポリシー × 山の表**）。🔑**1つの山の勝率でポリシーを一般化しない** |
...
- **2026-09-20 時点**（第421〜424バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ `S-17` 第1〜2段 ＋ `S-25` の測り直し）
