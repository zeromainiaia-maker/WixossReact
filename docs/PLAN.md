...
**直近＝2026-09-21＝§5.7 `S-6` 第2段＝調整できる数値49個を全部振れるようにして全数走査し、多次元 A/B に掛けた**（全文は [BUGFIXES.md](./BUGFIXES.md) 第430バッチ）
- 🏁**①配管**＝残り29個を `CpuPolicy` へ載せた（`strengthWeights` 14／`keywordValues` 8／`planWeights` 6／`guardKeepValue` 1）。
  **値は1つも変えていない**（旧名は実体を指す別名）。🔴**接頭辞が要る**＝`strength.` / `keyword.` / `plan.`（**`energy` は両方に在る**）。
  🔑**挙動不変の裏取り2本**＝`selfplay` ゲートが第1段と同一結果／**旧20個の分岐の数字が1件も変わらなかった**。
- 📏**②走査＝93候補 × 6デッキ × 2シード＝1,128対戦**。**新29個のうち判断を変えたのは11個だけ**（`strength.search` 10/12・`strength.draw` 7/12 …）。
  **分岐0 は 8→36件**で、原因はまた**山**＝`keyword.*` は6デッキ合計0枚が10件／`plan.*` は `keyCards` 1枚・`combos` 1つしか無く順位が反転しない。
  ⇒ 🔑**`plan.*` はこの6デッキでは原理的に測れない**（`S-14` の前提が数字になった）。
- 🔴🔑**③多次元 A/B＝4束とも全6デッキで「差があるとは言えない」**（各96戦・止まり0）＝**合計 A2連勝 24 - 1勝1敗 144 - B2連勝 24＝ちょうど五分**。
  **分岐しているのに勝率が動かない**（`actionBias` は 12/12 で分岐して勝率 42.9%）。
- 🔴**これが `S-6` の本当の壁**＝**192組の75%が1勝1敗＝勝敗の二値が情報の4分の3を捨てている**。
  1候補8分で決着する組は10〜14しか出ず、下限が50%を超えるには70%以上が要る＝**検出できるのは大きな差だけ＝重みの微調整は原理的に測れない**。
- ⤵**`S-27` を登録**＝**勝敗ではなく「席入れ替えの対ごとのライフ差」で測る**＝捨てている144組がそのまま標本になる（`hostLife`/`guestLife` は既にある）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-27`**（測定台の感度＝連続量で測る。これが無いと `S-6` は先へ進めない）。次に `S-26`・`S-22`・`S-24`・`S-14`。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 5**（🆕`S-27`・`S-26`・`S-22`・`S-24`・`S-14`＝**`S-6` は第2段まで完了して `S-27` 待ち**）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4350**）｜**実機 PASS**（`verifyFullMatch.mjs cpu`＝6ターン/131手/218s）｜反転確認＝旧20個の分岐の数字が第1段と完全一致（配管の付け替えで挙動不変） |
...
| 🔥`S-27` | 🔴**A/B の感度を上げる＝勝敗の二値ではなく「連続量」で測る**。実測（`S-6` 第2段・4束 × 96戦）＝**192組のうち144組（75%）が1勝1敗**＝**席入れ替えで相殺した残りしか標本にならない**（決着する組は1候補あたり10〜14）。⇒ 下限が50%を超えるには**70%以上**が要る＝**重みの微調整は原理的に測れない**。🔑**直し方**＝1勝1敗の組も「**何点差で勝ったか**」を持っている＝**対ごとの符号つきの差**（残ライフ差・決着ターン差）を足し上げれば、**捨てている144組がそのまま標本になる**。`scripts/headlessSelfPlay.ts` は既に `hostLife`／`guestLife`／`turns` を返している＝**集計（`scripts/selfPlayStats.ts`）に1本足すだけ**（純関数なので golden で固定できる）。⚠**主の判定を差し替えない**＝組の勝率は残し、**連続量は「差が出そうかを安く見る」補助**として並べる（勝率こそが目的関数だから）。⚠**ライフ差は上限7でサチる**＝デッキ切れ勝ちは別扱いが要る。 | 小〜中 | `S-6` が先へ進む＝同じ8分で桁違いに細かい差が見える |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1段・第2段まで済／`S-27` 待ち）**→ 🔥**`S-27`（測定台の感度）**→ `S-26`（グロウが盤面を見ない）→ `S-14`（作戦データ）。
