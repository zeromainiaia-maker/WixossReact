...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxi) 第4波＝tail-splice（前置きのアクションはゲートの外）＋前波 triage の訂正1件（残9→7）**（golden **1142→1145**・census **1367 据置**（`BASELINE_HIGH=1367`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **250枚 据置**〔署名 97 据置〕・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects` 冪等）
  - **tail-splice**＝「…そうした場合、**カードを１枚引き**、対戦相手が手札を３枚捨てないかぎり、対戦相手にダメージを与える」（`WX24-P2-022-E1`）。丸ごと包むと「**回避されたら引きも消える**」過少実行になるため第1波で除外②として丸ごと据置＝**ダメージが無条件**の過剰実行が残っていた。前置きをゲートの外に出し、末尾の帰結だけを包む。
  - **⚠前置きは再パースしない**（連用形「〜引き、」の単独文化は変換が要り別の退化を招く）。**基準読みの前段ステップをそのまま使い**、「**基準読みの最終ステップ ＝ クローズ以降の帰結を単独パースした読み**」が JSON 完全一致したときだけ切る＝分割で読みが変わっていないことの証明（第1波の純加算チェックと同じ役割）。
  - **正準形の実在証明**＝`WX24-P3-050-E1`（MANUAL 手配線）が `SEQUENCE[SET_CANCEL_ATTACK_FLAG, OPPONENT_PAY_OPTIONAL, CONDITIONAL{…, LIFE_CRASH}]`＝まさにこの形。**新機構ではなく既存の手配線を規則化しただけ**。機械走査で除外②に落ちていたのは全CSV 4効果だけ（`WX24-P2-022` 消化／`WX24-P3-050` は MANUAL で既に正しい／`WX24-P4-011`・`WXDi-P06-023` は遅延トリガー設置が前置きで tail-splice にも乗らず据置）。
  - **⚠自分の簿記も疑う（第4波の教訓）**＝第3波で「前置きにアクションがある2効果」の片方に入れていた `WXDi-P13-075-E1` は**誤り**で、専用 STUB `UPKEEP_OR_NO_UP` が回避条件ごと engine（`lrig_upkeep_condition`）＋UI（UPフェイズの支払い/手札捨てダイアログ・CPU 自動支払い）に**実装済み＝既に正しい**。標準ペアで包むと相手に CHOOSE が2回出る。engine 直叩きの golden を1本足して固定した。
  - **次の一手**＝**Opus：①「コストの合計」族の STUB/UNKNOWN 14件②`WX11-043-E1` の `crossOnly` 欠落③タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**④`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑤§6.4 の配置数制限すり抜け7効果⑥§6.3 I＝`WX25-P3-028-E2` の3機構⑦(lxvi) の据置2枚⑧🆕 (lxi) 残7のうち「遅延トリガー設置＋その中身をゲート」2効果**（`WX24-P4-011-E3`／`WXDi-P06-023-E2`。設置を残したまま中身だけ包む機構が要る）／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝(lxi) 第3波の相手側 CHOOSE 4択目／(lxv) の条件つき任意コスト 36枚／(lxiv) の対象ピッカー前置 61枚ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxi) 第4波後 最新値（本行を直近の正とする）**：census **1367 据置**（`BASELINE_HIGH=1367`）、golden **1142→1145**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **250枚／署名グループ 97 据置**、manual field loss 0。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WX24-P2-022-E1`）、`build:effects` 冪等。**新語彙0本**＝既存の `OPPONENT_PAY_OPTIONAL`＋`CONDITIONAL(IS_MY_TURN)` ペアを置く位置を変えただけ（engine 無改修）。
