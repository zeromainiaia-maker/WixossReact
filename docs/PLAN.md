...
**直近＝2026-09-17＝第398バッチ：ユーザー裁定4件（`R-50`／`R-48`／`R-45b`／`R-27`）を実装し、ルール台帳の読み待ちを残1に**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- `R-50`＝ピースは**ルリグトラッシュ**（いまの実装が正）と決着＝コード変更なし。台帳の規則文を訂正。
- 🔴`R-48`＝**基本は配置制限／レベルが変動して超過したらそのシグニをトラッシュ**＝`実効レベル > センターのレベル` かつ `実効レベル > 印字レベル` のときだけ落とす（印字のまま超過している盤面は配置制限の領分＝落とさない）。
- 🔴`R-45b`＝**クラフトは場を離れるとゲームから取り除かれる**（`シグニ/レゾナクラフト` live 10枚）＝`excluded` へ。ルリグデッキへ戻すと**同じクラフトを何度でも出し直せる**側に倒れる。
- 🔴`R-27`＝**強制終了でも予約済みの追加ターンを開始する**＝`applyForcedTurnEnd` が `resolveTurnHandover` を見るようにし、アップを受ける側も切り替えた（強制終了3経路とも `keepTurn` を見る）。
- ＋`R-54`（〈ルリグ〉限定は**センタールリグだけ**を参照）を台帳に追加＝実装と一致を確認して golden で固定。
- 🔴**前バッチの実測を訂正**＝「ルリグのレベルを下げる効果は0枚」は**誤り**（`SP38-005` が1枚ある）。regex が「レベルを－１する」を取り落としていた。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔴**`R-47` のユーザー判断1件**（§5.6.2b）／**`C-8`**（対話応答の pure 化）／`R-48`・`R-45b` の残件（§5.6.2b の下表） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は engine・UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／**CPU 完成度 1**（§5.6 `C-8`）／ルール台帳 ⚠1（読み待ち） |
| 🔧**ゲート** | `npm run gates` 全緑（golden 4294） |
---
...
| 🔴**⑥** | **§5.6.2b ルール解釈待ち `R-nn`** | **1** | **Claude は取らない**＝読みを決めないと盤面からカードを消す／消さない側の実装になる | §5.6.2b の表（全文は [RULES.md](./RULES.md) §2） |
...
| 🔥`C-9` | **ルール規則の棚卸し**（バトル／ダメージ／フェイズ／ゾーン移動の行き先） | M〜L | **台帳＝[RULES.md](./RULES.md)**（公式ルールの1文 × 実装箇所 × 状態）。**2026-09-17 着手**＝✅1／🔴→✅13／👀20／**⚠1**（残りは `R-47` の読み待ち1件だけ＝**§5.6.2b の表**）。取り方は RULES.md §4（⚠を上から・写経を全部数える・純関数＋golden） |

#### 5.6.2b ルール解釈待ち（`R-nn`・🔴Claude は取らない）

> **全文（規則の原文・実装箇所・状態）は [RULES.md](./RULES.md) §2**。ここは**何を決めればいいか**の索引だけ。
> 🏁**2026-09-17 にユーザー裁定で4件決着**＝`R-50`（ピース＝ルリグトラッシュ＝いまの実装が正）／
> `R-48`（配置制限＋レベル変動で超過したら落とす）／`R-45b`（クラフトは場を離れると除外）／
> `R-27`（強制終了でも追加ターンを開始する）。⇒ 実装は第398バッチ（BUGFIXES）。

| ID | 決めること | いまの実装 | 決まったらやること |
|---|---|---|---|
| `R-47` | **同じルリグタイプのルリグが複数場にあるとき、落とすのは「センター以外」全部か、センターと重なったものだけか** | 実装なし（突き合わせる箇所が `src/` に1つも無い） | 前者だと**アシスト同士**が同タイプのとき両方消える。⚠アシストルリグの `CardClass` は**57種すべてがセンターにも存在する**（実測）＝同タイプの組み合わせは構築できる |

**決着した項目の残件（Claude が取ってよい・§5.3 へ登録するかは着手時に判断）**

| 出所 | 残っていること |
|---|---|
| `R-48` | **ルリグ側のレベル低下**（`SP38-005`＝「対戦相手のルリグ１体…それのレベルを－１する」・live 1枚）で生じる超過が未対応＝実効レベル読みが `attack_phase_level_overrides`（相手 state に載る）を見ていない |
| `R-45b` | **バニッシュ以外の離場**（効果で手札／デッキ／トラッシュへ置かれる場合）＝`removeFromField` の呼び出し71箇所は `banishDestination` を通らない |
