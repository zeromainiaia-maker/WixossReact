...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377＝(i)配線ギャップ 第6バッチ後）**＝golden **1436**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1053**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **211枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 476**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第7バッチ。★★セルから取る。**
>    - **① `noGuard × TRASH_CARD[filter]`（miss 7・has 61）★★＝最も明確な穴**。同じ入口で61件が既に配線済み＝「同じ入口の他の効果は正しいのにこれだけ落ちている」。
>    - **② `levelRange × BANISH{SIGNI}`（miss 6・has 58）★★** ＝続き377 で BANISH{ALL} 側は直したので、**残りは count 指定側の BANISH ビルダー**。同型なので続き377 の span 方式がそのまま使える見込み。
>    - **③ `hasRiseIcon` 31（has 8）★／`hasCrossIcon` 22（has 2）★** ＝`WX16-026-BURST`「トラッシュから何でも回収できる」・`WX07-010-E1`「自分の全シグニに+1000」＝**実害が明確**。
>    - ⚠**着手前に必ず読む4つの罠**＝(a) **素の `parseStoryFilter(文全体)` を対象フィルタに使わない**＝**対象名詞句に隣接**するときだけ拾う（続き376d）。(b) **曖昧なら付けない**（クラス2つ以上の span は片方だけ載せると**原文と逆の過小実行**）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**。**live diff だけ見ると MANUAL/カード単位 PRESERVE の差分を見落とす**（続き377 で 12件中2件）。(d) **`matchesFilter` は `excludeSelf` を見ない**＝**exec 側の配線を必ず確認する**（続き377 で `execBanish` が未配線と判明）。
>    - **✅「他の」ゲートの棚卸しは続き377 で完了**＝`hasOtherSelfSigniNoun` の全13使用箇所を確認し、`excludeSelf` を立てるだけの箇所は定義どおり正しく、gate が穴になっていた2箇所（BANISH{ALL}・UP）を是正済み。**この系統は打ち止め。**
>    - **(iv) 計器較正の残**＝条件節クラスタは**279件が269テンプレ＝ほぼ単発**で較正クラスタではないと実測済み。**較正で大きく減る余地はほぼ枯れた**＝以後は (i) 中心でよい。
...
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない（グロウ／ベット／アンコール／【出】コストは未検証）。
>    - ⚠**見積もりの現在地**＝残 census 1053／実バグ率は較正で母集団が締まったぶん**約75%**＝**実バグ約800件**。**続き377 は1バッチで12効果＋engine 1本**（続き376 は5バッチ66効果＝1バッチ13効果）＝**バッチ収穫は逓減していない＝被覆マトリクスが効いている。**
