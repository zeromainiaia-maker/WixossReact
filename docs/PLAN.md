...
- 🏁**セッション（2026-08-25・続き659・Opus 5）＝§5.3 `O-44`＋`O-45` を消化＝①発生源クラス限定の規約を fail-closed に統一 ②「下に置く」の名詞句修飾落ちを4ビルダーで塞いだ**
  - ✅**`O-44`＝規約は原文から決めた**（機械的に揃えない・PLAN の指示どおり）。この語彙を使う live 効果は**2件だけ**で、どちらも「あなたの＜毒牙＞のシグニの**効果によって**」＝**原因の特定が意味の一部**＝fail-closed が忠実。
  - 🔴**真因は2段だった**＝①コレクタが `decreaseSources.length > 0` でクラス判定ごと素通り ②`detectPowerDecreaseSources` が「1件でも `srcCardNum` 未記録なら空配列」。**①だけ直すと逆に永久非発火**（`temp_power_mods` へ書く85経路のうち `srcCardNum` を刻むのは `execPowerModify` 系だけ）。**中央 diff の `causeSourceCardNum` へ寄せてから** fail-closed にして両立させた。
  - 🔑**反転シナリオを1度作り直した**＝最初の反転は**旧実装でも PASS**した。選択を挟む `POWER_MODIFY_PER_*` は `POWER_MODIFY` へ変換して `selectOrInteract` に渡すので `srcCardNum` が入るため。**fail-open が牙を剥くのは `count:'ALL'`**（`applyMod` が直接書く）側だった＝**「その語彙を使う経路」を実測せずに反転を組むと、直っていなくても PASS する反転ができる**。
  - ✅**`O-45`＝登録票は1件だが実測すると9効果**（live 41ノードを全数で見た）。レベル句・クラス句・カード名・「《ガードアイコン》を持たない」を**捕捉するだけで捨てていた**のが4ビルダーに散っていた。**原文の名詞が「カード」なのに `cardType:'シグニ'` を決め打ち**する**逆向き（過小）の穴**も同居していた。
  - 🔑**「AとB」は `selectionConstraint.groups` で書けた**（engine は honor 済み）＝`cardNames` だけだと**同名2枚**を選べてしまう。逆翻訳にも `groups` 表示を足した。
  - ⚠**golden にテストを1本足すだけで別テストが落ちる**＝`mkState` は POOL カーソルを進める。**`withSavedCursor` で包む**（包み忘れて `続き386 群A WXDi-P08-004-E1` が FAIL した）。
  - ⚠**収穫マージは「リーフが減る変更」を held に落とす**（`cardType` 削除）＝`heldReview.mjs --adopt` で6枚を明示採用した（leaf diff を1件ずつ確認）。

**▶ 次の一手**＝**§5.3 の `O-70`（本セッションで登録＝`O-45` の残り＝「下に置く」の複合対象）か、`O-68`／`O-69`／`O-66`②③**。
- 🔑**「発生源不明のとき発火させるか」は原文で決まる**（`O-44`）＝「〜の効果によって」は原因の特定が意味の一部＝**fail-closed**。残り3語彙（`trashSourceStory`／`banishedSourceStory`／`milledSourceStory`）と規約が揃ったので、**新しい `*SourceStory` を足すときも fail-closed に倒す**。
- 🔑**反転は「直す前に走らせて赤いこと」を確かめてから採用する**（§4.4 の8j と同根）＝緑のまま通る反転は反転ではない。
---
