...
**直近＝2026-09-22＝🏁**§5.7 `S-6` をクローズ（重みの調整では伸びないことを ablation で測り切った）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第467バッチ）

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-35`**（効かない評価項を「判断が実際に行われる場所」へ移す＝`S-6` の後継） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 🏁**機構 worklist 0**／🏁**実機 `V-nn` 0**／実装キュー **0**／🏁**CPU 完成度 0**／**CPU の強さ 1**（🔵`S-35`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い** |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4399**）｜**実機 PASS**（`v286HiddenInfoNotInSharedLog`・反転確認あり） |
| 🆕📏**`S-6` 第3段の結論** | **勝率で調整できるのは「探索が動かせる量」だけ**＝`openLane` -1.44 [-2.11, -0.76]／`fieldPowerScale` -0.73／**`lrigLevel` は48組すべて1手も変えない**。負の対照 `no-board-eval`（全重み0）＝**-3.60 [-4.44, -2.77]**＝**台は大きな差なら検出できる** |
...
| ID | 内容 | 規模 | 何が良くなるか |
|---|---|---|---|
| 🔵`S-35` | **効かない評価項を「判断が実際に行われる場所」へ移す**（`S-6` 第3段の後継）。📏**根拠（実測・96戦ずつ）**＝`lrigLevel=0` は **48組すべて1勝1敗＝1手も変えない**／`life=0` は **6山中5山で同一手順**／`growReady=0` も決着0。🔑**理由は値の大小ではなく「探索が動かせない量」だから**＝グロウは `GROW` フェイズ（探索の外＝貪欲）、ライフが減るのはアタックの解決＝**`MAIN` の探索が比べる盤面では値が変わらず、定数オフセットとして argmax から消える**。🔴**地平線を伸ばすだけでは解けない**（`life=0,searchAttacks=1` vs `searchAttacks=1` ＝ **-0.29 [-0.68, 0.10]＝差なし**）。⇒ **グロウの価値は `cpuGrowReserve`／GROW の候補比較へ、ライフの価値はアタック宣言の判断へ書く**。⚠**着手前に「その判断が候補を比べているか」を確かめる**（比べていないなら重みを置く場所が無い＝`S-26` と同じ形） | 中 | 効いていない重みが実際の判断に効くようになる |

🔵**残1**＝`S-35` だけ（🏁`S-6` は 2026-09-22 にクローズ＝第3段で「重みの調整では伸びない」を測り切った＝BUGFIXES 第467バッチ）。🏁**消化済み（`S-1`〜`S-34`）の行は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「§5.7 取る順の消化済みの行（退避）」へ移した。**
