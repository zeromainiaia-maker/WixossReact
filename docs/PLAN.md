...
- **🆕 セッション（2026-07-29〜30・Opus〔Claude Opus 5〕が指示と検証＋codex-work が実装）＝タスク12(xxix)(1) を6波（第10〜15波）で消化し、残在庫を全件仕分けして実質クローズ**（golden **1063→1075**・census **1400→1394**〔`BASELINE_HIGH=1394`〕・smoke 10726 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held 293→**292枚/107署名**・manual field loss 0。**全6波でゲート全緑／JSON per-effect diff は毎回スコープと完全一致**）
  - **通しの主題＝「`optionalOnPlayCostStub` が写せないコスト語彙」を潰し切る**。写せず**実機で一度も発火しなかった**任意cost【出】は **27→4**（23効果が新たに実働）。**残4件はすべて明示保留（honest defer）＝理由つきで不発を維持**しており、**(xxix)(1) は「実働 or 明示保留」に全件仕分け完了**。
  - **第10波＝ゾーン徴収4語彙12効果**（`trashArtsFromLrigDeck` 6／`removeOppVirus` 3／`deckTrash` 2／`charmTrash` 1）。アーツ候補判定を `battle/artsTrashCost.ts` の純関数へ切り出して UI/engine 共有。🔴同時に `WX25-P1-107-E1` の target filter 脱落（`owner:'any'`＋filter なし＝**相手のシグニでも強化できた**過剰実行）と `WXK06-084-E1` の `discard:1` 脱落を是正。
  - **第11波＝相互制約つきコスト4効果**。既存 `SelectionConstraint` を `EffectCost.energyTrash`／`handToUnderSelf` へ通す。支払い可否を枚数ではなく**制約を満たす部分集合の存在探索**にした点が要。`distinct:'class'` が「共通するクラスを持たない」と一致することは `satisfiesSelectionConstraint` の実装で確認済み（新語彙不要）。
  - **第12波＝「この方法で〜したカード」参照2効果**。`handToEnergy`／`energyTrash` で動かした札のレベル（合計）を既存 `last_*` 記録→`levelEqualsVar`→2解決器で解き、`OPTIONAL_ON_PLAY_COST_REF_DEFERRED` を**0件**に。記録のリセットは Pattern⑤の pay/skip 分岐前。
  - **第13波＝`fieldTrashGroups`／`fieldToLrigTrash` 2効果**。engine 支払可否は UI 純関数 `fieldTrashGroupsAffordable` を直接利用。新 `TRASH.destination`（省略時 `'trash'`）でレゾナ本体だけルリグトラッシュへ。
  - **第14波＝🔴`underSelfTrash` が2つの別物を1キーに潰していた**のを分離。原文「**この**シグニの下」A群13効果（全て【起】）と「**あなたの**シグニの下」B群4効果で候補範囲が違うのに、parser 自身が `// → underSelfTrash (任意シグニ)` と**別物と認めたうえで同キーに入れて**いた（型コメントは「このシグニの下」＝型・parser・原文の三者が不一致）。B群を新 `underAnySigniTrash` へ分離し、複数シグニを横断する共通純関数 `battle/underAnySigniCost.ts` で【出】2効果を実働化。⚠**変更前は17効果すべて支払い実装が無く、実際に支払えたものは0件**だった。
  - **第15波＝可変枚数コスト2効果**（`charmTrashVariable`／新 `lrigDownVariable`）。既存 Pattern⑦（ウィルス除去数の選択）と同型の 0..N `CHOOSE` に載せ、ダウンしたルリグのレベル合計を記録して本体倍率へ渡す。Claude 実測でチャーム0/1/3枚＝0/−7000/−21000、ルリグ Lv1+2+3＝−6000 を確認。
  - **🔴残4件（すべて明示保留・`costUnparsed` 維持）**＝`WXDi-P12-031-E2`（複数ゾーン全捨ての合計枚数≧6 参照が1効果専用機構）／`WXDi-P03-019-E1`（「すべてのシグニ」が自分だけか両者か原文から確定できない＝どちらに倒しても片方が誤り）／`WXDi-CP02-100-E1`（トラッシュ→デッキ下の新コスト経路が engine/UI 双方に必要）／`WXK03-070-E1`（コストは既存 `energyTrashGroups` に載るが、🔴**本体の「対象の対戦相手のシグニ1体をエナゾーンに置き」が丸ごと欠落**しており、コストだけ直すと半実装になる）。
  - **⚠このセッションで3回踏んだ落とし穴（次も必ず疑う）**＝①**codex が新規に書いた日本語だけが CP932 で潰れる**（第10波は live JSON の filter が `"cardType":"???"`、第11波は `BASELINE_HIGH` のコメント冒頭）。`manualEffects.ts` 側は無傷なので**実機は正常・golden も census も緑のまま素通り**する＝**ゲートに検出器が無い**。検査は `git diff --name-only` の**全ファイル**に対し `U+FFFD` と**3文字以上**連続の `?`（`.ts` の `??` は nullish coalescing なので2文字だと誤検出）。②**支払い地点は engine と BattleScreen の2つある**＝第12波は engine 側だけ記録を書いており、**通常召喚では動的 filter が `noMatch` に倒れて効果が丸ごと no-op**だった（シグニなので通常召喚こそ主要経路）。golden は React を叩けないので「参照側と払う側の対応」をデータ不変条件として固定した。③**新規コスト語彙は3関数（`resolveOptionalCostSpec`／`canAffordOptionalCostSpec`／`optionalCostPaySteps`）＋`SUPPORTED` の4箇所**を必ず揃える。
  - **委譲運用の所見**＝第13〜15波は**差し戻し0・Claude 側の是正0が3波連続**。指示書に**per-effect の誤り明細**と**過去波の失敗モード**を書くほど事故率が下がる法則は6波とも再現。codex は defer 判断が正確で、フェイク実装より defer を選ぶ（第15波の4件保留はすべて理由が妥当）。Claude 側の是正は3件（第10波の文字化け、第11波の文字化け、第12波の通常召喚 no-op）で、**いずれも計器に映らない種類**だった。
  - **次の一手**＝**Opus：🆕単独バッチ候補（過剰実行・優先）**〔🔴`WX24-P4-072-E1`＝`PLAY_FREE` から「コストの合計が捨てた枚数以下」の制約が丸ごと落ちて**どんな高コストスペルでも無料で撃てる**（経路が ACTIVATED で別）／エクシードルリグ6件の本体不一致（`WX24-P4-011-E2` の即時UP・`WX24-P4-018-E2` の unless 脱落 等）／`WX20-063-E2`「コスト合計0」・`WXK06-082-E1`「《幻竜 ドラマジ》以外」の filter 脱落／🆕`WXK03-070-E1` の**本体欠落**（上記）〕**／(xxix) 残4件は機構コスト待ちで据え置き**（着手するなら「複数ゾーン全捨ての枚数記録」が2件に効く）**／(lv) 残2経路（CPU シグニ召喚・CPU グロウ＝先に「CPU に任意効果をどう選ばせるか」の方針決め）／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋**(xxix) で新設/改修した支払いUI（エクシード選択・場シグニ選択＋`fieldTrashGroups`・可変枚数捨て〔チャーム/ルリグダウン〕・ライフクラッシュのバースト接続・ルリグデッキのアーツ選択・シグニの下からの横断選択）**＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(xxix) 15波後の最新値（本行を (xxix) 系の正とする）**：**任意cost【出】母集団 981／`optionalOnPlayCostStub` で写せない 4**（＝`costUnparsed` の4件のみ。**すべて明示保留＝理由つきで不発を維持**しており `OPTIONAL_ON_PLAY_COST_REF_DEFERRED` は**0件**。内訳と保留理由は §4 進捗サマリ参照）。**`costUnparsed` 総数 21／AUTO・ON_PLAY・任意 4**。golden **1075**、census **1394**（`BASELINE_HIGH=1394`）、smoke **10726/10726**、fuzz 全0、lint 0 errors/228 warnings、同型★0、held **292枚/107署名**、manual field loss 0。⚠**80/71/59/54/43/35/33/27/20/15/12/10/8/6 等の旧値は母数が違う**（波ごとに新語彙が増えたため）＝**投入前に必ず `npx tsx scripts/archive/xxixResidualCensus.ts` で数え直す**（実関数 `optionalOnPlayCostStub`／`wrapOptionalOnPlay` を import して live JSON を全数走査する計測スクリプト。簿記の数字は信用しない）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1033**・smoke **10726** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1412**（回帰ゲート）・held **251枚/107署名**（2026-07-29 タスク12(lv) 実測）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（(xlvi)／(xlii)／(l)／(lv)。**(xxix) は 2026-07-30 に完全クローズ**＝残4件は明示保留）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
