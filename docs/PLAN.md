...
**直近＝2026-09-17＝§5.7 新設（CPU の強さ）＋`S-1` クローズ＝カードの強さ表**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**ユーザー決定**＝対戦中に LLM は呼ばない（費用・無料枠・小型 LLM の弱さ）＝**探索＋評価のゲーム AI**。順は `S-1` 強さ表 → `S-2` CPU デッキの作戦データ → `S-3` 1ターン分のシミュレータの見積もり（機械学習はその後）。
- `S-1`＝`cpuCardStrength.ts`＝効果 JSON から除去・ドロー・エナ・サーチ・妨害・耐性・キーワード等の特徴量を数えてパワー換算（原文 regex なし）。召喚・対象・サーチ・捨て札・**エナチャージ（旧＝手札の先頭固定）**に使う。
- 🔴**踏んだ罠**＝①初版は `TRASH <DECK_CARD>`（デッキを削る）を除去に数え、削りのシグニが上位を独占 ⇒ 除去は対象が場のシグニのときだけ ②CPU の対話応答の `useEffect` から後ろで定義した `effectsMap` を参照して **React Compiler の lint error** ⇒ effect を定義の後ろへ移設 ③実機で CPU が【ガード】を2枚とも場に出した ⇒ 最後の1枚は手札に残す。
- 実機＝CPU 通し対戦 PASS×2／関連8シナリオ PASS。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7 `S-2`**（CPU デッキの作戦データ＝キーカード・コンボ・優先順位）→ `S-3`（1ターン分のシミュレータの見積もり） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（`S-2`・`S-3`）／**リリース作業 1**（RELEASE.md） |
