...
| `S-37` | 🏁**済（2026-09-26・BUGFIXES 第478バッチ）**＝使いどころは**アタックフェイズに使える札だけ**（守り／攻め／両方／使わない・その札が持つ窓だけ）。スペルは一覧から外した。アシストグロウをアタックフェイズの窓にも配線。🆕同日＝ルール確認（相手のアーツステップでも使える）を受けて全入口を開けた（RULES `R-30`・第479バッチ） | ユーザー（2026-09-25「後で調整する」／2026-09-26 仕様決定） |
