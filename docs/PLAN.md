...
- 🏁**セッション（2026-08-25・続き657・Opus 5）＝§5.3 `O-67` を消化＝コストの「〜をバニッシュする：」に受け皿（`EffectCost.fieldBanish`）を新設し、行き先をエナゾーンへ是正した**。gates 全緑（**golden 2778→2781**／**census 574→573**（ベースラインも更新）／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260／同型★0）。**実機は新規4本＋回帰9本＝計13本が2回連続 ALL PASS＋反転確認4通り。**
  - ✅**母集団は実測2効果**（原文の**コスト句**に「をバニッシュする：」を含む全数）。登録票どおりで隠れた追加なし。
  - 🔴**壊れ方が2通りあった**＝①`WX05-044-E1` は parser 規則が**レベル節を必須**にしていたので「**他の**＜古代兵器＞の…」にマッチせず**コスト句が丸ごと落ちて実質コストなしで撃てた** ②`WX25-P1-022-E1` は `fieldTrash` に化けて**行き先がトラッシュ**（バニッシュならエナが1枚増えるのに資源を失う＝§4.4 罠8f と同型）。
  - 🔴**作業中に3つ目の穴を発見してその場で塞いだ**＝**ルリグ【起】の実行経路（`performLrigActivated`）には場シグニ系コストの支払いが1行も無かった**＝②の `fieldTrash` すら**誰も払っておらず提示だけして踏み倒せた**（可否ゲートにも検算なし）。
  - 🔑**支払いは `payFieldBanishCost` 1本に寄せた**（`removeFromField`→`banishDestination`）＝**シグニ【起】とルリグ【起】が同じ関数を呼ぶ**。`banishDestination` を通すので相手の `banish_redirect`（エナの代わりにトラッシュ）等の置換も効く。
  - ⚠**`fieldTrash` に「行き先フラグ」を足す形にしなかった**＝`fieldTrashCostCards`（`ON_TRASH` の原因弁別 ctx）へ載ると**バニッシュがトラッシュ由来として観測される**。
  - 🔑**対照は「同じカード・同じ盤面・押す【起】だけ違う」で作れた**＝`WX05-044` は E1（バニッシュ）と E2（場からトラッシュ）を両方持つ＝行き先の違いを1枚のカードで撃ち比べられる。
  - ⚠**ゾーンに入る綴りは経路ごとに違う**＝`fieldTrash` は `getCardNum` で接尾辞を落とし（`WX04-097`）、バニッシュは engine 経由なので**インスタンスIDのまま**（`WX04-097#1`）＝実機の照合はカード番号へ正規化する。⚠**`H.queryState()` の `energy`/`trash` は枚数**でカードID配列は `energyCards`/`trashCards`（罠15）。

**▶ 次の一手**＝**§5.2 段2 のバッチを取る**（残 OPEN **678**）。⚠**2026-08-25 続き656 の実測＝第2層で 5件以上の群は 17群120件しかなく、上位3群はいずれも parser の場所が別々だった**＝**軸でも第2層でも束ならない群がある**。⇒ **具体的な語彙・文型に降りて生データ（CSV原文 × live JSON）で母集団を測る**ほうが速い。🔴**`census:wiring` の `cardClass × SIGNI[filter]` は掘り尽くし済み（§5.4 (i) に `[x]`）＝再度掘らないこと。**次の候補は `cardClass × (filter無)` 9（★★・条件節用法が濃いので要仕分け）／`powerRange × GRANT_KEYWORD{SIGNI}` 6（★）。§5.3 側では **`O-66`②③（未着手のまま残置）**が残っている。
- 🔑**「行き先が違う」は型を分けて表す**（`O-67`）＝既存キーに行き先フラグを足すと、そのキーを読む**別の計器・別の ctx**（`fieldTrashCostCards`＝`ON_TRASH` の原因弁別）が誤観測する。
- 🔑**コストの受け皿を足したら「提示」と「支払い」の両方を数える**＝`O-67` では**ルリグ【起】だけ支払いが1行も無かった**（提示は通るので踏み倒せる）。**支払い経路は grep で全部数えてから配線する**（`O-66` の「実行地点を数える」と同じ規律）。
...
- **2026-08-25 続き657 後（本行が直近の正）**：
  ...
  **意味照合 残 OPEN 678（不動＝`O-67` は §5.3 の機構項目で台帳の母集団外）**、
  **実機シナリオ +23**（🆕`O-67`＝`o67BanishCostToEnergy`/`o67TrashCostControl`/`o67LrigBanishCostPaid`/`o67LrigBanishUnpayable` 4本、`O-66`＝`b66LifeCrashCap*` 2本、`O-65`＝`b65KeyTrash*`/`b65Wall*` 4本、`O-64`＝`b64PowerThreshold*` 3本、`O-63`＝`b63TurnOwner*` 2本、第44〜46 の8本）＝**`O-63`〜`O-67` の13本は ALL PASS を2回連続**、
