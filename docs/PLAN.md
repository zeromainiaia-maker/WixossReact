...
**直近＝2026-09-16＝第381バッチ：`O-528` をユーザー判断（読みA）でクローズ＝R6-0 の (消滅) 0**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 【チャーム】が既に付いているシグニは新しい【チャーム】の対象にできない。`execAttachCharm` の通常経路と一斉付与の両方で除外（`ATTACH_CHARM` 44効果に効く）。読み方ルール**規則66**。
- 全カード × 3変種で (消滅) **0件**＝R6-0 の完了条件①を満たした（②再現率は 6/16 で未達のまま＝R6-2 はやらない）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.2 round6 の R6-1**（不変条件センサス・I1 は 0 でゲート化できる）→ その I3 で `O-527` の母集団を測る |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden 4264） |
...
- **2026-09-16 時点**（第381バッチ＝`O-528` 読みA でクローズ）
  - 📊**進捗3計器**＝Sheet1 要対応 **2 / 863**（held 1・mech 1）｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**
  - 📦**在庫**＝機構 worklist **1**（索引G 1＝`O-527`）｜実機 **0**｜実装キュー **0**｜round6 **R6-0 済（①消滅0・②未達）→ R6-1**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden 4264）
