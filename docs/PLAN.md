...
- **続き427（アシストルリグのアタック機構）が持ち込んだ未検証UI 3件**＝**engine 側は golden で固定したが、アタック宣言・ガード応答・CPU は `BattleScreen` にしかなく golden では原理的に踏めない**。前提＝`WX25-P1-048`（ピース）を使う＝**場にルリグ3体で合計3色以上**が要る。**✅2026-08-11（続き435）＝`scripts/verifyBattleDrive.mjs` に4シナリオ新設し実機ヘッドレス検証で以下を確認**（`assistAttackNoFlag`／`assistAttackSkipConfirm`／`assistAttackCpuSequence`／`assistAttackNoEligibleAdvance`、いずれもPASS・既定order登録済み）：
  ...
  - [ ] **使った後にアタックできる**＝CPU側（`assistAttackCpuSequence`）はセンター→左アシスト→右アシストの順で自動アタックし、各ダウン・ライフ3クラッシュまで確認**PASS**。⚠**人間側のクリックで左右連続アタックを踏む `assistAttackBoth` シナリオはドライバのクリック手順バグで未完成のままFAIL**（左アシストのアタック→ダウン→ガード無しクラッシュまでは実測確認できたが、右アシスト側の「アタック」ボタン検出でスクリプトが詰まる＝engine側の疑いは薄い。CPU版で同じ経路がPASSしているため）。人間側の直接確認は次回セッションでドライバを直してから。
...
- **続き426（任意コスト脱落トリップワイヤの残0化）が持ち込んだ未検証UI 3件**＝**pay/skip モーダルが新たに出る**3カード。golden は構造を固定したが提示・徴収は踏めない。**⚠2026-08-11（続き435）＝`verifyBattleDrive.mjs` にシナリオ6件を試作し実機ヘッドレス検証を試みたが、skip側3件中2件のみPASS・pay側は全件ドライバのクリック手順バグで未完成**（`fezoneDoubleCostSkip` PASS＝skipで青1/手札2/対象すべて不変を確認。`connectSpinningChoice4Pay`／`connectSpinningChoice4Insufficient`／`fezoneDoubleCostPay` はFAIL＝いずれもピース/エナ選択UIのクリック対象特定でスクリプトが詰まり本題（pay成立可否）まで到達できていない＝**engine側のバグとは確認できていない**、シナリオ側の作り直しが必要）。
  - [ ] 🔴**踏み倒しが塞がったか**（`WXDi-P14-002` の選択肢④＝「手札を２枚捨ててもよい。そうした場合、対戦相手のライフクロス１枚をクラッシュする」）＝**手札が1枚以下のとき「支払う」が選べない**（`canAfford`）。⚠**手札が足りないのにライフがクラッシュしたら回帰**（旧実装がまさにそれ）。併せて**【使用条件】**（場のルリグ3体で合計3色以上）を満たさないとピース自体が使えないことも見る。**未確認（ドライバ未完成＝`connectSpinningChoice4Pay`/`Insufficient`）**。
  - [ ] 🔴**《青》＋手札2枚の複合コスト**（`WXDi-P14-044`＝アタックフェイズ開始時）＝pay を選ぶと**エナから《青》1つと手札2枚の両方**が減る。⚠**《青》が減らなければ回帰**（旧実装はコスト句ごと脱落していた）。スキップすると相手シグニはデッキの一番下へ**行かない**。**skip側は確認済みPASS（`fezoneDoubleCostSkip`）。pay側は未確認（`fezoneDoubleCostPay`がドライバ未完成でFAIL）**。
