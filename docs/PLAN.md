...
4. **§5c census 条件節の残**（**🆕続き105で確認＝ARTS_USED拡張＝「このターンにあなたが(色)のアーツを使用していた場合」4枚がWX24-D1-11/D2-11/D3-11/D4-11で条件節が丸ごと脱落する真バグ（ON_ATTACK_SIGNI/ON_TURN_ENDが無条件発火）と確認済み。既存`turn_arts_used`state fieldは色を区別しないbooleanのみ＝色別トラッキング（`turn_arts_used_colors`等）の新設とCondition型拡張が要る新規機構＝Sonnet単点修正の対象外**）・「代わりに」WX25-P2-068/070・「あり」複合条件WXDi-P11-048。
...
- **🆕 セッション（2026-07-13・続き106・Opus 4.8・PLAN §3 Opusタスク12(vi-2)/(v)/(vi)＝`applyDirectAction` の型対応漏れ6型を修正・smoke SKIP 258→1）**
  ...
  - **次の一手（Opus）＝** **(vi-5)/(x) 等のトリガー usageLimit 書き戻しは実機ドライバ付きの専用セッション推奨**（clean な2サイトと戦闘解決の risky な3サイトが混在）。当面は §6.2 系統②残（GRANT_PROTECTION 広域24件・SEQUENCE内 WX08-017・LAYER付与 WX15-031）・§6.1 未実装action型・(vii)(viii) 構造的バグ・§5c census文型バッチ・タスク6(「代わりに」置換機構＝WX25-P3-116/WX06-006/WX16-021 等が待つ) から選ぶ。
