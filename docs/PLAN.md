...
| 🆕(xlix) | **【常】出撃制限が `ADD_TO_FIELD` へ mis-parse（続き228・Opus観測）**＝WX19-030-E1「このシグニは対戦相手の場に【ウィルス】が３つ以上ある場合にしか新たに場に出すことができない」が出撃条件（SELF_PLAY_CONDITION 相当）ではなく「直前に選んだカードを場に出す」アクションへ化けている。engine が実アクションとして誤実行しうる。「〜場合にしか（新たに）場に出すことができない」idiom の同型枚数を要棚卸し（小口の可能性）。 |
...
| 🆕(li) | **`getKeyPieceActions`（BattleScreen.tsx:10765-10772）がキーの ACTIVATED 能力を timing↔phase 照合せず surface（続き237・Opus観測）**＝phase∈{MAIN,ATTACK_ARTS,…} かつ ACTIVATED なら timing を無視して全部出す。キー ACTIVATED の timing 分布は ATTACK_ARTS 60/MAIN 47/両方3/SPELL_CUTIN 2/全3-1＝**MAIN 専用がアタックフェイズにも／ATTACK_ARTS 専用（《アタックフェイズアイコン》）がメインにも出る 107+ 能力の広域な既存緩さ**。シグニ【起】（10443-10445：MAIN は timing 未指定 or MAIN／アタックは ATTACK_ARTS 含む）と同型の timing↔phase 照合を入れれば直る。**要専用 golden（64 キー全体の挙動変化＝退化ゼロ確認）**。WXK08-005 のレベルゲート（続き237）は効くが《アタックフェイズアイコン》のフェイズ限定はこの緩さのまま。 |
...
- **🆕 セッション（2026-07-21・続き236・Sonnet（Opusタスク16を試行）・**WX17-032「あなたのシグニがバトルによって正面以外のシグニをバニッシュしたとき、そのアタックしているシグニをアップする」を消化＝timing regex に「正面以外の」追加＋新設 `triggerCondition.banishedNotFront`（engine `battleBanishEntries` でアタッカー正面ゾーンと被バニッシュゾーンの不一致を判定）。UP先「そのアタックしているシグニ」は any_ally scope で能力ホスト≠実アタッカーになりうるため `thisCardOnly` は誤り＝新設 `targetsBattleAttacker`（`StackEntry.battleAttackerCardNum`／`ExecCtx.battleAttackerCardNum` 経由）で解決＝タスク16[B]が実際には新規 engine 配線を要した1例。golden+2（構造固定＋実行検証）。census:timing 41→**40**効果・golden 540→**542**・census 1839→**1838**）**（続き235 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  - **共通の型**＝timing 判定 regex が**能動形しか見ておらず受身形を取りこぼし ON_PLAY（召喚だけで発火）へ誤フォールバック**していた [A]（engine 収集関数は既存）を2件。
  - **(1) ON_ACCE 受身形（WX15-059-E1）**＝`シグニに【アクセ】が付いたとき` のみ見て `シグニがアクセされたとき` を落としていた。timing 判定＋scope 抽出の regex に受身形を追加＝fresh が ON_ACCE + any_ally を生成（curated は MANUAL で元々正しく fresh だけ化けていた・該当1枚）。
  - **(2) ON_HAND_DISCARDED watcher 受身形＋正面ダウン（WDA-F02-17-E2）**＝場の watcher が手札捨てに反応する効果が (a) timing ON_PLAY 化け (b) 対象「このシグニの正面のシグニ」owner:self 潰れ、で二重に壊れていた（E3「**この**カードが手札から…」＝捨てられたカード自身 ON_TRASH self とは別物）。修正3点＝①parser timing に `手札からカードN枚がトラッシュに置かれたとき`→ON_HAND_DISCARDED（collectHandDiscardTriggers 既存）②parser DOWN 対象「正面のシグニ」→owner:opponent+`frontOfSelf`③engine `execDown` に frontOfSelf 解決追加（execBanish と同型・相手 2-zi 絞り）。E3 が MANUAL でカード全体が build:effects 保護されるため curated E2 を fresh 一致に手術パッチ（held ドリフト無し）。
  - **ゲート**＝全ゲート緑（golden **540**〔parser2+engine1〕・smoke 10722全OK・fuzz 全0・census **1839** 維持〔timing 是正は高signal欠落計器の対象外〕・同型★0・lint 0 errors・typecheck）。decompile 再生成済。詳細 BUGFIXES 続き235。
  - **次の一手（Opus）**＝タスク16 の残る [A] はほぼ枯渇（残41効果/38クラスタは大半 [C]§6.3 機構待ち＝「シグニの下からトラッシュ」3・「アタックを効果によって無効に」2・ソウル付与・OR複合トリガー・累積カウント／[B] 軽量拡張＝正面以外 filter・milled level/story filter）。次は**タスク12 の生き残り在庫**（(xliv)残テール・(xxxix)残・(xxii)残50件・(xlii)残）か**§6.3 tractable**（凍結leftStateFilter のバトル離脱対応＝続き233 の残〔`banishDestination` に除去前 state 配線・(xliv)(a) BANISH_REDIRECT 属性フィルタと共通〕）。**Sonnet はタスク1（§7 実機検証）＋タスク6（新語彙着地分の再収穫）**。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 542・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1838。
>
...
- **🆕 GRANT_PROTECTION 効果耐性の広域テール残（2026-07-21・続き239・§6.2 系統②タスク9 の非tractable分）**＝subjectFilter/条件/from で表現できない限定を持つ「…効果を受けない」系。いずれも近似すると偽陰性/過剰保護になるため据置：**(a) 発生源プロパティ限定**＝WXEX2-36（源が《ライズアイコン》を持たない）・WXK11-021（源が《ライフバースト》ではない）・WX11-027（源が対戦相手のライフバースト効果）＝解決中効果の発生源カードの属性を collectEffectImmuneSigni へ渡す機構が要る（sourceCostMin と同型で拡張可能）。**(b) self-except**＝WX17-001「《カーニバル ―Ｑ―》は自身以外の効果を受けない」＝「自分の効果を除く全効果耐性」の except-self 軸。**(c) 相手エナゾーンのカード免疫**＝WXK11-020（場のシグニではなくエナゾーンのカードが対象）。**(d) トリガー元付与**＝WX14-049/WXEX1-58（「そのレゾナ」＝出現条件で消費したレゾナへ次ターン付与）・WXK10-080/WD18-008（対象選択したシグニへ付与だが parser が count:'ALL' を吐く＝targetsTriggerSource/選択結果保持が要る）。**(e) 動的盤面条件**＝WX12-Re09（「場のシグニ3体が共通色を持たない場合」このシグニ免疫）。**(f) POWER_MODIFY 免疫5件**（WX05-024-E1/WX12-033-E3/WX20-023-LAYER/WX22-013-E1/WXK03-018-E3）＝「対戦相手の効果によってパワーは増減/＋されない」が `GRANT_PROTECTION from:['POWER_MODIFY']` に mis-parse され**どのコレクタにも配線されず no-op**（正しくは STUB `PREVENT_POWER_MINUS_BY_OPP` 系＝別parser修正）。
