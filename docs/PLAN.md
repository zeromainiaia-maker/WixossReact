...
- **🆕 セッション（2026-07-12・続き74・Sonnet 5・§3 Sonnetタスク1 §7実機検証の継続＝R42②で実バグを発見）**＝PLAN §7「ON_CHARM_TO_TRASH」残②「バトルバニッシュでhostが離脱したとき」を`charmToTrashBattle`シナリオで検証。**❌FAIL（2回連続再現）＝実バグを確認（未修正・Opus引き継ぎ）**＝戦闘（アタックの力比べ）でチャーム付きシグニがバニッシュされると、ground truth（チャームが正しくguest.trashへ移動）は正常なのに、watcher（WX16-Re05のON_CHARM_TO_TRASH）が一度も発火しない。**確定した原因**＝バトル解決を担当する`resolvePendingSigniBattleFor`（`BattleScreen.tsx:6344`）が独自のトリガーリストを構築し、`collectCharmToTrashTriggers`の呼び出し元である`collectBoardDiffTriggers`（`resolveStackNext`/`handleEffectInteraction`のみで使用）を一切呼ばない＝**効果によるバニッシュでは発火するが、実戦で最も頻繁な通常戦闘でのバニッシュでは機能しない**という実害の大きいバグ。既存の`charmToTrash`（効果banish経路）はPASS維持。**修正はせずOpusタスク12へ登録**（PLAN §3・§7・詳細 VERIFY_BROWSER.md）。`charmToTrashBattle`は`order`配列には追加せず（FAIL）。**変更範囲＝`scripts/verifyBattleDrive.mjs`のみ（シナリオ追加）＝engine/parser不変のため typecheck のみ確認・gates再実行不要**。
  ...
  - **次の一手＝(A)Opus＝§3 Opusタスク1＝GRANT_QUOTED_AUTO_ABILITY内側parse**（WX24-P1-017／WX25-P3-038）。**(B)Opus＝Opusタスク12（最優先候補・観測結果が4件に増加）＝続き70の「多段インタラクションSEQUENCE途中ラウンドの盤面差分トリガー見逃し」バグ**（BUGFIXES/PLAN§7 R30参照）**に加え、続き72の2件（WXDi-P11-040 excludeSelf未実装・WX25-P2-055 owner誤り）・続き73の1件（WXDi-P13-047 turnOwnerゲート未実装）・続き74の1件（ON_CHARM_TO_TRASHのバトルbanish経路未配線＝`collectCharmToTrashTriggers`を`resolvePendingSigniBattleFor`へ追加する横展開で直る見込み）も同タスクへ合流**。**(C)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め＋採用**（続き71で整理済みのパターンA〜Fから着手）。**(D)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②③・他多数＝ON_TARGETED①系列・ON_LRIG_GROW②・R42②は完了）。**(E)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（verifyBattleDriveバッチ状態汚染）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
