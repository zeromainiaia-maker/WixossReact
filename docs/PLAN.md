...
| 🆕(xxii) | **✅続き143（Opus）で20件消化＝第1バッチ12件〔「そのカードが…の場合」→LAST_PROCESSED_MATCHES 拡張＋盤面状態条件持ち上げ `parseHoistStateCondition`〕＋第2バッチ8件〔結果カウント閾値 Cluster B＝汎用カウント条件 `parseThisWayGenericCount`・記録動詞限定＋捕捉不能形は据置〕。census 2213→2206・golden 337・同型★0維持。残＝多分岐（第2枝以降が bare step）・Cluster B の合計/すべて/種類/枚数系・否定3・手札加え/捨てる動詞（記録経路要確認）。詳細 BUGFIXES 続き143。** **PARTIAL刻印 IS_MY_TURN化 127件＝過剰実行バグ確定（続き138・Sonnet・タスク9トリアージ）**。「その後/この方法で、[条件]の場合」という後置条件節を parser が抽出できず無言で常時true化（`effectParser.ts:2488`）。**152件全件を原文照合＝127件全件が真に偽になり得る条件で§9-9のLIFE_BURST慣例には非該当＝全件(a)実害あり**。文型3クラスタ：属性判定65（レベル/色/センタールリグ等）・結果カウント閾値59（「N枚以上捨てた場合」等）・否定条件3（「〜しなかった場合」＝WD14-012-E2 等は捨てても捨てなくても自壊が発火し最も実害大）。**IDリスト全件は `docs/_partial_triage.txt`**。修正はparser規則の追加（条件節抽出）＝新規機構は不要、定型パターンなので一括処理向き。 |
...
- **🆕 セッション（2026-07-15・続き143・Opus 4.8・PLAN §3 Opusタスク12(xxii)＝IS_MY_TURN化127件バグの第1バッチ消化）**
  - **✅ タスク12(xxii) 第1バッチ12件消化**＝後置条件節が抽出できず無言で常時true化（IS_MY_TURN化＝過剰実行）していた127件のうち、engine 対応済み条件型で直せる均一形を parser 規則追加のみで是正。①`parseLastProcessedMatchesCondition` を「それが」に加え**「そのカードが…の場合」**へ拡張（デッキトップ公開→レベル/クラス参照の典型系統）。②前段の記録に依存しない**盤面状態条件の持ち上げ fallback**（`parseHoistStateCondition`）を新設し `STATE_CONDITION_CLAUSES` で照合。是正内訳＝LAST_PROCESSED_MATCHES 8（WX19-024/WX24-P3-047/059/062/WXDi-P01-059/074/082/WX10-031）／LRIG_STORY 2（WX12-014/WX13-025）／SELF_POWER_GTE 2（WX26-CP1-066/WXDi-P07-065）。`heldReview --adopt` で採用・機械diffで「意図した12枚のみ変更」を確認・逆翻訳を原文照合。
  - **検証**：`npm run gates` 全緑（typecheck／**golden 335〔+1〕**／smoke 全0／fuzz 全0／lint 0 error）。**census 2213 → 2210**（`BASELINE_HIGH` 更新済み）。`npm run regen` で同型★0維持。詳細 BUGFIXES 続き143。
  - **次の一手**：**Opus＝タスク12(xxii) の残消化を継続**（残115件・`docs/_partial_triage.txt`）＝(a) 多分岐「レベル1の場合…レベル2の場合…」（PR-459A/WX06-018/WXK09-003 等・後続枝が「その後」なしで持ち上げ対象外＝**parser の文分割ロジックに手を入れる必要あり**）／(b) 結果カウント閾値 Cluster B 59件（「この方法でN枚以上〜した場合」＝直前の可変アクション結果依存・`TRASHED_*_COUNT_GTE` 系の抽出拡張で多くが engine 対応済み）／(c) 否定条件 Cluster C 3件（WD14-012 等・**engine に「前アクションが起きなかった」条件型が要る**＝機構寄り）。ほかタスク12在庫＝(xxiii)リコレクト分割8件〔WX25-P1-00X系列は§6.3級 GRANT機構〕／(xxiv)発生源フィルタ脱落8件〔コスト元ホスト能力クラスを engine に通す配線が要る〕／(vii)(viii)per-card構造修正／(xii)WXEX1-19無限ループ／(xxi)collectOppDrawTriggers→本丸タスク1（引用付与の内側parse）。Sonnet＝タスク8（semantic audit）が在庫、及び続き142のフルバッチ完走確認。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 335・smoke/fuzz 全0・同型★0・census 2210。
>
