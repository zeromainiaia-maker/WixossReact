...
- **🆕 セッション（2026-08-14・続き475／475b／475c・Opus 5）＝**§7 実機検証の続き＋そこで見つけた engine バグの修正**＝**V-01 と V-10 の両ブロックが決着（11シナリオすべて緑）**。**§3 在庫3件のうち (cxxv) は「シナリオ偽陽性」として取り下げ、(cxxvi)(cxxx)(cxxix) は engine 修正で残0クローズ**。**Opusタスク12 の在庫は 8件→5件**。ゲート全緑（**golden 1964→1968**＝トリップワイヤ4本追加・census 831 据置・smoke 10688／SKIP 0・fuzz 全0・lint 0 errors/259 warnings・**live JSON changed 0**）。**
  ...
  - **🏁③ 実バグ3件を真因まで確定→全件クローズ**＝**(cxxx)【新規】離場置換の「決定」が消費されず残留**（`applyEffectLeaveSubstitutes` は消費済み ctx を返すのに、**置換不成立側で呼び出し側が捨てて**いた＝**11経路すべて同じ形**）／**(cxxvi)** 相互身代わりの instance 複製＝**身代わりで先に場を離れた instance を離場ループが再処理**し、`removeFromField` が空振りしたまま移動先へ2枚目を push（＝カードが増える）／**(cxxix)** `lifeCrash` の身代わりが **apply 側に分岐が無く末尾の `trashStackSpell` へフォールスルー**して「0枚トラッシュ」で成立＝**ライフを払わずバニッシュ回避**。⇒ **`sub.ctx` の無条件採用／`isOnFieldTop` ガード／`lifeCrash` の実装＋`isImplementedSubstituteCost` の安全弁**で修正。
  ...
  - **▶ 次の一手**＝**Opus 側**：**(cxxvii)(cxxviii)**（**同型の「対象種別の取り違え」**＝`NEGATE_ATTACK{owner}` と `WXDi-P04-051-E1` の timing。**どちらも再現シナリオが赤で待機**しているので調査ゼロで着手できる）。次点＝§6.4 O-20 の残21件・O-19。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き475／475b／475c（§7 V-01・V-10 決着＋(cxxvi)(cxxx)(cxxix) の engine 修正）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、**golden 1968**（**1964→+4**＝(cxxvi)(cxxx)(cxxix)×2 のトリップワイヤ）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。**live JSON の変更は 0**（変更は `src/engine/effectExecutor.ts`・`scripts/goldenTest.ts`・`scripts/verifyBattleDrive.mjs`・docs）。🆕**実機シナリオ（`verifyBattleDrive.mjs`）＝既定 order の総数は据置**（新規追加0・**判定を直したのが6本**・**id を1本 rename**＝`effectBanishLifeCrashSubstituteNotOnEffect` → `effectBanishLifeCrashSubstitutePaysLife`）。**緑/赤の内訳が 45緑12赤 → 52緑5赤**（V-01 の6本＋V-10 の5本がすべて緑）。🆕**Opusタスク12 の在庫＝5件**（**(cxxv) 取り下げ・(cxxvi)(cxxx)(cxxix) 残0クローズ**）。
