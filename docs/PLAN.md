...
| (lxix) | **🆕 「あなたのパワーN以上のシグニ1体がアタックしたとき」が `scope:'self'`＋パワー条件なしで両方向に誤り（2026-07-31・タスク16 第4波の実測で発見）**＝`WXDi-P02-079-E2`（パワー15000以上→【エナチャージ１】）と `WXK07-030-E2`（同→1枚引き＋【エナチャージ１】）。live はどちらも `timing:['ON_ATTACK_SIGNI']`／**`triggerScope:'self'`／`triggerFilter:null`** ⇒ ①**能力保持シグニ自身がアタックすればパワー不問で発火**（過剰実行）②**他の適格シグニがアタックしても発火しない**（過小実行）の**両方向に誤り**。⚠**timing は付いているので `census:timing` には出ない**＝計器の外にある。**🟢 直し方は第4波の成果がそのまま流用できる**＝主語を `any_ally` にし `triggerFilter.powerRange.min` を出す parser 規則（第4波で `ON_SIGNI_BANISH_OPPONENT` 用に書いたものの `ON_ATTACK_SIGNI` 版）＋**収集側が `matchesFilter` に実効パワーを渡す**こと。⚠**`effectivePowers` のキーは raw（`getCardNum()` で丸めない）**＝第4波で踏んだ罠。⚠投入前に **`ON_ATTACK_SIGNI` 側の収集経路が `triggerFilter` を主体に適用しているか**を実コードで確認すること（`ON_SIGNI_BANISH_OPPONENT` と同じとは限らない）。**次の一手の最有力候補。** |
...
- **🆕 セッション（2026-07-31・続き301・Opus 5〔Claude 実測＋検証〕＋codex-work〔実装〕）＝タスク16 第4波＝ユーザー指示で「1件ずつ投げる」方針へ切替。`WXDi-P06-072-E1` 1効果のみ消化・Claude が是正1件**（golden **1170→1174**・census **1363 据置**（`BASELINE_HIGH=1363`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**234 warnings 据置**・manual field loss 0・同型★0・**held 251枚/98署名 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects`・`regen` 冪等／`census:timing` **22→21**）
  - **① 台帳（`_timing_census_triage.txt`）の停止理由が2点とも誤りだった**＝(a) しきい値は **15000 ではなく 8000**（原文実測。台帳内で値が揺れていた）(b)「`triggerFilter` は被バニッシュ側に適用され主体 power を読まない」は **`ON_BANISH` の話**であって、この効果が使う **`ON_SIGNI_BANISH_OPPONENT` では `triggerFilter` は「バニッシュした側」に適用される**（`BattleScreen.tsx:8117`／仕様は `types/effects.ts:2197` に明記）。⇒**主体を絞る受け皿は既に在り、新軸は不要**だった。
  - **② 足りなかったのは2点だけ**＝(A) parser の主語 regex（`effectParser.ts:6578`）が `(?:＜[^＞]+＞の)?` しか許さず「パワー8000以上の」で非マッチ (B) **`matchesFilter` の第3引数 `effectivePower` を呼び出し側が省略していた**（`powerRange` 対応も実効パワー引数も既存で、`effectivePowers` も同スコープに在った）。⇒**新語彙ゼロ**で着地。
  - **③ 波及ゼロを投入前に証明**＝`effectivePower` は `matchesFilter` の `powerRange` 分岐でしか使われず（`execUtils.ts:616`）、`ON_SIGNI_BANISH_OPPONENT/_BATTLE` の全55効果中 `triggerFilter` 持ちは4件・**全部 `story` のみで `powerRange` は0件**。regex 側も該当文型は全数走査で**1件のみ**。
  - **④ 🔴 Claude 検証で是正＝`effectivePowers` のキー形**。codex は `effectivePowers.get(getCardNum(myTopNum))` と**素の cardNum へ丸めて**いたが、`calcFieldPowers` は**場のスタック頂点をそのままキーにする**（`effectEngine.ts:1345`）ため **`#N` 付きインスタンスID（トークン/複製）で lookup が外れ、黙って表記パワーへフォールバック＝このバッチの目的が無効化**される。同ファイルの他の `effectivePowers.get(...)` は全て raw。**`cardMap` は `getCardNum()` 必須／`effectivePowers` は raw** という2つの Map のキー形の違いをコメントで固定した。⚠**golden では検出不能**（helper に実効パワーを直接渡すため実機のキー引きを通らない）。
  - **⑤ 新ガードレール2本が1回で効いた**＝前波で作った「golden は集計行より前に置く」（codex は正しく前へ配置）と「しきい値・列挙値は台帳でなく原文/実データで数える」（codex が台帳の 15000 を自力で訂正）。**失敗モードを具体化して指示書に載せると次の回で定着する**という §8 の傾向がまた再現。
  - **次の一手**＝**Opus：🆕 タスク16 残＝`[B維持]` 7件**。🆕**最優先の獲物＝`WXDi-P02-079-E2`／`WXK07-030-E2`**（本波で発見・**タスク16 の枠外**）＝原文「あなたのパワー15000以上のシグニ1体が**アタックしたとき**」なのに live は `ON_ATTACK_SIGNI`／**`scope:'self'`／パワー条件なし**＝**両方向に誤り**（能力保持シグニ自身がアタックすればパワー不問で発火し、他の適格シグニのアタックでは発火しない）。**本波で作った `triggerFilter.powerRange`＋実効パワーの受け渡しがそのまま流用できる**ので次に投げやすい（→ タスク12(lxix)）。それ以外の `[B維持]` 7件＝`ON_HAND_DISCARDED` 系3（機構3種混在＋呼び出し元7箇所＝**1件ずつ**）／`WXDi-P06-038-E1`／`WX05-020-E1`／`WXDi-P11-063-E2`／`WXDi-CP02-068-E1`。⚠**7件とも live は `timing:[]` の安全停止＝過剰実行ではない**ので退化させないことが最優先。**その他**＝タスク12(lxviii)／§6.3 J 残11／(lxvii) CPU `ON_TURN_END` 未収集／§6.4 配置数制限すり抜け7効果／(lxi) 残5／(xxix) 残4／(lv) 残2経路。**Sonnet：§7 実機検証＝在庫はここだけ**。⚠**今回の1効果も実機UI未検証**（続き298〜301 の分が積み上がっている）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 続き301（タスク16 第4波＝バニッシュ主体の実効パワー条件・1効果）後 最新値（本行を直近の正とする）**：census **1363 据置**（`BASELINE_HIGH=1363`）、golden **1170→1174**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**234 warnings 据置**、manual field loss 0、同型★0（5986枚・265群）、**held 251枚／署名グループ 98 据置**。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WXDi-P06-072-E1`）、`build:effects`・`regen` 冪等。**`npm run census:timing` 22→21 効果/21クラスタ**（⚠**実質19**＝`WXDi-P09-079-E1`／`WXK10-052-E1` は既知の偽陽性）。**タスク16 残＝`[B維持]` 8→7**。**新語彙0本**＝`triggerFilter.powerRange` も `matchesFilter` の `effectivePower` 引数も既存で、parser の主語 regex 拡張＋呼び出し側が第3引数を渡すようにしただけ。engine 新設は pure helper 1本（`battleBanisherMatchesTrigger`＝BattleScreen インラインを golden から検査可能にするための切り出し）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1174**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1363**（回帰ゲート）・held **251枚/98署名**（2026-07-31 タスク16 第4波後に実測）。
>
