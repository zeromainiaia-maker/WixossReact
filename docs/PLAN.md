...
**直近＝2026-09-22＝🔴**非公開情報が共有ログに出ていた（`V-285` の横展開・8系統 約180効果）**（ユーザー指示・全文は [BUGFIXES.md](./BUGFIXES.md) 第465バッチ）

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔵**§5.7 `S-6`**（機械学習＝CPU の強さの残り1件）／§5.3 `O-537`（`revealPicked` を parser で埋める） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**） |
| 📦**在庫** | **機構 worklist 1**（`O-537`）／🏁**実機 `V-nn` 0**／実装キュー **0**／🏁**CPU 完成度 0**／**CPU の強さ 1**（🔵`S-6`）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | **`O-537` だけ**（deck→hand サーチ 7効果の札名＝伏せると 272効果の正当な公開が消えるので parser 先行） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4395**）｜**実機 PASS**（`v286HiddenInfoNotInSharedLog`・反転確認あり） |
| 🔑**この回の道具** | `src/engine/hiddenInfo.ts`（`logCardLabel`＝公開領域の札だけ名前を出す／`privateLine`＝自分だけに見える行）・`addPrivateLog`・`scripts/archive/censusHiddenInfoLeak.mjs`（トレースで漏れを全数で数える） |
...
| **②** | **§5.3 機構 worklist `O-nn`** | **1** | 新しい型・評価器・engine が要るもの | §5.3 の索引 |
...
**取る順**＝①実機（寝かせるほど切り分けが高くつく）→ ②機構（索引の並び順）→ ③実装キュー（機構不要の候補だけ）→ ④CPU 完成度 → 🔥**④b CPU の強さ**（`S-6`）→ ⑤意味照合（①〜④b が空のとき）。
🔑**2026-09-22 現在は ①②③④ が全部 0 なので ④b（§5.7 `S-6`）が本線**。作業中に見つけた engine/parser のバグは**その場で直す**（既定）か、新機構が要るなら §5.3 へ `O-nn` で登録する（§2 の1巡と同じ）。
...
**残0**。直近の返済＝`V-286`（非公開の札の名前が共有ログに出ていた＝`V-285` の横展開8系統・2026-09-22）／`V-285`（【トラップ】の中身が相手のログ・画面に見えていた）／`V-284`（じゃんけんの「あいこ」で止まらない）。
...
| ID | 一行 | 規模 |
|---|---|---|
| `O-537` | **デッキサーチの「公開し」を `revealPicked` に落とす**（§5.1 `V-286` の残り）＝live の deck→hand サーチ **279効果のうち 272 が原文に「公開」なのに `revealPicked` が付いているのは2効果だけ**。付ければ engine 側で「公開されていない7効果だけ札名を伏せる」判定ができる（いま zone だけで伏せると 272効果の正当な公開が消えるので手を付けていない）。⚠**逆翻訳と `を公開` ログが 272効果で増える**＝`npm run regen` と golden の差分を必ず見る | M |
...
| ID | 内容 | 規模 | 何が良くなるか |
|---|---|---|---|
| 🔵`S-6` | **機械学習①＝自己対戦で重みを調整**。🏁**第1段（分岐スクリーニング＝`npm run selfplay:scan`）**・🏁**第2段（調整できる49個を全数走査＋多次元 A/B 4束）は済み**＝🔑**微調整に差が無いことを測り切った**（192組の75%が1勝1敗＝勝敗の二値では大きな差しか検出できない）。🔵**残り＝第3段を取るなら ①大きく振る（桁で変える）②効く場所が無い数値を先に直す ③測れる山を用意する（`plan.*` は `keyCards` 1枚・`combos` 1つしか無い）**のどれか。🔴**同じ幅で振り直しても結論は変わらない**（実測済み・登録票＝[PLAN_DETAIL.md](./PLAN_DETAIL.md)） | 大 | 手で決めた重みを勝率で置き換える |

🔵**残1**＝`S-6` だけ。🏁**消化済み（`S-1`〜`S-5`・`S-7`・`S-9`〜`S-34`）の行は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「§5.7 取る順の消化済みの行（退避）」へ移した。**
