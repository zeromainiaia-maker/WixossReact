...
16. **🆕 timing 語彙センサスの消化（`npm run census:timing`・続き75新設）＝いま最も費用対効果が高い新worklist（実質タスク1と2の間に置く優先度）**。**「engine に収集関数があるのに parser がその timing を一度も生成していない」穴**を機械検出する計器。**静的ギャップ＝29種**（ON_MAIN_PHASE_START／ON_SPELL_USE／ON_EXCEED_COST／ON_RISE／ON_SIGNI_BECOMES_DRIVE／ON_HAND_DISCARDED／ON_ARTS_USE／ON_BECOME_BEAT 等が MANUAL でしか使われていない）。**動的計測＝239効果 / 157クラスタ**が `ON_PLAY`（＝「場に出たとき」）へ誤フォールバック中（明細 `docs/_timing_census.txt`・**履歴 376→353→323→294→281→270→239**）＝**召喚しただけで発火する幻覚**。**✅消化済み（続き75・計132枚・census 1557→1542）**＝`ON_SIGNI_BANISH_OPPONENT`（50枚）・`ON_MAIN_PHASE_START`（30枚）・`ON_SPELL_USE`（18枚・**engine の相手側 watcher 未配線もセットで是正**）・`ON_EXCEED_COST`（11枚）・`ON_RISE`（6枚）・`ON_SIGNI_BECOMES_DRIVE`/`ON_BECOME_BEAT`/`ON_ARTS_USE`（10枚）。
...
- **🆕 セッション（2026-07-12・続き75・Opus 4.8・**Opusタスク12を全6件消化して在庫ゼロ化→続けてタスク1の第1弾**・実機11シナリオ全PASS・golden 192・census 1557維持）**
  ...
  - **次の一手＝(A)Opus＝§3 Opusタスク16＝timing 語彙センサスの消化を続ける（最優先・`npm run census:timing`）**＝**残 270効果が「場に出たとき」に誤発火している**（履歴 376→353→323→294→281→270・**続き75で計122枚是正**）。**次の上位クラスタ**＝`ドライブ状態になったとき`11（ON_SIGNI_BECOMES_DRIVE）／`手札からトラッシュに置かれたとき`10（ON_TRASH）／`ガードステップ以外で手札を捨てたとき`8（ON_HAND_DISCARDED）／`アーツを使用したとき`7（ON_ARTS_USE）／`【ビート】になったとき`7（ON_BECOME_BEAT）…**いずれも engine 側に収集関数が既にある**＝同じ型で消化できる。**1クラスタ≒15〜30分・+6〜30枚**が続き75での実績（MANUAL カードは curated が既に正しい timing を持つことが多く、機械diffで弾けば安全）。**engine 側に収集関数が既にあるものは parser に regex 1本＋scope 抽出で直る**（続き75で計50枚を実証）。手順・ガードレール（トリガー句を除去しない／MANUAL温存・EXILE→TRASH 据置系を機械diffで除外）は §3 タスク16 に明記。**(A')Opus＝タスク1の第2弾**＝WX25-P3-038 の内側「代わりに」置換（タスク6の置換機構と合流）・`GRANT_LRIG_ABILITY` の ON_PLAY 誤デフォルト（タスク5）。**(B)Opus＝Opusタスク10＝CHOOSE held残53枚の最終見極め**（続き71のパターンA〜F）。**(C)Sonnet＝§3 Sonnetタスク1＝§7実機検証の継続**（**R30の発火経路が開通したので R30②③（turnOwner:self ゲート・《ターン1回》）が新たに検証可能**。他に ON_COIN_PAID③④・ON_LRIG_GROW③④・ON_TARGETED②・R43②・R44②③ 等。**ON_TARGETED①③系列・ON_LRIG_GROW②・R42②・R30①は完了**）。**(D)持ち越しの小口は §3 Opusタスク5・Sonnetタスク3（driverバッチ状態汚染＝本セッションでも `deployRestrict` がバッチ実行時のみ FAIL＝再現性を確認）・Sonnetタスク10（WXK04-003 ボタンラベル）へ整理済み。**
