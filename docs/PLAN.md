...
- **🆕 セッション（2026-07-07・続き39後半・Sonnet 5・§7 実機検証シナリオ横展開＝R37新規シナリオ追加）**＝`scripts/verifyBattleDrive.mjs` に新シナリオ **`powerzero`**（WD11-013→WX21-067）を追加し、**R37「対戦相手のシグニのパワーが0以下になったとき」の①発火自体を実UIで確認**（PASS＝盤面ログ「1枚ドロー／[自分] アイン＝テトロドの【自】効果（パワー0以下時）」）。カード選定で2度の試行錯誤＝(1)WD22-037-UG（裏面UG型＝「シグニの効果によって場に出た場合」限定カードは通常召喚ボタン自体がUIに出ない）(2)WD11-013（ミュウ限定＝**Team制限が実際に summon UI をゲートすると判明**＝センタールリグをミュウのWX08-004に変更して解決）。**⚠13シナリオ一括実行では`lrigundermoved`/`keywordgained`/`powerzero`の3件がFAILしたが、個別再実行では全てPASS**＝カード/engineのバグではなく既存コードに注釈済みの「バッチ実行時のみの状態汚染」がこの3件の並びに連鎖したもの（driver側の技術的負債・別途follow-up）。詳細 BUGFIXES 最上部・VERIFY_BROWSER.md。engine/JSON無変更（gates再実行不要）。**次の一手＝§7残る実機検証項目（R30-R46のうち未着手＝R30/R31/R36/R38-R46・ON_LRIG_ATTACK_STEP_START全体未検証等）から1件ずつ`verifyBattleDrive.mjs`にシナリオ追加→観測記録（バグ発見時の修正はOpus）。または verifyBattleDrive のバッチ実行時状態汚染の根本修正（driver側テスト分離強化）。あるいはOpus側で(1)`GRANT_TO_PLACED_SIGNI`実装(2)census「動的比較35枚」(3)引用内CHOOSE（WXDi-D09-P20）が続き36から未着手のまま残る。**
