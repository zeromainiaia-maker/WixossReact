...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔴**ユーザー判断待ち5件**（[RULES.md](./RULES.md) の `R-27`／`R-45b`／`R-47`／`R-48`／`R-50` 前半＝どれも「読みを決めないと盤面からカードを消す／消さない側の実装になる」）／**`C-8`**（対話応答の pure 化） |
...
| 🔥`C-9` | **ルール規則の棚卸し**（バトル／ダメージ／フェイズ／ゾーン移動の行き先） | M〜L | **台帳＝[RULES.md](./RULES.md)**（公式ルールの1文 × 実装箇所 × 状態）。**2026-09-17 着手**＝✅1／🔴→✅10／👀19／**⚠3＋❓1**（残りは全部**ルールの読み待ち**＝`R-27`／`R-45b`／`R-47`／`R-48`（＋`R-50` 前半））。取り方は RULES.md §4（⚠を上から・写経を全部数える・純関数＋golden） |
