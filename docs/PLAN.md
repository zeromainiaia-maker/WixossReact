...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377f＝(i)配線ギャップ 第11バッチ後）**＝golden **1450**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1030**（回帰ゲート）・manual field loss 0・lint 0 errors/**250 warnings**（⚠`--cache` 使用のため実数とズレることがある＝簿記前に `rm -rf node_modules/.cache/eslint` して測り直す）・held **208枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 338**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第12バッチ。⚠セルを取ったらまず全数分類→系統が見えたらその系統の語彙全体で母集団を取り直す（第11バッチはセル外に3件あった）。**
>    - **① `cardClass × (filter無)` 28（has 88）★★は「1 regex で N 効果」型ではないと確定した**（続き377f で全数分類済み）＝残り28の内訳は **LOOK_PICK_CHAIN 10／構造欠落 8／条件節 4／コスト用法 2／他 timing のトリガー主語 4**。**このセルはもう index として使い切った**＝下の (ii)(iii) 在庫として取る。
>    - **② `cardClass × SIGNI[filter]` 18（441）★★／`cardClass × TRASH{HAND_CARD}` 8（68）★★／`cardClass × TRASH_CARD[filter]` 7（284）★★** ＝未分類の ★★セル。
>    - **③ `eachDistinctLevel × TRASH_CARD[filter]` 20（has 0）／`× SIGNI[filter]` 12（0）／`× BANISH{SIGNI}` 7（0）** ＝**同入口に配線済みなし＝機構未実装の可能性**だが miss 39 は最大の塊。**まず「機構が無いのか、キー綴りが違うのか」を確かめる**（trap (h)）。
>    - **④ `powerRange × GRANT_KEYWORD{SIGNI}` 9（8）★／`levelExact × POWER_MODIFY{SIGNI}` 9（7）★** ＝小さいが has があり穴が濃い。⚠`powerRange` は「パワーを＋N」＝**action の値**との取り違えが濃い（**較正候補**）。
>    - **⑤ `isDisona` 24（has 31）** ＝続き376d で判定器は直したが未配線の入口が残る。
>    - ⚠**着手前に必ず読む13の罠**＝(a) **素の `parseStoryFilter`/`parseLevelFilter`(文全体) を対象フィルタに使わない**＝**`signiClause*Filter` 3兄弟**（クラス／アイコン／レベル）を使う。**新語彙はまずここに兄弟を足せないか見る。** (b) **曖昧なら付けない**（別ピック2本の span に AND で載せると原文と逆の過小実行）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**。(d) **exec 側の配線を必ず確認する**（`matchesFilter` は `excludeSelf`/`upToCount` を見ない）。(e) **セルは入口であって終点ではない**＝ビルダー全体を読む。(f) **枚数・値だけ先に直さない**＝構造が誤ったままだと誤りを増幅する。(g) **★★セルの miss 数は見込み件数ではない**（実測で 6件中5件／22件中14件が偽陽性）。(h) **同じ概念に2つのキー綴りが併存していないか先に確かめる**（`hasCrossIcon` vs `hasIcon`／`story` vs `banishedSourceStory`）。(i) **同じ語彙でも入口ごとに壊れ方が違う**＝1つ直したら同族の入口も必ず見る。(j) **A/B の件数＝直った件数ではない**（live が MANUAL で既に正しい分がある）。(k) **採用前の確認は JSON パースによる構造比較で**（文字列 strip はネスト括弧で誤検知する）。(l) 🆕**セルは母集団の索引であって母集団そのものではない**＝系統が見えたらその語彙全体で全CSV走査して取り直す。(m) 🆕**安全弁はコメントの規律ではなく関数の戻り値にする**（「未知の修飾語が残ったら配線しない」を `null` 戻り値で構造化）。
>    - **✅消化済み**＝「他の」ゲート棚卸し（377）／`noGuard`（377b）／アイコン系（377c）／`levelRange`（377d）／全体バフの語順＋据置held（377e）／ON_ATTACK_SIGNI の味方側トリガー主語＋`excludeSelf` の過剰発火（377f）。
>    - **(ii) 機構ギャップの安い在庫**＝`parseStatus:UNKNOWN` の完全no-op **4件**／`PARTIAL` **23件**／`LOOK_PICK_CHAIN` の exact-N 表現ギャップ **23効果**（⚠続き377f の分類で**`cardClass × (filter無)` の10件がここに合流**＝「デッキの上からN枚公開→**その中から＜X＞のシグニ**」でステップが丸ごと `UNKNOWN`。filter 以前にアクションが無い）／「場に《ライズアイコン》を持つシグニがN体ある場合／あるかぎり」の条件型 **5件**／左右のシグニゾーン限定の filter キーが無い 3効果（`WX25-P3-093-E1`/`E2`／`WXK10-078-E1`。`centerZoneOnly` はある）／🆕**【ソウル】が付いているシグニの filter キーが無い 1効果**（`WXDi-P04-016-E1`＝盤面状態なので `matchesStateFilter` 側の語彙）。
>    - **(iii) 構造混線の在庫**＝`WXK05-043-E1`（owner 取り違え＋level 脱落＋条件節脱落の3重）／`WX24-P3-059-E1` ほか §5d-0 (iii) の登録分／🆕**続き377f で分類した8件**＝`WDK10-001-E3`・`WXEX2-48-E1`（「トラッシュから＜X＞のシグニ1枚を対象とし…それをライフクロスに加える」が **`ADD_TO_LIFE{fromTop}`＝デッキの一番上から**になっている）／`WDK07-E07-E1`・`WXK10-074-E1`（**【アクセ】付与が丸ごと脱落**し `SHUFFLE_DECK` だけ残る）／`WX05-080-E1`（`ATTACH_CHARM` の charm 側が「このシグニ」でなく SIGNI 選択）／`WX25-P2-079-E1`（`AWAKEN_SIGNI` に対象が無い）／`WX16-003-E1`／`WD14-011-BURST`。
>    - **Sonnet 側**＝§7 実機検証（タスク1）。⚠コイン支払いは `BattleScreen.tsx` の**10経路**のうち ACTIVATED【起】1本しか実走していない。
>    - ⚠**見積もりの現在地**＝census **1030**／被覆マトリクス miss **338**。6バッチの実績＝12／30／17／8／15／11効果。**census が動かない回でも効果は直る**（377c は census 据置で17効果）＝**census の増減だけで成果を測らない。**
