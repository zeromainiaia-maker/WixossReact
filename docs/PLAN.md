...
| 🆕(xxxii) | **ON_TRASH／ON_BLOOD_CRYSTAL_ARMOR の any_ally scope 脱落（続き181発見・未修正）**。ON_BANISH と同型で parser に「あなたの…シグニ1体が〜したとき」→any_ally の規則が無く self へ潰れる＝**WX24-P1-015-E1**（ルリグ・「あなたのレベル2以下の＜悪魔＞のシグニ1体がコストか効果によって場からトラッシュに置かれたとき」）・**WDK08-L01-E1**（ルリグ・「あなたの＜紅蓮＞のシグニ1体が血晶武装状態になったとき」）。engine 側の LRIG 走査は続き181で通したので**残りは parser 規則＋（レベル/コスト起因の）triggerFilter 抽出のみ**。ON_BANISH 側の据置4件（アタックフェイズ前置き2＝WX18-002-E1/WXEX1-18-E1・チャーム付き＝WXK07-074-E1・動的レベル比較＝WXK11-018-E1）も同枠 |
