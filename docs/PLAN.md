...
- **🆕 セッション（2026-07-21・続き243・Opus・**タスク12(xxix)「そのシグニの【出】能力」クラスタ（≈30効果として登録）を**忠実表現化**＝**挙動上は偽陽性**と確定したうえで死アクションを畳んだ。census 1825維持・golden 557→**562**）**（続き236 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **忠実表現化（ユーザー選択＝最小修正・低リスク）**＝死アクションを配置アクションの `suppressOnPlay` フラグへ畳む。**型**（AddToField/RevealUntilToField/LookPickChainStage）＋**parser** `foldSuppressOnPlay`（`parseCardEffects` 末尾の単一チョークポイント＝全経路網羅・アンカー探索は ADD_TO_FIELD/REVEAL_UNTIL_TO_FIELD/LOOK_PICK_CHAIN(field)/SEARCH/REVEAL_AND_PICK/CONDITIONAL埋没/入れ子SEQUENCE。アンカー無しは畳まず据置）＋**engine 前方安全**（REVEAL_UNTIL_TO_FIELD 3経路で suppressOnPlay 時に自身【出】を積まない）＋**decompiler**。**76効果を折込**（AUTO 49枚を heldReview で純fold機械確認採用・MANUAL/混在17枚を byte安定 round-trip で直接 fold）。
  - **残22効果は据置**＝配置アンカー無し（STUB配置8・非配置/misparse14）。死ブロックのまま no-op で挙動不変＋STUB実装時の抑制意図マーカーとして前方安全。
  - **ゲート**＝全ゲート緑（typecheck／**golden 562**〔fold5件追加＝直畳み/CHOOSE枝内/CONDITIONAL越し/JSON忠実性/非該当据置〕・smoke 10722全OK・fuzz 全0・census **1825** 維持・同型★0・lint 0 errors）。`npm run regen` 済。詳細 BUGFIXES 続き243。
  - **次の一手（Opus）**＝(xxix) 残＝真の§6.3級のみ（ルリグかシグニ union＝NEGATE_ATTACK 対象種別／**別系統 MISSING＝ADD_TO_FIELD 自身【出】が本来誘発すべきが未発火**〔当クラスタ調査で発見・大型で回帰リスク大〕／BET／unless）。他は**タスク12 生き残り在庫**（(xliv)残テール・(xxxix)残・(xxii)残・(xlii)残）か **§6.3 tractable**。**Sonnet はタスク1（§7 実機検証）＋タスク6（新語彙着地分の再収穫）**。
### 📊 恒久指標（維持中・逐次更新）
