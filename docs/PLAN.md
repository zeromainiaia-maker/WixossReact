...
- **🆕 セッション（2026-07-22・続き251・Opus 4.8＋Codex 分担・**[P1_COMPLETION_ROADMAP](./P1_COMPLETION_ROADMAP.md) **バッチ1「状態条件節の持ち上げ」第3波＝盤面/ゾーン状態条件の丸ごと脱落33効果を採用**（census 1792→**1761**・golden 588→**590**）**（続き250 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **次の一手（Opus/Codex）**＝**バッチ1第4波候補**＝(a)条件節クラスタ残の同機構群（センタールリグ条件系＝LRIG_LEVEL/LRIG_COLOR/クラス 約8・「このターンに〜していた場合」履歴系は engine counter 有無で仕分け）(b)第2波/第3波見送りの機構解消（REVEAL_AND_PICK 内 did-it continuation・hand-or-energy 宛先・owner:any 場条件・能力なし filter）。⚠**codex への指示書に「buildEffectsJson へ force-adopt を仕込むの禁止・採用は heldReview --adopt 経由」を明記**（CODEX_GUIDE §5 に追記済み）。**Sonnet はタスク1（§7 実機検証）**＝distinctColors/HAS_KEY/hasCharm/TURN_OWNER 配線の実機スポットチェック（要実機検証）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 590・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1761。
>
