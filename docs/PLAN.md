...
**補欠（在庫が尽きたら）**＝(a) **timing census 残128（113クラスタ）の振り分け台帳作成**＝`docs/_timing_census.txt` の各クラスタについて engine 収集関数の有無を機械照合し「§6.3 送り（受け皿なし）」と「Opusタスク16（parser 語彙のみ不足）」に仕分ける。PLAN は「残128は全て機構待ち」と結論しているが**クラスタ単位の裏取り台帳は未作成**。~~(b) `textNoJson` 56枚の実体確認~~ **✅続き170（Sonnet）で完了＝56枚全件が正当・真の欠落0件**。内訳＝①デュアルカラーエナ注記52枚「（エナコストを支払う際、このカードは○か○１つとして支払える）」＝`Color`列が既に2色文字列（例"青緑"）で`costs.ts`の`cardColor.includes(color)`が構造的に対応済み、EffectTextは重複するルール注記。②WXDi-P13-023（ナナシ）のコイン獲得＝CSV`Coin`列を`BattleScreen.tsx`がゲーム開始時/グロウ時に直接参照（5箇所）、DSL効果不要。③WX24-D1-TK1／WX24-P1-TK2A／WX26-CP1-TK01（リミットアッパー/ルリグバリア/シグニバリアの各トークン）＝トークンの効果本体はBattleScreen内にハードコード実装済み（`limit_upper_token`・`countBarrierTokens`等）でJSON化不要。詳細 BUGFIXES 続き170。
...
- **🆕 セッション（2026-07-16・続き169・Codex CLI〔gpt-5.6-sol・reasoning high〕が実装／Claude〔Opus 4.8〕が検証・PLAN §3 Opusタスク12(xxvii) Cluster D＝timing 取り違え（【自】が ON_PLAY 化）を消化）**
  ...
  - **採用＝SPDi43-12-E2（内側 SPDi43-12-sub-E1）**＝【起】バイブスMAX でセンタールリグに付与する AUTO 能力「対戦相手のエナゾーンからカードがトラッシュに置かれたとき、このルリグをアップする」の timing を `ON_PLAY`→`ON_ENERGY_TO_TRASH`（`triggerCondition.energyTrashedOwner:opponent`）へ是正。**engine 小改修**＝`collectEnergyToTrashTriggers`（triggerCollect:864）に**付与済みルリグ AUTO（`lrig_granted_auto_effects`）の走査を追加**（従来は場のシグニ effectsMap のみ＝付与能力が timing を持っても no-op だった）。既存の相手ルリグ付与AUTO走査（:1768）と同パターン。
  - **Claude 検証（engine 変更ゆえ重点）**＝①`lrig_granted_auto_effects` が実在フィールドと確認（effectExecutor:4111 で GRANT_LRIG_ABILITY 実行時に格納・:1768 で既存走査あり）②機械 diff で変更は SPDi43-12-E2 の1効果のみ（granted ability の timing＋triggerCondition）③**発火経路3点接続を確認**＝parser emit→`lrig_granted_auto_effects` 格納→BattleScreen:2671/2673 が energy→trash 時に collector 呼び出し→追加ブロック走査④追加ブロックの `checkActiveCondition`/`ctx.turnPhase`/`evalUseCondition` が既存同一シグネチャと一致⑤census 2044・golden 374・同型★0・gates（smoke/fuzz 全0）独立再実行。**退化ゼロ**。
  - **§6.3 据置3枚（collector 不在＝新規イベント機構要）**＝WX12-006-E2「アップ状態になったとき」（"becomes up" timing/collector が engine に無い＝`ON_SIGNI_BECOMES_UP` 新設要）／WXDi-P11-066-E1「対戦相手の効果でこのカードが捨てられた」（`ON_HAND_DISCARDED` は場の watcher 用で手札カード自身の非コスト捨てを拾わず、発生源帰属も渡らない）／WXDi-P11-007-E1「エナ→手札 or 場」（`ON_ZONE_MOVED` は場のシグニゾーン間移動専用で from/to 情報も OR 条件も持てない）。
  - **次の一手**：Opus＝(xxiii) リコレクト分割（§6.3級 GRANT 機構）・(xxvii) 残 Cluster B（duration・非 POWER_MODIFY 型）/E（機構欠落）・タスク1残（【常】アタックできない家族）・IS_MY_TURN化残104。**新機構タスク**＝`ON_SIGNI_BECOMES_UP`（WX12-006 系）・手札カード自身の効果捨て timing（WXDi-P11-066 系）・エナ発ゾーン移動 timing（WXDi-P11-007 系）・合計レベル制約・複数レベル探索・相手手札選択・遅延効果・イベント来歴・ターン所有アーキ改修。Sonnet＝`oppDrawOwnEffectOnly` PASS 反転確認（続き162 在庫）＋§7フルバッチ回帰。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 374・smoke/fuzz 全0・同型★0・census 2044。
>
