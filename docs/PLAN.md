...
**直近＝2026-09-21＝§5.7 `S-31` ② 第3段＝自分の盤面・デッキから払うコスト（撃てない【起】 83 → 37）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第446バッチ）
- 🔴**母集団を数え直した**＝前回の残り一覧は**その文脈の gate が提示しない【起】**（入口がトラッシュ・手札・エナ）を含んでいた
  ＝📏**場のシグニ 697効果中 51 → 21／ルリグ 567効果中 32 → 16**（合計 83 → 37＝**46効果**が撃てるようになった）。
- 🔴**別系統のバグを1つ潰した**＝**`cost.deckTrash`（live 9効果）はどちらの実行経路にも支払いが1行も無く、人間も CPU も踏み倒していた**
  ⇒ 🆕`deckTrashCost.ts` を両経路から呼ぶ。**JSON も逆翻訳も正しいのでどの計器にも映らない形**。
- 入れたもの＝**提示ゲートの検算2本**（`charmTrashAffordable`／`removeOppVirusAffordable`＝これが無いと CPU が無限ループ）
  ＋**`pickCpuUnderSelfTrashKeys`**（下から落とす・集合制約は組を探す）＋**ゾーン選択を行き先違いの3キーへ**
  （`fieldTrash`／`fieldBanish`／`fieldToDeckTop`）＋**ルリグ【起】の場コスト配線**（🔴第442バッチの「実行側に口が無い」は誤り＝口は在った）。
- 📏**実カード照合**（engine だけ・決定論）＝提示された **21（場のシグニ）＋8（ルリグ）すべてで支払い選択が成立**＝
  「提示は通るのに払えない」は0件。⚠🔴**A/B では測れない**＝**ユーザー作27デッキの異なり367枚に該当カードが1枚も無い**（`S-20` と同型）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31` ②の残り**（撃てない【起】37＝全部1〜5効果の小山）。次に `S-29` |
...
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**撃てない【起】は残 37効果**（`S-31` ②）／うち4キーは**支払い funnel 自体が無い**（人間も踏み倒し） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4364**）｜**実機 PASS**（通し対戦＝`VERIFY_DECK` と `VERIFY_DECK_MECH` の両方） |
...
| 🔥`S-31` | **CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望）。🏁**①は測って否定**（第444バッチ）／🏁**③は消化**（第445バッチ＝札の使いどころ）／🏁**②は3段まで**（手札捨て／エナ・場／自分の盤面・デッキ）＝**撃てない【起】 210 → 108 → 37**（第442・446バッチで母集団も文脈別に数え直した）。**残り＝②の続きだけ**（場のシグニ 21／ルリグ 16＝**全部1〜5効果の小山**）。🔴**うち `selfToDeckBottom`／`chargeCounterRemove`／`fieldToLrigTrash`／`trashArtsFromLrigDeck` は CPU の話ではなく支払いの穴**（実行経路に支払いが無い＝人間も踏み倒している）＝取る順は「支払い funnel → gate の検算 → allowlist」。 | 中 | 作った CPU デッキを意図どおりに回せる |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15`・🏁`S-23`・🏁`S-20`・🏁`S-19` を消化）**＝~~`S-15`（候補列挙器）~~ → 🏁~~`S-18`（次ターン項）~~ → 🏁~~`S-16`（前半探索＝**配線済み・既定オフ・互角**）~~ → 🏁~~`S-21`（探索の目的関数）~~ → 🏁~~`S-17`（後半＝攻撃探索＝**全段3段完了・既定オフ・互角**）~~ → 🔵**`S-6`（機械学習＝🏁第1〜2段／微調整は差が無いと測り切った）**→ 🏁~~`S-27`（測定台の感度）~~ → 🏁~~`S-26`（エナチャージがターンをまたいでいない）~~ → 🏁~~`S-28`（場のシグニをチャージ）~~ → 🏁~~`S-22`（対象選択が効果の意味を見ていない）~~ → 🏁~~`S-24`（自己対戦のマリガン）~~ → 🏁~~`S-30`（測って否定＝CPU は退化していない）~~ → 🏁~~`S-14`（作戦データの「使い方」）~~ → 🔥**`S-31`（CPU の作戦をもっと細かく＝ユーザー要望・🏁①③と②の3段まで消化）** → 🏁~~`S-32`（対象の指示）~~ → `S-29`（宣言の帰結のコスト＝母集団未計測）。
...
- **2026-09-21 時点**（第421〜446バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`・🏁`S-31` ②・🏁`S-32` 第1段・①・🏁`S-31` ② 第2段・🏁`S-32`・🏁`S-31` ①・🏁`S-31` ③）
  ...
  - 📏**CPU が撃てない【起】**（§5.7 `S-31` ②・🆕**その文脈の gate が提示するものだけを分母にする**）＝**場のシグニ 697効果中 21（3.0%）／ルリグ 567効果中 16（2.8%）＝合計 37**（第439・442・446バッチで 210 → 108 → 37）。🔴**過去の大きい数字は分母の取り方が違う**＝旧「648（24.6%）」は文脈を混ぜた値／旧「83」は**入口が場ではない【起】**（トラッシュ・手札・エナ＝`cpuOffFieldActivate` の担当）を含んでいた
