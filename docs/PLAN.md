...
- **🆕 セッション（2026-08-13・続き455・Opus 5）＝§6.4 **O-16** 第2波＝**能力喪失のゾーン継続**と**複数ゾーン指定**を機構化。2効果を是正し、**O-3 の据置2件も同時に解除**。golden 1942→1944・census 832 据置・held 106枚/47群 据置**。
  - **🔴① 直した2効果はどちらもゾーンに紐づく能力喪失**＝`WXEX2-04-E3` は live が `REMOVE_ABILITIES{owner:'self', count:1}`＝**自分のシグニ1体から能力を奪う自傷**（指定ゾーンも期間も落ちていた）／`WX25-P3-014-E2` は「シグニゾーンを**２つまで**指定し」の**指定ステップそのものが JSON に存在しなかった**。
  - **⭐② 機構は続き454 の型をもう1つ足すだけ**＝`FieldGrant` に `kind:'abilityLoss'`（`kind:'power'` が先例）。読み手は既存 funnel `activeFieldGrantsForSigni`（zone/filter/condition を毎回評価）に projection を1本足し、`collectContinuousAbilitiesRemovedSigni` がゾーン走査で拾う。⚠**per-card 側が正しい札もある**＝`WX24-P4-002-E1` は原文が「（このアーツの使用後に場に出たシグニはこの効果の影響を受けない）」と**明記**＝原文がこの区別を書き分けている。
  - **⭐③ 指定ゾーンの複数対応**＝`designated_zone?: number` は1つしか持てず「２つまで指定」を表現できなかった。`designated_zones?: number[]` を追加し、**読みを `designatedZones()` 1本へ集約**（旧単一値も吸収＝セーブ互換）。`DESIGNATE_SIGNI_ZONE` は `count` を取り複数選択 CHOOSE を出す。⚠**指定のたびに前回分を空にする**（追記式なので古いゾーンが混ざると指定していないゾーンにまで効く）。
  - **🔴④ 併せて是正した過剰実行**＝`BLOCK_OPP_ZONE_PLACEMENT` の `designated_zone ?? 0` は**未指定のときゾーン1を問答無用で禁止する**フォールバックだった。`designatedZones()` へ寄せて、空なら「禁止するゾーンがない」で止まる。
  - **⚠⑤ parser は3点**＝(a)連用形「シグニゾーンをNつまで指定し、…」を接続詞分割の候補に追加（**この語形が無かったため指定ステップが丸ごと生成されていなかった**）(b)指定文の**枚数を読む**（語順が2通り）(c)「（そこ｜それらの｜指定された）シグニゾーンにあるシグニは能力を失う」を `zoneSource:'designated' + count:'ALL'` へ。
  - **⚠⑥ golden の autopilot は CHOOSE で先頭1つしか選ばない**＝複数選択の検証は `resumeChoose(['zone_0','zone_2'])` で明示的に駆動した。**「run で通った＝複数選択が動く」ではない**。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-16 の残5**＝次の型は **`FieldGrant{kind:'blockAction'}`**（アタック禁止のゾーン継続2件）。パワー→能力喪失と2つ先例ができたので同じ形で載る (b)**O-3 の残31**（`STUB` 系が主体。⚠型ごとに受け皿を実測してから）(c)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵＝続き452 の `SP38-006` 外科パッチが先例）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452〜455 の4件が積まれた（相手応答モーダル／ターン跨ぎの能力喪失／ゾーン指定→後から配置／2ゾーン選択UI）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き455（§6.4 O-16 第2波＝能力喪失のゾーン継続＋複数ゾーン指定）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1944（+2＝ゾーン能力喪失の E2E／2ゾーン指定の E2E）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 34フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）。live effect 単位 diff **2効果・effectId 増減 0/0**。
- **旧行（2026-08-13 続き454・§6.4 O-16＝指定シグニゾーンの配線）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1942（+2＝指定ゾーンの現ターン継続 E2E／live 6効果の「保存先と読み手の一致」）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 34フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）。live effect 単位 diff **6効果・effectId 増減 0/0**。
...
| **O-16** | **「指定したシグニゾーン」の残り（別機構待ち）** | 5効果 | 🏁続き454＝**パワー修正6効果**（`applyActiveFieldGrant`＝現ターンのゾーン継続／`alignDesignatedZoneOwner`＝保存先を読み手から揃える）／🏁続き455＝**能力喪失のゾーン継続2効果**（`FieldGrant{kind:'abilityLoss'}` ＋ **複数ゾーン指定**＝`designated_zones` と読み手 funnel `designatedZones()`）。**残＝別機構が要る5件**：(a)**アタック禁止のゾーン継続**＝`WXDi-P11-046-E2`／`WXDi-P11-055-E1`（`BLOCK_ACTION{ATTACK}` が per-signi。**`kind:'blockAction'` を足すのが次の型**） (b)`WDK10-009-E2`＝`STUB{POWER_MOD_PER_COUNT}`（レベル比例）が指定ゾーンを読まない (c)`WXDi-D09-P13-E1`＝**そもそも別物**にパースされている（`STUB{LRIG_GROW_RESTRICT}`） (d)`SP38-006-E1` の1つ目の内側能力＝対象種別・体数・期間の3軸とも未忠実（カードは PARTIAL 温存）。⚠**計器の注意**＝`zoneSource` キーの有無だけで数えると `BLOCK_OPP_ZONE_PLACEMENT`（`zoneBlockSource:'designated'` で自前に読む）を**偽陽性**に拾う |
