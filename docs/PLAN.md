...
| (lxi) | **✅第1波（「対戦相手が」形・29カード30効果）＋第2波（「対戦相手は」＝主語分配形・8カード9効果）とも消化済み（2026-07-30）。残17効果はいずれも別機構待ちの honest defer**。⚠**第2波の教訓＝「ゲートが無い」と決めつけず、現状 JSON と engine ハンドラの実装まで読むこと**（13効果を同一文型と括ったが実際は6種の別バグ＝過剰実行／owner が self の別物／executor の dispatch 欠落で完全 no-op／符号逆／専用 STUB が完全実装済みなのに後続ステップが二重実行／既に正しい）。**🆕残17の内訳**＝①複文「以下の2つから1つを選ぶ」2（`SPDi43-02-E1`／`SPDi43-07-E1`）②回避コストが engine 非対応5（`WX22-025-E3`／`WXDi-P16-088-E1`／`WXK05-009-E2`〔可変《無》〕／`WXK06-047-E1`／`WXK06-067-E1`〔「デッキの上に置く」型の回避〕）③対象節以外の前置き5（`WX24-P2-022-E1`／`WX24-P4-011-E3`／`WX25-CP1-012-E1`／`WXDi-P06-023-E2`／`WXDi-P13-075-E1`）④「そのシグニ」照応2（`WXDi-P08-007-E1`／`WXEX2-25-E1`）⑤純加算チェック不一致1（`WXDi-P05-TK01A-E1`）⑥3機構一体1（`WX24-P4-007-E1`＝`DO_THREE_THINGS` は別カード用の text パターンで完全 no-op。回避が「自分のシグニをトラッシュ」＋標準形＋トラッシュ移動禁止の3つが一体）⑦制限系で別機構1（`WXDi-P11-009-E3`＝配置制限を `LRIG_GROW_RESTRICT` にすり替えている）。**engine の回避手段語彙**＝`costColors`／`opponentHandDiscard`(+`ALL`)／`opponentHandDiscardFilter`／`opponentEnergyTrash`(+`ALL`)。②を開けるなら「自分のシグニをトラッシュ」「カードをデッキの上に置く」枝の追加が次の水平展開。詳細 BUGFIXES 2026-07-30 の先頭2節 |
| (lxii) | **🆕 `CONDITIONAL_DISCARD` は型宣言だけあって executor に dispatch が無い（2026-07-30 発見）**。`src/types/effects.ts` に `ConditionalDiscardAction` があり `execStubPart2.ts` に `stub.id === 'CONDITIONAL_DISCARD'` のハンドラもあるが、**実データは `{type:'CONDITIONAL_DISCARD'}`（アクション型）で出てくるため一生マッチしない**＝完全 no-op。タスク12(lxi) 第2波で `WX11-044-BURST` が標準ゲートへ移った結果、**残る唯一の使用者は `WD16-016-BURST`**「対戦相手の手札が５枚以下の場合、対戦相手は手札を１枚捨てる。６枚以上の場合、代わりに２枚捨てる。」＝(lxi) とは別物の**手札枚数分岐**。`CONDITIONAL{HAND_COUNT}` 系の既存条件で組み直すのが筋（型ごと退役できる見込み）。 |
| (lxiii) | **🆕 `WX15-033-E2` の fresh が `CONDITIONAL{LIFE_COMPARE_OPP}` を落とす（2026-07-30 発見・held 据置中）**。原文「対戦相手の中央のシグニゾーンにあるシグニ１体を対象とし、**対戦相手のライフクロスの枚数があなたより多い場合**、それをバニッシュする」で、curated は条件を持つが fresh は条件を落として**無条件バニッシュ**になる。そのため (lxi) 第2波では `WX15-033` をカード単位採用できず、`WX15-033-BURST` だけ curated へ直接配線した。**このカードを held から解くには先にこのドリフトを直す**（同型の「〜の枚数があなたより多い場合」文型が他にも巻き込まれていないか要確認）。 |

> **✅残0でクローズした在庫の完了行原文は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-07-30 整理」節へ退避**＝**(xxix)**〔任意【出】コスト・計15波・残4は明示保留〕／**(xlvi)**〔look-pick 計73効果・第1〜17波〕／**(lvii)**〔census キー表 `pickUpTo` 較正〕／**(lviii)**〔トラップ公開 LPC 移行9効果・残1は honest defer〕／**(lix)**〔`split_top_bottom` 全4効果〕／**(l)**〔置換イベント横取り3文型を含む A群/B群 全体・残0〕／**🆕(lx)**〔①`WX25-P1-056-E1` の非バニッシュ離場→バニッシュ置換を engine 9サイトへ配線／②`WX12-020-E3` の「この方法で捨てた枚数」比例＝新語彙 `deltaPerLastProcessedCount`。2026-07-31 に残0クローズ・詳細 BUGFIXES 2026-07-31 節〕。それ以前の消化済み在庫〔(i)〜(lvi) の大半〕は同ファイルの 2026-07-19・07-24・07-28・07-29 の各退避節。
...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lx) 残2件を実働化し (lx) を残0クローズ**（golden **1128→1130**・census **1373 据置**（`BASELINE_HIGH=1373`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（5986枚・265群）・held **288枚 据置**・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 1 / added 0 / removed 0**／`build:effects` 冪等）
  - **①`WX25-P1-056-E1`（非バニッシュ離場→バニッシュ置換）を engine で実働化**＝新設 `applyEffectLeaveReplaceBanishSubstitute` を**非バニッシュ離場の9サイト**（`execBounce`／`execSendToEnergy`／`execTrash`（場）／`execTransferToDeck` の各 apply ループ＋`applyDirectAction` の BOUNCE/SEND_TO_ENERGY/TRASH/EXILE/TRANSFER_TO_DECK）へ配線。バニッシュ先は `banishDestination`（execBanish と同じ置換走査つき）。**バニッシュ経路には入れない**（原文「その移動がバニッシュによるものでないなら」）。**JSON 無変更**＝STUB id をそのまま engine 側の宣言として読む（`BANISH_SUBSTITUTE` と同じ慣例）。
  - **⚠ここが唯一の近似＝「してもよい」は自動適用**。場離れ各経路は同期的な ctx 変換で対話 pause を張れないため、既存2つの離場置換（`findEffectLeavePowerReductionSubstitute`／`applyEffectLeaveLrigAbilitySubstitute`）と同じ決定論的近似に揃えた。**対話実装があるのはバトルバニッシュの `BANISH_SUBSTITUTE`（BattleScreen 側）だけ**＝実機で選ばせるなら3つまとめて対話化する（§6.4 に明記した）。
  - **②`WX12-020-E3`（この方法で捨てた手札1枚につき－6000）は三重に間違っていた**＝(a) `TARGET_AND_DISCARD_HAND` は直後が `CONDITIONAL(IS_MY_TURN)` でないと**既定の帰結が BANISH** になるため**相手シグニを1体バニッシュ**していた (b) 捨てる枚数が**1枚固定**（「好きな枚数」でない） (c) `POWER_MOD_BY_HAND_COUNT` が**現在の手札枚数**×－6000 を**相手シグニ全体**に乗せていた。**新語彙1本 `PowerModifyAction.deltaPerLastProcessedCount`**（倍率元＝直前ステップで実際に処理した枚数）で `SELECT_TARGET_ONLY→STORE_LAST_PROCESSED_TARGETS→TRASH{HAND_CARD ALL upTo}→POWER_MODIFY{targetsStored}` の正準形（タスク12(liii) と同型）へ。
  - **⚠parser の落とし穴＝文ごとの `steps.push` は入れ子 SEQUENCE を畳まない**。内側 SEQUENCE が pause すると**外側 continuation が内側の continuation を上書き**して残りステップが黙って落ちる（`execSequence` の pending 合成）。よって2文にまたがる文型は `splitSentences` **前**に全文で捕捉し、**平坦な SEQUENCE** を返すこと。
  - **`execPowerModify` の `targetsStored` を自動適用に**＝「それのパワーを…」の対象は先行の対象宣言ステップで確定済み。従来は候補1件の選択UIを再提示し、同じ対象へ **ON_TARGETED が二度**立っていた。実データの該当は `WXDi-P03-089`（2箇所）と本件のみ。decompiler も `targetsStored` の主語を「それ」に直し、`WXDi-P03-089`／`WXK06-032`／`WX25-P1-103` の逆翻訳が原文一致に改善（表示のみ）。
  - **次の一手**＝**Opus：①🆕 `CONDITIONAL_DISCARD` の executor 欠落**（型宣言と STUB-id ハンドラはあるが `{type:'CONDITIONAL_DISCARD'}` を捌く dispatch が無い＝残る唯一の使用者 `WD16-016-BURST`〔手札枚数分岐〕が完全 no-op）**②🆕 `WX15-033-E2` の fresh が `CONDITIONAL{LIFE_COMPARE_OPP}` を落とすドリフト**（採用すると無条件バニッシュへ退化するので held 据置中）**③「コストの合計」族の STUB/UNKNOWN 14件④`WX11-043-E1` の `crossOnly` 欠落⑤タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑥`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑦§6.4 の配置数制限すり抜け7効果⑧§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**タスク12(lx) の未検証UI 2件**（`WX12-020-E3` の「好きな枚数捨てる」ピッカー／`WXDi-P03-089` の対象再選択が消えたこと）／(lxi) 第1・第2波の UI 6件／§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lx) 残0クローズ後 最新値（本行を直近の正とする）**：census **1373 据置**（`BASELINE_HIGH=1373`）、golden **1128→1130**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288枚/106署名 据置**（(lx)② で1枚 held に落ち同じ回で採用）、manual field loss 0。live per-effect 差分 **changed 1 / added 0 / removed 0**（`WX12-020-E3`）、`build:effects` 冪等。**新語彙は2本**＝`PowerModifyAction.deltaPerLastProcessedCount`（倍率元＝直前ステップの処理枚数）と engine 内部の `applyEffectLeaveReplaceBanishSubstitute`（JSON 語彙は増えない＝既存 STUB id を宣言として読む）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1130**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1373**（回帰ゲート）・held **288枚/106署名**（2026-07-31 タスク12(lx) クローズ後に実測）。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（**現存は (lxii)〔`CONDITIONAL_DISCARD` の executor 欠落〕・(lxiii)〔`WX15-033-E2` の条件脱落ドリフト〕・(lv) 残2経路**（(lxi) は第1・第2波とも消化済みで残17は別機構待ち）。**(l) は A群/B群とも 2026-07-30 に残0クローズ、🆕(lx) も 2026-07-31 に残0クローズ**、**(xxix) も完全クローズ**＝残4件は明示保留、**(xlvi)／(xlii)／(lix)／(lviii) も残0**）＋**タスク16 残＝timing 半分32効果のみ**（`cost.underSelfTrash` は 2026-07-30 に残0クローズ。**着手前に振り分け台帳の再トリアージが要る**＝§3 タスク16 の行を読む）**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxii)(xxix)(xxxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証）＝在庫はここだけで、Opus が機構バッチを回すたびに積み上がっている**（未検証UIの単一 worklist は **§7「残る実機検証項目」に集約済み**＝§4 進捗サマリと二重に持たない）。⚠**§6.3 H の4機構は engine/golden では固定したが UI 経路は計器に一切映っていない**＝ここが最優先。タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
...
- **🆕 I. `WX25-P3-028-E2`（エクシード本体・2026-07-30 に honest defer）**＝原文「このターンと次のターンの間、あなたはリフレッシュできない。以下を３回行う。「あなたか対戦相手のデッキの上からカードを６枚トラッシュに置く。」」。**3機構が一体で要る**＝①**リフレッシュ不可の2ターン寿命**（チョークポイントは `src/engine/refresh.ts` の `applyRefreshState` 1本＝呼び出しは `effectExecutor.ts:5528` と `screens/battle/battleUtils.ts:106` の**2箇所**。`PlayerState` に寿命フィールドを足して早期 return し、**ターン境界リセット3箇所**へ載せる。隣接する `next_refresh_replaced` の命名・寿命管理に揃える）②**N回反復**（実データ**5効果**＝`WX25-P3-028-E2`／`WXDi-CP01-024-E1`／`WXDi-CP02-047-E1`／`WXDi-P07-007-E3`／`WXDi-P08-007-E3`。先例は `REVEAL_UNTIL_TO_FIELD` の `repeat` 再帰＝`effectExecutor.ts:4123-4146`）③**各反復での「あなたか対戦相手」選択**（既存 `CHOOSE` で書けないか先に試す）。**現行 live は誤分類のまま**＝`STUB{LRIG_GROW_RESTRICT}`（＝ルリグのグロウ制限＝原文と無関係。`parseSentencePart1.ts:482` に同型の誤マッチ是正の先例あり）＋`STUB{REPEAT_N_TIMES}`＋自分固定の6枚ミル1回。**部分実装しないこと**（反復だけ通してリフレッシュ不可を放置すると、デッキアウト・ロックが片肺になる）。
