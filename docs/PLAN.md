...
**直近＝2026-09-20＝🏁§5.7 `S-15`＝CPU の候補列挙器 `listCpuMoves`**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🏁**`src/screens/battle/cpuMoves.ts`**＝その盤面・フェイズで CPU が取れる手を全部 `CpuMove[]`（12種・支払いの内訳まで決めた形）で出す。各 `pickCpu*` は「列挙 `listCpu*`／選ぶ」に割り、**`tryCpu*` も同じ入力の組み立てを通す**（探索と本番で列挙の道が1本）。
- **挙動不変**＝自己対戦3シードの全ログが変更前と1行も違わない。
- 🆕🔑**常時の照合＝「打った手は必ず列挙に出ている」**（`observeMoves`／`observeChoice`）を `selfplay`（gates）と golden が毎回回す。反転確認済み。
- 🔴**初回照合の 11手の食い違い＝観測位置**＝MAIN は**1回の呼び出しで召喚ループ → ライズ／【起】へ進む**（1呼び出し≠1手）⇒ `S-16` は召喚ループを1手ずつに割る。
- 🔑**実測**＝MAIN の候補 中央2・p90 8・最大18／**engine だけの1手適用 平均0.3ms**（登録時見積もり 80〜115ms は実経路の値）＝**`S-16` の粗選別は幅をほぼ絞らなくてよい**。
- 🔴**実機で既存の穴を1つ直した**＝CPU 対戦の見張りが「人間の応答待ち」の盤面では DB を読み直さず、通知を1件取りこぼすとガード確認のまま永久停止（T2 手詰まり）⇒ 読み直しは常にする（再実行の抑止は据置）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-16`**（前半ブロックの探索＝`applyCpuMove` と先読みの情報漏れ塞ぎから）。🆕**`S-20`（A/B の山が型を踏まない）は探索の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 7**（`S-16`〜`S-20`・`S-14`・`S-6`）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先読みの情報漏れ（`S-16` の前提）②先攻が勝てない 28.1%（`S-19`）③**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4333**＝±0・`selfplay` が打った手の照合 68手/外れ0）｜**実機** `node scripts/verifyFullMatch.mjs cpu` 1回目 FAIL（見張りのバグ＝同日修正）→ **PASS**（9ターン / 231手 / 318秒） |
...
| 🔥`S-16` | **前半ブロックの探索**＝`ENERGY`〜`ATTACK_ARTS` を**1つの探索単位**にしてビーム探索（目的関数＝**アタックステップに入る時点の盤面**）。🆕**材料は揃った**＝`listCpuMoves`（`S-15`）／engine だけの適用は1手 0.3ms＝幅はほぼ絞らなくてよい。⚠**`applyCpuMove`（探索用）はまだ無い**・**MAIN の召喚ループは1回の呼び出しで複数体置く**＝1手ずつに割る（PLAN_DETAIL `S-15`）。⚠**着手前に塞ぐ**＝いまの先読みは**山の順序を覗いている**（`simulateEffect` が実 state を `structuredClone` → `execDraw`（`effectExecutor.ts:382`）が `state.deck.slice(0, n)`＝**本物の次の1枚**を引く）。 | 大 | **手順の入れ替え**と**資源配分**を CPU が検討するようになる（いまはどちらも固定台本）。⤵**`S-13` はここに吸収** |
...
🔑**取る順（2026-09-19 ユーザー決定・第2版／2026-09-20 に 🏁`S-9`・🏁`S-10`・🏁`S-15` を消化）**＝~~`S-15`（候補列挙器）~~ → **`S-16`（前半探索）**→ `S-17`（後半＝攻撃探索）→ `S-18`（次ターン項）→ `S-14`（作戦データ）→ `S-6`（機械学習）。
...
- **2026-09-20 時点**（第410バッチ＝🏁§5.7 `S-15` 候補列挙器）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 7**（`S-16`〜`S-20`・`S-14`・`S-6`＝🏁`S-15` を消化）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4333**・`census:traceinv` I1=0・`selfplay` 1戦 決着＋打った手の照合 68手/外れ0）｜**実機** `verifyFullMatch.mjs cpu` PASS（修正後）
  - 📏**BattleScreen**＝**6,287行**（±0）｜`controller/cpuTurn.ts` 1,575 → 1,366行（列挙を `cpuMoves.ts` 543行へ）
  - 🆕📊**CPU の候補数**（`--census-moves`・4戦 432盤面）＝ENERGY 中央5／GROW 0〜1／**MAIN 中央2・p90 8・最大18**／ATTACK_ARTS 0〜1｜**engine だけの1手適用 平均0.3ms・最大4.5ms**
  - 📊**自己対戦 A/B**＝`legacy-power` vs `default`＝**370戦で 50.0% [44.9, 55.1]**｜**先攻の勝率 33.5% [28.9, 38.5]**（370戦・`S-19`）（据置＝今回は試合が同一）
