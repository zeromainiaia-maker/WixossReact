...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15` → `O-10`（明示 defer **残 3 id/3件**。続き507〜514 で7巡済み＝残りはすべて1件ずつ実機構が要る〔`ON_TARGETED` の収集地点／ピースのカットイン窓／グロウ履歴 state＋A-B面フリップ〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 3 id / 3件 | 🆕**2026-08-16 続き514（7巡目）＝4種/4件 → 3種/3件**（`WX12-023-E1`）。原文「【常】：対戦相手のトラッシュとルリグトラッシュにあるカードは能力を失い、効果を受けない。」🔑**据置理由「相手トラッシュに触る全効果に及ぶ」は古かった**＝トラッシュを発生源にする候補列は**`movableTrashCandidates`（`execUtils.ts`）の1点 funnel**（8呼び出しが全部そこを通る）。「効果を受けない」は**候補0**で表す（`isOwnTrashMoveLocked`＝トラッシュ移動ロックと同じ形＝アクションは「対象がない」で自然に no-op し、盤面を巻き戻さなくてよい）。「能力を失う」の読み手は**2つだけ**＝①トラッシュ起動【起】のボタン ②ルリグトラッシュ由来の継承【起】。⚠**「効果を受けない」は主語を問わない**＝原文は誰の効果とも書いていないので、**持ち主自身のトラッシュ回収も止まる**（ロック札）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き514）。⚠**全経路 実機未検証**＝§7 送り。**■残 3 種 / 3 件（すべて1件ずつ実機構が要る）**＝`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`〔**3部品**＝①「このターンにセンタールリグがグロウしていない」を数える state が**存在しない**②チェックゾーンの A/B 面フリップ ③特定カードへのコスト無しグロウ。⚠**A面（`WXDi-P16-001A`）は CardData の CSV に無い**（B面だけ）＝実アプリで到達可能かの確認が先〕／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔ピース使用への**応答窓**が無い＝カットインは現状スペルにしか無い〕／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔`ON_TARGETED` が **`resumeSelectTarget` の適用後**に積まれるので「対象の無効化」の余地が無い＝収集地点を適用前へ動かすのが前提（続き511 で特定）〕。**定期的に前提を再判定する**（続き507〜514 の計17種のうち、**「機構が無い」が本当だったのは3種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・**funnel の後付け／見落とし**・自分で書いた据置理由の陳腐化・未消費 id だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
