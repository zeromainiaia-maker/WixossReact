...
**直近＝2026-09-18＝§5.7 `S-5c` 第3段 完了＝ルール処理4本と CPU の1手を画面から出した**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 移設（すべて逐語＝元コードとの機械 diff が空）＝`resolvePendingSigniBattleFor`（1,588行）／`doPhaseAdvance`（683）／`confirmEndDiscard`（234）／`handleCutinPass`（194）／**`cpuTurnAction`（1,362）** → `controller/` の5ファイル。**BattleScreen 12,012 → 7,639行**（今日の通算 16,930 → 7,639）。
- 🔑**`cpuTurnAction` の「画面から45名前」は分類すると簡単だった**＝材料（`PerformCtx`）／盤面から計算し直せる値（`drawCount`・`contBlocked`）／薄いラッパ／**実行関数の画面ラッパ14本＝「行動の口」`CpuTurnActions` として注入**（UI コールバック込みのまま＝人間と同じ経路）／残り3つ（全カード・作戦データ・パワー0の即時処理）。
- 移設で見つけた2件＝付与キーワードを見ている人の側で引いていた（CPU の追加アタックが起きない）／前段の `op`→`opS`。
- 🆕ハーネス2件＝`v85`（CPU の宣言は乱数なのに「1」決め打ち＝変更前から揺れていた）を宣言値で判定／通し対戦が**Lv超過の候補を押し続けてスキップに届かない**手詰まり（`O-534` 以後）を修正。
- 検証＝`gates` 全緑／実機 シグニアタック系20本・ルール処理系17本・**CPU 系49本**（落ちたものは全て変更前コードでも同じ／並び順の揺れ）／CPU 通し対戦 通常デッキ・機構デッキとも PASS。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-5d`**＝相手側の応答と人間側ターンの駆動（ヘッドレスで CPU 同士を回す）。🏁`S-5c` 第3段（ルール処理4本＋`cpuTurnAction`）は 2026-09-18 完了＝画面側に残る実行は UI の入口だけ |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1** |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 3**（`S-5`・`S-6`・`S-8`）／**リリース作業 1**（RELEASE.md）／🆕**実機シナリオの既存 FAIL 68**（全件実行で可視化した85本のうち A・B の17本は 2026-09-19 解消・残り C 未分類68＝[_drive_full_fail_2026-09-19.md](./_drive_full_fail_2026-09-19.md)） |
| ⚠**直近の不具合** | 🔴**CPU のアタックで倒れた人間のシグニの【バニッシュ時】がバトル前の盤面を読めていなかった**（`op`→`opS`）／CPU の付与キーワード（正面以外追加アタック等）が引けなかった（`dynamicKeywords.my`）＝どちらも「人間・CPU 共用の関数が画面の視点を掴んでいた」型 |
| ⚠**バグ報告** | 🏁**1件を処理**＝「クラッシュのログが2行」（人間側アタック解決の二重実行＝`ownCommitsArrived`） |
