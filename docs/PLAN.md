...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝**全ゲート緑（2026-08-07 続き377b＝(i)配線ギャップ 第7バッチ後）**＝golden **1440**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★**0**（265群）・census **1048**（回帰ゲート）・manual field loss 0・lint 0 errors/**248 warnings**・held **211枚／104群**・実機 driver 既定order **128件**。**被覆マトリクス miss 464**（`npm run census:wiring`）。
>
...
>
> 0. 🎯**次の一手＝(i) 配線ギャップの第8バッチ。★★セルから取る。**
>    - **① `levelRange × BANISH{SIGNI}`（miss 6・has 58）★★** ＝続き377 で BANISH{ALL} 側は直したので、残りは **count 指定側の BANISH ビルダー**。同型なので続き377 の「隣接名詞句 span」方式がそのまま使える見込み。
>    - **② `hasRiseIcon` 31（has 8）★／`hasCrossIcon` 22（has 2）★** ＝`WX16-026-BURST`「トラッシュから何でも回収できる」・`WX07-010-E1`「自分の全シグニに+1000」＝**実害が明確**。入口は `TRASH_CARD[filter]`／`SIGNI[filter]`／`SEARCH[filter]`／`POWER_MODIFY{SIGNI}` に散っている。
>    - **③ `isDisona` 24（has 31）** ＝続き376d で判定器は直したが、まだ載っていない入口が残る。
>    - ⚠**着手前に必ず読む6つの罠**＝(a) **素の `parseStoryFilter(文全体)` を対象フィルタに使わない**＝**対象名詞句に隣接**するときだけ拾う（続き376d）。(b) **曖昧なら付けない**（クラス2つ以上の span）。(c) **auto-commit があるので `git stash` で A/B が取れない**＝ベースラインコミットから `git show <sha>:<path>` で parser を取り出して**全カード fresh をダンプ→戻して再ダンプ→diff**（**live diff だけ見ると MANUAL/カード単位 PRESERVE の差分を見落とす**）。(d) **`matchesFilter` は `excludeSelf`/`upToCount` を見ない**＝**exec 側の配線を必ず確認する**（続き377 で `execBanish`、続き377b で `execTransferToDeck` が未配線と判明）。(e) 🆕**セルは入口であって終点ではない**＝★★セルを取ったら**そのビルダー全体を読む**（続き377b は《ガードアイコン》11効果のつもりが、同じビルダーの source 構造バグ19効果に当たった）。(f) 🆕**枚数・値だけ先に直さない**＝構造が誤ったままだと**誤りを増幅する**。
>    - **✅「他の」ゲートの棚卸しは続き377 で完了**（全13使用箇所を確認済み）。**`noGuard` セルも続き377b で消化**（残 miss 6 は has=0 の機構寄り＋CHOOSE 構造1件）。
>    - **(iv) 計器較正の残**＝条件節クラスタは**279件が269テンプレ＝ほぼ単発**で較正クラスタではないと実測済み。**較正で大きく減る余地はほぼ枯れた**＝以後は (i) 中心でよい。🆕ただし**「キー名は違うが engine では等価」型の誤検出**は残っている（続き377b の `hasGuard:false`≡`noGuard:true`）＝`censusWiring.ts` の `jsonRe` で個別に潰す。
...
>    - ⚠**見積もりの現在地**＝残 census 1048／実バグ率は較正で母集団が締まったぶん**約75%**＝**実バグ約790件**。**続き377b は1バッチで30効果**（続き377 は12＋engine 1／続き376 は5バッチ66効果）＝**バッチ収穫は逓減していない＝被覆マトリクスが効いている。**
