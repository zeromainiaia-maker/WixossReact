...
**▶ 次の一手**＝機構側なら 🆕**`O-135`**（**スペル解決経路が中央 diff を通らない**＝`ON_REFRESH` と
`ON_DECK_SHUFFLED` 以外のトリガー族が丸ごと収集されない。着手順序は §5.3 に記載）。
同じ「コストの入口」を続けるなら **`O-86`**（アーツ／スペルの実効コストを UI 層が原文 EffectText の
30本超 regex で毎回決めている＝**`census:enginetext` は `src/engine/` しか走査しないのでこの機械は1行も映らない**。
⚠**先に計器**）。⭐**`O-123` の隣にある未着手は `O-136`（`SP36-001` 1枚）と `O-118`（エクシード色の選択権）＝どちらも S。**
実機側は **`V-89`**（`lookReorderCanTrash` の FAIL）。
