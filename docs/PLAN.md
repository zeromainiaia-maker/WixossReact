...
| 🆕(xxxvii) | **アタック不可付与の据置4効果（続き205・Opus）**＝`GRANT_KEYWORD{アタックできない}` への寄せで fresh が正しくなったが、**アタックノード以外にも骨格差がある**ため機械採用から除外した4件＝**WX06-002-E1**（「代わりに」条件分岐で CONDITIONAL 構造ごと変化）・**WX18-003-E1**（curated が `STUB:BET_MECHANIC` 丸ごと／fresh は CHOOSE 2択に展開＝ベット機構との整合判断が要る）・**WDK05-T10-E1**（同上・curated にアタック不可ノードが存在しない）・**WDK11-007-E1**（2回付与の条件節構造差）。**いずれも fresh の方が原文に近い可能性が高いが、CHOOSE/BET 構造の採否は個別判断が要る**。 |
...
- **🆕 セッション（2026-07-19・続き204-205・Opus 4.8・**タスク7クローズ**＝`PREVENT_DAMAGE`＋`COST_SUBSTITUTE` 実装／**タスク1(a)**＝アタック不可付与の対象決め打ち是正28効果・census 1963→1948・golden 451→461）**
  ...
  - **✅タスク7をクローズ**＝残1型 `COST_SUBSTITUTE`（同じく完全no-op・2効果）も実装。**新機構は足さず既存のエナ色オーバーライド（`colorOverrideMap`）に載せた**＝エナ支払いはそもそもエナからのトラッシュなので「代わりにこのカードをエナからトラッシュしてよい」は「エナにあるこのカードが色Xとして支払える」と完全に等価（WX08-042＝緑なのに《白》・WX21-044＝黒なのに《緑》のオフカラー代替）。`collectEnergyTrashSubstituteInfo` に分岐追加のみで支払いUI・可否判定の既存経路にそのまま乗る。decompiler の生JSON漏れ（`コスト:{"banish_self":true}`）も是正し2効果とも原文一致に。golden 454→**456**。**これで §6.1 未実装action型の残は0**。
  - **⚠既存の配線穴を発見（新規バグではない）**＝`myEnergyTrashSubInfo` が**グロウ経路に渡っていない**ため、「あなたが《X》を支払う際」に含まれるはずのグロウ支払いでは代替が効かない。先行実装の `ENERGY_SUBSTITUTE_TRASH_SIGNI` 等と共通＝§3 Opusタスク12 に登録。
  - **✅タスク1(a)＝アタック不可付与の対象決め打ちを是正**（当初「granted CONTINUOUS の BLOCK_ACTION 収集機構が要る」と見立てていたが、**機構は不要**と判明）。parser が引用付与「…は「【常】：アタックできない。」を得る」で**対象を一切読まず** `BLOCK_ACTION{SIGNI/any/count:1}` 決め打ちにしていたため、**原文がルリグ対象でもルリグは素通りし無関係なシグニが止まる二重誤り**だった（`NEGATE_ATTACK` で先に直した Opusタスク10 パターンB と同型の穴）。**多数派表現（102効果）の `GRANT_KEYWORD{アタックできない}` に寄せるだけで engine 変更ゼロ**＝engine/BattleScreen のルリグアタック判定はこの経路しか見ないため、乗り換えた瞬間にルリグ対象が実際に効くようになる。対象種別（LRIG/CENTER_LRIG_OR_SIGNI/SIGNI）・体数・upTo・duration を原文から読むよう変更。
  - **採用は2段の機械ガードで28効果**（アタックノードだけを抜いた比較＋アタック部を中立化した骨格の完全一致）。**骨格が違う4効果（WX06-002/WX18-003/WDK05-T10/WDK11-007）は据置**＝個別判断へ。**⚠自分で作り込んだ罠を1つ潰した**＝upToCount を `includes('まで')` で見ると「ターン終了時**まで**」に必ず当たり全効果が「N体まで」に化ける（数詞隣接判定へ修正・golden に回帰テスト）。census 1963→**1951**・golden 456→**459**。さらに**タスク12(xxxviii) の閾値フィルタ脱落も同セッションで消化**＝「パワー20000以下の」等を対象節スコープで抽出（全文スキャンは別文節の数値を拾うので禁止）・census 1951→**1949**・golden→**460**。**さらに抽出を巻き上げて付与系4規則（【キーワード】/アタックできない/引用キーワード/シャドウ）で共有＋色フィルタも追加**＝全効果 diff で変化2件のみ（WXDi-P07-009-E1／WXDi-P09-053-E1）・census→**1948**・golden→**461**。
  - **次の一手**＝Opus はタスク2残＝WXK08-005（キーの使用タイミング動的付与＝§6.3級）・タスク12在庫（続き204b のグロウ経路プラミング穴／今回の据置4効果）。Sonnet は§3タスク6の未採用37効果の精密diff・採用判定。Sonnet は§3タスク6の未採用37効果の精密diff・採用判定（据置のまま実行可能）。
...
- **🆕 語彙センサス（過剰効果＋幻覚＝両方向の計器）**：`npm run census`（`scripts/vocabCensus.ts`）。**現ベースライン＝高シグナル欠落 1948【効果単位】**（2026-07-19 続き205＝アタック不可付与の対象決め打ち是正28効果で1963→1951・対象節の閾値フィルタで1951→1949・兄弟規則へ横展開で1949→1948、続き203＝終止形サーチ退化＋levelGtLastProcessed で1967→1965・前置CHOOSE復元＋ルリグデッキ戻し幻覚除去12枚採用で1965→1963。続き201＝`TRANSFER_TO_DECK` 幻覚除去で1968→1967、続き200＝名前filter 2効果の復元で1970→1968）。**この数字から増えたら回帰（exit 1）／減ったら `BASELINE_HIGH` とここを実数更新**。前提＝`docs/_effect_srctext.json` が最新であること。明細 `docs/_vocab_census.txt`、過去の計測履歴は [PLAN_DETAIL.md](./PLAN_DETAIL.md) §4／BUGFIXES 続き109以降。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 451・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1963。
>
