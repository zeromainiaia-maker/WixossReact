...
| 🆕(xxii) | **後置条件節の IS_MY_TURN 誤変換（当初127件）**＝続き143〜212 の12バッチ＋続き241＋続き283〜285＋**続き293**で系統消化。**続き293 で live 実害を全数確定**＝刻印42件を live JSON の `CONDITIONAL{IS_MY_TURN}` 実数で機械分類し、**真の実害16／live 既に正形の残骸25（fresh parser のみ失敗・触るな）／元から正しい1（WX20-053-E2）**に分離したうえで**9効果を消化**（支払いゲート4＝`PAID_ADDITIONAL_COST`／結果記録5＝`lastProcessedCards`）。**残10件**＝**[C]7件**（WX10-025 選択色追跡／WX15-067 使用時ウィルス支払い数／WXDi-P11-010A 実効リミット／WXDi-P13-003A 使用条件＋踏み倒しグロウ／WXK08-029 ビート総数／WX04-029 アタック幾何／WXDi-P03-044-E2 の前置条件＝origin付きターン累計エナ）＋**見送り3件**（WXK11-024-E2＝`TRANSFER_TO_DECK count:'ALL' optional` が選択UIを出さない既存穴が先／WXK06-031-E1＝レベル1〜4を各1枚の SEARCH 集合と先行対象の同時保存／WDK08-Y01-E1＝手札4枚公開の結果と先行対象の別保持）。⚠**`docs/_partial_report.txt` の刻印件数は実害件数ではない**（live 正形の残骸を含む）。[A]/[B]/[C] 分類は `docs/_partial_triage.txt` 2026-07-27段3節、詳細は BUGFIXES 続き293。 |
...
- **🆕 セッション（2026-07-28 続き293・Opus＋codex実装／Opus検証・差し戻し2件）＝§3 タスク12(xxii) 後置条件 IS_MY_TURN 誤変換の live 実害を9効果消化**（golden 808→815・census 1521→1519・held 267→263枚/112→109署名）
  - **投入前実測でスコープを作り直した**＝PLAN の「残23件」も `docs/_partial_report.txt` の「IS_MY_TURN化 39件」も**実害件数ではなかった**。刻印42件を live JSON の `CONDITIONAL{IS_MY_TURN}` 実数で機械分類し、**真の実害16件／live 既に正形の残骸25件（fresh parser だけが失敗）／元から正しい1件（WX20-053-E2）**に分離。16件を engine 実装まで読んで **12件を投入・4件を [C] 見送り**へ。
  - **採用9効果**＝(A)支払いゲート4件（WXDi-P06-036-E1／WXDi-P03-044-E2／WX24-P4-045-E1／WDK11-001-E1）を `OPTIONAL_COST → PAID_ADDITIONAL_COST` の**同一SEQUENCE直結**へ。(B)結果記録5件（WX22-043-E1 手札アクセ2枚→エナ／WXDi-P01-082-E1 回収成功時だけ手札捨て／WXK03-048-E1 実際に戻った遊具2体／PR-K049-E1 双方デッキ底ミルのレベル合計6＋正面限定／WXK09-100-E1 双方ミルに同レベルなし）を `lastProcessedCards` の実移動記録へ配線。**新 condition 型 0本**（`TRASHED_DISTINCT_LEVELS_GTE.allSigniDistinct` の1フィールドのみ）。
  - **指示書で先出しした2つの罠は両方とも回避された**＝①`execConditional` は偽時に ctx を素通しするので前段の記録が生き残る（→WXDi-P01-082 は level1 枝の内側へ入れ子化）②`PAID_ADDITIONAL_COST` は `execSequence` の look-ahead でしかペアにならず、`evalCondition` に到達すると無条件 false＝分岐が死ぬ（→WDK11-001 は `HAS_CARD_IN_FIELD` の then を SEQUENCE 化）。codex は同型の穴（`MILL` 空振り時に古い記録が残る）を**自力で発見して閉じた**。
  - **⚠検証で差し戻し2件**＝**(1) WX24-P4-045-E1 が真逆の効果だった**＝`ADD_TO_LIFE{owner:'opponent'}` で対象の相手シグニが**相手のライフクロス**へ入っていた（＝相手にライフを与える）。原文「それをライフクロスに加える」は加える先を無修飾＝使用者側（CSV 全文で、相手ライフに加える文型は必ず「対戦相手は/対戦相手の…」と明示される）。`execAddToLife` の `fromField` が場の持ち主とライフの持ち主を同一視していたのが engine 側の真因＝両者を分離して是正（`fromField` の利用者はデータ全体でこの1効果のみ＝波及なし）。**(2) その誤りを固定していた既存 golden も是正**（`otherState.life_cloth` を期待していた）。
  - **⚠採用9件のうち4件は本バッチの実装ではない**＝`PR-K049`／`WX24-P4-045`／`WX22-043`／`WXK03-048` の manual override は**ベースライン時点で既に `manualEffects.ts` にあり golden も緑**だったが、`public/data/effects_*.json` に反映されておらず、**App は JSON を直接 fetch する**（`App.tsx:91`・runtime に `mergeManualEffects` を通さない）ため**実機では壊れたままだった**。goldenTest は `mergeManualEffects` 経由なので**この乖離を golden は検出できない**。**教訓＝`manualEffects.ts` に書いただけでは実機に載らない。`npm run build:effects` で JSON を再生成してコミットするまでが1セット**（hollow fix の新パターン・CODEX_GUIDE §5 に追記済み）。
  - **見送り3件**＝WXK11-024-E2（`TRANSFER_TO_DECK count:'ALL' optional` が選択UIを出さない既存穴が先）／WXK06-031-E1（レベル1〜4を各1枚の SEARCH 集合と先行対象の同時保存）／WDK08-Y01-E1（手札4枚公開の結果と先行対象の別保持）。WXDi-P03-044-E2 の前置条件「このターンにデッキから3枚以上エナへ移動」は origin 付きターン累計が無く [C] のまま。
  - **検証**＝`npm run gates` 全緑・`npm run regen` 済・同型★0（5986枚）。**per-effect 機械 diff で 9効果ちょうど・兄弟効果の巻き添え0・変更禁止25件の変化0**を確認。codex の golden は構造 assert 中心（JSON regex＋`evalCondition` 真偽表）で経路の生死を検査しないため、**実挙動 E2E を4本追加**（WX24-P4-045 の支払い/skip 両方向とライフの行き先／WDK11-001 のネスト内 look-ahead 成立と場条件偽の不発／PR-K049 の成立·不成立と正面以外に載らないこと／WX22-043 の不成立方向）。⚠**実機未検証**。
  - **次の一手**＝**Opus：タスク12 の在庫（(lv)／(xxii) 残＝[C]7件＋見送り3件／(xxxix)残）または §6.3 残機構／タスク14 の残43**。**Sonnet：タスク1（§7 実機検証）＝レゾナ召喚UIの残り①ATTACK 窓 ②SPELL_CUTIN 窓 ③REQUIRES_NEW_FLOW ④支払いトリガー発火**。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 815・smoke 10725 全0（SKIP も 0）・fuzz 全0・同型★0・census 1519（＝`BASELINE_HIGH`・回帰ゲート）・held 263枚/109署名。
>
...
>
> 1. **自分のモデル側のタスク表（§3）から取る**。**Opus の主戦場＝§6.3 の機構実装（上記の機構単位・実IDは `docs/_p1_classification.txt`）＋タスク12 の生き残り在庫（(xliv)・(xxxix)・(xxii)残10件＝[C]7＋見送り3・(xlii)の残）＋タスク16 残**（(i)〜(xl) の大半は消化済み＝1行✅サマリ参照。(vii)(viii)(xxix)(xliii) は完全クローズ）。**Sonnet の主力は タスク1（§7 実機検証＝(xi) skip検証・(xxxvi) グロウ支払いUI・傀儡 resume・エナ復帰・5a エナ焼き filter・selectionConstraint UI・opponentSelects UI・比例カウントほか）**＝タスク8 clean群（3,574枚）は任意。タスク4（キュー）は枯渇したので取らない（理由は §3 Sonnet 表）。
