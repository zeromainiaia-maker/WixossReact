...
- 🏁**セッション（2026-08-28・続き694・Opus 5 単独）＝`O-121`。「このターンに…バニッシュしていた場合」の台帳が無く、条件ごと落ちて無条件発火だった。**
  ...
  - 🔑**既存の `signi_banished_this_turn` は流用できない**＝あれは**バニッシュされた側**に積む件数で、
    「誰が」も「何によって」も持たない。原文の主語は**バニッシュした側**。
    ⇒ `PlayerState.opp_signi_banished_this_turn: Array<{by,byEffect}>` を `signi_downed_this_turn` と同じ形で新設。
  - ✅**型システムの網羅性ガードが全部仕事をした**＝`Record<union,true>` 2本・`turnScopedState` の Record・
    `checkActiveCondition` 末尾の `never` 代入で**評価器/ターン境界/レジストリの登録漏れを typecheck が止めた**。
    golden の型数トリップワイヤ4本も落ちた＝**足し忘れが素通りする経路が無い。**
  - ⚠**記録地点は2つ**（バトル解決／中央 diff）。**`ON_SIGNI_BANISH_OPPONENT` はバトル経路にしか配線されていない**ので、
    バトル側の記録を落とすと条件が永久に成立しない＝恒久 no-op になる。
  - ⚠**parser＝この timing だけ先頭アンカーの hoist が効かない**（トリガー句を除去しない規約のため）。
    1文型だけを効果組み立ての直前で `CONDITIONAL` へ包んだ。**汎用 hoist をこの timing へ広げない。**
  - 🔴**実機で踏んだ罠**＝`battleCardMap` は**対戦に登場するカードしか持たない**ので、デッキ外のカード番号で
    台帳を仕込むと `matchesFilter(undefined,…)` が false になり**数えられない**（fail-closed）。
    切り分けは「**条件を外して本体だけにするプローブ**」で1回で確定できた。

**▶ 次の一手**＝**`O-122`（【出現条件】で支払ったカードを engine が記録していない）**。
実測済み＝`WX07-009-E1`（Sheet1）「このレゾナの**出現条件で同じ名前のシグニ２体をトラッシュに置いていた場合**」＝
live は条件が無く**出ただけで常に2枚エナチャージ**。母集団は**1効果**（原文「出現条件で」は全 CSV で2枚だが `WD11-007` は参照していない）。
当て＝召喚時に支払ったカード番号を `last_appearance_cost_cards` へ記録（`resonaSummon.ts` が唯一の funnel）→
`Condition` を1本＋**両評価器＋golden の3点セット**。
⚠**`O-121` で確認したとおり、型を足すと `Record<union,true>` 2本・`turnScopedState` の Record・
`checkActiveCondition` 末尾の `never` 代入・golden の型数トリップワイヤ4本が全部落ちる**＝そこを埋めるまでが1セット。
**Sheet1 の残り機構待ちは `O-108` / `O-109` / `O-110` / `O-112` / `O-113` / `O-117` / `O-122`〜`O-129`＝1項目＝1巡**
