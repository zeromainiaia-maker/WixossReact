...
**▶ 次の一手**＝**§5.2 Sheet2 バッチ2**（残 **107件**／シート別では Sheet2 が最多のまま）。
🔑**取り方はバッチ1 の学びに従う**＝**1カード1 finding の HIGH** から、`axis` が `owner/範囲` `count/upTo` `filter.*` のものを10件。
**findings が多いカードから取らない**（複雑で機構待ちになりやすい）。
**1件＝原文を読み直して `manualEffects.ts` に手書き → `build:effects` → `syncManualLive.ts` → 逆翻訳を目視 → `golden -- --only` ＋ `smoke`。**
**10件たまったら `gates` 1回・commit 1回・BUGFIXES 10行**（§2.1 ④⑥）。実機は `src/screens/` を触った回だけ（§2.2）。
...
**遅いレーン（機構が要る）の在庫**＝`O-135`（スペル解決経路が中央 diff を通らない。`O-138` の前提でもある）／
`O-86`（アーツ／スペルの実効コストを UI 層が原文 regex で決めている＝**先に計器**）／
`O-137` `O-138` `O-139`（本バッチで分離）／`O-136`／`O-118`。実機側は **`V-89`**。
