...
| 🏁`S-33` | ~~**エナゾーンのアクセ【起】を CPU が使わない**~~（バグ報告 `a4d1cc1b`） | 中 | **2026-09-22 クローズ**＝提示判定（`offFieldActivateGate`）に `isEnergyAcceActivated`＋付け先の有無を足し、人間の専用ボタンも同じ判定へ（同名2枚目が付けられなかった人間側のバグも解消）。CPU は自分自身をエナとして払わない。golden「§5.7 S-33」／実機 `s33EnergyAcceSameNameTwice` PASS |
