...
- 🏁**セッション（2026-08-25・続き647・Opus 5）＝§5.2 段2 第41バッチ＝`GRANT_KEYWORD` の**付与期間**と**対象の持ち主**が落ちる**（**実装は Codex へ委譲**・母集団実測／指示書／検証／実機は Claude）。gates 全緑（**golden 2683→2709**／census 601 据置／smoke 10693 全0・SKIP 0／fuzz 全0／lint 0 errors・warnings 260（増分0）／同型★0）。**実機2本が2回連続 PASS＝1巡が §2.2 の⑤まで完了。台帳 713→708。差し戻し0・是正3。**
  - ✅**19効果を修正**＝①**期間**（10効果）＝原文「このターン」なのに `duration:'PERMANENT'`＝**ゲーム終了まで【ダブルクラッシュ】等が残る過剰実行** ②**持ち主**（12効果）＝原文「あなたの」なのに `target.owner:'any'`＝**対戦相手のシグニにも付けられる**。実測で `PERMANENT 242→232`／`UNTIL_END_OF_TURN 409→419`。
  - 🔑**真因は `O-56` と同型＝parser が持っている情報を捨てている**＝`parseSentencePart1` が**句点で分割された断片**「そうした場合、〜それは【K】を得る」だけを見るため、先行文の「あなたのシグニ1体を対象とし」と期間句を参照できず `any`／`PERMANENT` へフォールバックしていた。
  - 🔴**母集団の数え方で4回踏み外した**（すべて指示書に明記して Codex へ渡した）＝①CSV のカード行 grep＝38（**同じカードの別効果**に当たる）②`CardEffect.duration`＝165（🔴**`GrantKeywordAction` は自前の `duration` を必須で持つ**＝効果レベルは無関係）③「ターン終了時まで」を素朴に拾う＝46（「**次の対戦相手の**ターン終了時まで」を巻き込む）④入れ子まで再帰＝+7（**引用能力の内側は外側の期間に従う**）。⇒ 実測 **20効果**。
  - ⭐**Codex が Claude の母集団を1件訂正**＝`WX25-P3-023-E1` は「②**対戦相手は**【みこみこ親衛隊】1つを得る」で、Claude の正規表現が**トリガー句の「あなたのシグニ1体がアタックしたとき」**に当たっていた誤検出。**据え置いたうえで非採用を golden で固定**（§5-4）。⇒ **19効果**。
  - 🔴**実機で「判別力ゼロの assert」を削除した（最重要の是正）**＝元シナリオは「ターン境界を跨ぐと失効する」を見ていたが、**現 engine は `keyword_grants` を duration に関係なくターン末に一括クリアする**＝**修正前の `PERMANENT` でも同じく消える＝常に緑になる偽の対照**。⚠**Codex 自身がコメントで気付いていながら残していた**（代替の `fetch(effects.json)` は挙動ではなくデータを見ているだけ）。⇒ 実機は「付与が実 UI で実際に乗る」に絞り、**期間の固定は golden へ委ねた**。
  - 🔴**道中で既存のエンジン問題を発見**＝任意コストの直後の「対象1体」で**対象ピッカーが出ず効果元へ自動付与される**（`WX25-P3-059-E1` で実測）。⚠**本バッチ起因ではない**（修正前の `any` でも同じ札が選ばれる）＝**`O-61`** へ登録。**同じ形は232効果あるが未分類**。
  - ⚠**Codex は3バッチ連続でシナリオを未実行のまま引き渡した**（環境に Chromium 無し）。**走らせたら2本とも落ち**、内訳は判別力ゼロの assert 1件＋ドライバの腐り1件＋観測不能な前提1件。

**▶ 次の一手**＝**§5.2 の段2 消化を続ける**（残 OPEN **708**）。⚠**着手前に必ず `node scripts/archive/semanticAuditLedger.mjs` で数え直す。**
🔑**第2層（軸 × live の action 型）で割る手順が確立した**（続き647 で実証）＝①`--list` で OPEN 全件を出す ②live JSON の action 型と突き合わせる ③5件以上のサブ群を出す ④**生データへ戻して母集団を数え直す**。**次の候補**＝`(軸なし) × CHOOSE`（21件/16効果）、`(軸なし) × CONDITIONAL`（13件/10効果）、`(軸なし) × POWER_MODIFY`（11件/11効果）、`(軸なし) × SEARCH`（10件/10効果）、`filter.story × POWER_MODIFY`（7件/7効果）。
🔴**母集団の数え方は「4つの引き算」を毎回やる**（続き647 で4回踏み外した）＝①**カード行ではなく効果単位**（`docs/_effect_srctext.json`）②**action 自前のフィールドを見る**（`CardEffect` 側と別物のことがある）③**既に正しい群を引く**（長期 duration・別主語）④**引用能力の内側を除く**（外側の器に従う）。
🆕**`O-61`（任意コスト直後の対象ピッカーが出ない）は「まず232件の機械分類」から**＝いきなり直さない。`O-60` と同じ扱い。
🆕**Codex 委譲の実機シナリオは3バッチ連続で全部落ちた**（`O-49`／`O-56`／第41）。**シナリオは Codex に書かせても「未実行の草案」として扱う**＝検証側で必ず実行し、①判別力ゼロの assert が無いか ②ドライバが実際に UI を進められるか ③観測しようとしている状態が engine で本当に発生するか の3点を毎回見る。⚠**特に①は「修正前でも緑」になるので、放置すると偽の安心を生む。**
