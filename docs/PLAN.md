...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕が指示と検証＋codex-work が実装）＝「コストの合計が N 以下／以上」の束縛が丸ごと落ちていた14効果を是正（過剰実行）**（golden **1085→1090**〔codex 4本＋Claude 是正1本〕・census **1391→1386**〔`BASELINE_HIGH=1386`〕・smoke **10679** 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・held **286枚/106署名 据置**・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 14 / added 0 / removed 0** で申告と完全一致・`build:effects` 冪等）
  - **投入前実測で母集団を作り直した**＝PLAN の候補は4件だったが、原文に「コストの合計」を含む効果は **50件**あった。機械分類して**実際に実行されて害が出ている14効果**に絞り、**偽陽性11件**（`costMin`/`costMax`/`costThreshold`/`COUNTER_SPELL.maxCost` が既に正しい6+4件＋`WXK05-048-E1`＝「使用コストの合計は０以下にならない」は**自分のコスト下限**で filter ではない）と、**STUB/UNKNOWN で実行本体が無い14件**（filter を足しても意味がない）をスコープ外に切った。
  - **🔴また「機構はあるのに parser が繋いでいない」だった**（前バッチと同型の失敗パターン）＝`TargetFilter.costMin/costMax`（`matchesFilter` が `effectEngine.ts` と `execUtils.ts` の**両方**で評価）・`PlayFreeAction.costThreshold`（`execPlayFree` が候補列挙と実行の両方で判定）・`excludeCardName` はいずれも既存。**新語彙ゼロで静的11効果が閉じた**。
  - **実害の中身**＝`WX24-P4-072-E1` は手札を**0枚**捨てても（`discardUpTo:3`）**どんな高コストスペルでも無料**／`WX21-038-E1`・`PR-466-E3`・`WXK01-021-E1-G` はコスト上限を無視して任意スペルを無料使用／`WX06-030-E1`・`WX20-063-E2` は「コストの合計が０のスペル」の指定が消えて**任意のスペルを回収・サーチ**／`WX11-043-E1`・`WX19-004-E1`③ は「N以上」の下限消滅／`WXK06-082-E1` は**自分と同名**《幻竜　ドラマジ》を除外できていなかった／`WXK03-070-E1` は本体前半「エナゾーンに置く」が丸ごと欠落（`SEQUENCE[SEND_TO_ENERGY, BOUNCE]` へ復元・2体は別対象）。
  - **動的閾値2件は支払い枚数の記録から配線**＝`costThresholdFromPaidCount{source:'discard'|'energyTrash', plus}` を既存 `costThreshold` の隣に足し、**engine（`execTrash{asCost}`）と実機（`BattleScreen.executeSigniActivated`／`executeLrigGranted`＋純関数 `activatedDiscardPaidCount`/`activatedEnergyTrashPaidCount`）の両支払い地点**へ配線（§5-20 の片肺配線を回避）。`WX24-P1-044-E2` は「1枚**以上**」なので `energyTrash.atLeast` を新設し `SigniActivatedModal` の複数選択も解放（**波及は当該1効果のみ**＝`atLeast` を持つ効果を全数走査して確認）。
  - **⚠Claude 検証で是正1件＝動的閾値の膨張**：`last_cost_energy_trash_count` は `execTrash{asCost}` が **`+=` で累算**するのに、任意コスト解決の冒頭でクリアされる兄弟（`last_cost_energy_trash_level_sum`）と**寿命が揃っていなかった**＝前の効果で払った枚数が閾値に足し込まれ**上限が膨らむ**（コスト0で高コストスペルが撃てる方向）。クリアを揃え、**クリアを外す変異で狙った1本だけ FAIL** することを確認して golden で固定。
  - **codex の申告品質**＝14/14 採用・数値申告（ゲート・per-effect diff・held・エンコーディング）は全項目が独立実測と一致。**BUGFIXES を先頭 `##` で追記**（前回の指摘が1回で定着）。見送りの申告も的確（`SPDi43` の「《リコレクトアイコン》を持たない」は `hasIcon` の union に無いため未表現・`WXK03-070` の3枚指定エナコストは `costUnparsed` 維持＝**コスト踏み倒しは残る**と明記）。
  - **次の一手**＝**Opus：①エクシードルリグ6件の本体不一致**（`WX24-P4-011-E2` 次回アタック時アップ→即時UP／`WX24-P4-014-E2` 差分ドロー→固定1枚／`WX24-P4-015-E2` 任意ライフクラッシュ＋動的対象数→強制＋固定1体／`WX24-P4-017-E2` スペル＋青シグニ各1枚→青シグニのみ／`WX24-P4-018-E2` 相手の手札3枚支払い回避→無条件バニッシュ／`WX25-P3-028-E2` リフレッシュ不可の誤分類＋3回の選択脱落。詳細 BUGFIXES 2026-07-29 §「live 原文照合」節）**②「コストの合計」族の STUB/UNKNOWN 14件**（本体が無い＝実装すれば今回の filter がそのまま効く）**③🆕`WX11-043-E1` の `crossOnly` 欠落**（原文「《クロスアイコン》…の右【出】」なのにクロス外でも【出】発火。今回の原文照合で発見・在庫外）**／(l) A群残4・(xxix) 残4は機構コスト待ち／(lv) 残2経路／ゲート脱落 [B]20件／タスク16 残／§6.3 残機構**。**Sonnet：§7 実機検証**＝🆕**`SigniActivatedModal` の「エナ1枚以上」複数選択と、捨て枚数に応じた `PLAY_FREE` 候補の絞り込み**（`WX24-P4-072`＝0枚捨てならコスト0スペルだけが出るか）＋`DECLARE_PARITY_OPPONENT` の相手応答UI＋キー付与【起】のエクシード支払いUI＋分割UI＋宣言UI＋トラップ設置UI＋(xxix) の各支払いUI＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 「コストの合計」束縛14効果 最新値（本行を直近の正とする）**：census **1391→1386**（`BASELINE_HIGH=1386`）、golden **1085→1090**、smoke **10679/10679** 全0・SKIP0、fuzz 全0、lint 0 errors/**228 warnings**、同型★0（5986枚）、held **286枚/106署名 据置**、manual field loss 0。live per-effect 差分 **changed 14 / added 0 / removed 0**、`build:effects` 冪等。新語彙は `costThresholdFromPaidCount`（動的上限）と `energyTrash.atLeast` の2本のみ＝**いずれも当該2効果だけに付与**（全数走査で確認）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1090**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1386**（回帰ゲート）・held **286枚/106署名**（2026-07-30 「コストの合計」束縛 実測）。
>
