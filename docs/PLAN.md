...
**▶ 次の一手**＝**B15＝「このターンの出来事の台帳＋トリガーの由来限定」**（Claude が直接実装。codex は使わない）。
実測済みの母集団＝**【ランサー】由来のライフクラッシュ3効果**（`WX07-042-E1` / `WX19-028-E1` / `WX19-071-E1` は
live に条件がゼロ＝**通常アタックのクラッシュでも発火する過剰実行**。付与形の `WX24-P2-055` が4件目）＋
**「このターンに…バニッシュしていた場合」2効果**（`WX11-031-E1` / `WXK02-034-E1` ＝どちらも条件ごと落ちて無条件発火）＋
**「出現条件で」1効果**（`WX07-009-E1`）。当ての目星は §5.3 の `O-120`〜`O-122` に書いた。
...
| **O-117** | 🆕**「使用コストで《白》《赤》《青》《緑》《黒》すべてが支払われている場合」＝支払った色を条件にする** | M | **2026-08-27 Sheet1 B13 で分離**＝`WX05-016`（エンドホール）。■印刷コストは **《無》×５** なのに、parser が**効果文の色リストをコストとして採用**して `白1赤1青1緑1黒1` に化けている。■⚠**今の形は安全側**＝「5色払ったときだけターン終了」を「5色ちょうどでしか使えない」に狭めた**過小実行**で、過剰実行は作っていない。**印刷コストへ戻すだけだと `FORCE_END_TURN` が無条件になり過剰実行へ裏返る**ので、条件の受け皿ができるまで**据置**（§5-2″ の同型）。■**取り方**＝支払いで消費したエナの色を `last_paid_colors` として記録し、`PAID_COLORS_INCLUDE_ALL` 条件で読む。⚠記録地点は `planEnergyPayment` の1本に寄せる（アーツ／スペル／【起】で別実装にしない）。 |
...
| **O-125** | 🆕🔴**`BLOCK_ACTION{DRAW_OUTSIDE_DRAW_PHASE}` は engine のどこからも読まれていない死にフラグ** | S〜M | **2026-08-27 続き692 で発見**＝`WX05-022-E1`（Sheet1）【常】「対戦相手は自分のターンの間、グロウフェイズとドローフェイズ以外でカードを引いたりカードを手札に加えることができない」。■`actionId:'DRAW_OUTSIDE_DRAW_PHASE'` の出現箇所は `parseSentencePart1.ts:973`（生成側）**の1つだけ**＝消費地点ゼロ＝**何も禁止していない**（findings は「ドローしか禁止していない」と言うが**実態はもっと悪い**）。■🔑**兄弟枝にはある**＝`DRAW_OR_ADD_TO_HAND_BY_EFFECT` は `effectExecutor.ts:176` / `:2786` で消費されている（§5-8′ の型）。■併せて直す＝原文にある**「自分のターンの間」**と**グロウフェイズの例外**も live に無いし、`until:'END_OF_TURN'` は【常】に対して誤り。母集団＝`WX05-022` / `WXK10-010` の2枚。 |
