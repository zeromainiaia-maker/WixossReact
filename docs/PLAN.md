...
- **🆕 セッション（2026-08-15・続き495・Opus 5）＝§6.4 **O-30「〈コスト〉を支払わないかぎり、X」の回避ゲートが落ちていたクラスを解体＝11効果の挙動是正**（**過剰実行 8／払う側の取り違え 2／文言・逆翻訳のみ 1**）。ゲート全緑（**golden 2042**＝+2・**census 831 据置**・smoke 10688 / SKIP 0・lint 0/260）。live changed **11効果/10カード**。
  - **🔴① 在庫は「1効果」ではなく母集団44カードだった**＝PLAN の記述（`WX24-P4-001-E1` の1件）は着手前の実測で覆った。`/を支払わないかぎり[、,]/`（**読点つき**＝後続に帰結が来る型）は **46箇所/44カード**で、主語が「**対戦相手が**払う」形（32箇所＝`matchOpponentUnlessGate` で実装済み）と「**自分＝能力の持ち主が**払う」形に割れる。後者には**規則が1つも無く**、前置きが丸ごと落ちて下流の汎用規則が帰結（バニッシュ）だけを拾う＝**無条件バニッシュ**の過剰実行が8効果あった。⚠読点なし（「〜を支払わないかぎり**アタックできない**」＝制限型・30箇所）は §6.4 O-28 の領分で別枠。
  - **🔑② 新機構は要らなかった**＝正準形 `SEQUENCE[STUB{OPTIONAL_COST, unlessPay}, CONDITIONAL{PAID_ADDITIONAL_COST, then:[], else:<帰結>}]` は、隣接 `STUB+CONDITIONAL` の既存 Pattern が skip 枝に `conditional.else` を使うのでそのまま動く（`WXDi-P04-040-E1` が既存の手本）。⚠**本体が解けないときは据置**（前置きだけ食って帰結を落とすと今度は過少実行になる）。
  - **🔴③ 払うのは「付与された側」＝`ownerState`**（2効果の取り違えを是正）＝`WX24-P2-044-E1/E2`（MANUAL）は `OPPONENT_PAY_OPTIONAL`＝**このカードを使った側**が払う逆向きだった。`granted_effects` は**付与先の持ち主の state** に積まれ（`execGrantEffect`）、`collectAttackerSelfTriggers` が `playerId: attackerId`（＝そのシグニの持ち主）で積む（`BattleScreen.tsx:8093`）ので、`ownerState` は付与された側になる。🔑**同じ action の `BANISH{owner:'self', thisCardOnly}` が成立している時点でこれは確定する**（使った側が owner なら対象が自分の場に無く no-op のはず）。⚠**golden がこの誤りを写していた**（`PLAN 6.3 batch 8 B-1` の「opponent pays exactly 3 energy」）＝正方向へ書き換え。⚠**この test を `withSavedCursor` で包むと直後の `(xlvi) wave17` がフィクスチャ割り当てカーソルのズレで落ちる**（既知の結合・コメント済み）。
  - **🔑④ 「支払わないかぎり」形を汎用の任意コストの文言に潰さない**＝`StubAction.unlessPay` を新設（機構は不変＝pay→`then`／skip→`else`。**文言と逆翻訳だけ**を変える）。選択肢は「発動する／スキップ」→**「支払う／支払わない」**（既定のままだと「払うと自分のシグニが助かる」場面で**払わない方が得に見える**表示になり実機で判断できない）、逆翻訳は**原文どおり「《無》…を支払わないかぎり、X」**へ。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残8（`npm run census:stubs` の A群が常時の在庫表・機構ごとに独立着手可）＝`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）／`OPP_CHOSEN_SIGNI_ATTACK_LOCK`（`WXDi-P08-030`）／`NEXT_OPP_TURN_END_BODY`（2効果＝遅延予約機構）。→ 🆕**O-31**（「対戦相手が払う」側の残り＝続き495 の実測で**帰結が「そのシグニ」照応**の形などが据置のまま。母集団は `/対戦相手(が|は)[^。]{0,20}を支払わないかぎり[、,]/` で数え直す）→ **O-28 の残23綴り** → **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → **O-26** → **O-27** → **O-29**。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き495 の3経路**＝(a)`WX25-P2-038-E1`（付与されたシグニでアタック→**「支払う（コスト:《無》×4）／支払わない」**が出て、**払えば残る／払わなければバニッシュ**＝負方向＋対照の対）(b)**払うのは付与された側**（＝アタックした側のエナが減り、カードを使った側は減らない）(c)エナ不足時に支払い枝が選べないこと。ほか続き488〜494 の未検証分。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き495（§6.4 O-30「支払わないかぎり」の回避ゲート）後 最新値（本行が直近の正）**：census **831 据置**（較正なし）、**golden 2042**（+2＝挙動1〔払う→X が起きず ownerState から引かれる／払わない→X／エナ不足で pay 枝が出ない／文言〕・live データ不変条件1。ほかに `PLAN 6.3 batch 8 B-1` を正方向へ書き換え、`(ci)` の母集団カウントを `OPPONENT_PAY_OPTIONAL` 75→**73**・costColors 付き 40→**38** へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 45群**（8枚採用＋MANUAL 3効果は外科パッチ）、lint **0 errors / 260 warnings**、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0・明示 defer 23種/26件**（据置）。🆕**live JSON changed 11効果/10カード**（`WXDi-P02-041`／`WXDi-P04-040`／`WXDi-P05-017`／`WXDi-P08-058`／`WX24-P2-004`／`WX24-P2-044`(2効果)／`WX24-P4-001`／`WX24-P4-029`／`WX25-P2-004`／`WX25-P2-038`。CSV 非改変）。🆕**挙動是正 11**（過剰実行 8＝無条件バニッシュ／払う側の取り違え 2／文言・逆翻訳のみ 1）。🆕**新語彙＝`StubAction.unlessPay`**（機構は不変で**文言と逆翻訳だけ**を「支払わないかぎり」形へ）。⚠**3経路とも実機未検証**（§7 送り）。⚠**母集団の残り**＝「対戦相手が払う」側32箇所は実装済みだが**帰結が「そのシグニ」照応**の形などが据置（→ O-31）。
...
| **O-31** | 🆕**「対戦相手が〈コスト〉を支払わないかぎり、X」の残り** | 未実測 | 2026-08-15 続き495 の母集団実測（`/を支払わないかぎり[、,]/`＝46箇所/44カード）のうち、**「対戦相手が払う」側32箇所**は `matchOpponentUnlessGate`／`matchOpponentWaUnlessGate` で実装済みだが、**据置と明記されている形が残っている**＝(a)帰結が「**そのシグニ**」照応（トリガー元参照＝`WXDi-P08-007`）＝単文パースで owner が self へ反転する退化を避けるため据置 (b)`matchOpponentWaUnlessGate` が `/【ガード】ができない|アタックできない|アタックを無効|アップしない|新たに配置できない/` を**明示除外**している帰結（`WXDi-P06-002`／`WXDi-P13-075`＝「対戦相手のセンタールリグはアップしない」など）。⚠**着手前に「支払い語彙を持つか」を効果単位で数え直すこと**＝続き495 のカード単位スキャンは、同じカードの別効果が支払い語彙を持つだけで PAY と出る**偽陰性**があった（`WX24-P4-001` が実際にそれ）。⚠**自分が払う**側（本体）は続き495 で消化済み＝`OPTIONAL_COST{unlessPay}` の正準形。|
