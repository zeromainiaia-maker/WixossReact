...
- **🆕 セッション（2026-08-16・続き500・Opus 5）＝§6.4 **O-34 クローズ**＝**明示 defer 5件を全解体（残0）**。ゲート全緑（**golden 2072**＝+11・**census 826→825**＝`BASELINE_HIGH` 実数更新・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0/260・held **104枚/44群**＝群構成は据置）。live changed **11効果 / 11カード**。`census:stubs` A群 **20種/22件 → 15種/17件**（**無言 no-op は 0 のまま**）。🆕`census:goldentypes` **未カバー2型 → 0**。新機構＝`STRIP_ATTACHED_AND_UNDER`／`USE_SEARCHED_SPELL_OR_TRASH`／`DECK_SIGNI_LEVEL_OVERRIDE_ALL`＋読み手 `deckSigniOverrideLevel`／`PER_OWN_LRIG_COLOR_SCALE`／`DECLARED_ICON_HAND_DISCARD_BANISH`（4段対話）／条件 `LRIG_ANY_TEAM_COUNT`／フィルタ `noDeployConditionIcon`／`stripTrapIconClause`。
  - **🔴① 明示 defer の在庫は母集団の一部しか映さない**（続き489〜493 の教訓が再発）＝5件すべてで原文 regex を引き直すと**同族の別カードがもっと壊れていた**。実測5効果＝`WX18-029-E1`（**相手シグニ本体をトラッシュ**する過剰実行）／`WXDi-P07-041-E2`（対象宣言がゴミ `GRANT_KEYWORD{'　　'}` に化けて丸ごと不発）／`WXDi-P08-064-E1`（「青のルリグ1体につき」が**青いシグニを凍結**＝色の掛かり先の取り違え）／`WX25-P3-050-E1` の【使用条件】節（ゴミ `GRANT_KEYWORD{使用条件}` に化けて**白節ごと消失**）／【トラップアイコン】節の**スペル本体 tail-splice**（5枚中4枚＝唱えた瞬間にトラップ本体まで走る）。**うち3件は `DEFERRED_*` ですらないので `census:stubs` に映らない。**
  - **🔑② 同じ除去規則を型ごとに書き分けない**＝【トラップアイコン】節の除去はシグニ側にしか無く、スペル枝は素通りだった（`stripTrapIconClause` へ集約）。⚠**スペル枝は `card` をそのまま `parseSpellEffect` に渡す経路がある**＝除去済み文字列を渡し忘れると4枚とも漏れる（実装中に踏んだ）。
  - **🔑③ setter だけある死フィールドは「読み手を1本足す」で生き返る**＝`deck_signi_level_override` は setter 3箇所に対し読み手0だった。デッキ公開（停止条件＋当たり札判定の**両方**）とデッキ探索（`searchCardMap` 差し替え）へ配線。⚠**片方だけ**に通すと「止まるのに当たらない／当たるのに止まらない」でズレる。
  - **🔑④ 段を跨ぐ参照は `carried*` に焼き込む**＝`lastProcessedCards` は CHOOSE / SELECT_TARGET の resume を跨いで生存しない（`fixedCardNums` と同じ理由）。4段対話（対象→手札→宣言→判定）は各段で次段の stub に束縛して運ぶ。
  - **🔴⑤ golden が壊れた形を「据置」として固定していた**（続き495／499 と同じクラス）＝「チーム名不問条件は誤昇格しない」の assert が、**先頭節が本文へ流れ込んで白節ごと消える**壊れた状態を正としていた。専用条件 `LRIG_ANY_TEAM_COUNT` を実装して正方向へ書き換えた（⚠`LRIG_TEAM_COUNT{team:''}` での代用は `includes('')` が常に真＝**無条件で通る**）。
  - **▶ 次の一手【Opus 側】**＝`O-32`（`REPEAT_N_TIMES` の二重実行3効果。⚠**本文の parse を先に直す**）→ `O-33` の**残り**（`SigniAttackBan.zones`＝ゾーン限定）→ `O-28` の残23綴り → `O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-10`（明示 defer 15 id/17件の前提再判定）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き500 の主な経路**＝(a)`WX19-064-E1`③／`WX18-029-E1`（相手シグニを選ぶと**付随物と下カードだけ**が落ちて**シグニは場に残る**）(b)`WX20-077-E2`（サーチ後に「使う／トラッシュに置く」の二択が出て、**どちらでも手札に残らない**）(c)`WXDi-P12-055-E1`（相手側に**6択の宣言モーダル**が出る＝PvP の相手表示／CPU 自動応答。外れたときだけ対象がバニッシュ＝**負方向＋対照の対**）(d)`WX25-P3-050-E1`（【チーム】3体が揃わないと使えない／色ごとに回数が変わる）(e)`WXK07-034-E1`（①②を両方選ぶと②が Lv4 として2枚めくる）。⚠**すべて実機未検証**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-16 続き500（§6.4 **O-34 クローズ**＝明示 defer 5件を全解体）後 最新値（本行が直近の正）**：census **825**（826→825。`BASELINE_HIGH` を実数更新済み＝**実装で live に語彙が載ったぶん**）、**golden 2072**（+11＝付随物剥がしの本命/対照/自己剥がし3・【トラップアイコン】節の live 不変条件1・デッキ内レベル上書きの対1・自ルリグ色スケールの0/1/2体の対1・`WX25-P3-050-E1` の live 不変条件1・`LRIG_ANY_TEAM_COUNT` の対1・サーチ札の使う/捨てるの対1・宣言アイコン判定の対1・未カバー2型の予約1。ほかに「チーム名不問条件は誤昇格しない」assert を正方向へ、Condition 型数 116→117 を更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **104枚 / 44群**（群構成は据置＝新規の採用待ちバックログを増やしていない）、lint **0 errors / 260 warnings**、`census:stubs` A群＝**15種/17件**（20種/22件から −5種/−5件。**無言 no-op は 0 のまま**）、🆕`census:goldentypes` **未カバー 0型**（続き492 から残っていた `RESERVE_DRAW_PHASE_REPLACEMENT`／`SET_LRIG_BASE_LIMIT` を消化）。🆕**live JSON changed 11効果 / 11カード**（CSV 非改変）。🆕**新機構＝`STRIP_ATTACHED_AND_UNDER`（＋`SELECT_TARGET_ONLY` の `owner:'any'` 経路）／`USE_SEARCHED_SPELL_OR_TRASH`（＋`INTERNAL_USE_SEARCHED_SPELL`／`INTERNAL_TRASH_SEARCHED_CARD`）／`DECK_SIGNI_LEVEL_OVERRIDE_ALL`＋読み手 `deckSigniOverrideLevel`（デッキ公開・デッキ探索の2経路）／`PER_OWN_LRIG_COLOR_SCALE`／`DECLARED_ICON_HAND_DISCARD_BANISH`（4段対話＋`StubAction.carriedCardNum`/`carriedTargetNum`）／条件 `LRIG_ANY_TEAM_COUNT`／フィルタ `noDeployConditionIcon`／`stripTrapIconClause`（スペル枝の【トラップアイコン】除去）**。⚠**全経路 実機未検証**（§7 送り）。
