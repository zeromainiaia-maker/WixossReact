...
| 🆕(xlvii) | **「対戦相手のルリグがアタックしたとき」に**自分の**付与AUTO を発火させる収集経路が engine に無い（続き218i・Opus）**＝`ON_ATTACK_LRIG` の収集は BattleScreen 7832 が **`my.lrig_granted_auto_effects` しか見ない**（＝自分のルリグがアタックしたときの自分の付与のみ）。`ON_ATTACK_SIGNI` 側は `collectFieldTriggers`(triggerCollect 2429) が `opState.lrig_granted_auto_effects` を any_opp/any で拾う経路を持つのに、ルリグアタックには対応物が無い。**実害**＝(a)`WX15-002-E2`「対戦相手のセンタールリグ１体がアタックしたとき」＝ルリグ単独主語のため据置＝no-op 継続（拾うと ON_ATTACK_SIGNI に載って相手シグニのアタックで誤発火するため近似不可）(b)続き218i で是正した「シグニかルリグ」3効果の**ルリグ側の半分**（`markSilentFallback`＋PARTIAL 刻印済み・シグニ側は正しく発火）。**必要な機構**＝ルリグアタック時に**防御側**の `lrig_granted_auto_effects` を any_opp/any scope で収集する経路（ON_ATTACK_SIGNI 側 2429 と同型）を BattleScreen のルリグアタック処理へ追加。小口だが両プレイヤー分の収集順・ガード応答（`pending_lrig_attack`）との噛み合わせ確認が要る |
