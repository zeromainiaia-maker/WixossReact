...
**直近＝2026-09-22＝🏁§5.6 `C-11`／`C-12`／`C-13`＝カットイン窓を3方向で閉じた**（全文は [BUGFIXES.md](./BUGFIXES.md) 第454バッチ）
- 🔴**`C-11`＝打ち消す価値を見ていなかった**（v1 は「打ち消せるなら打ち消す・エナの軽い順」だけ）＝
  先読み（`cpuLookahead`）で**「打ち消さなかった盤面」と「打ち消して札を失った盤面」**を採点して比べる（`cutinCounterGain`）。
  閾値は**ポリシー `cutinGainMin`（既定 4000）**＝📏実測の2つの帯の間（ドローだけのスペル **1,000〜3,000**／
  自分のシグニが消えるスペル **5,000〜7,000**・エナコスト 0〜2 のどれでも反転しない）。旧挙動は `legacy-cutin`。
  🔴📏**自己対戦で実際に化けた**＝`タマ改`×`レベル2止め` 3戦で、旧は《ドーピング》（アーツ）を
  **《噴流する知識》＝「カードを1枚引く」だけのスペル**に吐いていた（`census:play` の `cutin` 1 → 0）＝**登録票の症状そのもの**。
- 🔴**`C-12`＝ピース応答窓は「応答すると止まる」状態だった**＝`C-10` 第2段が no-op の `ui` を渡していたので、
  **最新盤面を読み直せず応答完了フラグが書けない**。⇒ `resolvePendingPiece`（41行）を画面から `controller/` へ出して
  **画面・ヘッドレスの両方が同じ1本を呼ぶ**形にし、`fetchLatest` を CPU へ配った。
  ⚠**口が無い呼び出し元はピース窓に応答しない**（`allowPieceWindow`）＝止まるより撃たない側へ倒す。
- **`C-13`＝レゾナのカットイン**＝**受け皿は在った**（`pickCpuResonaSelection`／`pickCpuResonaZone`）＝繋ぐだけで済んだ。
  枠（レベル・リミット）は `cpuSummonBudget` の1本を配る（写経しない）。実行は人間と同じ `performSummonSigni`。
- ⚠🔑**分岐スクリーニング（`S-6` 第1段）は「同じ山どうし」では 0/18 だった**のに、**山が違う対戦では化けた**＝
  🔑**篩の母集団に「窓が開く組み合わせ」が入っていないと、分岐0 は何も言っていない。**
- 📏**ピース窓・レゾナは実デッキに0枚**＝自己対戦でも実機でも踏めない＝**golden が直接指標**（`census:play` は `pending` で別枠）。
