...
>    - **【Opus 側】(3) §6.4 の未消化 worklist**＝番号つき表（`O-n` は着手順ではなく参照用の固定ID・欠番はそのまま）。**🆕2026-08-15 に `O-20`（全文 regex 読み・20サイト）と `O-21`（到達不能ハンドラ・19 id）を消化**（詳細は PLAN_DETAIL「2026-08-15 整理⑮」）。**🆕2026-08-15 続き484 で `O-22`（(a)(b)(c) 全部）／`O-23`／`O-24` も消化**（詳細は PLAN_DETAIL「2026-08-15 整理⑯」）。**次の候補＝`O-19`（watcher の `triggerScope` を parser が書く恒久解＝`O-20` の最後の1行を統合済み・**計器に映らない事故を起こす層**）／`O-3` の残26（`npm run census:stubs` の A群が常時の在庫表）／`O-4`（UNKNOWN 25ノード/25カード）**。⚠**在庫は §3-1 のとおり必ず実測してから着手する**＝続き483 では簿記の「58 id」が実測19 id だった（入れ子分岐を重複と誤検出していた）。⚠**`census:stubs` は「ハンドラの有無」で実装ありを判定する**＝ログを1行出すだけのハンドラは素通りする（続き459 の教訓）。
