...
- **🆕 セッション（2026-07-30・Opus 5〔Claude〕）＝タスク12(lxi) 第2波「対戦相手**は**〜しないかぎり」＝主語分配形 8カード9効果を配線**（golden **1120→1128**・census **1375→1373**（`BASELINE_HIGH=1373`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（5986枚・265群）・held **288枚/106署名 据置**・manual field loss 0・BOM/U+FFFD 新規0。ゲート独立実行で全緑／live per-effect 差分 **changed 9 / added 0 / removed 0**）
  - **⚠前セッションの簿記を訂正した**＝第1波の残を「『対戦相手は』形13効果＝同一文型の最大の弾」と書いたが、**これは粗い括りで誤り**。1件ずつ現状 JSON と engine ハンドラまで読むと**6種類の別バグ**だった＝①ゲート欠落の過剰実行（`SPDi43-01`／`WXK05-001`／`WX15-033`）②帰結が `TRASH{HAND_CARD owner:**self** ALL}`＝**自分の手札を全部捨てる**別物（`WX24-P4-023`）③型 `CONDITIONAL_DISCARD` に**executor の dispatch が無く完全 no-op**（`WX11-044`）④専用 STUB の非回避枝が `ENERGY_CHARGE`＝**相手にエナを与える**符号逆（`WDK10-001`）⑤専用 STUB が**ゲートを完全実装済み**なのに2文目から独立に生えた `TRANSFER_TO_DECK` が**回避を無効化する二重実行**（`WXDi-P01-028`）⑥**概ね正しかった**（`WXDi-P16-091`）。**「ゲートが無い」と決めつけず engine ハンドラの実装まで読むこと**が教訓。
  - **engine は既存語彙の水平展開のみ**＝`opponentHandDiscard` と同形で `opponentEnergyTrash?: number|'ALL'`／`opponentHandDiscardFilter?: TargetFilter`／`opponentHandDiscard` の `'ALL'` 拡張を追加し、`OPPONENT_PAY_OPTIONAL` の選択肢ビルダーに枝を足しただけ（resume 経路は `choiceId !== 'pay'` を汎用に扱うので無改修）。available 判定つき＝資源不足の回避手段は選べず、`ALL` は該当0枚なら枝を出さない。
  - **⚠第1波の「従来の読みを帰結に流用する」手は第2波では使えない**＝`opponentSelects` や対象数の是正は `parseSingleSentence` の**後段 post-pass** が `e.action` に対して行うため、文内で包んだ時点で射程外になり**静かに落ちる**（`SPDi43-01-E1` で対象数 2→1・`opponentSelects` 脱落を実測）。よって帰結は原文どおり**明示構築**へ切り替えた。拾えない帰結は据置＝誤った帰結にゲートだけ足さない。
  - **逆翻訳の穴も塞いだ**＝`OPPONENT_PAY_OPTIONAL` の描画が**回避手段を1つしか出さず**（`costColors` と `opponentHandDiscard` が両方あると前者が消える既存の穴も）、新しい回避手段が逆翻訳から無言で消えていた。全部を「AするかBしてもよい」形で併記するよう是正。
  - **残（honest defer・17効果）**＝複文2／回避コストが engine 非対応5／対象節以外の前置き5／「そのシグニ」照応2／純加算チェック不一致1／3機構一体1（`WX24-P4-007-E1`＝`DO_THREE_THINGS` は別カード用パターンで完全 no-op）／制限系で別機構1（`WXDi-P11-009-E3`）。内訳は BUGFIXES 冒頭節。
  - **次の一手**＝**Opus：①🆕 `CONDITIONAL_DISCARD` の executor 欠落**（型宣言と STUB-id ハンドラはあるが `{type:'CONDITIONAL_DISCARD'}` を捌く dispatch が無い＝残る唯一の使用者 `WD16-016-BURST`〔手札枚数分岐〕が完全 no-op）**②🆕 `WX15-033-E2` の fresh が `CONDITIONAL{LIFE_COMPARE_OPP}` を落とすドリフト**（採用すると無条件バニッシュへ退化するので held 据置中）**③`WX25-P1-056-E1`**（`leaveReplaceBanish` の no-op＝共通離場フックで閉じられる見込み）**④「コストの合計」族の STUB/UNKNOWN 14件⑤`WX11-043-E1` の `crossOnly` 欠落⑥タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑦`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑧§6.4 の配置数制限すり抜け7効果⑨§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・(lx) 2件・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**相手側 CHOOSE に3つ目の枝（エナトラッシュ）が出るケース**／第1波が持ち込んだUI 5件／§6.3 H／I′ の5件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-30 タスク12(lxi) 第2波後 最新値（本行を直近の正とする）**：census **1375→1373**（`BASELINE_HIGH=1373`）、golden **1120→1128**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288枚/106署名 据置**（採用6枚と新規落ち6枚が相殺）、manual field loss 0。live per-effect 差分 **changed 9 / added 0 / removed 0**（held 採用6カード＋カード単位 PRESERVE のため curated 直配線2効果）、`build:effects` 冪等。**新語彙は engine の水平展開3つのみ**＝`StubAction.opponentEnergyTrash`／`opponentHandDiscardFilter`／`opponentHandDiscard` の `ALL` 拡張（いずれも既存 `opponentHandDiscard` と同形）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1128**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1373**（回帰ゲート）・held **288枚/106署名**（2026-07-30 タスク12(lxi) 第2波後に実測）。
>
