...
**直近＝2026-09-23＝🏁**§5.7 `S-35` をクローズ（移し先は実測で存在せず。代わりに重みの効きセンサスを作った）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第468バッチ）

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🏁**本線キューは全部0**＝次の一手はユーザーの設計判断（下の「残っているもの」を参照） |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4402**）｜**実機 PASS**（`v286HiddenInfoNotInSharedLog`・反転確認あり） |
| 🆕📏**`S-35` の結論** | **グロウ／ルリグアタックは候補が最大1**（6デッキ・351盤面）＝**重みを移す先が存在しない**。新計器 `npm run census:weights` が **1戦30秒で全13重みの効き**を出す（`lrigLevel` 6山とも0／`life` は `WD06` だけ＝**A/B と完全一致**） |
