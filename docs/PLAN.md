...
**直近＝2026-09-20＝🏁§5.7 `S-21`＝探索の目的関数（計器を割り、コストを払わせ、A/B の読み方を直した）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔴**登録票の前提「111手中58手で打たない」は過大**だった―内訳は **候補0 12（判断ではない）／適用0 0／却下 46**で、
  却下46のうち **34 は ENERGY フェイズ**（本番は MAIN でしか探索を呼ばない）⇒ **本番の問題は MAIN の 12件だけ**。
- 🔴**見つけた穴**＝探索が【起】の**エナ以外のコストを1つも払っていなかった**（live **569効果**＝`down_self` 284 ほか）→ `payCpuSelfCostSim`。
- **入れた数値2本**＝`turnDamage`（手番側の「実際に殴れるレーン」だけ）・`actionBias`（行動への下駄）―**どちらも既定 0＝実機不変**。
- 🔴🔑**A/B の読み方そのものを直した**＝席入れ替えの2戦は相関するのに戦単位の Wilson で見ていた―
  主の判定を**組**へ（`pairRate`）。実例＝`damage-only` は 160戦 48.8% と出たが**80組中78組が1勝1敗＝実質の標本は2組**。
- **A/B 3本とも差なし⇒ `default` には何も採用しない**。次は `S-17`（終端をアタック後へ）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-17`**（後半ブロックの探索）＝`S-21` で**目的関数を重みで動かす道は尽きた**（3本とも差なし）⇒ **終端をアタック後に置く**のが残りの道。🆕**`S-20`（A/B の山が型を踏まない）は探索の A/B を回す前に要る**。`S-19`（先攻が勝てない）はどこへでも差し込める。🏁**§5.3 機構 worklist は残0** |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **0**／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 5**（`S-17`・`S-19`・`S-20`・`S-14`・`S-6`＝🏁`S-21` を消化）／リリース作業 **1**（RELEASE.md）／実機シナリオの既存 FAIL **0** |
| ⚠**直近の不具合** | 🔴**未修正で登録済み**＝①先攻が勝てない（`S-19`・この回の A/B でも **先攻 33.8〜36.9%**）②**A/B の山が型を踏まない**（`S-20`）。 |
| ⚠**バグ報告** | なし |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4342**＝+1）｜**実機不要**（`src/data/` `public/data/` 未変更・画面はポリシーを渡さない）＝代わりに **HEAD との `selfplay` 全一致**を反転確認とした |
---
...
- **2026-09-20 時点**（第416バッチ＝🏁§5.7 `S-21` 探索の目的関数）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **0**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 5**（`S-17`・`S-19`・`S-20`・`S-14`・`S-6`＝🏁`S-21` を消化）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4342**＝+1・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機不要**＝代わりに **HEAD との `selfplay --games 3 --seed 11` 全一致**（反転確認）
  - 🆕📏**CPU の判断の母集団**＝`ACTIVATED` **2,632効果**のうち **569効果がエナ以外の宣言コスト**を持つ
    （`down_self` 284 / `coin` 130 / `trash_self` 74 / `trash_key` 46 / `fieldDown` 15 / `lrigDown` 13 / `discardAll` 6 / `energyTrashAll` 5 / `acceTrash` 3 / `bounceSelf` 1 / `fieldExileSelf` 1）。
  - 🆕📊**探索といまの選択の一致**（幅4・深さ4・2戦・111手）＝同じ **40**／違う **13**／打たない **58**
    （内訳＝候補0 **12**／適用0 **0**／却下 **46**、うち **34 は ENERGY**＝本番は MAIN でしか探索を呼ばない）。
  - 🆕📊**A/B（組で見た `default` の勝率＝主の判定）**＝vs `search-act` **70.0%** [48.1, 85.5]（決着 20組）｜
    vs `search-damage-act` 57.9% [36.3, 76.9]（決着 19組）｜vs `damage-only` 0.0% [0.0, 65.8]（決着 **2組**）⇒ **3本とも差なし**。
    ⚠**先攻の勝率 33.8〜36.9%**（`S-19`）が分散の本体。
