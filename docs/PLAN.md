...
- **🆕 セッション（2026-07-10・続き61・Opus 4.8・§4タスク(C) resume経路トリガー取りこぼしの根本修正）**＝続き58/60（Sonnet）が実機で発見した「対象選択(SELECT_TARGET/CHOOSE)を挟んで resume経路で完了する効果ではトリガーの大半を取りこぼす」系統的 engine バグ（R43/R46/R39/R36＝4シナリオ実機FAIL）を、**場当たり的 inline 追加ではなく収集関数の統合リファクタで根本解決**（PLAN §6.3 の修正方針どおり・Opus 分担）。**修正**＝両経路（`resolveStackNext` の done分岐／`handleEffectInteraction` の resume done分岐）から共通で呼べる component-closure ヘルパー `collectBoardDiffTriggers(afterHost, afterGuest, {causeOwnerId, causeSourceCardNum})` を新設（`BattleScreen.tsx`・`collectFreezeInline` 直後）。約20種の盤面差分トリガー（BANISH/TRASH/DRAW/MILL/CHARM/ENERGY_TO_TRASH/REFRESH/OPP_POWER_DECREASED/MOVED_TO_DECK/FROZEN/ALLY_PLAY/MATERIAL_USED/BANISH_OPP_BY_EFFECT/LRIG_UNDER_MOVED/DECK_SHUFFLED/KEYWORD_GAINED/ON_PLAY+BLOOM/ENERGY_FROM_TRASH/ARMOR）を before(`bs.*`) vs after 比較で収集し entries＋once_per_turn 反映済み host/guest を返す。中央diff の当該509行と resume の inline群91行を**双方この1呼び出しに置換**（中央diffの収集順を厳密保存＝既存挙動不変）。action型固有（COLLAB/REVEAL_UNTIL_TO_FIELD/ON_ARTS_USE/FORCE_END_TURN）は resume の pending_effect に元action型が無く再現不能のため中央diffに inline 据置。**二重発火なし**（done=true/false 排他・多段は各ステップ最新 bs.* 基準）。**実機検証（`verifyBattleDrive.mjs`・ライブ）**＝FAILだった4シナリオが全**PASS反転**（oppPowerDecreased R46／energyToTrash R43／outsideDrawPhase R39〔SEQUENCE内TRASH対話を挟むDRAW＝in-band-aidでは潰せなかった本命〕／handDiscard R36）。**回帰なし**＝freezetrigger/drawBySourceStory/leaveFieldToHand/banishbyeffect/refreshTrigger/keywordgained を再実行し全PASS維持。4シナリオを既定 `order` に復帰済み。**ゲート**＝`npm run gates` 全緑（typecheck／golden 176／smoke CRASH0／fuzz 0／census 1572不変／lint 0 errors）。詳細 BUGFIXES 最上部・PLAN §6.3/§7。
  - **次の一手＝(A)Opus＝逆翻訳忠実性の systematic課題＝durational付与の「ターン終了時まで」でaction内durationが落ち decompileの期間注記（（ターン終了時まで））が脱落する系。🆕 母集団を機械抽出済み＝132枚（`docs/_duration_lead_population.txt`・型別 GRANT_KEYWORD 115/REMOVE_ABILITIES 23・MANUAL含8/AUTO124・drop分布 drop1=110/drop2=21/drop3=1。抽出スクリプト `scripts/archive/extractDurationLeadPopulation.mjs`＝症状〔decompile 原文count>逆翻訳count〕∩原因〔付与系action内 duration/until が end-of-turn でない〕の AND 交差）。次＝系統確立→各カードverify（§5b decompileテール・engine非依存・⚠一括置換禁止＝続き59でparser側の先頭duration一般復元は curated=PERMANENT慣例102枚と衝突しhold 124→219激増。修正はdecompiler側で原文照合し注記復元 or MANUAL効果はJSON手編集が素直）**。**(B)Opus＝effect-restriction（配置数制限）**（WXK11-074等・§6 BLOCK機構・実機検証必須）。**(C)Opus＝§3 Opusタスク3 動的比較 lrig相対の残**（WXEX2-25-E3・WXK07-025-E2のDRAW+condition復元）。**(D)Opus＝§3 Opusタスク4 引用内CHOOSE＋GRANT_QUOTED_AUTO_ABILITY内側parse。(E)Sonnet＝§7実機検証の継続**（R30/R44/R45①残・WXDi-CP02-082（R36②）・**🆕 R42（ON_CHARM_TO_TRASH・WX16-Re05）は続き61で `collectCharmToTrashTriggers` も統合ヘルパー配線済み＝resume経路の懸念は解消済みなので実機検証可**・ON_COIN_PAID/ON_LRIG_GROW/ON_TARGETED の follow-up項目）。**(F)follow-up（低優先）：`handleSelectZoneForEffect`/`handleSelectSigniZoneForEffect`（SELECT_ZONEで終わる効果の最終resume）は元々収集器を持たず本ヘルパー未配線＝別種の取りこぼし可能性（今回スコープ外・低頻度）。(G)持ち越し：SEQUENCE下流「そうした場合」IS_MY_TURN常時真連鎖の精緻化・PR-Di038 duration・WX25-P2-095・verifyBattleDriveバッチ状態汚染。**
...
- [ ] **🆕 durational付与の「ターン終了時まで」action内duration脱落＝期間注記の逆翻訳脱落（Opusタスク(A)・母集団抽出済み132枚）**＝`docs/_duration_lead_population.txt`（抽出 `scripts/archive/extractDurationLeadPopulation.mjs`）。GRANT_KEYWORD 115/REMOVE_ABILITIES 23（MANUAL含8/AUTO124）。原因＝`parseActionTextInner`（`effectParser.ts:1398`）が先頭「ターン終了時まで、」を strip し下流の付与規則（`t.includes('ターン終了時まで')`判定）に渡らず action内 duration が PERMANENT/missing に落ちる（文中の一部構文＝「対象とし、ターン終了時まで、」等も同様に落ちる）。**⚠一括置換禁止**（続き59＝parser側の一般復元は curated=PERMANENT慣例と衝突しhold激増）＝**decompiler側で原文照合し注記復元 or MANUAL効果はJSON手editの型で系統確立→各カードverify**。engine非依存（`execGrantKeyword` は UNTIL_OPP_TURN_END 以外を同一 keyword_grants でターン終了時クリア＝PERMANENT と機能同一）。
