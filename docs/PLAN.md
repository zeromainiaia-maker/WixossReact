...
| (lxv) | **🆕 タスク12(lxiv) の消化で積み残した held 15枚（2026-07-31）**＝対象宣言の正準形化は入ったが、**同じカードに本バッチと無関係な既存ドリフトが同居**していて一括採用できなかった分。主因は `IF(<gate>)[STUB:OPTIONAL_COST]` の**条件節を fresh が落とす**形（`WX24-P4-066-E1` の `ENERGY_COUNT`／`WX25-P2-095-E2` の `THIS_CARD_IS_AWAKENED`／`WX25-P3-069-E1` の `HAS_CARD_IN_FIELD`／`WXK03-045-E1` の `TRASH_HAS_CARD`）＝**採用すると条件が消えて過剰実行になる**ので据置。ほかに `WXDi-P12-066`／`WXDi-CP02-090`／`WXDi-P16-049` は差分効果が2つ以上ある。**このドリフト（条件つき任意コストの条件脱落）を先に直せば15枚まとめて解ける**見込み。詳細 BUGFIXES 2026-07-31 (lxiv) 節 |
...
- **🆕 セッション（2026-07-31・Opus 5〔Claude〕）＝タスク12(lxiv)＝対象宣言のフィルタが「そうした場合」の本体まで届かない 61枚＋「手札がN枚になるように捨てる」（残0クローズ）**（golden **1132→1134**・census **1373→1370**（`BASELINE_HIGH=1370`）・smoke **10679** 全0・SKIP0・fuzz 全0（seed 12648430）・lint 0 errors/**230 warnings 据置**・同型★0（265群）・held **288→277枚**〔署名 104→109〕・manual field loss 0。ゲート全緑／live per-effect 差分 **changed 63 / added 0 / removed 0**／`build:effects` 冪等）
  - **登録時は1枚のつもりが、機械走査で同型93効果**＝「〈対象宣言〉を対象とし、〈任意コスト〉てもよい。そうした場合、それを〈除去〉」で対象宣言の**フィルタ／体数／owner** が帰結に届かず「**どのシグニでも撃てる**」過剰実行（BANISH 77／TRANSFER_TO_DECK 8／BOUNCE 4 ほか）。**新機構0**＝タスク12(liii) と同じ正準形 `SELECT_TARGET_ONLY→STORE_LAST_PROCESSED_TARGETS→〈コスト〉→gate{targetsStored}` へ組み替える post-pass を追加し **61枚採用**。
  - **⚠SELECT/STORE はコストステップより前**に挿すこと。コストとゲートの間に挟むと `execSequence` の did-it ゲート（前段の直後が `CONDITIONAL{IS_MY_TURN}` かを見る）が外れ、**コストを払わなくても本体が撃てる**別のバグになる。コスト位置を同定できない形は触らない。
  - **投入前の実測で潰した落とし穴3つ**＝①**owner の反転**（名詞句の先頭一致だと「レベルが**あなたの**場に…以下の**対戦相手の**シグニ」が self に化け、**自分のシグニを撃つ**）→ 最後の「対戦相手の／あなたの」から取る ②**束縛が効かない型**＝`bindToStoredTarget` の BINDABLE に `UP/DOWN/FREEZE` があるのに **executor が `targetsStored` を honor していなかった**（黙って無視＝「対象を選び直させるだけ」の退化）→ 3つとも executor を配線し UP を追加、効かない場合は組み替えない ③**`lastProcessedCards` 依存の破壊**（`WXDi-P01-059-E1` の「そのカードがレベル１のシグニの場合」）→ 挿入位置〜帰結に `LAST_PROCESSED` 系があれば触らない。
  - **engine：did-it ゲートが「包み条件が不成立」を空振りと読めるようにした**（`effLastProcessed`）。対象宣言を前置した結果 `lastProcessedCards` が埋まり、**条件不成立でも「そうした場合」の本体が撃てる**ようになっていた（(lxiii) の `gateStep` だけでは足りなかった）。体数同期も追加＝「２体まで対象とし…それらを」が `count:1` に落ちていた（`WXDi-P02-043-E1`）。
  - **②「手札が**N枚になるように**捨てる」**＝`TRASH{count: 閾値−N}` と固定枚数を焼き込んでおり、手札が閾値ちょうどのときしか正しくなかった（8枚でも3枚しか捨てない＝過小）。`TrashAction.untilHandCount` を新設して実行時差分化（`WX18-032-E2`。エナ版は既存 `EQUALIZE_ENERGY` で正しく該当0）。
  - **次の一手**＝**Opus：①🆕 タスク12(lxv)＝(lxiv) で積み残した held 15枚**（`IF(<gate>)[STUB:OPTIONAL_COST]` の**条件節を fresh が落とす**既存ドリフトが同居していて一括採用できない。**このドリフトを先に直せば15枚まとめて解ける**）**②「コストの合計」族の STUB/UNKNOWN 14件③`WX11-043-E1` の `crossOnly` 欠落④タスク16 の timing 半分32効果**（着手前に `docs/_timing_census_triage.txt` の再トリアージが要る）**⑤`ON_TRASH under_signi` の3効果がコスト起因で発火しない⑥§6.4 の配置数制限すり抜け7効果⑦§6.3 I＝`WX25-P3-028-E2` の3機構**／(xxix) 残4・(lv) 残2経路・§6.3 残機構（C/E/G）。**Sonnet：§7 実機検証＝在庫はここだけ**＝🆕**(lxiv) の対象ピッカー前置 61枚**（支払い前に対象を選ぶUIへ変わった＝影響枚数が最大）／(lx)(lxii)(lxiii) の未検証UI／(lxi) 第1・第2波の UI 6件ほか（**単一 worklist は §7「残る実機検証項目」**）。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-31 タスク12(lxiv) 残0クローズ後 最新値（本行を直近の正とする）**：census **1373→1370**（`BASELINE_HIGH=1370`）、golden **1132→1134**、smoke **10679/10679** 全0・SKIP0、fuzz 全0（seed 12648430）、lint 0 errors/**230 warnings 据置**、同型★0（5986枚・265群）、held **288→277枚／署名グループ 104→109**、manual field loss 0。live per-effect 差分 **changed 63 / added 0 / removed 0**（61枚採用＋純改善2）、`build:effects` 冪等。**新語彙1本**＝`TrashAction.untilHandCount`（＋engine の `UP/DOWN/FREEZE` に `targetsStored` を配線）。
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden **1134**・smoke **10679** 全0（SKIP も 0）・fuzz 全0・同型★0・census **1370**（回帰ゲート）・held **277枚/109署名**（2026-07-31 タスク12(lxiv) クローズ後に実測）。
>
