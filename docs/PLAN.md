...
- **🆕 セッション（2026-07-13・続き106・Opus 4.8・PLAN §3 Opusタスク12(vi-2)/(v)/(vi)＝`applyDirectAction` の型対応漏れ6型を修正・smoke SKIP 258→1）**
  ...
  - **次の一手（Opus）＝** タスク12 の残在庫（**(vi-5)残＝二面コレクタ3種の usageLimit 書き戻し**・(vii)続き89・(viii)続き90 の構造的バグ・(x)続き104 `collectFieldTriggers` usageLimit 欠落32枚）から選ぶ。または タスク1(引用内側 parse)・タスク6(「代わりに」置換機構)。**Sonnet 発見バグの在庫が厚い＝タスク12 継続が費用対効果高**。
