...
- **🆕 セッション（2026-08-11・続き424〜425・Opus 5）＝§6.4 の機構2本。「型は正しいのに読む側が居ない」「主語も極性も反転」。golden 1771→1774・census 854 据置**。
  - **🔴① 続き424＝CONTINUOUS の `FORCE_SIGNI_ATTACK` は engine のどこからも読まれていなかった**＝強制アタックには軸が2本（ターン限定フラグ `must_attack_signi`／印字・付与の【常】）あり、**CONTINUOUS は `executeAction` を通らない**のでフラグは永久に立たない。受け皿（フラグ・警告モーダル・バナー・ソフトロック回避）は**全部あった**のに、この1点で印字【常】6カードが恒久 no-op（`WD07-004`／`WX14-018`／`WX20-Re07〜09`＋`WX12-010`）。`resolveForcedSigniAttack` に2軸を一本化し、呼び出し元3箇所を寄せた。
  - **🔴② 続き424＝`manualEffects.ts` に効果を「足す」と live に永久に届かない（第4の死角）**＝richness ガードが「effectId 集合が変わるカードは丸ごと温存」なので、*上書き*は届くのに *新規 id の追加*だけが黙って捨てられていた。**手書き効果3件が長期欠落**（`WXK04-003-DECORE`＝【デコレ】そのもの／`WXK04-042-E1b`／`WXK05-030-MULTIENA`）。⚠**MANUAL/PARTIAL の追加だけ**を採る（AUTO まで採ると `WD01-016` の【マルチエナ】が二重に載る）。
  - **🔴③ 続き425＝「対戦相手は手札をN枚捨ててもよい」を自分が捨てていた**＝part1 の任意手札捨て規則が**先頭アンカー無し**で、主語が「対戦相手は」でも `owner:'self'` に落ちる。3効果が自傷（`WXDi-P05-037-E1`／`WXDi-P07-010-E2`／`WXDi-P09-064-E1`＝最後の1枚は引くのも自分で完全反転）。**engine は `opponentHandDiscard` を最初から持っていて parser が生成していなかっただけ**。
  - **🔴④ 続き425＝`OPPONENT_PAY_OPTIONAL` の極性が1種類しかなかった**＝(a)「〜しないかぎり、X」＝払わなかったら X（65効果・engine と一致）と (b)「〜てもよい。**そうした場合**、X」＝払ったら X（2効果）が同じ形に落ちており、(b) は**意味が真逆**（`SPDi43-06-E1`／`WXDi-P05-037-E1`＝相手が何もしないと自分のアタックが無効化される）。`thenOnPay` を新設。**極性は次の文にしか現れない**＝文単位のパース規則では原理的に判定できず、効果本文まるごとを見る後段で刻む必要がある。
  - **⭐⑤ 「計器の網の外」を原文の文型で数え直す**＝`SPDi43-06` はどの worklist にも載っていなかった（トリップワイヤは「素の手札 TRASH」しか見ないのでコストが《無》のこの札は掛からない）。live 全 `OPPONENT_PAY_OPTIONAL` を原文の「ないかぎり」/「そうした場合」で仕分けて母集団を2件と確定させてから直した。
  - **⭐⑥ 後段の外科パッチは前段の変換を巻き戻す**＝`WX25-P2-082`/`WX25-P2-100` は `applyOptionalHandDiscardCost` が正準形にした後、cardNum アンカーの `applyResultConditionalWave3` が action を丸ごと組み直して素の `TRASH{optional}` へ戻していた（＝払わずに「そうした場合」ゲートを通せる）。**パッチ側が正準形を直接書く**よう是正。トリップワイヤ 7→3件。
  - **✅⑦ ゲート**＝`npm run gates` 全緑（golden **1774（+3）** / smoke 全0・SKIP 0 / fuzz 全0 / census **854 据置** / lint 0 errors）。`EFFECT_TYPE_MISSING_CONTINUOUS` **1→0**。`census:stubs` A群 16→18件（増分は明示 defer 2件＝**無言 no-op は 7件のまま**）。**要実機検証6件**（強制アタックのフェイズ進行ゲート3／相手側 CHOOSE の提示・向き3）は §7 に登録済み。
  - **📋⑧ 次の一手**＝**Opus 側**＝(a)**§6.4 A群の着手可能2件**（`ASSIST_LRIG_ATTACK_THIS_TURN`＝アシストルリグのアタック経路そのもの／`UNDER_CARD_AS_ENERGY_COST`＝**支払いUIの経路が15モーダル超**で要設計）(b)**トリップワイヤ残3件**（`WXDi-P10-039-E2`／`WXDi-P14-002-E1`／`WXDi-P14-044-E1`）(c)**所有者語の「前」に付く修飾が filter に載らない**（`WX25-P3-014-E1` 等・20枚規模）(d)🔴`WX25-CP1-092-E1`（幻コスト最後の1件）(e)**残 A群17件の手札コスト変換**。**Sonnet 側**＝§7 実機検証（**続き424〜425 の強制アタック／相手側 CHOOSE を最優先**）／`/census-batch`。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-11 続き424〜425（§6.4 強制アタック機構の配線／手書き効果の新規追加が live へ届かない死角／「対戦相手は〈コスト〉てもよい」の主語・極性）後 最新値（本行が直近の正）**：census **854**、golden **1774（+3）**、`MANDATORY_SUSPICIOUS` **1**（`EFFECT_TYPE_MISSING_CONTINUOUS` は **0**）、smoke 全0・SKIP 0、fuzz 全0（200ゲーム）、lint 0 errors、manual field loss 0、`census:stubs` A群 **18件**（明示 defer 11・**無言 no-op 7**）、UNKNOWN **36ノード**、golden `FORCED_HAND_COST_KNOWN` の🔴側 **3件**（7→3）。⚠`POWER_VALUE_MISMATCH` は **1**（`WX15-027`＝計器の誤検出。続き418 の「代わりに」加算モデルを読めていないだけで実害なし）。
...
- ~~**`checkAllEffects` の `MANDATORY_SUSPICIOUS` 精査＋`verifyEffects` の誤検出除外**~~ **✅2026-08-10 消化（続き408）**＝**33件を1件ずつ仕分けたら真バグ3件／誤検出30件**だった。誤検出6系統をルールで潰し、**`POWER_VALUE_MISMATCH` 6→0／`MILL_COUNT_MISMATCH` 3→0／`OPTIONAL_SUSPICIOUS` 2→0／`EFFECT_TYPE_MISSING_CONTINUOUS` 15→1（残1＝真バグ `WX12-010`）／`verifyEffects` のタイミング 4→0**。**計器の精度改善が本体**（続き407 の教訓「過剰報告する計器は無いより悪い」の実践）。詳細は BUGFIXES 2026-08-10（続き408）。
  ...
  - ~~**📋 トリップワイヤ側の残7件**~~ **✅2026-08-11 に4件消化（続き425）＝残3件**（`WXDi-P10-039-E2` 二段の任意／`WXDi-P14-002-E1` CHOOSE 選択肢の内側／`WXDi-P14-044-E1` action のトップが CONDITIONAL）。消化した4件＝`WX25-P2-082-E1`・`WX25-P2-100-E1`（**cardNum アンカーの外科パッチ `applyResultConditionalWave3` が任意コスト化を後段で組み直して巻き戻していた**＝パッチ側が正準形を直接書くよう是正）／🔴`WXDi-P05-037-E1`・`WXDi-P07-010-E2`（**原文の主語は「対戦相手は」＝所有者反転の自傷**。下記「対戦相手は〈コスト〉てもよい」参照）。
    - （旧記述）**残7件**（カード単位では他効果が任意なので `MANDATORY_SUSPICIOUS` には出ない・golden `FORCED_HAND_COST_KNOWN` の 🔴 側）＝`WX25-P2-082`／`WX25-P2-100`（**「代わりに」畳み込みが parser 後段で再構築し任意コスト化を巻き戻す**）／`WXDi-P07-010`（多分岐の内側）／`WXDi-P10-039`（二段の任意）／`WXDi-P14-002`・`WXDi-P16-048`（CHOOSE 選択肢の内側）／`WXDi-P14-044`（action のトップが CONDITIONAL）。🔴`WXDi-P05-037-E1` は**別バグ同居**＝原文「**対戦相手は**手札を２枚捨ててもよい」なのに `owner:self`（自分が捨てさせられる）。
