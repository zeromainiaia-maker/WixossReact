...
- **🆕 セッション（2026-07-22・続き249・Opus 4.8＋Codex 分担・**[P1_COMPLETION_ROADMAP](./P1_COMPLETION_ROADMAP.md) **バッチ1「状態条件節の持ち上げ」第1波**＝新条件型 `TURN_OWNER` 新設＋`LIFE_COUNT` 節拡張で**ターン所有者条件17＋ライフクロス枚数8＝25効果採用**（条件丸ごと脱落の無条件発火を是正）。census 1817→**1799**・golden 579→**584**）**（続き248 は [PLAN_PROGRESS.md](./PLAN_PROGRESS.md) 先頭へ退避）
  ...
  - **次の一手（Opus/Codex）**＝**バッチ1第2波**＝条件節クラスタ（`docs/_census_clusters.txt`「条件節」384テンプレ）から次の同機構群を切る（候補＝「それ/そのカードが〈属性〉の場合」参照カード属性系 約30効果＝LAST_PROCESSED_* 語彙の続き）。⚠新 CLAUSES は parser の **`STATE_HOIST_BATCH1_CARDS` でカード限定**＝第2波は対象カードを集合へ追加（or 集合撤廃＋heldReview 全数レビュー）。(xlix) permissive 据置4枚の Condition 語彙化も引き続き tractable。**Sonnet はタスク1（§7 実機検証）**＝TURN_OWNER の isOwnerTurn 配線9箇所は実機スポットチェック推奨（要実機検証）。
### 📊 恒久指標（維持中・逐次更新）
...
### 📌 次の一手（推奨順）
> **cold start＝まず `npm install` → `npm run gates`（全ゲート一括・数秒）が緑になることを確認する。** 現状＝golden 579・smoke/fuzz 全0（SKIP も 0）・同型★0・census 1817。
>
