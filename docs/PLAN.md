...
- **🆕 セッション（2026-08-13〜14・続き465〜474・Opus 5）＝**Codex 委譲で10バッチ連投**＝§7 実機挙動（P3）を**採番 worklist `V-01`〜`V-24` へ再編**したうえで **V-01〜V-10 を一巡**し、**新規シナリオ57本**を既定 order へ登録（**45本が緑**）。**実バグ5件**（(cxxv)(cxxvi)(cxxvii)(cxxviii)(cxxix)）を検出して**Opusタスク12 へ在庫化**。⚠**engine / parser / live JSON / `src/` は1行も変更していない**（運用ルール＝発見バグは即修正せず在庫化）。ゲートは全て据置（golden **1964**・census **831**・smoke 10688／SKIP 0・fuzz 全0・lint 0 errors/259 warnings・live JSON changed **0**）。**
  - **✅① §7 を採番 worklist へ再編**＝**決着済み8ブロック（続き459/458/457/427/434/431/424/425）を [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-13 整理⑬」へ退避**し、残作業を **`V-01`〜`V-24`**（🅰§3待ち3件／🅱最優先14件／🅲通常7件）に採番。**採番は固定**（消化しても詰めない＝`O-nn` と同じ運用）。**📌「実機シナリオを書くときの必読」11項目**も新設。
  - **✅② 消化した worklist**＝**V-04**（エナ支払い funnel）／**V-05**（対象宣言）／**V-06①②**／**V-07①②**／**V-08**（handDiscard）／**V-09**（任意性脱落）／**V-10**（F-3 身代わり・部分）。
  - **🔴③ 検出した実バグ5件**＝**(cxxv)** 離場置換の対話が `BANISH{count:数値}` 経路で発火しない（`count:'ALL'` は正しい＝**同じ機構が入口で割れている**）／**(cxxvi)** 相互身代わりで**同一インスタンスがエナに複製**／**(cxxvii)**「このアタックを無効にする」が**自分のアタックに効かない**（`NEGATE_ATTACK{owner:'opponent'}`＝live 全73件が opponent で、**自分のアタック時に発火するのはこの2効果だけ**）／**(cxxviii)** `WXDi-P04-051-E1` は timing 誤りで **`fieldDown:3` が構造的に支払い不能＝恒久 no-op**（攻撃者が先にダウンし3面盤面では最大2体しか残らない）／**(cxxix)** F-3 身代わりが**「コスト0」で成立**しライフを払わずバニッシュ回避（**機構未確定・調査順を在庫に記載**）。
  - **⭐④ Codex 委譲の型が完成**＝**10バッチとも「起案のみ・実行は検証側」**で回り、**指示書の誤りを Codex が訂正したのは通算24回**。特に効いたのは **(a)投入前に経路図を file:line の表で渡す (b)罠を行番号つきで先出しする (c)「静的に到達不能なら `canAfford極性不一致` で FAIL させる」設計を許可する**（**(cxxviii) はこれで一撃確定**）。🔴**Codex の "NO" と訂正は信頼できる**＝「凍結ではアタック不可にならない」「`pendingCandidates` の index を `pick-<idx>` に使うのは誤り（opp_field は reverse）」「`TRASH` が CONDITIONAL の外でも Pattern⑤ が pay 側へ包むので辞退時は走らない」など、**こちらの見立てを実コードで覆した**。
  - **🔴⑤ 検証側で潰したシナリオ偽陽性が5種**（→§7 📌 に恒久化）＝①**負方向テストは対照必須** ②**盤面だけを見る判定も偽陽性**（機構を1度も通らなくても同じ盤面になる＝**問いログ等の「機構が動いた証拠」を必須条件に**）③**`pendingEffect==null && stackLen===0` は効果の開始前と完了後の両方で true** ④**`pick-<idx>` は DB 配列 index ではない**（`opp_field` は reverse／`data-card-num` で狙う）⑤**DOM 描画待ちは全ヘルパ3秒に揃える**（`queryState` は Supabase 直照会で **DOM より先に真になる**）。
  - **⚠⑥ 環境事故**＝ハーネスの10分 kill 1回（**`codex.exe` は生存して完走・レポートは約9分後に出力**）／`0xC0000142` 1回（**通算8回目**・最終工程のみ実行不能＝**BUGFIXES を先に書かせる指示**により損失ゼロで検証側が引き取り）。
  - **▶ 次の一手**＝**Opus 側**：**(cxxv)〜(cxxix) の5件が最優先**（**全件に再現シナリオが既にあり、直せば赤→緑に反転する**＝調査ゼロで着手できる）。特に **(cxxvii)(cxxviii) は同型の「対象種別の取り違え」**なので併せて見るとよい。次点＝§6.4 O-20 の残21件・O-19。**Sonnet 側**：**§7 の `V-11`〜`V-24`**（`V-11` 配置制限／`V-12` アタック可否ゲート／`V-13` トラッシュ起動UI が手つかず）。⚠**V-10 のシナリオ3本は「ログ文言を実装から取る」「`asks` の期待値を実測に合わせる」だけで緑にできる**（engine の問題ではない）。⚠**シグニ【起】経路は `SigniActivatedModal.tsx:265` にエナ候補の testid が無く踏めない**＝V-04① の残り12経路を踏むなら **testid 追加が先**。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-14 続き465〜474（Codex 委譲10連投＝§7 実機検証 V-01〜V-10 一巡）後 最新値（本行が直近の正）**：census **831 据置**（`BASELINE_HIGH` 831）、golden **1964 据置**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 24種 42件**。**live JSON の変更は 0**（本セッションは driver と docs のみ＝engine/parser/`src/` 不変）。🆕**実機シナリオ（`verifyBattleDrive.mjs`）＝既定 order に +57本**（465 の7／466 の6／467 の6／468 の6／469 の6／470 の6／471 の6／472 の6／473 の4／474 の5＝**うち45本が緑・12本は §3 在庫バグを検出する赤のトリップワイヤ**）。🆕**Opusタスク12 の在庫＝8件**（(cxxiii)(cxix)(cxx 系の既存3件)＋本セッション登録の **(cxxv)(cxxvi)(cxxvii)(cxxviii)(cxxix)**）。
