...
- **🆕 セッション（2026-08-15・続き491・Opus 5）＝§6.4 **O-3 の「フェイズ／ターンのスキップ」系統を解体＝6効果の挙動是正**（**無言 no-op 4＋恒久 no-op 2**）。あわせて **PvP のアタックステップ封じ4効果**と **CPU の追加ターン**も是正。ゲート全緑（**golden 2027**＝+8・**census 831 据置**・lint 0/260）。
  - **🔴① 「STUB の在庫」を数えても半分しか見えない（今回いちばん効いた計測）**＝原文 regex（「〜フェイズ／ステップ／ターンをスキップする」13効果）で数え直すと壊れていたのは6効果で、**そのうち4件は STUB ですらなかった**＝`BLOCK_ACTION{ENERGY_PHASE}`（消費地点ゼロ＝`WX05-018-E1`）／`BLOCK_ACTION{SIGNI_ATTACK_PHASE}`（消費側は `SIGNI_ATTACK_STEP` ＝**綴りが1つズレて**`WX16-001-E3` が不発）／`STUB{SKIP_MAIN_PHASE}`（**ログを1行出すだけのハンドラ**＝`census:stubs` は「実装済み」と誤判定・続き459 の教訓の実例）／`SP38-006-E4`（アタック側が丸ごと脱落＋グロウ側は `END_OF_TURN`＝**スキップ対象でないターン**を封じていた）。**A群に出るのは残り2件だけ。**
  - **🔑② 新機構＝フェイズスキップを「次のフェイズを決める1点」に集約**（`PHASE_SKIP_BLOCK_IDS` ＋ `resolveNextPhaseWithSkips`＝旧 `resolveNextPhaseWithAttackStepBlocks` を表駆動へ一般化）。**フェイズ内の個別アクションを1つずつ封じない**（封じ漏れが1つでもあると無言ですり抜ける＝続き488 の `PAY_ENERGY_COST` と同じ設計）。⚠**「◯◯フェイズ開始時」フックは遷移元（`phase`）ではなく遷移先（`nextPhase`）で判定する**へ全部付け替えた＝①飛ばしたフェイズの開始時処理は走らない ②次に実際に入るフェイズの開始時処理はちゃんと走る、を1つの書き換えで満たす（通常進行では挙動不変）。⚠**CONTINUOUS 由来の封じは `blocked_actions` に載らない**＝`calcContinuousBlockedActions(...).forSelf` を渡し忘れると【常】が丸ごと no-op（golden にトリップワイヤ）。⚠**`GROW` はこの表に載せない**（「グロウフェイズをスキップする」7効果は従来の行動封じ近似のまま＝フェイズごと飛ばすと `ON_GROW_PHASE_START` の発火が7効果ぶん同時に変わる）。
  - **🔑③ 新機構＝ターンプレイヤー交代の判定を1関数へ**（`resolveTurnHandover`）＝`extra_turn`（追加ターン）と新設 `skip_next_turn`（`WD20-006-E1`）は**結果が同じ**（交代しない）。ターン終了は**3経路**（人間の通常／手札上限の捨て札あり／CPU）あり、軸ごとに分岐を書くと必ずどれかで漏れる。🔴**実際に漏れていた**＝CPU のターン終了は `activeUserId: user.id` 直書きで交代判定を持たず、**CPU の追加ターンも人間のターンスキップ予約も効かなかった**（`WD20-006` の原文は「**このターンが対戦相手のターンで**」＝この経路こそが本命）。golden にソース走査のトリップワイヤ（BattleScreen に**ちょうど3回**）。
  - **🔴④ 条件節を落としたまま機構だけ足すと「過剰実行」になる**＝`WD20-006-E1` の受け皿 STUB は「このターンが対戦相手のターンで、あなたがベットしていなかった場合、」を**丸ごと捨てていた**。`CONDITIONAL{AND[TURN_OWNER{opponent}, IS_BETTING{negate:true}]}` へ組み替え（`IS_BETTING.negate` 新設）、golden は**3通りの対**（相手ターン×非ベット／相手ターン×ベット／自分ターン×非ベット）で固定した。
  - **🔴⑤ 波及して見つかった PvP 限定バグ**＝`ATTACK_ARTS_OP` は `NON_TURN_PLAYER_PHASES`＝**進行ボタンを持つのが非ターンプレイヤー**なのに、`doPhaseAdvance` は `my.blocked_actions` でスキップ判定していた＝**「相手のシグニアタックステップを飛ばす」札4効果**（`WX09-Re02-E1`／`WXK01-007`／`WXDi-P09-031`／`WXK11-001`）が PvP で無言ですり抜けていた（CPU 戦は専用経路なので影響なし）。
  - **▶ 次の一手【Opus 側】**＝**O-3** の残9（A群が常時の在庫表）。`UNTIL_NEXT_MAIN_PHASE_CLAUSE`（`WXK01-002`＝基本リミット上書き＋ダメージ無効＋次ドロー2枚の3機構複合）／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH`（`WDK06-R09`）／`SEED_BLOOM_BOUNCE_OCCUPANT`（`WDK07-Y07`）。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → **O-26**（コスト節の自己トラッシュ脱落2件）。⚠**着手時は受け皿 id ではなく原文 regex で母集団を数え直す**（続き489/490/491 で3回とも「A群に出ていない隣の効果のほうが壊れていた」）。
  - **▶ 次の一手【Sonnet 側】**＝**§7 実機検証**。🆕**続き491 のフェイズスキップは全経路が未検証**＝(a) メインフェイズスキップ（`WXEX2-19`＝相手ターンで GROW→ATTACK_ARTS へ飛ぶか・`ON_MAIN_PHASE_START` が走らないか）(b) エナフェイズスキップ（`WX05-018`＝DRAW→GROW・`ON_GROW_PHASE_START` は走るか＝**負方向＋対照の対**）(c) ターンスキップ（`WD20-006`＝CPU ターン終了後にもう一度 CPU のターンが来るか）(d) 「このメインフェイズを終了する」の自動進行（`WXK06-078`）。ほか続き488〜490 の3経路（追加のアタックフェイズ／エナ支払い封じ／`AttackHandDiscardCostModal`）も未消化。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き490（§6.4 O-3 `ATTACK_TAX_HAND_DISCARD` 解体）後 最新値（本行が直近の正）**：census **831 据置**（較正なし＝今回は STUB→実装で高シグナルが増えなかった）、**golden 2019**（+2＝gate の両方向1・live 形1）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **107枚 / 48群**（据置）、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **27種/29件 → 26種/28件**）。🆕**live JSON changed 2カード**（`SP38-003`／`WXDi-P05-022`。CSV 非改変）。🆕**挙動是正 2効果**（恒久 no-op 2＝うち1件は `GRANT_KEYWORD.keyword` に文が入った**計器に映らない無言 no-op**）。🆕**新機構＝`SigniAttackBan.unlessPayHandDiscard`＋`signiAttackBanCost`（軸をまとめて返す）＋`AttackHandDiscardCostModal`**。⚠**新モーダルは実機未検証**（§7 送り）。

> **過去の計測 12 行（続き478〜489）は [PLAN_DETAIL.md](./PLAN_DETAIL.md)「2026-08-15 整理⑰」節へ退避**（2026-08-15）。**直近の正は上の1行**。
...
| **O-3** | **「次のターンの間」系統の残り** | 7効果（受け皿7種） | 🏁**続き490＝`ATTACK_TAX_HAND_DISCARD`（1）を解体＝`SigniAttackBan.unlessPayHandDiscard` を新設**（「手札をN枚捨てないかぎりアタックできない」＝**アタックするごとに**払う。判定は既存 `signiAttackGate`／引き落としは `performSigniAttack`／選択は新モーダル `AttackHandDiscardCostModal`）。**同型の無言 no-op 1件（`WXDi-P05-022-E1`）も同時に是正**＝引用文が丸ごと `GRANT_KEYWORD.keyword` に入り`hasKeyword` の照合外で**一度も効いていなかった**（どの計器にも映らないクラス）。残りは `UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`SKIP_NEXT_TURN` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／`NEXT_OPP_TURN_MAIN_PHASE_SKIP` 1／`DECLARED_SPELL_NAME_LOCK` 1／`GAIN_OPP_LRIG_TYPE` 1／`NON_FIELD_ZONE_MOVE_IMMUNITY` 1／`FIELD_SIGNI_TO_CHECK_ZONE` 1。🔑**続き490 の教訓**＝**「支払えば通る」制限は軸ごとに別関数を生やさない**（`signiAttackBanCost` が`{colorless, handDiscard}` をまとめて返す＝軸を足しても「片方だけ見る gate」が生まれない）。以下は旧記録＝🏁**続き489＝`NEXT_OPP_ATTACK_PHASE_START`（2）を解体＝遅延予約 `DELAY_TO_NEXT_OPP_ATTACK_PHASE` を新設**（予約は**予約した側**に積み、`ON_ATTACK_PHASE_START` の collector が `opState` 側を読む＝`pending_opponent_attack_facedown_returns` と同じ走査軸＝自分のアタックフェイズでは発火しない）。あわせて裏向きルリグゾーン（`PLACE_FACEDOWN_LRIG_ZONE`／`REVEAL_FACEDOWN_LRIG_ZONE`）と `SIGNI_ATTACK_BAN.levelFromLastProcessed` を新設し、**同じ受け皿に居なかった同型2枚（`WX25-P2-051`／`WXK01-003`）も同時に是正**＝**5効果の挙動是正**。🔴**`negated_attacks` は型に「このターン」と書いてありながら失効地点が1つも無く永続していた**（`turnScopedState` へ登録して解消）。残りは `UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`SKIP_NEXT_TURN` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`ATTACK_TAX_HAND_DISCARD` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／`NEXT_OPP_TURN_MAIN_PHASE_SKIP` 1／`DECLARED_SPELL_NAME_LOCK` 1／`GAIN_OPP_LRIG_TYPE` 1／`NON_FIELD_ZONE_MOVE_IMMUNITY` 1／`FIELD_SIGNI_TO_CHECK_ZONE` 1。🔑**続き489 の教訓**＝**受け皿 id は母集団の一部しか映さない**（同じ原文句を持つ4効果のうち A群に出ていたのは2件だけで、残り2件は「ハンドラ持ちの汎用 STUB」に落ちて**もっと壊れていた**）＝**着手時は原文 regex で母集団を数え直す**。以下は旧記録＝🏁**続き488＝`EXTRA_ATTACK_PHASE`（2）／`SELF_RESTRICT_THIS_TURN`（2）／`UNPARSED_NEXT_OPP_TURN_CLAUSE`（1）を解体**＝新機構 `ADD_EXTRA_ATTACK_PHASE`（`resolveNextPhaseAfterAttack` が ATTACK_LRIG の次を END でなく ATTACK_ARTS に戻す1点・`onStart` は追加フェイズ開始時に実行）／既存 `SIGNI_DEPLOY_BAN` へキー無し ban（`ALL_BAN`）／新 `blocked_actions:'PAY_ENERGY_COST'`（判定は `buildEnergyPayPool` を空にする1点）／新 `TRASH_ENERGY_AT_TURN_END`／`POWER_MODIFY.deltaFromZone`＝**8効果の挙動是正**。残りは `NEXT_OPP_ATTACK_PHASE_START` 2（**遅延本体の即時実行だけは停止済み**＝機構は未実装）／`UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`SKIP_NEXT_TURN` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`ATTACK_TAX_HAND_DISCARD` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／`NEXT_OPP_TURN_MAIN_PHASE_SKIP` 1／`DECLARED_SPELL_NAME_LOCK` 1／`GAIN_OPP_LRIG_TYPE` 1／`NON_FIELD_ZONE_MOVE_IMMUNITY` 1／🆕`FIELD_SIGNI_TO_CHECK_ZONE` 1。🔑**続き488 の教訓**＝**「遅延タイミング宣言」の後続文は絶対に即時実行しない**（`WXK06-026`＝メインフェイズ中に全シグニアップ／`SPDi43-24`＝使った瞬間に相手アタック無効／`WX22-010`＝場出し＋ルリグアップが即時、の3件が同じ壊れ方だった）。機構が入るまでは SEQUENCE 畳み込みで**後続を落とす**のが正しい（過少側に倒す）。🔑**`ADD_TO_FIELD` は `source` が無いと engine で「デッキの一番上」を場に出す**＝参照先が defer された「それらを場に出し」を残すと無関係なシグニが増える。以下は旧記録＝🏁続き448/450（採用14）／🏁続き453＝`REMOVE_ABILITIES.until` の死フィールド解消（7効果）／🏁続き459＝`LRIG_GROW_RESTRICT` ゴミ箱の解体（受け皿8つを固有 `DEFERRED_*` へ分割＝隠れていた26効果を A群に可視化）／🏁続き486＝`_THIS_TURN_OPP_CLAUSE`（7効果）を解体＝新機構 `SIGNI_ATTACK_BAN`／既存 `PREVENT_DAMAGE`／盤面リセットで**5効果の恒久 no-op を解消**／🏁**続き487＝`_NEXT_OPP_TURN_CLAUSE`（5）と `_THIS_AND_NEXT_TURN_CLAUSE`（4）を解体**＝新機構 `SIGNI_DEPLOY_BAN`（配置禁止を**場に出す側**に載せ `deployLimitBlockReason` 1本で判定・寿命はターン数カウントダウン）で3枚、`blocked_actions` の `:NEXT_TURN` 予約＋新 `USE_LRIG_ACT` で1枚、文跨ぎ照応の解決で1枚＝**7効果の挙動是正**。**在庫は `npm run census:stubs` の A群で常時見える**＝残りは `EXTRA_ATTACK_PHASE` 2／`NEXT_OPP_ATTACK_PHASE_START` 2／`SELF_RESTRICT_THIS_TURN` 2／`UNTIL_NEXT_MAIN_PHASE_CLAUSE` 1／`SKIP_NEXT_TURN` 1／`ATTACKED_SIGNI_TARGET_BY_KEY_TRASH` 1／`SEED_BLOOM_BOUNCE_OCCUPANT` 1／`ATTACK_TAX_HAND_DISCARD` 1／`OPP_DECLARED_ARTS_NAME_LOCK` 1／`OPP_CHOSEN_SIGNI_ATTACK_LOCK` 1／🆕`NEXT_OPP_TURN_MAIN_PHASE_SKIP` 1（**実装レシピは parser のコメントに記載済み**）／🆕`DECLARED_SPELL_NAME_LOCK` 1／🆕`GAIN_OPP_LRIG_TYPE` 1／🆕`NON_FIELD_ZONE_MOVE_IMMUNITY` 1。**機構ごとに独立して着手できる**。⚠**期間だけ直すと「誤った対象の効果」が長持ちする**＝対象軸が壊れている札は据置（`WX11-038-E2`）。🔑**続き486/487 の教訓**＝①**受け皿 id の件数だけ見て設計しない**（7件に5機構・9件に7機構が混ざっていた。原文を並べると「大半は既存 funnel に乗る」と分かる）②**STUB を実装で置き換えると census が +N する**（`vocabCensus` は STUB を含む効果を別バケツへ逃がすため、その効果の他の未表現語彙が同時に昇格する）＝毎回「計器の較正漏れ」か「本物の穴」かを仕分ける |
...
| **O-10** | **明示 defer の棚卸し** | 15 id | ⚠**2026-08-15 実測値**（続き484 で `DEFERRED_CHARM_POWER_MINUS_MULTIPLIER` を新設して 14→15 id。`census:stubs` の A群は全体で **25 種 / 43 件**＝この15 ＋ O-3 の10）。旧＝14 id（16件・2026-08-13 実測／O-2 の消化で 16→14 id）。`npm run census:stubs` の A群に `DEFERRED_*` として可視化済み。**理由が「先にやることがある」型は前提が消えたら着手できる**（例＝`DEFERRED_UNDER_CARD_AS_ENERGY_COST` は funnel 完成後に実装できた）。**定期的に前提を再判定する** |
...
| **O-13** | **実機UI未検証（→§7送り）** | 数件 | エナ支払い元 funnel（14サイト＋14モーダル）ほか、続き409〜413 の A群実装（相手ルリグデッキ選択モーダル・新規 CHOOSE 等）。🆕**続き452＝相手応答モーダルの実表示**（PvP の相手側／CPU 自動応答）＝`WXEX2-84-E2` が最も踏みやすい。🆕**続き453＝ターン境界を跨いだ能力喪失**（次ターンでの発火抑止／2ターン後の復帰）。🆕**続き488〜490 の3経路**＝**追加のアタックフェイズ**（`WXK06-026`＝2周目の `ON_ATTACK_PHASE_START` が走るか）／**エナ支払い封じ**（`SPK01-10`＝《色×0》は通る・エナ1以上は通らない**対**）／**`AttackHandDiscardCostModal`**（`SP38-003`＝手札が足りる/足りない**対**）。**ヘッドレスでは検証できない層** |
