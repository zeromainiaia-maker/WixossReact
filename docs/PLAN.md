...
- **🆕 セッション（2026-07-29・Opus〔Claude Opus 5〕が指示と検証＋codex-work が実装）＝タスク12(xxix) を5波連続で消化**（golden **1033→1050**・census **1412→1400**〔`BASELINE_HIGH=1400`〕・smoke 10726 全0・fuzz 全0・lint 0 errors/228 warnings 据置・同型★0・manual field loss 0。**全波でゲート全緑／差し戻し0**）
  - **第1波＝🔴エクシードの踏み倒し（過剰実行）21効果**。`SigniOnPlayCostModal` が `cost.exceed` を一切読まず「コスト: なし」と表示して確定できた＝**ルリグの下を1枚も払わずに【出】エクシードN が撃てた**。支払い実体 `INTERNAL_PAY_EXCEED`・Pattern③④・`CutinModal` のエクシード選択UI は**全部実装済み**で、Pattern⑤ とこのモーダルにだけ配線が無かったのが真因＝**新機構ゼロで解消**。同時に **Pattern⑤ が `handDiscardGroups` を完全に無視していた踏み倒し**（`WX20-058-E1`＝手札2枚を払わず3枚引ける）も是正。UI 判定は `exceedPoolOf`/`canPayExceed`/`paySelectedExceed` の純関数に切り出して golden から検証（fuzz/smoke は BattleScreen を通らないため）。副産物として `WXDi-P11-076-E1`（スペル・エクシード7）の踏み倒しも同時に解消。
  - **第2波＝`costUnparsed` 17効果を既存コスト語彙へ構造化**。`handToEnergy`/`handToUnderSelf`/`lifeTrash`/`lifeToHand`/`discard+discardFilter` はいずれも**支払い実装が既にあってデータが無い**状態だった（後者3つは live 使用0件）。⚠**`costUnparsed` 47件のうち14件は原文にコスト句が無い偽陽性**（【クロス自】【絆自】の《ターン1回》ヘッダ）＝「47件が worklist」を鵜呑みにしない。**`SUPPORTED` には足さない**＝`OptionalCostSpec` が払えないコストを engine 経路へ流すと踏み倒しになるため。
  - **第3波＝「N枚まで／好きな枚数 捨てる」7効果**。engine は **action 側（`COUNT_BASED_DRAW_OR_POWER`→`INTERNAL_CBDOP_AFTER_DISCARD`）が SELECT_TARGET で自分で徴収する**設計で、この経路に乗っていない7効果が**捨てが一度も起きず `lastProcessedCards`=0 の no-op** だった。🔴`WXDi-P13-041-E1` は no-op に加え**相手シグニ3体全部**にパワー修正する過剰実行（原文は「1体を対象とし」）＝両方同時に是正。既存経路の欠陥3件（空 `lastProcessedCards` の先取り／`WXDi-P05-031-E2` の「枚数と同じ数のシグニ」を比例ドローと誤認／`WXDi-P16-038-E1` の「+1」が0枚選択で消える）も同時に解消。
  - **第4波＝`fieldTrash` 18効果**。🔴**先に群A＝コスト限定の脱落4件を是正**（`WDK17-011-E2`「傀儡状態の」／`WX16-055-E1`「《ライズアイコン》を持つ」／`WXDi-P02-057-E1`「他の赤の」／`WX14-045-E1`「レベル1と2の2体」が素の `{cardType:'シグニ'}` ＝**通常召喚モーダルで本来払えないシグニでも払えていた**）。**順序が逆だと「払えないはずのコスト」のまま engine 経路にも面積が広がる**。`fieldTrashGroups` はモーダル未対応だったので判定を `fieldLimit.ts` の純関数へ切り出してUIごと対応（副次的に `canAfford` が「選んだゾーンが本当に候補か」を検証するようになった）。コスト参照（`last_field_trash_level`/`last_cost_trashed_puppet`）も engine 側 `TRASH{asCost}` で記録するようにし、🔴`WXDi-P00-067-E1` の落ちていた `powerLtLastProcessed`（＝無条件バニッシュだった過剰実行）も復元。
  - **第5波＝`handToEnergy` 7＋`handToUnderSelf` 4**。既存 action（`ENERGY_CHARGE{HAND_CARD}`・`PLACE_UNDER_SIGNI{source:'hand'}`）だけで表現。**支払い可否に「効果元シグニが在場か」を追加**＝`execPlaceUnderSigni` は在場でないと黙って no-op を返すため、無いと踏み倒しになる。`WXDi-P16-080-E1` は本体が「この方法でエナに置いたシグニと同じレベル」を参照し live action に該当 filter が無いため、`OPTIONAL_ON_PLAY_COST_REF_DEFERRED` カードゲートで明示保留（包むとエナから無制限回収）。
  - **⚠簿記が実測とズレていた点2つ（次セッションはここを疑う）**＝①指示書に書いた「写せない59」は第1波時点の値で、第2波が新語彙を作った分だけ母数が増えて stale だった（正しくは **71→54→43**）②`docs/_held_review.txt` は `21a24900` 以降再生成されておらず、**held 251→293 の +42 は本セッション起因ではない**（ベースラインコミットの worktree で再生成して 293/107 の一致を確認済み）。**投入前に必ず数え直す**（CODEX_GUIDE §3-1）。
  - **委譲運用の所見**＝codex は**指示書の実測表に書いた誤りは正確に直し、書かなかった部分の原文照合が甘い**という既知傾向のまま。一方で「見送り理由」「スコープ外で見つけた不一致」の自主申告は安定して高品質（第1波でエクシードルリグ6件の本体不一致、第4波で `WX20-063-E2`/`WXK06-082-E1` の filter 脱落を申告）。**Claude 側が投入前に per-effect の誤り明細を作るほど事故率が下がる**という §7 の教訓が5波とも再現した。Claude 検証で足した是正は2件（第3波の cost 落としを `discardUpTo` 限定へ／第5波のカード除外をバラの文字列リテラルから名前付きゲートへ）。
  - **次の一手**＝**Opus：(xxix)(1) 残り**〔「写せない」**43効果**＝`lrigDown` 7／`beat_signi` 6+1／`trashArtsFromLrigDeck` 6／`life_crash` 4＋`lifeTrash` 3＋`lifeToHand` 1（**第6波として投入中**）／`discardUpTo` 4／`removeOppVirus` 3／`deckTrash` 2／単発7〕**／(xxix)(2) 残**〔`costUnparsed` **25**（AUTO/ON_PLAY/任意は **11**）＝相互制約・可変資源・複数ゾーン全捨てで新機構待ち〕**／🆕単独バッチ候補**〔🔴`WX24-P4-072-E1`＝`PLAY_FREE` から「コストの合計が捨てた枚数以下」の制約が丸ごと落ちて**どんな高コストスペルでも無料で撃てる**過剰実行（経路が ACTIVATED で別）／エクシードルリグ6件の本体不一致（`WX24-P4-011-E2` の即時UP・`WX24-P4-018-E2` の unless 脱落 等＝**第1波でコストを払うようになった分いま効いてくる**）／`WX20-063-E2`「コスト合計0」・`WXK06-082-E1`「《幻竜 ドラマジ》以外」の filter 脱落〕**／(lv) 残2経路（CPU シグニ召喚・CPU グロウ）／ゲート脱落 [B]20件／タスク16 の [B]29件＋watcher [B]4件／§6.3 残機構**。**Sonnet：§7 実機検証の横展開**＝**分割UI（最優先）**＋宣言UI＋トラップ設置UI＋**(xxix) の支払いUI（エクシード選択・場シグニ選択・可変枚数捨て＝今セッションで新設/改修した窓）**＋レゾナ召喚UI の ATTACK/SPELL_CUTIN 窓。

### 📊 恒久指標（維持中・逐次更新）

- **🆕 2026-07-29 タスク12(xxix) 5波後の最新値（本行を (xxix) 系の正とする）**：**任意cost【出】の母集団 958／`optionalOnPlayCostStub` で写せない 43**（第1波前は 71。⚠80/59 等の旧値は母数が違う＝第2波が `handToEnergy`/`handToUnderSelf`/`lifeTrash`/`lifeToHand` を新たに構造化して母数が増えたため。**投入前に必ず数え直す**）。**`costUnparsed` 総数 25／AUTO・ON_PLAY・任意 11**（初期47／33。⚠47のうち14件は原文にコスト句が無い偽陽性）。golden **1050**、census **1400**（`BASELINE_HIGH=1400`）、smoke **10726/10726**、fuzz 全0、lint 0 errors/228 warnings、同型★0、manual field loss 0。
