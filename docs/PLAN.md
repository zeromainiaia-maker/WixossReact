...
>
> 1. **自分のモデル側のタスク表（§3）から取る**＝Opus は「機構・語彙を新しく開く」バッチ（推奨＝タスク11 BEHAVIOR_AUDIT 高シグナル22の仕分け／タスク12 Sonnet が積んだバグ在庫／タスク1〜6 の新語彙）。Sonnet は定型消化（推奨＝タスク1 §7 実機検証の横展開／タスク4 キュー再生成＋一次トリアージ）。
