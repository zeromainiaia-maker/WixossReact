...
- **🆕 セッション（2026-08-16・続き506・Opus 5）＝§6.4 **O-8／O-9 クローズ**（ともに**残0**）。ゲート全緑（**golden 2102**＝+8・**census 823 据置**・smoke 10688 / SKIP 0・fuzz 全0・同型★0（265群）・lint 0 errors・held **102枚 / 44群**＝据置）。live changed **3効果 / 3カード**。`census:stubs` A群 **18種/22件 → 16種/20件**（明示 defer 2種を実装で解体・無言 no-op 0 のまま）。新機構＝`collectForcedAttackZones`／`FORCED_ATTACK_ORDER`／`StubAction.opponentHandDiscardUpTo`／`PlayerState.facedown_release_by_payment`。
  ...
  - **▶ 次の一手【Opus 側】**＝`O-25` の残3件＋【常】引用クラス → `O-26` → `O-27` → `O-29` → `O-31` の残2 → `O-11`／`O-14`／`O-15`／`O-18` → `O-10`（明示 defer **残 6 id/8件**。続き507〜510 で4巡済み＝残りはどれも1件ずつ実機構が要る。最も近いのは `DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔要るのは「対象の無効化」1機構〕）。⚠`O-33` の据置分＝`WDK09-001-E2`（動的ゾーン）も残っている。**§6.4 で未着手の大物は `O-1`（CPU AI 拡張）だけ**。
...
| **O-10** | **明示 defer の棚卸し** | 6 id / 8件 | 🆕**2026-08-16 続き510（4巡目）＝8種/10件 → 6種/8件**。**■`DEFERRED_ATTACK_NEGATE_IMMUNITY_SELF`（`WX24-P4-016-E3`）**＝原文「このターン、**あなたの効果によって**シグニのアタックは無効にならない」の意味を先に確定させた＝ちより／【マジックボックス】系は「【ライフバースト】を持たない場合、**このアタックを無効にし**、〈利得〉」＝**自分の効果で自分のアタックを無効にする足枷**を持ち、この【起】はそれをターン中だけ外す。⇒ `own_effects_cannot_negate_signi_attack_this_turn` を新設し、**アタック無効化を書き込む3地点**（`SET_CANCEL_ATTACK_FLAG`／`SET_CANCEL_OPP_ATTACK_FLAG`／`NEGATE_ATTACK`〔シグニ対象のみ〕）で照会する。⚠**利得側**なので落ちていても盤面は壊れず「使ってもデメリットが消えない」**過少実行**として現れる（A群にも映らない型）。**■`DEFERRED_OPP_TURN_ARTS_COST_REDUCTION_ONCE`（`WXK03-071-E1`）**＝続き507 で「据置」にした理由（収集器が無い／コスト算出の入口が3箇所）は**両方とも既に解決済みだった**＝`ARTS_COST_REDUCTION_BY_COST_THRESHOLD` が同じ層の収集器を持ち、3入口は `computeArtsEffectiveCost` の`artsThresholdReductions` で**既に1本化**されていた（`modalCtx` 経由）。⇒ `collectOppTurnArtsCostReductions` を足して `minTotalCost:0` の項として合流。1回で自壊する分は `lost_ability_effect_ids_this_turn`（続き507 の機構）へアーツ使用の確定地点で刻む。🔴**隣接発見＝`artsThresholdReductions` は最初に一致した1件で `return` していた**＝宣言が2つあると片方が黙って消えていた（累積へ是正）。詳細は [BUGFIXES.md](./BUGFIXES.md) 2026-08-16（続き510）。⚠**全経路 実機未検証**＝§7 送り。**■残 6 種 / 8 件（どれも1件ずつ実機構が要る）**＝`DEFERRED_LEAVE_REPLACE_PAY_TO_LOSE_ABILITY`(3)〔対話つき離場置換の policy 差し替えと対〕／`DEFERRED_CHECK_ZONE_FLIP_FREE_GROW`〔**3部品**＝①「このターンにセンタールリグがグロウしていない」を数える state が**存在しない**②チェックゾーンの A/B 面フリップ ③特定カードへのコスト無しグロウ〕／`DEFERRED_COUNTER_TEAM_PIECE_CUTIN`〔ピース使用へのカットイン窓〕／`DEFERRED_DECLARE_NUMBER_AND_ATTACK_PHASE_RESTRICTIONS`〔数字宣言＋3種の制限〕／`DEFERRED_FLIP_SELF_FACE_DOWN_UP`〔裏向き→表向きの往復自体は `facedownSigni.ts` で書けるが、**観測できる効果は「新しいオブジェクトになって相手の対象指定が外れる」**＝*対象の無効化*という別機構〕／`DEFERRED_TRASH_AND_LRIG_TRASH_ABILITY_LOSS_IMMUNITY`〔トラッシュ内カードの能力喪失＋効果免疫＝相手トラッシュに触る全効果に及ぶ〕。**定期的に前提を再判定する**（続き507〜510 の計12種のうち、**「機構が無い」が本当だったのは3種だけ**＝残りは原文の誤読・同族の取り残し・`MANUAL` 刻印による陳腐化・funnel の後付け・**自分で書いた据置理由の陳腐化**だった）。⚠**在庫表は原文 regex と併用する**（続き491＝6効果中4件・続き492＝8効果中7件・続き507＝14効果中4件が STUB ですらなかった）。 |
