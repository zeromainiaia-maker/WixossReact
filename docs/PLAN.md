...
**Opusタスク12＝未消化の在庫 3件（2026-08-10 続き404・408・417 で登録）**。
...
| 🆕(cxxiv) | **`WX25-CP1-091` の ON_TURN_END 任意コストが実機で出ない**＝自ターン終了時に「このシグニの下から＜ブルアカ＞を3枚トラッシュ／置かない」の pay/skip が出るはずだが、**「ターン終了」ボタンを押した後の盤面変化が一切観測されない**（スタック・エナ・トラッシュ不変、`pEff=-`）。⚠**原因未確定**＝シナリオ側（ターンが実際に終わっていない／別のモーダルが残っている）か engine 側（`ON_TURN_END` watcher がこの盤面で収集されない）かの切り分けが未了 | 続き436（PLAN §7 実機検証バッチ2）＝`kokonaUnderThreePay`/`ThreeSkip`/`Insufficient` の3件が同じ止まり方をした | **切り分けが先**＝driver で `queryState` の phase／turn を観測して「ターンが実際に進んだか」を確定させる。進んでいるのに発火しないなら `collectTurnTriggers` 側、進んでいないなら driver 側。⚠3シナリオとも同じ止まり方＝共通原因の可能性が高い |
...
- **🆕 セッション（2026-08-13・続き457・Opus 5）＝§6.4 **O-16 完了**＝残2件（**動的 delta のゾーン grant**／**キーとシグニの能力喪失**）をどちらも消化。**キー能力喪失フラグの実装バグ2つも同時に露出・修正**。golden 1946→1951・census 832 据置・held 106枚/47群 据置**。
  - **🔴① `WDK10-009-E2`＝真 no-op だった**（「指定されたシグニゾーンにあるシグニのパワーを**そのシグニのレベル１につき**－2000」）。`STUB{POWER_MOD_PER_COUNT}` の消費地点は **CONTINUOUS 収集ループ1本だけ**で、ACTIVATED から実行されるとどこにも消費が無い。⭐`FieldGrant{kind:'power'}` に **`perTargetLevel`（動的 delta）** を新設。⚠**適用時に固定 delta へ焼き込まない**＝ゾーン継続なので、後からそのゾーンへ出た**別レベルのシグニにはそのシグニのレベル**が効く。倍率は grant 適用のたびに `effectiveSigniLevel` から決める。
  - **🔴② `SP38-006-E1` の1つ目の内側能力＝3軸とも別物**（キーが丸ごと落ち／シグニ1体だけ／期間が永続）。⭐`RemoveAbilitiesAction.alsoKeys` を新設し、既存フラグ `keys_abilities_disabled` へ倒す（キーは `field.signi` に居ないので per-card の `abilities_removed` では表せない）。**PARTIAL 温存カードなので effectId アンカーの外科パッチ**（続き452 と同型・`parseStatus` は `E4` の近似が残るため PARTIAL のまま）。
  - **🔴③ フラグ側に実装バグが2つあった**（②で頼る前に露出）＝(a)**半分しか効いていなかった**＝`keys_abilities_disabled` は CONT 収集8箇所でしか読まれず、**`triggerCollect` の AUTO 6箇所と `key_piece_extra` は素通り**。新設 funnel **`activeKeyAbilitySources(state)`** に「キーを能力源として列挙する全経路」を通した (b)**永久に戻らなかった**＝「このターン」限定なのに `turnScopedState` 未登録で、turn-end **4経路のうち2経路でしか手書きクリアされていなかった**（§6.4 O-3 の `abilities_removed` と全く同じ穴）。登録して手書き2箇所を削除＝**T2 トリップワイヤが再発を止める**。
  - **⭐④ 副次で2効果が改善**＝「このターン、」を期間語として読むようにしたため `SPDi47-01-E2`／`WX24-P4-013-E3` が `PERMANENT`→`UNTIL_END_OF_TURN`。**engine 挙動は不変**（この経路の PERMANENT は現ターン扱いに丸められる）だが、**逆翻訳だけが「永続」に見える偽陽性**が消えた。
  - **⚠⑤ 在庫は実測してから着手した**＝(a) の語形は全CSVで**1効果のみ**（除数つき「レベル２につき」は live 0件）＝Part4 の STUB 規則は**将来の除数つきを黙って誤パースさせない**フォールバックとして残した。
  - **▶ 次の一手**＝**Opus 側**：(a)**O-3 の残31**（`STUB` 系が主体。⚠**型ごとに `NEXT_TURN` の受け皿が engine にあるかを先に実測**＝続き453 の `REMOVE_ABILITIES.until` が「型にフィールドがある＝動く」ではなかった実例）(b)**O-6**（`WX25-P3-038-E1` の MANUAL 不可侵＝今回の `SP38-006` 外科パッチが直近の先例）(c)**O-4 UNKNOWN 25ノード/25カード**（1カード1機構の単発が大半）(d)**O-1 CPU AI 拡張**（唯一の大物・単独フェーズ扱い）。⚠**今回の隣接発見は `O-17` として登録済み**（`WXK05-010-E2` のキー対象取り違え／`WX24-P4-013-E3` のレベル・領域脱落）。**Sonnet 側**：**§7 実機検証**＝O-13 に続き452〜457 の6件（相手応答モーダル／ターン跨ぎの能力喪失／ゾーン指定→後から配置／2ゾーン選択UI／ゾーン封じ／**キー能力喪失のターン終了復帰**）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-13 続き457（§6.4 O-16 完了＝動的 delta のゾーン grant／キーとシグニの能力喪失）後 最新値（本行が直近の正）**：census **832 据置**（`BASELINE_HIGH` 832）、golden **1951（+5＝レベル比例ゾーン継続の E2E／`WDK10-009-E2` live 契約／キーとシグニの能力喪失 E2E／funnel の `key_piece_extra` 契約／`SP38-006-E1-G` の3軸 live 契約）**、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（グループ265）、held（parserWorklist）**106枚 / 署名グループ 47件**（据置）、lint **0 errors / 259 warnings**（据置）、**ターン限定 PlayerState レジストリ 35フィールド（34→35＝`keys_abilities_disabled` を登録）**、**UNKNOWN 25ノード / 25カード**（据置）、`MANDATORY_SUSPICIOUS` **0**、`census:stubs` A群＝**無言 no-op 0**／**明示 defer 14種 16件**（据置）、**`FieldGrant` の kind 3種**（`power`〔`perTargetLevel` で動的 delta 可〕／`abilityLoss`／`blockAction`）。live effect 単位 diff **4効果・effectId 増減 0/0**。
...
>    - **【Opus 側】(3) §6.4 の未消化 worklist**＝2026-08-12 に **`O-1`〜`O-15` の番号つき表へ整理済み**（消化済みの詳細は PLAN_DETAIL「2026-08-12 整理⑧」へ退避）。⚠**`O-2` は続き452／`O-3` の第3波は続き453／`O-16` は続き454〜457 で消化済み**（消化済みは PLAN_DETAIL「2026-08-13 整理⑨/⑪」へ退避）。**在庫は §3-1 のとおり実測してから着手する**＝続き453 では簿記の「約16効果」が**実測38効果**だった。**次の一手の候補＝`O-3` の残31（`STUB` 系が主体。⚠型ごとに受け皿を実測してから）／`O-6`（`WX25-P3-038-E1` の MANUAL 不可侵＝続き457 の `SP38-006` 外科パッチが直近の先例）／`O-4`（UNKNOWN 25ノード／25カード）／`O-17`（続き457 の隣接発見2件）**。
>    - **【Sonnet 側】(4) §7 実機検証の続き**＝続き436 で `keycost-energy-*`／`my|op-lrig-slot-*`／`card-action-*`+`data-action-label` を整備したので**新規シナリオが安くなっている**。未検証ブロックは §7 に20以上。まず **§3 (cxxiv)＝`WX25-CP1-091` の ON_TURN_END が出ない件の切り分け**（`queryState` の phase を観測して「ターンが実際に進んだか」を確定する）から。
...
| **O-3** | **「次のターンの間」系統の残り** | 実測31効果 | 🏁続き448/450（採用14）／🏁続き453＝**`REMOVE_ABILITIES.until` の死フィールド解消（7効果）**。⚠**簿記の「約16効果」は誤り**＝2026-08-13 に実測すると**原文が「次の…ターン（の間/まで）」なのに JSON に次ターンマーカーが無い効果は38件**あった（うち7件を続き453 で消化）。残31件の内訳は `STUB` 各種（`LRIG_GROW_RESTRICT`／`BLOCK_OPP_ZONE_PLACEMENT`／`POWER_MOD_PER_COUNT` ほか）が主体で、型つきは `CHOOSE` 3／`BLOCK_ACTION` 2／`GRANT_KEYWORD` 2／`BANISH`・`GRANT_PROTECTION`・`GRANT_LRIG_ABILITY`・`TRANSFER_TO_DECK` 各1。⚠**型ごとに `NEXT_TURN` の受け皿が engine にあるかを先に実測**（「型にフィールドがある＝動く」ではない＝続き453 の `REMOVE_ABILITIES.until` が実例）。🏁**続き455 で据置2件（`WXEX2-04-E3`／`WX25-P3-014-E2`）が解除**＝O-16 の指定ゾーン軸が入ったため。**残る据置は `WX11-038-E2` 1件のみ**（「次のターンの**メインフェイズ**の間」＝フェイズ限定の受け皿が無い）。⚠**期間だけ直すと「誤った対象の効果」が長持ちする**＝対象軸が壊れている札は据置 |
...
| **O-17** | **能力喪失の対象軸が落ちている2件**（続き457 の隣接発見） | 2効果 | (a)`WXK05-010-E2`＝原文「対戦相手の**キー**１枚を対象とし、ターン終了時まで、それは能力を失う」なのに live は `REMOVE_ABILITIES{SIGNI, count:1}`＝**対象種別がシグニに化けている**（キー1枚を選ぶ機構が要る。⚠`alsoKeys`＝「すべてのキー」とは別＝**1枚を選ぶ**ので `keys_abilities_disabled` では表せない）(b)`WX24-P4-013-E3`＝原文「このターン、この方法で公開されたシグニと**同じレベルの**、対戦相手の**すべての領域にある**シグニは能力を失う」なのに live は**場の全シグニ**＝レベル条件と領域（手札/エナ/トラッシュ）が両方落ちた**過剰効果**。⚠`levelEqLastProcessed` フィルタは既にある（`WX24-P3-063-E1` が MANUAL で使用中）＝要るのは `LOOK_AND_REORDER` の結果を `lastProcessedCards` へ載せる配線と、場以外の領域の受け皿 |
...
- **🆕 続き457（§6.4 O-16 完了＝動的 delta のゾーン grant／キーの能力喪失）が持ち込んだ未検証UI 3件**＝engine は golden で固定したが、**ターン境界を跨ぐ復帰**と**キーの【自】が止まること**は実機でしか踏めない。
  - [ ] **レベル比例のゾーン継続**（`WDK10-009` の【起】＝手札1枚を捨てて相手のシグニゾーン1つを指定）＝指定ゾーンのシグニのパワーが**レベル×2000 下がる**（レベル3なら－6000）。⭐**そのゾーンのシグニを入れ替えると新しいシグニのレベルで再計算される**（固定 delta に焼き込んでいたらここで－6000 のまま残る）。⚠**自分の次のターンが始まったら戻る**（相手ターン中は効いたまま）。
  - [ ] 🔴**キーの能力喪失が【常】だけでなく【自】も止めるか**（`SP38-006` の内側【起】＝エクシード1）＝相手の場にキー（【自】を持つもの）を置いて発動し、**その【自】が発火しなくなる**ことを見る。⚠**従来は CONT しか止まっていなかった**ので、ここが変わらなければ funnel が届いていない。
  - [ ] 🔴**キー能力喪失がターン終了で戻るか（4経路すべて）**＝①手札上限の捨て札なしでターンを終える ②捨て札ありで終える ③CPU のターン終了 ④自分のターン終了。⚠**登録前は①③で永久に戻らなかった**（`turnScopedState` 未登録＋手書きクリアが2経路だけ）。**戻らない経路があれば登録が効いていない**。
...
- **続き426（任意コスト脱落トリップワイヤの残0化）が持ち込んだ未検証UI 3件**＝**pay/skip モーダルが新たに出る**3カード。golden は構造を固定したが提示・徴収は踏めない。**⚠2026-08-11（続き435）＝`verifyBattleDrive.mjs` にシナリオ6件を試作したが skip側2件のみPASS。✅続き436＝真因は「UI に安定セレクタが無い」ことと判明し testid 新設で書き直した結果 `fezoneDoubleCostPay` が PASS へ反転**（残る `connectSpinningChoice4Pay`/`Insufficient` は**セレクタ問題は解消**し、ピースの効果発火という別の壁で止まっている＝下記）。
  - [ ] 🔴**踏み倒しが塞がったか**（`WXDi-P14-002` の選択肢④＝「手札を２枚捨ててもよい。そうした場合、対戦相手のライフクロス１枚をクラッシュする」）＝**手札が1枚以下のとき「支払う」が選べない**（`canAfford`）。⚠**手札が足りないのにライフがクラッシュしたら回帰**（旧実装がまさにそれ）。併せて**【使用条件】**（場のルリグ3体で合計3色以上）を満たさないとピース自体が使えないことも見る。**⚠2026-08-11（続き436）＝セレクタ問題は解消したが別の壁で未達**（`connectSpinningChoice4Pay`/`Insufficient`）＝`KeyUseModal` のエナ選択は**完全に機能**するようになり（`keycost-energy-0/1/2` を選択→energy 3→0→セット成功）、**その後に効果が一切発火しない**（`pEff=-`）ところで止まる。実測＝`WXDi-P14-002-E1` は `effectType:'ACTIVATED'`／`timing:['MAIN']` なのに `executeKeyPiece`（`BattleScreen.tsx:6613`）は `queueCardEffects(..., ['AUTO'], ['ON_PLAY'], ...)` しか積まない＝**ピースの効果は KEY スロットの【起】から別途起動する設計**。⇒ シナリオに【起】起動の1手を足す必要がある（§3 の新規登録も参照＝**印刷 Cost と E1 の `cost.energy` が同一なので二重請求の疑い**）。
  - [x] 🔴**《青》＋手札2枚の複合コスト**（`WXDi-P14-044`＝アタックフェイズ開始時）＝pay を選ぶと**エナから《青》1つと手札2枚の両方**が減る。⚠**《青》が減らなければ回帰**（旧実装はコスト句ごと脱落していた）。スキップすると相手シグニはデッキの一番下へ**行かない**。**skip側は確認済みPASS（`fezoneDoubleCostSkip`）。✅2026-08-11（続き436）＝pay側も実機PASS**（`fezoneDoubleCostPay`・2回連続）＝`energy 1→0`（《青》1）**かつ** `hand 2→0`（2枚）の**両方**が徴収され、対象がデッキの一番下へ移動することを確認＝コスト句脱落への退化なし。
  - [ ] **二段の任意**（`WXDi-P10-039`＝このカードが捨てられたとき）＝①「手札を1枚捨てる／スキップ」→②払った場合だけ「《青》《無》を支払う／スキップ」→③払った場合だけトラッシュから場に出る。⚠①をスキップしたら②が**出ない**こと。⚠「そのターン終了時」の遅延は未実装（②③が即座に来て正しい）。**前提バグ（続き60発見のON_TRASH self resume取りこぼし＝`handDiscard`シナリオ）は2026-08-11時点で解消済みと確認**（`handDiscard`が既定orderに復帰＝PASS）。B3自体のシナリオはまだ未作成＝次回。
...
- **🆕 続き433（§6.4 UNKNOWN＝一時レゾナの返却）が持ち込んだ未検証UI 2件**＝engine は golden で固定したが、**ターン終了処理は実機でしか踏めない**（2経路ある）。
  - [ ] 🔴**出したレゾナがターン終了時にルリグデッキへ戻るか**（`WX07-050`＝【出】でレゾナを出す／`WX16-Re18`＝【起】）＝場に出たレゾナが**自分のターン終了時に消え、ルリグデッキへ戻る**（トラッシュではない）。⚠**居座ったら回帰**（従来はずっと場に残る過剰効果だった）。
  - [ ] **手札が多いターンでも戻るか**＝ターン終了時に手札上限の**ディスカードが発生する**盤面で同じことをする。⚠BattleScreen のターン終了処理は2経路あり、**ディスカードを挟む側でも戻る**こと（片方だけだと「手札が多いターンだけ戻らない」無言の不整合）。
  - [ ] 📋**未実装として送る**＝`WX16-Re18` の「レゾナを**２枚まで**」は**1枚しか出ない**（複数枚の任意配置UIが未実装）＝過少なのでルール違反ではない。

- **🆕 続き431（§6.4 ライフクラッシュ置換）が持ち込んだ未検証UI 3件（最優先）**＝engine は golden で固定したが、**ダメージ解決の分岐**は実機でしか踏めない。⚠**3枚とも従来「生きた過剰効果／自傷」だった**ので、**まず退化していないこと**から見る。
  - [ ] 🔴**`WX24-P4-009` を使った瞬間に自分のデッキが減らないか**（回帰確認）＝アーツを使う → トラッシュがデッキに戻ってシャッフルされるだけで、**デッキは10枚削れない**。そのあと**相手のシグニのアタックでライフをクラッシュされる場面**で、代わりにデッキ上10枚がトラッシュへ行きライフは減らない。⚠**使用時に10枚減ったら回帰**。
  - [ ] 🔴**`WX25-P3-004` を使った瞬間に相手のライフが減らないか**（回帰確認）＝アーツを使う → **相手のライフはそのまま**。そのあと**相手のシグニのアタック**でライフをクラッシュされる場面で、代わりに**相手のライフが1枚クラッシュ**される（【ライフバースト】確認フローが相手側で走る）。⚠**使用時に1枚割れたら回帰**。
  - [ ] **`WXDi-CP01-023`（月ノ美兎）と限定の効き**＝アシストルリグを出す → そのターン、**相手のシグニのアタック**でクラッシュされるたびに代わりにデッキ上5枚がトラッシュへ（回数制限なし）。⚠**ルリグアタックのダメージでは置換されない**こと／**効果によるライフクラッシュでも置換されない**こと（「シグニのアタックによって」限定）。併せて `WX25-P1-010`（「シグニによって」限定）が**ルリグアタックを置換しない**ことも確認する（従来は置換していた）。
  - [ ] 📋**未実装として送る（バグではない）**＝「〜してもよい」の**辞退がまだできない**（自動適用）。原文の「デッキがN-1枚以下の場合は置き換えられない」が効くのでデッキアウトはしない＝対話化は離場置換 M2 と同じ枠組みで別バッチ。

- **🆕 続き430（§6.4 離場置換の対話化）が持ち込んだ未検証UI 3件（最優先）**＝engine は golden で固定したが、**問いが被害側に出る／答えが engine に届く**のは実UIでしか踏めない。⚠**壊れ方が無言**（問いが出ない・答えが潰れる・辞退したのに置換される、のいずれも盤面ログは「置換した/しない」としか言わない）。
  - [ ] 🔴**被害側に問いが出て、答えが効くか**＝相手の場に `WX12-024`（幻怪　オバケイテン系＝「このシグニがバニッシュされる場合、代わりにあなたの他の＜電機＞のシグニ１体をバニッシュしてもよい」）と他の＜電機＞シグニを置き、**こちらの効果でそれをバニッシュ**する。**相手側の画面に「場離れの置換」モーダル**が出て、①置換を選ぶと victim が残り犠牲シグニが消える ②**「置換しない」を選ぶと victim が普通にバニッシュされる**。⚠**②が効かない（必ず置換される）なら回帰**＝従来の自動適用のまま。⚠**モーダルが出ずに素通りしたら無言バグ**。
  - [ ] 🔴**CPU が被害側のときに止まらないか**＝同じ盤面を CPU 側に作って効果でバニッシュする。**CPU が自動応答して解決が進む**こと（従来と同じく先頭の選択肢＝置換を選ぶ）。⚠**ここで固まるとターンが進まないソフトロック**になるので最優先で見る。
  - [ ] **複数体を同時に動かす場合**＝「対戦相手のシグニすべてを手札に戻す」等（`count:'ALL'` 経路）で、**置換候補を持つ体ごとに1回ずつ問われ**、答え終わってから移動がまとめて起きる。⚠途中の問いで**残りの対象が落ちたら回帰**（適用前に全部聞く設計なので落ちないはず）。
  - [ ] 📋**未実装として送る（バグではない）**＝(a) **CPU は常に先頭の選択肢（＝最も安い置換）を選ぶ**＝盤面評価はしない近似 (b) `WX14-026` の**ライフクラッシュは選択肢に出るが engine の自動適用はしない**（CPU が被害側だと先頭に来れば選ばれる＝【ライフバースト】確認フローとの整合は §7 で要確認）。

- **🆕 続き428（§6.4 エナ支払い元の一本化＋`UNDER_CARD_AS_ENERGY_COST`）が持ち込んだ未検証UI 4件（最優先）**＝**支払いモーダル14本＋支払いサイト14箇所を funnel（`screens/battle/energyPaySource.ts`）へ寄せた大改修**。純関数は golden で固定したが、**モーダルの候補描画と控除は golden では原理的に踏めない**。⚠**まず「回帰していないこと」から見る**（下の①が最重要）。
  - [ ] 🔴**既存のエナ支払いが全経路で従来どおり動くか**（回帰確認・`UNDER_CARD_AS_ENERGY_COST` を持つカードが場に**居ない**盤面で）＝**グロウ／アーツ／スペル／キー使用／キー【起】／アシストグロウ／アシスト【起】／シグニ【起】／エナ【起】／手札【起】／トラッシュ【起】／ルリグ付与【起】／カットイン／【出】コスト**の各経路で、**選んだエナだけが減ってトラッシュへ行く**。⚠**エナが1枚も減らなかったら `applyTo` の呼び忘れ**（funnel の設計上そういう壊れ方をする）。
  - [ ] 🔴**`WXDi-P10-041`（羅植姫 タナバタ）の下カードで払えるか**＝自分のアタックフェイズ開始時にデッキ上2枚がこのシグニの下に置かれる → **アタックフェイズ中**にアーツ／【起】のコスト選択画面を開くと、**エナゾーンのカードに続けて「下のカード」が候補に並ぶ**（長押し/hover で「羅植姫　タナバタの下」と出る）。選んで支払うと**そのカードがトラッシュへ行き、シグニ本体は場に残る**。
  - [ ] **フェイズと上限のゲート**＝**メインフェイズでは下のカードが候補に出ない**／**相手のターンでも出ない**（カットイン窓を含む）／**1ターンに3枚払ったらそれ以降は候補に出ない**（ターンをまたぐと復活する）。
  - [ ] **`WXDi-P10-041-E3` と噛み合うか**＝下のカードを払った後にこのシグニが場を離れると、「下にあったカード1枚をトラッシュからエナゾーンへ」の対象に**払ったカードも含まれる**（トラッシュに居るので正しい）。
  - [ ] 📋**未実装として送る（バグではない）**＝(a) 候補の**視覚的マーカーが `title` 属性だけ**＝エナゾーンのカードと見た目が同じ（枠色/バッジを足すのは §7 の UI 改善）。(b) **残り上限より下カードが多いとき「どれを払うか」を選べない**（上から残り枚数ぶんだけ候補に出る＝過払いにはならない安全側の近似）。(c) **CPU は下カードから支払わない**（AI ヒューリスティックの近似＝ルール違反にはならない）。

- **🆕 続き425（「対戦相手は〈コスト〉てもよい」の主語・極性是正）が持ち込んだ未検証UI 3件（最優先）**＝**相手側 CHOOSE（`opponentResponds`）の提示と CPU 自動応答は golden では原理的に踏めない**。
  - [ ] 🔴**極性が正しいか**（`SPDi43-06` か `WXDi-P05-037` でアタックする）＝原文「対戦相手は〈コスト〉を支払ってもよい。そうした場合、**このアタックを無効にする**」。**相手が支払ったときだけアタックが無効**になり、**支払わなければアタックは通る**。⚠**逆になっていたら回帰**（旧実装は相手が何もしないと自分のアタックが無効化されていた）。CPU 相手なら CPU の自動応答がどちらを選ぶかをログで確認する。
  - [ ] 🔴**捨てる側が相手か**（`WXDi-P05-037` でアタック）＝プロンプトは**対戦相手側**に出て、減るのは**対戦相手の手札2枚**。⚠**自分の手札が減ったら回帰**（旧実装は自分が2枚捨てて自分のアタックを無効化していた）。
  - [ ] **`WXDi-P09-064` の【出】**＝場に出すと**対戦相手**に「手札を2枚捨てる／支払わない」が出て、捨てたら**対戦相手が2枚引く**。⚠自分の手札・自分のドローが動いたら回帰。⚠「2枚**まで**」は現状 0枚か2枚の二択に丸めてある（1枚だけの選択肢が出なくて正しい）。

- **🆕 続き424（§6.4 強制アタック機構の配線）が持ち込んだ未検証UI 3件（最優先）**＝engine 側は golden で固定したが、**強制アタックの enforcement は `BattleScreen` のフェイズ進行ゲートにしかなく golden では原理的に踏めない**。
  - [ ] 🔴**相手の印字【常】で自分がフェイズを進められないか**（相手ルリグ `WD07-004`／`WX14-018`／`WX20-Re07〜09` のいずれか）＝自分のアタックフェイズで**アタック可能なシグニを残したまま「次へ」を押すと「⚠ アタックしなければなりません」が出て進めない**。全部アタックすれば進める。⚠**従来はこの【常】が完全に無視されていた**（＝素通りできたら回帰）。
  - [ ] **強制アタックバナーが出るか**＝上と同じ盤面の ATTACK_SIGNI フェイズで画面上部に「⚠ あなたのシグニは可能ならばアタックしなければなりません」が出る。相手ターン側からは緑バナー（「対戦相手のシグニは〜」）。
  - [ ] **アタックできないシグニでソフトロックしないか**＝強制中に「アタックできない」付与／パワー上限／コスト不足のシグニだけが残った場合、**警告は出ずに次フェイズへ進める**（`mustAttackRemainingZones` はアタックボタンが出るゾーンだけを数える）。⚠ここが壊れると**ターンが進まないソフトロック**になるので最優先で見る。
  - [ ] （併せて）**`WX12-010` の【出】**＝場に出したとき「対戦相手のすべてのシグニを好きなように配置し直す」モーダルが出て、**辞退もできる**こと。⚠2段目「移動したシグニをアップしてもよい」は**明示 defer**（出なくて正しい）。

- **🆕 続き423（対象宣言の脱落＝owner/count/パワー制限）が持ち込んだ未検証UI 2件（最優先）**＝**16効果**で対象の範囲が変わった。
  - [ ] 🔴**所有者と体数**（`WXDi-P02-009-E3`＝「対戦相手のシグニを**２体まで**対象とし…そうした場合、それらを手札に戻す」）＝**対戦相手のシグニを最大2体**選べて、それらが手札に戻る。⚠**自分のシグニが選択候補に出たら回帰**（旧実装は自分のシグニ1体が戻っていた）。0体・1体の選択も通ること（「まで」）。
  - [ ] **パワー制限が候補に効くか**（`WX06-CB01-E1`＝「対戦相手の**パワー3000以下の**シグニ1体」）＝**3000より大きいシグニは選択候補に出ない**。⚠旧実装は無差別に選べた。

- **🆕 続き422（幻コスト第2波＋下カードコストの絞り込み）が持ち込んだ未検証UI 2件（最優先）**
  - [ ] **下カードコストの候補が絞られるか**（`WXDi-P11-042-E1`＝「このシグニの下から**赤の**シグニ1枚をトラッシュに置いてもよい」）＝**下の赤いシグニだけが選択候補**になり、赤が無ければ**支払い不可**として出る。⚠続き421 ではこの filter が**型にも無い死フィールド**で、逆翻訳にだけ出て engine は無視していた（続き422 で配線）。
  - [ ] 🔴**捨てさせる向き**（`WXDi-P14-060-E1`）＝「相手の手札1枚を公開 → 捨てさせるか選ぶ → 捨てさせたら**相手が**1枚引く」。⚠**自分の手札が減ったら回帰**（旧実装は自分が1枚失ったうえで相手が1枚引く真逆だった）。辞退したら**相手のドローも起きない**こと。

- **🆕 続き421（🔴幻の手札コストの是正）が持ち込んだ未検証UI 2件（最優先）**＝**16効果**でコストの徴収先が変わった（従来は原文と無関係に**手札**が1枚落ちていた）。
  - [ ] 🔴**エナゾーンから正しく徴収されるか**（`WX24-P1-047-E1`＝「エナゾーンからレベル１のシグニ２枚をトラッシュに置いてもよい」）＝**支払うと自分のエナが2枚減り、手札は減らない**。⚠**手札が1枚減ったら回帰**（旧実装がまさにそれで、しかもエナは減っていなかった）。レベル1のシグニがエナに2枚無ければ**支払い不可**として出ること。
  - [ ] **自己トラッシュコストが二重に取られないか**（`WX06-CB01-E1`）＝「このシグニを場からトラッシュに置いてもよい」だけが提示され、**手札は一切減らない**（旧実装は手札1枚＋このシグニの両方を失っていた）。

- **🆕 続き420（`TARGET_AND_DISCARD_HAND` → `OPTIONAL_COST{handDiscard}`）が持ち込んだ未検証UI 3件（最優先）**＝**18効果**で「手札を捨てる／捨てない」のモーダルが**新たに出る**（従来は engine が手札の末尾を無言で捨てていた＝UI が一度も出ていなかった）。golden は構造と filter を固定したが、**実際に候補が絞られて出るか・辞退できるか**は踏めない。
  - [ ] **絞り込みが効くか**（`WX18-001-E3`＝「手札から＜悪魔＞のシグニを1枚捨てる」）＝**手札の＜悪魔＞シグニだけが選択候補**になる（他のカードはグレーアウト／選べない）。⚠従来は**末尾の1枚が問答無用で落ちて**いたので、ここが直った実感が一番出る。
  - [ ] **辞退できるか**（`WXDi-CP01-027-E3`＝「《ガードアイコン》を持つシグニを1枚捨ててもよい」）＝**捨てないを選ぶと本体（手札に戻す）も走らない**（「そうした場合」ゲート）。⚠**捨てないのに本体が走ったら回帰**。
  - [ ] 🔴**ルリグを対象にするか**（`WX25-CP1-004-E1` 選択肢②）＝「対戦相手のルリグ1体をダウンする」で**対戦相手のセンタールリグ**がダウンする。⚠**自分のシグニがダウンしたら回帰**（旧実装がまさにそれだった）。同カードの選択肢③は**シグニ**版なので、②③を続けて撃って**対象が入れ替わらない**ことも見る。

- **🆕 続き416〜417（§6.4 任意性脱落の系統消化）が持ち込んだ未検証UI 4件（最優先）**＝engine 側は golden で固定したが、**任意コストの pay/skip モーダルが新たに出るカードが 140枚超**あり、実UIでの提示・支払い徴収は golden では踏めない。
  - [ ] **手札捨ての任意コスト（`OPTIONAL_COST{handDiscard}`）**＝`WXK09-041`（【自】アタック時「手札から＜天使＞のシグニを1枚捨ててもよい。そうした場合、対戦相手の手札を1枚見ないで捨てさせる」）で ①「任意コスト：支払いますか？」が出る ②**スキップで手札が減らず本体も走らない** ③支払うと**＜天使＞のシグニだけ**が選択候補になる ④**手札に＜天使＞が1枚も無いとき「支払う」が選べない**（`canAfford`）ことを見る。
  - [ ] **効果まるごと任意（`OPTIONAL_ACTIVATE`）**＝`WX07-003`（【出】「カードを1枚引いてもよい。そうした場合、手札を1枚捨てる」）で「発動する／発動しない」が出て、**発動しないなら引きも捨てもしない**こと。⚠**ON_PLAY 経路**（通常召喚と効果配置の両方）で出ることを確認する。
  - [ ] **自己トラッシュコスト（`OPTIONAL_TRASH_SELF`）**＝`WX24-P2-060`（【自】メイン開始時「対戦相手のパワー5000以下のシグニ1体を対象とし、このシグニを場からトラッシュに置いてもよい。そうした場合、それを手札に戻す」）で ①「このシグニを場からトラッシュに置きますか？」が出る ②**スキップなら自分のシグニも相手のシグニも動かない** ③支払うと**自シグニがトラッシュへ行ってから**相手が手札に戻ること。
  - [ ] **新設 `fieldDown`（アップ状態の自シグニをダウン＋色）**＝`WXDi-P04-051`（【自】ルリグアタック時「あなたのアップ状態の白のシグニ3体をダウンし《白》を支払ってもよい」）で ①アップ白シグニが**3体そろっていないと「支払う」が選べない** ②支払うと**3体ともダウンし**《白》も徴収される ③そのあとルリグがアップし能力を失うこと。あわせて `WXK08-052`（「このシグニの下にあるカード1枚をトラッシュに置いてもよい」＝`underAnySigniTrash{fromThis}`）で**他のシグニの下のカードが候補に出ない**ことを見る。
