...
| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-17`**（後半＝アタックの探索）。次に `S-22`（CPU の対象選び）・`S-24`（自己対戦がマリガンを踏まない）・`S-14`・`S-6`。🏁**§5.3 機構 worklist は残0** |
...
| 🔧**ゲート** | `npm run gates` 全緑（golden **4346**＝この回 +1）｜**実機 PASS**＝`verifyFullMatch.mjs cpu`（6ターン/133手）｜反転確認＝`planFor('guest')` を旧実装に戻すと golden `§5.7 S-23` が FAIL |
...
- **2026-09-20 時点**（第421バッチ＝§5.7 `S-23` 本物のデッキで測る／`S-20`・`S-19` 決着）
  ...
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4346**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS**（6ターン/133手）
