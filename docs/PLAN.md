...
- 🏁**セッション（2026-08-28・続き705／706・Opus 5 単独）＝`O-133` 完了。孤児 MANUAL スタンプ 402 → 12（B群 0）。**
  ユーザー指示＝**「pull する」→「回す（gates）」→「`O-133` の B を解消する」→「PARTIAL は解凍する／最後まで続ける」
  →「実バグだけ選んで parser 修正」→「次のバッチで区切る」**。全13バッチ（第3〜第15）。
  📊**孤児 MANUAL スタンプ 402 → 12**（**B 0** / A 0 / C 0 / **D 9**＝凍っていない ＋ parser 由来3）
  ...
  `keywordgained`／`revealDeckTopBanish` **ALL PASS**（`lookReorderCanTrash` の FAIL は本セッション起因ではない＝`V-89`）。
  - ✅**parser を15本以上直した**（いずれも「凍っていたので誰も気づけなかった」層。詳細は BUGFIXES.md）：
    任意コスト句の逆翻訳復元／ON_PLAY watcher の allowlist・語順・`levelParity`／「その」「この」照応の束縛／
    **ON_RISE の照応が `owner:'any'` の自由選択＋`PERMANENT` に化けていた**／`commonClass`／
    **公開文の前に置かれた対象宣言の限定（パワーN以下）が落ちていた**／ON_DRAW の `outsideDrawPhase`・`any_opp`／
    **「このシグニを手札に戻す」の owner がトリガー句に引きずられて恒久 no-op だった**／
    本文中の `BEAT_CONDITION`／ルリグトラッシュの `level.max`／**「代わりに」分岐の持続期間継承**。
  - 🔴**最大の発見＝凍結は「そのカードの他の効果」まで止める。** 収穫マージは**カード単位**で held に落とすので、
    golden に「原因不明」として長く登録されていた `WD22-029-G-E2` の parse 失敗は、**兄弟 `E1` の孤児スタンプが原因**だった。
    ⇒ **原因不明の parse 失敗を見たら、まず同じカードに孤児スタンプが無いか見る。**
  - 🔴**「デッキの一番上を公開する。それが〈条件〉の場合」族33件が旧設計のまま凍っていた**＝
    `O-35` の改善が1つも届いておらず、**`O-35` が名指しで直したカードすら旧形のまま**だった。27件を現行表現へ移行。
  - ⚠🔴**「B群 0」の読み方＝根治は176件、残り217件は「見えない凍結」→「出所のある凍結」への格下げ。**
    計器の定義は「live が MANUAL/PARTIAL なのに `manualEffects.ts` に定義が無い」なので、**移設すれば定義上 0 になる**。
    **移設分は凍ったまま**で parser 改善は届かない。変わったのは**どの計器にも映らない状態を脱した**ことだけ。
    ⇒ **根治の続きは `censusManualDrift` の「削除候補（parser 出力と実体同一）」**（現在 0＝まだ parser は再現できていない）。
  - ✅**217効果を `manualEffects.ts` へ逐語移設**（第12・14・15バッチ）＝**parser が原理的に出せない専用 STUB**／
    **表示情報（CHOOSE の説明的ラベル）を live だけが持つ**／**カード固有の表現**。
    **live の値は1バイトも変えていない**（A/B で実体変化0を実測）＝凍結は続くが**出所が生まれ**、
    以後 `censusManualDrift` が監視する（parser が追いつけば「削除候補」に載る）。
  - ✅**計器を3つ較正**＝①`--unfreeze` が明示 id でも B を拒んでいた ②**parser 自身が同じ印を出す効果**を母集団から除外
    ③`BASELINE_ORPHAN_MANUAL` を 12 へ（以後この数が増えたら「また出所の無いスタンプを押した」合図）。
  - 🔴**手順の穴を潰した**＝`--unfreeze` → `build:effects` だけでは live は旧値のまま残る
    （自動採用は「純粋上位集合」だけ。キーの削除・差し替えは `_held_fresh.json` に落ちる）＝**4段手順**を O-133 に明記。

**▶ 次の一手**＝**`censusManualDrift.ts` の「削除候補（実体同一）」**を定期的に見る
（`O-133` で 217効果を `manualEffects.ts` へ移したので、parser が追いつくたびにそこへ載る＝畳みどころ）。
