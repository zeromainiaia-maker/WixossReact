...
- **`npm run census`（`scripts/vocabCensus.ts`）**＝語彙センサス＝**両方向94計測**（原文修飾句76パターン＋数値/構造/逆方向18計測）×JSON対応語彙の突き合わせで**過剰効果（フィルタ/条件/制限/構造の脱落）と幻覚（原文に無い効果/数値）**を検出（既存網の死角＝盤面が変化するバグ）。高シグナル2023枚ベースライン（続き18拡張後）・超過で exit 1・明細 `docs/_vocab_census.txt`。
