...
- **セッション（2026-08-24・続き637・Opus 5）＝§5.1 の 🅱／🅲 を残0クローズ（V-84/85/83/04/16/87/19/86）→ そのまま §5.3 の O-47／O-48 も残0クローズ**。新規実機シナリオ **24本**／**engine の実バグ4件を発見・修正**／gates 全緑（**golden 2654**）。
  - 🔴**O-47 バトルで負けた／相打ちのアタッカーがバニッシュされない**（`else` が `「…はバトルに敗北」` とログするだけ）＝**ルールの根幹で全対戦に効く**。盤面だけでなく **ON_BANISH／ON_LEAVE_FIELD もアタッカー側へ配線**した（飛ばすと「消えるのに誰も反応しない」新しい穴を作る）。🎁**進行不能バグ2件（`wd16016BurstOpponentDiscard`／`forcedAttackAdvancesAfterAllAttacked`）が同時に解けた**＝どちらもアタッカーが消えないせいで止まっていた。
  - 🔴**O-48 バトルのバニッシュがスタックごとエナへ行く**＝下にあったカードがトラッシュに落ちず `fromLeftFieldUnder`（4効果）がバトル経路で恒久 no-op。**engine 側の効果バニッシュは元から正しい**＝壊れていたのは BattleScreen だけ。
  - 🔴**V-19 一時レゾナのカード消失**（ターン終了の出口3つのうち通常枝だけ `lrig_deck` への戻し入れが欠落）／🔴**V-83 `TRASH{DECK_CARD}` がミル発生源を書かず誘発が恒久 no-op**（＜悪魔＞の自デッキミルは35枚以上がこの経路）。
  - **V-04 は母数を実測で確定**＝`planEnergyPayment` は **15サイト**（資料の 17/14/13 はどれも支払いサイト数ではなかった）。**全サイトが `applyTo` を呼ぶこと**を golden の静的ゲートで固定＝**実機は1経路ずつしか踏めないので母集団は静的に守る**。
  - 🔑**engine のルールを直す前にベースラインを取る**のが今回いちばん効いた（§4.4 の 8j）＝修正後の赤を「自分が壊した／元から赤い／盤面が旧挙動前提で腐った」の3つに切り分けられた。

**▶ 次の一手**＝**§5.1 🅳 を上から取る**（`V-30` 境界越え → `V-35` → `V-44(b)(c)(d)` → `V-45(d)` → `V-58` 済 → `V-63`）。残りは **V エントリ 6件＋続き616〜634 の21バッチ**。⚠**§5.3 O-49**（バニッシュ先変更のアタッカー側ミラー）も新規在庫。
...
- **2026-08-24 続き637 後（本行が直近の正）**：
  ...
  **実機シナリオ定義総数 500**（+24＝V-84/85/83/04/16/87/86/O-47/O-48。V-19 は既存4本の修理）、version **0.502**。（⚠`census:goldentypes` は続き552d 以降 未再計測）
