...
### 📌 次の一手（推奨順）
> まず `npm install` → `npm run typecheck && npm run golden && npm run smoke && npm run fuzz` が全部緑になることを確認（CIでも自動実行される）。これが回れば環境OK。現状＝golden 131/131・smoke/fuzz 全0・同型★0・census 1765/1765・parserWorklist held 24（LOSS12/VALUE12）。
>
> **現在の主作業＝§5c census文型バッチの継続消化（続き23確立のパイプライン・cold startはこの5行）**：
> ① `npm run census:clusters` → `docs/_census_clusters.txt` を枚数順に見てテンプレを選ぶ（未処理上位＝「代わりに」昇格置換126枚＝else付きCONDITIONALのsequence層対応が要る最大の構造系。機構待ちは §6.3 へ送る。⚠着手前に続き24-27の型＝**全数機械分類で偽陽性を先に切る**：「それが＜C＞73枚」は70枚・「次にダメージ46枚」は38枚が既存表現済みの偽陽性だった。「場に《X》13枚」続き26・「ベット9枚」続き27はいずれも偽陽性0＝全実バグ。**「代わりに」は逆に構造が要る＝機械分類で追加/置換を先に仕分ける**）
