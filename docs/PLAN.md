...
| 🆕(xxii) | **後置条件節の IS_MY_TURN 誤変換（当初127件）**＝続き143〜212 の12バッチ＋続き241＋続き283で系統消化（LAST_PROCESSED 系条件の一般化・`STATE_CONDITION_CLAUSES` 拡充・`unwrapWrappedRecorder` 等）。**残39件**（fresh report 実測。段3着手前41→39）＝続き283で `TRANSFER_TO_HAND` の「手札に加え」parser語彙と ALL 即時経路の結果記録を配線し SPDi44-04-E2／WX25-P1-026-E2 を消化。WXDi-P01-082 は前段ラップ内の移動成功を証明できず honest defer。残は前段 STUB（非記録アクション）か新規状態追跡機構（ビート数/リミット/ターン累計エナ/did-search/アタック幾何/追加コスト・色支払い）が要る。全41件の [A]/[B]/[C] 再分類は `docs/_partial_triage.txt` 2026-07-27段3節、現行明細は `docs/_partial_report.txt`。 |
...
- **C. IS_MY_TURN 誤変換の未消化サブ系統**＝**残39効果**（2026-07-27 fresh parser 実測。stale「残53」を訂正、段3開始41→続き283で2効果消化）。公開系 REVEAL 前段／エナ置き／デッキ加え／単一カード公開判定。全数分類は `docs/_partial_triage.txt` 2026-07-27段3節。
