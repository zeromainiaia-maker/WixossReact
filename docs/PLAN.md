...
**直近＝2026-09-21＝🏁§5.7 `S-14`＝コンボに「使い方」を持たせた（出す／【起】で使う／アーツで撃つ／スペルで使う）**（全文は [BUGFIXES.md](./BUGFIXES.md) 第438バッチ）
- **コンボ＝順番に打つ手の列**（2手に限らない）＝`{num, use}`。🔴**旧形 `{first, then}` は「出す → 出す」として読める**（保存済みを壊さない）。
- 🔑**「済んだ」の判定は使い方ごと**（場にいる／このターンその【起】を使った／ルリグトラッシュ／トラッシュ）。
  **`available` にトラッシュとエナを入れた**＝入れないとトラッシュ【起】のコンボが永久に動かない。
- **編集 UI**＝「カード」＋「使い方」を選んで **手を足す → コンボに追加**。**配線**＝探索の `moveBonus` → `cpuPlanMoveStep` → `planUseBonus`。
  🔴**`effectId` からカード番号を正規表現で削り出さない**（接尾辞が開いた集合＝実測95件外れる）＝`choice.cardNum`／ルリグはセンターで代用。
- 📏**実測**＝コンボが判断を変えた対戦 **6/6**（最初の分岐 12手目）。✅`WD06` リュウグウの【起】5〜6回/戦・✅`WD08` ネビュラのトラッシュ【起】を毎戦。
  ⚠🔴**`WD16` は動かない**＝その【起】のコスト（手札から選んで捨てる）を**CPU がそもそも払えない**⇒ §5.7 `S-31`。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | 🔥**§5.7 `S-31`**（CPU の作戦をもっと細かく＝ユーザー要望）。次に `S-29`。🔵`S-6` 第3段は「大きく振る」なら |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（**live JSON を1バイトも触っていない回＝3計器は動かない**） |
| 📦**在庫** | 🏁**機構 worklist 0**／実機 `V-nn` **1**（`V-283`）／実装キュー **0**／CPU 完成度 **0**／**CPU の強さ 2**（🔥`S-31`・`S-29`＝🏁`S-14` を消化）／リリース作業 **1**／**作戦データの入力 21デッキ**（ユーザー作業） |
| ⚠**直近の不具合** | 🏁**未修正で登録済みのものは無い**。⚠**`WD16` のコンボが動かない**＝`S-31`（CPU が払えないコスト） |
| ⚠**バグ報告** | 🏁**未消化 0**。次は必ず **`--replay` から**（§5.6.4b の 3.） |
| 🔧**ゲート** | `npm run gates` 全緑（golden **4357**）｜**実機 PASS**（7ターン/175手/230s ＋ `v268CpuDeckPlan`） |
...
| 順 | キュー | 残 | 中身 | 測り直すコマンド |
|---|---|---|---|---|
| **①** | **§5.1 実機 `V-nn`** | **1** | `src/screens/` を触った回・機構を足した回の返済先 | §5.1 の表 |
...
| 🔥`S-31` | 🆕**CPU の作戦をもっと細かく設定できるように**（2026-09-21 ユーザー要望「今後はさらに CPU 作戦を細かく設定できるといいね」）。📏**`S-14` の下書きで出てきた「いま書けないもの」3つ**＝①**「N体並べる」条件**（`SP06`／`SP07`／`天使軸1`＝「名前の異なる＜原子＞3体で全体＋2000」等＝**2枚の順序では表せない**・実測3デッキ）②🔴**CPU が払えないコスト**（`WD16` の `WX09-048` Ｆ・Ｍ・Ｓ の【起】＝「手札から＜電機＞を1枚捨てる」＝`cpuActivate.ts` の `CPU_AUTO_PAYABLE_COST_KEYS` に `discard` が無く**そもそも撃てない**＝コンボを書いても動かない）③**条件つき**（相手の盤面・ライフ・ターン数で切り替える）／**アーツの使いどころ**（守り／攻め）／**使わない札**の指定。⚠**②は作戦データの話ではない**（CPU が撃てる範囲の拡張）＝先に取るべき。⚠**着手前に「どれが欲しいか」をユーザーに確かめる**（①〜③で実装が全く別物）。 | 中〜大 | 作った CPU デッキを意図どおりに回せる |
...
- **2026-09-21 時点**（第421〜438バッチ＝§5.7 `S-23`／`S-20`・`S-19` 決着 ＋ 🏁`S-17` 第1〜3段 ＋ 🏁`S-25`・既定の引き上げ ＋ 🏁`S-6` 第1〜2段 ＋ 🏁`S-27`・`S-26`・`S-28`・`S-22`・`S-24`・`S-30` ＋ 🏁`S-14`）
  - 📊**進捗3計器**＝Sheet1 要対応 **1 / 863**｜意味照合 段2 台帳 残 OPEN **0**｜census 高シグナル **1 / BASELINE 1**（**live JSON 未変更＝3計器は動かない**）
  - 📦**在庫**＝🏁**機構 worklist 0**｜実機 `V-nn` **1**｜実装キュー **0**｜**CPU 完成度 0**｜**CPU の強さ 3**（🔵`S-6`・🆕`S-31`・`S-29`）｜**作戦データの入力 21デッキ**（ユーザー作業）｜リリース作業 **1**｜実機シナリオの既存 FAIL **0**｜🏁**未消化のバグ報告 0**
  - 🔧**ゲート**＝`npm run gates` 全緑（golden **4357**・`census:traceinv` I1=0・`selfplay` 外れ0）｜**実機＝`verifyFullMatch.mjs cpu` PASS 9本**（6/133手・8/191手・7/174手・7/170手・7/161手・6/139手・7/164手・7/165手・8/177手・**7/175手**）
