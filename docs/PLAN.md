...
- **🆕 2026-08-16 続き503（§6.4 **O-28 クローズ**＝キーワードのゴミ綴り 残0）後 最新値（本行が直近の正）**：census **823**（825→823。`BASELINE_HIGH` 実数更新済み）、**golden 2088**（+6＝綴り正準化のトリップワイヤ1・`normalizeKeywordName`/`textHasKeyword`/`hasKeyword` の綴り吸収1・文がキーワードに入っていないトリップワイヤ1・引用【常】3形の live 不変条件1・`selfHandLte` の対1・`DOUBLE_OWN_POWER_MINUS` の owner 1。ほかに `WDK08-Y11-E2` の綴り assert と `WX25-P2-071-E1` の「壊れた形を固定していた」assert を正方向へ更新）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **102枚 / 44群**（103→102）、lint **0 errors / 260 warnings**、`census:stubs` A群＝**18種/22件**（15種/17件から +3種／+5件＝**明示 defer にした分**。無言 no-op は 0 のまま）・C群 **172種/253箇所 据置**、`census:goldentypes` **未カバー 0型**。🆕**live JSON changed 38効果 / 37カード**（CSV 非改変）。🆕**新機構＝`normalizeKeywordName`／`textHasKeyword`（キーワード綴りの照合 funnel）／`AssassinScope.selfHandLte`／明示 defer 3種（`DEFERRED_CONVERT_ENERGY_COLOR`／`_ATTACK_WHILE_DOWN`／`_LEAVE_FIELD_REPLACE_WITH_DOWN`）**。🔧`DOUBLE_OWN_POWER_MINUS` の owner 是正。⚠**全経路 実機未検証**（§7 送り）。
