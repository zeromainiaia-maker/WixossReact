...
**直近＝2026-09-17＝§5.6 `C-8` クローズ＝CPU の対話応答を純関数＋方針つきに（§5.6 の `C-nn` はこれで全部🏁）**（全文は [BUGFIXES.md](./BUGFIXES.md)）
- 🔑**ユーザー決定＝混合**＝損得が分かる選択は得な方（相手の強いシグニを除去・自分の強いシグニを強化・失うなら弱い札から）／断る肢があればする（払えるなら払う）／「AかB」は seed 付き乱数で**両側の分岐を踏む**。
- 判断は `src/screens/battle/cpuInteraction.ts`（golden `§5.6 C-8`＝反転で FAIL）。`BattleScreen` の直書き（「押せる先頭」固定・対象は完全ランダム）を撤去。
- 🔴**踏んだ罠**＝engine の「してもよい」は大半が `declines` の印を持たず、**ID（`skip` 64箇所・`none`・`no`）とラベル**で断る側を表す＝印だけで判定すると「支払う／支払わない」が乱数になり、**CPU の応答に頼る実機6本が FAIL** した ⇒ `isDeclineOption`。
- 🔴**古い実機シナリオを1本直した**＝`wxk06067CrossZoneStubFires` は消えた「無料 pay 枝」を合格条件にしていた（HEAD でも FAIL）⇒ 原文どおり「手札＋エナ合計2枚を置いてシグニを守る」へ。
- 実機＝🆕`V-267` PASS／CPU の応答に頼る既存11本 PASS／CPU 通し対戦（MECH）PASS。`census:play` に「選択肢：する側／断る側」を追加（この1戦では選択肢が回ってこず0回）。

| 軸 | いまの値 |
|---|---|
| 🔥**次に取るもの** | **§5.7 `S-1`**（カードの強さ表）→ `S-2`（CPU デッキの作戦データ）→ `S-3`（1ターン分のシミュレータの見積もり） |
| 📊**進捗3計器** | Sheet1 要対応 **1 / 863**／台帳 残 OPEN **0**／census 高シグナル **1 / BASELINE 1**（今回は UI 層＝3計器の対象外） |
| 📦**在庫** | 機構 worklist **0**／実機 **0**／実装キュー **0**／**CPU 完成度 0**（🏁`C-0`〜`C-8`）／ルール台帳 ⚠0（残件0）／**リリース作業 1**（RELEASE.md） |
...
| 🔥**④b** | 🆕**§5.7 CPU の強さ `S-nn`** | **3** | 探索＋評価のゲーム AI（対戦中に LLM は呼ばない） | §5.7.1 の表 |
...
| ID | 内容 | 規模 | 何が良くなるか |
|---|---|---|---|
| 🔥`S-1` | **カードの強さ表**＝効果 JSON（構造化済み）から除去・ドロー・エナ・サーチ・耐性・強化などの**特徴量**を全カード分取り出し、パワー換算の強さにする（原文 regex は書かない） | 数時間〜1日 | 召喚する札・除去の対象（相手の脅威度＝パワー＋効果）・サーチ・捨て札の選び方が「パワーだけ」から脱する。特徴量は `S-4` の機械学習の入力にもなる |
| `S-2` | **CPU デッキごとの作戦データ**＝キーカード（手札に残す・エナに置かない・マリガンで探す）／コンボ（「A の後に B」「A が場にいるとき C」）／優先順位。デッキ編成の［CPUデッキ］タブで編集、`decks` に保存 | 1〜2日 | デッキ固有の動き・コンボ（探索なしで決まる）。下書きは Claude Code がデッキの原文から作れる |
| `S-3` | **1ターン分のシミュレータの見積もり**＝`BattleScreen` のターン進行（フェイズ・誘発の収集・効果の順番決め・支払い）のうち、画面から切り離す範囲を実測して工数を出す（**まず調査だけ**＝§5.6.6 の「数字で言えてから」） | 調査 | 先読み（「この順で出したら」を試す）と機械学習の土台。ターン進行のバグ探しにも使える |
| `S-4` | （`S-3` の後）先読み＋自己対戦で評価の重みを調整（機械学習①） | 大 | 数手先・勝率で測れる強さ |
