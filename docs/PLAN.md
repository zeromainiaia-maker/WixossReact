...
| ID | 何をするか | 出所 |
|---|---|---|
| `S-36` | **作戦の「効果の狙い方」を拡張する**＝いまの狙い方は3つ（強いもの／落とせるもの／弱いもの）と条件4つだけ。2026-09-25 に**効果単位（E1/E2/ライフバースト）**で書けるようにしたので、次は狙い方・条件の語彙を増やす。受け皿＝`resolveCpuTargetMode`／`pickCpuTargets`（`cpuDeckPlan.ts`／`cpuInteraction.ts`） | ユーザー（2026-09-25「良い実装なので後で拡張する」） |
