...
- **🆕 セッション（2026-08-15・続き489・Opus 5）＝§6.4 **O-3 の `NEXT_OPP_ATTACK_PHASE_START`（2効果）を解体＝5効果の挙動是正**（**過剰実行3＋対象取り違え1＋永続化バグ1**）。ゲート全緑（**golden 2017**＝+5・**census 831 据置**・lint 0/259）。
  - **🔴① 受け皿 id は母集団の一部しか映さない（今回いちばん効いた計測）**＝原文「次の対戦相手のアタックフェイズ開始時」を regex で数えると**5効果**あり、A群（`DEFERRED_NEXT_OPP_ATTACK_PHASE_START`）に出ていたのは**2件だけ**だった。残りのうち `WX25-P2-051-E2` は **即時に相手の全シグニをレベル無制限でダウン**、`WXK01-003-E3` は **自分のルリグ**をアタック不可にしていた＝**A群の2件より壊れていた**のに、`STUB{SOUL_OP}`／`STUB{PREVENT_ATTACK_UNTIL_OPP_ATTACK_PHASE}` という**ハンドラ持ちの汎用 id**へ落ちていて計器に映らなかった。**着手時は必ず原文 regex で母集団を数え直す。**
  - **🔑② 遅延予約は「予約した側」に積み、collector は `opState` を読む**＝新機構 `DELAY_TO_NEXT_OPP_ATTACK_PHASE`。`ON_ATTACK_PHASE_START` は**自分のアタックフェイズでも走る**ので `myState` を見ると同一ターン内で即発火する。走査軸は既存 `pending_opponent_attack_facedown_returns` と同じ（`playerId: opId` の合成エントリ）。⚠ターン境界を跨ぐので `turnScopedState` にも `delayed_triggers`（THIS_TURN 限定）にも**載せない**。
  - **🔑③ 遅延本体は `splitSentences` の前に全文で切り出す**＝`SPDi43-24-E2` の本体は「…1体を対象とする。このターン、それがアタックしたとき、…」の**2文**。文単位に切ると本体が後続ステップとして並び、予約した瞬間に走る（続き488 と同じ壊れ方）。句より前（「裏向きでルリグゾーンに置く」）は**いま実行する**ので `SEQUENCE[prefix, DELAY{body}]` に組む。
  - **🟢④ 本体の機構はほぼ既存で足りた**＝`SPDi43-24-E2` は `NEGATE_ATTACK{CENTER_LRIG_OR_SIGNI, escapeDiscard:3}`（「手札3枚を捨てないかぎり」の**回避モーダルまで実装済み**）、`WX25-P2-051-E2` は既存 `levelEqLastProcessed`、`WXK01-003-E3` は `GRANT_KEYWORD{CENTER_LRIG_OR_SIGNI}`。**新規に要ったのは遅延予約と裏向きルリグゾーン（`PLACE_FACEDOWN_LRIG_ZONE`／`REVEAL_FACEDOWN_LRIG_ZONE`）だけ**＝在庫を開く前に「既存で書けないか」を先に確かめると新機構が半分で済む。
  - **🔴⑤ `negated_attacks` が永続していた（続き487 の教訓の再発）**＝型コメントに「このターン次にアタックしたとき無効にされる」と書いてあるのに**失効地点が1つも無い**。消費は「そのカードがアタックしたとき」だけなので、**狙われた側がアタックしなければゲーム終了まで残る**（`SPDi43-24-E2` は防御札なので必ず踏む）。`negated_attacks`／`negated_attacks_escape` を `turnScopedState` へ登録して解消。**「期間つき」と書いてあるフィールドは失効地点を grep して実測する。**
  - **⚠⑥ 計器の較正は narrow なキーで行う**＝census が +4 したので4クラスタを較正したが、「トリガー:アタックしたとき」に `NEGATE_ATTACK` を鍵として足すと**既存の高シグナル11件を丸ごと隠す**。実測して `escapeDiscard`（live 15件・うち当該クラスタの高シグナルは対象1件のみ）へ絞った。**較正キーは追加前に「何件を消すか」を数える。**
  - **▶ 次の一手**＝**O-3** の残8（A群が常時の在庫表）。`ATTACK_TAX_HAND_DISCARD`（`SP38-003`＝アタックごとの手札3枚捨て。⚠今回入れた `escapeDiscard` は**1回きりの回避**なので流用不可）／`SKIP_NEXT_TURN`／`UNTIL_NEXT_MAIN_PHASE_CLAUSE` など。→ **O-4**（UNKNOWN 25ノード/25カード）→ **O-25 の残** → 🆕**O-26**（コスト節の自己トラッシュ脱落2件）。

### 📊 恒久指標（最新1件のみ・履歴は PLAN_DETAIL）

- **🆕 2026-08-15 続き489（§6.4 O-3 `NEXT_OPP_ATTACK_PHASE_START` 解体）後 最新値（本行が直近の正）**：census **831 据置**（⚠一度 835 まで増えたが計器較正4件で戻した＝`DELAY_TO_NEXT_OPP_ATTACK_PHASE`／`levelFromLastProcessed`／`escapeDiscard`×2。**`NEGATE_ATTACK` を鍵にすると既存11件を隠すので narrow なキーに絞った**）、**golden 2017**（+5＝新機構3・live 形1・永続化バグのトリップワイヤ1）、smoke **10688 / SKIP 0**、fuzz 全0、**同型★ 0**（265群）、held **107枚 / 48群**、lint **0 errors / 259 warnings**（据置）、**UNKNOWN 25ノード / 25カード**（据置）、`census:stubs` A群＝**無言 no-op 0**（据置。明示 defer は **28種/31件 → 27種/29件**）。🆕**live JSON changed 4カード**（`SPDi43-24`／`WX24-P3-014`／`WX25-P2-051`／`WXK01-003`。CSV 非改変）。🆕**挙動是正 5効果**（過剰実行3＋対象取り違え1＋永続化バグ1）。🆕**新機構＝`DELAY_TO_NEXT_OPP_ATTACK_PHASE`／`PLACE_FACEDOWN_LRIG_ZONE`／`REVEAL_FACEDOWN_LRIG_ZONE`／`SIGNI_ATTACK_BAN.levelFromLastProcessed`**。🆕**`negated_attacks`／`negated_attacks_escape` を turnScopedState へ登録**（永続バグ解消）。
