export type ViewMode = 'LOGIN' | 'START' | 'DECK_LIST' | 'DECK_EDITOR' | 'MATCHMAKING' | 'BATTLE' | 'SPECTATE';
...
export interface PlayerState {
  ...
  /**
   * 🆕**`crash_to_trash_instead` の寿命が「そのアタックの間」だけ**という印（§5.3 `O-367`・2026-09-14）。
   * `WX19-034-E1`「【自】：あなたのセンタールリグがアタックしたとき、…**そのアタックの間**、
   * クラッシュされたカードはエナゾーンに置かれる代わりにトラッシュに置かれる」。
   * ⚠**読み手を増やさない**＝消費地点（`BattleScreen` のクラッシュ解決）は `crash_to_trash_instead` だけを見る。
   *   この印は `clearEndOfAttackEffects` が**アタック終了時に本体ごと落とす**ためだけに在る。
   ...
   * `clearEndOfAttackEffects` がアタック終了時に**ここに控えた分だけ `keyword_grants` から引く**。
   * 🔑**別ストアにしなかった理由**＝キーワードの読み手は engine と UI に多数あり、
   *   新しいストアを作ると必ず読み漏れが出る（§4.2「書き込み先を誰が読むか」）。
   */
  keyword_grants_this_attack?: Record<string, string[]>;
  /**
  ...
  /**
   * 🆕**§5.3 `O-236`（2026-09-04）＝ルリグの「1ターンにアタックできる上限」。**
   * `WXDi-D04-011-E1`（ENDLESS-PUNCHLINE）「このルリグは**ダウン状態でもアタックでき**、
   * １ターンにこのルリグが**アタックできる上限は３になる**」。
   * 🔴既定は「1回」＝`lrig_has_attacked`（真偽値）で表していたので、**2回目以降を表せなかった**。
   * ⚠**上限が未設定なら従来どおり**（真偽値の門だけが効く）＝既存カードの挙動は変えない。
   */
  lrig_attack_limit_this_turn?: number;
  /** 🆕§5.3 `O-236`＝そのターンにセンタールリグがアタックした回数（上限つきのときだけ数える）。 */
  lrig_attack_count_this_turn?: number;
  /** 🆕§5.3 `O-236`＝ルリグが**ダウン状態でもアタックできる**（シグニ用の `ATTACK_WHILE_DOWN` とは別軸）。 */
  lrig_attack_while_down_this_turn?: boolean;
  /**
   * 🆕**§5.3 `O-246`（2026-09-04）＝「このターン、あなたの効果によってあなたのデッキの上から
   * **特定の値の枚数**のカードを公開する場合、代わりに**1枚多く公開してもよい**」（`WX06-033-E1`）。**
   * 🔴旧 `STUB{GRID_REVEAL_PLUS}` は `grid_reveal_plus_one_this_turn` を立てるだけで**読み手が0人**だった。
   * ⚠**「してもよい」＝公開のたびに問う**（このフラグは「問う権利」であって自動加算ではない）。
   */
  reveal_count_plus_one_this_turn?: boolean;
  /**
   ...
   * 🔴**原文の要点は「許可の期間」**＝解決時に使うのではなく、**そのターンのうちなら後で使ってよい**。
   *   旧 live は `PLAY_FREE{source:'trash'|'opp_trash'}` で、engine の実体は `ADD_TO_HAND`＝
   *   **選んだスペルが手札に来る**（相手のトラッシュからも奪う）＝原文と別物だった。
   * ⚠**`from` は「どちらのトラッシュに在るか」**＝`'opponent'` の1枚は**相手のトラッシュに置いたまま**使う。
   * ⚠ターン終了でクリア（`turnScopedState.ts`）。
   */
  trash_spells_usable_this_turn?: { cardNum: string; from: 'self' | 'opponent' }[];
