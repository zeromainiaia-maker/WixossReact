...
export type CardTypeFilter =
  | 'シグニ' | 'ルリグ' | 'アーツ' | 'スペル'
  // 🆕`'ピース/クラフト'`（§5.3 `O-321`①・2026-09-11 第276）＝CSV の `Type` はピース系が**3値**
  //   （`ピース` / `リレーピース` / `ピース/クラフト`。live 実測 119 / 2 / 1 枚）。
  //   ⚠`matchesFilter` の `cardType` は**完全一致**なので、`isPieceCardType` と同じ集合を書けるようにする
  //     （2値だけだと「ピースを使用していた場合」が `ピース/クラフト` を取りこぼす）。
  | 'キー' | 'ピース' | 'リレーピース' | 'ピース/クラフト' | 'アシストルリグ' | 'レゾナ';
...
export interface TargetFilter {
  ...
  /**
   * 🆕**あなたのセンタールリグのレベルより低い**（2026-09-07・`WX09-037-E1`）＝`levelLtOppLrig` の**鏡**。
   * 原文「**あなたの**センタールリグより低いレベルを持つ〈対象〉」は live 実測 **1効果**だけだが、
   * `levelLtOppLrig`（＝対戦相手側）とは**参照するルリグが逆**で、流用すると別のカードを対象にする。
   * `resolveDynamicFilter` が `ownerState` の中央ルリグ（`field.lrig` 頂点）のレベル −1 を `level.max` へ解決。
   * ⚠**参照不能／レベル非数値なら制限なしへフォールバック**（＝fail-open。`levelLtOppLrig` と同じ規約に揃えた）。
   */
  levelLtOwnLrig?: boolean;
...
export interface StubAction {
  ...
  trashAllScope?: {
    ...
    keys?: boolean;
    /**
     * 🆕**§5.0（2026-09-08・`WX16-033-E1`）＝「〈owner〉の**すべてのシグニゾーンにある、すべてのカード**を
     * トラッシュに置き」。** シグニ本体とその下のカード（`zones:['signi']` が既に流す）に加えて、
     * **シグニゾーンに同居する付随カード＝【チャーム】／【アクセ】／【トラップ】**もトラッシュへ送る。
     * ⚠**既定（未指定）は従来どおり付随カードを残す**＝「すべてのシグニをトラッシュに置き」型
     * （`WXEX2-21-E3` ほか）の意味を変えない。
     */
    zoneAttachments?: boolean;
    /** 影響を受けるプレイヤー。 */
   ...
   * 🆕**`'both_lrig_trash'`＝両プレイヤーのルリグトラッシュ**（§5.3 `O-259` 第10・2026-09-07）。
   *   選択後はカードが実際に在った側を見て、自分側／相手側の既存使用経路へ委譲する。
   * ⚠`value` は段階（`'picked'`）に使っているので、領域は別キーで運ぶ。
   */
  value2?: string;
  /**
   * 🆕**「それの使用コストは《青×2》減る」＝効果で使わせるスペルの軽減**（§5.3 `O-259` 第3・2026-09-05）。
   * `USE_SPELL_FROM_TRASH_PAYING_COST` が印刷コストを色の配列へ展開したあと、**同じ色を書かれた数だけ
   * 取り除いてから**支払わせる。
   * 🔴**色は一致する分しか消えない**＝《青×2》の軽減は《無》を減らさない（原文どおり）。
   * ⚠**無いと従来どおり印刷コスト満額**（fail-closed＝安くならないだけで、嘘の割引は生えない）。
   */
  useSpellCostReduction?: EnergyCost[];
  /** 「コストをN倍支払って使用する」。印刷コストを色配列へ展開した直後に配列全体をN倍する。 */
  useSpellCostMultiplier?: number;
  /**
   * 🆕**「コストを支払わずに使用してもよい」**＝同じ選択経路のまま支払いだけを飛ばす
   * （§5.3 `O-259` 第3・2026-09-05・`WX20-059-E2`）。
   * 🔑**別 STUB を立てない**＝「手札から候補を選ばせる」部分こそが壊れていた本体で、
   *   支払いの有無は最後の1分岐でしかない。
   */
  ...
  /**
   * 🆕**§5.3 `O-281`（2026-09-08）＝「それを（コストを支払わずに）**限定条件を無視して**使用する」**
   * （`WXK09-002-E1` の1効果だけ）。`USE_SPELL_FROM_TRASH_PAYING_COST` の候補絞り込みで
   * `meetsRestriction` を**飛ばす**印。
   * 🔴**既定（未指定）は「限定条件を検査する」**＝旧実装は engine のどこでも `Restriction` を見ておらず、
   *   この STUB を使う live 12効果すべてが限定条件を無視して使えていた（11件が過剰実行）。
   * 🔑**同名の `PlayFreeAction.ignoreRestrictions`（`O-264`）と同じ意味・同じ判定関数**を使う。
   */
  ignoreRestrictions?: boolean;
  /**
   * 🆕**§5.3 `O-260`（2026-09-05）＝「（このターン、）それがチェックゾーンから別の領域に移動される場合、
   * 代わりにゲームから除外される」**（7効果）を**使用そのものに載せる**フラグ。
   * 🔴**別ステップ（`SEQUENCE` の次の `STUB{EXILE_FROM_CHECK_ZONE}`）では届かない**＝
   *   この形は「候補選択 → 支払い CHOOSE → スペル本体の対象選択」と**対話が3重にネスト**し、
   *   外側の SEQUENCE の残りステップが継続に載り切らずに落ちる（2026-09-05・実機で実測）。
   * ⇒ **使ったカードを配置する瞬間に `excluded` へ入れる**（`finalizeUsedCardPlacement` も
   *   `excluded` に在るカードは既定ゾーンへ置き直さないので、置換の実効と一致する）。
   */
  ...
  declareNameFilter?: TargetFilter;
  /**
...
export interface CardEffect {
  ...
  outsideMainPhase?: boolean;                        // 「あなたのメインフェイズ以外で」発生したイベントのみ
    ...
    /**
     * 🆕**`byEffect` の緩和**（§5.4 (b)・2026-09-06 第190バッチ・`WD21-017-E1`）＝
     * 原文「**効果か**レゾナの出現条件**によって**、バニッシュされるか場からトラッシュに置かれたとき」。
     * 🔴**`forResonaCondition` とは向きが逆**＝あちらは「レゾナ出現条件**のときだけ**」という**排他ゲート**で、
     *   こちらは「効果起因**または**レゾナ出現条件」という**和集合**。片方で他方を代用できない
     *   （排他にすると効果起因が落ち、`byEffect` だけだとレゾナ出現条件が落ちる＝どちらも原文とずれる）。
     * ⚠**`byEffect: true` と併記して使う**（この単独では何も緩和しない）。
     * ⚠**レゾナ出現条件が分かるのは場→トラッシュの collector だけ**なので、
     *   他経路（バニッシュ等）では従来どおり `byEffect` の意味のまま。
     */
    orResonaCondition?: boolean;
  ...
  kizunaIcon?: boolean;
  /**
   * 🆕**原文のアイコンが【出】だった**（2026-09-16・§5.3 `O-416`・**ユーザー判断＝読みB**）。
   *
   * 🔴**`effectType`／`timing` では【出】と【自】を区別できない**＝
   *   【出】も「【自】：このシグニが場に出たとき」も どちらも `AUTO` ＋ `timing:['ON_PLAY']` になる。
   * 🔑**ユーザー判断（2026-09-16）＝カードテキストの「【自】能力」は【出】能力を含まない。**
   *   ⇒ 「対戦相手のシグニの【自】能力が発動する場合、〈コスト〉を支払わないかぎり何もしない」
   *     （`SPDi43-01-E2`）のゲートは【出】に掛けてはいけない。読み手は `isSigniAutoAbility`。
   * ⚠**未設定（`undefined`）は「【自】として扱う」**＝`manualEffects.ts` 由来など marker を持たない効果は
   *   従来どおりゲートの対象（fail-safe＝既存挙動を変えない側）。**明示的に `true` のものだけ外す。**
   */
  onPlayIcon?: boolean;
