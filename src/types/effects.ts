...
export type ActiveCondition =
  ...
  | { type: 'HAS_CARD_IN_FIELD'; owner: Owner; filter: TargetFilter; excludeSelf?: boolean; minCount?: number; distinctNames?: boolean; distinctPhraseJa?: 'kinds' } // minCount: フィルタ一致シグニがN体以上あるか（省略=1。「＜美巧＞が3体あるかぎり」=minCount:3。WX04-004-E1）。distinctNames:true で名前が異なるものをN種以上数える（「それぞれ名前の異なる＜原子＞が3体」=minCount:3+distinctNames。WX12-Re01）。distinctPhraseJa:'kinds' は decompiler 表示用＝同じ述語の別語形「＜C＞のシグニがN種類以上ある」（WX25-CP1-041/045 等）を原文どおりに戻すためのヒント（意味は同一・engine は参照しない）
