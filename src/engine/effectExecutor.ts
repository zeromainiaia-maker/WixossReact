...
function execTrash(a: TrashAction, ctx: ExecCtx): ExecResult {
  ...
  if (tgt.type === 'ENERGY_CARD') {
    ...
    function applyTrashEnergy(selected: string[], c: ExecCtx): ExecCtx {
      const s = ownerState(tgt.owner, c);
      // PREVENT_ZONE_MOVE_BY_OPP: 相手効果でエナをトラッシュに移動させない（動的計算版）
      if (tgt.owner === 'opponent' && c.otherProtectedZones?.includes('energy')) {
    ...
    case 'STUB': {
      ...
      if (stub.id === 'SONG_FRAGMENT') {
        ...
        if (songCardsInEnergy.length > 1) {
          const internalSF: import('../types/effects').StubAction = { type: 'STUB', id: 'INTERNAL_SONG_FRAGMENT' };
        ...
        if (songEff) {
          ...
          return executeAction(songEff.action, addLog(songCtx, `【歌のカケラ】発動：${songCardData?.CardName ?? songCard}`));
        }
      ...
      if (stub.id === 'INTERNAL_SONG_FRAGMENT') {
        ...
        if (songEffISF) {
          ...
          return executeAction(songEffISF.action, addLog(songCtxISF, `【歌のカケラ】発動：${songCardDataISF?.CardName ?? selectedSF}`));
        }
      ...
      if (stub.id === 'CHOSEN_TO_ENERGY_OR_HAND') {
        ...
        const toHandCTEOH: import('../types/effects').StubAction = { type: 'STUB', id: 'INTERNAL_TRASH_TO_HAND' };
        const toEnaCTEOH: import('../types/effects').StubAction = { type: 'STUB', id: 'INTERNAL_TRASH_TO_ENERGY' };
        return needsInteraction(ctx, {
          type: 'CHOOSE', count: 1,
          options: [
