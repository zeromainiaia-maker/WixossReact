...
export function execStub(
  ...
  if (stub.id === 'INTERNAL_DECLARE_CARD_NAME') {
    ...
    return done(addLog({ ...ctx, ownerState: newOwnerIDCN }, `[${nameDCN}]宣言`));
  }
  ...
  if (stub.id === 'CHANGE_SIGNI_COLOR') {
    ...
    // 繝ｬ繝吶Ν繝輔ぅ繝ｫ繧ｿ・医後Ξ繝吶ΝN莉･荳九・繧ｷ繧ｰ繝九搾ｼ・    const toHWCSC = (s: string) => s.replace(/[０-９]/g, c => String.fromCharCode(c.charCodeAt(0) - 0xFEE0));
  ...
  if (stub.id === 'DO_THREE_THINGS') {
    ...
    // 縲娯蔵繝代Ρ繝ｼN莉･荳九・逶ｸ謇九す繧ｰ繝九ｒ繝舌ル繝・す繝･縲・    if (!logsDTT.length) {
      ...
      if (banishPwrM) {
        const toHWB = (s: string) => s.replace(/[０-９]/g, c => String.fromCharCode(c.charCodeAt(0) - 0xFEE0)).replace('荳・,'0000'');
