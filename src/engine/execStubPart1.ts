...
export function execStubPart1(
  ...
  // 🆕**§5.3 `O-351`（2026-09-13・第300バッチ）＝ラベルを実装に合わせた。**
  //   🔴旧ラベルの「またはシグニ+キー」は**このハンドラが一度も行わない挙動**（キーには触らない）で、
  //     しかも `WXDi-P05-007-E3`（原文＝「このターン終了時、**あなたの**手札とエナゾーンを流す」）が
  //     この id に同居していた＝**1つの id に2文型**。あちらは `DELAY_TO_THIS_TURN_END` ＋
  //     `TRASH_ALL_SIGNI_AND_KEY{trashAllScope}` へ移したので、**残るのは `WX11-020-E1` の1効果だけ**。
  //   ⚠新しいカードをここへ足さない（原文が「相手エナ全部＋相手シグニ全部」でないなら別の受け皿）。
