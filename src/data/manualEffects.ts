...
export const MANUAL_EFFECTS: Record<string, CardEffect[]> = {
  ...
  //   それが手札／エナに在るときだけ動く。原文が「**手札から**〜1枚を」「**ルリグデッキから**〜1枚を」と
  //   **別のカードを選ぶ**形だと、効果元は候補ではないので**恒久 no-op** になる（無言）。
  // 🔑受け皿は `AttachAcceAction` の2段選択（段1＝アクセ札／段2＝ホスト）＝`fromHand` は既存、
  //   `fromLrigDeck` を同バッチで新設した（列挙 `execAttachAcce` と除去 `applyDirectAction` の対で足す）。

  // ── WXDi-P09-007（コードイート　ドーナツ）E2
  //   原文＝【起】《無》：あなたのシグニ１体を対象とし、あなたのルリグデッキからクラフトであるシグニ１枚を
  //         それの【アクセ】にする。
  // ⚠E1 がルリグデッキへ足すクラフト3種（`ADD_CARD_TO_LRIG_DECK`）を、この E2 が拾って付ける対になっている。
  // ⚠「クラフトである」＝ルリグデッキに居るシグニは事実上クラフトだけなので、`signiFilter` は cardType のみ。
  "WXDi-P09-007": [
    {"effectId":"WXDi-P09-007-E2","effectType":"ACTIVATED","timing":["MAIN"],
     "cost":{"energy":[{"color":"無","count":1}]},
     "action":{"type":"ATTACH_ACCE","targetSigniOwner":"self","sourceOwner":"self","fromLrigDeck":true,
       "signiFilter":{"cardType":"シグニ"},"targetFilter":{"cardType":"シグニ"}},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  // ── WXK05-026（コードイート　ハイパーオムライス）E1
  //   原文＝【出】：あなたの＜調理＞のシグニ１体を対象とし、あなたの手札から＜調理＞のシグニ１枚を
  //         それの【アクセ】にする。
  // ⚠**ホストもアクセ札も＜調理＞**（`targetFilter` と `signiFilter` の両方に載せる）＝
  //   片方だけだと原文より広い／狭い別の効果になる。
  "WXK05-026": [
    {"effectId":"WXK05-026-E1","effectType":"AUTO","timing":["ON_PLAY"],
     "action":{"type":"ATTACH_ACCE","targetSigniOwner":"self","sourceOwner":"self","fromHand":true,
       "signiFilter":{"cardType":"シグニ","cardClass":"調理"},"targetFilter":{"cardType":"シグニ","cardClass":"調理"}},
     "duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],

  // ══════════════════════════════════════════════════════════════════════════════
  // §5.3 `O-283`（2026-09-08）＝「ルリグの能力」をコストなしで使う（カードを使う経路とは別）
  // ══════════════════════════════════════════════════════════════════════════════
  // 🔴**旧 live はどちらも `STUB{PLAY_FREE}`**＝あれは「**カード**を使う」経路で、対象は
  //   `lastProcessedCards[0]`（無ければ**効果元自身**）を `parseCardEffects` して
  //   **最初の ACTIVATED/【出】**を実行する。ルリグの能力は「カード」ではないので、
  //   `WX22-014-E3` は**同じカードの E1（【常】のリミット上昇）**を、`WX21-Re04-E1` は
  //   **そのアーツ自身**を撃とうとして噛み合わない＝丸ごと別の効果だった。
  // 🔑同バッチで足した `STUB{USE_OWN_LRIG_ABILITY_FREE}` が受け皿＝**エクシード能力だけ**を列挙し、
  //   複数あれば選ばせ、**能力の持ち主を `sourceCardNum` に据えて**コストなしで実行する。

  // ── WX21-Re04（サーバント　Ｑ２）E1
  //   原文＝対戦相手のターンの間、このカードの使用コストは《無》《無》《無》になる。
  //         あなたのルリグのエクシードの値が３以下の能力１つをコストを支払わずに使用する。（使用タイミングを無視する）
  // ⚠「**あなたのルリグ**の」＝センター＋アシスト（`lrigAbilityScope:'all_lrigs'`）。
  // ⚠「エクシードの値が**３以下**」＝`maxExceed:3`。⚠`altCostOppTurn` は旧 live のまま維持する。
  "WX21-Re04": [
    {"effectId":"WX21-Re04-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],
     "cost":{"energy":[{"color":"無","count":1}]},
     "altCostOppTurn":[{"color":"無","count":1},{"color":"無","count":1},{"color":"無","count":1}],
     "action":{"type":"STUB","id":"USE_OWN_LRIG_ABILITY_FREE","maxExceed":3,"lrigAbilityScope":"all_lrigs"},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  // ── WX22-014（幻怪姫　タウィル）E3
  //   原文＝【起】《ターン１回》《アタックフェイズアイコン》エクシード１：このルリグのエクシード能力１つを
  //         コストを支払わずに使用する。この能力はあなたの場に《差し伸べし者　タウィル》がいる場合にしか使用できない。
  // ⚠「**この**ルリグの」＝センタールリグだけ（`lrigAbilityScope` を付けない）。上限指定も無い。
  // ⚠使用条件（《差し伸べし者　タウィル》）とコスト（エクシード１）と《ターン１回》は旧 live のまま。
  "WX22-014": [
    {"effectId":"WX22-014-E3","effectType":"ACTIVATED","timing":["ATTACK_ARTS"],
     "condition":{"type":"HAS_CARD_IN_FIELD","owner":"self","filter":{"cardName":"差し伸べし者　タウィル"}},
     "cost":{"exceed":1},
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  // §5.3 `O-286`（2026-09-08）＝追加コストで「実際に払った色」を記録して分岐させる
  // ══════════════════════════════════════════════════════════════════════════════
  // ── WX21-Re18（無垢なる宝剣）E1
  //   原文＝このスペルを使用する際、使用コストとして追加でエナゾーンからカードを２枚以上トラッシュに置く。
  //         これの使用コストとして追加で白のカードがトラッシュに置かれた場合、対象の対戦相手のシグニ１体を手札に戻す。
  //         赤の場合、対象の対戦相手のパワー12000以下のシグニ１体をバニッシュする。
  //         青の場合、カードを２枚引き、手札を１枚捨てる。
  //         緑の場合、対象の対戦相手のパワー12000以上のシグニ１体をバニッシュする。
  //         黒の場合、あなたのトラッシュから対象のシグニ１枚を手札に加える。
  // 🔴旧 live は追加コストが `STUB{OPTIONAL_COST, costText}` の**生文字列**（engine は読まない）で、
  //   5つの分岐が**全部無条件に順次実行**されていた＝手札戻し＋バニッシュ×2＋ドロー捨て＋トラッシュ回収が
  //   **コスト0で毎回全部**走る、このシートで最も過剰な効果のひとつだった。
  // 🔑同バッチで足した3つが受け皿＝①記録側 `last_cost_energy_trash_colors`
  //   （`execTrash{asCost}` の**一括経路と1枚ずつ経路の両方**で書く）②条件 `COST_ENERGY_TRASHED_COLOR`
  //   ③`energyTrash.atLeast`（「２枚以上」＝上限なし・下限は `selectionConstraint.minCount`）。
  ...
  "WX21-Re18": [
    {"effectId":"WX21-Re18-E1","effectType":"ACTIVATED","timing":["MAIN"],
     ...
     "action":{"type":"SEQUENCE","steps":[
       ...
       {"type":"CONDITIONAL","condition":{"type":"COST_ENERGY_TRASHED_COLOR","color":"青"},
        "then":{"type":"SEQUENCE","steps":[
          {"type":"DRAW","owner":"self","count":2},
          {"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":1}}]}},
       {"type":"CONDITIONAL","condition":{"type":"COST_ENERGY_TRASHED_COLOR","color":"緑"},
        "then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"min":12000}},"upToCount":false}}},
       {"type":"CONDITIONAL","condition":{"type":"COST_ENERGY_TRASHED_COLOR","color":"黒"},
        "then":{"type":"TRANSFER_TO_HAND","source":{"type":"TRASH_CARD","owner":"self","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}}}
     ]},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  // ══════════════════════════════════════════════════════════════════════════════
  // §5.3 `O-280`（2026-09-08）＝legacy catch-all `STUB{TARGET_AND_DISCARD_HAND}` の残りを1件ずつ閉じる
  // ══════════════════════════════════════════════════════════════════════════════
  // 🔴**catch-all の実体**（`effectExecutor.ts:5365`）＝「**相手シグニ1体を選んでバニッシュし、手札を
  //   1枚捨てる**」に固定されている。対象の絞り込みも、捨てる札の絞り込みも、順序も、枚数も原文と無関係。
  //   ⇒ 当たったカードは**別の効果に化ける**（下の2件はどちらも原文と共通点がほとんど無かった）。

  // ── PR-195（羅星姫　アルデバラン）E3
  //   原文＝【起】《ダウン》：対戦相手は自分の凍結状態のシグニ１体につき手札を１枚捨てる。
  //         この能力はあなたの場にクロス状態のシグニがある場合にしか使用できない。
  // 🔴旧 live は catch-all ＝**相手シグニ1体をバニッシュして自分が手札を1枚捨てる**＝丸ごと別の効果。
  ...
  //         レベル２、レベル３、レベル４、レベル５についても同様に行う。
  // 🔴旧 live は `STUB{TRADE_BANISH_SELF_SIGNI}` ＋ `STUB{RULE_REMINDER_TEXT}` ＝**丸ごと別の効果**。
  // 🔑2つの機構が要った＝①**レベル1〜5の5回反復**（原文が5文ぶんを1文に畳んでいる）
  //   ②**場とエナを跨ぐ1回の対象選択**（同バッチで `EffectTarget.extraZones` を TRASH へ配線した）。
  // ⚠**反復は `REPEAT` ではなく5ステップに展開する**＝レベルが回ごとに変わるので回数だけでは表せない。
  // ⚠各ステップは独立した対象選択＝**該当レベルが居なければそのレベルだけ空振り**する（原文どおり）。
  "WXEX1-09": [
    {"effectId":"WXEX1-09-E2","effectType":"ACTIVATED","timing":["ATTACK_ARTS"],
     "cost":{"coin":4},
     "action":{"type":"SEQUENCE","steps":[
       {"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":1},"extraZones":["energy"],"upToCount":false}},
       {"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":2},"extraZones":["energy"],"upToCount":false}},
       {"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":3},"extraZones":["energy"],"upToCount":false}},
       {"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":4},"extraZones":["energy"],"upToCount":false}},
       {"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":5},"extraZones":["energy"],"upToCount":false}}
     ]},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  // ── WX25-CP1-092（黒舘ハルナ(正月)）E1
  //   原文＝【自】：あなたのアタックフェイズ開始時、対戦相手のシグニを３体まで対象とし、
  //         それらのシグニ１体につきあなたのエナゾーンから＜ブルアカ＞のカード１枚を
  //         トラッシュに置いてもよい。そうした場合、ターン終了時まで、それらのパワーを－3000する。
  // 🔴旧 live は catch-all ＝**対象1体・エナ支払いなし・手札を1枚捨てる**＝コストも対象数も別物。
  // 🔑同バッチで足した `energyTrashCountFromTargetCount`（宣言した対象の**体数**で払う）が受け皿。
  ...
  "WX25-CP1-092": [
    {"effectId":"WX25-CP1-092-E1","effectType":"AUTO","timing":["ON_ATTACK_PHASE_START"],"triggerScope":"self",
     "action":{"type":"SEQUENCE","steps":[
       {"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":3,"filter":{"cardType":"シグニ"},"upToCount":true},"abortIfNoCandidate":true},
       {"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},
  ...
  // ── WXDi-P00-018（コード・ハート　Ｍｉｌｕ//メモリア）E1
  //   原文＝【出】：あなたは手札をすべて捨て、捨てたカードの枚数に１を加えた枚数のカードを引く。
  //         対戦相手は手札をすべて捨て、捨てたカードの枚数から１を引いた枚数のカードを引く。
  // 🔴旧 live は**後半の1文が丸ごと catch-all**（相手シグニ1体をバニッシュ＋自分が手札1枚捨て）に化けていた。
  // 🔑受け皿は既存の `DRAW_DISCARD_COUNT_PLUS_N`＝同バッチで**引く側**（`drawDiscardOwner`）を足しただけ。
  //   ⚠`-1` は `Math.max(0, …)` で下限を切る（切らないと `deck.slice(0,-1)` でデッキがほぼ全部手札に入る）。
  // ⚠捨てる枚数は `lastProcessedCards` から読む＝**各プレイヤーの `TRASH` の直後**に置くこと
  //   （2つを1つの SEQUENCE に並べると、後半が前半の捨て枚数を読む）。
  "WXDi-P00-018": [
    {"effectId":"WXDi-P00-018-E1","effectType":"AUTO","timing":["ON_PLAY"],
     "action":{"type":"SEQUENCE","steps":[
       ...
       {"type":"SEQUENCE","steps":[
         ...
         {"type":"STUB","id":"DRAW_DISCARD_COUNT_PLUS_N","drawDiscardPlus":-1,"drawDiscardOwner":"opponent"}]}
     ]},
     "duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],

  // ── WXK05-003（コード・ハート　ＬｏＶ//ＭＥＭＯＲＹ）E1 の選択肢④
  //   原文④＝対戦相手のシグニ１体を対象とし、手札からレベル４のシグニを１枚捨てる。
  //           そうした場合、それをバニッシュする。
  // 🔴旧 live の④は catch-all ＝**手札のどれでも1枚**捨てればよく、しかも**捨てる前にバニッシュ**していた
  //   （catch-all は「対象へ then を適用 → そのあと手札1枚」の順で固定）＝
  //   **レベル4のシグニを持っていなくてもバニッシュできる**（コスト踏み倒し）。
  // 🔑正準形＝`SELECT_TARGET_ONLY` → `STORE_LAST_PROCESSED_TARGETS` → `TRASH{HAND_CARD, filter}` →
  //   `BANISH{targetsStored, conditional:true}`。`conditional` は「そうした場合」＝**直前が
  //   `lastProcessedCards` を残したときだけ**実行する（捨てられなければバニッシュしない）。
  // ⚠**①〜③は旧 live のまま1バイトも変えない**（正しく出来ている＝ここで壊さない）。
  // ⚠`conditionChoose`（センターが＜ＬｏＶ＞なら3つまで）と `upTo` もそのまま維持する。
  "WXK05-003": [
    {"effectId":"WXK05-003-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],
     "cost":{"energy":[{"color":"赤","count":3},{"color":"無","count":1}]},
     "action":{"type":"CHOOSE","choose_count":2,"from_count":4,"upTo":true,
      "conditionChoose":{"condition":{"type":"LRIG_STORY","owner":"self","story":"ＬｏＶ"},"thenChooseCount":3,"thenUpTo":true},
      "choices":[
       {"choiceId":"c0","label":"選択肢1","action":{"type":"SEQUENCE","steps":[
         {"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":"ALL"}},
         {"type":"DRAW","owner":"self","count":4}]}},
       {"choiceId":"c1","label":"選択肢2","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":7000}},"upToCount":false}}},
       ...
       {"choiceId":"c3","label":"選択肢4","action":{"type":"SEQUENCE","steps":[
         {"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"abortIfNoCandidate":true},
         {"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},
         {"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":1,"filter":{"cardType":"シグニ","level":4}}},
         {"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"targetsStored":true,"conditional":true}]}}
      ]},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  "PR-195": [
    {"effectId":"PR-195-E3","effectType":"ACTIVATED","timing":["MAIN"],
     "condition":{"type":"HAS_CARD_IN_FIELD","owner":"self","filter":{"cardType":"シグニ","crossState":true}},
     "cost":{"down_self":true},
     "action":{"type":"TRASH","target":{"type":"HAND_CARD","owner":"opponent","count":1,
       "countFromZone":{"zone":"field","owner":"opponent","filter":{"cardType":"シグニ","isFrozen":true}}}},
     "duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],

  // ══════════════════════════════════════════════════════════════════════════════
  // §5.3 `O-273`（2026-09-08）＝【チャーム】を「好きな数」トラッシュする
  // ══════════════════════════════════════════════════════════════════════════════
  // ── WX07-021（アトラク・パニッシュ）E1
  //   原文＝対象の好きな数のあなたの場にある【チャーム】をトラッシュに置く。ターン終了時まで、
  //         対象の対戦相手のシグニ１体のパワーをこの方法でトラッシュに置いた【チャーム】１枚につき－8000する。
  // 🔴旧 live は `REMOVE_CHARM{count:'ALL'}` ＝**「好きな数」が表せず全チャームを強制トラッシュ**
  //   （後続のパワー減少も常に最大＝チャームを温存する選択が消えていた）。
  // 🔑受け皿は既に在った＝同型 `WX07-045-E1`（【出】…【チャーム】を好きな数トラッシュに置く：）が使っている
  //   `STUB{OPTIONAL_COST, charmTrashVariable}` ＋ `POWER_MODIFY_PER_CHARM{sourceLocation:'trashed_this_effect'}`。
  //   `effectExecutor.ts:6144` の可変枚数コスト先取りが **0..N の CHOOSE** を engine 側で出すので、
  //   ⇒ **`src/screens/` は触らない＝実機不要**（登録票の「選択UIが要る／実機まで必須」は stale）。
  // ⚠**どのチャームを外すかはゾーン順の自動選択**＝`INTERNAL_PAY_CHARM_TRASH_VARIABLE`（`execStubPart1.ts:778`）
  //   も UI 側（`BattleScreen.tsx:13963`）も先頭ゾーンから取る。既存の同型と同じ近似に揃えてある。
  "WX07-021": [
    {"effectId":"WX07-021-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],
     "cost":{"energy":[{"color":"黒","count":1}]},
     "action":{"type":"SEQUENCE","steps":[
       {"type":"STUB","id":"OPTIONAL_COST","charmTrashVariable":{"min":0},
        "costText":"あなたの場にある【チャーム】を好きな数トラッシュに置く"},
       {"type":"POWER_MODIFY_PER_CHARM",
        "target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},
        "deltaPerCharm":-8000,"sourceOwner":"self","sourceLocation":"trashed_this_effect",
  ...
  //     《トラップアイコン》を発動させる。」
  // 🔴**旧 live は三重に壊れていた**（`WX17-044-E2`）＝
  //   ①除外が**コストに1つも入っていない**（`cost` は《青×1》《無×1》だけ）＝踏み倒して撃てる
  //   ②本体1歩目が `TRASH{TRASH_CARD, owner:self, count:1}`＝**「トラッシュのカードを1枚捨てる」という別動作**
  //     （原文は「トラッシュにある**このカード**をゲームから除外する」）
  //   ③「そうした場合、」が `CONDITIONAL{IS_MY_TURN}` に化けていた（無言フォールバック）。
  //   さらに `trashActivated` が立たないので、**トラッシュUIから一度も提示されない恒久 no-op** でもあった。
  // 🔑**他9効果（第184バッチ）は除外がコスト欄にあるので parser の funnel で拾えたが、この1枚だけは
  //   本文に書かれている**＝同型が他に無い（実測1効果）ので PLAN §2.0 の速いレーンで手書きする。
  // 🔑**除外は `cost.trashExile.self` に置く**＝原文の「そうした場合」＝支払い成立が本体の前提。
  //   `trashActivateCost.ts` が「トラッシュに無ければ支払い不能」へ倒すので踏み倒しは構造で塞がる。
  // 🔑**本体は `trapTargetScope:'explicit'`**（§5.3 `O-263`）＝「あなたの【トラップ】１つを**対象とし**」
  //   ＝**プレイヤーが選ぶ**（旧はどの入口でも先頭の【トラップ】が勝手に発動していた）。
  // ⚠**E1（スペル本体）は触らない**＝live のほうが正しいので `syncManualLive --effect` で E2 だけ届ける。
  'WX17-044': [
    {"effectId":"WX17-044-E2","effectType":"ACTIVATED","timing":["MAIN","ATTACK_ARTS"],"cost":{"energy":[{"color":"青","count":1},{"color":"無","count":1}],"trashExile":{"self":true}},"action":{"type":"STUB","id":"ACTIVATE_TRAP","trapTargetScope":"explicit"},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL","trashActivated":true},
  ],
  ...
  // ── (D) 「あなたのレベル３のルリグ１体を対象とし」＝使用条件へレベル3を足す（ピース4枚） ──
  ...
  'WX24-P4-017': [
    {"effectId":"WX24-P4-017-E3","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"青","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"RECOLLECT_GATE","minArts":4},{"type":"INSTALL_DELAYED_TRIGGER","duration":"THIS_TURN","trigger":{"timing":"ON_DRAW"},"effect":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"delta":-4000,"duration":"UNTIL_END_OF_TURN"}},{"type":"INSTALL_DELAYED_TRIGGER","duration":"THIS_TURN","trigger":{"timing":"ON_HAND_DISCARDED"},"effect":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"delta":-4000,"duration":"UNTIL_END_OF_TURN"}}]},"duration":"UNTIL_END_OF_TURN","mandatory":false,"parseStatus":"MANUAL","usageLimit":"once_per_game"},
  ],
  ...
  'WXDi-CP02-078': [
    {"effectId":"WXDi-CP02-078-E1","effectType":"ACTIVATED","timing":["MAIN"],"usageLimit":"once_per_turn","cost":{"discard":1,"discardFilter":{"story":"ブルアカ"}},"action":{"type":"SEQUENCE","steps":[{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"thisCardOnly":true}},"delta":5000,"duration":"UNTIL_OPP_TURN_END"},{"type":"GRANT_EFFECT","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"thisCardOnly":true}},"duration":"UNTIL_OPP_TURN_END","effect":{"effectId":"WXDi-CP02-078-E1-GRANT","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"STUB","id":"OPTIONAL_COST","costColors":["青"]},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"delta":-5000,"duration":"UNTIL_END_OF_TURN","targetsStored":true}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  ...
  // 🆕§5.0 実装キュー 第223バッチ＝原文「①あなたのシグニ１体を対象とし、ターン終了時まで、それは
  //   【ダブルクラッシュ】を得る。②**そのシグニ**がレベル４以上の場合、追加で**それ**は【アサシン】を得る。」
  //   ＝「そのシグニ」「それ」は①で選んだ**同一の1体**への照応。旧 live は2本目の GRANT_KEYWORD が
  //   `owner:'any', explicitTarget:true`（新規に相手シグニも選べる独立選択）＋`duration:'PERMANENT'`
  //   （原文にない恒久化）の二重の誤りだった。真因は `applyExplicitTargetMarker`（`effectParser.ts`）＝
  //   外側テキストに「シグニ…を対象とし」が1回でもあると木全体の GRANT_KEYWORD へ無差別に
  //   `explicitTarget:true` を刻む後処理で、「そのシグニ」照応を認識する枝がそもそも無い。
  //   受け皿は既存の `targetsLastProcessed`＋`CONDITIONAL{LAST_PROCESSED_MATCHES}`（1本目の選択結果を
  //   2本目が引き継ぐ）。同文カード `WXK03-TK-01B-E1`（token）も同型。母集団2効果。
  "WX17-004": [
    {"effectId":"WX17-004-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],"cost":{"energy":[{"color":"赤","count":1},{"color":"緑","count":1},{"color":"無","count":1}]},"action":{"type":"CHOOSE","choose_count":1,"from_count":4,"choices":[{"choiceId":"c0","label":"選択肢1","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":12000}},"upToCount":false}}},{"choiceId":"c1","label":"選択肢2","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"min":12000}},"upToCount":false}}},{"choiceId":"c2","label":"選択肢3","action":{"type":"SEQUENCE","steps":[{"type":"GRANT_KEYWORD","target":{"type":"SIGNI","owner":"self","count":1,"explicitTarget":true},"keyword":"ダブルクラッシュ","duration":"UNTIL_END_OF_TURN"},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":{"min":4}}},"then":{"type":"GRANT_KEYWORD","target":{"type":"SIGNI","owner":"self","count":1},"targetsLastProcessed":true,"keyword":"アサシン","duration":"UNTIL_END_OF_TURN"}}]}},{"choiceId":"c3","label":"選択肢4","action":{"type":"SEQUENCE","steps":[{"type":"ADD_TO_LIFE","owner":"self","count":1,"fromTop":true},{"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":2}}]}}],"conditionChoose":{"condition":{"type":"OR","conditions":[{"type":"LRIG_STORY","owner":"self","story":"リル"},{"type":"LRIG_STORY","owner":"self","story":"メル"}]},"thenChooseCount":2,"thenUpTo":true}},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WXK03-TK-01B": [
    {"effectId":"WXK03-TK-01B-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],"cost":{"energy":[{"color":"赤","count":1},{"color":"緑","count":1},{"color":"無","count":1}]},"action":{"type":"CHOOSE","choose_count":1,"from_count":4,"choices":[{"choiceId":"c0","label":"選択肢1","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":12000}},"upToCount":false}}},{"choiceId":"c1","label":"選択肢2","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"min":12000}},"upToCount":false}}},{"choiceId":"c2","label":"選択肢3","action":{"type":"SEQUENCE","steps":[{"type":"GRANT_KEYWORD","target":{"type":"SIGNI","owner":"self","count":1,"explicitTarget":true},"keyword":"ダブルクラッシュ","duration":"UNTIL_END_OF_TURN"},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":{"min":4}}},"then":{"type":"GRANT_KEYWORD","target":{"type":"SIGNI","owner":"self","count":1},"targetsLastProcessed":true,"keyword":"アサシン","duration":"UNTIL_END_OF_TURN"}}]}},{"choiceId":"c3","label":"選択肢4","action":{"type":"SEQUENCE","steps":[{"type":"ADD_TO_LIFE","owner":"self","count":1,"fromTop":true},{"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":2}}]}}],"conditionChoose":{"condition":{"type":"OR","conditions":[{"type":"LRIG_STORY","owner":"self","story":"リル"},{"type":"LRIG_STORY","owner":"self","story":"メル"}]},"thenChooseCount":2,"thenUpTo":true}},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  // 🆕§5.0 実装キュー 第223バッチ＝原文「【自】：…バニッシュしたとき、《無》を支払ってもよい。**そうした場合**、
  //   このシグニを場からトラッシュに置く。**その後**、あなたのトラッシュからシグニ１枚を対象とし、それを
  //   ダウン状態で場に出す。」＝「その後」節は「そうした場合」節の帰結に連なる＝**任意コストを払ったときだけ**
  //   の一連の流れ。旧 live は `ADD_TO_FIELD` が `CONDITIONAL` の**兄弟**に並んでおり、コストを払わなくても
  //   （＝自身をトラッシュに置かなくても）トラッシュから場に出せる過剰実行だった（`execSequence` が
  //   `remaining` を無条件 `continuation` にする共有 combinator＝25カードが逆の挙動に依存するため engine 側は
  //   触らない）。`CONDITIONAL.then` へ `SEQUENCE[TRASH, ADD_TO_FIELD]` として畳んだ。母集団1効果。
  "WX15-061": [
    {"effectId":"WX15-061-E1","effectType":"AUTO","timing":["ON_SIGNI_BANISH_OPPONENT"],"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_COST","costColors":["無"]},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"SEQUENCE","steps":[{"type":"TRASH","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"cardType":"シグニ","thisCardOnly":true}}},{"type":"ADD_TO_FIELD","owner":"self","source":{"type":"TRASH_CARD","owner":"self","count":1,"upToCount":false,"filter":{"cardType":"シグニ"}},"asDown":true}]}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL","triggerScope":"self"},
  ],
  "WX20-006": [
  ...
  "WXDi-D04-011": [
    {"effectId":"WXDi-D04-011-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"白","count":0}]},"condition":{"type":"AND","conditions":[{"type":"LRIG_TEAM_COUNT","owner":"self","team":"CardJockey","operator":"gte","value":3},{"type":"LRIG_LEVEL","owner":"self","operator":"gte","value":1,"allFieldLrigs":true},{"type":"LRIG_LEVEL","owner":"self","operator":"gte","value":3}]},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"LRIG_ATTACK_LIMIT","lrigAttackLimit":{"limit":3,"whileDown":true}},{"type":"INSTALL_DELAYED_TRIGGER","duration":"THIS_TURN","once":true,"trigger":{"timing":"ON_ATTACK_LRIG"},"effect":{"type":"SEQUENCE","steps":[{"type":"SHUFFLE_DECK","owner":"self"},{"type":"STUB","id":"REVEAL_DECK_TOP_AND_REDUCE_LRIG_ATTACK_LIMIT","revealReduceLrigLimit":[{"level":1,"reduce":2},{"level":2,"reduce":1}]}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}
  ],
  "WXDi-D04-021": [
    {"effectId":"WXDi-D04-021-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"白","count":1},{"color":"無","count":1}]},"action":{"type":"CONDITIONAL","condition":{"type":"LRIG_TEAM_COUNT","owner":"self","team":"CardJockey","operator":"gte","value":3},"then":{"type":"REVEAL_AND_PICK","owner":"self","revealCount":7,"pickCount":2,"pickUpTo":true,"then":{"type":"ADD_TO_HAND","owner":"self"},"remainder":{"location":"deck","position":"bottom"}},"else":{"type":"REVEAL_AND_PICK","owner":"self","revealCount":7,"filter":{"cardType":"シグニ"},"pickCount":2,"pickUpTo":true,"then":{"type":"ADD_TO_HAND","owner":"self"},"remainder":{"location":"deck","position":"bottom"}}},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WXDi-D05-007": [
  ...
  "WXDi-P11-076": [
    {"effectId":"WXDi-P11-076-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"緑","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_COST","costText":"使用コストとして追加でエクシード７を支払ってもよい","exceed":7}]},{"type":"ENERGY_CHARGE","target":{"type":"TRASH_CARD","owner":"self","count":1,"upToCount":false,"filter":{"cardType":"シグニ","colorMatchesLrig":true}}},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"TRANSFER_TO_HAND","source":{"type":"ENERGY_CARD","owner":"self","count":2,"upToCount":true,"filter":{"cardType":"シグニ","hasGuard":false}}}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WXDi-P13-027": [
  ...
  "WX24-P1-020": [
    {"effectId":"WX24-P1-020-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"赤","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":10000}},"upToCount":false}},{"type":"REVEAL_AND_PICK","owner":"self","revealCount":5,"filter":{"cardType":"シグニ","story":"宝石"},"pickCount":2,"pickUpTo":true,"remainder":{"location":"deck","position":"bottom","reorder":true},"then":{"type":"SEQUENCE","steps":[{"type":"REVEAL"},{"type":"ADD_TO_HAND","owner":"self"}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WX24-P1-036": [
  ...
  "WX25-P2-066": [
    {"effectId":"WX25-P2-066-E1","effectType":"AUTO","timing":["ON_PLAY"],"action":{"type":"SEQUENCE","steps":[{"type":"REVEAL_AND_PICK","owner":"self","revealCount":7,"filter":{"cardType":"スペル"},"pickCount":1,"pickUpTo":true,"then":{"type":"ADD_TO_HAND","owner":"self"},"remainder":{"location":"deck","position":"bottom","shuffle":true},"pickNoun":"スペル"},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_COUNT_GTE","value":1},"then":{"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":1}}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WX25-P3-007": [
  ...
  "WX25-CP1-027": [
    {"effectId":"WX25-CP1-027-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"青","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"REVEAL_AND_PICK","owner":"self","revealCount":5,"filter":{"cardClass":"ブルアカ"},"pickCount":2,"pickUpTo":true,"pickNoun":"カード","then":{"type":"ADD_TO_HAND","owner":"self"},"remainder":{"location":"deck","position":"bottom","reorder":true}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"color":"青","cardClass":"ブルアカ"},"minCount":1},"then":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":12000}},"upToCount":false},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"STUB","id":"OPPONENT_PAY_OPTIONAL","opponentHandDiscard":2},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":12000}},"upToCount":false},"targetsStored":true}}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WX25-CP1-029": [
  ...
  "WX25-CP1-061": [
    {"effectId":"WX25-CP1-061-E1","effectType":"AUTO","timing":["ON_TURN_END"],"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_COST","costText":"あなたの手札から＜ブルアカ＞のカードを３枚まで公開してもよい"},{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"cardType":"シグニ","thisCardOnly":true}},"delta":2000,"deltaPerLastProcessedCount":true,"perLastProcessed":{},"duration":"UNTIL_OPP_TURN_END"}]},"duration":"UNTIL_END_OF_TURN","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WX25-CP1-069": [
  ...
  "WXK10-044": [
    {"effectId":"WXK10-044-E1","effectType":"AUTO","timing":["ON_PLAY"],"action":{"type":"TRANSFER_TO_DECK","source":{"type":"HAND_CARD","owner":"opponent","count":1},"shuffle":false,"position":"bottom","opponentSelects":true},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL","triggerScope":"any_opp","triggerCondition":{"placedOnGateZone":true}},
  ...
  "WDK13-017": [
    {"effectId":"WDK13-017-E1","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"DECK_REVEAL_UNTIL","deckRevealUntil":{"until":"signi","count":2,"allTo":"trash"}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_LEVEL_SUM","operator":"gte","value":6},"then":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"delta":-1000}}]},"duration":"UNTIL_END_OF_TURN","mandatory":true,"parseStatus":"MANUAL","triggerScope":"self"},
  ],
  "WDK14-008": [
  ...
  "SP27-014": [
    {"effectId":"SP27-014-E2","effectType":"AUTO","timing":["ON_TRASH"],"action":{"type":"CHOOSE","choose_count":1,"from_count":3,"choices":[{"choiceId":"SP27-014-E2-c1","label":"カードを1枚引く","action":{"type":"DRAW","owner":"self","count":1}},{"choiceId":"SP27-014-E2-c2","label":"デッキの一番上のカードをエナゾーンに置く","action":{"type":"ENERGY_CHARGE_FROM_DECK","owner":"self","count":1}},{"choiceId":"SP27-014-E2-c3","label":"手札を1枚捨て、対戦相手のシグニ1体をバニッシュする","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"STUB","id":"OPTIONAL_COST","handDiscard":{"count":1}},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"targetsStored":true}}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL","triggerCondition":{"byOpponentEffect":true,"fromAnyZone":true}},
  ],
  // 🆕2026-09-09（S-3・意味照合triage・codex-work引き継ぎ）＝末尾「スペルの場合、対戦相手はそのカードを
  //   （捨てる）」の「そのカード」＝公開したそのカード自身。旧 JSON は無指定の TRASH{HAND_CARD,opponent,1}＝
  //   相手手札の任意1枚を捨てさせており、公開したカードと無関係の1枚を捨てられた（過小/別対象実行）。
  //   `STORE_LAST_PROCESSED_TARGETS` で公開直後に固定し、最後の分岐だけ `targetsStored` で同一カードを指す
  //   （他の分岐は「対戦相手のシグニ1体を対象とし」＝場の任意のシグニなので無変更）。
  "PR-459A": [
    {"effectId":"PR-459A-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],"cost":{"energy":[{"color":"無","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"REVEAL_OPP_HAND_CARD"},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":1}},"then":{"type":"ENERGY_CHARGE_FROM_DECK","owner":"self","count":3}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":2}},"then":{"type":"DRAW","owner":"self","count":3}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":3}},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":4}},"then":{"type":"TRASH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"シグニ","level":5}},"then":{"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":1}}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_MATCHES","filter":{"cardType":"スペル"}},"then":{"type":"TRASH","target":{"type":"HAND_CARD","owner":"opponent","count":1},"targetsStored":true,"optional":false}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "SPDi43-28": [
    {"effectId":"SPDi43-28-E1","effectType":"AUTO","timing":["ON_ATTACK_LRIG"],"triggerScope":"any_ally","triggerFilter":{"cardName":"335　アキノ"},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_COST","costText":"手札から白のカードを３枚捨ててもよい"},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"SEQUENCE","steps":[{"type":"UP","target":{"type":"LRIG","owner":"self","count":1}},{"type":"REMOVE_ABILITIES","target":{"type":"LRIG","owner":"self","count":1},"until":"UNTIL_END_OF_TURN"}]}}]},"duration":"UNTIL_END_OF_TURN","mandatory":true,"parseStatus":"MANUAL","usageLimit":"once_per_turn"},
  ],
  "SPDi43-30": [
  ...
  "WXK10-055": [
    ...
    {"effectId":"WXK10-055-E1","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"TRASH_CARD","owner":"self","count":1,"filter":{"cardType":"シグニ","cardClass":"美巧"}},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"TRASH","target":{"type":"SIGNI","owner":"self","count":1},"optional":true},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"TRANSFER_TO_HAND","source":{"type":"TRASH_CARD","owner":"self","count":1,"filter":{"cardType":"シグニ","cardClass":"美巧"}},"targetsStored":true}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ...
  "WDK07-E08": [
    {"effectId":"WDK07-E08-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK","SPELL_CUTIN"],"cost":{"energy":[{"color":"青","count":1}]},"action":{"type":"CHOOSE","choose_count":1,"from_count":3,"choices":[{"choiceId":"down","label":"対戦相手のシグニ1体をダウンする","action":{"type":"DOWN","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}}},{"choiceId":"draw","label":"カードを2枚引く","action":{"type":"DRAW","owner":"self","count":2}},{"choiceId":"counter","label":"スペルの効果を打ち消し、それをトラッシュから対戦相手の手札に戻す","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"COUNTER_SPELL"},{"type":"TRANSFER_TO_HAND","source":{"type":"TRASH_CARD","owner":"opponent","count":1,"filter":{"cardType":"スペル"}},"targetsStored":true},{"type":"NAME_BAN","targetSelf":false,"duration":"TURN"}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  "WDK13-007": [
  ...
  // 🆕2026-09-09（S-3・意味照合triage・codex-work引き継ぎ）＝選択肢③「このターン、あなたは対戦相手の
  //   効果によってダメージを受けない」は回数無制限の期間つき無効化＝ PREVENT_NEXT_DAMAGE（1回消費）ではなく
  //   PREVENT_DAMAGE{until:UNTIL_END_OF_TURN}（既存・期間中無制限）を使う。
  "SPK01-13": [
    {"effectId":"SPK01-13-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK","SPELL_CUTIN"],"cost":{"energy":[{"color":"白","count":0}]},"action":{"type":"CHOOSE","choose_count":1,"from_count":5,"choices":[{"choiceId":"c0","label":"選択肢1","action":{"type":"TRANSFER_TO_HAND","source":{"type":"TRASH_CARD","owner":"self","count":1,"upToCount":false,"filter":{"cardType":"シグニ"}}}},{"choiceId":"c1","label":"選択肢2","action":{"type":"EXILE","target":{"type":"TRASH_CARD","owner":"opponent","count":2,"upToCount":true}}},{"choiceId":"c2","label":"このターン、対戦相手の効果によってダメージを受けない","action":{"type":"PREVENT_DAMAGE","owner":"self","until":"UNTIL_END_OF_TURN","scope":"OPP_EFFECT"}},{"choiceId":"c3","label":"選択肢4","action":{"type":"GRANT_KEYWORD","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"keyword":"アタックできない","duration":"UNTIL_END_OF_TURN"}},{"choiceId":"c4","label":"対戦相手のすべてのシグニは効果で得ている能力を失う（ターン終了時まで）","action":{"type":"REMOVE_ABILITIES","target":{"type":"SIGNI","owner":"opponent","count":"ALL"},"grantedOnly":true,"until":"UNTIL_END_OF_TURN"}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  ...
  "WXDi-P14-083": [
    {"effectId":"WXDi-P14-083-E1","effectType":"AUTO","timing":["ON_ATTACK_PHASE_START"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":5000}},"upToCount":false},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"STUB","id":"OPTIONAL_DISCARD_HAND_CLASS","handCardPick":{"filter":{"cardType":"シグニ","story":"電音部"},"count":1}},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"targetsStored":true}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WX24-P3-068": [
    {"effectId":"WX24-P3-068-E1","effectType":"AUTO","timing":["ON_ATTACK_PHASE_START"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_DISCARD_HAND_CLASS","handCardPick":{"filter":{"cardType":"シグニ","story":"水獣"},"count":1}},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":5000}},"upToCount":false}}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WX24-P4-016": [
  ...
  // 数字宣言の保存値を既存 GUARD_LV_DECLARED で消費し、同レベルのガード制限を立てる。
  "WXK07-001": [
    {"effectId":"WXK07-001-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"赤","count":0}]},"condition":{"type":"OR","conditions":[{"type":"LRIG_NAME_CONTAINS","owner":"self","name":"花代"},{"type":"LRIG_LEVEL","owner":"self","operator":"gte","value":4}]},"action":{"type":"GRANT_LRIG_ABILITY","abilities":[{"effectId": "WXK07-001-E1-GRANT", "effectType": "AUTO", "timing": ["ON_ATTACK_PHASE_START"], "action": {"type": "SEQUENCE", "steps": [{"type": "STUB", "id": "DECLARE_NUMBER"}, {"type":"BLOCK_ACTION","target":{"type":"PLAYER","owner":"opponent","count":1},"actionId":"GUARD_LV_DECLARED","until":"END_OF_TURN"}, {"type": "STUB", "id": "BLOCK_COLORLESS_ENERGY_PAY"}, {"type": "CONDITIONAL", "condition": {"type": "LRIG_LEVEL", "owner": "opponent", "operator": "gte", "value": 4}, "then": {"type": "BLOCK_ACTION", "target": {"type": "PLAYER", "owner": "opponent", "count": 1}, "actionId": "ARTS_LIMIT_1", "until": "END_OF_TURN"}}]}, "duration": "INSTANT", "mandatory": true, "parseStatus": "MANUAL", "triggerScope": "self"}],"rawText":"【自】：あなたのアタックフェイズ開始時、数字１つを宣言する。対戦相手はこのアタックフェイズの間、無色のカードでエナコストを支払えず、宣言された数字と同じレベルのシグニで【ガード】ができず、自身のセンタールリグがレベル４以上の場合一度しかアーツを使用できない。","permanent":true,"targetedCenter":true},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"},
  ],
  ...
  "WX25-P3-054": [
    {"effectId":"WX25-P3-054-E2","effectType":"AUTO","timing":["ON_TRASH"],"triggerScope":"any_opp","triggerFilter":{"cardType":"シグニ"},"condition":{"type":"HAS_CARD_IN_FIELD","owner":"self","filter":{"cardName":"解明の巫女　ユキ"}},"action":{"type":"REVEAL_AND_PICK","owner":"self","revealCount":5,"filter":{"cardType":"シグニ","cardClass":"迷宮"},"pickCount":1,"then":{"type":"ADD_TO_HAND","owner":"self"},"remainder":{"location":"deck","position":"bottom","reorder":true}},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL","usageLimit":"once_per_turn","triggerCondition":{"turnOwner":"self","fromZones":["field"]}},
  ],
  "WXDi-P10-044": [
  ...
  // 🏁**§5.3 `O-284`（2026-09-08）＝「自身以外の効果を受けない」を原文どおりにした。**
  //   原文＝【常】：《カーニバル　―Ｑ―》は**自身以外の効果を受けない**。
  //   🔴旧 live は `sourceOwner:'opponent'` ＝**対戦相手の効果だけ**を遮断していた（旧コメントは
  //     「自分側の分はほぼ不活性なので意図的に defer」と書いていたが、それは**近似ではなく別の意味**）。
  //   🔑2つを同バッチで足した＝①`exceptSelfSource`（**このカード自身の能力**だけを例外にする
  //     identity 限定。`exceptSource` の型限定ではアシストルリグまで通ってしまう）
  //     ②`ExecCtx.ownEffectImmuneNums`（`collectEffectImmuneSigni` を**自分側視点でも**呼ぶ対の計算）。
  //   ⚠`sourceOwner:'any'` にしただけでは効かない＝呼び出しが片側専用だったのが本体（`O-284` の登録票）。
  //   ⚠E2 の「このルリグ」は `O-75` で既存の LRIG 付与形へ戻してある＝ここでは触らない。
  "WX17-001": [
    {"effectId":"WX17-001-E1","effectType":"CONTINUOUS","action":{"type":"GRANT_PROTECTION","fromAll":true,"sourceOwner":"any","exceptSelfSource":true,"duration":"PERMANENT"},"duration":"PERMANENT","mandatory":true,"parseStatus":"MANUAL"},
  ...
  "WDK16-06H": [{"effectId":"WDK16-06H-E1","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","condition":{"type":"LRIG_NAME_CONTAINS","owner":"self","name":"楓"},"action":{"type":"CONDITIONAL","condition":{"type":"SUBSCRIBER_COUNT","operator":"gte","value":100},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}},"else":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":8000}},"upToCount":false}}},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}],
  ...
  "WXK03-034": [{"effectId":"WXK03-034-E1","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","condition":{"type":"TURN_HAND_DISCARD_GTE","value":2},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerLteSelf":true},"upToCount":false},"abortIfNoCandidate":true},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"STUB","id":"OPTIONAL_COST","costColors":["赤"]},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerLteSelf":true},"upToCount":false},"targetsStored":true}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
    {"effectId":"WXK03-034-E1b","effectType":"CONTINUOUS","activeCondition":{"type":"TURN_HAND_DISCARD_GTE","value":2},"action":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"thisCardOnly":true}},"delta":2000},"duration":"PERMANENT","mandatory":true,"parseStatus":"MANUAL"}],
  ...
  "SP27-015": [{"effectId":"SP27-015-E3","effectType":"CONTINUOUS","action":{"type":"GRANT_ACCE_HOST_ABILITY","abilities":[{"effectId":"SP27-015-E3-G","effectType":"ACTIVATED","timing":["ATTACK_ARTS","MAIN"],"usageLimit":"once_per_turn","cost":{"acceTrash":2},"action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}]},"duration":"PERMANENT","mandatory":true,"parseStatus":"MANUAL"},
  ...
  "WXDi-D07-003": [{"effectId":"WXDi-D07-003-E1","effectType":"CONTINUOUS","action":{"type":"GRANT_SOUL_HOST_ABILITY","abilities":[{"effectId":"WXDi-D07-003-E1-G","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","usageLimit":"once_per_turn","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","powerRange":{"max":12000}},"upToCount":false}},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}]},"duration":"PERMANENT","mandatory":true,"parseStatus":"MANUAL"}],
  ...
  "WXDi-P04-015": [{"effectId":"WXDi-P04-015-E1","effectType":"CONTINUOUS","action":{"type":"GRANT_SOUL_HOST_ABILITY","abilities":[{"effectId":"WXDi-P04-015-E1-G","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","usageLimit":"once_per_turn","action":{"type":"BANISH","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","level":{"max":2}},"upToCount":false}},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}]},"duration":"PERMANENT","mandatory":true,"parseStatus":"MANUAL"}],
  ...
  'WD07-012': [
    ...
    {"effectId":"WD07-012-E2","effectType":"AUTO","timing":["ON_PLAY"],"cost":{"energy":[{"color":"黒","count":1}]},"action":{"type":"SEQUENCE","steps":[{"type":"TRANSFER_TO_DECK","source":{"type":"TRASH_CARD","owner":"self","count":4,"upToCount":false,"filter":{"cardType":"シグニ","story":"古代兵器"},"selectionConstraint":{"distinct":"level"}},"shuffle":false,"position":"bottom"},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_COUNT_GTE","value":4},"then":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false},"delta":-10000,"duration":"UNTIL_END_OF_TURN"}}]},"duration":"UNTIL_END_OF_TURN","mandatory":false,"parseStatus":"MANUAL"},
  ],
  ...
  'WXDi-P15-056': [
    {
      ...
      timing: ['ON_ATTACK_SIGNI'],
      triggerScope: 'self',
      condition: { type: 'AND', conditions: [{ type: 'SAME_ZONE_HAS_GATE' }, { type: 'HAS_CARD_IN_FIELD', owner: 'self', filter: { cardName: '防衛者ＭＣ．ＬＩＯＮ－３ｒｄ' } }] },
      action: { type: 'SEQUENCE', steps: [
        { type: 'STUB', id: 'OPTIONAL_COST', costColors: ['白', '白'] },
        { type: 'CONDITIONAL', condition: { type: 'PAID_ADDITIONAL_COST' }, then: { type: 'SEQUENCE', steps: [{ type: 'UP', target: { type: 'SIGNI', owner: 'self', count: 1, filter: { thisCardOnly: true } } }, { type: 'REMOVE_ABILITIES', target: { type: 'SIGNI', owner: 'self', count: 1, filter: { thisCardOnly: true } }, until: 'UNTIL_END_OF_TURN' }] } },
      ] },
      duration: 'INSTANT',
      mandatory: false,
      parseStatus: 'MANUAL',
  ...
  // countChoose の既存 `$ref:'center_lrig_level'` で、センタールリグのレベル数までを実行時に解決する。
  'WXDi-P14-002': [
    {
      ...
      ], countChoose: { count: { $ref: 'center_lrig_level' }, upTo: true } },
      duration: 'INSTANT',
      mandatory: false,
      parseStatus: 'MANUAL',
    },
  ...
  //   任意コスト STUB+CONDITIONAL(PAID_ADDITIONAL_COST) の then 内へ公開とBOUNCEをまとめ、未払い時は両方を止める。E2（【出】数字宣言）は自動パース維持。
  "WXK10-031": [
    {"effectId":"WXK10-031-E1","effectType":"AUTO","timing":["ON_ATTACK_SIGNI"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_COST","costColors":["無"]},{"type":"CONDITIONAL","condition":{"type":"PAID_ADDITIONAL_COST"},"then":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"DECK_REVEAL_UNTIL","deckRevealUntil":{"until":"signi","allTo":"trash"}},{"type":"BOUNCE","target":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ","levelLtLastProcessed":true},"upToCount":false}}]}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}
  ],
  ...
  "WX26-CP1-048": [
    {"effectId":"WX26-CP1-048-E2","effectType":"AUTO","timing":["ON_PLAY"],"action":{"type":"CONDITIONAL","condition":{"type":"THIS_CARD_PLACED_BY_CLASS","cardClass":"プリオケ"},"then":{"type":"SEQUENCE","steps":[{"type":"TRASH","target":{"type":"ENERGY_CARD","owner":"opponent","count":1}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_SHARES_COLOR_WITH_LRIG","owner":"opponent"},"then":{"type":"CHOOSE","choose_count":1,"from_count":2,"opponentResponds":true,"choices":[{"choiceId":"charge","label":"エナチャージ1をする","action":{"type":"ENERGY_CHARGE_FROM_DECK","owner":"opponent","count":1}},{"choiceId":"skip","label":"エナチャージしない","action":{"type":"SEQUENCE","steps":[]}}]}}]}},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}
  ],

  // ===== G158: E1 全シグニ異クラス条件で【エナチャージ2】 / E2 プライマル（技名）：エナのシグニを手札へ、5枚以上でルリグに無敵付与 =====
  // 🆕**2026-09-10 §5.3 `O-324`＝E1（両カードとも）を削除した**＝`LEADING_STATE_CLAUSES` に
  //   「〈誰か〉の場にあるすべてのシグニがそれぞれ共通するクラスを持たない場合」を足したので
  //   **parser が同じものを出せるようになった**（`§6.4 O-42` の影武者トリップワイヤが検出）。
  //   ⚠**E2 は MANUAL のまま**＝「それらがそれぞれ共通するクラスを持たない場合」の選択制約は
  //   parser がまだ出せない（この巡で手で足した）。
  "SPDi44-04": [
    {"effectId":"SPDi44-04-E2","effectType":"ACTIVATED","timing":["MAIN"],"usageLimit":"once_per_game","cost":{"energy":[{"color":"緑","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"TRANSFER_TO_HAND","source":{"type":"ENERGY_CARD","owner":"self","count":"ALL","upToCount":true,"filter":{"cardType":"シグニ"},"selectionConstraint":{"distinct":"class"}}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_COUNT_GTE","value":5},"then":{"type":"GRANT_LRIG_ABILITY","rawText":"【常】：あなたは対戦相手の効果によってダメージを受けない。","abilities":[{"effectId":"SPDi44-04-E2-GRANT","effectType":"CONTINUOUS","action":{"type":"STUB","id":"PREVENT_DAMAGE_FROM_OPP_EFFECTS"},"duration":"UNTIL_OPP_TURN_END","mandatory":true,"parseStatus":"MANUAL"}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}
  ],
  "WX25-P1-026": [
    {"effectId":"WX25-P1-026-E2","effectType":"ACTIVATED","timing":["MAIN"],"usageLimit":"once_per_game","cost":{"energy":[{"color":"緑","count":0}]},"action":{"type":"SEQUENCE","steps":[{"type":"TRANSFER_TO_HAND","source":{"type":"ENERGY_CARD","owner":"self","count":"ALL","upToCount":true,"filter":{"cardType":"シグニ"},"selectionConstraint":{"distinct":"class"}}},{"type":"CONDITIONAL","condition":{"type":"LAST_PROCESSED_COUNT_GTE","value":5},"then":{"type":"GRANT_LRIG_ABILITY","rawText":"【常】：あなたは対戦相手の効果によってダメージを受けない。","abilities":[{"effectId":"WX25-P1-026-E2-GRANT","effectType":"CONTINUOUS","action":{"type":"STUB","id":"PREVENT_DAMAGE_FROM_OPP_EFFECTS"},"duration":"UNTIL_OPP_TURN_END","mandatory":true,"parseStatus":"MANUAL"}]}}]},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}
  ],
  ...
  // ①の解決は既存 COUNTER_TEAM_PIECE_AND_EXILE、使用機会は既存のチームピース応答窓を使う。
  // §6.4 O-10（続き517）＝**使用条件を復元**した。
  // 原文「【使用条件】【チーム】＜きゅるきゅるーん☆＞ このピースは、対戦相手が【使用条件】【チーム】を持つ
  //   ピースを使用する際、**カットインして使用できる**。…」
  // 🔴従来の `AND(..., OPP_USING_TEAM_PIECE)` は応答窓だけに使用を限定し、通常の自ターン使用を失わせていた。
  //   ⇒ チーム条件 AND (IS_MY_TURN OR OPP_USING_TEAM_PIECE) とし、通常使用と追加のカットイン機会を両立する。
  "WXDi-P05-006": [
    {"effectId":"WXDi-P05-006-E1","effectType":"ACTIVATED","timing":["MAIN","ATTACK"],"condition":{"type":"AND","conditions":[{"type":"LRIG_TEAM_COUNT","owner":"self","team":"きゅるきゅるーん☆","operator":"gte","value":3},{"type":"OR","conditions":[{"type":"IS_MY_TURN"},{"type":"OPP_USING_TEAM_PIECE"}]}]},"cost":{"energy":[{"color":"青","count":0}]},"action":{"type":"CHOOSE","choose_count":1,"from_count":2,"choices":[
  ...
  "WXK03-048": [
    {"effectId":"WXK03-048-E1","effectType":"AUTO","timing":["ON_ATTACK_PHASE_START"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[
      {"type":"STUB","id":"OPTIONAL_COST","costColors":["白","白","無","無"],"costText":"このシグニを場からトラッシュに置いてもよい"},
  ...
  "WXDi-P13-003A": [
    {"effectId":"WXDi-P13-003A-E1","effectType":"ACTIVATED","timing":["MAIN"],"cost":{"energy":[{"color":"無","count":0}]},"condition":{"type":"AND","conditions":[{"type":"CENTER_LRIG_NOT_GROWN_THIS_TURN","owner":"self"},{"type":"OR","conditions":[{"type":"FIELD_LRIGS_HAVE_COLORS","owner":"self","colors":["白"]},{"type":"FIELD_LRIGS_HAVE_COLORS","owner":"self","colors":["黒"]}]}]},"action":{"type":"STUB","id":"MAYU_ENCOUNTER_FLIP_AND_GROW"},"duration":"INSTANT","mandatory":false,"parseStatus":"MANUAL"}
  ],
...
{"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL","effectId":"WX25-P1-054-E2","effectType":"AUTO","timing":["ON_HEAVEN"],"crossOnly":true,"usageLimit":"once_per_turn","activeCondition":{"type":"HAS_CARD_IN_FIELD","owner":"self","filter":{"cardName":"合炎奇炎　タマヨリヒメ之参"}},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"OPTIONAL_TRASH_ENERGY_CLASS"},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"LIFE_CRASH","owner":"opponent","count":1,"triggerBurst":true}}]}}],
  ...
  "WX14-072": [
    {"effectId":"WX14-072-E1","effectType":"AUTO","timing":["ON_PLAY"],"action":{"type":"CHOOSE","choose_count":1,"from_count":2,"choices":[{"choiceId":"WX14-072-E1-c1","label":"手札から＜天使＞のシグニ1枚を公開する","condition":{"type":"HAND_COUNT_FILTER","owner":"self","filter":{"cardType":"シグニ","story":"天使"},"operator":"gte","value":1},"action":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"HAND_REVEAL_CLASS_SIGNI","handCardPick":{"filter":{"cardType":"シグニ","story":"天使"},"count":1}},{"type":"SEARCH","from":{"location":"deck","owner":"self"},"filter":{"cardType":"シグニ","story":"天使"},"maxCount":1,"then":{"type":"TRASH","target":{"type":"DECK_CARD","owner":"self","count":1}},"afterSearch":{"type":"SHUFFLE_DECK","owner":"self"}}]}},{"choiceId":"WX14-072-E1-c2","label":"このシグニを場からトラッシュに置く","action":{"type":"TRASH","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"cardType":"シグニ","thisCardOnly":true}}}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WX14-075": [
  ...
  "WXK04-090": [
    {"effectId":"WXK04-090-E1","effectType":"AUTO","timing":["ON_PLAY"],"action":{"type":"CHOOSE","choose_count":1,"from_count":2,"choices":[{"choiceId":"WXK04-090-E1-c1","label":"手札から＜水獣＞のシグニ1枚を公開する","condition":{"type":"HAND_COUNT_FILTER","owner":"self","filter":{"cardType":"シグニ","story":"水獣"},"operator":"gte","value":1},"action":{"type":"STUB","id":"HAND_REVEAL_CLASS_SIGNI","handCardPick":{"filter":{"cardType":"シグニ","story":"水獣"},"count":1}}},{"choiceId":"WXK04-090-E1-c2","label":"このシグニを場からトラッシュに置く","action":{"type":"TRASH","target":{"type":"SIGNI","owner":"self","count":1,"filter":{"cardType":"シグニ","thisCardOnly":true}}}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"},
  ],
  "WXK05-010": [
  ...
  // ══════════════════════════════════════════════════════════════════════════════
  // §5.3 `O-320`（2026-09-11）＝`WXEX2-13-E1`「あなたのデッキから【ライフバースト】を持つカード１枚を
  //   探してチェックゾーンに置き、デッキをシャッフルする。そのカードのライフバーストを発動する。」
  // 🔴旧 live＝`SEQUENCE[SHUFFLE_DECK, STUB{TRIGGER_LIFE_BURST}]`＝**検索が丸ごと無い**ので
  //   `TRIGGER_LIFE_BURST` が `lastProcessedCards` 不在で `?? ctx.sourceCardNum` へ落ち、
  //   **効果元シグニ自身の【ライフバースト】を撃っていた**（原文と無関係なカードが発動する）。
  // 🔑**受け皿は全部在った**＝`SEARCH{from:deck, filter:{hasLifeBurst}}` は `WD06-018-BURST` /
  //   `WX16-Re08-E1` が既に使っており、`resumeSearch` は選んだカードを `lastProcessedCards` へ載せてから
  //   `then` を実行する＝`STUB{TRIGGER_LIFE_BURST}` がそのまま繋がる（engine 変更ゼロ）。
  // ⚠**デッキから抜かないのは既存の近似に合わせた意図的な据置**＝`TRIGGER_LIFE_BURST` は `field.check` へ
  //   載せるだけで元ゾーンから除かない（`WX13-032-E2` はデッキトップを戻してから／`WXEX1-11-E2` は
  //   相手ライフを見てから撃つ＝同じ形）。ここだけ抜くと族の中で挙動が割れる。
