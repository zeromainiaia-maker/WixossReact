...
export const MANUAL_EFFECTS: Record<string, CardEffect[]> = {
  ...
  // タスク12(viii)（続き137）: 第1能力「【自】ターン終了時、手札を1枚捨ててもよい。そうした場合、次の対戦相手のターン終了時まで、あなたのすべての＜ブルアカ＞のシグニのパワーを＋4000する」が parser に丸ごと欠落していた（parser は E1=絆自のみを産出）。既存 E1（絆自）は触らず、欠落していた第1能力を E2 として**追加のみ**（＝収穫マージで pure-superset として自動採用される。E1 を relabel すると「損失リスク」で held に落ちるため）。TRASH HAND_CARD optional+CONDITIONAL(IS_MY_TURN) は WX24-P4-050-E1 と同型。⚠decompile 表示順は E1(絆自)→E2(捨てる)で原文の印字順とは逆になるが timing 独立のため挙動に影響なし。
  "WX25-CP1-062": [
    {"effectId":"WX25-CP1-062-E2","effectType":"AUTO","timing":["ON_TURN_END"],"triggerScope":"self","action":{"type":"SEQUENCE","steps":[{"type":"TRASH","target":{"type":"HAND_CARD","owner":"self","count":1},"optional":true},{"type":"CONDITIONAL","condition":{"type":"IS_MY_TURN"},"then":{"type":"POWER_MODIFY","target":{"type":"SIGNI","owner":"self","count":"ALL","filter":{"cardType":"シグニ","story":"ブルアカ"}},"delta":4000,"duration":"UNTIL_OPP_TURN_END"}}]},"duration":"INSTANT","mandatory":true,"parseStatus":"MANUAL"}
  ],
