...
export function parseSentencePart3(t: string): EffectAction | null {
  ...
  //   ⚠素の `OPTIONAL_COST`（payload 無し）はコスト0＝**タダで強い方の効果が撃てる**過剰効果になる。
  {
    ...
    // ⚠🔴**「追加で」を必須にすると《リコレクトアイコン》分割の後で当たらない**（2026-09-05 第147バッチ・`WX25-P3-003-E1`）＝
    //   `effectParser.ts` のリコレクト分割は見出し regex `《リコレクトアイコン》［N枚以上］(代わりに|追加で)?` で
    //   **「追加で」ごと食べてから** bonus をパースするので、ここへ来る文は「エクシード３を支払ってもよい」になる。
    //   当たらないと素の `OPTIONAL_COST`（payload 無し）＝**エクシードがタダ**で強い方の効果が撃てる過剰実行。
    const m = t.match(/^(?:その後、)?(?:この(?:スペル|ピース|アーツ|カード)を使用する際、)?(?:使用コストとして)?(?:追加で)?エクシード([０-９\d]+)を支払ってもよい$/);
    if (m) {
      return {
        type: 'STUB', id: 'OPTIONAL_COST',
  ...
  // ---- シグニゾーンを指定する ----
  {
    ...
    const dszM = t.match(/(シグニのない)?(?:あなたの|対戦相手の)?シグニゾーン(?:([０-９\d]+)つを|を([０-９\d]+)つ(?:まで)?)指定する/);
    if (dszM) {
      const countDsz = parseNum(dszM[2] ?? dszM[3] ?? '1');
      return {
        type: 'STUB', id: 'DESIGNATE_SIGNI_ZONE',
        ...(countDsz > 1 ? { count: countDsz } : {}),
        ...(dszM[1] ? { requireEmptyZone: true } : {}),
      } as StubAction;
    }
    const dszNoCountM = t.match(/(シグニのない)?(?:あなたの|対戦相手の)?シグニゾーンを指定する/);
    if (dszNoCountM) {
      return {
        type: 'STUB', id: 'DESIGNATE_SIGNI_ZONE',
        ...(dszNoCountM[1] ? { requireEmptyZone: true } : {}),
      } as StubAction;
    }
  }
