...
const TTH_FILTER_BATCH2_WAVE1_CARDS = new Set([
  ...
  'WX11-018',
  // 🆕§5.3 `O-133` B群 第8バッチ（2026-08-28）＝「あなたのトラッシュから**それぞれ共通する色を持たず
  //   無色ではない**シグニ２枚を対象とし、それらを手札に加える」（`WXDi-P02-031`／`WXDi-P13-034`）。
  //   `nonColorless` が落ちて**無色シグニまで拾える過剰効果**だった（live は手で MANUAL 化してあった＝
  //   凍っていたので計器に映らなかった）。⚠「それぞれ共通する色を持たず」＝**選んだ2枚どうしの制約**は
  //   対象側に受け皿が無いので**まだ表せない**（コスト側の `SelectionConstraint` と同型の機構が要る）。
  'WXDi-P02-031', 'WXDi-P13-034',
...
export function parseSentencePart1(t: string, cardNum?: string): EffectAction | null {
  ...
  if (t.match(/能力を失[うい]/) || t.match(/能力を新たに得られない/)) {
    ...
    // 段2 第45バッチ:「〈誰か〉の**センタールリグと**すべてのシグニは能力を失う」＝`keysAndSigni` の兄弟。
    // 🔴従来は「センタールリグと」が挟まるだけで上の `対戦相手のセンタールリグは能力を失` 専用枝に当たらず、
    //   この汎用枝の **SIGNI 固定**へ落ちて**ルリグ側が丸ごと落ちていた**（`WX19-022-BURST`／`WXDi-P10-005-E3`）。
    const centerLrigAndSigni = /(?:センター)?ルリグと(?:すべての)?シグニ(?:は|が)[^。]*?能力を(?:失[うい]|新たに得られない)/.test(t);
