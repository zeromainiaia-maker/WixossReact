...
const STATE_CONDITION_CLAUSES_V2: Array<[RegExp, (g: string[]) => Condition]> = [
  ...
  // 「このシグニに【チャーム】が付いている場合」（§6.4 O-25(d)・`WXK07-043-E1/E2`）＝上の【アクセ】版の兄弟。
  // 🔴従来は条件節が丸ごと落ちたうえ、E1 では**条件節の「【チャーム】」を付与キーワードとして拾って**
  //   `GRANT_KEYWORD{keyword:'チャーム'}`＝**原文と無関係な別物**に化けていた（本体のバニッシュ耐性は消失）。
  //   E2 は「追加でカードを１枚引く」が**無条件**になっていた（過剰実行）。
  // ⚠下の `THIS_CARD_HAS_ATTACHED`（チャーム/アクセ/ソウルの合計）より**前**に置く（種別指定つきが優先）。
  [/この(?:シグニ|カード)に【チャーム】が付いている場合/,
  ...
  //   「四度目」は盤面全体の通算でしか成立しない）＝engine が `attacked_signi_ids` の件数で判定する。
  // ⚠**「一度目か二度目」は拾わない**（`WX10-018`／`WX17-006`／`SP27-016`）＝あちらは「そのアタックを無効にする」
  //   側の別機構（`negateNthAttack` のカウントダウン窓）が既に実装済み。regex を「N度目の場合」に限定して避ける。
  [/そのアタックがこのターン([一二三四五六七八九十])度目の場合/,
    g => ({ type: 'ATTACK_ORDINAL_THIS_TURN', owner: 'self', operator: 'eq',
      value: '一二三四五六七八九十'.indexOf(g[0]) + 1 })],
  // 「このシグニにカード（N枚）が付いている場合」＝種別を問わない付与（【チャーム】/【アクセ】/【ソウル】の合計。
  //   THIS_CARD_HAS_ATTACHED・§6.3 J-2・WXK10-049-E2）。従来は条件節が丸ごと落ちて無条件【ランサー】だった。
  //   ⚠上の【アクセ】限定より後に置く（種別指定つきは専用の条件型が優先）。
  [/このシグニにカード(?:([０-９\d]+)枚)?が付いている場合/,
    g => ({ type: 'THIS_CARD_HAS_ATTACHED', ...(g[0] ? { minCount: parseNum(g[0]) } : {}) })],
...
/**
 * 引用文が丸ごと `GRANT_KEYWORD.keyword` へ流れ込んだ「〈コスト〉を払わないかぎりアタックできない」を
 * `SIGNI_ATTACK_BAN{targetsStored, unlessPay…}` へ組み替える
 * （§6.4 O-3＝手札捨て版・`WXDi-P05-022-E1`／§6.4 O-28＝《無》×N 版・5効果）。
 *
 * 🔴`keyword` は**正式名でしか照合されない**ので、文がそのまま入ると**一度も効かない無言 no-op**
 *   （狙われたシグニ／ルリグが無条件でアタックできる）。`hasKeyword` の照合外なのでどの計器にも映らない。
 * 正準形は `SELECT_TARGET_ONLY → STORE_LAST_PROCESSED_TARGETS → 〈ban〉{targetsStored}`
 *   （`WX10-024-E2` の《無》×N 版と同じ組み立て）。
 *
 * ⚠**コスト句の無い「アタックできない」（live 132件）は対象外**＝`keyword_grants` の正規語彙なので
 *   regex はコスト句を必須にする。
 * ⚠対象がルリグでも**同じ形**を作る（シグニ／ルリグの仕分けは engine が確定した対象の Type で行う＝
 *   「対戦相手のルリグかシグニ1体」は選ばれるまでどちらか決まらない）。
 */
function rewriteAttackTaxKeywordGrant(action: EffectAction): EffectAction {
  ...
  // 「あなたが」＝付与先の持ち主（＝アタックする側）を指す自明の主語なので落としてよい。
  const kw = gk.keyword.replace(/^あなたが/, '');
  const hand = kw.match(/^手札を([０-９\d]+)枚捨てないかぎりアタックできない$/);
  const colorless = kw.match(/^((?:《無》)+)を支払わないかぎりアタックできない$/);
  if (!hand && !colorless) return action;
  ...
  return { type: 'SEQUENCE', steps: [
    ...
    { type: 'SIGNI_ATTACK_BAN', owner: banOwner, targetsStored: true,
      ...(hand ? { unlessPayHandDiscard: parseNum(hand[1]) } : {}),
      ...(colorless ? { unlessPayColorless: (colorless[1].match(/《無》/g) ?? []).length } : {}),
    } as EffectAction,
  ] } as SequenceAction;
}
...
function parseSingleSentenceInner(text: string): EffectAction {
  ...
  // 「このシグニより低いレベルを持つあなたのシグニN体を対象とし、…」（§6.3 J-4・WXK11-018-E2）＝効果元シグニの
  // レベルを基準にした動的レベルフィルタ（`levelLtSelf`＝**型にも engine にもあるのに parser が合成していなかった**
  // ＝doc コメントが WXK11-018 を名指ししている語彙）。上の levelLtOppLrig と同じ「先頭修飾句を剥がして刻む」型。
  // 剥がさないと汎用ターゲット解析が「このシグニ」に釣られて `thisCardOnly`（＝自分自身）へ誤解決する。
  {
    const m = text.trim().match(/^このシグニより低いレベルを持つ(?:、)?(.+)$/s);
    if (m) {
      const inner = parseSingleSentence(m[1]);
      let stamped = false;
      const walk = (a: EffectAction | undefined | null): void => {
        if (stamped || !a || typeof a !== 'object') return;
        const tgt = (a as { target?: { type?: string; owner?: string; filter?: Record<string, unknown> } }).target;
        if (tgt && tgt.type === 'SIGNI') {
          tgt.filter = tgt.filter ?? { cardType: 'シグニ' };
          delete tgt.filter.thisCardOnly;   // 「このシグニより低いレベル」＝**自分自身ではない**
          tgt.filter.cardType = 'シグニ';
          tgt.filter.levelLtSelf = true;
          stamped = true;
          return;
        }
        if (a.type === 'SEQUENCE') for (const st of (a as SequenceAction).steps) { walk(st); if (stamped) return; }
  ...
  const t = text.trim().replace(/。$/, '')
    ...
    // §6.4 O-3：「この方法で加えたアタックフェイズ（の）開始時、〜」＝追加フェイズの開始時本文。
    // ここで剥がして本文だけを解析し、下の SEQUENCE 畳み込みが `ADD_EXTRA_ATTACK_PHASE.onStart` へ移す。
    .replace(/^この方法で加えたアタックフェイズの?開始時、/, '')
  ...
  {
    // ⚠**読点なしの形も受ける**（§6.4 O-31・続き537）＝`WXDi-P11-044-E2`
    //   「《無》を支払わないかぎりこのシグニを場からトラッシュに置く。」は読点が無いだけで同じ形なのに
    //   前置きが丸ごと落ち、下流の汎用規則が本体だけを拾って **`thisCardOnly` も無い
    //   「自分のシグニ1体を選んでトラッシュ」** に化けていた（払っても落ちる／自身以外も落とせる）。
    // ⚠**「〜できない」で終わる本体は拾わない**＝「〜を支払わないかぎりアタックできない／【ガード】が
    //   できない／配置できない」は制限型（§6.4 O-28・ガード税・`SIGNI_ATTACK_BAN` 等）の領分で、
    //   読点の有無で区別していた従来のガードをここで**本体の形**に置き換える。実測28効果のうち
    //   読点なしの consequence 型は本件1件だけ（他は制限型か「対戦相手は」の別経路）。
  ...
  {
    // ⚠表と分割正規表現は**モジュール直下の単一定義**（`CONJ_FIN` / `CONJ_SPLIT_RE`）を使う。
    //   ここに写しを作ると、下の「条件節つきチェーンの脱落補完」（§6.4 O-11）と語彙がズレる。
    ...
    if (conjM && !isSearchLike && !/対象とし、|として、|とき|場合|代わりに/.test(t)) {
      ...
      // SELF_TO_DECK_TOP は「このシグニ」だけを動かす単動作 STUB（engine 実装済み）なので、
      // WXK03-051 の右半分に限って安全に同居できる。他の文全体 STUB は従来どおり分割しない。
      const safeRightStub = left.type === 'ENERGY_CHARGE_FROM_DECK'
        && right.type === 'STUB' && right.id === 'SELF_TO_DECK_TOP';
      if (left.type !== 'UNKNOWN' && right.type !== 'UNKNOWN' && (right.type !== 'STUB' || safeRightStub)) {
        return { type: 'SEQUENCE', steps: [left, right] } as SequenceAction;
      }
    }
  }
...
const STATE_COND_BATCH4_ACTIONS: Record<string, string> = {
  ...
  // ⚠**このリテラルは `parseSingleSentence` の後処理を通らない**（table が action を丸ごと差し替える）＝
  //   `rewriteAttackTaxKeywordGrant`（§6.4 O-28）が届かないので、正準形をここに直接書く。
  //   旧リテラルは引用文が丸ごと `keyword` に入っていた＝`hasKeyword` の照合外で**一度も効かない無言 no-op**。
  'WX11-012-E1': '{"type":"CONDITIONAL","condition":{"type":"LRIG_STORY","owner":"self","story":"タマ"},"then":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"SIGNI_ATTACK_BAN","owner":"opponent","targetsStored":true,"unlessPayColorless":4}]},"else":{"type":"SEQUENCE","steps":[{"type":"STUB","id":"SELECT_TARGET_ONLY","selectTarget":{"type":"SIGNI","owner":"opponent","count":1,"filter":{"cardType":"シグニ"},"upToCount":false}},{"type":"STUB","id":"STORE_LAST_PROCESSED_TARGETS"},{"type":"SIGNI_ATTACK_BAN","owner":"opponent","targetsStored":true,"unlessPayColorless":3}]}}',
...
function parseActionTextInner(text: string): EffectAction {
  ...
  // abilities は expandGrantLrigAbilities が rawText から展開する（既存規則と同方式）。
  {
    ...
    // 🆕「あなたのレベルN以上のセンタールリグ１体を対象とし、**次のあなたのエナフェイズ終了時まで、
    //    それのリミットを＋Nし**、それは以下の能力を得る。『…』」＝**リミット修正と能力付与の複合**
    //    （§6.4 O-27・続き536・`WX24-P3-001/003/005/007/009` の5枚）。
  ...
  const sentences = splitSentences(text).filter(s => {
    ...
    // 「**そのターン終了時**、…」＝上の兄弟（§6.4 O-14(b)・`WXDi-P08-010-E3`）。直前の文が
    // 「次の対戦相手のターンの間、〜」で定めたターンを指す。⚠書き換えないと**遅延宣言そのものが消え**、
    // 本文だけが即時実行される＝原文「そのターン終了時に…バニッシュする」が**使った瞬間の全体バニッシュ**へ化ける
    // （実測＝`BANISH{SIGNI owner:'any', count:'ALL'}` が即走り、**自分のシグニまで巻き込む**）。
    .map((s, i, all) => {
      if (i === 0 || !/^そのターン終了時[、,]/.test(s.trim())) return s;
      return /次の対戦相手のターン/.test(all[i - 1]) ? s.replace(/^そのターン終了時/, '次の対戦相手のターン終了時') : s;
    });
  // CHOOSEパターン共通ヘルパー
  ...
  if (sentences.length === 1) {
    ...
    //   「〈対象宣言〉、〈任意コスト〉てもよい。**そうした場合、それを〜**」の2文構成だと、
    //   ①②行は上の番号フィルタで落ち、**この帰結文だけが生き残って単文パスへ流れる**
    //   ＝CHOOSE が丸ごと消えたうえ、先行詞のない「それを〜する」が `owner:'self'` へ反転していた
    //   （🔴`WXDi-P07-055-BURST`／`WXDi-P07-072-BURST` は**自分のシグニを手札に戻す/デッキに送る**、
    ...
    if (
      ...
      (/^(?:その中から|残りを|以下の)/.test(s.trim()) || isOptionTail)
    ) {
...
function splitEffectBlocks(text: string): string[] {
  ...
  const normalized = text
    ...
    // 🆕**キーワード単独の【常】ブロック**も同型（§6.4 O-11・続き532）＝
    //   `【常】：【アサシン】【常】：【ダブルクラッシュ】`（`WX13-030`）のように**キーワードだけの能力**が
    //   句点なしで次のマーカーへ直結する。分割しないと**後続のブロックが丸ごと欠落**する。実測8カード＝
    //   【アサシン】`WX13-030`/`WXK01-036`/`WXK10-039`／【ランサー】`WXK10-018`/`WXDi-P04-040`／
    //   【シャドウ】`WX22-025`／【シャドウ（スペル）】`WXDi-P09-048`/`WX25-P2-118`。
    // ⚠**マーカー自身（【自】【出】【起】）を区切りに足してはいけない**＝
    //   「対戦相手のシグニの**【自】【出】【起】能力**が発動する場合」（`WXDi-P08-044`）のように
    //   **文中の参照**として並ぶ形があり、そこで割ると文が壊れる。区切りに足すのはキーワード token だけ。
    .replace(new RegExp(
      `(【エナチャージ[０-９\\d]+】|【シュート】|【ダブルクラッシュ】|【アサシン】|【ランサー】|【シャドウ(?:（[^）]*）)?】)(?=${MARKER_PAT})`,
      'g'), '$1。')
  ...
  switch (marker) {
    ...
    case '自':
      ...
      timing = trigText.includes('《ヘブン》したとき') ? ['ON_HEAVEN']
             ...
             // 🆕§6.4 O-7：「このシグニがアタックしたとき、**そのアタック終了時**、〜」＝発火はアタック宣言時では
             //   なく**そのアタックの解決が終わった地点**（`collectAttackEndTriggers`）。従来は `trigText` が
...
function parseArtsEffect(card: CardData): CardEffect | null {
  ...
  const printedUseConditionPatterns: { pattern: RegExp; build: (match: RegExpMatchArray) => Condition }[] = [
    ...
    {
      // 【チーム】**いずれかのチーム**＝チーム名を名指しせず「3体が同じチーム」だけを要求する
      // （`WX25-P3-050`・§6.4 O-34(d)）。🔴従来はこの1枚だけ先頭節が本文へ流れ込み、
      //   `GRANT_KEYWORD{keyword:'使用条件'}`（§6.4 O-28 のゴミ keyword）に化けて
      //   **続く「白のルリグ1体につき【ルリグバリア】」節ごと消えていた**。
      pattern: /^【使用条件】【チーム】いずれかのチーム/,
      build: () => ({ type: 'LRIG_ANY_TEAM_COUNT', owner: 'self', value: 3 }),
    },
    {
...
export function parseCardEffects(card: CardData): CardEffect[] {
  ...
  if (baseType === 'アーツ' || baseType === 'ピース' || baseType === 'リレーピース') {
    ...
    // 🔴**【トラップアイコン】節はスペル本体の続きではない**（§6.4 O-34(a) で発見・シグニ側は
    //   13625 行で既に除去済みだったのにスペル側だけ素通りしていた）＝下段で独立した `-TRAP` effect
    //   として解析されるのに、本体側にも残るので**スペルを唱えた瞬間にトラップ本体まで走る**。
    //   実測 5枚中4枚が該当（`WX15-053`＝バニッシュ／`WX17-044`＝ダメージ無効／`WX17-071`＋
