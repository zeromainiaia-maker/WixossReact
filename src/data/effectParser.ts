...
function parseSingleSentenceInner(text: string): EffectAction {
  ...
  //   当たらない＝**終止形へ正規化してから**パースする。
  {
    ...
    const conjM = t.match(/^(.*?(?:バニッシュし|ダウンし|アップし|凍結し|手札を[^、。]{0,6}捨て|場に出し|場からトラッシュに置き|デッキの一番上のカードをエナゾーンに置き|デッキの上からカードを[０-９\d]+枚トラッシュに置き|すべてのシグニをエナゾーンに置き|【エナチャージ[０-９\d]+】をし|デッキの一番上のカードをライフクロスに加え|カードを[０-９\d]+枚引き|自分のシグニ[０-９\d]+体を選びトラッシュに置き|自分のエナゾーンからカード[０-９\d]+枚を選びトラッシュに置き))、(.+)$/s);
