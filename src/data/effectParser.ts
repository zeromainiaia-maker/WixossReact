...
function parseSingleSentenceInner(text: string): EffectAction {
  ...
  {
    const CONJ_FIN = CONJ_FIN_TABLE;
    void ([
      [/バニッシュし$/, 'バニッシュする'], [/ダウンし$/, 'ダウンする'], [/アップし$/, 'アップする'],
      [/凍結し$/, '凍結する'], [/捨て$/, '捨てる'], [/場に出し$/, '場に出す'],
      // タスク12(lxxvii)：列挙外だった具体的な動詞句だけを追加する。⚠包括的な「し$」では割らない
      //   （名詞の「〜し」を巻き込む）。枚数は [０-９\d]+ で一般化してよい＝**特定カードの本文を埋め込まない**
      //   （決め打ちにすると次の同型カードが黙って落ち、外した人が理由を復元できない）。
    ...
    // 「シグニゾーンをNつ(まで)指定し、…」（§6.4 O-16・`WX25-P3-014-E2`）＝**指定と本体の2動作**。
    // ⚠この連用形が候補に無かったため、2ゾーン指定の札は指定ステップが丸ごと生成されず、
    //   後段が「対戦相手のシグニ1体を対象とし」に潰れていた（指定が無いので engine も読めない）。
    const conjM = t.match(/^(.*?(?:バニッシュし|ダウンし|アップし|凍結し|手札を[^、。]{0,6}捨て|場に出し|場からトラッシュに置き|シグニゾーンを?[０-９\d]+つ(?:まで)?指定し|デッキの一番上のカードをエナゾーンに置き|デッキの上からカードを[０-９\d]+枚トラッシュに置き|すべてのシグニをエナゾーンに置き|【エナチャージ[０-９\d]+】をし|デッキの一番上のカードをライフクロスに加え|カードを[０-９\d]+枚引き|自分のシグニ[０-９\d]+体を選びトラッシュに置き|自分のエナゾーンからカード[０-９\d]+枚を選びトラッシュに置き))、(.+)$/s);
...
function parseActionTextInner(text: string): EffectAction {
  ...
  for (const s of loopSentences) {
    ...
    const thenM = clean.match(/^(?:そうした場合、|その後、(?:[^、]+の?場合、|この方法で.+(?:支払った|た)場合、)|この方法で.+(?:支払った|た)場合、|あなたの登録者数が[０-９\d]+万人を達成していて、この方法で.+た場合、|(?:《[^》]+》)+を支払った場合、|それが【ライフバースト】を(?:持たない|持つ)場合、|それが[^、。]+の場合、)/);
