...
export function parseSelfComparison(text: string): Partial<TargetFilter> {
  ...
  // 🆕「**この方法で捨てた／公開した／場に出した**シグニのパワーの半分以下」（2026-09-11・§5.3 `O-320`・
  //   `WDK10-015-E1`）＝基準が**効果元自身ではなく直前に処理した札**。⚠`powerLteSelfHalf` より**先に**判定する
  //   （「このシグニ」を含まないので後勝ちでも当たらないが、将来「このシグニが捨てた」形が来たとき取り違える）。
  //   🔴従来この形の規則が無く、フィルタが丸ごと落ちて**相手のどのシグニでもバニッシュ**できる過剰効果だった。
  if (/この方法で(?:捨てた|公開した|場に出した|トラッシュに置いた)シグニのパワーの半分以下/.test(text)) return { powerLteLastProcessedHalf: true };
...
export function parseStoryFilter(text: string): Partial<TargetFilter> {
  ...
  // 🆕🔴**ガードを2つ足した**（2026-09-05・第141バッチ）＝上の位置判定は**過剰にも発火する**。
  //   held（収穫マージの温存）に **5枚**の退化として溜まっていた（`WDK16-13` `WX12-Re10` `WX14-069`
  //   `WX24-P1-064` ほか）。⚠**live は正しいので実機は壊れていなかった**＝温存機構が止めていた。
  //   (b)🔴**text 全体が条件節そのもの**（末尾が「…場合、」）のときは、落とすと**クラスが丸ごと消える**
  //       ＝`parseThisWayGenericCount` のように**条件節だけ**を渡す呼び出しがある
  //       （`WDK16-13`「この方法でレベル２以下の＜電機＞のシグニが公開された場合、」＝
  //        レベル限定だけが残って**どのクラスでも成立する過剰実行**になっていた）。
  //   (a)🔴**「を対象と」より前にクラスがある**なら、それは条件節ではなく**対象の修飾語**
  //       ＝`WX24-P1-064`「あなたの＜宝石＞のシグニ１体を対象とし、…『…であるかぎり、…』を得る」で、
  //        **引用能力の中の「かぎり、」**にガードが当たって対象の＜宝石＞まで消えていた。
  //   ⚠**位置で割ること**＝「を対象と」の**後ろ**にクラスがある形（`WX09-025`「シグニ１体を対象とし、
  //     あなたの場に＜鉱石＞か＜宝石＞のシグニが合計３体ある場合、それをバニッシュする」）は
  //     **従来どおり落とす**（対象は無限定）。単に「を対象と」の有無で分けると、この形が過小実行に反転する。
  const gate = (() => {
    const m = text.match(/^.*?(?:場合|かぎり|とき)、/s);
    if (!m) return null;
    const prefix = m[0];
    if (text.slice(prefix.length).trim() === '') return null;          // (b) 全体が条件節
    const tgt = prefix.search(/を対象と/);
    if (tgt >= 0) {
      const lastCls = prefix.lastIndexOf('＜');
      if (lastCls >= 0 && lastCls < tgt) return null;                  // (a) クラスが対象指定より前
    }
    return m;
  })();
