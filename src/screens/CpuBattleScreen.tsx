...
export default function CpuBattleScreen({ user: _user, myDeckId, decks, cards, onBack }: Props) {
  ...
  const processUp = useCallback((g: CpuGameState): CpuGameState => {
    ...
    const newS: PlayerState = {
      ...
      hand_signi_guard_enabled: undefined,
  ...
  const lrigAttack = useCallback((g: CpuGameState): CpuGameState => {
    ...
    if ((attacker.keyword_grants?.[lrigInstId] ?? []).includes('アタックできない')) return g;
    ...
    switch (g.phase) {
      ...
      case 'ATTACK_SIGNI': {
        ...
        const upZones = cpuAttackLimitHit ? [] : [0, 1, 2].filter(i => {
          ...
          // cannotAttackSigni: 常在効果・keyword_grants による攻撃不可チェック
          if (cpuContBlocked.cannotAttackSigni.has(topId)) return false;
          return true;
        });
        if (upZones.length > 0) {
  ...
  const getPlayerSigniZoneActions = useCallback((rawZoneIdx: number): CardAction[] => {
    ...
    if (gs.phase === 'ATTACK_SIGNI') {
      ...
      const playerContBlocked = calcContinuousBlockedActions(gs.player, gs.cpu, true, effectsMap, cardMap);
      if (stack?.length && topId && !isDown && !attackLimitHit && canPayAtkCost && !playerContBlocked.cannotAttackSigni.has(topId)) {
