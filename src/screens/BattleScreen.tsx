...
import type { BattleStateRow, PlayerState, CardData, PendingSpell, PendingEffect, PendingInteractionDef, StackEntry, EffectStack } from '../types';
...
import { acceCardsAt, allAcceCards, cloneAcceSlots, countAcce, hasAcceAt } from '../utils/acce';
      ...
      if (inter.type === 'SELECT_TARGET') {
        if (inter.totalPowerMax !== undefined) {
          ...
          for (const n of shuffled) {
            ...
            if (canAddToSelection(selected, n, inter.selectionConstraint, new Map(cards.map(c => [c.CardNum, c])))) selected.push(n);
          }
        }
        ...
        for (const n of inter.visibleCards) {
          if (selected.length >= count) break;
          if (canAddToSelection(selected, n, inter.selectionConstraint, new Map(cards.map(c => [c.CardNum, c])))) selected.push(n);
        }
