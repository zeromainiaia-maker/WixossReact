...
collectEffectImmuneSigni, collectContinuousGrantedKeywords, collectBanishSubstitutes, collectForcedFrontAttackZones, collectGrowCostReductions} from '../engine/effectEngine';
...
import { CPU_PLAYER_ID, CPU_ACTION_DELAY, generateUUID, shuffle, InstanceMap, parsePowerVal, assignInstanceIds, assignGuestInstanceIds, drawCards, jankenWinner } from './battle/battleUtils';
import { fmtHandDiscardSigniLabel, fmtDiscardFilterLabel, parseGrowCost, removeNColorFromCost, applyContinuousCostDecreases, applyGrowCostReduction, computeArtsEffectiveCost, isMultiEna, canAffordGrowCost, parseCoinCost, parseBetOptions, parseEncoreCost, canAffordWithExtraCost, energyCostToString, findCounterSpellMaxCost, effectEnergyCostStr } from './battle/costs';
...
import { computeFieldSigniLimit, fieldTrashGroupsSatisfied, fieldTrashGroupsAffordable, reduceFieldSigniToLimit } from './battle/fieldLimit';
...
import type { BattleModalCtx } from './battle/modals/types';
...
export default function BattleScreen({ user, roomId, myDeckId, cards, onBack }: Props) {
  ...
  const resolveStackNext = async () => {
    ...
    try {
      ...
      if (!result.done) {
        ...
        // ON_ALLY_PLAY_OR_OPP_HAND_DISCARD（OR複合・WXDi-P11-064）: 「あなたのターンの間」のみ＝ターンプレイヤーを
        // controller として、この解決で味方シグニが場に出た（play枝）か相手手札がトラッシュに置かれた（discard枝・⚠自効果限定は近似）
        // 場合に発火。play枝は triggerFilter（他の＜天使＞）で絞る。
        {
          const apTurnIsHost = stack.turnPlayerId === bs.host_id;
          const apTurnBefore = apTurnIsHost ? bs.host_state : bs.guest_state;
          const apTurnAfter = apTurnIsHost ? hostState : guestState;
          const apOppBefore = apTurnIsHost ? bs.guest_state : bs.host_state;
          const apOppAfter = apTurnIsHost ? guestState : hostState;
          ...
          if (allyPlaced.length > 0 || oppDiscarded > 0) {
            const ap = collectAllyPlayOrOppDiscardTriggers(stack.turnPlayerId, apTurnAfter, allyPlaced, oppDiscarded);
            if (ap.usedOncePerTurnIds.length > 0) {
              const key = apTurnIsHost ? 'host_state' : 'guest_state';
              const base = (update[key] as PlayerState) ?? apTurnAfter;
              update[key] = { ...base, actions_done: [...(base.actions_done ?? []), ...ap.usedOncePerTurnIds] };
            }
            if (ap.entries.length > 0) {
              const baseStackAP = (update.effect_stack as typeof stackAfter) ?? null;
              update.effect_stack = baseStackAP
                ? pushToStack(baseStackAP, ap.entries)
                : initStack(stack.turnPlayerId, ap.entries);
            }
          }
        }

        // ON_MATERIAL_USED（self/any_ally・改造素材機構 Step3b）: この解決で MARK_MATERIAL_TARGET が
        // material_used_targets を積んだ場合、対象シグニ所有者の「このシグニに/他の味方に使用されたとき」を発火し、
        // 処理後に material_used_targets をクリア（transient）。
        for (const muIsHost of [true, false]) {
          const muOwnerId = muIsHost ? bs.host_id : bs.guest_id;
          const muBefore = (muIsHost ? bs.host_state : bs.guest_state)?.material_used_targets ?? [];
          const muAfterState = (update[muIsHost ? 'host_state' : 'guest_state'] as PlayerState) ?? (muIsHost ? hostState : guestState);
          ...
          if (newTargets.length > 0) {
            ...
            // material_used_targets をクリア（＋発火した once_per_turn を actions_done へ）
            const keyMU = muIsHost ? 'host_state' : 'guest_state';
            update[keyMU] = { ...muAfterState, material_used_targets: [],
              actions_done: [...(muAfterState.actions_done ?? []), ...mu.usedOncePerTurnIds] };
            if (mu.entries.length > 0) {
              const baseStackMU = (update.effect_stack as typeof stackAfter) ?? null;
        ...
        // ON_LRIG_UNDER_MOVED（C1・WXDi-P04-042）: この解決でセンタールリグの下からカードが移動した場合、
        // 「あなたのターンの間」＝ターンプレイヤーの場シグニの self【自】を発火（collector が turn 限定をゲート）。
        for (const luIsHost of [true, false]) {
          const luOwnerId = luIsHost ? bs.host_id : bs.guest_id;
          const luBefore = luIsHost ? bs.host_state : bs.guest_state;
          const luAfter = luIsHost ? hostState : guestState;
          if (countLrigUnderMoved(luBefore, luAfter) > 0) {
            const lu = collectLrigUnderMovedTriggers(luOwnerId, luAfter);
            if (lu.usedOncePerTurnIds.length > 0) {
              const keyLU = luIsHost ? 'host_state' : 'guest_state';
              const baseLU = (update[keyLU] as PlayerState) ?? luAfter;
              update[keyLU] = { ...baseLU, actions_done: [...(baseLU.actions_done ?? []), ...lu.usedOncePerTurnIds] };
            }
            if (lu.entries.length > 0) {
              const baseStackLU = (update.effect_stack as typeof stackAfter) ?? null;
              update.effect_stack = baseStackLU
                ? pushToStack(baseStackLU, lu.entries)
                : initStack(stack.turnPlayerId, lu.entries);
            }
          }
        }

        // ON_DECK_SHUFFLED（C1・PR-470A）: この解決でデッキがシャッフルされた（deck_shuffled_count 増加）場合、
        // そのプレイヤーの場シグニの self【自】を発火。⚠execShuffleDeck 経由のみ（リフレッシュ等は未計上）。
        for (const dsIsHost of [true, false]) {
          const dsOwnerId = dsIsHost ? bs.host_id : bs.guest_id;
          const dsBefore = dsIsHost ? bs.host_state : bs.guest_state;
          const dsAfter = dsIsHost ? hostState : guestState;
          if (detectDeckShuffled(dsBefore, dsAfter)) {
            const ds = collectDeckShuffledTriggers(dsOwnerId, dsAfter);
            if (ds.usedOncePerTurnIds.length > 0) {
              const keyDS = dsIsHost ? 'host_state' : 'guest_state';
              const baseDS = (update[keyDS] as PlayerState) ?? dsAfter;
              update[keyDS] = { ...baseDS, actions_done: [...(baseDS.actions_done ?? []), ...ds.usedOncePerTurnIds] };
            }
            if (ds.entries.length > 0) {
              const baseStackDS = (update.effect_stack as typeof stackAfter) ?? null;
              update.effect_stack = baseStackDS
                ? pushToStack(baseStackDS, ds.entries)
                : initStack(stack.turnPlayerId, ds.entries);
            }
          }
        }
    ...
    try {
      ...
      if (!effectivelyEmpty && opTopCardNum && opTopCard) {
        ...
        if (myPower >= opPower) {
          ...
          appendBattleLogs([`${myCardName}が${opCardName}をバニッシュ${redirectBanish ? '（トラッシュへ）' : redirectBanishToHand ? '（手札へ）' : frozenToDeckBottom ? '（凍結→デッキ下）' : frozenToTrash ? '（凍結→トラッシュ）' : banishToLrigTrash ? '（ルリグトラッシュへ）' : energyToDeckBottom ? '（エナ代替→デッキ下）' : defenderLeaveExile ? '（除外＝トラッシュへ）' : ''}`]);
          }
  ...
  const modalCtx: BattleModalCtx = { my, op, isMyTurn, loading, battleCards, battleCardMap, effectsMap, myEnaAllMulti, myColorlessOverrides, myColorSubs, pickLongPressTimer, setExpandedPickImgUrl, activeCostMods, myEnergyExtraColors, myEnergyTrashSubInfo, myLrigNameAliases, myArtsThresholdReductions, isActionBlocked };
