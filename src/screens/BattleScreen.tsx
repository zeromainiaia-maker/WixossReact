...
export default function BattleScreen({ user, roomId, myDeckId, cards, onBack }: Props) {
  ...
  const handleCutinPass = async () => {
    ...
    try {
      ...
      console.error(`[DIAG-CUTIN] done=${result.done} cntB=${bs.host_state?.deck_shuffled_count ?? 0} cntA=${(hostState as PlayerState).deck_shuffled_count ?? 0} suEntries=${spellUseEntries.length} resCnt=${(result.ownerState as PlayerState).deck_shuffled_count ?? 0}`);
