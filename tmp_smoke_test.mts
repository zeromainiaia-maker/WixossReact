...
// 1. RECOLLECT_GATE 未達（アーツ2枚 < 必要4枚）→ 残りステップ実行されない
{
  ...
  check('RECOLLECT_GATE未達', r.ctx.logs.some((l: string) => l.includes('リコレクト条件未達')) && r.ctx.ownerState.hand.length === 0,
    `logs=${JSON.stringify(r.ctx.logs)} hand=${r.ctx.ownerState.hand.length}`);
}
...
// 2. RECOLLECT_GATE 達成（アーツ4枚）→ 後続DRAWが実行される
{
  ...
  check('RECOLLECT_GATE達成', r.ctx.logs.some((l: string) => l.includes('リコレクト条件達成')) && r.ctx.ownerState.hand.length === 1,
    `logs=${JSON.stringify(r.ctx.logs)} hand=${r.ctx.ownerState.hand.length}`);
}
