...
| 日付 | 実装内容 | 対象STUB |
|------|---------|---------|
| 2026-06-02 v0.169 | 6件✅化: GATE(blocked_actions['ATTACK:id']をBattleScreen/CpuBattleScreenアタック時チェック)/DEPLOY_RESTRICT(signi_deploy_power_limitをhandleSummonSigni+CPUメインフェイズ+placeSigniでチェック+UI「パワー制限」表示)/ARTS_IMMOVABLE(execStub line 2271でARTS_USE_DISCARD_LRIG_DECKがフィルタ済み確認)/CONDITIONAL_PER_TRASH(テキスト解析→N枚閾値→ドロー実装済み確認) | 4種6件 |
