...
## 2026-06-11 タイミング不一致修正ラウンドで新設したSTUB
...
|-----------|---------|--------|---------|
| CONT | ✅ DRIVE_SIGNI_POWER_DOUBLE_CRASH | WXK01-001 | effectEngineでドライブシグニ+3000、BattleScreenでダブルクラッシュ付与 |
| AUTO(出) | ✅ OPP_PUNISHER_CHOICE | WXK05-001 | CHOOSE opponentResponds:true（手札2捨/エナ3/シグニ1） |
| AUTO(自) | ✅ BANISH_FACING_IF_SELF_POWER_GE_15000 | WD17-009 | sourceCardNum+effectivePowersで正面バニッシュ |

**ログのみ📝**（execStub既定フォールバック）:

| effectType | STUB ID | カード | 効果概要 |
|-----------|---------|--------|---------|
| AUTO(出) | ✅ LRIG_TRASH_TO_UNDER_AND_RETURN_ARTS | WX05-001, WXEX2-84 | ルリグトラッシュの全ルリグをfield.lrig先頭に、アーツをlrig_deckへ（v0.270） |
| AUTO(出) | ✅ LIFE_CLOTH_LOOK_TRASH_REFILL | WX05-010 | 全ライフクロスを任意枚数トラッシュ→デッキ上から同数補充（並び替えは近似・省略）（v0.271） |
| AUTO(自) | ✅ ARTS_SELF_RECYCLE_ON_TRIGGER | WX10-015, WX10-027, WXK01-041 | collectTurnTriggers/collectFieldTriggers/performLifeBurstResponseのlrig_trash走査追加。CHOOSE(pay/skip)+INTERNAL_ARTS_RECYCLE_EXECUTEでlrig_trash→lrig_deck shuffle（v0.281） |
| AUTO(出) | ✅ DECLARE_ZONE_FOR_CLASS_CHANGE | WX14-032 | CHOOSE(deck/hand/signi/trash)→declared_class_zones記録。ExecCtx構築時にapplyDeclaredZoneClassOverrideでcardMap上書き（CardClass:'精元',Color:''）（v0.283） |
| ACTIVATED | ✅ OPP_LRIG_LOSE_ABILITY | WX20-003 | 相手ターン専用（CONT無効化フラグ実装、カットイン未実装のため実用上は不発）（v0.272） |
| CONT | ✅ UNLIMITED_KEYS | WXK03-003A | PlayerField.key_piece_extra追加。lrig CONT UNLIMITED_KEYS検出→2枚目以降をkey_piece_extraへ格納。getKeyPieceActionsで全キーのACTIVATED効果収集（v0.282） |
| AUTO(自) | ✅ BANISH_IF_DISCARDED_3_THIS_TURN | WXK03-021 | turn_hand_discarded_countカウンター追加、ON_ATTACK_PHASE_START配線済み（v0.273） |
| CONT | ✅ SELF_BUFF_BY_UNDER_CARDS | WXK05-035 | 下のLv4×3枚条件で+2000（effectEngine applyEffects）。耐性は近似（v0.269） |
| AUTO(出) | ✅ SELF_TRASH_UNLESS_TRASH_OTHERS | WXK10-039 | 他の＜原子＞2体をトラッシュに置かないかぎりこのシグニをトラッシュ（v0.267） |
| AUTO(出) | ✅ DRAW_IF_CHARGED_CLASS | WDK07-E01 | 直前のエナチャージで＜調理＞シグニが置かれた場合1ドロー（v0.269） |
| CONT | UNDER_CARD_HOST_BUFF | WXDi-P05-060 | ※JSONに存在しないため不要（WXDi-P05-060のCONT効果はPOWER_MODIFY済み） |
| CONT | ✅ TREAT_AS_LEVEL1_IN_DECK_TRASH | WXDi-P05-086 | collectDeckTrashLevel1Nums(effectEngine)+ExecCtx.deckTrashLevel1Nums追加。execSearchでオーバーライドcardMapを構築しType:'シグニ',Level:'1'を適用（v0.280） |
| AUTO(自) | ✅ TRASH_UNDER_SPELLS_POWER_MINUS | WXDi-P10-040 | アタック時、下のスペルを好きな枚数トラッシュ→1枚につき-5000（v0.268） |
| AUTO(自) | ✅ BUFF_HOST_WHEN_PLACED_UNDER | WXDi-P11-063 | TRAP_OPERATION拡張（の下に置いてもよい検出）+INTERNAL_PLACE_SELF_UNDER_SIGNI（trash→signi stack配置+ON_PLACED_UNDER_SIGNI即時発火）。E2 actionをSTUBのみに変更。BUFFはstack.indexOf探索でhost特定→temp_power_mods+2000（v0.284） |
| CONT | ✅ TREAT_AS_CLASS_ALL_ZONES | WXDi-CP02-103, WX26-CP1-100, WX26-CP1-101 | すべての領域で特定クラスとして扱う（collectTreatAsClassAllZones+matchesFilter allZoneOverrides対応）（v0.274） |

**2026-06-13 v0.279で✅化（5件）**:

| STUB ID | カード | 実装内容 |
|---------|--------|---------|
| GAIN_LRIG_BARRIER | WX24-P1-001, WX25-P3-001 | lrig_barrier+1（PlayerStateフィールドはv0.278で追加済み） |
| EXILE_SELF_AFTER_USE | PR-378 | sourceCardNumをfieldから除去→trash追加（ゲームから除外の近似） |
| ATTACH_SEARCHED_AS_ACCE | WXK10-074 | lastProcessedCards[0]をsourceCardNumに設定→ATTACH_ACCE |
| GROW_CENTER_IF_LEVEL_LTE_OPP | WXK02-029 | 自Lv≤相手Lvなら lrig_deck先頭をlrig追加（コストなし近似） |
| TRASH_ATTACHED_OR_UNDER_CARD | WXK10-018 | acce/charm/under-cardの最初の候補を自動トラッシュ（近似） |
...
## 2026-06-12 アクション不一致修正ラウンド3（144→0件）で新設したSTUB

**実装済み✅**（execStubPart3）:

| STUB ID | カード | 効果概要 |
|---------|--------|---------|
| INTERNAL_DRAW_PER_CENTER_LEVEL | WXK10-004, WX26-CP1-003 | センタールリグのレベル1につき1ドロー |
| INTERNAL_CHARGE_PER_CENTER_LEVEL | WXK10-004, WX26-CP1-003 | センタールリグのレベル1につきエナチャージ1 |

**実装済み ✅ (v0.267)**:

| STUB ID | カード | 効果概要 |
|---------|--------|---------|
| PREVENT_NEXT_DAMAGE_THIS_TURN | WXK07-002 | PREVENT_NEXT_DAMAGEと同じprevnet_next_damage+1で実装（execStubPart1） |
| OPP_ENERGY_REDUCE_TO_N | WXK06-055 | 相手エナがstub.value超の場合、差分をトラッシュへ（execStubPart3） |
| BANISH_FACING_IF_SELF_POWER_GE_15000 | WD17-009 | 自パワー>=15000なら正面相手シグニをバニッシュ（sourceCardNum+effectivePowers参照、execStubPart3） |
| OPP_PUNISHER_CHOICE | WXK05-001 | 相手が3択（手札2捨て/エナ3トラッシュ/シグニ1トラッシュ）を選択（CHOOSE opponentResponds:true、execStubPart3） |
| DRIVE_SIGNI_POWER_DOUBLE_CRASH | WXK01-001 | ルリグCONT→ドライブシグニに+3000（effectEngine applyEffects）＋ダブルクラッシュ付与（BattleScreen handleSigniAttack） |

**ログのみ📝**（execStub既定フォールバック）:

| STUB ID | カード | 効果概要 |
|---------|--------|---------|
| ✅ EXILE_SELF_AFTER_USE | PR-378 | 使用後このカードをゲームから除外（近似: トラッシュへ、v0.279） |
| ✅ GROW_CENTER_IF_LEVEL_LTE_OPP | WXK02-029 | 自Lv≤相手Lvなら無コストグロウ（lrig_deck先頭→lrig追加、v0.279） |
| ✅ TRASH_ATTACHED_OR_UNDER_CARD | WXK10-018 | 最初の付属カード（アクセ/チャーム/下カード）を自動トラッシュ（近似、v0.279） |
| ✅ ATTACH_SEARCHED_AS_ACCE | WXK10-074 | lastProcessedCards[0]をsourceCardNumに設定→SELECT_TARGET→ATTACH_ACCE（v0.279） |
| ✅ GAIN_LRIG_BARRIER | WX24-P1-001, WX25-P3-001 | lrig_barrier+1（v0.278でPlayerState追加済み、v0.279でSTUB実装）|

**新設timing**: `ON_LIFE_CRASHED`（WXDi-P02-037「ライフクロスがクラッシュされたとき」）
→ ✅ **2026-06-12 配線済み**: 全クラッシュ経路がチェックゾーン（`field.check`）経由で `handleLifeBurstResponse` に
集約されることを利用し、そこで `collectSelfEventTriggers('ON_LIFE_CRASHED')` がトリガーを収集してスタックに積む。
`usageLimit:'once_per_turn'` は `actions_done`（effectId）で制御。`ON_GUARD`（WXDi-P02-035「あなたが【ガード】したとき」）も
同時に配線済み（`handleGuardResponse` のガード成立分岐＋代替ガード2種）。
残る近似: `LIFE_CRASH{triggerBurst:false}`（チェックゾーンを経由せず直接トラッシュ）と
エンジン内STUB（CRASH_LIFE_TO_HAND等）によるクラッシュ、CPU自身のライフクラッシュでは発火しない。

**choiceTextParser拡張**: 「あなたのすべてのシグニのパワーを＋N」→POWER_MODIFY(self ALL)を追加
...
| 日付 | 実装内容 | 対象STUB |
|------|---------|---------|
| 2026-06-12 v0.257 | 未配線timing 5種を配線（トリガー配線ラウンド3）: ON_REVEALED_FROM_HAND（9カード、公開スタブ3種→新設マーカーINTERNAL_MARK_REVEALED_FROM_HAND/NAMEDでhand_revealed_justフラグ→BattleScreen useEffect発火）/ON_HAND_DISCARDED（花岡ユズ、execTrash+execDiscardBothのhand_discarded_justフラグ+コスト捨て2箇所、twice_per_turn新設、triggerFilter:ブルアカ）/ON_DISCARDED_AS_COST（ユーグレナ、【出】【起】コスト捨て時）/ON_SPELL_USE（APEX2、handleCutinPass+once_per_turn+電機condition）/ON_EXCEED_COST（フラクタル・ケージ、executeLrigGranted）。ON_PLACED_UNDER_SIGNI（無心の豪圧）は「スペルをシグニの下に置く」機構自体が未実装（TRAP_OPERATION誤マッピング）のため配線不能と判定し文書化 | INTERNAL_MARK_REVEALED_FROM_HAND/NAMED新設 |
| 2026-06-12 v0.256 | 未配線timing 3種を配線（トリガー配線ラウンド2）: ON_ATTACK_PHASE_START（WXEX2-03/WXDi-D09-P16/WXDi-P00-040/WXK03-021、doPhaseAdvanceのMAIN→ATTACK_ARTS移行時にcollectTurnTriggers汎用化で収集。「各アタックフェイズ開始時」のWXEX2-03は自ターンのみ発火の近似）/ON_SIGNI_BANISH_BATTLE（WXDi-P02-046 ファラリス=scope self・WX24-P4-058 ジガネマル=any_ally、handleSigniAttackのバトルバニッシュ成立時。バニッシュ代替系・追加ゾーンバトルでは発火しない近似）/ON_ACCE_ATTACH（WXK04-003 オーバークロック=ルリグ監視・SPK01-11 ラズベリー=アクセカード自身、checkAndFireOnAcceTriggersForOwner拡張）。JSON修正: WX24-P4-058-EX1にtriggerScope:any_ally、WXK04-003-E1にusageLimit:once_per_turn追加 | timing配線（STUBなし） |
| 2026-06-12 v0.255 | 未配線timing 2種を配線: ON_LIFE_CRASHED（WXDi-P02-037 ダッキ「ライフクロスがクラッシュされたとき1ドロー」）/ON_GUARD（WXDi-P02-035 ヤエキリ「あなたが【ガード】したとき1ドロー」）。BattleScreenに collectSelfEventTriggers 新設（自フィールドシグニのAUTO収集、once_per_turn=actions_done制御、BLOCK_OWN_SIGNI_AUTO/FROZEN_LOSES_ABILITIES対応）。配線点: handleLifeBurstResponse（全クラッシュ経路がfield.check経由で集約）/handleGuardResponse＋代替ガード2種。queueCardEffectsにextraEntries引数追加。EffectTimingユニオンに2種追加 | timing配線（STUBなし） |
| 2026-06-12 v0.253 | アクション不一致ラウンド2（zerom側、Sheet2 62→0件・計217→144件）: ①②③④選択肢解析をchoiceTextParser.tsに共通化（CMCBC/CHOOSE_N_FROM_LIST/BET_MECHANIC/INTERNAL_BET_SHOW_4/INTERNAL_ECRV_APPLYの5箇所が同一解析を共有、バニッシュ各種/エナ置き/ライフ追加/クラッシュ/ルリグダウン・凍結/相手エナトラッシュ/クラスサーチ等を全箇所で解析可能に）。新規✅: REVEAL_TOP_BANISH_BY_LEVEL_SUM(WX17-028本実装)/SUMMON_FROM_TRASH(従来未実装だった選択肢ハンドラ)/SUMMON_RESONA_FROM_LRIG_DECK(WX20-069)/REVEAL_TOP_LEVEL_ROUTE(WX12-CB02レベル別5分岐)/INTERNAL_BANISH_ALL_POWER_GTE/INTERNAL_FREEZE_OPP_LRIG。REVEAL_PICK_HAND_SHUFFLE_BOTTOMにthen:'energy'対応（選んだカードをエナへ）。BETの選択数をテキストから解析（従来2/4固定）。新規📝: OPP_LRIG_LOSE_ABILITY | choiceTextParser共通化+新規6種 |
| 2026-06-12 v0.252 | アクション不一致ラウンド1（ymsty側、318→217件）: 新設✅3種=DRAW_UNTIL_HAND_SIZE（手札N枚まで引く、SPK16-13E）/HAND_EXCESS_TO_ENERGY（手札N枚超過分をエナへ、WDK08-Y08）/DRAW_IF_CHARGED_CLASS（直前チャージがクラス一致なら1ドロー、WDK07-E01を本実装）。CMCBC改善（ドローN枚対応・全シグニ凍結対応）。UNDER_CARD_HOST_BUFFはJSON側で実CONTINUOUSに置換し不要化。新設timing ON_GUARD（未配線、WXDi-P02-035）。TRIGGER_OWN_LIFE_CRASHED_DRAW（WXDi-P02-037）はクラッシュトリガー配線がなく未実装のまま残存 | 4種+ |
...
| 2026-06-02 v0.160 | 34件新規追加: ✅化(ALL_PLAYER_MILL/CRASH_LIFE_TO_HAND/DECK_MILL_UNTIL_CLASS/DECLARE_CLASS/GROW_COST_ZERO/LIMIT_OPP_ATTACK_ONCE/LRIG_LIMIT_MODIFY/NEGATE_ABILITY/POWER_EQUAL_TO_SELF_POWER/POWER_MINUS_PER_OWN_LEVEL/POWER_MOD_BY_LRIG_LEVEL/POWER_MOD_BY_TRASH_CLASS_COUNT/PREVENT_SIGNI_ABILITY_LOSS_BY_OPP/PREVENT_SIGNI_DOWN_BY_OPP/SEED_BLOOM_OPTIONAL/SEED_FLOWER_OP/SEED_HAND_AND_BLOOM_FROM_DECK_TOP/SHUFFLE_DECK_POWER_HALF/SKIP_MAIN_PHASE/SUPPRESS_GAIN_ABILITY/SUPPRESS_LIFEBURST_COLOR_CONDITION/SUPPRESS_OPP_SIGNI_ABILITIES/TRAP_TO_SIGNI_IF_ZONE_EMPTY/ENERGY_COLOR_SUBSTITUTE_赤_OR_青_TO_白)。ENERGY_*_TRASH_*エナ代替4件✅。カウント70件更新。削除4件 | 38件 |
| 2026-06-02 v0.160 | 34件新規追加: ✅化(ALL_PLAYER_MILL/CRASH_LIFE_TO_HAND/DECK_MILL_UNTIL_CLASS/DECLARE_CLASS/GROW_COST_ZERO/LIMIT_OPP_ATTACK_ONCE/LRIG_LIMIT_MODIFY/NEGATE_ABILITY/POWER_EQUAL_TO_SELF_POWER/POWER_MINUS_PER_OWN_LEVEL/POWER_MOD_BY_LRIG_LEVEL/POWER_MOD_BY_TRASH_CLASS_COUNT/PREVENT_SIGNI_ABILITY_LOSS_BY_OPP/PREVENT_SIGNI_DOWN_BY_OPP/SEED_BLOOM_OPTIONAL/SEED_FLOWER_OP/SEED_HAND_AND_BLOOM_FROM_DECK_TOP/SHUFFLE_DECK_POWER_HALF/SKIP_MAIN_PHASE/SUPPRESS_GAIN_ABILITY/SUPPRESS_LIFEBURST_COLOR_CONDITION/SUPPRESS_OPP_SIGNI_ABILITIES/TRAP_TO_SIGNI_IF_ZONE_EMPTY/ENERGY_COLOR_SUBSTITUTE_赤_OR_青_TO_白)。ENERGY_*_TRASH_*エナ代替4件✅。カウント70件更新。削除4件 | 38件 |
| 2026-06-07 v0.222 | 3件新規✅化: ①RETURN_ANGEL_SIGNI_TO_DECK(WX06-001-E2: トラッシュの天使シグニ7枚→デッキ下+conditional BANISHサポート。BanishAction.conditional追加+execBanish条件チェック実装) ②RETURN_UNIQUE_ANGEL_SIGNI_TO_DECK(WX06-001-E3: 名前の異なる天使シグニ7種→デッキ下) ③FROZEN_LOSES_ABILITIES(WX09-Re01 CONT: effectEngineのapplyEffectsで凍結シグニのCONT効果をスキップ+BattleScreen collectFieldTriggersで凍結シグニのAUTOをスキップ) | RETURN_ANGEL_SIGNI_TO_DECK/RETURN_UNIQUE_ANGEL_SIGNI_TO_DECK/FROZEN_LOSES_ABILITIES |
| 2026-06-07 v0.221 | 1件✅化: GRANT_TURN_TRIGGER_3RD_DOWN(BattleScreen植物3回目ダウン時の発動効果をBANISHのみ→SEQUENCE[BANISH+TRANSFER_TO_HAND(エナ1枚)+DRAW 1]に修正。CpuBattleScreen processUpにturn_trigger_3rd_plant_down/turn_plant_down_countリセット追加) | GRANT_TURN_TRIGGER_3RD_DOWN |
| 2026-06-07 v0.220 | 1件実装: ✅TRASH_AT_TURN_END(PlayerState.turn_end_field_trash_targets追加→execStub.tsで対象を記録→BattleScreenターン終了時にフィールドから自動トラッシュ→ターン開始時にリセット) | TRASH_AT_TURN_END |
| 2026-06-07 v0.219 | 2件実装: ✅NEGATE_SPELL(PlayerState.spell_negated_this_turn追加→handlePendingSpell内でcasterState.spell_negated_this_turnがtrueかつコスト合計5以下のスペルを自動打ち消し)+⚡GRANT_TURN_TRIGGER_3RD_DOWN(PlayerState.turn_trigger_3rd_plant_down/turn_plant_down_count追加→executeSigniActivated内でdown_selfコスト支払い時に植物シグニカウント追跡→3回目にBANISH相手シグニ1体のStackEntry追加) | NEGATE_SPELL/GRANT_TURN_TRIGGER_3RD_DOWN |
| 2026-06-07 v0.218 | 5件実装: ✅FORCE_COLOR_BLACK(collectFieldSigniExtraColorsにルリグからの黒追加処理)+✅REORDER_LIFE_CLOTHS(CHOOSE 0〜n枚→INTERNAL_REORDER_LIFE_APPLY→デッキ上補充)+✅MOVE_LRIG_TRASH_UNDER(ルリグ→lrig_deck末尾、白/黒アーツ→lrig_deck先頭)+✅INHERIT_LRIG_TRASH_ABILITIES(BattleScreen getMyLrigFieldActions内でルリグトラッシュの起動能力をCHOOSEに追加) | FORCE_COLOR_BLACK/REORDER_LIFE_CLOTHS/MOVE_LRIG_TRASH_UNDER/INHERIT_LRIG_TRASH_ABILITIES |
| 2026-06-07 v0.216 | 3件実装: ✅MILLAction型追加(effects.ts+effectExecutor.tsにexecMill実装・useDeclaredCount対応でDECLARE_NUMBER+MILL構造を機能化)+✅OPTIONAL_RETURN_TO_LRIG_DECK(CHOOSE pay/skip→INTERNAL_RETURN_LRIG_TO_DECK→ルリグデッキ復帰)+DECLARE_AND_MILL STUBはeffects.jsonでDECLARE_NUMBER+MILL移行済みのためログのみ維持 | MILLAction/OPTIONAL_RETURN_TO_LRIG_DECK |
...
| 2026-06-02 v0.160 | 34件新規追加: ✅化(ALL_PLAYER_MILL/CRASH_LIFE_TO_HAND/DECK_MILL_UNTIL_CLASS/DECLARE_CLASS/GROW_COST_ZERO/LIMIT_OPP_ATTACK_ONCE/LRIG_LIMIT_MODIFY/NEGATE_ABILITY/POWER_EQUAL_TO_SELF_POWER/POWER_MINUS_PER_OWN_LEVEL/POWER_MOD_BY_LRIG_LEVEL/POWER_MOD_BY_TRASH_CLASS_COUNT/PREVENT_SIGNI_ABILITY_LOSS_BY_OPP/PREVENT_SIGNI_DOWN_BY_OPP/SEED_BLOOM_OPTIONAL/SEED_FLOWER_OP/SEED_HAND_AND_BLOOM_FROM_DECK_TOP/SHUFFLE_DECK_POWER_HALF/SKIP_MAIN_PHASE/SUPPRESS_GAIN_ABILITY/SUPPRESS_LIFEBURST_COLOR_CONDITION/SUPPRESS_OPP_SIGNI_ABILITIES/TRAP_TO_SIGNI_IF_ZONE_EMPTY/ENERGY_COLOR_SUBSTITUTE_赤_OR_青_TO_白)。ENERGY_*_TRASH_*エナ代替4件✅。カウント70件更新。削除4件 | 38件 |
| 2026-06-05 v0.205 | GAIN_ABILITY_THIS_GAME 5パターン追加実装: LB全無効(game_suppress_lb+BattleScreen LBチェック統合)/メインフェイズ開始時ドロー(game_main_draw+GROW→MAIN移行)/グロウ時ドロー(game_grow_draw+グロウ処理)/手札上限+N(game_hand_size_bonus+collectHandLimits)/エナフェイズ開始時ドロー(game_energy_phase_draw+DRAW→ENERGY移行)。対象: WXK08-028/WXDi-P11-004/WX24-P4-036/WX25-P2-005 | GAIN_ABILITY_THIS_GAME |
| 2026-06-05 v0.204 | GRANT_ABILITY_INNER_TEXT 4パターン追加実装: ダウン保護（対戦相手の効果によってダウンしない→keyword_grants __down_protect__+collectDownProtectedSigni拡張）/パワー弱体保護（〜パワーは－されない→__power_minus_protect__+collectPowerProtectedSigni拡張）/ダメージ保護（対戦相手の効果によってダメージを受けない→prevent_lrig_damage）/能力取得禁止（新たに能力を得られない→__ability_gain_block__+collectAbilityGainProtectedSigni拡張）。CONTINUOUS/AUTO両方を共通フラグで処理 | GRANT_ABILITY_INNER_TEXT |
| 2026-06-05 v0.204 | fix: effectExecutor.ts/execStub.ts の ?? || 混在エラー5箇所修正（rolldownビルドエラー対応） | 修正のみ |
