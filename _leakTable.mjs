...
if (wantIds.length === 0 || wantIds[0] === '--clean') {
  // 集計のみ。--clean: STUBが1個だけの行（原文抽出しやすい）に限定
  const cleanOnly = wantIds[0] === '--clean';
  const cnt = new Map();
  for (const r of rows) {
    const nStub = (r.rendered.match(/\[STUB:/g)||[]).length;
    if (cleanOnly && nStub !== 1) continue;
    for (const id of r.ids) cnt.set(id, (cnt.get(id)||0)+1);
  }
