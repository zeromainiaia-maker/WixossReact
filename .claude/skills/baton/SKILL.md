...
1. **いま §1「現在地（直近1セッション）」に載っている要約を丸ごと切り取り**、`docs/PLAN_PROGRESS.md` の
   「過去セッション要約」の**先頭**（新しいものが上）へ貼る。
2. §1 を**今回の作業の要約に書き換える**。§1 に2件並べない。
   ⚠**旧記述は「§4」だった**（2026-08-23 の PLAN 再編で進捗サマリは §1 へ移り、§4 は教訓集になった）＝
   2026-09-06 に §4 が [LESSONS.md](../../../docs/LESSONS.md) へ分離されて完全に別物になったので修正した。

🔴**書き方＝物語ではなく「現在地の表」。日記文を書かない。**
PLAN §1 は **15行以内**に収める。入れるのは次の4つだけ：

| 軸 | 中身 |
|---|---|
| 🔥**次に取るもの** | §5 のどこから取るかを順番に（①→②→③） |
| 📊**進捗3計器** | Sheet1 要対応／台帳 残 OPEN／census 高シグナル。**動かなかったら理由を1行** |
| 📦**在庫** | 機構 worklist 残項目数／実機 残 `V-nn` 数／実装キュー残数 |
| 🔧**ゲート** | 全緑かどうかと golden 件数（詳細は §6 に書く＝ここには書かない） |

🔴**禁止**＝「第NNNバッチで〜した」「〜を移した」「経緯は〜」といった**日記文を PLAN に書かない**。
**何をしたかの全文は `BUGFIXES.md` の先頭**、**過去の要約は `PLAN_PROGRESS.md`**、**再利用できる知識は `LESSONS.md`**。
⚠**§5（作業キュー）にも教訓・クローズ履歴を書かない**＝残数・生きている行・取り方のルールだけを置く。
