...
> 🆕🔴**2026-09-01＝このスキルは「在庫切れで休止中」。** `npm run census` の高シグナルは **11 / BASELINE 12**（旧ベースライン 1872 → 460 → 245 → 91 → 11）まで落ち、
> **残り11は「新しい機構の縦切り」だけ**と続き749 が全数確定している＝**文型バッチで束ねられる在庫はもう無い**。
> ⚠**`census:clusters` の実測＝1テンプレあたり 1.03効果**（340効果 / 330テンプレ）＝**parser に regex を1本足しても平均1効果しか直らない。**
> ⇒ **いま取るべきは PLAN §5.3（機構 worklist）の索引 A→B→C。** このスキルは**新カード追加などで census が再び二桁後半へ戻ったときに再開**する。
> 🔴**「census に出ている＝直すべき」ではない**＝先行セッションが理由つきで見送った効果が混ざっており、**golden の「見送り契約」が唯一の歯止め**（2026-08-31 に5件が赤で差し戻された）。
