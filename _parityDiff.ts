// 一時スクリプト：指定カードの curated JSON vs fresh parse の leaf 差分を表示（終わったら削除）
...
for (const f of FILES) { const j = JSON.parse(execSync(`git show HEAD:public/data/${f}`,{maxBuffer:1e9}).toString()); for (const [k,v] of Object.entries(j)) existing.set(k, v as unknown[]); }
...
for (const id of ids) {
  ...
  console.log(`\n== ${id} ==`);
  console.log(`原文: ${(row.EffectText??'').slice(0,160)}`);
  if (!cur) { console.log('curated JSONなし'); continue; }
  const em = leafMap(cur), fm = leafMap(fresh);
  for (const [p,v] of em) {
    if (!fm.has(p)) console.log(`  LOST  ${p} = ${JSON.stringify(v)}`);
    else if (JSON.stringify(fm.get(p))!==JSON.stringify(v)) console.log(`  DIFF  ${p}: curated=${JSON.stringify(v)} fresh=${JSON.stringify(fm.get(p))}`);
  }
  for (const [p,v] of fm) if (!em.has(p)) console.log(`  EXTRA ${p} = ${JSON.stringify(v)}`);
}
