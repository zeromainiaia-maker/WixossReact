...
- 🆕🔴**2026-09-01 に PLAN を再編（第2回）＝計器主導から機構主導へ。`docs/PLAN.md` の §5 冒頭の表を必ず読む。**
  ...
  - 🆕**§5.3 は「母集団順の索引 A〜F」になった**＝**索引の並びがそのまま取る順**（旧「取る順」表は廃止）。**A（母集団2桁 8件）→ B（3〜8効果 8件）→ C（1〜2効果 28件）**の順に取る。**D は母集団未計測 33件＝取る前に実測する**（`node scripts/archive/censusMechPopulation.mjs --id O-nn`）。**E は計器の較正＝挙動は変わらない**。
