...
- 🆕**`npm run census:costtext`** — **UI コスト層の全文 regex センサス**（2026-09-02新設・§5.3 `O-86`）＝**「カードを使うときの実効コストを UI 層が原文 regex で決めている箇所」**を全数で測る（`scripts/censusCostText.ts`・明細 `docs/_census_costtext.txt`／`npx tsx scripts/censusCostText.ts --id <部分文字列>` で1規則の当たりカード全部）。🔴**`census:enginetext` では1行も映らない**＝あちらは `src/engine/` しか走査しないが、コストの置換／軽減／追加は **`src/screens/battle/costs.ts` ほかの UI 層**が `card.EffectText` を毎回読み直して決めている（＝A群が0になってもコストの意味は原文 regex のまま）。**3分類**＝**A🔴 COST**（実効コストを決める＝本命の worklist）／**B GATE**（使用可否だけ）／**C OTHER**。🔑**「当たっているカード数」であって「壊れている件数」ではない**＝読み方は `census:enginetext` の miss と同じ。🔑**真の worklist は「規則に当たる ∧ コスト payload（`costScaling`/`conditionalEnergyReduction`）が無い」カード**（payload 済みは `computeArtsEffectiveCost` の構造上**既に regex から降りている**）。🔴**ゲート（ratchet）＝A群の規則本数が増えたら exit 1・減っても exit 1**（払い戻したら `BASELINE_COST_RULES` を実測値へ下げる）。`npm run gates` に同梱。⚠**計器を書き足すときの罠3つ**＝①ネストした arrow const を関数境界と誤認すると追跡中の原文変数が消える ②`new RegExp(\`…${'${'}定数}…\`)` のテンプレ regex と複数行呼び出しは行単位では拾えない ③**原文を引数で受け取る関数**（`parseUseTimeCostReduction(effectText)` 等）を数えないと `parse*` 群が丸ごと消える。
